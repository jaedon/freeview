...
typedef struct
{
	block_state KeySched1, KeySched2, KeySched3;
} block_state3;
...
static void block_decrypt3(block_state3 *self,
			  unsigned char *in,
			  unsigned char *out)
{
...
static void block_encrypt3(block_state3 *self,
			  unsigned char *in,
			  unsigned char *out)
{
	des_cblock output, output2;
...
void encrypt (char *key, char *plaintext, char *ciphertext)
{
	...
	block_init(&state, key, 8);

	block_encrypt(&state, plaintext, ciphertext);
}
...
void decrypt (char *key, char *ciphertext, char *plaintext)
{
	...
	block_decrypt(&state, ciphertext, plaintext);
}

static void block_init3(block_state3 *self, unsigned char *key,
		       int keylength)
{
	char oddkey[24];
	int i;
...
void encrypt3 (char *key, char *plaintext, char *ciphertext)
{
	...
	block_init3(&state, key, 24);

	block_encrypt3(&state, plaintext, ciphertext);
}
...
void decrypt3 (char *key, char *ciphertext, char *plaintext)
{
	...
	block_decrypt3(&state, ciphertext, plaintext);
}
...
/**
	new api
	seperate init & process

*/
static block_state		des_state;
static block_state3		des3_state;

void des_init(char *key)
{
	block_init(&des_state, key, 8);
}

void des3_init(char *key)
{
	block_init3(&des3_state, key, 24);
}

void encrypt_process(char *plaintext, char *ciphertext)
{
	block_encrypt(&des_state, plaintext, ciphertext);
}

void decrypt_process(char *ciphertext, char *plaintext)
{
	block_decrypt(&des_state, ciphertext, plaintext);
}

void encrypt3_process(char *plaintext, char *ciphertext)
{
	block_encrypt3(&des3_state, plaintext, ciphertext);
}

void decrypt3_process(char *ciphertext, char *plaintext)
{
	block_decrypt3(&des3_state, ciphertext, plaintext);
}
