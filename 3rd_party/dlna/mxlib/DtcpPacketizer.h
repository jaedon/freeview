...
#ifdef _POSIX
typedef	long long size_new;
#else
//typedef	size_t size_new;
typedef	__int64 size_new;
#endif
...
struct dtcp_stream_t
{
	...
	size_new dtcp_stream_content_length;      ///< Source - Byte lenght of content stream, Sink - Cumulative content stream length
	size_new dtcp_stream_content_remaining;   ///< Source - Number of bytes remaining in content stream
//	MX_INT64 dtcp_stream_content_length;      ///< Source - Byte lenght of content stream, Sink - Cumulative content stream length
//	MX_INT64 dtcp_stream_content_remaining;   ///< Source - Number of bytes remaining in content stream
	...
	size_new dtcp_packet_content_length;      ///< Source / Sink - Byte length of PCP
	size_new dtcp_packet_content_remaining;   ///< Source / Sink - Number of content bytes processed
    size_new dtcp_packet_bytes_remaining;     ///< Source / Sink - Number of content byts to process
    size_new dtcp_packet_padding_bytes;       ///< Sink - Number of padding bytes for this PCP

	size_new max_packet_size; 				///< Source - Max PCP size for content stream

//	MX_INT64 dtcp_packet_content_length;      ///< Source / Sink - Byte length of PCP
//	MX_INT64 dtcp_packet_content_remaining;   ///< Source / Sink - Number of content bytes processed
//    MX_INT64 dtcp_packet_bytes_remaining;     ///< Source / Sink - Number of content byts to process
//    MX_INT64 dtcp_packet_padding_bytes;       ///< Sink - Number of padding bytes for this PCP

//	MX_INT64 max_packet_size;                 ///< Source - Max PCP size for content stream
...
int DtcpPacketizer_CreateContentSource(struct dtcp_stream_t *dtcp_stream, int content_emi,
                             size_new content_length, size_new max_packet_size);

int DtcpPacketizer_SetClientIP( struct dtcp_stream_t *dtcp_stream , long ip );
...
int DtcpPacketizer_PacketizeData(struct dtcp_stream_t *dtcp_stream,
				  /* IN */ char *clear_content, /* IN */ size_new clear_content_size,
				  /* IN/OUT */ char **encrypted_data, /* IN/OUT */ size_new *encrypted_data_size,
                  /* OUT */ size_new *total_content_processed);
...
int DtcpPacketizer_DepacketizeData(struct dtcp_stream_t *dtcp_stream,
				  /* IN */ char *encrypted_data, /* IN */ size_new encrypted_data_size,
				  /* IN/OUT */ char *clear_content, /* IN/OUT */ size_new *clear_content_size,
                  /* OUT */ size_new *total_data_processed);
