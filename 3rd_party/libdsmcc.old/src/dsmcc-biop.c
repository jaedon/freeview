...
int dsmcc_biop_process_srg(struct biop_message *bm,struct cache_module_data *cachep, struct cache *filecache)
{
	...
	nSvcCntxCnt = (int)(Data[off]);
	off += (nSvcCntxCnt + 1);

	bm->body.srg.msgbody_len = (Data[off] << 24) | (Data[off+1] << 16) | (Data[off+2] << 8) | Data[off+3];
	off+=4;
	...
	for (i = 0; i < bm->body.srg.bindings_count; i++)
	{
		...
		j = 0;
		{
			if (!strcmp("dir", bm->body.srg.binding.name.comps[j].kind))
			{
				dsmcc_cache_dir_info (filecache, 0,0,NULL,&bm->body.srg.binding, j);
			}
			else if (!strcmp("fil",bm->body.srg.binding.name.comps[j].kind))
			{
				dsmcc_cache_file_info (filecache, 0,0,NULL,&bm->body.srg.binding, j);
			}
			else if (!strcmp("ste",bm->body.srg.binding.name.comps[j].kind))
			{
				dsmcc_cache_event_info (filecache, 0,0,NULL,&bm->body.srg.binding, j);
			}
	  }
...
void dsmcc_biop_process_dir (struct biop_message *bm, struct cache_module_data *cachep, struct cache *filecache)
{
	...
	bm->body.dir.msgbody_len = (Data[off] << 24) | (Data[off+1] << 16) | (Data[off+2] << 8) | Data[off+3];
	off+=4;
	...
	for(i = 0; i < bm->body.dir.bindings_count; i++)
	{
		...
		{
			if (!strcmp("dir",bm->body.dir.binding.name.comps[j].kind))
			{
				dsmcc_cache_dir_info(filecache, cachep->module_id, bm->hdr.objkey_len, bm->hdr.objkey, &bm->body.dir.binding, j);
			}
			else if (!strcmp("fil", bm->body.dir.binding.name.comps[j].kind))
			{
				dsmcc_cache_file_info(filecache, cachep->module_id, bm->hdr.objkey_len, bm->hdr.objkey, &bm->body.dir.binding, j);
			}
			else if (!strcmp("ste", bm->body.dir.binding.name.comps[j].kind))
			{
				dsmcc_cache_event_info(filecache, cachep->module_id, bm->hdr.objkey_len, bm->hdr.objkey, &bm->body.dir.binding, j);
			}
		}
...
void dsmcc_biop_process_file(struct biop_message *bm,struct cache_module_data *cachep, struct cache *filecache)
{
	...
	bm->body.file.msgbody_len = (Data[off] << 24) | (Data[off+1] << 16) | (Data[off+2] << 8) | Data[off+3];
	off+=4;

	bm->body.file.content_len = (Data[off] << 24) | (Data[off+1] << 16) | (Data[off+2] << 8)  | Data[off+3];
	off+=4;
...
void dsmcc_biop_process_event(struct biop_message *bm,struct cache_module_data *cachep, struct cache *filecache)
{
	int off = 0;
	int nSvcCntxCnt;
	unsigned char *Data = cachep->data+cachep->curp;
	...
	bm->body.eve.msgbody_len = (Data[off] << 24) | (Data[off+1] << 16) | (Data[off+2] << 8) | Data[off+3];
	off+=4;

	bm->body.eve.taps_count = (unsigned int)Data[off];
	off++;

	dsmcc_cache_event(filecache, bm, cachep);

	cachep->curp += (bm->body.file.content_len + off);
}
...
void dsmcc_biop_process_data(struct cache *filecache, struct cache_module_data *cachep)
{
	...
	len = (desc != NULL) ? desc->data.compressed.original_size : cachep->size;
	cachep->curp = 0;
	...
	while(cachep->curp < len)
	{
		...
		{
			dsmcc_biop_process_event(&bm, cachep, filecache);
		}
		else
		{
			/* Error */
...
int dsmcc_biop_process_module_info(struct biop_module_info *modinfo, unsigned char *Data)
{
	...
	modinfo->mod_timeout	= (Data[0] << 24) | (Data[1] << 16) | (Data[2] << 8) | Data[3];
	modinfo->block_timeout	= (Data[4] << 24) | (Data[5] << 16) | (Data[6] << 8) | Data[7];
	modinfo->min_blocktime	= (Data[8] << 24) | (Data[9] << 16) | (Data[10] << 8) | Data[11];
