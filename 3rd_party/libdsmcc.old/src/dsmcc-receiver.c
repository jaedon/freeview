...
dsmcc_free(struct dsmcc_status *status) {
	...
	for (i=0; i<MAXCAROUSELS; ++i )
	{
		struct object_carousel *car = &(status->carousels[i]);
...
dsmcc_add_module_info(struct dsmcc_status *status, struct dsmcc_section *section, struct obj_carousel *car) {
	...
	{
		found = 0;
		for (cachep = car->cache; cachep!=NULL; cachep=cachep->next)
		{
			if(cachep->carousel_id == dii->download_id && cachep->module_id == dii->modules[i].module_id)
			{
			/* already known */
...
/* by Dylan - set URL prefix */
#if 1
			car->filecache->pid = status->buffers->pid;
#endif
		...
		unsigned int	unCacheStatus = 0;
			...
			if (unCacheStatus == 0)
			{
				if (status->debug_fd)
				{
					fprintf (status->debug_fd, "[%s] complete all caching  ###############################################\n", __FUNCTION__);
				}
				...
				if (g_libdsmcc_complete_callback != NULL)
				{
					if (status->debug_fd)
					{
						fprintf (status->debug_fd, "[%s] send complete callback \n", __FUNCTION__);
					}

					g_libdsmcc_complete_callback ((status->buffers != NULL) ? status->buffers->pid : 0, car); /* if you need the more infomation regarding to the biop, refer the filecache struct */
				}
