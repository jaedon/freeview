...
var ignoreKey, convertedKey;

var ooif_helper =
{
    getAppMgr: function(e) {
        if (typeof ooif_helper.appMgr == 'undefined')
            ooif_helper.appMgr = oipfObjectFactory.createApplicationManagerObject();

        return ooif_helper.appMgr;
    },
	...
	handleKey: function(e) {
		try {
            if(e.event.keyCode === ignoreKey) // trash useless Afterevent
                return;
            convertedkey = null;

			ooif_helper.getAppMgr().__key(e.event.type, e.event.keyCode);
		} catch (e)	{
			// we are unable to create DAE ApplicationManager so we can assume that
			// DAE applications are not suported by Opera (due to missing proper NJSE)
			// or
			// __key() is called inside non-DAE-application environment
			window.opera.removeEventListener('AfterEvent.keypress', ooif_helper.handleKey, false);
			window.opera.removeEventListener('AfterEvent.keydown', ooif_helper.handleKey, false);
			window.opera.removeEventListener('AfterEvent.keyup', ooif_helper.handleKey, false);
		}
	},

    triggerApplicationLoadedEvent: function(e) {
		try {
            //check that it is the page loading, not just an image
            if(e.event.target instanceof Document)
			ooif_helper.getAppMgr().__triggerApplicationLoadedEvent();
		} catch (e)	{
			// we are unable to create DAE ApplicationManager so we can assume that
			// DAE applications are not suported by Opera (due to missing proper NJSE)
			// or
			// __triggerApplicationLoadedEvent() is called inside non-DAE-application environment
		}
	},
    ...
    triggerKey: function(e) {
        try {
            ...
            if(e.event.keyCode === convertedKey) // already converted key.
                return;
  					ignoreKey = null;                
            ...
            //to match keyCode with P- according to the request of J-COM.
            switch( e.event.keyCode ){
                case 461: //VK_back
                    ooif_helper.makeKeyEvent(e.event, 27);
                    break;
		case 463: //VK_play_pause
                    ooif_helper.makeKeyEvent(e.event, 179);
                    break;
               
		case 413: //VK_stop
                    ooif_helper.makeKeyEvent(e.event, 178);
                    break;
		    case 417 : //VK_fastforward
                    ooif_helper.makeKeyEvent(e.event, 228);
                    break;
						    case 412: //VK_rewind
                    ooif_helper.makeKeyEvent(e.event, 227);
                    break;
            ...
            switch( e.event.keyCode ){
                case 106: //VK_11
                    ooif_helper.makeKeyEvent(e.event, 501);
                    break;

                case 107: //VK_12
                    ooif_helper.makeKeyEvent(e.event, 502);
                    break;

                case 113: //VK_DTT
                    ooif_helper.makeKeyEvent(e.event, 503);
                    break;

                case 114: //VK_BS
                    ooif_helper.makeKeyEvent(e.event, 504);
                    break;

                case 115: //VK_CATV
                    ooif_helper.makeKeyEvent(e.event, 505);
                    break;

                case 111: //VK_VOD
                    ooif_helper.makeKeyEvent(e.event, 507);
                    break;

                case 101: //VK_EPG
                    ooif_helper.makeKeyEvent(e.event, 509);
                    break;

                case 116: //VK_CH_RETURN
                    ooif_helper.makeKeyEvent(e.event, 514);
                    break;

                case 450: //VK_AV_AUDIO
                    ooif_helper.makeKeyEvent(e.event, 516);
                    break;

                case 416: //VK_RECORD
                    ooif_helper.makeKeyEvent(e.event, 528);
                    break;

                case 126: //VK_SKIP
                    ooif_helper.makeKeyEvent(e.event, 529);
                    break;

                case 125: //VK_REPLAY
                    ooif_helper.makeKeyEvent(e.event, 530);
                    break;

                case 425: //VK_NEXT_CHAPTER
                    ooif_helper.makeKeyEvent(e.event, 531);
                    break;

                case 424: //VK_PREVIOUS_CHAPTER
                    ooif_helper.makeKeyEvent(e.event, 532);
                    break;

                case 412: //VK_REWIND
                    ooif_helper.makeKeyEvent(e.event, 533);
                    break;

                case 417: //VK_FASTFORWARD
                    ooif_helper.makeKeyEvent(e.event, 534);
                    break;

                case 463: //VK_PLAYBACK
                    ooif_helper.makeKeyEvent(e.event, 535);
                    break;

                case 413: //VK_STOP
                    ooif_helper.makeKeyEvent(e.event, 536);
                    break;

                case 102: //VK_LIST
                    ooif_helper.makeKeyEvent(e.event, 539);
                    break;

                case 119: //VK_NW_TOGGLE
                    ooif_helper.makeKeyEvent(e.event, 520);
                    break;

				case 433: //VK_STORE_FAVORITE_0
					ooif_helper.makeKeyEvent(e.event, 511);
					break;
   ...
   makeKeyEvent: function(event, code) {
...
oipfObjectFactory.createSubVideoBroadcastObject = function() {
    return ooif_helper.createVisualObject("video/subBroadcast");
};

oipfObjectFactory.createHumaxVideoMpegObject = function() {
    return ooif_helper.createVisualObject("video/vnd.humax.mpeg");
};

// OIPF 2.1 cross application event handling [4.4.7]
// begin {
window.opera.addEventListener('BeforeEvent.load', ooif_helper.triggerApplicationLoadedEvent, false);

window.opera.addEventListener('BeforeEvent.keypress', ooif_helper.triggerKey, false);
window.opera.addEventListener('BeforeEvent.keydown', ooif_helper.triggerKey, false);
window.opera.addEventListener('BeforeEvent.keyup', ooif_helper.triggerKey, false);

window.opera.addEventListener('AfterEvent.keypress', ooif_helper.handleKey, false);
window.opera.addEventListener('AfterEvent.keydown', ooif_helper.handleKey, false);
window.opera.addEventListener('AfterEvent.keyup', ooif_helper.handleKey, false);
// } end [4.4.7]
...
KeyEvent.VK_REWIND = 533;
Event.prototype.VK_REWIND = 533;
KeyEvent.VK_STOP = 536;
Event.prototype.VK_STOP = 536;
...
KeyEvent.VK_RECORD = 528;
Event.prototype.VK_RECORD = 528;
KeyEvent.VK_FAST_FWD = 534;
Event.prototype.VK_FAST_FWD = 534;
...
KeyEvent.VK_TRACK_PREV = 532;
Event.prototype.VK_TRACK_PREV = 532;
KeyEvent.VK_TRACK_NEXT = 531;
Event.prototype.VK_TRACK_NEXT = 531;
...
KeyEvent.VK_SURROUND_MODE_NEXT = 516;
Event.prototype.VK_SURROUND_MODE_NEXT = 516;
...
KeyEvent.VK_INFO = 509;
Event.prototype.VK_INFO = 509;
...
KeyEvent.VK_PLAY_PAUSE = 535;
Event.prototype.VK_PLAY_PAUSE = 535;

/* Humaxadded */
KeyEvent.VK_TVRADIO_TOGGLE = KeyboardEvent.DOM_VK_NUMPAD2;
Event.prototype.VK_TVRADIO_TOGGLE = KeyboardEvent.DOM_VK_NUMPAD2;
KeyEvent.VK_LIST = KeyboardEvent.DOM_VK_NUMPAD3;
Event.prototype.VK_LIST = KeyboardEvent.DOM_VK_NUMPAD3;
KeyEvent.VK_SETTING = KeyboardEvent.DOM_VK_NUMPAD4;
Event.prototype.VK_SETTING = KeyboardEvent.DOM_VK_NUMPAD4;
KeyEvent.VK_OPTION = 457;
Event.prototype.VK_OPTION = 457;
KeyEvent.VK_PLAYER = 539;
Event.prototype.VK_PLAYER = 539;
KeyEvent.VK_WIDE = KeyboardEvent.DOM_VK_NUMPAD7;
Event.prototype.VK_WIDE = KeyboardEvent.DOM_VK_NUMPAD7;
KeyEvent.VK_SEARCH = KeyboardEvent.DOM_VK_NUMPAD8;
Event.prototype.VK_SEARCH = KeyboardEvent.DOM_VK_NUMPAD8;
KeyEvent.VK_RECORDINGS = KeyboardEvent.DOM_VK_NUMPAD9;
Event.prototype.VK_RECORDINGS = KeyboardEvent.DOM_VK_NUMPAD9;
/* JAPAN */
KeyEvent.VK_11 = 501;
Event.prototype.VK_11 = 501;
KeyEvent.VK_12 = 502;
Event.prototype.VK_12 = 502;
KeyEvent.VK_PROGRAM = KeyboardEvent.DOM_VK_SEPARATOR;
Event.prototype.VK_PROGRAM = KeyboardEvent.DOM_VK_SEPARATOR;
KeyEvent.VK_DATA_BROADCAST = KeyboardEvent.DOM_VK_SUBTRACT;
Event.prototype.VK_DATA_BROADCAST = KeyboardEvent.DOM_VK_SUBTRACT;
KeyEvent.VK_DIGITS = KeyboardEvent.DOM_VK_DECIMAL;
Event.prototype.VK_DIGITS = KeyboardEvent.DOM_VK_DECIMAL;
KeyEvent.VK_VOD = 507;
Event.prototype.VK_VOD = 507;
KeyEvent.VK_BROWSER = KeyboardEvent.DOM_VK_F1;
Event.prototype.VK_BROWSER = KeyboardEvent.DOM_VK_F1;
KeyEvent.VK_DTT = 503;
Event.prototype.VK_DTT = 503;
KeyEvent.VK_BS = 504;
Event.prototype.VK_BS = 504;
KeyEvent.VK_CATV = 505;
Event.prototype.VK_CATV = 505;
KeyEvent.VK_LAST = 514;
Event.prototype.VK_LAST = 514;
KeyEvent.VK_RECOMMAND = KeyboardEvent.DOM_VK_F6;
Event.prototype.VK_RECOMMAND = KeyboardEvent.DOM_VK_F6;
KeyEvent.VK_ETC = KeyboardEvent.DOM_VK_F7;
Event.prototype.VK_ETC = KeyboardEvent.DOM_VK_F7;
KeyEvent.VK_BROADCAST_NETWORK = 520;
Event.prototype.VK_BROADCAST_NETWORK = 520;
KeyEvent.VK_WPS = KeyboardEvent.DOM_VK_F9;
Event.prototype.VK_WPS = KeyboardEvent.DOM_VK_F9;
KeyEvent.VK_HIDDEN_MENU = KeyboardEvent.DOM_VK_F10;
Event.prototype.VK_HIDDEN_MENU = KeyboardEvent.DOM_VK_F10;
/* Humaxadded */
KeyEvent.VK_MULTI_CUSTOM = KeyboardEvent.DOM_VK_F11;
Event.prototype.VK_MULTI_CUSTOM = KeyboardEvent.DOM_VK_F11;
KeyEvent.VK_MULTI_CUSTOM_SUCCESS = KeyboardEvent.DOM_VK_F12;
Event.prototype.VK_MULTI_CUSTOM_SUCCESS = KeyboardEvent.DOM_VK_F12;
KeyEvent.VK_MULTI_CUSTOM_FAIL = KeyboardEvent.DOM_VK_F13;
Event.prototype.VK_MULTI_CUSTOM_FAIL = KeyboardEvent.DOM_VK_F13;
/* JAPAN */
KeyEvent.VK_REPLAY = 530;
Event.prototype.VK_REPLAY = 530;
KeyEvent.VK_SKIP = 529;
Event.prototype.VK_SKIP = 529;
...
HTMLObjectElement.prototype.addStreamEventListener = function(targetURL, eventName, listener)
{
	...
	{
		if (eventName == streamEvents[i].getAttribute('stream_event_name') || eventName == streamEvents[i].getAttributeNS('urn:dvb:mis:dsmcc:2009', 'stream_event_name'))
		{
			streamEventId = streamEvents[i].getAttribute('stream_event_id') || streamEvents[i].getAttributeNS('urn:dvb:mis:dsmcc:2009', 'stream_event_id');
			break;
		}
	}
	...
	return this.internalAddXMLStreamEventListener(targetURL, eventName, parseInt(streamEventId), parseInt(componentTag), listener);
}
