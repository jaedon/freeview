...
*/
typedef int (jsplugin_allow_access)(const char *protocol, const char *hostname, int port, const char *url);
...
/** Set security permission required to access the object. Without calling this
    function the permission of an object defaults to default_permission specified in
    plugins capablilities structure.
    @param obj object which permission you want to modify.
    @param permission string describing permission assigned to an object
 */
typedef int (jsplugin_cb_set_permission)(const jsplugin_obj *obj, const char *permission);
...
struct JSP_STRUCT_ALIGN jsplugin_cap {
	...
	/** Plugin's default security permission. All jsplugin objects created in this plugin will require this permission to be usable.
        If the permission of a specific object needs to be changed jsplugin_cb_set_permission should be used.
	*/
	const char* default_permission;
};
...
struct JSP_STRUCT_ALIGN jsplugin_callbacks {
	...
	jsplugin_cb_set_permission *set_permission;
};
