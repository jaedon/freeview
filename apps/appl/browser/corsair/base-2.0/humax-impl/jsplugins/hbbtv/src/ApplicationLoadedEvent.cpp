...
ApplicationLoadedEvent::ApplicationLoadedEvent(jsplugin_obj *app, jsplugin_obj *c)
	: Event(c, "ApplicationLoaded"), application(app)
{
	if (application)
		addProtege(application);
}

int ApplicationLoadedEvent::getter(jsplugin_obj *obj, const char *name, jsplugin_value *result)
{
	ENTRY;
...
//virtual
int ApplicationLoadedEvent::serialize(jsplugin_value **argv)
{
	if (!application)
		return 0;
