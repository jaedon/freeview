...
int ApplicationManager::getter(jsplugin_obj *obj, const char *name, jsplugin_value *result)
{
	...
	GET_FUNCTION(loadPortal, obj, "n", JSP_GET_VALUE_CACHE);
#ifdef OIPF
...
bool ApplicationManager::isOipfApplication(ApplicationIdentifier appId)
{
	...
	return native_AppManIsOipfApplication(appId, &isOipfApp) == OOIF_RETURN_OK && isOipfApp;
}
...
void ApplicationManager::onApplicationManagerEvent(ApplicationManagerEvent event,
        ApplicationIdentifier appId)
{
    if ( isOipfApplication(appId) == TRUE )
    {
        OOIF_LOG_DEBUG("ApplicationManager event received %s, but it's oipf appId: %p\n", GetEventName(event), appId);
        return;
    }
...
void ApplicationManager::onApplicationDestroyed(ApplicationIdentifier appId)
{
    if ( isOipfApplication(appId) == TRUE )
    {
        OOIF_LOG_DEBUG("onApplicationDestroyed event received , but it's oipf appId: %p\n",  appId);
        return;
    }

	OOIF_LOG_DEBUG("Application destroyed callback received\n");
...
//interface for hd+SmartTV project
int ApplicationManager::loadPortal(jsplugin_obj *this_obj, jsplugin_obj *function_obj,int argc, jsplugin_value *argv, jsplugin_value *result)
{
	ENTRY;
	CHECK_ARGUMENT_COUNT(0);

	NATIVE_CALL(native_loadPortal());
...
void ApplicationManager::receiveKey(const char* event_name, int _keyCode)
{
	...
	{
		if (e = new ApplicationKeyEvent(ctx, event_name, _keyCode, nextActiveAppId))
		{
			ApplicationManager::sendKeyEventToApp(nextActiveAppId, e);
			delete e;
		}
