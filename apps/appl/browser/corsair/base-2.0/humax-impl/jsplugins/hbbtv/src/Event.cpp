...
OOIFBoolean hasEventPostfix(const char* str)
{
...
{
	eventName = HLIB_STD_StrDup(name);
	classType = CLASS_EVENT;

	if (hasEventPostfix(name))
	{
		stringRep = HLIB_STD_StrDup(name);
	}
	else
	{
		int len = strlen(name) + strlen("Event") + 1;
		stringRep = (char*)HLIB_STD_MemAlloc(len);
		snprintf(stringRep, len, "%s%s", name, "Event");
	}
}
...
void Event::initialize()
{
	...
	if (r != JSP_CREATE_OK)
	{
		OOIF_LOG_ERROR("Event object could not be created");
		return;
	}
...
Event::~Event()
{
	HLIB_STD_MemFree(eventName);
	HLIB_STD_MemFree(stringRep);
}
...
{
	OOIF_LOG_WARNING("This event has not implemented serialize()");
	return 0;
}
*/
