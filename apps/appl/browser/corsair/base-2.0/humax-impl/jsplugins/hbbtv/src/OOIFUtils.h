...
#ifndef __OOIF_UTILS_H
#define __OOIF_UTILS_H
/**
   Class containing static utility methods for OOIF
*/
...
public:
	...
	/** Creates a new string collection from the char* and int length in strings. This version of string collection can hold '\0' characters inside string. Frees strings and string_lenghts after use.*/
	static StringWithLengthCollection *CreateStringWithLengthCollection(int count, char **strings, int *string_lenghts);

	/** Returns a pointer to a temporary buffer that may be used to store
	 * strings that OOIF needs to retain ownership of when they need to be
	 ...
	 * but it isn't necessary to call it at all, excecpt when OOIF is unloaded.
	 *
	 ...
	 * sufficient to call it when OOIF is unloaded.
	 */
	 ...
	 */
	static int deserializeIntegerArray(const char *array, int *count, OOIFNumber **values);
