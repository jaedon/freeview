...
/** 
 *@note humax check whether the application is broadcast-related or not.
 *so humax modify code from native_PermissionsIsBroadcastRelated() to native_HmxPermissionsIsBroadcastRelated().
 */
...
#define SECURITY_CHECK(obj, check_type, fail_action) \
	do { \
		...
		is_secure = (check_type == CHECK_TRUSTED) ? \
				...
				(native_HmxPermissionsIsBroadcastRelated(window, host) == OOIF_RETURN_OK); \
		if (!is_secure) { \
			fail_action; \
		} \
	} while (0)
...
#define GET_SECURITY_CHECK(check_type) \
	SECURITY_CHECK(obj, check_type, THROW_SECURITY_ERR(obj, result, JSP_GET_EXCEPTION))
...
#define SET_SECURITY_CHECK(check_type) \
	SECURITY_CHECK(obj, check_type, THROW_SECURITY_ERR(obj, value, JSP_PUT_EXCEPTION))
...
#define CALL_SECURITY_CHECK(check_type, result) \
	do { \
		SECURITY_CHECK(this_obj, check_type, THROW_SECURITY_ERR(this_obj, result, JSP_CALL_EXCEPTION)); \
	} while (0)
