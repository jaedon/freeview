...
/**
	Super class for event listeners in Lovefilm. Implements the generic
...
{
private:
	pthread_mutex_t lovefilm_mutex;
...
protected:
	...
	NJSEReturnCode internalAddEventListener(const char *eventName, jsplugin_value *listener, bool intrinsic);
	NJSEReturnCode internalRemoveEventListener(const char *eventName, jsplugin_value *listener);
	NJSEReturnCode removeIntrinsicListener(const char *eventName);
	NJSEReturnCode findIntrinsicListener(const char *eventName, jsplugin_obj** retval);
