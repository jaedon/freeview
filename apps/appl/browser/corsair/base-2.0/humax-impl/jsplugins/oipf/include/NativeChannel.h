...
typedef enum {
	...
	,CHANNEL_CAS_FTA = 0,
	CHANNEL_CAS_NAGRA = 1,
	CHANNEL_CAS_IRDETO = 2,
	CHANNEL_CAS_NDS = 3,
	CHANNEL_CAS_VIACCESS = 4,
	CHANNEL_CAS_CONAX = 5,
	CHANNEL_CAS_MEDIAGUARD = 6,
	CHANNEL_CAS_CRYPTOWORKS = 7,
	CHANNEL_CAS_JPBCAS = 10,
	CHANNEL_CAS_JPCCAS = 11,
	CHANNEL_TYPE_ALL = 100
#endif
} ChannelType;
...
//	4000서비스 기준, PlugIn SVC Loading --> 17sec - 10sec
#define	FAST_SVC_LOAD

#ifdef	FAST_SVC_LOAD
	#define	FAST_LOAD_CACHE_SIZE		417			//	estimated size = DxNAME_LEN + CCID_LEN + TunerIDLEN + OpType
#endif
...
#endif /* OIPF */
#ifdef HMX_WEBUI
	OOIFNumber  svcSection;
	OOIFBoolean isHD;
	char *svcoptype;
	char *defaultAuth;
	OOIFBoolean hasTeletext;
#endif /* HMX_WEBUI */
#ifdef JLABS
	OOIFNumber serviceType;
	OOIFNumber threeDigit;
#endif

#ifdef	FAST_SVC_LOAD
	char *pszPtr;
	char tuningInfo[128];
	char szCache[FAST_LOAD_CACHE_SIZE];
#endif
} ChannelData;
...
#ifdef	HMX_WEBUI
	struct {
		ChannelHandle	*updated;
		int				count;
	} channel_list_update_data;
#endif
...
OOIFReturnCode native_ChannelConfigCreateChannelMask(OOIFBoolean *retval);
#endif
#ifdef	HMX_WEBUI
...
OOIFReturnCode native_ChannelSetNewChannel(ChannelHandle identifier, OOIFBoolean newChannel);
...
OOIFReturnCode native_ChannelGetdefaultAuth(ChannelHandle identifier, const char **retval);
...
#ifdef JLABS
OOIFReturnCode native_ChannelConfigIsValidBDR_JLabs(OOIFNumber bdr, OOIFBoolean* retval);
