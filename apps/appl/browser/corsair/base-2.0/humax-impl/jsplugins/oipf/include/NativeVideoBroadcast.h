...
typedef enum {
	...
	BROADCAST_EVENT_SELECTED_COMPONENT_CHANGED = 14
#endif /* OIPF || HBBTV_1_2_1*/
#if defined(HMX_WEBUI)
	,
	/** This event corresponds to the onCasMessageNotify */
	BROADCAST_EVENT_CASMESSAGE_NOTIFY = 15,
	/** This event corresponds to the onCasUiNotify */
	BROADCAST_EVENT_CASUI_NOTIFY = 16,
	/** This event corresponds to the onTrickRestrictModeChanged */
	BROADCAST_EVENT_TRICK_RESTRICT_MODE_CHANGED = 17,
	/** This event corresponds to the onRctNotify */
	BROADCAST_EVENT_SI_RCT_CHANGED = 18,
	/** This event corresponds to the onNctNotify */
	BROADCAST_EVENT_SI_NCD_CHANGED = 19,
	BROADCAST_EVENT_REQUEST_PINCODE = 20,

	/** This event corresponds to the onMheg */
	BROADCAST_EVENT_MHEG_REQUEST_TUNE = 21,
	BROADCAST_EVENT_MHEG_INTERRUPT = 22,

	BROADCAST_EVENT_SI_NETWORK_URI_CHANGED = 23,

	/**  */
	BROADCAST_EVENT_TSR_STATE_CHANGED = 24,
	
BROADCAST_EVENT_MHEG_ICS_STAUS = 25,
#endif // HMX_WEBUI
#ifdef JLABS
	,BROADCAST_EVENT_EMM_AUTO_DISPLAY_MESSAGE_NOTIFY,
	BROADCAST_EVENT_EMM_NOTIFY_NEW_MESSAGE,
	BROADCAST_EVENT_DRM_ERROR_NOTIFY,
	BROADCAST_EVENT_BML_ERROR,
	BROADCAST_EVENT_CA_LINK_MSG_NOTIFY,
	BROADCAST_EVENT_EVENT_RELAY_NOTIFY,
	BROADCAST_EVENT_EWS_NOTIFY,
	BROADCAST_EVENT_BML_START,
	BROADCAST_EVENT_BML_END,
	BROADCAST_EVENT_BS_CONTROL_STARTED,
	BROADCAST_EVENT_BML_VOD,
#endif // JLABS
} BroadcastEvent;
...
typedef union {
...
#endif /* OIPF || HBBTV_1_2_1 */
#if defined(HMX_WEBUI)
	/** used for BROADCAST_EVENT_CASMESSAGE_NOTIFY */
	struct {
		OOIFNumber module;
		OOIFNumber slot;
		OOIFNumber session;
	...
	} cas_message_notify_data;
	struct {
		OOIFNumber linkCount;
		void *rctInfo;
	} rct_info;
	struct {
		OOIFNumber startTimeOfChange;
		OOIFNumber ASODuration;
		const char *ncdmessage;
	} ncd_changed_info;
	struct {
		const char* msg;
	} request_pincode_data;
	struct {
		OOIFNumber svcUid;
		OOIFNumber tuneMode;
		ChannelHandle channel;
	...
	struct {
		OOIFNumber interruptFlag;
	} mheg_interrupt_info;
	
	struct {
		OOIFNumber mhegIcsFlag;
	} mheg_ics_status;
	...
	struct {
		OOIFBoolean isPaused;
	} tsr_status;
#endif // HMX_WEBUI
#ifdef JLABS
	/** used for BROADCAST_EVENT_EMM_AUTO_DISPLAY_MESSAGE_NOTIFY as described in JLabs-023 B.3.2.1.2 */
	struct {
		jlabsEmmAutoDisplayMessageHandle adm_info;
	} emm_auto_display_message_notify_data;
	/** used for BROADCAST_EVENT_EMM_NOTIFY_NEW_MESSAGE as described in JLabs-023 B.3.2.1.2 */
	struct {
		OOIFNumber type;
		OOIFNumber msg_id;
		jlabsEmmMessageHandle new_msg;
	} emm_notify_new_message_data;
	/** used for BROADCAST_EVENT_DRM_ERROR_NOTIFY as described in JLabs-023 B.3.2.1.2 */
	struct {
		OOIFNumber error_number;
		const char* drm_system_id;
	} drm_error_notify_data;
	/** used for BROADCAST_EVENT_BML_ERROR as described in JLabs-023 B.3.2.1.2 */
	struct {
		OOIFNumber error_number;
	} bml_error_data;
	/** used for BROADCAST_EVENT_CA_LINK_MSG_NOTIFY */
	struct {
		ChannelHandle channel;
		const char *message;
	} ca_link_message_data;
	/** used for BROADCAST_EVENT_EVENT_RELAY_NOTIFY as described in JLbabs-023 C.2.3.1.2 */
	struct {
		ChannelHandle channel;
	} event_relay;
	/** used for BROADCAST_EVENT_EWS_NOTIFY as described in JLbabs-023 C.2.3.1.2 */
	struct {
		ChannelHandle		channel;
		TunerHandle		tuner;
	} ews;
	/** used for BROADCAST_EVENT_BML_START as described in JLbabs-023 C.2.3.1.2 (J:COM extension) */
	struct {
		OOIFBoolean autoFlag;
	} bml_start;
	/** used for BROADCAST_EVENT_BML_END as described in JLbabs-023 C.2.3.1.2 (J:COM extension) */
	struct {
		OOIFNumber cause;
		char *param;
	} bml_end;
...
#ifdef JLABS
   BROADCAST_COMPONENT_TYPE_SUPERIMPOSE = 3,
   BROADCAST_COMPONENT_TYPE_DATA = 4,
#endif
...
#if defined(HMX_WEBUI)
/**
   This function corresponds to the HUMAX OIPF function
   createChannelObject(Integer majorChannel, Integer minorChannel) as
   defined by HUMAX
   \param id VideoBroadcast instance identifier
   \param idType As defined in DAE 7.13.1.3
   \param majorChannel As defined by HUMAX
   \param minorChannel As defined by HUMAX
   \param retval Retval as defined in DAE 7.13.1.3
   \return OOIF return code
*/
