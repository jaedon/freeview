...
AVComponentDataWrapper::~AVComponentDataWrapper()
{
	...
	DATA_FREE_STRING(language2);
#endif
#ifdef JLABS
	DATA_FREE_STRING(language2);
	DATA_FREE_STRING(text);
	if( data.multiviewNum > 0)
	{
		int i =0;
		for( i =0; i<data.multiviewNum; i++ )
		{
			HLIB_STD_MemFree( data.multiview[i] );
		}
		HLIB_STD_MemFree(data.multiview);
	}
#endif // JLABS
}
...
#if defined(OIPF) || defined(HBBTV_1_2_1)
	DATA_GET_NUMBER_OR_UNDEFINED(componentTag);
	DATA_GET_NUMBER_OR_UNDEFINED(pid);
	if (data_wrapper)
	{
...
#if defined(HMX_WEBUI)
	DATA_GET_NUMBER(subType);
	DATA_GET_RENAMED_STRING(language2, language2);
#endif
