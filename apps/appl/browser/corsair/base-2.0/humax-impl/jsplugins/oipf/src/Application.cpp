...
#ifdef HMX_WEBUI
std::string Application::m_ContentUrl;
#endif
...
int Application::getter(jsplugin_obj *obj, const char *name, jsplugin_value *result)
{
	...
	GET_FUNCTION(startWidget, obj, "obs", JSP_GET_VALUE_CACHE);
#else
...
int Application::createApplication(jsplugin_obj *this_obj, jsplugin_obj *function_obj, int argc, jsplugin_value *argv, jsplugin_value *result)
{
	...
	/* if we call native_ApplicationCreateApplication with null or 0 length string , it will be crashed */
	if ( STRING_VALUE(argv[0]) == NULL || strlen(STRING_VALUE(argv[0])) == 0 ){
...
#if defined(HMX_WEBUI)
		if ( argc == 0 )
			NATIVE_CALL(native_ApplicationActivateInput(app->appId, true));
		else
...
#if defined(CONFIG_OP_JCOM)
	char * widgetName;
	native_WidgetDescriptorGetString(WIDGET_NAME, wd->getIdentifier(), (const char**)&widgetName);
	HLIB_CMD_CaptureLog((const HCHAR*)"WidgetStart,WidgetName=%s", widgetName);
#endif
...
#if defined(HMX_WEBUI)
	if(argc == 3 && TYPE_STRING(argv[2]))
	{
		const char *url = STRING_VALUE(argv[2]);
		if(url)
			Application::m_ContentUrl = url;
	}
#endif
