...
ChannelList::ChannelList(ChannelHandle *handles, int count)
{
	ENTRY;
...
bool	ChannelList::removeChannel (Channel *_channel)
{
    ENTRY;

    if (_channel == NULL || _channel->data_wrapper == NULL)
    	return false;

    for (int i = 0; i < getSize(); i++) {
    	Channel *channel = internalGetItem(i);
    	if (channel->data_wrapper)
    	{
    		if (strcmp(channel->data_wrapper->data.ccid, _channel->data_wrapper->data.ccid) == 0) {
    			NATIVE_WARN_NOK(native_ChannelConfigRemoveChannel(_channel->getIdentifier()));

    			internalRemoveIndex(i);
    			return true;
    		}
    	}
    }
...
bool	ChannelList::removeChannelList (void)
{
    ENTRY;

    NATIVE_WARN_NOK(native_ChannelConfigRemoveChannelList());
...
// TODO: 현재로서는 ipBroadcastID은 NULL이다.. 어떻게 할지 결정해야함.
#if 0 //def OIPF
