...
{
	event_name = HLIB_STD_StrDup(eventName);
}


EventListener::~EventListener()
{
	HLIB_STD_MemFree(event_name);
}
...
void EventListener::dispatch(Event* event, jsplugin_obj *context)
{
	...
	if (!argv[0].u.object)
	{
		...
		OOIF_LOG_ERROR("failed to create event to dispatch");
		return;
	}
	...
	{
		OOIF_LOG_ERROR("Dispatch event failed");
	}
	else
	{
		event->dispatched();
...
void IntrinsicEventListener::dispatch(Event* event, jsplugin_obj *context)
{
	...
	int r = g_opera_callbacks->call_function(context, context,
	        ...
	        event->getListenerCallback());
	if (r != JSP_CALL_NO_VALUE)
		OOIF_LOG_ERROR("Dispatch intrinsic event failed");
	delete[] argv;
}
...
{
	if (listener)
		this->listener = HLIB_STD_StrDup(listener);
}
...
void IntrinsicEvalEventListener::dispatch(Event* event, jsplugin_obj *context)
{
	...
	if (r != JSP_CALLBACK_OK)
		OOIF_LOG_ERROR("Dispatch intrinsic string event failed");
}
...
{
	pthread_mutex_init(&ooif_mutex, NULL);
...
EventTarget::~EventTarget()
{
	...
	pthread_mutex_unlock( &ooif_mutex );
	pthread_mutex_destroy(&ooif_mutex);
}
...
void EventTarget::findNewPollingContext()
{
	...
	if (!targets.empty())
	{
		...
		if (r != JSP_POLL_OK)
			OOIF_LOG_ERROR("Failed to set up polling callbacks");
	}
}
...
void EventTarget::addNewListener(EventListener *listener)
{
	...
	for (list<EventListener*>::iterator it = listeners.begin(); it!=listeners.end(); it++)
	{
		...
		if (listener->matchListener(current))
		{
			...
			{
				OOIF_LOG_DEBUG("replacing old event listener");
				removeListener(current);
				break;
			}
			else
			{
				OOIF_LOG_DEBUG("listener is a duplicate, not adding");
				delete listener;
				return;
			}
		}
	}
	OOIF_LOG_DEBUG("Adding new %s listener", listener->getEventName());
...
OOIFReturnCode EventTarget::internalAddEventListener(const char *eventName, jsplugin_value *listener, bool intrinsic)
{
	ENTRY;

	if (!listener)
	{
		OOIF_LOG_WARNING("Trying to add a NULL listener.");
		return OOIF_RETURN_OK;
	}

	OOIFReturnCode check = checkListenerAccessAllowed(eventName);
	if (check != OOIF_RETURN_OK)
		return check;
...
OOIFReturnCode EventTarget::internalRemoveEventListener(const char *eventName,
											            jsplugin_value *listener)
{
	...
	{
		OOIF_LOG_DEBUG("removing event listener on %s", eventName);
...
OOIFReturnCode EventTarget::removeIntrinsicListener(const char *eventName)
{
	OOIFReturnCode check = checkListenerAccessAllowed(eventName);
	if (check != OOIF_RETURN_OK)
		return check;
	...
	if (matching_listener)
	{
		OOIF_LOG_DEBUG("removing intrinsic listener on %s", eventName);
		removeListener(matching_listener);
	}
...
OOIFReturnCode EventTarget::findIntrinsicListener(const char *eventName, jsplugin_obj **retval)
{
	OOIFReturnCode check = checkListenerAccessAllowed(eventName);
	if (check != OOIF_RETURN_OK)
		return check;
...
OOIFReturnCode EventTarget::checkListenerAccessAllowed(const char *eventName)
{
...
int EventTarget::removeEventListener(jsplugin_obj *this_obj, jsplugin_obj *function_obj, int argc, jsplugin_value *argv, jsplugin_value *result)
{
	...
	if (argc != 2 && argc != 3)
	{
		OOIF_LOG_WARNING("Invalid argument count to removeEventListener() %d "
		                 "instead of 2 or 3 - aborting", argc);
		return JSP_CALL_ERROR;
	}
	if (argv[0].type == JSP_TYPE_STRING && argv[1].type == JSP_TYPE_NATIVE_OBJECT)
	...
	{
		OOIF_LOG_WARNING("Invalid argument passed to removeEventListener() - aborting");
		return JSP_CALL_ERROR;
	}
}
