...
int FavouriteList::getter(jsplugin_obj *obj, const char *name, jsplugin_value *result)
{
	...
	GET_FUNCTION(insertBefore2, obj, "nsn", JSP_GET_VALUE_CACHE);
	GET_FUNCTION(remove, obj, "n", JSP_GET_VALUE_CACHE);
	GET_FUNCTION(commit, obj, "", JSP_GET_VALUE_CACHE);
	GET_FUNCTION(commit2, obj, "", JSP_GET_VALUE_CACHE);

	return JSP_GET_NOTFOUND;
}

int FavouriteList::setter(jsplugin_obj *obj, const char *name, jsplugin_value *value)
{
	ENTRY;

	NATIVE_SET("name", native_FavouriteListSetName(getIdentifier(), STRING_VALUE(value[0])));
...
//static
int FavouriteList::insertBefore2(jsplugin_obj *this_obj, jsplugin_obj *function_obj, int argc, jsplugin_value *argv, jsplugin_value *result)
{
	CHECK_ARGUMENT_COUNT(3);
	FavouriteList *instance = EXTRACT_OBJECT_OR_THROW(instance, this_obj, FavouriteList);
	...
	Channel *newChannel = new Channel(id);
	newChannel->changeMajorChannel(NUMBER_VALUE(argv[2]));
...
int FavouriteList::commit2(jsplugin_obj *this_obj, jsplugin_obj *function_obj, int argc, jsplugin_value *argv, jsplugin_value *result)
{
	ENTRY;
	CHECK_ARGUMENT_COUNT(1);

	FavouriteList *instance = EXTRACT_OBJECT_OR_THROW(instance, this_obj, FavouriteList);
	OOIFNumber svcType = NUMBER_VALUE(argv[0]);
	FAV_LIST_COMMIT2(svcType, instance);
	RETURN_BOOLEAN(1, JSP_CALL_VALUE);
}
