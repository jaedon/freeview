...
#ifdef JLABS
	INTRINSIC_EVENT_GET(PowerStateChangeError);
#endif
...
#ifdef JLABS
	INTRINSIC_EVENT(PowerStateChangeError);
#endif
...
#ifdef JLABS
		case LOCAL_SYSTEM_EVENT_POWER_STATE_CHANGE_ERROR:
		{
			OOIF_LOG_DEBUG("PowerStateChangeError event received");
			OOIFNumber reason = info->power_state_change_error_data.reason;
			e = new PowerStateChangeErrorEvent(context, reason);
			break;
		}
#endif
		default:
