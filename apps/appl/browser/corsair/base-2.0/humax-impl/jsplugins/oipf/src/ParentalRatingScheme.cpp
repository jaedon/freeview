...
int ParentalRatingScheme::getter(jsplugin_obj *obj, const char *name, jsplugin_value *result)
{
	 ...
	 */
	GET_FUNCTION(changeThreshold, obj, "s", JSP_GET_VALUE);
#endif
 ...
 * This function originate from JLABS, But HUMAX WEB UI will be use, also.
 */
int ParentalRatingScheme::changeThreshold(jsplugin_obj *this_obj, jsplugin_obj *function_obj, int argc, jsplugin_value *argv, jsplugin_value *result)
{
	CHECK_ARGUMENT_COUNT(1);
	ParentalRatingScheme *prs = EXTRACT_OBJECT_OR_THROW(prs, this_obj, ParentalRatingScheme);
	const char *pcRatingValue = NULL;
	unsigned int ratingValue = 0;
	OOIFNumber retval;
	pcRatingValue = STRING_VALUE(argv[0]);
	if(pcRatingValue == NULL)
	{
		retval = 1;
	}
	else
	{
		ratingValue = HLIB_STD_StrToUINT32(pcRatingValue);
		NATIVE_CALL(native_ParentalRatingSchemeChangeThreshold(prs->getIdentifier(),ratingValue, &retval));
	}
