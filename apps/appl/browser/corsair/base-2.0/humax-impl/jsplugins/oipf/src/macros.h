...
#define ENTRY OOIF_LOG_DEBUG("=> Entering\n")
...
#define RETURN_STRING(value, returntype) \
	do { \
		...
		if (!str_value) { \
			OOIF_LOG_WARNING("null value found when string was expected  \n"); \
...
#define GET_FAIL \
	do { \
		OOIF_LOG_WARNING("Failed to find the following variable: %s  \n", name); \
...
#define CHECK_ARGUMENT_COUNT_MIN_MAX(min, max) \
	do { \
		if (argc < min) { \
			OOIF_LOG_WARNING("EE Invalid argument count: %d instead of %d  \n", argc, min); \
			THROW_WRONG_ARG_ERR(this_obj, result, JSP_CALL_EXCEPTION); \
		} \
		else if (argc > max) { \
			OOIF_LOG_WARNING("Invalid argument count: %d instead of %d  \n", argc, max); \
		} \
	} while (0)
...
#define CHECK_ARGUMENT(index, wantedtype) \
	do { \
		if (argv[index].type != wantedtype) { \
			OOIF_LOG_WARNING("EE Invalid parameter:, in parameter %d  \n", index); \
			THROW_WRONG_ARG_ERR(this_obj, result, JSP_CALL_EXCEPTION); \
		} \
	} while (0)
...
#define _INTRINSIC_EVENT(event) \
	do { \
		if (value->type == JSP_TYPE_NATIVE_OBJECT) \
			...
			OOIF_LOG_WARNING("Invalid listener received when trying to set an intrinsic listener for %s  \n", event); \
	} while (0)
...
#define HANDLE_OBJECT_CREATE(instance, returntype) \
	do { \
		...
		*trace = ObjectInstance::gc_proxy; \
		...
		if (r != JSP_CALLBACK_OK) \
			OOIF_LOG_WARNING("Failed to add attr_change listener  \n"); \
		r = g_opera_callbacks->set_param_set_listener(object_obj, ObjectInstance::param_set_proxy); \
		if (r != JSP_CALLBACK_OK) \
			OOIF_LOG_WARNING("Failed to add param_set listener  \n"); \
...
#define TYPE_NUMBER(arg) \
	(arg.type == JSP_TYPE_NUMBER && !OOIFUtils::isNaN(arg.u.number))
...
#define NATIVE_GET(native_getter) \
	do { \
		AsyncHandler::setToken(obj); \
		OOIFReturnCode ret = native_getter; \
		AsyncHandler::setToken(NULL); \
		switch (ret) \
		{ \
		case OOIF_RETURN_OK: \
			break;	/*handle in calling macro */ \
		case OOIF_RETURN_VALUE_NULL: \
			RETURN_NULL(JSP_GET_VALUE); \
		case OOIF_RETURN_VALUE_UNDEFINED: \
			RETURN_UNDEFINED(JSP_GET_VALUE); \
		case OOIF_RETURN_ASYNC: \
			AsyncHandler::trackToken(obj); \
			RETURN_NULL(JSP_GET_DELAYED); \
		case OOIF_RETURN_ERROR: \
			THROW_GENERIC_ERR(obj, result, JSP_GET_EXCEPTION); \
		case OOIF_RETURN_OOM_ERROR: \
			THROW_OOM_ERR(obj, result, JSP_GET_EXCEPTION); \
		case OOIF_RETURN_TYPE_ERROR: \
			THROW_TYPE_ERR(obj, result, JSP_GET_EXCEPTION); \
		case OOIF_RETURN_SECURITY_ERROR: \
			THROW_SECURITY_ERR(obj, result, JSP_GET_EXCEPTION); \
		case OOIF_RETURN_NOT_SUPPORTED: \
			return JSP_GET_NOTFOUND; \
		default: \
			OOIF_LOG_ERROR("Unexpected return type received from native function  \n"); \
			return JSP_GET_NOTFOUND; \
		} \
	} while (0)
#else //OOIF_ASYNC
#define NATIVE_GET(native_getter) \
	do { \
		...
		case OOIF_RETURN_NOT_SUPPORTED: \
			return JSP_GET_NOTFOUND; \
		default: \
			OOIF_LOG_ERROR("Error in return type from native function \n"); \
			return JSP_GET_NOTFOUND; \
		} \
	} while (0)
#endif // OOIF_ASYNC
	...
	do { \
		if (strcmp(name, identifier) == 0) { \
			...
			_GET_COLLECTION(context, native_getter, \
							OOIFUtils::CreateStringCollection(SIZE, NATIVE_ARRAY), \
							returntype); \
		} \
	} while (0)
...
#define NATIVE_SET(id, native_setter) \
	do { \
		if (strcmp(name, id) == 0) { \
			OOIFReturnCode ret = native_setter; \
			_NATIVE_SET(ret); \
		} \
	} while (0)
...
#define NATIVE_CALL(native_caller) \
	do { \
		OOIFReturnCode ret = native_caller; \
		_NATIVE_CALL(ret); \
	} while (0)
...
#define NATIVE_CALL_HANDLE_OBJ(native_caller, instance_creator, obj) \
	do { \
		...
		if (obj) \
		{ \
			WINDOW = OOIFUtils::getWindow(obj); \
			HOST = OOIFUtils::getHost(obj); \
		} \
		OOIFReturnCode ret = native_caller; \
		...
		{ \
			*result = instance_creator; \
			...
			OOIF_LOG_WARNING("non-OK return value from native call to create object (return code = %d) \n", ret); \
		} \
		return ret; \
	} while (0)
...
#define NATIVE_WARN_NOK(native_func) \
	do { \
		if ((native_func) != OOIF_RETURN_OK) \
			OOIF_LOG_WARNING("Non-OK return type from native function when OK was expected \n"); \
	} while (0)

#ifdef OIPF
// OIPF favorite list handling
#define FAV_LIST_COMMIT(instance) \
	do { \
		/* Now we have to commit all our channels to the terminal  */ \
		/* We simply serialize the ccid strings and pass them down */ \
		int size = instance->getSize(); \
		char **ccids = new char*[size]; \
		for (int i=0; i<size; i++) { \
			ccids[i] = HLIB_STD_StrDup(instance->internalGetItem(i)->data_wrapper->data.ccid); \
		} \
		OOIFReturnCode _ret = native_FavouriteListCommit(instance->getIdentifier(), ccids, size); \
		for (int i=0; i<size; i++) { \
			HLIB_STD_MemFree(ccids[i]); \
		} \
		delete[] ccids; \
		NATIVE_CALL(_ret); \
	} while (0)

#define FAV_LIST_COMMIT2(svcType, instance) \
	do { \
		/* Now we have to commit all our channels to the terminal  */ \
		/* We simply serialize the ccid strings and pass them down */ \
		int size = instance->getSize(); \
		char **ccids = new char*[size]; \
		int *lcnList = new int[size]; \
		for (int i=0; i<size; i++) { \
			ccids[i] = HLIB_STD_StrDup(instance->internalGetItem(i)->data_wrapper->data.ccid); \
			lcnList[i] = instance->internalGetItem(i)->data_wrapper->data.majorChannel; \
		} \
		OOIFReturnCode _ret = native_FavouriteListCommit2(instance->getIdentifier(), ccids, lcnList, size, svcType); \
		for (int i=0; i<size; i++) { \
			HLIB_STD_MemFree(ccids[i]); \
		} \
		delete[] ccids; \
...
#define SECURITY_CHECK(obj, check_type, fail_action) \
	do { \
		bool is_secure; \
		char *host = OOIFUtils::getHost(obj); \
		long int window = OOIFUtils::getWindow(obj); \
		is_secure = (check_type == CHECK_TRUSTED) ? \
				(native_PermissionsIsTrusted(window, host) == OOIF_RETURN_OK ) : \
				(native_PermissionsIsBroadcastRelated(window, host) == OOIF_RETURN_OK); \
		if (!is_secure) { \
			fail_action; \
		} \
	} while (0)
...
// for handle_object
#define HANDLE_SECURITY_CHECK(check_type) \
			SECURITY_CHECK(global_obj, check_type, return JSP_OBJECT_ERROR);

#define HANDLE_GUARDED_SECURITY_CHECK(value, match, obj, check_type) \
	do { \
		if (!strcmp(value, match)) { \
			HANDLE_SECURITY_CHECK(check_type); \
		} \
	} while (0)

// for getters
#define GET_SECURITY_CHECK(check_type) \
	SECURITY_CHECK(obj, check_type, THROW_EXCEPTION(obj, "Error", "SecurityError", result, JSP_GET_EXCEPTION))

#define GET_GUARDED_SECURITY_CHECK(val, check_type) \
	do { \
		if (!strcmp(val, name)) { \
			GET_SECURITY_CHECK(check_type); \
		} \
	} while (0)

// for setters
#define SET_SECURITY_CHECK(check_type) \
	SECURITY_CHECK(obj, check_type, THROW_EXCEPTION(obj, "Error", "SecurityError", value, JSP_PUT_EXCEPTION))

#define SET_GUARDED_SECURITY_CHECK(val, check_type) \
	do { \
		if (!strcmp(val, name)) { \
			SET_SECURITY_CHECK(check_type); \
		} \
	} while (0)

// for function calls
#define CALL_SECURITY_CHECK(check_type, result) \
	do { \
		SECURITY_CHECK(this_obj, check_type, THROW_EXCEPTION(this_obj, "Error", "SecurityError", result, JSP_CALL_EXCEPTION)); \
	} while (0)

// for checkListenerAccessAllowed
#define LISTENER_ACCESS_SECURITY_CHECK(check_type) \
	do { \
		SECURITY_CHECK(getHost(), check_type, return OOIF_RETURN_SECURITY_ERROR); \
	} while (0)


#define LISTENER_ACCESS_GUARDED_SECURITY_CHECK(val, check_type) \
	do { \
		if (!strcmp(val, eventName)) { \
			LISTENER_ACCESS_SECURITY_CHECK(check_type); \
		} \
	} while (0)
...
#define NATIVE_SECURITY_CHECK(obj) \
	do { \
		char *host = OOIFUtils::getHost(obj); \
		if (strncmp(host, "file", 4)) return OOIF_RETURN_SECURITY_ERROR;\
	} while (0)
#endif
