...
(function() {
	...
	XMLHttpRequest.prototype.__defineSetter__('onreadystatechange', function( actualListener ){
		...
		return orscSetter.call(this, ( actualListener ? intermediateListener : null ));
	});
})();
...
(function() {
	...
	if ( oipfObjectFactory.createApplicationManagerObject().getOwnerApplication(window.document) )
	{
			...
			// Added codes to show Parent Application(the main page of TV Apps) after this application is destoryed
			//app.deactivateInput();
			//app.hide();
...
// Fix the problem that application is destoryed when the user press back key on RCU on menu not main menu in Youtube V4
(function() {
	var dispatchEvent = HTMLElement.prototype.dispatchEvent;
	HTMLElement.prototype.dispatchEvent = function(e) {
		var handled = false;
		if ( location.hostname.indexOf('youtube.com') != -1 )   { 
			if (e.type == "keydown" && e.which == 27) {
				console.log(e);
				console.log("WARNNIG! Key(" + e.which + ") generated from [" + window.location.hostname + "] PREVENTED");
				handled = true;
			}
		}
		if (!handled) {
