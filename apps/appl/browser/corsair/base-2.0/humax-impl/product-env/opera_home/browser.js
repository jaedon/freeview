// 
/**
...
(function(opera){
	...
	var bjsversion=' Opera Linux SDK 5852 core 2.12.407, January 18, 2013. Active patches: 72 ';
...
if(!opera.ignoreNavigationBlocking){opera.ignoreNavigationBlocking=function(){
	var elm;
	opera.addEventListener('BeforeEvent.keydown',function(e){
			if(e.event.shiftKey && e.event.keyCode >= 37 && e.event.keyCode <=40){
				elm=document.activeElement;
			}
	},false);
	opera.addEventListener('AfterEventListener.keydown',function(e){
		if(e.event.shiftKey && e.event.defaultPrevented && elm==document.activeElement){
...
// PATCH-138, Asia-region Generic Patches
	...
	if((hostname.indexOf('tokyo.jp')>-1)||(hostname.indexOf('lg.jp')>-1)){
	...
	} else if(hostname.contains('.bing.')){
		if(pathname.indexOf('/images')==0){opera.ignoreNavigationBlocking()}
		
		log('ITVSDK-8037, Prevent spatnav from getting stuck in bing.com image search filter menus');
	} else if(hostname.contains('.blogger.')){
		opera.ignoreNavigationBlocking();
		log('ITVSDK-8287, Prevent spatnav from getting stuck in blogger.com main menu');
	} else if(hostname.contains('.paypal.')){
		opera.ignoreNavigationBlocking();
		log('ITVSDK-8037, Prevent spatnav from getting stuck in paypal.com main menu');
		...
		opera.ignoreNavigationBlocking();
		log('ITVSDK-8287, Prevent spatnav from getting stuck in taobao.com main menu');
	} else if(hostname.endsWith('tmall.com')){
		opera.ignoreNavigationBlocking();
		log('ITVSDK-8287, Prevent spatnav from getting stuck in tmall.com main menu');
		...
		// ITVSDK-8238, Prevent spatnav from getting stuck in menus and search results on google image search
		// PATCH-343, Disable Google Search suggestions
	
		if(href.indexOf('tbm=isch')>-1){
			opera.addEventListener('BeforeEvent.keydown',function(e){e.preventDefault()});
			opera.ignoreNavigationBlocking();
		}
		...
		if(pathname.indexOf('/watch')>-1 || pathname.indexOf('/embed')>-1){
		 opera.ignoreNavigationBlocking();
		}
		log('PATCH-1086, youtube - work around old fullscreen spec usage\nITVSDK-8283, Prevent spatnav from getting stuck in youtube videos embedded with iframes');
		...
		log('PATCH-1068, amazon - avoid looping hash decode');
