...
void SimpleApplicationBoundaryDescriptor::AddBoundaryExtension(const char* ext_byte)

{
	if(boundary_extension_count > 0)
	{
		boundary_extension_length[boundary_extension_count] = strlen(ext_byte);
		strncpy((char *)boundary_extension_byte[boundary_extension_count], ext_byte, strlen(ext_byte));
