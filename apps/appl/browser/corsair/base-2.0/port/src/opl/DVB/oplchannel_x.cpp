...
#include "../DVB/cfavouritemanager.h"
...
static HCHAR *opl_channel_GetStrCodeRate(DxSat_CodeRate_e eCodeRate)
{
	switch(eCodeRate)
	{
		case eDxSAT_CODERATE_AUTO: return (HCHAR *)"AUTO"; break;
		case eDxSAT_CODERATE_NONE: return (HCHAR *)"NONE"; break;
		case eDxSAT_CODERATE_1_2: return (HCHAR *)"1/2"; break;
		case eDxSAT_CODERATE_2_3: return (HCHAR *)"2/3"; break;
		case eDxSAT_CODERATE_3_4: return (HCHAR *)"3/4"; break;
		case eDxSAT_CODERATE_5_6: return (HCHAR *)"5/6"; break;
		case eDxSAT_CODERATE_7_8: return (HCHAR *)"7/8"; break;
		case eDxSAT_CODERATE_3_5: return (HCHAR *)"3/5"; break;
		case eDxSAT_CODERATE_4_5: return (HCHAR *)"4/5"; break;
		case eDxSAT_CODERATE_5_11: return (HCHAR *)"5/11"; break;
		case eDxSAT_CODERATE_6_7: return (HCHAR *)"6/7"; break;
		case eDxSAT_CODERATE_8_9: return (HCHAR *)"8/9"; break;
		case eDxSAT_CODERATE_9_10: return (HCHAR *)"9/10"; break;
		default: break;
	}
...
static HCHAR *opl_channel_GetStrPolar(DxSat_Polarization_e ePolar)
{
	switch(ePolar)
	{
		case eDxSAT_POLAR_HOR: return (HCHAR *)"H"; break;
		case eDxSAT_POLAR_VER: return (HCHAR *)"V"; break;
		case eDxSAT_POLAR_LEFT: return (HCHAR *)"L"; break;
		case eDxSAT_POLAR_RIGHT: return (HCHAR *)"R"; break;
		default: break;
	}
...
static HCHAR *opl_channel_GetStrDVBTSystem(DxTer_System_e eDvbTSystem)
{
	switch(eDvbTSystem)
	{
		case eDxTER_DVB_T: return (HCHAR *)"DVB-T"; break;
		case eDxTER_DVB_T2: return (HCHAR *)"DVB-T2"; break;
...
static HCHAR *opl_channel_GetStrTBandWidth(DxTer_BandWidth_e eBandWidth)
{
	switch(eBandWidth)
	{
		case eDxTER_BANDWIDTH_8M: return (HCHAR *)"8M"; break;
		case eDxTER_BANDWIDTH_7M: return (HCHAR *)"7M"; break;
		case eDxTER_BANDWIDTH_6M: return (HCHAR *)"6M"; break;
		case eDxTER_T2BANDWIDTH_5MHZ: return (HCHAR *)"T2_5M"; break;
		case eDxTER_T2BANDWIDTH_10MHZ: return (HCHAR *)"T2_10M"; break;
		case eDxTER_T2BANDWIDTH_1P7MHZ: return (HCHAR *)"T2_1P7M"; break;
...
static HCHAR *opl_channel_GetStrTTransmode(DxTer_TransMode_e eTransMode)
{
	switch(eTransMode)
	{
		case eDxTER_TRANSMODE_AUTO: return (HCHAR *)"AUTO"; break;
		case eDxTER_TRANSMODE_2K: return (HCHAR *)"2K"; break;
		case eDxTER_TRANSMODE_8K: return (HCHAR *)"8K"; break;
		case eDxTER_T2TRANSMODE_1K: return (HCHAR *)"T2_1K"; break;
		case eDxTER_T2TRANSMODE_4K: return (HCHAR *)"T2_4K"; break;
		case eDxTER_T2TRANSMODE_16K: return (HCHAR *)"T2_16K"; break;
		case eDxTER_T2TRANSMODE_32K: return (HCHAR *)"T2_32K"; break;
...
static HCHAR *opl_channel_GetStrTGuardInterval(DxTer_GuardInterval_e eGuardInterval)
{
	...
	{
		case eDxTER_GUARDINTERVAL_AUTO: return (HCHAR *)"AUTO"; break;
		case eDxTER_GUARDINTERVAL_1_4: return (HCHAR *)"1/4"; break;
		case eDxTER_GUARDINTERVAL_1_8: return (HCHAR *)"1/8"; break;
		case eDxTER_GUARDINTERVAL_1_16: return (HCHAR *)"1/16"; break;
		case eDxTER_GUARDINTERVAL_1_32: return (HCHAR *)"1/32"; break;
		case eDxTER_T2GUARDINTERVAL_1_128: return (HCHAR *)"T2_1/128"; break;
		case eDxTER_T2GUARDINTERVAL_19_128: return (HCHAR *)"T2_19/128"; break;
		case eDxTER_T2GUARDINTERVAL_19_256: return (HCHAR *)"T2_19/256"; break;
...
static char *
	opl_channel_GetTextBuffer (ChannelManager_t *instance, unsigned int size)
{
	if (instance == NULL)
	{
		instance = opl_channel_GetManager(NULL);
		if (instance == NULL)
			return NULL;
	}

	if (instance->charBuf == NULL)
	{
		instance->charBuf = HLIB_CBUF_New(8192);
		if (instance->charBuf == NULL)
			return NULL;
	}
	return (char *)HLIB_CBUF_GetBuffer(instance->charBuf, (HINT32)size);
}
...
static void opl_channel_EventListener (HINT32 event, const HINT32 *args, void *userdata)
{
...
#ifdef USE_NEW_FAV
		CFavouriteManager::getInstance()->buildWithSystemChannels();
#endif
...
static HBOOL opl_channel_FreesatUpdateChecker (const HCHAR *typeName, const void *_old, const void *_new, HBOOL *updated)
{
                                                                ...
                                                                HBOOL *updated)
{
    *updated = FALSE;
    ...
    // TODO: transponder, network, providerµµ ÇÊ¿äÇÑ°¡.
...
static HBOOL opl_channel_BaseUpdateChecker (const HCHAR *typeName, const void *_old, const void *_new, HBOOL *updated)
{
...
static HBOOL opl_channel_UkDttUpdateChecker (const HCHAR *typeName, const void *_old, const void *_new, HBOOL *updated)
{
	DxService_t *pOld = (DxService_t*)_old;
	DxService_t *pNew = (DxService_t*)_new;
	...
	if (pOld && pNew)	// updated
	{
		if (strcmp(typeName, DxNAMEOF(DxService_t)) == 0)
		{
			if (	INT_UPDATED(uid)
				...
				|| (STR_UPDATED(name) && pNew->renamed)
				|| INT_UPDATED(removed)
				|| INT_UPDATED(ex.tva.ucGuidanceType)
				|| INT_UPDATED(ex.tva.ucGuidanceMode)
				|| STR_UPDATED(ex.tva.szGuidanceStr)
...
{
				HUINT32 i;
				printf("\n\n"C_CODE_F_YELLOW"========================================================"C_CODE_RESET"\n", pOld->name, pNew->name);
				printf(C_CODE_F_YELLOW"Svc Name ( %s :%s) "C_CODE_RESET"\n", pOld->name, pNew->name);
				printf(C_CODE_F_YELLOW"CH DB Update Check [%d %d %d %d : %d %d %d %d : %d %d %d %d : %d]"C_CODE_RESET"\n\n",
									INT_UPDATED(uid)
									,INT_UPDATED(tsuid)
									,INT_UPDATED(prvuid)
									,MEM_UPDATED(grpuids, (sizeof(HINT32) * DxGROUP_LEN))
				...
				if(INT_UPDATED(uid))									printf("uid changed (0x%x -> 0x%x)\n", pOld->uid, pNew->uid);
				if(INT_UPDATED(tsuid))									printf("tsuid changed (0x%x -> 0x%x)\n", pOld->tsuid, pNew->tsuid);
				if(INT_UPDATED(prvuid))								printf("prvuid changed (0x%x -> 0x%x)\n", pOld->prvuid, pNew->prvuid);
				if(MEM_UPDATED(grpuids, (sizeof(HINT32) * DxGROUP_LEN)))
				{
					for(i = 0; i <DxGROUP_LEN ; i++)
						printf("grpuids changed [%d](0x%x, 0x%x)\n", i, pOld->grpuids[i], pNew->grpuids[i]);
				}
				if(INT_UPDATED(svcid))									printf("svcid changed (0x%x -> 0x%x)\n", pOld->svcid, pNew->svcid);
				if(INT_UPDATED(tsid))									printf("tsid changed (0x%x -> 0x%x)\n", pOld->tsid, pNew->tsid);
				if(INT_UPDATED(onid))									printf("onid changed (0x%x -> 0x%x)\n", pOld->onid, pNew->onid);
				if(INT_UPDATED(lcn))									printf("lcn changed (0x%x -> 0x%x)\n", pOld->lcn, pNew->lcn);
				if(STR_UPDATED(name) && pNew->renamed)				printf("name changed : renamed(%d) (%s ->  %s)\n", pNew->renamed, pOld->name, pNew->name);
				if(INT_UPDATED(removed))								printf("removed changed (0x%x ->  0x%x)\n", pOld->removed, pNew->removed);
				if(INT_UPDATED(ex.tva.ucGuidanceType))					printf("GuidanceType changed (0x%x ->  0x%x)\n", pOld->ex.tva.ucGuidanceType, pNew->ex.tva.ucGuidanceType);
				if(INT_UPDATED(ex.tva.ucGuidanceMode))				printf("GuidanceMode changed (0x%x ->  0x%x)\n", pOld->ex.tva.ucGuidanceMode, pNew->ex.tva.ucGuidanceMode);
				if(STR_UPDATED(ex.tva.szGuidanceStr))					printf("GuidanceStr changed(%s -> %s)\n", pOld->ex.tva.szGuidanceStr, pNew->ex.tva.szGuidanceStr);
				printf(C_CODE_F_YELLOW"========================================================"C_CODE_RESET"\n\n");
}
#endif
			}
		}
...
void	OPL_Channel_AddListener (OPL_ChannelManager_t manager, OPL_ChannelListener_t listener)
{
	...
	if (listener)
	{
		...
		APK_META_SVC_SetUpdateChecker(opl_channel_UkDttUpdateChecker);
#else
		APK_META_SVC_SetUpdateChecker(opl_channel_BaseUpdateChecker);
#endif
...
int		OPL_Channel_GetFreesatGenre (OPL_ChannelManager_t manager, char ***genre)
{
	...
	list = APK_META_SVC_GetGroupList(); // ¿�? freesat °¡n¿1ࡀ�½n.
...
OPL_BOOL 			OPL_Channel_IsPossibleToTuneByUpDown(OPL_Channel_t channel, int tuningRange)
{
    ...
    if (service)
    {
        /*svc type?Ì TV, Radio, Data ¼ÂÁß ÇÏ³ª°¡ ¾Æ´Ï¶ó¸é ¼±±¹ÇÒ ¼ö ¾ø´Â Ã¤³Î?Ì´Ù */
        ...
        switch(tuningRange)
        {
            case eTuningRange_TV:
                /*¼±±¹´ë»ó?Ì TV?Îµ¥ svc type?Ì TV°¡ ¾Æ´Ï¶ó¸é ¼±±¹ÇÒ ¼ö ¾ø´Ù.*/
            ...
            case eTuningRange_Radio_Data:
                /*¼±±¹´ë»ó?Ì RADIO_DATA?Îµ¥ svc type?Ì radio ¶Ç´Â data °¡ ¾Æ´Ï¶ó¸é ¼±±¹ÇÒ ¼ö ¾ø´Ù.*/
                if(!(service->svcType & (eDxSVC_TYPE_RADIO|eDxSVC_TYPE_DATA)))
                {
                    return FALSE;
                }
                break;
                /*¼±±¹´ë»ó?Ì all?Ì¶ó¸é ¼¼ Å¸?Ô ¸ðµÎ ¼±±¹ °¡´É*/
            case eTuningRange_All:
                break;
                /*¼±±¹´ë»ó?Ì ±× ¿Ü¶ó¸é(ºÒ°¡´É) ¿?µ¿?Û?» ¸·±â?§ÇØ ¼±±¹?» ¸·´Â´Ù*/
            default:
                return FALSE;
        }
        flag = service->ex.japan.eUserFlag;
        /*ch mask, temp service, skip¼³Á¤, skipchupdown?Ì °í·ÁµÇ¾î¾ß ÇÔ.*/
        /*JCOMÇâ¿¡´Â skipÃ¤³Î?Ì ¾ø±â ¶§¹®¿¡ on/off¸¦ º¼ ÇÊ¿ä ¾ø?½.
        skipÃ¤³Î ?ÚÃ¼¸¦ ±¸ÇöÇÒ ÇÊ¿ä´Â ¾ø?¸³ª, jlabs spec?Ç invisible flag¶§¹®¿¡ µ¿?Û ±¸Çö?º ÇØ³õ?º »óÅÂ*/
...
void	OPL_Channel_RemoveChannelList (OPL_ChannelManager_t manager)
{
	...
	CFavouriteManager::getInstance()->reset();
...
int	OPL_Channel_GetNetworkIDList(OPL_NetworkType_e netType, OPL_ChannelManager_t manager, int ** ppNetworkList)
{
    ...
    *ppNetworkList = (int*)HLIB_STD_MemAlloc(sizeof(int)*n);
    if(*ppNetworkList == NULL)
    {
        HxLOG_Error("Out of memory!!\n");
        return 0;
...
OPL_ChannelID_e	OPL_Channel_GetIdType (OPL_Channel_t channel)
{
    ...
    case eDxDELIVERY_TYPE_SAT:	//
        ...
        if (ts && ts->tuningParam.sat.tuningInfo.eTransSpec == eDxSAT_TRANS_DVBS2)
        {
        ...
        ts = APK_META_SVC_GetTransponder(service->tsuid);
        if (ts && ts->tuningParam.ter.eSystem == eDxTER_DVB_T2)
        {
...
int		OPL_Channel_GetOrgNetworkID (OPL_Channel_t channel)
{
	...
	if(service == NULL)
	{
		HxLOG_Error("[%s:%d] service is NULL \n", __FUNCTION__, __HxLINE__);
		return 0;
	}
...
int		OPL_Channel_GetMajorChannel (OPL_Channel_t channel)
{
    ...
    HxLOG_Trace();
    DxService_t *service = (DxService_t*)channel;
	return CFavouriteManager::getInstance()->getLCN(service->uid);
#else
    DxService_t	*service = (DxService_t *)channel;
...
OPL_BOOL		OPL_Channel_HasTeletext (OPL_Channel_t channel)
{
	...
	// PID_NULL === 0x2000
    return (service->ttxPid != 0x2000) ? OPL_TRUE : OPL_FALSE;
}
...
void		OPL_Channel_SetNewChannelFlag(OPL_Channel_t channel, OPL_BOOL newChannel)
{
    ...
    if (flag)
        service->updateSvcFlag = (DxSvcUpdateFlag_e)HxMACRO_BITSET(service->updateSvcFlag, eDxSVC_UPDATE_NEWCHANNEL);
    else
        service->updateSvcFlag = (DxSvcUpdateFlag_e)HxMACRO_BITCLR(service->updateSvcFlag, eDxSVC_UPDATE_NEWCHANNEL);
...
#if defined( CONFIG_OP_JAPAN )
{
    /* jlabs ½ºÆå¿¡ ?ÇÇØ¼­ ?Ïº»Çâ?º provider name ?¸·Î.. */
...
const char *	OPL_Channel_GetLogoURL (OPL_Channel_t channel)
{
    ...
    if (APK_META_SVC_GetLogoUrl(service->uid, &stLogo) == NULL)
		return empty;

#if defined(CONFIG_SUPPORT_IPEPG_CH_LOGO_STORE_HDD)
	url = (HCHAR*)THAPI_GetThumbnailURL(stLogo.ucServerLogoUrl, 0, "/mnt/hd1", FALSE);
#else
	url = THAPI_GetThumbnailURL(stLogo.ucServerLogoUrl, 0, "/var/lib/humaxtv_user", FALSE);
#endif

	if (url == NULL)
		return empty;

	len = strlen(url) + 32/*file://+alpha*/;
	buf = opl_channel_GetTextBuffer(NULL, len);
	if (buf == NULL)
		return empty;
	HxSTD_PrintToStrN(buf, len, "file://%s", url);
...
#elif defined(CONFIG_OP_UK_DTT)
	if (strcmp(field, "guidancesdttext") == 0)
	{
		value = (HCHAR *)(service->ex.tva.szGuidanceStr);
	}
	else if (strcmp(field, "selectable") == 0)
	{
		value = service->ex.tva.ucUserSelectable ? "true" : "false";
	}
	else if (strcmp(field, "guidancesdttype") == 0)
	{
		snprintf(temp, 32, "%d", service->ex.tva.ucGuidanceType);
		value = temp;
	}
	else if (strcmp(field, "guidancesdtmode") == 0)
	{
		snprintf(temp, 32, "%d", service->ex.tva.ucGuidanceMode);
		value = temp;
	}
    ...
    switch(eNetType)
    {
        case eOPL_DTT_NETWORK:
            for(i = 0; i < length ; i++)
            {
                ...
                if(service != NULL)
                {
                    // TODO: ´ëÇ¥Ã¤³Î ±¸ºÐÇÏ´Â ¹æ¹ý?Ì ¸Â´ÂÁö È®?Î ÇÊ¿ä
...
int		OPL_Channel_GetOneTouch(OPL_Channel_t channel)
{
    ...
    // TODO: spec¿¡¼­ ?ÇµµÇÑ °ª?Ì ¸Â´ÂÁö È®?Î ÇÊ¿ä
...
{
    DxService_t *service  = (DxService_t*)hChannel;
    DxService_t *cmdService = (DxService_t*)hCmpChannel;
    RETURN_IF(service == NULL, OPL_FALSE);
    RETURN_IF(cmdService == NULL, OPL_FALSE);
#if defined(CONFIG_OP_JAPAN)
...
{
	CVideoResourceManager *manager = NULL;
	OplVideoBroadcastHandle *pvideoBroadcastHandle = NULL;
	OplBroadcastPlayState_e ePlayState = OPLBROADCAST_PLAY_STATE_UNREALIZED;
	if(pChannel == NULL)
	{
		HxLOG_Error("[%s] pChannel is NULL\n",__FUNCTION__);
		return OPL_FALSE;
	}
	manager  = CVideoResourceManager::getInstance();
	if(manager == NULL)
	{
		HxLOG_Error("[%s] CVideoResourceManager::getInstance returned NULL\n",__FUNCTION__);
		return OPL_FALSE;
	}
	pvideoBroadcastHandle = manager->getMainVideoBroadcastHandle();
	if(pvideoBroadcastHandle == NULL)
	{
		HxLOG_Error("[%s] getMainVideoBroadcastHandle() returned NULL\n",__FUNCTION__);
		return OPL_FALSE;
	}

	if(!pvideoBroadcastHandle->getPlayState(&ePlayState))
	{
		HxLOG_Error("[%s] pvideoBroadcastHandle->getPlayState returned false\n",__FUNCTION__);
		return OPL_FALSE;
	}

	if(ePlayState != OPLBROADCAST_PLAY_STATE_PRESENTING)
	{
		HxLOG_Error("[%s] ePlayState is %d\n",__FUNCTION__,ePlayState);
		return OPL_FALSE;
	}
	*pChannel = pvideoBroadcastHandle->getCurrentChannel();
	return OPL_TRUE;
}
...
#if  defined(CONFIG_OP_PVR_BASED_ON_TVANYTIME)
{
	...
	pDAD = service->ex.tva.szDefaultAuth;

	if(pDAD && HxSTD_StrLen(pDAD) > 0)
	{
		HxSTD_memcpy(pucDefautAuth, pDAD, DxDEFAULT_AUTHORITY_LEN);
		return ;
	}
	else
	{
		DxTransponder_t *ts = NULL;

		ts = APK_META_SVC_GetTransponder(service->tsuid);
		if(ts != NULL)
		{
			pDAD = ts->ex.tva.defaultAuth;
			...
			{
				DxNetwork_t *network = NULL;
				network = APK_META_SVC_GetNetwork(ts->netuid);
				if(network!= NULL)
				{
					pDAD = network->ex.tva.defaultAuth;
...
#if 0
int OplChannelManager::getChannelsCount(void)
{
	int nCount = 0;

	if ( OPL_ChannelManager_GetChannelsCount(m_channelManager, &nCount) != BPL_STATUS_OK )
	{
		return 0;
	}
...
OplChannel *OplChannelManager::getChannelByIndex(int nIndex)
{
	OplChannel_t *ptChannel = OPL_ChannelManager_GetChannelByIndex(m_channelManager, nIndex);
	if ( !ptChannel )
	{
		return NULL;
	}

	OplChannel *channelObj = new OplChannel();
	if ( !channelObj )
	{
		return NULL;
	}

	HxSTD_MemCopy(channelObj, ptChannel, sizeof(*channelObj));

	OPL_ChannelManager_FreeChannel(m_channelManager, ptChannel);
	ptChannel = NULL;

	return channelObj;
}

OplChannel *OplChannelManager::getChannelByCCID (const char *ccid)
{
	OplChannel_t *ptChannel = OPL_ChannelManager_GetChannelByCCID(m_channelManager, ccid);
	if ( !ptChannel )
	{
		return NULL;
	}
