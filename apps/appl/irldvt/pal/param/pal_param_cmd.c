...
STATIC int pal_param_SetCmd(void *pvArgList)
{
	...
	if (bList)
	{
		...
		nResult = pal_param_CmdSetValueList(ulStart, ulEnd, pucData, pszPrintBuf);
	}
	else
	{
		HCHAR				szPreFix[palParam_CMD_STR_BUFF_SIZE];
		...
		else
		{
			...
			if (pszArg)
			{
				nResult = pal_param_CmdSetValueFromArg(eItem, pucData, pszArg, pszPrintBuf);
				HLIB_CMD_Printf("%s : %s\n", szPreFix, pszPrintBuf);
			}
			else
			{
