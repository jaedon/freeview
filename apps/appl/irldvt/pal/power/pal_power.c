...
HERROR PAL_PWR_TurnOffMainBoard(HBOOL bPowerSaveMode)
{
	...
	{
		HxLOG_Error("PAL_PWR_TurnOffMainBoard(DI_POWER_OFFMODE_POWERSAVING_READY)\n");
	...
	if(bPowerSaveMode == TRUE)
	{
		HxLOG_Error("PAL_PWR_TurnOffMainBoard(DI_POWER_OFFMODE_STANDBY_POWERSAVING)\n");
		return DI_POWER_SetPowerOff(DI_POWER_OFFMODE_STANDBY_POWERSAVING);
	}
	else
	{
		HxLOG_Error("PAL_PWR_TurnOffMainBoard(DI_POWER_OFFMODE_STANDBY_NORMAL)\n");
...
HERROR PAL_PWR_SetWakeupRcuKey(HUINT32 keyGroup)
{
	...
	while(keyGroupIndex != eWAKEUP_RCUKEY_MAX)
	{
		...
		{
			printf("DI_KEY_SetWakeupRcuKey index[0x%x]\n", keyGroupIndex);
...
HERROR PAL_PWR_SetFastBootStandby(void)
{
#if defined(CONFIG_SUPPORT_S3MODE_FASTBOOT)
	...
	if(DI_POWER_SetStandby(DI_PM_POWERSTATE_DEEP_SLEEP, stSrc/*DI_PM_WAKEUP_SRC src*/ ) == DI_ERR_OK)
	{
		return ERR_OK;
	}
#endif
