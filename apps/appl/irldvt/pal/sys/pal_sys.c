...
STATIC HERROR pal_sys_MakeVerText (HUINT8 *szHeadStr, HUINT32 ulVerNum, HUINT8 *szText, HUINT32 ulVerByteNum)
{
...
HERROR PAL_SYS_MakeSystemVerString (VersionType_t eSystemType, HUINT32 ulSystemVer, HUINT8 *szSysVerStr)
{
	...
	{
	case eVERSION_TYPE_NEXTOTA:
		pal_sys_MakeVerText (NULL, ulSystemVer, szSysVerStr, 4);
		return ERR_OK;

	case eVERSION_TYPE_LOADER:
#if defined(CONFIG_MW_CAS_IRDETO)
		pal_sys_MakeVerText (palSys_LOADER_IRDTO_VER_HEAD, ulSystemVer, szSysVerStr, 3);
#else
	{
		...
		pal_sys_MakeVerText (ucVersionHeader, ulSystemVer, szSysVerStr, 2);		// MODEL~ X.XX
	}
#endif
		return ERR_OK;
	...
	case eVERSION_TYPE_APPL:
		pal_sys_MakeVerText (SYSTEM_APP_VER_HEAD, ulSystemVer, szSysVerStr, 4);
		return ERR_OK;
...
HERROR PAL_SYS_GetMicomVerString(HUINT8 *szVerStr)
{
	...
	pal_sys_MakeVerText (ucVersionHeader, ulSystemVer, szVerStr, 4);
...
HERROR PAL_SYS_SetResolution( HUINT32 ulDiResolution )
{
	DI_ERR_CODE		 hErr=0;
	DI_NVRAM_VIDEO_RESOLUTION_E	 eDiResol=ulDiResolution;


	hErr  = DI_NVRAM_SetField (DI_NVRAM_FIELD_HD_RESOULTION, 0, (void *)&eDiResol, sizeof(DI_NVRAM_VIDEO_RESOLUTION_E));
