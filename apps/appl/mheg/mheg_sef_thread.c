...
typedef	struct
{
	...
	MhegSef_TableCompleteMode_e	eSiTableCompleteMode;	/**< table 단위의 수집 모드와 section 단위의 수집 모드 두 가지가 있다 */
	...
	MhegSef_TimeOutMode_e		eTimeoutMode;			/**< timeout 처리 모드로 once와 repeat이 있다. continue mode에서 활용됨 */

	MhegSef_Callback_t			pfnFreeSectionFunc;		/**< free callback, filter stop시 불려진다. */
	...
	MhegSef_FilterLength_e		eFilterLen;				/**< section filter length로 8, 16 바이트를 지원한다. 현재 드라이버가 8, 16바이트 지원함 */
	...
	MhegSef_RawData_t			stRawData;				/**< section을 저장할 구조체 */
...
STATIC HERROR mheg_sef_ThreadProcessOneSection(mhegSef_FilterTable_t *pstFilterTable, HUINT8 *pucRawSec)
{
	...
	if(mheg_sef_ThreadMakeTable(pstFilterTable, pucRawSec) == mhegSef_COMPLETE_COLLECT_SECTION)		//	테이블 다 모았다.
	{
		bForcedParse = TRUE;
