...
void
ActionClass_execute(ActionClass *a, ExternalReference *ref) // hmkim : org is ActionClass_execute(ActionClass *a, OctetString *caller_gid)
{
	...
	while(list)
	{
		...
		{
			verbose_class("ActionClass: %s, ElementaryAction (%s) calling...", ExternalReference_name(ref), ElementaryAction_name(&list->item));
			ElementaryAction_execute(&list->item, ref); // hmkim : org is ElementaryAction_execute(&list->item, caller_gid);
		}
		list = list->next;
	}
