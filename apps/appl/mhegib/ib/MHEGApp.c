...
MHEGApp_loadApplication(MHEGApp *m, OctetString *derfile)
{
	...
	/* assert */
	if(MHEGEngine_isNotAbsoluteFilename(derfile))
	{
	...
	VK_memset(m->app, 0x00, sizeof(InterchangedObject));
	...
	len = ftell(der);
	rewind(der);
	verbose("MHEGApp_loadApplication: '%.*s' %d bytes", derfile->size, derfile->data, len);
...
MHEGApp_loadScene(MHEGApp *m, OctetString *derfile)
{
	...
	VK_memset(m->scene, 0x00, sizeof(InterchangedObject));
	...
	len = ftell(der);
	rewind(der);
	verbose("MHEGApp_loadScene: '%.*s' %d bytes", derfile->size, derfile->data, len);
