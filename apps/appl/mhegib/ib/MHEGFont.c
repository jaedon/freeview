...
{
	VK_memset(f, 0x00, sizeof(MHEGFont));

	return;
}

/*
 * only font allowed by UK MHEG Profile is MHEG_REC_FONT_UK1
...
{
	OctetString UK_default_font = { 14, (unsigned char *) MHEG_REC_FONT_UK1 }; /* UK MHEG Profile */
	OctetString CI_default_font1 = { 14, (unsigned char *) MHEG_REC_FONT_CI1 }; /* CI+ MHEG Profile */
	OctetString CI_default_font2 = { 14, (unsigned char *) MHEG_REC_FONT_ci1 }; /* CI+ MHEG Profile */
	OctetString *str;
	...
	if(OctetString_cmp(str, &UK_default_font) != 0 && OctetString_cmp(str, &CI_default_font1) != 0 && OctetString_cmp(str, &CI_default_font2) != 0)
	{
	...
	/* default is the only font */ // TODO : 어쨌거나 무조건 MHEGFont_defaultName() 한다? 변경 필요...
	MHEGFont_defaultName(font);
...
/*
 * default font is MHEG_REC_FONT_UK1
...
static char *_default_font_name = "HumaxFont"; // static char *_default_font_name = NULL;
...
MHEGFont_defaultName(MHEGFont *font)
{
	...
	if(_default_font_name == NULL)
	{
		...
		if(i > 0)
			error("Font '%s' not available; using '%s' for '%s'", try_font[0], fontname, MHEG_REC_FONT_UK1);
	}
...
MHEGFont_setAttributes(MHEGFont *font, OctetString *str)
{
	...
	else
	{
		...
		if(get_font_attr(&p, &left, out, sizeof(out)))
			...
			font->size = MHEGFontDefaultSize;
		/* line spacing */
		if(get_font_attr(&p, &left, out, sizeof(out)))
			font->line_spc = atoi(out);
		else
			font->line_spc = MHEGFontDefaultLineSpace;
		/* letter spacing */
		if(get_font_attr(&p, &left, out, sizeof(out)))
			font->letter_spc = atoi(out);
		else
			font->letter_spc = MHEGFontDefaultLetterSpace;
	}
...
//	int num_lines;
	...
	case Justification_centre:
		if(elem_list)
		{
			...
			{
				fontSize = MHEGFontDefaultSize;
			}
			MHEGIB_GetFontHeight(fontSize, &fontHeight);

			/* tail element is the lowest */
			ymax = elem_list->prev->item.y + yOffsetTop + yOffsetBottom;

			if (((int) box->y_length - ymax) < 2)
			{
				fontHeight = 0;
			}
			...
			for(elem = elem_list; elem; elem = elem->next)
			{
				elem->item.y += yOffsetTop + (((int) box->y_length - ymax - fontHeight) / 2);
			}
		}
		break;
...
{								\
	...
	VK_memcpy(&colour_stack[0], col, sizeof(MHEGColour));	\
	...
	while(size > 0)
	{
		...
		else	/* (possibly multibyte) character */
		{
			...
			if(new_elem)
			{
				...
				/* current colour */
				VK_memcpy(&elem->item.col, current_colour, sizeof(MHEGColour));
			...
			if(wrap && size > len && is_breakable(tok))
			{
				...
				do
				{
					...
					elem->item.width = xpos + ext->width;
			...
			else
			{
				...
				if(wrap
				&& break_start != NULL
				&& xpos + ext->width > available_width)
				{
					/*
...
char_extents(MHEGFont *f, int xpos, Justification hori, int previous, int measure)
{
	...
	MHEGIB_GetCharWidth(f->size, measure, &lCharWidth);
	_ext.xOff = _ext.width = lCharWidth;
