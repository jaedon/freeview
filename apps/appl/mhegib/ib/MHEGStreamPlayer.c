...
static LIST_OF(VideoFrame) *free_vframes = NULL;
static pthread_mutex_t free_vframes_lock = PTHREAD_MUTEX_INITIALIZER;

static LIST_TYPE(VideoFrame) *
...
static LIST_OF(AudioFrame) *free_aframes = NULL;
static pthread_mutex_t free_aframes_lock = PTHREAD_MUTEX_INITIALIZER;

static LIST_TYPE(AudioFrame) *
...
MHEGStreamPlayer_init(MHEGStreamPlayer **p)
{
	...
	if(!created)
	{
		VK_memset(&player, 0x00, sizeof(MHEGStreamPlayer));
	...
	verbose_av_stream("MHEGStreamPlayer_init: %u users", player.nusers);
...
//	StreamClass_Deactivation() 시에 중복 처리되므로 여기서는 삭제
//	MHEGStreamPlayer_stop(&player);
	...
	verbose_av_stream("MHEGStreamPlayer_fini: %u users", player.nusers);
...
MHEGStreamPlayer_setServiceID(MHEGStreamPlayer *p, int id)
{
	...
	if(p == NULL)
	{
		fatal("MHEGStreamPlayer_setServiceID: p=NULL");
		return;
	}
	...
	verbose_av_stream("MHEGStreamPlayer_setServiceID: %d", id);
...
MHEGStreamPlayer_setVideoStream(MHEGStreamPlayer *p, VideoClass *video)
{
	...
	if(p == NULL)
	{
		fatal("MHEGStreamPlayer_setVideoStream: p=NULL");
		return;
	}
	...
	if(video)
		verbose_av_stream("MHEGStreamPlayer_setVideoStream: tag=%d", video->component_tag);
	else
		verbose_av_stream("MHEGStreamPlayer_setVideoStream: NULL");
...
MHEGStreamPlayer_setAudioStream(MHEGStreamPlayer *p, AudioClass *audio)
{
	...
	if(p == NULL)
	{
		fatal("MHEGStreamPlayer_setAudioStream: p=NULL");
		return;
	}
	...
	if(audio)
		verbose_av_stream("MHEGStreamPlayer_setAudioStream: tag=%d", audio->component_tag);
	else
		verbose_av_stream("MHEGStreamPlayer_setAudioStream: NULL");
...
MHEGStreamPlayer_play(MHEGStreamPlayer *p)
{
	...
	if(p == NULL)
	{
		fatal("MHEGStreamPlayer_play: p=NULL");
		return;
	}
	...
	verbose_av_stream("MHEGStreamPlayer_play: service_id=%d audio_tag=%d%s video_tag=%d%s", p->service_id, p->audio_tag, p->have_audio == true ? "(have)" : "", p->video_tag, p->have_video == true ? "(have)" : "");

	if(p->playing)
	{
		verbose_av_stream("MHEGStreamPlayer_play: already playing");
		return;
	}
	...
	{
		verbose_av_stream("MHEGStreamPlayer_play: A/V disabled");
		return;
	}
	if(!p->have_video && !p->have_audio)
	{
		verbose_av_stream("MHEGStreamPlayer_play: A/V not available");
		return;
	}
...
MHEGStreamPlayer_stop(MHEGStreamPlayer *p)
{
	...
	if(p == NULL)
	{
		fatal("MHEGStreamPlayer_stop: p=NULL");
		return;
	}
	...
	verbose_av_stream("MHEGStreamPlayer_stop");

	/* are we playing */
	if(!p->playing)
	{
		verbose_av_stream("MHEGStreamPlayer_stop: not playing");
		return;
	}
...
decode_thread(void *arg)
{
	...
	verbose_av_stream("MHEGStreamPlayer: decode_thread() started");
	...
	if(p->have_video && p->video_pid != -1)
	{
		...
		verbose_av_stream("MHEGStreamPlayer: Video: stream type=%d codec=%s", p->video_type, codec->name);
	}
	...
	if(p->have_audio && p->audio_pid != -1)
	{
		...
		verbose_av_stream("MHEGStreamPlayer: Audio: stream type=%d codec=%s", p->audio_type, codec->name);
	...
	while(!p->stop && !feof(p->ts->ts))
	{
		...
		{
			verbose_av_stream("MHEGStreamPlayer: decoder got unexpected/untimed packet");
		}
		av_free_packet(&pkt);
	}
	...
	verbose_av_stream("MHEGStreamPlayer: decode_thread() stopped");
...
video_thread(void *arg)
{
	...
	verbose_av_stream("MHEGStreamPlayer: video_thread() started");
	...
	do
	{
		...
		if(buffered != last_buffered)
			verbose_av_stream("MHEGStreamPlayer: buffered %f seconds of video", buffered);
	...
	if(p->stop)
	{
		...
		verbose_av_stream("MHEGStreamPlayer: video thread stopped before any output");
		return NULL;
	}
	...
	while(!p->stop)
	{
		...
		if(vf == NULL)
		{
			verbose_av_stream("MHEGStreamPlayer: videoq is empty");
		...
		if(drop_frame)
		{
			verbose_av_stream("MHEGStreamPlayer: dropped video frame %u (usecs=%d)", nframes, usecs);
		}
		else
		{
...
//			XFlush(d->dpy);
	...
	verbose_av_stream("MHEGStreamPlayer: video_thread() stopped");
...
audio_thread(void *arg)
{
	...
	verbose_av_stream("MHEGStreamPlayer: audio_thread() started");
	...
	if(p->have_video)
	{
		...
		while(!done)
		{
			...
			if(p->stop)
			{
				verbose_av_stream("MHEGStreamPlayer: audio thread stopped before any output");
				return NULL;
			}
	...
	else
	{
		...
		do
		{
			...
			if(p->audioq != NULL)
				...
				buffered = 0.0;
			pthread_mutex_unlock(&p->audioq_lock);
			verbose_av_stream("MHEGStreamPlayer: buffered %f seconds of audio", buffered);
			/* let the decoder have a go */
	...
	verbose_av_stream("MHEGStreamPlayer: audio params: format=%d rate=%d channels=%d", format, rate, channels);
...
//			verbose_av_stream("MHEGStreamPlayer: audioq is empty");
			/* give the decoder a bit of time to catch up */
			pthread_yield();
			continue;
		}
...
/* if we missed decoding a sample, play silence */
		...
		{
			verbose_av_stream("MHEGStreamPlayer: dropped audio frame (%f ahead)", vpts - af->pts);
		}
/* TODO handle case when video is ahead */
	...
	verbose_av_stream("MHEGStreamPlayer: audio_thread() stopped");
