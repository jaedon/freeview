...
{
	VK_memset(v, 0x00, sizeof(SceneClassInstanceVars));
...
#if (CLS_VERBOSE)
	verbose_class("SceneClass: %s; Preparation", ExternalReference_name(&s->rootClass.inst.ref));
#endif
...
SceneClass_Activation(SceneClass *s)
{
...
#if (CLS_VERBOSE)
	verbose_class("SceneClass: %s; Activation", ExternalReference_name(&s->rootClass.inst.ref));
#endif
	...
	if(!s->rootClass.inst.AvailabilityStatus)
	{
		...
		#if 0 // hmkim : commented out (위의 SceneClass_Preparation() 내부에서 처리된다).
	...
	{
		verbose_todo("%s; %s; aspect_ratio=%d:%d", __FUNCTION__, ExternalReference_name(&s->rootClass.inst.ref), s->aspect_ratio.width, s->aspect_ratio.height);
	}

	/* do Activation of the GroupClass */
	if(s->have_on_start_up)
		ActionClass_execute(&s->on_start_up, &s->rootClass.inst.ref); // mintes : org is ActionClass_execute(&s->on_start_up, &s->rootClass.inst.ref.group_identifier);
	...
	RootClass_Activation(&s->rootClass);
#endif
...
SceneClass_Deactivation(SceneClass *s)
{
...
#if (CLS_VERBOSE)
	verbose_class("SceneClass: %s; Deactivation", ExternalReference_name(&s->rootClass.inst.ref));
#endif
	...
	if(s->have_on_close_down)
		ActionClass_execute(&s->on_close_down, &s->rootClass.inst.ref); // hmkim : org is ActionClass_execute(&s->on_close_down, &s->rootClass.inst.ref.group_identifier);
...
SceneClass_Destruction(SceneClass *s)
{
...
#if (CLS_VERBOSE)
	verbose_class("SceneClass: %s; Destruction", ExternalReference_name(&s->rootClass.inst.ref));
#endif
	...
	RootClass_Destruction(&s->rootClass);
#endif
...
{
//	verbose_class("SceneClass: %s; SetCachePriority (ignored)", ExternalReference_name(&t->rootClass.inst.ref));

#if 1 // hmkim : added.
{
	int NewCachePriority = GenericInteger_getInteger(&params->new_cache_priority, caller_gid);

	verbose_class("SceneClass: %s; SetCachePriority; %d", ExternalReference_name(&t->rootClass.inst.ref), NewCachePriority);

	/* ISO/IEC 13522-5 : NewCachePriority shall be set within the range [0, 255] */
	if (NewCachePriority < 0 || NewCachePriority > 255)
	{
		error("SceneClass: %s; SetCachePriority: NewCachePriority %d is set within the range [0, 255]", ExternalReference_name(&t->rootClass.inst.ref), NewCachePriority);
		return;
	}
...
{
	verbose_class("SceneClass: %s; SetTimer", ExternalReference_name(&t->rootClass.inst.ref));
...
SceneClass_SendEvent(SceneClass *t, SendEvent *params, OctetString *caller_gid)
{
	...
	verbose_class("SceneClass: %s; SendEvent", ExternalReference_name(&t->rootClass.inst.ref));
...
{
	error("SceneClass: %s; SetCursorShape not supported in UK / CI+ MHEG Profile", ExternalReference_name(&t->rootClass.inst.ref));
...
{
	error("SceneClass: %s; SetCursorPosition not supported in UK / CI+ MHEG Profile", ExternalReference_name(&t->rootClass.inst.ref));
...
{
	error("SceneClass: %s; GetCursorPosition not supported in UK / CI+ MHEG Profile", ExternalReference_name(&t->rootClass.inst.ref));
...
#if 1 // hmkim : added.
{
	int NewInputRegister = GenericInteger_getInteger(&params->new_input_register, caller_gid);

	verbose_class("SceneClass: %s; SetInputRegister; %d", ExternalReference_name(&t->rootClass.inst.ref), NewInputRegister);

	if (t->input_event_register != NewInputRegister)
	{
		t->input_event_register = NewInputRegister;
	}
}
#endif
...
void
SceneClass_SetInputMask(SceneClass *t, SetInputMask *params, OctetString *caller_gid)
{
	verbose_todo("%s not yet implemented", __FUNCTION__);

#if 0// ssgoo : added.
{
	OctetString *oct = GenericOctetString_getOctetString(&params->new_input_mask, caller_gid);

	verbose_class("SceneClass: %s", ExternalReference_name(&t->rootClass.inst.ref), oct);
	verbose_class("Size[%s], SetInputMask [0x%02X][0x%02X][0x%02X]", oct->size, oct->data[0, oct->data[1], oct->data[2]]);
