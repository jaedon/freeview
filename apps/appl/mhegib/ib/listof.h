...
/*
 * move the given LIST_TYPE from the list 1 to list 2 (hmkim)
 */
#define LIST_MOVE(PHEAD1, PHEAD2, ITEM)				\
do								\
{								\
	/* is ITEM the head and the tail */ 		\
	if(*(PHEAD1) == ITEM && (ITEM)->next == NULL)		\
	{							\
		*(PHEAD1) = NULL;				\
	}							\
	/* is ITEM just the head */ 			\
	...
	{							\
		/* make head->prev point to the new tail */ \
		(*(PHEAD1))->prev = (ITEM)->prev;		\
		/* unlink ITEM from the new tail */ 	\
		(*(PHEAD1))->prev->next = NULL; 		\
	}							\
	else							\
	{							\
		/* unlink ITEM from the previous one */ 	\
	...
	{						\
		/* add it to the tail */		\
		(ITEM)->prev = (*(PHEAD2))->prev;	\
		(ITEM)->next = NULL;			\
		(*(PHEAD2))->prev->next = ITEM; 	\
		(*(PHEAD2))->prev = ITEM;		\
	}						\
}								\
while(0)
