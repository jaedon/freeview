...
mpegts_demux_frame(MpegTSContext *ctx, AVPacket *frame)
{
	...
	do
	{
		...
		if(ctx->is_start == 0)
		{
			...
			VK_memcpy(pes->frame_data + pes->frame_size, packet.data, packet.size);
	...
	VK_memcpy(frame->data, pes->frame_data, pes->frame_size);
	...
	pes->frame_data = frame_data;
	pes->frame_size = packet.size;
	VK_memcpy(pes->frame_data, packet.data, packet.size);
...
read_packet(FILE *ts, uint8_t *buf)
{
	...
	*buf = 0;
	...
	if(resync > 0)
		verbose_av_stream("MPEG TS demux: lost sync; skipped %u bytes", resync);
...
handle_packet(MpegTSContext *ctx, const uint8_t *packet)
{
	...
	{
		verbose_av_stream("MPEG TS demux: ignoring unexpected PID %d", pid);
		return;
	}
...
{
	VK_memset(pes, 0x00, sizeof(PESContext));
...
mpegts_push_data(PESContext *pes, const uint8_t *buf, int buf_size, int is_start)
{
	...
	while(buf_size > 0)
	{
		...
		case MPEGTS_HEADER:
			...
			if(len > buf_size)
				len = buf_size;
			VK_memcpy(pes->header + pes->data_index, p, len);
			pes->data_index += len;
			p += len;
			buf_size -= len;
