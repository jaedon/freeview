...
readpng_get_image(unsigned char *png_data, unsigned int png_size, png_uint_32 *width, png_uint_32 *height)
{
	...
	if(setjmp(png_jmpbuf(png_ptr)))
	{
		...
		if(row_pointers != NULL)
			HLIB_STD_MemFree(row_pointers);
		if(rgba != NULL)
			HLIB_STD_MemFree(rgba);
		return NULL;
	}
	...
	if((rgba = (unsigned char *) HLIB_STD_MemAlloc(rowbytes * (*height))) == NULL)
	{
		png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
		return NULL;
	}

	if((row_pointers = (png_bytepp) HLIB_STD_MemAlloc((*height) * sizeof(png_bytep))) == NULL)
	{
		png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
		HLIB_STD_MemFree(rgba);
		return NULL;
...
read_mem(png_structp png_ptr, png_bytep data, png_size_t length)
{
	...
	VK_memcpy(data, read_mem_state.data + read_mem_state.used, length);
