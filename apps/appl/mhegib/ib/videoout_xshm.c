...
static void *vo_xshm_init(void);
static void vo_xshm_fini(void *);
static void vo_xshm_prepareFrame(void *, VideoFrame *, unsigned int, unsigned int);
static void vo_xshm_drawFrame(void *, int, int);
...
vo_xshm_prepareFrame(void *ctx, VideoFrame *f, unsigned int out_width, unsigned int out_height)
{
	...
	|| v->resize_out.width != out_width || v->resize_out.height != out_height)
	{
		...
		if((v->sws_ctx = sws_getContext(f->width, f->height, f->pix_fmt,
			...
			fatal("Out of memory");
		verbose_av_stream("videoout_xshm: input=%d,%d output=%d,%d", f->width, f->height, out_width, out_height);
...
#if 0
	if((v->current_frame = (XImage *)XShmCreateImage(d->dpy, d->vis, d->depth, ZPixmap, NULL, &v->shm, out_width, out_height)) == NULL)
