...
static	PageSegment_t	*nx_prism_subtitle_find_page_segment(DVBData_t *dvbInfo, HUINT16 page_id)
{
	HxList_t	*list;
...
static	RegionSegment_t	*nx_prism_subtitle_find_region_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT16 region_id)
{
	HxList_t		*list;
...
static	CLUTSegment_t	*nx_prism_subtitle_find_clut_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT16 clut_id)
{
	HxList_t		*list;
...
static	ObjectSegment_t	*nx_prism_subtitle_find_object_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT32 object_id)
{
	HxList_t		*list;
...
static	void	nx_prism_subtitle_delete_page_segment(DVBData_t *dvbInfo, HUINT16 page_id)
{
	HxList_t	*list = NULL;
...
static	void	nx_prism_subtitle_delete_region_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT32 region_id)
{
	RegionSegment_t	*segment = NULL;

	while ( (segment = nx_prism_subtitle_find_region_segment(dvbInfo, page_id, region_id)) )
	{
		HxLOG_Debug("free : pObjectList [%X]\n", segment->pObjectList);
		if (segment->pObjectList)
		{
			NX_APP_Free(segment->pObjectList);
		...
		{
			NX_APP_Free(segment->color_table);
...
static	void	nx_prism_subtitle_delete_clut_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT16 clut_id)
{
	CLUTSegment_t	*segment = NULL;

	while ( (segment = nx_prism_subtitle_find_clut_segment(dvbInfo, page_id, clut_id)) )
	{
		HxLOG_Debug("free : CLUT [%X]\n", segment->pCLUT);
		if (segment->pCLUT)
		{
			NX_APP_Free(segment->pCLUT);
			segment->pCLUT = NULL;
		}
...
static	void	nx_prism_subtitle_delete_object_segment(DVBData_t *dvbInfo, HUINT16 page_id, HUINT32 object_id)
{
	ObjectSegment_t	*segment = NULL;

	while ( (segment = nx_prism_subtitle_find_object_segment(dvbInfo, page_id, object_id)) )
	{
		HxLOG_Debug("free : pObjectData [%X]\n", segment->pObjectData);
		if (segment->pObjectData)
		{
			NX_APP_Free(segment->pObjectData);
...
static	void	nx_prism_subtitle_delete_all_segment_by_pageid(DVBData_t *dvbInfo, HUINT16 page_id)
{
	HxLOG_Print("(+)nx_prism_subtitle_delete_all_segment_by_page_id (%d)\n", page_id);
	nx_prism_subtitle_delete_page_segment(dvbInfo, page_id);
	nx_prism_subtitle_delete_region_segment(dvbInfo, page_id, 0xFFFF);
	nx_prism_subtitle_delete_clut_segment(dvbInfo, page_id, 0xFFFF);
	nx_prism_subtitle_delete_object_segment(dvbInfo, page_id, 0xFFFFFFFF);
	HxLOG_Print("(-)nx_prism_subtitle_delete_all_segment_by_page_id\n");
}

static	void	nx_prism_subtitle_delete_all_segment(DVBData_t *dvbInfo)
{
	HxList_t		*page_list = NULL;
	PageSegment_t	*page_segment = NULL;

	HxLOG_Print("(+)nx_prism_subtitle_delete_all_segment\n");
	while ((page_list = dvbInfo->page_segment))
	{
		page_segment = HLIB_LIST_Data(page_list);
		nx_prism_subtitle_delete_all_segment_by_pageid(dvbInfo, page_segment->u16PageId);
	}
	HxLOG_Print("(-)nx_prism_subtitle_delete_all_segment\n");
}

static	void	nx_prism_subtitle_parse_page_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
	HINT32			cnt = 0;
	HUINT16			page_id = 0;
	...
	{
		nx_prism_subtitle_delete_all_segment_by_pageid(dvbInfo, page_id);
	}

	page_segment = nx_prism_subtitle_find_page_segment(dvbInfo, page_id);
	...
	page_segment = (PageSegment_t*)NX_APP_Calloc(sizeof(PageSegment_t));
	if (page_segment == NULL)
	{
	...
	{
		page_segment->pRgList = (RegionList_t*)NX_APP_Calloc(page_segment->ulRegionNum * sizeof(RegionList_t));
		if (page_segment->pRgList == NULL)
		{
			page_segment->ulRegionNum = 0;
			NX_APP_Free(page_segment);
			page_segment = NULL;

			HxLOG_Error("page_segment->pRgList NX_APP_Calloc error..\n");
			return;
		}
...
static	void	nx_prism_subtitle_parse_region_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
	...
	region_segment = nx_prism_subtitle_find_region_segment(dvbInfo, page_id, region_id);
...
//		else
//			nx_prism_subtitle_delete_region_segment(dvbInfo, page_id, region_id);
		reuse_segment = TRUE;
	} else
	{
		region_segment = (RegionSegment_t*)NX_APP_Calloc(sizeof(RegionSegment_t));
		if (region_segment == NULL)
		{
...
//	region_segment = (RegionSegment_t*)NX_APP_Calloc(sizeof(RegionSegment_t));
	...
	{
		NX_APP_Free(region_segment->pObjectList);
	}
	region_segment->pObjectList = NULL;
	...
	if (region_segment->u1RegionFillFlag)
	{
		...
		{
			NX_APP_Free(region_segment->color_table);
		}
	...
	if ( region_segment->ulObjectNum == 0 )
	{
		...
		NX_APP_Free(region_segment);
		return;
	}

	region_segment->pObjectList = (ObjectList_t *)NX_APP_Calloc(region_segment->ulObjectNum * sizeof(ObjectList_t));
...
static	void	nx_prism_subtitle_parse_clut_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
	HUINT32		segment_len, idx, i;
	HUINT16		page_id;
	HUINT8		clut_id;
	...
	clut_segment = nx_prism_subtitle_find_clut_segment(dvbInfo, page_id, clut_id);
	if (clut_segment)
	{
		if (clut_segment->u4ClutVersion == ((HxMACRO_Get8Bit(p + 7) & 0xF0) >> 4))
		{
			...
			nx_prism_subtitle_delete_clut_segment(dvbInfo, page_id, clut_id);
	}

	clut_segment = (CLUTSegment_t*)NX_APP_Calloc(sizeof(CLUTSegment_t));
	...
	if (clut_segment->ulClutNum == 0 )
	{
		...
		NX_APP_Free(clut_segment);
		return;
	}

	HxLOG_Debug("\tCLUT Number of Entry : (%d)\n", (int)clut_segment->ulClutNum);

	clut_segment->pCLUT = (CLUTentry_t *)NX_APP_Calloc(clut_segment->ulClutNum * sizeof(CLUTentry_t));
...
static	void	nx_prism_subtitle_parse_object_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
	HUINT32		segment_len;
	HUINT16		page_id;
	HUINT16		object_id;
	...
	object_segment = nx_prism_subtitle_find_object_segment(dvbInfo, page_id, object_id);
	if (object_segment)
	{
		if (object_segment->u4ObjectVersion == ((HxMACRO_Get8Bit(p + 8) & 0xF0) >> 4))
		{
			...
			nx_prism_subtitle_delete_object_segment(dvbInfo, page_id, object_id);
	}

	object_segment = (ObjectSegment_t *)NX_APP_Calloc(sizeof(ObjectSegment_t));
	...
	{
	    NX_APP_Free(object_segment);
		object_segment = NULL;
		return;
	}

	object_segment->pObjectData = (HUINT8 *)NX_APP_Calloc(object_segment->ulObjectSize);
...
static	void	nx_prism_subtitle_parse_display_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
//	HUINT32		segment_len;
	HUINT16		page_id;
...
//	segment_len = HxMACRO_Get16Bit(p + 4) + 6;
	...
	{
		display_segment = (DisplaySegment_t*)NX_APP_Calloc(sizeof(DisplaySegment_t));
		if (display_segment == NULL)
		{
...
static	void	nx_prism_subtitle_parse_end_segment(DVBData_t *dvbInfo, HUINT8 *p)
{
//	HUINT16		page_id;
				...
				if ((page_id == pstDVBInfo->usCompPageId) || (page_id == pstDVBInfo->usAncPageId))
				{
					switch (*(p + 1))
					{
						case	DVB_SBTL_SEG_PCS:			//	0x10
							...
							nx_prism_subtitle_parse_page_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_PCS\n");
							break;
						case	DVB_SBTL_SEG_RCS:			//	0x11
							HxLOG_Print("(+)DVB_SBTL_SEG_RCS\n");
							nx_prism_subtitle_parse_region_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_RCS\n");
							break;
						case	DVB_SBTL_SEG_CDS:			//	0x12
							HxLOG_Print("(+)DVB_SBTL_SEG_CDS\n");
							nx_prism_subtitle_parse_clut_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_CDS\n");
							break;
						case	DVB_SBTL_SEG_ODS:			//	0x13
							HxLOG_Print("(+)DVB_SBTL_SEG_ODS\n");
							nx_prism_subtitle_parse_object_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_ODS\n");
							break;
						case	DVB_SBTL_SEG_DDS:			//	0x14
							HxLOG_Print("(+)DVB_SBTL_SEG_DDS\n");
							nx_prism_subtitle_parse_display_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_DDS\n");
							break;
						case	DVB_SBTL_SEG_EDS:			//	0x80
							HxLOG_Print("(+)DVB_SBTL_SEG_EDS\n");
							nx_prism_subtitle_parse_end_segment(pstDVBInfo, p);
							HxLOG_Print("(-)DVB_SBTL_SEG_EDS\n");
							break;
						case	DVB_SBTL_STUFFING:			//	0xFF
							HxLOG_Print("(+)DVB_SBTL_STUFFING\n");
							nx_prism_subtitle_delete_all_segment(pstDVBInfo);
							HxLOG_Print("(-)DVB_SBTL_STUFFING\n");
							break;
						default:
							segment_len = 1;
...
static HERROR nx_prism_subtitle_GetBitFlagSub(HUINT8 *ptr, HUINT32 size, HUINT32 pos, HUINT32 *val)
{
	HUINT8	ch;
	HUINT32	bytePos;
...
static HINT32 nx_prism_subtitle_GetBitData(HUINT8 *ptr, HUINT32 *pos, HINT32 cnt)
{
	HERROR 	hErr;
	HINT32	i, res, val;
	...
	for (i = 0; i < cnt; i++)
	{
		hErr = nx_prism_subtitle_GetBitFlagSub(ptr, MAX_SBTL_PIXEL_BIT_SIZE, *pos + i, &val);
		if ( hErr != ERR_OK )
		{
			return 0;
		}
...
static	void	nx_prism_subtitle_put_pixel(HUINT32 **ppulAddr, ONDK_Color_t color, HINT32 cnt)
{
	HINT32	i;
	HUINT32	*p;
...
static	void	nx_prism_subtitle_decode2bpCode(HUINT32 *target_addr, HUINT8 *pData, HINT32 *cur_index, HUINT8 *map_table, ONDK_Color_t *color_table)
{
	...
	while (1)
	{
		val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 2);
		if (val != 0)
		{
			val = (map_table == NULL) ? val : map_table[val];
			nx_prism_subtitle_put_pixel(&p, color_table[val], 1);
		}
		else
		{
			val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 1);
			if (val == 0x01)
			{
				runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 3);
				val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 2);
				val = (map_table == NULL) ? val : map_table[val];

				nx_prism_subtitle_put_pixel(&p, color_table[val], runLen + 3);
			}
			else
			{
				val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 1);
				if (val == 0x00)
				{
					...
					if (val == 0x02)
					{
						runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4);
						val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 2);
						val = (map_table == NULL) ? val : map_table[val];

						nx_prism_subtitle_put_pixel(&p, color_table[val], runLen + 12);
					}
					else if (val == 0x03)
					{
						runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 8);
						val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 2);
						val = (map_table == NULL) ? val : map_table[val];

						nx_prism_subtitle_put_pixel(&p, color_table[val], runLen + 29);
					}
					else if (val == 0x00)
					{
					...
					else if (val == 0x01)
					{
						...
						nx_prism_subtitle_put_pixel(&p, color_table[val], 2);
					}
				}
				else
				{
...
static	void	nx_prism_subtitle_decode4bpCode(HUINT32 *target_addr, HUINT8 *pData, HINT32 *cur_index, HUINT8 *map_table, ONDK_Color_t *color_table)
{
	HUINT32		nBitPos = 0, *p = NULL;
	HINT32		val = 0, runLen = 0;
	...
	while (1)
	{
		val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4);
		if (val != 0)
		{
			val = (map_table == NULL) ? val : map_table[val];
		...
		else
		{
			...
			if (val == 0x00)
			{
				...
				if (runLen != 0x00)
				{
					...
					runLen += 2;
					nx_prism_subtitle_put_pixel(&p, color_table[val], runLen);
				}
				else
				{
			...
			else
			{
				...
				if (val == 0x00)
				{
					...
					runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 2) + 4;
					val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4);
					val = (map_table == NULL) ? val : map_table[val];
					nx_prism_subtitle_put_pixel(&p, color_table[val], runLen);
				}
				else
				{
					...
					if (val == 0x02)
					{
						...
						runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4) + 9;
						val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4);
						val = (map_table == NULL) ? val : map_table[val];
					...
					else if (val == 0x03)
					{
						...
						runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 8) + 25;
						val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 4);
						val = (map_table == NULL) ? val : map_table[val];
...
static	void	nx_prism_subtitle_decode8bpCode(HUINT32 *target_addr, HUINT8 *pData, HINT32 *cur_index, HUINT8 *map_table, ONDK_Color_t *color_table)
{
	HUINT32		nBitPos = 0, *p = NULL;
	HINT32		val = 0, runLen = 0;
	...
	while (1)
	{
		val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 8);
		if (val != 0)
		{
			val = (map_table == NULL) ? val : map_table[val];
			nx_prism_subtitle_put_pixel(&p, color_table[val], 1);
		...
		else
		{
			...
			if (val == 0x00)
			{
				val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 7);
				if (val != 0x00)
				{
					val = (map_table == NULL) ? 0 : map_table[0];

					nx_prism_subtitle_put_pixel(&p, color_table[val], val);
				}
				else
				{
					*cur_index = *cur_index + ((nBitPos >> 3) + 1) + 1;
			...
			{
				runLen = nx_prism_subtitle_GetBitData(pData, &nBitPos, 7);
				val = nx_prism_subtitle_GetBitData(pData, &nBitPos, 8);
				val = (map_table == NULL) ? val : map_table[val];
...
static	void	nx_prism_subtitle_draw_string_to_region(ONDKWindowHandle _window, HINT32 obj_x, HINT32 obj_y, ObjectSegment_t *object_segment)
{
...
static	void	nx_prism_subtitle_draw_object_to_region(RegionSegment_t *region_segment, HINT32 obj_x, HINT32 obj_y, ObjectSegment_t *object_segment)
{
	...
	while (buf_index < top_field_len)
	{
		...
		switch (pData[buf_index++])
		{
			case	SBTL_2BIT_PIXEL_STRING:
				...
				nx_prism_subtitle_decode2bpCode(pulTargetAddr, (HUINT8*)&pData[buf_index], &buf_index, map_table, region_segment->color_table);
				break;
			...
			case	SBTL_4BIT_PIXEL_STRING:
				...
				nx_prism_subtitle_decode4bpCode(pulTargetAddr, (HUINT8*)&pData[buf_index], &buf_index, map_table, region_segment->color_table);
				break;

			case	SBTL_8BIT_PIXEL_STRING:
				nx_prism_subtitle_decode8bpCode(pulTargetAddr, (HUINT8*)&pData[buf_index], &buf_index, NULL, region_segment->color_table);
				break;
...
static	void	m_subtitle_process_dvb(ONDKWindowHandle _window,HUINT16 page_id, DVBData_t *pstDVBInfo)
{
	...
	page_segment = nx_prism_subtitle_find_page_segment(pstDVBInfo, page_id);
	if (page_segment == NULL)
	{
	...
	for (i = 0; i < page_segment->ulRegionNum; i++)
	{
		...
		region_segment = nx_prism_subtitle_find_region_segment(pstDVBInfo, page_id, page_segment->pRgList[i].u8RegionId);
		if (region_segment == NULL)
		{
		...
		color_segment = nx_prism_subtitle_find_clut_segment(pstDVBInfo, page_id, region_segment->u8ClutId);
		...
		region_segment->color_table = (ONDK_Color_t*)NX_APP_Calloc(sizeof(ONDK_Color_t) * clut_no);
		if (region_segment->color_table == NULL)		{

			HxLOG_Critical("Fail NX_APP_Calloc().\n");
			region_segment->color_table = NULL;
			continue;
		}
		...
		for (j = 0; j < region_segment->ulObjectNum; j++)
		{
			...
			switch (region_segment->pObjectList[j].u2ObjectType)
			{
				case	SBTL_OBJECT_TYPE_BITMAP:
					object_segment = nx_prism_subtitle_find_object_segment(pstDVBInfo, page_id, region_segment->pObjectList[j].u16ObjectId);
					if (object_segment)
						nx_prism_subtitle_draw_object_to_region(region_segment, obj_x, obj_y, object_segment);
					break;
				case	SBTL_OBJECT_TYPE_CHARACTER:
				case	SBTL_OBJECT_TYPE_STRING_OF_CHAR:
					object_segment = nx_prism_subtitle_find_object_segment(pstDVBInfo, page_id, region_segment->pObjectList[j].u16ObjectId);
					if (object_segment)
						nx_prism_subtitle_draw_string_to_region(_window, obj_x, obj_y, object_segment);
					break;
				default:
