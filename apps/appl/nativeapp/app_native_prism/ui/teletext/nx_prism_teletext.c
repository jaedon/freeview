...
#if 0// defined(CONFIG_UI_ALVIN) || defined(CONFIG_OP_NTVPLUS) || defined(CONFIG_OP_MOSTELECOM)|| defined(CONFIG_OP_GREECE_NOVA)|| defined(CONFIG_OP_ERTELECOM)
...
HERROR local_ttxctrl_ShowTtx (AP_TtxContent_t *pContents, HUINT8 ucM, HUINT8 ucX, HUINT16 usSubcode)
{
	...
	NX_PRISM_Teletext_Show( ucM, ucX, usSubcode, pContents->bIsEbuSbt);
...
ONDK_Result_t       local_ttxctrl_Invalidate(AP_TtxContent_t *pContents,Handle_t handle)
{
    ...
    if(ONDK_GWM_APP_Get(NX_PRISM_TtxMenuUi_Proc) == NULL && hsurface != NULL)
    {
...
void	local_ttxctrl_SetNavigationType (AP_TtxContent_t *pContents, HxCountry_e eCountry)
{
	...
	NX_PRISM_Teletext_SetControl( eTTX_CONTROL_NAVI, (HUINT32)eTtxNavi);
}
...
void	local_ttxctrl_SetLanguageGroup (AP_TtxContent_t *pContents, HxCountry_e eCountry)
{
    ...
    NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE, (HUINT32)eLangGroup);
}
...
static ONDK_Result_t local_ttxctrl_MsgGwmDestory(AP_TtxContent_t *pContents)
{
	...
	if(ONDK_GWM_APP_Get((ONDK_GWM_Callback_t)NX_PRISM_TtxMenuUi_Proc))
		ONDK_GWM_APP_Destroy((ONDK_GWM_Callback_t)NX_PRISM_TtxMenuUi_Proc);

	if ( pContents->bRunning )
	{
		/* Get Last Page Address*/
		hErr = NX_PRISM_Teletext_GetPageAddress(
		...
		/* Hide Page. */
		hErr = NX_PRISM_Teletext_Hide();
    	if(hErr == ERR_OK)
            HxLOG_Assert("NX_PRISM_Teletext_Hide");
...
static ONDK_Result_t local_ttxctrl_MsgGwmKeyDown(AP_TtxContent_t *pContents, HINT32 p1)
{
	...
	switch(p1)
	{
		...
		case KEY_OPT:
		    ONDK_GWM_APP_Create(ONDK_UPPER_WINDOW,"NX_PRISM_TtxMenuUi_Proc", APP_DEFAULT_PRIORITY, NX_PRISM_TtxMenuUi_Proc, 0, 0, 0, 0);
		...
		case KEY_8:
		case KEY_9:
			NX_PRISM_Teletext_SetControl( eTTX_CONTROL_NUMERIC, (HUINT32)(p1 - KEY_0));
			return ONDK_MESSAGE_CONSUMED;
		...
		case KEY_ARROWUP:
		case KEY_ARROWDOWN:
			NX_PRISM_Teletext_SetControl( eTTX_CONTROL_ARROWKEY, (HUINT32)(p1 - KEY_ARROWLEFT));
			return ONDK_MESSAGE_CONSUMED;
		...
		case KEY_YELLOW:
		case KEY_BLUE:
			NX_PRISM_Teletext_SetControl(eTTX_CONTROL_COLORKEY, (HUINT32)(p1 - KEY_RED));
			return ONDK_MESSAGE_CONSUMED;
		...
		case KEY_TEXT:
			//HAPI_BroadcastSignal("signal:onTeletextState", 1, "i", 0);
			//ONDK_GWM_SendMessage(NX_PRISM_Teletext_Proc,MSG_GWM_DESTROY,0,0,0,0);
			ONDK_GWM_APP_Destroy(NULL);
			return ONDK_MESSAGE_CONSUMED;
...
ONDK_Result_t	NX_PRISM_Teletext_Proc(HINT32 message, Handle_t handle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	switch (message)
	{
		...
		**********************************************************************/
	case MSG_GWM_CREATE :
		ONDK_GWM_APP_SetAppName(ONDK_GWM_APP_GetThis(), "NX_PRISM_Teletext_Proc");
...
void           NX_PRISM_Teletext_deliveryPacket(HCHAR* datatype, HINT32 len, HUINT8* p)
{
	...
	HUINT32			param1, param2, i, length;
	HUINT8			*pdata = p;
	...
	{
		if(NX_PRISM_SubtitleGetType() == eOxMP_SUBTITLETYPE_Ebu)
		{
			pGwmApp = ONDK_GWM_APP_Get((ONDK_GWM_Callback_t)NX_PRISM_Teletext_Proc);
			if(pGwmApp == NULL)
				ONDK_GWM_APP_Create(ONDK_UPPER_WINDOW,"NX_PRISM_Teletext_Proc", APP_DEFAULT_PRIORITY, NX_PRISM_Teletext_Proc, (HINT32)g_hTeletextWinHandle, TRUE, 0,0);
		}
	}
	...
	else
	{
		...
		{
			length = HxMACRO_Get32Bit(pdata);			pdata += 4;
			NX_PRISM_Teletext_Decoding(pdata);	//1  Decoding
			pdata += length;
		}
	}
}
