...
static HERROR	local_ttx_dump (HUINT16 usPageNum, HUINT16 usSubcode)
{
	...
	if (gttxdump.pageBuf == NULL)
	{
		...
		gttxdump.pageBuf = (HUINT8 *)NX_APP_Calloc(gttxdump.bufSize);
		if (gttxdump.pageBuf == NULL)
			return ERR_FAIL;
	}
...
static HERROR local_ttx_Start(void)
{
	...
	{
		s_pucTtxMemory	= (HUINT8 *)NX_APP_Malloc(TTX_MEMORY_SIZE);
		if (s_pucTtxMemory == NULL)
		{
...
void   NX_PRISM_Teletext_Init(void)
{
	...
	if ( hErr != VK_OK )
	{
	    HxLOG_Critical("NX_PRISM_Teletext_Init MSGQ fail..");
	}
...
HERROR NX_PRISM_Teletext_StartService(void)
{
	...
	HxLOG_Print("NX_PRISM_Teletext_StartService\n");
...
HERROR NX_PRISM_Teletext_StopService(void)
{
	...
	HxLOG_Print("NX_PRISM_Teletext_StopService\n");
...
HERROR	NX_PRISM_Teletext_Decoding (HUINT8 *pucPES)
{
	...
	//	return NX_PRISM_Teletext_StartService(hwindow);
...
HERROR NX_PRISM_Teletext_Hide(void)
{
    ...
    HxLOG_Print("NX_PRISM_Teletext_Hide\n");
...
HERROR NX_PRISM_Teletext_SetSutitleFontType(HBOOL bVariableFont)
{
	...
	HxLOG_Print("NX_PRISM_Teletext_SetSutitleFontType(bVariableFont:%d)\n", (int)bVariableFont);
...
{
	if(s_hTtxDecoder == HANDLE_NULL)
	{
		return HANDLE_NULL;
	}
	return s_hTtxDecoder;
}
...
//	HBOOL			bVariableFont;
	...
	if(NX_PRISM_Teletext_Show(TTX_NULL_MAGAZINE,TTX_NULL_PAGE ,TTX_NULL_SUBCODE,subtitle) ==ERR_FAIL)
		HLIB_CMD_Printf("NX_PRISM_Teletext_Show Error.. \n" );
...
{
	//if(NX_PRISM_Teletext_Hide() ==ERR_FAIL)
	//	HLIB_CMD_Printf("NX_PRISM_Teletext_Hide Error.. \n" );
	NX_PRISM_Teletext_Hide();
	return HxCMD_OK;
}
...
int local_ttx_CMD_ColorKey(void *szArgList)
{
	...
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_COLORKEY,arg1)== ERR_FAIL)
...
int local_ttx_CMD_ArrowKey(void *szArgList)
{
	...
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_ARROWKEY,arg1)== ERR_FAIL) //0:Left, 1:Right,2:Up, 3:Down,
		HLIB_CMD_Printf("Teletext_CMD_ColorKey Error!\n");
...
int local_ttx_CMD_Input(void *szArgList)
{
	...
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_NUMERIC,arg1)== ERR_FAIL)
...
{
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_INDEX,0)== ERR_FAIL)
		HLIB_CMD_Printf("Teletext_CMD_Index Error!\n");
	return HxCMD_OK;
}
int local_ttx_CMD_Cancel(void *szArgList)
{
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_CANCEL,0)== ERR_FAIL)
...
int local_ttx_CMD_Mix(void *szArgList)
{
	...
	switch(arg1)
	{
	case 0:			hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_0_PERCENT); break;
	case 10:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_10_PERCENT); break;
	case 20:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_20_PERCENT); break;
	case 30:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_30_PERCENT); break;
	case 40:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_40_PERCENT); break;
	case 50:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_50_PERCENT); break;
	case 60:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_50_PERCENT); break;
	case 70:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_60_PERCENT); break;
	case 80:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_70_PERCENT); break;
	case 90:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_80_PERCENT); break;
	case 100:		hErr=NX_PRISM_Teletext_SetControl(eTTX_CONTROL_MIX,eTTX_MIX_90_PERCENT); break;
	}
...
{
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_REVEAL,0)== ERR_FAIL)
...
{
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_SUBPAGE,0)== ERR_FAIL)
...
int local_ttx_CMD_Double(void *szArgList)
{
	...
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_DOUBLE,arg1)== ERR_FAIL)
		HLIB_CMD_Printf("Teletext_CMD_Subpage Error!\n");
	return HxCMD_OK;
}
...
{
	if(NX_PRISM_Teletext_SetControl(eTTX_CONTROL_HOLD,0)== ERR_FAIL)
		HLIB_CMD_Printf("Teletext_CMD_Subpage Error!\n");
	return HxCMD_OK;
}
...
int local_ttx_CMD_Navi(void *szArgList)
{
	...
	switch(arg1)
	{
	case 0: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_NAVI,eTTX_NAVI_FLOF); break;
	case 1: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_NAVI,eTTX_NAVI_TOP); break;
	case 2: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_NAVI,eTTX_NAVI_TRACE); break;
	case 3: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_NAVI,eTTX_NAVI_LAST); break;
	}
...
int local_ttx_CMD_Language(void *szArgList)
{
	...
	switch(arg1)
	{
	case 0: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE,eTTX_LANG_LATIN_WEST_EU); break;
	case 1: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE,eTTX_LANG_LATIN_EAST_EU); break;
	case 2: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE,eTTX_LANG_HEBREW_ARABIC); break;
	case 3: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE,eTTX_LANG_CYRILLIC); break;
	case 4: hErr =NX_PRISM_Teletext_SetControl(eTTX_CONTROL_LANGUAGE,eTTX_LANG_LAST); break;
	}
...
#define	hCmdHandle		"TTX_CMD"
	...
	HLIB_CMD_RegisterWord(hCmdHandle,local_ttx_CMD_Hide,
			"hide",																/* keyword */
			"NX_PRISM_Teletext_Hide page",												/*   help  */
			"hide");															/*  usage  */
