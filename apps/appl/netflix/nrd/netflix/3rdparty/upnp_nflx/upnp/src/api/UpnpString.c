...
static size_t strnlen_(const char *s, size_t n)
{
    const char *p = (const char *)memchr(s, 0, n);
    return p ? (size_t)(p - s) : n;
}

static char *strndup_(const char *__string, size_t __n)
{
    const size_t strsize = strnlen_(__string, __n);
    char *newstr = (char *)malloc(strsize + 1);

    memcpy(newstr, __string, strsize);
    newstr[strsize] = 0;
    return newstr;
}
