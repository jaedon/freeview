...
function simpleHttpRequest(url, success, failure, method, data) {
    ...
    if(method=="0"){
		...
		method = "GET";
 		...
 		method = "DELETE";
		if(delLocation == ''){
			mdx.app.log("Delete Location Not Defined, must come from Post respose.");
   		}
		else {
			url = delLocation;
		}
        ...
        if (request.readyState == 4) {
            if ((request.status == 200)||(request.status == 201)){
		        ...
		        if(request.getResponseHeader('Location') != null) {
		    	    mdx.app.log("response header: " + request.getResponseHeader('Location'));
			        delLocation = request.getResponseHeader('Location');
		        }
		        ...
		        try{	
			        ...
			        if(data=='initial'){
				        ...
				        cleanUrl = request.getResponseHeader('Application-URL');
			        }
...
function changeInterface(){
	...
	mdx.interfaceChanged(interfaceName, interfaceConnected, ipaddress, ssid);
}

function launchNetflix(){
    mdx.app.log("Launching Netflix");
    dialUsn = document.getElementById('dialUsn').value;
    mdx.discovery.launchNetflix(dialUsn);
}

function isRemoteDeviceReady(){
    mdx.app.log("isRemoteDeviceReady called");
    dialUsn = document.getElementById('dialUsn1').value;
    mdx.discovery.isRemoteDeviceReady(dialUsn);
}
...
$(document).ready(function() {
    ...
    $(".regpair").button({text: true, label: "REGPAIR"});
    ...
    var clickDispatcher = function(event) {
        ...
        case "list":
            mdx.app.fixture.targets = mdx.discovery.getRemoteDeviceList();
            	...
            	if (cmd == "sendMessage") {
			        ...
			        eqAnchor.append(eq);
             	    ...
             	    mdx.app.log("Send HTTPRequest: " + args[0] + " " + args[1] + " " + postorget);
          	        simpleHttpRequest(args[0], mdx.app.log, mdx.app.log, postorget);
    ...
    var getCpuMemoryUsage = function() {
        if(nrdp.ps3) {
            nrdp.ps3.getCPUUtilization( function(result)
                                        {
                                            cCurrentCpu = (Math.round(result*1000)/1000)*100;
                                            if(cCurrentCpu < cMinCpu || cMinCpu == 0) { cMinCpu = cCurrentCpu; }
            ...
            nrdp.ps3.getAvailableMemory(function(result)
                                        {
                                            cCurrentMemory = (result/1000000)*1000/1000;
                                            nrdp.log.info("PS3-MEMORY-AVAILABLE: memory:" + result);
                                            if(cCurrentMemory < cMinMemory || cMinMemory == 0) { cMinMemory = cCurrentMemory; }
                                            if(cCurrentMemory > cMaxMemory) { cMaxMemory = cCurrentMemory; }
                                            $("#c-min-memory-usage-val").val(cMinMemory + 'MB');
                                            $("#c-max-memory-usage-val").val(cMaxMemory + 'MB');
                                            $("#c-cur-memory-usage-val").val(cCurrentMemory + 'MB');
                                        });
        }
        else {
    ...
    var regPairClickHandler = function(event) {
	    ...
	    if (uuid.indexOf('DIAL')!=-1) {
		    ...
		    if (_state.pairing.handle != -1) {
 		    ...
 		    var handle =  mdx.pair.regPairRequest(uuid, pin.val());
        	...
        	mdx.app.log("Command: regpair(" + uuid + ", " + pin.val() + ") ", "command");
        }
    }
    ...
    var controllerRegPairResponse = function(evt) {
    	var uuid = currentPairingDevice.attr('uuid');
    	_state.pairing.handle = evt.pairingContext;

	    mdx.app.log("REGPAIR RESPONSE- " + JSON.stringify(evt, null, 2));

	    if((evt.errorCode == "0")||(evt.errorCode == undefined)){
		    setPaired(currentPairingDevice);
	    } else {
		    mdx.app.log("REGPAIR REJECTED FROM TARGET");
	    }    	
    }
        ...
        for (var i=0; i < args.length; i++) {
	        ...
	        if(label == "carg-1"){
		        if(cmd=="sendMessage"){
			        ...
			        var argInputTxt = '<select name="' + label + '" id="' + label + '"><option value="'+cleanUrl + 'Netflix">Netflix</option>';
			        argInputTxt += '<option value="'+cleanUrl + 'YouTube">YouTube</option>';
			        argInputTxt += '</select>';
		        ...
		        if(cmd=="httpRequest"){
			        ...
			        var argInput = $('<input id="' + label + '" class="arg-text" name="' + label + '" type="' + params[cmd].type[i] + '" value={"catalogId":"http://movies.netflix.com/Example_Short_23.976/70136810","trackId":"1234"} />');
		        }
		        ...
		        if(cmd=="httpRequest"){
			        var argInput = $('<input id="' + label + '"  name="postorget" type="radio" name="type" value="1" class="postorget"/><span class="arg-text">Status(GET)</span><input id="' + label + '" name="postorget" type="radio" name="type" value="0" class="postorget" /><span class="arg-text">Launch(POST)</span><input id="' + label + '" name="postorget" type="radio" name="type" value="2" class="postorget" /><span class="arg-text">Stop(DELETE)</span>');
		        }
		        else{
    ...
    mdx.app.getRemoteDeviceList = function() {

        deviceList = mdx.discovery.getRemoteDeviceList();
        mdx.app.log("Remote device list- " + JSON.stringify(deviceList, null, 2));
    };
    var discoverEvent = function(event) {
        if (event.type == "devicefound") {	
	        mdx.app.log("DEVICE FOUND- " + JSON.stringify(event, null, 2));
            mdx.app.fixture.targets = mdx.discovery.getRemoteDeviceList();
            var myuuid = event.UUID;
	        var uuidclass = '.'+ myuuid;
            var device = {selectorKey:"netflix", loc:event.location, uuid:myuuid, usn:event.USN, friendlyName:event.friendlyName };
    ...
    var addDevice = function(device) {
        ...
        line = line.next();
        line.children("span:first").text(device.friendlyName);
    ...
    mdx.app.addMdxEvtListeners = function() {
        ...
        mdx.discovery.addEventListener("devicelost", logThisEvent);
        mdx.discovery.addEventListener("remoteDeviceReady", logThisEvent);
        ...
        mdx.pair.addEventListener("regpairresponse", controllerRegPairResponse);
        ...
        mdx.addEventListener("mdxstate", logThisEvent);
    ...
    $('.regpair').live("click", regPairClickHandler);
