...
void TextScriptRuns::addRun(uint32_t start, uint32_t end, const std::string &script)
{
    Base::add(TextScriptRun::SharedPtr(new TextScriptRun(start, end, script)));
}

void TextScriptItemizer::processScripts(TextScriptRuns& textRuns, UnicodeString &text)
{
    ScriptRun scriptRun(text.getBuffer(), 0, text.length());

    while (scriptRun.next()) {
        textRuns.addRun(scriptRun.getScriptStart(), scriptRun.getScriptEnd(), uscript_getShortName(scriptRun.getScriptCode()));
    }
}
...
UBool ScriptRun::next()
{
    ...
    for (scriptStart = scriptEnd; scriptEnd < charLimit; scriptEnd += 1) {
        ...
        UScriptCode sc;
        if (ch >= 0xFF00 && ch <= 0xFFEF) { // Report Hani for fullwidth/halfwidth forms
            sc = USCRIPT_HAN;
        } else {
