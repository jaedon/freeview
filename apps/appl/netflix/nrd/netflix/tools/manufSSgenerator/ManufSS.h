...
#define nfSS_FILENAME 						"/manuf_ss.bin"
 ...
 * that live in the TEE. Note that for Crypto, we define two client IDs; this
...
#define nfSS_CLIENT_ID_TEE_CRYPTO_RO   {0x9d,0x4a,0x88,0xc4,0x32,0x62,0x78,0x7c,0x8f,0x27,0xba,0xed,0xa8,0x71,0x61,0x85}
#define nfSS_CLIENT_ID_TEE_CRYPTO_RW   {0x72,0x90,0xae,0x07,0xbd,0x8d,0x07,0x74,0xac,0x6a,0xde,0x07,0xd5,0xa7,0xce,0xc9}
...
typedef enum teeSectionID
{
	...
	teeSID_CRYPTO_RO = 0xF00FEE01,
	teeSID_CRYPTO_RW = 0xF00FEE02,
...
#define nsMAX_SS_CRYPTO_RO_SIZE 126
 ...
 * add more Crypto keys, make sure they are different than the ones defined
 * below. Using an enum is a good way to ensure that this is true.
 *
 * Here are the keys used by the Crypto agent for ESN, Kpe, Kph:
 */
enum crypto_RW_AgentClientKeys
{
	// these elements are dynamically updated
	cryptoAGENT_KEY_KCE = 0x00000001,
	cryptoAGENT_KEY_KCH = 0x00000002,
};

enum crypto_RO_AgentClientKeys
{
	// these elements are write-protected
	cryptoAGENT_KEY_ESN = 0x80000001,
	cryptoAGENT_KEY_KPE = 0x80000002,
	cryptoAGENT_KEY_KPH = 0x80000003,
};
...
public:
	 ...
	 * Purpose: constructor
	 *
	 * Inputs: targetDir - Path to target directory where manuf_ss.bin exists
	 * 							or will be created
	 *         idfile - Path to idfile
	 *         drmDir - Path to directory containing DRM files
	 *
	 *********************************************************************/
	ManufSS(std::string targetDir, std::string idfile,
			std::string drmDir);

	/*********************************************************************
	 * Function: create
	 *
	 * Purpose: checks if the manuf_ss.bin file exists in target directory
	 *
	 ...
	 * Function: create
	 *
	 * Purpose: creates the manuf_ss.bin in targetDirPath
	 *
...
private:
	std::string manufSSPath;
	std::string idfilePath;
	std::string drmDirPath;
	 ...
	 *
	 *********************************************************************/
	static int addMgmtSection(uint8_t **BufPtrPtr, size_t MaxLength);

	/*********************************************************************
	 * Function: addCryptoSection
	 *
	 * Purpose: add Crypto read-only section to secure store file
	 *
	 * Inputs: bufPtrPtr - pointer to pointer to buffer
	 *         maxLength - bytes available in *BufPtrPtr
	 ...
	 *
	 *********************************************************************/
	static int addCryptoSection(uint8_t **bufPtrPtr, size_t maxLength,
	 ...
	 *
	 *********************************************************************/
	static int addDrmSection(uint8_t **bufPtrPtr, size_t maxLength,
			std::string &drmDir);
	 ...
	 *
	 *********************************************************************/
	static int sha1(uint8_t *dataPtr, uint32_t dataLength, uint8_t *resultPtr);
	 ...
	 *
	 *********************************************************************/
	static int hmacSha256(uint8_t *dataPtr, uint32_t dataLength,
			uint8_t *keyPtr, uint32_t keyLength, uint8_t *resultPtr);

	/*********************************************************************
	 * Function: trimNewLine
	 *
	 * Purpose:  removes the trailing \n, \r or \r\n characters from end
	 *
	 * Inputs:  strPtr - pointer to string
	 *
	 *********************************************************************/
	static void trimNewLine(char** strPtr);
     ...
     *
     *********************************************************************/
    static int checkDrmFiles(std::string &drmDir);
