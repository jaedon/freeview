							  	  	  	 ...
							  	  	  	 nfTeeClientId_t	ClientId )
{
	...
	if ( InMsgBufLength <  nfMIN_NFLX_DHDERIVE_REQ_SIZE  )
	{
		// this buffer is too small to hold a valid message
		CRYPTODBG("%s: InMsgBufLength is %d, expected size is at least %d\n",
				__FUNCTION__, InMsgBufLength,  nfMIN_NFLX_DHDERIVE_REQ_SIZE );
		return (nfTeeResult_t) nfTCR_OPERAND_TOO_SHORT;
	}
							  	  	  ...
							  	  	  nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	// do it
	ResponseCode = caDhGenKeys( ClientId, Generator,
							  	  	   	    ...
							  	  	   	    nfTeeClientId_t	ClientId )
{
	...
	if ( htonl(ReqPtr->DataLength) != (InMsgBufLength - sizeof(uint32_t)) )
	{
									   ...
									   nfTeeClientId_t	ClientId )
{
	...
	OutLength = teeHmacSha256( ((nfTeeHmacSha256Request_t *)InMsgPtr)->Data, DataLength,
									   	   	 ...
									   	   	 nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	DataLength = teeNtohl(((nfTeeHmacSha256UpdateRequest_t *)InMsgPtr)->DataLength);
	...
	if ( HmacKeyLength == nfTEE_HMAC_SHA256_KEY_SIZE_BYTES )
	{
		...
		if ( ResponseCode == nfTGR_SUCCESS )
		{
			*MsgLengthPtr = OutLength;
			...
			if ( OutLength == 0 )
			{
				CRYPTODBG("%s: HMAC is empty --> no HMAC data?\n", __FUNCTION__);
				ResponseCode = (nfTeeResult_t) nfTCR_OPERAND_TOO_SHORT;
			}
		}
	}
	else
									   	     ...
									   	     nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	// okay, call the HMAC operation, putting result into OutMsg
	OutLength = teeHmacSha256( ((nfTeeHmacSha256VerifyRequest_t *)InMsgPtr)->Data, DataLength,
							 	HmacKeyPtr, HmacKeyLength,
								ComputedHmacValue );
								 	 	  ...
								 	 	  nfTeeClientId_t	ClientId )
{
	...
	DataLength = teeNtohl(((nfTeeAesCbcDecryptRequest_t *)InMsgPtr)->DataLength);

	// validate DataLength
	if ( DataLength != (int)(InMsgBufLength - sizeof(nfTeeAesCbcDecryptRequest_t)) )
	{
		// this buffer is too small to hold a valid message
		CRYPTODBG("%s: InMsg size mismatch (%d/%d)!\n", __FUNCTION__, DataLength,
				(InMsgBufLength - sizeof(nfTeeAesCbcDecryptRequest_t)));
		return (nfTeeResult_t)nfTCR_MALFORMED_DATA;
	}
	...
	if ( AesKeyLength == nfTEE_AES_128_KEY_SIZE_BYTES )
	{
		...
		OutLength = teeAes128CbcDecrypt( ((nfTeeAesCbcDecryptRequest_t *)InMsgPtr)->EncryptedData,
								 	 	  	  	...
								 	 	  	  	nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	if ( AesKeyLength == nfTEE_AES_128_KEY_SIZE_BYTES )
	{
		...
		CRYPTODBG("%s: caAes128CbcDecryptUpdate() returned %08x\n", __FUNCTION__, ResponseCode );
								 	 	   	   ...
								 	 	   	   nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	if ( AesKeyLength == nfTEE_AES_128_KEY_SIZE_BYTES )
	{
		...
		CRYPTODBG("%s: calling caAes128CbcDecryptFinal with room for %d bytes\n", __FUNCTION__, OutMsgBufLength );
		...
		CRYPTODBG("%s: caAes128CbcDecryptFinal() returned %08x\n", __FUNCTION__, ResponseCode );
								 	 	  ...
								 	 	  nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	{
		// okay, call the AES encrypt operation, putting ciphertext into OutMsg
		OutLength = teeAes128CbcEncrypt( ((nfTeeAesCbcEncryptRequest_t *)InMsgPtr)->Data,
...
}/* end caDispatchAesCbcEncryptMsg */
								 	 	  	  	...
								 	 	  	  	nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	DataLength = teeNtohl(((nfTeeAesCbcEncryptUpdateRequest_t *)InMsgPtr)->DataLength);

	// validate DataLength
	if ( DataLength != (int)(InMsgBufLength - sizeof(nfTeeAesCbcEncryptUpdateRequest_t)) )
	{
		// this buffer is too small to hold a valid message
		CRYPTODBG("%s: InMsg size mismatch (%d/%d)!\n", __FUNCTION__, DataLength,
				(InMsgBufLength - sizeof(nfTeeAesCbcEncryptUpdateRequest_t)));
		return (nfTeeResult_t)nfTCR_MALFORMED_DATA;
	}
	...
	if ( AesKeyLength == nfTEE_AES_128_KEY_SIZE_BYTES )
	{
		...
		CRYPTODBG("%s: caAes128CbcEncryptUpdate() returned %08x\n", __FUNCTION__, ResponseCode );
							 	 	  ...
							 	 	  nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	/*
	 * validate the Algorithm and KeyFormat values.
	 */
	switch ( Algorithm )
	{
		case nfCA_HMAC:
							 	 	  ...
							 	 	  nfTeeClientId_t	ClientId )
{
	...
	*MsgLengthPtr = 0;
	...
	if ( ResponseCode == nfTGR_SUCCESS )
	{
		...
		*MsgLengthPtr = sizeof(nfTeeExportKeyResponse_t) + DataLength;
	}
	else
	{
