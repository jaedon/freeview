...
// size of local crypto work buffer
...
caKeyObject_t caKeyStore[caMAX_KEYS] = {  { 0, 0, {0,.ctxt.rawContext={0,{0}}}, 0, 0, 0, .CryptoContext={0,caICCS_UNINIT,0,NULL} } };
...
/*
 * cryptoAgent has its own device unique keys.
 */
...
{
    if ( ( Index < 0 ) || ( Index >  caKeyEndIndex ))
        return caKeyInvalidHandle;
...
{
    int nextIndex;
    nfCryptoKeyHandle_t allocatedHandle = caKeyInvalidHandle;

    for ( nextIndex = caKeyStartIndex; nextIndex <= caKeyEndIndex; nextIndex++ )
    {
        if (!caKeyStore[nextIndex].InUse)
        {
            caKeyStore[nextIndex].InUse = 1;
            allocatedHandle = caGetHandleFromIndex(nextIndex);
            break;
        }
    }
    ...
    CRYPTODBG("keyStore: [");
    for ( nextIndex = caKeyStartIndex; nextIndex <= caKeyEndIndex; nextIndex++ ) {
        if ( caGetIndexFromHandle(allocatedHandle) == nextIndex ) CRYPTODBG("+");
        else if ( caKeyStore[nextIndex].InUse )                   CRYPTODBG("#");
        else                                                      CRYPTODBG("_");
    }
    CRYPTODBG("]\n");
#endif
...
static int caDeallocateKeyHandle(nfCryptoKeyHandle_t Handle)
{
    ...
    {
        CRYPTODBG("%s: invalid handle %d\n", __FUNCTION__, Handle);
        return -1;
    }

    if (!caKeyStore[Index].InUse) {
        CRYPTODBG("%s: attempting to deallocate unallocated handle %d\n", __FUNCTION__, Handle);
        return -1;
    }

#if 0
    CRYPTODBG("keyStore: [");
    for ( int nextIndex = caKeyStartIndex; nextIndex <= caKeyEndIndex; nextIndex++ ) {
        if ( Index == nextIndex )               CRYPTODBG("-");
        else if ( caKeyStore[nextIndex].InUse ) CRYPTODBG("#");
        else                                    CRYPTODBG("_");
    }
    CRYPTODBG("]\n");
        ...
        nfKeyUsageFlagsWord_t    KeyUsageFlags )
{
    ...
    if (caKeyInvalidIndex == index) {
        CRYPTODBG("%s: invalid handle %d\n", __FUNCTION__, KeyHandle);
        return nfTCR_INVALID_KEY_HANDLE;
    }

    if (!caKeyStore[index].InUse) {
        CRYPTODBG("%s: handle %d is not currently allocated\n", __FUNCTION__, KeyHandle);
        return nfTCR_INVALID_KEY_HANDLE;
    }
...
static nfTeeCryptoResult_t caImportNamedKeys()
{
    ...
    if (caEsnLength > caMAX_ESN_LEN) {
        INITDBG("%s: ESN length %luB larger than available storage %dB\n", __FUNCTION__, caEsnLength, caMAX_ESN_LEN);
    ...
    if ( caNamedKeyStore[caNK_INDEX_KPE].KeyLength > sizeof caNamedKeyStore[caNK_INDEX_KPE].KeyBits ) {
        INITDBG("%s: KPE data too large to store (%lu|%zu)\n", __FUNCTION__, caNamedKeyStore[caNK_INDEX_KPE].KeyLength, sizeof caNamedKeyStore[caNK_INDEX_KPE].KeyBits);
        return nfTCR_BAD_PARAMETER;
    }
    ...
    if ( caNamedKeyStore[caNK_INDEX_KPH].KeyLength > sizeof caNamedKeyStore[caNK_INDEX_KPH].KeyBits ) {
        INITDBG("%s: KPH data too large to store (%lu|%zu)\n", __FUNCTION__, caNamedKeyStore[caNK_INDEX_KPH].KeyLength, sizeof caNamedKeyStore[caNK_INDEX_KPH].KeyBits);
        return nfTCR_BAD_PARAMETER;
    }
    ...
    if ( caNamedKeyStore[caNK_INDEX_KPW].KeyLength > sizeof caNamedKeyStore[caNK_INDEX_KPW].KeyBits ) {
        INITDBG("%s: KPW data too large to store (%lu|%zu)\n", __FUNCTION__, caNamedKeyStore[caNK_INDEX_KPW].KeyLength, sizeof caNamedKeyStore[caNK_INDEX_KPW].KeyBits);
        return nfTCR_BAD_PARAMETER;
    }
    ...
    // copy client id
    RspPtr = (nfTeeStorageSimpleReadRsp_t *)caStorageResponseBuf;
    uint32_t ClientIdLength = teeNtohl(RspPtr->DataLength);
    INITDBG("%s: ClientId DataLength is %d (we expect 16)\n", __FUNCTION__, ClientIdLength );
    if ( ClientIdLength > sizeof caClientIdList[caCID_INDEX_NRDP_APP] ) {
        INITDBG("%s: ClientId data too large to store (%lu|%zu)\n", ClientIdLength, sizeof caClientIdList[caCID_INDEX_NRDP_APP]);
        return nfTCR_BAD_PARAMETER;
    }
    (void) teeMemcpy( caClientIdList[caCID_INDEX_NRDP_APP], RspPtr->Data, ClientIdLength );
#if 0
    ...
    // now, copy key
    RspPtr = (nfTeeStorageSimpleReadRsp_t *)caStorageResponseBuf;
    if ( teeNtohl(RspPtr->DataLength) != sizeof caInternalKey ) {
        INITDBG("%s: invalid internal key size (%lu|%zu)\n", teeNtohl(RspPtr->DataLength), sizeof caInternalKey);
        return nfTCR_BAD_PARAMETER;
    }
    (void) teeMemcpy(caInternalKey, RspPtr->Data, sizeof caInternalKey);
#if 0
    ...
    {/* begin local scope limiter */
        ...
        if ( ( rc = teeSha384(caInternalKey, caAES_128_KEY_SIZE_BYTES, TempBuf) ) != 0 )
        {
        ...
        nfTeeCryptoContextHandle_t    *CryptoContextHandlePtr )
{
            ...
            || ( caKeyStore[KeyIndex].CryptoContext.CryptoContextPtr != NULL ) )
    {
        CRYPTODBG("%s: invalid crypto context (%d, %d, %p)\n", __FUNCTION__,
                ...
                caKeyStore[KeyIndex].CryptoContext.CryptoContextPtr );
    ...
    if ( caKeyStore[KeyIndex].CryptoContext.CryptoContextPtr != NULL )
    {
        int rc;
        ...
        /* TODO: unclear whether CryptoContextHandlePtr points to storage of the same size of caKeyStore[x].CryptoContext */
        (void) teeMemcpy( CryptoContextHandlePtr,
        ...
        uint32_t                        DataLength )
{
            ...
            || ( !caKeyStore[KeyIndex].InUse ) )
    {
        ...
        return nfTCR_BAD_PARAMETER;
            ...
            || ( caKeyStore[KeyIndex].CryptoContext.CryptoContextPtr != ContextPtr->CryptoContextPtr ) )
    {
        CRYPTODBG("%s: invalid crypto context (%d/%d, %d/%d, %p/%p)\n", __FUNCTION__,
                caKeyStore[KeyIndex].CryptoContext.State,  ContextPtr->State,
                caKeyStore[KeyIndex].CryptoContext.Validator, ContextPtr->Validator,
                caKeyStore[KeyIndex].CryptoContext.CryptoContextPtr,
                ContextPtr->CryptoContextPtr );
        ...
        uint32_t                        *DataLengthPtr )
{
    ...
    ActualLength = teeAes128CbcDecryptUpdate( ContextPtr->CryptoContextPtr,
        ...
        uint32_t                        *DataLengthPtr )
{
    ...
    ActualLength = teeAes128CbcDecryptFinal( ContextPtr->CryptoContextPtr,
            DataPtr, DataMaxLength );
        ...
        uint32_t             *EsnLengthPtr )
{
    ...
    if ( teeMemcmp(ClientId, caClientIdList[caCID_INDEX_NRDP_APP], sizeof(nfTeeClientId_t)) == 0 )
    {
        ...
        if ( caEsnLength <= EsnMaxLength )
        {
            ...
            CRYPTODBG("%s: returning %d for ESN length.\n", __FUNCTION__, caEsnLength);
...
{
    int                 i;
    nfCryptoKeyHandle_t KeyHandle = caKeyInvalidHandle;
    nfKeyContext_t      KeyContext;
    nfTeeCryptoResult_t Result;
    ...
    for ( i = 0; i < caNK_INDEX_INVALID; ++i )
    {
        ...
        if ( caNamedKeyStore[i].KeyNameLength == KeyNameLength
          && teeMemcmp( caNamedKeyStore[i].KeyName, KeyName, KeyNameLength ) == 0 )
        {
            if ( caNamedKeyStore[i].KeyHandle == caKeyInvalidHandle )
            {
                ...
                if (caNamedKeyStore[i].KeyFormat != nfKF_RAW) {
                    ...
                    Result = nfTCR_UNSUPPORTED_KEY_ENCODING;
                    goto exit_err;
                }

                KeyContext.ContextType = caKC_RAW;
                KeyContext.ctxt.rawContext.KeyDataLength = caNamedKeyStore[i].KeyLength;
                if (caNamedKeyStore[i].KeyLength > sizeof KeyContext.ctxt.rawContext.KeyData) {
                    CRYPTODBG("%s: not enough room to copy key data from context (%lu|%zu)\n", caNamedKeyStore[i].KeyLength, sizeof KeyContext.ctxt.rawContext.KeyData);
                    Result= nfTCR_BUFFER_TOO_SMALL;
                    goto exit_err;
                }
                ...
                KeyHandle = caAllocateKeyHandle();
                if ( caKeyInvalidHandle == KeyHandle ) {
...
/* UNREACHABLE */
exit_err:
    if ( caKeyInvalidHandle != KeyHandle ) caDeallocateKeyHandle( KeyHandle );
    return Result;
        ...
        nfKeyType_t            *KeyTypePtr )
{
    ...
    if (0 == caReconcileAlgVsUsage(Algorithm, KeyUsageFlags)) {
        ...
        Result = nfTCR_OPERATION_NOT_ALLOWED;
        goto exit_err;
    }
    ...
    switch (Algorithm) {
        ...
        case nfCA_HMAC:
            ...
            /* NRDAPP-4381: at this point, it is still unclear whether the key data in KeyData can fit in the KeyData buffer in the key context */
            if (DataLength > sizeof KeyContext.ctxt.rawContext.KeyData) {
                CRYPTODBG("%s: key data too large to store into key context (%lu|%d)\n", __FUNCTION__, DataLength, sizeof KeyContext.ctxt.rawContext.KeyData);
                Result = nfTCR_BUFFER_TOO_SMALL;
                goto exit_err;
            }
            /* end NRDAPP-4381 */
            ...
            CRYPTODBG("%s: Raw context with key data length %d...\n", __FUNCTION__ , KeyContext.ctxt.rawContext.KeyDataLength);
            break;
    ...
    KeyHandle = caAllocateKeyHandle();
    if (KeyHandle == caKeyInvalidHandle) {
        CRYPTODBG("%s: returning KEY_STORE_FULL...\n", __FUNCTION__);
        Result = nfTCR_KEY_STORE_FULL;
        goto exit_err;
    }
    ...
    CRYPTODBG("%s: calling caInsertKey with handle %d, keytype %d, Algorithm %d, KeyUsageFlags %08x\n",
            __FUNCTION__, KeyHandle, KeyType, Algorithm, KeyUsageFlags );

    Result = caInsertKey(ClientId, KeyHandle, &KeyContext, KeyType, Algorithm, KeyUsageFlags );
    if ((nfTeeCryptoResult_t) nfTGR_SUCCESS != Result) {
        goto exit_err;
    }

    *KeyHandlePtr = KeyHandle;
    *KeyTypePtr = KeyType;
        ...
        uint32_t                *DataLengthPtr )
{
    ...
    int index = caGetIndexFromHandle(KeyHandle);
    if ((index == caKeyInvalidIndex) || (!caKeyStore[index].InUse))
    {
        return nfTCR_BAD_PARAMETER;
    }
    ...
    if (!(KeyPtr->KeyUsageFlags & nfKUF_EXTRACTABLE))
    {
        ...
        if ( KeyDataLength < KeyPtr->KeyContext.ctxt.dhContext.PubKeyLength) {
            // not enough space in outgoing buffer
            CRYPTODBG("%s: key is too long for passed buffer (%d/%d)\n", __FUNCTION__, KeyPtr->KeyContext.ctxt.dhContext.PubKeyLength, KeyDataLength );
    ...
    switch (KeyPtr->Algorithm)
    {
        ...
        case nfCA_AES_KW:
            {
                if ( KeyDataLength < KeyPtr->KeyContext.ctxt.rawContext.KeyDataLength )
                {
                    // not enough space in outgoing buffer
                    CRYPTODBG("%s: key is too long for passed buffer (%d/%d)\n", __FUNCTION__, KeyPtr->KeyContext.ctxt.rawContext.KeyDataLength, KeyDataLength );
                    return nfTCR_BUFFER_TOO_SMALL;
                }
        ...
        nfKeyUsageFlagsWord_t     *KeyUsageFlagsPtr )
{
    ...
    if ((KeyTableIndex == (int)caKeyInvalidHandle) || (!caKeyStore[KeyTableIndex].InUse))
    {
        ...
        uint32_t                SealedKeyMaxLength )
{
    ...
    *((uint32_t *) BytePtr) = caSealedKeyVersion;
    ...
    // get IV into WorkBuf
    if ( ! teeCryptoRandomBytes( BytePtr, caAES_128_KEY_SIZE_BYTES ) )
    {
        // this is fatal
    ...
    nfSealedKeyHeader_t *hdr = (nfSealedKeyHeader_t *)SealedKeyPtr;

    hdr->MagicHi = nfSEALEDKEY_MAGIC_HI;
    hdr->MagicLo = nfSEALEDKEY_MAGIC_LO;
    ...
    if ( hdr->KeyUsageFlags & nfKUF_EXTRACTABLE )
    {
        // not allowed
    ...
    // else copy key bits
    /* TODO: review whether size check above is enough to prevent out-of-bounds write */
    (void) teeMemcpy(hdr->KeyBits, KeyPtr, hdr->KeyBitCount );

    // how big is nfSealedKeyHeader_t + key bits?
    PlaintextLength = sizeof(nfSealedKeyHeader_t) + hdr->KeyBitCount;
...
#if defined(HUMAX_PLATFORM_BASE)
	{// Data Encrypted By H/W
		//BlockSize is 16.
		CiphertextLength = PlaintextLength + (caAES_128_KEY_SIZE_BYTES - (PlaintextLength % caAES_128_KEY_SIZE_BYTES));
	...
	uint8_t caEncHMACKey[caHMAC_SHA256_KEY_SIZE_BYTES];

	{// HMACKey Encrypted By H/W
		//printf("\n\n@@@@ (%s) Encrypt By DI_MI_XXX(SealedKey HMAC Key) @@@@\n\n", __FUNCTION__);
		uint8_t *bufferptr = (uint8_t *)DI_MI_DECRYPTOR_SOC_Malloc(caHMAC_SHA256_KEY_SIZE_BYTES);
		memset(bufferptr, 0, caHMAC_SHA256_KEY_SIZE_BYTES);
		memcpy(bufferptr, (void *)caInternalHMACKey, caHMAC_SHA256_KEY_SIZE_BYTES);
		DI_MI_DECRYPTOR_SOC_OTPKEY_DoEncrypt((unsigned char *)bufferptr, (unsigned char *)bufferptr, (unsigned int)caHMAC_SHA256_KEY_SIZE_BYTES);
		memcpy((void *)caEncHMACKey, (void *)bufferptr, caHMAC_SHA256_KEY_SIZE_BYTES);
		DI_MI_DECRYPTOR_SOC_Free(bufferptr);
		bufferptr = NULL;
	}
    HmacLength = teeHmacSha256( WorkBuf, HmacDataLength, caEncHMACKey, caHMAC_SHA256_KEY_SIZE_BYTES, &WorkBuf[HmacDataLength] );
#else
    HmacLength = teeHmacSha256( WorkBuf, HmacDataLength, caInternalHMACKey, caHMAC_SHA256_KEY_SIZE_BYTES, &WorkBuf[HmacDataLength] );
#endif
        ...
        uint8_t                 KeyData[] )
{
    ...
    /* NRDAPP-4389: SealedKeyLength must be properly tested against caHMAC_SHA256_KEY_SIZE_BYTES */
    if (SealedKeyLength < caHMAC_SHA256_KEY_SIZE_BYTES) {
        CRYPTODBG("%s: sealed key data too short (%d|>=%d)\n", SealedKeyLength, caHMAC_SHA256_KEY_SIZE_BYTES);
        return nfTCR_SEALED_KEY_DATA_TOO_LARGE;
    }
	...
	if ( teeHmacSha256( WorkBuf, HmacLength, caEncHMACKey,
...
#else
    // compute HMAC over version + IV + encrypted data
    if ( teeHmacSha256( WorkBuf, HmacLength, caInternalHMACKey,
                caHMAC_SHA256_KEY_SIZE_BYTES, HmacValue ) != caHMAC_SHA256_KEY_SIZE_BYTES )
    {
        CRYPTODBG("%s: internal error computing HMAC\n", __FUNCTION__);
        return nfTCR_INTERNAL_ERROR;
	...
	{// Data Decrypted By H/W
 		//printf("\n\n@@@@ (%s) Encrypt By DI_MI_XXX(SealedKey Data) @@@@\n\n", __FUNCTION__);
		//BlockSize is 16.
 		uint8_t *bufferptr = (uint8_t *)DI_MI_DECRYPTOR_SOC_Malloc((uint32_t)EncryptedDataLength);
		memset(bufferptr, 0, (uint32_t) EncryptedDataLength);
		memcpy((void *)bufferptr, (void *)BytePtr, (uint32_t) EncryptedDataLength);
		DI_MI_DECRYPTOR_SOC_OTPKEY_DoDecrypt((unsigned char *)bufferptr, (unsigned char *)bufferptr, (unsigned int) EncryptedDataLength);
		memcpy(KeyBuf, bufferptr, (uint32_t) EncryptedDataLength);
		DI_MI_DECRYPTOR_SOC_Free(bufferptr);
    ...
    DecryptedDataLength = teeAes128CbcDecrypt( BytePtr, EncryptedDataLength,
...
#define caCONCAT_KEY_LEN (caKPE_LEN + caKPH_LEN) // concatentate Kpe|Kph
    ...
    rc = teeHmacSha256( ConcatKeys, caCONCAT_KEY_LEN, Salt, caKPW_LEN, FirstResult );
    if ( rc != caSHA256_LEN )
    {
    ...
    rc = teeHmacSha256( Info, caKPW_LEN, FirstResult, caSHA256_LEN, SecondResult );
    if ( rc != caSHA256_LEN )
    {
 ...
 *
 *********************************************************************/
static nfTeeCryptoResult_t caDeriveNflxDhKeys( nfTeeClientId_t     ClientId,
...
#define caSHA384_LEN 48
    ...
    // compute HmacKey
    if ( teeSha384( KeyDerivationKeyPtr, KeyDerivationKeyLength, HmacKey) != 0 )
    {
    ...
    // now, compute Kce|Kch
    if ( teeHmacSha384( SharedSecretPtr, SharedSecretLength,
    ...
    // finally, insert key
    Result = caInsertKey( ClientId, KceHandle, &KeyContext, nfKT_SECRET, nfCA_AES_CBC,
    ...
    Result = caInsertKey( ClientId, KchHandle, &KeyContext, nfKT_SECRET, nfCA_HMAC,
    ...
    Result = caInsertKey( ClientId, KcdHandle, &KeyContext, nfKT_SECRET,
        ...
        nfCryptoKeyHandle_t    *KcdHandlePtr )
{
    ...
    Result = caGetKeyInfoFromHandle( ClientId, KeyDerivationKeyHandle, &KeyType, &Algorithm, &KeyUsageFlags );
    if ( Result != (nfTeeCryptoResult_t) nfTGR_SUCCESS )
    {
    ...
    if ( ( ( Algorithm != nfCA_NFLX_DH ) && ( Algorithm != nfCA_AES_KW ) )
      || ( KeyType != nfKT_SECRET ) )
    {
        CRYPTODBG("%s: inconsistent key policy (%d/%d/%d) for kdk\n", __FUNCTION__, Algorithm, KeyType, KeyUsageFlags );
        Result = nfTCR_OPERATION_NOT_ALLOWED;
        goto ERROR;
    }
    ...
    rc = teeComputeDhSharedSecret( DhContextPtr->Generator,
    ...
    if ( Result != (nfTeeCryptoResult_t) nfTGR_SUCCESS )
    {
        CRYPTODBG("%s: failed key derivation\n", __FUNCTION__);
        goto ERROR;
    }
...
// NOTREACHED
ERROR:
    if ( KcdHandle != caKeyInvalidHandle ) (void) caDeallocateKeyHandle( KcdHandle );
    if ( KchHandle != caKeyInvalidHandle ) (void) caDeallocateKeyHandle( KchHandle );
    if ( KceHandle != caKeyInvalidHandle ) (void) caDeallocateKeyHandle( KceHandle );
    return Result;
...
{
    nfCryptoKeyHandle_t     PrivKeyHandle = caKeyInvalidHandle;
    nfKeyContext_t          KeyContext;
    int                     rc;
    ...
    if (ModulusLength > sizeof KeyContext.ctxt.dhContext.Modulus) {
        CRYPTODBG("%s: modulus too large to store (%lu|%zu)\n", ModulusLength, sizeof KeyContext.ctxt.dhContext.Modulus);
        Result = nfTCR_BAD_PARAMETER;
        goto exit_err;
    }
...
/* NOTREACHED */
exit_err:
    if ( caKeyInvalidHandle != PrivKeyHandle ) (void) caDeallocateKeyHandle( PrivKeyHandle );
    return Result;
}/* end caGenDhKeys */
        ...
        uint32_t            DeletedHandlesList[] )
{
    ...
    if ( TotalKeyCount == 0 )
    {
        CRYPTODBG("%s: no keys to delete for this client ID.\n", __FUNCTION__ );
        *NumHandlesPtr = 0;
        return (nfTeeCryptoResult_t) nfTGR_SUCCESS;
    }
