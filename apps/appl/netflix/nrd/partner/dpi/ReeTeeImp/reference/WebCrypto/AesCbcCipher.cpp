...
//
//////////
AesCbcCipher::AesCbcCipher(const DataBuffer& key, const DataBuffer& iv)
    : key_(key), iv_(iv), keyLength_(static_cast<KeyLength>(key.size()))
{
    assert(!key_.isEmpty());
...
//
//////////
bool AesCbcCipher::encrypt(const DataBuffer& in, /*out*/DataBuffer& out)
{
...
//
//////////
bool AesCbcCipher::decrypt(const DataBuffer& in, /*out*/DataBuffer& out)
{
    // according to the openssl docs:
    // the amount of data written may be as large as (in.size() + cipher_block_size - 1)
    size_t outSize = in.size() + AES_BLOCK_SIZE - 1;
