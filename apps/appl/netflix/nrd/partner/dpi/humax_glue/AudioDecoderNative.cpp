...
{
    m_stSpecificData.extradata_size = 0;
    m_stSpecificData.extradata = NULL;
#if 0 /* dragon */
...
{
    if(m_stSpecificData.extradata_size > 0)
    {
        if(m_stSpecificData.extradata != NULL)
        {
            delete [] m_stSpecificData.extradata;
            m_stSpecificData.extradata = NULL;
        }
    }
                                ...
                                AudioESPlayerNative::DecodedAudioBuffer& decodedAudioBuffer)
{
    ...
    if(pSampleAttr != NULL)
    {
        if(pstUVAWriteFrame != NULL)
        {
            ...
            pstUVAWriteFrame->pts = pSampleAttr->getPTS();
            ...
            pstUVAWriteFrame->ext_size = m_stSpecificData.extradata_size;
            pstUVAWriteFrame->ext_data = m_stSpecificData.extradata;
        }
        else
        {
            Log::error(TRACE_MEDIAPLAYBACK, "@@@@@ [AudioDecoderNative::%s:%d] pstUVAWriteFrame is NULL\n", __func__, __LINE__);
        }
    }
        ...
        if (got_frame_ptr>0){
            ...
            if (decodedBytes > capacity){
                ...
                if (codecContext_->sample_fmt!=AV_SAMPLE_FMT_S16){  //need resample
                    ...
                    int numSample = swr_convert(swr_,
                                &outputBuffer,
                                decoded_frame_->nb_samples,
...
bool AudioDecoderNative::updateAttributes(const AudioAttributes* attributes)
{
    ScopedMutex scopedMutex(DecoderLock::mutexForOpenClose());
    bool bRet = true;
    uint32_t unCodecTag = 0;
    int32_t nCsoAudioCodec = 0;
    
    if(attributes == NULL)
    {
        Log::error(TRACE_MEDIAPLAYBACK, "@@@@@ [AudioDecoderNative::%s:%d] attributes is NULL\n", __func__, __LINE__);
        bRet = false;
        goto func_exit;
    }
    ...
    {
        m_stSpecificData.extradata_size = attributes->mCodecSpecificDataLen;
    }
    
    if(m_stSpecificData.extradata_size > 0)
    {
        m_stSpecificData.extradata = new unsigned char[m_stSpecificData.extradata_size + FF_INPUT_BUFFER_PADDING_SIZE];
        if(m_stSpecificData.extradata == NULL)
        {
            Log::error(TRACE_MEDIAPLAYBACK, "@@@@@ [AudioDecoderNative::%s:%d] m_stSpecificData.extradata is NULL\n", __func__, __LINE__);
            bRet = false;
            goto func_exit;
        }
        memcpy(m_stSpecificData.extradata, attributes->mCodecSpecificData, attributes->mCodecSpecificDataLen);
    }

#if 0
    unCodecTag = MediaUtils::getFourCCFromCodecParam(attributes->mCodecParam);
