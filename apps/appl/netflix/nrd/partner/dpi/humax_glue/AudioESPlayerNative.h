...
public:
...
#if 1
    virtual void setVolume(double target, uint32_t duration, IAudioMixer::Ease ease); // TODO
    virtual double getVolume();
#endif

    bool isDisabledOrPending();
    double getLastCurrentAudioPts();
    void setLastCurrentAudioPts(double dblCurPts);

    static AudioESPlayerNative* getAudioPlayer() { return s_hAudioPlayer; }
    
    static void MutexLock()
    {
        int32_t nRet = 0;
    
        nRet = pthread_mutex_lock(&s_mtxAudioPlayer);
        if(nRet)
        {
            NTRACE(TRACE_MEDIAPLAYBACK, "@@@@@ [%s:%d] Failed to lock s_mtxAudioPlayer, nRet = %d\n", __func__, __LINE__, nRet);
        }
    }
    ...
    static void MutexUnlock()
    {
        ...
        nRet = pthread_mutex_unlock(&s_mtxAudioPlayer);
        if(nRet)
        {
            NTRACE(TRACE_MEDIAPLAYBACK, "@@@@@ [%s:%d] Failed to unlock s_mtxAudioPlayer, nRet = %d\n", __func__, __LINE__, nRet);
        }
    }
...
private:
  friend class PlaybackGroupNative;
    AudioESPlayerNative(unsigned int session_id);
    ...
    bool getAudioCodecFromAttributes(const ISampleAttributes *pSampleAttr, int32_t *pnCsoAudioCodec);
    
    shared_ptr<SampleWriterNative> mSampleWriter;
    shared_ptr<AudioDecoder> mAudioDecoder;
    ...
    double                        m_dblLastCurAudioPts;
    
    static AudioESPlayerNative   *s_hAudioPlayer;
    static pthread_mutex_t        s_mtxAudioPlayer;
};
