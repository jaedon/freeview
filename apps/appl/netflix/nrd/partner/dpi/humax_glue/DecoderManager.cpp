...
#endif
*/
#if 1 /* defined(BUILD_HUMAX_AUDIO_DECODER) */
...
#elif defined(BUILD_HUMAX_AUDIO_DECODER)
#   warning "audio decoder! --> HUMAX"
    audioDecoder.reset(new AudioDecoderNative(numOutputChannels));
#else
...
shared_ptr<VideoDecoder> DecoderManager::createVideoDecoder(const VideoAttributes& attributes,
                                                            ...
                                                            uint32_t pipelineId)
#else
shared_ptr<VideoDecoder> DecoderManager::createVideoDecoder(const VideoAttributes& attributes, uint32_t pipelineId)
#endif
{
...
/*
    ...
    videoDecoder = VideoDecoderNative::create(attributes, referenceClock, rendererInfo, pipelineId);
#elif defined(BUILD_HUMAX_VIDEO_DECODER)
#   warning "video decoder! --> HUMAX"
    videoDecoder = VideoDecoderNative::create(attributes, pipelineId);
#else
