...
static DeviceLibNative *sDevLibNative = 0;
...
static void SuspendCB(void)
{
    AudioMixerOutputBRCM* mMixerOutputBrcm = 0;
    if(!sDevLibNative)
    {
        return;
    }

    mMixerOutputBrcm = (AudioMixerOutputBRCM*)(sDevLibNative->getAudioMixerOutput().get());
  
    if(mMixerOutputBrcm)
    {
        Log::error(TRACE_DPI, "Stop Mixer by suspend");
        (void)mMixerOutputBrcm->Stop();
    }
}

extern void ApplicationManagerRegisterSuspendCallback( void(*pfnCB)(void) );
    ...
    mutex_(DEVICELIB_MUTEX, "DeviceLibNative")
{
    ...
    NTRACE(TRACE_DPI, "++[DeviceLibNative]++\n");
    ...
    ApplicationManagerRegisterSuspendCallback(SuspendCB);   
#if 0 /* HUMAX : need to review */
    teeStoragePtr_.reset(new TeeApiStorage((uint32_t) theSystem_->getSecureStoreSize()));
#else
...
#ifdef AUDIO_MIXER_SOFTWARE
#ifdef AUDIO_MIXER_OUTPUT_BRCM
    theMixerOutput_.reset(new AudioMixerOutputBRCM);
    theMixer_.reset(new AudioMixerSoftware(theMixerOutput_));
    /* Register audio mixer output to ESManager */
    theElementaryStreamManager_->registerAudioMixerOutput(theMixerOutput_);    
#endif
#endif
...
#else
    textToSpeechPtr_.reset(new TextToSpeechHumax);
#endif
...
DeviceLibNative::~DeviceLibNative()
{
    ...
    NTRACE(TRACE_DPI, ">> NativeBridge::deinit\n");
    NativeBridge::deinit(); // NXL_Deinit

    /* cleanup after nexus release() */
    if (1) ScreenDFBPostCleanup("~DeviceLibNative"); /* To prevent nexus/dfb crash after ScreenDFB cleanup */
...
shared_ptr<IAudioMixerOutput> DeviceLibNative::getAudioMixerOutput()
{
    return theMixerOutput_;
}
...
IDeviceLib *createDeviceLib()
{
    ...
    if( access( "/usr/lib/libnxlink.so", F_OK ) == 0) {
        sleep(5);
    }
#endif
    
    NTRACE(TRACE_DPI, ">> NativeBridge::init\n");
    NativeBridge::init();
    return new DeviceLibNative();
}

shared_ptr<IAudioMixerOutput> createAudioMixerOutput()
{
