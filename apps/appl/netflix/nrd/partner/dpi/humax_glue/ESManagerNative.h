...
typedef struct ESManagerNativeResourceMap
{
...
public:
    ...
    /**
     * HUMAX Playback Resources
     */
    ESManagerNativeResourceMap_t* getResourceMap();
	void requestResourceMap();
	...
	void registerAudioMixerOutput(shared_ptr<IAudioMixerOutput> &mMixerOutputHandle);
	void attachMixerOutput();
	void detachMixerOutput();
	void pauseMixerTransfer();
	void resumeMixerTransfer();
...
private:
    ...
    ESManagerNativeResourceMap_t     m_stResourceMap;
	shared_ptr<IAudioMixerOutput>   mMixerOutput;
};
