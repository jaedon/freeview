...
public:
    ...
    virtual void init(shared_ptr<EventDispatcher> eventDispatcher);
    ...
    /* AVOutput */
    void updateVideoProfiles();
    void updateAudioProfiles();
    void updateSupportedVideoOutput();
    void updateActiveVideoOutput();
    
    bool hasScreenSaver();
    bool isAudioBypass() const;
    bool checkMultiAudioChannel();
    bool check1080pVideoResolution();
    bool hasOutputScart();
    ...
    // For QABridge
    NFErr setVideoPipelineCapabilities(const std::vector<struct VideoPipelineCapability>& videoPipelineCaps);

    // For QABridge
    void setVideoProfileEnvelope(ISystem::CodecProfile maxHEVCProfileEnvelope,
                                 ISystem::CodecProfile maxAVCProfileEnvelope);

    // For QABridge
    void getVideoProfileEnvelope(ISystem::CodecProfile &maxHEVCProfileEnvelope,
                                 ISystem::CodecProfile &maxAVCProfileEnvelope);
    ...
    // For QABridge
    static void setDolbyVisionRendering(bool renderBL, bool renderEL);

    static void getDolbyVisionRendering(bool& renderBL, bool& renderEL);

    static void setDolbyVisionFileDump(std::string fileName);
    ...
    enum IpConnectivityMode mIpConnectivity;
    ...
    shared_ptr<NetworkMonitor>  mNwMon;

/*
 *  End of network
 */
    /*  System volume/output monitor */
    class AVOutputMonitor : public Application::Timer
    {
    public:
        AVOutputMonitor(FileSystem* sys)
            : Application::Timer(2000), //check status every 2 sec
              m_sys(sys)
        {
        }

        virtual std::string describe() const { return "DPI::AVOutputMonitor"; }
        virtual void timerFired();
        /* bool checkAVOutput(); */
    private:
        FileSystem* m_sys;
    ...
    //AVOutput monitor members
    shared_ptr<AVOutputMonitor>  mAVOutMon;
 ...
 */
    ...
    /**
     * get display panel size in cm (not inch)
     */
    virtual void getDisplaySize(uint32_t& widthInCm, uint32_t& heightInCm);

    void setTeeStorage(shared_ptr<TeeApiStorage> teeStoragePtr);
private:
    // RAM copy of the secure storage.
    std::map<std::string, std::string> mLastMap;
#if 0
    static void onHdmiEventCallback(uint32_t p1, uint32_t p2, uint32_t p3);
#endif
...
protected:
    ...
    std::vector<ContentProfile> mAudioDecodeProfiles; // TODO: check
    std::vector<EaseProfile>    mAudioEaseProfiles;   // TODO: check
     ...
     * Dual Video
     */
    std::vector<struct VideoPipelineCapability> mVideoPipelineCapabilities;
    ISystem::CodecProfile mQaBridgeHEVCProfileEnvelope;
    ISystem::CodecProfile mQaBridgeAVCProfileEnvelope;
...
private:
    ...
    bool mHasDDPlus; /* HAVE_DDPLUS */
    bool mSupport3DVideo;
    bool mSupport1080PVideo;
    bool mAudioBypass;
    bool mScreensaverOn;
