...
int ManufSS::createContent(std::string &idFilename, std::string &drmDir, DataBuffer &content)
{
    int         rc;
    int         bytesToWrite;
    ...
    NTRACE(TRACE_TEE, "calling AddMgmtSection().\n" );
    ...
    NTRACE(TRACE_TEE, "calling AddNtbaSection().\n" );
    ...
    {
        NTRACE(TRACE_TEE, "calling AddDrmSection().\n" );
    ...
    NTRACE(TRACE_TEE, "Calculating pad.\n" );
    ...
    NTRACE(TRACE_TEE, "padLength is %d\n", padLength );
    ...
    NTRACE(TRACE_TEE, "secstore file is %d bytes\n", bytesToWrite );
    p = fileBuffer;
    content.append<uint8_t>(p, bytesToWrite);
    NTRACE(TRACE_TEE, "secstore file is written to output buffer.\n");
...
int ManufSS::addMgmtSection( uint8_t **p_data, uint32_t *p_remaining_bytes )
{
    ...
    NTRACE(TRACE_TEE, "sectionLength is %d\n", sectionLength );
...
int ManufSS::processEsn(FILE *ifp, uint8_t **p_data, uint32_t *p_remaining_bytes) {
    ...
    NTRACE(TRACE_TEE, "XXX - what about esn validation???\n" );
...
int ManufSS::addNtbaSection( uint8_t **p_data, uint32_t *p_remaining_bytes, std::string &idFilename )
{
    // no mercy
...
int ManufSS::addDrmSectionItem( uint8_t **p_data, uint32_t *p_remaining_bytes, std::string &drmDir, const char *fileName, drm_RO_AgentClientKeys keyId ) {
    ...
    NTRACE(TRACE_TEE, "opening >%s< for reading.\n", fileName );
    ...
    NTRACE(TRACE_TEE, ">%s< is %zu bytes in length.\n", fileName, dataSz );
...
int ManufSS::addDrmSection( uint8_t **p_data, uint32_t *p_remaining_bytes, std::string &drmDir)
{
    // no mercy
    if ((p_data == NULL) || (*p_data == NULL)
          || (p_remaining_bytes == NULL)
                                     ...
                                     uint8_t     *aesKeyPtr )
{
    ...
    NTRACE(TRACE_TEE, "calling AES_cbc_encrypt(%p, %p, %d )\n", cleartextPtr, temp_data, cleartextLength );
                         ...
                         uint8_t     *resultPtr )
{
    ...
    NTRACE(TRACE_TEE, "called with %d bytes of data, %d-byte key.\n",
    ...
    NTRACE(TRACE_TEE, "HMAC length is %d\n", length );
