...
uint32_t MediaUtils::getFourCCFromCodecParam(const std::string& codecParam)
{
    ...
    if(codecParam.compare("mp4a.40.2, mp4a.40.5") == 0)
    {
        return 0x1610; // MEDIASUBTYPE_MPEG_HEAAC
    } else if(codecParam.compare("ec-3.A6") == 0)
    {
        return 0xeac3;  // DOLBY DIGITAL PLUS
    } else if (codecParam.compare("ovrb") == 0)
    {
        // FIX-ME: Haven't verified that this is the
        // http://tools.ietf.org/search/draft-gellens-mime-bucket-bis-09 way to
        // specify vorbis audio.
    ...
    Log::error(TRACE_MEDIAPLAYBACK, "PlaybackDeviceNative::getFourCCFromCodecParam(): "
