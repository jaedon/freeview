...
DRM_VOID* PlayReadyDrmSystem::oemContext_;
Mutex PlayReadyDrmSystem::drmAppContextMutex_(DRMSYSTEM_MUTEX, "DrmStore");
extern "C" DRM_VOID * Oem_Create(void);
extern "C" DRM_VOID Oem_Destroy( DRM_VOID *f_pOEMContext);
...
static int8_t do_check_file_exist(const char *pathname)
{
	struct stat st;
  ...
  :mSessionId(0)
{
	...
	oemContext_ = NULL;
#endif
    const std::string dir = "/var/lib/humaxtv/drm/playready25";//dataDirectory + "/dpi/playready";
    const std::string store = "/var/lib/humaxtv/drm/playready25/sample.hds";//dataDirectory + "/dpi/playready/storage/" + STORE_FILENAME;
...
#endif
	if(do_check_file_exist("/var/lib/humaxtv/drm/playready25/playready25.bin") != TRUE)
	{
	...
	oemContext_ = (DRM_VOID*)Oem_Create();
	if(oemContext_ == NULL)
	{
		printf("Oem_Create failed!\n");
	}
...
NFErr PlayReadyDrmSystem::initialize()
{
    ...
    if( oemContext_ != NULL )
    {
        err  = Drm_Initialize(appContext_.get(), oemContext_,
#if defined (PLAYREADY2)
...
#if defined (PLAYREADY2)
	CSO_UVA_GetSecureClock(appContext_.get());
