...
class PlayReadyDrmSystem : public IDrmSystem, public enable_shared_from_this<PlayReadyDrmSystem>
{
public:
    PlayReadyDrmSystem();
    ...
    virtual NFErr destroyDrmSession(shared_ptr<IDrmSession> drmSession);
    ...
    virtual netflix::NFErr getSecureStop(const std::vector<unsigned char> &sessionID,
                                         /*out*/shared_ptr<ISecureStop> &secureStop);
     ...
     * helper functions
     */
    static shared_ptr<DRM_APP_CONTEXT> getDrmAppContext();
...
private:
    ...
    static shared_ptr<DRM_APP_CONTEXT> appContext_;
	static DRM_VOID* oemContext_;
