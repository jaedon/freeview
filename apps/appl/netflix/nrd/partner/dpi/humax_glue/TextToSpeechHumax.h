...
#ifndef TEXTTOSPEECH_HUMAX
#define TEXTTOSPEECH_HUMAX
...
class TextToSpeechHumax : public ITextToSpeech
{
public:
    TextToSpeechHumax();
    virtual ~TextToSpeechHumax();

    virtual void setListener(const shared_ptr<Listener>& listener);

    virtual Capabilities getCapabilities() const;

    //virtual void say(unsigned int id, const std::string& text, Pitch pitch, Rate rate);
