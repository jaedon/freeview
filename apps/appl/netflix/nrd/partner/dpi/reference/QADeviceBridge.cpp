...
#define HANDLE_CAPABILITY(type, cap)                            \
    ...
    HANDLE_CAPABILITY(bool, hasOnScreenKeyboard);
...
Variant QADeviceBridge::setSupportedVideoOutput(const Variant &data)
{
    ...
    // here, just for QA testing, event is sent
...
Variant QADeviceBridge::setActiveVideoOutput(const Variant &data)
{
   ...
   // tokenize the arguments
    ...
    StringTokenizer::tokenize(videoOutputStates, videoOutputState, ";");
...
Variant QADeviceBridge::setProfiles(const Variant &data)
{
    ...
    vector<string> avail;
    StringTokenizer::tokenize(profiles, avail, ";");

    for (vector<string>::iterator av = avail.begin(); av != avail.end(); ++av) {
        vector<string> tokens;
        StringTokenizer::tokenize(*av, tokens, ",");
...
Variant QADeviceBridge::setVideoPipelineCapabilities(const Variant &data)
{
    NFErr err;
    ...
    /*
      ...
      var videoPipelineCapability2 = {maxHEVCCodecProfile:9, maxAVCCodecProfile:3};
      var videoPipelineCapabilities = [videoPipelineCapability1, videoPipelineCapability2];
      nrdp.qa.device.setVideoPipelineCapabilities(videoPipelineCapabilities);
     */
    bool ok;
    Variant result;
    if ( !data.contains("videoPipelineCapabilities") ){
        return Variant("invalidArgumentError", "videoPipelineCapabilities");
    }
    Variant obj = data["videoPipelineCapabilities"];
    Variant::ArrayIterator it;
    for (it = obj.arrayBegin(); it != obj.arrayEnd(); ++it)
    {
    ...
    err = mSystem->setVideoPipelineCapabilities(videoPipelineCapabilities);
    if(err.ok()){
        mSystem->mEventDispatcher->capabilityChanged();
        return Variant("result", true);
    } else {
        return Variant("result", false);
    }
}

Variant QADeviceBridge::setVideoProfileEnvelope(const Variant &data)
{
    // read out maxHEVC/maxAVC
    bool ok;
    const ISystem::CodecProfile maxHEVC
        = (ISystem::CodecProfile)data.mapValue<int>("maxHEVCProfileEnvelope", &ok);
    const ISystem::CodecProfile maxAVC
        = (ISystem::CodecProfile)data.mapValue<int>("maxAVCProfileEnvelope", &ok);

    mSystem->setVideoProfileEnvelope(maxHEVC, maxAVC);
    return Variant("result", true);
}

Variant QADeviceBridge::getVideoProfileEnvelope(const Variant &data)
{
    NRDP_UNUSED(data);
    ISystem::CodecProfile maxHEVC;
    ISystem::CodecProfile maxAVC;
    mSystem->getVideoProfileEnvelope(maxHEVC, maxAVC);
    Variant map;
    map["maxHEVCProfileEnvelope"] = maxHEVC;
    map["maxAVCProfileEnvelope"] = maxAVC;
    return map;
}

Variant QADeviceBridge::setDolbyVisionRendering(const Variant &data)
{
    bool ok;
    const bool renderBL = data.mapValue<int>("renderDolbyVisionBL", &ok);
    const bool renderEL = data.mapValue<int>("renderDolbyVisionEL", &ok);
    mSystem->setDolbyVisionRendering(renderBL, renderEL);
    return Variant("result", true);
}

Variant QADeviceBridge::getDolbyVisionRendering(const Variant &/*data*/)
{
    bool renderBL;
    bool renderEL;
    mSystem->getDolbyVisionRendering(renderBL, renderEL);
    Variant map;
    map["renderDolbyVisionBL"] = renderBL;
    map["renderDolbyVisionEL"] = renderEL;
    return map;
}

Variant QADeviceBridge::setDolbyVisionFileDump(const Variant &data)
{
    const std::string fileName =  data.mapValue<std::string>("fileName");
    mSystem->setDolbyVisionFileDump(fileName);
    return Variant("result", true);
}
...
#define MethodList(Method)                      \
    ...
    Method(getVideoProfileEnvelope)             \
    Method(sendCommand)                         \
    ...
    Method(setVideoPipelineCapabilities)        \
    Method(setVideoProfileEnvelope)             \
    Method(setVolumeControlType)                \
    Method(toggleScreensaver)

MethodTable(QADeviceBridge, MethodList);

unsigned QADeviceBridge::requestFlags(const Request &request) const
{
    switch (request.method()) {
    case Methods::QADeviceBridge::getVideoProfileEnvelope:
    case Methods::QADeviceBridge::getDolbyVisionRendering:
...
QADeviceBridge::QADeviceBridge(shared_ptr<FileSystem> system)
    : NfObject("device", Methods::QADeviceBridge::methods, Methods::QADeviceBridge::MethodCount),
      mSystem(system)
{
}
...
Variant QADeviceBridge::invoke(int method, const Variant &data)
{
    ...
    case Methods::QADeviceBridge::getAuthenticationType: {
    ...
    case Method::setSubtitleSupported: {
        ...
        break; }
#endif
    case Methods::QADeviceBridge::configurePreshared: {
        vm = configurePreshared(data);
        break; }
#if 0
    case Methods::QADeviceBridge::configureWeb: {
        vm = configureWeb(data);
        break; }
#endif
    case Methods::QADeviceBridge::configureModelGroupKey: {
        vm = configureModelGroupKey(data);
        break; }
    case Methods::QADeviceBridge::setLanguage: {
        vm = setLanguage(data);
        break; }
    case Methods::QADeviceBridge::setCapability: {
        vm = setCapability(data);
        break; }
    case Methods::QADeviceBridge::toggleScreensaver: {
        vm = toggleScreensaver(data);
        break; }
    case Methods::QADeviceBridge::setProfiles: {
        vm = setProfiles(data);
        break; }
    case Methods::QADeviceBridge::sendCommand: {
        vm = sendCommand(data);
        break; }
    case Methods::QADeviceBridge::setVideoOutputResolution: {
        vm = setVideoOutputResolution(data);
        break; }
    case Methods::QADeviceBridge::getIpConnectivityMode: {
        vm = getIpConnectivityMode( data );
        break; }
    case Methods::QADeviceBridge::setIpConnectivityMode: {
        vm = setIpConnectivityMode( data );
        break; }
    case Methods::QADeviceBridge::setSupportedVideoOutput: {
        vm = setSupportedVideoOutput( data );
        break; }
    case Methods::QADeviceBridge::setActiveVideoOutput: {
        vm = setActiveVideoOutput( data );
        break; }
    case Methods::QADeviceBridge::setVolumeControlType: {
        vm = setVolumeControlType( data );
        break; }
    case Methods::QADeviceBridge::setVideoPipelineCapabilities: {
        vm = setVideoPipelineCapabilities ( data );
        break; }
    case Methods::QADeviceBridge::setVideoProfileEnvelope: {
        vm = setVideoProfileEnvelope ( data );
        break; }
    case Methods::QADeviceBridge::getVideoProfileEnvelope: {
        // this is synchronous
        return getVideoProfileEnvelope( data );
    }
    case Methods::QADeviceBridge::setDolbyVisionRendering: {
        vm = setDolbyVisionRendering( data );
        break;
    }
    case Methods::QADeviceBridge::getDolbyVisionRendering: {
        return getDolbyVisionRendering( data );
    }
    case Methods::QADeviceBridge::setDolbyVisionFileDump: {
        vm = setDolbyVisionFileDump( data );
        break;
    }
