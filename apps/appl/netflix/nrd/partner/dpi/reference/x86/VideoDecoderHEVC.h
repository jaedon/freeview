...
/**
 * @class VideoDecoderHEVC VideoDecoderHEVC.h
 * @brief Wrapper class of HEVC video decoders
 */
class VideoDecoderHEVC : public VideoDecoder
{
public:
    static shared_ptr<VideoDecoderHEVC> create(const VideoAttributes& attributes,
                                               ReferenceClockNative& referenceClock,
                                               const VideoRenderer::VideoRendererInfo& rendererInfo,
                                               uint32_t pipelineId = 0)
    {
        bool error;
        shared_ptr<VideoDecoderHEVC> decoder;
        decoder.reset(new VideoDecoderHEVC(attributes, referenceClock, rendererInfo, pipelineId, error));
        return decoder;
    }
...
private:
     ...
     */
    VideoDecoderHEVC(const VideoAttributes& attributes,
                     ReferenceClockNative& referenceClock,
                     const VideoRenderer::VideoRendererInfo& rendererInfo,
                     uint32_t pipelineId,
                     bool& error);
