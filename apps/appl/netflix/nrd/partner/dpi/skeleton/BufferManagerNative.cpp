...
BufferManagerNative::BufferManagerNative(uint32_t audioPoolSize,
...
BufferManagerNative::~BufferManagerNative()
{
    close();
}

NFErr BufferManagerNative::open( uint32_t audioPoolUnitSize,
                                 uint32_t videoPoolUnitSize)
{
    NTRACE(TRACE_BUFFERMANAGER, "open");
    ...
    for(i = 0; i < numBufferDataTypes_; i ++)
    {
        ...
        if(bufferPoolDesc_[i].bufferPoolReqSize_ > 0)
        {
            ...
            {
                Log::error(TRACE_BUFFERMANAGER, "BufferManagerNative::open() Error: "
                           "allocation failure for BufferManagerNative buffer pool %d."
...
void BufferManagerNative::close()
{
...
void BufferManagerNative::setBufferPoolUnitSize(uint32_t dataType,
                                                uint32_t unitSize)
{
...
uint32_t BufferManagerNative::getBufferPoolSize(BufferDataType dataType)
{
    uint32_t i = static_cast<uint32_t>(dataType);
    assert(i < numBufferDataTypes_);
...
uint32_t BufferManagerNative::getBufferPoolFreeSpace(BufferDataType dataType)
{
    uint32_t i = static_cast<uint32_t>(dataType);
    assert(i < numBufferDataTypes_);
...
NFErr BufferManagerNative::alloc(BufferDataType dataType,
                                 uint32_t size,
                                 Buffer& buffer)
{
...
NFErr BufferManagerNative::free(Buffer& buffer)
{
