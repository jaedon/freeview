 ...
 * prohibited.
 */
#ifndef BUFFERMANAGERNATIVE_H
#define BUFFERMANAGERNATIVE_H

/** @file BufferManagerNative.h - Reference implementation of the
 * IBufferManager interface.
 *
 * BufferManagerNative is the reference application's implementation of the
 * IBufferManager interface. A device partner may use or modify this header and
 * accompanying .cpp as needed.
 *
 * In the reference application, DeviceLibNative, which is created once when the
 * application starts and persists until the application exits, creates one
 * instance of the buffer manager which persists until DeviceLibNative is
...
/**
 * @class BufferManagerNative BufferManagerNative.h
 * @brief Implementation of device buffer management functionality in native
 *        ansi C/C++ environment.
 */
class BufferManagerNative : public IBufferManager
{
public:
    BufferManagerNative(uint32_t audioPoolSize,
                        uint32_t videoPoolSize);

    virtual ~BufferManagerNative();
