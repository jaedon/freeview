...
Handle_t	TTX_DECODER_Create	(void *pvMemory, const HUINT32 ulLength)
{
	...
	pDec = (TTX_Decoder_t *)AP_Calloc(sizeof(TTX_Decoder_t));
	if (pDec == NULL)
	{
		return HANDLE_NULL;
	}
...
static void local_ttx_ArrangeCache	(TTX_Decoder_t *pDec)
{
	...
	pTimeStamp	= (TTX_TimeStamp_t *)AP_Malloc(sizeof(TTX_TimeStamp_t) * TTX_MAX_PAGE_NUM);
	if (pTimeStamp == NULL)
	{
		return;
	}
...
static void	local_ttx_DebugPacketMonitor (TTX_Decoder_t *pDec, const HUINT8 ucM, const HUINT8 ucY, const HUINT8 aucPacket[])
{
	...
	{
		#if defined(WIN32)
		pDec->fpPacket = fopen("ttx\\packet_monitor.txt", "w");
		#else
