...
var fnMakeRecordingListReferenceItem = function(recordingItemList, channelId, finished) {
    ...
    if (eventIDList.length <= 0) {
        ...
        dsrvEpg.findProgrammes({
            channel : channelId,
            filter : "rfepg", // this function is intended to match the recording item's deeplink values of ipepg.
            ...
            cb : function(result) {
                ...
                if (recordingLen > 0 && ProgrammeLen > 0) {
                    for ( i = 0; i < ProgrammeLen; i += 1) {
                        // Find a same programmeID between recordingList and EPGSearch result.
...
class cDLDetailInfo extends __KERNEL__ implements ifSVCDetailInfo {
    ...
    getProgrammeDetailList(param): void {
        var programmeDetailItem:any = param.programmeDetailItem;
        var i, cb = param.cb;
        ...
        var keyword = null;
        var fnMakeProgrammeItemList = (list) => {
            ...
            if (programmeDetailItemList.length > 1) {
                programmeDetailItemList = this.sortProgrammeDetailList(programmeDetailItemList);
