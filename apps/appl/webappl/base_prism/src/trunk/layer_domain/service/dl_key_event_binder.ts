...
import __KEY_CATCHER__ = require('lib/local/key_catcher')

class cDLKeyEventBinder extends __KERNEL__ implements ifSVCKeyEventBinder {
    private _element: Document;
    private _target: any;
    private _appMgr: any = null;
    private _countRepeat: number = 0;
    private _userListener = null;
    private _fnHookers: Array<(aKeyCode: number, aRepeat: number) => boolean> = [];
    private _listener = (aEvent: KeyboardEvent) => {
        ...
        aEvent.preventDefault();
        this._deliver(aEvent);
    };
    private _listenerKeyUp = (aEvent: KeyboardEvent) => {
        if (aEvent.metaKey) {
            return;
        }
        aEvent.preventDefault();
    ...
    constructor(aServiceName: string) {
        ...
        if (!ENV.isModeRelease) {
            var keyCatcher = new __KEY_CATCHER__(hx,
                (aFx) => {
                    this.reqAddHooker(aFx);
                },
                (aFx) => {
                    this.reqRemoveHooker(aFx);
                }
            );
            ...
            //    });
            //});
            keyCatcher.registerKeyCode([2, 5, 8, 0, 0, 0, 2], "reload web page", () => {
            ...
            //    });
            //});
            keyCatcher.registerKeyCode([5, 9, 4, 5, 3, 4, 5], "reset MDS Cache && authenticate", () => {
                var category = [
                    'meta','image',
                    'moved', 'auth'
                ];
            ...
            //});
            keyCatcher.registerKeyCode([5, 9, 4, 5, 6, 7, 8], "tvportal device reset", () => {
            ...
            keyCatcher.registerKeyCode([5, 9, 4, 5, 1, 2, 3], "tvportal disable", () => {
                hx.svc.TV_PORTAL.disableHumaxServerForTest();
            });
            keyCatcher.registerKeyCode([5, 9, 4, 5, 1, 2, 4], "tvportal enable", () => {
                hx.svc.TV_PORTAL.enableHumaxServerForTest();
            });
            keyCatcher.registerKeyCode([5, 9, 4, 5, 1, 2, 5], "network disconnect", () => {
                hx.svc.SETTING_UTIL.disconnectNetworkForTest();
            });
            keyCatcher.registerKeyCode([5, 9, 4, 5, 1, 2, 6], "network connect", () => {
            ...
            keyCatcher.registerKeyCodeNumber(hx.key.KEY_BLUE, "Blue Key - Voice Assistant", () => {
            ...
            */
            this.reqAddHooker(function(aKeyCode, aRepeat: number) {
    ...
    private _hooking(aKeyCode, aRepeat?) {
        var handled = false;
    ...
    private _deliverInSTB(aEvent: KeyboardEvent) {
        if (!this._hooking(aEvent.which, this._countRepeat)) {
            this._target.onKey(aEvent.which, this._countRepeat++, aEvent.type);
        }
    }
    private _deliverInEmulation(aEvent: KeyboardEvent) {
        if (!this._hooking(aEvent.which)) {
            var appMgr: ApplicationManager = this._appMgr;
            var keyString = hx.key[aEvent.which] || aEvent.which;
            var keyCode = hx.key[keyString];
            var topMost = (<any>appMgr)._topMostApp;
            ...
            if (topMost && topMost._url) {
                var elChildApps: NodeListOf<HTMLIFrameElement> = document.getElementsByTagName('iframe');
                for (var i = 0, len = elChildApps.length; i < len; i++) {
                    if (elChildApps[i].src === topMost._url) {
                        var e: KeyboardEvent = <KeyboardEvent>window.crossBrowser_initKeyboardEvent("keydown", {
                            which: keyCode,
                            keyCode: keyCode
                        });
                        try {
                            if (!elChildApps[i].contentDocument.body.dispatchEvent(e)) {
                                // This KEY should be consumed on emulator
                                //hx.log("info", "[cDLKeyEventBinder] : builtInApp pass the key");
                                //this._target.onKey(keyCode, this._countRepeat, aEvent.type);

                            }
                        } catch(ex) {
                            alert("keyCode: " + hx.key[keyCode] + "[" + keyCode + "] - " + ex.toString());
//                            appMgr.removeOIPFApplication()
                        }
                        break;
                    }
                }
    ...
    private _deliver(aEvent: KeyboardEvent) {
        throw 'overrided';
    }

    bind(aElement: Document, aTarget: any, aUserListener: (aEvent: KeyboardEvent) => void = null) {
        if (!aTarget.onKey) {
