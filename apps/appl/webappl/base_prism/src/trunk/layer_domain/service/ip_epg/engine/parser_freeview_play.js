define(["require", "exports"], function (require, exports) {
    ...
    var util = {
        ...
        programIdParser: function (programId) {
            var ret = {
                onid: '',
                tsid: '',
                svcid: '',
        ...
        parentalRatingParser: function (href) {
            switch (href) {
        ...
        audioPurposeParser: function (nodeList) {
            for (var i = 0; i < nodeList.length; i++) {
                var href = nodeList[i].getAttribute('purpose');
        ...
        verticalSizeParser: function (size) {
            var isHD = false;
    ...
    var parserElement = {
        ondemand: function (ondemand) {
            ...
            if (nodeInstanceDescription) {
                var nodelistGenre = nodeInstanceDescription.querySelectorAll('Genre');
        ...
        programInformation: function (programInformation) {
            if (!programInformation) {
                return null;
            }
            var bd = programInformation.querySelector('BasicDescription');
            ...
            if (keywords) {
                ret.keyword = Array.prototype.map.call(keywords, function (el) {
    ...
    var parserXML = {
        ...
        program: function (xml) {
            var root = xml.documentElement;
            var programeInformation = root.querySelector('ProgramInformation');
            var bd = programeInformation.querySelector('BasicDescription');
            ...
            var ret = {
                ...
                synopsis: '',
                keyword: Array.prototype.map.call(bd.querySelectorAll('Keyword'), function (el) {
                    return el.textContent;
                }),
                genre: [],
                parentalGuidance: {
        ...
        application: function (xml) {
            ...
            for (var i = 0; i < groupInformations.length; i++) {
                distributor[groupInformations[i].querySelector('GroupType').getAttribute('value')](groupInformations[i]);
            }
            var crid, position, groupId, title, xmlAitUrl, imageUrl, relatedMaterials;
            for (var i = 0, groupInformation; i < applications.length; i++) {
                groupInformation = applications[i];
                crid = '';
                position = 9999;
                ...
                for (var j = 0; j < relatedMaterials.length; j++) {
                    ...
                    if (how === 'urn:tva:metadata:cs:HowRelatedCS:2012:19') {
                        imageUrl = material.querySelector('MediaUri').textContent;
                    }
                    else if (how.indexOf('urn:fvc:metadata:cs:ImageVariantCS:2017-02') === 0) {
                        imageUrl = material.querySelector('MediaUri').textContent;
                    }
                    else if (how === 'urn:tva:metadata:cs:HowRelatedCS:2012:10.5') {
        ...
        ondemand: function (xml) {
            ...
            for (i = 0; i < len; i += 1) {
                distributor[groupinfo[i].querySelector('GroupType').getAttribute('value')](groupinfo[i]);
            }
        ...
        category: function (xml) {
            ...
            for (var i = 0, lenGroup = groupInformations.length; i < lenGroup; i += 1) {
                groupInformation = groupInformations[i];
                groupId = groupInformation.getAttribute('groupId');
