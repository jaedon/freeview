...
var dateFormat = function () {
    var token = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTtj])\1?|[LloSZ]|"[^"]*"|'[^']*'/g,
...
//ADD by jihong
dateFormat.set = function(masks, dayNames,monthNames) {
    if(masks !== undefined) {
        var n;
        for(n in masks) {
            if(masks.hasOwnProperty(n)){
                dateFormat.masks[n] = masks[n];
            }
        }
    }
...
Date.prototype.dateAfter = function(time) {
	var a = this;
	return new Date(a.getTime() + time);
};