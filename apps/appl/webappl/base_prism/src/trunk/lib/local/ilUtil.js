define(["require", "exports", "model/model", "pages/util/provider", "pages/util/controls", "layer_domain/service/dl_dsrvNetflix"], function (require, exports, __MODEL__, __PROVIDER__, __CONTROLS__, __NETFLIX__) {
    ///<reference path="../../def/controls.d.ts" />
    ///<reference path="../../def/prism.d.ts" />
    ///<reference path="../../def/sugar.d.ts" />
    var il;
    (function (il) {
        il.castNumber = function (n) {
            var one = 1;
            return n * one;
        };
        il.sequence = function (ar) {
            var len = ar.length;
        ...
        il.makeDigitNumber = function (aData, digitNumber) {
            if (digitNumber === void 0) { digitNumber = 4; }
            var str = '', i = 0;
        ...
        il.ani = {
            ...
            getTransition: function () {
                var transitionCmd = '-o-transition';
            ...
            getTransitionDuration: function () {
                var transitionDuration = '-o-transition-duration';
                if (hx.emulMode || $.browser.webkit) {
            ...
            getTransitionEnd: function () {
                var transitionEnd = 'otransitionend';
                if (hx.emulMode || $.browser.webkit) {
            ...
            action: function (aParam /* {targets, cls, cb, type} */) {
                ...
                if (startStyle) {
                    targets[0].css(il.ani.getTransitionDuration(), '0ms');
                    targets[0].css(startStyle);
                }
                aniTimeoutID = setTimeout((function (targets) {
                    return function () {
                        var i, elem;
                        for (i = 0; i < aniCount; i += 1) {
                            ...
                            if (cb) {
                                elem.addEventListener(il.ani.getTransitionEnd(), aniCB(elem, cb), false);
                            }
                            if (transition) {
                                targets[i].css(il.ani.getTransition(), transition);
                            }
                            // add style
                ...
                function aniCB(elem, cb) {
                    return function handler(e) {
                        ...
                        elem.removeEventListener(il.ani.getTransitionEnd(), handler, false);
                    };
                }
                return aniTimeoutID;
            },
            action2: function (aParam /* {targets, cls, cb, type} */) {
                var target = aParam.target, cb = aParam.cb, aniInfo = aParam.aniInfo, aniTimeoutID;
                target.css(il.ani.getTransitionDuration(), '0ms');
                ...
                aniTimeoutID = setTimeout(function () {
                    ...
                    target.css(il.ani.getTransition(), aniInfo.transition);
            ...
            sequence: function (aParam) {
                var that = il.ani, seq = that.sequence, handler = [];
                sequenceLoop(aParam);
                function sequenceLoop(q) {
                    var elem = q.shift(), aniTimeoutID;
             ...
             * @return undefined
             * */
            bind: function (aParam) {
                var start = aParam.start, end = aParam.end, aniClass = aParam.aniClass || 'aniActive';
                aParam.target.bind(il.ani.getTransitionEnd(), function (e) {
        ...
        il.circularArray = function (array) {
            if (array) {
                this.setArray(array);
            }
        };
        il.circularArray.prototype = {
            getArray: function () {
                return this._array;
            },
            ...
            clone: function () {
                var ret = new il.circularArray(this._array.slice(0));
                ret.setCur(this._idxCur);
                return ret;
            }
        };
        il.drawCanvas = function (used, available, xPos, yPos, radiusValue, canvasId) {
        ...
        //from utility.js
        il.isInWizardPage = false;
        il.isGoingOperationFromStandby = false;
        il.criticalTasking = {
            crHDDTasking: false,
            crChannelSearchTasking: false,
            crOTATasking: false
        };
        il.isCriticalTasking = function () {
            var isCr = false, a;
        ...
        il.makeChNumToStr = function (chNum) {
            var a = this;
        ...
        il.slideTextAnimate = function (target, value, interval) {
            target.animate({ scrollLeft: value }, interval, function () {
                target[0].scrollLeft = 0;
            });
        };
        il.convertCountryId = function (countryName) {
            /**********************************************/
        ...
        il.openWaitingDlg = function (layeredGroupControl, callback) {
            var def = Q.defer();
        ...
        il.createProgrammeDetailForSugar = function (param) {
            ...
            if (!param.waitingDlg) {
                waitingDlg = il.openWaitingDlg(parent);
            }
            else {
            ...
            var query = {
                ...
                cb: function (programmeDetailList, activeIndex) {
                    ...
                    dsrvRecordings2.addEventCb('RecordingCompleted', detailDlg, function (param) {
                        var item = param.item;
                    ...
                    dsrvRecordings2.addEventCb('RecordingItemAdded', detailDlg, function (param) {
                        var item = param.item;
                        var pdeRecordingItem = dsrvDetailInfo.createRecordingItem(item);
                        var dataArray = dataProvider.getDataArray();
                    ...
                    dsrvRecordings2.addEventCb('ScheduledItemAdded', detailDlg, function (param) {
                        var item = param.item;
                    ...
                    dsrvRecordings2.addEventCb('RecordingItemUpdated', detailDlg, function (param) {
                        var item = param.item;
                        var pdeRecordingItem = dsrvDetailInfo.createRecordingItem(item);
                        var dataArray = dataProvider.getDataArray();
                        var index = fnFindItemIndex(dataArray, pdeRecordingItem, __MODEL__.IPdeRecordingItem);
                    ...
                    dsrvRecordings2.addEventCb('RecordingItemRemoved', detailDlg, function (param) {
                        var item = param.item;
                        var pdeRecordingItem = dsrvDetailInfo.createRecordingItem(item);
                        var dataArray = dataProvider.getDataArray();
                        var index = fnFindItemIndex(dataArray, pdeRecordingItem, __MODEL__.IPdeRecordingItem);
                    ...
                    dsrvRecordings2.addEventCb('ScheduledItemRemoved', detailDlg, function (param) {
                        var item = param.item;
                        var pdeScheduledItem = dsrvDetailInfo.createScheduledItem(item);
            ...
            detailDlg.onRequestRemoveRecording = function (item, forced) {
                ...
                var fnDeleteRecording = function () {
                    if (!item) {
            ...
            detailDlg.fnGetSameSeriesScheduleList = function (item) {
                var i, j, k;
                var ret = [];
            ...
            detailDlg.onRequestMdsSchedule = function (aPdeItem, aIndex) {
                ...
                if (sourceItem && sourceItem.channel) {
                    hx.svc.IP_EPG.getSchedules(sourceItem.channel.sid, sourceItem.getStartTime() * 1000, 6).then(function (res) {
                        ...
                        if (onNowLegacyId) {
                            for (var i = 0; i < length; i++) {
                                ...
                                if (onNowLegacyId === mdsLegacyId) {
                                    ...
                                    hx.svc.IP_EPG.getDetailInfo(crid).then(function (info) {
                                        data.program = info.querySelector('ProgramInformation');
                                        aPdeItem.setMdsData({
                                            mdsData: data,
                                            launchLocation: 'epg'
                                        });
                                        aPdeItem.getThumbnailUrl(function () {
        ...
        il.launchApp = function (name, appId, fnError, where, data) {
        ...
        il.openDeepLink = function (name, url, onLoaded, onUnloaded, onRequestChangePage, where) {
            il.clearPage(where);
        ...
        il.isOverMaxResolution = function (resolution) {
            var ret = false;
            if (resolution) {
        ...
        il.getSubtitleLanguageText = function (code, index) {
            var text = '';
            switch (code) {
                case 'en-EN':
                case 'en-US':
                ...
                if (that.bInit === false) {
                    (function () {
                        if (ENV.op === ENV.listOP.AUSDTT) {
                            that.majorGenreList = ['LOC_GENRE_AUS_MOVIE_ID', 'LOC_GENRE_AUS_NEWS_ID', 'LOC_GENRE_AUS_ENTERTAINMENT_ID', 'LOC_GENRE_AUS_SPORT_ID', 'LOC_GENRE_AUS_CHILDRENS_ID', 'LOC_GENRE_AUS_MUSIC_ID', 'LOC_GENRE_AUS_ARTS_CULTURE_ID', 'LOC_GENRE_AUS_CURRENT_AFFAIRS_ID', 'LOC_GENRE_AUS_EDU_INFO_ID', 'LOC_GENRE_AUS_INFOTAINMENT_ID', 'LOC_GENRE_AUS_SPECIAL_ID', 'LOC_GENRE_AUS_COMEDY_ID', 'LOC_GENRE_AUS_DRAMA_ID', 'LOC_GENRE_AUS_DOCUMENTARY_ID'];
                            // not use minorGenre
                            that.minorGenreList = [];
                            that.majorGenreAllIDList = ['LOC_ALL_ID', 'LOC_GENRE_AUS_MOVIE_ID', 'LOC_GENRE_AUS_NEWS_ID', 'LOC_GENRE_AUS_ENTERTAINMENT_ID', 'LOC_GENRE_AUS_SPORT_ID', 'LOC_GENRE_AUS_CHILDRENS_ID', 'LOC_GENRE_AUS_MUSIC_ID', 'LOC_GENRE_AUS_ARTS_CULTURE_ID', 'LOC_GENRE_AUS_CURRENT_AFFAIRS_ID', 'LOC_GENRE_AUS_EDU_INFO_ID', 'LOC_GENRE_AUS_INFOTAINMENT_ID', 'LOC_GENRE_AUS_SPECIAL_ID', 'LOC_GENRE_AUS_COMEDY_ID', 'LOC_GENRE_AUS_DRAMA_ID', 'LOC_GENRE_AUS_DOCUMENTARY_ID'];
                            that.minorGenreAllIDList = [];
                        }
                        else if (ENV.op === ENV.listOP.UKDTT) {
                            that.majorGenreList = ['LOC_GENRE_MOVIE_ID', 'LOC_GENRE_NEWS_AND_FACTUAL_ID', 'LOC_GENRE_AUS_ENTERTAINMENT_ID', 'LOC_GENRE_AUS_SPORT_ID', 'LOC_GENRE_AUS_CHILDRENS_ID', 'LOC_GENRE_AUS_ENTERTAINMENT_ID', 'LOC_GENRE_NEWS_AND_FACTUAL_ID', 'LOC_GENRE_NEWS_AND_FACTUAL_ID', 'LOC_GENRE_EDUCATION_ID', 'LOC_GENRE_LIFESTYLE_ID', '', '', '', '', 'LOC_GENRE_AUS_DRAMA_ID'];
                            // not use minorGenre
                            that.minorGenreList = [];
                            that.majorGenreAllIDList = ['LOC_ALL_ID', 'LOC_GENRE_MOVIE_ID', 'LOC_GENRE_NEWS_AND_FACTUAL_ID', 'LOC_GENRE_AUS_ENTERTAINMENT_ID', 'LOC_GENRE_AUS_SPORT_ID', 'LOC_GENRE_AUS_CHILDRENS_ID', 'LOC_GENRE_EDUCATION_ID', 'LOC_GENRE_LIFESTYLE_ID', 'LOC_GENRE_AUS_DRAMA_ID'];
                            that.minorGenreAllIDList = [];
                        }
                        else {
        ...
        il.getBGColor = function (aImgUrl, aCoordinate_X, aCoordinate_Y) {
        ...
        il.highlightInnerHTML = function (aTarget, aKeyword, aTag, aColor) {
            if (!aTarget || !aKeyword) {
                return null;
            }
            ...
            var textArr = [];
            var startIdx = 0, keywordIdx = 0, targetLen = targetString.length, keyLen = aKeyword.length, remainedText = targetString;
        ...
        il.updatePageBackground = function () {
        ...
        var fnProgramIdParser = function (programId) {
            var ret = {
                onid: '',
                tsid: '',
                svcid: '',
        ...
        il.compareProgrammeID = function (programmeID1, programmeID2) {
            var bSame = false;
    ...
    (function ($) {
        ...
        function _makeMto1CallBack(num, output) {
            var aArgs = [];
    ...
    (function ($) {
        ...
        var getTransitionDuration = function () {
            var transitionDuration = '-o-transition-duration';
            if (hx.emulMode || $.browser.webkit) {
                transitionDuration = '-webkit-transition-duration';
            }
        ...
        $.fn.ani = function (param) {
            var target = this, cb = param.cb, style = param.style, trans = param.trans, cls = param.cls, transVal, delay = trans ? trans.delay : null, aniTimeoutID;
            // Apply Start Style
            ...
            aniTimeoutID = setTimeout(function () {
                ...
                if (elem) {
                    if (trans) {
                        var transitionVal = ['all', (trans.dur || 0) + 'ms', (trans.timing || 'cubic-bezier(0.675, 0.290, 0.000, 0.930)')].join(' ');
    ...
    (function ($) {
        ...
        //        return target;
        //    };
        $.fn.afterTransitionOneShot = function (aCallback, aWithEvent) {
            if (aWithEvent === void 0) { aWithEvent = false; }
            var target = this;
            if (aWithEvent) {
                var element = target[0];
                var durations = ['0'];
