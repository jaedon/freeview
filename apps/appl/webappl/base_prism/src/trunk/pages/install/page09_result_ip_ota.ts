        ...
        if (ENV.op === ENV.listOP.AUSDTT || ENV.op === ENV.listOP.UKDTT) {
            if (state === 'not') {
                if (this._status === 'result_2') {
                    noSearchResultString = '<strong>' + "Unfortunately no channels were found during setup." + '</strong>' +
                        "Please check your aerial. Select PREVIOUS and press OK to try searching for channels again.";
                    ...
                    this._resultSetEl.innerHTML = '<strong>' + "We recommend connecting your box to the internet to make full use of our services." + '</strong>' +
                        "Select FINISH and press OK to start watching Live TV.";
            ...
            if (state === 'not') {
                ...
                if (this._status === 'result_2') {
                    ...
                    this._resultSetEl.style.textAlign = 'center';
                    this._resultSetEl.style.width = '900px';
                    this._resultSetEl.innerHTML = "You have now successfully completed the setup process." +
                        "<br>" +
                        "Select FINISH and press OK to start watching Live TV.";
                }
            }
...
class DetectNewSoftware extends __PAGE_KERNEL__ {
    _parent = null;
    _waiting = null;
    _ssuDialog = null;
    _copyWaiting = null;
    ...
    showSuccessMessageForSES(param, callback) {
        var that = this;
        ...
        if (isForcedOTA(otaType)) {
            that._ssuDialog = hx.msg('ssuDialog', {
                ...
                text : hx.l('LOC_OTA_WILLBE_DESC_ID'), //hx.l('LOC_OTA_FORCED_DESC_ID'),
    ...
    showSuccessMessage(currentVersion, newVersion, cb) {
        var that = this;
    ...
    showSuccessUSBMessage(a, successCB) {
        var that = this;
        var yes = hx.l('LOC_YES_ID'), no = hx.l('LOC_NO_ID');
