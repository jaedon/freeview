...
// file name : pages/live/hotkey.js
...
class cCPgHotkey extends __PAGE_KERNEL__{
    ...
    _on_alKey (aParam : any) {
        var that = this;
        var notifierVisible : boolean = that.getPageVisible('CPgNotifier');
        ...
        var dlgOpened : boolean = notifierVisible || messageOpened || buttonDlgOpened || queryPasswordDlgOpened || selectionDlgOpened || globalDialogOpened;
    ...
    _multiCustomMsgCreate (aCustomCode : any) : void {
        var text : string = hx.l('LOC_MULTICUSTOMMSG_ID') + ' ' + aCustomCode;
    ...
    _getRCUCustomCode (aCommand : string) : void {
        var retObj : any, rcuCustomCode : any = '';
    ...
    _getNetworkAvailable () : boolean {
        return hx.svc.SETTING_UTIL.getNetworkAvailable();
    }

    _isDnsAvailable () : boolean {
    ...
    _getCheckUpTvAppsHelp () : any {
        return hx.svc.WEB_STORAGE.getHelp();
    }
    
    _setCheckUpTvAppsHelp (aParam : any) : any {
     ...
     */
    _pageChange (aPage : string, aPath? : string) : void {
