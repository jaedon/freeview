...
define(["require", "exports", "pages/page_kernel", "model/model", "layer_domain/service/dl_dsrvNetflix", "pages/util/provider"], function (require, exports, __PAGE_KERNEL__, __MODEL__, __NETFLIX__, __PROVIDER__) {
    var cCPgHubProxy = (function (_super) {
        __extends(cCPgHubProxy, _super);
        // There are 5 types and 11 applications to use
        function cCPgHubProxy() {
            _super.call(this);
            this.MAX_DATA_COUNT = 11;
            this.TIMEOUT_DATA_PROCESSING = 10000;
            this._bActive = false;
            this._bVisible = false;
            this._acceptPrivacyPolicy = false;
            this._status = ['Disconnected', 'Disconnected'];
            this._localAreaNet = 'Disconnected';
            this._internet = 'Disconnected';
            this._appList = [];
            this._playerList = [];
            this._contentList = [];
            this._bAppListUpdate = false;
            ...
            this._bRespondedFVC = false; // Freeview Players
            this._bRespondedHMX = false; // Humax Portal Apps
            this._bRespondedTPK = false; // Today's Picks
            this._isProcessingFVC = null; // check for processing Freeview Players
            this._isProcessingHMX = null; // check for processing Humax Portal Apps
            this._isProcessingTPK = null; // check for processing Today's Picks
            //_refreshTimerTPK = null;    // a timeout for refresh of Today's Picks
            this.TOrderPlayerType = ['/promoted_players', '/main_players', '/other_players', '/sub_players', '/etc_players'];
        }
        cCPgHubProxy.prototype.create = function () {
            var _this = this;
            hx.logadd('hubProxy');
            var that = this;
            that.$this = $(this);
            ...
            var help2 = hx.config.help2.tvapps;
            ...
            hx.svc.IP_EPG.addEventCb('getAuthToken', that, function () {
                that._getFVCAppList(true);
                that._getTodaysPicks(true);
            });
            var bmNetworkState;
            var mTvAppsHelpStatus;
            ...
            var fnGetNetworkAvailable = function () {
                return dsrvSettingUtil.getNetworkAvailable();
            };
            var fnIsDnsAvailable = function () {
            ...
            var fnGetCheckUpTvAppsHelp = function () {
                return dsrvWebStorage.getHelp();
            };
            var fnSetCheckUpTvAppsHelp = function (param) {
                return dsrvWebStorage.setHelp({
                    tvapps: param
                });
            };
            var fnChangePage = function (homeMenuItem, path) {
                var result, request, nochannel = false;
                var pageTable = {
                    tvguide: 'CPgGuide',
            ...
            dsrvAppMgr.createApplication({
                ...
                onLoaded: function (appRpcUtil) {
                    ...
                    that._appRpcUtil.notifyHomeMenuSelected(function (homeMenuItem, path) {
            ...
            dsrvSettingUtil.addEventCb('wirelessStopConnect', that, function () {
                ...
                if (dsrvSettingUtil.checkWireConnection() === false && dsrvSettingUtil.checkWirelessConnection() === true) {
                    ...
                    if (that._bVisible === true) {
                        that._appRpcUtil.setHomeData(that._appList, 2 /* EApp */);
                    }
                }
            });
        ...
        cCPgHubProxy.prototype.destroy = function () {
            if (this._imageDlg) {
                this._imageDlg.destroy();
                delete this._imageDlg;
            }
        };
        cCPgHubProxy.prototype.isActive = function () {
            return this._bActive;
        };
        //visible () : boolean {
        //    return this._bVisible;
        //}
        cCPgHubProxy.prototype._requestHomeDataList = function () {
            var that = this;
            that._getFVCAppList();
            that._getTvPortalAppList();
            that._getTodaysPicks(true);
        };
        cCPgHubProxy.prototype._refreshProcessingTimeouts = function () {
            var that = this;
            ...
            if (that._bRespondedTPK) {
                ...
                that._isProcessingTPK = setTimeout(function () {
                    if (that._isProcessingTPK) {
                        clearTimeout(that._isProcessingTPK);
                        that._isProcessingTPK = null;
                    }
        ...
        cCPgHubProxy.prototype._getFVCAppList = function (bForced) {
            var _this = this;
            var that = this;
            ...
            that._isProcessingFVC = setTimeout(function () {
                if (that._isProcessingFVC) {
                    clearTimeout(that._isProcessingFVC);
                    that._isProcessingFVC = null;
                }
            ...
            hx.svc.IP_EPG.getApplication(2 /* EColour_1_1 */).then(function (info) {
                ...
                if (info) {
                    ...
                    for (var key in parsed) {
                        if (that.TOrderPlayerType.indexOf(key) > -1) {
                            fvcPlayers = parsed[key];
                            for (var i = 0; i < fvcPlayers.length; i++) {
                                reqCount++;
                                (function (index, memberOfType, title, contentUrl) {
                                    hx.svc.IP_EPG.getThumbnail(fvcPlayers[i].image, 100).done(function (thumbnailUrl) {
                                        resCount++;
                ...
                var fnSortByOrder = function (list) {
                    list.sort(function (a, b) {
                        ...
                        ret = _this.TOrderPlayerType.indexOf(a.PlayerType) - _this.TOrderPlayerType.indexOf(b.PlayerType);
        ...
        cCPgHubProxy.prototype._getTvPortalAppList = function (bForced) {
            var a = this;
            ...
            a._isProcessingHMX = setTimeout(function () {
                if (a._isProcessingHMX) {
                    ...
                    fn: function (smartSearchAppList) {
                        ...
                        hx.svc.TV_PORTAL.getDefaultAppList({
                            fn: function (defaultAppList) {
                                ...
                                a._appRpcUtil.setHomeData(a._appList, 2 /* EApp */);
        ...
        cCPgHubProxy.prototype._rearrangeAppList = function (aList) {
            var _this = this;
            ...
            for (i = 0; i < length; i += 1) {
                if (!aList[i].PlayerType) {
                    aList[i].PlayerType = (aList[i].ModelAppTypeName === "STBLAUNCHER") ? ((aList[i].name === "Netflix" || aList[i].name === "YouTube") ? '/main_players' : '/sub_players') : '/etc_players';
                }
                homeAppList.push(aList[i]);
            }
            var fnMergeLists = function (oldList, newList) {
                var bSame = false;
                $.each(newList, function (idx1, newElement) {
                    bSame = false;
                    $.each(oldList, function (idx2, oldElement) {
        ...
        cCPgHubProxy.prototype._getTodaysPicks = function (bForced) {
            var that = this;
            if (hx.emulMode) {
            }
            if (!hx.svc.IP_EPG.isAvailableToUseAuth()) {
                                                ...
                                                ondemandItem.getContentOwningLogo({
                                                    updateCb: function (receivedUrl) {
                                                        ...
                                                        if (++resCount >= resultLength) {
                                                            ...
                                                            that._appRpcUtil.setHomeData(that._contentList, 1 /* EContent */);
        ...
        cCPgHubProxy.prototype._networkConnection = function () {
            var that = this;
        ...
        cCPgHubProxy.prototype._checkNetworkStatus = function (paramLocalAreaNet, paramInternet) {
        ...
        cCPgHubProxy.prototype._on_networkStatus = function (type, result) {
            var that = this;
        ...
        cCPgHubProxy.prototype._on_alKey = function (param) {
            var bConsumed = false;
            switch (param.alKey) {
                case hx.key.KEY_ESCAPE:
                case hx.key.KEY_POWER:
        ...
        cCPgHubProxy.prototype._hideHub = function () {
            this._appRpcUtil.hide();
        };
        cCPgHubProxy.prototype._showHub = function () {
            var that = this;
            if (that.startHelp()) {
                return;
            }
        ...
        cCPgHubProxy.prototype._on_alMessageToPage = function (param) {
            var that = this;
            if (param.query === 'show') {
        ...
        cCPgHubProxy.prototype._updateLanguage = function () {
        ...
        cCPgHubProxy.prototype.startHelp = function () {
