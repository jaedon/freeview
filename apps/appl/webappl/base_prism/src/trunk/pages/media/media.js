...
define(["require", "exports", "model/model", "pages/page_kernel"], function (require, exports, __MODEL__, __PAGE_KERNEL__) {
    // file name : pages/media/media.js
    /*jslint sloppy:true, nomen:true, vars:true */
    var cCPgMedia = (function (_super) {
        __extends(cCPgMedia, _super);
        function cCPgMedia() {
            _super.call(this);
            hx.logadd("CPgMedia");
        }
        cCPgMedia.prototype.create = function (aContentType, aInfo) {
            var _this = this;
            var that = this;
            var mSearchInfo = {
            ...
            var fnGetUserItemList = function (aId, aContentType) {
                var items = [];
                var i, length, info, infoList, userItemList = [];
            ...
            var fnRequestMediaItems = function (aServer, aFolder) {
                var fullPath, upperFolder, serverPath, folderPath;
                var waitingDlg, serverId, path, category, keyword;
            ...
            var fnRequestPlay = function (aItemKey, aSelectedItem) {
                hx.log('CPgMedia', hx.util.format('fnRequestPlay() - itemKey : {0}', aItemKey));
                var i, length, item, items = [];
                var selectedIndex = 0;
                var provider = mediaPage.getContentDataProvider();
                var dataArray = provider.getDataArray();
                var index = 0;
                var storageType = mSearchInfo.server.getStorageType();
            ...
            var onContentBrowse = function (aMediaItemList) {
                if (that._waitingDlg) {
                    that._waitingDlg.close();
            ...
            mediaPage.onMediaServerSelected = function (param) {
                var item = param.item;
            ...
            mediaPage.onMediaItemSelected = function (param) {
                var item = param.item;
            ...
            mediaPage.onRequestUserServerList = function (aFinished, aContentType) {
                setTimeout(function () {
            ...
            mediaPage.onRequestLocalServerList = function (aFinished) {
                setTimeout(function () {
                    var i, length, server, serverList = [];
            ...
            mediaPage.onRequestChangePage = function (aParam) {
                var dsrvFrontPanel = hx.svc.FRONT_PANEL;
            ...
            mediaPage.onRequestRemoveRecentItem = function (aParam) {
            ...
            mediaPage.onRequestAddToPlaylist = function (aParam) {
                var i, length, item, count = 0;
                var items = aParam.items;
            ...
            mediaPage.onRequestRemoveToPlaylist = function (aParam) {
                var i, length, item;
                var type, id;
                var items = aParam.items;
                var count = 0;
            ...
            dsrvStorageUtil.addEventCb('DetachProgressOnGoing', that, function (e) {
                hx.log('CPgMedia', 'DetachProgressOnGoing');
            });
            dsrvStorageUtil.addEventCb('DetachProgressFinished', that, function (e) {
                hx.log('CPgMedia', 'DetachProgressFinished');
                mediaPage.showNotificationMsg(hx.l('LOC_STORAGE_REMOVED_SAFELY_ID'));
            });
            dsrvStorageUtil.addEventCb('DetachProgressError', that, function (e) {
                var errMsg = e.errMsg;
                hx.log('CPgMedia', hx.util.format('DetachProgressError - errMsg: {0}', errMsg));
                mediaPage.showNotificationMsg(errMsg);
            });
            dsrvMediaServerManager.addEventCb('ServerAdded', that, function (e) {
                var server = e.server;
            ...
            dsrvMediaServerManager.addEventCb('ServerRemoved', that, function (e) {
                var server = e.server;
                ...
                if (currentServer && currentServer.getStorageType() === storageType) {
                    if (TStorageType.EDlna === storageType && currentServer.getUdn() === server.getUdn()) {
                        ...
                        if (mSearchInfo.response && mNetworkAvailable) {
                            if (_this._mediaPage) {
                                _this._mediaPage.showNotificationMsg(hx.l('LOC_MSG_PLAYBACK_ERROR_04_ID'));
                            }
                        }
                        setTimeout(function () {
                            if (mediaPage) {
            ...
            dsrvMediaItemManager.addEventCb('ContentBrowse', that, function (e) {
                var mediaItemList = e.mediaItemList;
                onContentBrowse(mediaItemList);
            });
            dsrvMediaItemManager.addEventCb('ContentOperate', that, function (e) {
                var i, length, ticket, result, file, fileList;
                ticket = e.ticket;
                result = e.result;
                fileList = e.fileList;
            ...
            dsrvMediaItemManager.addEventCb('MountSucceeded', that, function (e) {
                var server = e.server;
                var name = server.getName();
                var path = server.getPath();
            ...
            dsrvMediaItemManager.addEventCb('MountFailed', that, function (e) {
                var server = e.server;
                var name = server.getName();
                var path = server.getPath();
        ...
        cCPgMedia.prototype.destroy = function () {
        ...
        cCPgMedia.prototype.isActive = function () {
            return this._isVisible;
        };
        //visible () : boolean {
        //    return this._isVisible;
        //}
        cCPgMedia.prototype._setVisible = function (aVisible) {
            this._isVisible = aVisible;
            this.sendEventToEntity({
                alEvent: 'alCheckPageVisible'
            });
        };
        cCPgMedia.prototype._getAvailableLocalServerList = function (aContentType) {
            var localServerList = hx.svc.MEDIA_SERVER_MANAGER.getAvailableLocalServerList({
                contentType: aContentType
            });
            return localServerList;
        };
        cCPgMedia.prototype._getAvailableNetworkServerList = function (aContentType) {
            var networkServerList = hx.svc.MEDIA_SERVER_MANAGER.getAvailableNetworkServerList({
                contentType: aContentType
            });
            return networkServerList;
        };
        cCPgMedia.prototype._exitToLive = function () {
            this.sendEventToEntity({
                alEvent: 'alChangePage',
        ...
        cCPgMedia.prototype._on_alMessageToPage = function (aParam) {
        };
        cCPgMedia.prototype._on_alKey = function (aParam) {
