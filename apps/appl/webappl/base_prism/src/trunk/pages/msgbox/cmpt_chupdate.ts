...
export class chupdate extends __COMPONENT_KERNEL__ {
    MAX_CH_SHOW_NUM = 4;
    FIRST_SCROLL_POSITION = 10;
    LAST_SCROLL_POSITION = 130;
    NEW_TYPE_TOP_POSITION = 118;
    REPLACED_TYPE_TOP_POSITION = 197;
    BOX_ELEMENT_HEIGHT = 183;
    SCROLL_ELEMENT_LEFT = 649;
    TEXT_FONT_SIZE = 27;
    chListManager: any;
    _scrollEl: any;
    _focusElList: any;
    ...
    create(aParam) {
        ...
        $.extend(that, aParam, {
            ...
            _msg_type: aParam.msg_type,     //new or replaced
        ...
        var i: number, len: number = that._btn_title_arr.length;
        var btnEl: any = document.createElement('p');
        for (i = 0; i < len; i += 1) {
    ...
    _show_chList() {
        ...
        //get lastShowIdx
        var lastShowIdx: number = that.chListManager.chList.length - that.chListManager.curIdx;
        ...
        var dtEl: any = document.createElement('dt');
        var ddEl: any = document.createElement('dd');
        for (i = that.chListManager.curIdx; i < lastShowIdx; i += 1) {
            var dtCloneEl: any = dtEl.cloneNode();
            dtCloneEl.innerHTML = that.chListManager.chList[i].name;
            var ddCloneEl: any = ddEl.cloneNode();
    ...
    _set_scrollPosition() {
        if (this.chListManager.curIdx === 0) {
            ...
            var scrollMoveSize: number = (this.LAST_SCROLL_POSITION - this.FIRST_SCROLL_POSITION) / (this.chListManager.chList.length - this.MAX_CH_SHOW_NUM);
    ...
    _on_alKey(aParam) {
        ...
        var chListLen: number = this.chListManager.chList.length;
        var i: number, len: number = this._focusElList.length;
        var focusedEl, focusedIdx: number;
        for (i = 0; i < len; i += 1) {
...
export class selectchlist extends __COMPONENT_KERNEL__ {
    MAX_CH_SHOW_NUM = 6;
    chListManager;
    _focusElList;
    _notiEl;
    ...
    _show_chList(aFocusedIdx) {
        ...
        //get firstShowIdx
        var firstShowIdx: number = this.chListManager.curIdx;
        ...
        var curIdx: number = this.chListManager.curIdx; //store curIdx
        this.chListManager.setCur(firstShowIdx);
        var chListEl: any = document.createElement('p');
        ...
        for (i = 0; i < channelCount; i += 1) {
            ...
            this.chListManager.inc();
            if (i < this.MAX_CH_SHOW_NUM - 1) { //do not focus to lastEl
