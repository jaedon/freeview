...
/*global $ */
class cCPgRecControl extends __PAGE_KERNEL__{
    entity : any;
    NUM_MAX_RECORD_LIST = 1500;
    _reminderMsg : any = null;
    _startMsg : any;
    _scheduledMsg : any;
    _prevScheduledItem : any;
    _notiNumOfRecordingsIsReachingLimitMsg : any;
    _statusMsg : any;
    _failRecordMsg : any;
    _failScheduledRecordMsg : any;
    _subConflictMsg : any;
    _isVisible : boolean;
    _fnAfterChannelChange: () => void;
    _popup : any = null;
    ...
    create (aEntity) : void {'use strict';
        ...
        var _actionExceptionHandling = (cancelItem, requestedScheduledItem) : void => {
            if (cancelItem) {
                    ...
                    finished : function() {
                        setTimeout(function() {
                            if (bTimeBased) {
                                ...
                                if (factoryType !== __MODEL__.eTScheduledFactoryType.ERECORDING) {
        ...
        var _showConflictWindow = (aParam, aFinished) => {
            var _calcModifyEndTime = (aNewSchedule, aConflictList) : number => {
                var i : number, ti : number, cftSubList : any, cftSchedule : any;
                var nowSec : number = hx.il.nowSec();
                var startTime : number = Math.floor(aNewSchedule.getStartTime() / 1000);
                var endTime : number = startTime + aNewSchedule.getDuration();
                var modifyEndTime : number = endTime;
                var bModify : boolean = true;
                ...
                for ( i = 0, ti = aConflictList.length; i < ti; i += 1) {
                    ...
                    if (cftSubList && cftSubList.length) {
                        ...
                        if (cftst < startTime) {//TODO:check padding time
                            bModify = false;
                            break;
                        } else {
                ...
                if (modifyEndTime === endTime) {
                    ...
                    if ((modifyEndTime - startTime) < (5 * 60)) {//5min
                        modifyEndTime = 0;
                    } else {
            ...
            var itemList : any = [head];
            var i : number, ti : number, headScheduledItem : any, subList : any;
            var recName : string;
            var item : any;
            var alProgramme: Programme;
            if (alternativeList) {
                text = tr('LOC_CANNOT_BE_SCHEDULED_PROGRAMME_ID');
                head._one += " : ";
                head._two = "<span>" + this.__makeDispTime({
                    'startTime': Math.floor(requestedScheduledItem.getStartTime() / 1000),
                    'duration': requestedScheduledItem.getDuration(),
                ...
                var CRID: string;
                var index;
                var fnPush = (aItem, aPrefix, aType, aProgramme = null) => {
                    var startTime;
                    var duration;
                    var text;
                    if (aProgramme) {
                    ...
                    var title = "<span>" + this.__makeDispTime({
                        'startTime': startTime,
                        'duration': duration,
                ...
                for (i = 0, ti = conflictList.length; i < ti; i += 1) {
                    ...
                    var oSubList: any = [];
                    if (subList.length > 1) {
                        var j: number, tj: number, subRec: any;
                                ...
                                if (modifyEndTime) {
                                    setTimeout(()=> {
                                        if (fnRetryWithDuration) {
                                            var modifyDuration: number = modifyEndTime - Math.floor(requestedScheduledItem.getStartTime() / 1000);
        ...
        var _showDurationChangeDlg = (aParam, aFinished)=> {
            var that = this;
            var nowSec : number = hx.il.nowSec();
            var scheduledItem : any = aParam.relatedScheduledRecording//
            , targetCh : any = aParam.targetChannel//
            , evtList : any = aParam.eventList//
            , currentEvtIdx : number = aParam.currentEventIndex//
            , currentEvt : any = aParam.currentEvnet;
            //param.relatedRecordingItem
            var endEvt = currentEvt;

            var recDurDlg : any, recDurDlgParam : any;
            var adjDurTime : number = scheduledItem.getDuration(), adjDurTimeByEvt : any;

            var scheduleStartTimeSec : number = Math.floor(scheduledItem.getStartTime() / 1000);
            var scheduleEndTime : number = scheduleStartTimeSec + scheduledItem.getDuration();
            var scheduleEndTimeStr : string  = (new Date(scheduleEndTime * 1000)).format('HH:MM');
            var referTime : number;
            var first_arrow : string;
            var p_text : string;
            ...
            p_text = targetCh.majorChannel + ' ' + targetCh.name//
            + ' (' + (scheduledItem.getStartTime()).format('HH:MM')//
            + '-' + scheduleEndTimeStr//
            + ', ' + Math.round(scheduledItem.getDuration() / 60) + hx.l('LOC_MINS_ID') + ')';
                ...
                'callback_fn' : function(_status, _value) {
                    switch(_status) {
                        ...
                        case 'first_right':
                            ...
                            var endTimeSec : number = scheduleStartTimeSec + adjDurTime;
                        ...
                        case 'second_right':
                            ...
                            var programme : any = evtList[currentEvtIdx];
                            if (programme) {
                                ...
                                recDurDlg.setListTextArrow('second'//
                                , hx.l('LOC_CHANGE_RECDUR_PROG_ID') + ' "' + programme.getName() + '"'//
                                , arrowDirection//
                                , ' (~' + (new Date(programme.getEndTime() * 1000)).format('HH:MM') + ')');
                            }
                            break;
                        case 'ok':
                            var chDurTime = 0;
        ...
        dsrvRecordings.addEventCb('ShowRecordingStopWindow', that, (param) => {
            _fnShowStopRec(param, function() {
                var noWork;
            });
        });
        dsrvRecordings.addEventCb('ShowConflictWindow', that, (param) => {
            _showConflictWindow(param, function (_status, _data) {
                var noWork;
            });
        });

        dsrvRecordings.addEventCb('ShowupTPConflictPopup', that, (param) => {
            showupTPConflictPopup(param);
        });

        dsrvRecordings.addEventCb('NotiFoundRecommendationProgramme', that, (aProgramme: __PROVIDER__.CProgrammeProvider) => {
            var itemList = [];
            var fnPush = (aItem, aPrefix, aType, aProgramme = null) => {
                var title = "<span>" + this.__makeDispTime({
                    'startTime': Math.floor(aItem.getStartTime() / 1000),
        ...
        dsrvRecordings.addEventCb('ShowAlternativeWindow', that, (param) => {
            _showConflictWindow(param, function (_status, _data) {
                var handled = false;
                var newRec: __MODEL__.COipfScheduledItem = param.requestedScheduledItem;
                var fnCancelRecording = (aSchedule) => {
                    if (!aSchedule) {
                ...
                if (_status === 'stop') {
                    var selected: __MODEL__.COipfScheduledItem = _data._recording;
                    ...
                    if (!type) {
                        ...
                        var factoryType = __MODEL__.eTScheduledFactoryType.EALTERNATIVE_RECORDING;
                        if (selected.isSeries()) {
                            factoryType = __MODEL__.eTScheduledFactoryType.EALTERNATIVE_SERIES_RECORDING;
                        }
                        hx.log("info", "[ALTERNATIVE HANDLER] hx.svc.RECORDINGS.removeScheduledRecording");
        ...
        dsrvRecordings.addEventCb('ShowChangeDuration', that, (param) => {
            if(!that._popup){
        ...
        dsrvRecordings.addEventCb('NotiRecordingStarted', that, (param) => {
            ...
            if (hx.svc.AV_CTRL.isLiveOnGoing() === true) {
                ...
                var curChObj : any = hx.svc.CH_LIST.getCurrentChannel();
                if (curChObj.channel) {
                    ...
                    if (bChangeable === false) {
                        ...
                        var channel : any = startedItem.getChannelInstance();
                        if(!!channel) {
        ...
        dsrvRecordings.addEventCb('NotiRecordingStop', that, (param) => {
            var item : any = param.item;
            var isRecordingDeleted : boolean = param.deleted;
            if (that.getCenterPageName() === 'CPgStandby') {
                return;
            }
        ...
        dsrvRecordings.addEventCb('NotiRecordingStartError', that, (param) => {
            var item : any = param.item;
            var reason : any = param.reason;
            var bScheduled : any = param.bScheduled;
            if (that.getCenterPageName() === 'CPgStandby') {
                return;
            }
            switch (reason) {
                ...
                case 'MAX_RECORD_LIST':
                    ...
                    if (bScheduled) {
                        that._failScheduledRecordMsg = hx.msg('ssuDialog', {
                            text : "Recording your scheduled programme has failed because up to 1500" +
                            " recordings are created only. Go to RECORDINGS menu and free up the storage and try again.",
                            first_line_arr : [this.__makeDispTime({
                        ...
                        that._failRecordMsg = hx.msg('confirm', {
                            text : "You cannot record this programme because up to 1500 recordings are created only. Go to" +
                            " RECORDINGS and free up the storage and try again.",
                            auto_close : false,
                            btn_title_arr : ['OK'],
                            callback_fn : (param) => {
                                if (param === 'OK') {
        ...
        dsrvRecordings.addEventCb('NotiScheduledStartError', that, (param) => {
            var item : any = param.item;
            var text : string = null;
            var dlg : any = null;
            var tpConflictParam : any = param.tpConflictParam;
            var bValidOnNetflix;
            ...
            if (!item.isReminder()) {
                if (hx.il.criticalTasking.crOTATasking === true) {// during OTA, does not show rec start error, because
            ...
            var setResult = (item, res : number)=> {
                var channel, bChangeable : boolean = false;
        ...
        dsrvRecordings.addEventCb('ScheduledItemSet', that, (param) => {
    ...
    __makeDispTime(aData, aFormatAlternative = false): string {
        'use strict';
        var startTime : Date, endTime : Date;
        var strTime : string, tempStr : string;
