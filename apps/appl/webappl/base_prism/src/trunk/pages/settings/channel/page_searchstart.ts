...
class cCPgSearchStart extends __KERNEL_SETTINGS__{
    progress : any;
    $tv : any;
    $radio : any;
    $colum : any;
    ...
    _transponderCount : number = 0;
    _isSearching : string = 'true';
    _getChupdateEvt : boolean = false;
    _tvListindex : any = [];
    _radioListindex : any = [];
    ...
    _tempRadioList = [];
    _lockingDataList = [];
    _isLoadPreferList : boolean = false;
    ...
    create () {
        ...
        if (ENV.op !== ENV.listOP.AUSDTT && ENV.op !== ENV.listOP.UKDTT) {
            ...
            var typeObj : any = hx.svc.WEB_STORAGE.getLastSearchType();
            var searchType : string = typeObj.searchType;

            this._searchType = searchType;

            var tpSelectorObj : any = this.dsrvChannelSearch.getTransponderSelectorAll();
    ...
    _on_ChListEvt (aRetObj) {
        ...
        var tvLength : number, radioLength : number, TotalCnt : number;
    ...
    _on_channelScan (aParam) {
        var j : number, k : number, p : number, u : number, satName : string, showString : string;
        var l_antennaId : number;
        var useTvLength : number, useRadioLength : number;
        ...
        var pola : string = '', codeRate : string = '', transpec : number = 0;
        ...
        if (ENV.op !== ENV.listOP.AUSDTT && ENV.op !== ENV.listOP.UKDTT) {
            var defaultSatName : string = 'ASTRA 1 (19.2E) ';
        ...
        if (ENV.op !== ENV.listOP.AUSDTT && ENV.op !== ENV.listOP.UKDTT) {// for 'antennaConnection' function of satellite model, not
            ...
            if (hx.config.antConTpList === 'searched') {
                ...
                if (aParam.signalStrength >= 10) {
                    ...
                    this._lockingDataList.push({
                        antId : aParam.tpInfo.antennaId,
                        tpData : [parseInt(aParam.tpInfo.startFrequency, 10) * 1000, //
                        parseInt(aParam.tpInfo.symbolRate, 10) * 1000, //
                        aParam.tpInfo.polarisation, codeRate, //
                        aParam.tpInfo.modulationModes, transpec, 0]
                    });
                }
            } else if (hx.config.antConTpList === 'hard') {
    ...
    _on_ChannelListUpdate () {
        ...
        } else if (this._searchType === 'scd' || this._searchType === 'motor'
        || (ENV.op !== ENV.listOP.AUSDTT && ENV.op !== ENV.listOP.UKDTT && this._lockingDataList.length <= 0)) {
            svcChannelSearch.saveChannelSearchResult();
    ...
    _on_alClicked (aParam) : boolean {
        var data : any = aParam.alData, targetPage : string = null, bConsumed : boolean = false;
        ...
        if (data.me === this._button) {
            ...
            /* WHEN SAVE BUTTON */
                ...
                /*
                 ...
                 that.dsrvChannelSearch.pauseScan();
                 */
            }
        }
        return bConsumed;
