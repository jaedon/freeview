...
class cCPgSignalTest extends __KERNEL_SETTINGS__{
    TMenuItemSignalTestList = {
        SIGNAL_TUNER1 : 0,
        SIGNAL_TUNER2 : 1,
        SIGNAL_TUNER3 : 2,
        SIGNAL_TUNER4 : 3
    ...
    create () {
        ...
        $('#set .wrap.sigTest').ani({
            ...
            cb : ()=> {
                ...
                hx.svc.SETTING_UTIL.addEventCb('NotiSignalInfo', that, (aParam) => {
    ...
    _on_alClicked (aParam) : boolean {
        var data : any = aParam.alData, target : any = data.me, bConsumed : boolean = false, antennaConnectionStr : string;
        switch (target) {
            case this._doneBtn:
                bConsumed = true;
    ...
    getChNumFromFreq (aFreq : string, aBandwidth : string) : string {
        var chNum : string, length : number, i : number;
        var frequency : number = parseInt(aFreq, 10);
    ...
    getSwitchedSatelliteName (aName) {
        var that = this;

        function _SatNameConverter(name) {
            var convName : string = null;
            var satelNameIndex : number = 0;
