...
class cCPgNetworkServer extends __KERNEL_SETTINGS__{
    TMenuList = {
        NETWORKSERVER : 0,
        RENAMESERVER : 1,
        SERVERSTATUS : 2,
    ...
    _updateTimer : any = undefined;
    _isNetServerOn : boolean = false;
    _serverIpAddress : string = '';
    _isChangePageInSetting : boolean = false;
    _isServerRunning : boolean = false;
    _testConResult : string = '';
    _retObj : any;
    _startHour : string;
    _startMin : string;
    _endHour : string;
    ...
    _okBtn : any;
    _restartBtn : any;
    _networkMainMenu : any;
    _onTimerMainMenu : any;
    _$noti : any;
    _isServerActivationTime : string;
    _serverName : any;
    _serverActivation : any;
    _$boxInfo : any;
    _subList : any;
    _$setStge : any;
    _activationSubList : any;
    ...
    _$contextMenu : any;
    _title : any;
    _$networkSubMenu : any;
    _currentServerSubMenu : any;
    _$networkStage : any;
    _status : any;
    _$wrapBtn : any;
    _$networkMenu : any;
    ...
    constructor() {
        ...
        this._contextListMap = //
            [['LOC_NETWORKSERVER_ID', 'LOC_RENAME_SERVER_ID', 'LOC_SERVER_STATUS_ID', 'LOC_SERVER_ACTIVATION_TIME_ID'], ['LOC_ON_ID', 'LOC_OFF_ID'], ['LOC_OFF_ID', 'LOC_SETTIMER_ID'], ['LOC_SERVER_ACTIVATION_TIME_ID', 'LOC_ACTIVATION_TIME_PERIOD_ID']];
    }
    ...
    create () {
        ...
        hx.svc.SETTING_UTIL.addEventCb('ResultEvent', that, (aEvt, aState, aServerInfo) => {
    ...
    _on_ResultEvent (aEvt, aState, aServerInfo) {
        hx.logadd('ResultEvent');
        hx.log('ResultEvent', 'event state : ' + aState);
        hx.log('ResultEvent', 'event event : ' + aEvt);

        if (aEvt === this.EVENT.START_RESULT) {//start
            if (aState === this.RESULTSTATE.TRUE) {// true
                this._isServerRunning = true;
            }
        } else if (aEvt === this.EVENT.STOP_RESULT) {//stop
            if (aState === this.RESULTSTATE.TRUE) {// true
                this._isServerRunning = false;
            }
        } else if (aEvt === this.EVENT.TEST_CONNECTION_RESULT) {//testcon
    ...
    _on_alClicked (aParam) {
        ...
        var yesBtn : string , noBtn : string;
        var msgbox : any;
        var retObj = null;
    ...
    _conv_updatePeriod_to_secondValueString (aUpdatePeriod) {
        var startHour : number = parseInt(aUpdatePeriod.substring(0, 2), 10);
        var startMinute : number = parseInt(aUpdatePeriod.substring(3, 5), 10);
        var endHour : number = parseInt(aUpdatePeriod.substring(8, 10), 10);
        var endMinute : number = parseInt(aUpdatePeriod.substring(11, 13), 10);
        var durationMin : number;
    ...
    _makesubList (aParam) {
        var index : number, subFocusIndex : number;
        var topPx : string;
        var dataList;
    ...
    _makeonoffsubList (aParam) {
        ...
        $('#set #networkStage .menu .dep2').ani({
            ...
            cb : ()=> {
                this.append(this._activationSubList);
                this._activationSubList.on();
            }
        });
    }
    _setUpdatePeriod () {
        var that = this, convUpdatePeriod : any = {};
        ...
        function _calculateTime(_input) {
            var calStartMin : any  = parseInt(_input.substring(0, 2), 10) * 60 + parseInt(_input.substring(2, 4), 10);
            var calEndMin : any = parseInt(_input.substring(4, 6), 10) * 60 + parseInt(_input.substring(6, 8), 10);
    ...
    _setNetServer (aIndex : number) {
        var that = this;
        var $netServerSub = this._networkMainMenu.getItem(0).find('em');
        var netServerKey : string = hx.l(this._contextListMap[1][aIndex]);
        var bCommand : boolean;
    ...
    _setActivationTime (aIndex : number) {
        var that = this;
        var $serverAtivationTimeSub = that._networkMainMenu.getItem(3).find('em');
        var activationOffTimerKey : string = hx.l(that._contextListMap[2][aIndex]);
    ...
    _setSubPowerOffTimer (aIndex : number) {
        ...
        var offTimerKey : string = hx.l(that._contextListMap[1][aIndex]);
    ...
    _updateServerActivationTime (bEnable?) {
        var that = this;
        if (bEnable === true) {
            that._isServerActivationTime = 'On';
