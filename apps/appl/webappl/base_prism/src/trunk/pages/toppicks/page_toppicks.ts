...
class CPgTopPicks extends __PAGE_KERNEL__ {
    private _page;
    private _hmxContents;
    private _processingHmxContents;
    private _detailDlg;
    ...
    init(aPageUrl?) {
        ...
        //    }
        //});
        hx.svc.RECORDINGS.addEventCb('NotiRecordingStop', that, (param) => {
            var item = param ? param.item : null;
            if (that._page && item) {
    ...
    getRecommendationGroup(groupId, updateUICb) {
        var that = this;
    ...
    getSchedule(channel, startTime, prog, width, updateCb) {
        if (channel) {
            var splitStr = channel.ccid.split('.');
            if (splitStr.length > 0) {
                var sid = splitStr[splitStr.length - 1];
                hx.svc.IP_EPG.getSchedules(sid, startTime * 1000, 6).then((res) => {
    ...
    searchProgrammes(aType, data, updateCb) {
        if (!data) {
                        ...
                        for (var j = 0; j < programmesLength; j += 1) {
                            var key = (function(aList, aItem) {
                                ...
                                for (i = 0, ti = aList.length; i < ti; i += 1) {
                                    if ((aItem.getProgrammeID() && (aList[i].getProgrammeID() === aItem.getProgrammeID()))
                                    || (aItem.getSeriesID() && (aList[i].getSeriesID() === aItem.getSeriesID()))
                                    || (aItem.getProgrammeCRID() && (aList[i].getProgrammeCRID() === aItem.getProgrammeCRID()))) {
                                        index = i;
                                        break;
                                    }
                                }
    ...
    showProgrammeDetail(aProgrammeDetailItem) {
        var that = this;
    ...
    private _dataAPI  = {
        getOnDemandCategories: (updateCb, waitingCb?) => {
            if (waitingCb) {
                waitingCb();
            }
        ...
        getOnDemandGroup: (groupId, updateCb, waitingCb?) => {
            if (waitingCb) {
                waitingCb(groupId);
            }
            this.getRecommendationGroup(groupId, updateCb);
        },
        getOnNowChannels: () => {
            if (hx.emulMode) {
                return [];
        ...
        searchOnNowEPGByCcid: (ccid, updateCb) => {
            var that = this;
            var channel;
            ...
            hx.svc.EPG.getProgrammes({
                ...
                cb: (result) => {
                    ...
                    var getThumbnail = ((channel, startTime) => {
                        return (onNow, width, updateCb) => {
                        ...
                        if (hdChannel) {
                            hx.svc.EPG.findProgrammes({
                                ...
                                cb: function(aResult,b,c) {
                                    ...
                                    updateCb({
                                        result: result,
                                        getThumbnail: ((channel, startTime) => {
                                            return (onNow, width, updateCb) => {
                                                that.getSchedule(channel, startTime, onNow, width, updateCb);
                                            }
        ...
        getChannelLogo: (ccid, width, updateCb) => {
            var splitStr = ccid.split('.');
            if (splitStr.length > 0) {
                var sid = splitStr[splitStr.length - 1];
                if (sid) {
                    hx.svc.IP_EPG.getChannelLogo(sid, width).then((receivedUrl) => {
        ...
        showProgrammeDetail: (aItem) => {
            ...
            if (aItem instanceof __MODEL__.COipfRecordingItem) {
                ...
                hx.svc.IP_EPG.getDetailInfo(aItem.getProgrammeCridMds()).then((info) => {
        ...
        getHumaxRecommendation: (aType, aUpdateCb, aWaitingCb?) => {
            var that = this;
        ...
        getThumbnail: (prog, width, updateCb) => {
            return this.getSchedule(prog.channel, prog.getStartTime(), prog, width, updateCb);
        },
        requestRecord: (ccid) => {
            var that = this;
