//file name : pages/tvguide/cGuideChannelList.js
...
class cGuideChannelList extends __VIEW__ {
    _strCasID: string[] = ['CAS_FTA', 'CAS_NAGRA', 'CAS_IRDETO', 'CAS_NDS', 'CAS_VIACCESS', 'CAS_CONAX', 'CAS_MEDIAGUARD', 'CAS_CRYPTOWORKS'];
    _sUPSIDE: string = 'dummy-up';
    _sDOWNSIDE: string = 'dummy-down';
    casIDs: any[] = [];
    channels: any[] = [];
    dummyUpChannels: any[] = [];
    dummyDownChannels: any[] = [];
    channelGroup: any = {};
    isChangeable: any[] = [];
    liveChannelInfo: any = {};
    currentRowIndex: number = 0;
//    _bPrismLiteSmode: boolean = false;
    $: JQuery;
    _ulBox: any;
    _focusClass : string = 'on';
    _textAniValueOffset: number = 7;
    _textAniIntervalOffset: number = 45;
    _textAnimate: any;
...
//        this._setPrismAndPrismLiteMode();
    ...
    draw(param?) {
        var i, length, channel: __PROVIDER__.CChannelProvider, majorChannel, name, logoURL, imgObj, channels = this.channels;
        ...
        for (i = 0, length = channels.length; i < length; i += 1) {
            ...
            var fnGetImage = (aElement, serviceId) => {
                if (this._isWaitingScroll() || (param && param.keyRepeatCount)) {
                    hx.log('warning', '[draw] Do not request Channel Logo during Scrolling');
                    return;
                }
                hx.svc.IP_EPG.getChannelLogo(serviceId).then((url) => {
                    ...
                    if (!this._isValidServiceIdToUpdate(serviceId)) {
                    ...
                    var elImg = aElement.querySelector('img');
                    if (elImg) {
                        //hx.log('warning', 'hide image tag by fail to get its own');
    ...
    doScroll(param) {
        ...
        var after = () => {
            ...
            if (this.ANIMATION_EFFECT) {
                ...
                if (direction === 'up') {
                    ...
                    top = getTop($li, "last");
                    start = top + this.ROW_HEIGHT;

                    $liC.detach();
                    $liC.each((i, li) => {
                        li.style.top = start + i * this.ROW_HEIGHT + 'px';
                    });
                    $container.append($liC);
                } else {
                    $liC = $li.slice(-movingCount);
                    top = getTop($li, "first");
                    start = top + (-movingCount * this.ROW_HEIGHT);
                    $liC.detach();
                    $liC.each((i, li) => {
                        li.style.top = start + i * this.ROW_HEIGHT + 'px';
                    });
                ...
                $liU = $li.slice(0, this.ROW_COUNT);
                $liM = $li.slice(this.ROW_COUNT, this.ROW_COUNT * 2);
                $liD = $li.slice(this.ROW_COUNT * 2, this.ROW_COUNT * 3);
                $liU.attr('class', this._sUPSIDE);
                $liM.attr('class', '');
                $liD.attr('class', this._sDOWNSIDE);
                this.bindRowList($liM);
            }
            if (this.onScrollStarted) {
                this.onScrollStarted(chParam)
            }
        ...
        var anim = () => {
            ...
            before();
            this.animation.state = 'scrolling';
    ...
    _isValidServiceIdToUpdate(aServiceId) {
