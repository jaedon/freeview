...
module mPrism {
    ...
    var PlayString = 'PLAY';
    var findLinkageInfo = (item, aCallback: (HDProgramme: __PROVIDER__.cPdeProgrammeItem, SDProgramme: __PROVIDER__.cPdeProgrammeItem) => void) => {
        ...
        handler: {
            ...
            'RECORD_THIS_PROGRAMME': function (item) {
                return {
                    ...
                    selected: false,
                    focusable: false,
                    action: function (item: __PROVIDER__.cPdeProgrammeItem, detailDlg) {
    ...
    var fnGetActionList = function (item, previousItemType?) {
        ...
        switch (itemType) {
            ...
            case __MODEL__.IProgrammeDetailItem.type.programme:
                ...
                if (item.getAvailability() === __PROVIDER__.EOndemandAvailable.EAvailable) {
                    key += 'Deeplink';
                }
                break;
            case __MODEL__.IProgrammeDetailItem.type.recording:
                ...
                if (ENV.op === ENV.listOP.UKDTT) {
                    if(item.parent_activeViewType === "tobedeleted"){//(item.isKept() === true){
                        key += 'Keep';
                    }
                }
                if (item.isSeries() === true) {
                    if (item.isScheduledAsSeries() === false) {
                        ...
                        if (item.isPendingSeriesSchedule() ||
                            (item.isOnAir() && isSeriesRecording === false)) {
                ...
                if (programmeItem) {
                    if (programmeItem.getAvailability() === __PROVIDER__.EOndemandAvailable.EAvailable) {
                        key += 'Deeplink';
                    }
                }
                break;
    ...
    var fnOptionSubMenuHandle = function(a) {
        ...
        var listControl:__CONTROLS__.CListControl = a._contextMenuDlg.contextMenu().getFocusedChild();
    ...
    class CEpisodeListView extends __CONTROLS__.CViewGroupControl {
        _ctrlEpisodeList: __CONTROLS__.CListControl;
        constructor(dataProvider, activeIndex) {
            super(null);
    ...
    class CActionListView extends __CONTROLS__.CVerticalLayoutControl {
        _actionList: any[];
        _lastItemType : string;
        _item: any;
        constructor(item) {
            super(null);
            this.addClass('ep');
        ...
        update(item,reason? : string) {
            ...
            var itemDrawer = function (index, $item, that) {
                ...
                if (index === 0) {
                    ...
                    var thumbnailUrl = actionItem ? (actionItem.getThumbnailUrl((receivedUrl) => {
                            if (receivedUrl) {
                    ...
                    if (hx.il.checkMdsUrl(thumbnailUrl)) { // with width
    ...
    class CTitleView extends __CONTROLS__.CVerticalLayoutControl {
        _currentItem: any;
        constructor(item) {
            super(null);
            var a = this;
    ...
    class CDescriptionView extends __CONTROLS__.CVerticalGroupControl {
        view: any;
        _currentItem: any;
        constructor(item) {
            super(null);
            ...
            ctrlView.doDrawItem = function () {
                ...
                if (a._currentItem) {
                    ...
                    } catch(e) { hx.log('error', '[prism.dlg.programmedetail.js][getGuidance] exception!!!'); }
                    if (guidance) {
                        guidance = guidance.text || "";
                    }
                    if (ENV.op === ENV.listOP.UKDTT && !!guidance) { //&& guidance === undefined
    ...
    export class CProgrammeDetailDlg extends __PRISM_CDIALOG__.CDialog {
        _isHideBack: boolean;
        _destroyParam;
        _activeIndex: number;
        _ctrlEpisodeList: __CONTROLS__.CListControl;
        ...
        _loading: boolean;
        _delete: boolean;
        _multiDelete: boolean;
        _multiKeep: boolean;
        _refresh: boolean;
        _cancelWholeSeries: boolean = false;
        _$pde: JQuery;
        _$wrap: JQuery;
        _programmeDetailProvider: __CONTROLS__.CDataProvider;
        _deleteNum: number[];
        _keepNum: number[];
        _deleteIdx: number;
        _confirm;
        _programmeDetailList: any;
        _checkedItem;
        _comboDlg: any;
        menuStep;
        onRequestRemoveRecording: (aItem?) => void;
        onRequestClose: () => void;
        onRequestMdsSchedule: (aItem, aIndex) => void;
        private _viewTitle: CTitleView;

        constructor(layeredGroupControl, isHideBack?, destroyParam?) {
        ...
        setProgrammeDetailList(programmeDetailProvider, activeIndex) {
            var a = this;
        ...
        getProgrammeDetailProvider() {
            return this._programmeDetailProvider;
        }
        setProgrammeDetailProvider(programmeDetailList) {
        ...
        openNotification(aText: String) {
            prism.createNotificationDlg(this._parent, aText).open();
        }
        openHDAlternative(aButtonInfo: ifPrismDlgButtonInfo[]) {
            prism.createHDAlternativeButtonDlg(this._parent, aButtonInfo).open();
        }
        updateEvent(aProgramme: __PROVIDER__.cPdeProgrammeItem, reason?:string) {
            var provider: __CONTROLS__.CDataProvider = <__CONTROLS__.CDataProvider>this.getProgrammeDetailProvider();
            if (provider) {
        ...
        // Private
        _create(programmeDetailProvider, activeIndex) {
            var a = this;
