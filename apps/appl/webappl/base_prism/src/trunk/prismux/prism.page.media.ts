/**
 * Created by njkim2 on 2017-10-25.
 */
/// <reference path="../def/prism.d.ts" />
/// <reference path="../def/sugar.d.ts" />
// file name : prismux/prism.page.media.js
...
module mPrism {
	...
	var mViewType = __PRISM_CPAGE__.EViewType.ELIST;;
	...
	var fnDoFileOperation = function (a: CMediaPage, operation, data, server?) {
	...
	var fnOptionMenuHandleDelete = function (a: CMediaPage, item) {
	...
	var fnOptionMenuHandleCopy = function (a: CMediaPage, item) {
	...
	var fnOptionMenuHandleMove = function (a: CMediaPage, item) {
		a.onRequestLocalServerList(function (serverList) {
			if (serverList.length) {
				var selectionDlg = prism.createStorageSelectionDlg2(a);
				selectionDlg.setSelectionItems(fnSortStorageList(serverList));
	...
	var fnOptionMenuHandleStopCopying = function (a: CMediaPage, item) {
		a.doStopCopying();
	};
	var fnOptionMenuHandleStopMoving = function (a: CMediaPage, item) {
		a.doStopMoving();
	};
	var fnOptionMenuHandleSelect = function (a: CMediaPage, item) {
	...
	var fnOptionMenuHandleSelectAll = function (a: CMediaPage, item) {
		var newItem;
		var dataControl = a.getDataControl();
		var data = dataControl._drawnItems.getItemsByArray();
	...
	var fnOptionMenuHandleCancel = function (a: CMediaPage, item) {
	};
	var fnOptionMenuHandleEject = function (a: CMediaPage, item) {
		var id = item.getId();
		a.doEject(id);
	};
	var fnOptionMenuHandleRename = function (a: CMediaPage, item) {
	...
	var fnOptionMenuHandleDeleteAllFiles = function (a: CMediaPage, item) {
	...
	var fnOptionMenuHandleAddToPlaylist = function (a: CMediaPage, items) {
		a.onRequestAddToPlaylist({
			items: items
		});
	};
	var fnOptionMenuHandleRemoveToPlaylist = function (a: CMediaPage, items) {
		a.onRequestRemoveToPlaylist({
			items: items
		});
		a.removeMediaItem(items);
	};
	var fnOptionMenuProvider = function (a: CMediaPage, activeFocusInfo) {
		var ret;
	...
	var fnCreateViewType = function(a) {
		var viewTypeBtnDrawer = function(index, $item) {
			...
			if (mViewType === __PRISM_CPAGE__.EViewType.ELIST) {
				if (index === 0) {
	...
	var fnCreateContent = function (a) {
		...
		dataControl.addEventHandler('FocusChanged', dataControl, ($old, $new) => {
	...
	var fnNotFoundMsgOnOff = function (a: CMediaPage, onOff) {
	...
	var fnLoadingMsgOnOff = function (a: CMediaPage, onOff, text?) {
	 ...
	 * Event Handlers
	 **************************************************/
	var onMediaServerSelected = function (a: CMediaPage, param) {
		var item = param.item;
		var $item = param.$item;
	...
	var onMediaItemSelected = function (a: CMediaPage, param) {
		var item = param.item;
	...
	export class CMediaPage extends __PRISM_CPAGE__.CPage {
		_contentDataProvider: __CONTROLS__.CDataProvider;
		_localServerDataProvider: __CONTROLS__.CDataProvider;
		_networkServerDataProvider: __CONTROLS__.CDataProvider;
		_userServerDataProvider: __CONTROLS__.CDataProvider;
		_localServerListControl: __CONTROLS__.CListControl;
		_networkServerListControl: __CONTROLS__.CListControl;
		_userServerListControl: __CONTROLS__.CListControl;
		_viewTypeControl: __CONTROLS__.CHorizontalLayoutControl;
		_initServerId: string;
		_title: string;
		_type: any;
		_currentOperation: any;
		_dataControl: __CONTROLS__.CDataControl;
		_itemDrawer: (aParam) => void;
		_viewDlg: any;
		_menuView;
		_contentView: __CONTROLS__.CVerticalGroupControl;
		_mediaViewContainer: __CONTROLS__.CViewGroupControl;
		_notificationDlg : any;
		_currentPath: string;
		_currentServer: string;
		_currentServerId: any;

		doStopCopying: () => void;
		doStopMoving: () => void;
		doEject: (aID) => void;
		doRename: (aID, aName) => void;

		onRequestLocalServerList: (aServerList) => void;
		onRequestAddToPlaylist: (aServerList) => void;
		onRequestRemoveToPlaylist: (aServerList) => void;
		onMediaServerSelected: (aParam) => void;
		onMediaItemSelected: (aParam) => void;
		onRequestChangePage: (aParam) => void;
		onRequestRemoveRecentItem: (aParam) => void;
		...
		hasFile(aPath: string) {
			return false;
		}
		setUserServerDataProvider(dataProvider) {
			this._userServerDataProvider = dataProvider;
		}
		setLocalServerDataProvider(dataProvider) {
			this._localServerDataProvider = dataProvider;
		}
		setNetworkServerDataProvider(dataProvider) {
			this._networkServerDataProvider = dataProvider;
		}
		setInitServerId(initServerId) {
		...
		addMediaItems(mediaItemList) {
			...
			if (a._contentDataProvider.getCount() === 0) {
				...
				if (mSelectedServer.getViewType() === __MODEL__.IMediaServer.TViewType.EOneDepthView && a._contentView.hasFocus()) {
					...
					//});
					/*focusedChildIndex = a.getFocusedChildIndex();
		...
		doKeyOption() {
			...
			if (!consumed) {
				consumed = super.doKeyOption.call(a);
			}
			return consumed;
		}
		addServer(server) {
	...
	export class CMusicPage extends CMediaPage {
		constructor($el) {
			super($el);
			var a = this;
	...
	export class CVideoPage extends CMediaPage {
		constructor($el: JQuery) {
			super($el);
			var a = this;
	...
	export class CPhotoPage extends CMediaPage {
		constructor($el: JQuery) {
			super($el);
			var a = this;
