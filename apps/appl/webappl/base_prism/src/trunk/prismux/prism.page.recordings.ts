...
module mPrism {
    class CRecordUtil {
        ...
        WIDTH_GRID_CHANNEL_LOGO = 50;
        WIDTH_LIST_CHANNEL_LOGO = 85;
        fnMatchSeries(a, b) {
        ...
        fnCompareStartTimeDescent(a, b) {
            a = a instanceof Array ? a[0] : a;
            b = b instanceof Array ? b[0] : b;
            return (b.getStartTime()) - (a.getStartTime());
        }
        fnCompareStartTimeAscent(a, b) {
            a = a instanceof Array ? a[0] : a;
            b = b instanceof Array ? b[0] : b;
            return (a.getStartTime()) - (b.getStartTime());
        }
        fnCompareStartTimeGreatThen(a, b) {
            a = a instanceof Array ? a[0] : a;
            b = b instanceof Array ? b[0] : b;
            return (a.getStartTime()) > (b.getStartTime());
        }
        fnCompareStartTimeLessThen(a, b) {
            a = a instanceof Array ? a[0] : a;
            b = b instanceof Array ? b[0] : b;
            return (a.getStartTime()) < (b.getStartTime());
        }
        fnCompareAlphabet(a, b) {
            a = a instanceof Array ? a[0] : a;
            b = b instanceof Array ? b[0] : b;
        ...
        fnMakeScheduledList(scheduledItemList, sortAscent) {
        ...
        fnIsSameSeries(aGroupCRIDs1, aGroupCRIDs2) {
            var i, j;
            var ret = false;
        ...
        fnMakeListByRecent(recordingItemList) {
            var i = 0, j = 0, recordingTotal = 0, recentTotal = 0, ret = [];
            var recordingItem: any = null, recentItem: any = null;
        ...
        fnMakeListByProgramme(a, recordingItemList, sortAscent?) {
        ...
        fnMakeListByChannel(recordingItemList) {
        ...
        fnMakeListByDayOfWeek(recordingItemList) {
        ...
        fnMakeListByStorage(recordingItemList) {
        ...
        fnMakeListByTobedeleted(recordingItemList, sortAscent?) {
        ...
        fnGroupingItemByCopyControl(list) {
        ...
        prependTagWithClass(prepend, $parent, className) {
        ...
        appendTagWithClass(append, $parent, className, tag?) {
            var $target = $parent.children('.' + className);
            if (append) {
                if ($target.length === 0) {
        ...
        fnUpdateStorageInfo(a) {
            if (a._storageInfo && a._$storageStatusArea) {
                ...
                if (hx.config.recStorageStatusType2 === true){
                    a._$storageStatusArea.find('.info').html('<span style="font-weight:bold">HD</span>' + '<span> ' + hdFreeSize + ((hdFreeSize <= 1) ? 'hr' : 'hrs') + ' / </span>' + '<span style="font-weight:bold">SD</span>' + '<span> ' + sdFreeSize + ((sdFreeSize <= 1) ? 'hr' : 'hrs') + '</span><br>' +
                        '<span style="font-weight:bold">Total:</span><span> ' + recordingItemList.length + ' Recordings</span>');
                    ...
                    a._$storageStatusArea.find('.info').html('HD' + ' ' + hdFreeSize + ' ' + ((hdFreeSize <= 1) ? 'hr' : 'hrs') + ' / ' + 'SD' + ' ' + sdFreeSize + ' ' + ((sdFreeSize <= 1) ? 'hr' : 'hrs') + '<br>' +
                        'Total : ' + recordingItemList.length + ' Recordings');
                    prism.drawStorageGraphRect(a._storageInfo.total - a._storageInfo.free, //
                        a._storageInfo.total, //
                        0, 0, //
                        a._$storageStatusArea.find('canvas')[0]);
                }
            }
        }
        fnGetSelectedItems(a) {
        ...
        fnPropagateItemInsertedEvent(dpSource, item, dpTarget, fnGroupMatch, fnCompare) {
        ...
        fnPropagateItemRemovedEvent(dpSource, items, dpTarget, ascent) {
        ...
        fnPropagateRecItemRemovedEvent(dpSource, items, dpTarget, ascent?) {
        ...
        fnPropagateItemUpdateEvent(dpSource, keys, dpTarget) {
        ...
        fnPropagateTobedeletedItemUpdateEvent(dpSource, keys, dpTarget) {
        ...
        fnMenuLayout_onSchedule(a, index, $item) {
        ...
        fnMenuLayout_onTobedeleted(a, index, $item) {
            a.onRequestRecordingData(function (recordingDataProvider) {
                var array = _.filter(recordingDataProvider.getDataArray(), function(aItem: __MODEL__.COipfRecordingItem) {
...
//                        recordingDataProvider.removeEventHandler(a);
        ...
        fnRecordingGrid_onItemSelected(a, recordingGrid, param) {
        ...
        fnTobedeletedList_onItemSelected(a, recordingGrid, param) {
    ...
    class CRecordDraw {
        fnRecordingListItemDrawer($item, item, title, isSeries, count, isNew, isLocked, isSelected, isRecording, className, isChecked) {
                ...
                if (item.getServiceId()) {
                    if ($channel) {
                        ...
                        if (logoUrl) {
                            ...
                            if (channel) {
                                setTimeout(() => {
                                    hx.svc.IP_EPG.getChannelLogo(channel.sid, CRecUtil.WIDTH_LIST_CHANNEL_LOGO).then((url) => {
                                        $logo = $channel.children('.logo');
        ...
        fnRecordingGridItemDrawer($item, item, title, isSeries, count, isNew, locked, isSelected, isRecording, className, isChecked) {
            ...
            if (logoUrl) {
                ...
                setTimeout(() => {
                    hx.svc.IP_EPG.getChannelLogo(item.getServiceId(), CRecUtil.WIDTH_GRID_CHANNEL_LOGO).then((url) => {
                        if (url) {
        ...
        fnRecordingItemDrawerByRecent(param) {
            var items = param.item;
            ...
            if (SViewType === __PRISM_CPAGE__.EViewType.ELIST) {
                CRecDraw.fnRecordingListItemDrawer(param.$item, //
                    item, //
                    item.getTitle(), //
                    false, //
                    count,
                    ...
                    isLocked, //
                    isSelected, //
                    isRecording, 'byProgramme', items._isChecked);
            } else {
                CRecDraw.fnRecordingGridItemDrawer(param.$item, //
                    item, //
                    item.getTitle(), //
                    false, //
                    count,
        ...
        fnRecordingItemDrawerByProgramme(param) {
            var items = param.item;
            if (!( items instanceof Array)) {
                items = [items];
            }
            ...
            if (SViewType === __PRISM_CPAGE__.EViewType.ELIST) {
                CRecDraw.fnRecordingListItemDrawer(param.$item, //
                    item, //
                    isSeries ? item.getSeriesTitle() : item.getTitle(), //
                    isSeries && (count > 0), //
                    count,
                    isNew, //
                    isLocked, //
        ...
        fnRecordingItemDrawerByChannel(param) {
            var items = param.item;
            var isSelected = (items._isSelected) ? true : false;
            ...
            if (SViewType === __PRISM_CPAGE__.EViewType.ELIST) {
                CRecDraw.fnRecordingListItemDrawer(param.$item, //
                    item, //
                    prism.makeChannelNumberString(item.getChannelNo()) + ' ' + item.getChannelName(), //
                    (count > 0), //
                    count,
                    isNew, //
                    false, //
                    isSelected, //
                    isRecording, 'byChannel', items._isChecked);
            } else {
                CRecDraw.fnRecordingGridItemDrawer(param.$item, //
                    item, //
                    htmlTitle, //
                    ...
                    isSelected, //
                    isRecording, //
                    'byChannel', items._isChecked);
            }
        }
        fnRecordingItemDrawerDayOfWeek(param) {
            var items = param.item;
            var isSelected = (items._isSelected) ? true : false;
            var isNew = CRecUtil.fnListHasNewItem(items);
            var item = items[0];
            ...
            if (item) {
                if (SViewType === __PRISM_CPAGE__.EViewType.ELIST) {
                    CRecDraw.fnRecordingListItemDrawer(param.$item, //
                        item, //
                        CRecUtil.fnStringDayOfWeek(item.getStartTime().getDay()), //
                        (count > 0), //
                        count,
                        isNew, //
                        false, //
                        isSelected, //
                        isRecording, 'byDayOfWeek', items._isChecked);
                } else {
                    CRecDraw.fnRecordingGridItemDrawer(param.$item, //
                        item, //
                        CRecUtil.fnStringDayOfWeek(item.getStartTime().getDay()), //
                        ...
                        false, //
                        isSelected, //
                        isRecording, null, items._isChecked);
                }
            }
        }
        fnRecordingItemDrawerStorage(param) {
            var items = param.item;
            var isSelected = (items._isSelected) ? true : false;
            var isNew = CRecUtil.fnListHasNewItem(items);
            var item = items[0];
            ...
            if (SViewType === __PRISM_CPAGE__.EViewType.ELIST) {
                CRecDraw.fnRecordingListItemDrawer(param.$item, //
                    ...
                    false, //
                    isSelected, //
                    isRecording, 'byStorage', items._isChecked);
            } else {
                CRecDraw.fnRecordingGridItemDrawer(param.$item, //
                    item, //
                    item.getStorageName(), //
        ...
        fnRecordingItemDrawerByExtention(param) {
            var items = param.item;
            if (!( items instanceof Array)) {
                items = [items];
            }
        ...
        fnOpenScheduledDlg(a, item?) {
            a.onRequestChannelGroupInfo(function (channelGroupInfo, currentChannelId) {
                ...
                settingDlg.onInputChanged = function (input) {
                    ...
                    if (!bValid) {
                        hx.msg('response', {
                            text: hx.l('LOC_REC_INVALID_DURATION_INPUT_ID'), // temporary
                            auto_close: true,
                            close_time: 3000
                        });
                    }
        ...
        fnLoadingMsgOnOff(a, onOff, text?) {
    ...
    class CRecordFileOperation {
        fnOptionMenuProvider(a, activeFocusInfo) {
            var ret;
            if (activeFocusInfo) {
                            ...
                            switch (a._optionMenuOnGoing) {
                                ...
                                default:
                                    ...
                                    if ((a._activeViewType === 'programme') || a._dpRecordingExtView) {
                                        if (item instanceof Array) {//
                                            if (item[0].isSeries()) {
                                                if (CRecUtil.fnListHasLockedWholeItem(item)) { //all items locked
                                                    menuItems.push(menuItemList[CRecUtil.TOptionMenuItem.EUnLock]);
                                                } else if (CRecUtil.fnListHasLockedItem(item)) { // at least one item locked
                                                    menuItems.push(menuItemList[CRecUtil.TOptionMenuItem.ELock]);
                                                    menuItems.push(menuItemList[CRecUtil.TOptionMenuItem.EUnLock]);
                                                } else { // all items unlocked
                                                    menuItems.push(menuItemList[CRecUtil.TOptionMenuItem.ELock]);
                                                }
                                            } else {
                                                if (fnCheckIsLocked(item)) {
        ...
        fnOptionMentHandleDelete(a, items) {
            if (a._activeViewType === 'recent') {
                a.doKeyOption();
                ...
                prism.processRequest(a, null, 'doRecentItemDelete', {
                    ...
                    var recentList: any = hx.svc.HISTORY.getPlaybackList();
        ...
        fnOptionMentHandleLock(a, items, name) {
            var i, ti, locked = false;
            if (items[0].isSeries()) {
        ...
        fnOptionMentHandleKeep(a, items) {
            var confirm = prism.createButtonDlg(a);
        ...
        fnOptionMenuHandleKeepCheckedItems(a, items) {
            var data = a._dpTobedeleted;
            var confirm = prism.createButtonDlg(a);
        ...
        fnOptionMentHandleStopRecording(a, items) {
            var confirm = prism.createButtonDlg(a);
        ...
        // copy or move
        fnOptionMentHandleOperation(a, items, OnGoning, processRequest, NoStroageNoti, Protect, Protect2) {
        ...
        fnOptionMentHandleCopy(a, items) {
            //copy..
        ...
        fnOptionMentHandleMove(a, items) {
            //move..
        ...
        fnOptionMentHandleStopCopy(a, items) {
        ...
        fnOptionMentHandleStopMove(a, items) {
        ...
        fnOptionMentHandleSelectAllCancel(a) {
            var data;
            var i, ti, items;
        ...
        fnOptionMentHandleDeleteCheckedSchedules = function(a, items) {
            var data = a._ScheduleProvider;
            var confirm = prism.createButtonDlg(a);
        ...
        fnOptionMentHandleDeleteSchedule(a, items) {
        ...
        fnOptionMentHandleReschedule(a, itemList) {
            var item = itemList[0];
            a.reloadContextMenuItems();
            CRecDraw.fnOpenScheduledDlg(a, item);
        }
        fnOptionMentHandleSelectSchedule = function(a, itemList) {
            var item = itemList[0];
        ...
        fnOptionMentHandleSelectAllSchedule = function(a, itemList) {
            var item = itemList[0];
            if (!a._isSelectAll) {
                var data = a._ScheduleProvider;
                var dataArray = data.getDataArray();
        ...
        fnOptionMentHandleSelectTobedeleted = function(a, itemList) {
            var item = itemList[0];
        ...
        fnOptionMentHandleSelectAllTobedeleted = function(a, itemList) {
            var item = itemList[0];
            if (!a._isSelectAll) {
        ...
        fnOptionMentHandleSortBy = function(a) {
    ...
    var SViewType = __PRISM_CPAGE__.EViewType.ELIST;

    class CRecordingsPage extends __PRISM_CPAGE__.CPage {
        private _viewTypeControl;
        private _recordingView;
        private _contentView;
        private _storageInfo;
        private _isMultiSelect: boolean;
        private _$storageStatusArea: JQuery;
        private _menuVLayout;
        private _dpRecordings;
        private _dpRecordingExtView;
        private _recordingViewContainer;
        private _$contentsIndex;
        private _dpTobedeleted;
        _activeViewType;
        private _tempViewType;
        private _isSelectAll: boolean;
        private _isExtMultiSelect: boolean;
        private _ScheduleProvider = null;
        private ESortType = {
            ETIME : 0,
            EALPHABET : 1
        };
        private _sortType = hx.svc.WEB_STORAGE.getSortTypeInRecordings() || this.ESortType.ETIME;
        private STR_TARGET_SORT_TYPE_ARR = ["Sort A-Z", "Sort by Time"];
        private _activeViewIndex;

        onRequestRecordingData: Function;
        constructor($el) {
            super($el);
        }
        init($el) {
        ...
        setStorageInfo(freeSizeGb, totalSizeGb) {
            var a = this;
            a._storageInfo = {
                free: freeSizeGb,
                total: totalSizeGb
            ...
            if (!a.$layer()) {
                a.createLayer({
                    fnAfterCreate: function () {
                        ...
                        var fnButtonDrawer = function (index, $item, text, type, aFocusable = true) {
                        ...
                        menuVLayout.onItemSelected = function (index, $item) {
                            ...
                            } else if (buttonInfo[index].type === 'tobedeleted') { //CRecUtil.fnMakeListByTobedeleted(array)
                        ...
                        var recordingView;
                        if (SViewType === __PRISM_CPAGE__.EViewType.EGRID) {
                        ...
                        recordingView.addEventHandler('FocusChanged', recordingView, ($old, $new) => {
                            var dpRecordings = a._dpRecordings;
                            if (dpRecordings) {
                        ...
                        recordingView.addEventHandler('ActiveFocusedGained', recordingView, ($item) => {
                            var dpRecordings = a._dpRecordings;
                            if (dpRecordings) {
        ...
        setViewConfig(aRecordingDataProvider, type) {
            var a = this;
            a._menuVLayout._$self.find('.prism-button').removeClass('selected');
            ...
            if (a._recordingView instanceof __CONTROLS__.CListControl) {
                SViewType = __PRISM_CPAGE__.EViewType.ELIST;
            } else {
                SViewType = __PRISM_CPAGE__.EViewType.EGRID;
            }
            var handler = {
                'programme': function () {
            ...
            if (sortedList.length > 0) {
                a._viewTypeControl.show();
                //CRecDraw.fnContentsIndexOnOff(a, true);
            } else {
                a._viewTypeControl.hide();
                //CRecDraw.fnContentsIndexOnOff(a, false);
            }
            if (type === 'recent') {
                var recentList = hx.svc.HISTORY.getPlaybackList();
                if (!!recentList) {
            ...
            a.onRequestRecordingData(function (dataProvider) {
                ...
                recordingDataProvider.addEventHandler('RecItemRemoved', a, function (keys, items) {
                    ...
                    var _fnProcessRemoveEvent = function () {
                        ...
                        } else// endof if watingQue.lenght
                        {
        ...
        drawGridRecordings() {
            ...
            recordingGrid.addEventHandler('FocusChanged', recordingGrid, ($old, $new) => {
                var dpRecordings = a._dpRecordings;
                if (dpRecordings) {
                    var newIndex = parseInt($new.attr('data'));
                    var oldIndex = parseInt($old.attr('data'));
            ...
            recordingGrid.addEventHandler('ActiveFocusedGained', recordingGrid, ($item) => {
                var dpRecordings = a._dpRecordings;
                if (dpRecordings) {
                    var focusIndex = parseInt($item.attr('data'));
                    if (isNaN(focusIndex)) {
        ...
        drawListRecordings() {
            ...
            recordingList.addEventHandler('FocusChanged', recordingList, ($old, $new) => {
                var dpRecordings = a._dpRecordings;
                if (dpRecordings) {
                    var newIndex = parseInt($new.attr('data'));
                    var oldIndex = parseInt($old.attr('data'));
            ...
            recordingList.addEventHandler('ActiveFocusedGained', recordingList, ($item) => {
                var dpRecordings = a._dpRecordings;
                if (dpRecordings) {
                    var focusIndex = parseInt($item.attr('data'));
                    if (isNaN(focusIndex)) {
        ...
        onRecordingFileOperation(param) {
            var a = this;
