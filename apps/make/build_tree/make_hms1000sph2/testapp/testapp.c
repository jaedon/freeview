...
bool InitDisplay(const AppConfig *config)
{
...

   {
      ...
      DFBCHECK(layer->CreateWindow( layer, &desc, &window1 ) );
		DFBCHECK(window1->SetApplicationID(window1,eAWINDOW_TESTAPP));
...
int main(int argc, char** argv)
{
	...
	{
		HAPI_SendKeyGrab(eAWINDOW_TESTAPP, TRUE);
		...
		{
			if (HAPI_GetInputEvent(eAWINDOW_TESTAPP, &stInputEvent) == TRUE)
			{
				printf("key received ... key_symbol : [%d] pass it\n", stInputEvent.key_symbol);
				HAPI_ReturnKeyEvent(eAWINDOW_TESTAPP, eHINPUTRET_PASS, &stInputEvent);
			}
