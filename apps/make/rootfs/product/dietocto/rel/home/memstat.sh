...
then
	...
	then
		...
		## shko fix.
		vmRss=`awk '/^Rss:/{ sum += $2 } END { print sum }' /proc/$PID/smaps`
		...
		then
			...
			let vmRss=${vmRss}*1024

			echo -e "$Private  + $Shared = $Sum \t $vmRss \t $Name"
		fi
	fi
fi
}
...
totalrss=`awk -v column_numberrss=6 '{ totalrsssum += $column_numberrss} END { print totalrsssum }' /tmp/res2`
...
#echo -e "Private \t + \t Shared \t = \t RAM used \t vmRss used \t Program"
...
#here we use function that make conversion
		echo -e "Data"
		echo -en "`convert $a`  \t $b \t `convert $c`  \t $d \t `convert $e` \t `convert $f` \t $g"
		echo ""
		fi
	done
done < /tmp/res
...
#echo "--------------------------------------------------------"
echo -e "Total"
echo -e "`convert $total` \t `convert $totalrss`"
#echo "========================================================"
