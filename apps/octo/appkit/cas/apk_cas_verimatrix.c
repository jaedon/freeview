...
STATIC APK_CAS_VERIMATRIX_Notifier_t s_pfAppNotifier;

#define ___PRIVATE_FUNCTION___
#define _____NOTIFIER_FUNCTION_____
STATIC void apk_cas_verimatrix_onStatusChanged(void)
{
#if 0 // not yet used.
...
HERROR APK_CAS_VERIMATRIX_Init(void)
{
	OAPI_CAS_VERIMATRIX_Init(apk_cas_verimatrix_onStatusChanged);
	return ERR_OK;
}

HERROR APK_CAS_VERIMATRIX_RegisterNotifier(APK_CAS_VERIMATRIX_Notifier_t fnNotifier)
{
	if(NULL == fnNotifier)
	{
...
HERROR APK_CAS_VERIMATRIX_UnregisterNotifier(void)
{
...
HERROR	APK_CAS_VERIMATRIX_GetIptvHybridLibVersion(HCHAR *pszVersion)
{
	HERROR						hErr;
	OxCasVerimatrixStatus_t		stStatus;

	hErr = OAPI_CAS_VERIMATRIX_GetStatus(&stStatus);
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszVersion, stStatus.szIptvHybridVersion , OAPI_CAS_VERIMATRIX_VERSION_LENGTH);
	}

	return hErr;
}

HERROR	APK_CAS_VERIMATRIX_GetWebLibVersion(HCHAR *pszVersion)
{
	HERROR						hErr;
	OxCasVerimatrixStatus_t		stStatus;
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszVersion, stStatus.szWebVersion , OAPI_CAS_VERIMATRIX_VERSION_LENGTH);
	}
...
HERROR	APK_CAS_VERIMATRIX_GetGatewayLibVersion(HCHAR *pszVersion)
{
	HERROR						hErr;
	OxCasVerimatrixStatus_t		stStatus;
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszVersion, stStatus.szGatewayVersion , OAPI_CAS_VERIMATRIX_VERSION_LENGTH);
	}
...
HERROR		APK_CAS_IRDETO_GetIrdetoManCode(HCHAR *pszMancode)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszMancode, stStatus.szMancode, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
...
HERROR		APK_CAS_IRDETO_GetIrdetoHwCode(HCHAR *pszHWCode)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszHWCode, stStatus.szHWCode, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
...
HERROR		APK_CAS_IRDETO_GetIrdetoVariant(HCHAR *pszVariant)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszVariant, stStatus.szVariant, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
...
HERROR		APK_CAS_IRDETO_GetIrdetoSysId(HCHAR *pszSysId)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszSysId, stStatus.szSysId, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
...
HERROR		APK_CAS_IRDETO_GetIrdetoKeyVer(HCHAR *pszKeyVer)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszKeyVer, stStatus.szKeyVer, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
...
HERROR		APK_CAS_IRDETO_GetIrdetoSigVer(HCHAR *pszSigVer)
{
	...
	if(hErr == ERR_OK)
	{
		HLIB_STD_StrUtf8NCpy(pszSigVer, stStatus.szSigVer, OAPI_CAS_IRDETO_SYSTEM_INFO_COMPONENT_LENG);
	}
	else
	{
