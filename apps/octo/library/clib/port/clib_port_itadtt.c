...
/**************************************************************************************************/
#define	CLIB_PORT_ITADTT_LOGICAL_NUMBER_TV			850 // Main Overflow (the Assignment Zone)
#define	CLIB_PORT_ITADTT_LOGICAL_NUMBER_RADIO		850 
...
/**************************************************************************************************/
HBOOL			CLIB_PORT_FilterInItaDtt(CListAttr_t *pstAttr, DxService_t *pstSvcInfo)
{
...
HINT32			CLIB_PORT_GetServiceNumberItaDtt(CListAttr_t *pstAttr, DxService_t *pstSvcInfo, HINT32 idx)
{
	HINT32	ulLogicalNumber;
	CLibInt_ITADTT_t	*pstIntInfo;
	...
	{
		pstIntInfo = (CLibInt_ITADTT_t*)pstAttr->aucData;
		ulLogicalNumber = pstIntInfo->nCurrentLogicalNumber++;
	}
	return ulLogicalNumber;
}

CListAttr_t*		CLIB_PORT_InitSessionItaDtt(HUINT32 ulContentsType, HUINT32 ulOperatorType, HUINT32 ulExt)
{
	CListAttr_t	*pstAttr;
	CLibInt_ITADTT_t	*pstIntInfo;

	pstAttr = (CListAttr_t*)HLIB_STD_MemCalloc(sizeof(CListAttr_t) + sizeof(CLibInt_ITADTT_t));
	pstIntInfo = (CLibInt_ITADTT_t*)pstAttr->aucData;
	if (ulContentsType == CLIB_TYPE_RADIO)
	{
		pstIntInfo->nCurrentLogicalNumber = CLIB_PORT_ITADTT_LOGICAL_NUMBER_RADIO;
	}else
	{
		pstIntInfo->nCurrentLogicalNumber = CLIB_PORT_ITADTT_LOGICAL_NUMBER_TV;
	}
