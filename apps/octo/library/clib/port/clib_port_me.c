...
#define	CLIB_PORT_ME_LOGICAL_NUMBER_TV			1
#define	CLIB_PORT_ME_LOGICAL_NUMBER_RADIO		5000

/**************************************************************************************************/
#define _________CLIB_PORT_Private_Prototype_________________________________________________
/**************************************************************************************************/

static HUINT8*	clib_port_me_GetFirstAlphabetCharacter(HUINT8* pSvcName)
{
...
static HINT32	clib_port_me_MatchSvcNameAnyAlphabet(HUINT8 ucAlphabetIdx, HUINT8 *pSvcName)
{
	HUINT8		*pChar = NULL;
	...
	pChar = clib_port_me_GetFirstAlphabetCharacter(pSvcName);
...
/**************************************************************************************************/
HBOOL			CLIB_PORT_FilterInMiddleEast(CListAttr_t *pstAttr, DxService_t *pstSvcInfo)
{
	...
	{
		return	clib_port_me_MatchSvcNameAnyAlphabet(pstAttr->ulExt, pstSvcInfo->name);
	}
...
HINT32			CLIB_PORT_GetServiceNumberMiddleEast(CListAttr_t *pstAttr, DxService_t *pstSvcInfo, HINT32 idx)
{
	HINT32	ulLogicalNumber;
	CLibInt_ME_t	*pstIntInfo;
	...
	{
		pstIntInfo = (CLibInt_ME_t*)pstAttr->aucData;
		ulLogicalNumber = pstIntInfo->nCurrentLogicalNumber++;
	}
	return ulLogicalNumber;
}

CListAttr_t*	CLIB_PORT_InitSessionMiddleEast(HUINT32 ulContentsType, HUINT32 ulOperatorType, HUINT32 ulExt)
{
	CListAttr_t	*pstAttr;
	CLibInt_ME_t	*pstIntInfo;

	pstAttr = (CListAttr_t*)HLIB_STD_MemCalloc(sizeof(CListAttr_t) + sizeof(CLibInt_ME_t));
	pstIntInfo = (CLibInt_ME_t*)pstAttr->aucData;
	if (ulContentsType == CLIB_TYPE_RADIO)
	{
		pstIntInfo->nCurrentLogicalNumber = CLIB_PORT_ME_LOGICAL_NUMBER_RADIO;
	}else
	{
		pstIntInfo->nCurrentLogicalNumber = CLIB_PORT_ME_LOGICAL_NUMBER_TV;
	}
