...
CListAttr_t*		CLIB_PORT_InitSessionSes(HxVector_t *pstSvcList, HUINT32 ulContentsType, HUINT32 ulOperatorType, HUINT32 ulExt)
{
	...
	// TODO: 1) if channel list existed in lcn area -> lcn 영역 뒤에 non-lcn 영역을 붙인다.
	// TODO: 2) if channel list not existed in lcn area -> 100부터 non-lcn 영역을 붙인다.

	// LCN 영역의 TV / RADIO Max lcn number을 알아온다.
