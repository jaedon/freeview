...
#define	PATCH_INVALID_MAGIC_CODE
#define	PATCH_SOCKET_DISCONNECTED_ISSUE
...
//#define	SOCKET_INVALIDMAGIC(soc)(((HxSocket_t *)soc)->magic != SOCKET_MAGIC_CODE)
#define	SOCKET_INVALIDMAGIC(soc)(FALSE)
...
typedef struct
{
	...
	HCHAR				szIpAddr[32];
#if defined(PATCH_INVALID_MAGIC_CODE)
	HULONG				ulSemId;
#endif
...
#if defined(PATCH_INVALID_MAGIC_CODE)
		HxSEMT_Create((HUINT32 *) &(sock->ulSemId), "SockSema", 0);
#endif
...
static void		socket_Close (HxSocket_t *sock)
{
	if (sock)
	{
		...
		if (sock->ulSemId)
		{
			HxSEMT_Destroy(sock->ulSemId);
		}
#endif

		sock->magic		= ~SOCKET_MAGIC_CODE;
		sock->flags		= eHxSOCKET_FLAG_NULL;
		sock->ulSemId	= 0;
		sock->fd		= -1;
...
static HERROR		socket_Write(HxSocket_t *info, const void *pvData, HUINT32 size, HUINT32 requestId)
{
	...
	if (err != ERR_OK)
	{
		HxLOG_Error(" SOCKET WRITE ERROR!!!!!! (0x%X, %d) (%d) \n", (HUINT32) info, info->fd, size);
	}
	return err;
}
						...
						)
{
	...
	HxLog_NullCheckReturnValue(info, -1);
	fd = info->fd;
	...
	if (result == SOCKET_RESULT_OK)
	{
		...
		{
			HxLOG_Critical("() magiccode does NOT matched!!! (%X != %X) [%s]\n", header.magicCode, SOCKET_MAGIC_CODE, HLIB_STD_GetCurProcessName());
			info->flags &= ~eHxSOCKET_UNDEAD;
			return -1;
		}
...
#if defined(PATCH_SOCKET_DISCONNECTED_ISSUE)
	...
	if (info != NULL)
	{
		...
		{
			socket_client_Close(info);		/* Don't do this, here. Caller does not know it.. */
			return -1;
		}
	}
#else
	...
	do
	{
		...
		for (i = 0 ; i < n ; i++)
		{
			...
			if (result == SOCKET_RESULT_OK)
			{
				...
				{
					HxLOG_Critical("() magiccode does NOT matched!!! (%X != %X) [%s]\n", header.magicCode, SOCKET_MAGIC_CODE, HLIB_STD_GetCurProcessName());
					HxLOG_Critical("() SocketInfo (0x%X,%d) (%08X,%d) \n", i, n, (HUINT32) client, (HINT32) client->fd);
					continue;
				}
				rxdata = socket_AllocData(info, header.size);
				if (rxdata == NULL)
...
#else
				socket_Close(client);	/* Open is called here, then Close should be called here ? */
				client = NULL;
#endif
				n--; i--;
#else
...
static HINT32		socket_client_RepairConnection (HxSocket_t *sock)
{
	...
	if (sock->fd >= 0)
	{
		...
		HxSEMT_Get(sock->ulSemId);
		sock->flags &= ~eHxSOCKET_RECONNECT;

		HxLOG_Warning("(%s) connnection repaired. and flush all data(%d bytes) to destination!\n"
...
static HERROR	socket_client_Buffering (HxSocket_t *sock, const void *data, HUINT32 size, HUINT32 requestId)
{
	...
	if (buf)
	{
		...
		HxLOG_Critical("(%s) socket doesn't connected yet buffering(%u/%u bytes)\n"
					,  socket_ToString(sock), size, client->bufferSize);

		if (client->bufferSize > SOCKET_BUFFER_THRESHOLD)
		{
			HxLOG_Critical("(%s) socket buffer overflow (threshold:%u)\n"
						, socket_ToString(sock), SOCKET_BUFFER_THRESHOLD);
...
static HxSocket_Client_t*	socket_client_Open(HxSocket_Type_e eSocketType, const HCHAR *pszIPAddr, void *targetPort, HINT32 nTry, const HCHAR * file, HUINT32 line)
{
	...
	do
	{
		...
		if (eSocketType == HxSOCKET_UDS)
			HxLOG_Critical("(UDS, %s) %d Fail to Connect with (\'%s\')\n", targetPort, failed, strerror(errno));
		else
			HxLOG_Critical("(INET, %d) %d Fail to Connect with (\'%s\')\n", (int)targetPort, failed, strerror(errno));
	...
	do
	{
		...
		if (info->flags & eHxSOCKET_RECONNECT)
		{
			if (socket_client_RepairConnection(info) >= 0)
				continue;
			if (timeout == SOCKET_FOREVER)
				HLIB_STD_TaskSleep(10);			/* Need to check value for performance. 100 is OK ? */
			else
				return -1;
		}
		else
...
#if 0
		if (timeout == SOCKET_FOREVER)
			HLIB_STD_TaskSleep(10);					/* Need to check value for performance. It should be removed. */
#endif
...
STATIC HERROR	msgsocket_Close(HINT32 sock)
{
	...
	case eHxSOCKET_CLIENT:
		...
		HxSEMT_Get(info->ulSemId);
		info->pSocketList = HLIB_LIST_Remove(info->pSocketList, info);
		HxSEMT_Release(info->ulSemId);
...
{
	HxLOG_Trace();
	HxLog_NullCheckReturnValue((HxSocket_t *)sock, ERR_FAIL);

	if (SOCKET_INVALIDMAGIC(sock))
	{
		HxLOG_Error("(%s) Invalid magic (0x%X)!!!\n", socket_ToString((HxSocket_t *)sock));
		return ERR_FAIL;
	}
				...
				)
{
	...
	HxLog_NullCheckReturnValue((HxSocket_t *)info, -1);

	if (SOCKET_ERROR(sock))
	{
		HxLOG_Error("(%s) error flag on!!!\n", socket_ToString(info));
		return -1;
	}

	if (SOCKET_INVALIDMAGIC(info))
	{
		HxLOG_Error("(%s) Invalid magic (0x%X)!!!\n", socket_ToString(info), info->magic);
		return -1;
	}
...
HERROR	HLIB_SOCKET_Write(HINT32 sock, const void *pvData, HUINT32 size, HUINT32 requestId)
{
	...
	HxLog_NullCheckReturnValue((HxSocket_t *)info, ERR_FAIL);
	...
	{
		hErr = socket_client_Buffering(info, pvData, size, requestId);
		HxSEMT_Release(info->ulSemId);
		return hErr;
	}
	hErr = socket_Write(info, pvData, size, requestId);
	HxSEMT_Release(info->ulSemId);
#else
	if (SOCKET_IS_CLIENT(info) && (info->flags & eHxSOCKET_RECONNECT))
	{
...
HUINT32	HLIB_SOCKET_GetClients (HINT32 sock, HINT32 *clients, HUINT32 n)
{
	...
	HxLog_NullCheckReturnValue((HxSocket_t *)info, 0);
...
HERROR	HLIB_SOCKET_SetSendBufferSize (HINT32 sock, HUINT32 ulSize)
{
	...
	if (setsockopt( info->fd, SOL_SOCKET, SO_SNDBUF , &size, sizeof(int) ) == -1 )
	{
		HxLOG_Error("set send size failed. \n");
		return ERR_FAIL;
	}
...
HERROR	HLIB_SOCKET_SetReceiveBufferSize (HINT32 sock, HUINT32 ulSize)
{
	...
	if (setsockopt( info->fd, SOL_SOCKET, SO_RCVBUF , &size, sizeof(int) ) == -1 )
	{
		HxLOG_Error("set send size failed. \n");
		return ERR_FAIL;
	}
