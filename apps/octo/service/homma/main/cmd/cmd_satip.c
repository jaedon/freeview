...
HINT32   cmd_satip_set_action(void *szArgList)
{
	...
	{
		HLIB_CMD_Printf("satip start\n");
		PL_SATIP_Start("SATIP TEST");
	}
	else if(VK_strcmp(aucStr, "STOP") == 0)
	{
		HLIB_CMD_Printf("satip stop\n");
		PL_SATIP_Stop();
	}
	else
	{
...
HINT32   cmd_satip_get_action(void *szArgList)
{
	HCHAR    *aucArg=NULL, aucStr[20];
...
void	HOMMA_CMD_Register_SATIP(void)
{
#if defined (CONFIG_DEBUG)
#define     hCmdHandle      "satip"
	HLIB_CMD_RegisterWord( hCmdHandle, cmd_satip_set_action, "set", "start | stop ", "satip set action");
	HLIB_CMD_RegisterWord( hCmdHandle, cmd_satip_get_action, "get", "dmslist | ", "satipget action");
#endif
}
