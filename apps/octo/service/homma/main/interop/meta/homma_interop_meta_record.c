...
HBOOL HOMMA_INTEROP_META_RECORD_Init(void)
{
	return APK_META_RECORD_Init();
}

void HOMMA_INTEROP_META_RECORD_EnableLoadEvent(void)
{
	APK_META_RECORD_EnableLoadEvent();
}

DxRecListData_t * HOMMA_INTEROP_META_RECORD_GetByUrl (const HCHAR *url)
{
	return APK_META_RECORD_GetByUrl(url);
}

void HOMMA_INTEROP_META_RECORD_Release (DxRecListData_t *recData)
{
	APK_META_RECORD_Release(recData);
}

void HOMMA_INTEROP_META_RECORD_AddNotifier(HOMMA_INTEROP_MetaRecord_Notifier_t notifier, void *userdata)
{
	APK_META_RECORD_AddNotifier(notifier, userdata);
}

#else // #ifdef HOMMA_USE_APPKIT_DIRECTLY else

//APK_META_RECORD_AddNotifier(__system_record_notifier,  NULL);
void HOMMA_INTEROP_META_RECORD_AddNotifier(OxMetaRecord_Notifier_t notifier, void *userdata)
{
	OAPI_META_RECORD_SetListener(notifier, NULL);
}
