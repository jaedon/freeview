...
#define DBG(x) do { printf("%-20s[%d] ", __func__, __LINE__); printf x; putchar('\n');} while(0)
#else
#define DBG(x)
#endif
...
int ns_set_ssl_ca_cert(struct ns_server *server, const char *cert) {  
  ...
  return SSL_CTX_load_verify_locations(server->ssl_ctx, cert, NULL) == 1 ? 0 : -2;
#endif
...
int ns_set_ssl_cert(struct ns_server *server, const char *cert) {
  ...
  if (cert != NULL &&
    ...
    return -2;
  } else if (SSL_CTX_use_PrivateKey_file(server->ssl_ctx, cert, 1) == 0) {
    return -4;
  }else {
...
static const char *status_code_to_str(int status_code) {
  switch (status_code) {
    ...
    case 451: return "Legal problems";
  ...
  *v = mg_strdup(value);
  ...
  if (ind == LISTENING_PORT) {
    ...
    if (res == -2) {
      ...
      error_msg = "Cannot load private-key";
	}else if (res == -3) {
