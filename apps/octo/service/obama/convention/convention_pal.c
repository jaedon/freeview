...
/* 모듈 내부 공유 헤더
	...
	_pal_ : Internal 표시 _ + 소문자 layer명 표시
	xxx_ : 소문자 module명 표시
	yyy.h : 소문자 sub module 표시

	설명 :
		PAL 모듈 내부에서 공유할 type 와 function 선언.
...
/* 모듈 파일의 local function
	...
	pal_ : 소문자로 layer 명 표시
	xxx_ : 소문자로 module 명 표시
	Function Name : 영문법을 지켜 동사 + 목적어 형식으로 대문자로 시작하는 단어를 이어 붙임
	
	설명 :
		PAL 모듈 안에서도 한 파일 안에서만 사용하기 위한 function으로 파일 내부 호출 용으로 사용.
