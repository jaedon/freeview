...
/* 모듈 헤더
	...
	xom_ : Extension을 타나내는 x + 소문자 layer명 표시
...
/* 모듈 내부 공유 헤더
	...
	_om_ : Internal 표시 _ + extension x 표시 + 소문자 layer명 표시
	xxx_ : 소문자 module명 표시
...
/* 모듈 파일의 local function
	...
	xom_ : Extension을 표시하는 x + 소문자로 layer 명 표시
	xxx_ : 소문자로 module 명 표시
	Function Name : 영문법을 지켜 동사 + 목적어 형식으로 대문자로 시작하는 단어를 이어 붙임
	ZZZ_ : 대문자로 파생명 표시
	
	설명 :
		OM 모듈 안에서도 한 파일 안에서만 사용하기 위한 function으로 파일 내부 호출 용으로 사용.
...
/* 모듈 내부의 global function
	...
	설명 :
		XOM 모듈이 여러 file로 구성될 경우 파일간에 공유하기 위한 global function으로 모듈 내부 파일간 호출 용으로 사용.
...
/* 모듈 API
	...
	XOM_ : Extension을 표시하는 X + 대문자로 layer 명 표시
...
/* 모듈 Procedure
	...
	xmproc_ : Extension을 표시하는 x + Manager bus 에 plug-in 되는 procedure 임을 나타냄
	xxx_ : 소문자로 module 명 표시
