...
// TODO: header 정리해서 아래 함수 include하도록 수정해야 함
extern BUS_Result_t xproc_cas_CiOperatorProfile(HINT32 message, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3);


HERROR om_builder_ciplussrch_StartSearch (HUINT32 ulBuilderId)
{
	...
	// TODO: 예전엔 AP_CAS_CIOP_SearchOperatorProfileProc() 이라는 mgr 사용하였으나
	// 지금은 xproc_cas_CiOperatorProfile()으로 변경된 것으로 보임
	// 그러나 eMEVT_SEARCH_ACTION_START 를 처리하지 않는 것으로 보여 이상함..
	// CI+ 담당자와 확인 필요
	pstContext->pfnSearchProc = xproc_cas_CiOperatorProfile;
...
HERROR om_builder_ciplussrch_StopSearch (HUINT32 ulBuilderId)
{
	omBldContext_t		*pstContext = om_builder_GetContextByBuilderId (ulBuilderId);
...
BUS_Result_t om_builder_ciplussrch_MsgSearchStatusChanged (Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	pstBuilderState->ulTotalTuningNum = 1;
	pstBuilderState->ulCompleteTuningNum = 1;
		...
		if (pstContext->ulSvcCount > 0)
		{
			...
			for (i = pstBuilderState->ulSearchedSvcNum; i < ulNum; i++)
			{
				...
				if(hErr == ERR_OK)
				{
					...
					pstBuilderSvcInfo->onId			= (HUINT32)DbSvc_GetOnId(&stSvcInfo);
					pstBuilderSvcInfo->tsId			= (HUINT32)DbSvc_GetTsId(&stSvcInfo);
					pstBuilderSvcInfo->svcId		= (HUINT32)DbSvc_GetSvcId(&stSvcInfo);
					...
					HxLOG_Debug("idx(%d=%d,%d) ulSvcNameLen(%d) svc(%s) \n",
							ulIdx, pstBuilderState->ulSearchedSvcNum, i,
							ulSvcNameLen,
	...
	if (hErr != ERR_OK)
	{
		HxLOG_Debug("[%s:%d] SVC_CH_GetSignalInfo() error !! actionId(%d) deliType(%d) \n",	__FUNCTION__, __LINE__, ulActionId, eDeliType);
	}
...
BUS_Result_t om_builder_ciplussrch_MsgSearchFinished (Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HUINT32					ulActionId = SVC_PIPE_GetActionId(hAction);
	omBldContext_t			*pstContext;
	BUS_Result_t			eResult = MESSAGE_PASS;
