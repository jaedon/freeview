...
********************************************************************/
/**
	@file	  om_builder_optplistsrch.c
	@brief
...
HERROR om_builder_optplistsrch_StartSearch (HUINT32 ulBuilderId)
{
	...
	// add op tp list, 지정된 tp list를 초기화 하고, action proc에서 추가될 수 있도록 한다
...
HERROR om_builder_optplistsrch_StopSearch (HUINT32 ulBuilderId)
{
	omBldContext_t		*pstContext = om_builder_GetContextByBuilderId (ulBuilderId);
...
BUS_Result_t om_builder_optplistsrch_MsgSearchStatusChanged (Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	if ((hErr == ERR_OK) && (ulNum > 0))
	{
		...
		if (pstContext->ulSvcCount > 0)
		{
			...
			for (i = pstBuilderState->ulSearchedSvcNum; i < ulNum; i++)
			{
				...
				if (hErr != ERR_OK)
				{
					HxLOG_Debug("MGR_SEARCHRESULT_GetService() error \n");
					continue;
				}
				...
				HxLOG_Debug("idx(%d=%d,%d) nameLen(%d) svc(%s) type(0x%x)\n"
...
BUS_Result_t om_builder_optplistsrch_MsgSearchFinished (Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HUINT32					ulActionId = SVC_PIPE_GetActionId(hAction);
	omBldContext_t			*pstContext;
	BUS_Result_t			eResult = MESSAGE_PASS;
