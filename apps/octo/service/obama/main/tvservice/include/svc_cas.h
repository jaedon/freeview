...
enum
{
	...
	*/
	...
	/* - Description : key update event.
		    NAGRA에서는 PRM key 변경으로 사용
		    CONAX에서는 PVR Record/Playback key 변경시에 사용
	   - Parameters Usage :
		   handle : HANDLE_NULL
		            NAGRA PRM : HANDLE_NULL
		            CONAX PVR : Record / Playback Handle
		   p1 : slot id, p2 : cas group id (nagra, irdeto, conax  등 ), p3 : 없음.
	*/
	eSEVT_CAS_UPDATE_KEYINFO,

	eSEVT_CAS_PRM_PVR_UPDATE_KEYINFO,

	// ----------------------------------------
	/* CONAX */
	eSEVT_CAS_CONAX_PLAYBACK_ERROR,

	// ----------------------------------------

	/* - Description : CAK에서 Novaspread로 TransportSessionId를 관리하기위해 Action Handle과 Svc Handle을 넘겨준다 
	    - Parameter Usage :
	    p1 : action handle, p2 : svc handle
	*/
	eSEVT_CAS_UPDATE_NOVA_DRM_HANDLE,
...
typedef enum
{
	...
	eCasShowType_RecStop = 4,						// recording  __SOFTCELL_REC_EXCEPTION__
	eCasShowType_Max
} SvcCas_ShowType_e;
