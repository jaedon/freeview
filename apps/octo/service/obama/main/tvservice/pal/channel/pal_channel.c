...
#if defined(CONFIG_MW_CH_SATELLITE) || defined(CONFIG_MW_CH_TERRESTRIAL) || defined(CONFIG_MW_CH_CABLE)
...
STATIC void pal_ch_DiNotifyCallback(HUINT32 ulInstanceId, HUINT32 ulRequestId, DI_CH_Type_e etChType, DI_CH_SignalInfo_e etSignal)
{
	switch (etSignal)
	{
		case DI_CH_SIGNAL_LOCKED:
			HxLOG_Message("[SATIP_TIME] DI_CH_SIGNAL_LOCKED\n");
		...
		case DI_CH_SIGNAL_UNLOCK:
			HxLOG_Message("[SATIP_TIME] DI_CH_SIGNAL_UNLOCK\n");
...
#endif //#if defined(CONFIG_MW_CH_SATELLITE) || defined(CONFIG_MW_CH_TERRESTRIAL) || defined(CONFIG_MW_CH_CABLE)

#if defined(CONFIG_DEBUG) && defined(CONFIG_MW_CH_EMULATION)
	PAL_CH_EMU_Init((void *)pal_ch_DiNotifyCallback);
#endif
...
{
	if(ulTunerGroupId >= NUM_MAX_ACTION)
	{
		*pulSupportDeliveryType = eDxDELIVERY_TYPE_ALL;
	}
	else
	{
#if defined(CONFIG_DEBUG) && defined(CONFIG_MW_CH_EMULATION)
		if (PAL_CH_EMU_CheckIpTuneReady() == TRUE)
		{
			PAL_CH_EMU_GetSupportDeliveryType(ulTunerGroupId, pulSupportDeliveryType);
			return;
		}
#endif
...
HUINT32 PAL_CH_GetNumOfTunerGroup(void)
{
	...
	if (PAL_CH_EMU_CheckIpTuneReady() == TRUE)
	{
		return PAL_CH_EMU_GetNumOfTunerGroup();
	}
#endif
...
void PAL_CH_SetLoopthroughGPIO(HBOOL bHigh)
{
#if defined(CONFIG_MW_CH_SATELLITE)
	DI_CH_S_SetLoopthGpio(bHigh);
#endif

#if defined(CONFIG_MW_CH_TERRESTRIAL)
	DI_CH_T_SetLoopthGpio(bHigh);
#endif

#if defined(CONFIG_MW_CH_CABLE)
	/* not implemented at this time */
	//DI_CH_C_SetLoopthGpio(bHigh);
#endif
}
