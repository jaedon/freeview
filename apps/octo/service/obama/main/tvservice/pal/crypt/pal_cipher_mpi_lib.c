 ...
 */
 ...
 #else
  #define  s_mp_setz(dp, count) HxSTD_memset(dp, 0, (count) * sizeof(mp_digit)) // memset(dp, 0, (count) * sizeof(mp_digit))
 #endif /* MP_MEMSET */
 ...
 #else
  #define  s_mp_copy(sp, dp, count) HxSTD_memcpy(dp, sp, (count) * sizeof(mp_digit)) // memcpy(dp, sp, (count) * sizeof(mp_digit))
 #endif /* MP_MEMCPY */

 #define  s_mp_alloc(nb, ni)  OxCAS_Calloc(nb * ni) // calloc(nb, ni)
 #define  s_mp_free(ptr) {if(ptr) OxCAS_Free(ptr);} // {if(ptr) free(ptr);}
#endif /* MP_MACRO */
...
static void s_mp_setz(mp_digit *dp, mp_size count)
{
  ...
  for(ix = 0; ix < count; ix++)
    dp[ix] = 0;
#else
  HxSTD_memset(dp, 0, count * sizeof(mp_digit));
#endif
...
static void s_mp_copy(mp_digit *sp, mp_digit *dp, mp_size count)
{
  ...
  for(ix = 0; ix < count; ix++)
    dp[ix] = sp[ix];
#else
  HxSTD_memcpy(dp, sp, count * sizeof(mp_digit));
#endif
...
{
  return OxCAS_Calloc(nb * ni); // calloc(nb, ni);
