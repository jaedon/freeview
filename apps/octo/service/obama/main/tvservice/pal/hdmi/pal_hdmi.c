...
#define palHdmi_MAX_AUDIO_FORMATS		16
/**************************************************************************/
/********************      Typedef            *****************************/
/**************************************************************************/
typedef struct
{
	...
	struct
	{
		DxAudioCodec_e eFormat;
		DxHdmiAudioSR_e eSupportSR;
	} stInfo[palHdmi_MAX_AUDIO_FORMATS];
...
STATIC HdmiAudioSupport_t			s_stHdmiAudioCapability;
...
STATIC const HINT8 *pal_hdmi_GetDiHdmiResString(DI_HDMI_VIDEO_RESOLUTION eDiHdmiRes)
{
	switch (eDiHdmiRes)
	{
		...
		ENUM_TO_STR(DI_HDMI_RESOLUTION_3840x2160P);
...
STATIC DxResolutionStatus_e pal_hdmi_convert_di2pal_Resolution(DI_HDMI_VIDEO_RESOLUTION eDiHdmiRes)
{
	switch (eDiHdmiRes)
	{
		...
		case DI_HDMI_RESOLUTION_3840x2160P:
			return eDxRESOLUTION_STATUS_2160P;
...
STATIC DxAudioCodec_e pal_hdmi_convert_di2pal_AudioFormat(DI_HDMI_AUDIO_FORMAT eDiHdmiAfmt)
{
	DxAudioCodec_e ePalCodec = eDxAUDIO_CODEC_UNKNOWN;

	switch (eDiHdmiAfmt)
	{
		case DI_HDMI_AFORMAT_PCM:
			ePalCodec = eDxAUDIO_CODEC_PCM; break;
		case DI_HDMI_AFORMAT_AC3:
			ePalCodec = eDxAUDIO_CODEC_DOLBY_AC3; break;
		case DI_HDMI_AFORMAT_MPEG1:
			ePalCodec = eDxAUDIO_CODEC_MPEG; break;
		case DI_HDMI_AFORMAT_MP3:
			ePalCodec = eDxAUDIO_CODEC_MP3; break;
		case DI_HDMI_AFORMAT_MPEG2:
			ePalCodec = eDxAUDIO_CODEC_MPEG; break;
		case DI_HDMI_AFORMAT_AAC:
			ePalCodec = eDxAUDIO_CODEC_AAC; break;
		case DI_HDMI_AFORMAT_DTS:
			ePalCodec = eDxAUDIO_CODEC_DTS; break;
		case DI_HDMI_AFORMAT_AC3PLUS:
			ePalCodec = eDxAUDIO_CODEC_DOLBY_AC3P; break;
		default:
			break;
	}
...
STATIC HBOOL pal_hdmi_UpdateHdmiStatus(void)
{
    ...
    if(stDiReceiverInfo.bIsConnected == FALSE)
    {
        //HDCP KEY 를 가진 TV 에서 NON-HDCP 로 HDMI 를 갈아 끼운 경우
        //이전 HDCP-STATUS 가 남아있어, HDCP CHECK 가 되지 않던 이슈
        //http://svn:3000/issues/105223
        //http://svn.humaxdigital.com:3000/issues/101333
...
STATIC HBOOL pal_hdmi_UpdateCapability(void)
{
	...
	DxAudioCodec_e eAFormat = eDxAUDIO_CODEC_UNKNOWN;
	...
	HxSTD_MemSet( &s_stHdmiAudioCapability, 0x00, sizeof(s_stHdmiAudioCapability) );
	...
	else
	{
		...
		/* Sample Rate of Audio Format */
		for ( i = 0; i < palHdmi_MAX_AUDIO_FORMATS; ++i )
		{
			eAFormat = pal_hdmi_convert_di2pal_AudioFormat(stDiCapa.tAudioInfo[i].ulAudioFormat);
			if ( eDxAUDIO_CODEC_UNKNOWN == eAFormat )
				continue;

			s_stHdmiAudioCapability.stInfo[i].eFormat = eAFormat;
			s_stHdmiAudioCapability.stInfo[i].eSupportSR = stDiCapa.tAudioInfo[i].unSupportedFs;
			s_stHdmiAudioCapability.nFormatCnt++;
		}
...
STATIC HBOOL pal_hdmi_GetEdidInfo(void)
{

	DI_HDMI_EDID_INFO		stDiEdidInfo;
	HBOOL					bResult = FALSE;

	if(DI_HDMI_GetEdidInfo(&stDiEdidInfo) != DI_ERR_OK)
	{
		HxLOG_Error("\t\tERROR from DI_HDMI_GetEdidInfo(). \n");
		return bResult;
	}

	HxSTD_MemCopy(s_stHdmiStatus.stHdmiEdidInfo.vendorID, stDiEdidInfo.vendorID, sizeof(stDiEdidInfo.vendorID));
	HxSTD_MemCopy(s_stHdmiStatus.stHdmiEdidInfo.productID, stDiEdidInfo.productID, sizeof(stDiEdidInfo.productID));
	HxSTD_MemCopy(s_stHdmiStatus.stHdmiEdidInfo.serialNum, stDiEdidInfo.serialNum, sizeof(stDiEdidInfo.serialNum));
	HxSTD_MemCopy(s_stHdmiStatus.stHdmiEdidInfo.monitorName, stDiEdidInfo.monitorName, sizeof(stDiEdidInfo.monitorName));

	s_stHdmiStatus.stHdmiEdidInfo.manufWeek = stDiEdidInfo.manufWeek;
	s_stHdmiStatus.stHdmiEdidInfo.manufYear = stDiEdidInfo.manufYear;
	s_stHdmiStatus.stHdmiEdidInfo.edidVersion = stDiEdidInfo.edidVersion;
	s_stHdmiStatus.stHdmiEdidInfo.edidRevision = stDiEdidInfo.edidRevision;
	s_stHdmiStatus.stHdmiEdidInfo.firstVideoId = stDiEdidInfo.firstVideoId;
	s_stHdmiStatus.stHdmiEdidInfo.maxHorizSize = stDiEdidInfo.maxHorizSize;
	s_stHdmiStatus.stHdmiEdidInfo.maxVertSize= stDiEdidInfo.maxVertSize;
...
STATIC HBOOL pal_hdmi_CheckHdcp(void)
{
	...
	if(s_stHdmiStatus.bPlug == FALSE)
	{
		if( (s_stHdmiStatus.bOutputEnable != TRUE) || (s_stHdmiStatus.bHdcpEnable != TRUE))
		{
			if(s_stHdmiStatus.eHdcpStatus == eDxHDCPSTATUS_OFF)
			{
				/* This condition is TV off and go shutdown. HDCP has to be disabled. */
				diErr = DI_HDMI_EnableHDCP(FALSE);
				if(diErr != ERR_OK)
				{
					HxLOG_Error("\t\t DI_HDMI_EnableHDCP() Error !! (diErr:0x%X)\n",diErr);
				}
				HxLOG_Debug("HDMI unPlug.. Set hdcp status nodevice ...\n");
				return pal_hdmi_SetHdcpState(eDxHDCPSTATUS_NODEVICE);
			}
		}
	...
	{ /* HDCP 켜야 하는 상황 */
		if(s_stHdmiStatus.eHdcpStatus == eDxHDCPSTATUS_OFF || s_stHdmiStatus.eHdcpStatus == eDxHDCPSTATUS_NODEVICE)
		{ /* HDCP 안 켜져 있음. 켜야 함. */
...
STATIC HERROR pal_hdmi_PrintStatus(HBOOL bDebug)
{
	...
	if(bDebug == TRUE)
	{
		...
		HxLOG_Debug("\t\tEdidInfo x:%d,y:%d \n", s_stHdmiStatus.stHdmiEdidInfo.maxHorizSize,s_stHdmiStatus.stHdmiEdidInfo.maxVertSize);
	}
	else
	{
		...
		HLIB_CMD_Printf("\t\tEdidInfo x:%d,y:%d \n", s_stHdmiStatus.stHdmiEdidInfo.maxHorizSize,s_stHdmiStatus.stHdmiEdidInfo.maxVertSize);
	}
#endif
...
HERROR PAL_HDMI_GetHdcpVersion(DxHdcpVersion_e *pVersion)
{
	DI_ERR_CODE di_err;
	DI_HDMI_HDCP_VERSION	eHdcpVersion = eDxHDCP_VERSION_MAX;

	di_err = DI_HDMI_GetHdcpVersion(&eHdcpVersion);
	if(di_err != DI_ERR_OK)
		return ERR_FAIL;

	switch(eHdcpVersion)
	{
	case DI_HDMI_HDCP_VERSION_1_0:
		eHdcpVersion = eDxHDCP_VERSION_1_0;
		break;
	case DI_HDMI_HDCP_VERSION_1_1:
		eHdcpVersion = eDxHDCP_VERSION_1_1;
		break;
	case DI_HDMI_HDCP_VERSION_1_2:
		eHdcpVersion = eDxHDCP_VERSION_1_2;
		break;
	case DI_HDMI_HDCP_VERSION_1_3:
		eHdcpVersion = eDxHDCP_VERSION_1_3;
		break;
	case DI_HDMI_HDCP_VERSION_1_4:
		eHdcpVersion = eDxHDCP_VERSION_1_4;
		break;
	case DI_HDMI_HDCP_VERSION_2_0:
		eHdcpVersion = eDxHDCP_VERSION_2_0;
		break;
	case DI_HDMI_HDCP_VERSION_2_1:
		eHdcpVersion = eDxHDCP_VERSION_2_1;
		break;
	case DI_HDMI_HDCP_VERSION_2_2:
		eHdcpVersion = eDxHDCP_VERSION_2_2;
		break;
	default:
		eHdcpVersion = eDxHDCP_VERSION_MAX;
		break;
	}
	*pVersion = eHdcpVersion;
	HxLOG_Debug("eHdcpVersion[%d]\n",eHdcpVersion);
...
HERROR PAL_HDMI_IsSupportSampleRate(DxAudioCodec_e eAudioFormat, DxHdmiAudioSR_e eSamplingFreq, HBOOL *pbSupport)
{
        HINT32 i = 0;
        HINT32 nSupports = 0;
        if( TRUE != s_stHdmiStatus.bCapabilityValid || NULL == pbSupport)
                return ERR_FAIL;

        for ( i = 0; i < s_stHdmiAudioCapability.nFormatCnt; ++i )
        {
                if ( eAudioFormat == s_stHdmiAudioCapability.stInfo[i].eFormat )
                {
                        nSupports = s_stHdmiAudioCapability.stInfo[i].eSupportSR;
                        break;
                }
        }

        if ( nSupports & eSamplingFreq )
                *pbSupport = TRUE;
        else
                *pbSupport = FALSE;
