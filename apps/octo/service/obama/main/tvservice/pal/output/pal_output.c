...
STATIC HUINT32 						s_ulDecoderVolume;					/* Audio Volume */
...
STATIC HUINT32 pal_out_CheckComposite(void)
{
	...
	HBOOL				bAnalogOutout = TRUE;

	PAL_HDMI_GetStatus(&stHdmiStatus);

	if( s_bStbOutputEnable == TRUE)
	{
		if(stHdmiStatus.bPlug==TRUE)
		{
			// hdmi 연결된상태에 따라, analog output 을 결정한다.
			if(s_bStbPriorityHDMIoutput == TRUE)
				bAnalogOutout = FALSE;
		}
	}
	else
	...
	HxLOG_Debug(HxANSI_COLOR_YELLOW("s_bStbOutputEnable:[%d], s_bStbPriorityHDMIoutput:[%d]\n"),s_bStbOutputEnable,s_bStbPriorityHDMIoutput);
	HxLOG_Debug(HxANSI_COLOR_YELLOW("stHdmiStatus.bPlug[%d]\n"),stHdmiStatus.bPlug);
	HxLOG_Debug(HxANSI_COLOR_YELLOW("bAnalogOutout[%d]\n"),bAnalogOutout);

	/* 항상 둘다 enable 해 놓고 switch로 선택 하는 것이 DI의 요구 사항임. */
	s_stDiAnalogVideoOut.bCVBS = bAnalogOutout;
	s_stDiAnalogVideoOut.bRGB = bAnalogOutout;
	...
	PAL_SCART_SetOutput(bAnalogOutout);
#endif
	/*Depend on CONFIG_MW_OUTPUT_HDMI_PRIORITY*/
	PAL_SCALER_SetCompositorOsdOutput(bAnalogOutout);
...
STATIC HUINT32 pal_out_CheckAudioMute(void)
{
	...
	else
	{
	 	...
	 	{
			bDiAudioMute = TRUE;
			HxLOG_Debug("\t\tAudio Mute (User:%d, Decoder:%d, Bluetooth:%d, Hdmi:%d).\n"
									, s_bMasterAudioMute
									, s_bDecoderAudioMute
...
HERROR pal_out_CheckHdmiAudioOut(void)
{
	...
	HxLOG_Debug("\t(%s)\n", __FUNCTION__);
	HxLOG_Debug("\t\tHDMI Audio Out Format : %s, Main Stream Codec : %s, Sub Stream Codec\n"
								, OTL_ENUM2STR_DigitalAudioOutput(s_eHdmiAudioFormat)
								, OTL_ENUM2STR_AudioCodec(s_eMainStreamAudioCodec)
								,OTL_ENUM2STR_AudioCodec(s_eSubStreamAudioCodec));

	if(s_eSubStreamAudioCodec != eDxAUDIO_CODEC_UNKNOWN)
	{
		HxLOG_Debug("\t\t\tTranscode NOT setup. Force HDMI PCM\n");
	...
	else
	{
		...
		{
			if(PAL_HDMI_CheckAudioCodecAvailable(s_eMainStreamAudioCodec) == ERR_OK)
			{
				HxLOG_Debug("\t\tBypass stream codec %s\n", OTL_ENUM2STR_AudioCodec(s_eMainStreamAudioCodec));
				s_eHdmiAudioCodec = s_eMainStreamAudioCodec;

				eDiHdmiAudioMode = DI_AUDOUT_HDMI_BYPASS;
			}
			else if(PAL_HDMI_CheckAudioCodecAvailable(s_eHdmiTransCodec[s_eMainStreamAudioCodec]) == ERR_OK)
			{
				HxLOG_Debug("\t\tTranscode stream codec %s to %s\n"
					, OTL_ENUM2STR_AudioCodec(s_eMainStreamAudioCodec)
					, OTL_ENUM2STR_AudioCodec(s_eHdmiTransCodec[s_eMainStreamAudioCodec])
					);

				s_eHdmiAudioCodec = s_eHdmiTransCodec[s_eMainStreamAudioCodec];

				eDiHdmiAudioMode = DI_AUDOUT_HDMI_TRANSCODING;
				pal_out_ConvertAudioTransCodecPal2Di(s_eHdmiTransCodec[s_eMainStreamAudioCodec], &eDiHdmiTransCode);
			}
			else
			{
...
HERROR pal_out_CheckSpdifAudioOut(void)
{
	...
	HxLOG_Debug("\t%s()\n", __FUNCTION__);
	HxLOG_Debug("\t\tSPDIF Audio Out Format : %s, Main Stream Codec : %s, Sub Stream Codec : %s\n"
	...
	else
	{
		...
		{
			if(s_eMainStreamAudioCodec == s_eSpdifTransCodec[s_eMainStreamAudioCodec])
			{
				HxLOG_Debug("\t\t\tSelect SPDIF BYPASS\n");
				s_eSpdifAudioCodec = s_eMainStreamAudioCodec;
				eDiSpdifAudioMode = DI_AUDOUT_SPDIF_BYPASS;
			}
			else if(s_eSpdifTransCodec[s_eMainStreamAudioCodec] != eDxAUDIO_CODEC_UNKNOWN)
			{
				HxLOG_Debug("\t\tTranscode stream codec %s to %s\n"
					, OTL_ENUM2STR_AudioCodec(s_eMainStreamAudioCodec)
					, OTL_ENUM2STR_AudioCodec(s_eSpdifTransCodec[s_eMainStreamAudioCodec])
					);

				s_eSpdifAudioCodec = s_eSpdifTransCodec[s_eMainStreamAudioCodec];
				eDiSpdifAudioMode = DI_AUDOUT_SPDIF_TRANSCODING;
				pal_out_ConvertAudioTransCodecPal2Di(s_eSpdifTransCodec[s_eMainStreamAudioCodec], &eDiSpdifTransCode);
			}
			else
			{
				HxLOG_Debug("\t\t\tTranscode NOT setup. Force SPDIF PCM\n");
...
HERROR pal_out_GetHighPriorityHD(HBOOL *pEnable)
{
#if defined(CONFIG_MW_OUTPUT_HDMI_PRIORITY)
	DI_ERR_CODE			diErr;
	HINT32 				nDataSize =0;

	diErr = DI_NVRAM_GetSize(DI_NVRAM_FS_PARTITION_RW,(HUINT8*)HDMI_PRIORITY,&nDataSize);
	if(diErr != DI_ERR_OK || nDataSize <= 0) // set default nvram value by DB_PARAM
	{
		DB_PARAM_SetItem(eDB_PARAM_ITEM_PRIORITY_HDMIOUTPUT,(HUINT32)TRUE, 0);
		PAL_OUT_SetHighPriorityHD(TRUE); // write nvram..
	}
	else
	{
		HBOOL bDbValue = FALSE;
		(void)DI_NVRAM_Read(DI_NVRAM_FS_PARTITION_RW,(HUINT8*)HDMI_PRIORITY, 0,&s_bStbPriorityHDMIoutput,sizeof(HBOOL));
		if(DB_PARAM_GetItem(eDB_PARAM_ITEM_PRIORITY_HDMIOUTPUT,&bDbValue,0) == ERR_OK)
		{
			if(s_bStbPriorityHDMIoutput != bDbValue) // May be factory reset..
			{
				s_bStbPriorityHDMIoutput = bDbValue;
				HxLOG_Error("My be Factory default...[%d]\n",s_bStbPriorityHDMIoutput);
				PAL_OUT_SetHighPriorityHD(s_bStbPriorityHDMIoutput);
			}
		}
	}
#endif
	HxLOG_Debug("s_bStbPriorityHDMIoutput[%d]\n",s_bStbPriorityHDMIoutput);

	return ERR_OK;
}

HERROR pal_out_GetVideoBlankCopyRight(CopyRight_t *pCopyRight)
{
  	DI_VIDEO_WSS_SETTINGS_t stSet;
/*
0	0	CopyFreely	Unlimited copies may be made of the content.
0	1	CopyNoMore* One generation of copies has already been made; no further copying is allowed.
1	0	CopyOnce	One generation of copies may be made
1	1	CopyNever	No copies may be made of the content.
*/
	if(DI_VIDEO_GetWSS(&stSet) != DI_ERR_OK)
	{
		HxLOG_Error("[ERROR] DI_VIDEO_GetWSS()\n");
	}

    if(stSet.eCpbit == DI_VIDEO_WSS_NO_CP && stSet.eGebit == DI_VIDEO_WSS_GE_NO_REST)
        *pCopyRight = eCOPYRIGHT_COPY_FREE;
    else if(stSet.eCpbit == DI_VIDEO_WSS_NO_CP && stSet.eGebit == DI_VIDEO_WSS_GE_REST)
        *pCopyRight = eCOPYRIGHT_COPY_NOMORE;
    else if(stSet.eCpbit == DI_VIDEO_WSS_CP && stSet.eGebit == DI_VIDEO_WSS_GE_NO_REST)
        *pCopyRight = eCOPYRIGHT_COPY_ONCE;
    else
        *pCopyRight = eCOPYRIGHT_COPY_NEVER;

    HxLOG_Print("CopyRight[%d]\n",*pCopyRight);
    return ERR_OK;
}
...
STATIC void pal_out_PrintStatus(void)
{
	...
	HLIB_CMD_Printf(" bStbOutputEnable : %d , s_bStbPriorityHDMIoutput: %d \n", s_bStbOutputEnable,s_bStbPriorityHDMIoutput);

	HLIB_CMD_Printf(" bUserAudioMute : %d \n", s_bMasterAudioMute);
	HLIB_CMD_Printf(" bDecoderAudioMute : %d \n", s_bDecoderAudioMute);
	HLIB_CMD_Printf(" s_bBluetoothAudioMute : %d \n", s_bBluetoothAudioMute);

	HLIB_CMD_Printf(" Main eStreamAudioCodec : %s \n", OTL_ENUM2STR_AudioCodec(s_eMainStreamAudioCodec));
	HLIB_CMD_Printf(" Sub eStreamAudioCodec : %s \n", OTL_ENUM2STR_AudioCodec(s_eSubStreamAudioCodec));
	...
	HLIB_CMD_Printf(" ulDecoderVolume : %d \n", s_ulDecoderVolume);
...
STATIC int pal_out_ProcessCommand_analog_output(void *szArgList)
{
	char			*aucArg=NULL;
	...
	if( aucArg == NULL )
		return HxCMD_ERR;
	if(HxSTD_StrCmp(aucArg, "block") == 0)
		s_bStbPriorityHDMIoutput = TRUE;
	else if(HxSTD_StrCmp(aucArg, "unblock") == 0)
		s_bStbPriorityHDMIoutput = FALSE;
	else
		return HxCMD_ERR;
...
STATIC void pal_out_RegisterCommand(void)
{
	...
	HLIB_CMD_RegisterWord(hCmdHandle,
						...
						"show_sema4lock");
	HLIB_CMD_RegisterWord(hCmdHandle,
						pal_out_ProcessCommand_analog_output,
						"analog",
						"Set Analog output block or unblock",
						"block /unblock");
}
#endif
...
#endif
	pal_out_GetHighPriorityHD(&s_bStbPriorityHDMIoutput); //initialize status
	return ERR_OK;
}
...
HERROR PAL_OUT_SetHighPriorityHD(HBOOL bEnable)
{
	...
	ENTER_CRITICAL_PALOUT;
#if defined(CONFIG_MW_OUTPUT_HDMI_PRIORITY)
	if(bEnable != s_bStbPriorityHDMIoutput)
	{
		HxLOG_Debug("DI_NVRAM_Write(HDMI_PRIORITY:%d)\n",bEnable);
		diErr = DI_NVRAM_Write(DI_NVRAM_FS_PARTITION_RW, (HUINT8*)HDMI_PRIORITY, 0, &bEnable, sizeof(HBOOL));
		if(diErr != DI_ERR_OK)
		{
			HxLOG_Error("\t\tERROR DI_NVRAM_Write DI_NVRAM_FS_PARTITION_RW Eror!!!\n");
		}
	}
	s_bStbPriorityHDMIoutput = bEnable;
	pal_out_CheckComposite();
#endif
	LEAVE_CRITICAL_PALOUT;
...
HERROR PAL_OUT_SetHdmiAudioMute(HBOOL bMute)
{
	...
	if ( s_bHdmiAudioMute != bMute )
	{
		s_bHdmiAudioMute = bMute;
		DI_AUDIO_SetMute(DI_AUDIO_OUTPUT_HDMI, bMute);
	}
...
{
	DI_ERR_CODE diErr;
	DI_AUDIO_RUNNING_STATE nAudioState = DI_AUDIO_STATE_UNKNOWN;

	ENTER_CRITICAL_PALOUT;

	HxLOG_Debug("%s(%d)\n", __FUNCTION__, ulVolume);

	s_ulDecoderVolume = ulVolume;

	diErr = DI_AUDIO_GetState(1, &nAudioState);
	if(diErr == DI_ERR_OK && nAudioState != DI_AUDIO_STATE_UNKNOWN && DI_AUDIO_SetDecoderVolume(1, ulVolume) != ERR_OK)
	{
		LEAVE_CRITICAL_PALOUT;
		return ERR_FAIL;
	}
...
HERROR PAL_OUT_SetDigitalAudioOutCodec(DxAudioCodec_e eMainStreamAudioCodec, DxAudioCodec_e eSubStreamAudioCodec)
{
	...
	HxLOG_Debug("%s(Main : %s, Sub : %s)\n", __FUNCTION__, OTL_ENUM2STR_AudioCodec(eMainStreamAudioCodec), OTL_ENUM2STR_AudioCodec(eSubStreamAudioCodec));

	s_eMainStreamAudioCodec = eMainStreamAudioCodec;
	s_eSubStreamAudioCodec = eSubStreamAudioCodec;
							...
							)
{
	...
	{
		pal_out_GetVideoBlankCopyRight(&s_eVideoBlankCopyRight);
...
#if defined(CONFIG_MW_OUTPUT_HDAUDIO_SR_RESTRICT)
		if(bIsHRA)
			return DI_AUDIO_GetHRAOutputSampleRate((DI_AUDIO_SAMPLE_RATE_e)eInput, (DI_AUDIO_SAMPLE_RATE_e *)peOutput);
		else
			return DI_AUDIO_GetOutputSampleRate((DI_AUDIO_SAMPLE_RATE_e)eInput, (DI_AUDIO_SAMPLE_RATE_e *)peOutput);
#else
	switch (eInput)
	{
		case eDxHDMI_AUDIO_SR_32KHz :
			*peOutput = eDxHDMI_AUDIO_SR_32KHz; break;
		case eDxHDMI_AUDIO_SR_44KHz :
			*peOutput = eDxHDMI_AUDIO_SR_44KHz; break;
		case eDxHDMI_AUDIO_SR_48KHz :
			*peOutput = eDxHDMI_AUDIO_SR_48KHz; break;
		case eDxHDMI_AUDIO_SR_88KHz :
			*peOutput = eDxHDMI_AUDIO_SR_88KHz; break;
		case eDxHDMI_AUDIO_SR_96KHz :
			*peOutput = eDxHDMI_AUDIO_SR_96KHz; break;
		case eDxHDMI_AUDIO_SR_176KHz :
			*peOutput = eDxHDMI_AUDIO_SR_88KHz; break;
		case eDxHDMI_AUDIO_SR_192KHz :
			*peOutput = eDxHDMI_AUDIO_SR_96KHz; break;
		default :
			return ERR_FAIL;
	}
