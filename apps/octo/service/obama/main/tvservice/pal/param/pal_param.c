...
HCHAR *pal_param_GetItemStr(PAL_PARAM_Item_e eItem)
{
	...
	case ePAL_PARAM_LAUNCH_APP_MODE:					return palNameOf(ePAL_PARAM_LAUNCH_APP_MODE);
...
STATIC HERROR pal_param_ConverterFusingInfo(HBOOL bGet, PAL_PARAM_Item_e eItem, const void *pvSource, void *pvTarget)
{
	...
	case ePAL_PARAM_FUSING_INFO:
		if (bGet)	// DI -> PAL
		{
			...
			pstPalParam->ucRAMSCR = pstDiParam->ucRAMSCR;
			hErr = ERR_OK;
		}
		else		// PAL -> DI
		{
			...
			pstDiParam->ucRAMSCR = pstPalParam->ucRAMSCR;
			hErr = ERR_OK;
		}
		break;
	default:
...
STATIC HERROR pal_param_GetPackageVersion(HUINT8 *pucString)
{
	DI_ERR_CODE 	hErr;
	HUINT8 aucVer[4];
	HUINT32 ulTmpVersion = 0;
...
STATIC HERROR pal_param_SetNetflixESN(void* pucESN, HUINT32 ulSize)
{	
    DI_ERR_CODE		 hErr=0;

    hErr = DI_NVRAM_Write(DI_NVRAM_FS_PARTITION_RW, NETFLIX_ESN, 0, pucESN, ulSize);

	return (hErr == DI_ERR_OK) ? ERR_OK : ERR_FAIL;
}

STATIC HERROR pal_param_GetNetflixESN(void* pszESN, HINT32 nDataSize)
{	
    DI_ERR_CODE		 hErr;
    ...
    hErr = DI_NVRAM_Read(DI_NVRAM_FS_PARTITION_RW, NETFLIX_ESN, 0, pszESN, nDataSize);
...
#if defined(CONFIG_NETFLIX_APP)
	case ePAL_PARAM_NETFLIX_ESN:
        	{
            		hErr = pal_param_SetNetflixESN(pvData, ulSize);
        	}        
        	break;
#endif
...
#if defined(CONFIG_OTA_USE_PKG_VERSION)
	case ePAL_PARAM_SYSTEM_PACKAGE_VERSION:
		hErr = pal_param_GetPackageVersion((HUINT8 *)pvData);
		break;
#endif

#if defined(CONFIG_NETFLIX_APP)
	case ePAL_PARAM_NETFLIX_ESN:
        	hErr = pal_param_GetNetflixESN(pvData, (HINT32)ulSize);
        	break;    
#endif
