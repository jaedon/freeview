...
* requestId
* 0x   fff        ff       fff
*     order  demux   requestId
***********************************/
#define palSef_INVALID_REQUEST      		0x0fff
#define palSef_REQUEST_MASK					0x0fff
#define palSef_MAX_SEF_SENDING_MSG_Q_NUM			12		/* 시스템에서 사용 가능한 최대 sending msg queue 개수 - request id의 상위 12비트를 활용 */
#define palSef_MAKE_REQUESTID_WITH_DEMUX_AND_QID(ucDemuxId, ulReqId, nOrderIdx)	\
{ 												\
	ulReqId |= (0x80000000 >> nOrderIdx); 		\
	ulReqId |= ucDemuxId << 12;					\
}
#define palSef_MAKE_ORDERIDX_FROM_REQUESTID(nOrderIdx, ulReqId)	\
{                                                                       \
	HUINT32		ulTempReqId = (ulReqId & 0xfff00000);                   \
                                                                        \
	nOrderIdx = 0;                                                      \
...
typedef struct
{
	...
	palSef_DmxRequest_t	*astRequestList;	/* array처럼 사용할 것임. */
} palSef_Context_t;
...
typedef struct
{
	...
	palSef_DmxRequest_t	**ppstDmxRequestHead;	/* demux 개수 만큼 */
	palSef_Context_t	 *pstContext;			/* demux 개수 만큼 */
...
HERROR PAL_SEF_Init(void)
{
	...
	s_stSefContainer.pstContext = (palSef_Context_t *)OxMW_Malloc(s_stSefContainer.ulNumOfTotalDemux * sizeof(palSef_Context_t));
	HxSTD_memset(s_stSefContainer.pstContext, 0, s_stSefContainer.ulNumOfTotalDemux * sizeof(palSef_Context_t));

	s_stSefContainer.ppstDmxRequestHead = (palSef_DmxRequest_t **)OxMW_Malloc(s_stSefContainer.ulNumOfTotalDemux * sizeof(palSef_DmxRequest_t *));
	...
	nRet = VK_SEM_Create((HULONG *)&s_stSefContainer.ulRequestIdSem, "PSefSem", VK_SUSPENDTYPE_PRIORITY);
	if (VK_OK != nRet)
	{
		HxLOG_Critical("VK_SEM_Create err:\n");
		return ERR_FAIL;
	}
									...
									HBOOL		bCrcCheck)
{
	...
	palSef_MAKE_REQUESTID_WITH_DEMUX_AND_QID(ucDemuxId, ulReqId, nOrderIdx);
	pstRequest->ulRequestId				= ulReqId;
	pstRequest->ulUserData				= ulUserData;
	...
	/*
	  ulEmergencyStopMsgQid가 있을 경우만 Package기능이 On이 된다면
         ...
         더군다나 PAL_SEF_EnableSectionFilter가 MHEG과 RAW EIT Manager에서 사용되도록 협의됨에 따라
...
void pal_sef_PrintContext(HINT32 nPrintLevel)
{
	...
	HLIB_CMD_Printf(HxANSI_COLOR_YELLOW("s_stSefContainer.nNumOfUsedSecFilter  = %d\n"), s_stSefContainer.nNumOfUsedSecFilter);
	HLIB_CMD_Printf("s_stSefContainer.nNumOfTotalSecFilter = %d\n", s_stSefContainer.nNumOfTotalSecFilter);
...
STATIC void pal_sef_MakeRequestArray(HINT32 nNumOfFilter, palSef_DmxRequest_t **ppstReqHead)
{
	...
	pHeadList = (palSef_DmxRequest_t *)OxMW_Malloc(nNumOfFilter * sizeof(palSef_DmxRequest_t));
 ...
 * Description : pal_sef_GetRequest()함수는 PAL_SEF_DisableSectionFilter()
                       와 pal_sef_CbOnSection()함수안에서 정확하게 Match되는
...
STATIC palSef_DmxRequest_t *pal_sef_CheckExistRequestId(palSef_Context_t *pstContext, HUINT32 ulRequestId)
{
	...
	for(nReqIdx = 0; nReqIdx < pstContext->nSecFilterCapa; nReqIdx++)
	{
		...
		{
			HxLOG_Info("pstContext->astRequestList[%02d].ulRequestId:0x%X,ulRequestId:0x%X\n",
				nReqIdx,pstContext->astRequestList[nReqIdx].ulRequestId,ulRequestId);
			if((pstContext->astRequestList[nReqIdx].ulRequestId & palSef_REQUEST_MASK) == (ulRequestId & palSef_REQUEST_MASK))
			{ // found
...
STATIC HBOOL pal_sef_CheckData(PalSef_ReceiveMessage_t *pstRcvMsg, palSef_DmxRequest_t *pstRequest, HUINT8 *pucBuf)
{
	if(pstRequest->nSecCntInPackage > 1)
	{
		if(pstRequest->pstRawPackage == NULL)
		{
			pstRequest->pstRawPackage = (PalSef_RawData_t *)OxMW_Malloc(sizeof(PalSef_RawData_t));
		...
		{
			HxLOG_Error("Error : can't reach here\n");
			pal_sef_FreePackageData(pstRequest->pstRawPackage);
			pstRequest->pstRawPackage = NULL;
...
STATIC HERROR pal_sef_CheckEmergency(PalSef_ReceiveMessage_t *pstRcvMsg, PalSef_SendMessage_t *pstEmergencyMsg, HUINT32 *pulMsgQ, HBOOL *pbIsEmergency, HBOOL bEmergencyMsgSended)
{
	...
	*pbIsEmergency = FALSE;
	...
	if(ulCurMsgCnt >= (ulMaxMsgCnt / 2)) /* 50% 이상이면 nearly full */
	{
		...
		*/
		HxLOG_Print("[WARN] si recv msgq - nearly full (%d/%d) (req_id=0x%x)\n", ulCurMsgCnt, ulMaxMsgCnt, pstRcvMsg->ulRequestId);
		//HxLOG_Error("\\#");
...
STATIC DI_ERR_CODE pal_sef_CbOnSection(HUINT8 ucDemuxId, HUINT8 *pucBuf, HUINT32 ulRequestId)
{
	...
	if(pucBuf == NULL)
	{
		HxLOG_Error("[%d] Error : pucBuf is NULL : ulRequestId = 0x%08X\n", ucDemuxId, ulRequestId);
		return DI_ERR_ERROR;
	}
//++ for_debugging +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#ifdef _PAL_SEF_ALLOC_FREE_DEBUG_
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	if(pstRequest == NULL)
	{
		HxLOG_Print("[%d] Error : Cannot find Demux : ulRequestId = 0x%08X\n", ucDemuxId, ulRequestId);
		pal_sef_FreeDemuxSection(pucBuf);
		return DI_ERR_OK;
	}
	...
	stRcvMsg.ulUserData		= pstRequest->ulUserData;
...
#ifdef _PAL_SEF_ALLOC_FREE_DEBUG_
	*ppucBuf = (HUINT8 *)OxSI_Malloc(usBufSize+4);
#else
	*ppucBuf = (HUINT8 *)OxSI_Malloc(usBufSize);
#endif
	if (*ppucBuf == 0) {
