...
#endif
//#define WORKTHROUGH_RETRY_FILTER_WHEN_FAILED
											...
											PalSef_Callback_t	 pfnFree)
{
	...
	pstSiFilterParam->eTimeoutMode  = eTimeoutMode;
...
HUINT16 PAL_SEF_StartSecFilter(PalSef_FilterParam_t *pstSiFilterParam)
{
	...
	pstNewSiFilterParam = (PalSef_FilterParam_t*)OxSI_Malloc(sizeof(PalSef_FilterParam_t));
...
#if defined(WORKTHROUGH_RETRY_FILTER_WHEN_FAILED)
	{
		HUINT32			 ulCnt;

		for (ulCnt = 0; ulCnt < 15; ulCnt++)
		{
			ret = PAL_SEF_EnableSectionFilter(ucDemuxId, usPid,
												...
												pstFilterTable->eFilterLen, pstFilterTable->aucFilterValue, pstFilterTable->aucFilterMask, s_aucFilterMode, 0,
...
STATIC	HUINT32	pal_sef_ThreadRestartFilter(palSef_FilterTable_t *pstFilterTable)
{
	...
	ret = PAL_SEF_EnableSectionFilter(pstFilterTable->ucDemuxId, pstFilterTable->usPid, s_ulSiRcvMsgQId, s_ulSiSendMsgQId, pstFilterTable->nSecCntInPackage, pstFilterTable->nDurationInPackage, pstFilterTable->eFilterLen, pstFilterTable->aucFilterValue, pstFilterTable->aucFilterMask, s_aucFilterMode, 0, &pstFilterTable->ulLoadId, pstFilterTable->bCheckCrc);
	if(ret == ERR_OK)
	{
		if(pstFilterTable->stTagData.ulTimeOut > 0)
		{
...
void		pal_sef_PrintFilterTable(HINT32 nPrintLevel, HUINT16 usPid)
{
	...
	pal_sef_PrintContext(nPrintLevel);
}
