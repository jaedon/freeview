...
//	eHxCP_CodePage_e             eEncoding;                                  /**< Encoding Type. */
...
#if DISPLAY_LANG_Hx
	HxLANG_Id_e				eDispLang[SVC_CSO_SUB_LANGUAGE_MAX];      /* 전체 Language 종류. */
#else
	...
	POINTER_LIST_T              *pHeadPacket[SVC_CSO_SUB_LANGUAGE_MAX];   /* packet의 Head Pointer. */
#endif
	POINTER_LIST_T              *pCurrPacket[SVC_CSO_SUB_LANGUAGE_MAX];   /* 현재 Display 되고 있는 packet의 Pointer. */
} SvcCsoSub_Context_t;
...
static HULONG			s_ulSvcCsoSubSemaphore;			/**< Data 보호용. */
