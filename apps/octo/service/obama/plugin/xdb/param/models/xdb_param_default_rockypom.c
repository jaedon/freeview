/**
	@file     db_param_default_rockypom.c
	@brief    ROCKY-PoM project default data

	Description: default db 생성을 위한 default value 모음 (for CONFIG_PROD_ROCKYPOM)
	Module: DB/default \n
	Remarks : 										\n
...
STATIC const dbParam_DefaultItem_t	s_astHardCodingSystemItemList_ROCKYPOM[] =
{
...
HINT32 xdb_default_GetSystemConfigItemNum_ROCKYPOM(void)
{
	return sizeof(s_astHardCodingSystemItemList_ROCKYPOM) / sizeof(dbParam_DefaultItem_t);
}

dbParam_DefaultItem_t	*xdb_default_GetSystemConfigItem_ROCKYPOM(HUINT32 ulItemIdx)
{
	if (xdb_default_GetSystemConfigItemNum_ROCKYPOM() <= ulItemIdx)
		return NULL;

	return (dbParam_DefaultItem_t*)&s_astHardCodingSystemItemList_ROCKYPOM[ulItemIdx];
}
