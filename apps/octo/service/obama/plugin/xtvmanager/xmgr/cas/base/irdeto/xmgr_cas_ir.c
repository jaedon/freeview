...
STATIC BUS_Result_t xmgr_cas_ir_AlarmMSGProcess(IR_CAS_UI_Context *pContext, HINT32 message, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	if (p2 == AXI_STATUSCODE_IDX_None)
	{
		//BUS_MGR_Destroy(NULL);
		return ERR_BUS_MESSAGE_BREAK;
	}

	if (p2 == AXI_STATUSCODE_IDX_I07_4)
	{
...
//	if ((ulActionId == pstContext->ulActionId) && (TRUE == pstContext->bActionTaken))
 	{
		...
		{
			xmgr_cas_ir_AlarmMSGProcess(pstContext, message, hAction, eDxCAS_GROUPID_IR, AXI_STATUSCODE_IDX_ServiceUnknownE37, 0);
		}
		else if(eViewState == eLiveViewUi_NO_SIGNAL)
		{
			xmgr_cas_ir_AlarmMSGProcess(pstContext, message, hAction, eDxCAS_GROUPID_IR, AXI_STATUSCODE_IDX_NoSignalE48, 0);
		}
		...
		{
			xmgr_cas_ir_AlarmMSGProcess(pstContext, message, hAction, eDxCAS_GROUPID_IR, AXI_STATUSCODE_IDX_ServiceIsNotCurrentlyRunningE38, 0);
		}
		else if(eViewState == eLiveViewUi_NO_SIGNAL_TIMEOUT)
		{
			xmgr_cas_ir_AlarmMSGProcess(pstContext, message, hAction, eDxCAS_GROUPID_IR, AXI_STATUSCODE_IDX_SearchingForSignalE52, 0);
		}
	 ...
	 */
 	{
		...
		{
			eResult = xmgr_cas_ir_AlarmMSGProcess(pstContext, message, hAction, eDxCAS_GROUPID_IR, AXI_STATUSCODE_IDX_ServiceUnknownE37, 0);
		}
...
STATIC BUS_Result_t	xmgr_cas_ir_GetControlByte(IR_CAS_UI_Context *pstContext, HINT32 message, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	stIrdetoParam.ucHardwareCode = stHwcodeParm.ucHwCode;
	stIrdetoParam.ucLoadSequenceNumber = stLoadSqeNum.ucLoadSeqNum;
	stIrdetoParam.ucManufacturerCode = stManCodeParam.ucManCode;
	...
	if(hErr == ERR_OK)
	{
		...
		//if(hErr == ERR_OK) //어차피 0x3622 만 걸어서.  control byte를 못 받아 0 이건, bat를 못 찾아서 0이건...local_irdeto_otalive_ProcessBat_CheckControlByte 참고.
		{
...
BUS_Result_t 	xproc_cas_IrMain(HINT32 message, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	switch(message)
	{
		...
		case eSEVT_SI_BAT :
			...
			eBusResult = xmgr_cas_ir_GetControlByte(pContext, message, hAction, p1, p2, p3);
			break;
