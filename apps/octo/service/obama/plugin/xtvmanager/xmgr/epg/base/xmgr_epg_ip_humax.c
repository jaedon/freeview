...
STATIC BUS_Result_t xproc_epg_ip_humax(HINT32 message, Handle_t action, HINT32 p1, HINT32 p2, HINT32 p3);

STATIC HINT32	xmgr_epg_ip_humax_Compare (const void *arg1, const void *arg2)
{
...
STATIC void		xmgr_epg_ip_humax_RestartEPG (HumaxEpgMgr_t *mgr)
{
	HUINT32		actionId;
	Handle_t	hAction;
	Handle_t	hService;
	...
	err  = SVC_EPG_Stop (eEPG_KIND_DVB, eEPG_EVT_KIND_PF_ALL , hAction);
	err |= SVC_EPG_Stop (eEPG_KIND_DVB, eEPG_EVT_KIND_SCH_ALL, hAction);
...
STATIC void		xmgr_epg_ip_humax_OnDapiReceived (const HCHAR *name, HUID uid, const void *data, HUINT32 size, void *userdata)
{
	DAxIterator_t	iter;
	HxVector_t		*list;
	DxIpService_t	*svc;
	...
	if (data && size)
	{
		list = HLIB_VECTOR_NewEasy(128, (HxFreeFunc_t)HLIB_STD_MemFree_CB, (HxCompareFunc_t)xmgr_epg_ip_humax_Compare);
		if (list)
		{
	...
	BUS_PostMessage(xproc_epg_ip_humax, eMEVT_EPG_USER_COMMAND, (Handle_t)eHMXEPGCMD_IPSERVICE_UPDATED, (HINT32)list, (HINT32)userdata, 0);
}

STATIC void		xmgr_epg_ip_humax_OnDapiUpdated (const HCHAR *name, HUID uid, const void *data, HUINT32 size, void *userdata)
{
	//2 ASYNC FUNCTION으로 s_humaxEpgMgr 접근 불가!!!!
	(void)uid;
	(void)data;
	...
	HxLOG_Info("%s(%s)\n", __FUNCTION__, name);
	DAPI_GetAll(name, xmgr_epg_ip_humax_OnDapiReceived, NULL);
}


STATIC void		xmgr_epg_ip_humax_OnDapiStatusChanged (const HCHAR *name, DAxSTATUS_e status, void *userdata, ...)
{
	va_list	ap;
	HUINT32	count;
	...
	BUS_PostMessage(xproc_epg_ip_humax, eMEVT_EPG_USER_COMMAND, (Handle_t)eHMXEPGCMD_RF_OVERFLOW, (HINT32)HLIB_STD_StrDup(name), (HINT32)count, 0);
}


STATIC void		xmgr_epg_ip_humax_OnIpServiceUpdated (HumaxEpgMgr_t *mgr, HxVector_t *list, HBOOL firstTime)
{
	HxVector_t		*changes;
	...
	if (list == NULL)
	{
		...
		SVC_EPG_SetServiceConstraint(eOxEPG_TYPE_DVB, NULL);
		HLIB_VECTOR_Delete(mgr->list);
		mgr->list = NULL;
		if (!firstTime)
			xmgr_epg_ip_humax_RestartEPG(mgr);
		return;
	}
	...
	if (HLIB_VECTOR_Length(changes) > 0)
	{
		...
		SVC_EPG_SetServiceConstraint(eOxEPG_TYPE_DVB, changes);
		if (!firstTime)
			xmgr_epg_ip_humax_RestartEPG(mgr);
	}
	else
...
STATIC void		xmgr_epg_ip_humax_OnRfOverflow (HumaxEpgMgr_t *mgr, const HCHAR *name, HINT32 count)
{
	HERROR				hErr;
	HUINT32				actionId;
	Handle_t			hService;
...
STATIC void		xmgr_epg_ip_humax_RemovePastData (void)
{
	HCHAR	*query;
	HUINT32	time;
	HUINT32	n;
...
STATIC void		xmgr_epg_ip_humax_OnTimeout (HumaxEpgMgr_t *mgr, HINT32 timerId)
{
	if (timerId == HMXEPG_RM_PAST_DATA_TIMER)
	{
		// 이 기능은 base로 들어가야 하지만, 일단 지금은 이렇게...
		xmgr_epg_ip_humax_RemovePastData();
	}
}


STATIC void		xmgr_epg_ip_humax_OnUserCommand (HumaxEpgMgr_t *mgr, HINT32 cmd, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HxLOG_Trace();
	...
	case eHMXEPGCMD_IPSERVICE_UPDATED:
		xmgr_epg_ip_humax_OnIpServiceUpdated(mgr, (HxVector_t *)p1, (HBOOL)p2);
		break;

	case eHMXEPGCMD_RF_OVERFLOW:
		xmgr_epg_ip_humax_OnRfOverflow(mgr, (HCHAR *)p1, p2);
		if (p1)
		{
...
STATIC BUS_Result_t xproc_epg_ip_humax(HINT32 message, Handle_t action, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		if (s_humaxEpgMgr.changes == NULL)
		{
			...
			DAPI_GetAll(DxNAMEOF(DxIpService_t), xmgr_epg_ip_humax_OnDapiReceived, (void *)TRUE);
			DAPI_AddTableNotifier(DxNAMEOF(DxIpService_t), xmgr_epg_ip_humax_OnDapiUpdated, NULL);
	...
	case eMEVT_BUS_TIMER:
		xmgr_epg_ip_humax_OnTimeout(&s_humaxEpgMgr, p1);
		break;

	case eMEVT_EPG_USER_COMMAND:
		xmgr_epg_ip_humax_OnUserCommand(&s_humaxEpgMgr, (HINT32)action, p1, p2, p3);
		break;
...
HERROR xmgr_epg_IpHumaxStart_Base (void)
{
	...
	eRes = BUS_MGR_Create("xproc_epg_ip_humax", APP_DEFAULT_PRIORITY, xproc_epg_ip_humax, 0, 0, 0, 0);
