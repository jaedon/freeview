...
STATIC void xmgr_cdig2000c_OperationFrontControl(void)
{
...
STATIC void xmgr_cdig2000c_ShutDownFrontControl(void)
{
	HBOOL				bPowerSavingStandby;
...
#define _____KEY PRE-PROCESSING_CALLBACK_FUNCTION____
STATIC void P_KEY_PreProcessingCallback(HUINT32 ucKeyCode, KEY_PressMode_e pressMode, KEY_Device_t device)
{
	(void)ucKeyCode;
	(void)pressMode;
	(void)device;
...
STATIC BUS_Result_t xmgr_cdig2000c_msgBusCreate (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t proc_cdig2000c (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		eResult = xmgr_cdig2000c_msgBusCreate (hHandle, p1, p2, p3);
		break;
	case eMEVT_BUS_READY_SHUTDOWN :
	case eMEVT_BUS_GO_SHUTDOWN:
	{
		xmgr_cdig2000c_ShutDownFrontControl();
		break;
	}
	case eMEVT_BUS_GO_OPERATION:
	{
		xmgr_cdig2000c_OperationFrontControl();
		break;
	}
	default:
		break;
...
{
	BUS_MGR_Create (NULL, APP_PORTING_MGR_PRIORITY, proc_cdig2000c, 0, 0, 0, 0);
	(void)SVC_SYS_RegisterKeyPreProcessingEvtCallback(P_KEY_PreProcessingCallback);
...
HERROR xmgr_model_LiveGetSiSpec_CDIG2000C (MgrLive_START_t *pstStart, SvcSi_LiveSpec_t *pstSiSpec)
{
	...
	pstSiSpec->eVideo		= eSVCSI_ES_SELECT_Nordig;
...
HERROR	xmgr_model_SwupInitProduct_CDIG2000C (void)
{
	...
	HLIB_STD_StrUtf8NCpy(stModuleInfo.szModuleName, "CDIG2000C_SWUP", SWUPDATE_MODULE_NAME_LEN);
...
HERROR xmgr_model_RecordGetCasTypes_CDIG2000C (HUINT32 *pulCasTypes)
{
	...
	*pulCasTypes	=	eMgrRec_CasType_Conax;
...
HERROR xmgr_model_PlaybackGetCasTypes_CDIG2000C (HUINT32 *pulCasTypes)
{
	...
	*pulCasTypes	=	eMgrPb_CasType_Conax;
