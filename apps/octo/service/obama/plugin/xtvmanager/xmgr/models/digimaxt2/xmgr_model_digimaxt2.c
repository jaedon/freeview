...
STATIC void xmgr_digimaxt2_OperationFrontControl(void)
{
...
STATIC void xmgr_digimaxt2_ShutDownFrontControl(void)
{
	HBOOL				bPowerSavingStandby;
...
STATIC BUS_Result_t xmgr_digimaxt2_msgBusCreate (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t proc_digimaxt2 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		eResult = xmgr_digimaxt2_msgBusCreate (hHandle, p1, p2, p3);
		break;
	case eMEVT_BUS_READY_SHUTDOWN :
	case eMEVT_BUS_GO_SHUTDOWN:
	{
		xmgr_digimaxt2_ShutDownFrontControl();
		break;
	}
	case eMEVT_BUS_GO_OPERATION:
	{
		xmgr_digimaxt2_OperationFrontControl();
		break;
	}
	default:
		break;
...
{
	BUS_MGR_Create (NULL, APP_PORTING_MGR_PRIORITY, proc_digimaxt2, 0, 0, 0, 0);
	return ERR_OK;
}
...
HERROR xmgr_model_LiveGetSiSpec_DIGIMAXT2 (MgrLive_START_t *pstStart, SvcSi_LiveSpec_t *pstSiSpec)
{
	...
	pstSiSpec->ulActionSpec	= eSVCSI_ACT_SPEC_ItaDtt;
...
HERROR xmgr_model_SearchGetLcnPolicy_DIGIMAXT2 (DefaultLcnPolicy_t *peLcnPolicy, PreProgrammedChNumPolicy_t *pePreProgChNumPolicy)
{
	...
	if (peLcnPolicy != NULL)
	{
		*peLcnPolicy = eDEFAULTLCN_ITADTT;
	}
...
HERROR	xmgr_model_SwupInitProduct_DIGIMAXT2 (void)
{
	...
	HLIB_STD_StrUtf8NCpy(stModuleInfo.szModuleName, "ITA_SWUP", SWUPDATE_MODULE_NAME_LEN); // for test (swkim_p) : TODO
