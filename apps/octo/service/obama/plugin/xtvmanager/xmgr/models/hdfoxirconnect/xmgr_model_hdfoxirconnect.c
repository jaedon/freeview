...
STATIC void xmgr_hdfoxirconnect_OperationFrontControl(void)
{
...
STATIC void xmgr_hdfoxirconnect_ShutDownFrontControl(void)
{
	HBOOL				bPowerSavingStandby;
...
STATIC BUS_Result_t xmgr_hdfoxirconnect_evtCASCmd_IRDETO (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t xmgr_hdfoxirconnect_evtCASCmd (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	switch (eCasId)
	{
		case eDxCAS_GROUPID_IR:
			...
			eResult = xmgr_hdfoxirconnect_evtCASCmd_IRDETO(hHandle, p1, p2, p3);
			break;
		default:
...
STATIC BUS_Result_t xmgr_hdfoxirconnect_msgBusCreate (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t proc_hdfoxirconnect (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		eResult = xmgr_hdfoxirconnect_msgBusCreate (hHandle, p1, p2, p3);
		break;
	case eMEVT_BUS_READY_SHUTDOWN :
	case eMEVT_BUS_GO_SHUTDOWN:
	{
		xmgr_hdfoxirconnect_ShutDownFrontControl();
		break;
	}
	case eMEVT_BUS_GO_OPERATION:
	{
		xmgr_hdfoxirconnect_OperationFrontControl();
		break;
	}
	case eSEVT_CAS_CMD:
		HxLOG_Debug("[XMGR:IR4000HD(%d)] MESSAGE : eSEVT_CAS_CMD\n");
		eResult = xmgr_hdfoxirconnect_evtCASCmd(hHandle, p1, p2, p3);
		break;
...
{
	BUS_MGR_Create (NULL, APP_PORTING_MGR_PRIORITY, proc_hdfoxirconnect, 0, 0, 0, 0);
	return ERR_OK;
}
...
HERROR xmgr_model_LiveGetSiSpec_HDFOXIRCONNECT (MgrLive_START_t *pstStart, SvcSi_LiveSpec_t *pstSiSpec)
{
	...
	pstSiSpec->ulActionSpec	= eSVCSI_ACT_SPEC_Orf;
...
HERROR xmgr_model_SwupGetSiSpec_HDFOXIRCONNECT (MgrSwup_InArg_t *pstInArg, SvcSi_SwupCheckSpec_t *pstSiSpec)
{
...
HERROR	xmgr_model_SwupInitProduct_HDFOXIRCONNECT (void)
{
	...
	{
		HLIB_STD_StrUtf8NCpy(stModuleInfo.szModuleName, "ASTRA_SWUP", SWUPDATE_MODULE_NAME_LEN);
