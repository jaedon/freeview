...
STATIC void xmgr_hms1000t_OperationFrontControl(void)
{
...
STATIC void xmgr_hms1000t_ShutDownFrontControl(void)
{
	HBOOL				bPowerSavingStandby;
...
STATIC BUS_Result_t xmgr_hms1000t_msgBusCreate (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t proc_hms1000t (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		eResult = xmgr_hms1000t_msgBusCreate (hHandle, p1, p2, p3);
		break;
	case eMEVT_BUS_READY_SHUTDOWN :
	case eMEVT_BUS_GO_SHUTDOWN:
	{
		xmgr_hms1000t_ShutDownFrontControl();
		break;
	}
	case eMEVT_BUS_GO_OPERATION:
	{
		xmgr_hms1000t_OperationFrontControl();
		break;
	}
	default:
		break;
...
{
	BUS_MGR_Create (NULL, APP_PORTING_MGR_PRIORITY, proc_hms1000t, 0, 0, 0, 0);
	return ERR_OK;
}
...
HERROR xmgr_model_LiveGetSiSpec_HMS1000T (MgrLive_START_t *pstStart, SvcSi_LiveSpec_t *pstSiSpec)
{
	...
	pstSiSpec->ulActionSpec	= eSVCSI_ACT_SPEC_AusDtt;
...
HERROR xmgr_model_SearchGetLcnPolicy_HMS1000T (DefaultLcnPolicy_t *peLcnPolicy, PreProgrammedChNumPolicy_t *pePreProgChNumPolicy)
{
	...
	if (peLcnPolicy != NULL)
	{
		*peLcnPolicy = eDEFAULTLCN_AUSDTT;
	}
...
HERROR	xmgr_model_SwupInitProduct_HMS1000T (void)
{
	...
	pstCallback->pfnCreate			= XMGR_SWUP_Create_Dtt;
	pstCallback->pfnEnterAction		= XMGR_SWUP_EnterAction_Dtt;
	pstCallback->pfnLeaveAction		= XMGR_SWUP_LeaveAction_Dtt;
	pstCallback->pfnGetNextState		= XMGR_SWUP_GetNextState_Dtt;
	pstCallback->pfnDestroy			= XMGR_SWUP_Destroy_Dtt;
	pstCallback->pfnSetManualInfo	= XMGR_SWUP_SetManualInfo_Dtt;
	pstCallback->pfnGetHomeTpInfo	= XMGR_SWUP_GetHomeTpInfo_Dtt;
