...
STATIC void xmgr_icordpro_OperationFrontControl(void)
{
...
STATIC void xmgr_icordpro_ShutDownFrontControl(void)
{
	HBOOL				bPowerSavingStandby;
...
STATIC BUS_Result_t xmgr_icordpro_msgBusCreate (Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
...
STATIC BUS_Result_t proc_icordpro (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		eResult = xmgr_icordpro_msgBusCreate (hHandle, p1, p2, p3);
		break;
	case eMEVT_BUS_READY_SHUTDOWN :
	case eMEVT_BUS_GO_SHUTDOWN:
	{
		xmgr_icordpro_ShutDownFrontControl();
		break;
	}
	case eMEVT_BUS_GO_OPERATION:
	{
		xmgr_icordpro_OperationFrontControl();
		break;
	}
...
{
	BUS_MGR_Create (NULL, APP_PORTING_MGR_PRIORITY, proc_icordpro, 0, 0, 0, 0);
	return ERR_OK;
}
...
HERROR	xmgr_model_SwupInitProduct_ICORDPRO (void)
{
	...
	HLIB_STD_StrUtf8NCpy(stModuleInfo.szModuleName, "ICORDPRO_SWUP", SWUPDATE_MODULE_NAME_LEN);
