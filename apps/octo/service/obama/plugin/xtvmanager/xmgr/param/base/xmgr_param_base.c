...
/* OTA version */
#define	OTA_NOTIFICATION_VERSION_FOR_CHECK			1

typedef struct
{
	HUINT32 							ulIndex;
	HCHAR								szString[UPDATED_LIST_MAX_STRING_LENGTH_IN_A_LINE];
} xmgrParam_UpdatedListDataItem_t;
...
typedef struct
{
	...
	HUINT32 							ulLineNum;
	xmgrParam_UpdatedListDataItem_t		stListItem[UPDATED_LIST_MAX_LINE];
} xmgrParam_UpdatedList_t;
...
static xmgrParam_UpdatedList_t		stUpdatedList_base_v1[TOTAL_PAGE_NUM] =
{
	{
	...
	{
		2, eXmgrParam_UpdatedListimgType_New, 3,
		{
...
HERROR xmgr_param_Init_Base (void)
{
	return ERR_OK;
}

HERROR xmgr_param_SetFactoryDefault_Base(DxFactoryDefault_type_e eDefaultType)
{
	HERROR			hErr, hRes;
	HxLANG_Id_e 	eLangId;
	...
	case eDxFAC_DEF_TYPE_KEEP:
		{
			HBOOL	bKeepFactoryValue = (eDefaultType == eDxFAC_DEF_TYPE_KEEP) ? TRUE : FALSE;

			// 1. Back-Up Items => Clear Param Table => Restore
			hErr = DB_PARAM_LoadDefault(bKeepFactoryValue);
			if (ERR_OK == hErr)
			{
				HxLOG_Critical("DB_PARAM_LoadDefault(FALSE) Fail [%d]\n", hErr);
			}

			// 2. Get Default Language From DB Table : Product별 default language 가지고 오기
			hErr = DB_PARAM_GetDefaultItem(eDB_PARAM_ITEM_LANG_MAIN, (HUINT32 *)&eLangId);
			if (ERR_OK == hErr)
			{
				(void)MGR_PARAM_SetSysLanguage(eLangId);
			}
...
STATIC HCHAR *xmgr_param_EncodeUpdatedListData(HUINT32 ulOtaVersion, xmgrParam_UpdatedList_t *pstUpdatedListEvt, HUINT32 ulTotalPage)
{
	...
	xmgrParam_UpdatedListDataItem_t		*pstListItem = NULL;
...
HERROR xmgr_param_GetUpdatedList_Base(HUINT8 *pzUpdatedList)
{
//	xmgrParam_UpdatedList_t		stUpdatedListEvt;
...
#else
	pzProperty = xmgr_param_EncodeUpdatedListData(OTA_NOTIFICATION_VERSION_FOR_CHECK, stUpdatedList_base_v1, TOTAL_PAGE_NUM);
#endif
