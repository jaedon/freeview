...
STATIC xMgrSatIpContext_t* xmgr_satip_tara_GetContext (HUINT32 ulIndex)
{
...
STATIC MgrAction_Priority_e	xmgr_satip_tara_GetActPriority (HUINT16 usPriority)
{
...
STATIC void xmgr_satip_tara_ClearContext (xMgrSatIpContext_t *pstContext)
{
	if (NULL == pstContext)
		return;
...
STATIC HBOOL xmgr_satip_tara_CheckValidPidList (HINT32 nPidsCount, HUINT16 *pusPids)
{
...
STATIC HERROR xmgr_satip_tara_stopTranscoder (xMgrSatIpContext_t *pstContext)
{
	HERROR			 hErr = ERR_FAIL;
...
STATIC HERROR xmgr_satip_tara_SetupAction (HUINT32 ulActionId, MgrSatIp_Setup_t *pstSetup)
{
	MgrAction_Priority_e		 eActPriority = eAmPriority_RECORD;
	HERROR						 hErr;
	HBOOL						 bForce = FALSE;

	eActPriority = xmgr_satip_tara_GetActPriority(pstSetup->usPriority);
...
STATIC HERROR xmgr_satip_tara_RestartAction (HUINT32 ulActionId)
{
	xMgrSatIpContext_t	*pstContext = xmgr_satip_tara_GetContext (ulActionId - eActionId_BG_FIRST);
	...
	{
		if((TRUE == pstContext->stSetup.bDrm) || (TRUE == pstContext->stSetup.stStartArg.bTranscode))
		{
			hErr = SVC_SI_StopSatip (pstContext->hAction, pstContext->stSetup.stStartArg.hSvcHandle, TRUE);
			if (ERR_OK != hErr)
			{
				HxLOG_Error ("SVC_SI_StopSatip. Error!!!\n");			}

			if(TRUE == pstContext->stSetup.bDrm)
			{
		...
		{
			hErr = xmgr_satip_tara_stopTranscoder(pstContext);
			if (ERR_OK != hErr)
			{
				HxLOG_Error("xmgr_satip_tara_stopTranscoder. Error!!!\n");
			}
		}
		else
		{
...
HERROR xmgr_satip_tara_setupTranscoder(xMgrSatIpContext_t *pstContext)
{
	HERROR				hErr = ERR_FAIL;
#if defined(CONFIG_MW_MM_TRANSCODING)
...
STATIC BUS_Result_t xmgr_satip_tara_msgBusCreate (xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HERROR				hErr		= 	ERR_OK;
...
STATIC BUS_Result_t xmgr_satip_tara_msgBusDestroy (xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HERROR hErr = ERR_FAIL;
	...
	/* clean context */
	xmgr_satip_tara_ClearContext (pstContext);
...
STATIC HERROR xmgr_satip_tara_setPsiTable (xMgrSatIpContext_t *pstContext)
{
	HERROR						hErr = ERR_FAIL;
#if defined(CONFIG_MW_MM_TRANSCODING)
...
STATIC HERROR xmgr_satip_tara_startTranscoder (xMgrSatIpContext_t *pstContext)
{
	HERROR						hErr = ERR_FAIL;
#if defined(CONFIG_MW_MM_TRANSCODING)
	...
	hErr = xmgr_satip_tara_setPsiTable (pstContext);
	if (ERR_OK != hErr)
	{
		HxLOG_Error ("xmgr_satip_tara_setPsiTable err: hAction(0x%08x)\n", pstContext->hAction);
		return hErr;
	}
...
STATIC HERROR xmgr_satip_tara_startRecorder (xMgrSatIpContext_t *pstContext)
{
...
STATIC HERROR xmgr_satip_tara_startServices (xMgrSatIpContext_t *pstContext)
{
	HERROR hErr = ERR_FAIL;
	HxLOG_Trace();

	pstContext->stSetup.bPMTValid = FALSE;

	if (TRUE == pstContext->stSetup.stStartArg.bTranscode)
	{
		/* SAT>IP Transcoder */
		hErr = xmgr_satip_tara_setupTranscoder(pstContext);
		if (ERR_OK != hErr)
		{
			BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
			HxLOG_Critical("Critical : xmgr_satip_tara_setupTranscoder() hAction=0x%x\n", pstContext->hAction);
			return MESSAGE_BREAK;
		}

		hErr = xmgr_satip_tara_startTranscoder (pstContext);
		if(hErr != ERR_OK)
		{
			HxLOG_Error ("xmgr_satip_tara_startTranscoder err: hAction(0x%08x)\n", pstContext->hAction);
			BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
			return hErr;
		}
	}
	...
	{
		/* SAT>IP Recorder */
		hErr = xmgr_satip_tara_startRecorder (pstContext);
		if(hErr != ERR_OK)
		{
			HxLOG_Error ("xmgr_satip_tara_startRecorder err: hAction(0x%08x)\n", pstContext->hAction);
			BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
			return hErr;
		}
	}
...
STATIC HERROR xmgr_satip_tara_startSI(xMgrSatIpContext_t *pstContext)
{
	HERROR hErr = ERR_FAIL;
	SvcSi_SatipSpec_t stSiSpec;

	HxLOG_Debug ("------------<XMGR_SATIP:START SI Parsing>------------\n");
	HxLOG_Debug ("\t[hAction\t\t:\t0x%x]\n", pstContext->hAction);
	HxLOG_Debug ("------------<START SI Parsing:XMGR_SATIP>------------\n");

	hErr = MGR_SATIP_GetSatipSiSpec (&stSiSpec);
	if (ERR_OK != hErr)
	{
		HxLOG_Error ("MGR_SATIP_GetSatipSiSpec err: hAction(0x%08x), hSvc(0x%08x)\n", pstContext->hAction, pstContext->stSetup.stStartArg.hSvcHandle);
		return ERR_FAIL;
	}

	hErr = SVC_SI_StartSatip (pstContext->hAction, &stSiSpec, pstContext->stSetup.stStartArg.hSvcHandle, FALSE);
	if (ERR_OK != hErr)
	{
...
STATIC HERROR xmgr_satip_tara_startCas(xMgrSatIpContext_t *pstContext)
{
	HERROR	hErr = ERR_OK;

	HxLOG_Debug ("------------<XMGR_SATIP:START CAS Parsing>------------\n");
	HxLOG_Debug ("\t[hAction\t\t:\t0x%x]\n", pstContext->hAction);
	HxLOG_Debug ("------------<START CAS Parsing:XMGR_SATIP>------------\n");
	...
	hErr = MGR_CAS_StartService( pstContext->hAction, pstContext->stSetup.stStartArg.hSvcHandle, eCAS_ActionType_Recording, eDxCAS_GROUPID_NONE, NULL);
	if( ERR_OK != hErr )
	{
		HxLOG_Error("Critical Error!!!\n");
		return hErr;
...
BUS_Result_t xmgr_satip_MsgEvtSiPmtReceived_Base( xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HERROR 						 hErr;
	...
	if (!pstSvcInfo || !pstOrgSvcInfo || !pstDstSvcInfo)
	{
		HxLOG_Error("memory allocation failed:\n");
		goto END_FUNC;
	}

	hErr = DB_SVC_GetServiceInfo(pstContext->stSetup.stStartArg.hSvcHandle, pstOrgSvcInfo);
	if(hErr != ERR_OK)
	{
	...
	hErr = DB_SVC_GetServiceIdFromSvcHandle (pstContext->stSetup.stStartArg.hSvcHandle, &usSvcId);
	if (hErr != ERR_OK)
	{
	...
	hErr = MGR_LIVE_UpdateServiceInfo(pstContext->hAction, (HUINT32)usSvcId, pstOrgSvcInfo, pstDstSvcInfo, &bChangedPid);
	if ( hErr != ERR_OK)
	{
	...
	HxLOG_Debug("pstOrgSvcInfo != pstDstSvcInfo bChangedPid: %d \n", bChangedPid);
	HxLOG_Debug("Org video PID: 0x%x, audio PID: 0x%x, usTtxPid: 0x%x,  ucSubttlIdx: 0x%x,  usPcrPid: 0x%x\n",
	...
	HxLOG_Debug("Dst video PID: 0x%x, audio PID: 0x%x, usTtxPid: 0x%x,  ucSubttlIdx: 0x%x,  usPcrPid: 0x%x\n",
	...
	if (PID_NULL == DbSvc_GetVideoPid(pstDstSvcInfo))
	{
	...
	if (TRUE == bChangedPid)
	{
		HxLOG_Debug("Update service DB info.\n");
		hErr = DB_SVC_UpdateServiceInfo (pstContext->stSetup.stStartArg.hSvcHandle, pstDstSvcInfo);
		if(hErr != ERR_OK)
		{
			HxLOG_Error("DB_SVC_UpdateServiceInfo() Fail!!!\n");
			goto END_FUNC;
		}
	...
	if (FALSE == pstContext->stSetup.bPMTValid)
	{
		pstContext->stSetup.bPMTValid = TRUE;
	}

	hErr = xmgr_satip_tara_startServices(pstContext);
	if (ERR_OK != hErr)
	{
		HxLOG_Error ("xmgr_satip_tara_startServices err: hAction(0x%08x)\n", pstContext->hAction);
		goto END_FUNC;
	}
...
BUS_Result_t xmgr_satip_MsgEvtSiPmtTimeout_Base( xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HxLOG_Trace();

	if (eSatIpState_Stopped == pstContext->eState)
	{
		HxLOG_Debug("[%s] eSatIpState_Stopped! --> Ignore PMT TImeout\n", __FUNCTION__);
		return MESSAGE_PASS;
	}

	BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
	HxLOG_Error("xmgr_satip_MsgEvtSiPmtTimeout_Base. Error!!!\n");
...
BUS_Result_t xmgr_satip_MsgEvtSiPmtPidChanged_Base( xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HERROR			hErr = ERR_OK;
	...
	hErr = DB_SVC_GetServiceInfo(pstContext->stSetup.stStartArg.hSvcHandle, &stSvcInfo);
	if (ERR_OK != hErr)
	{
	...
	hErr = DB_SVC_UpdateServiceInfo(pstContext->stSetup.stStartArg.hSvcHandle, &stSvcInfo);
	if(hErr != ERR_OK)
	{
		HxLOG_Error("DB_SVC_UpdateServiceInfo() Fail!!!\n");
		return MESSAGE_PASS;
	...
	hErr = xmgr_satip_tara_startSI(pstContext);
	if( ERR_OK != hErr )
	{
		HxLOG_Error("Critical Error!!!\n");
		return MESSAGE_PASS;
...
STATIC BUS_Result_t xmgr_satip_tara_evtChLocked (xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HERROR	hErr = ERR_FAIL;
	...
	if((TRUE == pstContext->stSetup.bDrm) || (TRUE == pstContext->stSetup.stStartArg.bTranscode))
	{
		...
		if (ERR_OK != hErr)
		{
			HxLOG_Error ("xmgr_satip_tara_startSI err: hAction(0x%08x)\n", pstContext->hAction);
			BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
			return hErr;
		}
		...
		if(TRUE == pstContext->stSetup.bDrm)
		{
		hErr = xmgr_satip_tara_startCas(pstContext);
		if (ERR_OK != hErr)
		{
			HxLOG_Error ("xmgr_satip_tara_startCas err: hAction(0x%08x)\n", pstContext->hAction);
			BUS_SendMessage(NULL, eMEVT_SATIP_NOTIFY_START_FAILED, pstContext->hAction, eMgrSatIpFailed_CREATE, pstContext->stSetup.ulRsvSlotId, 0);
			return hErr;
		}
	}
...
STATIC BUS_Result_t xmgr_satip_tara_evtChNoSignal (xMgrSatIpContext_t *pstContext, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	HxLOG_Message("[SATIP_TIME]\n");
...
STATIC BUS_Result_t xmgr_satip_tara_protoProc (xMgrSatIpContext_t *pstContext, HINT32 message, Handle_t hAction, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	case eMEVT_BUS_CREATE:
		...
		HxLOG_Message(HxANSI_COLOR_YELLOW("*  XMGR : SATIP (TARA) : eMEVT_BUS_CREATE\n"));
		HxLOG_Message(HxANSI_COLOR_YELLOW("***************************************************************\n"));
		eRet = xmgr_satip_tara_msgBusCreate (pstContext, hAction, p1, p2, p3);
		break;

	case eMEVT_BUS_DESTROY:
		HxLOG_Message(HxANSI_COLOR_YELLOW("***************************************************************\n"));
		HxLOG_Message(HxANSI_COLOR_YELLOW("*  XMGR : SATIP (TARA) : eMEVT_BUS_DESTROY\n"));
		HxLOG_Message(HxANSI_COLOR_YELLOW("***************************************************************\n"));
		eRet = xmgr_satip_tara_msgBusDestroy (pstContext, hAction, p1, p2, p3);
		break;

	/*EVT-SI*/
	case eSEVT_SI_PMT:
		CHECK_SATIP_ACTION_HANDLE();
		HxLOG_Message(HxANSI_COLOR_YELLOW("MESSAGE : eSEVT_SI_PMT\n"));
		eRet = xmgr_satip_MsgEvtSiPmtReceived_Base(pstContext, hAction, p1, p2, p3);
		break;
	...
	case eSEVT_SI_PMT_PIDREMOVED:
		CHECK_SATIP_ACTION_HANDLE();
		HxLOG_Message(HxANSI_COLOR_YELLOW("MESSAGE : eSEVT_SI_PMT_TIMEOUT\n"));
		eRet = xmgr_satip_MsgEvtSiPmtTimeout_Base(pstContext, hAction, p1, p2, p3);
		break;

	case eSEVT_SI_PMT_PIDCHANGED :
		CHECK_SATIP_ACTION_HANDLE();
		HxLOG_Message(HxANSI_COLOR_YELLOW("MESSAGE : eSEVT_SI_PMT_PIDCHANGED\n"));
		eRet = xmgr_satip_MsgEvtSiPmtPidChanged_Base(pstContext, hAction, p1, p2, p3);
		break;
	...
	case eSEVT_CH_LOCKED :
		...
		eRet = xmgr_satip_tara_evtChLocked (pstContext, hAction, p1, p2, p3);
		break;

	case eSEVT_CH_NO_SIGNAL :
		CHECK_SATIP_ACTION_HANDLE();
		HxLOG_Message(HxANSI_COLOR_YELLOW("MESSAGE : eSEVT_CH_NO_SIGNAL\n"));
		eRet = xmgr_satip_tara_evtChNoSignal (pstContext, hAction, p1, p2, p3);
		break;
...
STATIC BUS_Result_t xmgr_satip_tara_procBg0 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (0), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg1 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (1), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg2 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (2), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg3 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (3), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg4 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (4), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg5 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (5), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg6 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (6), message, hHandle, p1, p2, p3);
}

STATIC BUS_Result_t xmgr_satip_tara_procBg7 (HINT32 message, Handle_t hHandle, HINT32 p1, HINT32 p2, HINT32 p3)
{
	if (eMEVT_BUS_CREATE == message)
	{
		BUS_MGR_SetMgrName (BUS_MGR_GetThis(), (HINT8 *)__FUNCTION__);
	...
	return xmgr_satip_tara_protoProc (xmgr_satip_tara_GetContext (7), message, hHandle, p1, p2, p3);
}

STATIC BUS_Callback_t xmgr_satip_tara_GetActionProc (HUINT32 ulActionId)
{
	if (xmgr_satip_tara_GetContext (ulActionId-eActionId_BG_FIRST) == NULL)
	{
	...
	case (eActionId_BG_FIRST + 0):
		return xmgr_satip_tara_procBg0;

	case (eActionId_BG_FIRST + 1):
		return xmgr_satip_tara_procBg1;

	case (eActionId_BG_FIRST + 2):
		return xmgr_satip_tara_procBg2;

	case (eActionId_BG_FIRST + 3):
		return xmgr_satip_tara_procBg3;

	case (eActionId_BG_FIRST + 4):
		return xmgr_satip_tara_procBg4;

	case (eActionId_BG_FIRST + 5):
		return xmgr_satip_tara_procBg5;

	case (eActionId_BG_FIRST + 6):
		return xmgr_satip_tara_procBg6;

	case (eActionId_BG_FIRST + 7):
		return xmgr_satip_tara_procBg7;
...
HERROR xmgr_satip_Init_TARA (void)
{
	/* Clear Context */
	{
		...
		{
			xmgr_satip_tara_ClearContext (&s_astSatIpCtx[i]);
		}
	}
	return ERR_OK;
}
...
HERROR xmgr_satip_Start_TARA (MgrSatIp_Setup_t *pstSetup, HUINT32 *pulActionId)
{
	...
	hErr = xmgr_satip_tara_SetupAction (ulActionId, pstSetup);
	if (ERR_OK != hErr)
	{
	...
	/* Create MSG Proc */
	{
		...
		hErr = BUS_MGR_Create (	NULL,
								APP_ACTION_PRIORITY,
								xmgr_satip_tara_GetActionProc (ulActionId),
								hAction,
								(HINT32)pstSetup,
								0,
								0);
...
HERROR xmgr_satip_ChangePids_TARA (HUINT32 ulActionId, HINT32 nPidsCount, HUINT16 *pusPids)
{
	...
	pstContext = xmgr_satip_tara_GetContext (ulActionId - eActionId_BG_FIRST);
	if (NULL == pstContext)
	{
	...
	{
		hErr = xmgr_satip_tara_RestartAction(ulActionId);
		if (ERR_OK == hErr)
		{
		...
		{
			HxLOG_Error("xmgr_satip_tara_RestartAction ActionId(%d) err:\n", ulActionId);
			return ERR_FAIL;
		}
	...
	if (pstContext->eState <= eSatIpState_Running)
	{
		if( xmgr_satip_tara_CheckValidPidList (nPidsCount, pusPids) == FALSE )
			return ERR_FAIL;
...
HERROR xmgr_satip_ChangePriority_TARA (HUINT32 ulActionId, HUINT16 usPriority)
{
	...
	eActPriority = xmgr_satip_tara_GetActPriority(usPriority);
...
HBOOL xmgr_satip_GetDrmFlag_TARA (HUINT32 ulActionId)
{
	...
	if(pstContext)
	{
		return pstContext->stSetup.bDrm;
	}
