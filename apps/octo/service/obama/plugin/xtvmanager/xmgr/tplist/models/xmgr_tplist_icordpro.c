...
HERROR	xmgr_tplist_GetSatHomeTuningInfoList_IcordPro (DbSvc_SatTuningInfo_t **pastList, HINT32 *pnNumList)
{
#if	defined(CONFIG_MW_CH_SATELLITE)
	static DbSvc_SatTuningInfo_t	 s_stHomeTuneInfo_Icordpro[] =
	{
		{	12603,	22000,	eDxSAT_POLAR_HOR,	eDxSAT_CODERATE_5_6,	eDxSAT_TRANS_DVBS, eDxSAT_PSK_QPSK, 0,	0,	0},
		{	12552,	22000,	eDxSAT_POLAR_VER,	eDxSAT_CODERATE_5_6,	eDxSAT_TRANS_DVBS, eDxSAT_PSK_QPSK, 0,	0,	0},
		{	10832,	22000,	eDxSAT_POLAR_HOR,	eDxSAT_CODERATE_5_6,	eDxSAT_TRANS_DVBS, eDxSAT_PSK_QPSK, 0,	0,	0},
	};

	*pastList = s_stHomeTuneInfo_Icordpro;
	*pnNumList = (sizeof(s_stHomeTuneInfo_Icordpro) / sizeof(DbSvc_SatTuningInfo_t));
...
STATIC HERROR xmgr_tplist_GetHomeTpListOta_IcordPro (DbSvc_TsInfo_t **ppstTsList, HUINT32 *pulNumTs)
{
	HUINT32					 ulCount, ulNumTs;
	DbSvc_TsInfo_t			*astTsInfo;
	HBOOL					 bAllocList;
	HINT32					 nNumList;
#if defined(CONFIG_MW_CH_TERRESTRIAL) || defined(CONFIG_MW_CH_SATELLITE)
	HERROR					 hErr;
#endif
#if	defined(CONFIG_MW_CH_SATELLITE)
	HUINT32 				 ulAntIdx;
	...
	ulNumTs = 0;
#if	defined(CONFIG_MW_CH_SATELLITE)
	hErr = xmgr_tplist_GetSatHomeTuningInfoList_IcordPro(&astSatList, &nNumList);
...
HERROR xmgr_tplist_GetHomeTpList_IcordPro (MgrTplist_HomeTsKind_e eHomeTsKind, DbSvc_TsInfo_t **ppstTsList, HUINT32 *pulNumTs)
{
	...
	case eHomeTs_OTA:		hErr = xmgr_tplist_GetHomeTpListOta_IcordPro(ppstTsList, pulNumTs);		break;
	default:				break;
	}
