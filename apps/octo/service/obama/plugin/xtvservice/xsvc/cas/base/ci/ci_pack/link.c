...
lslot ln_tab_slot[CARM_MAX_CISLOT_NUMBER];/* slot table									*/
...
/*									   										*/
/* ======================================================================== */
void ci_pack_link(void *vpParam)
{
unsigned char trans;	/* sender/receiver of received event */
	...
	if (! LINK_Init()) {
		HxLOG_Error("ci_pack_link : LINK_Init error !!!\n");
//		return -1;		/* error while Initialization */
	...
	while(1)		/* infinite loop */
	{
...
#endif
		{
			HxLOG_Error("ci_pack_link : ITC_WaitEvt . Error in return code !!!\n");
		...
		{
			HxLOG_Error("ci_pack_link : ITC_WaitEvt . Error in received event !!!\n");
	...
	if (ln_evt == MESSAGE)
	{
		switch (ln_header.type) {
			...
			default :			/* unknown message */
				HxLOG_Error("LINK_State : Unknown message is ignored 0x%x !!!\n", ln_header.type);
				break;
			}

//		HxLOG_Print(("LINK STATE TRANSITION MESSAGE TYPE <0x%x> : Ret = %d\n", ln_header.type,iRet));
	...
	{
		HxLOG_Error("LINK_State : Unknown event is ignored 0x%x !!!\n", ln_evt);
	}
...
unsigned short slot_id;	/* current slot_id									*/
	...
	HxLOG_Print("LINK_Connect : Start <Physical Status : %d>\n", ln_msg.lconn.status);
	...
	if (ln_msg.lconn.status == LN_OK)
	{
		...
		if (index < 0)
		{
			...
			HxLOG_Print(("LINK_Connect : can't get new instance or module already connected\n"));
	...
	if (index >= 0)
	{
		...
		if (tcid_index < 0)
		{
			...
			{	/* no more available transport connection */
				/* Send IHM_WARNING to IHM Task */
				HxLOG_Print(("LINK_Tpdu : no more available transport connection\n"));
				return 0;
			}
		}
		/* Call the state diagram */
		LINK_Action(index, TPDU, tcid_index, &mem);
		HxLOG_Print("LINK_Tpdu : LINK_Action <TPDU:%d>\n", mem);
	}
	else
		mem = 0;
...
int index;				/* return code : entry found */
	...
	for (index=0; index < svc_cas_DevCiGetNumberOfModule(); index++)
	{
		if (ln_tab_slot[index].state == NOT_USED)   /* take the first free entry, reserve it */
		{
        	...
        	{
				HxLOG_Error("LINK_GetIndex : wrong physical buffer size %d !!!\n", ln_msg.lconn.size);
				return -3;
        	}
        	else
        	{
				HxLOG_Print("LINK_GetIndex : new entry %d with slot %d\n", index, ln_msg.lconn.slot_id);
	...
	HxLOG_Print(("LINK_GetIndex : no more entry\n"));
