...
int iRet;			/* return code 				*/
	...
	HxLOG_Print("LINK_Action : index=%d state=%d evt=0x%x info=%d\n", index, ln_tab_slot[index].state, evt, info);
	...
	switch(ln_tab_slot[index].state) {
		...
		default :
			HxLOG_Error("LINK_Action : Unknown state %d !!!\n", ln_tab_slot[index].state);
			iRet = 0;	/* Error */
			break;
		}
...
int iRet;		/* return code  */
	...
	*mem = 1;	/* default : keep memory */
	switch(evt) {
		case LN_CONNECT :
			HxLOG_Print(("LINK_Idle : LN_CONNECT Received\n"));
    		...
    		if (! LINK_Send_Msg())
    		{
				HxLOG_Error("LINK_Idle : Can't send the TP_CONNECT for slot %d to TRANS Task !!!\n", ln_tab_slot[index].slot);
				return 0;		/* Error */
			}
			else
			{
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("LINK_Idle : protocol error . event %d received in %d state !!!\n", evt, ln_tab_slot[index].state);
			iRet = 0;
			break;
		}
...
unsigned char flag;		/* flag More/Last of the received LPDU 	*/
	...
	*mem = 0;	/* default : free memory */
	...
	switch(evt) {
		case LPDU :
			HxLOG_Print(("LINK_Connected : LPDU received\n"));
			...
			if (flag ==	LN_MORE)
			{
				...
				{
					HxLOG_Error("LINK_Connected : bad size (%d) for intermediate received LPDU on slot %d . The negociated size is %d\n",
				...
				if (ln_tab_tcid[index_tcid].lrecv == 0)
				{
					...
					{
						HxLOG_Error("LINK_Connected : can't allocate buffer for coding TPDU !!!\n");
						return 0;	/* Error */
					}
					ln_tab_tcid[index_tcid].lrecv = len;
					HxSTD_memcpy(ln_tab_tcid[index_tcid].precv, plpdu, len);
				}
				else
				{
					...
					if (pt == NULL)
					{
						HxLOG_Error("LINK_Connected : can't reallocate buffer for coding TPDU !!!\n");
						return 0;	/* Error */
					}
#else
					...
					HxSTD_memcpy(pt, ln_tab_tcid[index_tcid].precv, ln_tab_tcid[index_tcid].lrecv);
					ITC_DynFree(ln_tab_tcid[index_tcid].precv);
#endif
					ln_tab_tcid[index_tcid].precv = pt;
					HxSTD_memcpy(ln_tab_tcid[index_tcid].precv + ln_tab_tcid[index_tcid].lrecv, plpdu, len);
					ln_tab_tcid[index_tcid].lrecv += len;
				}
#if 0 // #ifdef _CI_DEBUG_
				{
					...
					{
						HxLOG_Print(" %02X", ln_tab_tcid[index_tcid].precv[i]);
					}
					HxLOG_Print("\n\n");
				}
#endif
			...
			{	/* end of LPDU  : send the complete TPDU */
				...
				{	/* one LPDU to code in TPDU and to send to the TRANS Task			*/
					...
					HxSTD_memcpy(ln_msg1.tpdu.ptpdu, plpdu, len);
				}
				else
				{
					/* Add the LPDU to the already received LPDU : reallocate the memory */
    			...
    			if (! LINK_Send_Msg1())
    			{
					HxLOG_Error("LINK_Connected : Can't send the TPDU for slot %d to TRANS Task !!!\n", ln_tab_slot[index].slot);
#ifdef FASTALLOC
		...
		case TPDU :
			HxLOG_Print(("LINK_Connected : TPDU received\n"));
		...
		case LN_ERROR :
			HxLOG_Print(("LINK_Connected : LN_ERROR Received\n"));
			...
			if ((unsigned char)info == LN_PHYSICAL)
			{
    			...
    			if (! LINK_Send_Msg())
    			{
					HxLOG_Error("LINK_Connected : Can't send the TP_DISCONNECT for slot %d to TRANS Task !!!\n", ln_tab_slot[index].slot);
					return 0;		/* Error */
				}
				else
				{
		...
		case LN_DISCONNECT :
			HxLOG_Print(("LINK_Connected : LN_DISCONNECT Received\n"));
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("LINK_Connected : protocol error . event %d received in %d state !!!\n", evt, ln_tab_slot[index].state);
			iRet = 0;
			break;
		}
...
int nb;					/* number of multiplexing try */
	...
	*mem = 0;	/* default : free memory		*/
	...
	switch(evt) {
		case LN_ACK:
			HxLOG_Print(("LINK_Send : LN_ACK received\n"));
...
//KJW - 2003.08.11, Should free tpdu memory after all relevant LPDUs are sent -> Solves descrambling prob. for AB sat. with Viaccess RED CAM
		...
		case TPDU :
			HxLOG_Print(("LINK_Send : TPDU Received\n"));
		...
		case LPDU :
			HxLOG_Print(("LINK_Send : LPDU Received\n"));
			iRet = LINK_Connected(index, evt, info, mem);
			break;

		case LN_ERROR :
			HxLOG_Print(("LINK_Send : LN_ERROR Received\n"));
			iRet = LINK_Connected(index, evt, info, mem);
			break;

		case LN_DISCONNECT :
			HxLOG_Print(("LINK_Send : LN_DISCONNECT Received\n"));
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("LINK_Send : protocol error . event %d received in %d state !!!\n", evt, ln_tab_slot[index].state);
			iRet = 0;
			break;
		}
...
unsigned char flag;		/* MORE / LAST flag									*/
	...
	HxLOG_Print(("LINK_SendLPDU Start\n"));
...
//KJW - 2003.08.11, Should free tpdu memory after all relevant LPDUs are sent -> Solves descrambling prob. for AB sat. with Viaccess RED CAM
	...
	{
		HxLOG_Error("LINK_SendLPDU : can't allocate buffer for coding TPDU !!!\n");
		return 0;	/* Error */
	}
	...
	*(ln_msg1.lpdu.plpdu + 1) = flag;
	HxSTD_memcpy(ln_msg1.lpdu.plpdu + 2, pt, len);

	if (! LINK_Send_Msg1())
	{
		HxLOG_Error("LINK_SendLPDU : Can't send the LPDU for slot %d to PHYSICAL Task !!!\n", ln_tab_slot[index].slot);
#ifdef FASTALLOC
