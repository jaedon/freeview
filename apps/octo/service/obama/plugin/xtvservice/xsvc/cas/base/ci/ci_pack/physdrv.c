...
/*
	...
	0 : don't use --> performance 가 중요시 된다면 이걸 써라 (디폴트)
	1 : use for write operation only --> write operation 시에만 robust 코드 적용 (CI+ 의 경우 큰 사이즈의 MHEG 테이터를 read 하게 되므로 read operation 시에는 아예 적용하지 않도록 하는 설정)
...
/*
	...
	LG 가 자기네 CI 에이징시의 문제 수정에 적용했다는 솔루션. 정말 이런 margin 이 필요한지는 다소 의문이지만 일단 적용해 둔다.

	주의 사항 :

	1. PHYS_ResetModule() 시 buffer nego 를 위한 R/W 에서는 사용해서는 안된다. 간혹 캠 삽입시 캠이 응답하지 않는 경우가 발생한다.
	2. CI+ 의 경우 큰 사이즈의 MHEG 테이터를 read 하게 되므로 read operation 시에는 사용해서는 안된다.

	이와 같은 이유로 timing margin 은 사실상 일반 write operation 시에만 적용됨.
*/
...
// 밑에 있는 변수는 svc_cas_DevCiWrite(), svc_cas_DevCiRead() 관련해서만 사용되고 이 함수들은 동시 호출이 안되므로 굳이 세마포어로 보호할 필요는 없음.
...
static char * get_tpdu_type (unsigned char *data, unsigned short len)
{
	...
	if (len >= 3) // 호스트에서 보내는 모든 TPDU 는 최소 5 바이트 이상이나...
	{
	...
	if (len >= 2) // 이건 TPDU 가 아니라 캠 초기화시의 버퍼 사이즈 nego 를 위한 2 바이트...
	{
		...
		{	/* wait for Module Insertion and Initialization */
			...
			case OK :	// 0x00
				...
				*len = infos.header.len + sizeof(st_drvheader);
				...
				iRet = PH_INSERT;
#if 1
				// IRHD-5100S, HD-FOX+ (BCM-7325 기반) 모델의 수정 사항.
				// 생산에서 사용하는 Irdeto 캠(Rev1.1)의 경우 캠 초기화 이후에 profile 이 안올라오는 문제가 있음.
				// 여기 딜레이를 주면 일단 해결은 되나 추가 검토 필요.
...
static unsigned char PHYS_InitModule(int index, unsigned int Host_Size, unsigned short *Negociate_Size)
{
	...
	// 드라이버 구현이 거의 이와 같이 되어 있으므로 일반적인 코드로 적용함.
 	...
 	#else
	VK_TASK_Sleep(3500); // 브로드컴 workaround : fail to read CIS w/ Neotion CAM for Canal Ready with development board and power unplugged. also see P_CI_Task().
	#endif
...
// 2010.03.16. Prevent 수정. unreachable: This code cannot be reached: "DRVTab[index].State = 1/*sh..."
//	DRVTab[index].State = DRV_IDLE;
...
/* SmarDTV CI+ old prototype 캠 : 511 bytes
...
static unsigned char PHYS_ResetModule(int index, unsigned int Host_Size, unsigned short *Negociate_Size)
{
	...
	{
		// 에러 아니나 주목할 만한 로그라서 넣어둠. 이 경우 지원 가능하다면 Host_Size (MAXLNSIZE) 를 늘려주는 것이 좋다.
...
In: 				int 					index:	 DRVTab index
				...
				int flag :			0 not used
									...
									2 if the calling function is PHYS_ResetModule -> 이제 PHYS_ResetModule()에서 이 함수를 호출하지 않음.
Out:			Nothing.
Return value:	Status:
				OK: 	No Pb.
...
static unsigned char PHYS_SendData(int index, unsigned char *pData, unsigned short Size_in /*, int flag */)
{
	...
	if (status & DA)
	{
		...
		PHYS_ReceiveIdle(index); /* <TODO_CI> : 이 함수에 에러가 발생하거나 이 직후 다시 DA 가 set 된 경우에 대한 처리에 대해선 좀더 고려 필요... */
	}
	...
	#if 0 // 이런 에러는 발생하지 않음.
	if ((Size_in == 0x0) || (DRVTab[index].State == DRV_CONNECT && Size_in > DRVTab[index].Negociated_size)) // 이런 일이 발생한다면 스택 코드에 뭔가 문제가 있다는 거다. 디버깅 필요.
	{
	...
	if (Size_in == DRVTab[index].Negociated_size) /* 현재 svc_cas_DevCiWrite() 구현 상 호스트의 CI+ authentication 데이터를 write 할 때 너무 오래 걸리게 되므로... */
	{
		if (svc_cas_DevCiWrite(DRVTab[index].Slot_id, Size_in, pData) != (int)Size_in)
		{
			HxLOG_Error("[CI_RW_ERROR] PHYS_SendData(type=%s,size=%d) : svc_cas_DevCiWrite error (%d line) !!!\n", get_tpdu_type(pData, Size_in), Size_in, __LINE__);
	...
	else /* Size_in > 2 and Size_in < nego size */
	{
		...
		for (i = 1; i < Size_in; i++)
		{
			...
			while (1)
			{
				...
				if (retry == PHYS_RETRY_RW_ERROR) /* 이 상황이 되면 여기서 포기하고 에러 리턴하여 전체 operation 을 다시 시작하도록 한다. */
				{
	...
	if (status & WE)
	{
		...
		/* really? one more try -> 여기서 WE 를 다시 체크하는 것은 불필요할 수도... 이런 상황이 발생된 경우를 보면 last byte 가 제대로 쓰여지지 않은 경우였음. */
		...
		if (status & WE)
		{
			...
			if (status & WE)
			{
				...
				/* down bit HC from Command register and return T_ERR */
				#if 0 /* 이 경우 에러 리턴하여 전체 operation 을 다시 시작하도록 하면 write 전에 WE 가 set 되어 있는 상태가 되고 이상하게도 Tsb with no DA 가 2번 읽힌다. 하여 그냥 OK 리턴하도록 처리한다. */
				if (PHYS_WriteCommand(index, 0x00, __LINE__) != 0x00) return(W_SEACH);
				return(T_ERR);
				#endif
			}
...
******************************************************************************/
/* <TODO_CI> : PHYS_ReceiveIdle() 함수는 다음과 같은 이유로 제거되어야 한다. 이 함수를 운용하는 것은 바람직하지 않다!

	현재 플로우 :
	호스트가 데이터 A를 write 하려 함 -> 마침 캠이 데이터 B를 보내려 함
	-> 이 함수를 통해 일단 B를 read 하여 임시 저장 -> A를 write -> 이후 read phase 에서 B에 대한 처리 수행 (상위 스택 전달)

	바림직한 플로우 :
	호스트가 데이터 A를 write 하려 함 -> 마침 캠이 데이터 B를 보내려 함
	-> A를 write 하려던 것을 임시 보류하고 read phase 로 전환하여 B를 read 하고 처리 (상위 스택 전달) -> 보류되었던 A를 write 한다.

	R/W 플로우는 전적으로 캠의 HC & FR / DA 및 폴링에 대한 응답에 의지하여 컨트롤 되므로 항상 캠 우선으로 처리되어야 캠 쪽 플로우와 맞게 처리가 된다.

	특히 데이터 A가 폴링이라면 다음 사항 주의 :
	...
	필요한 작업 :
	1. physical layer task 및 union 으로 설계된 ph_infos 등 변경 필요.
	2. 이 함수는 제거하고 이 함수가 호출되던 시점에서 write operation 을 보류하고 read phase 로 전환하도록 처리해야 함.
*/
static void PHYS_ReceiveIdle(int index)
{
	SLPDU *pSLPDU;
...
static unsigned char PHYS_ReceiveData(int index, unsigned char *pData, unsigned short *Size_out, int flag)
{
	...
	{
//		HxLOG_Print("<IIR>");
		if (CI_IsCiPlusCam(DRVTab[index].Slot_id) > 0) /* 일반 캠의 경우 오동작하는 넘이 있을 수 있으므로 CI+ 캠에 한해서만 하도록 하자. 그리고 간혹 status 가 잘못 읽히는 경우가 있으니 한 번 더 체크해 보자. */
		{
			if ((status = PHYS_ReadStatus(index, __LINE__)) == 0xff) return(W_SEACH);
	...
	if (!(status & DA))
	{
		return(M_DATA); /* if no data available */ /* PHYS_ReceiveIdle() 로부터 호출되어 이 리턴이 되면 에러인 것이다. 이 경우 PHYS_ReceiveIdle() 에서 에러 찍음. */
	}
	...
	if (*Size_out == DRVTab[index].Negociated_size) /* 현재 svc_cas_DevCiRead() 구현 상 큰 사이즈의 CI+ MHEG 테이터를 read 할 때 너무 오래 걸리게 되므로... */
	{
		if (svc_cas_DevCiRead(DRVTab[index].Slot_id, *Size_out, pData) != (int)*Size_out)
		{
			HxLOG_Error("[CI_RW_ERROR] PHYS_ReceiveData(flag=%d,size=%d) : svc_cas_DevCiRead error (%d line) !!!\n", flag, *Size_out, __LINE__);
	...
	else /* *Size_out > 2 and *Size_out < nego size */
	{
		...
		for (i = 1; i < *Size_out; i++)
		{
			...
			while (1)
			{
				...
				if (retry == PHYS_RETRY_RW_ERROR) /* 이 상황이 되면 여기서 포기하고 에러 리턴한다. */
				{
...
//		return(M_BUSY); /* <TODO_CI> : 이런 경우에 이 플로우에서 데이터를 다시 가져오도록 하는 것이 좋을 것 같은데... 좀더 고려 필요... -> physical() 에서 핸들링하도록 하였음 */
	}
	...
	for (index = 0; index < CARM_MAX_CISLOT_NUMBER; index++)
	{
		if (DRVTab[index].Slot_id == slot)
		{
			if (DRVTab[index].State == DRV_CONNECT)
			{
				...
				{
				return -1; // 캠이 꽂혀 있지 않다 (정상 상태)
				}
			}
	}

	return -1; // 캠이 꽂혀 있지 않다. (이상 상태)
}
	...
	if (currentCharPoint)		// if there is $compatible commend, then this Info string has to be checked.
	{
		...
		if(currentArrPoint)
		{
			...
			if(*currentArrPoint == '=')
			{
				...
				{
					*ciplus = atoi(currentArrPoint + 2); // 스펙에 따르면 currentArrPoint + 2 에서 무조건 숫자가 나와야 하는데, 만약 다른 문자가 입력 되어있을 경우에는 atoi 에서 0으로 리턴 된다. 즉, 데이터가 잘못 되었다고 보고 CI Plus 지원을 하지 않는 것으로 본다.

					HxLOG_Print("[CIS] CI+ CAM detected !! (ciplus = %d)\n", *ciplus);
				}
				else if(*(currentArrPoint + 1) == '-')
				{
					// 아직은 고려하지 않음. 하지만 나중에 추가 되어야 할 것임.
				}
				else if(*(currentArrPoint + 1) == '+')
				{
					// 아직은 고려하지 않음. 하지만 나중에 추가 되어야 할 것임.
				}
				else if(atoi(currentArrPoint + 1) > 0) //atoi 리턴 값이 0보다 크다는 것은 ciplus 값이 양수 값을 갖는 다는 의미
				{
		...
		if(currentArrPoint)
		{
			...
			if(*currentArrPoint == '=')
			{
				if(atoi(currentArrPoint + 1) > 0) 		//atoi 리턴 값이 0보다 크다는 것은 ciprof 값이 양수 값을 갖는 다는 의미
				{
