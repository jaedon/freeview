...
// 밑에 있는 변수는 CARM_CI_Write(), CARM_CI_Read() 관련해서만 사용되고 이 함수들은 동시 호출이 안되므로 굳이 세마포어로 보호할 필요는 없음.
...
//	HxLOG_Print("=== PHYS_InitModule :: PHYS_Card (%d:%d) ===\r\n",index,Ret);
	...
	// 드라이버 구현이 거의 이와 같이 되어 있으므로 일반적인 코드로 적용함.
 	...
 	#else
	// TODO: 기존에는 2초간 대기 시간이였는데, 몇몇 Test CAM이 2초 안에 ready 안되는 것이 있어 3.5초로 수정 (for Freesat G2 QT) 추후에 다시 2초 가량으로 바꿔야 함.
...
static unsigned char PHYS_SendData(int index, unsigned char *pData, unsigned short Size_in /*, int flag */)
{
	...
	if (ret != 0)
	{
		if(ret == -15)	// drv_cimax.h error 코드 참고 (CIMAX_CAM_WRITE_BUSY_ERROR)
		{
			return M_BUSY;
		}
...
static int PHYS_GetCiPlusInfoFromString(char* pcInfoStr, int nInfoLength, unsigned int *ciplus, unsigned int *ciprof)
{
	...
	if (currentCharPoint)		// if there is $compatible commend, then this Info string has to be checked.
	{
		...
		if(currentArrPoint)
		{
			...
			if(*currentArrPoint == '=')
			{
				if(atoi(currentArrPoint + 10) > 0) 		//해당 값은 0x00000001 이 string으로 있을 것이기 떄문에 10을 더함. ciprof가 OP이외에 사용 된다면 해당 부분 변경 필요.
				{
