	...
	switch (tag) {
		case session_number :
			...
			*pt++ = 2;		            	/* length_field 	*/
			...
			pt += 2;
			HxSTD_memcpy(pt, pinfos->datass.pbuff, pinfos->datass.len);	/* APDU */
		...
		case open_session_response :
			...
			*pt++ = pinfos->openrss.status; /* session_status	*/
			HxSTD_memcpy(pt, pinfos->openrss.ress_id, SSLENRESS);	/* resource_identifier */
			pt += SSLENRESS;
			SESS_CodSsnb(ssnb, pt);     	/* session_number	*/
			iRet = 5 + SSLENRESS;
			break;
		...
		case create_session :
			...
			*pt++ = 2 + SSLENRESS;     	 	/* length_field 	*/
			HxSTD_memcpy(pt, pinfos->create.ress_id, SSLENRESS);	/* resource_identifier */
			pt += SSLENRESS;
			SESS_CodSsnb(ssnb, pt);     	/* session_number	*/
			iRet = 4 + SSLENRESS;
			break;
		...
		default :
			HxLOG_Error("SESS_Code : Unknown Tag 0x%x !!!\n", tag);
			iRet = 0; // iRet = -1;
			break;
		}
	...
	*tag = *pt++;						/* SPDU's type 	*/
	...
	switch(*tag) {
		...
		case open_session_request :
			if (length != SSLENRESS)
				return 0; // return -2;
			HxSTD_memcpy(pinfos->openss.ress_id, pt, SSLENRESS);	/* resource_identifier */
			if(len != SSLENRESS + 2)
				return 0; // return -2;
			else
				length = len;
		...
		case create_session_response :
			...
			HxSTD_memcpy(pinfos->creatr.ress_id, pt, SSLENRESS);	/* resource_identifier */
			pt += SSLENRESS;
			SESS_DecodSsnb(pt, ssnb);						/* session_number	   */
