...
int iRet;			/* return code 				*/
	...
	switch(s_tab_ssnb[index].state) {
		...
		default :
			HxLOG_Error("SESS_Action : Unknown state %d !!!\n", s_tab_ssnb[index].state);
			iRet = 0;	/* Error */
			break;
		}
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case open_session_request :
			HxLOG_Print("SESS_Idle : open_session_request Received\n");
	        ...
	        HxSTD_memcpy(s_msg.open.ress_id, pinfos->openss.ress_id , SSLENRESS);

			/* Send the message */
    		if (! SESS_Send_Msg())
    		{
				HxLOG_Error("SESS_Idle : Can't send the RM_OPEN_SS_REQ message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
			...
			{
				/* Save the ress_id */
	           	HxSTD_memcpy(s_tab_ssnb[index].ress_id, pinfos->openss.ress_id , SSLENRESS);
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Idle : protocol error . event %d received in %d state !!!\n", evt, s_tab_ssnb[index].state);
...
unsigned long len;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case RM_OPEN_SS_RSP :
			HxLOG_Print("SESS_Open : RM_OPEN_SS_RSP evt. Received\n");
			...
			{
				HxLOG_Error("SESS_Open : can't allocate buffer for coding SPDU !!!\n");
			...
			{	/* the resource is on a DVB distant module ; routing is the tcid of the distant module */
				...
				{	/* can't open the distant session ; close the first part of the session */
					...
					infos.openrss.status = SS_NOTAVAILABLE;
       	        	HxSTD_memcpy(infos.openrss.ress_id , s_tab_ssnb[index].ress_id, SSLENRESS);
    				...
    				if (len == 0) // if (len < 0)
    				{
						HxLOG_Error("SESS_Open : Can't code an open_session_response for 0x%x%x%x%x !!!\n",
    				...
    				if (! SESS_Send_Msg())
    				{
						HxLOG_Error("SESS_Open : Can't send the open_session_response message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
						*mem = 0;		/* s_msg must be released */
						iRet = 0;		/* Error */
					}
					else
				...
				{	/* create the second link of the distributed session */
					/* ================================================= */
    	        	HxSTD_memcpy(infos.create.ress_id,  s_tab_ssnb[index].ress_id, SSLENRESS);
    				...
    				if (len == 0) // if (len < 0)
    				{
						HxLOG_Error("SESS_Open : Can't code a create_session for 0x%x%x%x%x !!!\n",
								pinfos->openrss.ress_id[0], pinfos->openrss.ress_id[1],
								pinfos->openrss.ress_id[2], pinfos->openrss.ress_id[3]);
						*mem = 0;		/* s_msg must be released */
						return 0;		/* Error */
    				...
    				if (! SESS_Send_Msg())
    				{
						HxLOG_Error("SESS_Open : Can't send the create_session message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
						*mem = 0;		/* s_msg must be released */
						iRet = 0;		/* Error */
					}
					else
			...
			{	/* the resource is on the local host */
				...
				else
				{
					if (pinfos->openrss.status == SS_OK)
					{
    					...
    					if (! SESS_Send_Msg1())
    					{
							HxLOG_Error("SESS_Open : Can't send the RM_SS_OPENED message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
							iRet = 0;		/* Error */
						}
						else
						{
		...
		case SS_TC_DELETED :
			HxLOG_Print("SESS_Open : SS_TC_DELETED evt. Received\n");
	    	/* Set the new state */
			s_tab_ssnb[index].state = IDLE;
			break;	/* end of SS_TC_DELETED processing */

		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Open : protocol error . event %d received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if ((evt > MSG_MAXVAL) || (evt == APDU))
				*mem = 0;		/* s_msg must be released if allocated */
			iRet = 1;
			break;
...
unsigned long len;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case APDU :
			HxLOG_Print("SESS_Active : APDU Received\n");
			...
			/* =========================================== */
				...
				{
					HxLOG_Error("SESS_Active : can't allocate buffer for coding SPDU !!!\n");
					return 0;	/* Error */
				}
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("SESS_Active : Can't code a session_number for ssnb %d !!!\n", s_tab_ssnb[index].ssnb);
    			...
    			if (! SESS_Send_Msg1())
    			{
					HxLOG_Error("SESS_Active : Can't send the SPDU message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
					ITC_DynFree(s_msg1.apdu.papdu);
					iRet = 0;		/* Error */
				}
				else
		...
		case session_number :
			HxLOG_Print("SESS_Active : session_number Received\n");
			...
			{	/* decode the SPDU and send the APDU to the RM Task */
				...
				{
					HxLOG_Error("SESS_Active : can't allocate buffer for coding APDU !!!\n");
					return 0;	/* Error */
				}
				...
				HxSTD_memcpy(s_msg1.apdu.papdu, pinfos->datass.pbuff, s_msg1.apdu.len);

				/* Send the message */
    			if (! SESS_Send_Msg1())
    			{
					HxLOG_Error("SESS_Active: Can't send the APDU message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
					ITC_DynFree(s_msg1.apdu.papdu);
					iRet = 0;		/* Error */
				}
				else
		...
		case close_session_request :
			HxLOG_Print("SESS_Active : close_session_request Received\n");
			...
			{	/* Send the response and send a RM_SS_CLOSED to the RM Task */
				...
				if (s_msg.spdu.pspdu == NULL)
				{
					HxLOG_Error("SESS_Active : can't re-allocate buffer for coding SPDU !!!\n");
					return 0;	/* Error */
				}
#else
{
				...
				if (s_msg.spdu.len <= SSCLOSERSPSIZE)
					HxSTD_memcpy(pt, s_msg.spdu.pspdu, s_msg.spdu.len);
				else
					HxSTD_memcpy(pt, s_msg.spdu.pspdu, SSCLOSERSPSIZE);
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("SESS_Active : Can't code an open_session_response for ssnb %d !!!\n", s_tab_ssnb[index].ssnb);
					return 0;		/* Error */
				}
				else
				{
    			...
    			if (! SESS_Send_Msg())
    			{
					HxLOG_Error("SESS_Active : Can't send the open_session_response message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
					iRet = 0;		/* Error */
				}
				else
				{
    				...
    				if (! SESS_Send_Msg1())
    				{
						HxLOG_Error("SESS_Active : Can't send the RM_SS_CLOSED message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
						iRet = 0;		/* Error */
					}
			...
			{   /* Forward the close session request toward the other side of the session */
				...
				/* Send the message */
   				if (! SESS_Send_Msg())
   				{
					HxLOG_Error("SESS_Active : Can't send the close_session_request message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
					iRet = 0;		/* Error */
				}
                else
                {
		...
		case RM_CLOSE_SS :
			HxLOG_Print("SESS_Active : RM_CLOSE_SS Received\n");
			...
			/* ================================================ */
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("SESS_Active : Can't code a close_session_request for ssnb %d !!!\n", s_tab_ssnb[index].ssnb);
					return 0;	/* Error */
				}
				else
				{
    			...
    			if (! SESS_Send_Msg())
    			{
					HxLOG_Error("SESS_Active : Can't send the close_session_request message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
					return 0;	/* Error */
				}
				else
				{
		...
		case SS_TC_DELETED :
			HxLOG_Print("SESS_Active : SS_TC_DELETED Received\n");
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Active : protocol error . event 0x%x received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if (evt > MSG_MAXVAL)
				*mem = 0;		/* s_msg must be released if allocated */
			iRet = 1;
			break;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case create_session_response :
			HxLOG_Print("SESS_Creation : SS_TC_DELETED Received\n");
			...
			if (HxSTD_memcmp(s_tab_ssnb[index].ress_id, pinfos->creatr.ress_id, SSLENRESS))
				return 0;	/* Error */
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("SESS_Creation : Can't code an open_session_response for 0x%x%x%x%x !!!\n",
    		...
    		if (! SESS_Send_Msg())
    		{
				HxLOG_Error("SESS_Creation : Can't send the open_session_response message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
				*mem = 0;		/* Free memory */
				iRet = 0;		/* Error */
			}
			else
		...
		case TIMESESS :
			...
			if (s_msg.spdu.pspdu == NULL)
			{
				HxLOG_Error("SESS_Creation : can't allocate buffer for coding SPDU !!!\n");
				return 1;		/* Error but return = 1 since no allocated memory to free */
			}
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Creation : protocol error . event 0x%x received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if ((evt > MSG_MAXVAL) || (evt == APDU))
				*mem = 0;		/* s_msg must be released if allocated */
			iRet = 1;
			break;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case create_session_response :
			...
			HxLOG_Print("SESS_Release : create_session_response Received\n");
			...
			if (pinfos->creatr.status == SS_OK)
			{
				...
				len = SESS_Code(close_session_request, s_tab_ssnb[index].rssnb, &infos, s_msg.spdu.pspdu);
   				if (len == 0) // if (len < 0)
   				{
					HxLOG_Error("SESS_Release : Can't code a close_session_request for 0x%x%x%x%x !!!\n",
							s_tab_ssnb[index].ress_id[0], s_tab_ssnb[index].ress_id[1],
							s_tab_ssnb[index].ress_id[2], s_tab_ssnb[index].ress_id[3]);
					return 0;		/* Error */
				}
				...
				/* Send the message */
   				if (! SESS_Send_Msg())
   				{
					HxLOG_Error("SESS_Release : Can't send the close_session_request message for tcid %d to RM Task !!!\n", s_tab_ssnb[index].tcid);
					iRet = 0;		/* Error */
				}
				else
				{
		...
		case TIMESESS :
			HxLOG_Print("SESS_Release : TIMESESS Received\n");
			/* same processing that in CREATION state */
			iRet = SESS_Creation(index, evt, pinfos, mem);
			break;	/* end of TIMESESS processing */

		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Release : protocol error . event 0x%x received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if ((evt > MSG_MAXVAL) || (evt == APDU))
				*mem = 0;		/* s_msg must be released if allocated */
			iRet = 1;
			break;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		...
		case close_session_response :
			HxLOG_Print("SESS_Deletion : close_session_response Received\n");
		...
		case SS_TC_DELETED :
			HxLOG_Print("SESS_Deletion : SS_TC_DELETED Received\n");
		...
		case TIMESESS :
			HxLOG_Print("SESS_Deletion : TIMESESS Received\n");
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Deletion : protocol error . event %d received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if ((evt > MSG_MAXVAL) || (evt == APDU))
				*mem = 0;		/* s_msg must be released if allocated */
			iRet = 1;
			break;
...
unsigned long len;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case close_session_response :
			HxLOG_Print("SESS_Close : close_session_response Received\n");
        	/* kill pending timer 	*/
			SESS_Kill_Timer(index);
			*mem = 0;		/* free memory */

		case TIMESESS :
			HxLOG_Print("SESS_Close : TIMESESS Received\n");
			...
			{	/* first case . See DESCRIPTION above */
				...
				if (! SESS_Send_Msg())
				{
					HxLOG_Error("SESS_Close : Can't send the RM_SS_CLOSED message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
			...
			{	/* second case . See DESCRIPTION above */
				...
				if (s_msg1.spdu.pspdu == NULL)
				{
					HxLOG_Error("SESS_Close : can't allocate buffer for coding SPDU !!!\n");
					return 1;	/* for memory management */
				}
				...
				else
				{
   				...
   				if (len == 0) // if (len < 0)
   				{
					HxLOG_Error("SESS_Close : Can't code a close_session_response for tcid %d !!!\n", s_msg1.spdu.tcid);
				...
				if (! SESS_Send_Msg1())
				{
					HxLOG_Error("SESS_Close : Can't send the close_session_response message for ssnb %d to RM Task !!!\n", s_tab_ssnb[index].ssnb);
					ITC_DynFree(s_msg1.spdu.pspdu);
					return 1;		/* for memory management */
				}
		...
		case SS_TC_DELETED :
			HxLOG_Print("SESS_Close : SS_TC_DELETED Received\n");
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("SESS_Close : protocol error . event 0x%x received in %d state !!!\n", evt, s_tab_ssnb[index].state);
			if ((evt > MSG_MAXVAL) || (evt == APDU))
				*mem = 0;		/* s_msg must be released if allocated */
			break;
		}
...
int iRet;		/* Return code */
    ...
    if (! SESS_Send_Msg())
    {
		HxLOG_Error("SESS_TCCreated : Can't send the RM_TC_CREATED message for tcid %d to RM Task !!!\n", tcid);
		iRet = 0;		/* Error */
	}
	else
		iRet = 1;		/* Success */
...
int iRet;		/* Return code */
	...
	HxLOG_Print("SESS_TCDeleted Start\n");

	/* Send the message */
    if (! SESS_Send_Msg())
	{
		HxLOG_Error("SESS_TCDeleted : Can't send the RM_TC_DELETED message for tcid %d to RM Task !!!\n", tcid);
		iRet = 0;		/* Error */
	}
	else
		iRet = 1;		/* Success */
...
int iRet;		/* Return code */
	...
	HxLOG_Print("SESS_DeleteTC Start\n");

    /* Send the message */
	if (! SESS_Send_Msg())
	{
		HxLOG_Error("SESS_DeleteTC : Can't send the SS_DELETE_TC message for tcid %d to TRANS Task !!!\n", tcid);
		iRet = 0;		/* Error */
	}
	else
...
unsigned long len;
	...
	infos.openrss.status = pinfos->openss.status;
   	HxSTD_memcpy(infos.openrss.ress_id, pinfos->openss.ress_id , SSLENRESS);

	HxLOG_Print("SESS_NotOpen Start\n");
	...
	if (len == 0) // if (len < 0)
	{
		HxLOG_Error("SESS_NotOpen : Can't code an open_session_response for 0x%x%x%x%x !!!\n",
	...
	if (! SESS_Send_Msg())
    {
		HxLOG_Error("SESS_NotOpen : Can't send the open_session_response message for tcid %d to RM Task !!!\n", pinfos->openss.tcid);
		iRet = 0;		/* Error */
	}
	else
	{
