...
unsigned char trans;	/* sender/receiver of received event */
	...
	if (! SimulRM_Init())
	{
		HxLOG_Error("simulRM : SimulRM_Init error !!!\n");
	...
	while(1)		/* infinite loop */
	{
...
#endif
		{
			HxLOG_Print(("SIMRM::ITC_WaitEvt . Error in return code\n"));
		...
		{
			HxLOG_Print(("SIMRM::ITC_WaitEvt . Error in received event\n"));
...
//	nr[1] = 0x40;		// MMI simulation on a remote module
	...
	if (t_evt == MESSAGE)
		...
		case RM_OPEN_SS_REQ :
				...
				if (!HxSTD_memcmp(st_msg.open.ress_id, nr, 4))
				{
...
*/
		...
		case APDU :
				HxSTD_memcpy(buff1, st_msg.apdu.papdu, st_msg.apdu.len);
		...
		case RM_SS_CLOSED :
			...
			default :			/* unknown message */
				HxLOG_Print(("SimulRM_State : Unknown message is ignored\n"));
...
int i;
	...
	st_msg.apdu.len  	= len;
	HxSTD_memcpy(st_msg.apdu.papdu, buff, len);
