...
//			break;
		...
		case t_data_more :
			*pt++ = tag;
			...
			pt += lenlen;
			*pt++ = tcid;
			HxSTD_memcpy(pt, pinfos->datatc.pbuff, pinfos->datatc.len-1); /* sub tcid length : 1 byte */
		...
		default :
			HxLOG_Error("TRPT_Code : Unknown Tag 0x%x !!!\n", tag);
			iRet = 0; // iRet = -1;
			break;
		}
...
unsigned char lenlen;	/* length of the length field	*/
	...
	*tag = *pt++;							/* TPDU's type 	*/
	...
	{
		HxLOG_Error("TRPT_Decode : TRPT_GetLength error !!!\n");
		return 0; // return -1;
	}
	...
	{
		HxLOG_Error("TRPT_Decode : tcid error !!!\n");
		return 0; // return -1;
	}
	...
	switch(*tag) {
		case t_sb :
			...
			{
				HxLOG_Error("TRPT_Decode : length error at %d lines !!!\n", __LINE__);
				return 0; // return -1;
			}
			pinfos->sbtc.SB_value = *pt & 0x80; /* don't control the reserved field which must be set to 0 */
			break;
		...
		case t_data_last :
		case t_data_more :
			if((len-2) - length < 0) 		   /* Test the length_field    added by thyou 2005. 10. 30 */
			{
				HxLOG_Error("TRPT_Decode : length error at %d lines !!!\n", __LINE__);
				return 0; // return -1;
			}
		...
		default :
			HxLOG_Error("TRPT_Decode : unknown tpdu !!!\n");
			return 0; // return -1;		/* Error : unknown tpdu */
		}
#endif
