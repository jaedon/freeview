...
extern unsigned char g_aucCiTrptTimoutRetryCnt[MAXTCID];
#endif
...
int mem1;			/* local memory management  */
	...
	HxLOG_Print("TRPT_Action : tcid %d : state %d timer=0x%x timerpoll=0x%x\n",
	...
	switch(t_tab_tcid[index].state) {
		case ACTIVE : 	 iRet = TRPT_Active(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : ACTIVE Received\n");
			break;

		case POLLING : 	 iRet = TRPT_Polling(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : POLLING Received\n");
		...
		case RECEPTION : iRet = TRPT_Reception(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : RECEPTION Received\n");
		...
		case POLLMORE :	 iRet = TRPT_Pollmore(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : POLLMORE Received\n");
		...
		case IDLE : 	 iRet = TRPT_Idle(index, evt, mem);
			HxLOG_Print("TRPT_Action: IDLE Received\n");
			break;

		case CREATION :  iRet = TRPT_Creation(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : CREATION Received\n");
			break;

		case DELETION :  iRet = TRPT_Deletion(index, evt, pinfos, mem);
			HxLOG_Print("TRPT_Action : DELETION Received\n");
			break;

		default :
			HxLOG_Error("TRPT_Action : Unknown state %d !!!\n", t_tab_tcid[index].state);
			iRet = 0;	/* Error */
			break;
		}
...
unsigned long len;
	...
	*mem = 1;	/* default */
	...
	switch(evt) {
		case TP_CONNECT :
			HxLOG_Print(("TRPT_Idle : TP_CONNECT Received\n"));
			...
			{
				HxLOG_Error("TRPT_Idle : can't allocate buffer for coding TPDU !!!\n");
				return 0;	/* Error */
			}
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Idle : Can't code a create_t_c for slot %d !!!\n", t_tab_tcid[index].slot_id);
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Idle : Can't send the create_t_c for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* t_msg.tpdu.ptpdu must be released */
				iRet = 0;		/* Error */
			}
			else
		...
		case t_sb :	/* t_sb received after a delete_t_c : ignored */
			HxLOG_Print(("TRPT_Idle : t_sb Received\n"));
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("TRPT_Idle : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
			iRet = 0;
			break;
		}
...
unsigned long len;
		...
		*mem = 1;		/* default */
		...
		switch(evt) {
		case TIMEOUT :
			HxLOG_Print(("TRPT_Creation : TIMEOUT Received\n"));
		...
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Creation : TP_DISCONNECT Received\n"));
		...
		case c_t_c_reply :
			...
			HxLOG_Print("TRPT_Creation : c_t_c_reply Start tcid = %d, Slot_id = %d\n", t_msg.creat.tcid, t_tab_tcid[index].slot_id /* t_msg.creat.slot_id */);

		    /* Send the message */
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Creation : Can't send the SS_TC_CREATED for tcid %d to SESS Task !!!\n", t_tab_tcid[index].tcid);
				*mem = 0;		/* free received buffer */
				return 0;		/* Error */
			}
		...
		case t_sb :
			HxLOG_Print(("TRPT_Creation : t_sb Received\n"));
			...
			{   /* t_sb(1) received : data available on module */
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("TRPT_Creation : Can't code a t_rcv for slot %d !!!\n", t_tab_tcid[index].slot_id);
					return 0;		/* Error */
				}
				else
				{
    			...
    			if (! TRPT_Send_Msg())
    			{
					HxLOG_Error("TRPT_Creation : Can't send the t_rcv for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
					iRet = 0;		/* Error */
				}
				else
				{
			...
			{   /* t_sb(0) received : no data available on module */
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("TRPT_Creation : Can't code a t_data_last(poll) for slot %d !!!\n", t_tab_tcid[index].slot_id);
					return 0;		/* Error 	   */
				}
				else
				{
    			...
    			if (! TRPT_Send_Msg())
    			{
					HxLOG_Error("TRPT_Creation : Can't send the tpdu(poll) for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
					iRet = 0;		/* Error 	   */
				}
				else
				{
		...
		case delete_t_c :
			HxLOG_Print(("TRPT_Creation : delete_t_c Received\n"));
			...
			{
				HxLOG_Error("TRPT_Creation : can't allocate buffer for coding TPDU !!!\n");
				return 0;	/* Error */
			}
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Creation : Can't code a d_t_c_reply for slot %d !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free received buffer */
				return 0;		/* Error */
			}
			else
    		...
    		if (! TRPT_Send_Msg1())
    		{
				HxLOG_Error("TRPT_Creation : Can't send the d_t_c_reply for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free received buffer */
				iRet = 0;		/* Error */
			}
			else
		...
		default :	/* protocol error, ignore the event */
			HxLOG_Error("TRPT_Creation : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
			iRet = 0;
			break;
		}
...
unsigned long len;
	...
	*mem = 1;		/* default */
	...
	switch(evt) {
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Active : TP_DISCONNECT Received\n"));
		...
		case SPDU:
			HxLOG_Print(("TRPT_Active : SPDU Received\n"));
			...
			if (t_msg1.tpdu.ptpdu == NULL)
			{
				HxLOG_Error("TRPT_Active : can't allocate buffer for coding TPDU !!!\n");
				return 0;	/* Error */
			}
			...
			if (t_msg.spdu.len <= MAXTPDUSIZE)
			{
				...
				len  	= TRPT_Code(t_data_last, t_tab_tcid[index].tcid, &infos, t_msg1.tpdu.ptpdu);
   				if (len == 0) // if (len < 0)
   				{
					HxLOG_Error("TRPT_Active : Can't code a t_data_last for slot %d !!!\n", t_tab_tcid[index].slot_id);
			...
			{   /* split SPDU into TPDUs, send the first TPDU */
				...
				len  	= TRPT_Code(t_data_more, t_tab_tcid[index].tcid, &infos, t_msg1.tpdu.ptpdu);
   				if (len == 0) // if (len < 0)
   				{
					HxLOG_Error("TRPT_Active : Can't code a t_data_more for slot %d !!!\n", t_tab_tcid[index].slot_id);
					ITC_DynFree(t_msg1.tpdu.ptpdu);  	/* free memory */
					return 0;		/* Error */
				}
				else
			...
			if (! TRPT_Send_Msg1())
			{
				HxLOG_Error("TRPT_Active : Can't send the t_data_last for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				ITC_DynFree(t_msg1.tpdu.ptpdu);  	/* free memory */
				iRet = 0;		/* Error */
			}
			else
		...
		case SS_DELETE_TC :
			HxLOG_Print(("TRPT_Active : SS_DELETE_TC Received\n"));
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Active : Can't code a delete_t_c for slot %d !!!\n", t_tab_tcid[index].slot_id);
				return 0;		/* Error */
			}
			else
			{
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Active : Can't send the delete_t_c for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				iRet = 0;		/* Error */
			}
			else
			{
		...
		case TIMEPOLL :
			HxLOG_Print(("TRPT_Active : TIMEPOLL Received\n"));
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Active : Can't code a t_data_last(poll) for slot %d !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free memory */
				return 0;		/* Error */
			}
			else
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Active : Can't send the tpdu(poll) for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free memory */
				iRet = 0;		/* Error */
			}
			else
		...
		/* AlphaCrypt light cam sometimes send t_sb without any prior object sent from host. so just ignore it (coming from I-CORD HD+ branch revision 24544, 2010-01-27)
			--> 하지만 원인이 분명치 않으므로 일단 막아둠. also see TRPT_Polling().
			--> Irdeto 캠으로도 동일한 상황 확인됨. 확인해 보니 이건 캠 문제가 아니라 캠과 호스트 간 R/W operation 에 문제가 발생한 경우에 서로 간의 state 가 틀어져서 발생하는 문제임.
			       호스트 쪽의 R/W 함수를 수정하면 해결될 문제임. see PHYS_SendData() and PHYS_ReceiveData(). */
#if 0
		case t_sb :
			HxLOG_Error("TRPT_Active : t_sb received !!!\n");
			break;

#endif

		default :	/* protocol error, release the transport connection */
			HxLOG_Error("TRPT_Active : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
...
unsigned long len;
	...
	*mem = 1;	/* default 													*/
	...
	switch(evt) {
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Polling : TP_DISCONNECT Received\n"));
		...
		case t_sb :
			HxLOG_Print(("TRPT_Polling : t_sb Received\n"));

#if 1 // #ifdef _INCLUDE_CI_TRPT_RECOVERY_
			g_aucCiTrptTimoutRetryCnt[index] = 0;
#endif
			...
			{   /* t_sb(1) received : data available on module */
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("TRPT_Polling : Can't code a t_rcv for slot %d !!!\n", t_tab_tcid[index].slot_id);
					return 0;		/* Error */
				}
				else
				{
    			...
    			if (! TRPT_Send_Msg())
    			{
					HxLOG_Error("TRPT_Polling : Can't send the t_rcv for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
					iRet = 0;		/* Error */
				}
				else
				{
			...
			{   /* t_sb(0) received : no data available on module */
				...
				if ((t_tab_tcid[index].len > 0) && (t_tab_tcid[index].pspdu != NULL))
				{
					...
					{	/* several TPDU to send ; send the first here */
						...
						if (t_msg1.tpdu.ptpdu == NULL)
						{
							HxLOG_Error("TRPT_Polling : can't allocate buffer for coding TPDU !!!\n");
							return 0;	/* Error */
						}
    					...
    					if (len == 0) // if (len < 0)
    					{
							HxLOG_Error("TRPT_Polling : Can't code a t_data_more for slot %d !!!\n", t_tab_tcid[index].slot_id);
							return 0;		/* Error */
						}
						else
						{
					...
					{	/* last TPDU to send */
						...
						if (len == 0) // if (len < 0)
						{
							HxLOG_Error("TRPT_Polling : Can't code a t_data_last for slot %d !!!\n", t_tab_tcid[index].slot_id);
							return 0;		/* Error */
						}
						else
						{
    				...
    				if (! TRPT_Send_Msg1())
    				{
						HxLOG_Error("TRPT_Polling : Can't send the tpdu(poll) for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
						iRet = 0;		/* Error */
					}
					else
					{
				...
				else
				{
					...
					else
					{
						...
						HxLOG_Error("TRPT_Polling : garbage the polling (0x%02x)\n", t_tab_tcid[index].tag);
		...
		case delete_t_c :
			HxLOG_Print(("TRPT_Polling : delete_t_c Received\n"));
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Polling : Can't code a d_t_c_reply for slot %d !!!\n", t_tab_tcid[index].slot_id);
				return 0;		/* Error */
			}
			else
			{
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Polling : Can't send the d_t_c_reply for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				iRet = 0;		/* Error */
			}
			else
				/* send the SS_TC_DELETED message */
		...
		case request_t_c :
			HxLOG_Print(("TRPT_Polling : request_t_c Received\n"));
			...
			if (new_index < 0)
			{
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("TRPT_Polling : Can't code a t_c_error for slot %d !!!\n", t_tab_tcid[index].slot_id);
    			...
    			if (! TRPT_Send_Msg())
    			{
					HxLOG_Error("TRPT_Polling : Can't send the t_c_error for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
					TRPT_FreeIndex(new_index);		/* free the new instance */
					iRet = 0;						/* Error */
				}
				else
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Polling : Can't code a new_t_c for slot %d !!!\n", t_tab_tcid[index].slot_id);
				TRPT_FreeIndex(new_index);		/* free the new instance */
				return 0;						/* Error */
			}
			else
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Polling : Can't send the new_t_c for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				TRPT_FreeIndex(new_index);		/* free the new instance */
				iRet = 0;						/* Error */
			}
			else
		...
		case SPDU :
			HxLOG_Print(("TRPT_Polling : SPDU Received\n"));
		...
		case SS_DELETE_TC :
			HxLOG_Print(("TRPT_Polling : SS_DELETE_TC Received\n"));
		...
		case TIMEOUT :
			HxLOG_Print(("TRPT_Polling : TIMEOUT Received\n"));
    		...
    		if (len == 0) // if (len < 0)
    		{
				HxLOG_Error("TRPT_Polling : Can't code a delete_t_c for slot %d !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free memory */
				return 0;		/* Error */
			}
			else
    		...
    		if (! TRPT_Send_Msg())
    		{
				HxLOG_Error("TRPT_Polling : Can't send the delete_t_c for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
				*mem = 0;		/* free memory */
				iRet = 0;		/* Error */
			}
			else
		...
		case TIMEPOLL :
			HxLOG_Print(("TRPT_Polling : TIMEPOLL Received\n"));
			break;

		/* AlphaCrypt cam sometimes send c_t_c_reply in this state. so just ignore it
			--> 하지만 원인이 분명치 않으므로 일단 막아둠. also see TRPT_Active().
			--> Irdeto 캠으로도 동일한 상황 확인됨. 확인해 보니 이건 캠 문제가 아니라 캠과 호스트 간 R/W operation 에 문제가 발생한 경우에 서로 간의 state 가 틀어져서 발생하는 문제임.
			       호스트 쪽의 R/W 함수를 수정하면 해결될 문제임. see PHYS_SendData() and PHYS_ReceiveData(). */
#if 0
		case c_t_c_reply :
			HxLOG_Error("TRPT_Polling : c_t_c_reply received !!!\n");
			break;
#endif

		default :	/* protocol error, release the transport connection */
			HxLOG_Error("TRPT_Polling : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
...
int alloc;		/* memory management										*/
	...
	*mem = 1;	/* default 													*/
	...
	switch(evt) {
		...
		case t_sb :
			HxLOG_Print(("TRPT_Pollmore : t_sb Received\n"));
		...
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Pollmore : TP_DISCONNECT Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case TIMEOUT :
			HxLOG_Print(("TRPT_Pollmore : TIMEOUT Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case delete_t_c :
			HxLOG_Print(("TRPT_Pollmore : delete_t_c Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case SS_DELETE_TC :
			HxLOG_Print(("TRPT_Pollmore : SS_DELETE_TC Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case SPDU :
			HxLOG_Print(("TRPT_Pollmore : SPDU Received\n"));
		...
		default :	/* protocol error, release the transport connection (in particular : request_t_c) */
			HxLOG_Error("TRPT_Pollmore : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
...
unsigned long len;
	...
	*mem = 1;		/* default */
	...
	switch(evt) {
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Reception : TP_DISCONNECT Received\n"));
		...
		case t_data_last :
			HxLOG_Print(("TRPT_Reception : t_data_last Received\n"));
			...
			{	/* t_data_last without t_data_more */
				...
				{
					HxLOG_Error("TRPT_Reception : can't allocate buffer for coding SPDU !!!\n");
					return 0;	/* Error */
				}
			}
            else
...
#else
{
				...
				if (pt == NULL)
				{
					HxLOG_Error("TRPT_Reception : can't reallocate buffer for coding SPDU !!!\n");
					return 0;	/* Error */
				}

				HxSTD_memcpy(pt, t_tab_tcid[index].rpspdu, t_tab_tcid[index].rlen);
			...
			HxSTD_memcpy(t_msg1.spdu.pspdu + t_tab_tcid[index].rlen, pinfos->datatc.pbuff, pinfos->datatc.len);

		    /* Send the message */
    		if (! TRPT_Send_Msg1())
    		{
				HxLOG_Error("TRPT_Reception : Can't send the SPDU for tcid %d to SESS Task !!!\n", t_tab_tcid[index].tcid);
		...
		case t_data_more :
			HxLOG_Print(("TRPT_Reception : t_data_more Received\n"));
...
#else
{
			...
			HxSTD_memcpy(t_tab_tcid[index].rpspdu + t_tab_tcid[index].rlen, pinfos->datatc.pbuff, pinfos->datatc.len);
		...
		case t_sb :
			HxLOG_Print(("TRPT_Reception : t_sb Received\n"));
			...
			{   /* t_sb(1) received : data available on module */
    			...
    			if (len == 0) // if (len < 0)
    			{
					HxLOG_Error("TRPT_Reception : Can't code a t_rcv for slot %d !!!\n", t_tab_tcid[index].slot_id);
					return 0;		/* Error */
				}
				else
				{
    			...
    			if (! TRPT_Send_Msg())
    			{
					HxLOG_Error("TRPT_Reception : Can't send the t_rcv for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
					iRet = 0;		/* Error */
				}
				else
				{
			...
			{   /* t_sb(0) received : no data available on module */
				...
				else
				{
					...
					HxLOG_Error("TRPT_Reception : garbage the polling (0x%02x)\n", t_tab_tcid[index].tag);
					t_tab_tcid[index].tag = 0;
				}
			}   /* end of t_sb(0) processing  */
			break;	/* end of t_sb processing */
		...
		case TIMEOUT :
			HxLOG_Print(("TRPT_Reception : TIMEOUT Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case delete_t_c :
			HxLOG_Print(("TRPT_Reception : delete_t_c Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case SS_DELETE_TC :
			HxLOG_Print(("TRPT_Reception : SS_DELETE_TC Received\n"));
			iRet = TRPT_Polling(index, evt, pinfos, mem);
			break;

		case SPDU :
			HxLOG_Print(("TRPT_Reception : SPDU Received\n"));
		...
		default :
			HxLOG_Error("TRPT_Reception : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
...
int iRet;	/* return code */
	...
	*mem = 1;		/* default */
	...
	switch(evt) {
		case TIMEOUT :
			HxLOG_Print(("TRPT_Deletion : TIMEOUT Received\n"));
		...
		case TP_DISCONNECT :
			HxLOG_Print(("TRPT_Deletion : TP_DISCONNECT Received\n"));
		...
		case d_t_c_reply :
			HxLOG_Print(("TRPT_Deletion : d_t_c_reply Received\n"));
		...
		case t_sb :
			HxLOG_Print(("TRPT_Deletion : Garbage previous received t_sb\n"));
			iRet = 1;
			break;

		default :	/* protocol error, ignore the event */
			HxLOG_Error("TRPT_Deletion : protocol error . event 0x%x received in %d state !!!\n", evt, t_tab_tcid[index].state);
			iRet = 0;
			break;
		}
...
int TRPT_Disc(int index)
{
	...
	HxLOG_Print(("TRPT_Disc Start\n"));

    /* Send the message */
	if (! TRPT_Send_Msg())
	{
		HxLOG_Error("TRPT_Disc : Can't send the SS_TC_DELETED for tcid %d to SESS Task !!!\n", t_tab_tcid[index].tcid);
		return 0;		/* Error */
	}
...
unsigned long len;
	...
	*mem = 1;		/* default */
	...
	HxLOG_Print(("TRPT_Release Start\n"));
	...
	if (alloc)
	{
		...
		if (t_msg.tpdu.ptpdu == NULL)
		{
			HxLOG_Error("TRPT_Release : can't allocate buffer for coding TPDU !!!\n");
			return 0;	/* Error */
		}
	}
	...
	if (len == 0) // if (len < 0)
	{
		HxLOG_Error("TRPT_Release : Can't code a delete_t_c for slot %d !!!\n", t_tab_tcid[index].slot_id);
		*mem = 0;		/* t_msg.tpdu.ptpdu must be released */
		return 0;		/* Error */
	}
	else
	...
	if (! TRPT_Send_Msg())
	{
		HxLOG_Error("TRPT_Release : Can't send the delete_t_c for slot %d to LINK Task !!!\n", t_tab_tcid[index].slot_id);
		*mem = 0;		/* t_msg.tpdu.ptpdu must be released */
		iRet = 0;		/* Error */
	}
	else
