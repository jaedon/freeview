...
int
der_decode_Tag_(FILE *der, struct der_tag *tag)
{
	unsigned int type;
	unsigned int len;
	unsigned char byte;
...
int
der_decode_OctetString_(FILE *der, FILE *out, int length)
{
	int left = length;
	unsigned char byte;
...
int
der_decode_Null_(FILE *der, FILE *out, int length)
{
	if(length != 0)
...
int
der_read_file_(FILE *in, unsigned int nbytes, void *buf)
{
	int nread;
...
#else // hmkim : modified.
int
der_error_at_(char *fn, int line, char *fmt, ...)
{
	va_list args;
