...
/* 주의 : 여기 구현된 bignum 라이브러리는 양수만을 취급한다. 따라서 b > a 인 경우에 대한 처리는 caller 쪽에서 핸들링해야 한다! */
...
void bn_mul(bignum *a, bignum *b, bignum *out)
{
	...
	if ((bn_cmp(a, &bn_zero) == 0) || (bn_cmp(b, &bn_zero) == 0)) // 불필요한 연산을 막기 위해 (자리수 오류 보정 포함).
	{
		out->len = 1;
		out->num[0] = 0x00;
		return;
	...
	if (bn_cmp(a, &bn_one) == 0) // 불필요한 연산을 막기 위해.
	{
		out->len = qLen;
	...
	if (bn_cmp(b, &bn_one) == 0) // 불필요한 연산을 막기 위해.
	{
		out->len = pLen;
...
static void bn_square(bignum *a, bignum *out)
{
	...
	if (bn_cmp(a, &bn_x10) < 0) // 불필요한 연산을 막기 위해 (자리수 오류 보정 포함).
	{
		out->len = 1;
		out->num[0] = x[0] * x[0];
		return;
...
static void bn_mul_2_n(bignum *a, int n, bignum *out)
{
	...
	if (bn_cmp(a, &bn_zero) == 0) // 불필요한 연산을 막기 위해 (자리수 오류 보정 포함).
	{
		out->len = 1;
		out->num[0] = 0x00;
		return;
...
static void bn_right_shift_n(bignum *a, int n, bignum *out)
{
	...
	if ((bn_cmp(a, &bn_zero) == 0) || (bn_cmp(a, &bn_one) == 0)) // 불필요한 연산을 막기 위해 (자리수 오류 보정 포함).
	{
		out->len = 1;
		out->num[0] = 0x00;
		return;
...
static void bn_pow(bignum *a, bignum *b, bignum *out)
{
	...
	if ((bn_cmp(a, &bn_zero) == 0) || (bn_cmp(a, &bn_one) == 0)) // 불필요한 연산을 막기 위해.
	{
		out->len = 1;
		out->num[0] = a->num[0];
		return;
