...
#define	GCC_PRINTFLIKE(fmt,var)	__attribute__((format(HxLOG_Print,fmt,var)))
#endif
#else
#ifndef	GCC_PRINTFLIKE
...
#include <vkernel.h>
#define	CALLOC(nmemb, size)	OxCAS_Calloc(nmemb * size) // calloc(nmemb, size)
#define	MALLOC(size)			OxCAS_Malloc(size) // malloc(size)
#define	REALLOC(oldptr, size)	OxCAS_Realloc(oldptr, size) // realloc(oldptr, size)
#define	FREEMEM(ptr)			OxCAS_Free(ptr) // free(ptr)
