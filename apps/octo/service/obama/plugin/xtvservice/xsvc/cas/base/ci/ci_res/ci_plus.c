/***************************************************************
* $Workfile:   ci_plus.c  $
* $Modtime:
* Auther : hmkim
*
* Desc : CI+ Protocol Porting Layer
*
* Copyright (c) 2008 by Humax Co., Ltd.
* All right reserved
****************************************************************/


/***************************************************************
* #ifdef statements and include
***************************************************************/

#include <pal_ci.h>
#include "ci_plus.h"
#include "ci_hci.h"
#include "./ci_cc_lib/ci_cc_cipher.h"
#include "./ci_cc_lib/ci_cc_x509.h"
#include "ci_mgr_int.h"

#if !defined(WIN32)
#include <unistd.h>			// for sync()
#else
#define	sync
#endif

#include <stdio.h>
#include <errno.h>

#include <db_common.h>		// for DB_JFFS2_PATH define
#include <db_param.h>
//#include <pal_key.h>		// for CI_MES_LicenseDownCbFunc()

#include <cb_list.h>		// for CI_SetCckToDsc(), CI_ApplyUriToExternalOutput() and callbacks
#include <isosvc.h>			// for CI_GetCountryCode() and CI_GetLanguageCode()

#include <_svc_cas_sub_dev_api.h>


/***************************************************************
* local definition
***************************************************************/



#define CI_HOST_LICENSE_CONST_FILE	DB_JFFS2_PATH "/ci_plus_licenseconst.bin" // host license constants 저장 : < 주의 > 이 내용은 생산 S/W 에도 적용되므로 절대 임의로 수정하지 말 것 !!
#define CI_AUTH_CTXS_1ST_FILE		DB_JFFS2_PATH "/ci_plus_authctxs_1st.bin" // authentication contexts 저장 : 1 slot 모델에선 이것만 사용된다.
#define CI_AUTH_CTXS_2ND_FILE		DB_JFFS2_PATH "/ci_plus_authctxs_2nd.bin" // authentication contexts 저장 : 2 slot 모델에선 이것도 사용된다.
#define CI_AUTH_CTXS_3RD_FILE		DB_JFFS2_PATH "/ci_plus_authctxs_3rd.bin" // authentication contexts 저장 : 2 slot 모델에선 이것도 사용된다.
#define CI_AUTH_CTXS_4TH_FILE		DB_JFFS2_PATH "/ci_plus_authctxs_4th.bin" // authentication contexts 저장 : 2 slot 모델에선 이것도 사용된다.
#define CI_AUTH_CTXS_5TH_FILE		DB_JFFS2_PATH "/ci_plus_authctxs_5th.bin" // authentication contexts 저장 : 2 slot 모델에선 이것도 사용된다.


#if defined(CONFIG_PROD_ENDIAN_LITTLE)
static unsigned short CI_ByteSwap_16(unsigned short x)
{
	return (x>>8) | (x<<8);
}

static unsigned int CI_ByteSwap_32(unsigned int x)
{
	x = ((x<<8)&0xFF00FF00) | ((x>>8)&0x00FF00FF);
	return (x>>16) | (x<<16);
}

#define BSWAP_16(x)	CI_ByteSwap_16(x)
#define BSWAP_32(x)	CI_ByteSwap_32(x)
#endif

/***************************************************************
* static / extern variables and structures
***************************************************************/

static unsigned char K[CI_CC_CIPHER_KEY_LEN]		= {0x62,0x79,0x20,0x68,0x6D,0x6B,0x69,0x6D,0x2C,0x20,0x68,0x75,0x6D,0x61,0x78,0x2E}; /* const */
static unsigned char IV[CI_CC_CIPHER_BLOCK_SIZE]	= {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};

static CI_PLUS_MODE s_enPlusMode[PAL_MAX_CISLOT_NUMBER] = {CI_PLUS_FREE_MODE, CI_PLUS_FREE_MODE};
static CI_HOST_MODE s_enHostMode = CI_HOST_MODE_NORMAL;

// CI+ data download task callback status variables
static HBOOL s_bIsLicenseDownCbRegSuccess = FALSE;
static HBOOL s_bIsLicenseDownTaskEnd = FALSE;

#if defined(CONFIG_CI_PLUS_AUTH_STORE_VER2)
extern CI_AUTH_CTXs s_stCiAuthCtxs;
#else
extern CI_AUTH_CTXs s_stCiAuthCtxs[CARM_MAX_CISLOT_NUMBER];
#endif

/***************************************************************
* global variables
***************************************************************/

CALLBACK_HANDLE		g_hCbNotifyCcReport = NULL;		// for ci_cc_main.c
CALLBACK_HANDLE		g_hCbNotifyPinCapaInfo = NULL;
CALLBACK_HANDLE		g_hCbNotifySetKey = NULL;			// for CI_SetCckToDsc()
CALLBACK_HANDLE		g_hCbNotifyUriControl = NULL;	// for CI_ApplyUriToExternalOutput()
CALLBACK_HANDLE		g_hCbNotifySrmControl = NULL;	// for CI_ApplySrmData()
CALLBACK_HANDLE		g_hCbNotifyHostTune = NULL;		// for ci_hci.c
CALLBACK_HANDLE		g_hCbNotifyPidReplace = NULL;	// for ci_hci.c
CALLBACK_HANDLE		g_hCbNotifyCamUpgrade = NULL;	// for ci_cu.c
#if defined(CONFIG_OP_CANALREADY)
CALLBACK_HANDLE		g_hCbNotifyCanalReadyInit = NULL;			// for ci_auth.c
CALLBACK_HANDLE		g_hCbNotifyCanalReadyMoralLevel = NULL;		// for ci_auth.c
CALLBACK_HANDLE		g_hCbNotifyCanalReadyPinReset = NULL;		// for ci_auth.c
#endif
#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
CALLBACK_HANDLE 	g_hCbNotifyCcPinStatus = NULL;				// for ci_cc_main.c
CALLBACK_HANDLE		g_hCbNotifyCcCnfData = NULL;				// for ci_cc_main.c
CALLBACK_HANDLE		g_hCbNotifyCcLicense = NULL;				// for ci_cc_main.c
CALLBACK_HANDLE		g_hCbNotifyCcPinEvent = NULL;				// for ci_cc_main.c
#endif

CALLBACK_HANDLE		g_hCbNotifyOperatorPrepareReq = NULL;		// for ci_op.c
CALLBACK_HANDLE		g_hCbNotifyOperatorTune = NULL;		// for ci_op.c
CALLBACK_HANDLE		g_hCbNotifyOperatorNit = NULL;		// for ci_op.c


/***************************************************************
* static function bodies
***************************************************************/



/***************************************************************
* extern function prototypes
***************************************************************/

extern int CiAmmi_RegisterCallback(CI_AMMI_CALLBACK_T pfnCallback);
extern int CiAmmi_FileRequest(unsigned short usSsNo, CI_AMMI_REQUEST_TYPE enReqType, unsigned char *pucReqName, unsigned long ulReqNameLen);
extern int CiAmmi_AppAbortRequest(unsigned short usSsNo, CI_AMMI_ABORT_REQUEST_CODE enAbortReqCode);

extern int CiCc_GetHostId(unsigned char *pucHostId);
extern int CiCc_GetBindedCamId(CI_SLOT_ID usSlotId, unsigned char *pucBindedCamId);
extern int CiCc_GetBindedCamBrandId(CI_SLOT_ID usSlotId, unsigned short *pusBindedCamBrandId);
extern int CiCc_GetNegotiatedCckCipher(CI_SLOT_ID usSlotId, CI_CC_KEY_CIPHER *penNegotiatedCckCipher);
extern void CiCc_ReqSetCckToDsc(CI_SLOT_ID usSlotId);
extern void CiCc_SetUriDefault(CI_SLOT_ID usSlotId, HBOOL bOn);
extern int CiCc_IsOpened(CI_SLOT_ID usSlotId);
extern int CiCc_SetOperatorProfileSupportInfo(CI_SLOT_ID usSlotId, HUINT8 ucNitVer, HUINT8 ucInfoVer);
extern int CiCc_GetAuthContextBySlotId(CI_SLOT_ID usSlotId, CI_AUTH_CTX 	*pstCiAuthCtx);
extern HBOOL CiCc_CheckCcAuthenticationComplete(HUINT16 usSlotId);
extern int PHYS_IsCiPlusCam(unsigned short slot);
extern int PHYS_OperatorProfileSupported(unsigned short slot);

#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
extern HULONG CiCc_GetCcResourceVersionBySlot(CI_SLOT_ID usSlotId);
extern int CiCc_PrepareRecordStart(CI_SLOT_ID usSlotId, Handle_t hAction, CI_OPERATING_MODE eOperatingMode, unsigned short usProgramNumber);
extern int CiCc_ChangeOperatingMode(CI_SLOT_ID usSlotId, unsigned int unSessionId, CI_OPERATING_MODE enOperatingMode, unsigned short usProgramNumber);
extern int CiCc_GetRecordingStatus(CI_SLOT_ID usSlotId, HUINT8 *pucCcRecordingStatus);
extern int CiCc_RecordStop(CI_SLOT_ID usSlotId, unsigned short usProgramNumber);
//extern int CiCc_Playback(CI_SLOT_ID usSlotId, unsigned int unSessionId, unsigned short usProgramNumber, unsigned char *pucLicenseData, unsigned short usLicenseDataLen);
extern int CiCc_CheckLicense(CI_SLOT_ID usSlotId, unsigned int unSessionId, unsigned char *pucLicenseData, unsigned short usLicenseDataLen);
extern int CiCc_GetPinCapaInfo(CI_SLOT_ID usSlotId, CI_PIN_CAPA_INFO *pstPinCapaInfo);
extern int CiCc_StoreCicamPinInfo(CI_SLOT_ID usSlotId, HUINT8 *pucPinInfo, UNIXTIME utPinSavedTime);
extern int CiCc_GetUriInfo(CI_SLOT_ID usSlotId, CIPLUS_UriInfo_t *pstUriInfo);
extern int CiCc_PinPlayback(CI_SLOT_ID usSlotId, HUINT32 unMetaIndex, Handle_t hPbAction, HUINT8 ucAgeLimit, HUINT8 *pucPrivateData);
extern int CiCc_SendHostLicense(CI_SLOT_ID usSlotId, HUINT32 unMetaIndex, Handle_t hPbAction, HUINT16 usProgramNum, HUINT8 *pucLicenseData, HUINT16 usLicenseDataLen);
#endif

extern int CiHci_TuneReply(CI_SLOT_ID usSlotId, CI_TUNE_STATUS eTuneResult);

extern int CiHlc_NotifyHostCountryInfoChanged(CI_SLOT_ID usSlotId);
extern int CiHlc_NotifyHostLanguageInfoChanged(CI_SLOT_ID usSlotId);

#ifdef CONFIG_OP_CANALREADY
extern int CiAuth_CanalReadyGetInitInfo(CI_SLOT_ID usSlotId, SvcCas_CiAuthInfo_t *pstCiAuthInfo);
extern int CiAuth_CanalReadyMoralityLevelUnlock(CI_SLOT_ID usSlotId);
#endif

extern int CiOp_SendOperatorSearchStart(CI_SLOT_ID usSlotId);
extern int CiOp_SendOperatorSearchCancel(unsigned short usSsNo);
extern int CiOp_SendOperatorTuneStatus(CI_SLOT_ID usSlotId, unsigned char ucStrength, unsigned char ucQuality, unsigned char ucStatus, HUINT8 *pucDes, HUINT8 ucDesLen, unsigned char ucNextDesNum);
extern HBOOL CiOp_GetOperatorProfileErrStatus(CI_SLOT_ID usSlotId);
extern HBOOL CiOp_CheckSimpleEntitlementChange(CI_SLOT_ID usSlotId);
extern int CiOp_SendOperatorExitBySlotId(CI_SLOT_ID usSlotId);
extern int CiOp_SendOperatorStatusReqBySlotId(CI_SLOT_ID usSlotId);
extern int CiOp_GetOperatorReservedTime(HUINT16 usSlotId, UNIXTIME *putTime);


/***************************************************************
* local function prototypes
***************************************************************/

#if defined(CONFIG_DEBUG)
static BOOL local_CI_IsNumber(unsigned char ch);
static BOOL local_CI_PrintCertificatePeriod(Certificate_t *cert);
static void local_CI_PrintCertificate(CI_HOST_LICENSE_CONST *pstCiHostLicenseConst);
#endif

/***************************************************************
* public function bodies
***************************************************************/

// get the proper data rate on the PCMCIA bus : return CI_PCMCIA_DATA_RATE_72_MBPS for 72 Mbps (host mandatory), CI_PCMCIA_DATA_RATE_96_MBPS for 96 Mbps (host optional)
unsigned char CI_GetDataRate (void)
{
	return CI_PCMCIA_DATA_RATE_72_MBPS; // 현 상태로 추가 코딩은 필요 없을 듯...
}

// register callback function to ci_ammi.c, which should be called by CI+ MHEG init flow. it returns -1 if there is any error
int CI_RegisterAppMmiCallback (CI_AMMI_CALLBACK_T pfnCallback)
{
	return CiAmmi_RegisterCallback(pfnCallback);
}

// request file or data to ci_ammi.c, which is called by CI+ MHEG engine. it returns -1 if there is any error
// - usSessionId    : CI_AMMI_MSG_REQ_START 메시지로 받은 usSessionId
// - enReqType      : CI_AMMI_REQUEST_TYPE
// - pucReqName     : enReqType 값에 따라 파일명 내지는 데이터명
// - ulReqNameLen   : pucReqName 의 길이 (사실상 unsigned char 이고 unsigned int 로 충분)
int CI_RequestFileorData (unsigned short usSessionId, CI_AMMI_REQUEST_TYPE enReqType, unsigned char *pucReqName, unsigned long ulReqNameLen)
{
	return CiAmmi_FileRequest(usSessionId, enReqType, pucReqName, ulReqNameLen);
}

// request app abort to ci_ammi.c, which is called by CI+ MHEG engine. it returns -1 if there is any error
// - usSessionId    : CI_AMMI_MSG_REQ_START 또는 CI_AMMI_MSG_FILE_ACK 메시지로 받은 usSessionId
// - enAbortReqCode : CI_AMMI_ABORT_REQUEST_CODE
int CI_RequestAppAbort (unsigned short usSessionId, CI_AMMI_ABORT_REQUEST_CODE enAbortReqCode)
{
	return CiAmmi_AppAbortRequest(usSessionId, enAbortReqCode);
}

// load CI+ host license constants data from non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_LoadHostLicenseConstants (CI_HOST_LICENSE_CONST *pstCiHostLicenseConst)
{
	unsigned short usOrgLen = sizeof(CI_HOST_LICENSE_CONST), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *C = NULL, *P = NULL;
	unsigned char D[32];

// ewlee 20101222 - for testing CI+
	if (pstCiHostLicenseConst == NULL)
	{
		HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : Invalid params !!!\n");
		return -1;
	}

	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : C Malloc fail !!!\n");
		return -1;
	}	
	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);	
	if(P == NULL)
	{	
		HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : P Malloc fail !!!\n");
		OxCAS_Free(C);
		return -1;
	}


#if defined(CI_PLUS_PRODUCTION_Credentials) || defined(CI_PLUS_TEST_Technology_from_TrustCenter) || defined(CI_PLUS_TEST_Technology_from_SmarDTV)
	{
		int ret;

		ret  = CI_SaveHostLicenseConstantsForTestKitOnly(TRUE, C);
		if(ret != ERR_OK)
		{
			HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : Can't read data !!!\n");
			OxCAS_Free(C);
			OxCAS_Free(P);
			return -1;
		}
	}
#else
	if (svc_cas_DevSysGetField(eCAS_DEV_FLASH_FIELD_CI_PLUS_KEY, 0, (void*)C, usAlignedLen) != ERR_OK)
	{
		HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : Can't read data !!!\n");
		OxCAS_Free(C);
		OxCAS_Free(P);
		return -1;
	}
#endif

	D_AES_CBC(K, C, P, usAlignedLen, IV);
	HxSTD_memcpy(pstCiHostLicenseConst, P, usOrgLen);

	OxCAS_Free(C);
	OxCAS_Free(P);

	SHA_256((unsigned char *)pstCiHostLicenseConst, usOrgLen-32, D);

	if (HxSTD_memcmp(pstCiHostLicenseConst->ucDigest, D, 32) != 0)
	{
		HxLOG_Error("[CI+]---> CI_LoadHostLicenseConstants()---> Error : Invalid digest !!!\n");
		return -1;
	}

#if defined(CONFIG_PROD_ENDIAN_LITTLE)
	pstCiHostLicenseConst->RootCertLen = BSWAP_16(pstCiHostLicenseConst->RootCertLen);
	pstCiHostLicenseConst->Host_BrandCertLen = BSWAP_16(pstCiHostLicenseConst->Host_BrandCertLen);
	pstCiHostLicenseConst->Host_DevCertLen = BSWAP_16(pstCiHostLicenseConst->Host_DevCertLen);
	pstCiHostLicenseConst->Host_CckCipher = BSWAP_32(pstCiHostLicenseConst->Host_CckCipher);
#endif

	#if defined(CONFIG_DEBUG)
	local_CI_PrintCertificate(pstCiHostLicenseConst);
	#endif

	return ERR_OK;
}

#ifdef CI_PLUS_PRODUCTION_Credentials
// save CI+ host license constants data to non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_SaveHostLicenseConstantsForTestKitOnly (HBOOL bRetHostLicense, unsigned char *pstCihostLicenseConst)
{
	CI_HOST_LICENSE_CONST stCiHostLicenseConst;

	unsigned short usOrgLen = sizeof(CI_HOST_LICENSE_CONST), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	unsigned char	RootCert[CI_MAX_CERT_LEN] = // from ciplus_root.cert -> cer
	{
		
	};
	unsigned char	Host_BrandCert[CI_MAX_CERT_LEN] = // from ciplus_humax_ca.cert -> cer
	{
		
	};
	unsigned char	Host_DevCert[CI_MAX_CERT_LEN] = // from 0A0BF75306A286DD-cert.der
	{
		
	};
	unsigned char	DH_p[CI_DH_P_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_g[CI_DH_G_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_q[CI_DH_Q_LEN] = // from Credentials 문서
	{
		0xE2, 0x9C, 0x11, 0x9B, 0x5E, 0x32, 0xDE, 0x82, 0x7C, 0x7B, 0xD1, 0x59, 0x42, 0xA0, 0x6B, 0xB2,
		0x2A, 0xE2, 0x8D, 0x29, 0x02, 0xC6, 0xD3, 0xDB, 0x48, 0x9F, 0xD1, 0xBF, 0xC5, 0xCF, 0x1E, 0xE3
	};
	unsigned char	HDQ_d[CI_HDQ_D_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	HDQ_p[CI_HDQ_P_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	HDQ_q[CI_HDQ_Q_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	HDQ_dP[CI_HDQ_DP_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	HDQ_dQ[CI_HDQ_DQ_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	HDQ_qInv[CI_HDQ_QINV_LEN] = // from 0A0BF75306A286DD-key.der
	{
		
	};
	unsigned char	PRNG_seed[16]	= {0xe7, 0xf1, 0xdd, 0x16, 0x56, 0x85, 0xa5, 0x70, 0xc6, 0x73, 0xec, 0x8c, 0x3c, 0xd1, 0xc5, 0xb2}; // from Credentials 문서
	unsigned char	SLK[16]			= {0xea, 0x74, 0xf4, 0x71, 0x99, 0xd7, 0x6f, 0x35, 0x89, 0xf0, 0xd1, 0xdf, 0x0f, 0xee, 0xe3, 0x00}; // from Credentials 문서
	unsigned char	SIV[16]			= {0xf7, 0x70, 0xb0, 0x36, 0x03, 0x61, 0xf7, 0x96, 0x65, 0x74, 0x8a, 0x26, 0xea, 0x4e, 0x85, 0x41}; // from Credentials 문서
	unsigned char	CLK[16]			= {0x3e, 0x20, 0x15, 0x84, 0x2c, 0x37, 0xce, 0xe3, 0xd6, 0x14, 0x57, 0x3e, 0x3a, 0xab, 0x91, 0xb6}; // from Credentials 문서

	HxSTD_memset(&stCiHostLicenseConst, 0x00, sizeof(CI_HOST_LICENSE_CONST));

	stCiHostLicenseConst.RootCertLen = CI_ROOT_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.RootCert, RootCert, stCiHostLicenseConst.RootCertLen);

	stCiHostLicenseConst.Host_BrandCertLen = CI_HOST_BRAND_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_BrandCert, Host_BrandCert, stCiHostLicenseConst.Host_BrandCertLen);

	stCiHostLicenseConst.Host_DevCertLen = CI_HOST_DEV_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_DevCert, Host_DevCert, stCiHostLicenseConst.Host_DevCertLen);

	HxSTD_memcpy(stCiHostLicenseConst.DH_p, DH_p, CI_DH_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_g, DH_g, CI_DH_G_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_q, DH_q, CI_DH_Q_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.HDQ_d, HDQ_d, CI_HDQ_D_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_p, HDQ_p, CI_HDQ_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_q, HDQ_q, CI_HDQ_Q_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dP, HDQ_dP, CI_HDQ_DP_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dQ, HDQ_dQ, CI_HDQ_DQ_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_qInv, HDQ_qInv, CI_HDQ_QINV_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.PRNG_seed, PRNG_seed, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SLK, SLK, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SIV, SIV, 16);
	HxSTD_memcpy(stCiHostLicenseConst.CLK, CLK, 16);

	// Host_ID, Host_CckCipher, HDQ_n (from 디바이스 인증서)
	{
		Certificate_t * pstDevCertData = NULL;
		int nScramblerCapabilities, len;

		pstDevCertData = X509_ParseRawCert(Host_DevCert, CI_HOST_DEV_CERT_LEN);
		if (pstDevCertData == NULL)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DevCert !!!\n");
			return -1;
		}

		// get Host_ID
		if (FALSE == GetDeviceID(pstDevCertData, stCiHostLicenseConst.Host_ID))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetDeviceID() !!!\n");
			return -1;
		}

		// get Host_CckCipher
		if (FALSE == GetScramblerCapabilities(pstDevCertData, &nScramblerCapabilities))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities() !!!\n");
			return -1;
		}
		if (nScramblerCapabilities == 0 || nScramblerCapabilities == 1)
		{
			stCiHostLicenseConst.Host_CckCipher = nScramblerCapabilities;
		}
		else // unknown value (reserved for future use) -> CI+ 스펙 1.2 기준에서 정의되지 않은 값일 경우엔 일단 CI_CC_KEY_CIPHER_DES_56_ECB 로 설정하도록 하자...
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities()---> unknown value %d !!!\n", nScramblerCapabilities);
			stCiHostLicenseConst.Host_CckCipher = CI_CC_KEY_CIPHER_DES_56_ECB;
		}

		// get HDQ_n
		if (FALSE == GetRSAPublicKey_N(pstDevCertData, stCiHostLicenseConst.HDQ_n, &len))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N() !!!\n");
			return -1;
		}
		if (len != CI_HDQ_N_LEN)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N()---> invalid len %d !!!\n", len);
			return -1;
		}
	}

	// HDQ_n (from 키 인증서), HDQ_d, HDQ_p, HDQ_q, HDQ_dP, HDQ_dQ, HDQ_qInv
	{
		// PC 프로그램의 경우, 해당 호스트 디바이스 인증서와 매칭되는 키 인증서로부터 유도.
		// 특히, HDQ_n 의 경우엔 위의 디바이스 인증서로 유도한 값과 키 인증서로부터 유도한 값이 서로 같은지 확인해야 한다.
	}

	// DHH, DHPH
	{
		bignum bn_DH_g, bn_DHH, bn_DH_p, bn_DHPH;
		int i;

		// generate a random nonce : DHH[256] -> DHH[0..15], DHH[16..31], ... , DHH[240..255] -> PRNG(&DHH[]) 16번 처리 -> concatenated to 256 bytes -> OK?
		for (i = 0; i < 256; i += CI_CC_CIPHER_BLOCK_SIZE)
			PRNG(PRNG_seed, &stCiHostLicenseConst.DHH[i]);

		// compute DHPH
		bn_init(&bn_DH_g, DH_g, CI_DH_G_LEN);
		bn_init(&bn_DHH, stCiHostLicenseConst.DHH, 256);
		bn_init(&bn_DH_p, DH_p, CI_DH_P_LEN);
		bn_mod_exp(&bn_DH_g, &bn_DHH, &bn_DH_p, &bn_DHPH);
		if (bn_DHPH.len != 256)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DHPH len (%d) !!!\n", bn_DHPH.len);
			return -1;
		}

		// The host checks that the computed key DHPH is of valid length and value : if 1 < DHPH < DH_p and DHPH ^ DH_q mod DH_p = 1
		{
			bignum bn_DH_q, out;
			if (bn_cmp(&bn_one, &bn_DHPH) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not bigger than 1 !!!\n");
				return -1;
			}
			if (bn_cmp(&bn_DHPH, &bn_DH_p) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not less than DH_p !!!\n");
				return -1;
			}
			bn_init(&bn_DH_q, DH_q, CI_DH_Q_LEN);
			bn_mod_exp(&bn_DHPH, &bn_DH_q, &bn_DH_p, &out);
			if (bn_cmp(&out, &bn_one) != 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH ^ DH_q mod DH_p not 1 !!!\n");
				return -1;
			}
		}

		HxSTD_memcpy(stCiHostLicenseConst.DHPH, bn_DHPH.num, bn_DHPH.len);
	}

	stCiHostLicenseConst.ucSavedFlag = 2;

#if defined(CONFIG_PROD_ENDIAN_LITTLE)
	stCiHostLicenseConst.RootCertLen = BSWAP_16(stCiHostLicenseConst.RootCertLen);
	stCiHostLicenseConst.Host_BrandCertLen = BSWAP_16(stCiHostLicenseConst.Host_BrandCertLen);
	stCiHostLicenseConst.Host_DevCertLen = BSWAP_16(stCiHostLicenseConst.Host_DevCertLen);
	stCiHostLicenseConst.Host_CckCipher = BSWAP_32(stCiHostLicenseConst.Host_CckCipher);
#endif

	SHA_256((unsigned char *)&stCiHostLicenseConst, usOrgLen-32, stCiHostLicenseConst.ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}

	HxSTD_memcpy(P, &stCiHostLicenseConst, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	if(bRetHostLicense == TRUE)
	{
		if(pstCihostLicenseConst == NULL)
		{
			HxLOG_Error("argument null !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		HxSTD_memcpy(pstCihostLicenseConst, C, usAlignedLen);
	}
	else
	{
		if (svc_cas_DevSysSetField(eCAS_DEV_FLASH_FIELD_CI_PLUS_KEY, 0, (void*)C, usAlignedLen) != ERR_OK)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Can't write data !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}

	OxCAS_Free(P);
	OxCAS_Free(C);

	return ERR_OK;
}
#endif // CI_PLUS_PRODUCTION_Credentials

#ifdef CI_PLUS_TEST_Technology_from_TrustCenter
// save CI+ host license constants data to non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_SaveHostLicenseConstantsForTestKitOnly (HBOOL bRetHostLicense, unsigned char *pstCihostLicenseConst)
{
	CI_HOST_LICENSE_CONST stCiHostLicenseConst;

	unsigned short usOrgLen = sizeof(CI_HOST_LICENSE_CONST), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	unsigned char	RootCert[CI_MAX_CERT_LEN] = // from ciplus_test_root_ca_R01.crt -> cer
	{
		
	};
	unsigned char	Host_BrandCert[CI_MAX_CERT_LEN] = // from ciplus_test_brand_ca_B52.crt -> cer
	{
		
	};
	unsigned char	Host_DevCert[CI_MAX_CERT_LEN] = // from H11-cert.der
	{
		
	};
	unsigned char	DH_p[CI_DH_P_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_g[CI_DH_G_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_q[CI_DH_Q_LEN] = // from Credentials 문서
	{
		0xD8, 0x5E, 0xAB, 0xE8, 0xBE, 0x5B, 0xE3, 0xD2, 0xCF, 0x50, 0xE7, 0x90, 0xA8, 0x6A, 0x3F, 0xAD,
		0xDA, 0xB6, 0xE8, 0xA5, 0x0F, 0x32, 0x87, 0x71, 0xFD, 0x0B, 0xF0, 0x52, 0xAE, 0x3C, 0xA9, 0x37
	};
	unsigned char	HDQ_d[CI_HDQ_D_LEN] = // from H11-key.der
	{
		
	};
	unsigned char	HDQ_p[CI_HDQ_P_LEN] = // from H11-key.der
	{
		
	};
	unsigned char	HDQ_q[CI_HDQ_Q_LEN] = // from H11-key.der
	{
		
	};
	unsigned char	HDQ_dP[CI_HDQ_DP_LEN] = // from H11-key.der
	{
		
	};
	unsigned char	HDQ_dQ[CI_HDQ_DQ_LEN] = // from H11-key.der
	{
		
	};
	unsigned char	HDQ_qInv[CI_HDQ_QINV_LEN] = // from H11-key.der
	{
		
	};

	unsigned char	PRNG_seed[16]	= {0xe5, 0x82, 0x97, 0x31, 0x7f, 0x8c, 0x60, 0x47, 0xdd, 0xd4, 0xa5, 0xd3, 0xd3, 0x2d, 0xc1, 0x5c}; // from Credentials 문서
	unsigned char	SLK[16]			= {0x9c, 0x69, 0xd1, 0x46, 0x70, 0x0d, 0x81, 0x6c, 0xfd, 0x49, 0x69, 0x71, 0x26, 0x93, 0xa3, 0x8a}; // from Credentials 문서
	unsigned char	SIV[16]			= {0xd1, 0xe8, 0xde, 0x32, 0x2e, 0x44, 0xd8, 0x7c, 0x56, 0x90, 0x81, 0x89, 0x5f, 0x50, 0x50, 0x35}; // from Credentials 문서
	unsigned char	CLK[16]			= {0xae, 0xdc, 0x1b, 0x80, 0x4c, 0x50, 0xd9, 0x5d, 0xad, 0x6d, 0x3d, 0x46, 0xac, 0xd6, 0x01, 0xa8}; // from Credentials 문서

	HxSTD_memset(&stCiHostLicenseConst, 0x00, sizeof(CI_HOST_LICENSE_CONST));

	stCiHostLicenseConst.RootCertLen = CI_ROOT_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.RootCert, RootCert, stCiHostLicenseConst.RootCertLen);

	stCiHostLicenseConst.Host_BrandCertLen = CI_HOST_BRAND_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_BrandCert, Host_BrandCert, stCiHostLicenseConst.Host_BrandCertLen);

	stCiHostLicenseConst.Host_DevCertLen = CI_HOST_DEV_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_DevCert, Host_DevCert, stCiHostLicenseConst.Host_DevCertLen);

	HxSTD_memcpy(stCiHostLicenseConst.DH_p, DH_p, CI_DH_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_g, DH_g, CI_DH_G_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_q, DH_q, CI_DH_Q_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.HDQ_d, HDQ_d, CI_HDQ_D_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_p, HDQ_p, CI_HDQ_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_q, HDQ_q, CI_HDQ_Q_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dP, HDQ_dP, CI_HDQ_DP_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dQ, HDQ_dQ, CI_HDQ_DQ_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_qInv, HDQ_qInv, CI_HDQ_QINV_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.PRNG_seed, PRNG_seed, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SLK, SLK, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SIV, SIV, 16);
	HxSTD_memcpy(stCiHostLicenseConst.CLK, CLK, 16);

	// Host_ID, Host_CckCipher, HDQ_n (from 디바이스 인증서)
	{
		Certificate_t * pstDevCertData = NULL;
		int nScramblerCapabilities, len;

		pstDevCertData = X509_ParseRawCert(Host_DevCert, CI_HOST_DEV_CERT_LEN);
		if (pstDevCertData == NULL)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DevCert !!!\n");
			return -1;
		}

		// get Host_ID
		if (FALSE == GetDeviceID(pstDevCertData, stCiHostLicenseConst.Host_ID))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetDeviceID() !!!\n");
			return -1;
		}

		// get Host_CckCipher
		if (FALSE == GetScramblerCapabilities(pstDevCertData, &nScramblerCapabilities))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities() !!!\n");
			return -1;
		}
		if (nScramblerCapabilities == 0 || nScramblerCapabilities == 1)
		{
			stCiHostLicenseConst.Host_CckCipher = nScramblerCapabilities;
		}
		else // unknown value (reserved for future use) -> CI+ 스펙 1.2 기준에서 정의되지 않은 값일 경우엔 일단 CI_CC_KEY_CIPHER_DES_56_ECB 로 설정하도록 하자...
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities()---> unknown value %d !!!\n", nScramblerCapabilities);
			stCiHostLicenseConst.Host_CckCipher = CI_CC_KEY_CIPHER_DES_56_ECB;
		}

		// get HDQ_n
		if (FALSE == GetRSAPublicKey_N(pstDevCertData, stCiHostLicenseConst.HDQ_n, &len))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N() !!!\n");
			return -1;
		}
		if (len != CI_HDQ_N_LEN)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N()---> invalid len %d !!!\n", len);
			return -1;
		}
	}

	// HDQ_n (from 키 인증서), HDQ_d, HDQ_p, HDQ_q, HDQ_dP, HDQ_dQ, HDQ_qInv
	{
		// PC 프로그램의 경우, 해당 호스트 디바이스 인증서와 매칭되는 키 인증서로부터 유도.
		// 특히, HDQ_n 의 경우엔 위의 디바이스 인증서로 유도한 값과 키 인증서로부터 유도한 값이 서로 같은지 확인해야 한다.
	}

	// DHH, DHPH
	{
		bignum bn_DH_g, bn_DHH, bn_DH_p, bn_DHPH;
		int i;

		// generate a random nonce : DHH[256] -> DHH[0..15], DHH[16..31], ... , DHH[240..255] -> PRNG(&DHH[]) 16번 처리 -> concatenated to 256 bytes -> OK?
		for (i = 0; i < 256; i += CI_CC_CIPHER_BLOCK_SIZE)
			PRNG(PRNG_seed, &stCiHostLicenseConst.DHH[i]);

		// compute DHPH
		bn_init(&bn_DH_g, DH_g, CI_DH_G_LEN);
		bn_init(&bn_DHH, stCiHostLicenseConst.DHH, 256);
		bn_init(&bn_DH_p, DH_p, CI_DH_P_LEN);
		bn_mod_exp(&bn_DH_g, &bn_DHH, &bn_DH_p, &bn_DHPH);
		if (bn_DHPH.len != 256)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DHPH len (%d) !!!\n", bn_DHPH.len);
			return -1;
		}

		// The host checks that the computed key DHPH is of valid length and value : if 1 < DHPH < DH_p and DHPH ^ DH_q mod DH_p = 1
		{
			bignum bn_DH_q, out;
			if (bn_cmp(&bn_one, &bn_DHPH) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not bigger than 1 !!!\n");
				return -1;
			}
			if (bn_cmp(&bn_DHPH, &bn_DH_p) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not less than DH_p !!!\n");
				return -1;
			}
			bn_init(&bn_DH_q, DH_q, CI_DH_Q_LEN);
			bn_mod_exp(&bn_DHPH, &bn_DH_q, &bn_DH_p, &out);
			if (bn_cmp(&out, &bn_one) != 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH ^ DH_q mod DH_p not 1 !!!\n");
				return -1;
			}
		}

		HxSTD_memcpy(stCiHostLicenseConst.DHPH, bn_DHPH.num, bn_DHPH.len);
	}

	stCiHostLicenseConst.ucSavedFlag = 2;

#if defined(CONFIG_PROD_ENDIAN_LITTLE)
	stCiHostLicenseConst.RootCertLen = BSWAP_16(stCiHostLicenseConst.RootCertLen);
	stCiHostLicenseConst.Host_BrandCertLen = BSWAP_16(stCiHostLicenseConst.Host_BrandCertLen);
	stCiHostLicenseConst.Host_DevCertLen = BSWAP_16(stCiHostLicenseConst.Host_DevCertLen);
	stCiHostLicenseConst.Host_CckCipher = BSWAP_32(stCiHostLicenseConst.Host_CckCipher);
#endif

	SHA_256((unsigned char *)&stCiHostLicenseConst, usOrgLen-32, stCiHostLicenseConst.ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}	

	HxSTD_memcpy(P, &stCiHostLicenseConst, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	if(bRetHostLicense == TRUE)
	{
		if(pstCihostLicenseConst == NULL)
		{
			HxLOG_Error("argument null  !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		HxSTD_memcpy(pstCihostLicenseConst, C, usAlignedLen);
	}
	else
	{
		if (svc_cas_DevSysSetField(eCAS_DEV_FLASH_FIELD_CI_PLUS_KEY, 0, (void*)C, usAlignedLen) != ERR_OK)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Can't write data !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}

	OxCAS_Free(P);
	OxCAS_Free(C);

	return ERR_OK;
}
#endif // CI_PLUS_TEST_Technology_from_TrustCenter

#ifdef CI_PLUS_TEST_Technology_from_SmarDTV
// save CI+ host license constants data to non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_SaveHostLicenseConstantsForTestKitOnly (HBOOL bRetHostLicense, unsigned char *pstCihostLicenseConst)
{
	CI_HOST_LICENSE_CONST stCiHostLicenseConst;

	unsigned short usOrgLen = sizeof(CI_HOST_LICENSE_CONST), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	unsigned char	RootCert[CI_MAX_CERT_LEN] = // from ciplusRootCert.c
		{};
	unsigned char	Host_BrandCert[CI_MAX_CERT_LEN] =
	{
		#if 1 // from ciplusBrandACert.c
		
		#else // from ciplusBrandBCert.c
		 
		#endif
	};
	unsigned char	Host_DevCert[CI_MAX_CERT_LEN] =
	{
		#if 1 // from ciplusDevice1Cert.c (1045 bytes)
		
		#else // from ciplusDevice2Cert.c (1051 bytes)
		 
		#endif
	};
	unsigned char	DH_p[CI_DH_P_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_g[CI_DH_G_LEN] = // from Credentials 문서
	{
		
	};
	unsigned char	DH_q[CI_DH_Q_LEN] = // from Credentials 문서
	{
		0xF9, 0x4F, 0x97, 0x2C, 0x07, 0xAD, 0xB1, 0x95, 0x8E, 0xD1, 0xB7, 0x36, 0x41, 0xD0, 0x95, 0xC2,
		0x09, 0x27, 0x54, 0x7A, 0xDC, 0x9F, 0xE8, 0x95, 0x94, 0xEB, 0x79, 0x65, 0xB4, 0xBC, 0xC7, 0xD7
	};
	unsigned char	HDQ_d[CI_HDQ_D_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	HDQ_p[CI_HDQ_P_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	HDQ_q[CI_HDQ_Q_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	HDQ_dP[CI_HDQ_DP_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	HDQ_dQ[CI_HDQ_DQ_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	HDQ_qInv[CI_HDQ_QINV_LEN] = // from ciplusDevice1Key.c and ciplusDevice2Key.c
	{
		
	};
	unsigned char	PRNG_seed[16]	= {0x7A, 0x98, 0x15, 0x38, 0x18, 0xC3, 0x78, 0x9B, 0xAA, 0xBE, 0xF6, 0x97, 0x59, 0x05, 0x26, 0xA0}; // from Credentials 문서
	unsigned char	SLK[16]			= {0x35, 0x6B, 0xF3, 0xB8, 0xB9, 0x3D, 0x75, 0x9C, 0xCF, 0x5C, 0x25, 0x53, 0x2B, 0xA0, 0xFF, 0x60}; // from Credentials 문서
	unsigned char	SIV[16]			= {0x09, 0x3C, 0xB1, 0x0B, 0xE6, 0xFF, 0x4D, 0xAD, 0x1A, 0x48, 0x85, 0x81, 0x34, 0x8B, 0x17, 0x62}; // from Credentials 문서
	unsigned char	CLK[16]			= {0x17, 0xB5, 0xF8, 0xE0, 0x39, 0xDA, 0x8A, 0x47, 0xDE, 0x05, 0x38, 0xB1, 0x8A, 0x6B, 0x26, 0x51}; // from Credentials 문서

	HxSTD_memset(&stCiHostLicenseConst, 0x00, sizeof(CI_HOST_LICENSE_CONST));

	stCiHostLicenseConst.RootCertLen = CI_ROOT_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.RootCert, RootCert, stCiHostLicenseConst.RootCertLen);

	stCiHostLicenseConst.Host_BrandCertLen = CI_HOST_BRAND_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_BrandCert, Host_BrandCert, stCiHostLicenseConst.Host_BrandCertLen);

	stCiHostLicenseConst.Host_DevCertLen = CI_HOST_DEV_CERT_LEN;
	HxSTD_memcpy(stCiHostLicenseConst.Host_DevCert, Host_DevCert, stCiHostLicenseConst.Host_DevCertLen);

	HxSTD_memcpy(stCiHostLicenseConst.DH_p, DH_p, CI_DH_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_g, DH_g, CI_DH_G_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.DH_q, DH_q, CI_DH_Q_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.HDQ_d, HDQ_d, CI_HDQ_D_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_p, HDQ_p, CI_HDQ_P_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_q, HDQ_q, CI_HDQ_Q_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dP, HDQ_dP, CI_HDQ_DP_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_dQ, HDQ_dQ, CI_HDQ_DQ_LEN);
	HxSTD_memcpy(stCiHostLicenseConst.HDQ_qInv, HDQ_qInv, CI_HDQ_QINV_LEN);

	HxSTD_memcpy(stCiHostLicenseConst.PRNG_seed, PRNG_seed, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SLK, SLK, 16);
	HxSTD_memcpy(stCiHostLicenseConst.SIV, SIV, 16);
	HxSTD_memcpy(stCiHostLicenseConst.CLK, CLK, 16);

	// Host_ID, Host_CckCipher, HDQ_n (from 디바이스 인증서)
	{
		Certificate_t * pstDevCertData = NULL;
		int nScramblerCapabilities, len;

		pstDevCertData = X509_ParseRawCert(Host_DevCert, CI_HOST_DEV_CERT_LEN);
		if (pstDevCertData == NULL)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DevCert !!!\n");
			return -1;
		}

		// get Host_ID
		if (FALSE == GetDeviceID(pstDevCertData, stCiHostLicenseConst.Host_ID))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetDeviceID() !!!\n");
			return -1;
		}

		// get Host_CckCipher
		if (FALSE == GetScramblerCapabilities(pstDevCertData, &nScramblerCapabilities))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities() !!!\n");
			return -1;
		}
		if (nScramblerCapabilities == 0 || nScramblerCapabilities == 1)
		{
			stCiHostLicenseConst.Host_CckCipher = nScramblerCapabilities;
		}
		else // unknown value (reserved for future use) -> CI+ 스펙 1.2 기준에서 정의되지 않은 값일 경우엔 일단 CI_CC_KEY_CIPHER_DES_56_ECB 로 설정하도록 하자...
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetScramblerCapabilities()---> unknown value %d !!!\n", nScramblerCapabilities);
			stCiHostLicenseConst.Host_CckCipher = CI_CC_KEY_CIPHER_DES_56_ECB;
		}

		// get HDQ_n
		if (FALSE == GetRSAPublicKey_N(pstDevCertData, stCiHostLicenseConst.HDQ_n, &len))
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N() !!!\n");
			return -1;
		}
		if (len != CI_HDQ_N_LEN)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : GetRSAPublicKey_N()---> invalid len %d !!!\n", len);
			return -1;
		}
	}

	// HDQ_n (from 키 인증서), HDQ_d, HDQ_p, HDQ_q, HDQ_dP, HDQ_dQ, HDQ_qInv
	{
		// PC 프로그램의 경우, 해당 호스트 디바이스 인증서와 매칭되는 키 인증서로부터 유도.
		// 특히, HDQ_n 의 경우엔 위의 디바이스 인증서로 유도한 값과 키 인증서로부터 유도한 값이 서로 같은지 확인해야 한다.
	}

	// DHH, DHPH
	{
		bignum bn_DH_g, bn_DHH, bn_DH_p, bn_DHPH;
		int i;

		// generate a random nonce : DHH[256] -> DHH[0..15], DHH[16..31], ... , DHH[240..255] -> PRNG(&DHH[]) 16번 처리 -> concatenated to 256 bytes -> OK?
		for (i = 0; i < 256; i += CI_CC_CIPHER_BLOCK_SIZE)
			PRNG(PRNG_seed, &stCiHostLicenseConst.DHH[i]);

		// compute DHPH
		bn_init(&bn_DH_g, DH_g, CI_DH_G_LEN);
		bn_init(&bn_DHH, stCiHostLicenseConst.DHH, 256);
		bn_init(&bn_DH_p, DH_p, CI_DH_P_LEN);
		bn_mod_exp(&bn_DH_g, &bn_DHH, &bn_DH_p, &bn_DHPH);
		if (bn_DHPH.len != 256)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Invalid DHPH len (%d) !!!\n", bn_DHPH.len);
			return -1;
		}

		// The host checks that the computed key DHPH is of valid length and value : if 1 < DHPH < DH_p and DHPH ^ DH_q mod DH_p = 1
		{
			bignum bn_DH_q, out;
			if (bn_cmp(&bn_one, &bn_DHPH) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not bigger than 1 !!!\n");
				return -1;
			}
			if (bn_cmp(&bn_DHPH, &bn_DH_p) >= 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH not less than DH_p !!!\n");
				return -1;
			}
			bn_init(&bn_DH_q, DH_q, CI_DH_Q_LEN);
			bn_mod_exp(&bn_DHPH, &bn_DH_q, &bn_DH_p, &out);
			if (bn_cmp(&out, &bn_one) != 0)
			{
				HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : DHPH ^ DH_q mod DH_p not 1 !!!\n");
				return -1;
			}
		}

		HxSTD_memcpy(stCiHostLicenseConst.DHPH, bn_DHPH.num, bn_DHPH.len);
	}

	stCiHostLicenseConst.ucSavedFlag = 2;

#if defined(CONFIG_PROD_ENDIAN_LITTLE)
	stCiHostLicenseConst.RootCertLen = BSWAP_16(stCiHostLicenseConst.RootCertLen);
	stCiHostLicenseConst.Host_BrandCertLen = BSWAP_16(stCiHostLicenseConst.Host_BrandCertLen);
	stCiHostLicenseConst.Host_DevCertLen = BSWAP_16(stCiHostLicenseConst.Host_DevCertLen);
	stCiHostLicenseConst.Host_CckCipher = BSWAP_32(stCiHostLicenseConst.Host_CckCipher);
#endif

	SHA_256((unsigned char *)&stCiHostLicenseConst, usOrgLen-32, stCiHostLicenseConst.ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}	

	HxSTD_memcpy(P, &stCiHostLicenseConst, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	if(bRetHostLicense == TRUE)
	{
		if(pstCihostLicenseConst == NULL)
		{
			HxLOG_Error("argument null !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		HxSTD_memcpy(pstCihostLicenseConst, C, usAlignedLen);
	}
	else
	{
		if (svc_cas_DevSysSetField(eCAS_DEV_FLASH_FIELD_CI_PLUS_KEY, 0, (void*)C, usAlignedLen) != ERR_OK)
		{
			HxLOG_Error("[CI+]---> CI_SaveHostLicenseConstants()---> Error : Can't write data !!!\n");
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}

	OxCAS_Free(P);
	OxCAS_Free(C);

	return ERR_OK;
}
#endif // CI_PLUS_TEST_Technology_from_SmarDTV

#if defined(CONFIG_CI_PLUS_AUTH_STORE_VER2)
static HUINT8 * local_CI_GetAuthFileName(HUINT32 unIndex)
{
	switch(unIndex)
	{
	case 0: return CI_AUTH_CTXS_1ST_FILE;
	case 1: return CI_AUTH_CTXS_2ND_FILE;
	case 2: return CI_AUTH_CTXS_3RD_FILE;
	case 3: return CI_AUTH_CTXS_4TH_FILE;
	case 4: return CI_AUTH_CTXS_5TH_FILE;
	default : return NULL;
	}
}

// init CI+ authentication contexts data and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_InitAuthContexts (void)
{
	HUINT8			*pucFileName;
	HUINT32			i;
	CI_AUTH_CTX		stCiAuthCtx;

	unsigned short usOrgLen = sizeof(CI_AUTH_CTX), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	HxSTD_memset(&stCiAuthCtx, 0x00, usOrgLen);
	SHA_256((unsigned char *)&stCiAuthCtx, usOrgLen-32, stCiAuthCtx.ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}	

	HxSTD_memcpy(P, &stCiAuthCtx, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	for(i = 0; i < CI_NUM_OF_AUTH_CTX; i++)
	{
		pucFileName = local_CI_GetAuthFileName(i);
		fpBinaryFile = fopen(pucFileName, "wb");
		if (fpBinaryFile == NULL)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : (%d) Can't open file - %s !!!\n", i, strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		nDataSize = fwrite(C, 1, usAlignedLen, fpBinaryFile);
		fclose(fpBinaryFile);

		if (nDataSize != usAlignedLen)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : (%d) Can't write data - %s !!!\n", i, strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}

	sync();

	OxCAS_Free(P);
	OxCAS_Free(C);

	if (CI_LoadAuthContexts(&s_stCiAuthCtxs))
	{
		HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't load contexts !!!\n");
		return -1;
	}

	return ERR_OK;
}

int CI_LoadAuthContexts(CI_AUTH_CTXs *pstCiAuthCtxs)
{
	HUINT32			i;
	HUINT8			*pucFileName;
	unsigned short	usOrgLen = sizeof(CI_AUTH_CTX), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char		*C = NULL, *P = NULL;
	unsigned char		D[32];

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	if (pstCiAuthCtxs == NULL)
	{
		HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Invalid params !!!\n");
		return -1;
	}

	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(C);		
		return -1;
	}	

	for(i = 0; i < CI_NUM_OF_AUTH_CTX; i++)
	{
		pucFileName = local_CI_GetAuthFileName(i);
		fpBinaryFile = fopen(pucFileName, "rb");

		if (fpBinaryFile == NULL)
		{
			HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Can't open file - %s !!!\n", strerror(errno));
			OxCAS_Free(C);
			OxCAS_Free(P);
			return -1;
		}

		nDataSize = fread(C, 1, usAlignedLen, fpBinaryFile);
		fclose(fpBinaryFile);

		if (nDataSize != usAlignedLen)
		{
			HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Can't read data - %s !!!\n", strerror(errno));
			OxCAS_Free(C);
			OxCAS_Free(P);
			return -1;
		}

		D_AES_CBC(K, C, P, usAlignedLen, IV);
		HxSTD_memcpy(&pstCiAuthCtxs->ctx[i], P, usOrgLen);

		SHA_256((unsigned char *)&pstCiAuthCtxs->ctx[i], usOrgLen-32, D);
		if (HxSTD_memcmp(pstCiAuthCtxs->ctx[i].ucDigest, D, 32) != 0)
		{
			OxCAS_Free(C);
			OxCAS_Free(P);

			HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Invalid digest !!!\n");
			return -1;
		}
	}

	OxCAS_Free(C);
	OxCAS_Free(P);

	return ERR_OK;
}

// save CI+ authentication contexts data for given slot to non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_SaveAuthContexts(void)
{
	HUINT8			*pucCtxFileName;
	HUINT32			i;
	CI_AUTH_CTX		*pstCtx;
	unsigned short usOrgLen = sizeof(CI_AUTH_CTX), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	for(i = 0; i < CI_NUM_OF_AUTH_CTX; i++)
	{
		pstCtx = &s_stCiAuthCtxs.ctx[i];

		SHA_256((unsigned char *)pstCtx, usOrgLen-32, pstCtx->ucDigest);

		P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
		if(P == NULL)
		{
			HxLOG_Error("malloc failed !!!\n");
			return -1;
		}	
		C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
		if(C == NULL)
		{
			HxLOG_Error("malloc failed !!!\n");
			OxCAS_Free(P);			
			return -1;
		}	

		HxSTD_memcpy(P, pstCtx, usOrgLen);
		OneAndZeroesPadding(P, usOrgLen);
		E_AES_CBC(K, P, C, usAlignedLen, IV);

		pucCtxFileName = local_CI_GetAuthFileName(i);
		fpBinaryFile = fopen(pucCtxFileName, "wb");
		if (fpBinaryFile == NULL)
		{
			HxLOG_Error("[CI+]---> CI_SaveAuthContexts()---> Error : (%d) Can't open file to write - %s !!!\n", i, strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		nDataSize = fwrite(C, 1, usAlignedLen, fpBinaryFile);
		fclose(fpBinaryFile);

		if (nDataSize != usAlignedLen)
		{
			HxLOG_Error("[CI+]---> CI_SaveAuthContexts()---> Error : (%d) Can't write data - %s !!!\n", i, strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		sync();
	}

	OxCAS_Free(P);
	OxCAS_Free(C);

	return ERR_OK;
}

#else

// init CI+ authentication contexts data and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_InitAuthContexts (CI_SLOT_ID usSlotId)
{
	CI_AUTH_CTXs stCiAuthCtxs;

	unsigned short usOrgLen = sizeof(CI_AUTH_CTXs), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	if (usSlotId >= 2)
	{
		HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Invalid params !!!\n");
		return -1;
	}

	HxSTD_memset(&stCiAuthCtxs, 0x00, usOrgLen);
	SHA_256((unsigned char *)&stCiAuthCtxs, usOrgLen-32, stCiAuthCtxs.ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}	

	HxSTD_memcpy(P, &stCiAuthCtxs, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	if (usSlotId == 0)
	{
		fpBinaryFile = fopen(CI_AUTH_CTXS_1ST_FILE, "wb");
		if (fpBinaryFile == NULL)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't open file - %s !!!\n", strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		nDataSize = fwrite(C, 1, usAlignedLen, fpBinaryFile);
		fclose(fpBinaryFile);

		if (nDataSize != usAlignedLen)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't write data - %s !!!\n", strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}
	else
	{
		fpBinaryFile = fopen(CI_AUTH_CTXS_2ND_FILE, "wb");
		if (fpBinaryFile == NULL)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't open file - %s !!!\n", strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}

		nDataSize = fwrite(C, 1, usAlignedLen, fpBinaryFile);
		fclose(fpBinaryFile);

		if (nDataSize != usAlignedLen)
		{
			HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't write data - %s !!!\n", strerror(errno));
			OxCAS_Free(P);
			OxCAS_Free(C);
			return -1;
		}
	}

	sync();

	OxCAS_Free(P);
	OxCAS_Free(C);

	if (CI_LoadAuthContexts(usSlotId, &s_stCiAuthCtxs[usSlotId]))
	{
		HxLOG_Error("[CI+]---> CI_InitAuthContexts()---> Error : Can't load contexts !!!\n");
		return -1;
	}

	return ERR_OK;
}

// load CI+ authentication contexts data for given slot from non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_LoadAuthContexts (CI_SLOT_ID usSlotId, CI_AUTH_CTXs *pstCiAuthCtxs)
{
	unsigned short usOrgLen = sizeof(CI_AUTH_CTXs), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *C = NULL, *P = NULL;
	unsigned char D[32];

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	if ((usSlotId >= 2) || (pstCiAuthCtxs == NULL))
	{
		HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Invalid params !!!\n");
		return -1;
	}

	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(C);		
		return -1;
	}	

	if (usSlotId == 0)
		fpBinaryFile = fopen(CI_AUTH_CTXS_1ST_FILE, "rb");
	else
		fpBinaryFile = fopen(CI_AUTH_CTXS_2ND_FILE, "rb");

	if (fpBinaryFile == NULL)
	{
		HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Can't open file - %s !!!\n", strerror(errno));
		OxCAS_Free(C);
		OxCAS_Free(P);
		return -1;
	}

	nDataSize = fread(C, 1, usAlignedLen, fpBinaryFile);
	fclose(fpBinaryFile);

	if (nDataSize != usAlignedLen)
	{
		HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Can't read data - %s !!!\n", strerror(errno));
		OxCAS_Free(C);
		OxCAS_Free(P);
		return -1;
	}

	D_AES_CBC(K, C, P, usAlignedLen, IV);
	HxSTD_memcpy(pstCiAuthCtxs, P, usOrgLen);

	OxCAS_Free(C);
	OxCAS_Free(P);

	SHA_256((unsigned char *)pstCiAuthCtxs, usOrgLen-32, D);

	if (HxSTD_memcmp(pstCiAuthCtxs->ucDigest, D, 32) != 0)
	{
		HxLOG_Error("[CI+]---> CI_LoadAuthContexts()---> Error : Invalid digest !!!\n");
		return -1;
	}

	return ERR_OK;
}

// save CI+ authentication contexts data for given slot to non-volatile memory and return -1 if there is any error (데이터 무결성 및 보안을 위해 데이터들은 SHA-256 으로 hashing 되고 AES-128-CBC 로 암호화된다)
int CI_SaveAuthContexts (CI_SLOT_ID usSlotId, CI_AUTH_CTXs *pstCiAuthCtxs)
{
	unsigned short usOrgLen = sizeof(CI_AUTH_CTXs), usAlignedLen = GetAlignedLen(usOrgLen);
	unsigned char *P = NULL, *C = NULL;

	FILE *fpBinaryFile = NULL;
	int nDataSize = 0;

	if ((usSlotId >= 2) || (pstCiAuthCtxs == NULL))
	{
		HxLOG_Error("[CI+]---> CI_SaveAuthContexts()---> Error : Invalid params !!!\n");
		return -1;
	}

	SHA_256((unsigned char *)pstCiAuthCtxs, usOrgLen-32, pstCiAuthCtxs->ucDigest);

	P = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(P == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		return -1;
	}	
	C = (unsigned char *)OxCAS_Malloc(usAlignedLen);
	if(C == NULL)
	{
		HxLOG_Error("malloc failed !!!\n");
		OxCAS_Free(P);		
		return -1;
	}	

	HxSTD_memcpy(P, pstCiAuthCtxs, usOrgLen);
	OneAndZeroesPadding(P, usOrgLen);
	E_AES_CBC(K, P, C, usAlignedLen, IV);

	if (usSlotId == 0)
		fpBinaryFile = fopen(CI_AUTH_CTXS_1ST_FILE, "wb");
	else
		fpBinaryFile = fopen(CI_AUTH_CTXS_2ND_FILE, "wb");

	if (fpBinaryFile == NULL)
	{
		HxLOG_Error("[CI+]---> CI_SaveAuthContexts()---> Error : Can't open file to write - %s !!!\n", strerror(errno));
		OxCAS_Free(P);
		OxCAS_Free(C);
		return -1;
	}

	nDataSize = fwrite(C, 1, usAlignedLen, fpBinaryFile);
	fclose(fpBinaryFile);

	if (nDataSize != usAlignedLen)
	{
		HxLOG_Error("[CI+]---> CI_SaveAuthContexts()---> Error : Can't write data - %s !!!\n", strerror(errno));
		OxCAS_Free(P);
		OxCAS_Free(C);
		return -1;
	}

	sync();

	OxCAS_Free(P);
	OxCAS_Free(C);

	return ERR_OK;
}

#endif

// get current UTC time and return -1 if there is any error ("YYYYMMDDHHMMSSZ" 형태의 UTC Time format 스트링을 얻어온다)
int CI_GetCurrentUtcTime (unsigned char szUtcTime[15])
{
	unsigned long	ulCurTime;	// UNIXTIME
	HxDATETIME_t	stDateTime;

	if (VK_CLOCK_GetTime(&ulCurTime))
		return -1;
	if (HLIB_DATETIME_ConvertUnixTimeToDateTime((UNIXTIME)ulCurTime, &stDateTime))
		return -1;

#ifdef CI_PLUS_TEST_Technology_from_SmarDTV /* SmarDTV CI+ 테스트 킷 스트림이 함께 제공된 인증서 날짜 정보와 맞지 않아 테스트용으로 추가한 코드임 */
	stDateTime.stDate.usYear = 2008;
	stDateTime.stDate.ucMonth = 12;
	stDateTime.stDate.ucDay = 25;
	stDateTime.stTime.ucHour = stDateTime.stTime.ucMinute = stDateTime.stTime.ucSecond = 0;
#endif

	snprintf(szUtcTime, 15, "%04d%02d%02d%02d%02d%02dZ",
		stDateTime.stDate.usYear, stDateTime.stDate.ucMonth, stDateTime.stDate.ucDay,
		stDateTime.stTime.ucHour, stDateTime.stTime.ucMinute, stDateTime.stTime.ucSecond);

	return ERR_OK;
}

// compare UTC Times : return 1 if t1 > t2, 0 if t1 = t2, -1 if t1 < t2
int CI_CompareUtcTimes (unsigned char szUtcTime1[15], unsigned char szUtcTime2[15])
{
	int cmp;

	cmp = HxSTD_memcmp(szUtcTime1, szUtcTime2, 15); // CI+ 스펙에 따르면 "YYMMDDHHMMSSZ" -> The year field shall be interpreted as 20YY... 라고 하니까 이렇게 단순 비교해도 괜찮을 듯은 한데...

	if (cmp > 0)
		return 1;

	if (cmp < 0)
		return -1;

	return 0;
}

// set descrambler key (CCKey) to descrambler and return -1 if there is any error
// when enKeyCipher is CI_CC_KEY_CIPHER_DES_56_ECB  : CCK is 64 bits, CIV is NULL
// when enKeyCipher is CI_CC_KEY_CIPHER_AES_128_CBC : CCK is 128 bits, CIV is 128 bits
int CI_SetCckToDsc (CI_SLOT_ID usSlotId, CI_CC_KEY_CIPHER enKeyCipher, CI_CC_KEY_MODE enKeyMode, unsigned char *pucCck, unsigned char *pucCiv)
{
	CI_NOTIFY_SET_KEY_FUNC 	pfnNotifySetKey;
	CIPLUS_KeyInfo_t		stKeyInfo;

	stKeyInfo.enKeyCipher = enKeyCipher;
	stKeyInfo.enKeyMode = enKeyMode;
	stKeyInfo.pucCck = pucCck;
	stKeyInfo.pucCiv = pucCiv;

	HxLOG_Info("CI_SetCckToDsc set key \n");

	Callback_Reset(g_hCbNotifySetKey);
	while ((pfnNotifySetKey = (CI_NOTIFY_SET_KEY_FUNC)Callback_GetNextFunc(g_hCbNotifySetKey)) != NULL)
	{
		HxLOG_Info("CI_SetCckToDsc set key call CB\n");
		if ((*pfnNotifySetKey)(usSlotId, &stKeyInfo) != ERR_OK)
			return -1;
	}

	return ERR_OK;
}

// apply URI to external interface and return -1 if there is any error
// - ucMode         : see CI_CC_URI_MODE_
// - ucAps          : see CI_CC_URI_APS_
// - ucEmi          : see CI_CC_URI_EMI_
// - ucIct          : see CI_CC_URI_ICT_
// - ucRct          : see CI_CC_URI_RCT_
// - ucDot			: see CI_CC_URI_DOT_
// - ulMinutesForRL : minutes for RL (0 means unlimited retention)
int CI_ApplyUriToExternalOutput (	CI_SLOT_ID usSlotId, CIPLUS_UriInfo_t stUriInfo)
{
	CI_NOTIFY_URI_CONTROL_FUNC 	pfnNotifyUriControl;

	Callback_Reset(g_hCbNotifyUriControl);
	while ((pfnNotifyUriControl = (CI_NOTIFY_URI_CONTROL_FUNC)Callback_GetNextFunc(g_hCbNotifyUriControl)) != NULL)
	{
		if ((*pfnNotifyUriControl)(usSlotId, &stUriInfo) != ERR_OK)
			return -1;
	}

	return ERR_OK;
}

// apply SRM data and return -1 if there is any error
int CI_ApplySrmData (CI_SLOT_ID usSlotId, unsigned char *pucSrmData, unsigned short usSrmDataLen)
{
	CI_NOTIFY_SRM_CONTROL_FUNC 	pfnNotifySrmControl;

	Callback_Reset(g_hCbNotifySrmControl);
	while ((pfnNotifySrmControl = (CI_NOTIFY_SRM_CONTROL_FUNC)Callback_GetNextFunc(g_hCbNotifySrmControl)) != NULL)
	{
		if ((*pfnNotifySrmControl)(usSlotId, pucSrmData, usSrmDataLen) != ERR_OK)
			return -1;
	}

	return ERR_OK;
}

// set plus mode for given slot (Host Service Shunning 을 위한 SI/SVC 연동 함수)
int CI_SetPlusMode (CI_SLOT_ID usSlotId, CI_PLUS_MODE enPlusMode)
{
	if (usSlotId >= PAL_CI_GetNumberOfModule())
	{
		HxLOG_Error("[CI+]---> CI_SetPlusMode()---> Error : Invalid usSlotId(%d) !!!\n", usSlotId);
		return -1;
	}

	s_enPlusMode[usSlotId] = enPlusMode;

	return ERR_OK;
}

// get plus mode for given slot (Host Service Shunning 을 위한 SI/SVC 연동 함수)
int CI_GetPlusMode (CI_SLOT_ID usSlotId, CI_PLUS_MODE *penPlusMode)
{
	if (usSlotId >= PAL_CI_GetNumberOfModule())
	{
		HxLOG_Error("[CI+]---> CI_GetPlusMode()---> Error : Invalid usSlotId(%d) !!!\n", usSlotId);
		return -1;
	}

	if (penPlusMode == NULL)
	{
		HxLOG_Error("[CI+]---> CI_GetPlusMode()---> Error : Invalid params !!!\n");
		return -1;
	}

	*penPlusMode = s_enPlusMode[usSlotId];

	return ERR_OK;
}

// set host mode
void CI_SetHostMode (CI_HOST_MODE enHostMode)
{
	s_enHostMode = enHostMode;
}

// get host mode
CI_HOST_MODE CI_GetHostMode (void)
{
	return s_enHostMode;
}

// get host country code as ISO 3166-1 alpha 3 (에러 처리는 자체적으로 핸들링 하기 때문에 항시 ERR_OK 리턴함)
int CI_GetCountryCode (unsigned char *pucCode)
{
	/* The country code is a 24-bit field that identifies the host country using 3 uppercase characters as specified by ISO 3166-1 alpha 3.
	   Each character is coded as 8-bits according to ISO 8859-1. */

	HUINT8 szIso3166Code[4];
	HERROR err;
	HxCountry_e	eCountryId;

	if (pucCode == NULL)
	{
		HxLOG_Error("[CI+]---> CI_GetCountryCode()---> Error : Invalid params !!!\n");
		return -1;
	}

//	eCountryId = MWC_PARAM_GetCountryId();
	err = DB_PARAM_GetItem(eDB_PARAM_ITEM_COUNTRY_ID, (HUINT32 *)&eCountryId, 0);
	if (ERR_OK != err)
	{
		HxLOG_Warning("DB_PARAM_GetItem(eDB_PARAM_ITEM_COUNTRY_ID) Error !!\n");
	}
	if(FALSE == HLIB_COUNTRY_IsValidCountryId(eCountryId))
	{
		HxLOG_Error("[CI+]---> CI_GetCountryCode()---> Error : DB_PARAM_GetItem(eDB_PARAM_ITEM_COUNTRY_ID)-(0x%X) !!!\n", eCountryId);
		eCountryId = eCountryID_GBR;
	}

#if 1
	{
		const HCHAR *pszIsoCountryCode = NULL;
		pszIsoCountryCode = HLIB_COUNTRY_IdTo3166_Alpha3(eCountryId);
		if (pszIsoCountryCode)
		{
			HxSTD_MemCopy(szIso3166Code, pszIsoCountryCode, 4);
			err = ERR_OK;
		}
		else
		{
			err = ERR_FAIL;
		}
	}
#else
	err = MWC_UTIL_GetIso3166CodeByCountryId((HxCountry_e)eCountryId, szIso3166Code);
#endif
	if (err != ERR_OK)
	{
		HxLOG_Error("[CI+]---> CI_GetCountryCode()---> Error : MWC_UTIL_GetIso3166CodeByCountryId (%d) !!!\n", err);
		szIso3166Code[0] = 'G';
		szIso3166Code[1] = 'B';
		szIso3166Code[2] = 'R';
	}

	pucCode[0] = szIso3166Code[0];
	pucCode[1] = szIso3166Code[1];
	pucCode[2] = szIso3166Code[2];

	HxLOG_Print("host country setting is %c%c%c\n", pucCode[0], pucCode[1], pucCode[2]);

	return ERR_OK;
}

// get host language code as ISO 639 Part 2 (에러 처리는 자체적으로 핸들링 하기 때문에 항시 ERR_OK 리턴함)
int CI_GetLanguageCode (unsigned char *pucCode)
{
	/*  The language code is a 24-bit field that identifies the language using 3 lowercase characters as specified by ISO 639 Part 2.
	    Each character is coded as 8-bits according to ISO 8859-1. */

	HINT32 nValue;
	HUINT8 szIso639Code1[4];
	HUINT8 szIso639Code2[4];
	HUINT8 szIso639Code3[4];
	HINT32 lMatchCount = 0;
	HERROR err;

	if (pucCode == NULL)
	{
		HxLOG_Error("[CI+]---> CI_GetLanguageCode()---> Error : Invalid params !!!\n");
		return -1;
	}

	err = DB_PARAM_GetItem(eDB_PARAM_ITEM_LANG_MAIN, (HUINT32 *)&nValue, 0);
	if (err != ERR_OK)
	{
		HxLOG_Warning("DB_PARAM_GetItem(eDB_PARAM_ITEM_LANG_MAIN) Error !!\n");
	}
	err = MWC_UTIL_Get639CodeByLangIdx((HxLANG_Id_e)nValue, &lMatchCount, szIso639Code1, szIso639Code2, szIso639Code3);
	if (err != ERR_OK)
	{
		HxLOG_Error("[CI+]---> CI_GetLanguageCode()---> Error : MWC_UTIL_Get639CodeByLangIdx (%d) !!!\n", nValue);
		szIso639Code1[0] = 'e';
		szIso639Code1[1] = 'n';
		szIso639Code1[2] = 'g';
	}

	pucCode[0] = szIso639Code1[0];
	pucCode[1] = szIso639Code1[1];
	pucCode[2] = szIso639Code1[2];

	HxLOG_Print("host language setting is %c%c%c\n", pucCode[0], pucCode[1], pucCode[2]);

	return ERR_OK;
}

// get Host ID and return -1 if there is any error (이 경우 ID는 empty 또는 invalid 로 간주)
// 이 함수의 호출은 CiCc_Task() 기동이 완료된 후에야 유효한 결과를 줌에 유의할 것
int CI_GetHostId (unsigned char ucHostId[8])
{
	return CiCc_GetHostId(ucHostId);
}

// get current binded CI+ CAM ID for given slot and return -1 if there is any error (이 경우 ID는 empty 또는 invalid 로 간주)
// 이 함수의 호출은 캠 삽입후 일정 시간 경과된 후에야 유효한 결과를 줌에 유의할 것
int CI_GetBindedCamId (CI_SLOT_ID usSlotId, unsigned char *pucBindedCamId)
{
	return CiCc_GetBindedCamId(usSlotId, pucBindedCamId);
}

// get current binded CI+ CAM Brand ID for given slot and return -1 if there is any error (이 경우 ID는 empty 또는 invalid 로 간주)
// BindedCamBrandId 값이 0 으로 리턴되는 경우 BindedCamBrandId 가 없는 것을 의미한다.
// 이 함수의 호출은 캠 삽입후 일정 시간 경과된 후에야 유효한 결과를 줌에 유의할 것
int CI_GetBindedCamBrandId (CI_SLOT_ID usSlotId, unsigned short *pusBindedCamBrandId)
{
	// SMiT : 14399 (0x383F), Neotion : 23308 (0x5B0C), SmarDTV : 2898 (0xB52)
	return CiCc_GetBindedCamBrandId(usSlotId, pusBindedCamBrandId);
}

// get negotiated scrambling cipher for given slot to set descrambler mode when descrambler open and return -1 if there is any error
// 이 함수의 호출은 캠 삽입후 일정 시간 경과된 후에야 유효한 결과를 줌에 유의할 것
int CI_GetNegotiatedCckCipher (CI_SLOT_ID usSlotId, CI_CC_KEY_CIPHER *penNegotiatedCckCipher)
{
	return CiCc_GetNegotiatedCckCipher(usSlotId, penNegotiatedCckCipher);
}

// request CC to set descrambler key (CCKey) to descrambler
// 이 함수의 호출은 캠 삽입후 일정 시간 경과된 후에야 유효한 결과를 줌에 유의할 것 -> 더이상 사용되지 않으나 추후 필요한 용도가 있을 수도 있어 일단 남겨두도록 한다.
void CI_ReqSetCckToDsc (CI_SLOT_ID usSlotId)
{
	CiCc_ReqSetCckToDsc(usSlotId);
}

// set URI default values and URI timer for given slot to do control external interface when zapping
// 이 함수의 호출은 캠 삽입후 일정 시간 경과된 후에야 유효한 결과를 줌에 유의할 것
void CI_SetUriDefault (CI_SLOT_ID usSlotId, HBOOL bOn)
{
	CiCc_SetUriDefault(usSlotId, bOn);
}

// check CI+ CAM is inserted for given slot : return positive value if CI+ CAM is inserted, 0 if no CAM or normal CAM is inserted
// 해당 슬롯의 캠이 CI+ 캠인지 일반 캠인지를 확인하여 리턴한다 (CI+ v1.2 호환 캠인 경우 1, CI+ v1.3 호환 캠인 경우 2, 일반 캠인 경우 0, USE_CIS_INFORMATION 가 사용된 경우 캠이 없으면 -1)
int CI_IsCiPlusCam (CI_SLOT_ID usSlotId)
{
	int nCiPlusVer;
	// CC resource가 열려 있는지 확인
	if (CiCc_IsOpened(usSlotId))
	{
		// CC resource가 열려 있고, CIS에 CI+ 정보가 있는 경우 CI+ 정보의 값을 return 함.
		// CI+ 정보 값이 1일 때: CI+ v1.2 지원을 의미
		// CI+ 정보 값이 2일 때: CI+ v1.3 지원을 의미
		nCiPlusVer = PHYS_IsCiPlusCam(usSlotId);

		if(nCiPlusVer > 0)
			return nCiPlusVer;
		else if(nCiPlusVer < 0)	// CAM이 삽입되어 있지 않은 경우
			return -1;
		else	// CC resource는 열려 있으나, CIS 에 CI+ 정보가 없는 경우 CI+ v1.2를 지원 한다고 인지.
			return 1;
	}
	else
	{
		nCiPlusVer = PHYS_IsCiPlusCam(usSlotId);

		if(nCiPlusVer > 0)	// cc resource는 열려있지 않으나, CIS에 CI+ 정보가 있는 경우 CIS 정보를 우선시 하여 CI+ 지원 CAM임을 판단.
			return nCiPlusVer;
		else if(nCiPlusVer < 0) // CAM이 삽입되어 있지 않은 경우
			return -1;
	}

	// cc resource가 열려있지 않고, CIS에 CI+ 정보도 없는 경우 CI+ CAM이 아니라고 판단 함.
	return 0;
}

// check whether operator profile is supported with the CAM for given slot : return 1 if supported, 0 if not supported, -1 if no CAM is inserted
HBOOL CI_OperatorProfileSupported (CI_SLOT_ID usSlotId)
{
	HBOOL bOpSupportByCis = PHYS_OperatorProfileSupported(usSlotId);

	if( bOpSupportByCis == -1)
	{
		HxLOG_Critical("Sequence Error!! Check it!!!\n");	// 여기를 타면 안될 것임. 이 메시지가 찍히면 원인을 꼭 잡도록.
		return FALSE;
	}

	return bOpSupportByCis;
}

#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
HULONG CI_GetCcResVersion(CI_SLOT_ID usSlotId)
{
	return CiCc_GetCcResourceVersionBySlot(usSlotId);
}

// signals the start of a CA protected service recording to the CICAM and return -1 if there is any error (CI+ v1.3)
// -> CA protected service 의 TSR 또는 레코딩시 사용 (CI mgr 에서 CI+ descrambling 시 호출)
// 이 경우 enOperatingMode 는 CI_OPERATING_MODE_WATCH_AND_BUFFER 또는 CI_OPERATING_MODE_UNATTENDED_RECORDING 이어야 한다.
// pucPinCode 는 option 이고 pucPinCode 를 전달하지 않는 경우 pucPinCode 는 NULL, unPinCodeLen 는 0 이어야 한다.
int CI_RecordStart (CI_SLOT_ID usSlotId, Handle_t hAction, CI_OPERATING_MODE eOperatingMode, unsigned short usProgramNumber)
{
	return CiCc_PrepareRecordStart(usSlotId, hAction, eOperatingMode, usProgramNumber);
}

// changes the operating mode and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 호출 이후에 모드 전환시 사용 (예: CI_OPERATING_MODE_WATCH_AND_BUFFER -> CI_OPERATING_MODE_TIMESHIFT)
int CI_ChangeOperatingMode (CI_SLOT_ID usSlotId, unsigned int unSessionId, CI_OPERATING_MODE enOperatingMode, unsigned short usProgramNumber)
{
	return CiCc_ChangeOperatingMode(usSlotId, unSessionId, enOperatingMode, usProgramNumber);
}

// inform the CICAM that the recording has stopped and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 호출 이후에 TSR 또는 레코딩 중지시 사용
int CI_RecordStop (CI_SLOT_ID usSlotId, unsigned short usProgramNumber)
{
	return CiCc_RecordStop(usSlotId, usProgramNumber);
}

int CI_GetRecordingStatus(CI_SLOT_ID usSlotId, HUINT8 *pucCcRecordingStatus)
{
	return CiCc_GetRecordingStatus(usSlotId, pucCcRecordingStatus);
}

// signals the start of a CA protected service playback to the CICAM and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 로 녹화된 서비스의 playback 시 사용 (content license 를 받은 경우에 한해)
// 주의 사항 :
// The license exchange on playback protocol is performed in parallel while playing the content to ensure that playback start is not delayed.
// If the Playback License response is not OK, or the response takes longer than 10 seconds the play back shall be stopped. <--- TODO: 작업 필요 (kkkim)
/*
int CI_Playback (CI_SLOT_ID usSlotId, unsigned int unSessionId, unsigned short usProgramNumber, unsigned char *pucLicenseData, unsigned int unLicenseDataLen)
{
	return CiCc_Playback(usSlotId, unSessionId, usProgramNumber, pucLicenseData, unLicenseDataLen);
}
*/
// interrogate the CICAM about the current status of a content license associated with a recording and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 로 녹화된 서비스의 license status 및 남아 있는 play count 정보를 얻기 위해 사용 (content license 를 받은 경우에 한해)
int CI_CheckLicense (CI_SLOT_ID usSlotId, unsigned int unSessionId, unsigned char *pucLicenseData, unsigned int unLicenseDataLen)
{
	return CiCc_CheckLicense(usSlotId, unSessionId, pucLicenseData, unLicenseDataLen);
}

// get PIN capability info for given slot and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 호출 직전에 사용
int CI_GetPinCapaInfo (CI_SLOT_ID usSlotId, CI_PIN_CAPA_INFO *pstPinCapaInfo)
{
	return CiCc_GetPinCapaInfo(usSlotId, pstPinCapaInfo);
}

int CI_StoreCicamPinInfo (CI_SLOT_ID usSlotId, HUINT8 *pucPinInfo, UNIXTIME utPinSavedTime)
{
	return CiCc_StoreCicamPinInfo(usSlotId, pucPinInfo, utPinSavedTime);
}

int CI_GetUriInfo(CI_SLOT_ID usSlotId, CIPLUS_UriInfo_t *pstUriInfo)
{
	return CiCc_GetUriInfo(usSlotId, pstUriInfo);
}

int CI_PinPlayback (CI_SLOT_ID usSlotId, HUINT32 unMetaIndex, Handle_t hPbAction, HUINT8 ucAgeLimit, HUINT8 *pucPrivateData)
{
	return CiCc_PinPlayback(usSlotId, unMetaIndex, hPbAction, ucAgeLimit, pucPrivateData);
}

// signals the start of a CA protected service playback to the CICAM and return -1 if there is any error (CI+ v1.3)
// -> CI_RecordStart() 로 녹화된 서비스의 playback 시 사용 (content license 를 받은 경우에 한해)
// 주의 사항 :
// The license exchange on playback protocol is performed in parallel while playing the content to ensure that playback start is not delayed.
// If the Playback License response is not OK, or the response takes longer than 10 seconds the play back shall be stopped. <--- TODO: 작업 필요 (kkkim)
int CI_SendHostLicense (CI_SLOT_ID usSlotId, HUINT32 unMetaIndex, Handle_t hPbAction, HUINT16 usProgramNum, HUINT8 *pucLicenseData, HUINT16 usLicenseLength)
{
	return CiCc_SendHostLicense(usSlotId, unMetaIndex, hPbAction, usProgramNum, pucLicenseData, usLicenseLength);
}

#endif

#if defined(CONFIG_MW_CI_PLUS_HCI_V_2)
// notify host tuning result to CAM and return -1 if there is any error (CI+ v1.3)
// -> pfnNotifyHostTune 을 받은 후 그 결과 리턴시에 사용
int CI_TuneReply (CI_SLOT_ID usSlotId, HUINT32 ulTargetResource, CI_TUNE_STATUS eTuneResult)
{
	int	nReturn = ERR_OK;
	switch (ulTargetResource)
	{
		case CI_HCI_CLASS:
			nReturn = CiHci_TuneReply(usSlotId, eTuneResult);
			break;
		default:
			break;
	}
	return nReturn;
}
#endif

// notify host country setting is changed to CAM and return -1 if there is any error
int CI_NotifyHostCountrySettingChangedToCam (CI_SLOT_ID usSlotId)
{
	return CiHlc_NotifyHostCountryInfoChanged(usSlotId);
}

// notify host language setting is changed to CAM and return -1 if there is any error
int CI_NotifyHostLanguageSettingChangedToCam (CI_SLOT_ID usSlotId)
{
	return CiHlc_NotifyHostLanguageInfoChanged(usSlotId);
}

#ifdef CONFIG_OP_CANALREADY

// get the information of canal ready init_notify() and return -1 if there is any error
int CI_CanalReadyGetInitInfo (CI_SLOT_ID usSlotId, SvcCas_CiAuthInfo_t *pstCiAuthInfo)
{
	return CiAuth_CanalReadyGetInitInfo(usSlotId, pstCiAuthInfo);
}

// reply to canal ready morality_level_notify() and return -1 if there is any error
int CI_CanalReadyMoralityLevelUnlock (CI_SLOT_ID usSlotId)
{
	return CiAuth_CanalReadyMoralityLevelUnlock(usSlotId);
}

#endif

int CI_OperatorPrepareStatus(CI_SLOT_ID usSlotId, HBOOL bSearchAvailable)
{
	if(bSearchAvailable == TRUE)
		return CiOp_SendOperatorSearchStart(usSlotId);
	else
		return CiOp_SendOperatorSearchCancel(usSlotId);
}

int CI_OperatorTuneStatus(CI_SLOT_ID usSlotId, HUINT8 ucStrength, HUINT8 ucQuality, HUINT8 ucStatus, HUINT8 *pucDes, HUINT8 ucDesLen, HUINT8 ucNextDesNum)
{
#if defined(CONFIG_MW_CI_PLUS_V_1_3)
	HERROR hErr = ERR_OK;

	// no signal 인 경우 CICAM으로 APUD를 보내지 말고 next tp 로 tuning 을 시도해야 한다.
	if (ucStatus == 3)	// eCH_Tune_NoSignal
	{
		HxLOG_Error("No Signal ->> Try to tune next tp...\n");
		hErr = CiMgr_GetNextOperatorTune(usSlotId, ucNextDesNum);
		if (hErr == ERR_OK)
		{
			HxLOG_Error("...do nothing...\n");
			return ERR_FAIL;
		}
		HxLOG_Error("...continue...\n");
	}
#endif
	return CiOp_SendOperatorTuneStatus(usSlotId, ucStrength, ucQuality, ucStatus, pucDes, ucDesLen, ucNextDesNum);
}

HBOOL CI_GetOperatorProfileErrStatus (CI_SLOT_ID usSlotId)
{
	return CiOp_GetOperatorProfileErrStatus(usSlotId);
}

int CI_SetOperatorProfileSupportInfo(CI_SLOT_ID usSlotId, HUINT8 ucNitVer, HUINT8 ucInfoVer)
{
	// 이 내용은 ci_op.c 파일에서 직접 authContext를 열어서 관리해도 되나,
	// Operator Profile이 사용된다는 것은 CI+ CAM인 것이고 그렇다면 100% cc resource가 열리므로,
	// authContext를 주로 관리하는 cc resource에서 처리하도록 한다.
	return CiCc_SetOperatorProfileSupportInfo(usSlotId, ucNitVer, ucInfoVer);
}

int CI_GetAuthContextBySlotId(CI_SLOT_ID usSlotId, CI_AUTH_CTX *pstCiAuthCtx)
{
	return CiCc_GetAuthContextBySlotId(usSlotId, pstCiAuthCtx);
}

HBOOL CI_CheckCcAuthenticationComplete(CI_SLOT_ID usSlotId)
{
	return CiCc_CheckCcAuthenticationComplete(usSlotId);
}

HBOOL CI_CheckSimpleEntitlementChange(CI_SLOT_ID usSlotId)
{
	return CiOp_CheckSimpleEntitlementChange(usSlotId);
}

int CI_SendOperatorExitBySlotId(CI_SLOT_ID usSlotId)
{
	return CiOp_SendOperatorExitBySlotId(usSlotId);
}

int CI_SendOperatorStatusReqBySlotId(CI_SLOT_ID usSlotId)
{
	return CiOp_SendOperatorStatusReqBySlotId(usSlotId);
}

int CI_GetOperatorReservedTime(HUINT16 usSlotId, void *pvOut)
{
	UNIXTIME	*putTime;

	if(usSlotId >= PAL_CI_GetNumberOfModule())
	{
		HxLOG_Error("(%d)Slot Id \n", usSlotId);
		return ERR_FAIL;
	}

	putTime	= (UNIXTIME *)pvOut;

	return CiOp_GetOperatorReservedTime(usSlotId, putTime);
}


// for MES : CI+ host license constants 파일에서 호스트 아이디를 얻기 위한 함수임 (MES 프로토콜을 위한 함수임)
int CI_MES_GetHostIdFromLicenseConstDataFile (char* hostId)
{
	CI_HOST_LICENSE_CONST ciHostLicenseConst;
	Certificate_t * pstDevCertData = NULL;

	// CI+ host license constants 파일 로딩
	if (CI_LoadHostLicenseConstants(&ciHostLicenseConst))
	{
		HxLOG_Error("[CI+]---> GetHostIdFromLicenseConstDataFile()---> Error : Can't load CI+ host license constants !!!\n");
		return -1;
	}
	if (ciHostLicenseConst.ucSavedFlag != 1 && ciHostLicenseConst.ucSavedFlag != 2)
	{
		HxLOG_Error("[CI+]---> GetHostIdFromLicenseConstDataFile()---> Error : CI+ host license constants not saved !!!\n");
		return -2;
	}

	// 호스트 디바이스 인증서로부터 호스트 아이디 추출
	pstDevCertData = X509_ParseRawCert(ciHostLicenseConst.Host_DevCert, ciHostLicenseConst.Host_DevCertLen);
	if (pstDevCertData == NULL)
	{
		HxLOG_Error("[CI+]---> GetHostIdFromLicenseConstDataFile()---> Error : Invalid device certificate !!!\n");
		return -3;
	}
	if (FALSE == GetDeviceIDAsAsciiCode(pstDevCertData, hostId))
	{
		HxLOG_Error("[CI+]---> GetHostIdFromLicenseConstDataFile()---> Error : Can't get device id as ascii codes !!!\n");
		X509_FreeCert(pstDevCertData);
		return -4;
	}

	// 위에서 추출한 아이디와 PC 프로그램에서 저장한 아이디가 같은지 크로스 체크 : hostIds cross check (deviceCert <--> ciHostLicenseConst.Host_ID (8 bytes)
	{
		unsigned char ucHostIdBuf[8];
		GetDeviceID(pstDevCertData, ucHostIdBuf);
		if (HxSTD_memcmp(ucHostIdBuf, ciHostLicenseConst.Host_ID, 8))
		{
			HxLOG_Error("[CI+]---> GetHostIdFromLicenseConstDataFile()---> Error : cross check -> not same (deviceCert <-> ciHostLicenseConst.Host_ID) !!!\n");
			X509_FreeCert(pstDevCertData);
			return -5;
		}
	}

	X509_FreeCert(pstDevCertData);
	return ERR_OK;
}

// for MES : If CI+ host license constants data downloading is occured on booting process,
//           then unexpected error could be occured due to timing between CI+ downloading and CI+ initialization tasks.
//           Therefore, we use this callback func to wait CI+ host license constants data downloading task processing.
#if 0
HERROR  CI_MES_LicenseDownCbFunc (HINT32 lResult)
{
	/* CI+ Download 시작 시에 리모콘 입력 disable -> 종료 후에 enable
      (리모콘 입력에 의해 CI+ Download 영향 받으므로 이런 처리를 함)
      callback은 CI+ Download 시작 시, 종료 후 호출 */
	if (lResult == DI_FAC_DOWNLOAD_SIGNAL_CHECKED)
	{
		CARM_KEY_DisableKeyInput();
		return ERR_OK;
	}

    s_bIsLicenseDownTaskEnd = TRUE;
	CARM_KEY_EnableKeyInput();
	return ERR_OK;
}
#endif

void CI_MES_SetLicenseDownCbRegState (HBOOL bFlag)
{
	s_bIsLicenseDownCbRegSuccess = bFlag;
}

HBOOL CI_MES_IsLicenseDownCbRegSuccess (void)
{
	return s_bIsLicenseDownCbRegSuccess;
}

HBOOL CI_MES_IsLicenseDownTaskEnd (void)
{
	return s_bIsLicenseDownTaskEnd;
}

/*-------------------------------------------*/
/*  callback to notify for CI / CAS manager  */
/*-------------------------------------------*/

int CIPLUS_Init(void)
{
	g_hCbNotifyCcReport = Callback_CreateList();
	g_hCbNotifyPinCapaInfo = Callback_CreateList();
	g_hCbNotifySetKey = Callback_CreateList();
	g_hCbNotifyUriControl = Callback_CreateList();
	g_hCbNotifySrmControl = Callback_CreateList();
	g_hCbNotifyHostTune = Callback_CreateList();
	g_hCbNotifyPidReplace = Callback_CreateList();
	g_hCbNotifyCamUpgrade = Callback_CreateList();
#if defined(CONFIG_OP_CANALREADY)
	g_hCbNotifyCanalReadyInit = Callback_CreateList();
	g_hCbNotifyCanalReadyMoralLevel = Callback_CreateList();
	g_hCbNotifyCanalReadyPinReset = Callback_CreateList();
#endif
#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
	g_hCbNotifyCcPinStatus = Callback_CreateList();
	g_hCbNotifyCcCnfData = Callback_CreateList();
	g_hCbNotifyCcLicense = Callback_CreateList();
	g_hCbNotifyCcPinEvent = Callback_CreateList();
#endif
	g_hCbNotifyOperatorPrepareReq = Callback_CreateList();
	g_hCbNotifyOperatorTune = Callback_CreateList();
	g_hCbNotifyOperatorNit = Callback_CreateList();

	return ERR_OK;
}

int CIPLUS_AddNotifyFunc(CIPLUS_NOTIFY_FUNC_LIST notifyList)
{
	if (notifyList.pfnNotifyCcReport != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCcReport, (void *)notifyList.pfnNotifyCcReport) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CcReport !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CcReport notify success...\n");
	}

	if (notifyList.pfnNotifyPinCapaInfo != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyPinCapaInfo, (void *)notifyList.pfnNotifyPinCapaInfo) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : PinCapaInfo !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add PinCapaInfo notify success...\n");
	}

	if (notifyList.pfnNotifySetKey != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifySetKey, (void *)notifyList.pfnNotifySetKey) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : SetKey !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add SetKey notify success...\n");
	}

	if (notifyList.pfnNotifyUriControl != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyUriControl, (void *)notifyList.pfnNotifyUriControl) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : UriControl !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add UriControl notify success...\n");
	}

	if (notifyList.pfnNotifySrmControl != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifySrmControl, (void *)notifyList.pfnNotifySrmControl) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : SrmControl !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add SrmControl notify success...\n");
	}

	if (notifyList.pfnNotifyHostTune != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyHostTune, (void *)notifyList.pfnNotifyHostTune) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : HostTune !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add HostTune notify success...\n");
	}

	if (notifyList.pfnNotifyPidReplace != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyPidReplace, (void *)notifyList.pfnNotifyPidReplace) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : PidReplace !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add PidReplace notify success...\n");
	}

	if (notifyList.pfnNotifyCamUpgrade != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCamUpgrade, (void *)notifyList.pfnNotifyCamUpgrade) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CamUpgrade !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CamUpgrade notify success...\n");
	}

#if defined(CONFIG_OP_CANALREADY)
	if (notifyList.pfnNotifyCanalReadyInit != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCanalReadyInit, (void *)notifyList.pfnNotifyCanalReadyInit) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CanalReady Init !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CanalReady_Init notify success...\n");
	}

	if (notifyList.pfnNotifyCanalReadyMoralLevel != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCanalReadyMoralLevel, (void *)notifyList.pfnNotifyCanalReadyMoralLevel) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CanalReady Moral Level !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CanalReady_MoralLevel notify success...\n");
	}

	if (notifyList.pfnNotifyCanalReadyPinReset != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCanalReadyPinReset, (void *)notifyList.pfnNotifyCanalReadyPinReset) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CanalReady PinReset !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CanalReady_PinReset notify success...\n");
	}
#endif
#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
	if (notifyList.pfnNotifyCcPinStatus != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCcPinStatus, (void *)notifyList.pfnNotifyCcPinStatus) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CC PIN status !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CC PIN status notify success...\n");
	}
	if (notifyList.pfnNotifyCcCnfData != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCcCnfData, (void *)notifyList.pfnNotifyCcCnfData) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : Confirmation Data !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add Confirmation Data notify success...\n");
	}
	if (notifyList.pfnNotifyCcLicense != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCcLicense, (void *)notifyList.pfnNotifyCcLicense) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CICAM License !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CICAM License notify success...\n");
	}
	if (notifyList.pfnNotifyCcPinEvent != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyCcPinEvent, (void *)notifyList.pfnNotifyCcPinEvent) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : CC Pin Event !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add CC Pin Event notify success...\n");
	}
#endif

	if (notifyList.pfnNotifyOperatorPrepareReq != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyOperatorPrepareReq, (void *)notifyList.pfnNotifyOperatorPrepareReq) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : Operator Tune Ready !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add Operator Tune Ready notify success...\n");
	}

	if (notifyList.pfnNotifyOperatorTune != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyOperatorTune, (void *)notifyList.pfnNotifyOperatorTune) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : Operator Tune Info !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add Operator Tune Info notify success...\n");
	}

	if (notifyList.pfnNotifyOperatorNit != NULL)
	{
		if (Callback_AddFunc(g_hCbNotifyOperatorNit, (void *)notifyList.pfnNotifyOperatorNit) == NULL)
		{
			HxLOG_Error("[CI+]---> CIPLUS_AddNotifyFunc()---> Error : Operator CICAM NIT !!!\n");
			return -1;
		}

		HxLOG_Print("[CI+]---> CIPLUS_AddNotifyFunc()---> add Operator CICAM NIT notify success...\n");
	}

	return ERR_OK;
}

int CIPLUS_DeleteNotifyFunc(CIPLUS_NOTIFY_FUNC_LIST notifyList)
{
	if (notifyList.pfnNotifyCcReport != NULL)
		Callback_RemoveFunc(g_hCbNotifyCcReport, (void *)notifyList.pfnNotifyCcReport);

	if (notifyList.pfnNotifyPinCapaInfo != NULL)
		Callback_RemoveFunc(g_hCbNotifyPinCapaInfo, (void *)notifyList.pfnNotifyPinCapaInfo);

	if (notifyList.pfnNotifySetKey != NULL)
		Callback_RemoveFunc(g_hCbNotifySetKey, (void *)notifyList.pfnNotifySetKey);

	if (notifyList.pfnNotifyUriControl != NULL)
		Callback_RemoveFunc(g_hCbNotifyUriControl, (void *)notifyList.pfnNotifyUriControl);

	if (notifyList.pfnNotifySrmControl != NULL)
		Callback_RemoveFunc(g_hCbNotifySrmControl, (void *)notifyList.pfnNotifySrmControl);

	if (notifyList.pfnNotifyHostTune != NULL)
		Callback_RemoveFunc(g_hCbNotifyHostTune, (void *)notifyList.pfnNotifyHostTune);

	if (notifyList.pfnNotifyPidReplace != NULL)
		Callback_RemoveFunc(g_hCbNotifyPidReplace, (void *)notifyList.pfnNotifyPidReplace);

	if (notifyList.pfnNotifyCamUpgrade != NULL)
		Callback_RemoveFunc(g_hCbNotifyCamUpgrade, (void *)notifyList.pfnNotifyCamUpgrade);

#if defined(CONFIG_OP_CANALREADY)
	if (notifyList.pfnNotifyCanalReadyInit != NULL)
		Callback_RemoveFunc(g_hCbNotifyCanalReadyInit, (void *)notifyList.pfnNotifyCanalReadyInit);

	if (notifyList.pfnNotifyCanalReadyMoralLevel != NULL)
		Callback_RemoveFunc(g_hCbNotifyCanalReadyMoralLevel, (void *)notifyList.pfnNotifyCanalReadyMoralLevel);

	if (notifyList.pfnNotifyCanalReadyPinReset != NULL)
		Callback_RemoveFunc(g_hCbNotifyCanalReadyPinReset, (void *)notifyList.pfnNotifyCanalReadyPinReset);
#endif
#if defined(CONFIG_MW_CI_PLUS_CC_V_2)
	if (notifyList.pfnNotifyCcPinStatus != NULL)
		Callback_RemoveFunc(g_hCbNotifyCcPinStatus, (void *)notifyList.pfnNotifyCcPinStatus);
	if (notifyList.pfnNotifyCcCnfData != NULL)
		Callback_RemoveFunc(g_hCbNotifyCcCnfData, (void *)notifyList.pfnNotifyCcCnfData);
	if (notifyList.pfnNotifyCcLicense != NULL)
		Callback_RemoveFunc(g_hCbNotifyCcLicense, (void *)notifyList.pfnNotifyCcLicense);
	if (notifyList.pfnNotifyCcPinEvent != NULL)
		Callback_RemoveFunc(g_hCbNotifyCcPinEvent, (void *)notifyList.pfnNotifyCcPinEvent);
#endif

	if (notifyList.pfnNotifyOperatorPrepareReq != NULL)
		Callback_RemoveFunc(g_hCbNotifyOperatorPrepareReq, (void *)notifyList.pfnNotifyOperatorPrepareReq);
	if (notifyList.pfnNotifyOperatorTune != NULL)
		Callback_RemoveFunc(g_hCbNotifyOperatorTune, (void *)notifyList.pfnNotifyOperatorTune);
	if (notifyList.pfnNotifyOperatorNit != NULL)
		Callback_RemoveFunc(g_hCbNotifyOperatorNit , (void *)notifyList.pfnNotifyOperatorNit);

	return ERR_OK;
}

#if defined(CONFIG_DEBUG)
static BOOL local_CI_IsNumber(unsigned char ch)
{
	// 상수 범위 ->  - 48, 문자 범위 (A~F) -> - 55
	// '0' ~ '9'
	if ('0' <= ch && ch <= '9')
	{
		return TRUE;
	}
	return FALSE;
}

static BOOL local_CI_PrintCertificatePeriod(Certificate_t *cert)
{
	unsigned char szBeforeTime[16], szAfterTime[16];
	int iTimeCompareResult, i;

	if(cert == NULL)
	{
		return FALSE;
	}

	// YYMMDDHHMMSSZ format (length 13)
	if (13 != cert->tbsCertificate.validity.notBefore.choice.utcTime.size
		|| 13 != cert->tbsCertificate.validity.notAfter.choice.utcTime.size)
	{
		HxLOG_Error("Error : Invalid length !!!\n");
		return FALSE;
	}

	// and should be numbers
	for (i = 0; i < 12; ++i)
	{
		if (FALSE == local_CI_IsNumber(cert->tbsCertificate.validity.notBefore.choice.utcTime.buf[i]))
		{
			HxLOG_Error("Error : Invalid number !!!\n");
			return FALSE;
		}
		if (FALSE == local_CI_IsNumber(cert->tbsCertificate.validity.notAfter.choice.utcTime.buf[i]))
		{
			HxLOG_Error("Error : Invalid number !!!\n");
			return FALSE;
		}
	}
	if ('Z' != cert->tbsCertificate.validity.notBefore.choice.utcTime.buf[12]
		|| 'Z' != cert->tbsCertificate.validity.notAfter.choice.utcTime.buf[12])
	{
		HxLOG_Error("Error : Invalid format !!!\n");
		return FALSE;
	}

	HxSTD_memset(szBeforeTime, 0x00, 16);
	HxSTD_memset(szAfterTime, 0x00, 16);


	// get certificate time value
	szBeforeTime[0] = '2';
	szBeforeTime[1] = '0';
	HxSTD_memcpy(&szBeforeTime[2], cert->tbsCertificate.validity.notBefore.choice.utcTime.buf, 13);

	szAfterTime[0] = '2';
	szAfterTime[1] = '0';
	HxSTD_memcpy(&szAfterTime[2], cert->tbsCertificate.validity.notAfter.choice.utcTime.buf, 13);

	// If afterTime < beforeTime -> we consider afterTime is 21YY... (Not specificated, our opinion)
	iTimeCompareResult = CI_CompareUtcTimes(szBeforeTime, szAfterTime);
	if (iTimeCompareResult == 1)
	{
		szAfterTime[0] = '2';
		szAfterTime[1] = '1';
	}

	HxLOG_Print("BeforeTime (%s), AfterTime (%s)\n", szBeforeTime, szAfterTime);

	return TRUE;
}

static void local_CI_PrintCertificate(CI_HOST_LICENSE_CONST *pstCiHostLicenseConst)
{
	Certificate_t * pstRootCertData = NULL;
	Certificate_t * pstCamBrandCertData = NULL;
	Certificate_t * pstCamDevCertData = NULL;

	HxLOG_Print("[CI+]------------------------------------\n");

	pstRootCertData = X509_ParseRawCert(pstCiHostLicenseConst->RootCert, pstCiHostLicenseConst->RootCertLen);
	if (pstRootCertData == NULL)
	{
		HxLOG_Error("[CI+]---> X509_ParseRawCert()---> Error : Invalid RootCert !!!\n");
	}
	else
	{
		HxLOG_Print("[Root Cert]--\n");
		local_CI_PrintCertificatePeriod(pstRootCertData);
	}

	pstCamBrandCertData = X509_ParseRawCert(pstCiHostLicenseConst->Host_BrandCert, pstCiHostLicenseConst->Host_BrandCertLen);
	if (pstCamBrandCertData == NULL)
	{
		HxLOG_Error("[CI+]---> X509_ParseRawCert()---> Error : Invalid RootCert !!!\n");
	}
	else
	{
		HxLOG_Print("[Brand Cert]--\n");
		local_CI_PrintCertificatePeriod(pstCamBrandCertData);
	}

	pstCamDevCertData = X509_ParseRawCert(pstCiHostLicenseConst->Host_DevCert, pstCiHostLicenseConst->Host_DevCertLen);
	if (pstCamDevCertData == NULL)
	{
		HxLOG_Error("[CI+]---> X509_ParseRawCert()---> Error : Invalid RootCert !!!\n");
	}
	else
	{
		HxLOG_Print("[Dev Cert]--\n");
		local_CI_PrintCertificatePeriod(pstCamDevCertData);
	}
	HxLOG_Print("[CI+]------------------------------------\n");

	if (pstRootCertData)
			X509_FreeCert(pstRootCertData);
	if (pstCamBrandCertData)
			X509_FreeCert(pstCamBrandCertData);
	if (pstCamDevCertData)
			X509_FreeCert(pstCamDevCertData);
}


#endif

