...
lc_result LC_ReadLoaderPartition(lc_loader_pt_st *pLoaderPt)
{
    ...
    if (LC_SUCCESS == result)
    {
		...
		//dbgdump("read ld pt", mp_byte.bytes, mp_byte.length);
    }
    ...
    if (LC_SUCCESS == result)
    {
#if defined(V5_DEV_LOADER_PT_NON_CRYPT)
		LC_memcpy(mp_byte_clear.bytes, mp_byte.bytes, mp_byte.length);
#else
    ...
    if (LC_SUCCESS == result)
    {
        ...
        if (LC_SUCCESS == result)
        {
            ...
            {
				dbgerr2("LoaderPT CRC NOT matched [%08x], [%08x]\n", crc32, pLoaderPt->downloadInfo.crc32);
                result = LC_ERROR_INVALID_DATA;
            }
			else
			{
				dbgerr0("LoaderPT CRC matched ok.\n");
			}
        }
    }
...
lc_result LC_ReadSharedMemory(lc_shared_memory_st *pSharedMemory)
{
    ...
    {
#if defined(V5_DEV_LOADER_PT_NON_CRYPT)
		LC_memcpy(mp_byte_clear.bytes, mp_byte.bytes, mp_byte_clear.length);
#else
        result = LC_CryptModule(LC_FALSE, mp_byte_clear.bytes, mp_byte.bytes, mp_byte_clear.length);
#endif
    }
...
lc_result LC_WriteSharedMemory(lc_bool createNew, lc_shared_memory_st* pSharedMemory)
{
    ...
    if (LC_SUCCESS == result)
    {
        ...
        if (LC_TRUE == createNew)
        {
            ...
            mih_st.header_length = SHORT_MODULE_INFO_HEADER_SIZE;
            mih_st.module_size = LDR_SHARED_MEM_SIZE
                                + LDR_AREA_DOWNLOAD_INFO_SIZE;
    ...
    if (LC_SUCCESS == result)
    {
#ifdef V5_DEV_LOADER_PT_NON_CRYPT
	LC_memcpy(&module_byte.bytes[mih_st.header_length], mp_byte_clear.bytes, mih_st.module_size);
#else
        result = LC_CryptModule(
                    LC_TRUE,
...
dbgtrc;
    ...
    if (LC_SUCCESS == result)
    {
        ...
        if (LC_SUCCESS == result)
        {
			dbgprt2("mih_st.header_length %d, mh_byte.length %d\n", mih_st.header_length , mh_byte.length);
...
static lc_bool IsMulticastAddress(lc_uint32 multicastGroupAddress)
{
    /*Get the first byte of address. */
    lc_bool result = LC_TRUE;
    const lc_uchar firstByte = (lc_uchar)(multicastGroupAddress >> 24); /* host byte order */

    /* Multicast IP Address (IPv4):
     *     224.0.0.0 ~ 239.255.255.255
     */
    if(( firstByte < (lc_uchar)224 ) ||
       ( firstByte > (lc_uchar)239 ))
    {
        result = LC_FALSE;
    }
...
lc_result LC_VerifyIPParameters( const lc_ip_dl_info_st *pIpParameters )
{
    ...
    if (LC_TRUE != IsMulticastAddress(pIpParameters->address))
    {
        result = LC_ERROR_INVALID_PARAMETER;
    }
...
lc_result LC_SetDownloadFlag( lc_bool enableDl, lc_shared_memory_st *pSharedMemory )
{
	...
	dbgwarn0("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	dbgwarn1("!!!! LC_SetDownloadFlag, enableDl [%x]\n", enableDl);
    ...
    if (LC_SUCCESS == result)
    {
        ...
        if (LC_TRUE == enableDl)
        {
			...
			dbgwarn1("!!!! enableDl:TRUE ==> Set pSharedMemory->downloadIndicator = [%x]\n", pSharedMemory->downloadIndicator);
        }
    }
...
lc_result LC_SetDownloadType( lc_uchar dlType, lc_shared_memory_st *pSharedMemory )
{
	...
	dbgwarn0("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	dbgwarn1("!!!! LC_SetDownloadType, dlType [%x]\n", dlType);


    if (dlType > LCH_OTA_DOWNLOAD)
    {
        result = LC_ERROR_INVALID_PARAMETER;
    }
    ...
    if (LC_SUCCESS == result)
    {
		...
		dbgwarn2("!!!! dl type [%x] -> Set pSharedMemory->downloadIndicator = [%x]\n", dlType, pSharedMemory->downloadIndicator);
    }
...
lc_bool LC_GetDownloadFlag( const lc_shared_memory_st *pSharedMemory )
{
	...
	dbgprt2("Download Flag = [%x], pSharedMemory->downloadIndicator = [%x]\n", result, pSharedMemory->downloadIndicator);
    return result;
}

lc_uchar LC_GetDownloadType( const lc_shared_memory_st *pSharedMemory )
{
    lc_uchar dlType = LCH_NONE_DOWNLOAD;

    dlType = (pSharedMemory->downloadIndicator) & 0x0f;

    if (dlType > LCH_OTA_DOWNLOAD)
    {
        dlType = LCH_NONE_DOWNLOAD;
    }

	dbgprt2("Download Type [%x], pSharedMemory->downloadIndicator = [%x]\n", dlType, pSharedMemory->downloadIndicator);
