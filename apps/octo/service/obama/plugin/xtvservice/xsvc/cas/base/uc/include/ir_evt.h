...
/* __SOFTCELL_REC_EXCEPTION__
* TODO : UCCA SoftCell에서 recording이 안된다. 
*        view + rec 일 시에 Rec 에서 EMM stream_open할 시에  
*        SvcContext 값이 open 할 시에 SvcContext값과 다르게 나온다. 
*        나중에 lib 에서 정리 되면 지워야 하는 code 이다. 
*/
//#define __SOFTCELL_REC_EXCEPTION__
...
#elif defined(CONFIG_MW_CAS_IRDETO_UCAS)
	IREVTBW_SVC_TRIGGER 			= ( IREVTBW_SVC_OPENED | IREVTBW_SVC_GLOBAL_STATUS ),
#else
...
typedef union
{
	...
	//IREVT_CLASS_FLEXFLASH_STATUS_UPDATE
//	HUINT8	pucsecureCoreListStatus[IR_SC_INFO_USERDATA_LENGTH];
//	HUINT8	pucpackagesDownloadProgressInfo[IR_SC_INFO_USERDATA_LENGTH];
	HUINT16	usFlexiflash;
