...
//	DMX값은 Code에서 읽어 와야 하며 이 경우 가변적이어서 VD_DMX쪽및 기타 부분 변경이 필요함.
...
//	왜 이걸 했는지 모르겠음. 아마도 Descrambler를 두번 하는것으로 인해 한거 같은데.. 삭제함.
#if 0//defined(CONFIG_IRDETO_HMX_PVR)
#define HMX_VD_DSC_SLOT_MAX_IN_ONE_DEMUX 	16
#else
#define HMX_VD_DSC_SLOT_MAX_IN_ONE_DEMUX 	8 //16 //(16) // (1 svc has 4 es ) *  2 svc * 2 tuner, DSC 총 개수
#endif
...
#if defined(CONFIG_MW_CAS_IRDETO_SECURE) && defined(CONFIG_PROD_HM9502HD)
/*
	Softcell VD API 문서에 Secure Chip phase3를 사용하기 위해선 Loader Version 3이상 이어야 한다고 되어 있음.
	그런데 실제 동작하고는 상관이 없는 듯 하나 그래서 Spec에 있는데로 하는 것이 좋을 듯함여 V3를 사용함.
*/
#define VD_LOADER_VERSION				VD_LOADER_VERSION_V3
