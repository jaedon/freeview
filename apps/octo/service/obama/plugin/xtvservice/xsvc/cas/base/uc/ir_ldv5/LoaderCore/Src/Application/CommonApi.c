...
#if 1 // Hx modified.
lc_result LoaderCoreAPI_ReadDVBSSUParameters( lc_dvb_ssu_parameters *pDvbssuParameters, lc_buffer_st *pPrivateData)
{
    lc_result result = LC_SUCCESS;
    lc_shared_memory_st *pShardMem = LC_NULL;
    ...
    if (LC_SUCCESS == result)
    {
		...
		pPrivateData->bytes = malloc(LC_MANUFACTURER_PRIVATE_INFO_LENGTH);
		pPrivateData->length = LC_MANUFACTURER_PRIVATE_INFO_LENGTH;
		memcpy(pPrivateData->bytes, pShardMem->otaDownloadPara.privateInfo, LC_MANUFACTURER_PRIVATE_INFO_LENGTH);
    }
...
lc_result LoaderCoreAPI_WriteIPMulticastParameters( const lc_ip_multicast_parameters *pIpMulticastParameters)
{
    ...
    if (LC_NULL == pIpMulticastParameters)
    {
        result = LC_ERROR_NULL_PARAM;
    }
    ...
    /* Write serialized IP parameters into NVRAM Loader Pt */
    if (LC_SUCCESS == result)
    {
        result = LC_ReadSharedMemory(pShardMem);
        if( LC_SUCCESS == result)
        {
            ...
            pShardMem->otaDownloadPara.ipInfo.address = pIpMulticastParameters->ipAddress;
            pShardMem->otaDownloadPara.ipInfo.port = pIpMulticastParameters->port;
...
lc_result LoaderCoreAPI_ReadIPMulticastParameters( lc_ip_multicast_parameters *pIpMulticastParameters)
{
    ...
    if (LC_SUCCESS == result)
    {
        pIpMulticastParameters->ipAddress = pShardMem->otaDownloadPara.ipInfo.address;
        pIpMulticastParameters->port = pShardMem->otaDownloadPara.ipInfo.port;
...
lc_result LoaderCoreAPI_GetUKInfo(lc_uint16* pSystemId, lc_uint16* pKeyVersion, lc_uint16* pSignatureVersion)
{
    ...
    if (LC_SUCCESS == result)   /* UK module is authenticated. */
    {
        ...
        *pSystemId = ((pKeyUpdateMsg->abFullControl[4] << 8) | pKeyUpdateMsg->abFullControl[5]);
        *pKeyVersion = ((pKeyUpdateMsg->abFullControl[6] << 8) | pKeyUpdateMsg->abFullControl[7]);
        *pSignatureVersion = ((pKeyUpdateMsg->abFullControl[8] << 8) | pKeyUpdateMsg->abFullControl[9]);
    }
