 ...
 * bNitMatched: default LC_FALSE
 * bPmtMatched: default LC_FALSE
...
static lc_result HL_ParseSelectorByte(const lc_buffer_st *pData, lc_uint32 *pBitPos, lc_uint16 length, lc_bool *pMatch)
{
    ...
    if(SELECTOR_BYTE_LENGTH == length)
    {
        ...
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.manufacturerId);
        N2H16(&selectorByte.manufacturerId);
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.hardwareVersion);
        N2H16(&selectorByte.hardwareVersion);
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.reserved);
        N2H16(&selectorByte.reserved);
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.variant);
        N2H16(&selectorByte.variant);
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.subVariant);
        N2H16(&selectorByte.subVariant);
        result = MessageParser_GetBits(pData, pBitPos, 16, &selectorByte.softwareVersion);
        N2H16(&selectorByte.softwareVersion);
        result = MessageParser_GetBits(pData, pBitPos, 8, &selectorByte.triggerControlByte);
        result = MessageParser_GetBits(pData, pBitPos, 8, &selectorByte.downloadMode);
...
static lc_result HL_ParseServiceListDescriptor(const lc_buffer_st *pData, lc_uint32 *pBitPos, lc_uchar descLength)
{
    ...
    {
        result = MessageParser_GetBits(pData, pBitPos, 16, &serviceId);
        N2H16(&serviceId);
        result = MessageParser_GetBits(pData, pBitPos, 8, &serviceType);
...
static lc_result HL_ParseLinkageDescriptor(const lc_buffer_st *pData, lc_uint32 *pBitPos)
{
    ...
    result = MessageParser_GetBits(pData, pBitPos, 16, &transportId);
    N2H16(&transportId);
    result = MessageParser_GetBits(pData, pBitPos, 16, &originalNetworkId);
    N2H16(&originalNetworkId);
    result = MessageParser_GetBits(pData, pBitPos, 16, &serviceId);
    N2H16(&serviceId);
    result = MessageParser_GetBits(pData, pBitPos, 8, &linkageType);

    if(linkageType == 0x09) /* system software update service */
    {
        result = MessageParser_GetBits(pData, pBitPos, 8, &ouiDataLength);
        result = MessageParser_GetBits(pData, pBitPos, 24, &oui);
        result = MessageParser_GetBits(pData, pBitPos, 8, &selectorLength);

        if(selectorLength > 0)
        {
            result = HL_ParseSelectorByte(pData, pBitPos, selectorLength, &bMatchedSTB);
            if(LC_SUCCESS == result && LC_TRUE == bMatchedSTB)
            {
...
static lc_result HL_ParseDataBroadcastIdDescriptor(const lc_buffer_st *pData, lc_uint32 *pBitPos)
{
    ...
    result = MessageParser_GetBits(pData, pBitPos, 16, &dataBroadcastId);
    ...
    if(dataBroadcastId == 0x000A)
    {
        ...
        result = MessageParser_GetBits(pData, pBitPos, 8, &updateByte);
        updateByte &= 0x0F;
        result = MessageParser_GetBits(pData, pBitPos, 8, &versionInfo);
...
static lc_result HL_ParseDescriptor(const lc_buffer_st *pData, lc_uint32 *pBitPos, lc_uint16 length)
{
    ...
    {
        result = MessageParser_GetBits(pData, pBitPos, 8, &descTag);
        result = MessageParser_GetBits(pData, pBitPos, 8, &descLength);
        ...
        switch(descTag)
        {
            case 0x40:  /* network_name_descriptor */
                ...
                result = MessageParser_GetBits(pData, pBitPos, descLength * 8, &network_name);
            ...
            case 0x43:  /* satellite_delivery_system_descriptor */
                ...
                result = MessageParser_GetBits(pData, pBitPos, descLength * 8, &gDownloadInfo.privateData);
                break;
            ...
            case 0x47:
                ...
                result = MessageParser_GetBits(pData, pBitPos, descLength * 8, &bouquet_name);
...
static lc_result HL_ParseNITTransportLoop(const lc_buffer_st *pData, lc_uint32 *pBitPos, lc_uint16 length)
{
    ...
    while(skipLength < length)
    {
        result = MessageParser_GetBits(pData, pBitPos, 16, &transportStreamId);
        N2H16(&transportStreamId);
        result = MessageParser_GetBits(pData, pBitPos, 16, &originalNetworkId);
        N2H16(&originalNetworkId);
        result = MessageParser_GetBits(pData, pBitPos, 16, &transportDescLength);
    ...
    lc_si_parameters_for_dvb_ssu *pParameters)
{
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 8, &nitTID);
        result = MessageParser_GetBits(pSection, &bitPos, 16, &sectionLength);
        N2H16(&sectionLength);
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &networkId);
        N2H16(&networkId);
        result = MessageParser_GetBits(pSection, &bitPos, 8, &version);
        version = (version >> 1) & 0x1F;
        result = MessageParser_GetBits(pSection, &bitPos, 8, &sectionNumber);
        result = MessageParser_GetBits(pSection, &bitPos, 8, &lastSectionNumber);
    }

    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &networkDescLength);
        if (LC_SUCCESS == result)
        {
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &transportLoopLength);
        if (LC_SUCCESS == result)
        {
    ...
    lc_si_parameters_for_dvb_ssu *pParameters)
{
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 8, &pmtTID);
        result = MessageParser_GetBits(pSection, &bitPos, 16, &sectionLength);
        N2H16(&sectionLength);
        sectionLength &= 0x0FFF;
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &programNumber);
        N2H16(&programNumber);
        result = MessageParser_GetBits(pSection, &bitPos, 8, &version);
        version = (version >> 1) & 0x1F;
        result = MessageParser_GetBits(pSection, &bitPos, 8, &sectionNumber);
        result = MessageParser_GetBits(pSection, &bitPos, 8, &lastSectionNumber);
        result = MessageParser_GetBits(pSection, &bitPos, 16, &pcrPID);
        N2H16(&pcrPID);
        pcrPID &= 0x1FFF;
    }
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &programInfoLength);
    ...
    if(LC_SUCCESS == result)
    {
        ...
        {
            result = MessageParser_GetBits(pSection, &bitPos, 8, &streamType);
            result = MessageParser_GetBits(pSection, &bitPos, 16, &elementPID);
            N2H16(&elementPID);
            elementPID &= 0x1FFF;
            ...
            result = MessageParser_GetBits(pSection, &bitPos, 16, &esInfoLength);
            if(LC_SUCCESS == result)
            {
                N2H16(&esInfoLength);
                esInfoLength &= 0x0FFF;
    ...
    lc_si_parameters_for_dvb_ssu *pParameters)
{
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 8, &section.tableId);
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &section.sectionLength);
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &section.bouquetId);
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 8, &section.version);
        section.version = (section.version >> 1) & 0x1F;
        result = MessageParser_GetBits(pSection, &bitPos, 8, &section.sectionNumber);
        result = MessageParser_GetBits(pSection, &bitPos, 8, &section.lastSectionNumber);
    }

    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &section.bouquetDescLength);
        if(LC_SUCCESS == result)
        {
    ...
    if(LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pSection, &bitPos, 16, &section.transportStreamLoopLength);
        ...
        if(section.transportStreamLoopLength > 0)
        {
            ...
            {
                result = MessageParser_GetBits(pSection, &bitPos, 16, &section.transportStreamId);
                N2H16(&section.transportStreamId);
                result = MessageParser_GetBits(pSection, &bitPos, 16, &section.originalNetworkId);
                N2H16(&section.originalNetworkId);
                result = MessageParser_GetBits(pSection, &bitPos, 16, &section.transportDescLength);
