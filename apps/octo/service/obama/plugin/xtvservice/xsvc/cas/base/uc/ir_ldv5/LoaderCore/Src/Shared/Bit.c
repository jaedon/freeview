    ...
    const void *pSrc )
{
    ...
    if ((pBytes->length * 8) < (*pBitPos + bitlength))
    {
		dbgerr3("pBytes->length = %d, *pBitPos = %d, bitlength =%d \n", pBytes->length, *pBitPos, bitlength );
        result = LC_ERROR_INSUFFICIENT_BUFFER; /**< not enough bits left */
    }
    else
    {
    ...
    lc_serial_download_packet *pParsed)
{
    ...
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->data_id);
    ...
    if (LC_SUCCESS == result)
    {
        if (UART_DLH_ID == pParsed->data_id ||
            UART_MH_ID == pParsed->data_id )
        {
            result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->length);
    ...
    lc_serial_download_packet *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBytes(pBytes, pBytesPos, 1, &pParsed->data_id);
        if (UART_DLH_ID != pParsed->data_id &&
            UART_MH_ID != pParsed->data_id &&
            UART_DG_ID != pParsed->data_id )
        {
    ...
    if (LC_SUCCESS == result)
    {
        if (UART_DLH_ID == pParsed->data_id ||
            UART_MH_ID == pParsed->data_id )
        {
            result = MessageParser_GetBytes(pBytes, pBytesPos, 2, &pParsed->length);
            N2H16(&pParsed->length); /**< need to convert big-endian to host endian */
...
lc_result
MessageParser_ParseMDPMulticastDownloadPacket(
    const lc_buffer_st *pBytes,
    lc_uint32 *pBitPos,
    lc_multicast_download_packet *pParsed)
{
    lc_result result = LC_SUCCESS;
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->op_code);
    }

    /** transmission_sequence_number (2 bytes), ONLY applicalble at datagram packet */
    if (LC_SUCCESS == result)
    {
        if (IP_DG_ID == pParsed->op_code)
        {
            result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->transmission_sequence_number);
            N2H16(&pParsed->transmission_sequence_number); /**< need to convert big-endian to host endian */
        }
        ...
        if (LC_SUCCESS == result)
        {
            TRACE_LOG1(
                TRACE_DEBUG,
                "[PARSER] <MDP> transmission_sequence_number = %u\n",
                pParsed->transmission_sequence_number);
        }
    }

    /* point to the start address instead of memory allocation */
    if (LC_SUCCESS == result)
    {
        pParsed->payload = &pBytes->bytes[(*pBitPos)/8];

        /* skip this buffer */
        if (IP_DG_ID == pParsed->op_code)
        {
            *pBitPos += pBytes->length
                        - 1             /** op_code (1 byte) */
                        - 2;             /** transmission_sequence_number (2 bytes) */
        }
        else
        {
            *pBitPos += pBytes->length
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->crc16);
    ...
    lc_module_dlh_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->manufacturer_id);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->hardware_version);
    ...
    if (LC_SUCCESS == result)
    {
        TRACE_LOG2(
            TRACE_DEBUG,
            "[PARSER] <DH> hardware_version = %d[%x]\n",
            pParsed->hardware_version, pParsed->hardware_version);
    }

    /** loader_gte_v4 (1 bit), must be 0b0 for Loader V5 */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->loader_gte_v4);
    }

    /** loader_gte_v3 (1 bit), must be 0b0 for Loader V5 */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->loader_gte_v3);
    }

    /** loader_gte_v5 (1 bit), must be 0b1 for Loader V5 */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->loader_gte_v5);
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 5, &pParsed->reserved);
    }

    /** loader_minor_version (1 byte) */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->loader_minor_version);
        TRACE_LOG1(
            TRACE_DEBUG,
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->download_id);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->variant);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->sub_variant);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, pParsed->isign_version);
        TRACE_LOG1(
            TRACE_DEBUG,
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, pParsed->isign_serial_number);
        TRACE_LOG1(
            TRACE_DEBUG,
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->download_sequence_number);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->number_of_modules);
    ...
    if (LC_SUCCESS == result)
    {
        for (cnt = 0; cnt < (pParsed->number_of_modules); cnt++ )
        {
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->pModuleIdList[cnt].module_id);
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->pModuleIdList[cnt].module_version);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->header_signature_length);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes,
                            pBitPos,
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->download_signature_length);
    ...
    lc_module_mh_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->module_id);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->compressed);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->obscured);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->encrypted);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->original_size);
    ...
    if ( LC_TRUE == pParsed->compressed )
    {
        ...
        if (LC_SUCCESS == result)
        {
            result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->compressed_size);
        ...
        if (LC_SUCCESS == result)
        {
            result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->crc32);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->block_size);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->block_count);
    ...
    lc_module_dg_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->data_length);
    ...
    lc_module_dg_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBytes(pBytes, pBytePos, 2, &pParsed->module_id);
        N2H16(&pParsed->module_id); /**< need to convert big-endian to host endian */
        TRACE_LOG1(
            TRACE_DEBUG,
            "[PARSER] <DG> module id = 0x%04x\n",
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBytes(pBytes, pBytePos, 2, &pParsed->data_length);
        N2H16(&pParsed->data_length); /**< need to convert big-endian to host endian */
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBytes(pBytes, pBytePos, 2, &pParsed->crc16);
        N2H16(&pParsed->crc16); /**< need to convert big-endian to host endian */
        TRACE_LOG1(
            TRACE_DEBUG,
            "[PARSER] <DG> crc16 = 0x%04x\n",
    ...
    lc_module_info_header *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->header_length);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->module_size);
    ...
    if (LC_SUCCESS == result)
    {
            ...
            MODULE_DRV_ID_LOADER_CORE != pParsed->module_id)
        {
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->boot_check);
                TRACE_LOG1(
                    TRACE_DEBUG,
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 1, &pParsed->addressable);
                TRACE_LOG1(
                    TRACE_DEBUG,
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 6, &pParsed->reserved);
            }

            /** module_version (2 bytes) */
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->module_version);
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->memory_type);
                TRACE_LOG1(
                    TRACE_DEBUG,
            ...
            if (LC_SUCCESS == result)
            {
                ...
                {
                    result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->start_address);
            ...
            if (LC_SUCCESS == result)
            {
                result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->module_signature_length);
    ...
    lc_bbcb_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 24, pParsed->OUI);
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->loader_major_version);
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->loader_version_buildnumber);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->boot_check_algorithm);
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, pParsed->serial_number);
    }

    /** random_number (4 bytes) */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, pParsed->random_number);
    }
    ...
    lc_ota_dl_para_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 32, &pParsed->ipInfo.address);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->ipInfo.port);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->dvbssuInfo.pid);
    ...
    lc_shared_memory_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->downloadIndicator);
    }

    /** reserved (1 byte) */
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 8, &pParsed->reserved);
    }
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->migrationInfo.variant);
    ...
    if (LC_SUCCESS == result)
    {
        result = MessageParser_GetBits(pBytes, pBitPos, 16, &pParsed->migrationInfo.subvariant);
