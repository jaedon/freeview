...
// jichoi moved into ModulaManager.h file. #define MIN_MODULE_ID_APP (0x20)
    ...
    lc_uchar* pSignature)
{
    ...
    if (LC_SUCCESS == result)
    {
        pEm = LC_MALLOC(SIGNATURE_LENGTH);
        if (LC_NULL == pEm)
        {
            result = LC_ERROR_INSUFFICIENT_BUFFER;
        }
    ...
    if (LC_SUCCESS == result)
    {
        pkcsRet = Rsa_CheckHashMode(pSignature, pStatus->pRsaKey, pEm, &sha.mode);
    ...
    if (LC_SUCCESS == result)
    {
        ...
        pkcsRet = Rsa_VerifyPkcs(pEm, &stHash, sha.mode);
...
dbgtrc;
    ...
    if (LC_SUCCESS == result)
    {
        if (0 == mih_st.crc32)
        {
            ...
            TRACE_LOG1(
                TRACE_ERROR,
                "[Module_mng] Error: ID #%u CRC is NULL!\n",
                mih_st.module_id);
        }
        else
        {
            ...
            if (crc32 != mih_st.crc32)
            {
	            ...
	            TRACE_LOG1(
	                TRACE_ERROR,
	                "[Module_mng] Error: ID #%u CRC is NOT matched!\n",
	                mih_st.module_id);
            }
        }
    }
	...
	{
		TRACE_LOG2(TRACE_ERROR, "[Module_mng] Error parsing module header #%u, result:%d\n", moduleId, result);
	}
...
dbgtrc;
	...
	{
		TRACE_LOG2(TRACE_ERROR, "#[Module_mng] LC_ReadAndParseModuleInfoHeader error id #%u, result =%d\n", moduleId, result);
	}
...
lc_result LC_GetVariant(lc_uint16 *pVariant, lc_uint16 *pSubVariant)
{
    ...
    if (LC_SUCCESS == result)
    {
        ...
        if (LC_SUCCESS != result)
        {
            TRACE_LOG1(TRACE_ERROR, ("[Module_mng] Error loading existing variant!(result %d)\n"), result);
        }
    }
    ...
    lc_uchar* hash)
{
    ...
    if(LC_SUCCESS == result)
    {
        stHash.bytes = hash;
        stHash.length = hashLength;
        pkcsRet = Rsa_VerifyPkcs(signatureClear, &stHash, hashMode);
