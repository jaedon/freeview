...
//static MAILBOX_INFO	s_stIrUCMailboxInfo;
//#endif
...
void local_cas_ir_MSR_DRV_HashCodeConvert(char* szInSCNumber, char* szOutHashedCode );
...
void CAS_IR_MSG_DRV_ProcessDecoderDataEMM(Handle_t hAct, HUINT8 *p)
{
	...
	switch(ucDestinationID)
	{
		...
		case DECODER_MSG_DESTINATION_ID_ATTRIBUTED_DISPLAY:		// Attributed Display
			HxLOG_Print(" == start  attributed display == \n");
...
HBOOL CAS_IR_MSG_DRV_DeleteMail( HUINT8 ucMailIndex )
{
	...
	IRMSG_MUTEX_GET;
#if 0
	HxSTD_memset((HUINT8 *)&(_mailbox_table[ucMailIndex]), 0, SIZE_MAILBOX_TABLE);
	MSG_DRV_StorageUserMailWrite(ucMailIndex);
#else
...
void CAS_IR_MSG_DRV_SendMail (HUINT8 *szText)
{
	...
	strcpy(mail_msg.text, szText);
	mail_msg.msg_length	= strlen(szText);
	mail_msg.msg_class 	= EPG_MESSAGE_CLASS_FORCED_DISPLAY;
	mail_msg.ucType		= EPG_MESSAGE_TYPE_TEXT_MAILBOX;
	mail_msg.unixtime	= 0;
	VK_CLOCK_GetTime(&mail_msg.unixtime);
	HLIB_DATETIME_ConvertUnixTimeToDateTime(mail_msg.unixtime, &dt);
	mail_msg.add_date	= dt.stDate;
	mail_msg.add_time	= dt.stTime;

	local_cas_ir_MSG_DRV_MsgProcess(g_IrTpInfo[0].hAction, &mail_msg);
}
#endif

#if 0	// NOT USED FUNCTION
static void local_cas_ir_MSG_DRV_PrintMail (MAILBOX_TABLE *pMail)
{
	HUINT16 i;
	FILE	*fp;
	char	filename[128];
...
#if defined(WIN32)
	sprintf_s(filename, 128,
		"%04d_irdeto_mail_debug_%08u.txt",
#else
	snprintf(filename, 128,
		"/opt/%04d_irdeto_mail_debug_%08u.txt",
#endif
		++s_number,
	...
	fprintf(fp, "MAILBOX_TABLE[%4d-%02d-%02d/%02d:%02d]\n"
				"\tucType: %d\n"
				"\tucMsgNum: %d\n"
				"\tpriority: %d\n"
				"\tmsg_class: %d\n"
				"\tmsg_length: %d\n"
				"\tbAttributeMsg: %d\n"
				, pMail->add_date.usYear, pMail->add_date.ucMonth, pMail->add_date.ucDay
				, pMail->add_time.ucHour, pMail->add_time.ucMinute
				, pMail->ucType
				, pMail->ucMsgNum
				, pMail->priority
				, pMail->msg_class
	...
	for (i = 0 ; i < pMail->msg_length ; i++)
	{
		fprintf(fp, "%02X", pMail->text[i]);
		if (i == 0)
			;
		else if (i % 10 == 0)
			fprintf(fp, "\r\n");
			...
			switch(ucTag)
			{
				...
				case IR_UP_CA_TAG_ITO_DDEMM_REPORT_STATUS:
				{
					...
					if(usDataLength != 0)
					{
						...
						pstReportStatus->pucUsername[20] = '\0';
						p += 20;
						VK_MEM_Memcpy(pstReportStatus->pucPassword, p, 20);
						pstReportStatus->pucPassword[20] = '\0';
						p += 20;
...
void local_cas_ir_MSG_DRV_MsgProcess(Handle_t hAct, MAILBOX_TABLE *pMail_msg)
{
	...
	HxLOG_Print(" Msg_Type:0x%02X [0x%02X] \n",pMail_msg->Atrb_Display.ucMsg_Type,ATTRIBUTED_MESSAGE_TYPE_FINGERPRINT) ;
...
static HERROR local_cas_ir_MSG_DRV_CopyMailTextWithLocaleCode(HUINT8	*szMail, HUINT8 *pucData, HUINT16 *ulLength)
{
	...
	*(szMail++) = 0x46;
	...
	VK_memcpy(szMail, pucData, *ulLength);
	szMail[*ulLength] = '\0';
...
// arabic test code
	...
	{
		HUINT8 i=0;
		VK_Print("local_cas_ir_MSG_DRV_GetMailMessage] raw mail message string: ");
		for(i=0; i<mail_msg->msg_length; i++)
		{
			VK_Print("%x ", pbData[wIx + i]);
		}
		VK_Print("\n");
...
//		HxSTD_memcpy(mail_msg->text, &(pbData[wIx]), mail_msg->msg_length);
		mail_msg->text[mail_msg->msg_length] = '\0';
		HxLOG_Print(" Mail text : %s \n", mail_msg->text);        
		boRet = TRUE;
	}
...
#if 0 // timid mail is not integrated.
void MSG_DRV_DeleteExpiredNormalAttributedMail(HUINT32 ulTimerId, void *pLocalData)
{
	HUINT8 	i;
	HUINT8	nMailCnt;
	HUINT8	aMailIdxList[MAX_NUMBER_MAIL_MESSAGE];
	UNIXTIME	*receivedTime;

	HxLOG_Print("\n");

	receivedTime = (UNIXTIME *) pLocalData;

	if(receivedTime == NULL)
	{
		HxLOG_Print("receivedTime= NULL,\n", receivedTime) );
		return;
	}

	CAS_IR_MSG_DRV_GetMailInfo(&nMailCnt, aMailIdxList);

	for(i=0; i< nMailCnt; i++)
	{
		if(s_stIrMailboxInfo.valid[aMailIdxList[i]] == IR_MAIL_VALID_NOT_OK)
			continue;

		if(s_stIrMailboxInfo.mailbox_table[aMailIdxList[i]].unixtime == (*receivedTime))
		{
			CAS_IR_MSG_DRV_DeleteMail(aMailIdxList[i]);
			return;
		}
...
HUINT8 local_cas_ir_MSG_DRV_AddMailMessage( MAILBOX_TABLE *mail_msg )
{
	...
	HBOOL			boMailavailable;
	UNIXTIME		curUnixtime;
	HxDATETIME_Time_t		curTime;
	HxDATETIME_Date_t		curDate;
	HIR_ERR		hErr;
	...
	if( ucMailIndex < MAX_NUMBER_MAIL_MESSAGE )
	{
		...
		//attribute msg 에서 normal msg 이면서 duration이 0 이 아닌 경우  duration 만큼의 시간이
		//지난 후 자동 삭제될 수 있도록 Timer 를 건다.  자동 삭세지 메일을 구분하는 키는 curUnixTime
		//이다. key를 두는 이유는 자동삭제전 사용자가 RCU로 수동삭제할 수 있기 때문

		//Timed Mail 구현하지 않음
		/*
		if((s_stIrMailboxInfo.mailbox_table->bAttributeMsg == TRUE) &&
		(s_stIrMailboxInfo.mailbox_table->Atrb_Display.ucMsg_Type == 0x00) &&
		(s_stIrMailboxInfo.mailbox_table->Atrb_Display.unDuration != 0))
		{
		HxLOG_Print("VK_TIMER_EVENT ====\n");
		VK_TIMER_EventAfter(s_stIrMailboxInfo.mailbox_table->Atrb_Display.unDuration * 1000, MSG_DRV_DeleteExpiredNormalAttributedMail,
		(void *)&curUnixtime, sizeof(UNIXTIME), &nExpiredMailTimerId);
		...
		/**
		 * 왜 인덱스를 다시 간접인덱스로 바꿔서 메시지를 보내나?? ㅎㅎ
		 * 아무튼 현재 옥토에서는, APPL에서 메일 리스트를 읽지 않는 한,
		 * CAS로 부터 받은 메일 인덱스는 직접!! 접근할 수 있는 인덱스라고 생각하고 코딩되어 있으니
		 * 아래 코드는 막는다.
		 */
		//ucMailIndex = (HUINT8)j;	// return value is index of aucMailIndexList
	}
	else
	{
...
HUINT8 local_cas_ir_MSG_DRV_FindLowestPriorityMailMessage( void )
{
	...
	HUINT8		ucReadCount = 0;
	HUINT8		aucPrioGroup[MAX_NUMBER_MAIL_MESSAGE];
	HUINT8		ucPrioCount=0;
	HUINT8		ucLowestPrior = 0;
	UNIXTIME	earliestTime = 0;
	MAILBOX_TABLE	*pMail;
...
HUINT8 local_cas_ir_MSG_DRV_FindLowestPriorityMailMessage( void )
{
	HUINT8		i, j, bLowest_priority;
	int			priority_cnt = 0;
	int			read_cnt = 0;
	HUINT8		index_temp[MAX_NUMBER_MAIL_MESSAGE];
	HUINT8		index_temp2[MAX_NUMBER_MAIL_MESSAGE];
	HUINT8		ucFinalIndex = 0;

	// 이미 읽어버린 mail들만 모으고..
	for ( i = 0; i < MAX_NUMBER_MAIL_MESSAGE; i++ )
	{
		if ( s_stIrMailboxInfo.valid[i] == IR_MAIL_VALID_OK && s_stIrMailboxInfo.bReadStatus[i] == IR_MAIL_READ_STATUS_READ )
		{
			index_temp[read_cnt] = i;
			read_cnt++;
		}
	}

	// 이미 읽은 mail이 없으면 전체중에 scan하도록 변경해주고..
	if( read_cnt == 0 )
	{
		read_cnt = MAX_NUMBER_MAIL_MESSAGE;
	...
	// scan lowest priority mails within all read mails.
	priority_cnt = 0;
	bLowest_priority = 0xFF;
	for ( i = 0; i < read_cnt; i++ )
	{
		if ( s_stIrMailboxInfo.valid[index_temp[i]] == IR_MAIL_VALID_OK)
		{
			if(s_stIrMailboxInfo.mailbox_table[index_temp[i]].priority < bLowest_priority )
			{
				priority_cnt = 0;
				bLowest_priority = s_stIrMailboxInfo.mailbox_table[index_temp[i]].priority;
				index_temp2[priority_cnt++] = index_temp[i];
			}
			else if(s_stIrMailboxInfo.mailbox_table[index_temp[i]].priority == bLowest_priority )
			{
				index_temp2[priority_cnt++] = index_temp[i];
			}
			else
	...
	for (i = 0; i < MAX_NUMBER_MAIL_MESSAGE; i++)
	{
		...
		{
			if (s_stIrMailboxInfo.aucMailIndexList[i] == index_temp2[j])
			{
				ucFinalIndex = index_temp2[j];
				break;
			}
		}

		if (ucFinalIndex != 0xFF)
		{
			break;
		}
	}
	...
	if (i == MAX_NUMBER_MAIL_MESSAGE)
	{
		for (i = 0; i < MAX_NUMBER_MAIL_MESSAGE; i++)
		{
			if ( s_stIrMailboxInfo.valid[s_stIrMailboxInfo.aucMailIndexList[i]] == IR_MAIL_VALID_OK)
			{
				ucFinalIndex = s_stIrMailboxInfo.aucMailIndexList[i];
				break;
			}
		}
	}
...
//	ucTemp = ucTmpMethod & 0x01;
	HxLOG_Print("ucTemp: %d (1: Covert, 0: Overt)\n", ucTemp);
	if(ucTemp > 0)
	{
...
//arabic test code
	...
	{
		HUINT8 i=0;
		HxLOG_Print("local_cas_ir_MSG_DRV_GetAttributedDisplay] raw mail message string: ");
		for(i=0; i<mail_msg->msg_length; i++)
		{
			HxLOG_Print("%x ", pbData[wIx + i]);
		}
		HxLOG_Print("\n");
...
#if defined(CONFIG_MW_CAS_IRDETO_SUPPORT_COVERT_FINGERPRINT)
	if( mail_msg->Atrb_Display.ucMsg_Type == ATTRIBUTED_MESSAGE_TYPE_FINGERPRINT_COVERT_OVERT_OPTION)
	{

		// TODO: 1. 원래 spec상 해당 field는 enhanced의 속성으로 처리해야 할 듯.  (Enhanced Overt Fingerprinting 3rd party requirements)
		// TODO: UPC 는 enhanced FP 사양 제외.  (enhanced FP 를 사용하려면 이 field를 TLV 로 parsing해서 사용해야함)

		// TODO: 2. 그럼 출력해줘야 할 text는? Covert Fingeprinting 3rd party requirements 를 보면 softcell로 부터 smartcard ID message를 받아 처리해야 할 것으로 보임
		// TODO: 일단 SC 정보 읽고 hash 해서 출력하도록 고치자.
		MW_CAS_IRUC_API_CopySerialNum(0, mail_msg->text);
		mail_msg->msg_length = MWC_UTIL_DvbStrlen(mail_msg->text);
...
void local_cas_ir_MSG_DRV_Attributed_DisplayProcess(Handle_t hAct, HUINT8 *pData, HUINT16 usLength)
{
	...
	{
#if defined (CONFIG_MW_CAS_IRDETO_UCAS)
		if (xsvc_cas_IrGetSvcType((void*)hAct) == IR_CAS_TYPE_UC)
		{
			/*  Comment : hjlee3 
			     attribute fingerpint 로 fingerPrint 사용 한다.
			     그래서 humax hashcodeconvert 사용 할 필요가 없다.
			     나중을 위해서 code는 남겨 둔다.
	...
	{
		VK_Print("receive ATTRIBUTED_MESSAGE_TYPE_FINGERPRINT_COVERT_OVERT_OPTION \n");
#if defined(CONFIG_MW_CAS_IRDETO_SUPPORT_COVERT_FINGERPRINT)
		if (xsvc_cas_IrGetSvcType((void*)hAct) == IR_CAS_TYPE_UC)
		{
			HUMAX_VdSc_HashCodeConvert();
...
//	printf("jichoi: sizeof(MAILBOX_INFO) = %d\n", sizeof(MAILBOX_INFO) );
