...
static HERROR local_IRUC_ClientStatusInfo(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam);
static HERROR local_IRUC_ProductStatusInfo(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam);


/**************************************************************************************************/
#define ________Irdeto_ctrl_External_Functions_Body__________________________________________
/**************************************************************************************************/
// hjlee3 여서 contorl function 함수 등록 한다.
...
// ==> UCCA 관련 기능
	{
		...
		eCasCtrl_Get,
		NULL,					// Control
		local_IRUC_ClientStatusInfo, // Get
		NULL,	// Set
		NULL,	// Req
		NULL,	// Noti
	},

	{
		...
		eCasCtrl_Get,
		NULL,					// Control
		local_IRUC_ProductStatusInfo, // Get
		NULL,	// Set
		NULL,	// Req
		NULL,	// Noti
	},
...
static HERROR local_IR_LOADER_GetManuCode(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	hErr = xsvc_cas_IrLoaderGetManuCode(&pstOutParam->usManCode);
...
{
	HERROR			hErr;
	IR_LOADER_GetClientVersionOutParam_t	*pstOutParam;
	...
	pstOutParam = (IR_LOADER_GetClientVersionOutParam_t *)local_IR_GetOutParam(pstParam);
	if (pstOutParam == NULL)
	{
		return ERR_FAIL;
	}

	hErr = xsvc_cas_IrGetClientVersionString(&pstOutParam->ucClientVersion[0]);
...
static HERROR local_IR_LOADER_GetHWCode(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	hErr = xsvc_cas_IrLoaderGetHWCode(&pstOutParam->usHwCode);
...
static HERROR local_IR_LOADER_GetLoadSeqNum(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	hErr = xsvc_cas_IrLoaderGetLoadSeqNum(&pstOutParam->usLoadSeqNum);
...
static HERROR local_IR_GetMailBox(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	HxLOG_Info("[%s] [%d] ulControlType: %d\n", __FUNCTION__, __LINE__, ulControlType);

	if (ulControlType >= eCACTRL_IR_MAX)
	{
		HxLOG_Info("[%s] [%d] ulControlType: %d >= eCACTRL_IR_MAX\n", __FUNCTION__, __LINE__, ulControlType);
		return ERR_FAIL;
	}

	if (pstParam == NULL)
	{
		HxLOG_Info("[%s] [%d] pstParam == NULL\n", __FUNCTION__, __LINE__);
		return ERR_FAIL;
	}
	...
	if (pstOutParam == NULL)
	{
		HxLOG_Info("[%s] [%d] pstOutParam == NULL\n", __FUNCTION__, __LINE__);
		return ERR_FAIL;
	}
...
static HERROR local_IR_GetSringIdxFromErrorCode(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	if(pstOutParam->unStrIdx == IRUC_STATUS_MSG_NONE)
	{
		return ERR_FAIL;
	}
...
/* Commnet :hjlee3 status 얻어 오는 부분이 이상 하다.
 			CONFIG_MW_CAS_IRDETO_PPV 에 묵여서 사용 되지는 않는다
 			사용시에 bGetStatus 잘못 되서 다시 연결 해야 한다. */
//	MWC_UTIL_DvbStrncpy(pstOutParam->bGetStatus, &stBuffScStatus.status[0], UC_STATUS_LENGTH);
...
//	xsvc_cas_IrScInfo(pstOutParam->ucScSlotId , pstOutParam->pstIrApiScInfo);
	return ERR_OK;
}
...
static HERROR local_IR_GetNumberOfServices (HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	...
	pstOutParam->bGetStatus = xsvc_cas_IrUcSvcStatusGetNumberOfServices(&(pstOutParam->pstSvcStatusInfo), &pstOutParam->ucNumberOfServices);
...
/* TODO: hjlee3 아직 사용 하고 잇지 않다. 어케 사용 할지 추후에 확인 하자 ....
	xsvc_cas_IrUcSvcStatusGetInformationOfEachService(pstOutParam->eSvcType, pstOutParam->usHandle, pstOutParam->pstSvcStatusInfoEach);
	MWC_UTIL_DvbStrncpy(pstOutParam->bGetStatus, pstOutParam->pstSvcStatusInfoEach., TEXT_SIZE64);
*/
	return ERR_OK;
}
...
/* TODO : hjlee3 element  개수 어케 얻어올지 확인 해서 작업 해줘라. */
...
static HERROR local_IRUC_ClientStatusInfo(HUINT32 ulControlType, SvcCas_CtrlParam_t *pstParam)
{
	HERROR				hErr;
	HUINT32 			tmsDataLen;
	IR_GetClientInfoOutParam_t  	*pstOutParam;

/* TODO : hjlee3
          xsvc_cas_IrUcGetStatusClient  어케 연결 할 지 확인 해야 한다.
          ctrl 에 연결 확인 필요 하다.   */
	if (ulControlType >= eCACTRL_IR_MAX)
	{
		return ERR_FAIL;
	...
	pstOutParam = (IR_GetClientInfoOutParam_t *)local_IR_GetOutParam(pstParam);
	if (pstOutParam == NULL)
	{
		return ERR_FAIL;
	}

	hErr = xsvc_cas_IrUcGetStatusClient(pstOutParam->pstIrApiClientInfo, &tmsDataLen);
...
{
	HERROR						hErr;
	IR_GetProductInfoOutParam_t  	*pstOutParam;
	HxLOG_Error("+jyjung+ local_IRUC_ProductStatusInfo \n");
	...
	pstOutParam = (IR_GetProductInfoOutParam_t *)local_IR_GetOutParam(pstParam);
	if (pstOutParam == NULL)
	{
		return ERR_FAIL;
	}

	hErr = xsvc_cas_IrUcGetStatusProduct(&pstOutParam->productCnt, &pstOutParam->pstIrApiProductInfo);
