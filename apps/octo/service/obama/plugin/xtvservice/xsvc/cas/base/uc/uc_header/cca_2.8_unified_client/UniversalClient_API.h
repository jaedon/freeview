 ...
 * Size of status
 */
#define UC_SECURE_STATUS_LENGTH 24
 ...
 *  \note All Ixxx error status are interim status. They should not be displayed in the error banner.
 *  The Application can simply ignore Ixxx in the error banner.
 *  @{
 */
 ...
 * messages ::UC_SERVICE_ECM_MONITOR_STATUS and ::UC_SERVICE_EMM_MONITOR_STATUS.
...
/**
 * Connection Extended Data structure
 *
 * This structure is used by the ::UniversalClient_OpenConnection to set the connection extended data.
 *
 */
typedef struct _uc_connection_extended_data
{
    /**
     * 8-bit field to identify the selected mode of the scrambling algorithm. Reference document:<<ETSI EN 300 468>> Table 79.
     */
    uc_uint8 scramblingMode;
}uc_connection_extended_data;
...
/**
 * Reserved connection or SPI stream type
 */
#define UC_CONNECTION_STREAM_TYPE_RESERVED 5
 ...
 * It contains information about the method to protect CSSK.
 * The structure is used in these place:
 * 1, Pased to ::UniversalClientSPI_Device_SetExtendedCSSK, indicate the CSSK protection method.
 * 2, When client is in testing mode, pased to client through API ::UniversalClient_SetExtendedTestingKey,
 * this can notify the client with the key protection method applied by both CSSK and descrambling key.
 */
