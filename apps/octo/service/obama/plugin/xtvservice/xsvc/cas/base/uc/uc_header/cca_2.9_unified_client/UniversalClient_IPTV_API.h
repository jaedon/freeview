...
/** @defgroup iptvapi Cloaked CA Agent Pull EMM APIs
 ...
 *
 * See also \ref basic_emm_pulling "Basic EMM Pulling Flow"
 ...
 *
 * Application can call this function to submit CCIS addresses.
 * The amount of server address that application can submit by one time is currently limited to 1.
 * The type of CCIS address can be ::UC_PULL_EMM_WEB_SERVICE.
 * ::uc_web_service_type explains the type of CCIS address.
 *
 *  \note This function must be called after the Pull EMM service is opened through invoking ::UniversalClient_OpenService.
 ...
 * @param[in] serverAddressCount the number of the CCIS addresses submitted, currently limited to 1.
...
/**
 * Trigger the re-try of EMM Pulling request.
 *
 * \note The API is not used and can be ignored by the Device Application.
 *
 * This function will trigger the Cloaked CA Agent to retry the relevant EMM pulling request, for example the Entitlement Request.
 ...
 *
 *  \note Refer to \subpage pull_emm_retry.
