...
typedef enum {
    ...
    */
    ...
    /** (0x0000006B) Anchor failed(DeviceID/UA mismactch. For IP only STBs, the application should register again). */
    ...
    /** (0x00000097) This operation cannot be completed because PVR MSK and CPSK are missing. */
    UC_ERROR_MISSING_PVR_MSK_CPSK = 0x00000097
} uc_result;
 ...
 *
 * ECM Rejected. This message is sent when an ECM is rejected by the client for a generic reason. 
...
typedef enum
{
     ...
     * and the progress of Cloaked CA package downloaded, either the two items foresaid is changed, this message type will be triggered by Agent.
     *
    ...
    */
     ...
     *
     * This is the message that indicates the Device Domain ID change.
     *
     * For messages with this type, the \a lpVoid argument is NULL
    ...
    /**
     * (0x00000015) The number of Smartcard Sectors is changed.
     *
     * This is the message that notifies the number of smartcard sectors changed.
     *
     * For messages with this type, the \a lpVoid argument is NULL
     ...
     *
     * This is the message that requires manufacturer to display a message on screen. 
     * This message has no relationship with the Service Status Message. They can co-exist on screen and do not impact each other. 
     *
     ...
     *
     * This is the message that notifies SKE data is changed.
     *
     * For messages with this type, the \a lpVoid argument is NULL
     * The application will use ::UniversalClient_SKELP_GetSKEData to check the change.
     ...
     *
     * This is the message indicates information of IFCP Image changed. The information contains the status of IFCP Image
     * and the progress of IFCP Image downloaded, either the two items foresaid is changed, this message type will be triggered by Agent.
     *
     ...
     *
     * This is the message indicates the internal thread in Cloaked CA Agent has been initialized successfully and all APIs are now ready to be called.
     *
     * This message will be sent out immediately once the internal thread has finished the initialization. 
     * This message is newly added to facilitate some special business cases.
     *
 ...
 * Source type.
 *
 * It is used to indicate that something is from Cloaked CA or Smartcard CA (Softcell).
 * It is available for Unified Client only, which includes both Cloaked CA and Smartcard CA.
 */
typedef enum
{
...
typedef struct _uc_product_status
{
     ...
     * 
     * Indicate the product is from Cloaked CA or Smartcard CA. It is only available for Unified Client.
     * 
     */
...
typedef enum
{
     ...
     *
     * A descrambling key may arrive with this type when the scrambling solution is known to be
     * AES-128 RCBC or CSA3. The stream implementation may use this information as it sees fit.
     */
 ...
 * IFCP (Irdeto FlexiCore Processor) is newly introduced and supported by some Irdeto secure chipset models. 
...
typedef struct _uc_testing_key_info
{
    ...
    /**
     * The CW key ladder level. Currently only support 2 or 3.
     */ 
     ...
     * If key ladder level is 2, this parameter will be ignored.
     */ 
...
/**
 * Smartcard sector address per sector
 */
 ...
 *
 *  \note This function is not formally supported, and must be ignored in the current release.
 *
 ...
 */
/**
 * Query the hashcode of the device.
 ...
 * On input, pHashCode pHashCode->bytes point to actual memory to store hash code, pHashCode->length indicates max available bytes count. It is recommended to allocate 64 or more bytes for pHashCode.
 * On output, if the function is successfully, pHashCode will be updated with actual content and length of code.
 * 
 * @retval
 *    ::UC_ERROR_SUCCESS
 * @retval
 ...
 *      The application must set the uc_buffer_st::bytes member to point to a buffer in memory, and the uc_buffer_st::length
 *      member to the maximum length of the buffer, in bytes. The function will modify the contents of the buffer
 *      to contain the emm filter number and the Smartcard Unique Address in binary big-endian format, without any terminating character, and set the 
 ...
 *     \n
 *     \note     
 *     Suppose the emm filter number is 24(= 0x18) and the Smartcard Unique Address is 12345678(=0xBC614E), 
