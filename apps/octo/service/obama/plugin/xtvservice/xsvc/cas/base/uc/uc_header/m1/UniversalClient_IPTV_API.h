 ...
 *
 * For Cloaked CA Agent for IP only STBs: See also \ref basiciptv_application "Basic Application Flow for IPTV Client"
 * for an overview of how the IPTV Client API is used. 
  *  @{
 */
 ...
 *
 * For Cloaked CA Agent for IP only STBs: The application will submit multiple CCIS addresses at one time through this function.
 * There are two types of CCIS addresses, one is ::UC_CLIENT_REGISTRATION_WEB_SERVICE the other is ::UC_PULL_EMM_WEB_SERVICE.
 *
 *  \note For Cloaked CA Agent for IP only STBs: This function must be called after the IPTV service is opened through invoking ::UniversalClient_OpenService.
 * The beginning of the CCIS address must be "https://", other formats will be regarded as illegal input. 
 *
