...
typedef struct _uc_extra_trick_mode_control
{
     ...
     *                                              001     No skipping, fast forward limited to 2x speed, visible content
     *                                              010     No skipping, fast forward limited to 4x speed, visible content
     *                                              011     No skipping, fast forward limited to 8x speed, visible content
...
typedef struct _uc_device_security_state
{
     ...
     *
     * \code     
     * Field Description                    Bit#    Value   Description
     ...
     * \code 
     * Field Description    Bit#    Value   Description
     * JTAG status          1..0    00      JTAG Open
     *                              01      JTAG permanently closed
     *                              10      JTAG Keyed
     *                              11      Reserved for Future Usage
     * Reserved for future  2..31   0..0    Set all bits to zero
     *
     * \endcode 
     * Examples: 
     * 0x01 represents JTAG is permanently closed.
     * 0x02 represents JTAG is keyed.
     */
    uc_uint32 jtag;
     ...
     * \code 
     * Field Description                    Bit#    Value   Description
     * Secure CW mode                       0       0       Not enforced
     *                                              1       Permanently enforced
     * Secure PVR mode                      1       0       Not enforced
     *                                              1       Permanently enforced
     * 3 level ladder Secure CW mode        2       0       Not enforced
     *                                              1       3-level CW ladder enforced
     * 3 level ladder secure PVR mode       3       0       Not enforced
     *                                              1       3-level PVR ladder enforced
     * Reserved for future                  4..31   0..0    Set all bits to zero
     *
     * \endcode 
     * Examples: 
     * 0x01 represents only Secure CW mode is enforced in the 2-level ladder mode.
     * 0x03 represents both Secure CW and Secure PVR mode are enforced in the 2-level ladder mode.
     *
     */
    uc_uint32 cwMode;
...
typedef struct _uc_KLC_info
{
     ...
     *
     * \note Currently, ED_Ctrl is always 0. STB manufacturers must follow the chip's specification to set the ED_Ctrl for IFCP command construction.
     */
    uc_byte ED_Ctrl;
     ...
     *
     * \note Currently, extraData.bytes is always NULL. STB manufacturers must follow the chip's specification to set the Extra Data for IFCP command construction.
     */
 ...
 * 
 ...
 \endcode
 *
 * \note Currently, the ED_Ctrl and Extra Data are not available from the Cloaked CA Agent. STB manufacturers must follow the chip's specification to set ED_Ctrl and Extra Data for IFCP command construction.\n 
 *
 * \n
 *
...
typedef struct _uc_IFCP_input
{
    ...
    /**
     * Additional information that is needed by STB Application
     */
    union
    {
        /**
         * Additional information when command type is ::uc_IFCP_command_type::UC_IFCP_COMMAND_SET_DESCRAMBLING_KEY. 
         */
        ...
        /**
         * Additional information when command type is ::uc_IFCP_command_type::UC_IFCP_COMMAND_SET_PVR_SESSION_KEY. 
         */
 ...
 * This function is only used by Cloaked CA Agent for Secure Chipset based STBs.
 * Currently, this SPI is used for Pre-enablement feature or Secure Pre-enablement feature, which requires personalized data to work.
 *
