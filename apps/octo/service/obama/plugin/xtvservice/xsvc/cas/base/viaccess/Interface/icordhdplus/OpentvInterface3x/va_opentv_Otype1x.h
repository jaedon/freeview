...
typedef UINT32		va_stream_t;
typedef UINT16		va_msg_type_t;
typedef UINT16		va_msg_class_t;
...
typedef UINT8 va_request_type;			/* Common part of a request #####0.35e */
typedef UINT8 va_request_status;		/* #####0.35e */
...
typedef UINT32 va_program_number;		/* Program numbers */
...
typedef UINT32     va_soid;    /* Service Operator IDs */

typedef UINT32     va_token;   /* Money, tokens, credits */
...
/* PIN code */
typedef UINT8 va_pin_code[VA_PIN_CODE_LENGTH];


typedef struct          /* Dates */
{                       /* If a date is before 1/1/1980, */
	UINT16     year;   /* the via-access module transforms it in 1/1/1980. */
	UINT8      month;  /* If after 12/31/2107, */
	UINT8      day;    /* it transforms it in 12/31/2107... */
} va_date;
...
{
	UINT8      length; /* number of useful bytes in 'data' */
	UINT8      *data;
} va_generic_data;
...
typedef struct
{
	UINT8      preselection;
	UINT8      consultation;
	UINT8      maturity_rating;
	UINT8      impulse_purchase;
} va_lock;
...
typedef struct  /* VA_SOID_CLASS_SUBSCRIPTION_LIST_REQUEST */
{
	va_date_range           date;
	size_t                  class_length;
	UINT8                  custwd[VA_SOID_CUSTWD_MAX_LENGTH];
...
{
	UINT8          conversion_rate_numerator;
	UINT8          conversion_rate_denominator;
	UINT16         ippvt_base_rate;
...
typedef struct  /* VA_UA_REQUEST */
{
	va_common_request		c;
	size_t					ua_length;
	UINT8					ua[VA_UA_MAX_LENGTH];
} va_ua_request;

typedef struct  /* VA_MORALITY_LEVEL_REQUEST */
{
	va_common_request		c;
	UINT8					morality_level;
} va_morality_level_request;
...
typedef struct  /* VA_SOID_CONSULTATION_REQUEST */
{
	...
	va_soid					soid;
	UINT8					pi;
	UINT8					length_data_command;
	UINT8					data_command[VA_DATA_COMMAND_MAX_LENGTH];
	size_t					size;
	UINT8					*array;
} va_soid_consultation_request;
...
typedef struct  /* VA_INFO_QEV_REQUEST */
{
	...
	UINT8					qev_value[VA_QEV_VALUE_MAX_LENGTH];
} va_info_qev_request;
...
typedef enum
{
	VA_REQ_GOOD = 0,     /* OK, request has been registered */
...
typedef struct
{
   UINT8 type;								/* IL type for hardware type */
   UINT8 version;							/* IL type for hardware version */
} va_il_hard_t;
...
typedef struct
{
	...
	UINT8		value[VA_QEV_VALUE_MAX_LENGTH];
} va_qev_value_t;

typedef struct
{
	size_t		length;						/* terminal ID */
	UINT8		id[VA_QEV_TERMINAL_ID_MAX_LENGTH];
} va_terminal_id_t;
...
typedef UINT8* (*format_dial_fct_t) (	UINT8 *np, UINT8 *nc );
