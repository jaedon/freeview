...
HERROR xsvc_epg_Init_Hgs1000s(void)
{
	...
	stEpgCb.pfnEpgInitAction	=  xsvc_epg_InitAction_Tvtv;
	stEpgCb.pfnEpgStart		=  xsvc_epg_Start_Tvtv;
	stEpgCb.pfnEpgStop		= xsvc_epg_Stop_Tvtv;
	stEpgCb.pfnEpgStartContent	= xsvc_epg_StartContent_Base;
	stEpgCb.pfnEpgStopContent	= xsvc_epg_StopContent_Base;

	hErr = SVC_EPG_RegisterCallback(eEPG_KIND_TVTV, &stEpgCb);
...
HERROR xsvc_epg_InitResource_Hgs1000s(void)
{
	...
	{
		HxLOG_Print("[xsvc_epg_InitResource_Hgs1000s] EPGDB_CreateSvcSectionManager fail!!\n");
...
#if defined(CONFIG_MW_EPG_TVTV)
	// Raw Pool Init
	hError = SVC_EPGRAWMGR_InitResource();
	if(hError != ERR_EPG_OK)
	{
		HxLOG_Print("[xsvc_epg_InitResource_Hgs1000s] SVC_EPGDB_InitResource fail!!\n");
		goto EXIT_EPGMAIN_FUNC;
	}
#endif
