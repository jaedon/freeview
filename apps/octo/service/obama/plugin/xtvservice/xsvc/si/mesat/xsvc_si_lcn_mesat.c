...
HERROR xsvc_si_LcnSetServiceNumber_MeSat(Handle_t hAction, HUINT32 ulNum, DbSvc_Info_t *pstSvcArray, SRCHSVC_AppendFlag_t *peFlagArray)
{
	...
	else
	{
		...
		for (ulIndex = 0; ulIndex < ulNum; ulIndex++)
		{
			...
			if (DbSvc_GetSvcTypePtr(pstSvcInfo) != eDxSVC_TYPE_TV && DbSvc_GetSvcType(pstSvcInfo) != eDxSVC_TYPE_RADIO)
			{
				// Normal Numbering 시
