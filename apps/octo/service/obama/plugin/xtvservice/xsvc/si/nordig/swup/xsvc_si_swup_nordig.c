...
HERROR xsvc_si_SwupCheckGetSwupDownType_Nordig(HUINT8 *pucDownType)
{
	*pucDownType = s_ucSwupDownType;	//SvcSi_SWUP_TYPE_USER_SELECT;
	return ERR_OK;
}

HERROR xsvc_si_SwupCheckSetSwupDownType_Nordig(HUINT8 ucDownType)
{
...
HERROR xsvc_si_SwupCheckGetFilteringType_Nordig(xsvcSi_SwupData_t *pstSwupCheck)
{
...
STATIC HERROR xsvc_si_SwupCheckParseSelectorBytes_Nordig (HUINT8 *pucRaw, SvcSi_SwupSignal_t *pstSelector)
{
...
HERROR 	xsvc_si_SwupCheckFindProperOuiDataFromLinkDes_Nordig(SIxLinkDes_t *pstLinkDes)
{
	...
	for (ulSelectorIdxMulti = 0; ulSelectorIdxMulti < ulSelectorNum; ulSelectorIdxMulti++)
	{
		if (pstOuiSelectors[ulSelectorIdxMulti].ulOui == eSWUP_OUI_HUMAX || pstOuiSelectors[ulSelectorIdxMulti].ulOui == eSWUP_OUI_DVB)
		{
			...
			hErr = xsvc_si_SwupCheckParseSelectorBytes_Nordig(pucRaw, &stSwupInfo);
			if(hErr == ERR_OK)
			{
...
STATIC HERROR xsvc_si_SwupCheckFindLinkage_Nordig(svcSi_SwupCheck_Base_t *pstSwupCheckBase, SIxTable_Type_e eSiType, HUINT8 ucLinkType, HUINT32 *pulNumLink, DbSvc_TripleId_t **ppstTripleIdArray)
{
	HUINT32 				 ulActionId;
		...
		for (pstDesLoop = pstCurrPsi->pstDesLoop; pstDesLoop != NULL; pstDesLoop = pstDesLoop->next)
		{
			...
			if (*pucRawDes == eSIxDESCTAG_LINKAGE)
			{
				...
				if (pstLinkDes != NULL)
				{
					...
					{
						hErr = xsvc_si_SwupCheckFindProperOuiDataFromLinkDes_Nordig(pstLinkDes);
						if (hErr == ERR_OK)
						{
...
void xsvc_si_SwupCheckMsgEvtNitTable_Nordig (xsvcSi_SwupData_t *pstLiveSwupCheck, SvcSi_Msg_t *pstMsg)
{
	HERROR				 hErr;
	...
	if (pstMsg->unArg.stSection.ulTableStatus == eSI_TABLE_RECEIVED)
	{
		...
		hErr = xsvc_si_SwupCheckFindLinkage_Nordig(&pstLiveSwupCheck->stBase, eSTableType, eLINKAGE_ALL, &ulNumSsu, (DbSvc_TripleId_t**)&pstTripleIdArray);
		if(hErr == ERR_OK)
		{
		...
		{
			// NIT에서 Nordig Spec을 만족하지 않으면 user_select 로 설정한다..
			s_ucSwupDownType = SvcSi_SWUP_TYPE_USER_SELECT;
		}
	}
