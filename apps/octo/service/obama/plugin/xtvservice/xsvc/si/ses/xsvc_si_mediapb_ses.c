...
STATIC HERROR xsvc_si_MediaPbParseDtcpDesc_Ses(HUINT32 ulActionId, HUINT8 *pucRawDes)
{
	SIxSesDtcpDes_t	*pstDesc = NULL;
	...
	pstDesc = (SIxSesDtcpDes_t *)PAL_SIRAWPOOL_ParseDes (ulActionId, (HUINT8 *)pucRawDes);
	if (pstDesc == NULL)
	{
		HxLOG_Error("SIxSesDtcpDes_t parse failed..!!  \n");
		return ERR_FAIL;
	}
...
STATIC HERROR xsvc_si_MediaPbCheckDtcpFromPmt_Ses(HUINT32 ulActionId)
{
	...
	for (pstPmtLoop = pstPmtTable->pstDesLoop; NULL != pstPmtLoop; pstPmtLoop = pstPmtLoop->next)
	{
		...
		if (eSIxDESCTAG_SES_DTCP == *pucRawDes)
		{
			hErr = xsvc_si_MediaPbParseDtcpDesc_Ses(ulActionId, pucRawDes);
			if (hErr != ERR_OK)
			{
				HxLOG_Error("xsvc_si_MediaPbParseDtcpDesc_Ses() failed..!! \n");
			}
		}
	}
...
STATIC HERROR xsvc_si_MediapbOnPatReceived_Ses (svcSi_MediapbData_t *pstMediapb, Handle_t hAction, SvcSi_Arg_SecCb_t *pstSecCb)
{
...
STATIC HERROR xsvc_si_MediapbOnPmtReceived_Ses (svcSi_MediapbData_t *pstMediapb, Handle_t hAction, SvcSi_Arg_SecCb_t *pstSecCb)
{
	...
	default:
		...
		xsvc_si_MediaPbCheckDtcpFromPmt_Ses(PAL_PIPE_GetActionId(hAction));
		break;
	}
...
STATIC void xsvc_si_MediapbMsgCmdStartMediapb_Ses (svcSi_MediapbData_t *pstMediapb, SvcSi_Msg_t *pstMsg)
{
...
STATIC void xsvc_si_MediapbMsgCmdStopMediapb_Ses (svcSi_MediapbData_t *pstMediapb, SvcSi_Msg_t *pstMsg)
{
	(void)svc_si_MediapbStopSection (pstMediapb);
}

STATIC void xsvc_si_MediapbMsgEvtPatTable_Ses (svcSi_MediapbData_t *pstMediapb, SvcSi_Msg_t *pstMsg)
{
	(void)xsvc_si_MediapbOnPatReceived_Ses (pstMediapb, pstMsg->hAction, &(pstMsg->unArg.stSection));
}

STATIC void xsvc_si_MediapbMsgEvtPmtTable_Ses (svcSi_MediapbData_t *pstMediapb, SvcSi_Msg_t *pstMsg)
{
	(void)xsvc_si_MediapbOnPmtReceived_Ses (pstMediapb, pstMsg->hAction, &(pstMsg->unArg.stSection));
}

#define _____SI_MSG_PROCESS_FUNCTION_____
STATIC void xsvc_si_MediapbProcessMsg_Ses (SvcSi_Msg_t *pstMsg, void *pvData)
{
	...
	case eSIMSG_CMD_START_MEDIAPB:
		xsvc_si_MediapbMsgCmdStartMediapb_Ses (pstMediapb, pstMsg);
		break;

	case eSIMSG_CMD_STOP_MEDIAPB:
		xsvc_si_MediapbMsgCmdStopMediapb_Ses (pstMediapb, pstMsg);
		break;

	case eSIMSG_EVT_PAT_TABLE:
		xsvc_si_MediapbMsgEvtPatTable_Ses (pstMediapb, pstMsg);
		break;

	case eSIMSG_EVT_PMT_TABLE:
		xsvc_si_MediapbMsgEvtPmtTable_Ses (pstMediapb, pstMsg);
		break;
...
SvcSi_ActFunc xsvc_si_MediapbGetMediapbActionFunc_Ses (void)
{
#if defined (CONFIG_SUPPORT_ROVI)
	return xsvc_si_MediapbProcessMsg_Ses;
#else
	return NULL;
#endif
}

void *xsvc_si_MediapbAllocMediapbActionData_Ses (void)
{
#if defined (CONFIG_SUPPORT_ROVI)
...
void xsvc_si_MediapbFreeMediapbActionData_Ses (void *pvData)
{
#if defined (CONFIG_SUPPORT_ROVI)
	svcSi_MediapbData_t	*pstMediapb = (svcSi_MediapbData_t *)pvData;
