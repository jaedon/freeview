...
void	TRAPI_GetLog(const HCHAR *pszFile, const HCHAR *pszFunc, HUINT32 ulLine, HxLOG_Level_e eLev,HUINT32 ulErr_code, const HCHAR *pszFormat, ...)
{
	...
	pErIn = HLIB_MEM_Calloc(sizeof(TRAPI_ERIN_t));
	va_start(ap, pszFormat);
#ifdef CONFIG_DEBUG
	HLIB_LOG_v(pszFile, pszFunc, ulLine, eLev, pszFormat, ap);
#endif
	vsnprintf(pErIn->szLog,TRAPI_LOG_LEN,pszFormat,ap);
	va_end(ap);

	pErIn->ulErr_code	= ulErr_code;
	pErIn->ulLine 		= ulLine;
	pErIn->eLevel 		= eLev;
	pErIn->ulSystime	= HLIB_STD_GetSystemTime();
	HLIB_STD_StrNCpySafe(pErIn->szProcess,HLIB_STD_GetCurProcessName(),TRAPI_PROCESSNAME_LEN);
	HLIB_STD_StrNCpySafe(pErIn->szFilePath,pszFile,TRAPI_FILEPATH_LEN);
	HLIB_STD_StrNCpySafe(pErIn->szFunction,pszFunc,TRAPI_FUNC_LEN);

	trapi_rpc_erin_send(pErIn);
	HLIB_MEM_Free(pErIn);
}
