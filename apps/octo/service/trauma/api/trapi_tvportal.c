...
void 	TRAPI_GetAcceptPrivacy(HBOOL *pbAccept)
{
	HxLOG_Trace();
	*pbAccept = FALSE;
	if(HAPI_GetRegistryInt(TRAUMA_DB_ACCEPT_PRIVACY,pbAccept) ==ERR_FAIL)
	{
		*pbAccept = FALSE;
		HAPI_SetRegistryInt(TRAUMA_DB_ACCEPT_PRIVACY,FALSE);
	}
}
...
{
	HxLOG_Trace();
	trapi_rpc_acceptprivacy_send(bAccept);
}
