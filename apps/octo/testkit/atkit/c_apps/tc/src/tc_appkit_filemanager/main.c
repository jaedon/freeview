...
#define TC_FILEMANAGER_MANIFEST_FILE_NAME "/home/atkit/manifest/tc_appkit_filemanager.manifest"
#define TC_FILEMANAGER_RESULT_FILE_NAME "/home/atkit/result/tc_appkit_filemanager.result"
...
HINT32 local_tc_init(void)
{
	...
	TC_FILEMANAGER_StorageInit(s_manifest);
	TC_FILEMANAGER_DirectoryInit(s_manifest);
...
void local_tc_deInit(void)
{
	...
	TC_FILEMANAGER_StorageDeInit();
	TC_FILEMANAGER_DirectoryDeInit();
...
int main(int argc, char **argv)
{
	...
	printf("#####################################################################################\n");
	printf("Unit Test for Appkit FileManager validation!\n");
	printf("#####################################################################################\n");
	...
	s_manifest = HLIB_JSON_DecodeFile(TC_FILEMANAGER_MANIFEST_FILE_NAME);
	...
	hErr = HLIB_FILE_WriteFile((const HUINT8 *)TC_FILEMANAGER_RESULT_FILE_NAME, (void *)szResultText, ulResultLen);
	if(ERR_OK != hErr)
	{
		fprintf(stderr, "Fail to HLIB_FILE_WriteFile( %s ) - %d\n", TC_FILEMANAGER_RESULT_FILE_NAME, hErr);
		goto END_TC;
	}
