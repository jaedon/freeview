...
#define ATKIT_MENIFEST_FILE_NAME	"/home/atkit/manifest/tc_appkit_meta_recording.manifest"
#define ATKIT_RESULT_FILE_NAME		"/home/atkit/result/tc_appkit_meta_recording.result"
...
HINT32 local_tc_init(void)
{
	...
	TC_META_RECORDING_Init(s_manifest);
...
int main(int argc, char **argv)
{
	...
	printf("#####################################################################################\n");
	printf("Unit Test for Appkit validation!\n");
	printf("#####################################################################################\n");
	...
	s_manifest = HLIB_JSON_DecodeFile(ATKIT_MENIFEST_FILE_NAME);
	...
	hErr = HLIB_FILE_WriteFile((const HUINT8 *)ATKIT_RESULT_FILE_NAME, (void *)szResultText, ulResultLen);
	if(ERR_OK != hErr)
	{
		fprintf(stderr, "Fail to HLIB_FILE_WriteFile( %s ) - %d\n", ATKIT_RESULT_FILE_NAME, hErr);
		goto END_TC;
	}
