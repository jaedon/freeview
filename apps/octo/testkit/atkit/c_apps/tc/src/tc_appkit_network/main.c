...
#define TC_APPKIT_WIFI_MANIFEST_FILE_NAME "/home/atkit/manifest/tc_appkit_network.manifest"
#define TC_APPKIT_WIFI_RESULT_FILE_NAME "/home/atkit/result/tc_appkit_network.result"
...
HINT32 local_tc_init(void)
{
	...
	TC_WIFI_Init(s_manifest);
	TC_NETCONF_Init(s_manifest);
...
int main(int argc, char **argv)
{
	...
	printf("#####################################################################################\n");
	printf("Unit Test for Appkit Network validation!\n");
	printf("#####################################################################################\n");
	...
	s_manifest = HLIB_JSON_DecodeFile(TC_APPKIT_WIFI_MANIFEST_FILE_NAME);
	...
	hErr = HLIB_FILE_WriteFile((const HUINT8 *)TC_APPKIT_WIFI_RESULT_FILE_NAME, (void *)szResultText, ulResultLen);
	if(ERR_OK != hErr)
	{
		fprintf(stderr, "Fail to HLIB_FILE_WriteFile( %s ) - %d\n", TC_APPKIT_WIFI_RESULT_FILE_NAME, hErr);
		goto END_TC;
	}
