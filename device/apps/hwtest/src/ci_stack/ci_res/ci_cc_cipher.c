...
#define SHA_1_CHECK_BEGIN		VK_Print(" [->: %08X %08X %08X %08X %08X]\n", state[0], state[1], state[2], state[3], state[4]); SHA_1_ROUND_COUNT = 0;
#define SHA_1_CHECK			VK_Print(" [%02d: %08X %08X %08X %08X %08X]\n", SHA_1_ROUND_COUNT++, a, b, c, d, e);
#define SHA_1_CHECK_END		VK_Print(" [<-: %08X %08X %08X %08X %08X]\n", state[0], state[1], state[2], state[3], state[4]);
#else
...
#define SHA_256_CHECK_BEGIN		VK_Print(" [->: %08X %08X %08X %08X %08X %08X %08X %08X]\n", state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7]); SHA_256_ROUND_COUNT = 0;
#define SHA_256_CHECK			VK_Print(" [%02d: %08X %08X %08X %08X %08X %08X %08X %08X]\n", SHA_256_ROUND_COUNT++, a, b, c, d, e, f, g, h);
#define SHA_256_CHECK_END		VK_Print(" [<-: %08X %08X %08X %08X %08X %08X %08X %08X]\n", state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7]);
#else
...
static void SHA_Init(int x, SHA_CTX *context) /* Initialize new context */
{
	...
	{
		PrintErr(("[CICC]---> SHA_Init()---> Error : %d not supported !!!\n", x));
		return;
	}
...
static void SHA_Update(int x, SHA_CTX *context, unsigned char *data, unsigned long dataLen)
{
	...
	{
		PrintErr(("[CICC]---> SHA_Update()---> Error : %d not supported !!!\n", x));
		return;
	}
	...
	if ((numByteInBuffMod64 + dataLen) > 63)
	{
		...
		VK_memcpy(&context->buffer[numByteInBuffMod64], data, (numByteDataProcessed = 64 - numByteInBuffMod64));
	...
	VK_memcpy(&context->buffer[numByteInBuffMod64], &data[numByteDataProcessed], dataLen - numByteDataProcessed);
}
...
static void SHA_Final(int x, SHA_CTX *context, unsigned char *digest) /* Add padding and return the message digest. */
{
	...
	{
		PrintErr(("[CICC]---> SHA_Final()---> Error : %d not supported !!!\n", x));
		return;
	}
...
void SHA_x(int x, unsigned char *Msg, unsigned long msgLen, unsigned char *digest) /* Digest의 공간 크기는 SHA_x_DIGEST_SIZE 이어야 한다. */
{
	...
	if (x != 1 && x != 256)
	{
		PrintErr(("[CICC]---> SHA()---> Error : x(%d)---> must be 1 or 256 !!!\n", x));
		return;
	}
...
void SHA_Test(void)
{
	...
	if (VK_memcmp(sha_1_digest, result1, SHA_1_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	SHA_1(b, 56, sha_1_digest);
	if (VK_memcmp(sha_1_digest, result2, SHA_1_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	SHA_1(c, 43, sha_1_digest);
	if (VK_memcmp(sha_1_digest, result3, SHA_1_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	SHA_1(d, 44, sha_1_digest);
	if (VK_memcmp(sha_1_digest, result4, SHA_1_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	SHA_1(e, 0, sha_1_digest);
	if (VK_memcmp(sha_1_digest, result5, SHA_1_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	VK_Print("\n SHA_Test (SHA-1) End. \n");
	...
	if (VK_memcmp(sha_256_digest, result6, SHA_256_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	SHA_256(b, 56, sha_256_digest);
	if (VK_memcmp(sha_256_digest, result7, SHA_256_DIGEST_SIZE) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
...
static int I2OSP(bignum *x, bn_len_type xLen, bignum *X) /* converts a nonnegative integer to an octet string of a specified length */
{
	...
	if (x->len > xLen)
	{
		PrintErr(("[CICC]---> I2OSP()---> Error : integer too large !!!\n"));
		return -1;
	}

	X->len = xLen;
	VK_memset(X->num, 0x00, xLen - x->len);
	VK_memcpy(&X->num[xLen - x->len], x->num, x->len);
...
//	#define CHECK_RSAEP
	...
	if (bn_cmp(m, K->n) >= 0)
	{
		PrintErr(("[CICC]---> RSAEP()---> Error : message representative out of range !!!\n"));
		return -1;
	}
...
//	#define CHECK_RSADP
	...
	if (bn_cmp(c, K->n) >= 0)
	{
		PrintErr(("[CICC]---> RSADP()---> Error : ciphertext representative out of range !!!\n"));
		return -1;
	}
...
//	#define CHECK_RSASP1
	...
	if (bn_cmp(m, K->n) >= 0)
	{
		PrintErr(("[CICC]---> RSASP1()---> Error : message representative out of range !!!\n"));
		return -1;
	}
...
//	#define CHECK_RSAVP1
	...
	if (bn_cmp(s, K->n) >= 0)
	{
		PrintErr(("[CICC]---> RSAVP1()---> Error : signature representative out of range !!!\n"));
		return -1;
	}
	...
	*/
	...
	for (i = 0; outlen < maskLen; i++)
	{
		...
		else
		{
			...
			VK_memcpy(mask + outlen, digest, maskLen - outlen);
			outlen = maskLen;
		}
	}
...
//	#define CHECK_EMSA_PSS_ENCODE
	...
	VK_Print("EMSA_PSS_ENCODE : emLen = %d, emBits = %d\n", emLen, emBits);
#endif
...
#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : mHash = ");
	for (i = 0; i < SHA_1_DIGEST_SIZE; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
		VK_Print("%02x ", mHash[i]);
	}
	VK_Print("\n");
#endif
	...
	{
		PrintErr(("[CICC]---> EMSA_PSS_ENCODE()---> Error : emLen(%d) is less than hLen(%d) + sLen(%d) + 2 !!!\n", emLen, hLen, sLen));
		return -1;
	}

	// 4. Generate a random octet string salt of length sLen; if sLen = 0, then salt is the empty string.
	// -> I won't generate salt here. Use the given.

#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : salt = ");
	for (i = 0; i < sLen; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
		VK_Print("%02x ", salt->num[i]);
	}
	VK_Print("\n");
#endif
	...
	VK_memset(M_, 0x00, 8);
	VK_memcpy(&M_[8], mHash, hLen);
	VK_memcpy(&M_[8+hLen], salt->num, sLen);

#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : M' = ");
	...
	VK_memset(DB, 0x00, emLen - sLen - hLen - 2);
	DB[emLen - sLen - hLen - 2] = 0x01;
	VK_memcpy(&DB[emLen - sLen - hLen - 1], salt->num, sLen);

#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : DB = ");
...
#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : dbMask = ");
	for (i = 0; i < emLen - hLen - 1; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
		VK_Print("%02x ", dbMask[i]);
	}
	VK_Print("\n");
#endif
...
#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : maskedDB = ");
	for (i = 0; i < emLen - hLen - 1; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
	...
	// 13. Output EM.
	VK_memcpy(EM->num, DB, emLen - hLen - 1);
	VK_memcpy(&EM->num[emLen - hLen - 1], H, hLen);
	EM->num[emLen - 1] = 0xbc;

#ifdef CHECK_EMSA_PSS_ENCODE
	VK_Print("EMSA_PSS_ENCODE : EM = ");
	for (i = 0; i < emLen; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
		VK_Print("%02x ", EM->num[i]);
	}
	VK_Print("\n");
#endif
...
//	#define CHECK_EMSA_PSS_VERIFY
	...
	VK_Print("EMSA_PSS_VERIFY : emLen = %d, emBits = %d\n", emLen, emBits);
	VK_Print("EMSA_PSS_VERIFY : EM = ");
	for (i = 0; i < emLen; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
...
#ifdef CHECK_EMSA_PSS_VERIFY
	VK_Print("EMSA_PSS_VERIFY : mHash = ");
	for (i = 0; i < SHA_1_DIGEST_SIZE; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
	...
	if (emLen < hLen + sLen + 2)
	{
		PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : emLen(%d) is less than hLen(%d) + sLen(%d) + 2 !!!\n", emLen, hLen, sLen));
	...
	{
		PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : the rightmost octet of EM (0x%02x) does not have hexadecimal value 0xbc !!!\n", EM->num[emLen - 1]));
		return 0; // inconsistent
	}
...
#ifdef CHECK_EMSA_PSS_VERIFY
	VK_Print("EMSA_PSS_VERIFY : maskedDB = ");
	for (i = 0; i < emLen - hLen - 1; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
	...
	{
		PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : the leftmost octet in maskedDB (0x%02x) have wrong value !!!\n", EM->num[0]));
		return 0; // inconsistent
	}
...
#ifdef CHECK_EMSA_PSS_VERIFY
	VK_Print("EMSA_PSS_VERIFY : dbMask = ");
	for (i = 0; i < emLen - hLen - 1; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
	...
	for (i = 0; i < emLen - hLen - sLen - 2; i++)
	{
		if (DB[i] != 0x00)
		{
			PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : the leftmost octets of DB are not zero !!!\n"));
			return 0; // inconsistent
		}
	}
	if (DB[i] != 0x01)
	{
		PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : the octet at %d of DB (0x%02x) does not have hexadecimal value 0x01 !!!\n", i, DB[i]));
		return 0; // inconsistent
	}
	...
	VK_memset(M_, 0x00, 8);
	VK_memcpy(&M_[8], mHash, hLen);
	VK_memcpy(&M_[8+hLen], salt, sLen);

#ifdef CHECK_EMSA_PSS_VERIFY
	VK_Print("EMSA_PSS_VERIFY : M' = ");
	for (i = 0; i < 8 + hLen + sLen; i++)
	{
		if ((i & 15) == 0)
			VK_Print("\n");
	...
	if (VK_memcmp(H, H_, hLen))
	{
		PrintErr(("[CICC]---> EMSA_PSS_VERIFY()---> Error : hashes are not matched !!!\n"));
		return 0; // inconsistent
	}
...
static int EMSA_PKCS1_v1_5_ENCODE(bignum *M, bn_len_type emLen, bignum *EM) /* EMSA-PKCS1-v1_5-ENCODE function based on a hash function (SHA-1). */
{
...
int RSASSA_PSS_SIGN(RSA_private_key *K, bignum *M, bignum *salt, bignum *S) /* RSASSA-PSS-SIGN function */
{
	...
	*/
	...
	bn_len_type k = K->n->len, modBits, i;
	bignum EM, m, s;

	// 1. Apply the EMSA-PSS encoding operation to the message M to produce an encoded message EM of length, ceiling function((modBits-1)/8) octets
	//    such that the bit length of the integer OS2IP (EM) is at most modBits-1, where modBits is the length in bits of the RSA modulus n :
	...
	{
		PrintErr(("[CICC]---> RSASSA_PSS_SIGN()---> Error : EMSA_PSS_ENCODE !!!\n"));
		return -1;
	}
	...
	if (RSASP1(K, &m, &s) < 0)
	{
		PrintErr(("[CICC]---> RSASSA_PSS_SIGN()---> Error : message representative out of range !!!\n"));
		return -1;
	}
	...
	if (I2OSP(&s, k, S) < 0)
	{
		PrintErr(("[CICC]---> RSASSA_PSS_SIGN()---> Error : integer too large !!!\n"));
		return -1;
	}
...
int RSASSA_PSS_VERIFY(RSA_public_key *K, bignum *M, bignum *S) /* RSASSA-PSS-VERIFY function : return 1 if valid signature, 0 if invalid signature */
{
	...
	*/
	...
	bn_len_type k = K->n->len, modBits, emLen, i;
	bignum s, m, EM;

	// 1. Length checking : If the length of the signature S is not k octets, output "invalid signature" and stop.
	if (S->len != k)
	{
		PrintErr(("[CICC]---> RSASSA_PSS_VERIFY()---> Error : the length of the signature (%d) is not %d octets !!!\n", S->len, k));
	...
	if (RSAVP1(K, &s, &m) < 0)
	{
		PrintErr(("[CICC]---> RSASSA_PSS_VERIFY()---> Error : signature representative out of range !!!\n"));
		return 0; // invalid signature
	}

	// c. Convert the message representative m to an encoded message EM of length emLen = ceiling function((modBits-1)/8) octets,
	...
	{
		PrintErr(("[CICC]---> RSASSA_PSS_VERIFY()---> Error : integer too large !!!\n"));
		return 0; // invalid signature
	}

	// 3. Apply the EMSA-PSS verification operation to the message M and the encoded message EM to determine whether they are consistent : Result = EMSA-PSS-VERIFY (M, EM, modBits-1)
...
static void RSA_Test1(void)
{
	...
	// test 1 : 1024 bits RSA Key Pair 에 대해서 sign & verify
	VK_Print("\n RSA_Test (test 1) Start... \n");
	if (A_RSA(&PrivateKey1, &M1, &salt1, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
		for (i = 0; i < Sig.len; i++)
		{
			if ((i & 15) == 0)
				VK_Print("\n");
			VK_Print(" %02x", Sig.num[i]);
		}
		VK_Print("\n");
		if ((Sig.len == S1.len) && (VK_memcmp(Sig.num, S1.num, Sig.len) == 0))
			VK_Print(" valid signature generated \n"); // should be
		else
			VK_Print(" invalid signature !!!\n");
	}
	if (V_RSA(&PublicKey1, &M1, &S1))
		VK_Print(" valid signature verified \n"); // should be
	else
		VK_Print(" invalid signature !!!\n");
	VK_Print(" RSA_Test (test 1) End. \n");

	// test 2 : 1024 bits RSA Key Pair 에 대해서 sign (private key 두 가지 유형에 대해 각기) & verify
	VK_Print("\n RSA_Test (test 2) Start... \n");
#ifdef 	INCLUDE_1ST_FORM_TEST
	if (A_RSA(&PrivateKey2_1stForm, &M2, &salt2, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
		...
		VK_Print("\n");
		if ((Sig.len == S2.len) && (VK_memcmp(Sig.num, S2.num, Sig.len) == 0))
			VK_Print(" valid signature generated \n"); // should be
		else
			VK_Print(" invalid signature !!!\n");
	}
#endif
	if (A_RSA(&PrivateKey2_2ndForm, &M2, &salt2, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
	...
	if (V_RSA(&PublicKey2, &M2, &S2))
		VK_Print(" valid signature verified \n"); // should be
	else
		VK_Print(" invalid signature !!!\n");
	VK_Print(" RSA_Test (test 2) End. \n");

	// test 3 : 1031 bits RSA Key Pair 에 대해서 sign (private key 두 가지 유형에 대해 각기) & verify
	VK_Print("\n RSA_Test (test 3) Start... \n");
#ifdef 	INCLUDE_1ST_FORM_TEST
	if (A_RSA(&PrivateKey3_1stForm, &M3, &salt3, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
		...
		VK_Print("\n");
		if ((Sig.len == S3.len) && (VK_memcmp(Sig.num, S3.num, Sig.len) == 0))
			VK_Print(" valid signature generated \n"); // should be
		else
			VK_Print(" invalid signature !!!\n");
	}
#endif
	if (A_RSA(&PrivateKey3_2ndForm, &M3, &salt3, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
	...
	if (V_RSA(&PublicKey3, &M3, &S3))
		VK_Print(" valid signature verified \n"); // should be
	else
		VK_Print(" invalid signature !!!\n");
	VK_Print(" RSA_Test (test 3) End. \n");
}
...
static void RSA_Test2(void)
{
	...
	// test 4 : 2048 bits RSA Key Pair 에 대해서 sign (private key 두 가지 유형에 대해 각기) & verify
	VK_Print("\n RSA_Test (test 4) Start... \n");
#ifdef 	INCLUDE_1ST_FORM_TEST
	if (A_RSA(&PrivateKey4_1stForm, &M4, &salt4, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
		...
		VK_Print("\n");
		if ((Sig.len == S4.len) && (VK_memcmp(Sig.num, S4.num, Sig.len) == 0))
			VK_Print(" valid signature generated \n"); // should be
		else
			VK_Print(" invalid signature !!!\n");
	}
#endif
	if (A_RSA(&PrivateKey4_2ndForm, &M4, &salt4, &Sig))
		VK_Print(" ERROR !!!\n");
	else
	{
		VK_Print(" Signature :");
	...
	if (V_RSA(&PublicKey4, &M4, &S4))
		VK_Print(" valid signature verified \n"); // should be
	else
		VK_Print(" invalid signature !!!\n");
	VK_Print(" RSA_Test (test 4) End. \n");
}
...
unsigned short GetAlignedLen(unsigned short usOrgLen)
{
	if (usOrgLen == 0)
	{
//		PrintErr(("[CICC]---> GetAlignedLen()---> Error : usOrgLen is zero\n"));
		return CI_CC_CIPHER_BLOCK_SIZE;
	}

	if ((usOrgLen & CI_CC_CIPHER_BLOCK_SIZE_1) == 0) // if (usOrgLen % CI_CC_CIPHER_BLOCK_SIZE == 0)
	{
//		PrintErr(("[CICC]---> GetAlignedLen()---> Error : usOrgLen(%d)---> already aligned\n", usOrgLen));
		return usOrgLen;
	}
	...
	/* 입력 데이터의 공간 크기는 CI_CC_CIPHER_BLOCK_SIZE 의 배수이어야 한다. see GetAlignedLen(). */
{
	...
	if (usOrgLen == 0)
	{
//		PrintErr(("[CICC]---> OneAndZeroesPadding()---> Error : usOrgLen is zero\n"));
		pucData[0] = 0x80;
		VK_memset(&pucData[1], 0x00, CI_CC_CIPHER_BLOCK_SIZE-1);
		return;
	}

	if ((usOrgLen & CI_CC_CIPHER_BLOCK_SIZE_1) == 0) // if (usOrgLen % CI_CC_CIPHER_BLOCK_SIZE == 0)
	{
//		PrintErr(("[CICC]---> OneAndZeroesPadding()---> Error : usOrgLen(%d)---> no padding required\n", usOrgLen));
		return;
	}
	...
	VK_memset(&pucData[usOrgLen+1], 0x00, usPaddingLen-1);
}
	...
	/* Key 길이는 Nk*4 이어야 한다. PlainText, CipherText 길이는 CI_CC_CIPHER_BLOCK_SIZE 이어야 한다. */
{
	...
	{
		PrintErr(("[CICC]---> E_AES_x()---> Error : x(%d)---> must be 128 or 192 or 256 !!!\n", x));
		return;
	}
	...
	/* Key 길이는 Nk*4 이어야 한다. PlainText, CipherText 길이는 CI_CC_CIPHER_BLOCK_SIZE 이어야 한다. */
{
	if (x != 128 && x != 192 && x != 256)
	{
		PrintErr(("[CICC]---> D_AES_x()---> Error : x(%d)---> must be 128 or 192 or 256 !!!\n", x));
		return;
	}
	...
	/* Key 길이는 Nk*4 이어야 한다. PlainText, CipherText 길이(TextLen값)는 CI_CC_CIPHER_BLOCK_SIZE 의 배수이어야 한다. see OneAndZeroesPadding(). */
{
	...
	{
		PrintErr(("[CICC]---> E_AES_x_CBC()---> Error : TextLen(%d)---> padding required !!!\n", TextLen));
		return;
	}

	if (x != 128 && x != 192 && x != 256)
	{
		PrintErr(("[CICC]---> E_AES_x_CBC()---> Error : x(%d)---> must be 128 or 192 or 256 !!!\n", x));
		return;
	}
	...
	/* Key 길이는 Nk*4 이어야 한다. PlainText, CipherText 길이(TextLen값)는 CI_CC_CIPHER_BLOCK_SIZE 의 배수이어야 한다. see OneAndZeroesPadding(). */
{
	...
	if (TextLen == 0 || TextLen & CI_CC_CIPHER_BLOCK_SIZE_1) // if (TextLen == 0 || TextLen % CI_CC_CIPHER_BLOCK_SIZE)
	{
		PrintErr(("[CICC]---> D_AES_x_CBC()---> Error : TextLen(%d)---> padding required !!!\n", TextLen));
		return;
	}

	if (x != 128 && x != 192 && x != 256)
	{
		PrintErr(("[CICC]---> D_AES_x_CBC()---> Error : x(%d)---> must be 128 or 192 or 256 !!!\n", x));
		return;
	}
...
{
	unsigned short usAlignedLen, n, i = 0, j;
	...
	pucBuf = (unsigned char *)CA_MEM_Alloc(sizeof(unsigned char) * usAlignedLen);
	VK_memcpy(pucBuf, M, Len);
...
static void AES_ECB_Test(void)
{
	...
	if (VK_memcmp(AES_Test_ResultText, AES_Test_CipherText, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	...
	if (VK_memcmp(AES_Test_ResultText, AES_Test_PlainText, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
...
static void AES_CBC_Test(void)
{
	...
	if (VK_memcmp(AES_Test_ResultText, AES_Test_CipherText, 4*16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
	...
	if (VK_memcmp(AES_Test_ResultText, AES_Test_PlainText, 4*16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
...
static void AES_XCBC_MAC_Test(void)
{
	...
	if (VK_memcmp(MAC, MAC1, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M2, 3, MAC);
	if (VK_memcmp(MAC, MAC2, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M3, 16, MAC);
	if (VK_memcmp(MAC, MAC3, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M4, 20, MAC);
	if (VK_memcmp(MAC, MAC4, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M5, 32, MAC);
	if (VK_memcmp(MAC, MAC5, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M6, 34, MAC);
	if (VK_memcmp(MAC, MAC6, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");

	MAC_AES_XCBC(Key, M7, 1000, MAC);
	if (VK_memcmp(MAC, MAC7, 16) == 0)
		VK_Print(" OK"); // should be
	else
		VK_Print(" NOT OK !!!");
...
void PRNG_Test(void)
{
	...
	VK_Print(" PRNG_Test (Counter) End. \n");
