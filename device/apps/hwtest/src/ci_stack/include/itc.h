...
/* 수정 내력 :
   modified by thyou : 10 -> 50 -> 100
   kimjh2, hmm, if 100 msec transport layer timeout occurs!! : 100 -> 50
   hmkim, 2009-06-17 : TM_POLLING 400 -> 100, ITC_TIMEOUT 50 -> 20 으로 변경 (부팅에서 디스크램블링까지 2초 단축) --> Douglas 의 경우 원래 값 다시 적용함. */
#ifdef CONFIG_DOUGLAS
#define ITC_TIMEOUT	50	/* H/W ECO 안된? Douglas 플랫폼에선 아래 변경된 값 설정하면 간혹 부팅 후 CI+ 캠 초기화시 SAC 에러 내지는 full authentication 시 에러 발생함. 하여 Douglas 의 경우 원래 값 다시 적용함. */
#else
