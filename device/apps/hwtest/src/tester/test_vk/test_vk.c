...
#define VALID_TEST_UNIXTIME	(60 * 60 * 24 * 366)	// until 1971/01/02
...
static int test_vk_clock_time(void)
{
	...
	if (unix_time == VALID_TEST_UNIXTIME)
	{
...
static int test_vk_sem_timeout(void)
{
	...
	for (i=0; i<50; i++)
	{
		...
		{
			printf("%s(%4d) : VK_SEM_GetTimeout Pass , %d\n", __func__, __LINE__, timeouttest, tick2-tick1);
		}
		else if (tick2-tick1 < timeouttest)
		{
			printf("%s(%4d) : VK_SEM_GetTimeout Error, wanted timeout %3d, real timeout %3d\n", __func__, __LINE__, timeouttest, tick2-tick1);
			test_sem_passed = FALSE;
		}
		else
		{
			printf("%s(%4d) : VK_SEM_GetTimeout OK   , wanted timeout %3d, real timeout %3d\n", __func__, __LINE__, timeouttest, tick2-tick1);
		}
...
static int test_vk_task_join(void)
{
	...
	errCode = VK_TASK_Create(thread_vk_task_join, USER_PRIORITY0, NORMAL_TASK_STACK_SIZE, "thread_vk_task_join", NULL, &test_vk_task_id, FALSE);
	if(errCode != VK_OK)
	{
	    PrintError("%s(%d) : VK_TASK_Create Error!!\n", __func__, __LINE__);
	    goto __error_return_label;
...
static unsigned long s_tread_task_create_destory_id;
void thread_task_create_destory(void  *arg)
{
	...
	// destory task by self.
	errCode = VK_TASK_Destroy(s_tread_task_create_destory_id);
	if(errCode != VK_OK)
	{
...
static int test_vk_task_create_destory(void)
{
	...
	errCode = VK_TASK_Create(thread_task_create_destory, USER_PRIORITY0, NORMAL_TASK_STACK_SIZE, "thread_task_create_destory", (void *)&i, &s_tread_task_create_destory_id, FALSE);
	if(errCode != VK_OK)
	{
		PrintError("%s(%d) : VK_TASK_Create Error!!\n", __func__, __LINE__);
		return 2;
	...
	errCode = VK_TASK_Start(s_tread_task_create_destory_id);
	if(errCode != VK_OK)
	{
		PrintError("%s(%d) : VK_TASK_Start Error!!\n", __func__, __LINE__);
		return 1;
...
HUINT32 eventTask1;
HUINT32 sendEvent;
HUINT32 recieveTask0;
HUINT32 recieveTask1;
...
void thread_vk_task_send_receive1(void *arg)
{
	...
	ret = VK_EVENT_Receive(eventTask0, &recieveTask0);
	if(ret != VK_OK)
	{
		printf("%s(%d) : error in VK_EVENT_Receive\n", __func__, __LINE__);
		return;
	}
	...
	VK_EVENT_Send( eventTask1, sendEvent );
	...
	ret = VK_EVENT_Send(eventTask1,sendEvent);
}
...
void thread_vk_task_send_receive2(void *arg)
{
	...
	ret = VK_EVENT_Receive(eventTask1,&recieveTask1) ;
	if(ret != VK_OK)
	{
		printf("%s(%d) : error in VK_EVENT_Receive\n", __func__, __LINE__);
		return;
	...
	VK_EVENT_Send(eventTask0,sendEvent);
...
static int test_vk_msg_send_receive(void)
{
	...
	ret = VK_EVENT_Create(&eventTask0, "event_task0");
	if(ret != VK_OK)
	{
	...
	ret = VK_EVENT_Create(&eventTask1, "event_task1");
	if(ret != VK_OK)
	{
		PrintError("%s[%d] : VK_EVENT_Create() error\n",__func__,__LINE__);
		goto __error_return_label;
	...
	ret = VK_EVENT_Destroy(eventTask0);
	ret = VK_EVENT_Destroy(eventTask1);
