...
#define CI_ITC_TIMER_PERIOD			100 // modified by thyou.2007.10.12    10 --> 100
                                                      //For EMMA2, 1 tick = 10 ms
...
***************************************************************/
extern void physical(void *);				/* Entry point of PHYSICAL Task				*/
extern void link(void*);					/* Entry point of LINK Task					*/
extern void transport(void*);			/* Entry point of TRANS Task				*/
extern void session(void*);				/* Entry point of SESS Task					*/
...
void ItcKernelInit( void )
{
...
#ifdef	USE_CI_FLASH_SEM
	if (VK_SEM_Create(&s_hCiAccess,"ciaccess", VK_SUSPENDTYPE_PRIORITY) != VK_OK)
	{
		PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_SEM_Create() !!!\n"));
		return;
	}
#endif
	...
	VK_memset(s_CiItcTimer, 0, sizeof(CI_ITC_TIMER) * CI_MAX_OF_TIMER);

	/* initialize layer info handles.. parkbi add.. 020819 */
	VK_memset(s_CiItcLayerInfo, 0, sizeof(CI_ITC_LAYER_INFO) * NUM_OF_CI_ITC_LAYER);
	...
	for (i=0; i<NUM_OF_CI_ITC_LAYER; i++)
	{
		...
		{
			PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_MSG_Create() !!!\n"));
	...
	{
		PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_TASK_Create(itc_TimerMgr) !!!\n"));
	...
	{
		PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_TASK_Start(itc_TimerMgr) !!!\n"));
		// to be added deleting task
		return;
	}
	...
	for (i=0; i<NUM_OF_CI_ITC_LAYER; i++)
	{
		...
		{
			PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_TASK_Create(%s) !!!\n", s_CiTaskName[i]));
			// to be added deleting task
			return;
		}
	}
	...
	for (i=0; i<NUM_OF_CI_ITC_LAYER; i++)
	{
		...
		{
			PrintErr(("[CIITC]---> ItcKernelInit()---> Error : VK_TASK_Start(%s) !!!\n", s_CiTaskName[i]));
			// to be added deleting task
			return;
		}
	}
...
#endif
{
	...
	{
		PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : parameter error !!!\n"));
		return 0;
	}
	...
	if (hTask == 0)
	{
		PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : VK_TASK_GetCurrentId() !!!\n"));
		return 0;
	}
#else
	...
	{
		PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : itc_GetMsgHandle() !!!\n"));
		return 0;
	}
	...
	if (timeOut == ITC_INFINITE)
	{
		...
		if (nRet != VK_OK)
		{
			PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : VK_MSG_Receive() !!!\n"));
			return 0;
		}
	}
	else
	{
		...
		if (nRet != VK_OK)
		{
			...
			PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : VK_MSG_ReceiveTimeout() !!!\n"));
			return 0;
		}
	}
	...
	if (nRet == VK_OK)
	{
		if (ulMsg[0] == MESSAGE)
		{
			...
			*trans = (unsigned char)ulMsg[1];
			VK_memcpy((unsigned char *)infos, (unsigned char *)ulMsg[2], sizeof(xmsg));
			CA_MEM_Free((unsigned char *)ulMsg[2]);
		}
		else if (ulMsg[0] == TIMER)
		{
			...
			*trans = 0;
			...
			PrintErr(("[CIITC]---> ITC_WaitEvt()---> Error : wrong event !!!\n"));
	}
...
int ITC_MsgSend(unsigned char trans, unsigned char *txmsg )
{
	...
	if(txmsg == NULL)
	{
		PrintErr(("[CIITC]---> ITC_MsgSend()---> Error : parameter error !!!\n"));
		return 0;
	}
	...
	ulMsg[0] = MESSAGE;
	ulMsg[1] = trans;
	ulMsg[2] = (unsigned long)CA_MEM_Alloc(sizeof(xmsg));

	if(ulMsg[2] == (unsigned long)NULL)
	{
		PrintErr(("[CIITC]---> ITC_MsgSend(%d)---> Error : CA_MEM_Alloc() !!!\n", ucLayerId));
		return 0;
	}
	VK_memcpy((unsigned char *)ulMsg[2], txmsg, sizeof(xmsg));

#if 0
{
	xmsg *pMsg;

//	PrintDbg(("ITC(%d):MsgSend(%02x)\n", ucLayerId, trans));
	pMsg = (xmsg *)txmsg;
	PrintDbg(("\t[HEADER]:layerid[%d],trans[0x%x],type[0x%x],len[0x%x]\n",
				ucLayerId,
	...
	/* Transport Layer */
	case TP_CONNECT:
		PrintDbg(("\t[TP_CONNECT]:slot[%d]\n",
					pMsg->message.body.conn.slot_id));
		break;
	case TP_DISCONNECT:
		PrintDbg(("\t[TP_DISCONNECT]:slot[%d]\n",
					pMsg->message.body.deconn.slot_id));
		break;
	case TPDU:
		PrintDbg(("\t[TPDU]:slot[%d],tcid[0x%x],len[0x%x]\n",
	...
	/* Session Layer */
	case SS_TC_CREATED:
		PrintDbg(("\t[SS_TC_CREATED]:tcid[0x%x]\n",
					pMsg->message.body.creat.tcid));
		break;
	case SS_DELETE_TC:
		PrintDbg(("\t[SS_DELETE_TC]:tcid[0x%x]\n",
					pMsg->message.body.del.tcid));
		break;
	case SS_TC_DELETED:
		PrintDbg(("\t[SS_TC_DELETED]:tcid[0x%x]\n",
					pMsg->message.body.deleted.tcid));
		break;
	case SPDU:
		PrintDbg(("\t[SPDU]:tcid[0x%x],len[0x%x]\n",
	...
	{
		PrintErr(("[CIITC]---> ITC_MsgSend(%d)---> Error : VK_MSG_Send() !!!\n", ucLayerId));
		return 0;
	}
...
#endif
{
	...
	if (pTimer == NULL)
	{
		...
		PrintErr(("[CIITC]---> ITC_SetTimer()---> Error : timerId[0x%x] !!!\n", ident));
		return 0;
	}
	...
	{
		VK_SEM_Release(s_hCiTimerMgrSem);
		PrintErr(("[CIITC]---> ITC_SetTimer()---> Error : VK_TASK_GetCurrentId() !!!\n"));
		return 0;
	}
#else
...
int ITC_KillTimer(int ident)
{
	...
	if (pTimer == NULL)
	{
		PrintErr(("[CIITC]---> ITC_KillTimer()---> Error : timerId[0x%x] !!!\n", ident));
		VK_SEM_Release(s_hCiTimerMgrSem);
		return 0;
	}
...
{
	return (unsigned char *)CA_MEM_Alloc(size);
}

unsigned char * ITC_DynReAlloc(unsigned char * pt, unsigned long size)
{
	return CA_MEM_Realloc((void *)pt, size);
}
...
void CiItc_PrintTimerInfo(void)
{
	...
	for(i=0 ; i < CI_MAX_OF_TIMER; i++)
	{
		PrintErr(("s_CiItcTimer[%2d]::nTimerId:%d[0x%x], hMsg:0x%x, unTime:%d, unTimeOut:%d\n",
...
void CiItc_PrintLayerInfo(void)
{
	...
	for(i=0 ; i < NUM_OF_CI_ITC_LAYER; i++)
	{
		PrintErr(("s_CiItcLayerInfo[%d]::hTask:0x%x, hMsg:0x%x\n",
