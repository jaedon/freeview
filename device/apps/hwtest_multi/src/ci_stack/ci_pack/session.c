...
unsigned char trans;	/* sender/receiver of received event */
	...
	while(1)		/* infinite loop */
	{
...
#endif
		{
			PrintErr(("session : ITC_WaitEvt . Error in return code !!!\n"));
		...
		{
			PrintDbg(("session : ITC_WaitEvt . MESSAGE or TIMER\n"));
		...
		{
			PrintErr(("session : ITC_WaitEvt . Error in received event !!!\n"));
...
unsigned char *pt;	/* pointer to allocated memory */
	...
	if (s_evt == MESSAGE)
	{
		switch (s_header.type) {
			...
			default :			/* unknown message */
				PrintErr(("SESS_State : Unknown message is ignored !!!\n"));
				break;
		}
	}
	else	/* TIMER */
	{
		...
		{
			PrintErr(("SESS_State : Unknown timer id is ignored 0x%x !!!\n", s_header.type));
			iRet = 1;
		}
	}
...
int iRet;				/* return code 			 							*/
	...
	{	/* decoding error : the event is ignored */
		...
		PrintErr(("SESS_Spdu : Error while decoding a SPDU tag=0x%x ssnb=%d iRet=%ld !!!\n", tag, ssnb, len));
		return 0;
	}
	...
	if (tag != open_session_request)
	{
		...
		{	/* not found : the event is ignored */
    		/* Send IHM_WARNING to IHM Task 	*/
			PrintErr(("SESS_Spdu : Unknown ssnb %d !!!\n", ssnb));
	...
	else
	{
		...
		{
    		/* Send IHM_WARNING to IHM Task */
			PrintErr(("SESS_Spdu : No more available ssnb !!!\n"));
...
int index;	/* index of s_tab_ssnb[] for the current transport connection  	*/
	...
	PrintDbg(("SESS_Timeout : index=%d, state=%d !!!\n", index, s_tab_ssnb[index].state));
...
//			s_tab_ssnb[index].ssnb 	   = (index*2) + 1;	/* odd session number : 0 is reserved */
			...
			PrintDbg(("SESS_GetIndex : new entry %d ssnb=%d\n", index, (index*2)+1));
			return index;
		}
