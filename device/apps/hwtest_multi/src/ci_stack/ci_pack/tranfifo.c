...
int TRPT_PutFifo(int index, unsigned char tag, st_spdu *pt)
{
	...
	if (tag == SPDU)
		PrintDbg(("TRPT_PutFifo : %d element(s) SPDU %d %ld 0x%p\n", t_tab_fifo[index].entry, pt->tcid, pt->len, pt->pspdu));
	else
		PrintDbg(("TRPT_PutFifo : %d element(s) SS_DELETE_TC\n", t_tab_fifo[index].entry));
...
unsigned char TRPT_GetFifo(int index, st_spdu *pt)
{
	...
	if (tag == SPDU)
		PrintDbg(("TRPT_GetFifo : %d element(s) SPDU %d %ld 0x%p\n", t_tab_fifo[index].entry, pt->tcid, pt->len, pt->pspdu));
	else
		PrintDbg(("TRPT_GetFifo : %d element(s) SS_DELETE_TC\n", t_tab_fifo[index].entry));
