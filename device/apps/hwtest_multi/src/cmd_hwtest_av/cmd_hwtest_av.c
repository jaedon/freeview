...
void P_HdmiCecCallBack(void* pParam)
{
    ...
    HI_VK_MEM_Memcpy((void *)&s_HDMICecCaMsg[0], 0x00, 100);
...
int	P_UsbCallBack(HI_USB_Event_t etUsbEventType, void *pucUsbInfo)
{
	...
	case HI_USB_Event_Insert :
		...
		if (pUsbInfo->devCount > 0)
		{
			memset(&part, 0, sizeof(part));
			sprintf(s_UsbDevName, "/dev/%s", pUsbInfo->devName[0]);
			part.deviceName = s_UsbDevName;
			if (P_FdiskList(&part))
			{
			...
			switch(part.partition[0].sys_ind)
			{
				...
				case _FS_TYPE_HID_FAT32_LBA:
					...
					filteropts = "iocharset=utf8";			// UTF-8 support
					break;
				...
				case _FS_TYPE_LINUX:
					...
					break;
				default:
					P_SendNewMsgStringToHandler("USB device mount FAIL : unsupported FS");
					return -1;
			}

			sprintf(s_UsbDevName, "/dev/%s%i", pUsbInfo->devName[0], 1);
			r = mount(s_UsbDevName, "/media/", filesystemtype, MS_SYNCHRONOUS, filteropts);
			if (-1 == r && EINVAL == errno && _FS_TYPE_LINUX == part.partition[0].sys_ind)
			{
				r = mount(s_UsbDevName, "/media/", "ext2", 0, filteropts);
			}
			if(-1 == r)
			{
				P_SendNewMsgStringToHandler("USB device mount FAIL");
		...
		{
			P_SendNewMsgStringToHandler("No USB device INFO");
			return -1;
		}
		break;
...
int	P_HddCallBack(HI_HDD_Event_t etHddEventType,void *pucHddInfo)
{
	...
	case HI_HDD_Event_Pwron :
		sprintf(caMsg, "HDD device (%s) is Connected\n", pHddInfo->devName);
		P_SendNewMsgStringToHandler(caMsg);
		if (pHddInfo->devCount > 0)
		{
			sprintf(s_HddDevName, "/dev/%s%d", pHddInfo->devName, 2);
			r = mount(s_HddDevName, "/mnt/hd2/", "ext3", 0, NULL);
			if (-1 == r && EINVAL == errno)
			{
			...
			if(-1 == r)
			{
				P_SendNewMsgStringToHandler("HDD mount FAIL");
				return -1;
			}
		}
		else
		{
			P_SendNewMsgStringToHandler("No HDD INFO");
			return -1;
		}
		break;
...
void P_PvrStartEx(unsigned long tunerId, unsigned int isStop, unsigned int isUSB)
{
    ...
    else if ( isStop == 0 && isUSB == 1)
    {
        ...
        FILE *fpPid = fopen("/mnt/hd2/hdd_thermal_stream.info", "r+b");
		if(fpPid == NULL)
		{
...
static void P_MenuHandler_FlashErase(void *pMenu)
{
	...
	P_HWTEST_Send_CMD_ToTask("flash e 0x1E00000 0x200000 0");
}
static void P_MenuHandler_FlashWrite(void *pMenu)
{
    if ( pMenu != NULL )
	...
	P_HWTEST_Send_CMD_ToTask("flash w 0x1E00000 0xAA 0x200000 0");
}
static void P_MenuHandler_FlashRead(void *pMenu)
{
    if ( pMenu != NULL )
	...
	P_HWTEST_Send_CMD_ToTask("flash r 0x1E00000 0x200000 0");
}
static void P_MenuHandler_FlashVerify(void *pMenu)
{
    if ( pMenu != NULL )
	...
	P_HWTEST_Send_CMD_ToTask("flash v 0x1E00000 0xAA 0x200000 0;flash w 0x1E00000 0xFF 0x200000 0");
}
static void P_MenuHandler_FlashStressTestOn(void *pMenu)
{
    if ( pMenu != NULL )
...
static void P_MenuHandler_PVRPlaySwitch(void *pMenu)
{
	...
	P_HWTEST_Send_CMD_ToTask("pvr testhdd 3");
}
...
int main(void)
{
	...
	key_t key_id;
	struct msgbuf mybuf;
    extern void CI_HWTEST_Init();

	DI_Init();

	key_id = msgget(1279, IPC_CREAT|0666);
	if(key_id < 0)
	{
	...
	{
		printf("[%d]%s create message queue key_id=%d\n",__LINE__,__func__,key_id);
	}
		...
		if ((unsigned char)ucByte0==(unsigned char)(~ucByte1))
		{
			...
			else
			{
				...
				g_CurLockSetting[stHwTestMenu[ID_CHANNEL].iHighlightedItem].ucAudioCodec = pChLocking->ucAudioCodec;
    				...
    				/*if ( (ucByte0 >= 0 ) && ( ucByte0 <= 100 ) && ( (ucByte0 % 10) ==0 ) )*/
       				if ( ( ucByte0 <= 100 ) && ( (ucByte0 % 10) ==0 ) )
	    			{
...
///////////////////////////////////////////////////////////////////////
	...
	while(1)
	{
		if(msgrcv(key_id, (void *)&mybuf, sizeof(struct msgbuf), 4, 0) == -1)
		{
			printf("\n");
		}
		else
		{
			s_ulRcuKeyCode=mybuf.key;
		}
	}
