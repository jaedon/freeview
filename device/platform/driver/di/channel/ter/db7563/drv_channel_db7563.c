...
#define FIRST_WAIT_TIME			(450)
#define MAX_REACQUIRE_COUNTS 	(10)//(2)//(10)//(4)
#define WAIT_OFDM_LOCKING		(150)//(150)//(300)//(50)//(100)

#define FIX_MANUAL_TPS
...
static NEXUS_FrontendOfdmSettings s_stOfdmSettings[CONFIG_NUM_OF_TER_CHANNEL];
static NEXUS_FrontendHandle s_hFrontend[CONFIG_NUM_OF_TER_CHANNEL] = {NULL,};
static BKNI_EventHandle s_evStatusEvent[CONFIG_NUM_OF_TER_CHANNEL];

static HUINT16 s_usCellId[CONFIG_NUM_OF_TER_CHANNEL] = {0};
...
static void lock_callback(void *context, int param)
{
...
#endif
	CH_DI_Print(3, ("lock_callback is called...with context [0x%x]\n", hFrontend));

	if( s_hFrontend[0] == hFrontend )
	{
		CH_DI_Print(3, ("Master channel!!!\n"));
		nUnitId = 0;
	}
	else
	{
		CH_DI_Print(3, ("Slave channel!!!\n"));
		nUnitId = 1;
	}
	...
	CH_DI_Print(5, ("OFDM Lock callback: receiverLock=%d, fecLock=%d\n", ofdmStatus.receiverLock, ofdmStatus.fecLock));
#endif
...
static void async_status_ready_callback(void *context, int param)
{
    ...
    CH_DI_Print(5, ("async_status_ready_callback\n"));
...
static int ConvertBcmModulation( CH_TER_Moudulation_e etModulation )
{
	switch (etModulation)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertBcmModulation() etModulation=(%d) \n", etModulation));
			return -1;
	}
}
...
static int ConvertBcmHierarchy( CH_TER_Hierarchy_e etHierarchy )
{
	switch (etHierarchy)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertBcmHierarchy() etHierarchy=(%d) \n", etHierarchy));
			return -1;
	}
}
...
static int ConvertBcmCodeRate( CH_TER_CodeRate_e etCodeRate )
{
	switch (etCodeRate)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertBcmCodeRate() etCodeRate=(%d) \n", etCodeRate));
			return -1;
	}
}
...
static int ConvertBcmFftSize( CH_TER_TransMode_e etTransMode )
{
	switch (etTransMode)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertBcmFftSize() etTransMode(%d) \n", etTransMode));
			return -1;
	}
}
...
static int ConvertBcmGuardInterval( CH_TER_GuardInterval_e etGuardInterval )
{
	switch (etGuardInterval)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertBcmGuardInterval() etGuardInterval(%d) \n", etGuardInterval));
			return -1;
	}
}
#endif
...
static int ConvertDiModulation( NEXUS_FrontendOfdmModulation etModulation )
{
	...
	CH_DI_Print(5, (">>>>>>>>>>>>> ConvertDiModulation() etModulation=(%d) \n", etModulation));
	switch (etModulation)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertDiModulation() etModulation=(%d) \n", etModulation));
			return -1;
	}
}
...
static int ConvertDiHierarchy( NEXUS_FrontendOfdmHierarchy etHierarchy )
{
	switch (etHierarchy)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertDiHierarchy() etHierarchy=(%d) \n", etHierarchy));
			return -1;
	}
}
...
static int ConvertDiCodeRate( NEXUS_FrontendOfdmCodeRate etCodeRate )
{
	switch (etCodeRate)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertDiCodeRate() etCodeRate=(%d) \n", etCodeRate));
			return -1;
	}
}
...
static int ConvertDiFftSize( NEXUS_FrontendOfdmTransmissionMode etTransMode )
{
	switch (etTransMode)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertDiFftSize() etTransMode=(%d) \n", etTransMode));
			return -1;
	}
}
...
static int ConvertDiGuardInterval( NEXUS_FrontendOfdmGuardInterval etGuardInterval )
{
	switch (etGuardInterval)
	{
		...
		default:
			CH_DI_Print(0, ("Error : ConvertDiGuardInterval() etGuardInterval=(%d) \n", etGuardInterval));
			return -1;
	}
}
...
static HBOOL WaitOfdmLocked( int nChannelId )
{
	...
	while(1)
	{
		...
		{
			CH_DI_Print(3, ("Cancel locking status..."));
	...
	if( nRet != DI_CH_OK )
	{
		...
		CH_DI_Print(6,  ("**********************************************************************\n"));
		CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: nLoopCount=[%d]\n", nLoopCount));
	    CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: lockStatus=[%d]\n", status.lockStatus));
	    CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: acquireInProgress=[%d]\n", status.acquireInProgress));

	    if( status.lockStatus == NEXUS_FrontendLockStatus_eLocked)
	    {
	    	CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: NEXUS_FrontendLockStatus_eLocked \n"));
			CH_DI_Print(6,  ("**********************************************************************\n"));
	    	return OFDM_LOCKED;
	    }
	    else if(status.lockStatus == NEXUS_FrontendLockStatus_eNoSignal)
	    {
	    	CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: NEXUS_FrontendLockStatus_eNoSignal \n"));
			CH_DI_Print(6,  ("**********************************************************************\n"));
	    }
	    else if(status.lockStatus == NEXUS_FrontendLockStatus_eUnlocked)
	    {
	    	CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: NEXUS_FrontendLockStatus_eUnlocked \n"));
	    }
	    else
	    {
	    	CH_DI_Print(6, ("NEXUS_Frontend_GetFastStatus: NEXUS_FrontendLockStatus_eUnknown... Try one more... \n"));
	    }
		CH_DI_Print(6,  ("**********************************************************************\n"));

		if( nLoopCount >= MAX_REACQUIRE_COUNTS )
		{
			CH_DI_Print(3, ("########## nLoopCount=[%d] ########## \n", nLoopCount));
...
static void UpdateTunedOfdmParam( int nChannelId, NEXUS_FrontendOfdmStatus *pstOfdmStatus, CH_TER_TuneParam_t *pstTerTuneParam )
{
	...
	CH_DI_Print(5,  ("*********************** UpdateTunedOfdmParam[%d] *************************\n", nChannelId));

	CH_DI_Print(5,  (" modulation             [%d] \n", pstOfdmStatus->modulation));
	CH_DI_Print(5,  (" transmissionMode       [%d] \n", pstOfdmStatus->transmissionMode));
	CH_DI_Print(5,  (" guardInterval          [%d] \n", pstOfdmStatus->guardInterval));
	CH_DI_Print(5,  (" codeRate               [%d] \n", pstOfdmStatus->codeRate));
	CH_DI_Print(5,  (" hierarchy              [%d] \n", pstOfdmStatus->hierarchy));

	CH_DI_Print(5,  ("**************************************************************************\n"));

	pstTerTuneParam->etModulation 		= ConvertDiModulation( pstOfdmStatus->modulation );
	pstTerTuneParam->etTransMode 		= ConvertDiFftSize( pstOfdmStatus->transmissionMode );
	pstTerTuneParam->etGuardInterval 	= ConvertDiGuardInterval( pstOfdmStatus->guardInterval );
	pstTerTuneParam->etCodeRate 		= ConvertDiCodeRate( pstOfdmStatus->codeRate );
	pstTerTuneParam->etHierarchy 		= ConvertDiHierarchy( pstOfdmStatus->hierarchy );

	s_usCellId[nUnitId] = (HUINT16)pstOfdmStatus->cellId;

	if(pstTerTuneParam->etTerSystem == DI_CH_DVBT2)
	{
		nNumPlp = pstOfdmStatus->dvbt2Status.l1PlpStatus.numPlp;

		CH_DI_Print(5,  ("***********************   DVB-T2 PLP Info.   *************************\n"));
		CH_DI_Print(5,  (" numPlp               [%d] \n", pstOfdmStatus->dvbt2Status.l1PlpStatus.numPlp));
		for(i=0;i<nNumPlp;i++)
		{

			CH_DI_Print(5,  ("**********************************************************************\n"));
			CH_DI_Print(5,  (" plpId                [%d] \n", pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpId));
			CH_DI_Print(5,  (" plpGroupId           [%d] \n", pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpGroupId));
			CH_DI_Print(5,  (" plpPayloadType       [%d] \n", pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpPayloadType));
			CH_DI_Print(5,  (" plpType              [%d] \n", pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpType));
			if(pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpType != NEXUS_FrontendDvbt2PlpType_eCommon)
			{
				s_aucPlpID[nChannelId][nNumDataPlp] = pstOfdmStatus->dvbt2Status.l1PlpStatus.plp[i].plpId;
				nNumDataPlp++;
			}
		}
		CH_DI_Print(5,  (" nNumDataPlp               [%d] \n", nNumDataPlp));
...
int DB3461_ReInitDevice( int nChannelId )
{
    ...
    NEXUS_FrontendHandle frontend[2]={NULL,NULL};
    NEXUS_FrontendDeviceHandle frontendDevice[2];
    NEXUS_FrontendDeviceStatus frontendDeviceStatus[2];

    CH_DI_Print(3, ("DB3461_ReInitDevice[%d] ...S\n", nChannelId));
    ...
    if(bFirstTime == TRUE)
    {
      ...
      //NEXUS_Platform_OpenFrontend(nUnitId);
        ...
        for ( i = 0; i < 2; i++ )
        {
            ...
            if (!frontend[i])
            {
                CH_DI_Print(0, ("Unable to find OFDM-capable frontend\n"));
                return -1;
            }
        }
        ...
        while(1)
        {
            CH_DI_Print(0, ("NEXUS_FrontendDevice_GetStatus \n"));
            ...
            {
                CH_DI_Print(0, ("2 frontend open is okay!! \n"));
                break;
            }
            VK_TASK_Sleep(1000);
        }
    ...
    for ( i = 0; i < NEXUS_MAX_FRONTENDS; i++ )
    {
        ...
        s_hFrontend[nUnitId] = platformConfig.frontend[i];
        if (s_hFrontend[nUnitId]) {
            NEXUS_Frontend_GetCapabilities(s_hFrontend[nUnitId], &capabilities);
            /* Does this frontend support ofdm? */
    ...
    {
        CH_DI_Print(0, ("Error : Unable to find OFDM-capable frontend s_hFrontend[%d]\n", nUnitId));
        return DI_CH_ERR;
    }
    else
    {
        CH_DI_Print(0, ("Find OFDM-capable frontend s_hFrontend[%d]=[0x%x] \n", nUnitId, s_hFrontend[nUnitId]));
    }
...
void DRV_T_InstallApi( void )
{
	...
	CH_DI_Print(5, ("[DB3461] DRV_T_InstallApi()  OK!\n"));
...
int DB3461_InitDevice( int nChannelId )
{
	...
	CH_DI_Print(5, ("DB3461_InitDevice ...S\n"));
...
int DB3461_SetTune( int nChannelId,  HUINT8 *nChecklock, HUINT32 unWorkingId )
{
    ...
    CH_DI_Print(2, ( "etStream(%d)", pstTerTuneParam->etTerSystem ));
    if(pstTerTuneParam->etTerSystem == DI_CH_DVBT2)
    {
        CH_DI_Print(2, ( ">>>>> DI_CH_DVBT2 with ulPlpId(%d)\n", pstTerTuneParam->stT2Param.ulPlpId));
	...
	{
		CH_DI_Print(0, ( "etBandWidth(%d)", pstTerTuneParam->etBandWidth ));
	}
...
#if 0
	nRet = BKNI_WaitForEvent(s_evStatusEvent[nUnitId], 1000);
	if(nRet)
	{
		CH_DI_Print(3, ( ">>>>> Time out for Wait event... \n"));
	}
#endif
	...
	{
#if !defined(CONFIG_SUPPORT_ADVANCED_TUNING)
		UpdateTunedOfdmParam( nChannelId, &stOfdmStatus, pstTerTuneParam );
#endif
...
HBOOL DB3461_CheckLock( int nChannelId, HUINT32 ulWorkingId )
{
	...
	if( nRet != DI_CH_OK )
	{
		CH_DI_Print(0, ("Error : NEXUS_Frontend_GetFastStatus(0x%x)", nRet));
	}
	if(status.lockStatus == NEXUS_FrontendLockStatus_eLocked)
	{
#if defined(BCM_DEBUG_INFO)
		CH_DI_Print(3,  ("**********************************************************************\n"));
		CH_DI_Print(3,  (" status.lockStatus 					   [%d], [%d]:nUnitId\n", status.lockStatus, nUnitId));
		CH_DI_Print(3,  ("**********************************************************************\n"));
#endif
		return TRUE;
	}
...
int DB3461_GetStrengthQuality( int nChannelId, DI_CH_SignalQuality_t *pstSignalInfo)
{
    ...
    if( pstTerTuneParam->etTerSystem == DI_CH_DVBT )
    {
        ...
        rc =  NEXUS_Frontend_RequestOfdmAsyncStatus(s_hFrontend[nUnitId]);
        if(rc)
        {
            CH_DI_Print(CH_LOG_0, ("Error : NEXUS_Frontend_RequestOfdmAsyncStatus\n"));
        }
        ...
        rc =  NEXUS_Frontend_GetOfdmAsyncStatus(s_hFrontend[nUnitId],  &stOfdmStatus);
        if(rc)
        {
            CH_DI_Print(CH_LOG_0, ("Error : NEXUS_Frontend_RequestOfdmAsyncStatus\n"));
        }

        if(stOfdmStatus.fecLock)
        {
            bIslock = TRUE;
    ...
    else
    {
        ...
        {
            rc = NEXUS_Frontend_RequestDvbt2AsyncStatus(s_hFrontend[nUnitId], i);
            if(rc)
            {
            ...
            rc =  NEXUS_Frontend_GetDvbt2AsyncStatusReady(s_hFrontend[nUnitId], &dvbt2StatusReady);
            if(rc)
            {
            ...
            for (j=0; j < NEXUS_FrontendDvbt2StatusType_eMax; j++) 
            {
                ...
                {                            
#if defined(BCM_DEBUG_INFO)
                    CH_DI_Print(CH_LOG_4, ("Retrieving status type %d.\n", j));
#endif                        
                    rc = NEXUS_Frontend_GetDvbt2AsyncStatus(s_hFrontend[nUnitId], (NEXUS_FrontendDvbt2StatusType)j, &dvbt2Status);
                    if(rc)
                    {
                        CH_DI_Print(0, ("Error : NEXUS_Frontend_GetDvbt2AsyncStatus\n"));
                    }

                    if (j == NEXUS_FrontendDvbt2StatusType_eFecStatisticsPlpA) 
                    {
#if defined(BCM_DEBUG_INFO)
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.ldpcBER = %d\n",dvbt2Status.status.fecStatistics.ldpcBER));
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.bchCorBit = %d\n",dvbt2Status.status.fecStatistics.bchCorBit));          
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.bchTotBlk = %d\n",dvbt2Status.status.fecStatistics.bchTotBlk));
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.bchClnBlk = %d\n",dvbt2Status.status.fecStatistics.bchClnBlk));
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.bchCorBlk = %d\n",dvbt2Status.status.fecStatistics.bchCorBlk));
                            CH_DI_Print(CH_LOG_4, ("eFecStatisticsPlpA.bchUncBlk = %d\n",dvbt2Status.status.fecStatistics.bchUncBlk));
#endif
                    ...
                    else if (j == NEXUS_FrontendDvbt2StatusType_eBasic) 
                    {
                            if(dvbt2Status.status.basic.fecLock)
                                bIslock = TRUE;
                                                
                            fSnr = dvbt2Status.status.basic.snr/100.0;

                            CH_DI_Print(CH_LOG_4, ("===DVB-T2===================================================================\n"));
                            CH_DI_Print(CH_LOG_4, (" fecLock = %d\n", dvbt2Status.status.basic.fecLock));
                            CH_DI_Print(CH_LOG_4, (" signalStrength=%d\n", dvbt2Status.status.basic.signalStrength));
                            CH_DI_Print(CH_LOG_4, (" signalLevelPercent   =[%d] \n", dvbt2Status.status.basic.signalLevelPercent));
                            CH_DI_Print(CH_LOG_4, (" signalQualityPercent =[%d] \n",  dvbt2Status.status.basic.signalQualityPercent));
                            CH_DI_Print(CH_LOG_4, (" snr=%d\n", dvbt2Status.status.basic.snr));
                            CH_DI_Print(CH_LOG_4, ("============================================================================\n"));
                    }      
                }
            }
    ...
    if(bIslock)
    {
        ...
        if( pstTerTuneParam->etTerSystem == DI_CH_DVBT )
        {
            fInputPowerDbm                   = stOfdmStatus.signalStrength/10.0;
        }
        else
        {
            fInputPowerDbm                   = dvbt2Status.status.basic.signalStrength/10.0 - 14;
        }
...
int DB3461_EnableTsOutput( int nChannelId )
{
	...
	CH_DI_Print(3, (">>>>>>>>>>>> DB3461_EnableTsOutput !!!!!\n"));
...
int DB3461_DisableTsOutput( int nChannelId )
{
	...
	CH_DI_Print(3, (">>>>>>>>>>>> DB3461_DisableTsOutput !!!!!\n"));
	NEXUS_Frontend_Untune(s_hFrontend[nUnitId]);
	return DI_CH_OK;
}
...
CH_TER_AntStatus_e DB3461_GetAntState( int nChannelId )
{
	...
	CH_DI_Print(3, (">>>>>>>>>>>> DB3461_GetAntState !!!!!\n"));
...
{
    //NEXUS_FrontendOfdmStatus stOfdmStatus;
    int i=0, j=0,k=0, nNumPlp=0, nNumDataPlp=0;
    int nUnitId = 0;
    int nRet=0;
    ...
    for ( i = 0; i < NEXUS_FrontendDvbt2StatusType_eMax; ++i) 
    {
        nRet = NEXUS_Frontend_RequestDvbt2AsyncStatus(s_hFrontend[nUnitId], i);
        if(nRet)
        {
            CH_DI_Print(0, ("Error : NEXUS_Frontend_RequestDvbt2AsyncStatus\n"));
        }

        nRet = BKNI_WaitForEvent(s_evStatusEvent[nUnitId], 1000);
        if(nRet)
        {
            CH_DI_Print(0, ("Error : BKNI_WaitForEvent\n"));
        }

        nRet =  NEXUS_Frontend_GetDvbt2AsyncStatusReady(s_hFrontend[nUnitId], &dvbt2StatusReady);
        if(nRet)
        {
            CH_DI_Print(0, ("Error : NEXUS_Frontend_GetDvbt2AsyncStatusReady\n"));
        }
        ...
        for (j=0; j < NEXUS_FrontendDvbt2StatusType_eMax; j++) 
        {
            ...
            if (ready)
            {                            
                CH_DI_Print(CH_LOG_2, ("Retrieving status type %d.\n", j));

                nRet = NEXUS_Frontend_GetDvbt2AsyncStatus(s_hFrontend[nUnitId], (NEXUS_FrontendDvbt2StatusType)j, &dvbt2Status);
                if(nRet)
                {
                    CH_DI_Print(0, ("Error : NEXUS_Frontend_GetDvbt2AsyncStatus\n"));
                }

                if (j == NEXUS_FrontendDvbt2StatusType_eL1Plp) 
                {
                    CH_DI_Print(CH_LOG_2,  ("***********************   DVB-T2 PLP Info.   *************************\n"));
                    CH_DI_Print(CH_LOG_2,  (" numPlp               [%d] \n", dvbt2Status.status.l1Plp.numPlp));

                        nNumPlp = dvbt2Status.status.l1Plp.numPlp;

                    for(k=0;k<nNumPlp;k++)
                    {
                            CH_DI_Print(CH_LOG_2,  ("**********************************************************************\n"));
                            CH_DI_Print(CH_LOG_2,  (" plpId                [%d] \n", dvbt2Status.status.l1Plp.plp[k].plpId));
                            CH_DI_Print(CH_LOG_2,  (" plpGroupId           [%d] \n", dvbt2Status.status.l1Plp.plp[k].plpGroupId));
                            CH_DI_Print(CH_LOG_2,  (" plpPayloadType       [%d] \n", dvbt2Status.status.l1Plp.plp[k].plpPayloadType));
                            CH_DI_Print(CH_LOG_2,  (" plpType              [%d] \n", dvbt2Status.status.l1Plp.plp[k].plpType));
                        if(dvbt2Status.status.l1Plp.plp[k].plpType != NEXUS_FrontendDvbt2PlpType_eCommon)
                        {
                            s_aucPlpID[nChannelId][nNumDataPlp] = dvbt2Status.status.l1Plp.plp[k].plpId;
                            nNumDataPlp++;
                        }
                    }
                    CH_DI_Print(CH_LOG_2,  (" nNumDataPlp               [%d] \n", nNumDataPlp));
                    s_ucNumberPlp[nChannelId] = nNumDataPlp;
                    CH_DI_Print(CH_LOG_2,  ("**********************************************************************\n"));
                } 
            }
...
int DB3461_GetPlpInfo( int nChannelId, HUINT8 * pucPlpIds, HUINT8 * pucNumPlps )
{
	...
	{
		CH_DI_Print(0, (" s_ucNumberPlp (%d) ", s_ucNumberPlp[nUnitId]));
		nResult = DI_CH_ERR;
	}
...
int DB3461_GetCellId(int nChannelId, HUINT16 *pusCellId)
{
	...
	*pusCellId = s_usCellId[nUnitId];
	...
	CH_DI_Print(4,("DEMOD[%d] : *pusCellId : (0x%x)\n", nUnitId, *pusCellId));
...
int DB3461_GetLockedParam(int nChannelId, CH_TER_TuneParam_t * pstTerTuneParam)
{
	...
	VK_memcpy( pstTerTuneParam, pvSource, sizeof(CH_TER_TuneParam_t));
        ...
        nRet =NEXUS_Frontend_RequestOfdmAsyncStatus(s_hFrontend[nUnitId]);
        if( nRet != DI_CH_OK )
        {
            CH_DI_Print(0, ("Error : NEXUS_Frontend_RequestOfdmAsyncStatus(0x%x)", nRet));
        }
         
        BKNI_WaitForEvent(s_evStatusEvent[nUnitId], 1000);
   
//	nRet = NEXUS_Frontend_GetOfdmStatus(s_hFrontend[nUnitId], &stOfdmStatus); // This api is not supported anymore in URSR 1401. 
       nRet = NEXUS_Frontend_GetOfdmAsyncStatus(s_hFrontend[nUnitId], &stOfdmStatus);
	if( nRet != DI_CH_OK )
	{
		CH_DI_Print(0, ("Error : NEXUS_Frontend_GetOfdmStatus(0x%x)", nRet));
	}

	CH_DI_Print(CH_LOG_2,  ("*********************** DB3461_GetLockedParam[%d] *************************\n", nChannelId));

	CH_DI_Print(CH_LOG_2,  (" modulation             [%d] \n", stOfdmStatus.modulation));
	CH_DI_Print(CH_LOG_2,  (" transmissionMode       [%d] \n", stOfdmStatus.transmissionMode));
	CH_DI_Print(CH_LOG_2,  (" guardInterval          [%d] \n", stOfdmStatus.guardInterval));
	CH_DI_Print(CH_LOG_2,  (" codeRate               [%d] \n", stOfdmStatus.codeRate));
	CH_DI_Print(CH_LOG_2,  (" hierarchy              [%d] \n", stOfdmStatus.hierarchy));
