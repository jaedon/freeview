...
int init_usb_serial(int device_handle)
{
        ...
        device_handle = open("/dev/ttyACM0", O_RDWR );
        if (device_handle < 0)
        {
                VK_fprintf(VK_STDERR, "can't open /dev/ttyACM0, are you sure you have permission to use it?\n");
                return -1;
        }
...
static void* ReceiveMessage(void* pdata)
{
...
void enable_relative_motion(int value)
{
    ...
    relativeMotionParams = UAPI_createParamStruct(relativeMotionParams, 1, 0);
    UAPI_setParam(relativeMotionParams, 0, 5, (void*)&UINT8_value, sizeof(UINT8_value));
    UAPIConfigureFeature((void*)relativeMotionParams, UAPI_RF4CE_NIF, 0);
    UAPI_destroyParamStruct(relativeMotionParams);
    VK_fprintf(VK_STDERR, "press ok/select on remote\n");
}

void change_speed(int value)
{
    unsigned int UINT32_value = 2;
    UAPI_configParamStruct_t* relativeMotionParams = NULL;
    relativeMotionParams = UAPI_createParamStruct(relativeMotionParams, 1, 0);
    UAPI_setParam(relativeMotionParams, 0, 3, (void*)&UINT32_value, sizeof(UINT32_value));
    UAPIConfigureFeature((void*)relativeMotionParams, UAPI_RF4CE_NIF, 0);
    UAPI_destroyParamStruct(relativeMotionParams);
    VK_fprintf(VK_STDERR, "press ok/select on remote\n");
...
void RF4CE_Handler( int choice )
{
    int FreeSpace = 0;
    int total_size = 0;
    UAPI_Storage_BAT_entry_t* entries = NULL;
    unsigned char num_entries = 0;
    int status = 0;
    unsigned char BAT_entry;
    unsigned char data[64];
    int i;
    short length = 0;
    unsigned char seed = 0x01;
    unsigned char search = 0x01;
    for(i=0;i<64;i++)
    {
        data[i] = seed;
    }
    ...
    break;
    case 9:
        UAPIGetInfo(0x37, UAPI_RF4CE_NIF, REMOTE_0);
        UAPIGetInfo(0x43, 0,0);
    break;
    case 10:
        while(1)
        {
        if(search == 0xFF || search == 0x00)
            search = 0x01;
        VK_fprintf(VK_STDERR, "LOASOFJASDFOADSJF FOR %d\n", search);
        status = UAPI_Storage_ReadData(0,0, search++, data, &length);
        if(UAPI_SUCCESS == status)
        {
        VK_fprintf(VK_STDERR, "RECEIVED *************************************************************************** %d bytes ************************* : ", length);
        for(i=0;i<length;i++)
        ...
        {
            VK_fprintf(VK_STDERR, "error gotten, value : %d\n", status);
            if(status == 110)
                break;
        }
        }
    ...
    case 11:
        while(1)
        {
            status =  UAPI_Storage_WriteData(0,0,&BAT_entry, data, sizeof(data));
            seed++;
            for(i=0;i<64;i++)
                data[i] = seed;
            if(status == UAPI_SUCCESS)
            {
                VK_fprintf(VK_STDERR, "wrote BAT_entry = %d\n", BAT_entry);
            }
            else
            {
                VK_fprintf(VK_STDERR, "this is bad? %d\n", status);
                break;
            }
            if(seed == 0xFF)
                break;
    ...
    break;
    case 12:
        UAPI_Storage_ReadData(0,0,0x02, data, &length);
        break;
    case 13:
        UAPI_Storage_Delete_BAT_Entry(0,0, 0x02);
        UAPI_Storage_Delete_BAT_Entry(0,0, 0x04);

        UAPI_Storage_Delete_BAT_Entry(0,0, 0x0A);
        UAPI_Storage_Delete_BAT_Entry(0,0, 0x0B);
        break;
    case 14:
       while(1)
        {
         entries = UAPI_Storage_Get_BAT_List(0,0,entries,&num_entries);
        if(num_entries > 0)
        {
            for(i=0;i<num_entries;i++)
            {
                VK_fprintf(VK_STDERR, "entry %d, length %d\n", entries[i].ID, entries[i].length);
            }
        }
        }
    case 15:
        if( UAPI_SUCCESS == UAPI_Storage_GetTotalSize(0,0, &total_size))
            VK_fprintf(VK_STDERR, "size of stsorage area is %d\n", total_size);

        break;
    case 16:
        if( UAPI_SUCCESS == UAPI_Storage_Defrag(0,0))
            VK_fprintf(VK_STDERR, "defrag done\n");
        break;
    case 18:
        UAPI_Storage_GetFreeSpace(0,0, &FreeSpace);
       VK_fprintf(VK_STDERR, "free space is %d\n", FreeSpace);
    case 19:
        change_speed(1);
    break;
    case 20:
        UAPIBatteryStatusRequest(0,0);
    break;
    default:
        return;
    }
...
int main(void)
{
    ...
    UAPIRegisterCallback( UAPI_RAWSENSORDATA_NAME, RawSensor);
    /* initialize RF4CE */
    init_rf4ce();
