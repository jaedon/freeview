...
{
	ABR_QUEUE_NODE_t* leftNode = NULL;
	ABR_QUEUE_NODE_t* rightNode = NULL;
	//HINT32 rightId = 0;
	//HUINT64 rightBitrate = 0;
	HINT32 nCur = 0;
	...
	do
	{
		...
		PrintError("%s(%d) Debug> left(%"PRId64") right sz_offset(%d) is %"PRId64"\n",__FUNCTION__,__LINE__,leftNode->bufferOffset,rightNode->nId,rightNode->bufferOffset);
		if(leftNode->bufferOffset <= sz_BufferOffset
		&& ((rightNode->bufferOffset > sz_BufferOffset) || (leftNode->bufferOffset > rightNode->bufferOffset)))
		{
			pNode->nId = leftNode->nId;
			pNode->nType = leftNode->nType;
			pNode->offset = leftNode->offset;
			pNode->range  = leftNode->range;
			pNode->ullBitrate = leftNode->ullBitrate;
			pNode->bufferOffset = leftNode->bufferOffset;
			pNode->bComplete = leftNode->bComplete;
			pNode->header_size = leftNode->header_size;
//			PrintDebug("%s(%d) Debug> sz_voffset: %"PRId64" , nId : %d , ullBitrate : %lld\n",__FUNCTION__,__LINE__,rightNode->offset,rightNode->nId,rightNode->ullBitrate);
			PrintExit;
			VK_SEM_Release(pQueue->ulSema);
			return ABR_OK;
		...
		if(nCur == pQueue->nRear && rightNode->bufferOffset == sz_BufferOffset)
		{
			pNode->nId = rightNode->nId;
			pNode->nType = rightNode->nType;
			pNode->offset = rightNode->offset;
	...
	pNode->bufferOffset = -1;
	PrintExit;
	PrintError("%s(%d) Err> sz_BufferOffset(%"PRId64") is not EXIST\n",__FUNCTION__,__LINE__,sz_BufferOffset);
	VK_SEM_Release(pQueue->ulSema);
	return ABR_NO_RESOURCE;
}
