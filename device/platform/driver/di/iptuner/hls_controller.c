...
HBOOL HLS_CONTROLLER_DiscardSegmentsAndKeysUntilPlayListSequenceNum(HLS_CTRL hCtrl, HUINT64 playListSeqNum)
{
	...
	{
		pSegment = (m3u8Segment_t *)pSegNode->pData;
		if( pSegment->ullNum == playListSeqNum)
		{
			/* 모든 Segment들이 사라지는 경우를 방지하지 위해 체크  */
 			bFound = TRUE;
			break;
...
HLS_ERROR_e	HLS_CONTROLLER_GetTimeBySegment(HLS_CTRL hCtrl, const HUINT64 pullSegmentId, HUINT64 * ullTime)
{
	...
	while(pSegNode != NULL)
	{
		...
		if(pSegment != NULL && pSegment->ullNum == pullSegmentId)
		{
			*ullTime = pSegment->starttime_of_segment;
			PrintExit;
			return HLS_OK;
		}
		pSegNode  = pSegNode->next;
	}
	PrintError("%s(%d) Err> pullSegmentId(%"PRId64") is INVALID\n",__FUNCTION__,__LINE__,pullSegmentId);
//	P_SSTR_CONTROLLER_Unlock(hSstr);
	PrintExit;
	return HLS_FAIL;
}
