...
void  ASF_GenerateGuid ( ASF_GUID_t *p_asfguid )
{
	...
	p_asfguid->v2 = ( (HUINT64)rand() << 16 ) / RAND_MAX;
	p_asfguid->v3 = ( (HUINT64)rand() << 16 ) / RAND_MAX;

	for( CheckNum = 0; CheckNum < 8; CheckNum++ )
	{
		p_asfguid->v4[CheckNum] = ( (HUINT64)rand() * 256 ) / RAND_MAX;
	}
...
void  ASF_ParseHeader ( ASF_HEADER_t *hdr, HUINT8 *p_header, int i_header )
{
	...
	mms_buffer_initread( &tBuffer, p_header, i_header );
	mms_buffer_getguid( &tBuffer, &tAsfGuid );
	...
	mms_buffer_getmemory( &tBuffer, NULL, 30 - 16 );

	for( ;; )
	{
		mms_buffer_getguid( &tBuffer, &tAsfGuid );
		i_size = mms_buffer_get64( &tBuffer );
	...
	HUINT32 max_bitrate;	/* Maximum bitrate of the media (sum of all the stream) 	4 */
#endif
			mms_buffer_getmemory( &tBuffer, NULL, 16 );				/* skip GUID 16 */
			hdr->i_file_size			= mms_buffer_get64( &tBuffer);		/* read file_size 8 */

			mms_buffer_getmemory( &tBuffer, NULL, 8 );				/* skip creation_time 8 */
			hdr->i_data_packets_count   = mms_buffer_get64( &tBuffer );	/* read data_packet_count 8 */

			hdr->i_play_duration= mms_buffer_get64( &tBuffer );			/* read playduration 8 */
			hdr->i_send_duration= mms_buffer_get64( &tBuffer );			/* read sendduration 8 */

			mms_buffer_getmemory( &tBuffer, NULL, 8+4); 			/* send_duration 8, preroll 8, flags 4 */
			hdr->i_min_data_packet_size = mms_buffer_get32( &tBuffer );	/* read min_data_packet_size 4 */
			hdr->i_max_data_packet_size = mms_buffer_get32( &tBuffer );/* read max_data_packet_size 4 */
			hdr->i_max_bitrate= mms_buffer_get32( &tBuffer );			/* read max_bitrate 4 */

			mms_buffer_getmemory( &tBuffer, NULL, i_size - 24 - 16 - 8 - 8 - 8 - 8-8-8-4 - 4 -4 -4); /* sub read byte */
		}
		else if( INT_ASF_CompareGUID ( &tAsfGuid, &asf_object_header_extension_guid ) )
		{
			mms_buffer_getmemory( &tBuffer, NULL, 46 - 24 );
		}
		else if( INT_ASF_CompareGUID ( &tAsfGuid, &asf_object_extended_stream_properties_guid ) )
		{
			...
			mms_buffer_getmemory( &tBuffer, NULL, 84 - 24 );

			nCount1 = mms_buffer_get16( &tBuffer );
			nCount2 = mms_buffer_get16( &tBuffer );
			...
			for( CheckNum = 0; CheckNum < nCount1; CheckNum++ )
			{
				...
				mms_buffer_get16( &tBuffer );
				i_len = mms_buffer_get16( &tBuffer );
				mms_buffer_getmemory( &tBuffer, NULL, i_len );
			...
			{
				int i_len;
				mms_buffer_getmemory( &tBuffer, NULL, 16 + 2 );
				i_len = mms_buffer_get32( &tBuffer );
				mms_buffer_getmemory( &tBuffer, NULL, i_len );
			...
			{
				mms_buffer_getmemory( &tBuffer, NULL, i_size - nSubsize );
			}
		}
		else if( INT_ASF_CompareGUID ( &tAsfGuid, &asf_object_stream_properties_guid ) )
		{
			...
			*/
			...
			mms_buffer_getguid( &tBuffer, &tStream_type ); /* Stream Type 16 */
			mms_buffer_getmemory( &tBuffer, NULL, 16 ); /* Error Correction Type 16*/
			mms_buffer_getmemory( &tBuffer, NULL, 8 ); /* Time Offset 8*/
			uTypeSpecificDataLength = mms_buffer_get32( &tBuffer); /* Type-Specific Data Length 4*/
			//PrintDebug ("Type Specific Data Length = %u\n", uTypeSpecificDataLength);
			uErrorCorrectionDataLength = mms_buffer_get32( &tBuffer); /* Error Correction Data Length 4*/
			//PrintDebug ("Error Correction Data Length = %u\n", uErrorCorrectionDataLength);


			flags = mms_buffer_get16 ( &tBuffer ); /* Flags 2 */
			/* Stream ID Detection */
			...
			if( INT_ASF_CompareGUID ( &tStream_type, &asf_object_stream_type_video ) )
			{
				...
				mms_buffer_getmemory( &tBuffer, NULL, 4 ); /* Reserved 4*/
#if SUPPORT_ANALYZE_ASF_VIDEO			/* start of type specific data */
				EncodedImageWidth = mms_buffer_get32 ( &tBuffer ); /* Encoded Image Width 4 */
				PrintDebug("EncodedImageWidth = %d\n", EncodedImageWidth);
				EncodedImageHeight = mms_buffer_get32 ( &tBuffer ); /* Encoded Image Height 4 */
				PrintDebug("EncodedImageHeight = %d\n", EncodedImageHeight);
				ReservedFlags = mms_buffer_get8 ( &tBuffer ); /* Reserved Flags 1 */
				PrintDebug("ReservedFlags = %d\n", ReservedFlags);
				FormatDataSize = mms_buffer_get16 ( &tBuffer ); /* Format Data Size 2 */
				PrintDebug("FormatDataSize = %d\n", FormatDataSize);
				FormatData.headerSize = mms_buffer_get32 ( &tBuffer ); /* Format Data Size 4 */
				PrintDebug("FormatData.headerSize = %d\n", FormatData.headerSize);
				FormatData.biWidth = mms_buffer_get32 ( &tBuffer ); /* Image Width 4 */
				PrintDebug("FormatData.biWidth = %d\n", FormatData.biWidth);
				FormatData.biHeight = mms_buffer_get32 ( &tBuffer ); /* Image Height 4 */
				PrintDebug("FormatData.biHeight = %d\n", FormatData.biHeight);
				FormatData.biPlanes = mms_buffer_get16 ( &tBuffer ); /* Reserved 2 */
				PrintDebug("FormatData.biPlanes = %d\n", FormatData.biPlanes);
				FormatData.biBitCount = mms_buffer_get16 ( &tBuffer ); /* Bits Per Pixel Count 2 */
				PrintDebug("FormatData.biBitCount = %d\n", FormatData.biBitCount);

				char CompressedID[5];

				CompressedID[0] = mms_buffer_get8(&tBuffer);
				CompressedID[1] = mms_buffer_get8(&tBuffer);
				CompressedID[2] = mms_buffer_get8(&tBuffer);
				CompressedID[3] = mms_buffer_get8(&tBuffer);
				CompressedID[4] = 0;
				...
				FormatData.biSizeImage = mms_buffer_get32 ( &tBuffer ); /* Image Size 4 */
				PrintDebug("FormatData.biSizeImage = %d\n", FormatData.biSizeImage);
				FormatData.biXPelsPerMeter = mms_buffer_get32 ( &tBuffer ); /* Horizontal Pixels Per Meter 4 */
				PrintDebug("FormatData.biXPelsPerMeter = %d\n", FormatData.biXPelsPerMeter);
				FormatData.biYPelsPerMeter = mms_buffer_get32 ( &tBuffer ); /* Vertical Pixels Per Meter 4 */
				PrintDebug("FormatData.biYPelsPerMeter = %d\n", FormatData.biYPelsPerMeter);
				FormatData.biClrUsed = mms_buffer_get32 ( &tBuffer ); /* Colors Used Count 4 */
				PrintDebug("FormatData.biClrUsed = %d\n", FormatData.biClrUsed);
				FormatData.biClrImportant = mms_buffer_get32 ( &tBuffer ); /* Important Colors Count 4 */
				PrintDebug("FormatData.biClrImportant = %d\n", FormatData.biClrImportant);
				mms_buffer_Getmemory( &tBuffer, NULL, i_size - 20 - 24 - 32 - 16 - 2 - 35);
#else
				/* start of type specific data */
				mms_buffer_getmemory( &tBuffer, NULL, 11 );

				mms_buffer_getmemory( &tBuffer, NULL, 16 );
				{
					char CompressedID[5];
					CompressedID[0] = mms_buffer_get8(&tBuffer);
					CompressedID[1] = mms_buffer_get8(&tBuffer);
				...
				mms_buffer_getmemory( &tBuffer, NULL, i_size - 24 - 32 - 16 - 2 - 35);
#endif
			}
			else
			{
				if( INT_ASF_CompareGUID ( &tStream_type, &asf_object_stream_type_audio ) )
				{
					...
					mms_buffer_getmemory( &tBuffer, NULL, 4 ); /* Reserved 4*/
					AudioCodecId = mms_buffer_get16 ( &tBuffer ); /* Codec ID/Format Tag 2 */
					switch(AudioCodecId)
					{
						case 0x161:
					...
					ChannelNumber = mms_buffer_get16 ( &tBuffer ); /* Number of Channels 2 */
					PrintDebug("ChannelNumber = %d\n", ChannelNumber);
					SampleRate = mms_buffer_get32 ( &tBuffer ); /* Samples Per Second 4 */
					PrintDebug("SampleRate = %d\n", SampleRate);
					Bitrate = mms_buffer_get32 ( &tBuffer ); /* Average Number of Bytes Per Second 4 */
					PrintDebug("Bitrate = %d\n", Bitrate);
					BlockAlignment = mms_buffer_get16 ( &tBuffer ); /* Block Alignment 2 */
					PrintDebug("BlockAlignment = %d\n", BlockAlignment);
					BitperSample = mms_buffer_get16 ( &tBuffer ); /* Codec Specific Data Size 2 */
					PrintDebug("BitperSample = %d\n", BitperSample);
					CodecDataSize = mms_buffer_get16 ( &tBuffer ); /* Codec Specific Data Size 2 */
					PrintDebug("CodecDataSize = %d\n", CodecDataSize);
					hdr->stream[nStream_id].i_cat = MMS_ASF_STREAM_AUDIO;
					mms_buffer_getmemory( &tBuffer, NULL, i_size - 22 - 24 - 32 - 16 - 2 );
#else
					hdr->stream[nStream_id].i_cat = MMS_ASF_STREAM_AUDIO;
					mms_buffer_getmemory( &tBuffer, NULL, i_size - 24 - 32 - 16 - 2 );
#endif
				}
				else
				{
		...
		else if ( INT_ASF_CompareGUID ( &tAsfGuid, &asf_object_bitrate_properties_guid ) )
		{
			...
			i_count = mms_buffer_get16( &tBuffer );
			i_size -= 2;
			while( i_count > 0 )
			{
				nStream_id = mms_buffer_get16( &tBuffer )&0x7f;
				hdr->stream[nStream_id].i_bitrate =  mms_buffer_get32( &tBuffer );
				i_count--;
				i_size -= 6;
			}
			mms_buffer_getmemory( &tBuffer, NULL, i_size - 24 );
		}
		...
		else if ( INT_ASF_CompareGUID ( &guid, &asf_object_extended_content_encryption_guid ) )
		{
			...
			signType = mms_buffer_get32(&buffer);
			signLength = mms_buffer_get32(&buffer);
			...
			mms_buffer_getmemory ( &buffer, NULL, i_size - 24 - 4 - 4);
		}
#endif
		...
		else if ( INT_ASF_CompareGUID ( &tAsfGuid, &asf_object_digital_signature_guid ) )
		{
			...
			signType = mms_buffer_get32(&tBuffer);
			i_size-=4;
			signLength = mms_buffer_get32(&tBuffer);
			i_size-=4;
		...
		if( mms_buffer_readempty( &tBuffer ) )
		{
			PrintExit;
			return;
		}
...
static struct asf_streams_t *New_Asf_Streams(void)
{
	...
	(struct asf_streams_t *)DD_MEM_Alloc(sizeof(struct asf_streams_t));
...
{
	ASF_HEADER_t *info =
	(ASF_HEADER_t *)DD_MEM_Alloc(sizeof(ASF_HEADER_t));
		...
		if(Position > asf_header_size)
	{
		...
		switch(get32_le(streamh->type))
		{
			case MMS_ASF_AUDIO_STREAM: /* audio stream */
				...
				{
					asf_streams->n_audio++;
					asf_streams->audio_streams = (int *)RTSP_LIB_Realloc(asf_streams->audio_streams, asf_streams->n_audio * sizeof(int));
				}
			...
			case MMS_ASF_VIDEO_STREAM: /* video streams */
				...
				{
					asf_streams->n_video++;
					asf_streams->video_streams = (int *)RTSP_LIB_Realloc(asf_streams->video_streams, asf_streams->n_video * sizeof(int));
				}
