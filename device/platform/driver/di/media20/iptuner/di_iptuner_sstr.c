 ...
 * @brief VFIO Controller for Smooth Streaming.
 */
...
/*******************************************************************/
/* Start typedef */
typedef struct SSTR_SubtypeCodecTable_s {
    HUINT8 fourcc[4];
    HUINT8 subtype[8];
} SSTR_SubtypeCodecTable_t;

typedef struct SSTR_AudioTagCodecTable_s {
	HUINT32 audioTag;
    HUINT8 fourcc[4];
} SSTR_AudioTagCodecTable_t;
...
static const SSTR_SubtypeCodecTable_t subtypecodec_tags[] = {
    { "WMAP",	"WmaP" },
    { "WMAP",	"WmaPro" },
	{ "WMA2",	"WMA"},
	{ "WMA2",	"Wma"},
...
static const SSTR_AudioTagCodecTable_t audiotagcodec_tags[] = {
    { 353,	"WMA2" },
    { 354,	"WMAP" },
	{ 255,	"AACL" },
};
...
/**
 * @b Function @b Description <br>
 * bmp4 box buffer에 대한 Virture File I/O Sub Function ( uint16_little_endian ) <br>
 * bmp4 box buffer에 uint16(big-endian) 값을 Write 한다.  <br>
 *
 * @param[in] mem_file	: bmp4 box의 mem_file I/O handle
 * @param[in] data	: Write 할 uint16 value
...
static void
write_uint16_le(mem_file *fout, uint16_t data)
{
	uint8_t d[2];
	d[0] = data;
	d[1] = data >> 8;
...
/**
 * @b Function @b Description <br>
 * bmp4 box buffer에 대한 Virture File I/O Sub Function ( uint24_little_endian ) <br>
 * bmp4 box buffer에 uint24(big-endian) 값을 Write 한다.  <br>
 *
 * @param[in] mem_file	: bmp4 box의 mem_file I/O handle
 * @param[in] data	: Write 할 uint24 value
 ...
 */
static void
write_uint24_le(mem_file *fout, uint32_t data)
{
	uint8_t d[3];
	d[0] = data;
	d[1] = data >> 8;
...
/**
 * @b Function @b Description <br>
 * bmp4 box buffer에 대한 Virture File I/O Sub Function ( uint32_little_endian ) <br>
 * bmp4 box buffer에 uint32(big-endian) 값을 Write 한다.  <br>
 *
 * @param[in] mem_file	: bmp4 box의 mem_file I/O handle
 * @param[in] data	: Write 할 uint32 value
 ...
 */
static void
write_uint32_le(mem_file *fout, uint32_t data)
{
	uint8_t d[4];
	d[0] = data;
	d[1] = data >> 8;
...
/**
 * @b Function @b Description <br>
 * bmp4 box buffer에 대한 Virture File I/O Sub Function ( uint64_little_endian ) <br>
 * bmp4 box buffer에 uint64(big-endian) 값을 Write 한다.  <br>
 *
 * @param[in] mem_file	: bmp4 box의 mem_file I/O handle
 * @param[in] data	: Write 할 uint64 value
 ...
 */
static void
write_uint64_le(mem_file *fout, uint64_t data)
{
	uint8_t d[8];
	d[0] = data;
	d[1] = data >> 8;
 ...
 * @b Function @b Description <br>
 * bmp4 box buffer에 대한 Virture File I/O Sub Function ( BDAT BOX ) <br>
 * bdat Box(full box) 를 생성한다. CodecPrivateData 외에 추가 데이터를 사용할 경우 사용한다. <br>
 * 인자로 사용되는 data는 CodecPrivateData를 나타낸다.
 *
 * @param[in] mem_file	: bmp4 box의 mem_file I/O handle
 * @param[in] extdata : Write 할 추가 데이터
 * @param[in] extdata_len : 추가 데이터 의 크기
 * @param[in] data : Write 할 CodecPrivateData
 * @param[in] data_len : CodecPrivateData 의 크기
 *
 * @return  #  void <br>
 */
static void
write_bdat_box_ext(mem_file *fout,const uint8_t *extdata, size_t extdata_len,const uint8_t *data, size_t data_len)
{
	int offset = start_mp4_box(fout, "bdat");
	if(extdata_len > 0 && extdata != NULL)
	{
		write_data(fout, extdata, extdata_len);
	}
	write_data(fout, data, data_len);
	finish_mp4_box(fout, offset);
	return;
...
HINT32 P_STREAM_SSTR_FourCCToDICodec(HINT8* pFourCC)
{
	...
	{
		return DI_VIDEO_FORMAT_MEDIA_VC1;
	}
	...
	else if(!VK_strncmp(pFourCC,SSTR_CONST_FOURCC_AACH,4))
	{
		return DI_AUDFORMAT_AAC_PLUS_ADTS;
	}
	...
	else if(!VK_strncmp(pFourCC,SSTR_CONST_FOURCC_WMA2,4))
	{
		return DI_AUDFORMAT_WMA_STD;
	}
	else if(!VK_strncmp(pFourCC,SSTR_CONST_FOURCC_WMAP,4))
	{
		return DI_AUDFORMAT_WMA_PRO;
	}
	else
	{
		return 0;
...
HINT32 P_STREAM_SSTR_AudioTagToDICodec(HUINT32 audioTag)
{
	switch(audioTag)
	{
		case 353:
...
static HINT8* P_STREAM_SSTR_SubtypeToFourCC(HUINT8* pSubType)
{
	unsigned int i;
	...
	for(i = 0 ; i < sizeof(subtypecodec_tags)/sizeof(SSTR_SubtypeCodecTable_t) ; i++)
	{
		if(!VK_strcmp(subtypecodec_tags[i].subtype,pSubType))
		{
			return (HINT8*)subtypecodec_tags[i].fourcc;
		}
	}
	return NULL;
}

static HINT8* P_STREAM_SSTR_AudioTagToFourCC(HUINT32 audioTag)
{
	unsigned int i;

	for(i = 0 ; i < sizeof(audiotagcodec_tags)/sizeof(SSTR_AudioTagCodecTable_t) ; i++)
	{
		if(audiotagcodec_tags[i].audioTag == audioTag)
		{
			return (HINT8*)audiotagcodec_tags[i].fourcc;
		}
	}
	return NULL;
}
...
static SSTR_ERR P_STREAM_SSTR_DownloadFragment(DI_IPTUNER_t *pHTuner, HUINT32 unStream, HUINT32 unTrack, HUINT32 unFragment, SSTR_URL_t *pUrl, HUINT8 **pBuf, HUINT32 *unBufSize, HUINT64 *pullRTD)
{
	...
	{
		if(pAccess->info.i_httpstatecode == HTTP_STATUS_PRECONDITION_FAILED)
		{
			PrintError("%s(%d) : Error> Fragment is not ready on server(%s) \n",__FUNCTION__,__LINE__,pAccess->psz_path);
			nRet = SSTR_ERR_CONNECT_PRECONDITION_FAIL;
		}
		else
		{
...
static SSTR_ERR P_STREAM_SSTR_DownloadCurFragment(DI_IPTUNER_t *pHTuner, HUINT64 *pullRTD)
{
	HINT32 nRet = SSTR_ERR_OK;
	STREAM_ACCESS_t *pAccess = NULL;
	STREAM_ACCESS_t tAccess;
...
int P_STREAM_SSTR_CreateManifestInfo(SSTR_t* pSstr)
{
	...
	pManifest = SSTR_CONTROLLER_GetManifestInfo(pSstr->hCtrl);
	if(pManifest == NULL)
	{
		return DI_ERR_ERROR;
	}
	...
	pSSTRInfo->nStreamCnt = pManifest->nStreamCnt;
	for(HINT32 i = 0 ; i < pSSTRInfo->nStreamCnt && i < DI_IPTUNER_SSTR_MAX_STREAM ; i++)
	{
		pSSTRInfo->stStream[i].eType = (pManifest->pStream + i)->eType;
		pSSTRInfo->stStream[i].ulIndex = (pManifest->pStream + i)->ulIndex;
		pSSTRInfo->stStream[i].nNameLen = (pManifest->pStream + i)->nNameLen;
		pSSTRInfo->stStream[i].pName = (pManifest->pStream + i)->pName;
		pSSTRInfo->stStream[i].nLangLen = (pManifest->pStream + i)->nLangLen;
		pSSTRInfo->stStream[i].pLanguage = (pManifest->pStream + i)->pLanguage;
		pSSTRInfo->stStream[i].nTrackCnt = (pManifest->pStream + i)->nTrackCnt;
		for(HINT32 j = 0 ; j < pSSTRInfo->stStream[i].nTrackCnt && j < DI_IPTUNER_SSTR_MAX_TRACK ; j++)
		{
			pSSTRInfo->stStream[i].stTrack[j].ulIndex = ((pManifest->pStream + i)->pTrack+j)->ulIndex;
			pSSTRInfo->stStream[i].stTrack[j].ulBitrate = ((pManifest->pStream + i)->pTrack+j)->ulBitrate;
			pSSTRInfo->stStream[i].stTrack[j].pFourCC = ((pManifest->pStream + i)->pTrack+j)->pFourCC;
 ...
 */
HINT64  P_STREAM_SSTR_GetManifest(HINT8 *pUrl, HINT8 **pMetaBuff,HINT8 **pBaseUrl)
{
	HINT64 	nSumLength=0;
	HINT32	nPos = 0;
	...
	else
	{
		...
		tAccess.psz_path = VK_StrDup ( (void *) UTIL_GetOriginalURL ((const char *)pUrl) );
#if 0
		if(psCookie != NULL)
		{
		...
		else
		{
			...
			*pBaseUrl = (HINT8*)DD_MEM_Alloc( VK_strlen( tAccess.psz_path )+SSTR_CONST_PADDING);
			VK_memset(*pBaseUrl,0x00,VK_strlen( tAccess.psz_path )+SSTR_CONST_PADDING);
			VK_MEM_Memcpy(*pBaseUrl, tAccess.psz_path, VK_strlen( tAccess.psz_path ));
		}
	}
	...
	*pMetaBuff = (HINT8*) DD_MEM_Alloc (nContentLength+SSTR_CONST_PADDING);
	...
	do
	{
		...
		nRecvLength = 0;
		nRecvLength = STREAM_HTTP_Read ( &tAccess, (HUINT8 *)pBuffer, SSTR_CONST_HTTP_RECV_BUFFSIZE );
		if(nRecvLength > 0) /* normal case */
		{
			if(tAccess.info.i_size != nContentLength)
			{
				nSumLength = -1;
				break;
			}
		...
		else if (nRecvLength == 0 && tAccess.info.b_eof )
		{
			PrintDebug ("%s(%d) : Debug > End of File. and recv zero \n",__FUNCTION__,__LINE__);
			break;
		}
...
HUINT32	P_STREAM_SSTR_MakeBox (SSTR_t* pSstr)
{
	...
	mem_file memfd;
	HUINT8 szMem[SSTR_CONST_EXTRADATA_BUFFER] = {0};
	HUINT32 unExtDataLen = 0;
	PrintEnter;
	...
	else
	{
		...
		if(P_STREAM_SSTR_FourCCToDICodec(pFourCC) == 0) /* AV Stream Codec Check with FourCC */
		{
			...
			if(P_STREAM_SSTR_FourCCToDICodec(pFourCC) == 0)
			{
				pFourCC = P_STREAM_SSTR_SubtypeToFourCC(pFourCC);
				if(pFourCC == NULL && eType == SSTR_TYPE_AUDIO)
				{
					pFourCC = P_STREAM_SSTR_AudioTagToFourCC(SSTR_CONTROLLER_GetCurAudioTag(pSstr->hCtrl));
				}
			}
		}
		if(pFourCC == NULL)
		...
		if(!VK_strncmp(pFourCC,SSTR_CONST_FOURCC_WMAP,VK_strlen(SSTR_CONST_FOURCC_WMAP))
		|| !VK_strncmp(pFourCC,SSTR_CONST_FOURCC_WMA2,VK_strlen(SSTR_CONST_FOURCC_WMA2)))
		{
			VK_MEM_Memset(&memfd,0x00,sizeof(memfd));
			memfd.buf = &szMem[0];
			write_uint16_le(&memfd,SSTR_CONTROLLER_GetCurAudioTag(pSstr->hCtrl));
			write_uint16_le(&memfd,SSTR_CONTROLLER_GetCurAudioChannels(pSstr->hCtrl));
			write_uint32_le(&memfd,SSTR_CONTROLLER_GetCurAudioSamplingRate(pSstr->hCtrl));
			write_uint32_le(&memfd,SSTR_CONTROLLER_GetCurAudioBitrate(pSstr->hCtrl) / 8);
			write_uint16_le(&memfd,SSTR_CONTROLLER_GetCurAudioChannels(pSstr->hCtrl) * SSTR_CONTROLLER_GetCurAudioBitsPerSample(pSstr->hCtrl) / 8);
			write_uint16_le(&memfd,SSTR_CONTROLLER_GetCurAudioBitsPerSample(pSstr->hCtrl));
			if(pCodecPrivate != NULL && nCodecPrivateDataLen > 0)
			{
				write_uint16_le(&memfd,nCodecPrivateDataLen);
			}
			unExtDataLen += mem_file_tell(&memfd);
		}
//		write_bdat_box(&pSstr->mp4Box, (unsigned char*)szCodecPrivData, nCodecPrivateDataLen);
		write_bdat_box_ext(&pSstr->mp4Box,szMem,unExtDataLen,(unsigned char*)szCodecPrivData, nCodecPrivateDataLen);
...
//			PrintDebug ("%s(%d) Debug> Try To Update Delay %u( TF : %u, CF : %u )\n",__FUNCTION__,__LINE__,unDelay,unTotalFragments,unFragment);
		...
		for(i = 0 ; i < unTotalStreams ; i++)
		{
			...
			if(P_STREAM_SSTR_DownloadFragment(pHTuner,i,unTrack,(unTotalFragments - 1),&pSstr->tUpdateUrl,&pBuf,&unBufSize,NULL) == SSTR_ERR_OK)
			{
...
static void P_STREAM_SSTR_TextStream_Task(void *pParam)
{
	...
	while(TRUE)
	{
		if(pSstr->bReqTextFlush == TRUE || pHTuner->bExitDownload == TRUE)
		{
		...
		else
		{
			for(HUINT32 i = 0; i < pSstr->ulMaxText ; i++)
			{
				...
				if(nCur == nSize && (pSstr->bReqTextFlush != TRUE || pHTuner->bExitDownload != TRUE))
				{
...
SSTR_ERR	DI_STREAM_SSTR_GetMediaInfo (SSTR_HANDLE hSstr, DI_MEDIA_INFO_t *pstMediaInfo)
{
...
#if 0
	pstMediaInfo->stVideoTrack[nVideoIndex].video_codec = P_STREAM_SSTR_FourCCToDICodec(SSTR_CONTROLLER_GetCurTrackFourCC(pSstr->hCtrl, SSTR_TYPE_VIDEO));
	if(pstMediaInfo->stVideoTrack[nVideoIndex].video_codec == 0)
	{
		pstMediaInfo->stVideoTrack[nVideoIndex].video_codec = P_STREAM_SSTR_FourCCToDICodec(SSTR_CONTROLLER_GetCurStreamSubtype(pSstr->hCtrl, SSTR_TYPE_VIDEO));
	}
	if(pstMediaInfo->stVideoTrack[nVideoIndex].video_codec != 0)
	{
	...
	pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec =  P_STREAM_SSTR_FourCCToDICodec(SSTR_CONTROLLER_GetCurTrackFourCC(pSstr->hCtrl, SSTR_TYPE_AUDIO));
	if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec == 0)
	{
		pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec = P_STREAM_SSTR_FourCCToDICodec(SSTR_CONTROLLER_GetCurStreamSubtype(pSstr->hCtrl, SSTR_TYPE_AUDIO));
	}
	if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec != 0)
	{
	...
	for(HUINT32 i = 0 ; i < pstMediaInfo->ntracks ; i++)
	{
		...
		switch(SSTR_CONTROLLER_GetStreamType(pSstr->hCtrl, i))
		{
			...
			case SSTR_TYPE_AUDIO :
				...
				{
					nLangLen = nLangLen >= 256 ? 255 : nLangLen;
					VK_MEM_Memset(pstMediaInfo->stAudioTrack[nAudioIndex].uacAudioLang,0x0,256);
				...
				if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec == 0)
				{
					...
					if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec == 0)
					{
						pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec = P_STREAM_SSTR_FourCCToDICodec(P_STREAM_SSTR_SubtypeToFourCC(SSTR_CONTROLLER_GetStreamSubtype(pSstr->hCtrl, i)));
						if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec == 0)
						{
							pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec = P_STREAM_SSTR_AudioTagToDICodec(SSTR_CONTROLLER_GetCurAudioTag(pSstr->hCtrl));
						}
					}
				}
				if(pstMediaInfo->stAudioTrack[nAudioIndex].audio_codec != 0)
	...
	pProtectionHeader = SSTR_CONTROLLER_GetDecodedProtectionHeader(pSstr->hCtrl);
	if(pstMediaInfo->nWrmHeaderLen > 0 && pProtectionHeader != NULL)
	{
#if 1
		pstMediaInfo->pucWrmHeader = DD_MEM_Alloc(pstMediaInfo->nWrmHeaderLen+1);
		if(pstMediaInfo->pucWrmHeader != NULL)
		{
			VK_MEM_Memset(pstMediaInfo->pucWrmHeader,0x00,pstMediaInfo->nWrmHeaderLen+1);
			VK_MEM_Memcpy(pstMediaInfo->pucWrmHeader, pProtectionHeader, pstMediaInfo->nWrmHeaderLen);
...
SSTR_HANDLE DI_STREAM_SSTR_OpenController (HINT8 *pUrl)
{
	...
	while((nContentLength = P_STREAM_SSTR_GetManifest(pUrl,&pMetaBuff,&pBaseUrl)) <= 0)
	{
		nNetworkRetryCount++;
...
//				PrintDebug ("[PLAY] P_STREAM_SSTR_NextFragment \n");
		 ...
		 ***********************************************************************/
		ullRTD = 0;
		nRet = P_STREAM_SSTR_DownloadCurFragment(pHTuner,&ullRTD);
		if(nRet == SSTR_ERR_CONNECT_PRECONDITION_FAIL)
		{
			VK_TASK_Sleep(DI_IPTUNER_SSTR_CONST_SLEEP_FOR_WFD);
			continue;
		}
		else if(nRet == SSTR_ERR_DOWNLOAD_FAIL)
		{
			nNetworkRetryCount++;
		...
		else if(nRet == SSTR_ERR_CONNECT_FAIL)
		{
			...
			else
			{
				...
				if(nNetworkRetryCount < DI_IPTUNER_SSTR_CONST_RETRY_COUNT_ON_NET_ERROR)
				{
					HUINT32 unWaitTime = 0;
					while(unWaitTime < (DI_IPTUNER_SSTR_CONST_SLEEP_FOR_RETRY / DI_IPTUNER_SSTR_CONST_SLEEP_FOR_WFD))
					{
						if(pHTuner->bExitDownload == TRUE)
						{
							PrintDebug("%s(%d) Break Loop(Waiting for download Fragment)\n",__FUNCTION__,__LINE__);
							break;
						}
						VK_TASK_Sleep(DI_IPTUNER_SSTR_CONST_SLEEP_FOR_WFD);
						unWaitTime++;
				...
				{
					HUINT32 unWaitTime = 0;
					while(unWaitTime < (DI_IPTUNER_SSTR_CONST_SLEEP_FOR_SERVER / DI_IPTUNER_SSTR_CONST_SLEEP_FOR_WFD))
					{
						if(pHTuner->bExitDownload == TRUE)
						{
							PrintDebug("%s(%d) Break Loop(Waiting for download Fragment)\n",__FUNCTION__,__LINE__);
					...
					nVoDRecovery++;
					if( nVoDRecovery < DI_IPTUNER_SSTR_CONST_RETRY_COUNT_ON_NET_ERROR )
					{
						if(nNoTrackRecovery)
						{
				...
				ullRequestedSeekPoint = SSTR_CONTROLLER_GetCurFragmentStartTime(pSstr->hCtrl, SSTR_TYPE_VIDEO);
				(void)SSTR_CONTROLLER_GetFragmentByTime(pSstr->hCtrl,SSTR_CONTROLLER_GetCurAVStream(pSstr->hCtrl, SSTR_TYPE_AUDIO),ullRequestedSeekPoint,&ulFragmentForSeek);
				(void)SSTR_CONTROLLER_SetFragment(pSstr->hCtrl,SSTR_CONTROLLER_GetCurAVStream(pSstr->hCtrl, SSTR_TYPE_AUDIO), ulFragmentForSeek);
			}
			continue;
...
//			unStartTick = VK_TIMER_GetSystemTick(); /* 1000 tick == 1 sec */
	...
	}while (pHTuner->bExitDownload != TRUE); /* End Main Loop*/
	SSTR_BUFFERMGR_SetReadLock(pBufferMgr, FALSE);
...
*/
	...
	(void) VK_memset (&s_Msg, 0x0, sizeof(s_Msg));
	s_Msg.unMsg = TUNER_ADAPTIVE_MSG_SSTR_BUFFERING;
...
*/
	while(sz_Read > 0)
	{
		...
		{
			*ptReadSize = BFILE_ERROR_NO_DATA;
			PrintDebug("%s(%d) : Debug> Exit Read by SEEK Operation\n",__FUNCTION__,__LINE__);
			goto read_break;
		}
...
void DI_STREAM_SSTR_UnregisterCallback (const char *pUrl)
{
	...
	if(nRet<0)
	{
		PrintError("%s(%d) Error> DI_STREAM_SSTR_UnregisterCallback Fail (%s)\n",__FUNCTION__,__LINE__,pUrl);
	}
	PrintExit;
	return;
}
