...
/*!
 *	\brief 특정 Segment를 Controller에 설정 (Discontinuity 이전)
 *	\param hCtrl : HLS Controller Handle
 *	\param ullSegment : 설정할 Segment Id
 *	\return ErrorCode
 *	\see
...
/*!
 *	\brief 모든 media playlist 내의 ext-x-media-sequence 값 전의 segment 를 제거한다.
 *	\param hCtrl : HLS Controller Handle
 *	\param ullPlayListSeqNum : ext-x-media-sequence 값 
 *	\param  unStream : segment 정리할 Stream 번호
 *	\return TRUE, FALSE
 *	\see 
 */
 ...
 *	\brief Panic 상황인지 체크하여 패닉이면 Lowest Bitrate로 변경한다.
 *	\param hCtrl : HLS Controller Handle
 *	\return 현 Stream 의 Target duration 값
 *	\see 
 */
...
/*!
 *	\brief 재생 중인 Stream 의 sequnce time 을 가져온다. 
 *	\param hCtrl : HLS Controller Handle
 *	\param[out] *puSequenceTime : 재생 중인 Stream 의 sequnce time
 *	\return ErrorCode
 *	\see 
 */
...
/*!
 *	\brief live 시점 계산 ( endlist 로부터 3개 전의 segment 를 재생. )
 *	\param hCtrl : HLS Controller Handle
 *	\param[out] *pfTimeToLive : live 시점 시간
 *	\return ErrorCode
 *	\see 
 */
...
/*!
 *	\brief  restart 기능 on/off 때 time gap 을 계산
 *	\param hCtrl : HLS Controller Handle
 *	\param bOnLiveRestart : restart 기능 on/off 값 
 *	\return  time gap 값
 *	\see 
 */
