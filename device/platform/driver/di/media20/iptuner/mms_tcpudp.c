 ...
 * File Description:	     MMS - HTTP Controller
 * Module:
 * Remarks:
 */

/**
 * @defgroup MMS_TCPUDP_CONTROLLER
 * @brief MMS TCPUDP Controller.
 */
...
static int INT_MMS_SendCommand ( STREAM_ACCESS_t *pAccess, int iCommandType,
...
//			unlock( pMmstuSys->p_keepalive_thread );
		...
		case CMD_CONTROL_SPEED :
			PrintError ("%s (%d) CMD_CONTROL_SPEED (val=%d)\n",__FUNCTION__,__LINE__,(HINT32)val);
			{
				...
				var_buffer_t buffer;
				HUINT32    i_packet;
				i_packet =  (pAccess->info.i_pos - pMmstuSys->iHeaderSize ) / pMmstuSys->i_packet_length;
				MMS_BUF_Initwrite( &buffer, 0 );

				PrintDebug ("%s (%d) pAccess->info.i_pos(%"PRId64")\n",__FUNCTION__,__LINE__,pAccess->info.i_pos);
				/* seekpoint (8 bytes) */
				MMS_BUF_Add64 ( &buffer, pAccess->info.i_pos ); /* seek point in second TODO: first*/
				/* asfOffset (4 bytes) */
				MMS_BUF_Add32 ( &buffer, 0xffffffff );
				/* locationId (4 bytes) */
				MMS_BUF_Add32 ( &buffer, i_packet ); // begin from start
				/* frameOffset (4 bytes) : ignore*/
				MMS_BUF_Add8 ( &buffer, 0xff ); // stream time limit
				MMS_BUF_Add8 ( &buffer, 0xff ); //  on 3bytes ...
				MMS_BUF_Add8 ( &buffer, 0xff ); //
				MMS_BUF_Add8 ( &buffer, 0x00 ); // don't use limit
				/* playIncarnation (4 bytes) */
				MMS_BUF_Add32 ( &buffer, pMmstuSys->iMediaPktIdType ); //  on 3bytes ...
				/* rate (8 bytes) -5 ~ + 5 */
				MMS_BUF_Add64 ( &buffer, speed);

				INT_MMS_SendCommand ( pAccess, 0x28, pMmstuSys->iCommandLevel, 0x0001ffff, buffer.p_data, buffer.i_data );
				MMS_BUF_Free ( &buffer );
			}
			break;
...
int MMSTU_Seek ( STREAM_ACCESS_t * pAccess, HINT64 i_pos )
{
	...
	/* *** restart stream *** */
	MMS_BUF_Initwrite( &buffer, 0 );
	MMS_BUF_Addmemory(&buffer, &dRequestTime, 8);/* seek point in second */   /* position fields*/
	MMS_BUF_Add32( &buffer, 0xffffffff );					 /* asfOffset */
	MMS_BUF_Add32( &buffer, 0xffffffff ); /* begin from start*/ /* locationId */
	MMS_BUF_Add8( &buffer, 0x00 ); /* stream time limit */
	MMS_BUF_Add8( &buffer, 0x00 ); /*  on 3bytes ... */
	MMS_BUF_Add8( &buffer, 0x00 );
	MMS_BUF_Add8( &buffer, 0x00 ); /* don't use limit */
	MMS_BUF_Add32( &buffer, pMmstuSys->iMediaPktIdType );
	MMS_BUF_Add32( &buffer, 1000000 ); /*dwAccelBandwidth 1000Kbps*/
	MMS_BUF_Add32( &buffer, 60000 ); /* dwAccelDuration 60 sec */
	MMS_BUF_Add32( &buffer, 1000000 ); /* dwLinkBandwidth 1000Kbps*/

	(void) INT_MMS_SendCommand ( pAccess, 0x07, pMmstuSys->iCommandLevel, 0x0001ffff, buffer.p_data, buffer.i_data );
		...
		else if(int_ret == MMS_PACKET_CMD)
		{
			switch( pMmstuSys->iCommandType )
			{
				...
				case 0x05: /* LinkMacToViewerReportStartedPlaying */
					...
					if(GetDWLE( pMmstuSys->pCommand + 40 ) & 0x80000000)
					{
...
ssize_t MMSTU_Read ( STREAM_ACCESS_t *pAccess, HUINT8 *p_buffer, size_t i_len )
{
	...
	PrintDebug ("%s(%d) i_header(%d) i_packet_length(%d) i_media(%d) i_media_used (%d) i_pos(%"PRId64") try read(%d) \n"
				,__FUNCTION__
				,__LINE__
				,pMmstuSys->iHeaderSize
				,pMmstuSys->i_packet_length
				,pMmstuSys->iMediaSize
				,pMmstuSys->iMediaUsedSize
				,pAccess->info.i_pos
				,i_len);

	/* fail 이라면 Partial Packet을 넣어준다. */
	/* 현재 Position이 header size보다 작은경우 header를 먼저 카피해준다.*/
	//if( pAccess->info.i_pos < pMmstuSys->iHeaderSize || s_bFeedASFHeaderFirst == TRUE)
	if( pMmstuSys->bFeedASFHeaderFirst == TRUE)
	{
		PrintDebug("===FEED ASF HEADER FIRST\n");
		//i_copy = __MIN( i_len, pMmstuSys->iHeaderSize - pAccess->info.i_pos );
		i_copy = (size_t)__MIN( (HINT32)i_len, pMmstuSys->asfh.nRawAsfHeaderLen);

		//VK_memcpy( &p_buffer[i_data], &pMmstuSys->pHeader[pAccess->info.i_pos], i_copy );
		...
		pMmstuSys->iMediaUsedSize = pMmstuSys->i_packet_length;
	...
	while ( i_data < i_len )
	{
		...
		/* 버퍼의 자료를 먼저 복사 ; iMediaUsedSize */
		if( pMmstuSys->pMedia != NULL && pMmstuSys->iMediaUsedSize < pMmstuSys->iMediaSize)
		{
			i_copy = __MIN( i_len - i_data , pMmstuSys->iMediaSize - pMmstuSys->iMediaUsedSize );

			VK_memcpy( &p_buffer[i_data], &pMmstuSys->pMedia[pMmstuSys->iMediaUsedSize], i_copy );

			i_data += i_copy;

			pMmstuSys->iMediaUsedSize += i_copy;

			pAccess->info.i_pos += i_copy;
		}
		/* 한 패킷의 길이는 i_packet_length여야 하므로 모자라다면 0으로 Padding  */
		else if( pMmstuSys->pMedia != NULL && pMmstuSys->iMediaUsedSize < pMmstuSys->i_packet_length )
		{
			i_copy = __MIN( i_len - i_data, pMmstuSys->i_packet_length - pMmstuSys->iMediaUsedSize);
	...
	if( pAccess->info.i_pos < pMmstuSys->iHeaderSize )
	{
		const size_t i_copy = pMmstuSys->iHeaderSize - pAccess->info.i_pos;

		block_t *p_block = block_New( pAccess, i_copy );
		if( !p_block )
			return NULL;

		VK_memcpy( p_block->p_buffer, &pMmstuSys->pHeader[pAccess->info.i_pos], i_copy );
		pAccess->info.i_pos += i_copy;
		return p_block;
	}
	else if( pMmstuSys->pMedia && pMmstuSys->iMediaUsedSize < __MAX( pMmstuSys->iMediaSize, pMmstuSys->i_packet_length ) )
	{
		size_t i_copy = 0;
		size_t i_padding = 0;

		if( pMmstuSys->iMediaUsedSize < pMmstuSys->iMediaSize )
			i_copy = pMmstuSys->iMediaSize - pMmstuSys->iMediaUsedSize;
		if( __MAX( pMmstuSys->iMediaSize, pMmstuSys->iMediaUsedSize ) < pMmstuSys->i_packet_length )
			i_padding = pMmstuSys->i_packet_length - __MAX( pMmstuSys->iMediaSize, pMmstuSys->iMediaUsedSize );
		...
		if( i_copy > 0 )
			VK_memcpy( &p_block->p_buffer[0], &pMmstuSys->pMedia[pMmstuSys->iMediaUsedSize], i_copy );
		if( i_padding > 0 )
			VK_memset( &p_block->p_buffer[i_copy], 0, i_padding );

		pMmstuSys->iMediaUsedSize += i_copy + i_padding;
...
static int INT_MMS_Open ( STREAM_ACCESS_t  *pAccess, URL_t *p_url, int  i_proto )
{
	...
	char			*tmpBuffer=NULL;
	HUINT16			*p;
	HINT32			nServer_version=0;
	HINT32			nTool_version=0;
	HINT32          nUpdate_Player_Url=0;
	HINT32          nEncryption_type=0;
	HINT32          CheckNum=0;
	HINT32          i_streams;
	HINT32          i_first;
	char			*mediapath;
	...
	pMmstuSys->iCommandLevel = 1;          /* updated after 0x1A command */
	pMmstuSys->iSequenceNum = 0;
	pMmstuSys->iMediaPktIdType  = 0x04;
	pMmstuSys->iHeaderPktIdType = 0x02;
	pMmstuSys->i_proto = i_proto;
	pMmstuSys->iPacketSeqNum = 0;
	pMmstuSys->pHeader = NULL;
	pMmstuSys->iHeaderSize = 0;
	pMmstuSys->pMedia = NULL;
	pMmstuSys->iMediaSize = 0;
	pMmstuSys->iMediaUsedSize = 0;

	pAccess->info.i_pos = 0;
	pMmstuSys->i_buffer_tcp = 0;
	pMmstuSys->i_buffer_udp = 0;
	pMmstuSys->pCommand = NULL;
	pMmstuSys->iCommandSize = 0;
	pAccess->info.b_eof = false;

	/* *** send command 1 : connection request *** */
	MMS_BUF_Initwrite( &tBuffer, 0 );
	MMS_BUF_Add16( &tBuffer, 0x001c );
	MMS_BUF_Add16( &tBuffer, 0x0003 );
	...
	/* Prevent CID-12172 */
 	/* VK_sprintf( tmpBuffer, "NSPlayer/7.0.0.1956; {"ASF_GUID_FMT"}; Host: %s", ASF_GUID_PRINT( pMmstuSys->guid ), p_url->psz_host ); */
	...
	if(length > MMSTU_STRING_BUF_LEN)
	{
 	...
 	VK_snprintf( tmpBuffer, length+1, "NSPlayer/7.0.0.1956; {"ASF_GUID_FMT"}; Host: %s", ASF_GUID_PRINT( pMmstuSys->guid ), p_url->psz_host );

	MMS_BUF_AddUTF16( &tBuffer, tmpBuffer );
	DD_MEM_Free(tmpBuffer);
	...
	{
		MMS_BUF_Free( &tBuffer );
		INT_MMS_Close ( pAccess );
		return DI_IPT_EGENERIC;
	}

	nServer_version = GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 32 );
	nTool_version = GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 36 );
	nUpdate_Player_Url = GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 40 );
	nEncryption_type = GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 44 );
	p = (HUINT16*)( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 48 );

	GETUTF16( p, &pMmstuSys->pServerVer, nServer_version );
	GETUTF16( p, &pMmstuSys->pToolVer, nTool_version );
	GETUTF16( p, &pMmstuSys->pUpdatePlayerURL, nUpdate_Player_Url );
	GETUTF16( p, &pMmstuSys->pEncryptionType, nEncryption_type );

	PrintDebug (
			"%s(%d) 0x01 --> server_version:\"%s\" tool_version:\"%s\" update_player_url:\"%s\" encryption_type:\"%s\"\n",
			__FUNCTION__,__LINE__,
			pMmstuSys->pServerVer,
			pMmstuSys->pToolVer,
			pMmstuSys->pUpdatePlayerURL,
			pMmstuSys->pEncryptionType );

	/* *** should make an 18 command to make data timing *** */

	/* *** send command 2 : transport protocol selection *** */
	MMS_BUF_Reinitwrite( &tBuffer, 0 );
	MMS_BUF_Add32( &tBuffer, 0x00000000 );
	MMS_BUF_Add32( &tBuffer, 0x000a0000 );
	MMS_BUF_Add32( &tBuffer, 0x00000002 );

	VK_memset (tmpString,0,sizeof(tmpString));

	if( b_udp )
	{
		VK_snprintf( tmpString, sizeof(tmpString), "\\\\%s\\UDP\\%d", pMmstuSys->sz_bind_addr, 7000 ); // FIXME
	}
	else
	{
		VK_snprintf( tmpString, sizeof(tmpString), "\\\\192.168.0.1\\TCP\\1242"  );
	}

	MMS_BUF_AddUTF16( &tBuffer, tmpString );
	MMS_BUF_Add16( &tBuffer, '0' );
	...
	if( pMmstuSys->iCommandType == 0x03 )
	{
	...
	else if( pMmstuSys->iCommandType != 0x02 )
	{
	...
	/* else { continue; } */
	MMS_BUF_AddUTF16( &tBuffer, mediapath );

	(void) INT_MMS_SendCommand ( pAccess,
			0x05,
			pMmstuSys->iCommandLevel,
			0xffffffff,
			tBuffer.p_data,
			tBuffer.i_data );
	...
	if( pMmstuSys->iCommandType == 0x1A )
	{
	...
	if( pMmstuSys->iCommandType != 0x06 )
	{
		PrintError( "%s(%d) Error> unknown answer (0x%x instead of 0x06)\n",__FUNCTION__,__LINE__, pMmstuSys->iCommandType );
	...
	switch( GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE ) )
	{
		case 0x0001:
		...
		case -1:
		default:
			PrintError( "%s(%d) Error> error while asking for file %d\n",__FUNCTION__,__LINE__, GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE ) );
	...
	pMmstuSys->i_flags_broadcast = GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 12 );
	/* fileDuration fields */
	pMmstuSys->i_media_length 	= GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 24 );
	/* filePacketSize fields */
	pMmstuSys->i_packet_length 	= GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 44 );
	/* filePacketCount fields */
	pMmstuSys->i_packet_count 	= GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 48 );
	/* fileBitRate fields */
	pMmstuSys->i_max_bit_rate 	= GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 56 );
	/* fileHeaderSize fields */
	pMmstuSys->i_header_size 	= GetDWLE( pMmstuSys->pCommand + MMS_TCPUDP_CMD_HEADERSIZE + 60 );
	...
	MMS_BUF_Reinitwrite( &tBuffer, 0 );
	MMS_BUF_Add32( &tBuffer, 0 );
	MMS_BUF_Add32( &tBuffer, 0x8000 );
	MMS_BUF_Add32( &tBuffer, 0xffffffff );
	MMS_BUF_Add32( &tBuffer, 0x00 );
	MMS_BUF_Add32( &tBuffer, 0x00 );
	MMS_BUF_Add32( &tBuffer, 0x00 );
	MMS_BUF_Add64( &tBuffer, (((HUINT64)0x40ac2000)<<32) );
	MMS_BUF_Add32( &tBuffer, pMmstuSys->iHeaderPktIdType );
	MMS_BUF_Add32( &tBuffer, 0x00 );

	(void) INT_MMS_SendCommand ( pAccess, 0x15, pMmstuSys->iCommandLevel, 0x00, tBuffer.p_data, tBuffer.i_data );
	...
	if( pMmstuSys->iCommandType != 0x11 )
	{
		PrintError(
				"unknown answer (0x%x instead of 0x11)\n",
				pMmstuSys->iCommandType );
		MMS_BUF_Free( &buffer );
		INT_MMS_Close ( pAccess );
		return( -1 );
	}
	...
	for( ;; )
	{
		...
		if( pMmstuSys->iHeaderSize >= pMmstuSys->i_header_size )
		{
			PrintDebug ( "header complete(%zu)\n", pMmstuSys->iHeaderSize );
			break;
		}
		PrintDebug ( "%s (%d) header incomplete (%zu/%zu), reading more\n",__FUNCTION__,__LINE__, pMmstuSys->iHeaderSize, pMmstuSys->i_header_size );
	}
	...
	ASF_ParseHeader ( &pMmstuSys->asfh, pMmstuSys->pHeader, pMmstuSys->iHeaderSize );
	pMmstuSys->asfh.pRawAsfHeader = DD_MEM_Alloc(pMmstuSys->iHeaderSize); /* Must be Free in INT_MMS_Close */
	VK_memcpy(pMmstuSys->asfh.pRawAsfHeader, pMmstuSys->pHeader, pMmstuSys->iHeaderSize);
	pMmstuSys->asfh.nRawAsfHeaderLen = pMmstuSys->iHeaderSize;
    ...
    //ASF_STREAM_t stream[128];
    {
        ...
        {
            if( pMmstuSys->asfh.stream[CheckNum].i_cat != MMS_ASF_STREAM_UNKNOWN )
            {
                PrintData("%s(%d) : STREAM[%d].i_cat       (%s) \n",__FUNCTION__,__LINE__,CheckNum,pMmstuSys->asfh.stream[CheckNum].i_cat==1?"ASF_VIDEO_STREAM":"ASF_AUDIO_STREAM");
                PrintData("%s(%d) : STREAM[%d].i_bitrate   (%d) \n",__FUNCTION__,__LINE__,CheckNum,pMmstuSys->asfh.stream[CheckNum].i_bitrate);
                PrintData("%s(%d) : STREAM[%d].i_selected  (%d) \n",__FUNCTION__,__LINE__,CheckNum,pMmstuSys->asfh.stream[CheckNum].i_selected);
            }
        }
    }
#endif
	...
	for( CheckNum = 1; CheckNum < 128; CheckNum++ )
	{
		if( pMmstuSys->asfh.stream[CheckNum].i_cat != MMS_ASF_STREAM_UNKNOWN )
		{
			...
			if( i_first != -1 )
			{
				MMS_BUF_Add16( &tBuffer, 0xffff );
				MMS_BUF_Add16( &tBuffer, CheckNum );
			}
			else
			{
				i_first = CheckNum;
			...
			if( pMmstuSys->asfh.stream[CheckNum].i_selected )
			{
				MMS_BUF_Add16( &tBuffer, 0x0000 );
				PrintData ( "%s (%d) selecting stream[0x%x] %s (%d kb/s)\n"
							, __FUNCTION__
							,__LINE__
							, CheckNum
							, ( pMmstuSys->asfh.stream[CheckNum].i_cat == MMS_ASF_STREAM_AUDIO  ) ?  "audio" : "video"
							, pMmstuSys->asfh.stream[CheckNum].i_bitrate / 1024);
			}
			else
			{
				MMS_BUF_Add16( &tBuffer, 0x0002 );
	...
	(void) INT_MMS_SendCommand ( pAccess, 0x33,
			i_streams,
			0xffff | ( i_first << 16 ),
			tBuffer.p_data, tBuffer.i_data );

	INT_MMS_ReadCommand ( pAccess, 0x21, 0 );
	if( pMmstuSys->iCommandType != 0x21 )
	{
...
static int INT_MMS_Start ( STREAM_ACCESS_t  *pAccess, HUINT32 i_packet )
{
	...
	MMS_BUF_Initwrite( &tBuffer, 0 );
	MMS_BUF_Add64( &tBuffer, 0 ); /* seek point in second */
	MMS_BUF_Add32( &tBuffer, 0xffffffff );
	MMS_BUF_Add32( &tBuffer, i_packet ); // begin from start
	MMS_BUF_Add8( &tBuffer, 0xff ); // stream time limit
	MMS_BUF_Add8( &tBuffer, 0xff ); //  on 3bytes ...
	MMS_BUF_Add8( &tBuffer, 0xff ); //
	MMS_BUF_Add8( &tBuffer, 0x00 ); // don't use limit
	MMS_BUF_Add32( &tBuffer, pMmstuSys->iMediaPktIdType );

	(void) INT_MMS_SendCommand ( pAccess, 0x07, pMmstuSys->iCommandLevel, 0x0001ffff,
			tBuffer.p_data, tBuffer.i_data );
	...
	if( pMmstuSys->iCommandType != 0x05 )
	{
...
static void INT_MMS_Close ( STREAM_ACCESS_t  *pAccess )
{
	...
	if(pMmstuSys->pCommand != NULL)
	{
	FREENULL( pMmstuSys->pCommand );
	}

	if(pMmstuSys->pMedia != NULL)
	{
	FREENULL( pMmstuSys->pMedia );
	}

	if(pMmstuSys->pHeader != NULL)
	{
	FREENULL( pMmstuSys->pHeader );
	}

	if(pMmstuSys->pServerVer != NULL)
	{
	FREENULL( pMmstuSys->pServerVer );
	}

	if(pMmstuSys->pToolVer != NULL)
	{
	FREENULL( pMmstuSys->pToolVer );
	}

	if(pMmstuSys->pUpdatePlayerURL != NULL)
	{
	FREENULL( pMmstuSys->pUpdatePlayerURL );
	}

	if(pMmstuSys->pEncryptionType != NULL)
	{
	FREENULL( pMmstuSys->pEncryptionType );
	}
                            ...
                            HUINT8 *p_data, int i_data_old )
{
	 ...
	 *    TCPMessageHeader
	 *************************************/
	MMS_BUF_Add32( &tBuffer, 0x00000001 );    					/* start sequence */
	MMS_BUF_Add32( &tBuffer, 0xB00BFACE );						/* session ID */
	/* size after protocol type */
	MMS_BUF_Add32( &tBuffer, i_data + MMS_TCPUDP_CMD_HEADERSIZE - 16 );	/* messageLength */
	MMS_BUF_Add32( &tBuffer, 0x20534d4d );    /* protocol "MMS " */ /*seal*/
	MMS_BUF_Add32( &tBuffer, i_data_by8 + 4 );						/* chunkCount */
	MMS_BUF_Add32( &tBuffer, pMmstuSys->iSequenceNum ); pMmstuSys->iSequenceNum++;	/* seq + MBZ (always 0)*/
	MMS_BUF_Add64( &tBuffer, 0 );										/* timeSent : Receiver SHOULD ignore this fields */

	/* Start LinkViewerToMacXXXXXX Message */
	/*************************************
	 * make LinkViewerToMacXXXXXX Message
	 *************************************/
	MMS_BUF_Add32( &tBuffer, i_data_by8 + 2 );			/* chunkLength */
	MMS_BUF_Add32( &tBuffer, 0x00030000 | iCommandType ); 	/* MID : dir | command */
	MMS_BUF_Add32( &tBuffer, i_prefix1 );    				/* command specific */
	MMS_BUF_Add32( &tBuffer, i_prefix2 );    				/* command specific */

	/* specific command data */
	if( p_data && i_data > 0 )
	{
		MMS_BUF_Addmemory( &tBuffer, p_data, i_data_old );
	}
	...
	nRet = net_Write ( pMmstuSys->i_handle_tcp, NULL, tBuffer.p_data, tBuffer.i_data - ( 8 - ( i_data - i_data_old ) ) );

	(void) VK_SEM_Release ( pMmstuSys->lock_netwrite);

	if( nRet != tBuffer.i_data - ( 8 - ( i_data - i_data_old ) ) )
	{
...
static int INT_MMS_FillBuffer ( STREAM_ACCESS_t *pAccess )
{
	...
	do
	{
		...
		if( nTry * timeout > pMmstuSys->i_timeout )
		{
			PrintError("%s(%d) Error> no data received\n",__FUNCTION__,__LINE__);
			return -1;
		}

		if( nTry > 3 && (pMmstuSys->i_buffer_tcp > 0 || pMmstuSys->i_buffer_udp > 0) )
		{
			return -1;
		}
	...
	} while ( !(nRet = poll ( ufd, nfd, timeout)) || (nRet < 0 && errno == EINTR) );
...
{
 #define GET32( i_pos ) \
    ( pMmstuSys->pCommand[i_pos] + ( pMmstuSys->pCommand[i_pos +1] << 8 ) + \
      ( pMmstuSys->pCommand[i_pos + 2] << 16 ) + \
      ( pMmstuSys->pCommand[i_pos + 3] << 24 ) )
	...
	PrintEnter;
    VK_free( pMmstuSys->pCommand );
    pMmstuSys->iCommandSize = i_data;
    pMmstuSys->pCommand = DD_MEM_Alloc ( i_data );
    VK_memcpy( pMmstuSys->pCommand, p_data, i_data );

    *pi_used = i_data; /* by default */

    if( i_data < MMS_TCPUDP_CMD_HEADERSIZE )
    {
        PrintError ( "%s (%d) Error> truncated command (header incomplete)\n",__FUNCTION__,__LINE__ );
        pMmstuSys->iCommandType = 0;
        return -1;
    }
    ...
    if( i_length > pMmstuSys->iCommandSize )
    {
        PrintError ( "%s (%d) Error> truncated command (missing %zu bytes)\n",__FUNCTION__,__LINE__, (size_t)i_length - i_data  );
        pMmstuSys->iCommandType = 0;
        return -1;
    }
    else if( i_length < pMmstuSys->iCommandSize )
    {
        pMmstuSys->iCommandSize = i_length;
        *pi_used = i_length;
    }
    ...
    pMmstuSys->iCommandType = GET32( 36 ) & 0xffff;
#undef GET32
...
//        PrintError ( "%s(%d) Error> truncated packet (Declared %d bytes, Actual %d bytes)\n",__FUNCTION__,__LINE__, i_packet_length, i_data  );
    ...
    if( i_packet_id != pMmstuSys->iHeaderPktIdType &&
        i_packet_id != pMmstuSys->iMediaPktIdType )
    {
    ...
    if( iPacketSeqNum != pMmstuSys->iPacketSeqNum )
    {
#if 0
        /* FIXME for udp could be just wrong order ? */
        PrintError ( "detected packet lost (%d != %d)\n", iPacketSeqNum, pMmstuSys->iPacketSeqNum );
#endif
    }

    pMmstuSys->iPacketSeqNum = iPacketSeqNum + 1;

    if( i_packet_id == pMmstuSys->iHeaderPktIdType )
    {
        if( pMmstuSys->pHeader )
        {
            pMmstuSys->pHeader = realloc( pMmstuSys->pHeader,
                                          pMmstuSys->iHeaderSize + i_packet_length - 8 );
            VK_memcpy( &pMmstuSys->pHeader[pMmstuSys->iHeaderSize],
                    p_packet,
                    i_packet_length - 8 );
            pMmstuSys->iHeaderSize += i_packet_length - 8;
    ...
    {
        FREENULL( pMmstuSys->pMedia );
        pMmstuSys->pMedia = p_packet;
        pMmstuSys->iMediaSize = i_packet_length - 8;
        pMmstuSys->iMediaUsedSize = 0;
...
static int INT_MMS_ReceivePacket ( STREAM_ACCESS_t *pAccess )
{
	...
	for( ;; )
	{
		...
		if( pMmstuSys->i_buffer_tcp >= MMS_TCPUDP_CMD_HEADERSIZE )
		{
			if( GetDWLE ( pMmstuSys->buffer_tcp + 4 ) == 0xb00bface  )
			{
				if( GetDWLE( pMmstuSys->buffer_tcp + 8 ) + 16 <= (HUINT32)pMmstuSys->i_buffer_tcp )
				{
					b_refill = false;
				}
			}
			else if( GetWLE ( pMmstuSys->buffer_tcp + 6 ) <= pMmstuSys->i_buffer_tcp )
			{
				b_refill = false;
			}
		}
		...
		if( pMmstuSys->i_proto == MMS_PROTO_UDP && pMmstuSys->i_buffer_udp >= 8 &&
				GetWLE( pMmstuSys->buffer_udp + 6 ) <= pMmstuSys->i_buffer_udp )
		{
			b_refill = false;
		}
		...
		if( i_packet_tcp_type == MMS_PACKET_CMD && pMmstuSys->iCommandType == 0x1b )
		{
...
static int INT_MMS_ReceiveCommand ( STREAM_ACCESS_t *pAccess )
{
	...
	for( ;; )
	{
		...
		if( pMmstuSys->i_buffer_tcp > 0 )
		{
			...
			if( pMmstuSys->iCommandType == 0x1b )
			{
				(void) INT_MMS_SendCommand ( pAccess, 0x1b, 0, 0, NULL, 0 );
			}
			else
...
static int INT_MMS_ReadCommand ( STREAM_ACCESS_t *pAccess, int i_command1, int i_command2 )
{
	...
	for( i_count = 0; i_count < MMS_RETRY_MAX; )
	{
		...
		if( i_status < 0 || pMmstuSys->iCommandType == 0 )
		{
			i_count++;
		...
		else if( pMmstuSys->iCommandType == i_command1 ||
				pMmstuSys->iCommandType == i_command2 )
		{
			PrintExit;
			return DI_IPT_SUCCESS;
		}
