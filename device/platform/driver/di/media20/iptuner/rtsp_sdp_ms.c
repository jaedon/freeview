...
/************************* File Description ************************/
/*******************************************************************/
/* File Name:		$Workfile:   rtsp_sdp_ms.c  $
...
static void P_RTSP_SDP_MS_FreeMediaDesc(struct RTSP_SDP_MS_MediaDesc_s *ptr)
{
	PrintEnter;
...
{
	struct RTSP_MS_SdpMsCtx_s *ptr = NULL;

	PrintEnter;

	ptr = DD_MEM_Alloc(sizeof(struct RTSP_MS_SdpMsCtx_s));
	if (ptr == NULL)
	{
		PrintError("%s(%d) Error> Out of memory!\n", __FUNCTION__, __LINE__);
		return NULL;
	}
	VK_memset(ptr ,0, sizeof(struct RTSP_MS_SdpMsCtx_s));
	PrintExit;
	return ptr;
}


void RTSP_SDP_MS_FreeSdpMsCtx(struct RTSP_MS_SdpMsCtx_s *ptr)
{
	PrintEnter;
	...
	if (ptr->pRawAsfHeader != NULL)
	{
		DD_MEM_Free(ptr->pRawAsfHeader);
		ptr->pRawAsfHeader = NULL;
	}
	RTSP_LIB_FreeLikedList(ptr->streams,(void (*)(void *))P_RTSP_SDP_MS_FreeMediaDesc);
...
static struct RTSP_SDP_MS_MediaDesc_s *P_RTSP_SDP_MS_ParseMediaDesc(char **ppData)
{
	struct RTSP_SDP_MS_MediaDesc_s *p_mediaDesc = NULL;
	char *ptr = *ppData;
	char *buf = NULL;
	...
	p_mediaDesc = DD_MEM_Alloc(sizeof(struct RTSP_SDP_MS_MediaDesc_s));
	if (p_mediaDesc == NULL)
	{
		PrintError("%s(%d) Error> Out of memory!\n", __FUNCTION__, __LINE__);
		return NULL;
	}
	VK_memset(p_mediaDesc ,0, sizeof(struct RTSP_SDP_MS_MediaDesc_s));
	...
	else
	{
		...
		P_RTSP_SDP_MS_FreeMediaDesc(p_mediaDesc);
		return NULL;
	}
 ...
 */
struct RTSP_MS_SdpMsCtx_s *RTSP_SDP_MS_ParseSdp(char *pData)
{
	...
	char *decoded = NULL;
	HINT32 decoded_len = 0;		  /* don't touch it after here   */
	struct RTSP_MS_SdpMsCtx_s *p_sdpContext = NULL;
	struct RTSP_SDP_MS_MediaDesc_s *p_mediaDesc = NULL;

	PrintEnter;

	p_sdpContext = RTSP_SDP_MS_NewSdpMsCtx();
	...
	for(ptr = pData ; ptr && *ptr ; )
	{
		...
		else if(RTSP_LIB_ThisLineHasKeyword(ptr,"m=",&buf,&buf_len))  /* stream description */
		{
			p_mediaDesc = P_RTSP_SDP_MS_ParseMediaDesc(&ptr);	/* ptr is at the end of stream desc */
			if (p_mediaDesc == NULL)
			{
				PrintError("%s(%d) Error> Can't get SDP desc\n", __FUNCTION__, __LINE__);
				/*goto func_error;*/
			}
