...
SSTR_HEUR_ERR	SSTR_HEURISTICS_Run(SSTR_HEURISTICS_t *hHeuristics, HUINT64 ullFragSize, HUINT64 ullMs)
{
	...
	if( ullMs > ( (HUINT64)ulDuration * (HUINT64)SSTR_HEURISTICS_CONST_MILLISEC * hHeuristics->tConfig.ulDelayLimitPerc / 100))
	{
		SSTR_HEURISTICS_AddSample(hHeuristics, ullFragSize, ullMs);
	...
	else
	{
		switch(SSTR_HEURISTICS_GetHeuristicsState(hHeuristics))
		{
			case SSTR_HEUR_STATE_TIMID :
				...
				nBonusUp = (( (HUINT64)ulDuration * (HUINT64)SSTR_HEURISTICS_CONST_MILLISEC) * SSTR_HEURISTICS_CONFIG_CORRECTIONVALUE / 100 / ullMs) - 1;
			...
			case SSTR_HEUR_STATE_GREEDY :
				...
				&& (ullMs >= ( ( (HUINT64)ulDuration *  (HUINT64)SSTR_HEURISTICS_CONST_MILLISEC * hHeuristics->tConfig.ulEnterPanicDelayPerc / 100))))
				{
				...
				else
				{
					...
					**/
					 ...
					 * 2nd Delay Check
					**/
					if(ullMs < ( (HUINT64)ulDuration * (HUINT64)SSTR_HEURISTICS_CONST_MILLISEC * hHeuristics->tConfig.ulDelayPositivePerc / 100))
					{
						nPoint++;
...
/*					if(ullMs < ulDuration) */
			...
			case SSTR_HEUR_STATE_PANIC :
				...
				|| (ullMs <= ( ( (HUINT64)ulDuration * (HUINT64)SSTR_HEURISTICS_CONST_MILLISEC * hHeuristics->tConfig.ulExitPanicDelayPerc / 100))))
				{
