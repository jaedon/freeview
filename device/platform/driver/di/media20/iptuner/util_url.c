...
#if 0 /* Remove Prevent Defact */
    (void) VK_sprintf (pTmp,"%u",nNumber);
#else
    (void)VK_snprintf (pTmp, 64, "%u", nNumber);
#endif
...
#if 0
	PrintDebug ("%s(%d) [%s] nTargetLength[%d]\n",__FUNCTION__,__LINE__, pTarget, nTargetLength);
#endif
...
const char* UTIL_GetOriginalURL (const char *pUniqueUrl)
{
	const char *p = NULL;
	const char *q = NULL;
	const char *r = NULL;
