...
#define BT_DEFAULT_BT_COD						{0x08, 0x04, 0x24}
...
#if defined(CONFIG_BLUETOOTH_BLE)
#define BT_BLE_DEFAULT_APPL_UUID 9000
#endif
...
DRV_Error P_BT_AV_Start(void);
DRV_Error P_BT_AV_Stop(void);
DRV_Error P_BT_AV_SendAudioBuffer(tUIPC_CH_ID stUipcChannel, void *pvBuffer, HUINT32 ulLength);
...
#if defined(CONFIG_BLUETOOTH_BLE)
static DRV_Error P_BT_BLE_Init(void);
static DRV_Error P_BT_BLE_Deinit(void);
static DRV_Error P_BT_BLE_Connect(DI_BT_DEV_INFO_t *pstHidDevInfo);
static void P_BT_BLE_Cl_Profile_Callback(tBSA_BLE_EVT event, tBSA_BLE_MSG* p_data);
#endif
...
{
	return VK_snprintf(pszStrBdAddr, 17+1,"%02X:%02X:%02X:%02X:%02X:%02X",
...
static DRV_Error P_BT_Get_BdAddrFromCmdline(unsigned char *pucMacAddr)
{
	...
	while(fgets(szBuf, sizeof(szBuf), fp))
	{
		...
		p = VK_strstr(szBuf, "btmacaddr=");	/* btmacaddr */
		if (p)
		{
			p = p + VK_strlen("btmacaddr=");
			VK_sscanf(p, "%s", szBtMacString);
			szBtMacString[17] ='\0';
...
static DRV_Error P_BT_MGT_Read_Config(BT_DEV_SETTING_t *pstHostSettings)
{
	...
	else
	{
		...
		VK_strncpy((char *)pstHostSettings->ucName, (char *)app_xml_config.name, sizeof(app_xml_config.name));
		...
		VK_strncpy(pstHostSettings->cRootPath, app_xml_config.root_path, sizeof(app_xml_config.root_path));
...
static DRV_Error P_BT_MGT_Write_Config(BT_DEV_SETTING_t *pstHostSettings)
{
	...
	VK_strncpy((char *)xml_config.name, (char *)pstHostSettings->ucName, sizeof(xml_config.name));
	...
	VK_strncpy(xml_config.root_path, pstHostSettings->cRootPath, sizeof(pstHostSettings->cRootPath));
...
static DRV_Error P_BT_MGT_Open(BT_DEV_SETTING_t *pstLocalBt)
{
	...
	if (VK_strlen(pstLocalBt->cRootPath) >= sizeof(bsa_open_param.uipc_path))
	{
...
/*	DRV_Error drvResult = DRV_OK;*/
	...
	for (index = 0; index < s_stBtInstance.stPairDevInfo.xmlCnt; index++)
	{
		...
		{
			VK_MEM_Memcpy(pstDevList->stBtDevList[index].ucName, s_stBtInstance.stPairDevInfo.devs[index].name, sizeof(s_stBtInstance.stPairDevInfo.devs[index].name));
...
#if defined(CONFIG_BLUETOOTH_BLE)
			pstDevList->stBtDevList[index].ucBleAddrType = s_stBtInstance.stPairDevInfo.devs[index].ble_addr_type;
			pstDevList->stBtDevList[index].ucDevType = s_stBtInstance.stPairDevInfo.devs[index].device_type;

			if ((s_stBtInstance.stPairDevInfo.devs[index].device_type == DI_BT_DEV_TYPE_BLE))
			{
				pstDevList->stBtDevList[index].etAvailableProfile = DI_BT_PROFILE_BLE;
			}
			else
			{
...
static DRV_Error P_BT_GetDiscDevList(DI_BT_DEV_LIST_t *pstDevList)
{
	...
	for (index = 0 ; index < BT_NB_DISC_DEVICES; index++)
	{
		...
		{
			VK_MEM_Memcpy(pstDevList->stBtDevList[index].ucName, s_stBtInstance.discDevInfo.devs[index].device.name, sizeof(s_stBtInstance.discDevInfo.devs[index].device.name));
...
#if defined(CONFIG_BLUETOOTH_BLE)
			if (s_stBtInstance.discDevInfo.devs[index].device.device_type == DI_BT_DEV_TYPE_BLE)
			{
				pstDevList->stBtDevList[index].etAvailableProfile = DI_BT_PROFILE_BLE;
			}
			else
			...
			pstDevList->stBtDevList[index].ucBleAddrType = s_stBtInstance.discDevInfo.devs[index].device.ble_addr_type;
			pstDevList->stBtDevList[index].ucDevType = s_stBtInstance.discDevInfo.devs[index].device.device_type;
#else
			pstDevList->stBtDevList[index].etAvailableProfile = s_stBtInstance.discDevInfo.devs[index].device.services;
#endif
			ulNumDisc++;
...
#if defined(CONFIG_BLUETOOTH_BLE)
				PrintDebug("\tinq_result_type:%d ble_addr_type:%d device_type:%d",
...
#if defined(CONFIG_BLUETOOTH_BLE)
		stBsaServices |= (ulServiceMask & DI_BT_PROFILE_BLE)  ? BTA_BLE_SERVICE_MASK  : 0;
#endif
	}
...
//			P_BT_PrintDeviceList(&s_sCbEvtDevInfo);		/* DI_DEBUG */
	...
	case BSA_SEC_AUTHORIZE_EVT:	/* Authorization request *//* When A Remote Device tries to connect HMS */
		...
		if (VK_strlen((char *)p_data->authorize.bd_name) > 0)
		{
			app_xml_update_name_db(app_xml_remote_devices_db,
					APP_NUM_ELEMENTS(app_xml_remote_devices_db),
	...
	case BSA_SEC_BLE_KEY_EVT: /* BLE KEY event */
		PrintDebug("BSA_SEC_BLE_KEY_EVT\n");
		switch (p_data->ble_key.key_type)
		{
		case BSA_LE_KEY_PENC:
			PrintDebug("[%s] key_type: BTM_LE_KEY_PENC(%d)\n", __FUNCTION__,  p_data->ble_key.key_type);
			//APP_DUMP("LTK", p_data->ble_key.key_value.penc_key.ltk, 16);
			//APP_DUMP("RAND", p_data->ble_key.key_value.penc_key.rand, 8);
			PrintDebug("[%s]   ediv: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.penc_key.ediv);
			PrintDebug("[%s]   sec_level: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.penc_key.sec_level);
			PrintDebug("[%s]   key_size: %d\n", __FUNCTION__, p_data->ble_key.key_value.penc_key.key_size);
			break;
		case BSA_LE_KEY_PID:
			P_BT_ConvertHex2Str(p_data->ble_key.key_value.pid_key.static_addr, szBdAddr);
			PrintDebug("[%s] key_type: BTM_LE_KEY_PID(%d)\n", __FUNCTION__, p_data->ble_key.key_type);
			//APP_DUMP("IRK", p_data->ble_key.key_value.pid_key.irk, 16);
			PrintDebug("[%s]   addr_type: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.pid_key.addr_type);
			PrintDebug("[%s]   static_addr: %s\n", __FUNCTION__, szBdAddr);
			break;
		case BSA_LE_KEY_PCSRK:
			PrintDebug("[%s] key_type: BTM_LE_KEY_PCSRK(%d)\n", __FUNCTION__, p_data->ble_key.key_type);
			PrintDebug("[%s]   counter: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.pcsrk_key.counter);
			//APP_DUMP("CSRK", p_data->ble_key.key_value.pcsrk_key.csrk, 16);
			PrintDebug("[%s]   sec_level: %d\n", __FUNCTION__, p_data->ble_key.key_value.pcsrk_key.sec_level);
			break;
		case BSA_LE_KEY_LCSRK:
			PrintDebug("[%s] key_type: BTM_LE_KEY_LCSRK(%d)\n", __FUNCTION__, p_data->ble_key.key_type);
			PrintDebug("[%s]   counter: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.lcsrk_key.counter);
			PrintDebug("[%s]   div: %d\n", __FUNCTION__, p_data->ble_key.key_value.lcsrk_key.div);
			PrintDebug("[%s]   sec_level: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.lcsrk_key.sec_level);
			break;
		case BSA_LE_KEY_LENC:
			PrintDebug("[%s] key_type: BTM_LE_KEY_LENC(%d)", __FUNCTION__, p_data->ble_key.key_type);
			PrintDebug("[%s]   div: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.lenc_key.div);
			PrintDebug("[%s]   key_size: %d\n", __FUNCTION__, p_data->ble_key.key_value.lenc_key.key_size);
			PrintDebug("[%s]   sec_level: 0x%x\n", __FUNCTION__, p_data->ble_key.key_value.lenc_key.sec_level);
			break;
		case BSA_LE_KEY_LID:
			PrintDebug("[%s] key_type: BTM_LE_KEY_LID(%d)\n", __FUNCTION__, p_data->ble_key.key_type);
			break;
		default:
				PrintDebug("[%s] key_type: Unknown key(%d)\n", __FUNCTION__, p_data->ble_key.key_type);
			break;
		}
		...
		if ( status < 0)
		{
			PrintError("[%s] app_xml_write_remote_devices failed:%d\n",__FUNCTION__, status);
		}
		break;

	case BSA_SEC_BLE_PASSKEY_REQ_EVT:
		P_BT_ConvertHex2Str(p_data->ble_passkey_req.bd_addr, szBdAddr);
		PrintDebug("[%s] BSA_SEC_BLE_PASSKEY_REQ_EVT (Passkey Request) received from\n", __FUNCTION__);
		PrintDebug("[%s]   bd_addr: %s\n", __FUNCTION__, szBdAddr);
		BSA_SecPinCodeReplyInit(&pin_code_reply);
		bdcpy(pin_code_reply.bd_addr, p_data->pin_req.bd_addr);
		pin_code_reply.is_ble = TRUE;
		pin_code_reply.ble_accept = TRUE;
		pin_code_reply.ble_passkey = 0;
		status = BSA_SecPinCodeReply(&pin_code_reply);
		if (status != BSA_SUCCESS)
		{
			PrintError("[%s] BSA_SecPinCodeReply Failed %d", __FUNCTION__, status);
		}
		break;
#endif
...
static DRV_Error P_BT_TM_ReadVersion(DI_BT_VERSION_t *pstBTVersion)
{
	...
	pstBTVersion->status = bsa_read_version.status;
	VK_sprintf(pstBTVersion->ucFwVer, "%d.%d.%d.%d",
			bsa_read_version.fw_version.major, bsa_read_version.fw_version.minor,
			bsa_read_version.fw_version.build, bsa_read_version.fw_version.config);
	VK_sprintf(pstBTVersion->ucBsaVer, "%s", bsa_read_version.bsa_server_version);
	return DRV_OK;
}
...
static DRV_Error P_BT_Enable(void)
{
...
#if defined(CONFIG_BLUETOOTH_BLE)
	/* Initialize BLE Profile */
	drvResult = P_BT_BLE_Init();
	if(drvResult != DRV_OK)
	{
		PrintError("[%s] P_BT_BLE_Init failed!\n", __FUNCTION__);
	}
#endif
	...
	audio_fd = fopen(HUMAX_AUDIO_RECORD_FILE, "w");
	if(audio_fd < 0)
	{
		PrintError("[%s] File open error!!\n", __FUNCTION__);
	}
#endif
#if defined (BT_STATUS_CHECK)
...
#if defined(CONFIG_BLUETOOTH_BLE)
	/* Deinit BLE */
	drvResult = P_BT_BLE_Deinit();
	if(drvResult != DRV_OK)
	{
		PrintError("[%s] P_BT_BLE_Deinit failed!\n", __FUNCTION__);
	}
#endif
...
static DRV_Error P_BT_Connect(HUINT32 ulDevId)
{
	...
	else if((pstConnectDev->etAvailableProfile & DI_BT_PROFILE_HID) || ((pstConnectDev->ucClass[1]&0x1F) == BTM_COD_MAJOR_PERIPHERAL))
	{
		/* TODO: Phase 2? */
...
#if defined(CONFIG_BLUETOOTH_BLE)
	else if((pstConnectDev->etAvailableProfile & DI_BT_PROFILE_BLE))
	{
		drvResult = P_BT_BLE_Connect(pstConnectDev);
		if (drvResult != DRV_OK)
		{
			PrintError("[%s] P_BT_BLE_Connect failed:%d \n", __FUNCTION__, drvResult);
			stCbData.eEvent = DI_BT_NOTIFY_EVT_CONNECT_FAIL;
			stCbData.ulDevId = ulDevId;
			stCbData.ulNumOfData = s_sCbEvtDevInfo.ulNumOfData;
			stCbData.pucBdAddr = (HUINT8 *)&pstConnectDev->ucBdAddr;
...
static DRV_Error P_BT_Init(void)
{
	...
	else
	{
		...
		VK_snprintf(s_stBtInstance.stHostSettings.ucName, sizeof(s_stBtInstance.stHostSettings.ucName), BT_DEFAULT_BT_NAME);			/* Default : HMS-1000S */
		...
		VK_snprintf(s_stBtInstance.stHostSettings.cRootPath, sizeof(s_stBtInstance.stHostSettings.cRootPath), BT_DEFAULT_UIPC_PATH);	/* Default : BT_DEFAULT_UIPC_PATH*/

		VK_snprintf(s_stBtInstance.stHostSettings.ucPinCode,sizeof(s_stBtInstance.stHostSettings.ucPinCode), BT_DEFAULT_PINCODE);		/* Default : 0000 */
...
static DRV_Error P_BT_Open(void)
{
	...
	if (TRUE == P_BT_Dev_IsEnabled())
	{
		...
		VK_snprintf(cSysCmd, sizeof(cSysCmd), "bsa_server -u %s -p %s/%s.hcd -d %s -all=0 &", BT_DEFAULT_UIPC_PATH, BT_FIRMWARE_PATH, BT_CHIP, BT_DEVNAME);
...
static void P_BT_AV_Callback(tBSA_AV_EVT event, tBSA_AV_MSG *p_data)
{
	...
	case BSA_AV_STOP_EVT:
		...
		/* Audio Capture Stop */
		P_BT_NEXUS_AudioCapture_Stop();

		/* Update s_stBtAvDevInfo */
		if (p_data->stop.pause == FALSE)
		{
			for( i = 0 ; i < BT_AV_DEVICE_MAX;i++)
			{
				if(s_stBtAvDevInfo.stAvTable[i].bInUse == TRUE)
...
/*
	case BSA_AV_FEAT_EVT:
		PrintDebug("[%s] BSA_AV_FEAT_EVT Peer feature:%x\n", __FUNCTION__, p_data->rc_feat.peer_features);
*/
	default:
...
static DRV_Error P_BT_AV_Register(void)
{
	...
	{
		status = BSA_AvRegisterInit(&stParams_register);
		if (BSA_SUCCESS != status)
		{
			PrintError("[%s] BSA_AvRegisterInit failed status:%d \n", __FUNCTION__, status);
			return DRV_ERR;
		}

		status = BSA_AvRegister(&stParams_register);
		if (BSA_SUCCESS != status)
		{
			PrintError("[%s] BSA_AvRegister failed status:%d \n", __FUNCTION__, status);
			return DRV_ERR;
		}
...
static DRV_Error P_BT_AV_Deinit(void)
{
	...
	for (index = 0 ; index < BT_AV_DEVICE_MAX;index++)
	{
		...
		{
			PrintDebug("[%s] Disconnecting Av Device : %d\n", __FUNCTION__, s_stBtAvDevInfo.stAvTable[index].handle);

			if (s_stBtAvDevInfo.stAvTable[index].bIsStreamActive == TRUE)
			{
				drvResult = P_BT_AV_Stop();
				if (drvResult != DRV_OK)
				{
				...
				while (s_stBtAvDevInfo.stAvTable[index].bIsStreamActive != FALSE)
				{
					VK_TASK_Sleep(10);
				}
			}
...
DRV_Error P_BT_AV_Start(void)
{
	...
	ulSampleRate = P_BT_NEXUS_AudioCapture_GetStatusSampleRate();
	stParams_start.media_feeding.cfg.pcm.sampling_freq = ulSampleRate;
	PrintError("[%s] sampling frequency%d\n", __FUNCTION__, stParams_start.media_feeding.cfg.pcm.sampling_freq);

	UIPC_Ioctl(s_stBtAvDevInfo.stUipcChannel, UIPC_WRITE_BLOCK, NULL);

	stParams_start.feeding_mode = BSA_AV_FEEDING_ASYNCHRONOUS;
	...
	switch (event)
	{
		case BSA_HH_OPEN_EVT:
			...
			if (p_data->open.status == BSA_SUCCESS)
			{
				...
				s_sCbEvtDevInfo.stBtDevList[ulDevId].etDevStatus = DI_BT_DEV_CONNECTED;
#if defined(CONFIG_BLUETOOTH_BLE)
				if (p_data->open.le_hid == TRUE)
				{
					s_sCbEvtDevInfo.stBtDevList[ulDevId].etAvailableProfile |= DI_BT_PROFILE_BLE;
				}
				else
				{
...
static DRV_Error P_BT_BLE_Init(void)
{
	...
	if (status != BSA_SUCCESS)
	{
		PrintError("[%s] Unable to enable BLE status:%d\n", __FUNCTION__, status);
		return DRV_ERR;
	}

	BSA_BleClAppRegisterInit(&ble_appreg_param);
	ble_appreg_param.uuid.len = 2;
    ble_appreg_param.uuid.uu.uuid16 = BT_BLE_DEFAULT_APPL_UUID;
    ble_appreg_param.p_cback = P_BT_BLE_Cl_Profile_Callback;
	status = BSA_BleClAppRegister(&ble_appreg_param);
	if (status != BSA_SUCCESS)
	{
		PrintError("[%s] BSA_BleClAppRegister fails status:%d\n", __FUNCTION__, status);
		return DRV_ERR;
	}
...
static DRV_Error P_BT_BLE_Deinit(void)
{
	...
	if (status != BSA_SUCCESS)
	{
		PrintError("[%s] BSA_BleDisable fails status:%d\n", __FUNCTION__, status);
		return DRV_ERR;
	}
...
static DRV_Error P_BT_BLE_Connect(DI_BT_DEV_INFO_t *pstHidDevInfo)
{
	...
	hh_open_param.sec_mask = BTA_SEC_NONE | BSA_SEC_AUTHENTICATION | BSA_SEC_ENCRYPTION;

	status = BSA_HhOpen(&hh_open_param);
	if (status != BSA_SUCCESS)
	{
		PrintError("[%s] BSA_HhOpen failed: %d\n", __FUNCTION__, status);
	}
	else
	{
		PrintDebug("[%s]Connecting to BLE device:%s\n", __FUNCTION__, pstHidDevInfo->ucName);
		/* XML Database update (Service) will be done upon reception of HH OPEN event */
		/* Read the Remote device XML file to have a fresh view */
		...
		app_xml_update_ble_addr_type_db(app_xml_remote_devices_db,
                        APP_NUM_ELEMENTS(app_xml_remote_devices_db), pstHidDevInfo->ucBdAddr, pstHidDevInfo->ucBleAddrType);
		app_xml_update_device_type_db(app_xml_remote_devices_db,
                        APP_NUM_ELEMENTS(app_xml_remote_devices_db), pstHidDevInfo->ucBdAddr, pstHidDevInfo->ucDevType);
		/* Update database => write on disk */
		drvResult = app_write_xml_remote_devices();
		if (drvResult != DRV_OK)
		{
...
static void P_BT_BLE_Cl_Profile_Callback(tBSA_BLE_EVT event, tBSA_BLE_MSG* p_data)
{
    ...
    switch (event)
    {
        ...
        case BSA_BLE_CL_WRITE_EVT:
            PrintError("BSA_BLE_CL_WRITE_EVT, status:%d\n", p_data->cli_write.status);
            break;

        case BSA_BLE_CL_NOTIF_EVT:
            PrintError("BSA_BLE_CL_NOTIF_EVT BDA :%02X:%02X:%02X:%02X:%02X:%02X\n",
            ...
            PrintError("conn_id:0x%x, svrc_id:0x%04x, char_id:0x%04x, descr_type:0x%04x, len:%d, is_notify:%d\n",
            ...
            if(p_data->cli_notif.is_notify != TRUE)
            {
                ...
                PrintError("Receive Indication! send Indication Confirmation!\n");
                status = BSA_BleClIndConfInit(&ind_conf);
                if (status < 0)
                {
                    PrintError("BSA_BleClIndConfInit failed: %d\n", status);
                    break;
                }
                ...
                if (status < 0)
                {
                    PrintError("BSA_BleClIndConf failed: %d\n", status);
                    break;
                }
            }
            break;
        ...
        case BSA_BLE_CL_CACHE_SAVE_EVT:
            PrintDebug("BSA_BLE_CL_CACHE_SAVE_EVT\n");
            PrintDebug("evt:0x%x, attr:0x%x, num_attr:%d, conn_id:0x%x\n",
                      p_data->cli_cache_save.evt ,p_data->cli_cache_save.attr,
                      p_data->cli_cache_save.num_attr ,p_data->cli_cache_save.conn_id);
            PrintDebug("BDA:%02X:%02X:%02X:%02X:%02X:%02X  \n",
            ...
            {
                PrintDebug("\t Attr[0x%04x] handle[%d] uuid[0x%04x] inst[%d] type[%d] prop[0x%1x] is_primary[%d]\n",
        ...
        case BSA_BLE_CL_CACHE_LOAD_EVT:
            PrintError("BSA_BLE_CL_CACHE_LOAD_EVT\n");
            PrintError("BDA:%02X:%02X:%02X:%02X:%02X:%02X  id:0x%x\n",
            ...
            {
                PrintError("No information of BDA:%02X:%02X:%02X:%02X:%02X:%02X  \n",
                      p_data->cli_cache_load.bd_addr[0], p_data->cli_cache_load.bd_addr[1],
                      p_data->cli_cache_load.bd_addr[2], p_data->cli_cache_load.bd_addr[3],
                      p_data->cli_cache_load.bd_addr[4], p_data->cli_cache_load.bd_addr[5]);
                app_ble_client_load_attr(NULL, p_data->cli_cache_load.bd_addr, p_data->ser_create.server_if);
            }

            break;

        default:
            PrintError("EVT_NUM[%d]\n", event);
            break;
    }
}
#endif
...
DRV_Error DRV_BT_Init(void)
{
	...
	/* Load KO drivers */
	VK_MEM_Memset(cSysCmd, 0x0, SYSTEM_CMD_MAX);
	VK_snprintf(cSysCmd, sizeof(cSysCmd), "%s", "modprobe btusb.ko");
	(void)VK_SYSTEM_Command((char *)cSysCmd);

	VK_MEM_Memset(cSysCmd, 0x0, SYSTEM_CMD_MAX);
	VK_snprintf(cSysCmd, sizeof(cSysCmd), "%s", "modprobe bthid.ko");
	(void)VK_SYSTEM_Command((char *)cSysCmd);
	...
	/* Audio_Init*/
	vResult = P_BT_Audio_Init();
	if (vResult != DRV_OK)
	{
		PrintError("[%s] P_BT_Audio_Init failed:%d \n", __FUNCTION__, vResult);
		return DRV_ERR;
	}
...
DI_ERR_CODE DI_BT_ReadVersion(DI_BT_VERSION_t *pstBTVersion)
{
	...
	drvResult = P_BT_TM_ReadVersion(pstBTVersion);
	if (drvResult != DRV_OK)
	{
...
DI_ERR_CODE DI_BT_DelXmlFiles(void)
{
	...
	/* Delete .xml files */
	VK_MEM_Memset(cSysCmd, 0x0, SYSTEM_CMD_MAX);
	VK_snprintf(cSysCmd, sizeof(cSysCmd), "%s", "rm -f /var/lib/humaxtv_user/bt_*.xml");
...
DI_ERR_CODE DI_BT_SetTestMode(HBOOL bTestMode)
{
	...
	bsa_set_test_mode.test_mode_enable = bTestMode;
	bsa_status = BSA_TmSetTestMode(&bsa_set_test_mode);
	if (bsa_status != BSA_SUCCESS)
	{
		PrintError("[%s] BSA_TmSetTestMode failed mode_requested:%d status:%d\n",__FUNCTION__, bTestMode, bsa_status);
		return DI_ERR_ERROR;
	}
	return DI_ERR_OK;
}
...
HUINT32 DI_BT_GetPatchramCRC(void)
{
	...
	VK_snprintf(acBuf, sizeof(acBuf), "%s/%s.hcd", BT_FIRMWARE_PATH, BT_CHIP);

	fp = fopen(acBuf,"r");
	if (fp == NULL)
	{
		PrintError("[%s] ERR! while opening the file [%s]\n",__FUNCTION__, acBuf);
		dResult = DI_ERR_ERROR;
		goto DONE;
	}
...
DONE:
	...
	if(DI_ERR_OK != dResult)
	{
		return dResult;
	}else
	{
		return (HUINT32)(~ulTmpCrc32);
	}
}
...
DI_ERR_CODE DI_BT_SetDiscoverableMode(HBOOL bDiscoverableMode)
{
	...
	s_stBtInstance.stHostSettings.bEnable = TRUE;
	s_stBtInstance.stHostSettings.bDiscoverable = bDiscoverableMode;
	s_stBtInstance.stHostSettings.bConnectable = TRUE;
...
DI_ERR_CODE DI_BT_HID_SendData(HUINT32 ulDevId)
{
	...
	if(!(s_sCbEvtDevInfo.stBtDevList[ulDevId].etAvailableProfile & DI_BT_PROFILE_HID) && ((s_sCbEvtDevInfo.stBtDevList[ulDevId].ucClass[1]&0x1F) != BTM_COD_MAJOR_PERIPHERAL))
	{
		PrintError("[%s] : Error: %s do not support HID profile\n", __FUNCTION__, s_sCbEvtDevInfo.stBtDevList[ulDevId].ucName);
		return DI_ERR_ERROR;
	}

	pstDev = &s_sCbEvtDevInfo.stBtDevList[ulDevId];
	pstAppHHDev = app_hh_pdev_find_by_bdaddr(pstDev->ucBdAddr);

	BSA_HhSendDataInit(&hh_send_data_param);
	hh_send_data_param.handle = pstAppHHDev->handle;
	hh_send_data_param.data.length = 2;
	hh_send_data_param.data.data[0] = 0x01;
	hh_send_data_param.data.data[1] = 0xFF;
	status = BSA_HhSendData(&hh_send_data_param);

	if (status != BSA_SUCCESS)
	{
		PrintError("[%s] BSA_HhSet fail status:%d", __FUNCTION__, status);
		return DI_ERR_ERROR;
	}
