...
int VK_INTR_Init(void)
{
	...
	{
		VK_snprintf(name, sizeof(name)-1, "intr_msg%d", i);
		...
		VK_snprintf(name, sizeof(name)-1, "intr_task%d", i);
		err = VK_TASK_Create(
				intr_task,
				USER_PRIORITY11,
...
void VK_INTR_PrintIntrList(void)
{
	...
	while(p)
	{
		...
		VK_snprintf(str, sizeof(str)-1, "Name(%s)\tIntrTask%d\tCallback(0x%x)", p->name, (int)p->msgId, (unsigned int)p->CallBackFunc);
		VK_printf("\n##  %s", str);
