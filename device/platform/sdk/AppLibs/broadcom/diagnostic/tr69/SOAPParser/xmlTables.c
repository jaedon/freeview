...
XMLFUNC(fULCommandKey);
XMLFUNC(fUploadURL);
XMLFUNC(fUploadUser);
XMLFUNC(fUploadPwd);
XMLFUNC(fUploadDelaySec);
static XmlNodeDesc upLoadDesc[] = {
    {NULL,  "CommandKey", fULCommandKey,NULL},
    {NULL,  "FileType", fUploadFileType,NULL},
    {NULL,  "URL", fUploadURL,NULL},
    {NULL,  "Username", fUploadUser,NULL},
    {NULL,  "Password", fUploadPwd,NULL},
    {NULL,  "DelaySeconds", fUploadDelaySec,NULL},
    {NULL,  "SuccessURL", NULL,NULL},
    {NULL,  "FailureURL", NULL,NULL},
        {NULL, NULL, NULL, NULL}
};
...
static XML_STATUS fULCommandKey(const char *name, TOKEN_TYPE ttype, const char *value)
{
        BSTD_UNUSED(name);
        BSTD_UNUSED(value);

    if (ttype == TAGDATA){
        rpcAction->ud.uploadReq.commandKey = strdup(value);
    }
    return XML_STS_OK;
}
...
static XML_STATUS fUploadFileType(const char *name, TOKEN_TYPE ttype, const char *value)
{
    ...
    if (ttype == TAGDATA)
        rpcAction->ud.uploadReq.efileType = atoi(value);
    return XML_STS_OK;
}
static XML_STATUS fUploadURL(const char *name, TOKEN_TYPE ttype, const char *value)
{
        BSTD_UNUSED(name);
        BSTD_UNUSED(value);

    if (ttype == TAGDATA)
        rpcAction->ud.uploadReq.url = strdup(value);
    return XML_STS_OK;
}
static XML_STATUS fUploadUser(const char *name, TOKEN_TYPE ttype, const char *value)
{
        BSTD_UNUSED(name);
        BSTD_UNUSED(value);

    if (ttype == TAGDATA)
        rpcAction->ud.uploadReq.user = strdup(value);
    return XML_STS_OK;
}
static XML_STATUS fUploadPwd(const char *name, TOKEN_TYPE ttype, const char *value)
{
        BSTD_UNUSED(name);
        BSTD_UNUSED(value);

    if (ttype == TAGDATA)
        rpcAction->ud.uploadReq.pwd = strdup(value);
    return XML_STS_OK;
}

static XML_STATUS fUploadDelaySec(const char *name, TOKEN_TYPE ttype, const char *value)
{
        BSTD_UNUSED(name);
        BSTD_UNUSED(value);

    if (ttype == TAGDATA)
        rpcAction->ud.uploadReq.delaySec = atoi(value);
    return XML_STS_OK;
}
