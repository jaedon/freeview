...
typedef struct ACSState {
    ...
    char        *uploadCommandKey;    /* key for upload cmd*/
    char        *connReqURL;
    ...
    int         ulFaultStatus;  /* upload fault status */
    char        *ulFaultMsg;    /* upload fault message */
    time_t      startULTime;    /* start upload time */
    time_t      endULTime;      /* complete upload time*/
} ACSState;
#else
typedef struct ACSState {
    char        acsURL[DEF_CONFIG_LEN];        /* URL of ACS */
    char        acsNewURL[DEF_CONFIG_LEN];     /* New URL if URL has been changed*/
    char        acsUser[DEF_CONFIG_LEN];
    char        acsPwd[DEF_CONFIG_LEN];
    time_t      informTime;     /* next ACS inform Time */
    time_t      informInterval; /* inform interval */
    int         informEnable;   /* True if inform to be performed*/
    int         maxEnvelopes;   /* Number of max env returned in inform response*/
    ...
    int         noMoreRequests; /* don't send any more Req to ACS */
    RpcMethods  acsRpcMethods;  /* methods from GetRPCMethods response*/
    char        parameterKey[DEF_CONFIG_LEN];  /* update key for ACS - may be NULL */
    char        newParameterKey[DEF_CONFIG_LEN];  /* the pending key */
    char        rebootCommandKey[DEF_CONFIG_LEN]; /* key for reboot command key */
    char        downloadCommandKey[DEF_CONFIG_LEN];    /* key for download cmd*/
    char        uploadCommandKey[DEF_CONFIG_LEN];    /* key for upload cmd*/
    char        connReqURL[DEF_CONFIG_LEN];
    char        connReqPath[DEF_CONFIG_LEN];   /* path part of connReqURL -- used by listener */
    char        connReqUser[DEF_CONFIG_LEN];
    char        connReqPwd[DEF_CONFIG_LEN];
    char        kickURL[DEF_CONFIG_LEN];
    char        upgradesManaged;
    char        provisioningCode[DEF_CONFIG_LEN];
    int         retryCount;     /* reset on each ACS response*/
    int         fault;          /* last operation fault code */
    int         dlFaultStatus;  /* download fault status */
    char        dlFaultMsg[DEF_CONFIG_LEN];    /* download fault message */
    time_t      startDLTime;    /* start download time */
    time_t      endDLTime;      /* complete download time*/
    int         ulFaultStatus;  /* upload fault status */
    char        ulFaultMsg[DEF_CONFIG_LEN];    /* upload fault message */
    time_t      startULTime;    /* start upload time */
    time_t      endULTime;      /* complete upload time*/
} ACSState;
#endif
