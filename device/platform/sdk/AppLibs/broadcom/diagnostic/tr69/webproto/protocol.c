...
static void server_wait_for_ssl(void *handle) {
    ...
    if ((res = SSL_connect(pc->ssl)) <= 0) {
        ...
        if (sslres == SSL_ERROR_WANT_READ) {
            ...
            slog(LOG_DEBUG,"Error connecting to server(Possible certificate error): (res=%d,sslres=%d) %s",
                 res, sslres, errstr );
...
static char XWWWAuthenticateStr[]="X-WWW-Authenticate:";
...
void proto_ParseHdrs(tProtoCtx *pc, tHttpHdrs *hdrs)
{
    ...
    while ((n = proto_Readline(pc, buf, BUF_SIZE_MAX)) > 0) {
        ...
        } else if (strncasecmp(buf, XWWWAuthenticateStr, sizeof(XWWWAuthenticateStr)-1)==0) {
            cp =&buf[sizeof(XWWWAuthenticateStr)-1];
            cp += strspn(cp, " \t");
