 ...
 * Revision History:
 *
 * $brcm_Log: /AppLibs/opensource/directfb/src/broadcom_files/private/DirectFB/1.4.5/systems/bcmnexus/gl/gl_light.c $
 * 
	...
	GLsizei iCount )
{
	...
	GL_P_Material aMaterials[GL_P_LIGHT_MAX];
	...
	GL_P_CPY_LIGHT_COLOR( &ColorA, &pContext->Material.EmissionColor );
	GL_P_MUL_LIGHT_COLOR( &ColorB, &pContext->AmbientColor, pAmbientColor );
	GL_P_ADD_LIGHT_COLOR( &ColorA, &ColorA, &ColorB );
	GL_P_SET_LIGHT_COLOR( &ColorB, 0.0, 0.0, 0.0, 0.0 );

	for( ii = 0; ii < GL_P_LIGHT_MAX; ++ii )
	{
		if( pContext->bLight[ii] )
		{
			GL_P_Material *pMaterial = &aMaterials[iLightCount];
			GL_P_Light *pLight = &pContext->aLights[ii];

			GL_P_MUL_LIGHT_COLOR( &pMaterial->AmbientColor, pAmbientColor, &pLight->AmbientColor );
			GL_P_MUL_LIGHT_COLOR( &pMaterial->DiffuseColor, pDiffuseColor, &pLight->DiffuseColor );
			GL_P_MUL_LIGHT_COLOR( &pMaterial->SpecularColor, &pContext->Material.SpecularColor, &pLight->SpecularColor );

			pMaterial->fSpecularExponent = pContext->Material.fSpecularExponent;
			apLights[iLightCount++] = pLight;
		}
	}
	...
	for( jj = 0; jj < iCount; ++jj )
	{
		...
		GL_P_Coord3f *pNormal = (GL_P_Coord3f *) (&pVertices->fS);
		GL_P_Color DiffColor = ColorA;
		GL_P_Color SpecColor = ColorB;

		for( ii = 0; ii < iLightCount; ++ii )
		{
			GL_P_Material *pMaterial = &aMaterials[ii];
			...
			GL_P_CPY_LIGHT_COLOR( &Color0, &pMaterial->AmbientColor );

			if( fVectorDotProduct != 0.0 )
			{
				GL_P_MUL_LIGHT_CONSTANT( &Color2, &pMaterial->DiffuseColor, fVectorDotProduct );
				GL_P_ADD_LIGHT_COLOR( &Color0, &Color0, &Color2 );

				if( pMaterial->fSpecularExponent != 0.0 )
				{
					...
					fShininess = (GLfloat) exp( pMaterial->fSpecularExponent * log( fSpotDot ) );
					GL_P_MUL_LIGHT_CONSTANT( &Color1, &pMaterial->SpecularColor, fShininess );
				}
				else
				{
					GL_P_CPY_LIGHT_COLOR( &Color1, &pMaterial->SpecularColor );
				}
			}
			else
			{
