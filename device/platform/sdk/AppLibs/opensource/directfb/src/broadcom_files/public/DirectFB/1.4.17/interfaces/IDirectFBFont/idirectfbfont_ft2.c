...
D_DEBUG_DOMAIN(fontFT2, "Font/FT2", "DirectFB Freetype Font Implementation");
...
#define FT_BOLD_STRENGTH(n)                 ((n) * (1<<6) * 5 / 100)        /* 5% */
#define FT_BOLD_STRENGTH_OUTLINE(n)         ((n) * (1<<6) * 8 / 100)        /* 8% */
...
typedef struct {
     ...
     int          stroking_radius;
     FT_Stroker_LineCap     stroking_line_cap;
     FT_Stroker_LineJoin    stroking_line_join;
              ...
              CoreGlyphData *info )
{
     ...
     if (err) {
          ...
          {
              FT_Done_Glyph((FT_Glyph)data->bitmap_glyph);
          }
          return err;
     }
     ...
     if (info->width + info->start > surface->config.size.w) {
         D_ERROR("%s() Surface not wide enough to hold glyph; clipping glyph width.\n", __FUNCTION__);
          info->width = surface->config.size.w - info->start;
     }

     info->height = face->glyph->bitmap.rows;
     if (info->height > surface->config.size.h) {
         D_ERROR("%s() Surface not tall enough to hold glyph; clipping glyph height.\n", __FUNCTION__);
                ...
                CoreGlyphData *info )
{
     ...
     FT_Int   load_flags;
     FT_Stroker     stroker;
     FT_Glyph       orig_glyph, modified_glyph;
     ...
     load_flags = (unsigned long) face->generic.data;
     if (data->stroking_enabled || data->embolden)
     {
     ...
     if ((err = FT_Load_Glyph( face, index, load_flags ))) {
          D_DEBUG_AT(fontFT2, "Could not load glyph for character index #%d!\n", index );
...
init_freetype( void )
{
     ...
     if (!library) {
          D_DEBUG_AT(fontFT2, "Initializing the FreeType2 library.\n" );
...
release_freetype( void )
{
     ...
     if (library && --library_ref_count == 0) {
          D_DEBUG_AT(fontFT2, "Releasing the FreeType2 library.\n" );
...
Probe( IDirectFBFont_ProbeContext *ctx )
{
     ...
     D_DEBUG_AT(fontFT2, "Probe font `%s'.\n", ctx->filename );
           ...
           DFBFontDescription          *desc )
{
     ...
     data->transform_enabled  = (transform_matrix != NULL);
     data->embolden           = embolden;
     data->stroking_enabled   = (desc->flags & DFDESC_ATTRIBUTES) && (desc->attributes & DFFA_OUTLINED);
     data->stroking_line_cap  = FT_STROKER_LINECAP_ROUND;
     data->stroking_line_join = FT_STROKER_LINEJOIN_ROUND;
     if (!(desc->flags & DFDESC_OUTLINE_WIDTH))
          desc->outline_width = (1<<16);
     data->stroking_radius    = desc->outline_width;
     data->fixed_advance      = (desc->flags & DFDESC_FIXEDADVANCE) ? desc->fixed_advance : 0;
