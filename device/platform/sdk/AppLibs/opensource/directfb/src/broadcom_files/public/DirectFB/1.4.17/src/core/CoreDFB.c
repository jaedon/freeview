...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            Core_PushCalling();
            ret = ICore_Real__CreateSurface( obj, config, type, resource_id, hints, palette, ret_surface );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            Core_PushCalling();
            ret = ICore_Requestor__CreateSurface( obj, config, type, resource_id, hints, palette, ret_surface );
            Core_PopCalling();
                                ...
                                unsigned int *ret_length )
{
    ...
    CoreSurface *surface = NULL;
            ...
            return_args->result = ICore_Real__CreateSurface( obj, &args->config, args->type, args->resource_id, args->hints, args->palette_set ? palette : NULL, &surface );
            if (return_args->result == DFB_OK) {
