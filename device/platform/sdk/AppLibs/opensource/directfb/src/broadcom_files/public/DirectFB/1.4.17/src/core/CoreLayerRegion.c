...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            Core_PushCalling();
            ret = ILayerRegion_Real__FlipUpdateStereo( obj, left, right, flags );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            Core_PushCalling();
            ret = ILayerRegion_Requestor__FlipUpdateStereo( obj, left, right, flags );
            Core_PopCalling();
...
DFBResult
ILayerRegion_Requestor__FlipUpdateStereo( CoreLayerRegion *obj,
                    const DFBRegion                           *left,
                    const DFBRegion                           *right,
                    DFBSurfaceFlipFlags                        flags
)
{
  ...
  if (right) {
    ...
    ret = (DFBResult) CoreLayerRegion_Call( obj, FCEF_NONE, _CoreLayerRegion_FlipUpdateStereo, args, sizeof(CoreLayerRegionFlipUpdateStereo), return_args, sizeof(CoreLayerRegionFlipUpdateStereoReturn), NULL );
    if (ret) {
