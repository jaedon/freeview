...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            Core_PushCalling();
            ret = ISurface_Real__PreLockBuffer2( obj, role, eye, accessor, access, lock, ret_allocation );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            Core_PushCalling();
            ret = ISurface_Requestor__PreLockBuffer2( obj, role, eye, accessor, access, lock, ret_allocation );
            Core_PopCalling();
...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            Core_PushCalling();
            ret = ISurface_Real__GetStereoEye( obj, ret_eye );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            Core_PushCalling();
            ret = ISurface_Requestor__GetStereoEye( obj, ret_eye );
            Core_PopCalling();
...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        case COREDFB_CALL_DIRECT:{
            Core_PushCalling();
            ret = ISurface_Real__SetStereoEye( obj, eye );
            Core_PopCalling();
        ...
        case COREDFB_CALL_INDIRECT: {
            Core_PushCalling();
            ret = ISurface_Requestor__SetStereoEye( obj, eye );
            Core_PopCalling();
...
DFBResult
ISurface_Requestor__GetStereoEye( CoreSurface *obj,
                    DFBSurfaceStereoEye                       *ret_eye
)
{
    DFBResult           ret;
    CoreSurfaceGetStereoEye       *args = (CoreSurfaceGetStereoEye*) alloca( sizeof(CoreSurfaceGetStereoEye) );
    CoreSurfaceGetStereoEyeReturn *return_args = (CoreSurfaceGetStereoEyeReturn*) alloca( sizeof(CoreSurfaceGetStereoEyeReturn) );

    D_DEBUG_AT( DirectFB_CoreSurface, "ISurface_Requestor::%s()\n", __FUNCTION__ );

    ret = (DFBResult) CoreSurface_Call( obj, FCEF_NONE, _CoreSurface_GetStereoEye, args, sizeof(CoreSurfaceGetStereoEye), return_args, sizeof(CoreSurfaceGetStereoEyeReturn), NULL );
    if (ret) {
        D_DERROR( ret, "%s: CoreSurface_Call( CoreSurface_GetStereoEye ) failed!\n", __FUNCTION__ );
        return ret;
    }

    if (return_args->result) {
         /*D_DERROR( return_args->result, "%s: CoreSurface_GetStereoEye failed!\n", __FUNCTION__ );*/
         return return_args->result;
    }

    *ret_eye = return_args->eye;
...
DFBResult
ISurface_Requestor__SetStereoEye( CoreSurface *obj,
                    DFBSurfaceStereoEye                        eye
)
{
    DFBResult           ret;
    CoreSurfaceSetStereoEye       *args = (CoreSurfaceSetStereoEye*) alloca( sizeof(CoreSurfaceSetStereoEye) );
    CoreSurfaceSetStereoEyeReturn *return_args = (CoreSurfaceSetStereoEyeReturn*) alloca( sizeof(CoreSurfaceSetStereoEyeReturn) );

    D_DEBUG_AT( DirectFB_CoreSurface, "ISurface_Requestor::%s()\n", __FUNCTION__ );

    args->eye = eye;

    ret = (DFBResult) CoreSurface_Call( obj, FCEF_NONE, _CoreSurface_SetStereoEye, args, sizeof(CoreSurfaceSetStereoEye), return_args, sizeof(CoreSurfaceSetStereoEyeReturn), NULL );
    if (ret) {
        D_DERROR( ret, "%s: CoreSurface_Call( CoreSurface_SetStereoEye ) failed!\n", __FUNCTION__ );
        return ret;
    }

    if (return_args->result) {
         /*D_DERROR( return_args->result, "%s: CoreSurface_SetStereoEye failed!\n", __FUNCTION__ );*/
         return return_args->result;
    }
                                ...
                                unsigned int *ret_length )
{
    ...
    CoreSurfaceAllocation *allocation = NULL;
            ...
            return_args->result = ISurface_Real__PreLockBuffer2( obj, args->role, args->eye, args->accessor, args->access, args->lock, &allocation );
            if (return_args->result == DFB_OK) {
                CoreSurfaceAllocation_Throw( allocation, caller, &return_args->allocation_id );
            }
    ...
    CoreSurfaceAllocation *allocation = NULL;
        ...
        case _CoreSurface_GetStereoEye: {
            D_UNUSED
            CoreSurfaceGetStereoEye       *args        = (CoreSurfaceGetStereoEye *) ptr;
            CoreSurfaceGetStereoEyeReturn *return_args = (CoreSurfaceGetStereoEyeReturn *) ret_ptr;

            D_DEBUG_AT( DirectFB_CoreSurface, "=-> CoreSurface_GetStereoEye\n" );

            return_args->result = ISurface_Real__GetStereoEye( obj, &return_args->eye );
            if (return_args->result == DFB_OK) {
            }

            *ret_length = sizeof(CoreSurfaceGetStereoEyeReturn);

            return DFB_OK;
        }

        case _CoreSurface_SetStereoEye: {
            D_UNUSED
            CoreSurfaceSetStereoEye       *args        = (CoreSurfaceSetStereoEye *) ptr;
            CoreSurfaceSetStereoEyeReturn *return_args = (CoreSurfaceSetStereoEyeReturn *) ret_ptr;

            D_DEBUG_AT( DirectFB_CoreSurface, "=-> CoreSurface_SetStereoEye\n" );

            return_args->result = ISurface_Real__SetStereoEye( obj, args->eye );
            if (return_args->result == DFB_OK) {
            }

            *ret_length = sizeof(CoreSurfaceSetStereoEyeReturn);
