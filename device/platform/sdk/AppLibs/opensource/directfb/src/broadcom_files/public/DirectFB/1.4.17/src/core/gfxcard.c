...
void dfb_gfxcard_filltriangles( const DFBTriangle *tris, int num, CardState *state )
{
     ...
     if (dfb_gfxcard_state_check( state, DFXL_FILLTRIANGLES ) &&
         dfb_gfxcard_state_acquire( state, DFXL_FILLTRIANGLES ))
     {
          if (!D_FLAGS_IS_SET( card->caps.flags, CCF_CLIPPING ) &&
              !D_FLAGS_IS_SET( card->caps.clip, DFXL_FILLTRIANGLES ))
          {
               DFBPoint p[6];
               int      n;
               ...
               for (; i < num; i++) {
                ...
                hw = card->funcs.FillTriangles( card->driver_data,
                                                card->device_data, (DFBTriangle *)tris, num );
          }

          dfb_gfxcard_state_release( state );
     }
     else if (dfb_gfxcard_state_check( state, DFXL_FILLTRIANGLE ) &&
