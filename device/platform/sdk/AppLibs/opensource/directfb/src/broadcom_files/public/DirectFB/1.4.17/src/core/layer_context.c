     ...
     if (context->primary.region) {
          ...
          if (region_config.buffermode != DLBM_WINDOWS) {
               ...
               if (surface) {
                    ...
                    if (region->surface) {
                         if (memcmp(&region->config, &region_config, sizeof(region_config)) || region->surface->num_buffers == 0) {
                                    ...
                                    CoreLayerRegionConfig *config )
{
     ...
     if (funcs->AllocateSurface) {
          ...
          ret = dfb_surface_create( layer->core, &scon, type, shared->layer_id, DSHF_LAYER, NULL, &surface );
          if (ret) {
