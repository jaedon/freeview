                    ...
                    if (D_FLAGS_IS_SET( region->state, CLRSF_REALIZED )) {
                         ...
                         if (funcs->FlipRegion)
                              ret = funcs->FlipRegion( layer,
                                                       ...
                                                       region->region_data,
                                                       surface, flags,
                                                       update, &region->left_buffer_lock,
                                                       NULL, NULL );

                         /* Unlock region buffer since the lock is no longer needed. */
                         region_buffer_unlock(region, surface, true);
                    }
                    else {
                    ...
                    if (D_FLAGS_IS_SET( region->state, CLRSF_REALIZED )) {
                         ...
                         if (funcs->FlipRegion)
                              ret = funcs->FlipRegion( layer,
                                                       ...
                                                       region->region_data,
                                                       surface, flags,
                                                       left_update, &region->left_buffer_lock,
                                                       right_update, &region->right_buffer_lock);
                    ...
                    CoreSurfaceBufferRole  role )
{
     ...
     D_DEBUG_AT( Core_Layers, "%s(): region=%p, lock buffer=%p, %p\n",
                 __FUNCTION__, (void *)region,
                 (void *)region->left_buffer_lock.buffer,
                 (void *)region->right_buffer_lock.buffer);

     stereo = surface->config.caps & DSCAPS_STEREO;

     /* First unlock any previously locked buffer(s). */
     if (region->left_buffer_lock.buffer) {
          D_MAGIC_ASSERT( region->left_buffer_lock.buffer, CoreSurfaceBuffer );
            ...
            CoreSurface                *surface )
{
     ...
     region_buffer_unlock(region, surface, false);
