...
static const u8 lookup4to8[] = { 0x00, 0x12, 0x24, 0x36, 0x49, 0x5b, 0x6d, 0x7f, 0x92, 0xa4, 0xb6, 0xc8, 0xdb, 0xed, 0xff };
...
void
dfb_palette_generate_lut4_map( CorePalette *palette )
{
     unsigned int i;
     DFBColor     entries[256];
     ...
     D_DEBUG_AT( Core_Palette, "%s(): palette->num_entries=%d\n", __FUNCTION__, palette->num_entries );

     for (i=0; i<palette->num_entries; i++) {
          entries[i].a = i ? 0xff : 0x00;
          entries[i].r = lookup4to8[ ((i & 0x0f) > 0)?((i & 0x0f) - 1):0 ];
          entries[i].g = lookup4to8[ ((i & 0x0f) < 8)?(i & 0x0f):(0x0f - (i & 0x0f)) ];
          entries[i].b = lookup4to8[ (i & 0x0f) ];
     }
