                            ...
                            CoreSurface            **ret_surface )
{
     ...
     return CoreDFB_CreateSurface( core, &config, type, resource_id, hints, palette, ret_surface );
}
                          ...
                          CoreSurface *surface )
{
     ...
     switch (surface->config.format) {
          ...
          case DSPF_LUT4:
               dfb_palette_generate_lut4_map( palette );
               break;
                      ...
                      const CoreSurfaceConfig *config )
{
     ...
     if (  (config->flags == CSCONF_SIZE ||
          ...
          ((config->flags == (CSCONF_SIZE | CSCONF_FORMAT | CSCONF_CAPS)) && (config->format == surface->config.format) &&
           config->caps == surface->config.caps))  &&
                         ...
                         CoreSurfaceBufferLock  *ret_lock )
{
     ...
     ret = CoreSurface_PreLockBuffer2( surface, role,
                                       dfb_surface_get_stereo_eye(surface),
...
dfb_surface_clear_buffers( CoreSurface *surface )
{
     ...
     if (surface->config.caps & DSCAPS_STEREO) {
          DFBSurfaceStereoEye eye = dfb_surface_get_stereo_eye( surface );

          dfb_surface_set_stereo_eye( surface, (eye == DSSE_LEFT) ? DSSE_RIGHT : DSSE_LEFT );
