...
static int Aop_odd_start = 0;
static int Bop_odd_start = 0;
static int *Sop_odd_start = NULL;
...
/* 4bits specific */
static const int four_bits_shift[2][2] = { {0, 4}, {4, 0} };
static const int four_bits_len[2][2] = { {0, 0}, {0, 1} };
...
static const bool is_ycbcr[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = false,
};
...
static void Cop_to_Aop_4( GenefxState *gfxs )
{
     int    l =  gfxs->length;
     u8    *D   = (u8*)gfxs->Aop[0];
     u8     C   = (u8)(gfxs->Cop & 0x0f);
     u8     Ch  = (C << 4);

     /* The first pixel is the higher 4bits of the first byte,
      * so fill it first */
     if (Aop_odd_start) {
          *D = (*D & 0x0f) | Ch;
          -- l;
     ...
     /* If remained length is odd, then fill the last half byte */
     if (l & 1) {
          l = l >> 1;
          memset( (void*)D, (C | Ch), l );
          *(D+l) = (*(D+l) & 0xf0) | C;
     }
     else if (l) {
          l = l >> 1;
...
static GenefxFunc Cop_to_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Cop_to_Aop_16,
};
...
static void Cop_toK_Aop_alut8( GenefxState *gfxs )
{
     int    w    = gfxs->length+1;
     u8    *D    = gfxs->Aop[0];
     u32    Cop  = gfxs->Cop;
...
static const GenefxFunc Cop_toK_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Cop_toK_Aop_alut8,
};

/********************************* Bop_PFI_to_Aop_PFI *************************/

static void Bop_LUT4_to_Aop( GenefxState *gfxs )
{
     u8    *S     = (u8*)gfxs->Bop[0];
     u8    *D     = (u8*)gfxs->Aop[0];
     int    Ostep = gfxs->Ostep;
     register u8    T;
     register int   l = gfxs->length;

     /* we can call memmove directly in this case,
      * otherwise we must do it byte by byte */
     if (Aop_odd_start == Bop_odd_start) {
          /* left to right */
          if (Ostep > 0) {
               if (Aop_odd_start) {
                    *D = (*D & 0x0f) | (*S & 0xf0);
               ...
               if (l & 1) {
                    l = l >> 1;
                    direct_memmove( (void*) D, (void*) S, l );
                    *(D+l) = (*(D+l) & 0xf0) | (*(S+l) & 0x0f);
               }
               else {
                    l = l >> 1;
                    direct_memmove( (void*) D, (void*) S, l );
          ...
          if (Ostep > 0 || l <= 1) {
               /* Fill in the first higher half byte */
               if (Aop_odd_start) {
                    *D = (*D & 0x0f) | (*S << 4);
                    -- l;
                    ++ D;
               }
               /* Fill in the remained whole bytes */
               while (l > 1) {
                    T  = (*(S++) >> 4);
                    T |= (*S << 4);
                    *(D++) = T;
               ...
               /* Fill in the last lower half byte */
               if (l > 0) {
                    *D = (*D & 0xf0) | (*S >> 4);
               }
          }
          else {
               D += (l >> 1) - (((l & 1) || Aop_odd_start)?0:1);
               S += (l >> 1) - (((l & 1) || Bop_odd_start)?0:1);

               /* Fill in the last lower half byte */
               if (((l&1) && !Aop_odd_start) || (!(l&1) && Aop_odd_start)) {
                    *D = (*D & 0xf0) | (*S >> 4);
                    -- l;
                    -- D;
               }
...
static const GenefxFunc Bop_PFI_to_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Bop_LUT4_to_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Bop_32_to_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_16_to_Aop,
};
...
static const GenefxFunc Bop_PFI_toR_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_16_toR_Aop,
};
...
static void Bop_4_Kto_Aop( GenefxState *gfxs )
{
     u8    *D     = (u8*)gfxs->Aop[0];
     u8    *S     = (u8*)gfxs->Bop[0];
     u8     K     = (u8)gfxs->Skey & 0x0f;
     u8     Kh    = (K << 4);
     int    Ostep = gfxs->Ostep;
     register u8    spixel;
     register u8    T;
     ...
     if (Aop_odd_start == Bop_odd_start) {
          if (Ostep > 0) {
               ...
               while ( l > 1) {
                    ...
                    spixel = *S & 0x0f;
                    if (spixel != K) T = spixel;
                    spixel = (*S & 0xf0);
                    if (spixel != Kh) T |= spixel;
...
static void Bop_alut8_Kto_Aop( GenefxState *gfxs )
{
     int    w     = gfxs->length+1;
     u8    *D     = gfxs->Aop[0];
     u8    *S     = gfxs->Bop[0];
...
static GenefxFunc Bop_PFI_Kto_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_A1_LSB)]   = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_YV16)]     = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Bop_4_Kto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Bop_32_Kto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_alut8_Kto_Aop,
};
...
static GenefxFunc Bop_PFI_toK_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = NULL,
};
...
static void Bop_4_Sto_Aop( GenefxState *gfxs )
{
     int   l     = gfxs->length;
     int   i     = 0;
     int   p     = 0;
     ...
     int   SperD = gfxs->SperD;
     register u8 spixel;
     int  Ss = four_bits_shift [Bop_odd_start][0];

     if (Aop_odd_start) {
          spixel = (S[four_bits_len[Bop_odd_start][p&1]] >> Ss) & 0x0f;
          *D = (*D & 0x0f) | (spixel << 4);
          -- l;
          ++ D;
          i += SperD;
     ...
     while (l > 1) {
          spixel = (S[(p>>1) + four_bits_len[Bop_odd_start][p&1]] >> Ss) & 0x0f;

          i += SperD;
          p = i >> 16;

          spixel |= ((S[(p>>1) + four_bits_len[Bop_odd_start][p&1]] >> Ss) << 4);
          ...
          *D = spixel;
          ...
          Ss = four_bits_shift [Bop_odd_start][p&1];
          ++ D;
          l -= 2;
     }
...
static GenefxFunc Bop_PFI_Sto_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Bop_4_Sto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Bop_32_Sto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_16_Sto_Aop,
};
...
static void Bop_4_SKto_Aop( GenefxState *gfxs )
{
     int   l     = gfxs->length+1;
     int   i     = 0;
     int   p     = 0;
     ...
     int   SperD = gfxs->SperD;
     int   Ss = four_bits_shift [Bop_odd_start][0];
     int   Ds = four_bits_shift [Aop_odd_start][0];

     while (--l) {
          spixel = (S[(p>>1) + four_bits_len[Bop_odd_start][p&1]] >> Ss) & 0x0f;

          if (spixel != (Skey & 0x0f))
               *D = (*D & (0xf0 >> Ds)) | (spixel << Ds);
...
static void Bop_alut8_SKto_Aop( GenefxState *gfxs )
{
     int    w     = gfxs->length+1;
     int    i     = gfxs->Xphase;
     u8    *D     = gfxs->Aop[0];
...
static const GenefxFunc Bop_PFI_SKto_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_A1_LSB)]   = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_YV16)]     = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Bop_4_SKto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Bop_32_SKto_Aop,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_alut8_SKto_Aop,
};
...
static void Sop_lut4_Sto_Dacc( GenefxState *gfxs )
{
     int l     = gfxs->length+1;
     int i     = 0;
     int p     = 0;
     int SperD = gfxs->SperD;
     int Ss    = four_bits_shift [*Sop_odd_start][0];
     u8  spixel;
     ...
     while (--l) {
          spixel = (S[(p>>1) + four_bits_len[*Sop_odd_start][p&1]] >> Ss) & 0x0f;

          D->RGB.a = entries[spixel].a;
          D->RGB.r = entries[spixel].r;
          D->RGB.g = entries[spixel].g;
          D->RGB.b = entries[spixel].b;

          i += SperD;
          p = i >> 16;
          Ss = four_bits_shift [*Sop_odd_start][p&1];
...
static void Sop_alut8_Sto_Dacc( GenefxState *gfxs )
{
     int w     = gfxs->length+1;
     int i     = gfxs->Xphase;
     int SperD = gfxs->SperD;
...
static GenefxFunc Sop_PFI_Sto_Dacc[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Sop_lut4_Sto_Dacc,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Sop_abgr_Sto_Dacc,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sop_alut8_Sto_Dacc,
};
...
static const GenefxFunc Sop_PFI_SKto_Dacc[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sop_lut8_SKto_Dacc,
};
...
static GenefxFunc Sop_PFI_to_Dacc[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sop_lut8_to_Dacc,
};
...
static const GenefxFunc Sop_PFI_Kto_Dacc[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sop_lut8_Kto_Dacc,
};
...
static void Sacc_to_Aop_lut4( GenefxState *gfxs )
{
     ...
     while (l > 1) {
          ...
          T = 0;
          if (!(S->RGB.a & 0xF000)) {
               T = (dfb_palette_search( gfxs->Alut,
                         (S->RGB.r & 0xFF00) ? 0xFF : S->RGB.r,
                         (S->RGB.g & 0xFF00) ? 0xFF : S->RGB.g,
                         (S->RGB.b & 0xFF00) ? 0xFF : S->RGB.b,
                         (S->RGB.a & 0xFF00) ? 0xFF : S->RGB.a ) & 0x0f);
               mask -= 0x0f;
          }
          ++ S;
...
static GenefxFunc Sacc_to_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sacc_to_Aop_lut8,
};
...
static GenefxFunc Sacc_Sto_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sacc_Sto_Aop_lut8,
};
...
static const GenefxFunc Sacc_toK_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Sacc_toK_Aop_lut8,
};
...
static void Bop_a8_set_alphapixel_Aop_lut4( GenefxState *gfxs )
{
     int    l  = gfxs->length;
     u8    *S  = gfxs->Bop[0];
     u8    *D  = gfxs->Aop[0];
...
#define SET_ALPHA_PIXEL_LUT4_LOW(d,alpha) \
     switch (alpha) {\
          case 0xff: d = (d & 0xf0) | C;\
          case 0: break; \
          default: {\
               register u16 s = alpha+1;\
               DFBColor      dc = gfxs->Alut->entries[d & 0x0f];\
               u16         sa = alpha + dc.a;\
               dc.r = ((color.r - dc.r) * s + (dc.r << 8)) >> 8;\
               dc.g = ((color.g - dc.g) * s + (dc.g << 8)) >> 8;\
               dc.b = ((color.b - dc.b) * s + (dc.b << 8)) >> 8;\
               d = (d & 0xf0) |\
                    (dfb_palette_search( gfxs->Alut, dc.r, dc.g, dc.b,\
                                             sa & 0xff00 ? 0xff : sa ) & 0x0f);\
          }\
     }

#define SET_ALPHA_PIXEL_LUT4_HIGH(d,alpha) \
     switch (alpha) {\
          case 0xff: d = (d & 0x0f) | Ch;\
          case 0: break; \
          default: {\
               register u16 s = alpha+1;\
               DFBColor      dc = gfxs->Alut->entries[d >> 4];\
               u16         sa = alpha + dc.a;\
               dc.r = ((color.r - dc.r) * s + (dc.r << 8)) >> 8;\
               dc.g = ((color.g - dc.g) * s + (dc.g << 8)) >> 8;\
               dc.b = ((color.b - dc.b) * s + (dc.b << 8)) >> 8;\
     ...
     if (Aop_odd_start) {
          SET_ALPHA_PIXEL_LUT4_HIGH ( *D, *S);
          ++ D;
          ++ S;
          -- l;
     }
     while (l > 1) {
          SET_ALPHA_PIXEL_LUT4_LOW ( *D, *S );
          ++ S;
          SET_ALPHA_PIXEL_LUT4_HIGH ( *D, *S );
          ++ S;
          ++ D;
...
#undef SET_ALPHA_PIXEL_LUT4_LOW
#undef SET_ALPHA_PIXEL_LUT4_HIGH
}
...
static const GenefxFunc Bop_a8_set_alphapixel_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_A1_LSB)]   = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_YV16)]     = NULL,
     [DFB_PIXELFORMAT_INDEX(DSPF_LUT4)]     = Bop_a8_set_alphapixel_Aop_lut4,
     [DFB_PIXELFORMAT_INDEX(DSPF_ABGR)]     = Bop_a8_set_alphapixel_Aop_argb,
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_a8_set_alphapixel_Aop_lut8,
};
...
static const GenefxFunc Bop_a1_set_alphapixel_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_a1_set_alphapixel_Aop_lut8,
};
...
static const GenefxFunc Bop_a1_lsb_set_alphapixel_Aop_PFI[DFB_NUM_PIXELFORMATS] = {
     ...
     [DFB_PIXELFORMAT_INDEX(DSPF_ALUT8)]    = Bop_a1_lsb_set_alphapixel_Aop_lut8,
};
...
static void Sop_is_Aop( GenefxState *gfxs ) { gfxs->Sop = gfxs->Aop; gfxs->Ostep = gfxs->Astep; Sop_odd_start = &Aop_odd_start;}
static void Sop_is_Bop( GenefxState *gfxs ) { gfxs->Sop = gfxs->Bop; gfxs->Ostep = gfxs->Bstep; Sop_odd_start = &Bop_odd_start;}
...
bool gAcquire( CardState *state, DFBAccelerationMask accel )
{
     ...
     switch (gfxs->dst_format) {
          ...
          case DSPF_LUT8:
               ...
               break;
          case DSPF_ALUT8:
               gfxs->Cop  = (color.a << 8) + state->color_index;
               gfxs->Alut = destination->palette;
               break;
          case DSPF_ALUT44:
