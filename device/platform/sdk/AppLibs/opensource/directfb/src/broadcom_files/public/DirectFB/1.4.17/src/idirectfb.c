                         ...
                         IDirectFBSurface            **interface )
{
     ...
     DFBDisplayLayerConfig  config;
     DFBSurfaceHintFlags    hints = DSHF_NONE;
...
*/
     ...
     if (desc->flags & DSDESC_PREALLOCATED) {
          ...
          ret = CoreDFB_CreateSurface( data->core, &config, CSTF_PREALLOCATED, resource_id, hints, NULL, &surface );
          if (ret)
               return ret;
     }
     else
     {
          ...
          ret = CoreDFB_CreateSurface( data->core, &config, CSTF_NONE, resource_id, hints, NULL, &surface );
          if (ret)
               return ret;
     }
