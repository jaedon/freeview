...
)
{
    ...
    DataBufferGetDataReturn *return_args = (DataBufferGetDataReturn*) malloc( sizeof(DataBufferGetDataReturn) + length * sizeof(u8) );

    D_DEBUG_AT( DirectFB_DataBuffer, "IDataBuffer_Requestor::%s()\n", __FUNCTION__ );

    if (!return_args) {
        D_DERROR( ret, "%s: Failed to allocate temporary buffer failed!\n", __FUNCTION__ );
    }
...
)
{
    ...
    DataBufferPeekDataReturn *return_args = (DataBufferPeekDataReturn*) malloc( sizeof(DataBufferPeekDataReturn) + length * sizeof(u8) );
...
{
    DFBResult           ret;
    DataBufferPutData       *args = (DataBufferPutData*) malloc( sizeof(DataBufferPutData) + length * sizeof(u8) );
