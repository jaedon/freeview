...
static const char *config_usage_strings[]  = {
     ...
     "  layer-update-skip=<cnt>        Skip updating this layer <cnt> times (0 means update always)\n",
     ...
     "  [no-]fonts-videoonly           Store font glyphs in video memory\n"
     "  [no-]no-focus-hides-cursor     Hide the cursor when no window has focus\n"
     "\n",
...
static int config_read_cmdline( char *cmdbuf, int size, FILE *f )
{
     ...
     while(ret==1 && len<(size-1)) {
          len++;
#if defined(HUMAX_PLATFORM_BASE)	/* humax patch : not used and bug in config_read_cmdline */
          ret = fread(cmdbuf+len, 1, 1, f );
          if (*(cmdbuf+len) == 0 )
	          break;
#else
          ret = fread( ++cmdbuf, 1, 1, f );
...
static void config_allocate( void )
{
    int i, usage_length = 0;
     ...
     dfb_config->fonts_videoonly    = false;
...
DFBResult dfb_config_set( const char *name, const char *value )
{
     ...
     if (strcmp (name, "layer-update-skip" ) == 0) {
          DFBConfigLayer *conf = dfb_config->config_layer;

          if (value) {
               int skip_cnt;

               if (sscanf( value, "%d", &skip_cnt ) < 1) {
                    D_ERROR("DirectFB/Config '%s': Could not parse value!\n", name);
                    return DFB_INVARG;
               }

               conf->update_skip_cnt = skip_cnt;
          }
          else {
               D_ERROR("DirectFB/Config '%s': No value specified!\n", name);
               return DFB_INVARG;
     ...
     if (strcmp (name, "no-focus-hides-cursor" ) == 0) {
          dfb_config->no_focus_hides_cursor = true;
     } else
     if (strcmp (name, "no-no-focus-hides-cursor" ) == 0) {
          dfb_config->no_focus_hides_cursor = false;
     } else
     if (strcmp (name, "fonts-videoonly" ) == 0) {
          dfb_config->fonts_videoonly = true;
     } else
     if (strcmp (name, "no-fonts-videoonly" ) == 0) {
          dfb_config->fonts_videoonly = false;
     } else
