                   ...
                   SaWManProcess **ret_process )
{
     ...
     for (i=0; i<D_ARRAY_SIZE(dfb_config->layers); i++) {
          ...
          layer_id = dfb_layer_id(dfb_layer_at_translated(i));

          ret = add_tier( sawman, world, layer_id, dfb_config->layers[i].stacking );
          if (ret) {
...
init_hw_cursor( SaWMan *sawman )
{
     ...
     ret = dfb_layer_create_context( sawman->cursor.layer, true, &sawman->cursor.context );
     if (ret) {
