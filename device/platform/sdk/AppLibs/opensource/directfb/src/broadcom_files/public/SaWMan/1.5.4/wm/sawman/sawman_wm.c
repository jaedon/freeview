                 ...
                 int              height )
{
     ...
     tier->size.w = tier->config.width = tier->border_config.width = width;
     tier->size.h = tier->config.height = tier->border_config.height = height;
                  ...
                  CoreCursorUpdateFlags  flags )
{
     ...
     if (!tier->cursor_bs) {
          ...
          ret = dfb_surface_create_simple( wmdata->core, stack->cursor.size.w, stack->cursor.size.h,
                                           ...
                                           DFB_COLORSPACE_DEFAULT(context->config.pixelformat), DSCAPS_NONE,
                                           CSTF_SHARED | CSTF_CURSOR, 0, DSHF_CURSOR, NULL, &cursor_bs );
          if (ret) {
     ...
     if (updates_count) {
          switch (primary->config.buffermode) {
               ...
               case DLBM_BACKVIDEO:
                    /* Flip the updated regions. */
                    for (i=0; i<updates_count; i++) {
                         const DFBRegion *update = &updates[i];

                         DFB_REGION_ASSERT( update );

                         dfb_layer_region_flip_update( primary, update, DSFLIP_BLIT );
                    }
                    break;
