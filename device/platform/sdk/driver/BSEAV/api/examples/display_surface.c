 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: display_surface.c $
 * $brcm_Revision: 2 $
 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/api/examples/display_surface.c $
 * 
...
int main(void) {
	...
	cfg.pixel_format = bgraphics_pixel_format_cr8_y18_cb8_y08;
	surface = bsurface_create(graphics, &cfg);
	assert(surface);

	rc = bdecode_display_surface(decode, surface, window);
	assert(!rc);
