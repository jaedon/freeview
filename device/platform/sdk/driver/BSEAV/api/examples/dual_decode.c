 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/api/examples/dual_decode.c $
 * 
...
int main(void) {
	...
	btuner_t tuner0 = btuner_open(bconfig->resources.sds.objects[0]);
	btuner_t tuner1 = btuner_open(bconfig->resources.sds.objects[1]);
	...
	btuner_sds_params_init(&sds, tuner0);
	...
	band = btuner_tune_sds(tuner0, 1120000000, &sds);
	if (!band) goto error;
	...
	band = btuner_tune_sds(tuner1, 1120000000, &sds);
	if (!band) goto error;
