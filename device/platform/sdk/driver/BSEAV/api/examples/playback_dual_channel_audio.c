 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: playback_dual_channel_audio.c $
 * $brcm_Revision: 2 $
 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/api/examples/playback_dual_channel_audio.c $
 * 
...
/* This app performs dual channel audio decode. This is simultaneous
decode of one audio program and passthrough of another audio program.
*/
int main(void) {
	...
	/* this stream has two audio pids: one english, one spanish. */
	file = bplayback_file_open("videos/clifford_dualaudio.mpg", "videos/clifford_dualaudio.nav");
	if (!file) goto error;

	bstream_mpeg_init(&mpeg);
	mpeg.video[0].pid = 0x41;
	mpeg.audio[0].pid = 0x44; /* english */
	mpeg.audio[0].format = baudio_format_ac3;
	mpeg.audio[1].pid = 0x45; /* spanish */
	...
	/* the user is required to enable at least one output as compressed */
	...
	/* this enables dual channel audio */
	bdecode_get(decode, &decode_settings);
	decode_settings.audio.secondary_audio_program = 1;
	bdecode_set(decode, &decode_settings);
