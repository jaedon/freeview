 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: record_new_pid.c $
 * $brcm_Revision: 3 $
 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/api/tests/record_new_pid.c $
 * 
...
int main(void) {
	...
	if (brecord_start(record, recpump, livestream, recfile, &params))
		goto error;
	...
	printf("recording audio pid 0x34\n");

	sleep(10);
	mpeg.audio[1].pid = 0x24;
	mpeg.video[1].pid = 0x21;
	printf("recording audio pid 0x24\n");
	bstream_set_mpeg_parameters(livestream, &mpeg); /* recording new pid */
	sleep(20);

	bstream_close(livestream);
	bsettop_uninit();
