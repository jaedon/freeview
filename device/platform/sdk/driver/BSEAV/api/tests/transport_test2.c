 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: transport_test2.c $
 * $brcm_Revision: 2 $
 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/api/tests/transport_test2.c $
 * 
...
#define MAXMSG 5
	...
	/* run this test with msg_modules=base_xpt and verify the refcnt stays 1. */
	...
	for (i=0;i<MAXMSG;i++) {
		...
		if (!msg[i]) goto error;
		if (bmessage_start(msg[i], &params)) goto error;
	}
