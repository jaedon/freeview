...
//
//****************************************************************************
//  $Id: NonVolSettings.cpp 1.30 2006/11/30 15:48:21Z enderby Release $
//
                                 ...
                                 unsigned int &currentOffset, NonVolSection section)
{
    ...
    // correct.
    uint32 magicNumber;
    uint16 groupLength, bytesNotConsumed;
    ...
    {
        gErrorMsg(fMessageLogSettings, "ReadFrom")
            << "Derived class failed to read all of its settings from the buffer!" << endl;
...
ostream & BcmNonVolSettings::Print(ostream &outputStream) const
{
    ...
    outputStream << magicNumberString << "'\n"
                 << setw(30) << "Version (Permanent) = " << (int) fVersionMajor[kPermanentSection] 
                 << "." << (int) fVersionMinor[kPermanentSection] << '\n'
                 << setw(30) << "Version (Dynamic) = " << (int) fVersionMajor[kDynamicSection]
                 << "." << (int) fVersionMinor[kDynamicSection] << '\n'
                 << setw(30) << "Is Default (Permanent) = " << fIsDefault[kPermanentSection] << '\n'
