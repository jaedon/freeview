...
private:
...
#ifdef NPTL_SUPPORT
    static void LTOAlarmHandler (int signal_number, siginfo_t *extra, void *cruft);
#else
    ...
    long fStartTime;   // Start time of itimer, user large timer.
    long fStartTimeCurrentTimer;  // Start time of itimer using rootnode.
    
    static const ulong fTicksPerMs;
#if (NPTL_SUPPORT)
    timer_t myTimer; // Real time signal handle
    static long nTimeout;
#endif 
};
