...
//
//****************************************************************************
//  $Id: MacAddressCommandParameter.cpp 1.5 2001/03/12 20:16:28Z dpullen Release $
//
//  Filename:       MacAddressCommandParameter.cpp
//  Author:         David Pullen
...
//
// If pParameterName is NULL, then "MacAddress" will be used.
//
// Parameters:
...
// Returns:  N/A
//
BcmMacAddressCommandParameter::BcmMacAddressCommandParameter(unsigned int clientId, 
                                                             bool isOptional,
                                                             const char *pParameterName) :
    BcmCommandParameter(clientId, isOptional, pParameterName, "MacAddress"),
    fValue(kAllZerosMac)
{
    // Nothing else to do.
}
...
// Returns:  N/A
//
BcmMacAddressCommandParameter::~BcmMacAddressCommandParameter()
{
    // Nothing else to do.
}
...
//
ostream &BcmMacAddressCommandParameter::PrintValue(ostream &outstream) const
{
...
// Returns:  Nothing.
//
void BcmMacAddressCommandParameter::PrintValue(char string[64]) const
{
    fValue.Get(string);
}
...
const BcmMacAddress &BcmMacAddressCommandParameter::AsMacAddress(void) const
{
    return fValue;
}
...
// MAC Address.
bool BcmMacAddressCommandParameter::SetAsMacAddress(const BcmMacAddress &value)
{
    fValue = value;
...
//
bool BcmMacAddressCommandParameter::ParseImpl(const char *pParameterString, 
                                               unsigned int &offset)
{
    unsigned int newOffset = 0;

    // A MAC address knows how to parse itself from a string, so let it do its
    // job.
    //
    // NOTE - MAC address will not destroy its data if this fails; it does its
    ...
    if (!IsOptional())
    {
        PrintError(pParameterString + offset, "Must be a valid MAC Address!");
    }
