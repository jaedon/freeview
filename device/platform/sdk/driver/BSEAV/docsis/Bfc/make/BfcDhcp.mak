...
#
##############################################################################
#  $Id: BfcDhcp.mak 1.6 2004/08/16 19:05:42Z dpullen Release $
#
#  Filename:       BfcDhcp.mak
#  Author:         David Pullen
...
#  Description:
#
# 	This file contains the build "stuff" for the DHCP Client and Server
...
BFC_DHCP_DIR = Bfc/IpHelpers/Dhcp
...
# DhcpClientId is common for the client and server.  It is a wrapper object
# for the DHCP Client Id TLV that appears in DHCP messages.
BFC_DHCP_OBJECTS = DhcpClientId.o DhcpLeaseSettings.o DhcpOptionsRfc2132.o \
                   DhcpPacket.o

# Modules specific to the client.
BFC_DHCP_CLIENT_OBJECTS = DhcpClientIf.o DhcpClientLease.o DhcpClientSnoop.o \
                          DhcpClientThread.o

# Modules specific to the server.
BFC_DHCP_SERVER_OBJECTS = DhcpServerIf.o DhcpServerLease.o DhcpServerSnoop.o \
                          DhcpServerThread.o
...
BFC_DHCP_DEFINES = BCM_V2_DHCP_SUPPORT=1 \
                   BFC_INCLUDE_DHCP_SUPPORT=$(BFC_INCLUDE_DHCP_SUPPORT) \
		   BFC_INCLUDE_DHCP_SERVER_SUPPORT=$(BFC_INCLUDE_DHCP_SERVER_SUPPORT)
...
BFC_DHCP_INCLUDE_DIRS = $(BFC_DHCP_DIR)
...
BFC_DHCP_SOURCE_DIRS = $(BFC_DHCP_DIR)
...
# Add Dhcp components, if included by BfcSystemConfig.mak.
ifeq ($(BFC_INCLUDE_DHCP_SUPPORT), 1)

    BFC_OBJECTS += $(BFC_DHCP_OBJECTS) $(BFC_DHCP_CLIENT_OBJECTS)
 
    ifeq ($(BFC_INCLUDE_DHCP_SERVER_SUPPORT), 1)
        BFC_OBJECTS += $(BFC_DHCP_SERVER_OBJECTS)
    endif
    
    # Add Dhcp console components, if support for Console and DHCP Console
    # are both included.
    ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)
    
        ifeq ($(BFC_INCLUDE_DHCP_CONSOLE_SUPPORT), 1)

            BFC_OBJECTS += $(BFC_DHCP_CONSOLE_OBJECTS)

        endif
	
    endif

    BFC_COMPILER_DEFINES += $(BFC_DHCP_DEFINES)

    BFC_INCLUDE_DIRS += $(BFC_DHCP_INCLUDE_DIRS)
    
    BFC_SOURCE_DIRS += $(BFC_DHCP_SOURCE_DIRS)
    
    BFC_LIBRARIES += $(BFC_DHCP_LIBRARIES)
