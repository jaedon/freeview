...
#  $Id$
#
#  Filename:       BfcEthernetNetworking.mak
#  Author:         David Pullen
...
#  Description:
#
# 	This file contains the build "stuff" for the Ethernet interface part
...
BFC_ETHERNET_DIR = $(BFC_NETWORKING_DIR)/Ethernet
...
BFC_ETHERNET_NETWORKING_CONSOLE_OBJECTS = EnetHalCommandTable.o

BFC_ETHERNET_NETWORKING_SNMP_OBJECTS = etherLikeMib.o etherLikeBridge.o

BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS = EnetNiDriverEntry.o EnetNiInfo.o \
                                         EnetNiInit.o EnetNiMode.o \
	                     		 EnetNiRcv.o EnetNiSend.o \
			                 EnetPhy.o 

ifeq ($(BFC_INCLUDE_GIGA_BIT_ENET_SUPPORT), 1)
    BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS += \
        gigeEnetNiDriverEntry.o     gigeEnetNiInfo.o\
        gigeEnetNiInit.o            gigeEnetNiRcv.o \
        gigeEnetNiSend.o            gigeEnetPhy.o \
        gigeEnetNiMode.o 
endif

# If the build processor is 5365 or 5325...
ifeq ($(filter 5365 5325,$(buildProcessor)), $(buildProcessor))
    BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS += SwitchCorePortDriver.o EnetNiCam.o EnetNiLink.o
else
    ifeq ($(BCM_MANAGED_SWITCH_INCLUDED), 1)
        BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS += SerialManagementPortDriver.o 
    endif
endif

# The driver files are only used for some target Operating Systems (although it
# really follows the target, not the OS).
BFC_ETHERNET_NETWORKING_VxWorks_DRIVER_OBJECTS = $(BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS)
BFC_ETHERNET_NETWORKING_Win32_DRIVER_OBJECTS = 
BFC_ETHERNET_NETWORKING_Ecos_DRIVER_OBJECTS = $(BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS)
BFC_ETHERNET_NETWORKING_Qnx6_DRIVER_OBJECTS = $(BFC_ETHERNET_NETWORKING_DRIVER_OBJECTS)
...
BFC_ETHERNET_NETWORKING_DEFINES = ENET_HAL_INCLUDED=1 \
                                  BFC_INCLUDE_ETHERNET_SUPPORT=$(BFC_INCLUDE_ETHERNET_SUPPORT)

BFC_ETHERNET_NETWORKING_DEFINES += BFC_INCLUDE_GIGA_BIT_ENET_SUPPORT=$(BFC_INCLUDE_GIGA_BIT_ENET_SUPPORT)
...
BFC_ETHERNET_NETWORKING_INCLUDE_DIRS = $(BFC_ETHERNET_DIR)
...
BFC_ETHERNET_NETWORKING_SOURCE_DIRS = $(BFC_ETHERNET_DIR)
...
BFC_ETHERNET_NETWORKING_LIBRARIES = 
...
ifeq ($(BFC_INCLUDE_ETHERNET_SUPPORT), 1)

    # Core networking modules
    BFC_NETWORKING_OBJECTS += $(BFC_ETHERNET_NETWORKING_OBJECTS)

    # Console modules
    BFC_NETWORKING_CONSOLE_OBJECTS += $(BFC_ETHERNET_NETWORKING_CONSOLE_OBJECTS)

    # SNMP modules
    BFC_NETWORKING_SNMP_OBJECTS += $(BFC_ETHERNET_NETWORKING_SNMP_OBJECTS)

    # Driver modules
    BFC_NETWORKING_DRIVER_$(BFC_TARGET_OS)_SUPPORT_OBJECTS += $(BFC_ETHERNET_NETWORKING_$(BFC_TARGET_OS)_DRIVER_OBJECTS)


    BFC_NETWORKING_DEFINES += $(BFC_ETHERNET_NETWORKING_DEFINES)

    BFC_NETWORKING_INCLUDE_DIRS += $(BFC_ETHERNET_NETWORKING_INCLUDE_DIRS)

    BFC_NETWORKING_SOURCE_DIRS += $(BFC_ETHERNET_NETWORKING_SOURCE_DIRS)

    BFC_NETWORKING_LIBRARIES += $(BFC_ETHERNET_NETWORKING_LIBRARIES)
