...
#
##############################################################################
#  $Id: BfcHomePlugNetworking.mak 1.9 2005/03/11 17:26:08Z czacker-atl Release $
#
#  Filename:       BfcHomePlugNetworking.mak
#  Author:         David Pullen	/ Mark Newcomer
#  Creation Date:  September 10, 2004
#
##############################################################################
#  Description:
#
# 	This file contains the build "stuff" for the Home Plug interface part
...
BFC_HOMEPLUG_DIR = $(BFC_NETWORKING_DIR)/HomePlug
...
BFC_HOMEPLUG_NETWORKING_OBJECTS = HomePlugSnoop.o HomePlugThread.o \
                                  HomePlugNonVolSettings.o \
                				  Md5HomePlug.o \
                                  HomePlugNiHalIf.o

BFC_HOMEPLUG_NETWORKING_CONSOLE_OBJECTS = HomePlugNonVolSettingsCommandTable.o \
					  HomePlugCommandTable.o

#BFC_HOMEPLUG_NETWORKING_SNMP_OBJECTS =

#BFC_HOMEPLUG_NETWORKING_BRCM_RUNTIME_SNMP_OBJECTS = 

BFC_HOMEPLUG_NETWORKING_BRCM_FACTORY_SNMP_OBJECTS = brcmHomeplugFactoryMib.o \
                                                    brcmHomeplugFactoryBridge.o


BFC_HOMEPLUG_NETWORKING_DRIVER_OBJECTS = HomePlugPortDriver.o
                                          
# The driver files are only used for some target Operating Systems (although it
# really follows the target, not the OS).
#BFC_HOMEPLUG_NETWORKING_VxWorks_DRIVER_OBJECTS = $(BFC_HOMEPLUG_NETWORKING_DRIVER_OBJECTS)
#BFC_HOMEPLUG_NETWORKING_Win32_DRIVER_OBJECTS = 
BFC_HOMEPLUG_NETWORKING_Ecos_DRIVER_OBJECTS = $(BFC_HOMEPLUG_NETWORKING_DRIVER_OBJECTS)
...
BFC_HOMEPLUG_NETWORKING_DEFINES =  BFC_INCLUDE_HOMEPLUG_SUPPORT
...
BFC_HOMEPLUG_NETWORKING_INCLUDE_DIRS = \
        $(BFC_HOMEPLUG_DIR) 
...
BFC_HOMEPLUG_NETWORKING_SOURCE_DIRS = $(BFC_HOMEPLUG_DIR)
...
ifeq ($(BFC_INCLUDE_HOMEPLUG_SUPPORT), 1)
    ...
    BFC_NETWORKING_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_OBJECTS)

    # Console modules
    BFC_NETWORKING_CONSOLE_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_CONSOLE_OBJECTS)

    # SNMP modules
    #BFC_NETWORKING_SNMP_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_SNMP_OBJECTS)

    # Runtime and Factory MIB support modules.
    #BFC_NETWORKING_BRCM_RUNTIME_SNMP_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_BRCM_RUNTIME_SNMP_OBJECTS)
    BFC_NETWORKING_BRCM_FACTORY_SNMP_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_BRCM_FACTORY_SNMP_OBJECTS)

    # Driver modules
    BFC_NETWORKING_DRIVER_$(BFC_TARGET_OS)_SUPPORT_OBJECTS += $(BFC_HOMEPLUG_NETWORKING_$(BFC_TARGET_OS)_DRIVER_OBJECTS)

    BFC_NETWORKING_DEFINES += $(BFC_HOMEPLUG_NETWORKING_DEFINES)

    BFC_NETWORKING_INCLUDE_DIRS += $(BFC_HOMEPLUG_NETWORKING_INCLUDE_DIRS)

    BFC_NETWORKING_SOURCE_DIRS += $(BFC_HOMEPLUG_NETWORKING_SOURCE_DIRS)

    #BFC_NETWORKING_LIBRARIES += $(BFC_HOMEPLUG_NETWORKING_LIBRARIES)
