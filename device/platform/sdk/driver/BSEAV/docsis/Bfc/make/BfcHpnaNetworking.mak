...
#
##############################################################################
#  $Id: BfcHpnaNetworking.mak 1.2 2004/08/16 19:06:36Z dpullen Release $
#
#  Filename:       BfcHpnaNetworking.mak
#  Author:         David Pullen
...
#  Description:
#
# 	This file contains the build "stuff" for the HPNA interface part
...
BFC_HPNA_DIR = $(BFC_NETWORKING_DIR)/Hpna
...
BFC_HPNA_NETWORKING_OBJECTS = HpnaNiHalIf.o

BFC_HPNA_NETWORKING_CONSOLE_OBJECTS = HpnaHalCommandTable.o

BFC_HPNA_NETWORKING_SNMP_OBJECTS = 

BFC_HPNA_NETWORKING_DRIVER_OBJECTS = 

# The driver files are only used for some target Operating Systems (although it
# really follows the target, not the OS).
BFC_HPNA_NETWORKING_VxWorks_DRIVER_OBJECTS = $(BFC_HPNA_NETWORKING_DRIVER_OBJECTS)
BFC_HPNA_NETWORKING_Win32_DRIVER_OBJECTS = 
BFC_HPNA_NETWORKING_Ecos_DRIVER_OBJECTS = $(BFC_HPNA_NETWORKING_DRIVER_OBJECTS)
...
BFC_HPNA_NETWORKING_INCLUDE_DIRS = $(BFC_HPNA_DIR)
...
BFC_HPNA_NETWORKING_SOURCE_DIRS = $(BFC_HPNA_DIR)
...
ifeq ($(BFC_INCLUDE_HPNA_SUPPORT), 1)

    # Core networking modules
    BFC_NETWORKING_OBJECTS += $(BFC_HPNA_NETWORKING_OBJECTS)

    # Console modules
    BFC_NETWORKING_CONSOLE_OBJECTS += $(BFC_HPNA_NETWORKING_CONSOLE_OBJECTS)

    # SNMP modules
    BFC_NETWORKING_SNMP_OBJECTS += $(BFC_HPNA_NETWORKING_SNMP_OBJECTS)

    # Driver modules
    BFC_NETWORKING_DRIVER_$(BFC_TARGET_OS)_SUPPORT_OBJECTS += $(BFC_HPNA_NETWORKING_$(BFC_TARGET_OS)_DRIVER_OBJECTS)


    BFC_NETWORKING_DEFINES += $(BFC_HPNA_NETWORKING_DEFINES)

    BFC_NETWORKING_INCLUDE_DIRS += $(BFC_HPNA_NETWORKING_INCLUDE_DIRS)

    BFC_NETWORKING_SOURCE_DIRS += $(BFC_HPNA_NETWORKING_SOURCE_DIRS)

    BFC_NETWORKING_LIBRARIES += $(BFC_HPNA_NETWORKING_LIBRARIES)
