...
#
##############################################################################
#  $Id: BfcHttpServer.mak 1.31 2007/04/21 21:30:11Z dnguyen Release $
#
#  Filename:       BfcHttpServer.mak
#  Author:         David Pullen
...
# If vendor http server is specified, do NOT include our server
# It is include by default, so clear it.
ifeq ($(BCM_VENDOR_HTTP_SERVER),1)
BFC_INCLUDE_HTTP_SERVER_SUPPORT = 0
endif

INCLUDE_HTTP_MASTER_SERVER = 0

# We need to include the HTTP master if
# 1. Our HTTP server is used OR
ifeq ($(BFC_INCLUDE_HTTP_SERVER_SUPPORT),1)
INCLUDE_HTTP_MASTER_SERVER = 1
endif

# 2. UPnP support is included OR
ifeq ($(BCM_UPNP_SUPPORT),1)
INCLUDE_HTTP_MASTER_SERVER = 1
endif

# 3. We need a proxy to talk to a vendor HTTP server
ifeq ($(BCM_VENDOR_HTTP_SERVER),1)
INCLUDE_HTTP_MASTER_SERVER = 1
endif
...
BFC_HTTP_SERVER_DIR = Bfc/IpHelpers/Https
...
# These files are the actual HTTP Server.
BFC_HTTP_SERVER_OBJECTS = HttpServerThread.o \
                HttpServerFactory.o \
                HttpIpStackEventACT.o \
                HttpRequestAgent.o \
                HttpHelper.o \
		HttpFormManager.o \
		HttpConnection.o              
...
ifeq ($(BFC_INCLUDE_HTTP_SERVER_SUPPORT),1)
        BFC_HTTP_SERVER_OBJECTS += HttpConfigurationServerAgentFactory.o
endif
...
BFC_HTTP_SERVER_DEFINES = BFC_INCLUDE_HTTP_SERVER_SUPPORT=$(BFC_INCLUDE_HTTP_SERVER_SUPPORT)

ifeq ($(BCM_HTTP_FORM_AUTHORIZATION),1)
BFC_HTTP_SERVER_DEFINES += BCM_HTTP_FORM_AUTHORIZATION=1
endif
...
BFC_HTTP_SERVER_INCLUDE_DIRS = $(BFC_HTTP_SERVER_DIR)
...
BFC_HTTP_SERVER_SOURCE_DIRS = $(BFC_HTTP_SERVER_DIR)
...
ifeq ($(INCLUDE_HTTP_MASTER_SERVER), 1)

    BFC_OBJECTS += $(BFC_HTTP_SERVER_OBJECTS)
    
    BFC_COMPILER_DEFINES += $(BFC_HTTP_SERVER_DEFINES)

    BFC_INCLUDE_DIRS += $(BFC_HTTP_SERVER_INCLUDE_DIRS)
    
    BFC_SOURCE_DIRS += $(BFC_HTTP_SERVER_SOURCE_DIRS)
    
    BFC_LIBRARIES += $(BFC_HTTP_SERVER_LIBRARIES)
...
ifeq ($(BFC_INCLUDE_HTTP_SERVER_SUPPORT),1)
    ifeq ($(BCM_HTTP_CUSTOM_WEBUI),1)
    include $(BFC_MAKE_DIR)/BfcHttpWebUi$(BrcmMsoHttpMakefileExt).mak
    else
    include $(BFC_MAKE_DIR)/BfcHttpWebUiBrcm.mak
    endif
endif

############################################################################
## Vendor Web UI Support
############################################################################

ifeq ($(BCM_VENDOR_HTTP_SERVER),1)
    include $(BFC_MAKE_DIR)/BfcHttpWebUiVendor.mak
endif
