...
# For VxWorks, we have hacked the IP stack to fix various bugs.  These files
# are in this directory, and can just be linked directly.
BFC_IPSTACK_VXWORKS_OBJS_DIR = $(BFC_ROOT_DIR)/Bfc/vxWorks/patches/Tornado20/added_objs
...
BFC_IPSTACK_NETWORKING_OBJECTS = \
        BaseIpHalIf.o \
        IpStackManager.o \
        IpRoutingFilterSnoop.o

ifneq ($(BCM_ESTB_STANDALONE_APP_SUPPORT), 1)
BFC_IPSTACK_NETWORKING_CONSOLE_OBJECTS = IpHalCommandTable.o
endif

BFC_IPSTACK_NETWORKING_SNMP_OBJECTS = 

BFC_IPSTACK_NETWORKING_DRIVER_OBJECTS = \
        TcpipNiDriverEntry.o \
        TcpipNiInfo.o \
		TcpipNiInit.o \
        TcpipNiMode.o \
		TcpipNiRcv.o \
...
BFC_IPSTACK_NETWORKING_VxWorks_DRIVER_OBJECTS = \
        VxIpHalIf.o \
        bcmIPHalEnd.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/hostLib.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/in_pcb.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/ip_icmp.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/ip_output.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/ipProto.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/m2IcmpLib.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/m2IpLib.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/netShow.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/raw_ip.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/route.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/tcp_input.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/udp_usrreq.o \
        $(BFC_IPSTACK_VXWORKS_OBJS_DIR)/uipc_sock.o \
        $(BFC_IPSTACK_NETWORKING_DRIVER_OBJECTS)

BFC_IPSTACK_NETWORKING_Win32_DRIVER_OBJECTS = 

BFC_IPSTACK_NETWORKING_Ecos_DRIVER_OBJECTS = \
        ecosIpHalIf.o \
        bcmIPHal.o \
        $(BFC_IPSTACK_NETWORKING_DRIVER_OBJECTS)

BFC_IPSTACK_NETWORKING_Qnx6_DRIVER_OBJECTS = \
        qnxIpHalIf.o \
        bcmIPHal.o \
        $(BFC_IPSTACK_NETWORKING_DRIVER_OBJECTS)
...
BFC_IPSTACK_NETWORKING_VxWorks_DEFINES = MULTI_INF_SAME_SUBNET=1 BCM_DELAY_ENDLOAD=1 
BFC_IPSTACK_NETWORKING_Win32_DEFINES = 
BFC_IPSTACK_NETWORKING_Ecos_DEFINES = MULTI_INF_SAME_SUBNET=1
BFC_IPSTACK_NETWORKING_Qnx6_DEFINES = 
