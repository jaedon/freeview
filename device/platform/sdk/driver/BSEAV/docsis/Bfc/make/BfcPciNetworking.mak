...
#
##############################################################################
#  $Id: BfcPciNetworking.mak 1.1 2005/11/29 17:01:57Z kennylee Release $
#
#  Filename:       BfcPciNetworking.mak
#  Author:         David Pullen
...
#  Description:
#
# 	This file contains the build "stuff" for the PCI interface part
...
BFC_PCI_DIR = $(BFC_NETWORKING_DIR)/Pci
...
BFC_PCI_NETWORKING_OBJECTS = PciNiHalIf.o

BFC_PCI_NETWORKING_CONSOLE_OBJECTS = PciHalCommandTable.o

BFC_PCI_NETWORKING_SNMP_OBJECTS = 

BFC_PCI_NETWORKING_DRIVER_OBJECTS = PciNiDriverEntry.o PciNiInit.o \
                                    PciNiInfo.o PciNiMode.o \
				    PciNiSend.o PciNiRcv.o

# The driver files are only used for some target Operating Systems (although it
# really follows the target, not the OS).
BFC_PCI_NETWORKING_VxWorks_DRIVER_OBJECTS = $(BFC_PCI_NETWORKING_DRIVER_OBJECTS)
BFC_PCI_NETWORKING_Win32_DRIVER_OBJECTS = 
BFC_PCI_NETWORKING_Ecos_DRIVER_OBJECTS = $(BFC_PCI_NETWORKING_DRIVER_OBJECTS)
...
BFC_PCI_NETWORKING_DEFINES = PCI_HAL_INCLUDED=1
...
BFC_PCI_NETWORKING_INCLUDE_DIRS = $(BFC_PCI_DIR)
...
BFC_PCI_NETWORKING_SOURCE_DIRS = $(BFC_PCI_DIR)
...
ifeq ($(BFC_INCLUDE_PCI_CLIENT_SUPPORT), 1)
    ...
    BFC_NETWORKING_OBJECTS += $(BFC_PCI_NETWORKING_OBJECTS)

    # Console modules
    BFC_NETWORKING_CONSOLE_OBJECTS += $(BFC_PCI_NETWORKING_CONSOLE_OBJECTS)

    # SNMP modules
    BFC_NETWORKING_SNMP_OBJECTS += $(BFC_PCI_NETWORKING_SNMP_OBJECTS)

    # Driver modules
    BFC_NETWORKING_DRIVER_$(BFC_TARGET_OS)_SUPPORT_OBJECTS += $(BFC_PCI_NETWORKING_$(BFC_TARGET_OS)_DRIVER_OBJECTS)


    BFC_NETWORKING_DEFINES += $(BFC_PCI_NETWORKING_DEFINES)

    BFC_NETWORKING_INCLUDE_DIRS += $(BFC_PCI_NETWORKING_INCLUDE_DIRS)

    BFC_NETWORKING_SOURCE_DIRS += $(BFC_PCI_NETWORKING_SOURCE_DIRS)

    BFC_NETWORKING_LIBRARIES += $(BFC_PCI_NETWORKING_LIBRARIES)
