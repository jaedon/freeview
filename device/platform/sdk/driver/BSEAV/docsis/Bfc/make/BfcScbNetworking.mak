...
#
##############################################################################
#  $Id: BfcScbNetworking.mak 1.1 2006/09/20 20:06:32Z wfeng Release $
#
#  Filename:       BfcScbNetworking.mak
#  Author:         David Pullen
...
BFC_SCB_DIR = $(BFC_NETWORKING_DIR)/Scb
...
BFC_SCB_NETWORKING_OBJECTS = ScbNiHalIf.o

BFC_SCB_NETWORKING_CONSOLE_OBJECTS = ScbHalCommandTable.o

BFC_SCB_NETWORKING_SNMP_OBJECTS = 

BFC_SCB_NETWORKING_DRIVER_OBJECTS = ScbNiDriverEntry.o ScbNiInit.o \
                                    ScbNiInfo.o ScbNiMode.o \
				    ScbNiSend.o ScbNiRcv.o

# The driver files are only used for some target Operating Systems (although it
# really follows the target, not the OS).
BFC_SCB_NETWORKING_VxWorks_DRIVER_OBJECTS = $(BFC_SCB_NETWORKING_DRIVER_OBJECTS)
BFC_SCB_NETWORKING_Win32_DRIVER_OBJECTS = 
BFC_SCB_NETWORKING_Ecos_DRIVER_OBJECTS = $(BFC_SCB_NETWORKING_DRIVER_OBJECTS)
...
BFC_SCB_NETWORKING_DEFINES = SCB_HAL_INCLUDED=1
...
BFC_SCB_NETWORKING_INCLUDE_DIRS = $(BFC_SCB_DIR)
...
BFC_SCB_NETWORKING_SOURCE_DIRS = $(BFC_SCB_DIR)
...
ifeq ($(BFC_INCLUDE_UBUS_CLIENT_SUPPORT), 1)
    ...
    BFC_NETWORKING_OBJECTS += $(BFC_SCB_NETWORKING_OBJECTS)

    # Console modules
    BFC_NETWORKING_CONSOLE_OBJECTS += $(BFC_SCB_NETWORKING_CONSOLE_OBJECTS)

    # SNMP modules
    BFC_NETWORKING_SNMP_OBJECTS += $(BFC_SCB_NETWORKING_SNMP_OBJECTS)

    # Driver modules
    BFC_NETWORKING_DRIVER_$(BFC_TARGET_OS)_SUPPORT_OBJECTS += $(BFC_SCB_NETWORKING_$(BFC_TARGET_OS)_DRIVER_OBJECTS)


    BFC_NETWORKING_DEFINES += $(BFC_SCB_NETWORKING_DEFINES)

    BFC_NETWORKING_INCLUDE_DIRS += $(BFC_SCB_NETWORKING_INCLUDE_DIRS)

    BFC_NETWORKING_SOURCE_DIRS += $(BFC_SCB_NETWORKING_SOURCE_DIRS)

    BFC_NETWORKING_LIBRARIES += $(BFC_SCB_NETWORKING_LIBRARIES)
