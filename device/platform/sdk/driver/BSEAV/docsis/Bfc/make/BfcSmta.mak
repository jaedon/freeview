...
BFC_SMTA_OBJECTS += SmtaDownloadAPI.o \
                    MtaStatusEventCodes.o \
                    MtaEventPublisher.o \
                    MtaEventPublisherAPI.o \
                    SmtaDloadHelperThread.o \
                    SmtaSipNonVolSettings.o \
                    SmtaSipNonVolSettingsCommandTable.o \
                    EmtaProvNonVolSettings.o \
                    EmtaProvNonVolSettingsCommandTable.o \
                    MtaApplication.o \
                    MtaWanLinkStatusIndicatorAPI.o \
                    HttpFormRegistrarVoice.o \
                    HttpFormHandlerVoiceSetup.o  \
                    HttpFormHandlerVoiceAdvSetup.o  \
                    HttpFormHandlerVoiceTesting.o \
                    HttpFormHandlerVoiceRemoteSetup.o  \
                    HttpFormHandlerRgSwDownload.o \
                    MtaDhcpAPI.o \
                    AnnouncementDownloadThread.o


ifeq ($(BFC_INCLUDE_MTA_GATEWAY_SUPPORT),1)
    BFC_SMTA_OBJECTS += SmtaSpsCableHomeSystem.o \
                        SmtaStandalonePsThreadStatusClientACT.o
else
    BFC_SMTA_OBJECTS += SmtaSystem.o \
                        HttpFormHandlerSmtaSetup.o \
                        HttpFormHandlerRgTime.o \
                        HttpFormHandlerSwInfoSps.o \
                        HttpFormRegistrarSmtaOnly.o \
                        SmtaSntpAppIf.o  \
                        SmtaNonVolSettings.o \
                        SmtaNonVolSettingsCommandTable.o 

endif


ifeq ($(BCM_MANAGED_SWITCH_INCLUDED),1)
    BFC_SMTA_OBJECTS += ManageSwitchSnoop.o
endif

ifeq ($(BFC_INCLUDE_TLS_SUPPORT),1)
    BFC_SMTA_OBJECTS += MtaSslAPI.o
endif
...
BFC_SMTA_DEFINES += BCM_SMTA_SUPPORT=1

ifeq ($(BCM_MANAGED_SWITCH_INCLUDED),1)
  BFC_SMTA_DEFINES += MANAGED_SWITCH_INCLUDED=1
  BFC_SMTA_DEFINES += BCM_1112_SWITCH_INCLUDED=1
...
BFC_SMTA_INCLUDE_DIRS = $(BFC_SMTA_DIR)  $(BFC_SMTA_DIR)/Http \
                        ResidentialGateway/Http \
                        $(BFC_SMTA_DIR)/Sntp \
                        CableHome $(BFC_EDOCSIS_DIR) \
                        $(BFC_MTA_DIR)/Http Bfc/IpHelpers/Https \
                        Bfc/IpHelpers/Tls Mta/MtaApi Smta 

ifeq ($(BFC_INCLUDE_MTA_GATEWAY_SUPPORT),1)
	BFC_SMTA_INCLUDE_DIRS += SpsCableHomeSystem 
else
	BFC_SMTA_INCLUDE_DIRS += SmtaSystem 
endif
...
BFC_SMTA_SOURCE_DIRS = $(BFC_SMTA_DIR) $(BFC_SMTA_DIR)/Http \
                       ResidentialGateway/Http \
                       $(BFC_SMTA_DIR)/Sntp \
                       $(BFC_EDOCSIS_DIR) $(BFC_MTA_DIR)/Http Mta/MtaApi

# for now, a SMTA without gateway does not have web pages
ifeq ($(BFC_INCLUDE_MTA_GATEWAY_SUPPORT),1)
	BFC_SMTA_SOURCE_DIRS += SpsCableHomeSystem
else
	BFC_SMTA_SOURCE_DIRS += SmtaSystem
endif
...
ifeq ($(CXC_SIG_TLS),1)
    $(HH_LIB_COMMON_PATH)/libsipstk_tls.a    \
else
    $(HH_LIB_COMMON_PATH)/libsipstk.a        \
endif
...
BFC_OBJECTS += $(BFC_SMTA_OBJECTS) $(BFC_SMTA_SNMP_OBJECTS)

BFC_COMPILER_DEFINES += $(BFC_SMTA_DEFINES)

BFC_INCLUDE_DIRS += $(BFC_SMTA_INCLUDE_DIRS)

BFC_SOURCE_DIRS += $(BFC_SMTA_SOURCE_DIRS)

BFC_LIBRARIES += $(BFC_SMTA_LIBRARIES)

BFC_LIBRARIES_ABS_PATH += $(SMTA_LIBRARIES)
