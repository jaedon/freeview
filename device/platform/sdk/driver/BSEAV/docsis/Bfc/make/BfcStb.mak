...
#  Description:
#
# 	This file contains the build "stuff" for the STB subsystem, which
#	are common to all STB platforms. 
#
##############
#
...
# Core STB objects
BFC_STB_OBJECTS = StbApplication.o

# Run-time support for bcm3418 tuner and DPM code
BFC_STB_OBJECTS += bcm3418.o btnr_3418_smarttune.o btnr_3418_regio.o

# SNMP objects	
BFC_STB_SNMP_OBJECTS = 

                        
BFC_STB_SNMP_CONSOLE_OBJECTS = 

# Optional console STB objects
BFC_STB_CONSOLE_OBJECTS = 
...
BFC_STB_DEFINES += BFC_INCLUDE_ESTB_SUPPORT=1 BCM_SETTOP_SUPPORT=1 OS_ECOS=1

ifeq ($(BFC_INCLUDE_STB_CONSOLE_SUPPORT), 1)
    BFC_STB_DEFINES += BCM_STB_CONSOLE_SUPPORT=1
endif

ifeq ($(BCM_DSG_DUAL_PROCESSOR_INTERFACE), 1)
    BFC_STB_DEFINES += BCM_DSG_DUAL_PROCESSOR_INTERFACE=1
endif

ifeq ($(USE_PRIVATE_NETWORK), 1)
    BFC_STB_DEFINES += USE_PRIVATE_NETWORK=1
endif

ifeq ($(BCM_STB_DHCP_SERVER_NONVOL_CONTROL), 1)
    BFC_STB_DEFINES += BCM_STB_DHCP_SERVER_NONVOL_CONTROL=1
endif

# Run-time support for vendor mtm tuner
# BFC_STB_DEFINES += USE_VENDOR_TUNER
...
BFC_STB_INCLUDE_DIRS = $(BFC_STB_DIR) $(BFC_STB_DIR)/StbDocsisApi/Dsg \
		       $(BFC_STB_DIR)/Snmp
...
BFC_STB_SOURCE_DIRS = $(BFC_STB_DIR) $(BFC_STB_DIR)/StbDocsisApi/Dsg \
		      $(BFC_STB_DIR)/Snmp

BFC_STB_SOURCE_DIRS += CmDocsisSystem
...
# Add STB components, if included by BfcSystemConfig.mak.
ifeq ($(BFC_INCLUDE_STB_SUPPORT), 1)

	# Add STB components, if included by BfcSystemConfig.mak.
	BFC_OBJECTS += $(BFC_STB_OBJECTS) $(BFC_STB_CONSOLE_OBJECTS)

	# Add Ping console components, if support for Console and STB Console
	# are both included.
	ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)

		ifeq ($(BFC_INCLUDE_STB_CONSOLE_SUPPORT), 1)

			BFC_OBJECTS += $(BFC_STB_CONSOLE_OBJECTS)
	...
	ifeq ($(BFC_INCLUDE_SNMP_SUPPORT), 1)
		...
		BFC_OBJECTS += $(BFC_STB_SNMP_OBJECTS)

		# If we have SNMP support AND console support, then include
		# SNMP console objects.
		ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)
		    ifeq ($(BFC_CONFIG_MINIMAL_CONSOLE),0)
	              BFC_OBJECTS += $(BFC_STB_SNMP_CONSOLE_OBJECTS)
                endif
		endif

	endif

	BFC_COMPILER_DEFINES += $(BFC_STB_DEFINES)

	BFC_INCLUDE_DIRS += $(BFC_STB_INCLUDE_DIRS)

	BFC_SOURCE_DIRS += $(BFC_STB_SOURCE_DIRS)

endif


############################################################################
## STB Customer Extension Support
############################################################################
ifeq ($(BFC_INCLUDE_STB_VENDOR_SUPPORT), 1)
    include $(BFC_MAKE_DIR)/$(StbVendorExtensionMakefile)
endif
