...
#  Description:
#
#	This file contains the build "stuff" for the Telnet Console subsystem.
#	The Telnet Console is an optional feature that can be added to the
...
BFC_TELNET_CONSOLE_DIR = Bfc/IpHelpers/Telnet
...
BFC_TELNET_CONSOLE_OBJECTS = TelnetThread.o TelnetThreadIpStackEventACT.o

BFC_TELNET_CONSOLE_CMD_TABLE_OBJECTS = RemoteAccessCommandTable.o

BFC_TELNET_CONSOLE_SNMP_OBJECTS = 
                                  
BFC_TELNET_FACTORY_OBJECTS =

BFC_TELNET_BRCM_SNMP_OBJECTS = brcmTelnetBridge.o brcmTelnetMib.o \
                               brcmTelnetMgmtTrap.o \
                               brcmTelnetMgmtTelnetEventACT.o


# OS-specific Telnet Console support files.
BFC_VxWorks_TELNET_CONSOLE_OBJECTS = VxRemoteTerminalConsoleThread.o

BFC_Ecos_TELNET_CONSOLE_OBJECTS = ecosRemoteTerminalConsoleThread.o

BFC_Win32_TELNET_CONSOLE_OJECTS = 
...
BFC_TELNET_CONSOLE_DEFINES = BCM_TELNET_SUPPORT=1 \
                             BFC_INCLUDE_TELNET_CONSOLE_SUPPORT=1
...
BFC_TELNET_CONSOLE_INCLUDE_DIRS = $(BFC_TELNET_CONSOLE_DIR)
...
BFC_TELNET_CONSOLE_SOURCE_DIRS = $(BFC_TELNET_CONSOLE_DIR)
...
# Add Telnet Console components, if included by BfcSystemConfig.mak.
ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)

    ifeq ($(BFC_INCLUDE_TELNET_CONSOLE_SUPPORT), 1)

        BFC_OBJECTS += $(BFC_TELNET_CONSOLE_OBJECTS) $(BFC_$(BFC_TARGET_OS)_TELNET_CONSOLE_OBJECTS)

        # Include SNMP support.
	ifeq ($(BFC_INCLUDE_SNMP_SUPPORT), 1)

	    BFC_OBJECTS += $(BFC_TELNET_CONSOLE_SNMP_OBJECTS)
            
            # Add factory objects if we have factory support
            ifeq ($(BFC_INCLUDE_BRCM_FACTORY_MIB_SUPPORT), 1)
            
                BFC_OBJECTS += $(BFC_TELNET_FACTORY_OBJECTS)
            ...
            ifeq ($(BFC_INCLUDE_BRCM_RUNTIME_MIB_SUPPORT), 1)
                ...
                BFC_OBJECTS += $(BFC_TELNET_BRCM_SNMP_OBJECTS)
            
            endif

	endif

        # Include command table support if this is a full console build.
        ifeq ($(BFC_CONFIG_MINIMAL_CONSOLE), 0)

            BFC_OBJECTS += $(BFC_TELNET_CONSOLE_CMD_TABLE_OBJECTS)

        endif

        BFC_COMPILER_DEFINES += $(BFC_TELNET_CONSOLE_DEFINES)

        BFC_INCLUDE_DIRS += $(BFC_TELNET_CONSOLE_INCLUDE_DIRS)

        BFC_SOURCE_DIRS += $(BFC_TELNET_CONSOLE_SOURCE_DIRS)

        BFC_LIBRARIES += $(BFC_TELNET_CONSOLE_LIBRARIES)
