...
#
##############################################################################
#  $Id: BfcTftp.mak 1.6 2005/11/02 20:08:30Z dpullen Release $
#
#  Filename:       BfcTftp.mak
#  Author:         David Pullen
...
#  Description:
#
# 	This file contains the build "stuff" for the TFTP Client and Server
...
BFC_TFTP_DIR = Bfc/IpHelpers/Tftp
...
# Modules that are common for both the server and client.  Despite the name,
# TftpClientId is common for the client and server.  It is a wrapper object
# for the TFTP Client Id TLV that appears in TFTP messages.
BFC_TFTP_OBJECTS = Tftp.o

# OS-specific console support.
BFC_VxWorks_TFTP_OBJECTS = TftpClient.o

BFC_Win32_TFTP_OBJECTS = Win32Tftp.o

BFC_Ecos_TFTP_OBJECTS = TftpClient.o

BFC_Qnx6_TFTP_OBJECTS = TftpClient.o

BFC_TFTP_SERVER_OBJECTS = TftpFile.o TftpServerSession.o TftpServerThread.o

# This is for debugging/testing the TFTP server.
#BFC_TFTP_SERVER_OBJECTS += VariableSizeTftpFile.o
...
BFC_TFTP_DEFINES = BFC_INCLUDE_TFTP_SUPPORT=$(BFC_INCLUDE_TFTP_SUPPORT)

BFC_TFTP_DEFINES += BFC_INCLUDE_TFTP_SERVER_SUPPORT=$(BFC_INCLUDE_TFTP_SERVER_SUPPORT)
...
BFC_TFTP_INCLUDE_DIRS = $(BFC_TFTP_DIR)
...
BFC_TFTP_SOURCE_DIRS = $(BFC_TFTP_DIR)
...
# Add Tftp components, if included by BfcSystemConfig.mak.
ifeq ($(BFC_INCLUDE_TFTP_SUPPORT), 1)

    BFC_OBJECTS += $(BFC_TFTP_OBJECTS) $(BFC_TFTP_CLIENT_OBJECTS) $(BFC_$(BFC_TARGET_OS)_TFTP_OBJECTS)
    
    
    # Add Tftp console components, if support for Console and TFTP Console
    # are both included.
    ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)
    
        ifeq ($(BFC_INCLUDE_TFTP_CONSOLE_SUPPORT), 1)

            BFC_OBJECTS += $(BFC_TFTP_CONSOLE_OBJECTS)

        endif
	
    endif
    
    # Add TFTP server files if included
    ifeq ($(BFC_INCLUDE_TFTP_SERVER_SUPPORT), 1)
    
        BFC_OBJECTS += $(BFC_TFTP_SERVER_OBJECTS)
    
    endif

    BFC_COMPILER_DEFINES += $(BFC_TFTP_DEFINES)

    BFC_INCLUDE_DIRS += $(BFC_TFTP_INCLUDE_DIRS)
    
    BFC_SOURCE_DIRS += $(BFC_TFTP_SOURCE_DIRS)
    
    BFC_LIBRARIES += $(BFC_TFTP_LIBRARIES)
