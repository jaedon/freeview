...
#
##############################################################################
#  $Id: BroadcomCommonDataOnlyCm.mak 1.17 2007/06/04 14:29:17Z ceyles Release $
#
#  Filename:       BroadcomCommonDataOnlyCm.mak
#  Author:         David Pullen
#  Creation Date:  Jan 8, 2004
#
##############################################################################
#  Description:
#
...
# This product is a data-only CM, so it uses files from the Common directory and
# the CommonDataOnly directory.
BFC_BCM_COMMON_DATA_ONLY_CM_DIR = Cm/CmExtensionApi/Broadcom/CommonDataOnly \
				  Cm/CmExtensionApi/Broadcom/Common \
				  Cm/CmExtensionApi/Broadcom/Common/SNMP

# Additional directory for the HTTP Server.
BFC_BCM_COMMON_DATA_ONLY_CM_DIR += Cm/CmExtensionApi/Broadcom/CommonDataOnly/HttpServer

# OS-specific directories (if any).
ifeq ($(BFC_TARGET_OS), VxWorks)

    BFC_BCM_COMMON_DATA_ONLY_CM_DIR += Cm/CmExtensionApi/Broadcom/CommonDataOnly/vxWorks
...
BFC_BCM_COMMON_DATA_ONLY_CM_OBJECTS = VendorCmAppFactory.o \
                                      VendorCmApplication.o \
				      VendorExtensionCmStatusACT.o

# SNMP / OSS related vendor extension modules
BFC_BCM_COMMON_DATA_ONLY_CM_OBJECTS += CmVendorSnmpAgent.o \
                                       CpeVendorSnmpAgent.o \
                                       CmVendorEventLog.o 
				       
# HTTP content filter related vendor extension modules
BFC_BCM_COMMON_DATA_ONLY_CM_OBJECTS += VendorHttpSpoofResponses.o

# PR7114 - support for nonvol to store the standby switch state.
BFC_BCM_COMMON_DATA_ONLY_CM_OBJECTS += CmVendorNonVolSettings.o

BFC_BCM_COMMON_DATA_ONLY_CM_CONSOLE_OBJECTS = CmVendorNonVolSettingsCommandTable.o

# PR8674 - add support for multple "last known good" downstream frequencies.
BFC_BCM_COMMON_DATA_ONLY_CM_OBJECTS += GenericCmDownstreamScanThread.o

BFC_BCM_COMMON_DATA_ONLY_CM_BRCM_RUNTIME_OBJECTS = brcmCmMgmtExtMib.o brcmCmMgmtExtBridge.o

BFC_BCM_COMMON_DATA_ONLY_CM_BRCM_FACTORY_OBJECTS = 
...
BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES = BFC_CM_VENDOR_SUPPORT=1 \
                                      BFC_COMMON_VENDOR_SUPPORT=1
				      
ifeq ($(BFC_INCLUDE_RESET_STANDBY_SWITCH_SUPPORT), 1)
    BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES += BFC_INCLUDE_RESET_STANDBY_SWITCH_SUPPORT=$(BFC_INCLUDE_RESET_STANDBY_SWITCH_SUPPORT)
else
        ifeq ($(BFC_INCLUDE_RESET_SES_SWITCH_SUPPORT), 1)
                BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES += BFC_INCLUDE_RESET_SES_SWITCH_SUPPORT=$(BFC_INCLUDE_RESET_SES_SWITCH_SUPPORT)
        else
                ifeq ($(BFC_INCLUDE_RESET_STANDBY_SES_SWITCH_SUPPORT), 1)
                        BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES += BFC_INCLUDE_RESET_STANDBY_SES_SWITCH_SUPPORT=$(BFC_INCLUDE_RESET_STANDBY_SES_SWITCH_SUPPORT)
                endif
        endif
endif

ifeq ($(BFC_INCLUDE_POLLED_STANDBY_SWITCH), 1)
    BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES += BFC_INCLUDE_POLLED_STANDBY_SWITCH=$(BFC_INCLUDE_POLLED_STANDBY_SWITCH)
endif
...
# nonvol settings.
BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES += WEBS_PAGE_ROM=1
...
BFC_BCM_COMMON_DATA_ONLY_CM_INCLUDE_DIRS = $(BFC_BCM_COMMON_DATA_ONLY_CM_DIR)
...
BFC_BCM_COMMON_DATA_ONLY_CM_SOURCE_DIRS = $(BFC_BCM_COMMON_DATA_ONLY_CM_DIR)
...
BFC_BCM_COMMON_DATA_ONLY_CM_LIBRARIES = 
...
ifeq ($(BFC_INCLUDE_BRCM_FACTORY_MIB_SUPPORT), 1)
    ...
    BFC_OBJECTS += $(BFC_BCM_COMMON_DATA_ONLY_CM_BRCM_FACTORY_OBJECTS)
...
ifeq ($(BFC_INCLUDE_BRCM_RUNTIME_MIB_SUPPORT), 1)
    ...
    BFC_OBJECTS += $(BFC_BCM_COMMON_DATA_ONLY_CM_BRCM_RUNTIME_OBJECTS)

endif

# PR7114 - added nonvol support
ifeq ($(BFC_INCLUDE_CONSOLE_SUPPORT), 1)
    ...
    ifeq ($(BFC_CONFIG_MINIMAL_CONSOLE), 0)
        ...
        ifeq ($(BFC_INCLUDE_NONVOL_CONSOLE_SUPPORT), 1)
            ...
            BFC_OBJECTS += $(BFC_BCM_COMMON_DATA_ONLY_CM_CONSOLE_OBJECTS)
...
BFC_COMPILER_DEFINES += $(BFC_BCM_COMMON_DATA_ONLY_CM_DEFINES)

BFC_INCLUDE_DIRS += $(BFC_BCM_COMMON_DATA_ONLY_CM_INCLUDE_DIRS)
    
BFC_SOURCE_DIRS += $(BFC_BCM_COMMON_DATA_ONLY_CM_SOURCE_DIRS)
    
BFC_LIBRARIES += $(BFC_BCM_COMMON_DATA_ONLY_CM_LIBRARIES)
