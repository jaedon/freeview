...
****************************************************************************
*
*  Filename: rnonvolhost.c
*
****************************************************************************
*  Description:	This is Linux host application for the BCM
* 				BroadBand Modems that interface with 7xxx via PCI or SCB
*               For example, 3368/7038, 3255/7401 and 7118.
*
*  Updates: 8/21/2009   Created.
****************************************************************************/
...
static int configureNonVol( char * permcfg, char * dyncfg) {

  if ((fd_perm = open (permcfg, O_RDWR)) < 0)
  {
    printf ("Could not find %s.  Creating...\n", permcfg);
    fd_perm = open (permcfg, O_CREAT | O_RDWR);
    if (fd_perm <0)
    {
      printf ("rnonvolhost:  error: Could not create permanent non-vol file!\n");
      perror ("rnonvolhost");
      exit (1);
    }
  ...
  if ((fd_dyn = open (dyncfg, O_RDWR)) < 0)
  {
    printf ("Could not find %s.  Creating...\n", dyncfg);
    fd_dyn = open (dyncfg, O_CREAT | O_RDWR);
    if (fd_dyn <0)
    {
      printf ("rnonvolhost:  error: Could not create dynamic  non-vol file!\n");
      perror ("rnonvolhost");
      exit (1);
    }
...
static int NonVolCommandHandler (int command, int segment, int section, int size, void *buffer)
{
  ...
  if ((fd < 0) || ((command != NVOL_WRITE) && (command != NVOL_READ) && (command != NVOL_WRITE_LAST)))
  {
    /*
  ...
  else
  {
    ...
    else
    {
      ...
      if (command == NVOL_WRITE || command == NVOL_WRITE_LAST)
      {
...
#endif
        /* 
         *  the mtdocap driver flush the data to flash only if the write size is not
         * (MAX_MESSAGE_SIZE - 8). If the last segment has (MAX_MESSAGE_SIZE - 8) bytes,
         *  write 0 byte to indicate the end of the section.
         */
        if (command == NVOL_WRITE_LAST && (size == (MAX_MESSAGE_SIZE - 8))) {
            printf("Write 0 byte to indicate end of section\n");
            write (fd, (const void *) buffer, 0);
        }
      }
      else
      {
...
int main (int argc, char *argv[])
{
  ...
  if (configureNonVol(permNonVolFileName, dynNonVolFileName)) {
  ...
  while (1)
  {
    ...
    else
    {
      ...
      numBytesRead = recvfrom (cmdSocket, buffer, MAX_MESSAGE_SIZE, 0, (struct sockaddr *) &fromSockAddr, &sockLen);
	...
	{
        count =
          sendto (rspSocket, (const void *) cmdData, bytesDone, 0, (struct sockaddr *) &sockAddrIn, sizeof (sockAddrIn));	
	}
	else
	{
        count =
          sendto (rspSocket, (const void *) buffer, bytesDone, 0, (struct sockaddr *) &sockAddrIn, sizeof (sockAddrIn));
	}
        if (count < 0)
        {
