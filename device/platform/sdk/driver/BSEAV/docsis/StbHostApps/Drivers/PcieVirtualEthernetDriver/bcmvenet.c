...
typedef struct BcmVEnet_devctrl {
    ...
    volatile int dmaReady;                  /* dma ready status */
...
static char *mac3addr;
module_param(mac3addr, charp, 0000);
MODULE_PARM_DESC(mac3addr, "cm mac address");
#endif
...
static int bcmvenet_open(struct net_device *dev) {
    ...
    pDevCtrl->state = 1;
    if (pDevCtrl->dmaReady) {
        pci_write_config_dword(pDevCtrl->pdev, VENDOR_SPECIFIC_REG2, pDevCtrl->dmaReady);
...
#if defined(HUMAX_PLATFORM_BASE) // 131024 mskim2
static unsigned int firstPacketDiscardFlag = 1;
#endif
...
#if defined(HUMAX_PLATFORM_BASE) // 131024 mskim2
    if (firstPacketDiscardFlag) {
        pDevCtrl->int_lvl2->PciClear |= (L0_RX_DONE_IRQ  | MBOX0_IRQ);
//        pDevCtrl->int_lvl2->PciMaskClear |= (L0_RX_DONE_IRQ  | MBOX0_IRQ);

        firstPacketDiscardFlag = 0;
        return 0;
    }
#endif
    ...
    while ((cnt < budget)) {
        ...
        if (pDevCtrl->rx_new_bd  == cur_bd) {
            ...
            if ((pDevCtrl->dmactl->TxSwDescListCtrlSts & 0x30) == 0x10) {
                ...
                printk("dma busy\n");
#if defined(HUMAX_PLATFORM_BASE) // 131111 mskim2
printk(KERN_INFO CARDNAME "%s:%d cnt = %d\n", __FUNCTION__, __LINE__, cnt);
                cnt = 0;
#endif
                break;
            }
...
#if defined(HUMAX_PLATFORM_BASE)
		/*	E203 patch code. modification of communication error between 3383 and 7425 through the PCIe	*/
		pci_unmap_single(pDevCtrl->pdev, pDevCtrl->rx_cb[index].pa, ENET_MAX_MTU_SIZE, DMA_FROM_DEVICE);
#endif
...
static int __devinit bcmvenet_drv_probe(struct pci_dev *pdev, const struct pci_device_id *pid) {
    ...
    ndev = alloc_netdev_mq(sizeof(*pDevCtrl), "bcm%d", ether_setup, 1);
    if (ndev == NULL) {
...
#if defined(HUMAX_PLATFORM_BASE)
	if(mac3addr) {
		sscanf(mac3addr, "%02X:%02X:%02X:%02X:%02X:%02X", &mac3addr_buf[0],
								  &mac3addr_buf[1],
								  &mac3addr_buf[2],
								  &mac3addr_buf[3],
								  &mac3addr_buf[4],
								  &mac3addr_buf[5]);
		ndev->dev_addr[0] = (unsigned char)mac3addr_buf[0];
		ndev->dev_addr[1] = (unsigned char)mac3addr_buf[1];
		ndev->dev_addr[2] = (unsigned char)mac3addr_buf[2];
		ndev->dev_addr[3] = (unsigned char)mac3addr_buf[3];
		ndev->dev_addr[4] = (unsigned char)mac3addr_buf[4];
		ndev->dev_addr[5] = (unsigned char)mac3addr_buf[5];
	} else {
