...
static void logprintf(int severity, char *s, ...) {
    va_list args;
    va_start(args, s);
    vsyslog(severity, s, args);
}
...
int ethInterfaceUp(int secsToWait)
{
	...
	while (1)
	{
		...
		{
			logprintf(LOG_INFO, "Waiting for eth interface to come up. %d seconds elapsed", sleepSecsCount);
		}

		if (secsToWait && (sleepSecsCount > secsToWait))
		{
			logprintf(LOG_ERR, "Timeout expired and eth interface still unavailable...exiting");
			close(socketDescriptor);
			return errno;
		}
	}
...
static int configureNonVol(int *fd, char *cfg)
{
    *fd = open(cfg, O_RDWR);
	...
	{
        char buf[100];
		logprintf(LOG_INFO, "Could not open %s", cfg);
        if (memcmp(cfg, "/dev/", 5) == 0)
            return 1;
        logprintf(LOG_INFO, "Creating %s", cfg);
        *fd = creat(cfg, O_RDWR);
        if (*fd < 0)
        {
            logprintf(LOG_ERR, "Could not create %s", cfg);
            return 1;
        }
...
static int NonVolCommandHandler(int command, int segment, int section, int size, void *buffer)
{
	...
	if ((fd < 0) || ((command != NVOL_WRITE) && (command != NVOL_READ)))
	{
		...
		if (fd < 0)
		{
			logprintf(LOG_INFO, "**DEBUG**> Could not open nonvol file");
		}
		else
		{
			logprintf(LOG_INFO, "**DEBUG**> Invalid nonvol command!");
		}
#endif
	}
	else
	{
		...
		else
		{
			...
			if (command == NVOL_WRITE)
			{
				...
				{
#if (DEBUG > 0)
					logprintf(LOG_INFO, "**DEBUG**> Retrying write due to bad block");
#endif
					bytesWrittenOrRead = write(fd, (const void *)buffer, size);

				}
#if (DEBUG > 0)
				logprintf(LOG_INFO, "**DEBUG**> Wrote %d bytes at offset %d", bytesWrittenOrRead, offset);
#endif
			}
			else
			{
...
#if (DEBUG > 0)
				logprintf(LOG_INFO, "**DEBUG**> Read %d bytes from offset %d", bytesWrittenOrRead, offset);
#endif
			}
			...
			if (bytesWrittenOrRead != size)
			{
				 ...
				 */
				logprintf(LOG_ERR, "Incomplete nvol operation - requested: %d bytes, actual: %d bytes", size, bytesWrittenOrRead);
			}
		}
	}
...
int main(int argc, char *argv[])
{
    ...
    if (pid > 0) {
        ...
        printf("Fork failed, continuing as application\n");
    }

    openlog("rnonvolhost", LOG_CONS, LOG_DAEMON);

    logprintf(LOG_INFO, "Starting - version %s", VERSION);

	if (configureNonVol(&fd_perm, permNonVolFileName) || configureNonVol(&fd_dyn, dynNonVolFileName))
	{
		logprintf(LOG_ERR, "Could not configure NonVol %s or %s", permNonVolFileName, dynNonVolFileName);
		return -1;
	}
	/* 
	...
	if (ethInterfaceUp(0))
	{
		logprintf(LOG_ERR, "Eth interface never came up...exiting");
		return -1;
	}
	...
	if (configureEthInterface())
	{
		 ...
		 */
		logprintf(LOG_ERR, "Could not configure eth interace %s", interfaceName);
		return -1;
	}
	...
	{
		/* Handle error */
		logprintf(LOG_ERR, "Couldn't bind command socket! %s", strerror(errno));
		return -1;
	}
	...
	while (1)
	{
		...
		if (retCode)
		{
			...
			logprintf(LOG_ERR, "%s Bailing...", strerror(errno));
			return -1;
		}
		else if (ethDeviceNeedsSetup)
		{
			if (configureEthInterface())
			{
				logprintf(LOG_ERR, "Could not configure ethernet interface %s - %s", interfaceName, strerror(errno));
			}
			ethDeviceNeedsSetup = 0;
		}
		...
		{
			// logprintf("rnonvolhost: No data received!\n");
		}
		else
		{
			...
			if (numBytesRead <= 0)
			{
				logprintf(LOG_ERR, "Socket Receive Error - %s", strerror(errno));
			}
			else
			{
...
#if (DEBUG > 0)
				logprintf(LOG_INFO, "**DEBUG**> Received %d bytes on nvol cmd socket\n", numBytesRead);
#endif
...
#if (DEBUG > 0)
				logprintf(LOG_INFO, "**DEBUG**>  Cmd: %02x  Segment: %02x", cmd, segment);
				logprintf(LOG_INFO, "            Sec: %02x  Length: %02x\n", section, length);
#endif
				/* 
				...
				{
					logprintf(LOG_ERR, "Did not understand command!");
				}
				...
				if (bytesDone != length)
				{
					logprintf(LOG_ERR, "Nonvol operation return size did not match specified size - "
					    "specified size: %d  command returned: %d\n", length, bytesDone);
				}
...
#if (DEBUG > 0)
				logprintf(LOG_INFO, "**DEBUG**> Sending response of %d bytes", bytesDone);
#endif
				...
				if (count < 0)
				{
					logprintf(LOG_ERR, "Could not send message via response socket: %d! - %s", count, strerror(errno));
				}
