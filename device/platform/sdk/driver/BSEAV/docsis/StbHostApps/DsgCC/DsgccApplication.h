...
//  $Id$
//
//  Filename:       StbApplication.h
//  Author:         Kevin O'Neal
//  Creation Date:  October 14, 2004
//
//****************************************************************************

#ifndef DsgccApplication_H
#define DsgccApplication_H
...
class BcmCmDocsisApplication;
class BcmForwarder;
class BcmBaseIpHalIf;


#if (BCM_RMAGNUM_SUPPORT)
class BcmStbDownstreamControlThread;
class BcmStbRmagnumServerThread;
class BcmStbRmagnumHelperThread;
#endif

#if (BCM_DSG_SUPPORT)
class BcmDSGSvcMgrThread;
#endif
...
*   configuration.
*/
class BcmDsgccApplication : public BcmBfcApplication
{
public:
    ...
    /// API.
    ///
    BcmDsgccApplication(void);

    /// Destructor.  Frees up any memory/objects allocated, cleans up internal
    /// state.
    ///
    virtual ~BcmDsgccApplication();
...
public:  // Accessor methods that were previously implemented in the
    ...
    ///
    BcmCmDocsisCtlThread *GetDocsisCtlThread(void) const;
    ...
    ///      A pointer to the DOCSIS Ctl Thread instance.
    ///
    inline void SetDocsisCtlThread(BcmCmDocsisCtlThread * pCmDocsisCtlThread) {pfCmDocsisCtlThread = pCmDocsisCtlThread;}
    ...
    ///      A pointer to the DOCSIS Ctl Thread instance.
    ///
    inline void SetCmDocsisApplication(BcmCmDocsisApplication * pCmDocsisApplication) {pfCmDocsisApplication = pCmDocsisApplication;}

    /// Set the Forwarder instance that is being used.
    ///
    /// \return
    ///      A pointer to the Forwarder instance
    ///
    inline void SetSettopForwarder(BcmForwarder * pSettopForwarder) {pfSettopForwarder = pSettopForwarder;}
    
    #if (BCM_DSG_SUPPORT)
    inline BcmDSGSvcMgrThread *GetDsgThread() {return pfDsgThread;}
    #endif
...
public:
	...
	/// Helper method that retrieves and configures the eCM's private stack.
	/// The primary purpose is to set the static IP address from the values in
	/// nonvol, if the stack is present.
	///
	/// \return
	///     A pointer to the private stack instance, or NULL if it isn't
	///     enabled.
	///
	BcmBaseIpHalIf *ConfigurePrivateStack(void);

	/// Helper method that ads a forwarder rule to ensure that IP3 can only 
	/// talk to DSG transport HAL.
	/// Assuming at this moment, all interfaces have been registered with 
	/// the forwarder.
	///
	/// \return
	///
	void SecureEcmPrivateStack(void);

    #if (BCM_RMAGNUM_SUPPORT)
    void StartrMagnumThreadEarly(void);
    #endif

protected:

    /// Used to map old-school V2 application control calls to the new BFC
    /// object (me).
    BcmCmApplication *pfCmApplication;

    /// The DOCSIS Control Thread; handles the DOCSIS state machine.
    BcmCmDocsisCtlThread *pfCmDocsisCtlThread;

    BcmCmDocsisApplication *pfCmDocsisApplication;

    BcmForwarder *pfSettopForwarder;
    
	#if (BCM_RMAGNUM_SUPPORT)
	BcmStbDownstreamControlThread *pfStbDsCtlThread;
	BcmStbRmagnumServerThread *pfStbRpcSrvThread;
	BcmStbRmagnumHelperThread *pfStbRpcHelperThread;
	#endif
	
    #if (BCM_DSG_SUPPORT)
    BcmDSGSvcMgrThread *pfDsgThread;
    #endif

private:

    /// Copy Constructor.  Not supported.
    BcmDsgccApplication(const BcmDsgccApplication &otherInstance);

    /// Assignment operator.  Not supported.
    BcmDsgccApplication & operator = (const BcmDsgccApplication &otherInstance);
