...
#ifndef ESTBIFBRIDGE_H
#define ESTBIFBRIDGE_H
...
class interfacesGroupBridge : public ScalarMibBridge
{
  ...
  public:
    ...
    interfacesGroupBridge (BcmSnmpAgent *pAgent);
   ~interfacesGroupBridge ();
    ...
    unsigned int linuxIfSpeed(int fd, struct ifreq ifr);
    void linux_Interface_Scan_Init(void);
    int linux_Interface_Scan_Next(short *Index, char *Name,
                    struct ifnet *Retifnet, struct in_ifaddr *dummy);
    int linux_Interface_Scan_Get_Count(void);
    int linux_Interface_Index_By_Name(char *Name, int Len);
    int linux_Interface_Scan_By_Index(int Index, char *Name,
    ...
    ifEntryBridge *FindIfEntry (const char *ifDescr, int len);
    ...
    int linux_If_Type_From_Name(const char *pcch);
    unsigned int linux_getIfSpeed(int fd, struct ifreq ifr);
...
class ifEntryBridge : public TabularMibBridge
{
  ...
  public:
   ...
   ~ifEntryBridge ();
    ...
    inline int Assign_ifDescr(char *descr);
    inline void linuxIfTypeGuess();
    inline void Assign_ifSpeed(unsigned int speed);
    inline void Assign_ifPhysAddress (BcmMacAddress addr);
  ...
  protected:    
    ...
    BcmCounter64 fReceivedUnicastPackets;
    BcmCounter64 fReceivedMulticastPackets;
    BcmCounter64 fReceivedBroadcastPackets;

    uint32 fReceivedPacketsNotForwarded;

    BcmCounter64 fTransmittedOctets;
    BcmCounter64 fTransmittedUnicastPackets;
    BcmCounter64 fTransmittedMulticastPackets;
    BcmCounter64 fTransmittedBroadcastPackets;
...
inline int ifEntryBridge::Assign_ifDescr (char *descr)
{
  fIfDescr = descr;
  return fIfDescr.length();
}

// As it says in the title, this is just an educated
// guess at the interface type, based on ifDescr
// which we got from the Linux driver. So, if you do
// something cute, like naming your interface "fred0"
// then you will get a type of "other". Best bet
// is to have "eth" somwhere in the name.
inline void ifEntryBridge::linuxIfTypeGuess (void)
{
  fIfType = BcmHalIf::kIanaTypeOther;

  if (fIfDescr.length()) {
    if (fIfDescr.find(BcmString("lo")) == 0)
      fIfType = BcmHalIf::kIanaTypeSoftwareLoopback;
    else if (fIfDescr.find(BcmString("eth")) == 0)
      fIfType = BcmHalIf::kIanaTypeEthernet;
    else if (fIfDescr.find(BcmString("bcm")) == 0)
      fIfType = BcmHalIf::kIanaTypeEthernet;
    else if (fIfDescr.find(BcmString("usb")) == 0)
      fIfType = BcmHalIf::kIanaTypeUsb;
  } 
}


inline void ifEntryBridge::Assign_ifSpeed (unsigned int speed)
{
  fIfSpeed = speed;
  return;
}

inline void ifEntryBridge::Assign_ifPhysAddress (BcmMacAddress addr)
{
  fIfPhysAddress = addr;
  return;
}
