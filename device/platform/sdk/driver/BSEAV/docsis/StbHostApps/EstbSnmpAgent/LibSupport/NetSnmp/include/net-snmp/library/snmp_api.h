...
#define SNMP_FLAGS_PROBE_FAILED    0x0800        /* asynchronous probe failed */
#define SNMP_FLAGS_PROBE_PENDING   0x0400        /* asynchronous probe pending */
#define SNMP_FLAGS_DONT_PROBE      0x0200        /* don't probe for an engineID */
#define SNMP_FLAGS_NOTIFY_SESSION  0x0100        /* notify session, not agent */
#define SNMP_FLAGS_STREAM_SOCKET   0x0080
#define SNMP_FLAGS_LISTENING       0x0040        /* Server stream sockets only */
#define SNMP_FLAGS_SUBSESSION      0x0020
#define SNMP_FLAGS_STRIKE2         0x0002
#define SNMP_FLAGS_STRIKE1         0x0001
...
struct variable_list {
    ...
    size_t          val_len;
#if NETSNMP_FIXEDLEN_OID        /* PR 9824: SAVE MEMORY - always malloc & free */ 
...
#define NETSNMP_CALLBACK_OP_DISCONNECT		5
    ...
    // BRCM_MOD: PR 7336: async version of snmp_add that doesn't hang on
    // SNMPv3 engine ID probes.
    netsnmp_session *snmp_sess_add_async (netsnmp_session *in_session, 
                                          struct netsnmp_transport_s *transport);
    ...
    int             snmp_sess_async_send(void *, netsnmp_pdu *,
                                         netsnmp_callback, void *);
    int             snmp_async_queue(void *, netsnmp_pdu *, 
                                         snmp_callback, void *);
