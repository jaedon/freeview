                       ...
                       netsnmp_request_info *requests)
{
    ...
    for (; requests; requests = requests->next) {
        ...
        case MODE_SET_RESERVE1:
            ...
            if (vp && vp->findVar)
            {
                //printf ("old_api: findVar = %p\n", vp->findVar);
            
                #if (!NETSNMP_FIXEDLEN_OID)
                // HERE IS A BUG!  This code was assuming that the name is big
                // enough to hold what we are about to do.  This is not true
                // if this is a getnext, since the OID will be built for us by
                // the findVar function.  It worked just fine back when we had
                // everything being name_loc, but now that we allocate storage
                // (PR 9824) this is probably not big enough.
                if (reqinfo->mode == MODE_GETNEXT)
                {
                  // Allocate new storage as big as it can possibly be.
                  // Note that we must not change the length since it indicates
                  // the length of the OID, not the size of the buffer.
                  oid *NewName = (oid *) malloc (MAX_OID_LEN * sizeof(oid));
                  
                  // Copy old name to new name
                  memcpy (NewName, requests->requestvb->name, requests->requestvb->name_length * sizeof(oid));
                  
                  // Now we can free the old name and reassign to the new name.
                  SNMP_FREE(requests->requestvb->name);
                  requests->requestvb->name = NewName;
                }
                #endif
