...
#include "docsDevCpeTable.h"

/** Initialize the docsDevCpeTable table by defining its contents and how it's structured */
void
initialize_table_docsDevCpeTable(void)
{
    static oid      docsDevCpeTable_oid[] =
     ...
     */
    my_handler = netsnmp_create_handler_registration("docsDevCpeTable",
                                                     docsDevCpeTable_handler,
                                                     docsDevCpeTable_oid,
                                                     OID_LENGTH
                                                     (docsDevCpeTable_oid),
                                                     HANDLER_CAN_RWRITE);
    ...
    if (my_handler) {
        if (netsnmp_register_handler(my_handler)) {
            DEBUGMSGTL(("initialize_table_docsDevCpeTable",
...
/** Initializes the docsDevCpeTable module */
void
init_docsDevCpeTable(void)
{

    /*
     * here we initialize all the tables we're planning on supporting 
     */
    initialize_table_docsDevCpeTable();
}

/** handles requests for the docsDevCpeTable table */
int
docsDevCpeTable_handler(netsnmp_mib_handler *handler,
    ...
    for (request = requests; request; request = request->next) {
        ...
        case MODE_GET:
            ...
            if (retVal == SNMP_ERR_NOERROR) {
                ...
                case COLUMN_DOCSDEVCPESOURCE:
                    snmp_set_var_typed_value(var, ASN_INTEGER,
                                             (u_char *) tmpVal, tmpValLen);
                    break;

                case COLUMN_DOCSDEVCPESTATUS:
                    snmp_set_var_typed_value(var, ASN_INTEGER,
                                             (u_char *) tmpVal, tmpValLen);
                    break;
                ...
                default:
                     ...
                     */
                    snmp_log(LOG_ERR,
                             "problem encountered in docsDevCpeTable_handler: unknown column\n");
                }
            } else {
            ...
            break;
        case MODE_GETNEXT:
            DEBUGMSGTL(("docsDevCpeTable_handler", "GETNEXT\n"));
            /*
            ...
            if ((netsnmp_oid_is_subtree
                ...
                DEBUGMSGTL(("docsDevCpeTable_handler",
                ...
                if ((retVal == SNMP_ERR_NOSUCHNAME)
                    && (tmpOid[reginfo->rootoid_len + 1] < 3)) {
                    /*
                     * more columns to try 
                     */
        ...
        case MODE_SET_RESERVE1:
            printf("docsDevCpeTable_handler: Mode = MODE_SET_RESERVE\n");
            switch (var->name[reginfo->rootoid_len + 1]) {
            case COLUMN_DOCSDEVCPESTATUS:
        ...
        case MODE_SET_COMMIT:
            printf("docsDevCpeTable_handler: Mode = MODE_SET_COMMIT\n");
            /*
             * really do the set! 
             */
            switch (var->name[reginfo->rootoid_len + 1]) {
