...
#include "docsIfCmMacTable.h"

/** Initialize the docsIfCmMacTable table by defining its contents and how it's structured */
void
initialize_table_docsIfCmMacTable(void)
{
    static oid      docsIfCmMacTable_oid[] =
     ...
     */
    my_handler = netsnmp_create_handler_registration("docsIfCmMacTable",
                                                     docsIfCmMacTable_handler,
                                                     docsIfCmMacTable_oid,
                                                     OID_LENGTH
                                                     (docsIfCmMacTable_oid),
                                                     HANDLER_CAN_RWRITE);
    ...
    if (my_handler) {
        if (netsnmp_register_handler(my_handler)) {
            DEBUGMSGTL(("initialize_table_docsIfCmMacTable",
...
/** Initializes the docsIfCmMacTable module */
void
init_docsIfCmMacTable(void)
{

    /*
     * here we initialize all the tables we're planning on supporting 
     */
    initialize_table_docsIfCmMacTable();
}

/** handles requests for the docsIfCmMacTable table */
int
docsIfCmMacTable_handler(netsnmp_mib_handler *handler,
    ...
    for (request = requests; request; request = request->next) {
        ...
        case MODE_GET:
            ...
            if (retVal == SNMP_ERR_NOERROR) {
                ...
                case COLUMN_DOCSIFCMCMTSADDRESS:
                    snmp_set_var_typed_value(var, ASN_OCTET_STR,
                                             (u_char *) tmpVal, tmpValLen);
                    break;

                case COLUMN_DOCSIFCMCAPABILITIES:
                    snmp_set_var_typed_value(var, ASN_OCTET_STR,
                                             (u_char *) tmpVal, tmpValLen);
                    break;

                case COLUMN_DOCSIFCMRANGINGRESPTIMEOUT:
                    snmp_set_var_typed_value(var, ASN_TIMETICKS,
                                             (u_char *) tmpVal, tmpValLen);
                    break;

                case COLUMN_DOCSIFCMRANGINGTIMEOUT:
                ...
                default:
                     ...
                     */
                    snmp_log(LOG_ERR,
                             "problem encountered in docsIfCmMacTable_handler: unknown column\n");
                }
            } else {
            ...
            break;
        case MODE_GETNEXT:
            DEBUGMSGTL(("docsIfCmMacTable_handler", "GETNEXT\n"));
            /*
            ...
            if ((netsnmp_oid_is_subtree
                ...
                DEBUGMSGTL(("docsIfCmMacTable_handler",
        ...
        case MODE_SET_RESERVE1:
            printf("docsIfCmMacTable_handler: Mode = MODE_SET_RESERVE\n");
            switch (var->name[reginfo->rootoid_len + 1]) {
            case COLUMN_DOCSIFCMRANGINGRESPTIMEOUT:
                if (requests->requestvb->type != ASN_TIMETICKS) {
        ...
        case MODE_SET_COMMIT:
            printf("docsIfCmMacTable_handler: Mode = MODE_SET_COMMIT\n");
            /*
             * really do the set! 
             */
            switch (var->name[reginfo->rootoid_len + 1]) {
