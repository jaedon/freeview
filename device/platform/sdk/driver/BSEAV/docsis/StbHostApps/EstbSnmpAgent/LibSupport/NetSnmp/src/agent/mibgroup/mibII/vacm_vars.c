...
#define PRIVRW	(SNMPV2ANY | 0x5000)
     ...
     * register ourselves with the agent to handle our mib tree 
     */
    // BRCM_MOD: PR 7207 don't do this unless explicitly requested to by 
    // register_vacm_vars, above
    //REGISTER_MIB("mibII/vacm:sec2group", vacm_sec2group, variable1,
    //             vacm_sec2group_oid);
    //REGISTER_MIB("mibII/vacm:access", vacm_access, variable1,
    //             vacm_access_oid);
     ...
     */
    /*
    brcm_mod: we will use our own callbacks for these.
...
#define CONTEXTNAMEINDEXLEN 32
    ...
    {
        if (gp->status != SNMP_ROW_ACTIVE) /* BRCM_MOD PR6804 check for active group row*/
        {
            printf("vacm_in_view: GROUP Entry is present but NOT ACTIVE \n");
            return 2;
        }
    }
    ...
    {
        if (ap->status != SNMP_ROW_ACTIVE) /* BRCM_MOD PR6804 check for active access row*/
        {
            printf("vacm_in_view: ACCESS Entry is present but NOT ACTIVE \n");
            return 3;
        }
    }
    ...
    ////////////////////////////////////
    /* BRCM_MOD PR6843: check for active view */
    if (vp->viewStatus != SNMP_ROW_ACTIVE)
    {
        //printf("vacm_vars: VIEW IS NOT ACTIVE \n");
        DEBUGMSG(("mibII/vacm_vars", "View is NOT ACTIVE\n"));
        return 5;
    }
    /////////////////////////////////////
                   ...
                   size_t * var_len, WriteMethod ** write_method)
{
    ...
    int             groupSubtreeLen;
    /*
    BRCM_MOD changed secmodel from int to unsigned int.  This fixes a bug whereby
    if an OID index secmodel value greater than 32k was received, the compare 
    interpreted secmodel as -1 and the walk would not return the correct next value. */
    /*       int    secmodel; */
    unsigned int    secmodel;
    unsigned int    secnamelen;
    ...
    if (exact) {
        ...
        secnamelen = name[12];
        /* BRCM_MOD
           Do a check to see if the length of the securityName (index) is
           actually the number of bytes in the securityName (index).
        */ 
        if (secnamelen != groupSubtreeLen)
        {
            printf("Group Index SecurityName length is incorrect - return 0\n");
            return 0;
        }
                ...
                int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    /*
    BRCM_MOD changed secmodel and seclevel from int to unsigned int.  This fixes a bug whereby
    if an OID index secmodel value greater than 32k was received, the compare 
    interpreted secmodel as -1 and the walk would not return the correct next value. */
              ...
              int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof(long_return);
    ...
    case VIEWMASK:
        // PR 7544: use the viewMaskLen here, don't just calculate
        // the length based on subtree length.  Otherwise we can never
        // have the MIB default of empty string.
                    ...
                    u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        // BRCM_MOD PR6789 / 7011:  If a row has already been created, grab it 
        // and indicate via the (previously unused) bitMask field that the group
        // name has been set.  We'll check for this later in the ACTION phase
        // of the set to the RS object to verify that this required column
        // has been set.
        geptr = sec2group_parse_groupEntry(name, name_len);
        if (geptr)
          geptr->bitMask |= (1 << GROUPNAME);
                                ...
                                oid * name, size_t name_len)
{
    ...
    // BRCM-MOD: do this in ACTION, not commit.  For some bizarre
    // reason, Net-SNMP ignores error codes returned from COMMIT for this
    // object.  For other objects too?
    } else if (action == ACTION) {
        ...
        //printf ("write_vacmSecurityToGroupStatus, ACTION: geptr = %p\n", geptr);        
        ...
        if (geptr != NULL) {
            if (long_ret == RS_CREATEANDGO || long_ret == RS_ACTIVE) {
                 ...
                 */
                // BRCM_MOD: PR 7011.  Check the geptr->bitMask field to make
                // sure we've had the groupName set.
                //printf ("groupName = '%s', bitMask = %08X\n", geptr->groupName, geptr->bitMask);
                if ((geptr->groupName[0] == 0) && ((geptr->bitMask & (1 << GROUPNAME)) == 0)) {
                    free(newName);
                    return SNMP_ERR_INCONSISTENTVALUE;
                }
                ...
                if (geptr->status == RS_ACTIVE) {
                    ...
                    // BRCM_MOD PR 7056: Could be we're notReady but we have a set
                    // to groupName pending.  Allow the transition in that case.
                    if ((geptr->groupName[0] == 0) && ((geptr->bitMask & (1 << GROUPNAME)) == 0)) {
                        free(newName);
                        return SNMP_ERR_INCONSISTENTVALUE;
                    }
                             ...
                             u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE2)
    {
      ...
      if ((*((long *) var_val) != CM_EXACT)
      &&  (*((long *) var_val) != CM_PREFIX))
        return SNMP_ERR_WRONGVALUE;
    }
               ...
               oid ** subtree, size_t * subtreeLen)
{
    ...
    //printf ("view_parse_oid\n");

    /*
     * first check the validity of the oid 
     */
    if ((oidLen <= 0) || (!oidIndex)) {
        //printf ("  INCONSISTENTNAME 1\n");
        return SNMP_ERR_INCONSISTENTNAME;
    }
    ...
    *viewNameLen = viewNameL;
    ...
    for (i = 0; i < subtreeL; i++) {
        /* STUPID WRONG don't check > 255 for subtree, IDIOT!
    	if (oidIndex[i + viewNameL + 1] > 255) {
    	    free(*viewName);
    	    free(*subtree);
            printf ("  INCONSISTENTNAME 3\n");
    	    return SNMP_ERR_INCONSISTENTNAME;
    	}*/
    ...
    if (action == RESERVE2) {
        ...
        if ((rc =
             view_parse_oid(&name[VIEW_MIB_LENGTH],
                            ...
                            (u_char **) & newViewName, &viewNameLen,
                            (oid **) & newViewSubtree, &viewSubtreeLen))) {
            //printf ("  view_parse_oid failure %d\n", rc);
            return rc;
        }
                          ...
                          u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if (*((long *) var_val) != ST_VOLATILE)
        {
            // PR 7436: Allow a set to what we already are.
            vptr = view_parse_viewEntry(name, name_len);
            if ((vptr != NULL) && (*((long *) var_val) != vptr->viewStorageType))
              return SNMP_ERR_INCONSISTENTVALUE;
        }
                       ...
                       u_char * statP, oid * name, size_t name_len)
{
    ...
    // BRCM_MOD: (KO) model this after write_targetSpinLock so we can get things
    // checked at the appropriate SET phase
    if (action == RESERVE1) {
        if (var_val_type != ASN_INTEGER) {
            return SNMP_ERR_WRONGTYPE;
        }
        ...
        if (*((unsigned long *) var_val) != vacmViewSpinLock) {
            if (*((long *) var_val) < 0)
              return SNMP_ERR_WRONGVALUE;
            else
              return SNMP_ERR_INCONSISTENTVALUE;
