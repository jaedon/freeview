...
init_usmUser(void)
{
    // BRCM_MOD don't do config file stuff
    ...
    // BRCM_MOD PR7207: don't register this MIB unless we're explicitly
    // instructed to do so via register_usmUser, above.
//  REGISTER_MIB("snmpv3/usmUser", usmUser_variables, variable4,
//               usmUser_variables_oid);
}
    ...
    *var_len = sizeof(long_ret);        /* assume an integer and change later if not */
    ...
    if (vp->magic != USMUSERSPINLOCK) {
        ...
        if (((int) *length) <= (int) vp->namelen || rtest == -1) {
            ...
            // BRCM_MOD: make sure there is SOME kind of index.
            // PR6218 found this table would return an answer on a GET query
            // for usmUserSecurityName (for example) with no index.  So check 
            // for *length == 12 and exact == 1
            if ((*length == 12) && (exact == 1))
            {
              DEBUGMSG(("usmUser", "Request for column w/o index, uptr = NULL.\n"));
              uptr = NULL;
            }
            else
                      ...
                      u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if (*((unsigned long *) var_val) != usmUserSpinLock) {
            if (*((long *) var_val) < 0)
                       ...
                       u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        // BRCM_MOD: check this in RESERVE2, not RESERVE1
                          ...
                          u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        SNMP_FREE(optr);
        optr = NULL;
    } else if (action == FREEx || action == UNDO) {
        if ((uptr = usm_parse_user(name, name_len)) != NULL) {
            if (resetOnFail) {
                           ...
                           u_char * statP, oid * name, size_t name_len)
{
    ...
    // BRCM_MOD: check type only in RESERVE1, move length to RESERVE2
    if (action == RESERVE1) {
        resetOnFail = 0;
        ...
        /* BRCM_MOD PR6992: fix for SilverCreek 4.16.6 - add check for storage type */
        /* if (uptr->cloneFrom == NULL) { */
        if ((uptr->cloneFrom == NULL) && (uptr->userStorageType != ST_PERMANENT)) {
            return SNMP_ERR_INCONSISTENTNAME;
        }
        if (snmp_oid_compare(uptr->authProtocol, uptr->authProtocolLen,
                             usmNoAuthProtocol,
            ...
            if (snmp_oid_compare(uptr->authProtocol, uptr->authProtocolLen,
                ...
                if (snmp_oid_compare
                    ((oid *) var_val, var_val_len / sizeof(oid),
                     usmNoPrivProtocol,
                     sizeof(usmNoPrivProtocol) / sizeof(oid)) != 0) {
                    /*BRCM_MOD return WrongValue instead of WrongLength.  Fix for SilverCreek 4.17.2 */
                    //printf("priv prot d3 ret wrongvalue instead of inconsistent value\n");
                    return SNMP_ERR_WRONGVALUE;
                    //return SNMP_ERR_INCONSISTENTVALUE;
                }
            } else {
                           ...
                           u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        /* BRCM_MOD PR6992: fix for SilverCreek 4.19.7 - add check for storage type */
        /* if (uptr->cloneFrom == NULL) { */
        if ((uptr->cloneFrom == NULL) && (uptr->userStorageType != ST_PERMANENT)) {
            return SNMP_ERR_INCONSISTENTNAME;
        }
                         ...
                         u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        // BRCM_MOD: Check these in RESERVE2, not RESERVE1
        ...
        if (long_ret != ST_VOLATILE) 
            DEBUGMSGTL(("usmUser", "Only allow storageType VOLATILE per DOCSIS spec - ret inconsistent val\n"));
            return SNMP_ERR_INCONSISTENTVALUE;
    ...
    } else if (action == UNDO || action == FREEx) {
        if ((uptr = usm_parse_user(name, name_len)) != NULL && resetOnFail) {
...
{
    // Give a by if we're already active or notInService
    if ((uptr->userStatus == RS_ACTIVE)
    ||  (uptr->userStatus == RS_NOTINSERVICE))
        return 1;
                    ...
                    u_char * statP, oid * name, size_t name_len)
{
    ...
    // BRCM MOD: Check everything other than type in RESERVE2, not RESERVE1.
    } else if (action == RESERVE2) {
        if (var_val_len != sizeof(long_ret)) {
            DEBUGMSGTL(("usmUser",
