...
struct targetAddrTable_struct *aAddrTable = 0;
...
snmpTargetAddrTable_create(void)
{
    ...
    if (newEntry) {
        ...
        newEntry->tAddress   = 0;
        newEntry->tAddressLen = 0;  /*brcm_mod change tAddress to TAddressLen*/
        ...
        // BRCM_MOD: change default to VOLATILE per DOCSIS requirements
        newEntry->storageType = SNMP_STORAGE_VOLATILE; //SNMP_STORAGE_NONVOLATILE;
...
{
    if ((entry->tDomainLen == 0) || (entry->tAddressLen == 0) ||
        (entry->params == 0) || (entry->tAddressLen == 0))  /* brcm_mod - replace tAddress with tAddressLen */
        return 0;
    else
        return 1;
...
void register_snmpTargetAddrEntry(void)
{
    ...
    // BRCM_MOD PR 7207: don't register these objects unless explicitly 
    // instructed to do so via register_snmpTargetAddrEntry, above.
                        ...
                        size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof(long_ret);        /* assume an integer and change later if not */
    ...
    case SNMPTARGETADDRTADDRESS:
        if ((temp_struct->tAddressLen <= 0) ||
            (temp_struct->tAddress <= 0))      /*brcm_mod - replace tAddress with tAddressLen*/
            return (0);
                            ...
                            u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        // BRCM_MOD: check value (0..2147483647)
        if ((*((long *) var_val) < 0)
        ||  (*((long *) var_val) > 2147483647))
...
{
    /* BRCM_MOD add 0x0a, I think 0x0b is wrong but leave it anyway */
    return (c == 0x020 || c == 0x09 || c == 0x0d || c == 0x0b || c == 0x0a);
}
                           ...
                           u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if ((target =
            ...
            /* BRCM_MOD - no it is ok to change this if row is active */
            /*
            if (target->rowStatus == SNMP_ROW_ACTIVE) {
                DEBUGMSGTL(("snmpTargetAddrEntry",
                                ...
                                oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        /* BRCM MOD: only allow VOLATILE per DOCSIS requirements */
        if (long_ret != SNMP_STORAGE_VOLATILE)
            return SNMP_ERR_INCONSISTENTVALUE;
    ...
    if (action == RESERVE1) {
        ...
        if (target != NULL) {
            ...
            if (value == RS_NOTINSERVICE) {
                if (target->sess != NULL) {
                    // BRCM_MOD: don't do this.  Problem is, what if all the
                    // targets share the same session?  What then?  Plus,
                    // in any case they don't close the session if we
                    // delete a row, so why would you if it's just set to
                    // notInService?
...
write_targetSpinLock(int action,
                     ...
                     u_char * statP, oid * name, size_t name_len)
{
    // BRCM_MOD: (KO) Check type only in RESERVE1 phase, check length and value
    // in RESERVE2, then do the actual work in COMMIT.  This is because strictly
    // speaking, we must give wrongType errors priority over inconsistentValue
    // errors when we are processing multiple VB's in a single PDU.  Also, we
    // need to check for negative values and return wrongValue instead of
    // inconsistentValue for those.
    if (action == RESERVE1) {
        if (var_val_type != ASN_INTEGER) {
            return SNMP_ERR_WRONGTYPE;
        }
