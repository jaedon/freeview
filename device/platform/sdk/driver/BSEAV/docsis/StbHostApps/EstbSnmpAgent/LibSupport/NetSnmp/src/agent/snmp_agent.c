...
// Seems like 100 works OK too but 10 is the only value that has passed all the
// automated tests.  But if we ever get complaints about getbulks only returning
// 10 varbinds then we can bump this up.
#define MAX_BULK_VARBINDS 10 
...
unsigned long pdu_size_bytes (netsnmp_pdu *pdu)
{
  ...
  // PDU header size (see _clone_pdu_header in snmp_client.c)
  Size = sizeof(netsnmp_pdu);
  Size += pdu->enterprise_length;
  Size += pdu->community_len;
  Size += pdu->contextEngineIDLen;
  Size += pdu->securityEngineIDLen;
  Size += pdu->contextNameLen;
  Size += pdu->securityNameLen;
  Size += pdu->transport_data_length;

  // Variable list size (see _copy_varlist in snmp_client.c)
  pVB = pdu->variables;
  while ( pVB )
  {
      // Size of the netsnmp_variable_list structure
      Size += sizeof (netsnmp_variable_list);
  
      // Size of the value...
      Size += pVB->val_len;
      
      // If we are dynamically allocating OIDs to save RAM then we need to
      // add the OID size as well.  Older code with fixed length OIDs already
      // account for OID size in the size of the structure.
      #ifdef NETSNMP_FIXEDLEN_OID
      #if !NETSNMP_FIXEDLEN_OID
      Size += pVB->name_length * sizeof(oid);
      #endif
      #endif
                   ...
                   netsnmp_pdu *pdu, void *magic)
{
    ...
    if ((access_ret = check_access(pdu)) != 0) {
        if (access_ret == VACM_NOSUCHCONTEXT) {
            ...
            // BRCM_MOD do not send a trap from here, we handle authfail traps elsewhere.  This one calls
            // some mean netsnmp trap sending code.
                             ...
                             netsnmp_subtree *tp)
{
    ...
    // Various PRs (eg 11815, 11838, 11905, others) check for NULLs here - if 
    // we find any, print an error, check the heap, and return NULL.  The only
    // way we should ever get a NULL passed in is if we are low on heap.
    // Note that it is OK for tp to be NULL.  Also we now check for low heap
    // before this, so this shouldn't happen.  Still...
    if (asp == NULL)
      printf ("netsnmp_add_varbind_to_cache: asp = NULL\n");
    else if (asp->pdu == NULL)
      printf ("netsnmp_add_varbind_to_cache: asp->pdu = NULL\n");
    else if (varbind_ptr == NULL)
      printf ("netsnmp_add_varbind_to_cache: varbind_ptr = NULL\n");
    else
      NullArg = 0;
    ...
    if (tp == NULL) {
        ...
        {
          printf ("netsnmp_add_varbind_to_cache: asp->requests = NULL\n");
          return NULL;
        }
        ...
        // Misc PRs:  Check for NULL here too.
        if (request == NULL)
        {
          printf ("netsnmp_add_varbind_to_cache: request = NULL\n");
          return NULL;
        }
...
//        printf ("GETBULK: nonreps(errstat)=%d, maxreps(errindex)=%d, count=%d\n",
//          asp->pdu->errstat, asp->pdu->errindex, count);
        
        if (count)
        {
          // BRCM_MOD: Show some regard here for our session's max packet size.
          // We can't know exactly how big a packet will be based on how many
          // varbinds are requested until we actually encode it.  But we can
          // make a guess.  The smallest varbind we are likely to see is
          // going to be 6 octets (for any kind of 32 bit thing) for the value 
          // (TLV, where T and L fields are each 1 byte and the value is 4 bytes)
          // plus 10 octets for the OID (TLV: T, L 1 byte each and 8 bytes for
          // a small OID like sysDescr (.1.3.6.1.2.1.1.1.0).  So make an initial 
          // guess at how many varbinds we can reasonably expect to support and 
          // truncate if we have been asked for more than that.
          //
          // 12/29/2003: adjust for the number of varbinds if some bonehead
          // (or more likely an automated test suite) hits us with a GETNEXT
          // PDU with multiple varbinds in it.  Also, experimentally determined
          // a max number of VBs before we start taking more than 1 second to
          // process the request (leading to timeout issues), and restrict to
          // that value.
          // printf ("BULK: count=%d\n", count);
          if (asp->session->sndMsgMaxSize)
          {
            int vbGuess = asp->session->sndMsgMaxSize / 16;
            
            // Regardless of max PDU size, don't exceed a reasonable
            // number of varbinds lest we time out.  It's been experimentally
            // determined that we can comfortably process 75 varbinds in one
            // second (a typical timeout value).  So be a bit conservative and
            // draw the line at 64.
            // 1/5/2004: drop to 32, still having bulkwalk timeout issues @ 64.
            // 1/22/2004: drop to 16, timeouts remain @ 32
            // 1/25/2004: To hell with bulkwalks.  1.
            // 3/18/2004: Some certifying bodies get pissy if they see less than
            //            10 varbinds in a bulk request.  So satisfy them.  10.
            if (vbGuess > MAX_BULK_VARBINDS)
              vbGuess = MAX_BULK_VARBINDS;
            
            // If we have *MULTIPLE* getbulks (what kind of sicko...)
            // then divide by the number of VBs in the PDU.  Sigh...
            if (count)
              vbGuess /= count;
              
            // Just in case the math above works out to < 1...
            if (vbGuess <= 0)
              vbGuess = 1;
              
            // Handle the case of 10/count not working out to an 
            // even integer.  For example, 10/4 = 2.5, aka 2.  2*4 = 8.  So
            // boost by 1 in that case and send 12 rather than 8.  Better to
            // send too many than not enough.
            if ((vbGuess * count) < MAX_BULK_VARBINDS)
              vbGuess++;
              
            if (asp->pdu->errindex > vbGuess)
              asp->pdu->errindex = vbGuess;
          } 
          else
          {
            // Always hard-code to 1 VB per VB requested, regardless of
            // anything.  Net-SNMP is not very bulk-friendly and we keep 
            // getting all kinds of errors with automated SNMP protocol
            // tests.  This effectively makes a BULKGET the same as a NEXT and
            // bulkwalks the same as regular walks.
            // 3/18/2004: Some certifying bodies get pissy if they see less than
            //            10 varbinds in a bulk request.  So satisfy them.  10.
            if (asp->pdu->errindex > (MAX_BULK_VARBINDS/count))
            {
              asp->pdu->errindex = MAX_BULK_VARBINDS/count;
              if (asp->pdu->errindex < 1)
                asp->pdu->errindex = 1;
              ...
              // boost by 1 in that case and send 12 rather than 8.  Better to
              // send too many than not enough.
              if ((asp->pdu->errindex * count) < MAX_BULK_VARBINDS)
                asp->pdu->errindex++;
            }
          }
        }
        
//        printf ("After adjustment: nonreps(errstat)=%d, maxreps(errindex)=%d, count=%d\n",
//          asp->pdu->errstat, asp->pdu->errindex, count);
        // END BRCM_MOD
    ...
    for (varbind_ptr = asp->pdu->variables; varbind_ptr;
        ...
        case SNMP_MSG_SET:
            view = in_a_view(varbind_ptr->name, &varbind_ptr->name_length,
                             asp->pdu, varbind_ptr->type);
            // BRCM_MOD: This appears to be a bug in the Net-SNMP code.
            // If the object to be set is not in view, previously this
            // code would return NOT_WRITABLE but not actually set the
            // error status or error index.  Down the line, nobody paid
            // any attention to the return value here, and a SET request to an
            // object not in view would return noError, even though the actual
            // set would not have taken place.  So we must actually set
            // the error status and error index here.  Of secondary concern
            // is whether notWritable is the correct return value, WindManage
            // uses noAccess for this situation.  Since the BFC code uses
            // noAccess to hide factory objects, and since WindManage has
            // passed all the various SNMP test protocols, we'll change to
            // use noAccess. (PR 6168)
            if (view != VACM_SUCCESS)
            {
                // Assign an error status and index here if we have not already
                // done so.
                if (asp->pdu->errstat == SNMP_ERR_NOERROR)
                {
                  asp->pdu->errstat = SNMP_ERR_NOACCESS;
                  asp->pdu->errindex = vbcount;
                  
                  asp->status = SNMP_ERR_NOACCESS;
                  asp->index = vbcount;
                }
    ...
    // PR 11905: return the session status here, so we can return error codes
    // from any problem we might have encountered so far (like TOO_BIG if heap
    // is low)
    return asp->status;
}
...
netsnmp_reassign_requests(netsnmp_agent_session *asp)
{
    ...
    for (i = 0; i < asp->vbcount; i++) {
        //printf ("  2.%d: %p\n", i, asp->requests[i].requestvb);
        // PR 9731:  If we have a 'next' or 'bulk' request with multiple 
        // varbinds requested, and one or more of the varbinds starts with a 
        // value >= .2, we'll wind up with a NULL in this list and would
        // subsequently dereference that NULL and crash.  So check for NULL.
        // This doesn't address the core issue of why that NULL is in there,
        // but at least it keeps us from crashing.
...
void
xnetsnmp_check_outstanding_agent_requests(void)
{
