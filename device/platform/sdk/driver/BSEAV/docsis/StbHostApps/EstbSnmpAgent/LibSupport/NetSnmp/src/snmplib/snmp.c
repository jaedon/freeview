                           ...
                           u_char * var_val, size_t var_val_len)
{
    ...
    /*printf ("snmp_realloc_rbuild_var_op: ");*/
    
    start_offset = *offset;
    
    /*for (Loop = 0; Loop < *var_name_len; Loop++)
      printf (".%d", var_name[Loop]);*/
    ...
    rc = asn_realloc_rbuild_sequence(pkt, pkt_len, offset, allow_realloc,
                                     (u_char) (ASN_SEQUENCE |
                                               ASN_CONSTRUCTOR),
                                     *offset - start_offset);
    /*printf ("RC=%d, pkt_len=%u, offset=%u, start_offset=%u\n", rc, *pkt_len, *offset, start_offset);*/
    return rc;
}
