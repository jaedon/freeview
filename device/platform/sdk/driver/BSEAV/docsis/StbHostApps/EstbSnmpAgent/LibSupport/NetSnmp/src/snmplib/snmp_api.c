...
#ifdef VXWORKS
 char * __progname="bcm_netsnmp";  //added for vxworks
#endif
...
// External prototype from BcmHeapManager.h
unsigned long BcmHeapGetLargestAlloc(void);
#define LOW_HEAP_MARGIN 4096 // 4k
...
// PR 9824: make this configurable to save RAM
int RxBufLen = 65536;
...
// BRCM_MOD: made this non-static so we can call it externally
...
/* BRCM - added SNMP_MSG_TRAP below.  I hope that this was just an oversight... */
#define DEBUGPRINTPDUTYPE(token, type) \
    switch(type) { \
      ...
      case SNMP_MSG_TRAP2: \
        ...
        break; \
      case SNMP_MSG_TRAP: \
        DEBUGDUMPSECTION(token, "PDU-TRAP"); \
        break; \
 ...
 */
//brcm remove static here so we can call it from other places
void 
_init_snmp(void)
{
...
init_snmp(const char *type)
{
    ...
    DEBUGMSGTL(("init_snmp","1208 called init_snmp WARNING- THIS SHOULD NOT HAPPEN - done_init = <%d>\n",done_init));
    if (done_init) {
        return;
    }
                      ...
                      netsnmp_session * in_session)
{
    ...
    if ((session->flags & SNMP_FLAGS_DONT_PROBE) == SNMP_FLAGS_DONT_PROBE)
        return 1;
    DEBUGMSGTL(("snmpv3_engineID_probe","snmpv3_engineID_probe- here we go\n"));
    ...
    if (session->version == SNMP_VERSION_3) {
         ...
         */
        DEBUGMSGTL(("snmpv3_engineID_probe","1204 Installing the manager engine ID\n"));
        ...
        /*
        printf("Session pointers\n  slp->session<%p>\n  slp<%p>\n  in_session<%p>\n",slp->session,slp,in_session);
        printf("  slp->session->securityModel= <%d>\n",slp->session->securityModel);
        printf("  slp->session->securityLevel= <%d>\n",slp->session->securityLevel);
        DumpBuf(" slp->session->securityAuthProto", slp->session->securityAuthProto,slp->session->securityAuthProtoLen,'d');
        DumpBuf(" slp->session->securityPrivProto", slp->session->securityPrivProto,slp->session->securityPrivProtoLen,'d');
        DumpBuf(" slp->session->securityName", slp->session->securityName,slp->session->securityNameLen,'c');
        */
        
        slp->session->securityModel = USM_SEC_MODEL_NUMBER;
        ...
        if (create_user_from_session(slp->session) != SNMPERR_SUCCESS) {
            ...
            DEBUGMSGTL(("snmpv3_engineID_probe","FAILED to create new user from session\n"));
            return 0;
        }
    }
...
// BRCM_MOD PR 7336: Here's where we've received our probe response.  Process
// it just like it's done in snmpv3_engineID_probe.
int probeCallback(int operation, // status from snmpv3_engineID
                  netsnmp_session * session, // slp->session from snmpv3_engineID_probe_async
                  int reqid, // request PDU ID
                  netsnmp_pdu *pdu, // reply PDU
                  void *magic) // in_session from snmpv3_engineID_probe_async
{
    int i;
    netsnmp_session *in_session = magic;
    int status = operation;

    
    // Sanity: if session, pdu, or magic are NULL here, return 0.
    if ((session == NULL) || (pdu == NULL) || (magic == NULL))
      return 0;
    
    {
      // Be kind of lazy here.  We KNOW that this is a UDP domain system,
      // so take a shortcut, realizing that pdu->transport_data in this case
      // points to a sockaddr_in.
      struct sockaddr_in *pFrom = (struct sockaddr_in *)pdu->transport_data;
      
      // PR 7936: make sure pFrom is not NULL before doing this, lest we crash
      // when the callback is made for an aborted probe which has never yet
      // been sent.
      if (pFrom)
      {
          if (pFrom->sin_family == AF_INET6)
          {
             struct sockaddr *sa = (struct sockaddr*)pFrom;   
             ...
             if (getnameinfo(sa, sizeof(*sa), hbuf, sizeof(hbuf), sbuf,sizeof(sbuf), NI_NUMERICHOST | NI_NUMERICSERV)) 
                 ...
                 printf("Received probe response from %s/%s\n", hbuf, sbuf);
          
          }
          else
          {
              unsigned long ulFrom = htonl(pFrom->sin_addr.s_addr);
        
              // printf ("Received probe response from %d.%d.%d.%d.\n", 
              //         (ulFrom >> 24) & 0xFF,
              //         (ulFrom >> 16) & 0xFF,
              //         (ulFrom >> 8)  & 0xFF,
              //         (ulFrom >> 0)  & 0xFF);
...
*/
    /*
     * Handle engineID discovery.  This is generally done in 
     * _sess_process_packet but unfortunately it's not been called yet.
     * So we must do it here.  All it really entails is copying the 
     * engine ID from the PDU to our session.
     */
    if (!session->securityEngineIDLen && pdu->securityEngineIDLen) 
    {
      session->securityEngineID = (u_char *) malloc(pdu->securityEngineIDLen);
      ...
      memcpy(session->securityEngineID, pdu->securityEngineID, pdu->securityEngineIDLen);
      session->securityEngineIDLen = pdu->securityEngineIDLen;
      if (!session->contextEngineIDLen) 
      {
        session->contextEngineID = (u_char *) malloc(pdu->securityEngineIDLen);
        ...
        memcpy(session->contextEngineID, pdu->securityEngineID, pdu->securityEngineIDLen);
        session->contextEngineIDLen = pdu->securityEngineIDLen;
      }
    }
    ...
    {
        case STAT_SUCCESS:
            //printf ("STAT_SUCCESS: SNMPERR_INVALID_MSG\n");
            in_session->s_snmp_errno = SNMPERR_INVALID_MSG; /* XX?? */
            break;
        case STAT_ERROR:   /* this is what we expected -> Report == STAT_ERROR */
            //printf ("STAT_ERROR: SNMPERR_UNKNOWN_ENG_ID\n");
            in_session->s_snmp_errno = SNMPERR_UNKNOWN_ENG_ID;
            break;
        case STAT_TIMEOUT:
            //printf ("STAT_TIMEOUT: SNMPERR_TIMEOUT\n");
            in_session->s_snmp_errno = SNMPERR_TIMEOUT;
        default:
            printf ("unable to connect with remote engine: %s (%d)\n",
                    snmp_api_errstring(session->s_snmp_errno),
                    session->s_snmp_errno);
            break;
    }
    ...
    if (session->securityEngineIDLen == 0) 
    {
        //printf ("unable to determine remote engine ID\n");
        return 0;
    }

    in_session->s_snmp_errno = SNMPERR_SUCCESS;
/*        
    printf ("  probe found engineID:  ");
    for (i = 0; i < session->securityEngineIDLen; i++)
        printf ("%02x", session->securityEngineID[i]);
    printf ("\n");
*/
    /*
     * if boot/time supplied set it for this engineID 
     */
    //printf ("  Installing the manager engine ID\n");
...
/*
    printf("  session->securityModel= <%d>\n",session->securityModel);
    printf("  session->securityLevel= <%d>\n",session->securityLevel);
    DumpBuf(" session->securityAuthProto", session->securityAuthProto,session->securityAuthProtoLen,'d');
    DumpBuf(" session->securityPrivProto", session->securityPrivProto,session->securityPrivProtoLen,'d');
    DumpBuf(" session->securityName", session->securityName,session->securityNameLen,'c');
*/    
    ...
    {
        in_session->s_snmp_errno = SNMPERR_UNKNOWN_USER_NAME;       /* XX?? */
        printf ("  FAILED to create new user from session\n");
        return 0;
    }
    
    // Clear the flag that indicates we have a probe pending.
    session->flags &= ~SNMP_FLAGS_PROBE_PENDING;
    
    return 1;
}

// BRCM_MOD PR 7336: async version of this which doesn't block until we get
// a reply.  If the target doesn't answer, the blocking version will block
// for a LONG time, 45 seconds(!) if the default timeout and retry values are
// used.
int
snmpv3_engineID_probe_async(struct session_list *slp,
                      netsnmp_session * in_session)
{
    netsnmp_pdu    *pdu = NULL, *response = NULL;
    netsnmp_session *session;
    ...
    // If we get a NULL for SLP, then scan the list of sessions for a match
    // on in_session and grab slp from there.
    if (slp == NULL)
    {
      struct session_list *pSessList;
      
      snmp_res_lock(MT_LIBRARY_ID, MT_LIB_SESSION);
      pSessList = Sessions;
      while (pSessList)
      {
        if (pSessList->session == in_session)
        {
          slp = pSessList;
          break;
        }
    ...
    //printf ("snmpv3_engineID_probe_async for session %p, slp %p\n", in_session, slp);
    ...
    if ((session->flags & SNMP_FLAGS_DONT_PROBE)
    ||  (session->flags & SNMP_FLAGS_PROBE_PENDING))
        return 1;
    ...
    if (session->version == SNMP_VERSION_3) 
    {
        // If our 'in' session already has an engine ID, just copy it.
        if (in_session->securityEngineIDLen) 
        {
            ...
            memcpy (session->securityEngineID, in_session->securityEngineID, session->securityEngineIDLen);
        }
        ...
        else
        {
            if (snmpv3_build_probe_pdu(&pdu) != 0) 
            {
                printf ("Unable to create probe PDU for %d.%d.%d.%d\n",
                slp->transport->remote[0],slp->transport->remote[1],
                slp->transport->remote[2],slp->transport->remote[3]);
                session->flags |= SNMP_FLAGS_PROBE_FAILED;
                return 0;
            }
            ...
            // printf ("Probing for %d.%d.%d.%d, reqId = %08X slp = %p session = %p\n", 
            // slp->transport->remote[0],slp->transport->remote[1],
            // slp->transport->remote[2],slp->transport->remote[3],
            // pdu->reqid, slp, session);
            
            // Just do a snmp_sess_send here, don't wait for a reply.  Then
            // we're done unless the sess_send fails.
            if (snmp_sess_async_send(slp, pdu, probeCallback, in_session) == 0)
            {
...
// BRCM_MOD PR 7336: async version of snmp_add that doesn't hang on probes.
netsnmp_session *snmp_sess_add_async (netsnmp_session * in_session, netsnmp_transport *transport)
{
    struct session_list *slp;
    ...
    slp->internal->hook_pre = NULL;
    slp->internal->hook_parse = NULL;
    slp->internal->hook_post = NULL;
    slp->internal->hook_build = NULL;
    slp->internal->hook_realloc_build = NULL;
    slp->internal->check_packet = NULL;
    slp->internal->hook_create_pdu = NULL;
    ...
    if (slp->session->version == SNMP_VERSION_3) {
        ...
        //printf("Calling snmpv3_engineID_probe with slp<%p> and in_session<%p>\n",slp,in_session);
        if (!snmpv3_engineID_probe_async(slp, in_session)) {
...
create_user_from_session(netsnmp_session * session)
{
    ...
    //printf("CREATE USER FROM SESSION:\n");
    ...
    /* BRCM_MOD per DOCSIS SPEC only allow volatile, PR6567*/
    ...
    //printf("create_user_from_session -finish\n");
...
{
    struct usmUser *user;
    DEBUGMSGTL(("snmpv3_build_probe_pdu","Building the Probe pdu\n"));
    /*
     * create the pdu 
     */
    if (!pdu)
        return -1;
    // BRCM_MOD: SilverCreek test suite will not respond to a probe based
    // on a GET message, gotta use an INFORM message instead.  Strictly
    // speaking, the spec only requires that it be a 'request' so either
    // one should work, but sometimes you gotta code to the test...
    ...
    *pdu = snmp_pdu_create(SNMP_MSG_INFORM);
    ...
    //do we need to use  create_user_from_session???trc  
    //user->userStatus = RS_ACTIVE;
    //user->userStorageType = ST_VOLATILE;
             ...
             netsnmp_session * session, netsnmp_pdu *pdu)
{
    ...
    printf("session->securityLevel = <%d>\n",session->securityLevel);
    printf("session->securityModel = <%d>\n",session->securityModel);
    printf("pdu->securityModel = <%d>\n",pdu->securityModel);
    printf("session->securityName = <%s>\n",session->securityName);
    printf("pdu->securityName = <%s>\n",pdu->securityName);
    */
             ...
             u_char ** after_header, netsnmp_session * sess)
{
    ...
    int   myEngineIDLength;
    u_char myEngineID[USM_MAX_ID_LENGTH];
    ...
    //BRCM_MOD PR7509  Dont't continue here if this is a v1 or v2c packet.
    if (ver != 3)
    {
         //printf("snmpV3 parse  version = %d return BAD_VERSION\n",ver);
         //not sure what to return. bad_version seems appropriate
         return  SNMPERR_BAD_VERSION;
    }
    ...
    *length = pdu_buf_len;
    ...
    /* BRCM_MOD PR6790 */
    /* If the contextEngineId is zero and the securityEngineId is ours - this is an error */
    myEngineIDLength = snmpv3_get_engineID(myEngineID, USM_MAX_ID_LENGTH);

    if ( ( pdu->contextEngineIDLen == 0 ) 
           && ( pdu->securityEngineIDLen == myEngineIDLength) 
           && ( memcmp(pdu->securityEngineID, myEngineID, myEngineIDLength) == 0) ) {
        printf(" ContextEngineId == zero  AND securityEngineId is ours - UnknownPduHandler Error\n");
...
snmpv3_make_report(netsnmp_pdu *pdu, int error)
{
    ...
    switch (error) {
    case SNMPERR_UNKNOWN_PDU:                /* BRCM_MOD Add support for unknownPdu report */
        stat_ind = STAT_SNMPUNKNOWNPDUHANDLERS;
        err_var = unknownPduHandlers;
            ...
            netsnmp_pdu *pdu, u_char * data, size_t length)
{
    ...
    //BRCM_MOD PR 7581  SilverCreek 7.0.2.3 and 7.0.2.4.  Make sure the version is within
    // range for an int.
    if ((checkVer = snmp_parse_version(data, length)) < 0 ) 
    {
        printf("SNMP version is not a valid value <%d>\n",checkVer);
        snmp_increment_statistic(STAT_SNMPINASNPARSEERRS);
        session->s_snmp_errno = SNMPERR_BAD_VERSION;
        return -1;
    }
        ...
        */
        ...
        if (result) {
            if (!sessp) {
                 ...
                 */
                switch (result) {
                case SNMPERR_UNKNOWN_PDU:  /*BRCM_MOD added this one */
                case SNMPERR_USM_UNKNOWNENGINEID:
                case SNMPERR_USM_UNKNOWNSECURITYNAME:
                case SNMPERR_USM_UNSUPPORTEDSECURITYLEVEL:
                case SNMPERR_USM_AUTHENTICATIONFAILURE:
                 ...
                 netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
    ...
    /* BRCM_MOD: increase initial packet size from 2k to 16k.  Net-SNMP
       has real problems reallocating this if we get a packet bigger than
       the initial buffer size (crashes), so reduce the risk of that by
       starting out with a biggie.  We have plenty of RAM so this is OK. */
       
    if (PktSize > RxBufLen)
      PktSize = RxBufLen;

    if ((pktbuf = malloc(PktSize /*2048*/)) == NULL) {
        DEBUGMSGTL(("sess_async_send",
                    "couldn't malloc initial packet buffer\n"));
        session->s_snmp_errno = SNMPERR_MALLOC;
        return 0;
    } else {
        pktbuf_len = PktSize /*2048*/;
    }
...
// BRCM_MOD PR 7336: This function is (almost) exactly like _sess_async_send,
// except that it does not actually SEND the packet, only create it and
// queue it up in the expected responses list of the provided session.
int
snmp_async_queue(void *sessp, // netsnmp_session
                 netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
    // Get the session list from the supplied session pointer.  We are
    // skipping a couple of steps here compared to how snmp_async_send works,
    // just for the sake of expediency.
    struct session_list *slp = (struct session_list *) snmp_sess_pointer(sessp);
    netsnmp_session *session = sessp;
    struct snmp_internal_session *isp;
    netsnmp_transport *transport = NULL;
    u_char         *pktbuf = NULL, *packet = NULL;
    size_t          pktbuf_len = 0, offset = 0, length = 0;
    ...
    if (slp == NULL) {
        ...
        transport = slp->transport;
        if (!session || !isp || !transport) {
            DEBUGMSGTL(("sess_queue_pdu", "send fail: closing...\n"));
            return 0;
        }
    }
    ...
    if ((pktbuf = malloc(16384 /*2048*/)) == NULL) {
        DEBUGMSGTL(("sess_queue_pdu",
                    "couldn't malloc initial packet buffer\n"));
        session->s_snmp_errno = SNMPERR_MALLOC;
        return 0;
    } else {
        pktbuf_len = 16384 /*2048*/;
    }
    ...
    if (result < 0) {
        DEBUGMSGTL(("sess_queue_pdu", "encoding failure\n"));
        free(pktbuf);
        return 0;
    }
        ...
        if (transport->f_fmtaddr != NULL) {
            char           *dest_txt =
                transport->f_fmtaddr(transport, pdu->transport_data,
                                     pdu->transport_data_length);
            if (dest_txt != NULL) {
                snmp_log(LOG_DEBUG, "\nQueuing %u bytes to %s\n", length,
                         dest_txt);
                free(dest_txt);
            } else {
                snmp_log(LOG_DEBUG, "\nQueueing %u bytes to <UNKNOWN>\n",
                         length);
            }
        }
        xdump(packet, length, "");
...
/* Here's what we DON'T do...
    ...
    result = transport->f_send(transport, packet, length,
                               ...
                               &(pdu->transport_data_length));
*/
    result = 0; // Fake out the send, above.
    free(pktbuf);
    ...
    if (pdu->flags & UCD_MSG_FLAG_EXPECT_RESPONSE) {
        ...
        // Set the time and expiration to '0' so once the probe
        // is complete we won't wait around to send the actual inform.
        ...
        // Since we're queing this, set retries to -1 because it'll be sent
        // the first time in the timout routine.
        rp->retries = -1;
        if (pdu->flags & UCD_MSG_FLAG_PDU_TIMEOUT) {
            rp->timeout = pdu->time * 1000000L;
        } else {
                     ...
                     u_char * packetptr, int length)
{
  ...
  // printf (" _sess_process_packet slp at %p netsnmp_session at %p internal_session at %p transport at %p\ocket %d",
  //             slp, sp, isp, transport, transport->sock);
  //BRCM_MOD PR7296 SilverCreek 3.5.1.4 and 3.5.1.5.  Check that the length of 
  // the SNMP packet is the same as what is encoded in the packet.
  ret = asn_check_packet(packetptr,length);
  if (ret!=length)
  {
      printf(" ERROR - ASN.1 BER of packet does not equal length of packet");
      printf("   length = <%d>, encoded len = <%d> - return -1\n",length, ret);
      snmp_increment_statistic(STAT_SNMPINASNPARSEERRS);
      //PR15223 - fix mem leak
      if (opaque != NULL)
	     free(opaque);
  ...
  // BRCM_MOD PR 7388: Prevent memory leaks by identifying notification
  // sessions, and if this is not a PDU that they would handle (report or
  // response only), drop it before continuing.
  if (sp->flags & SNMP_FLAGS_NOTIFY_SESSION)
  {
    //printf ("  Notify session!\n");
    if ((pdu->command != SNMP_MSG_REPORT)
    &&  (pdu->command != SNMP_MSG_RESPONSE))
    {
      //printf ("  Dropping - not REPORT or RESPONSE!\n");
      // What error should I use here?  Doesn't really matter.
      ret = SNMPERR_INVALID_MSG;
    }
  }
  ...
  if (pdu->flags & UCD_MSG_FLAG_RESPONSE_PDU) {
     ...
     * Call USM to free any securityStateRef supplied with the message.  
     */
    // printf (" got a Response pdu - msg id %d \n", pdu->msgid);
    if (pdu->securityStateRef) {
      sptr = find_sec_mod(pdu->securityModel);
      if (sptr) {
	if (sptr->pdu_free_state_ref) {
	 ...
	 */
        // printf (" checking Response pdu msg id %d against net-snmp request list id %d for internal session at %p\n",
        //              pdu->msgid, rp->message_id, rp);
...
snmp_read(fd_set * fdset)
{
    ...
    for (slp = Sessions; slp; slp = slp->next) {
        ...
        // printf("snmp_read: we are calling snmp_sess_read with slp %p\n", slp);
        
        ret = snmp_sess_read((void *) slp, fdset);
        if (ret == -1)
        {
            //BRCM_MOD PR7467 and PR 7533: Given a TLV38 (with type 5) in the config file, some counters 
            //do not count properly.  The Inform session was incrementing counters before bailing.
            //We end up with double counts.
            //printf("snmp_sess_read ret -1, errno = <%d>\n",slp->session->s_snmp_errno);
            if( (slp->session->s_snmp_errno == SNMPERR_ASN_PARSE_ERR  ) ||
                (slp->session->s_snmp_errno == SNMPERR_USM_PARSEERROR ) ||
                (slp->session->s_snmp_errno == SNMPERR_BAD_PARSE ))
            {
                //if (slp->next != NULL) 
                if (slp->session->flags & SNMP_FLAGS_NOTIFY_SESSION)
                {
                    DEBUGMSGTL(("snmp_read: parse error<%d> - bump inPkts and don't check any more sessions\n",slp->session->s_snmp_errno));
                    snmp_increment_statistic(STAT_SNMPINPKTS);
                }
                else
                    DEBUGMSGTL(("snmp_read: parse error<%d> - don't check any more sessions\n",slp->session->s_snmp_errno));
...
_sess_read(void *sessp, fd_set * fdset)
{
    ...
    size_t          pdulen = 0, rxbuf_len = RxBufLen; //65536; PR 9824: configurable to save RAM
    u_char         *rxbuf = NULL;
