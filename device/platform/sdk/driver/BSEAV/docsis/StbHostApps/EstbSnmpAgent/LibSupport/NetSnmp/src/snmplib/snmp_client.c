                         ...
                         netsnmp_pdu *pdu, netsnmp_pdu **response)
{
    ...
    while (state->waiting) {
        ...
        DEBUGMSGTL(("snmp_sess_synch_response","timeout.tv_sec = <%d> timeout.tv_usec = <%d> block = <%d> numfds = <%d>\n",timeout.tv_sec, timeout.tv_usec, block, numfds));
        if (block == 1)
            tvp = NULL;         /* block without timeout */
        count = select(numfds, &fdset, 0, 0, tvp);
        DEBUGMSGTL(("snmp_sess_synch_response","snmp_client - Inform SELECT returned with a <%d>\n",count));
