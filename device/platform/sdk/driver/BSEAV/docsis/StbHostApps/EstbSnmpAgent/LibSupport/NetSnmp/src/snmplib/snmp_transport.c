...
#endif
#ifdef SNMP_TRANSPORT_UDPIPV6_DOMAIN
    /* BRCM_MOD: don't call netsnmp_udp6_ctor - we will call our own transport */
    /* netsnmp_udp6_ctor(); */
#endif
#ifdef SNMP_TRANSPORT_TCPIPV6_DOMAIN
    netsnmp_tcp6_ctor();
...
/* Returns a pointer to the domain which matches in_oid, NULL if no match.
   The main reason for this is to use in netsnmp_tdomain_unregister. */
netsnmp_tdomain *
netsnmp_tdomain_find(const oid * in_oid,
                        size_t in_len)
{
    netsnmp_tdomain *d = NULL;
