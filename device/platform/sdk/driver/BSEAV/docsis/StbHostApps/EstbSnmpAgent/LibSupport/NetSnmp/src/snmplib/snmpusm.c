...
/*BRCM mod - Remove 'static' from some globals so that we can use them in the glue files*/
struct usmUser *userList = NULL;
...
{                               /* IN     - v3 Message flags.              */
    ...
    /* BRCM_MOD add some vars for checking the active status of user, group, access */
    ...
    /* BRCM_MOD, PR11567  SilverCreek 4.6.10 sends engineId of 0 with AuthPriv flags.
       Expects UnknownEngineId response.
    */
    if (secLevel == SNMP_SEC_LEVEL_AUTHPRIV) 
    {
        if ( (*secEngineIDLen < 5) || (*secEngineIDLen > SNMP_MAX_ENG_SIZE) )
        {
            DEBUGMSGTL(("usm", "EngineIdLen was < 5 or > 32 - ret unknownEngineId\n"));
            //printf("xxxx EngineIdLen was < 5 or > 32 - ret unknownEngineId\n");
    ...
    //BRCM_MOD, PR7353 Need to handle case where engineId is zero but we have
    // a userName. PR7359: check upper limit size of engine Id. SilverCreek 4.6.6
    if ( (*secEngineIDLen < 5) || (*secEngineIDLen > SNMP_MAX_ENG_SIZE) )
    {
        DEBUGMSGTL(("usm", "EngineIdLen was < 5 or > 32 - ret unknownEngineId\n"));
        snmp_increment_statistic(STAT_USMSTATSUNKNOWNENGINEIDS);
    ...
    /* BRCM_MOD  PR6804: check that the user is active, moved group, and access checks to vacm_in_view */
    /* we may need to move this to vacm_in_view is we need a noaccess error code ret */
    
    /* Check that the user status is active */
    if (user->userStatus != SNMP_ROW_ACTIVE)
    {
        //printf("usm_process_in_msg: USER IS NOT ACTIVE \n");
        DEBUGMSG(("usm_process_in_msg","USER IS NOT ACTIVE \n"));
        return SNMPERR_USM_UNKNOWNSECURITYNAME;  //this is probably wrong return
    }
    /* END of BRCM_MOD */
    ...
    if (secLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
        ...
        if ( ((value_ptr = asn_parse_sequence(data_ptr, &remaining,
            ...
            /* BRCM_MOD PR6792  Changed above to also check if (remaining == 0) */
            /*  This indicates msgData field is zero length and is an error */
            //printf("asn_parse_Sequence failed\n");
            DEBUGMSGTL(("usm", "%s\n",
