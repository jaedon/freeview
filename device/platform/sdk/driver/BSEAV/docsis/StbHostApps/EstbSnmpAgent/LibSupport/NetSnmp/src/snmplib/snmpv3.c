...
setup_engineID(u_char ** eidp, const char *text)
{
    ...
    case ENGINEID_TYPE_MACADDR:
        {
            printf("Not supporting enginedID macAddr type use default\n");
         #if NOT_FOR_ECOS
            int             x;
...
init_snmpv3(const char *type)
{
     ...
     */
    // BRCM_MOD don't register these config callbacks
...
/*BRCM_MOD We are adding this function.  Need a way to set engineBoots. */
void snmpv3_set_local_snmpEngineBoots(int boots)
{
    DEBUGMSGTL(("snmpv3", "snmpv3_set_local_snmpEngineBoots: SETTING engineBoots to <%d>\n", boots));
    engineBoots = boots;
}
 ...
 */
#if (0)
// PR7842 BRCM_MOD: implement this function in the glue file.
u_long
