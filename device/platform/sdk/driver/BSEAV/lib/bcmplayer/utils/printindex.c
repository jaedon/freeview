 ...
 * Revision History:
 *
 * $brcm_Log: /BSEAV/lib/bcmplayer/utils/printindex.c $
 * 
 * 17   2/27/12 4:13p erickson
 * SW7425-2495: move BNAV_Entry encode/decode to public API for use with
 * BNAV_Player_ReadIndex
 * 
 * 16   7/1/11 10:14a vsilyaev
 * SW7422-14: Resolved warnings
 * 
 * 15   6/28/11 4:29p vsilyaev
 * SW7422-14: Changed index format to use access unit as a frame boundary
 * 
 * 14   6/16/11 3:28p erickson
 * SWDEPRECATED-2425: fix warning
 * 
 * 13   2/16/11 5:38p vsilyaev
 * SW7422-14: Print real data offset
 * 
 * 12   1/21/11 6:45p vsilyaev
 * SW7422-14: Fixed Win32 compile
 * 
 * 11   1/21/11 6:43p vsilyaev
 * SW7422-14: Fixed warnings
 * 
 * 10   1/21/11 6:38p vsilyaev
 * SW7422-14: Added support for printing SVC index
 * 
 * 9   1/5/11 11:51a vsilyaev
 * SW7422-14: Updated to support multiple pids and extended payload
 * entries
 * 
 * 8   6/21/10 4:31p erickson
 * SW7405-4249: added BNAV_Version_TimestampOnly option for indexing
 * audio-only or scrambled streams
 *
 * 7   4/16/08 10:34a erickson
 * PR40307: fix compilation
 *
 * 6   2/26/08 12:28p katrep
 * PR38429: Added AVS record and playback support
 *
 * 5   2/19/08 6:12p katrep
 * PR38979: Fixed -sct6 option, there are tpit entries now in the SCTs
 *
 * 4   2/5/08 3:57p gmohile
 * PR 38979 : Add Random Access indicator in index file
 *
 * 3   2/8/07 5:21p erickson
 * PR22543: make pts default
 *
 * 2   11/22/06 12:30p erickson
 * PR25230: fix warnings
 *
 * Irvine_BSEAVSW_Devel/22   6/26/06 12:02p erickson
 * PR21941: fixed warnings
 *
 * Irvine_BSEAVSW_Devel/21   3/6/06 1:21p erickson
 * PR19853: added VC1 PES support
 *
 * Irvine_BSEAVSW_Devel/20   1/6/06 10:16a erickson
 * PR17108: updated for magnum basemodules and 64 bit cpus
 *
 * Irvine_BSEAVSW_Devel/19   8/3/05 12:06p erickson
 * PR16138: fix compiler warnings, make "-s" completely silent on all non-
 * frame printouts
 *
 * Irvine_BSEAVSW_Devel/18   7/15/05 8:08p erickson
 * PR16138: added runtime check for AVC format
 *
 * Irvine_BSEAVSW_Devel/17   7/13/05 6:47p erickson
 * PR16138: converted 0&1 fields to seqhdr offset & size
 *
 * Irvine_BSEAVSW_Devel/16   7/13/05 2:12p erickson
 * PR16138: added AVC NAV version support and refactored
 *
 ***********************************************/
#include "bstd.h"
#include <stdio.h>
