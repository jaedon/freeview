...
*  ANY LIMITED REMEDY.
* 
* $brcm_Workfile: profile_extra.c $
* $brcm_Revision: 2 $
* $brcm_Date: 12/21/11 2:01p $
*
* Revision History:
*
* $brcm_Log: /BSEAV/lib/bprofile/build/profile_extra.c $
* 
* 2   12/21/11 2:01p vsilyaev
...
void b_perf_write_cfg(unsigned select, unsigned data)
{
    ...
    rc = open("/dev/brcm0", O_RDWR);
    if(rc<0) {(void)BERR_TRACE(BERR_OS_ERROR);goto err_open;}
    driver_fd = rc;
    arg.select = select;
    arg.value = data;
