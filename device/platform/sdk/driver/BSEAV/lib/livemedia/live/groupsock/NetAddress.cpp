...
NetAddressList::NetAddressList(char const* hostname)
  : fNumAddresses(0), fAddressArray(NULL) {
	...
	struct addrinfo *addrInfoTemp = NULL;
#endif
    ...
    // First, count the number of addresses:
    for( addrInfoTemp = addrInfo;addrInfoTemp !=NULL; addrInfoTemp->ai_next)
    {
    	++fNumAddresses;
    }
	// Next, set up the list:
    ...
    for(addrInfoTemp = addrInfo;addrInfoTemp !=NULL; addrInfoTemp->ai_next)
    {
	fAddressArray[i] = new NetAddress((u_int8_t const*)((struct sockaddr_in *)(addrInfoTemp->ai_addr))->sin_addr.s_addr, addrInfoTemp->ai_addrlen);
	  i++;     
    }
#endif
}
