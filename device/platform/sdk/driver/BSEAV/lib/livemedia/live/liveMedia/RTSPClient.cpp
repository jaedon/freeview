...
// File is modified by Broadcom Corporation
...
char* RTSPClient::sendOptionsCmd(char const* url,
  ...
  do {
	...
	delete[] fRealChallengeStr; fRealChallengeStr = strDup(&lineStart[16]);
      ...
      // JJ/BRCM 20061026 - save Server type for users of RTSPClient
      else if (_strncasecmp(lineStart, "Server: ", 8) == 0) {
...
Boolean RTSPClient::playMediaSession(MediaSession& session,
  ...
  do {
    ...
    unsigned cmdSize = strlen(cmdFmt)
      ...
      + strlen(authenticatorStr)
      + fUserAgentHeaderStrSize;
    if (fAdditionHeadersStr) 
      cmdSize += strlen(fAdditionHeadersStr);
    ...
    sprintf(cmd, cmdFmt,
	    ...
	    ++fCSeq,
	    fLastSessionId,
        fAdditionHeadersStr ? fAdditionHeadersStr : "",
	    scaleStr,
	    rangeStr,
	    authenticatorStr,
	    fUserAgentHeaderStr);
