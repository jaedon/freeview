...
static DRM_RESULT _MatchXMLCertRobustnessPlatformAndModel(
    ...
    __in        DRM_CONST_STRING *f_pdstrXMLCert,
    ...
    )
{
    ...
    */
    ...
    DRM_B64_DecodeW( (DRM_CONST_STRING*)&dstrSigningKey, &cbDestination, oDeviceCertSigningKey.y, 0 );
    ...
    */
    ...
    cbDestination = DRM_ECC160_PUBLIC_KEY_LEN;
    DRM_B64_DecodeW( (DRM_CONST_STRING*)&dstrModelPubKey, &cbDestination, oModelCertPubkey.y, 0 );
...
** Returns:     DRM_SUCCESS             - on success, the identifiers match
**              DRM_E_INVALIDARG        - if the machine ID parameter is invalid
**              DRM_E_MACHINEIDMISMATCH - if there is a mismatch
...
static DRM_RESULT _CompareMachineId(
    ...
    __in     DRM_CONST_STRING  *pdstrMachineId
)
{
    DRM_RESULT       dr                                      = DRM_SUCCESS;
    ...
    __in                                    DRM_DWORD         f_cbBinaryDeviceCert )
{
    ...
    */
    ...
    ChkBOOL( cbUniqueID == DRM_ID_SIZE, DRM_E_MACHINEIDMISMATCH );

    ChkBOOL( MEMCMP( f_pBlackBoxContext->cachedBCertValues.DeviceSerialNumber.rgb,
                     rgbUniqueId,
                     DRM_ID_SIZE ) == 0,
             DRM_E_MACHINEIDMISMATCH );
#endif /* !DRM_TEST_IGNORE_UNIQUE_ID */
ErrorExit:
    return dr;
}
    ...
    __in DRM_VOID        *f_pOEMContext )
{
    ...
    */
    ...
    {
        /*
        ** Load the key file. If the keyfile does not exist this call will fail.
        */
        dr = DRM_KF_Open(
            &poAppContextInternal->oKeyFileContext,
            FALSE,
        ...
        if( DRM_SUCCEEDED( dr ) )
        {
            ...
            dr = DRM_KF_LoadFromFile( &poAppContextInternal->oKeyFileContext, TRUE );
            if( DRM_SUCCEEDED( dr ) )
            {
                cbBinaryDeviceCert = poAppContextInternal->cbDRMHeaderData - ( poAppContextInternal->cbDRMHeaderData % SIZEOF( DRM_DWORD_PTR ) );
                ...
                if( DRM_SUCCEEDED( dr ) )
                {
                    ...
                    if( DRM_SUCCEEDED( dr ) )
                    {
                        /* Verify APP id, app version it's the right keyfile for this title */
                        if( poAppContextInternal->oKeyFileContext.keyfile.DeviceAppData.fValid )
                        {
                            dr = DRM_ACT_VerifyAppData(
                                    poAppContextInternal->oKeyFileContext.keyfile.DeviceAppData.xbbaAppId.pbDataBuffer +
                                    poAppContextInternal->oKeyFileContext.keyfile.DeviceAppData.xbbaAppId.iData,
                                    poAppContextInternal->oKeyFileContext.keyfile.DeviceAppData.xbbaAppId.cbData,
                                    poAppContextInternal->oKeyFileContext.keyfile.DeviceAppData.dwAppVersion );
                        }
                        else
                        {
                            dr = DRM_E_ACTIVATION_REQUIRED;
                        }
                    }
                }
            }
    ...
    else
    {
        ...
        */
        ChkDR( DRM_KF_Open( &poAppContextInternal->oKeyFileContext,
                            TRUE,
                            OEM_GENERIC_READ | OEM_GENERIC_WRITE ) );
        fKeyfileOpened = TRUE;
        ...
        */
        ChkDR( DRM_KF_LoadFromFile( &poAppContextInternal->oKeyFileContext, TRUE ) );
        ...
        if( DRM_WMDRM_IsWMDRMSupported() )
        {
            ...
            */
            ...
            if( DRM_SUCCEEDED( dr ) )
            {
                ...
                ChkDR( DRM_DCP_StopCertParserCache( &poAppContextInternal->oKeyFileContext,
                                                    &oParserCache ) );
        ...
        */
        ...
        if( fClearKeyHistory )
        {
            ...
            poAppContextInternal->oKeyFileContext.keyfile.KeyHistoryV2.fValid = FALSE;
            poAppContextInternal->oKeyFileContext.keyfile.KeyHistoryV2.pKeyPairHeadv2 = NULL;
        }
        ...
        {
            ChkDR( DRM_DDC_CreateSecureStoreKeyAndSaveKeyFile( f_pOEMContext, &poAppContextInternal->oKeyFileContext ) );
        }
    }
    ...
    if( DRM_WMDRM_IsWMDRMSupported() )
    {
        ...
        */
         ...
         || ( poAppContextInternal->oBlackBoxContext.cachedBCertValues.dwFeatureFlags & BCERT_FEATURE_BIT(DRM_BCERT_FEATURE_SECURE_CLOCK) ) )
        {
            ...
            */
            DRMASSERT( DRM_CLK_IsSecureClockSupported() );
            DRMASSERT( poAppContextInternal->oBlackBoxContext.cachedCertValues.dwFeatureFlags & DRM_FEATURE_SECURE_CLOCK );
            DRMASSERT( poAppContextInternal->oBlackBoxContext.cachedBCertValues.dwFeatureFlags & BCERT_FEATURE_BIT(DRM_BCERT_FEATURE_SECURE_CLOCK) );
        }
         ...
         || ( poAppContextInternal->oBlackBoxContext.cachedBCertValues.dwFeatureFlags & BCERT_FEATURE_BIT(DRM_BCERT_FEATURE_ANTIROLLBACK_CLOCK) ) )
        {
            ...
            */
            DRMASSERT( poAppContextInternal->oBlackBoxContext.cachedCertValues.dwFeatureFlags & DRM_FEATURE_ANTI_ROLLBACK_CLOCK );

            DRMASSERT( ( poAppContextInternal->oBlackBoxContext.cachedBCertValues.dwFeatureFlags & BCERT_FEATURE_BIT(DRM_BCERT_FEATURE_ANTIROLLBACK_CLOCK) )
    ...
    fKeyfileOpened = FALSE;
    ChkDR( DRM_KF_Close( &poAppContextInternal->oKeyFileContext ) );
...
ErrorExit:
    ...
    */
    if ( fKeyfileOpened )
    {
        DRM_KF_Close( &poAppContextInternal->oKeyFileContext );
    }
...
)
{
    ...
    */
    ...
    ChkDR( DRM_BCert_InitVerificationContext( &ftUnlimited,
                                              ...
                                              DRM_BCERT_CERTTYPE_DEVICE,
                                              f_pCryptoCtx,
                                              (f_dwFlags & DRM_DCP_VERIFY_DEVCERT),
                                              FALSE,
                                              rgdwKeyUsages,
                                              NO_OF(rgdwKeyUsages),
                                              FALSE,
