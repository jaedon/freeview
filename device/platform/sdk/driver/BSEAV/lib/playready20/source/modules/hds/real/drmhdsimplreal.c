...
#endif
    if (!Oem_File_Unlock(pHDS->fp, 0, 0xFFFFFFFF))
    {
        ChkDR(DRM_E_DSTLOCKFAILED);
    }
...
static DRM_NO_INLINE DRM_RESULT _HdsOpenSlot(
    ...
    IN       DRM_BOOL           f_fSRNNotAlreadyExclusivelyLocked,
    IN OUT   DRM_DWORD         *f_pcbSlotSize,
       OUT   _SlotContext      *f_pSlotCtx )
{
    DRM_RESULT dr      = DRM_SUCCESS;
    ...
    __in       DRM_DWORD                  dwSlotLockMode )
{
    ...
    while ( TRUE )
    {
        ...
        ** otherwise if we're only opening an existing slot we can lock SRN SHARED. */
        ...
        dr = _HdsOpenSlotFromHint(pNS, &hashKey, &uniqueKey, (_SlotHint*)&slotHint, TRUE, &cbSlotSize, pSlotCtx);
    ...
    __out                              DRM_DST_NAMESPACE_CONTEXT *f_pContextNS )
{
    ...
    /* Lock SRN EXCLUSIVE */
    LOCK_SRN_AND_ENTER_CS(pHDS, (f_eOpenMode | eDRM_DST_LOCKEXCLUSIVE));
    ...
    if( (pHDS->oSRN.nNsStoreRootBlockNum == 0) )
    {
        ...
        ** DRM_E_DSTNAMESPACEEXIST.  This is obviously not what we want.
    ...
    __in       DRM_BOOL                   f_fWait )
{
    ...
    while ( TRUE )
    {
        ...
        LOCK_SRN_AND_ENTER_CS( pNS->pHDS, eDRM_DST_LOCKEXCLUSIVE | (f_fWait ? eDRM_DST_LOCKWAIT : 0) );
        ...
        ** otherwise if we're only opening an existing slot we can lock SRN SHARED. */
        ...
        if( DRM_FAILED( dr ) || f_pSlotHint == NULL )
        {
            ChkDR( _HdsOpenSlot(pNS,
                                ...
                                TRUE,
                                f_dwMode,
                                (f_dwMode & eDRM_DST_OPEN_EXISTING),
                                f_pcbSlotSize,
                                pSlotCtx ) );
        }
    ...
    __in     DRM_DWORD              f_cbSize )
{
    ...
    */
    ChkDR( _HdsReopenSlotWithSRNLock( pSlotCtx, eDRM_DST_LOCKEXCLUSIVE | eDRM_DST_LOCKWAIT, pSlotCtx->eLockMode & eDRM_DST_LOCKMODE_MASK ) );
    ...
    __out          DRM_DST_ENUM_CONTEXT      *f_pContextEnum )
{
    ...
    /* Lock SRN SHARED among other processes. */
    LOCK_SRN_AND_ENTER_CS(pNS->pHDS, (f_eLockMode & (~eDRM_DST_LOCKEXCLUSIVE)));
    ...
    __inout   DRM_DST_SLOT_CONTEXT  *f_pContextSlot )
{
    ...
    */
    ChkDR( _HdsReopenSlotWithSRNLock( pSlotCtx, pEnum->eLockMode | eDRM_DST_LOCKEXCLUSIVE, pEnum->eLockMode ) );
#if DRM_SUPPORT_MULTI_THREADING
    fSRNLocked = TRUE;
#endif /* DRM_SUPPORT_MULTI_THREADING */
    ...
    __out_opt  DRM_DWORD             *f_pcbSlot )    /* current dataslot size, optional parameter */
{
    ...
    {
        /* Lock SRN SHARED among other processes. */
        LOCK_SRN_AND_ENTER_CS(pEnum->pNS->pHDS, (pEnum->eLockMode & (~eDRM_DST_LOCKEXCLUSIVE)));
