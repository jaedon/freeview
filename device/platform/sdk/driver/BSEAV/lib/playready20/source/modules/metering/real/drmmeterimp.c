    ...
    __out                              PUBKEY_P256                  *f_poServerPubKey )
{
    ...
    */
    ...
    {
        if( dr == DRM_E_DSTSLOTNOTFOUND ) /* Slot not found means there's just no CRL available which is fine */
        {
            dr = DRM_SUCCESS;
        }
    }
    ...
    __out                       DRM_DWORD *f_pcchCount )
{
    ...
    for ( ; ; )
    {
        ...
        ChkBOOL( oToken.TokenType == TOKEN_LONG, DRM_E_WRONGTOKENTYPE );

        DRM_BYT_CopyBytes( &oToken.val.lValue, 0, f_pbData, ibCurr + SIZEOF( DRM_DWORD ), SIZEOF( DRM_LONG ) );

        FIX_ENDIAN_DWORD( oToken.val.lValue );

        ibCurr += SIZEOF( PERSISTEDTOKEN );

        ChkDR( DRM_UTL_NumberToString( oToken.val.lValue,
                                       rgwchScratch,
    ...
    __in                  const DRM_DWORD     f_cbData )
{
    ...
    for ( ; ; )
    {
        ...
        */
        ...
        DRM_UTL_DemoteUNICODEtoASCII( ( DRM_WCHAR * )pbAction,
                                      ( DRM_CHAR * )pbAction,
...
__in_bcount( SIZEOF( _AES_KEY ) )                       DRM_BYTE      *f_pbAESKey,
...
)
{
    ...
    _AES_KEY      *poXMLAESKey      = (_AES_KEY *)f_pbAESKey;
...
**                          meter certificate.
**                          DRM_E_METERCERTNOTFOUND - if meter certificate
    ...
    __inout                                 DRM_DWORD                   *f_pcbScratch )
{
    ...
    */
    dr = DRM_XML_GetSubNodeByPathA( f_pdastrResponse->pszString,
                                    ...
                                    &dasstrMeterCert,
                                    g_chForwardSlash );
    ChkDRMap( dr, DRM_E_XMLNOTFOUND, DRM_E_METERCERTNOTFOUND );
    ...
    __out_opt                               DRM_RESULT                  *f_pResult )
{
    ...
    ChkDRMap( dr, DRM_E_METERCERTNOTFOUND, DRM_SUCCESS );
