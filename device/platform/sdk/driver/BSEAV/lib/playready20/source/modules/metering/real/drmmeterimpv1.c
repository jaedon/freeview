                                        ...
                                        DRM_DWORD *f_pcchRequired)
{
    ...
    while ((ibCurr + CB_OPERATION_MINIMUM - 1) < f_cbBuffer)
    {
        ...
        ChkBOOL( token.TokenType == TOKEN_LONG, DRM_E_WRONGTOKENTYPE );
        ...
        ChkDR( DRM_UTL_NumberToString (token.val.lValue, awchNum, NO_OF (awchNum), 0, 10, &dstrCount.cchString ) );
          ...
          IN DRM_DST             *f_pDatastore )
{
    ...
    ChkDRMap( DRM_SST_GetData (f_pmeterstore,
                          ...
                          &g_kidTID,
                           ...
                           f_pDatastore,
                           rgbSlotMinimum,
                          &cbSlot), DRM_E_DSTSLOTNOTFOUND, DRM_E_METERSTORE_DATA_NOT_FOUND );
...
static DRM_RESULT _ProcessOneKid(
    ...
    IN OUT _XMBContext      *f_pbAlignedBuffer)
{
    DRM_RESULT dr     = DRM_SUCCESS;
    DRM_DWORD  ibCurr = 0;
    ...
    while ( ((ibCurr + CB_OPERATION_MINIMUM - 1) < f_cbDataKID)  &&  DRM_SUCCEEDED(dr) )
    {
        ...
        ChkDR( DRM_UTL_NumberToString (token.val.lValue, awchNum, NO_OF (awchNum), 0, 10, &f_pdstrData->cchString ) );
...
static DRM_RESULT _ProcessAllKIDs(
    ...
    IN       DRM_STACK_ALLOCATOR_CONTEXT *f_pcontextStack,
       OUT   _XMBContext                 *f_pbAlignedBuffer,
...
static DRM_RESULT _BuildChallenge(
    IN OUT          _XMBContext                 *f_pbAlignedBuffer,
    __in_opt  const DRM_DWORD                   *f_pcbChallenge,
