...
DRMFORCEINLINE DRM_BOOL _IsEmptyDstr(
    __in_ecount( 1 )  const DRM_CONST_STRING *f_dstrIsEmpty )
{
    if (    NULL == f_dstrIsEmpty
         || NULL == PB_DSTR( f_dstrIsEmpty )
         || 0    == CB_DSTR( f_dstrIsEmpty ) )
    {
        return TRUE;
    }
    return FALSE;
    ...
    __out_ecount( 1 )               eDRM_HEADER_VERSION *f_peVersion )
{
    ...
    */
    if ( RECORDTYPE_WRMHEADER == poHeader->m_wRecordType )
    {
        ...
        */
        strXmlData.pwszString = ( DRM_WCHAR* )( f_pbHeader + SIZEOF( V4Header ) );
    ...
    else if ( V2_BOM == ( ( DRM_WCHAR* )f_pbHeader )[ 0 ] )
    {
        ...
        strXmlData.pwszString = ( DRM_WCHAR* )( f_pbHeader + SIZEOF_V2_BOM );
        strXmlData.cchString  = ( f_cbHeader - SIZEOF_V2_BOM ) / SIZEOF( DRM_WCHAR );
    }
    else
    {
        strXmlData.pwszString = ( DRM_WCHAR* )f_pbHeader;
    ...
    __in                      const DRM_DWORD f_cbBuffer )
{
    ...
    const DRM_BYTE  *pbCurrent       = (DRM_BYTE*) f_pbBuffer;
...
DRM_API DRM_NO_INLINE DRM_RESULT DRM_CALL DRM_PRO_GetRecord(
    ...
    __deref_out_bcount(*f_pcbRecord)       DRM_BYTE  **f_ppbRecord,
    ...
    */
    ...
    if ( fAddCustomAttributesNode )
    {
        ...
        if ( cchCustAttrFromV2Header > 0 )
        {
            ...
            dstrV4Header.pwszString = ( DRM_WCHAR* )( RightsManagementHeader.pbBuffer + RightsManagementHeader.dwCurrentPos );
...
static DRM_RESULT _drm_CalculateRMHeaderAesChecksum(
    ...
    __out_ecount( *f_pcbCheckSum )            DRM_BYTE                         *f_pbChecksum,
    __out                                     DRM_DWORD                        *f_pcbChecksum )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    __out                                     DRM_DWORD                        *f_pcbChecksum )
{
    ...
    MEMCPY( rgbBuffer, f_pbContentKey, f_cbContentKey );
    ...
    MEMCPY( f_pbChecksum, rgbBuffer, CHECKSUM_LENGTH );
    *f_pcbChecksum = CHECKSUM_LENGTH;
    ...
    __out   DRM_SUPPORTED_CIPHERS *f_peCipherType )
{
    ...
    */
    ChkDR( DRM_PRO_IsValidObject( f_pbPlayReadyHeader, f_cbPlayReadyHeader ) );
    ...
    __in                                      DRM_DWORD *f_pcbPlayReadyHeader )
{
    ...
    */
    ...
    {
        /* Skip the BOM. */
        dstrV2Header.pwszString = ( DRM_WCHAR* )( f_pbWmdrmHeader + SIZEOF( DRM_WCHAR ) );
        dstrV2Header.cchString = ( f_cbWmdrmHeader - SIZEOF( DRM_WCHAR ) ) / SIZEOF( DRM_WCHAR );
    }
    else
    {
        dstrV2Header.pwszString = ( DRM_WCHAR* )f_pbWmdrmHeader;
    ...
    __inout                               DRM_DWORD *f_pcbPlayReadyObject )
{
    ...
    */
    ChkDR( _drm_BufferStreamWriteWORD( &oBufferStream, f_fIncludeELS ? 2 : 1 ) );
