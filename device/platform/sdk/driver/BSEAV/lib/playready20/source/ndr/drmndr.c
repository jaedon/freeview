...
)
{
    ...
    DRM_RSA_PRIVATE_KEY  oPrivateKey           = {0};
    ...
    */
    f_pProximityContext->Address = (DRM_BYTE *)Oem_MemAlloc( f_pProximityContext->AddressSize + 1 );
...
ErrorExit:
    DRM_PROFILING_LEAVE_SCOPE;
    OEM_RSA_ZeroPrivateKey( &oPrivateKey );
    return dr;
}
...
)
{
    ...
    DRM_RSA_PRIVATE_KEY   oPrivateKey       = {0};
    DRM_DWORD             cbDevicePvtKey    = SIZEOF( DRM_RSA_PRIVATE_KEY );
    DRM_BYTE              rgbPlaintext[ __CB_DECL( DRM_RSA_CB_MAX_PLAINTEXT_MAX ) ] = { 0 };
    ...
    */
    ...
    ChkDR( OEM_RSA_GetPrivateKeyLength( &oPrivateKey, &eKeyLength ) );
    ChkBOOL( eKeyLength <= eDRM_RSA_MAXIMUM_KEY_LENGTH, DRM_E_PRIVKEYREADERROR );

    cbPlaintext = DRM_RSA_CB_MAX_PLAINTEXT( eKeyLength );

    ChkDR( OEM_RSA_OaepDecrypt( &oPrivateKey,
