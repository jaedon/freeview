...
)
{
    ...
    if( DRM_REVOCATION_IsRevocationSupported() )
    {
        ...
        {
            ChkDR( DRM_E_REVOCATION_BUFFERTOOSMALL );
        }
    }
...
)
{
    ...
    {
        OEM_RSA_ZeroPublicKey( pContext->certificateInfo.pDevicePublicKey );
...
**              DRM_E_INVALIDDEVICECERTIFICATE if the certificate in the registration message is invalid
**
**********************************************************************/
...
)
{
    ...
    */
    ...
    DRM_SHA_Update  ( (DRM_BYTE *) &idSerial, DRM_ID_SIZE, &contextSHA );
    ...
    if ( FALSE == fBypassValidation )
    {
        ...
        if ( NULL != pContext->certificateInfo.pDevicePublicKey )
        {
            OEM_SECURE_ZERO_MEMORY( pContext->certificateInfo.pDevicePublicKey, SIZEOF( DRM_RSA_PUBLIC_KEY ) );
            SAFE_OEM_FREE( pContext->certificateInfo.pDevicePublicKey );
        }
...
)
{
    ...
    if( !DRM_UTL_DASSTRStrings_CI_Equal( (DRM_CHAR*)pbAction, &dastrAction, &g_dastrDRM_RIGHT_PLAYBACK ) )
    {
 ...
 ErrorExit:
    ...
    if ( FALSE == fBypassValidation )
    {
        if ( NULL != certificateInfo.pDevicePublicKey )
        {
            OEM_RSA_ZeroPublicKey( certificateInfo.pDevicePublicKey );
            SAFE_OEM_FREE( certificateInfo.pDevicePublicKey );
        }
    }
...
)
{
    ...
    ChkMem( pRevocationLists = (DRM_RVK_LIST*) Oem_MemAlloc( cRevocationLists *
                                           SIZEOF( DRM_RVK_LIST ) ) );
