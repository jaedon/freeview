...
)
{
    ...
    case DRM_E_INVALIDDEVICECERTIFICATE:
...
**              DRM_E_LICENSENOTFOUND if a license must be aquired to deliver this content or
**              DRM_E_FILEREADERROR if an error occurred reading the file or
...
)
{
    ...
    *f_pdwEndOfData = 0;
    if ( pContentSession->hRawFile != OEM_INVALID_HANDLE_VALUE )
    {
        ...
        */
        *f_pdwEndOfData = ( *f_pcbNextDataBlock < cbWanted ) ? 1 : 0;
    }
    else
    {
        dr = Drm_Ndt_Asf_ReadData( f_pOEMContext,
                                   ...
                                   f_pbNextDataBlock,
                                   f_pcbNextDataBlock );
        *f_pdwEndOfData = ( dr == DRM_E_NOMORE ? 1 : 0);
        ChkDR( dr );
    }
