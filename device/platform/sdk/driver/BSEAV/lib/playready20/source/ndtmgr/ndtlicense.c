...
)
{
    ...
    /*
    ** If the file is already AES encrypted, we will be avoiding a redundant
    ** and expensive AES -> AES transcryption.  This means we need to use the
    ** Content Key from the original file.  We accomplish this by overwriting
    ** the first 16 bytes of the f_pLicContext->rgbKeyMaterial array with the
    ** Content Key obtained when the Bind() was done.
    */
    if ( f_pNdtContentSessionContext->asfContext.eCipherType == eDRM_AES_COUNTER_CIPHER )
    {
        DRM_CIPHER_CONTEXT *pCipherContext = (DRM_CIPHER_CONTEXT *)&(f_pNdtContentSessionContext->asfContext.decryptContext.rgbBuffer);
        MEMCPY( f_pLicContext->rgbKeyMaterial, pCipherContext->cipher.cipherAES.rgbContentKey, DRM_AES_KEYSIZE_128 );
    }
...
)
{
    ...
    case NDT_LICENSE_MODE_SIMPLE:
        pLicContext = (NDT_LICENSE_CONTEXT *) pContentSesContext->pLicenseContext;
        break;
...
)
{
    ...
    DRM_BYTE                        rgbKeyMaterial[DRM_RSA_CB_MODULUS_MAX] = {0};
    DRM_BYTE                       *pbXMR = NULL;
    DRM_DWORD                       cbXMR;
    DRM_XMR_RIGHTS                  Settings;
    WMDRMNET_POLICY_TRANSCRYPTPLAY *pPolicyTranscryptPlay;
    DRM_CRYPTO_CONTEXT             *poCrypto = NULL;
    DRM_RSA_SUPPORTED_KEY_LENGTHS   eDeviceKeyLength = eDRM_RSA_UNKNOWN;
    DRM_DWORD                       cbDeviceExponent = SIZEOF( deviceKey.dwExponent );
    DRM_BYTE                        rgbDeviceModulus[DRM_RSA_CB_MODULUS_MAX] = {0};
    ...
    ChkDR( OEM_RSA_ParsePublicKey(               f_pCertificateInfo->pDevicePublicKey,
                                                &eDeviceKeyLength,
                                                &cbDeviceExponent,
...
)
{
    ...
    DRM_XMR_SERIAL_NUMBER           xmrSerialNumber;
    DRM_DWORD                       cbKeyMaterial = 0;
    DRM_BYTE                        rgbKeyMaterial[__CB_DECL(DRM_RSA_CB_MODULUS_MAX)] = {0};
    DRM_BYTE                       *pbXMR = NULL;
    DRM_DWORD                       cbXMR;
    DRM_XMR_RIGHTS                  Settings;
    WMDRMNET_POLICY_TRANSCRYPTPLAY *pPolicyTranscryptPlay;
    ...
    DRM_RSA_SUPPORTED_KEY_LENGTHS   eDeviceKeyLength = eDRM_RSA_UNKNOWN;
    DRM_DWORD                       cbDeviceExponent = SIZEOF( deviceKey.dwExponent );
    DRM_BYTE                        rgbDeviceModulus[__CB_DECL(DRM_RSA_CB_MODULUS_MAX)] = {0};
    DRM_DWORD                       cbDeviceModulus = SIZEOF( rgbDeviceModulus );
    ...
    pPolicyTranscryptPlay = (WMDRMNET_POLICY_TRANSCRYPTPLAY *)
                             f_pPolicy->pbPolicy;
