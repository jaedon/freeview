...
static DRM_NO_INLINE DRM_BOOL bucket_multiply
         ...
         struct bigctx_t *f_pBigCtx)
{
    ...
    if (temps->bucket_occupied[ibucket]) {
        ...
        OEM_SECURE_DIGITTCPY( bloc,multiply_by,elng);
    }
    return OK;
} // end bucket_multiply
...

{
    ...
    //temps_raw is the buffer to be passed to the bignum_free, as per DRM_STK_Alloc_Aligned requirements
    //see comments on top of DRM_STK_Alloc_Aligned
    DRM_VOID *temps_raw = NULL;  
    ...
    temps = (temps_t *) bignum_alloc_align(SIZEOF(temps_t), SIZEOF(temps_t *), f_pBigCtx, &temps_raw);
    ...
    if (!OK) {
        ...
        */
        ...
        for (ibucket = 1; OK && ibucket <= max_bucket; ibucket++) {
            ...
            OEM_SECURE_DIGITTCPY( bloc,modulo->one,elng);
                                                      // Set bucket contents = 1        
        }  // for ibucket
        
        if (OK) {
            OEM_SECURE_DIGITTCPY( basepower,base,elng);    // basepower = base
        }
        carried = 0;
        ndoubling = 0;
    ...
    if (OK) {
        OEM_SECURE_DIGITTCPY( answer,temps->bucket_location[1],elng);
    }
    if (bucket_data != NULL) {
        bignum_free(bucket_data, f_pBigCtx);
    }
    if (temps != NULL) {
        bignum_free(temps_raw, f_pBigCtx);
    } 
    return OK;
} // end mod_exp
