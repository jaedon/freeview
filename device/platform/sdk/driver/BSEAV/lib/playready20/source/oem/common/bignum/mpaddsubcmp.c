...
*/
{
    ...
    for (i = 0; i != lng; i++) {
        ...
        if (bi >= carry) { // No carry propagation
            if (a != b) {
                OEM_SECURE_DIGITTCPY( b + i + 1,a + i + 1,( lng - i - 1));
            }
            return 0;
        }
        carry = 1;
...
/****************************************************************************/
DRM_API DRM_NO_INLINE DRM_LONG DRM_CALL compare_diff
        (const digit_t    a[],   // IN
         const DRM_DWORD   lnga,  // IN
         const digit_t    b[],   // IN
         const DRM_DWORD   lngb)  // IN
...
/****************************************************************************/
DRM_API DRM_NO_INLINE DRM_LONG DRM_CALL compare_same
        (const digit_t  a[],       // IN
         const digit_t  b[],       // IN
         const DRM_DWORD lng)       // IN
/*
...
/****************************************************************************/
DRM_API DRM_NO_INLINE DRM_LONG DRM_CALL compare_sum_diff
        (const digit_t     a[],
...
/****************************************************************************/
DRM_API DRM_NO_INLINE DRM_LONG DRM_CALL compare_sum_same
       (const digit_t  a[],      // IN
        const digit_t  b[],      // IN
        const digit_t  c[],      // IN
        const DRM_DWORD lng)      // IN
