...
DRM_API DRMINLINE DRM_UINT64 DRM_CALL _rotr64(DRM_UINT64 inp, DRM_DWORD shift)
{
    ...
    __in                                               DRM_DWORD           f_ibBufferOffset )
{
    ...
    */
    ...
    if ( !DRM_IS_DWORD_ALIGNED( f_rgbBuffer + __CB_DECL( ibOffset ) ) )
    {
        /*
...
#else /* 32 bit */
    {
        const DRM_DWORD *pdwIn  = ( DRM_DWORD * )f_rgdwBlock;
...
#else
    OEM_SECURE_DWORDCPY( pdwW256, ( DRM_BYTE * )f_rgdwBlock, SHA256_BLOCK_SIZE_IN_DWORDS );
#endif  /* BIGENDIAN */
