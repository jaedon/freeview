...
**
******************************************************************************/
static DRM_RESULT _GenerateKeyID ( OUT DRM_STRING *f_pdstrKeyID ) 
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT _GenerateContentKey( 
    IN const DRM_CONST_STRING *f_pdstrKeySeed, 
    IN const DRM_CONST_STRING *f_pdstrKID, 
    IN DRM_SUPPORTED_CIPHERS   eCipherType,
    OUT DRM_STRING            *f_pdstrContentKey ) 
{
    DRM_RESULT dr           = DRM_SUCCESS;
    ...
    IN OUT DRM_DWORD  *pcbHeader )
{
    ...
    dstrV4XML.pwszString = (DRM_WCHAR *)*ppbHeader;
    ...
    ChkDR( DRM_PRO_Create( &dstrV4XML, pbPlayReadyHeader, &cbPlayReadyHeader ) );
