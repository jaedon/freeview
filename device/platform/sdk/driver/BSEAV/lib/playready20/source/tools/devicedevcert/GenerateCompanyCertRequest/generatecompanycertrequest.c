...
typedef struct DRMSTATE
{
    ...
    DRM_RSA_PUBLIC_KEY  pubkeyDAC_RSA1024;
    DRM_RSA_PRIVATE_KEY privkeyDAC_RSA1024;
...
** 
******************************************************************************/
static void PrintUsage( IN const DRM_WCHAR *pwszAppName )
{
    printf("Syntax: %S [-?] \n\
...
DRM_RESULT _CreateDACRequest( __in DRMSTATE *f_pState )
{
    ...
    /*
    ** Writes RSA public key inside <PlayReadyND> tag
    */
    CallXmlBuilder( f_pState->pbXML, 
        DRM_XMB_OpenNode(f_pState->pbXML, &g_dstrTagDAC_PlayReadyND) );
    ...
    */
    ...
    ChkBOOL( Oem_File_Write( f_pState->fpDACRequest, (DRM_BYTE*)g_rgbFFFE, cbWrite, &cbWritten )==TRUE, DRM_E_FILEWRITEERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILEWRITEERROR );

    cbWrite = CB_DSTR( &dstrClose );
    ChkBOOL( Oem_File_Write( f_pState->fpDACRequest, dstrClose.pwszString, cbWrite, &cbWritten )==TRUE, DRM_E_FILEWRITEERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILEWRITEERROR );
...
DRM_RESULT _CreateOEMPrivKeys( __in DRMSTATE *f_pState )
{
    ...
    /*
    ** Writes RSA private key inside <PlayReadyND> tag
    */
    CallXmlBuilder( f_pState->pbXML, 
            DRM_XMB_OpenNode(f_pState->pbXML, &g_dstrTagDAC_PlayReadyND) );
    ...
    */
    ...
    cbWrite = SIZEOF( g_rgbFFFE );
    ChkBOOL( Oem_File_Write( f_pState->fpOEMPrivKeys, (DRM_BYTE*)g_rgbFFFE, cbWrite, &cbWritten )==TRUE, DRM_E_FILEWRITEERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILEWRITEERROR );

    cbWrite = CB_DSTR( &dstrClose );
    ChkBOOL( Oem_File_Write( f_pState->fpOEMPrivKeys, dstrClose.pwszString, cbWrite, &cbWritten )==TRUE, DRM_E_FILEWRITEERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILEWRITEERROR );
