...
/* This define is no longer present in the dev code, as we always support
** seeking by time. It may be removed from the test code at some point */
#define DRM_SUPPORT_SEEK_BY_TIME 1
...
static DRM_RESULT OpenFileIntoBuffer(IN const char* szFileName, OUT DRM_BYTE** ppBuffer, OUT DRM_DWORD* pcbBuffer)
{
    ...
    fh = Oem_File_Open(NULL, wszFilename, OEM_GENERIC_READ,  OEM_FILE_SHARE_NONE, OEM_OPEN_EXISTING, OEM_ATTRIBUTE_NORMAL);
...
static DRM_RESULT GetClearSampleFromFile(__in_z const char *szFileName, IN DRM_DWORD dwSampleNumber, IN DRM_DWORD cbSampleData, OUT DRM_BYTE *pbBuffer)
{
    DRM_WCHAR wszFilename[256];
    DRM_RESULT dr = DRM_SUCCESS;
    OEM_FILEHDL fh;
    DRM_DWORD cb;
    ...
    Oem_File_SetFilePointer( fh, jazz_clear_sample_offsets[dwSampleNumber], OEM_FILE_BEGIN, NULL );
...
DRM_RESULT DRM_CALL Test_SetPrivateKey(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR(OpenFileIntoBuffer(argv[0], &pTmp, &cbTmp));
    ChkMem(pTmp);
    ...
    {
        OEM_RSA_ZeroPrivateKey(g_pPrivateKey);
        g_pPrivateKey = NULL;
    }
...
DRM_RESULT DRM_CALL Test_SetRevocationList(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR( OpenFileIntoBuffer(argv[1],&pbBuffer,&cbBuffer) );
...
DRM_RESULT DRM_CALL Test_SetRegistrationSessionID(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkArg(argc == 1 && argv[0]);
    ParseDRMID(&g_RegistrationSessionID, argv[0]);
...
DRM_RESULT DRM_CALL Test_CheckDecryption(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR(OpenFileIntoBuffer(argv[0], &pbEncrypted, &cbEncrypted));
    ChkMem(pbEncrypted);

    ChkDR(OpenFileIntoBuffer(argv[1], &pbClear, &cbClear));
    ChkMem(pbClear);
...
DRM_RESULT DRM_CALL Test_CheckNotTranscrypted(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR( OpenFileIntoBuffer(argv[0], &pbActual, &cbActual) );
    ChkMem( pbActual );

    ChkDR( OpenFileIntoBuffer(argv[1], &pbExpected, &cbExpected) );
    ChkMem( pbExpected );
...
#ifdef __TMS320C55X__
    pAction = (DRM_BYTE *) OEM_malloc(g_cbAction + 1);
...
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_WCHAR  wszInputFileName[MAX_PATH];
    DRM_WCHAR  wszOutputFileName[MAX_PATH];
    DRM_DWORD  dwForceNewSession;
    DRM_WORD   wFileType;
    DRM_SUBSTRING dsubstr;
    DRM_STRING dstr;
    ...
    dstr.cchString = NO_OF( wszInputFileName );
    ...
    dsubstr.m_cch =  ( DRM_DWORD ) strlen(argv[3]);
    dstr.pwszString = wszOutputFileName;
    dstr.cchString = NO_OF( wszOutputFileName );
    DRM_UTL_PromoteASCIItoUNICODE(argv[3], &dsubstr, &dstr);
    ...
    g_hOutputFile = Oem_File_Open(NULL, wszOutputFileName, OEM_GENERIC_WRITE, OEM_FILE_SHARE_NONE, OEM_CREATE_ALWAYS, OEM_ATTRIBUTE_NORMAL );
    ChkBOOL( g_hOutputFile != OEM_INVALID_HANDLE_VALUE, DRM_E_FAIL );

#if DRM_SUPPORT_SEEK_BY_TIME
...
DRM_RESULT DRM_CALL Test_Drm_Pfd_Aavt_GetNextDataBlock(long argc, __in_ecount(argc) char *argv[])
{
    ...
    DRM_BYTE *pbClear = NULL;
    DRM_BYTE *pbDecrypted = NULL;
#endif /* DRM_SUPPORT_SEEK_BY_TIME */
...
DRM_RESULT DRM_CALL Test_Drm_Pfd_GetMTPStatusFromDRMRESULT(long argc, __in_ecount(argc) char *argv[])
{
    ...
    drRes = Drm_Pfd_GetMTPStatusFromDRMRESULT(DRM_E_INVALIDDEVICECERTIFICATE);
    ...
    drRes = Drm_Pfd_GetMTPStatusFromDRMRESULT(DRM_E_LICENSENOTFOUND);
...
DRM_RESULT DRM_CALL MODULE_PRETESTCASE(ConsoleNDTTests)(long lTCID, const char *strTCName)
{
    if( DRM_DVS_IsDeviceStoreSupported() )
    {
        ...
        DRM_WCHAR rgwchDataStore[] = {
            ONE_WCHAR('f', '\0'), ONE_WCHAR('i', '\0'), ONE_WCHAR('l', '\0'),
            ONE_WCHAR('e', '\0'), ONE_WCHAR('s', '\0'), ONE_WCHAR('\\','\0'),
            ONE_WCHAR('s', '\0'), ONE_WCHAR('a', '\0'), ONE_WCHAR('m', '\0'),
            ONE_WCHAR('p', '\0'), ONE_WCHAR('l', '\0'), ONE_WCHAR('e', '\0'),
            ONE_WCHAR('.', '\0'), ONE_WCHAR('h', '\0'), ONE_WCHAR('d', '\0'),
            ONE_WCHAR('s', '\0'), ONE_WCHAR('\0','\0')
        };
        ...
        DRM_WCHAR rgwchZPrivSig[] = {
            ONE_WCHAR('z', '\0'), ONE_WCHAR('p', '\0'), ONE_WCHAR('r', '\0'),
            ONE_WCHAR('i', '\0'), ONE_WCHAR('v', '\0'), ONE_WCHAR('s', '\0'),
            ONE_WCHAR('i', '\0'), ONE_WCHAR('g', '\0'), ONE_WCHAR('.', '\0'),
            ONE_WCHAR('d', '\0'), ONE_WCHAR('a', '\0'), ONE_WCHAR('t', '\0'),
            ONE_WCHAR('\0', '\0')
        };
        DRM_WCHAR rgwchZPrivEncr[] = {
            ONE_WCHAR('z', '\0'), ONE_WCHAR('p', '\0'), ONE_WCHAR('r', '\0'),
            ONE_WCHAR('i', '\0'), ONE_WCHAR('v', '\0'), ONE_WCHAR('e', '\0'),
            ONE_WCHAR('n', '\0'), ONE_WCHAR('c', '\0'), ONE_WCHAR('r', '\0'),
            ONE_WCHAR('.', '\0'), ONE_WCHAR('d', '\0'), ONE_WCHAR('a', '\0'),
            ONE_WCHAR('t', '\0'), ONE_WCHAR('\0', '\0')
        };
        DRM_WCHAR rgwchBDevCert[] = {
            ONE_WCHAR('b', '\0'), ONE_WCHAR('d', '\0'), ONE_WCHAR('e', '\0'),
            ONE_WCHAR('v', '\0'), ONE_WCHAR('c', '\0'), ONE_WCHAR('e', '\0'),
            ONE_WCHAR('r', '\0'), ONE_WCHAR('t', '\0'), ONE_WCHAR('.', '\0'),
            ONE_WCHAR('d', '\0'), ONE_WCHAR('a', '\0'), ONE_WCHAR('t', '\0'),
            ONE_WCHAR('\0', '\0')
        };
        ...
        dstrDataStoreSrc.pwszString = rgwchDataStore;
        dstrDataStoreSrc.cchString = NO_OF( rgwchDataStore );
        dstrDataStoreDst.pwszString = &(rgwchDataStore[6]);
        dstrDataStoreDst.cchString = NO_OF( rgwchDataStore ) - 6;
        ...
        ChkDR(_CopyFileToDRMFolder(rgwchBDevCert, &(rgwchBDevCert[0])));
        ChkDR(_CopyFileToDRMFolder(rgwchBGroupCert, &(rgwchBGroupCert[0])));
        ChkDR(_CopyFileToDRMFolder(rgwchZgPriv, &(rgwchZgPriv[0])));
        ChkDR(_CopyFileToDRMFolder(rgwchZPrivSig, &(rgwchZPrivSig[0])));
        ChkDR(_CopyFileToDRMFolder(rgwchZPrivEncr, &(rgwchZPrivEncr[0])));
...
DRM_RESULT DRM_CALL MODULE_POSTTESTCASE(ConsoleNDTTests)(long lTCID, const char *strTCName)
{
    if( DRM_DVS_IsDeviceStoreSupported() )
    {
        if( g_dwMediaSessionID != 0)
        {
            Drm_Pfd_Aavt_CloseMediaSession( g_pPfdDrmContext, g_dwMediaSessionID );
            g_dwMediaSessionID = 0;
        }
...
BEGIN_TEST_SCRIPT_LIST
    ...
    TEST_SCRIPT_ENTRY_CONDITIONAL("FunctionalTest-MaxContentSessions-5", DRM_DVS_IsDeviceStoreSupported)
#else
    TEST_SCRIPT_ENTRY_CONDITIONAL("FunctionalTest-MaxContentSessions-1", DRM_DVS_IsDeviceStoreSupported)
