...
DRM_RESULT DRM_CALL TestManagerSetHeader( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if ( wFileType == FILE_TYPE_UTF16 )
    {
        Log( "", "Header:%S", (DRM_WCHAR*) dstrHeader.pwszString );
    }
    else if ( wFileType == FILE_TYPE_UTF8 )
    {
    ...
    {
        cchHeaderType = DRMCRT_strlen( argv[4] );
        if ( ( cchHeaderType >= 4 ) &&
...
DRM_RESULT DRM_CALL TestManagerSetPROHeaderWithKID( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if ( argc > 2 && argv[2] != NULL )
    {
        ...
        oPROWithKID.pbKeyID = (DRM_BYTE*) wszProperty.pwszString;
...
DRM_RESULT DRM_CALL TestManagerSetKID( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    if( argv[0] != NULL )
    {
        ...
        */
        ChkDR( DRM_STR_UTF8toDSTR( ( DRM_CHAR* )( argv[0] ), DRMCRT_strlen( argv[0] ), &dstr ) );
    }
...
#if DBG
    Log( "Trace", "\t         KID %d = %S", ( *pdwKIDListIndex + 1 ), &( poKIDListData[ *pdwKIDListIndex ].rgwchKID ) );
...
DRM_RESULT DRM_CALL TestManagerSetV41Header( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if( argc > 0 )
    {
        pbWRMHeader = (DRM_BYTE *)argv[0];
    }

    if( argc > 1 && argv[1] != NULL )
    {
        cbWRMHeader = atoi( argv[1] );
    }
...
DRM_RESULT DRM_CALL TestManagerGetContentProperty( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    if ( fSavePROToFile )
    {
        ...
        ChkBOOL( SaveToFileW( dstrPROFileName.pwszString, pbProperty, *pcbProperty ), DRM_E_FILEWRITEERROR );
    }
