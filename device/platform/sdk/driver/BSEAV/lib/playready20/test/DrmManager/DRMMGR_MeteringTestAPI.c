...
static DRM_RESULT _GetMeterCertFromFile(const DRM_WCHAR *pwchInFile, DRM_WCHAR  *pwchMeterCert)
{
                                    ...
                                    OEM_ATTRIBUTE_NORMAL)) != OEM_INVALID_HANDLE_VALUE)
        {
            ...
            if (Oem_File_GetSize (hfile, &cbFile))
            {
                if ((pwchFile = (DRM_WCHAR*)Oem_MemAlloc (cbFile + 2)) != NULL)
                {
                    ...
                    {
                        MEMCPY(pwchMeterCert,pwchFile,cbFile);
                        dr = DRM_SUCCESS;
                    }
                    else
                    {
            ...
            {
                Log ("Trace", "Unable to get size for file file.\n");
            }
        }
        else
        {
...
static DRM_RESULT _GetMeteringDataAsXML(_XMBContext *pbData, DRM_DWORD cbData, PRIVKEY *pPrivKey, DRM_CONST_STRING *pretXML)
{
...
static DRM_RESULT _GetMeterResponseFromFile(const DRM_WCHAR *pwchInResponseFile, DRM_WCHAR  **pwchMeterResponse)
{
    DRM_RESULT dr = DRM_E_FILENOTFOUND;
    OEM_FILEHDL hfile         = OEM_INVALID_HANDLE_VALUE;
...
static DRM_RESULT _SignResponse(DRM_CONST_STRING *pdstrResponse,
...
DRM_RESULT DRM_CALL TestManagerGenerateMeterChallenge( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if ( argc > 3 )
    {
        ...
        */
        ...
        for ( index = 4; index + 2 < argc; index += 3 )
        {
...
DRM_RESULT DRM_CALL TestManagerGetLicenseMID( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    if ( ( argc > 4 + (DRM_LONG)dwExpectedMIDs) && IsDigitalOnlyString( argv[4 + (DRM_LONG)dwExpectedMIDs] ) )
    {
        ...
        */
        cbMIDs = atoi( argv[3 + (DRM_LONG)dwExpectedMIDs] );
    }

#pragma prefast(push)
#pragma prefast(disable:26015) /* Test conditions that may contradict SAL annotated API. */
...
#pragma prefast(pop)
    ...
    {
        dsstrMID.m_cch = DRMCRT_strlen( argv[0] );
        dsstrMID.m_ich = 0;
...
/*
** Parameter combination may violate SAL.
*/
#pragma prefast(push)
...
#pragma prefast(pop)
    ...
    if ( argv[0] != NULL )
    {
        ...
        */
        ChkDR( DRM_STR_UTF8toDSTR( ( DRM_CHAR* )( argv[0] ), DRMCRT_strlen( argv[0] ), &dstrMeterCert ) );
    ...
    {
        dsstrTemp.m_cch = DRMCRT_strlen( argv[1] );
        dsstrTemp.m_ich = 0;
