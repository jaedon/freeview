...
DRM_RESULT DRM_CALL TestManagerLicenseSyncEnumNext( long argc, __in_ecount( argc ) char **argv )
{
     ...
     && DRM_SNC_IsLicenseSyncSupported() )
    {
        ...
        if ( argv[0] != NULL )
        {
            cMaxCount = atoi(argv[0]);
        }
        if ( argv[1] != NULL )
        {
            cMaxHours = atoi(argv[1]);
        }
        ...
        if ( argc >= 5 )
        {
            ...
            #pragma prefast(suppress:28930)             /* ignore unused assignment warning, it's used */
            DRM_APP_CONTEXT_INTERNAL *pContextMGR = (DRM_APP_CONTEXT_INTERNAL *) poManagerContext;
            for( iKID = 4; iKID < argc ; iKID++ )
            {
                ...
                dsstrTemp.m_ich = 0;
                dsstrTemp.m_cch = DRMCRT_strlen( argv[iKID] );
                ...
                if( !fGoodKidOut )
                {
                    ...
                    dcstrHeader.pwszString = ( DRM_WCHAR * )pContextMGR->pbDRMHeaderData;
...
DRM_RESULT DRM_CALL TestPrintSyncStore( long argc, __in_ecount( argc ) char **argv )
{
     ...
     && DRM_SNC_IsLicenseSyncSupported() )
    {
        ...
        while( DRM_SUCCEEDED( dr ) )
        {
            ...
            DRM_UTL_DemoteUNICODEtoASCII( ( DRM_WCHAR * ) pMgrInternal->pbDRMHeaderData, pszHeader, cchHeader );
