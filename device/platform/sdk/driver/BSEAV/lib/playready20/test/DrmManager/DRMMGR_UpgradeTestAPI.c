...
static DRM_RESULT DRM_CALL _HashKeyfile(DRM_BOOL fCreate)
{
    ...
    ChkBOOL( Oem_File_GetSize( hKeyfile, &cbKeyfile ), DRM_E_FILEREADERROR );
    ChkFAIL( cbKeyfile <= DRM_KEYFILE_MAX_SIZE );

    ChkMem(pbKeyfile = (DRM_BYTE*)Oem_MemAlloc( cbKeyfile ));
    ChkBOOL( Oem_File_SetFilePointer( hKeyfile, 0, OEM_FILE_BEGIN, NULL ), DRM_E_FILESEEKERROR );
    ChkBOOL( Oem_File_Read( hKeyfile, pbKeyfile, cbKeyfile, &cbRead ), DRM_E_FILEREADERROR );
    ChkBOOL( cbRead == cbKeyfile, DRM_E_FILEREADERROR );
...
DRM_RESULT DRM_CALL TestManagerSetRobustnessVersion( long argc, __in_ecount( argc ) char **argv )
{
    ...
    else
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[2], DRMCRT_strlen( argv[2] ), 10, &dwIndex ) );
        switch ( dwIndex )
        {
            case 1:
                ChkDR( DRM_UTL_GetVersionFromStringAsDWORD( dstrSecurityVersion.pwszString,
...
DRM_RESULT DRM_CALL TestManagerValidateRobustnessVersion( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    ChkDR( DRMCRT_AtoDWORD( argv[1], DRMCRT_strlen( argv[1] ), 10, &dwIndex ) );
    ...
    switch ( dwIndex )
    {
        case 1:
            ...
            ChkDR( TST_UTL_GetWMDRMCert( (DRM_STRING*)&dstrDevCert, &cryptoCtx ) );
        ...
        case 2:
            ...
            dwOutVersion = 0;
            dwOutPlatform = 0;
            ChkDR( TST_UTL_GetPlayReadyCert(pbDevCert, &cbDevCert, &cryptoCtx) );
