...
*/
DRM_VOID DRM_CALL _CleanKFContextVar( IN const DRM_VOID *f_pvTestVar )
{
    DRM_KEYFILE_CONTEXT *pKFContext = NULL;
    DRM_TEST_VARIABLE *f_poTestVar = NULL;
    if ( f_pvTestVar == NULL )
        return;

    f_poTestVar = (DRM_TEST_VARIABLE*)f_pvTestVar;
...
**              DRM_E_TEST_NOMORE
********************************************************************************************/
DRM_RESULT DRM_CALL TST_MAP_GetDWORD( TST_DWORD_VALUE_MAP_ENTRY *f_pMap,
                                      DRM_ANSI_CONST_STRING     *f_pdastrSearch,
...
DRM_RESULT DRM_CALL Test_KF_SetCertificate( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if (( argc > 1) && (argv[1] != NULL ))
    {
        ...
        dastrCertType.cchString = DRMCRT_strlen( argv[1] );
...
DRM_RESULT DRM_CALL Test_Oem_KeyFile_EncryptKey( long argc, __in_ecount( argc ) char ** argv )
{
    ...
    dastrEncType.cchString = DRMCRT_strlen( argv[2] );
    ...
    if( dwEncType == KF_DECRYPTION_TYPE_CLEAR )
    {
        ...
        poEncKey->wKeySize = *pcbClearKey * BITS_PER_BYTE;
    }
    else
    {
        dr = DRM_BBX_KF_EncryptKey( NULL, pbClearKey, *pcbClearKey, eKeyType, poEncKey );
        if ( dr      == DRM_E_BUFFERTOOSMALL
          && argc    >  3
          && argv[3] != NULL )
        {
            MAP_TO_GLOBAL_TEST_VAR_Alloc( poEncKey->pbEncryptedKey, argv[3], poEncKey->cbEncryptedKey );
            ChkDR( DRM_BBX_KF_EncryptKey( NULL, pbClearKey, *pcbClearKey, eKeyType, poEncKey ) );
        }
        ChkDR( dr );
    }
ErrorExit:
...
DRM_RESULT DRM_CALL Test_Oem_KeyFile_DecryptKey( long argc, __in_ecount( argc ) char ** argv )
{
    ...
    dastrDecType.cchString = DRMCRT_strlen( argv[2] );
    ...
    dr = DRM_BBX_KF_DecryptKey( NULL, poEncKey, DRM_DKT_WMDRMPD_DEVICE, NULL, &cbClearKey );
    ...
    if ( dr == DRM_E_BUFFERTOOSMALL )
    {
        ...
        ChkDR( DRM_BBX_KF_DecryptKey( NULL, poEncKey, DRM_DKT_WMDRMPD_DEVICE, pbClearKey, &cbClearKey ) );
...
DRM_RESULT DRM_CALL Test_KF_SetPrivateKey( long argc, __in_ecount( argc ) char **argv )
{
    ...
    dastrKeyType.cchString = DRMCRT_strlen( argv[0] );
...
DRM_RESULT DRM_CALL Test_KF_VerifyContext( long argc, __in_ecount( argc ) char **argv )
{
           ...
           && DRMCRT_strncmp( argv[1], "CERT", 4) == 0 )
    {
        ...
        if( argc > 2 && argv[2] != NULL )
        {
            ...
            dastrCertType.cchString = DRMCRT_strlen( argv[2] );
            ChkDR( TST_MAP_GetDWORD( g_rgoKFCertTypeMap, &dastrCertType, &dwCertType ) );
