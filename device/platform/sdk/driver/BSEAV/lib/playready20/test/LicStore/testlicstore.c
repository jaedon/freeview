...
static const DRM_CONST_STRING _wcszLidTag          = CREATE_DRM_STRING( L"LID" );
static const DRM_CONST_STRING _wcszKidTag          = CREATE_DRM_STRING( L"KID" );
static const DRM_CONST_STRING _wcszLicenseTag      = CREATE_DRM_STRING( L"LICENSE" );
static const DRM_CONST_STRING _wcszDataTag         = CREATE_DRM_STRING( L"DATA" );
static const DRM_CONST_STRING _wcszLicensorInfoTag = CREATE_DRM_STRING( L"LICENSORINFO" );
...
DRM_RESULT UpdateLicenseKeys( DRM_WCHAR *pwszBuffer, DRM_DWORD cchBuffer, long iOption, long iNumLicense)
{
    ...
    */        
    ...
    else
    {
        ...
        */
        ...
        ChkMem( pbEmbeddedStoreData = ( DRM_BYTE * )Oem_MemAlloc( cbEmbeddedStoreData ) );
        ZEROMEM( pbEmbeddedStoreData, cbEmbeddedStoreData );
...
DRM_RESULT DRM_CALL Test_DRM_LST_GetLicenseKeys( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    wszXmlStr = wszXmlSubNode;
    ChkDR( DRM_XML_GetNode( &wszXmlStr, &_wcszDataTag, NULL, NULL, 0,NULL, &wszXmlSubNode ) );

    /*
    ** extract g_LID, g_KID and priority node 
    */
    wszXmlStr = wszXmlSubNode;
    ChkDR( DRM_XML_GetNode( &wszXmlStr, &_wcszLidTag, NULL, NULL, 0,&wszXmlNode, &wszLIDValue ) );
    ChkDR( DRM_XML_GetNode( &wszXmlStr, &_wcszKidTag, NULL, NULL, 0,&wszXmlNode, &wszKIDValue ) );
...
DRM_RESULT DRM_CALL Test_DRM_LST_GetLicense( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    if ( ( iOption & 4 ) != 0 )
    {
        ...
        if ( argv[3] != NULL )
        {
            ChkMem( g_pbLicense = ( DRM_BYTE* )Oem_MemAlloc( ( DRM_DWORD ) strlen( argv[3] ) + 1 ) );
            MEMCPY( g_pbLicense, argv[3], strlen( argv[3] ) + 1 );
        }
    }    
    /*
    ...
    */
    if ( ( iOption & 16 ) != 0 )
    {
        ...
        if ( argv[1] != NULL )
        {
            MEMCPY( g_KID.rgb, argv[1], min( strlen( argv[1] ) + 1, DRM_ID_SIZE ) );
        }
        else
        {
            iKIDNullFlag = 1;
    ...
    */
    if ( ( iOption & 32 ) != 0 )
    {
        ...
        if ( argv[2] != NULL )
        {
            MEMCPY( g_LID.rgb, argv[2], min( strlen( argv[2] ) + 1, DRM_ID_SIZE ) );
        }
        else
        {
            iLIDNullFlag = 1;
...
DRM_RESULT DRM_CALL Test_DRM_LST_InitEnum( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    if ( ( iOption & 16 ) != 0 )
    {
        ...
        if ( argv[1] != NULL )
        {
            TST_UTL_mbstowcs( pwszAttr, argv[1], strlen( argv[1] ) );
            wszKIDValue.pwszString = pwszAttr;
            wszKIDValue.cchString = ( DRM_DWORD )strlen( argv[1] );
    ...
    */
    if ( argv[3] != NULL )
    {
        ...
        MEMCPY( &g_oLicEnumContext, argv[3], strlen( argv[3] ) + 1 );             
    }
    else
    {
...
DRM_RESULT DRM_CALL Test_DRM_LST_InitReverseEnum( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    ...
    {
        MEMSET( &g_oLicEnumContext, 0, SIZEOF( g_oLicEnumContext ) );
        MEMCPY( &g_oLicEnumContext, argv[2], strlen( argv[2] ) + 1 );             
    }
    else
    {
        iEnumContextNullFlag = 1;
...
DRM_RESULT DRM_CALL Test_DRM_LST_EnumNext( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    ...
    {
        TST_UTL_mbstowcs( pwszAttr, argv[4], strlen( argv[4] ) );
        wszKIDValue.pwszString = pwszAttr;
        wszKIDValue.cchString = ( DRM_DWORD )strlen( argv[4] );
    ...
    if ( argv[5] != NULL && iLIDNullFlag != 1 )
    {
        ...
        TST_UTL_mbstowcs( pwszAttr, argv[5],  strlen( argv[5] ) + 1 );
        wszLIDValue.pwszString = pwszAttr;
        wszLIDValue.cchString= ( DRM_DWORD )strlen( argv[5] );
...
DRM_RESULT DRM_CALL Test_DRM_LST_InitEnumMultiple( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    if ( argv[3] != NULL )
    {
        ...
        MEMCPY( &g_oLicEnumContextMultiple, argv[3], strlen( argv[3] ) + 1 );             
    }
    else
    {
        iEnumContextNullFlag = 1;
...
DRM_RESULT DRM_CALL TestLicStoreAddMultiLicenses( long argc, __in_ecount(argc) char **argv )
{
    ...
    */ 
    ...
    while( iNumLicense-- > 0 )
    {
        UpdateLicenseKeys( ( DRM_WCHAR* )g_pbLicense, cbLicense / SIZEOF( DRM_WCHAR ), iOption, iNumKeyBase );
...
DRM_RESULT DRM_CALL TestLicStoreLog( long argc, __in_ecount(argc) char **argv )
{
    ...
    FILE      *fp             = NULL;
    DRM_CHAR   szFileName[32] = "LicstorePerfLog.txt";
...
DRM_RESULT DRM_CALL Test_DRM_LST_DeleteLicense( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkDR( DRM_UTL_StringToGuid( &dstrLID, ( DRM_GUID * )&oLID ) );
