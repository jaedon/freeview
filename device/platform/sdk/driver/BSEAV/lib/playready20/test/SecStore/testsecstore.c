...
DRM_RESULT DRM_CALL TestDRM_SST_GetData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        ...
        memcpy(Key1.rgb, argv[1], min(strlen(argv[1]) + 1, DRM_ID_SIZE));
        pKey1 = &Key1;
    } 

    if(argv[2]!=NULL){
        memset(Key2.rgb, 0, DRM_ID_SIZE);
        memcpy(Key2.rgb, argv[2], min(strlen(argv[2]) + 1, DRM_ID_SIZE));
        pKey2 = &Key2;
    } 

    if(argv[3]!=NULL){
        pbPassword=(DRM_BYTE*)Oem_MemAlloc( ( DRM_DWORD ) strlen(argv[3])+1);
        if(!pbPassword)
            ChkDR(DRM_E_FAIL);
        memcpy(pbPassword, argv[3],strlen(argv[3])+1);             
    } 
    ...
    if(argv[6]!=NULL){
        cbData=( DRM_DWORD ) strlen(argv[6])+1;
    ...
    /*verify the data*/
    if(argv[8]!=NULL){
        if(strncmp((char*)pbData,argv[8], strlen(argv[8]))){
            ChkDR(DRM_E_FAIL);
        }
    }
ErrorExit:
...
DRM_RESULT DRM_CALL TestDRM_SST_SetData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[3]!=NULL){
        ChkArg( strlen(argv[3]) >= DRM_SHA1_DIGEST_LEN );
...
DRM_RESULT DRM_CALL TestDRM_SST_OpenKeyTokens(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[3]!=NULL){
        ...
        if(!pbPassword)
            ChkDR(DRM_E_FAIL);
        if(strcmp(argv[3],"Password")){
            memcpy(pbPassword, argv[3],min(strlen(argv[3]) + 1, DRM_SHA1_DIGEST_LEN));             
        }
        else{
...
DRM_RESULT DRM_CALL TestDRM_SST_GetTokenValue(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    /*attribute name*/
    if(argv[1]!=NULL){
        ChkArg( strlen( argv[1] ) + 1 <= NO_OF( pwszAttr ) );
        TST_UTL_mbstowcs(pwszAttr, argv[1], strlen(argv[1])+1);
    ...
    if(argv[2]!=NULL && argv[3]!=NULL){
        if(TOKEN_LONG==(DRM_EXPR_TOKEN_TYPE)atoi(argv[2])){
            ...
            ChkArg( strlen( argv[3] ) + 1 <= NO_OF( pwszAttr ) );
            TST_UTL_mbstowcs(pwszAttr, argv[3],strlen(argv[3])+1 );
            ...
            if(myToken.TokenType!=TOKEN_BYTEBLOB || DRMCRT_strncmp((const DRM_CHAR*)myToken.val.byteValue.pbBlob,argv[3],strlen(argv[3]))){
                ChkDR(DRM_E_FAIL);
            }
        }
    }
...
DRM_RESULT DRM_CALL TestDRM_SST_SetTokenValue(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    if(myToken.TokenType==TOKEN_LONG){
        ...
        if(argv[3]!=NULL){
            ChkArg( strlen( argv[3] ) + 1 <= NO_OF( pwszString ) );
            TST_UTL_mbstowcs(pwszString, argv[3], strlen(argv[3])+1);
        ...
        if(argv[3]!=NULL)
        {
            myToken.val.byteValue.cbBlob = ( DRM_DWORD )strlen(argv[3])+1;
...
DRM_RESULT DRM_CALL TestDRM_SST_GetAllData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        cbData= ( DRM_DWORD ) strlen(argv[2])+1;
        ChkMem(pbData=(DRM_BYTE*)Oem_MemAlloc(cbData));
...
DRM_RESULT DRM_CALL PolicyCallback(
    IN const DRM_VOID                 *f_pvPolicyData,
...
DRM_RESULT DRM_CALL TestDRM_SST_DeleteKey(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[2]!=NULL){
        memset(Key1.rgb, 0, DRM_ID_SIZE);
        memcpy(Key1.rgb, argv[2], min(strlen(argv[2]) + 1, DRM_ID_SIZE));
        pKey1 = &Key1;
    } 

    if(argv[3]!=NULL){
        memset(Key2.rgb, 0, DRM_ID_SIZE);
        memcpy(Key2.rgb, argv[3], min(strlen(argv[3]) + 1, DRM_ID_SIZE));
        pKey2 = &Key2;
    } 
...
DRM_RESULT DRM_CALL TestDRM_SST_EnumNext(long argc, __in_ecount(argc) char **argv)
{
    ...
    /*verify key value*/
    if(argv[3]!=NULL){
        if(strncmp((const DRM_CHAR *)pKey1->rgb,argv[3], min(strlen(argv[3]) + 1, DRM_ID_SIZE))){
            ChkDR(DRM_E_FAIL);
        }
    }
...
DRM_RESULT DRM_CALL TestDRM_SST_CreateGlobalStorePassword(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR(DRM_SST_CreateGlobalStorePassword(g_szSecStorePassword, (DRM_BYTE*)poBBContext));
...
DRM_RESULT DRM_CALL TestDRM_SST_CreateLicenseStatePassword(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRM_KF_Open( poBBContext->pKeyFileContext, TRUE, OEM_GENERIC_READ ) );
    ChkDR( DRM_KF_LoadFromFile( poBBContext->pKeyFileContext, TRUE ) );
    ChkDR( DRM_BBX_SetupSecureStoreKey( poBBContext->pKeyFileContext, NULL ) );
    ChkDR( DRM_KF_Close( poBBContext->pKeyFileContext ) );
    ...
    ChkDR( DRM_BBX_Initialize( NULL, poBBContext ) );
    if(argv[0]!=NULL){
        memcpy(localLID.rgb, argv[0], min(strlen(argv[0])+1, DRM_ID_SIZE));
    }
...
DRM_RESULT DRM_CALL TestDRM_SST_OpenAndLockSlot(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[4]!=NULL){
        ...
        if(strcmp(argv[4],"Password")){
            memcpy(pbPassword, argv[4],min(strlen(argv[4]) + 1, DRM_SHA1_DIGEST_LEN));             
        }
        else{
            memcpy(pbPassword, g_szSecStorePassword,DRM_SHA1_DIGEST_LEN);             
        }
...
DRM_RESULT DRM_CALL TestDRM_SST_SetLockedData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argc > 1 && argv[1]!=NULL){
        cbData= ( DRM_DWORD ) strlen(argv[1])+1;
        ChkMem(pbData=(DRM_BYTE*)Oem_MemAlloc(cbData));
...
DRM_RESULT DRM_CALL TestDRM_SST_GetLockedData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argc > 1 && argv[1] != NULL )
    {
        ChkBOOL( 0 == memcmp( (char*)pbData,argv[1], min( strlen(argv[1]), cbData ) ), DRM_E_FAIL );
    }
ErrorExit:
    SAFE_OEM_FREE(pbData);
    return dr;
