...
DRM_RESULT DRM_CALL TestGenerateSyncListChallenge(long argc, __in_ecount(argc) char **argv)
{
    ...
    if( DRM_SNC_IsLicenseSyncSupported() )
    {
        ...
        if (argv[3] && pszChallenge){
            ...
            if(1&iOption){
                if (!DRMCRT_wcsnstr((DRM_WCHAR*)pszChallenge, DRMCRT_wcslen( (DRM_WCHAR*)pszChallenge ), wszConvertedKID, DRMCRT_wcslen( wszConvertedKID ) ) ){
                    ChkDR(DRM_E_FAIL);
                }
            }
            else if(2&iOption){
...
DRM_RESULT DRM_CALL TestSetHeader(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argv[1] != NULL )
    {
        ...
        {
            dstrHeader.pwszString = (DRM_WCHAR*)pbHeader;
    ...
    {
        ChkDR( DRMCRT_AtoDWORD( argv[3], DRMCRT_strlen( argv[3] ), 10, &cbHeader ) );
...
static DRM_RESULT tGetSystemTimeString( __inout_ecount(18) char* szSystemTimeString, long loffset)
{
    ...
    PUT_CHAR( szSystemTimeString, 10, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
    wTemp /= 10;
    PUT_CHAR( szSystemTimeString, 9, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
    PUT_CHAR( szSystemTimeString, 11, ':' );

    wTemp = tSysTime.wMinute;
    PUT_CHAR( szSystemTimeString, 13, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
    wTemp /= 10;
    PUT_CHAR( szSystemTimeString, 12, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
    PUT_CHAR( szSystemTimeString, 14, ':' );

    wTemp = tSysTime.wSecond;
    PUT_CHAR( szSystemTimeString, 16, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
    wTemp /= 10;
    PUT_CHAR( szSystemTimeString, 15, TST_UTL_ByteValueToASCII( wTemp % 10 ) );
...
DRM_RESULT DRM_CALL TestAddLicenseToStore(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    if( argv[3] != NULL )
    {
        ...
        {
            char pSystemTime[20];
            char *pszXMLEnding = "Z#:0]]></ACTION></ONSTORE>";
            long lOffset       = 0;
...
*/
DRM_RESULT GetLicenseEnum2(DRM_KID KID, DRM_DWORD* pdwCount, DRM_LICSTORE_CONTEXT* pLicStoreContext, DRM_LICSTOREENUM_CONTEXT** ppLicEnumContext)
{
    DRM_RESULT dr=DRM_SUCCESS;
...
DRM_RESULT DRM_CALL TestEnumLicense2(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR(GetLicenseEnum2(kid, &dwNOLicenses, &oLicStore, &pLicStoreEnumContext));
...
DRM_RESULT DRM_CALL TestGetSyncStoreEntry(long argc, __in_ecount(argc) char **argv)
{
     ...
     && DRM_SNC_IsLicenseSyncSupported() )
    {
        ...
        if ( fValidString && argc >= 3)
        {
            if ( *argv[2] != '\0' )
            {
                ChkBOOL( DRMCRT_wcsnstr( (DRM_WCHAR*)pManager->pbDRMHeaderData, DRMCRT_wcslen( (DRM_WCHAR*)pManager->pbDRMHeaderData ), dstrTest.pwszString, DRMCRT_wcslen( dstrTest.pwszString ) ) != 0, DRM_E_FAIL );
            }
            else
            {
