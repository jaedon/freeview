...
DRM_RESULT DRM_CALL CreateTraceFile( long argc, __in_ecount( argc ) char **argv )
{
    ...
    ChkBOOL( Oem_File_GetSize( fp, &cbToRead ), DRM_E_FILEREADERROR );
    ChkBOOL( cbToRead == cbExpected, DRM_E_FILEREADERROR );

    /* Read the entire trace.dat file */
    ChkBOOL( Oem_File_Read( fp, rgbRead, cbToRead, &cbRead ), DRM_E_FILEREADERROR );
...
** Returns:   DRM_SUCCESS on success.
**            DRM_E_INVALIDARG - not enough arguments
**            DRM_E_FILEREADERROR - file is not found
**********************************************************************/
