...
static DRM_RESULT SetupViewRightsContext(DRM_VIEW_RIGHTS_CONTEXT *pVRContext)
{
    ...
    memset(pVRContext, 0, sizeof(DRM_VIEW_RIGHTS_CONTEXT));
...
DRM_RESULT DRM_CALL TestAddLicense(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( TestLicGenMakeLicense( atoi( argv[0] ),
    ...
    LogBlob( "TestAddLicense", "XML License:", (DRM_BYTE*)(szLic.pwszString), szLic.cchString * SIZEOF(DRM_WCHAR) );
