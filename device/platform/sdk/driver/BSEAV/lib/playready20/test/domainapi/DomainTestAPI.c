...
DRM_RESULT _TestValidateChallenge(
    ...
    DRM_CHAR **ppchArgs)
{
    ...
    DRM_CHAR* xmlTags[] = {"soap:Body", "JoinDomain", "challenge", "Challenge", "Domain"};

    DRM_CHAR* xmlVerify[] = {"Version", "ServiceID", "AccountID", "FriendlyName", "CustomData"};
    ...
    for(index = 0; index < cbChallenge; index++) pchChallenge[index] = pbChallenge[index];
    ...
    **/
    ...
    for(index = 0; index < 5; index++)
    {
        ...
        dastrTag.cchString = DRMCRT_strlen(xmlTags[index]);
    ...
    for(index = 0; index < 5; index++)
    {
        ...
        dastrTag.cchString = DRMCRT_strlen(xmlVerify[index]);
        ChkDR(DRM_XML_GetSubNodeA(
            pchChallenge,
            &dsstrXmlNode,
            &dastrTag,
...
DRM_RESULT _TestDrm_GenerateChallenge(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN    DRM_APP_CONTEXT *pDrmContext,
    OUT    DRM_BYTE **ppbChallenge,
    OUT DRM_DWORD *pcbChallenge)
{
    /* Parameters */
    ...
    DRM_CHAR *pszFriendlyName = "PK Domains Test";
    DRM_DWORD cchFriendlyName = DRMCRT_strlen(pszFriendlyName);
    DRM_CHAR *pszCustomData = "Some Custom Data";
    DRM_DWORD cchCustomData = DRMCRT_strlen(pszCustomData);
    ...
    case 9: /* Get Custom Data */
        ...
        {
            cchCustomData = DRMCRT_strlen(pszCustomData);
        }
    ...
    case 6: /* Get Revision ID */
        ...
        {
            DomainID.m_dwRevision = atol(argv[5]);
        }
    ...
    case 2: /* See if context is NULL */
        ...
        {
            *pDrmContext->rgbOpaqueBuffer = *pDrmContext->rgbOpaqueBuffer + atoi(argv[1]);
        }
    ...
    case 11:
        ...
        {
            *pbChallenge = *pbChallenge + atoi(argv[10]);
        }
    }
...
DRM_RESULT _TestDrm_ProcessResponse(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN DRM_APP_CONTEXT *pDrmContext,
    OUT DRM_RESULT *pResult,
    OUT DRM_DOMAIN_ID *pDomainID,
...
DRM_RESULT _TestDrm_GetAdditionalResponseData(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN DRM_APP_CONTEXT *pDrmContext,
    OUT DRM_CHAR **ppbDataString,
    OUT DRM_DWORD *pcbDataString)
{
    DRM_BYTE *pbResponse = NULL;
    DRM_DWORD cbResponse = 0;
    DRM_DWORD cbResponseOffset = 0;
    ...
    case 4:
        ...
        else
        {
            ...
            */
            #pragma prefast(push)
            #pragma prefast(disable:28924) /* PreFast Complains about a redundant check of pdDataString, the check is Needed, */
            if ( pcbDataString != NULL && *pcbDataString == 0 && pbDataString == NULL )
            {
                ChkMem( pbDataString = (DRM_CHAR *) Oem_MemAlloc(1) );
            }
            #pragma prefast(pop)
        }
    }
...
DRM_RESULT _TestDrm_DomainCert_Find(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN DRM_APP_CONTEXT *pDrmContext,
    OUT DRM_BYTE **ppbCert,
    OUT DRM_DWORD *pcbCert)
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_DOMAIN_ID DomainID = {0};
    DRM_DOMAIN_ID *pDomainID = &DomainID;
    ...
    case 4: /* Get Revision ID */
        ...
        {
            DomainID.m_dwRevision = atol(argv[3]);
        }
    ...
    case 1: /* See if context is NULL */
        ...
        {
            *pDrmContext->rgbOpaqueBuffer = *pDrmContext->rgbOpaqueBuffer + atoi(argv[0]);
        }
    }
...
DRM_RESULT _TestDrm_DomainCert_InitEnum(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN DRM_APP_CONTEXT *pDrmContext,
    OUT DRM_DOMAIN_CERT_ENUM_CONTEXT *pCertContext)
{
    switch(argc)
    {
    case 2:
    ...
    return Drm_DomainCert_InitEnum(pDrmContext, pCertContext) ;
}
...
DRM_RESULT _TestDrm_DomainCert_EnumNext(
    IN long argc,
    IN __in_ecount(argc) char **argv,
    IN DRM_DOMAIN_CERT_ENUM_CONTEXT *pCertContext,
    OUT DRM_DWORD *pcCert,
    OUT DRM_DOMAINCERT_INFO *pCertInfo)
{
    DRM_DWORD cCert = 0;
    ...
    IN __in_ecount(argc) char **argv)
{
    ...
    long             args               = argc;
    DRM_CHAR        *argverif[5]; /* For challenge validation */
    DRM_DWORD        dwMaxChallengeSize = 0;

    if(argc > 12) args = 12;

    dr2 = _TestDrm_GenerateChallenge(args,
        argv,
        m_pContextManagerDomain,
        &pbChallenge,
        &cbChallenge);
    ...
    {
        dwMaxChallengeSize = atol(argv[13]);
