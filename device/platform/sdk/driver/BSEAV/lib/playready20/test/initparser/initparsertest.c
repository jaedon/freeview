        ...
        __in_ecount(argc) char **argv )
{
    ...
    else if ( strcmp( argv[0], "zerolen" ) == 0 )
    {
        ...
        TST_UTL_mbstowcs( wszXMLData, argv[0], strlen( argv[0]) + 1 ); /* assign the string to something != NULL */
    ...
    else
    {
        ...
        oEnumContext.dstrXMLData.pwszString = wszXMLData;
        oEnumContext.dstrXMLData.cchString = (DRM_DWORD)DRMCRT_wcslen( wszXMLData );
    ...
    if ( eExpectedType != DRM_INIT_UNKNOWN ) /* if expected initiator message type is present then the rest of expected data should be here too. */
    {
        ...
        {
            ChkArg( strlen( argv[3] ) + 1 <= NO_OF( wszExpectedCustomData ) );
            
            TST_UTL_mbstowcs( wszExpectedCustomData, argv[3], strlen( argv[3]) + 1 );
        }
        
        if ( strcmp( argv[4], "nullstring" ) )
        {
            ChkArg( strlen( argv[4] ) + 1 <= NO_OF( wszExpectedURL ) );
            
            TST_UTL_mbstowcs( wszExpectedURL, argv[4], strlen( argv[4]) + 1 );
        }
    }
    ...
    while ( dr != DRM_E_NOMORE && dwIndex <= dwExpectedIndex ) 
    {
        ...
        else 
        {
            ...
            */
            ...
            dstrTmpStr.cchString = (DRM_DWORD)DRMCRT_wcslen( wszExpectedCustomData );
            ...
            dstrTmpStr.cchString = (DRM_DWORD)DRMCRT_wcslen( wszExpectedURL );
            ...
            if ( argc == 7 )
            {
                ChkArg( strlen( argv[6] ) + 1 <= NO_OF( wszParam2 ) );
                
                TST_UTL_mbstowcs( wszParam2, argv[6], strlen( argv[6]) + 1 );
                switch( oInitiatorData.eType )
                {
                    case DRM_INIT_JOINDOMAIN:
                    case DRM_INIT_LEAVEDOMAIN:
                        ...
                        {                    
                            ...
                            dstrTmpStr.cchString = (DRM_DWORD)DRMCRT_wcslen( wszParam2 );        
                    ...
                    case DRM_INIT_METERING:
                        ...
                        {                    
                            ...
                            ChkDR( DRMCRT_wcsntol ( wszParam2, (DRM_DWORD)DRMCRT_wcslen( wszParam2 ), &lExpectedPackets ) );
        ...
        __in_ecount(argc) char **argv )
{
    ...
    DRM_CONST_STRING *pdstrXMLData = NULL;
    DRM_INITIATORS_ENUM_CONTEXT oEnumContext = {0};
    ...
    else
    {
        ...
        TST_UTL_mbstowcs( wszXMLData, argv[0], strlen( argv[0]) + 1 );
        dstrXMLData.pwszString = wszXMLData;
        dstrXMLData.cchString = (DRM_DWORD)DRMCRT_wcslen( wszXMLData );
    ...
    if ( argc >= 2 )
    {
        ChkArg( strlen( argv[1] ) + 1 <= NO_OF( wszInitiators ) );
        
        TST_UTL_mbstowcs( wszInitiators, argv[1], strlen( argv[1]) + 1 );

        dstrTmpStr.pwszString = wszInitiators;
        dstrTmpStr.cchString = (DRM_DWORD)DRMCRT_wcslen( wszInitiators );
        ...
        __in_ecount(argc) char **argv )
{
    ...
    */
    ...
    else
    {
        ...
        {
            if ( strlen(argv[0]) > 5 )  /* read the file name */
            {
    ...
    if ( argc == 4 )
    {
        if ( strcmp( argv[3], "nullstring" ) )
        {
            ChkArg( strlen( argv[0] ) + 1 <= NO_OF( wszCustomData ) );
            
            TST_UTL_mbstowcs( wszCustomData, argv[3], strlen( argv[3]) + 1 );

            dstrExpectedCustomData.pwszString = wszCustomData;
            dstrExpectedCustomData.cchString = (DRM_DWORD)DRMCRT_wcslen( wszCustomData );
        }
    }
