...
DRM_RESULT DRM_CALL TestScanDevicePrivateKey( DRM_APP_CONTEXT *pManagerContext )
{
    ...
    DRM_DWORD i = 0;
    PRIVKEY pvkDevice = {0};
    DRM_APP_CONTEXT_INTERNAL *pContext = (DRM_APP_CONTEXT_INTERNAL*)pManagerContext;
    DRM_BB_CONTEXT *pBBContext = NULL;

    ChkArg( pContext!=NULL );

    ChkDR( tGetBBContext( &pBBContext ) );

    /*
    ** Get the device private key
    */
    ChkDR( _GetDevicePrivkey( pBBContext, NULL, &pvkDevice ) );

    tFreeBBContext( pBBContext );
    pBBContext = NULL;

    for (i = 0; i < DRM_APP_CONTEXT_BUFFER_SIZE - SIZEOF(PRIVKEY) + 1; i++)
    {
        if ( !MEMCMP( ( (DRM_BYTE*)pContext ) + i, (DRM_BYTE*)&pvkDevice, SIZEOF( PRIVKEY ) ) )
        {
            ...
            ChkDR( DRM_E_TEST_DEVICE_PRIVATE_KEY_INCORRECTLY_STORED );
    ...
    __in                              const DRM_CONST_STRING    *f_pdstrDeviceStoreName )
{
    ...
    const char       c_rchUrlDefault[]                          = "http://FWINDIV-01.redmond.corp.microsoft.com/PlayReady/ACT/Activation.asmx?WSDL";
...
DRM_BOOL DRM_CALL LaunchDRMHelperRoutine(LPCTSTR pwszProgrm, LPCTSTR pwszParams)
{
    ...
    if (FALSE == bRtn)
    {
        Log( "Warning", "LaunchDRMHelperRoutine: ShellExecuteEx failed 0x%x", 0/*GESTATE*/ );
        goto ErrorExit;
    }
