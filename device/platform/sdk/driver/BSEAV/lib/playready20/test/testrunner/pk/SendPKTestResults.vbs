...
' see sendWMDRMPKTestResult.vbs -? for more on commandline options.
...
const constDefaultBuildNo = "2.0.0"
...
' Main Program ends
'
If g_cBVTFail > 0 or g_cBVTMissed > 0 then
	wscript.quit(5)
...
Class TestAreaResults
    ...
    public property get AreaPassed
        InitIfNeeded
        if FailedCount( constRunAll ) > 0 or MissedCount( constRunAll ) or m_fError = true then 
            AreaPassed = false
        else
            AreaPassed = true
        end if
    ...
    public property get ExpectedCount( getType )
        ...
        ExpectedCount = 0
        if ( getType and constRunBVT ) = constRunBVT then
            ExpectedCount = ExpectedCount + m_cBVT.Expected
        end if
        if ( getType and constRunDRT ) = constRunDRT then
            ExpectedCount = ExpectedCount + m_cDRT.Expected
        end if
        if ( getType and constRunFunc ) = constRunFunc then
            ExpectedCount = ExpectedCount + m_cFunc.Expected
        end if
        if ( getType and constRunPerf ) = constRunPerf then
            ExpectedCount = ExpectedCount + m_cPerf.Expected
        end if
    end property
    ...
    ''          </testinfo>
    ''          <LOGRESULTS CaseNo="Case#" Result="PASS" Ticks="#####">
    ''              <Title>Some Passing Test Case</Title>
    ''          </LOGRESULTS>
    ''          <LOGRESULTS CaseNo="Case#" Result="FAIL" Ticks="#####">
    ...
    ''      "pass"      -- if all tests pass and none were missed
    ''      "some-fail" -- if more that 50% of the tests pass or some were missed
    ...
    public function GetCellString(fRelativePath)
        ...
        if PassPercentage( constRunFunc + constRunPerf ) < 50 or FailedCount( constRunBVT + constRunDRT ) > 0 or m_fError then
            strRet = "<tr class=""fail"">"
        elseif PassPercentage( constRunAll ) < 100 or MissedCount( constRunAll ) then
        ...
        if m_fError = false then    
            strRet = strRet & vbNewLine & vbTab & "<td>" & cstr(MissedCount( constRunAll )) & "</td>" _
                ...
                & vbNewLine & vbTab & "<td>" & cstr(FailedCount( constRunFunc + constRunPerf ) ) & "</td>" _
                & vbNewLine & vbTab & "<td>" & cstr(PassedCount( constRunFunc + constRunPerf )) & "</td>" _
                & vbNewLine & vbTab & "<td>" & cstr(PassPercentage( constRunFunc + constRunPerf )) & "%</td>" _
