...
extern DRM_RESULT DRM_CALL CalculateCheckSum( __in DRM_BYTE *pbData, __in DRM_DWORD cbData, __out DRM_BYTE *pCheckSum, __in DRM_DWORD cbCheckSum, __in DRM_KID *pKID);
EXIT_PKTEST_NAMESPACE;

extern DRM_RESULT _GetRights(DRM_CONST_STRING *dstrNode, LICENSERIGHTS **ppRetRights, DRM_CONST_STRING dstrRIV);
...
static DRM_RESULT tDRM_KG_GenerateContentKey( __in DRM_CONST_STRING *pszKeySeed, __in DRM_CONST_STRING *pszKid, __out DRM_CONST_STRING *pszKey)
{
    /* Legacy: Not V3XMR, so pass FALSE */
    return tDRM_KG_GenerateContentKey2( pszKeySeed, pszKid, pszKey, FALSE );
}
                                                          ...
                                                          __in  const DRM_WCHAR *pwszOverrideKID)
{
    ...
    ChkDR(_GetRights(&dstrNode, &pRights, dstrRiv));
    ...
    else
    {
        ...
        ChkDR( _GenerateContentKey( pdstrServerSeed, &dstrKID, pcbContentKey, ppContentKey, 0, NULL, XMR_VERSION_1, FALSE ) );
    }
