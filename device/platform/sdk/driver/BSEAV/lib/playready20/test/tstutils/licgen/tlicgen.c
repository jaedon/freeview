...
#else
const DRM_CONST_STRING g_cszRights = CREATE_DRM_STRING ( 
...
DRM_RESULT TestLicGenInit(DRM_DWORD dwLicMaxLength)
{
    ...
    MEMSET(g_rgbCryptoContext, 0, DRM_PKCRYPTO_CONTEXT_BUFFER_SIZE);
    
    MEMSET(g_keys, 0, SIZEOF(DRM_CONST_STRING) * MAX_LICGEN_KEYS);
...
DRM_RESULT TestLicGenSetKeys(const char *setWhat, const char *szKey)
{
     ...
     && strcmp( szKey, "RANDOM" ) == 0 )
    {
        ...
        */
        ...
        g_keys[i].pwszString = (DRM_WCHAR*)Oem_MemAlloc( cchRandomID * SIZEOF( DRM_WCHAR ) );
...
#pragma prefast(pop)
    ...
    dr = DRM_KG_GenerateContentKeyB64((DRM_WCHAR*)pszKeySeed->pwszString, pszKeySeed->cchString, (DRM_WCHAR*)pszKid->pwszString, cchKID, eDRM_RC4_CIPHER, DRMCIPHERKEYLEN_RC4, NULL, &cchKey, pbScratchKeySeed, pszKeySeed->cchString * SIZEOF(DRM_WCHAR));
...
static DRM_RESULT _CalculateChecksum(DRM_BYTE *pbData, DRM_DWORD cbData, DRM_CONST_STRING *pStr)
{
    DRM_RESULT dr=DRM_SUCCESS;
...
#if SIXTEEN_BIT_ADDRESSING
    /*
    ** The first byte needs to be the length of contentKey, and since we cannot address the byte
    ** on a 16-bit machine, we have to do shift and OR operation here
    */
    f_pbPlain[0] = ((DRM_BYTE)*f_pcbPlain << 8) | pbPlain[0];
#else
...
#if SIXTEEN_BIT_ADDRESSING
    /*
    ** Using pbContentKey instead of pbPlain+1 to avoid incorrect signature on 16-bit, since pbPlain+1
    ** would start with the second byte of the Content key, leaving out the first byte
    */
...
static DRM_RESULT AddSignature(DRM_STRING *szData, DRM_CONST_STRING *pszSigningPrivateKey, DRM_DWORD dwOption)
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT AddSymSignature(DRM_STRING *szData, DRM_ANSI_STRING *pdastrSlk)
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT AddRevocationData(DRM_WCHAR *wszType, DRM_CONST_STRING *pdstrValue)
{
    DRM_RESULT dr;
