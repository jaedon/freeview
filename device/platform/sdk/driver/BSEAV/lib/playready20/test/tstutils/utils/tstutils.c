...
**
*********************************************************/
DRM_LONG DRM_CALL TST_UTL_CompareSystemTime( __in DRMSYSTEMTIME *f_pLeftTime,
                                            __in DRMSYSTEMTIME *f_pRightTime )
{
...
DRM_RESULT DRM_CALL TST_UTL_MakeExplicitPathString( __in_z const DRM_CHAR *f_pszDir, __in_z const DRM_CHAR *f_pszFileName, __out_ecount(1) DRM_CONST_STRING *f_pdstrResult )
{
    ...
    cchFullPath = DRMCRT_strlen( f_pszDir ) + DRMCRT_strlen( f_pszFileName ) + 1;
                                                                         ...
                                                                         DRM_BOOL    f_fFuzzPath )
{
    ...
    if ( f_pszFolder != NULL )
    {
        ...
        if ( cchFolder                >  0
            && f_pszFolder[cchFolder-1] != g_chPathSeparator )
        {
            PUT_CHAR( pszPath, cchStored, g_chPathSeparator );
            cchStored++;
        }
    }
...
DRM_RESULT DRM_CALL MakeDRMString(DRM_CONST_STRING *pwszStr, const char *szStr)
{
    ...
    if ( szStr )
    {
        ...
        ChkMem( temp = Oem_MemAlloc( CB_NATIVE_BYTE * ( native_length + 1 ) ) );
        ChkDR( DRM_STR_StringCchCopyA( temp, native_length + 1, szStr ) );
        fnative = temp;
        ...
        ChkMem( pwszStr->pwszString = ( const DRM_WCHAR* )Oem_MemAlloc( CB_NATIVE_BYTE * native_length ) );
        f_pad.m_cch = native_length+1;
        f_pad.m_ich = 0;
           /*Convert Ansi to UNICODE using the dev function rather than TST_UTL_mbstowcs*/
        DRM_UTL_PromoteASCIItoUNICODE((const DRM_CHAR *)temp,&f_pad,(DRM_STRING *)pwszStr);
        pwszStr->cchString = (DRM_DWORD)(native_length);
        SAFE_OEM_FREE(temp);
#else
        pwszStr->cchString = (DRM_DWORD)TST_UTL_mbstowcs(NULL, szStr, strlen(szStr));
...
int DRM_CALL CompDRMString(DRM_CONST_STRING *pwszStr, const char *szStr)
{
    DRM_RESULT       dr   = DRM_SUCCESS;
    int              iRet = 0;
...
DRM_RESULT DRM_CALL TST_UTL_AtoUI64(
    __in_ecount( f_cchString ) DRM_CHAR   *f_pszString,
    ...
    __out_ecount( 1 )          DRM_UINT64 *f_pui64Value )
{
    ...
    if ( ichTemp < f_cchString && *pszTemp == '-' )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( f_pszString,
                                ichTemp,
                                ( ichTemp > 2 && ( f_pszString[1] == 'X' || f_pszString[1] == 'x' ) ) ? 16 : 10,
                                &dwHigh ) );

        ichTemp = f_cchString - ichTemp - 1;
        ChkDR( DRMCRT_AtoDWORD( pszTemp,
                                ichTemp,
                                ( ichTemp > 2 && ( pszTemp[1] == 'X' || pszTemp[1] == 'x' ) ) ? 16 : 10,
                                &dwLow ) );
    }
    else
    {
        ChkDR( DRMCRT_AtoDWORD( f_pszString,
                                f_cchString,
                                ( f_cchString > 2 && ( f_pszString[1] == 'X' || f_pszString[1] == 'x' ) ) ? 16 : 10,
                                &dwLow ) );
    }
...
DRM_DWORD DRM_CALL TST_UTL_NumberToAnsiString(
    ...
    IN      DRM_DWORD   f_dwRadix,
    IN      DRM_DWORD   f_iWidth,
    IN      DRM_DWORD   f_cbString,
    OUT __out_bcount_z(f_cbString) DRM_CHAR   *f_pszString )
{
    DRM_DWORD  iString = 0;
    ...
    {
        PUT_CHAR( f_pszString, iString, TST_UTL_ByteValueToASCII( ( DRM_BYTE ) ( dwTemp % f_dwRadix ) ) );
        dwTemp /= f_dwRadix;
        iString++;
        if ( dwTemp == 0 )
        {
    ...
    {
        DRM_CHAR temp = GET_CHAR(f_pszString, iString);
        PUT_CHAR( f_pszString, iString, GET_CHAR(f_pszString, iWidth - 1 - iString ) );
        PUT_CHAR( f_pszString, iWidth - iString - 1, temp);
        iString++;
    }
    ...
    __in DRM_DWORD f_cchMaxCount)
{
    ...
    ChkArg( cchLength > 0 );
    chCurrent = GET_CHAR( f_pszSrc, iCurrent );
    while( iCurrent < cchLength && chCurrent != '\0' )
    {
        PUT_CHAR( f_pszDst, iCurrent, chCurrent );
        iCurrent++;
        chCurrent = GET_CHAR( f_pszSrc, iCurrent );
    }
...
**
*****************************************************************************/
DRM_RESULT DRM_CALL TST_UTL_LogLicenseStateData( const DRM_CHAR *szOutputPrefix, DRM_LICENSE_STATE_DATA *pDRMLicenseStateData )
{
    DRM_RESULT dr           = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL TST_UTL_BinaryFileCompare(
    IN const DRM_CHAR* f_pszFolder1,
    IN const DRM_CHAR* f_pszPath1,
    IN const DRM_CHAR* f_pszFolder2,
    IN const DRM_CHAR* f_pszPath2 )
{
    DRM_RESULT  dr        = DRM_SUCCESS;
    DRM_DWORD   cbFile1   = 0;
    DRM_DWORD   cbFile2   = 0;
    ...
    ChkBOOL( LoadTestFile( f_pszFolder1, f_pszPath1, &pbBuffer1, &cbFile1 ), DRM_E_FILEREADERROR );
    ChkBOOL( LoadTestFile( f_pszFolder2, f_pszPath2, &pbBuffer2, &cbFile2 ), DRM_E_FILEREADERROR );
...
int TST_UTL_mbstowcs (DRM_WCHAR *destination,char *source,int  length){
    ...
    if(destination != NULL){
        ...
        testDRM_16B_Pack8BitBytes(source,strlen(source)+1,NULL,DRM_PACK_BYTES_IN_PLACE);
        DRM_UTL_PromoteASCIItoUNICODE((const DRM_CHAR *)source,&f_pad,&f_pdstrout);
        }
    else{
...
static DRM_RESULT _ConvertV2ToPlayReady(
    __in_bcount(f_cbV2Header)                     DRM_BYTE  *f_pbV2Header,
...
static DRM_RESULT _ConvertHeader(
    ...
    __in                      DRM_DWORD         f_cbHeader,
    __in                      OEM_FILEHDL       f_hOutputFile,
    __in                      DRM_CONST_STRING *f_pdstrSilentURL,
    __in                      DRM_CONST_STRING *f_pdstrNonSilentURL,
    __in                      DRM_CONST_STRING *f_pdstrDSID,
    __inout                   DRM_DWORD        *f_pcbCurrHeader,
...
static DRM_RESULT _ConvertASFHeader(
    __in  DRM_CONST_STRING *f_pdstrInput,
    __in  DRM_CONST_STRING *f_pdstrOutput,
    __in  DRM_CONST_STRING *f_pdstrSilentURL,
    __in  DRM_CONST_STRING *f_pdstrNonSilentURL,
    __in  DRM_CONST_STRING *f_pdstrDSID,
    ...
    __out DRM_DWORD        *f_pdwNewObjCount )
{
    ...
    */
    cbHeader = (DRM_DWORD) *(DRM_WCHAR*)( pbHeader + SIZEOF( DRM_GUID ) );
    dwObjCount = (DRM_DWORD) *(DRM_WCHAR*)( pbHeader + SIZEOF( DRM_GUID ) + SIZEOF( DRM_INT64 ) );
    ...
    __in DRM_DWORD         f_dwNewObjCount )
{
    ...
    */
    Oem_File_SetFilePointer( hInputFile, f_cbOldHeader, OEM_FILE_BEGIN, NULL );
    Oem_File_SetFilePointer( hOutputFile, f_cbNewHeader, OEM_FILE_BEGIN, NULL );
...
DRM_RESULT TST_UTL_IsConditionTrue( __out DRM_BOOL *f_pfConditionTrue, __in long f_lConditionCount, __in_ecount( f_lConditionCount ) const char **f_ppszConditions )
{
    ...
    *f_pfConditionTrue = FALSE;
    ...
    for ( iCondition = 0; iCondition < f_lConditionCount; iCondition++ )
    {
        ...
        if ( 0 == strcmp( pszCondition, pszChk ) )
        {
            if ( fChk )
            {
                fConditionMet = TRUE;
...
DRM_RESULT DRM_CALL TST_UTL_ConvertFileFromWMDRMToPlayReady(
    IN DRM_CONST_STRING *f_pdstrInput,
    IN DRM_CONST_STRING *f_pdstrOutput,
    IN DRM_CONST_STRING *f_pdstrSilentURL,
    IN DRM_CONST_STRING *f_pdstrNonSilentURL,
    IN DRM_CONST_STRING *f_pdstrDSID )
{
    DRM_RESULT dr            = DRM_SUCCESS;
    ...
    __deref_out DRM_BYTE  **f_ppbBuffer)
{
    ...
    dasstrBuffer.m_cch = DRMCRT_strlen( f_szSource );
...
**
*********************************************************/
DRM_RESULT DRM_CALL TST_UTL_CopyFileToDRMDirectoryA( __in_z DRM_CHAR *f_pszFile )
{
    DRM_RESULT       dr                                               = DRM_SUCCESS;
...
**
*********************************************************/
DRM_RESULT DRM_CALL TST_UTL_CopyFileFromDRMDirectoryA( __in_z DRM_CHAR *f_pszFile )
{
    DRM_RESULT       dr                                               = DRM_SUCCESS;
    DRM_CHAR         rgszRunDirectory[DRM_MAX_PATH]                   = { 0 };
    DRM_WCHAR        rgwszDRMDirectory[GET_DRM_FILE_NAME_PATH_LENGTH] = { 0 };
