...
const DRM_WCHAR * DRM_CALL TST_UTL_wcsistr(__in const DRM_WCHAR *f_pwszStr, __in const DRM_WCHAR *f_pwszStrSearch)
{
    ...
    dwLenStr = DRMCRT_wcslen( f_pwszStr );
    dwLenSearch = DRMCRT_wcslen( f_pwszStrSearch );
                                    ...
                                    __in const DRM_CONST_STRING *f_pdstrSource )
{
    ...
    {
        f_pdstrTarget->pwszString = ( DRM_WCHAR* )Oem_MemAlloc( len + SIZEOF( DRM_WCHAR ) );  /* allocate size + 1. */
