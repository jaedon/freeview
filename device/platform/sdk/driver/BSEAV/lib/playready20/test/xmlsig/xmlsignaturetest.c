...
static DRM_XML_KEYINFO_TYPE _GetKeyInfoFormat(
    IN     DRM_DWORD  f_cbKeyInfoFormat )
{
...
static DRM_RESULT _GetECC256KeysImitation(
    OUT PUBKEY_P256 *f_poDevPubKey,
    OUT PRIVKEY_P256 *f_poDevPrivKey  )
{
...

{
    ...
    ChkDR( DRM_XMLSIG_GetSimpleECC256KeyInfoNodeCharCount(
            _GetECC256KeyFormat( atol( argv[0] ) ),
            &cchSize ) );
...

{
    ...
    oXMLKey.m_eKeyInfoType = _GetKeyInfoFormat( atol( argv[0] ) );
    oXMLKey.m_eKeyFormat = _GetECC256KeyFormat( atol( argv[1] ) );
    ...
    IN __in_ecount(argc) char **argv )
{
    ...
    oXMLKey.m_eKeyInfoType = _GetKeyInfoFormat( atol( argv[1] ) );
    oXMLKey.m_eKeyFormat = _GetECC256KeyFormat( atol( argv[2] ) );
