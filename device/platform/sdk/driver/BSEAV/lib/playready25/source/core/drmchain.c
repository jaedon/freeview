...
#define DRM_BUILDING_DRMCHAIN_C
    ...
    __in    const DRM_VOID                    *f_pv )
{
    ...
    for( ; iRight > 0; )
    {
        ...
        iRight--;
        f_pFFLicense->pLicEval->fLicenseFromPRND = f_pFFLicense->rgfLicenseFromPRND[f_pFFLicense->dwChainDepth];
        ...
        ChkBOOL( fEvalResult, DRM_E_RIGHTS_NOT_AVAILABLE );
    ...
    */
    else
    {
        ...
        ChkBOOL(
         ...
         || dwXMRVersion == XMR_VERSION_3, DRM_E_INVALID_LICENSE);
    ...
    */
    if ( f_pFFLicense->pBindingInfo[0].m_fXMRLicense )
    {
        ...
        */
        ...
        {
            ChkDR( DRM_E_LICENSE_MISMATCH );   /* we are told to ignore this type of license. */
        }
        ...
        */
        ...
        {
            if( f_pFFLicense->rgfLicenseFromPRND[f_pFFLicense->dwChainDepth] )
            {
                /* Ignore metering for PRND leaf license, fail for PRND simple/root license */
        ...
        */
        if ( f_pFFLicense->dwChainDepth != DRM_MAX_LICENSE_CHAIN_DEPTH - 1 )
        {
            ...
            else if ( XMR_IS_UPLINKX_VALID( f_pFFLicense->pLicEval->plicenseXMR ) )
            {
                ...
                {
                    DRM_DWORD cb = 0;
                    ChkDR( DRM_DWordMult( f_pFFLicense->pLicEval->plicenseXMR->containerOuter.containerKeys.UplinkX.cEntries, DRM_AES_KEYSIZE_128, &cb ) );
...
static DRM_RESULT _RemapEvaluationErrors( __in DRM_RESULT drOrig, __in DRM_RESULT drDefault )
{
    ...
    */
    if (   drOrig == DRM_E_DOMAIN_BIND_LICENSE
        ...
        || drOrig == DRM_E_UPLINK_LICENSE_NOT_FOUND )
    {
        return drOrig;
    }
    else
...
**
** Parameters:
**      [f_pCipherContext]    : If valid key is found, cipher context is initialized.
    ...
    __in    const DRM_VOID                    *f_pv )
{
    ...
    if( f_pCipherContext != NULL )
    {
        dr = DRM_APPP_CipherKeySetup( f_pFFLicense->pLicEval->plicenseXMR,
        ...
        /* Cipher key setup should have failed if this is a root license. */
        AssertChkArg( !XMR_IS_UPLINK_KID_VALID( f_pFFLicense->pLicEval->plicenseXMR ) && !XMR_IS_UPLINKX_VALID( f_pFFLicense->pLicEval->plicenseXMR ) );
    }
    ...
    /* Give ui64RealTimeExpiration to cipher context so it can be evaluated during decrypt calls */
    if( f_pCipherContext != NULL )
    {
        f_pCipherContext->ui64RealTimeExpiration = f_pFFLicense->ui64RealTimeExpiration;
        f_pCipherContext->pOEMContext = f_pFFLicense->pLicEval->pcontextBBX->pOEMContext;
    }

ErrorExit:
    if ( DRM_FAILED( dr ) && f_pCipherContext != NULL )
    {
...
** Arguments:
**           f_pFFLicense   -- DRM_FFLICENSE the license
** Returns:
    ...
    __in    const DRM_VOID                    *f_pv )
{
    ...
    *f_pdwChainDepth = 0;
    ...
    if ( f_pFFLicense->dwChainDepth == 0 )
    {
        ...
        */
        ...
        f_pFFLicense->ui64RealTimeExpiration = DRM_UI64HL(MAX_UNSIGNED_TYPE(DRM_DWORD),MAX_UNSIGNED_TYPE(DRM_DWORD));
    }
    ...
    */
    ...
    if( DRM_UI64Les( f_pFFLicense->pLicEval->ui64RealTimeExpiration, f_pFFLicense->ui64RealTimeExpiration ) )
    {
        f_pFFLicense->ui64RealTimeExpiration = f_pFFLicense->pLicEval->ui64RealTimeExpiration;
    }
    ...
    if( DRM_SNC_IsLicenseSyncSupported() && dr == DRM_E_LICENSE_EXPIRED )
    {
    ...
    */
    ...
    if ( fChained )
    {
        DRM_RESULT drCached = DRM_E_UPLINK_LICENSE_NOT_FOUND;

        /*
        ** We're at the maximum chain depth that we support
        */
        ChkBOOL( f_pFFLicense->dwChainDepth + 1 < DRM_MAX_LICENSE_CHAIN_DEPTH, DRM_E_LICENSE_NOT_FOUND );
        ...
        */
        ...
        while( TRUE )
        {
            DRM_BOOL fLicenseFromHDSSaved = FALSE;
                ...
                */
                if ( f_pFFLicense->pLicStoreEnum[f_pFFLicense->dwChainDepth + 1].eMode == eDRM_LICENSE_STORE_MODE_NONE )
                {
                    ...
                    if( DRM_SNC_IsLicenseSyncSupported() )
                    {
                        ...
                        */
                        if ( DRM_E_SYNC_ENTRY_NOT_FOUND == DRM_SNC_GetSyncStoreEntry(
            ...
            f_pFFLicense->rgfLicenseFromPRND[f_pFFLicense->dwChainDepth + 1] =
                ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth + 1] == f_pFFLicense->pLicStorePRND );
            f_pFFLicense->rgfLicenseFromEST[f_pFFLicense->dwChainDepth + 1] =
                ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth + 1] == f_pFFLicense->pLicStoreEST );

            /* License is from persisted storage unless it's from PRND or NST stores */
            fLicenseFromHDSSaved = f_pFFLicense->pLicEval->fLicenseFromHDS;
            DRMASSERT( fLicenseFromHDSSaved ==
                ( ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth] != f_pFFLicense->pLicStorePRND )
               && ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth] != f_pFFLicense->pLicStoreNST  )
               && ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth] != f_pFFLicense->pLicStoreEST  ) ) );
            f_pFFLicense->pLicEval->fLicenseFromHDS =
                ( ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth + 1] != f_pFFLicense->pLicStorePRND )
               && ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth + 1] != f_pFFLicense->pLicStoreNST  )
               && ( f_pFFLicense->rgpLicStoreCurrent[f_pFFLicense->dwChainDepth + 1] != f_pFFLicense->pLicStoreEST  ) );
            ...
            f_pFFLicense->pLicEval->fLicenseFromHDS = fLicenseFromHDSSaved;
    ...
    else
    {
        ...
        */
        ...
        {
            ChkDR( _ProcessEndOfChain( f_pCipherContext, f_pFFLicense, f_pfnPolicyCallback, f_pv ) );
        }
    }
    ...
    for( ; iCount < f_dwChainDepth ; iCount++ )
    {
        ...
        */
        ...
        for( ; iRights > 0; )
        {
            ...
            iRights--;
            f_pFFLicense->pLicEval->fLicenseFromPRND = f_pFFLicense->rgfLicenseFromPRND[iCount];
            ChkDR( DRM_LEVL_PerformOperations( f_pFFLicense->pLicEval,
                                               DRM_LICENSE_EVAL_ACTION,
            ...
            ChkBOOL( !fExisted || fEvalResult, DRM_E_RIGHTS_NOT_AVAILABLE );
        }
...
static DRM_RESULT _ProcessExtensionsXMR(
    __in    const DRM_XMR_EXPLICIT_ANALOG_VIDEO_PROTECTION   *f_pxmrVideoRestrictions,
    __in    const DRM_XMR_EXPLICIT_DIGITAL_AUDIO_PROTECTION  *f_pxmrAudioRestrictions,
    ...
    __in          DRM_STACK_ALLOCATOR_CONTEXT                *f_pstack )
{
    ...
    const DRM_XMR_VIDEO_OUTPUT_CONFIGURATION_LIST *plistVideoOutputConfiguration = NULL;
    const DRM_XMR_VIDEO_OUTPUT_CONFIGURATION_LIST *plistAudioOutputConfiguration = NULL;
