...
#define DRM_BUILDING_DRMHDRBUILDER_C
#include <drmtypes.h>
...
static DRM_RESULT _SignTheHeader(
    ...
    const DRM_CONST_STRING   *pdstrPrivKey,
...
DRM_API DRM_RESULT DRM_CALL DRM_HB_GetContextSize(
    ...
    __out                                     DRM_DWORD *pcbHBContext )
{
    DRM_RESULT dr=DRM_SUCCESS;
    const _HBContext *pHB=(const _HBContext*)pbHBContext;
    ...
    __out                                     DRM_DWORD          *pcbHeaderXML )
{
    ...
    if (pHB->eState!=eHBAddedData && pHB->eState!=eHBSigning)
    {
        if (!pHB->fKIDAdded)
        {
            ChkDR(DRM_E_NO_KID_IN_HEADER);
        }
        else if (!pHB->fLAINFOAdded)
        {
            ChkDR(DRM_E_NO_LAINFO_IN_HEADER);
        }
        else if (!pHB->fChecksumAdded)
        {
            ChkDR(DRM_E_NO_CHECKSUM_IN_HEADER);
        }
        else
        {
...
DRM_API DRM_RESULT DRM_CALL DRM_HB_SetKeyID(
    __in_bcount( SIZEOF( _HBContext ) )       DRM_BYTE         *pbHBContext,
    __in                                const DRM_CONST_STRING *pdstrKID )
{
    DRM_RESULT dr=DRM_SUCCESS;
    _HBContext *pHB=(_HBContext*)pbHBContext;
    ...
    if ( pHB->fKIDAdded )
    {
        ChkDR(DRM_E_DUPLICATED_HEADER_ATTRIBUTE);
    }
...
DRM_API DRM_RESULT DRM_CALL DRM_HB_SetChecksum(
    __in_bcount( SIZEOF( _HBContext ) )       DRM_BYTE         *pbHBContext,
    __in                                const DRM_CONST_STRING *pdstrContentKey )
{
    DRM_RESULT dr = DRM_SUCCESS;
    _HBContext      *pHB        = (_HBContext*)pbHBContext;
    DRM_DWORD        cchEncoded = 0;
    ...
    ChkBOOL( !pHB->fChecksumAdded, DRM_E_DUPLICATED_HEADER_ATTRIBUTE);
    ChkBOOL( pHB->fKIDAdded, DRM_E_NO_KID_IN_HEADER ); /* KID is now required due to AES checksum algorithm */
