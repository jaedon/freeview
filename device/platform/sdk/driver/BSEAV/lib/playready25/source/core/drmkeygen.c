...
#define DRM_BUILDING_DRMKEYGEN_C
    ...
    __inout                            DRM_DWORD *f_pcchKeyseed )
{
    ...
    *f_pcchKeyseed = DRM_KEYSEED_BASE64_LEN+1;
    ...
    for (i = 0; i < DRM_KEYSEED_BASE64_LEN; i++)
    {
        ...
        DRM_BYTE val = (DRM_BYTE)( (GET_BYTE(rgbKeyseed, i)) % 62 );
        ...
        f_pwszKeyseed[i] = (DRM_WCHAR)WCHAR_CAST(val);
    }
...
{
    DRM_SHA_CONTEXT  sha1;
    DRM_SHA_CONTEXT  sha2;
    DRM_RESULT       dr   = DRM_SUCCESS;
    DRM_DWORD        i    = 0;
    DRM_BYTE         rgbHashA[SHA256_DIGEST_SIZE_IN_BYTES];
    DRM_BYTE         rgbHashB[SHA256_DIGEST_SIZE_IN_BYTES];
    DRM_BYTE         rgbHashC[SHA256_DIGEST_SIZE_IN_BYTES];
    ...
    OEM_SECURE_MEMCPY( &sha2, &sha1, SIZEOF( DRM_SHA_CONTEXT ) ); /* Optimization: Save off current SHA state for use in hash B */
    ...
    OEM_SECURE_MEMCPY( &sha1, &sha2, SIZEOF( DRM_SHA_CONTEXT ) ); /* Optimization: Save off current SHA state for use in hash C */
    ...
    */
    ...
    {
        f_pbContentKey[i] = (DRM_BYTE)( rgbHashA[i] ^ rgbHashA[i + DRM_AES_KEYSIZE_128]
    ...
    __inout                            DRM_DWORD          *f_pcchPubKey )
{
    ...
    ChkDR( OEM_ECC_GenKeyPair_P160( f_pCrypto->rgbCryptoContext, &pub, &priv ) );
