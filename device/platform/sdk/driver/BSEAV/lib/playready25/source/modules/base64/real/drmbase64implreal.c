         ...
         ichSource -= CCH_B64_IN_QUARTET)
    {
        ...
        default:
            PUT_BYTE(f_pbDestination,
                     ibInsert  + --cchDecode,
                     (DRM_BYTE)( ((GET_BYTE(rgbOutput, 2) & 0x03) << 6) |   GET_BYTE(rgbOutput, 3) ));
            __fallthrough;
        case 3:
            PUT_BYTE(f_pbDestination,
                     ibInsert  + --cchDecode,
                     (DRM_BYTE)( ((GET_BYTE(rgbOutput, 1) & 0x0F) << 4) | ((GET_BYTE(rgbOutput, 2) & 0x3C) >> 2) ));
            __fallthrough;
        case 2:
            PUT_BYTE(f_pbDestination,
                     ibInsert  + --cchDecode,
                     (DRM_BYTE)( (GET_BYTE(rgbOutput, 0)          << 2) | ((GET_BYTE(rgbOutput, 1) & 0x30) >> 4) ));
        }
    }
    ...
    __in                               DRM_DWORD  f_fFlags )
{
    ...
    */
    ...
    while (TRUE)
    {
        ...
        const DRM_BYTE uc1 = (DRM_BYTE)( ((iInput + 1) < f_cbSource) ? GET_BYTE(f_pbSource, iInput + 1) : 0 );
        const DRM_BYTE uc2 = (DRM_BYTE)( ((iInput + 2) < f_cbSource) ? GET_BYTE(f_pbSource, iInput + 2) : 0 );
    ...
    __in                               DRM_DWORD   f_dwFlags )
{
    ...
    */
    ...
    while (TRUE)
    {
        ...
        f_pwszEncoded[iOutput]     = (DRM_WCHAR)WCHAR_CAST(GET_BYTE(pszMapping,   uc0 >> 2));
        f_pwszEncoded[iOutput + 1] = (DRM_WCHAR)WCHAR_CAST(GET_BYTE(pszMapping, ((uc0 << 4) & 0x30) | ((uc1 >> 4) & 0xf)));
        f_pwszEncoded[iOutput + 2] = (DRM_WCHAR)WCHAR_CAST(GET_BYTE(pszMapping, ((uc1 << 2) & 0x3c) | ((uc2 >> 6) & 0x3)));
        f_pwszEncoded[iOutput + 3] = (DRM_WCHAR)WCHAR_CAST(GET_BYTE(pszMapping,                               uc2 & 0x3f));
