...
#define DRM_BUILDING_DRMDEVICESTORE_C
...
static DRM_RESULT _CalculateKeyPassword(
    ...
    __in                              const DRM_ID *f_pid,
    __out_ecount(DRM_SHA1_DIGEST_LEN)       DRM_BYTE f_rgbPassword[DRM_SHA1_DIGEST_LEN] )
{
    /* Assuming DRM_BBX_HashValue checks arguments */
    return DRM_BBX_HashValue( (const DRM_BYTE*)f_pid, SIZEOF ( DRM_ID ), f_rgbPassword, f_pcontextBBX);
}

PREFAST_PUSH_DISABLE_EXPLAINED(__WARNING_COUNT_REQUIRED_FOR_WRITABLE_BUFFER, "f_pbBase length defined by f_cbBase and f_ibOffset" )
static DRM_RESULT _WriteDeviceStoreEntryItem(
    __in_ecount(f_cbName)                 const DRM_BYTE  *f_pbName,
    __in                                  const DRM_DWORD  f_cbName,
    __in_ecount(f_cbContent)              const DRM_BYTE  *f_pbContent,
    __in                                  const DRM_DWORD  f_cbContent,
    __inout_ecount(f_cbBase - f_ibOffset)       DRM_BYTE  *f_pbBase,
    __in                                        DRM_DWORD  f_cbBase,
    __in                                        DRM_DWORD  f_ibOffset )
PREFAST_POP
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT _ReadDeviceStoreEntryItem(
    __out_ecount(1)       DRM_DWORD *f_pibName,
    __out_ecount(1)       DRM_DWORD *f_pcbName,
    __out_ecount(1)       DRM_DWORD *f_pibContent,
    __out_ecount(1)       DRM_DWORD *f_pcbContent,
    __in            const DRM_BYTE  *f_pbBase,
    __inout               DRM_DWORD *f_pibOffset,
...
static DRM_RESULT _GetDirtyFlag( const _DeviceHandle *pDeviceHandle, DRM_DWORD dwIndex, DRM_BOOL *pfDirty )
{
    DRM_DWORD   i = 0;
    DRM_RESULT  dr = DRM_SUCCESS;
    ...
    __out    DRM_DEVICESTORE_HANDLE       *f_pDeviceHandle )
{
    ...
    */
    ...
    /* dr should be DRM_E_DST_SLOT_NOT_FOUND */
    ...
    __out   DRM_DEVICESTORE_HANDLE       *f_pDeviceHandle )
{
    ...
    *f_pDeviceHandle = NULL;
    ...
    dr = DRM_SST_GetData(   pDeviceStore->pSecureStoreContext,
                            ...
                            pbData,
                            &cbData );
    if( DRM_E_FILENOTFOUND == dr || DRM_E_DST_SLOT_NOT_FOUND == dr )
    {
...
{
    DRM_RESULT                    dr = DRM_SUCCESS;
    const _DeviceHandle          *pDeviceHandle = (const _DeviceHandle *)f_hDeviceHandle;
    ...
    __in     DRM_BYTEBLOB                     f_AttrValue )
{
    ...
    else
    {
        ...
        */
        ...
        ChkDR( _WriteDeviceStoreEntryItem( f_AttrName.pbBlob, f_AttrName.cbBlob, f_AttrValue.pbBlob, f_AttrValue.cbBlob, pbNew, cbNewEntry, pDeviceHandle->cbEntry ) );
