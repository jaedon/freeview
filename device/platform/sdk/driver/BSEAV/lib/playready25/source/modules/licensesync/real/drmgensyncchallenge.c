...
#define DRM_BUILDING_DRMGENSYNCCHALLENGE_C
...
static DRM_DWORD _RequiredCharsForChallenge(DRM_DWORD cKIDsIn)
{
...
/*********************************************************************
 *                    PRIVATE FUNCTION _SyncCountTest
 *
 * purpose: compare license count to threshold criterion
 *********************************************************************/

static DRM_RESULT _SyncCountTest(
    __in        DRM_DWORD                  cMaxCount,
    __in  const DRM_LICENSE_STATE_DATA    *plsd,
    __out       DRM_BOOL                  *pfRequired)
{
...
/*********************************************************************
 *                    PRIVATE FUNCTION _SyncExpirationTest
 *
...
static DRM_RESULT _SyncExpirationTest(
    ...
    else
    {
        ...
        */
        case DRM_LICENSE_STATE_COUNT:
            ChkDR(_SyncCountTest (cMaxCountIn, plsd, &fSyncRequired));
            break;

        case DRM_LICENSE_STATE_UNTIL:
            ChkDR(_SyncExpirationTest (pOEMContext, cMaxHoursIn, plsd, 0, &fSyncRequired, pui64ThreshholdTime));
            break;
        ...
        */
        case DRM_LICENSE_STATE_FROM_UNTIL:
            ChkDR(_SyncExpirationTest (pOEMContext, cMaxHoursIn, plsd, 1, &fSyncRequired, pui64ThreshholdTime));
            break;
    ...
    __in DRM_LICENSE_STATE_DATA *f_plsd )
{
    ...
    else
    {
        ...
        {
        case DRM_LICENSE_STATE_UNTIL:
            _SyncExpirationTest (f_pOEMContext, 0, f_plsd, 0, &fSyncRequired, &ui64ThresholdTime);
            break;

        /* don't sync yet if a license's FROM time hasn't started yet */

        case DRM_LICENSE_STATE_FROM_UNTIL:
            _SyncExpirationTest (f_pOEMContext, 0, f_plsd, 1, &fSyncRequired, &ui64ThresholdTime);
            break;
...
static DRM_RESULT _WriteKIDToList(
    ...
    __inout       _XMBContext *f_pcontextXMB )
{
    DRM_RESULT       dr      = DRM_SUCCESS;
    ...
    __out           DRM_BOOL                  *f_pfExpired )
{
    ...
    *f_pfExpired = TRUE;
    ...
    if( dr == DRM_E_SYNC_ENTRY_NOT_FOUND )
    {
