...
#define DRM_BUILDING_DRMSYNCSTORE_C
    ...
    if( f_pcchContentHeader != NULL )
    {
        ...
        if( cbSlot == SIZEOF( DRM_DWORD ) )
        {
             ...
             && f_pwszContentHeader != NULL )
            {
                ...
                ChkBOOL( cbSlot == ( *f_pcchContentHeader * SIZEOF ( DRM_WCHAR ) ), DRM_E_FILE_READ_ERROR );
            }
            else
            {
    ...
    __in           DRM_BOOL                   f_fCreateIfNotExisting )
{
    ...
    */
    ...
    if( f_pdstrContentHeader != NULL )
    {
        ...
        FIX_ENDIAN_DWORD(cchContentHeader);     /* Use consistent (little-endian) byte order on disk */
        ChkDR( DRM_DST_SlotWrite( pcontextSync->pDatastore,
                                  &pcontextSync->contextDSTSlot,
                                  SIZEOF( DRM_DWORD ),
                      (DRM_BYTE *)&cchContentHeader,
                                  &cbSlot));
        FIX_ENDIAN_DWORD(cchContentHeader);     /* Restore to native byte order */
    ...
    case DRM_SYNC_UPDATE_AGGREGATE:
        if( f_pcontextViewRightsIn != NULL )
        {
            ...
            */
            ...
            {
                (DRM_VOID)DRM_SNC_SetStoreDataForKID( f_pcontextSYN, f_pkid, NULL, &lsd, (const DRM_STRING*)f_pdstrContentHeader, TRUE );
            }
        }
        else
        {
            ...
            */
            (DRM_VOID)DRM_SNC_SetStoreDataForKID(f_pcontextSYN, f_pkid, NULL, NULL, (const DRM_STRING*)f_pdstrContentHeader, TRUE);
        }
        break;
