...
#define DRM_BUILDING_DRMPERFORMANCEREAL_C
...
static DRM_CHAR *s_rgpszDRM_PERF_LogFunctionString[] = {
    ...
    "Drm_ACT_GenerateChallengeInternal",
    "Drm_ACT_ProcessWSDLResponseInternal",
    "Oem_Aes_CtrProcessData",
};

#define DRM_PERF_MAX_SCOPE_ID_LENGTH    35
...
**                DRM_E_FILE_WRITE_ERROR,  if we cannot succefully write to the file
**
******************************************************************************/
    ...
    __in const OEM_FILEHDL   f_hFile )
{
    ...
    ChkBOOL( Oem_File_Write( f_hFile, rgcDWordASCII, cchDWordASCII, &cbWritten ), DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == cchDWordASCII, DRM_E_FILE_WRITE_ERROR );
...
static DRM_RESULT _writeStringtoFile(
    __in const DRM_CHAR     *f_pszValue,
    __in const OEM_FILEHDL   f_hFile )
{
    DRM_RESULT    dr         = DRM_SUCCESS;
    DRM_DWORD     cbWritten  = 0;

    ChkBOOL( Oem_File_Write( f_hFile, (DRM_VOID *)f_pszValue, (DRM_DWORD)DRMCRT_strlen( f_pszValue ), &cbWritten ), DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == DRMCRT_strlen( f_pszValue ), DRM_E_FILE_WRITE_ERROR );
    ...
    __in   const      eDRM_PERF_LogFunction f_eLogFunction )
{
    ...
    if ( s_fDRM_PERF_CollectData )
    {
        ...
        */
        ...
        ChkArg( ((DRM_DWORD)f_eLogFunction) < ((DRM_DWORD)NO_OF( s_rgpszDRM_PERF_LogFunctionString )) );
