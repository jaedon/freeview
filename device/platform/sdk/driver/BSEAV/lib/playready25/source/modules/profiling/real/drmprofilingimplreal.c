...
#define DRM_BUILDING_DRMPROFILINGIMPLREAL_C
...
#if PROFILE_STACK_SIZE
{
    ...
    if(pPerfContext->m_nCurrScope == 2)
    {
        ...
        {
            ChkBOOL(Oem_File_Write(pPerfContext->m_fpStackSize, &pScopeContext->m_dwScopeID, SIZEOF(DRM_DWORD), &dwNow), DRM_E_FILE_WRITE_ERROR );
            ChkBOOL(Oem_File_Write(pPerfContext->m_fpStackSize, &pPerfContext->m_dwMaxStackSize, SIZEOF(DRM_DWORD), &dwNow), DRM_E_FILE_WRITE_ERROR );
        }
