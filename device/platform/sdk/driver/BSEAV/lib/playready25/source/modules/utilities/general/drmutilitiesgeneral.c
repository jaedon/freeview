...
static DRM_NO_INLINE DRM_BOOL _HexStringToDword(
    __in            const DRM_WCHAR *f_pwsz,
    __out_ecount(1)       DRM_DWORD *f_pdwValue,
    __in                  DRM_LONG   f_cDigits )
{
    DRM_LONG iDigit;
    DRM_BOOL fOK = TRUE;
    DRM_DWORD dwValue = 0;
   ...
   __inout                           DRM_DWORD  *f_pdwVersion )
{
    ...
    *f_pdwVersion = 0;
    ...
    while ((dwIndex < 4) && (cchRemaining > 0))
    {
        ...
        {
            cchFromStartToEnd = (DRM_DWORD)( pwszEnd - pwszStart );
        }
        ...
        else
        {
            dwIndex++;
            pwszStart = ++pwszEnd;
            cchRemaining = ( f_cchVersion - (DRM_DWORD)( pwszStart - f_pwszVersion ) );
        }
    }
   ...
   __inout                           DRM_DWORD *f_pdwVersion )
{
    ...
    *f_pdwVersion = 0;
    ...
    while ((dwIndex < 4) && (cchRemaining > 0))
    {
        ...
        {
            cchFromStartToEnd = (DRM_DWORD)( pszEnd - pszStart );
        }
        ...
        else
        {
            dwIndex++;
            pszStart = ++pszEnd;
            cchRemaining = ( f_cchVersion - (DRM_DWORD)( pszStart - f_pszVersion ) );
        }
    }
   ...
   __out_ecount( VERSION_LEN )       DRM_WORD    rgwVersion[VERSION_LEN] )
{
    ...
    for( iCount = 0; iCount < VERSION_LEN - 1; iCount++ )
    {
        ...
        if ( ich == cchVersion)
        {
            ChkDR( DRM_E_CPRMEXP_INVALID_ARGUMENT );
        }
...
DRM_API DRM_NO_INLINE DRM_RESULT DRM_CALL DRM_UTL_StringToLittleEndianGuid(
    ...
    __out      DRM_GUID         *pGuid )
{
    ...
    if (! _HexStringToDword(pdstrString->pwszString + 1, &pGuid->Data1, 8))
    {
        ChkDR(DRM_E_INVALIDARG);
    }
    if (! _HexStringToDword(pdstrString->pwszString + 10, &dwValue, 4))
    {
        ChkDR(DRM_E_INVALIDARG);
    }

    pGuid->Data2 = (DRM_WORD) dwValue;

    if (! _HexStringToDword(pdstrString->pwszString + 15, &dwValue, 4))
    {
        ChkDR(DRM_E_INVALIDARG);
    }

    pGuid->Data3 = (DRM_WORD) dwValue;

    if (! _HexStringToDword(pdstrString->pwszString + 20, &dwValue, 2))
    {
        ChkDR(DRM_E_INVALIDARG);
    }
    ...
    if (! _HexStringToDword(pdstrString->pwszString + 22, &dwValue, 2))
    {
        ChkDR(DRM_E_INVALIDARG);
    }
    ...
    for (ib = 2; ib < 8; ib++)
    {
        if (! _HexStringToDword(pdstrString->pwszString  + (((ib - 1) * 2) + 23), &dwValue, 2))
        {
            ChkDR(DRM_E_INVALIDARG);
        }
...
/*
** This function operates only on Native byte-order GUIDs
*/
    ...
    *pwszBuffer-- = g_wchNull;
    ...
    else
    {
        while( f_dwValue != 0 )
        {
            ...
            /* store the digit */
            *pwszBuffer-- = (DRM_WCHAR)WCHAR_CAST(_ByteValueToASCII((DRM_BYTE)(f_dwValue % f_dwRadix)));
            cchOut++;
    ...
    __in                                               const DRM_ANSI_CONST_STRING *f_pdastr )
{
    ...
    else
    {
        ...
        {
            if( DRMCRT_towlower( (DRM_WCHAR)WCHAR_CAST( GET_BYTE( f_pszBase, f_pdasstr->m_ich + i ) ) )
             != DRMCRT_towlower( (DRM_WCHAR)WCHAR_CAST( GET_BYTE( f_pdastr->pszString, i ) ) ) )
            {
                return FALSE;
            }
        }
        ...
        */
        while( iKID < DRM_ID_SIZE )
        {
            ...
            */
            ...
            {
                PUT_BYTE( f_pkid->rgb, iKID, (DRM_BYTE)NATIVE_WCHAR(dstrKID.pwszString[iKID + (dstrKID.cchString - DRM_ID_SIZE)]) );
            }
            else
            {
                PUT_BYTE( f_pkid->rgb, iKID, (DRM_BYTE)NATIVE_WCHAR(dstrKID.pwszString[iKID]) );
            }
            iKID++;
        }
        dr = DRM_SUCCESS;
...
DRM_API DRM_NO_INLINE DRM_VOID DRM_CALL _promoteASCIItoUNICODE(
    ...
    __in                const DRM_DWORD      f_cch )
{
    DRM_DWORD ich = 0;

    for (ich = 0; ich < f_cch; ich++)
    {
        f_pwszString [ich] = (DRM_WCHAR)WCHAR_CAST(GET_CHAR(f_pszBase, f_pdasstr->m_ich + ich));
    }
}
    ...
    __in                       DRM_DWORD  cchMax )
{
    ...
    while( cchMax             > 0
        && pwszStr[ich] != g_wchNull )
    {
        PUT_BYTE( pszStrOut, ich, (DRM_BYTE)NATIVE_WCHAR(pwszStr[ich]) );
        cchMax--;
        ich++;
    }
        ...
        for(i = 0; i < NO_OF( g_rgXMLEncodeMapping ); i++)
        {
            if( g_rgXMLEncodeMapping[ i ].fEncodeOnly )
            {
                                    ...
                                    g_rgXMLEncodeMapping[ i ].pdstrDecodedString->cchString ) == 0 )
                {
                  ...
                  /* Do not encode '"' if the string is not going to be used as attribute text. */
                  && ( f_fAttributeText || *pwszSrc != g_wchQuote ) )
            {
                DRM_DWORD dwLen = 0;
                fWasEscaped = TRUE;
    ...
    __in                     DRM_DWORD  cb )
{
    ...
    for( i = 0; i < cb; i++ )
    {
        PUT_BYTE(pbLHS, i, (DRM_BYTE)( GET_BYTE(pbLHS, i) ^ GET_BYTE(pbRHS,i) ) );
    }
}
   ...
   __out                       DRM_GUID  *pDrmGuid )
{
    ...
    LITTLEENDIAN_BYTES_TO_WORD( pDrmGuid->Data2,  pbData, ibGuidOffset );
    ...
    __in                       DRM_DWORD  f_cb )
{
    ...
    for( i = 0; i < f_cb; i++ )
    {
        b = (DRM_BYTE)( GET_BYTE( f_pbIn, i ) << 1 );

        if( i < f_cb - 1 )
        {
            PUT_BYTE( f_pbOut, i, (DRM_BYTE)( b | ( GET_BYTE( f_pbIn, i + 1 ) >> 7 ) ) );
        }
        else
        {
