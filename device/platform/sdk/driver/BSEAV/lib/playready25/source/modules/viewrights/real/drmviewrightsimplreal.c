...
#define DRM_BUILDING_DRMVIEWRIGHTSIMPLREAL_C
...
static DRMINLINE DRM_BOOL _NegInfDate( __in const DRMFILETIME *f_pft )
{
...
static DRMINLINE DRM_BOOL _InfDate( __in const DRMFILETIME *f_pft )
{
...
static DRMINLINE DRM_BOOL _IsExpired( __in DRM_VOID *pOEMContext, __in const DRMFILETIME *f_pft )
{
    if( _InfDate( f_pft ) )
    {
        return FALSE;
...
static DRMINLINE DRM_BOOL _IsDateUsable(
    ...
    __in const DRMFILETIME *f_pftBegin,
    __in const DRMFILETIME *f_pftEnd )
{
...
static DRMINLINE DRM_BOOL _OverlappingDates(
    __in const DRMFILETIME *f_pftBeginDate1,
    __in const DRMFILETIME *f_pftEndDate1,
    __in const DRMFILETIME *f_pftBeginDate2,
    __in const DRMFILETIME *f_pftEndDate2 )
{
...
static DRMINLINE DRM_VOID _ReadDates(
    __in  const DRM_LICENSE_STATE_DATA    *f_pstatedata,
    __out       DRMFILETIME               *f_pstateBegDate,
    __out       DRMFILETIME               *f_pstateEndDate )
{
...
static DRM_RESULT _GetSecStateAttr(
    __in            DRM_SECSTORE_CONTEXT *f_contextSST,
    __in      const DRM_CONST_STRING     *f_pdstrAttribute,
    __out_opt       DRM_DWORD            *f_pdwValue,
    __out_opt       DRMFILETIME          *f_pft )
{
...
static DRM_VOID _SetExpiryCategory(
    __inout       DRM_LICENSE_STATE_DATA    *f_pstatedata,
    __in    const DRMFILETIME               *f_pftBeginDate,
    __in    const DRMFILETIME               *f_pftEndDate )
{
    DRM_DWORD dwExpiryType = (DRM_DWORD)( ( ( !_NegInfDate( f_pftBeginDate ) ) << 1 ) + !_InfDate( f_pftEndDate ) );
...
static DRM_RESULT _GetV2LicenseInfo(
    ...
    __out         LicenseInfo             *f_plicenseInfo,
...
static DRM_BOOL _ConvertLicenseInfoToStateData(
    __in  const LicenseInfo               *f_plicenseinfo,
    __out       DRM_LICENSE_STATE_DATA    *f_pstatedata )
{
...
static DRM_VOID _AggregateStateDataAndStateData(
    __in          DRM_VOID               *pOEMContext,
    __inout       DRM_LICENSE_STATE_DATA *f_pstatedata1,
    __in    const DRM_LICENSE_STATE_DATA *f_pstatedata2,
    __in          DRM_BOOL                fFirstHasPriority)
{
...
DRMINLINE static DRM_VOID _AggregateStateDataAndStateDataMultiple(
    ...
    __inout       DRM_LICENSE_STATE_DATA *f_rgstatedata1,
    __in    const DRM_LICENSE_STATE_DATA *f_rgstatedata2,
    __in          DRM_DWORD               f_cstatedata )
{
    for( ; f_cstatedata > 0; )
    {
        f_cstatedata--;
...
static DRM_RESULT _QueryRightForCandoAction(
    __in    const DRM_CONST_STRING        *f_dstrAction,
    __inout       DRM_VIEW_RIGHTS_CONTEXT *f_pcontextLQ,
    __inout const DRM_DST                 *f_pDatastore,
    __out         DRM_BOOL                *f_pfUpdateWithPMExpiry,
    __out         DRM_CONST_STRING const **f_ppdstrActionUsed )
{
...
static DRM_RESULT _UseRightForCandoAction(
    ...
    __in           DRM_BOOL                 f_fUpdateWithPMExpiry,
    __in           DRMFILETIME             *f_pft,
    ...
    if( DRM_UTL_DSTRStringsEqual( f_dstrAction, &g_dstrWMDRM_RIGHT_COPY ) )
    {
        if( DRM_UTL_DSTRStringsEqual( f_pdstrRightUsed, &g_dstrWMDRM_RIGHT_COPY ) )
        {
            if( f_pcontextLQ->fIsWMDRMPDDevice )   /* is WMDRM-PD device */
            {
                ...
                if( fLicTimeBased
                 && DRM_ARCLK_IsAntirollbackClockUnsupported()
                 && DRM_CLK_IsSecureClockUnsupported() )
                {
...
static DRM_VOID _AddLicResultCache(
    ...
    __in           DRM_BOOL                 f_fHasParentLink,
    __in     const DRM_KID                 *f_pparentKID,
    __in_opt const DRM_DWORD                f_rgLicResult[DRM_MAX_RIGHTS_SUPPORTED],
    __in           DRM_DWORD                f_dwUplinkVector,
    __in_opt const LicenseInfo              f_rgLicInfo[DRM_MAX_RIGHTS_SUPPORTED] )
{
...
static DRM_BOOL _GetLicResultCache(
    __inout                                          DRM_VIEW_RIGHTS_CONTEXT *f_pcontextLQ,
    __in                                       const DRM_LID                 *f_pLID,
    __out_ecount(1)                                  DRM_BOOL                *f_pfHasParentLink,
    __out_ecount(1)                                  DRM_KID                 *f_pparentKID,
    __out_ecount_opt(DRM_MAX_RIGHTS_SUPPORTED)       DRM_DWORD                f_rgLicResult[DRM_MAX_RIGHTS_SUPPORTED],
    __out_opt                                        DRM_DWORD               *f_pdwUplinkVector,
    __out_ecount_opt(DRM_MAX_RIGHTS_SUPPORTED)       LicenseInfo              f_rgLicInfo[DRM_MAX_RIGHTS_SUPPORTED] )
{
    DRM_DWORD iSlot   = 0;
    DRM_DWORD iAction = 0;
...
static DRM_RESULT _LoadNextLicense(
    ...
    __out                                      DRM_BOOL                *f_pFromCache,
    __out                                      DRM_LID                 *f_pLID,
    __out                                      DRM_BOOL                *f_pfHasParentLink,
    __out                                      DRM_KID                 *f_pparentKID,
    __out_ecount_opt(DRM_MAX_RIGHTS_SUPPORTED) DRM_DWORD               *f_rgLicResult,
    __out_opt                                  DRM_DWORD               *f_pdwUplinkVector,
    __out_ecount_opt(DRM_MAX_RIGHTS_SUPPORTED) LicenseInfo             *f_rgLiceInfo )
{
    DRM_RESULT dr     = DRM_SUCCESS;
...
static DRM_RESULT _IsLicenseSelectable(
    __inout       DRM_LICSTOREENUM_CONTEXT *f_pcontextLIC,
    __inout       DRM_VIEW_RIGHTS_CONTEXT  *f_pcontextLQ,
    __inout const DRM_DST                  *f_pDatastore,
    __in          DRM_BOOL                  f_fDeleteExpiredLicenses,
    __out         DRM_BOOL                 *f_pLicenseDeleted )
{
    DRM_RESULT dr      = DRM_SUCCESS;
...
static DRM_BOOL _EvalCurrLicforCandoActions(
    ...
    __out                          DRM_DWORD               *f_pdwUpSearchVector )
{
          DRM_RESULT        dr                  = DRM_SUCCESS;
...
static DRM_RESULT _QueryRightForCandoActions(
    ...
    __in                          DRM_DWORD                f_dwSearchVector,
    __in                          DRM_DWORD                f_dwChainDepth,
    __in                          DRM_DWORD                f_cActions,
    __in_ecount(f_cActions) const DRM_CONST_STRING        *f_rgpdstrAction[],
    __out_ecount(1)               DRM_BOOL                *f_pfLicFound,
    __out_ecount(1)               DRM_BOOL                *f_pfRecentExpired,
...
static DRM_RESULT _QueryRightForAction(
    ...
    __in    const DRM_VOID                *f_pvCallbackParameter,
    __out         DRM_BOOL                *f_pfUpdateWithPMExpiry,
    __out         DRM_CONST_STRING const **f_ppdstrActionUsed )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_BOOL _CollectLicStateforRequestedActions(
    ...
    __inout  const DRM_DST                 *f_pDatastore,
    __in           DRM_DWORD                f_cActions,
    __in     const DRM_CONST_STRING* const  f_rgpdstrAction[],
    __in           DRM_BOOL                 f_fHasParentLink,
    __out          LicenseInfo              f_rgLicinfo[],
    __in           DRMPFNPOLICYCALLBACK     f_pfnPolicyCallback,
    __in     const DRM_VOID                *f_pvCallbackParameter)
{
    DRM_DWORD               iAction             = f_cActions;
...
static DRM_BOOL _GetRootVariance(
    __inout       DRM_VIEW_RIGHTS_CONTEXT *f_pcontextLQ,
    __in          DRM_DWORD                f_dwChainDepth,
    __in          DRM_DWORD                f_cRootVariance,
    __in    const DRM_KID                 *f_pkidParent,
    __out         DRM_DWORD               *f_piCurrVariance )
{
    DRM_BOOL  fResult = FALSE;
    DRM_DWORD i = 0;
...
static DRM_RESULT _AddRootVariance(
    __inout       DRM_VIEW_RIGHTS_CONTEXT *f_pcontextLQ,
    __in          DRM_DWORD                f_dwChainDepth,
    __in          DRM_DWORD                f_iCurrVariance,
    __in    const DRM_KID                 *f_pkidParent )
{
    DRM_RESULT dr        = DRM_SUCCESS;
    DRM_DWORD  iAction   = 0;
...
static DRM_RESULT _QueryAndAggregateLicStatesForKID(
    ...
    __in           DRM_DWORD                 f_cActions,
    __in     const DRM_CONST_STRING*  const  f_rgpdstrAction[],
    __out          DRM_LICENSE_STATE_DATA    f_rgStateData[],
