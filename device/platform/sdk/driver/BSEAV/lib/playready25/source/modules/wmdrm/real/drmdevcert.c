...
#define DRM_BUILDING_DRMDEVCERT_C
...
static DRM_RESULT _GetCertAttribute(
    __in     const DRM_CONST_STRING        *f_pdstrDevCert,          /* devcert */
    __in           eDRM_DEVCERT_ATTRIBUTES  f_eAttribute,            /* devcert attr to get */
    __out_opt      DRM_CONST_STRING        *f_pdstrNode,             /* extracted attr node with <node> tag */
    ...
    __out_opt DRM_CONST_STRING             *f_pdstrValue )
{
      ...
      && f_poParserCache != NULL )
    {
        ...
        DRM_WORD                           wAttribute   = 0;

        ChkDR( DRM_DWordToWord( f_eAttribute, &wAttribute ) );
        ...
        if ( !fUseCache )
        {
            ...
            ChkDR( DRM_DWordPtrToWord( (DRM_DWORD_PTR)( dstrNode.pwszString - f_pdstrDevCert->pwszString ), &oEntry.m_iNodeStart ) );
            oEntry.m_iNodeLength = ( DRM_WORD )dstrNode.cchString;

            ChkDR( DRM_DWordPtrToWord( (DRM_DWORD_PTR)( dstrValue.pwszString - f_pdstrDevCert->pwszString ), &oEntry.m_iValueStart ) );
...
static DRM_RESULT _DCP_VerifyCert(
    ...
    __in           eDRM_DEVCERT_ATTRIBUTES       f_eDataLocation,
    __in           eDRM_DEVCERT_ATTRIBUTES       f_eSignatureLocation )
{
    DRM_RESULT          dr              =   DRM_SUCCESS;
...
ErrorExit:
    if( DRM_FAILED( dr ) )
    {
        dr = DRM_E_INVALID_DEVCERT_TEMPLATE;
    }
    return dr;
}
