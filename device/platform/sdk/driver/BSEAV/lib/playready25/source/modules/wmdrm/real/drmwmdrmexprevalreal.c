...
#define DRM_BUILDING_DRMWMDRMEXPREVALREAL_C
#include <drmwmdrm.h>
...
static DRM_RESULT DRM_CALL _ExtractStringToken( DRM_CONST_STRING* pdstrExpression, TOKEN* pToken );
static DRM_RESULT DRM_CALL _ExtractSymbolToken( DRM_CONST_STRING* pdstrExpression, TOKEN* pToken );

static DRM_RESULT _PerformOperation(
    __in const TOKEN                       *pOperator,
    __inout    DRM_WMDRM_EXPR_EVAL_CONTEXT *pEEContext );

/* Stub'd functions */
static DRM_NO_INLINE DRM_RESULT _EvalVariable(
    TOKEN                       *pToken,
...
static DRM_NO_INLINE DRM_BOOL _IsOperand(DRM_EXPR_TOKEN_TYPE TokenType)
{
    return (
...
static DRM_BOOL _IsPreUnaryOp(DRM_EXPR_TOKEN_TYPE TokenType)
{
    return (
...
static DRM_NO_INLINE DRM_BOOL _IsBinaryOp( DRM_EXPR_TOKEN_TYPE TokenType)
{
    return (
        TokenType == TOKEN_ADD     ||
        TokenType == TOKEN_SUB     ||
...
static DRM_NO_INLINE DRM_RESULT DRM_CALL _ExtractStringToken(
    ...
    TOKEN            *pToken )
{
    ...
    {
        dr = DRM_E_CPRMEXP_STRING_UNTERMINATED;
        goto ErrorExit;
    }
...
static DRM_NO_INLINE DRM_RESULT DRM_CALL _ExtractSymbolToken(
    DRM_CONST_STRING *pdstrExpression,
    TOKEN            *pToken )
{
    const DRM_WCHAR* pwcStart    = NULL;
    ...
    if ( pdstrExpression->cchString > 0 && *(pdstrExpression->pwszString) == g_wchOpenParen )
    {
        ...
        {
            dr = DRM_E_CPRMEXP_INVALID_FUNCTION;
        }
    }
    else
    {
    ...
    DRM_EXPR_TOKEN_TYPE *psLastToken)
{
    ...
    if (*(pdstrExpression->pwszString) == g_wchQuote)
    {
        ...
        pdstrExpression->pwszString++;
        pdstrExpression->cchString--;
        dr = _ExtractStringToken( pdstrExpression, pToken );
    ...
    else if (DRMCRT_iswalpha(*(pdstrExpression->pwszString)))
    {
        ...
        dr = _ExtractSymbolToken( pdstrExpression, pToken );
    }
    ...
    else
    {
        ...
        dr = DRM_E_CPRMEXP_INVALID_TOKEN;
        pdstrExpression->pwszString++;
        pdstrExpression->cchString--;
    }
...
static DRM_NO_INLINE DRM_LONG _InputPrecedence(DRM_EXPR_TOKEN_TYPE tokenType)
{
    switch(tokenType)
    {
    case TOKEN_OPEN:
...
static DRM_NO_INLINE DRM_RESULT _TokenStackPush(
    __inout       TOKEN      rgTokenStack[],
    __inout       DRM_DWORD *pdwStackHead,
    __in    const TOKEN     *pNewToken )
{
...
static DRM_NO_INLINE DRM_RESULT _TokenStackPop(
    TOKEN      rgTokenStack[],
    ...
    TOKEN     *pNewToken )
{
    if( *pdwStackHead == 0 )
    {
        return DRM_E_CPRMEXP_MISSING_OPERAND;
    }
    *pdwStackHead -= 1;
    ...
    __out         TOKEN                       *ptResult )
{
    ...
    for( dr = GetToken( &dstrExpression, &Token, &sLastToken ); DRM_SUCCEEDED( dr ); dr = GetToken( &dstrExpression, &Token, &sLastToken ) )
    {
        ...
        if (_IsOperand((DRM_EXPR_TOKEN_TYPE)Token.TokenType))
        {
            lRank++;
        }
        else if (_IsBinaryOp((DRM_EXPR_TOKEN_TYPE)Token.TokenType))
        {
            lRank--;
        }
        ...
        if (lRank != 0 && lRank != 1)
        {
            dr = DRM_E_CPRMEXP_ISOLATED_OPERAND_OR_OPERATOR;
            goto ErrorExit;
        }
        ...
        if (_IsOperand((DRM_EXPR_TOKEN_TYPE)Token.TokenType))
        {
            ChkDR( _TokenStackPush( pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &Token ) );
        ...
        {
            l_InputPrecedence = _InputPrecedence(TOKEN_CLOSE);
            while ( 0 < pEEContext->ExprEvalData.dwOperatorStack &&
                StackPrecedence((DRM_EXPR_TOKEN_TYPE)pEEContext->ExprEvalData.rgOperatorTokenStack[pEEContext->ExprEvalData.dwOperatorStack - 1].TokenType) >= l_InputPrecedence)    /* Loop through operators in the stack and perform the operations if their precedence is right */
            {

                ChkDR( _TokenStackPop( pEEContext->ExprEvalData.rgOperatorTokenStack, &( pEEContext->ExprEvalData.dwOperatorStack ), &newToken ) );
                ChkDR( _PerformOperation(&newToken, pEEContext ) );
            ...
            if ( 0 != pEEContext->ExprEvalData.dwOperatorStack)
            {
                ...
                {
                    ChkDR( _TokenStackPop( pEEContext->ExprEvalData.rgOperatorTokenStack, &( pEEContext->ExprEvalData.dwOperatorStack ), &newToken ) );
                    dr = _PerformOperation(&newToken, pEEContext );
                     /*&pEEContext->ExprEvalData.rgOperandTokenStack, &pEEContext->ExprEvalData.rgOperatorTokenStack, &fShortCircuit, pEEContext->pvOpaqueData); */
                    if( DRM_FAILED( dr ) )
                    {
                        goto ErrorExit;
        ...
        l_InputPrecedence = _InputPrecedence((DRM_EXPR_TOKEN_TYPE)Token.TokenType);
        while (0 != pEEContext->ExprEvalData.dwOperatorStack &&
            StackPrecedence((DRM_EXPR_TOKEN_TYPE)pEEContext->ExprEvalData.rgOperatorTokenStack[pEEContext->ExprEvalData.dwOperatorStack - 1].TokenType) >= l_InputPrecedence)
        {
            ChkDR( _TokenStackPop( pEEContext->ExprEvalData.rgOperatorTokenStack, &( pEEContext->ExprEvalData.dwOperatorStack ), &newToken ) );
            dr = _PerformOperation(&newToken, pEEContext );
            if( DRM_FAILED( dr ) )
        ...
        else
        {
                ...
                Token.TokenType == TOKEN_IF)
            {
                ...
                dr = _TokenStackPop( pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &newToken );
                if ( DRM_FAILED( dr ) )
                {
                    dr = DRM_E_CPRMEXP_MISSING_OPERAND;
                    goto ErrorExit;
                }
                if ( newToken.TokenType == TOKEN_VARIABLE)
                {
                    dr = _EvalVariable(&newToken, pEEContext);    /* This can change the type of NODE after evaluation */
                    if( DRM_FAILED( dr ) )
                    {
                        goto ErrorExit;
                    }
                ...
                ChkDR( _TokenStackPush( pEEContext->ExprEvalData.rgOperandTokenStack, &( pEEContext->ExprEvalData.dwOperandStack ), &newToken) );
            }
        ...
        ChkDR( _TokenStackPush(pEEContext->ExprEvalData.rgOperatorTokenStack, &( pEEContext->ExprEvalData.dwOperatorStack ), &Token) );
        ...
        else if (_IsPreUnaryOp((DRM_EXPR_TOKEN_TYPE)Token.TokenType))
        {
    ...
    if (dr == DRM_E_NOMORE)
    {
        ...
        while (0 != pEEContext->ExprEvalData.dwOperatorStack )
        {
            ChkDR( _TokenStackPop( pEEContext->ExprEvalData.rgOperatorTokenStack, &(pEEContext->ExprEvalData.dwOperatorStack), &Token) );
            ChkDR( _PerformOperation(&Token, pEEContext ) );
        ...
        {
            dr = DRM_E_CPRMEXP_NO_OPERANDS_IN_EXPRESSION;
            goto ErrorExit;
        }

        ChkDR( _TokenStackPop( pEEContext->ExprEvalData.rgOperandTokenStack, &( pEEContext->ExprEvalData.dwOperandStack ), &Token) );

        if ( 0 != pEEContext->ExprEvalData.dwOperandStack )
        {
            dr = DRM_E_CPRMEXP_TOO_MANY_OPERANDS;
            goto ErrorExit;
        }
        ...
        {
            ChkDR( _EvalVariable( &Token, pEEContext ) );
        }
...
/* _PerformOperation & _EvalVariable ( this is a big one with the different types of variables we might have. ) */
...
static DRM_NO_INLINE DRM_RESULT _PerformOperation(
    __in const TOKEN                       *pOperator,
    __inout    DRM_WMDRM_EXPR_EVAL_CONTEXT *pEEContext )
{
    DRM_RESULT dr = DRM_SUCCESS;/*vinceyu:replaced DRM_E_CPRMEXP_NOERROR */
    TOKEN Operand1;
    TOKEN Operand2;
    TOKEN OperandResult;
    DRM_INT64 i64Value;
    ...
    case TOKEN_NOT:
        ...
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand1 ) );
        ...
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand2 ) );
        if (Operand2.TokenType != TOKEN_UNARYSYNC)
        {
            dr = DRM_E_CPRMEXP_UNKNOWN_PARSE_ERROR; /* This should not really happen. */
            goto ErrorExit;
        }
        ...
        if (!pEEContext->ExprEvalData.fShortCircuit)
        {
            ...
            {
                ChkDR( _EvalVariable( &Operand1, pEEContext) );
            }
            ...
            if (pOperator->TokenType == TOKEN_MINUS)
            {
                ...
                {
                    dr = DRM_E_CPRMEXP_UNDERFLOW;
                    goto ErrorExit;
                }
                OperandResult.val.lValue = (DRM_LONG)DRM_I64ToUI32(i64Value);
            }
        ...
        ChkDR( _TokenStackPush(pEEContext->ExprEvalData.rgOperandTokenStack, &( pEEContext->ExprEvalData.dwOperandStack ), &OperandResult) );
        break;
    case TOKEN_ADD:
    case TOKEN_SUB:
    case TOKEN_MUL:
    ...
    case TOKEN_MOD:
        ...
        if (!pEEContext->ExprEvalData.fShortCircuit)
        {
            ...
            {
                ChkDR(_EvalVariable(&Operand2, pEEContext));
            }
    ...
    case TOKEN_POSTDECR:
        ...
        if (pOperator->TokenType == TOKEN_PREINCR || pOperator->TokenType == TOKEN_PREDECR)
        {
            ...
            if (Operand2.TokenType != TOKEN_UNARYSYNC)
            {
                dr = DRM_E_CPRMEXP_UNKNOWN_PARSE_ERROR;
                goto ErrorExit;
            }
        }
        ...
        if (!pEEContext->ExprEvalData.fShortCircuit)
        {
            ...
            Operand2.val.lValue = (DRM_LONG)DRM_I64ToUI32(i64Value);
            if (pOperator->TokenType == TOKEN_PREINCR || pOperator->TokenType == TOKEN_PREDECR)
            {
        ...
        break;
    case TOKEN_ASSIGN:
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand2 ) );  /* The right side operand of = */
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand1 ) );  /* The left side operand (i.e variable) of = */
    ...
    case TOKEN_NOTEQ:
    case TOKEN_EQ:
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand2 ) );  /* The right side operand of comparison */
        ChkDR( _TokenStackPop(pEEContext->ExprEvalData.rgOperandTokenStack,&(pEEContext->ExprEvalData.dwOperandStack), &Operand1 ) );  /* The left side operand of comparison */
        if (!pEEContext->ExprEvalData.fShortCircuit)
        {
            /* Perform the operation on the two operands and produce result. */
            if (Operand1.TokenType == TOKEN_VARIABLE)
    ...
    case TOKEN_COMMA:
        ...
        {
            ChkDR( DRM_E_CPRMEXP_MISSING_OPERAND );
        }
        break;
    case TOKEN_FUNCTION:
        {
    ...
    case TOKEN_ORSKIP:
        ...
        /* evaluated. */
        ChkDR( _TokenStackPush(pEEContext->ExprEvalData.rgOperandTokenStack, &( pEEContext->ExprEvalData.dwOperandStack ), &Operand1) );
        break;
    case TOKEN_COLONSKIP:
        /* We need matchig TIF */
        ...
        ChkDR(_TokenStackPop (pEEContext->ExprEvalData.rgOperatorTokenStack,&(pEEContext->ExprEvalData.dwOperatorStack), &Operand1 ) );

        ChkDR(_TokenStackPop (pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &OperandResult ) );
        ChkDR(_TokenStackPop (pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &Operand2 ) );
        ChkDR(_TokenStackPop (pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &Operand1 ) );
        ChkDR(_TokenStackPush(pEEContext->ExprEvalData.rgOperandTokenStack, &(pEEContext->ExprEvalData.dwOperandStack), &Operand1) );
        break;
    ...
    case TOKEN_COLON:
        ...
        {
            ChkDR( _TokenStackPush(pEEContext->ExprEvalData.rgOperandTokenStack, &( pEEContext->ExprEvalData.dwOperandStack ), &Operand2) );
        }
