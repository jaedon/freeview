   ...
   __in    const DRM_RSA_PUBLIC_KEY_2048BIT *f_pKeyRSAPublic )
{
    ...
    DRM_BYTE   rgbModulus[DRM_RSA_CB_MODULUS_MAX_2048BIT] = { 0 };
    DRM_DWORD  cbModulus                                  = SIZEOF( rgbModulus );
    DRM_BYTE   rgbExponent[DRM_RSA_CB_PUBEXP]             = { 0 };
    DRM_DWORD  cbExponent                                 = SIZEOF( rgbExponent );
    DRM_CHAR   rgbWorkBuffer[CCH_BASE64_EQUIV(DRM_RSA_CB_MODULUS_MAX_2048BIT)] = { 0 };
    ...
    __in                              DRM_CRYPTO_CONTEXT            *f_pCryptoCtx )
{
    ...
    else
    {
        ...
        ChkBOOL( f_cchRSA >= (DRM_DWORD)CCH_SIGNATURE_RSA( (DRM_DWORD)eKeyLength ), DRM_E_BUFFERTOOSMALL );
    }
    ...
    */
    ...
    if ( f_dwSignType == DRM_DSIG_TYPE_WMDRMNET )
    {
        /* WMDRMND-style signing: use a caller-provided signature of the data section */
    ...
    else
    {
        if ( ( f_dwFlags & DRM_CDS_USE_SIGNATURE_PARAMETER ) == 0 )
        {
            DRM_DWORD     cbSignature    = CB_SIGNATURE_RSA( eDRM_RSA_MAXIMUM_KEY_LENGTH_2048BIT );
            DRM_BYTE      rgbSignature  [CB_SIGNATURE_RSA( eDRM_RSA_MAXIMUM_KEY_LENGTH_2048BIT )]  = { 0 };
            DRM_CHAR      rgchSignature [CCH_SIGNATURE_RSA( eDRM_RSA_MAXIMUM_KEY_LENGTH_2048BIT )] = { 0 };
            ...
            dastr.cchString = (DRM_DWORD)CCH_SIGNATURE_RSA( eKeyLength );
    ...
    __in          DRM_CRYPTO_CONTEXT            *f_pCryptoCtx  )
{
    ...
    {
        DRM_BYTE   rgbRSA  [__CB_DECL(CB_SIGNATURE_RSA( eDRM_RSA_MAXIMUM_KEY_LENGTH_2048BIT ))]      = { 0 };
        DRM_DWORD  cbRSA = SIZEOF( rgbRSA );
        DRM_CHAR   rgchRSA [__CB_DECL(CCH_SIGNATURE_RSA( eDRM_RSA_MAXIMUM_KEY_LENGTH_2048BIT ))]     = { 0 };
