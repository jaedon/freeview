...
#define DRM_BUILDING_NDTASF_INTERNAL_C
    ...
    __out_opt  ASF_STREAM_INFO       **ppStreamInfo)
{
    ...
    __analysis_assume( NULL != pAsfContext );
...
static DRM_RESULT _GetNextReadySample(
    __inout const NDT_ASF_CONTEXT * const pAsfContext,
    ...
    __out      DRM_BYTE               **ppData )
{
    ...
    {
        DRM_LONG lOffset = 0;
        if( DRM_FAILED( DRM_DWordToLong( DRM_UI64Low32( nOffset ), &lOffset ) ) )
        {
            *ppData = NULL;
            return (DRM_DWORD)0;
        }

        if ( !Oem_File_SetFilePointer( pAsfContext->hInFile, lOffset, OEM_FILE_BEGIN, NULL ) )
        {
            *ppData = NULL;
            return (DRM_DWORD)0;
        }
...
static DRM_RESULT _SaveHeaderObjectToNewHeader(
    __inout       ASF_TRANSCRYPT_CONTEXT * const pAsfTranscrypt,
    __out   const DRM_GUID               * const pObjId,
    __out   const DRM_UINT64             * const pqwObjSize
)
{
    DRM_RESULT dr = DRM_SUCCESS;
...
PRAGMA_WARNING_PUSH     /* The BMIH structure is declared differently in headers for different platforms.  Disable the warning that results on platforms where it uses signed types. */
PRAGMA_WARNING_DISABLE(4365)
...
)
{
    ...
    if ( !fFoundAESPayload )
    {
        ...
        wSaveSize = (DRM_WORD)0xffff;
        SaveWORDOffset( pNewHeader, wSaveSize, dwOffset );
        dwSaveSize = (DRM_DWORD)0;
        ...
        wSaveSize = (DRM_WORD)( wPayloadExtnSysCount + 1 );
        dwOffset = dwPayloadExtnSysCountOffset;
        ChkArg( pPayloadExtnSysCount );
...
)
{
    ...
    for ( i = 0; i < cRecords; i++ )
    {
        ...
        if ( DRM_SUCCEEDED( _FindStreamInStreamInfoList(pAsfContext, (DRM_WORD)(wFlags & 0x7f), &pStreamInfo) ) )
        {
        ...
        {
            /* Not added yet */
            _AddToStreamInfoList(pAsfContext, (DRM_WORD)( wFlags & 0x7f ), dwAverageBitrate, CLSID_ASFStreamTypeUnknown, NULL);
        }
    }
...
)
{
    ...
    __analysis_assume( NULL != pbFileBuffer );
    __analysis_assume( NULL != pdwTotalUsed );
    __analysis_assume( NULL != pdwSize );
...
)
{
    ...
    pPayloadInfo->fIsKeyFrame = (DRM_BYTE)( ( b & 0x80 ) >> 7 );
    ...
    */
    ...
    {
        DRM_BYTE bPlus = 0;
        ChkDR( DRM_DWordToByte( pAsfContext->dwAesPayloadExtensionAdditionalBytes, &bPlus ) );
        ChkDR( DRM_ByteAdd( b, bPlus, &bPlus ) );
        SaveBYTEOffset( pDataBuffer, bPlus, dwOffset );
    }
...
)
{
    ...
    if ( TRUE == pPacketInfo->fMultiPayloads )
    {
        ...
        pPacketInfo->dwPayLenTypeOffset = 0;
        pPacketInfo->bPayLenType = (DRM_BYTE)( b >> 6 );
        pPacketInfo->bPayBytes = 0;
        pPacketInfo->dwPayloads = (DRM_BYTE)( b & 0x3f );
...
/******************************************************************************/
static DRM_RESULT _DRMDecryptRC4(
    __in                         DRM_DECRYPT_CONTEXT * const f_pDecryptContext,
    __inout_bcount(f_cbDataSize) DRM_BYTE            * const f_pbData,
...
/******************************************************************************/
static DRM_RESULT _DRMDecryptAES(
    __in DRM_DECRYPT_CONTEXT * const f_pDecryptContext,
    __in NDT_CRYPTO_DATA_ASF * const f_pCryptoData )
{
    DRM_RESULT                   dr     = DRM_SUCCESS;
    DRM_AES_COUNTER_MODE_CONTEXT ctrCtx = {0};

    DRM_PROFILING_ENTER_SCOPE( PERF_MOD_NDT_ASF, PERF_FUNC__DRMDecrypt );

    DRMASSERT( f_pDecryptContext                != NULL );
    ChkArg( f_pCryptoData                    != NULL );
    ChkArg( f_pCryptoData->SampleData.pbBlob != NULL );

    ctrCtx.qwInitializationVector = f_pCryptoData->qwSampleID;
    ctrCtx.qwBlockOffset          = DRM_UI64Div( f_pCryptoData->qwOffset, DRM_UI64( DRM_AES_BLOCKLEN ) );
    ctrCtx.bByteOffset            = ( DRM_BYTE )DRM_UI64Low32( DRM_UI64Mod( f_pCryptoData->qwOffset, DRM_UI64(DRM_AES_BLOCKLEN) ) );

    ChkDR( DRM_BBX_CPHR_Decrypt( ( DRM_CIPHER_CONTEXT* )f_pDecryptContext,
                                 &ctrCtx,
                                 f_pCryptoData->SampleData.cbBlob,
                                 f_pCryptoData->SampleData.pbBlob ) );
...
)
{
    ...
    ChkDR( DRM_WordSub( pState->pPayloadInfo->wTotalSize, pState->pPayloadInfo->wPayloadSize, &pState->wPayStart ) );
    ChkDR( DRM_WordAdd( pState->pPayloadInfo->wPacketOffset, pState->wPayStart, &pState->wPayStart ) );
    ...
    if ( pState->tParseState == P_UNCOMPRESSEDPAYLOAD )
    {
        ...
        if ( ( !pAsfContext->fEncryptClear ) )
        {
            if( pAsfContext->eCipherType == eDRM_RC4_CIPHER )
            {
                ChkDR( _DRMDecryptRC4( &pAsfContext->decryptContext, pData, dwNowWanted ) );
            }
            else if( pAsfContext->eCipherType == eDRM_AES_COUNTER_CIPHER )
            {
                ...
                ChkDR( _DRMDecryptAES( &pAsfContext->decryptContext, &cryptoData ) );
            }
        }
    ...
    else
    {
        ...
        if ( pState->pPayloadInfo->wTotalDataBytes > wNextSubPayloadSize )
        {
            ChkDR( DRM_WordAdd( wNextSubPayloadSize, 1, &wBytesRead ) );
        }
...
)
{
    ...
    ASF_PARSER_STATE             *pState                = &(pAsfContext->ParserState);
    const ASF_TRANSCRYPT_CONTEXT *pAsfTranscryptContext = &(pAsfContext->ASFTranscryptContext);
    const ASF_DATA_CONTEXT       *pData                 = &(pAsfTranscryptContext->Data.DataContext);
...
)
{
    ...
    __analysis_assume( f_pAsfTranscryptContext != NULL );
    __analysis_assume( f_pbBuffer              != NULL );
    __analysis_assume( f_pdwBytesWritten       != NULL );
...
)
{
    DRM_RESULT  dr = DRM_SUCCESS;
    ASF_PARSER_STATE *pState = NULL;
    const ASF_TRANSCRYPT_HEADER_CONTEXT *pHeader = NULL;
...
)
{
    ...
    {
        DRM_LONG lSeekPos = 0;
        ChkDR( DRM_DWordToLong( DRM_UI64Low32( qwSeekPos ), &lSeekPos ) );
        if( !Oem_File_SetFilePointer( pAsfContext->hInFile, lSeekPos, OEM_FILE_BEGIN, &dwCurrentPos ) )
        {
            ChkDR( DRM_E_ND_FAILED_SEEK );
        }
    }
    ...
    __out                    DRM_BOOL               * const pfEndOfPackets )
{
    ...
    ASF_PARSER_STATE *pState = NULL;
    const ASF_DATA_CONTEXT *pData = NULL;
    DRM_DWORD cbBufferSize = 0;
    DRM_DWORD dwBytesWritten = 0;
...
{
    DRM_RESULT  dr = DRM_SUCCESS;
    const ASF_PARSER_STATE *pState = NULL;
    ASF_TRANSCRYPT_CONTEXT *pAsfTranscryptContext;
...
)
{
    if ( pFileInfo )
    {
        ...
        if( pFileInfo->pPlayReadyObj != NULL )
        {
            SAFE_OEM_FREE( pFileInfo->pPlayReadyObj->pbData );
            SAFE_OEM_FREE( pFileInfo->pPlayReadyObj );
        }
        ZEROMEM( pFileInfo, SIZEOF( ASF_FILE_INFO ) );
    }
...
)
{
    ...
    DRM_DWORD dwBytesWritten = 0;
    DRM_DWORD dwOffset = 0;
    const ASF_TRANSCRYPT_CONTEXT *pAsfTranscryptContext = NULL;
 ...
 */
    ...
    if ( !pState->fSeekPositionSet )
    {
        ...
        {
            DRM_LONG lOffset = 0;
            ChkDR( DRM_DWordToLong( DRM_UI64Low32(DRM_I2UI64(pState->iFirstPacketOffset)), &lOffset ) );
            Oem_File_SetFilePointer(pAsfContext->hInFile, lOffset, OEM_FILE_BEGIN, NULL);
        }
    }
...
)
{
    ...
    while ( DRM_SUCCEEDED( _GetNextReadySample( pAsfContext, &pCurrentSample, &fReady ) ) )
    {
        ...
        if ( P_NEW_PACKET == pState->tParseState )
        {
            ...
            {
                DRM_LONG lSeekPos = 0;
                ChkDR( DRM_DWordToLong( DRM_UI64Low32(DRM_I2UI64(pState->iCurrentFileOffset)), &lSeekPos ) );
                if( !Oem_File_SetFilePointer( pAsfContext->hInFile, lSeekPos, OEM_FILE_BEGIN, NULL ) )
                {
                    ChkDR(DRM_E_FAIL);
                }
            }
    ...
    if ( TRUE == pAsfContext->fProtectedContentPlayback )
    {
        ...
        for (i = 0; i < cBuffers; i++)
        {
            ...
            if ( ( !pAsfContext->fEncryptClear ) )
            {
                if( pAsfContext->eCipherType == eDRM_RC4_CIPHER )
                {
                    ChkDR( _DRMDecryptRC4( &pAsfContext->decryptContext, pMediaBuffer->blobBuffer.pbBlob, pMediaBuffer->blobBuffer.cbBlob ) );
                }
                else if( pAsfContext->eCipherType == eDRM_AES_COUNTER_CIPHER )
                {
                    ChkDR( _DRMDecryptAES( &pAsfContext->decryptContext, &cryptoData ) );
    ...
    else
    {
        ...
        if ( TRUE == pState->fHeaderDone )
        {
            ...
            if ( pState->fSeekPositionSet )
            {
                ...
                {
                    DRM_LONG lSeekPos = 0;
                    ChkDR( DRM_DWordToLong( DRM_UI64Low32( DRM_I2UI64(pState->iCurrentPacketOffset) ), &lSeekPos ) );
                    if ( !Oem_File_SetFilePointer( pAsfContext->hInFile, lSeekPos, OEM_FILE_BEGIN, NULL ) )
                    {
                        ChkDR( DRM_E_FAIL );
                    }
