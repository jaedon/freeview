...
#define DRM_BUILDING_PACKETWRITER_C
...
PREFAST_PUSH_DISABLE_EXPLAINED(__WARNING_COUNT_REQUIRED_FOR_WRITABLE_BUFFER, "pbReplicatedData length defined by pMediaObjectWithAttributes" )
...
)
{
    ...
    if ( 0 < pAsfPacketWriterContext->m_ASFPayloadInfos.dwCurrentSize )
    {
        ...
        {
            dwResult = (DRM_DWORD)( pFinalPayload->wPacketOffset + pFinalPayload->wTotalSize );
        }
    }
...
)
{
    ...
    ChkDR( DRM_DWordSub( pPayload->wTotalSize, pPayload->wTotalDataBytes, &cbPayloadHeader ) );
    ...
    DRM_BYT_CopyBytes( pPayloadHeaderBuffer->blobBuffer.pbBlob,
                       ...
                       pPayload->rgbRepData,
                       2 * SIZEOF( DRM_DWORD ),
                       (DRM_SIZE_T)( pPayload->bRepData - STANDARD_REPLICATED_DATA_SIZE ) );
