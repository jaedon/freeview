...
#define DRM_BUILDING_SENDWINDOW_C
...
PREFAST_PUSH_DISABLE_EXPLAINED(__WARNING_NONCONST_PARAM_25004, "pAsfMuxSendWindowContext is modifed.  Prefast is wrong.")
...
static DRM_RESULT FragmentableObject_PopUsedSample(
    __inout     ASF_MUX_QUEUE_OBJECT_CONTEXT * const pAsfMuxQueueObjectContext,
    __out_opt   ASF_SAMPLE                         **ppUsedSample
);
...
DRM_VOID ASFMuxQueueObject_Uninitialize(
    __inout_opt ASF_MUX_QUEUE_OBJECT_CONTEXT * const pAsfMuxQueueObjectContext
)
{
...
)
{
    ...
    pASFPayloadInfo->fIsKeyFrame = (pAsfMuxQueueObjectContext->m_dwFlags & MF_SAMPLE_FLAG_CLEANPOINT) != 0;
