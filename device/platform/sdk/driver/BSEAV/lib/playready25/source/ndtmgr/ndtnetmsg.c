...
#define DRM_BUILDING_NDTNETMSG_C
#include <drmcrt.h>
#include <oemndt.h>
#include <ndtnetmsg.h>
#include <ndtreg.h>
...
{
    _PackOctets( MessageDescriptor, (const DRM_BYTE *)Value, SIZEOF(DRM_ID)  );
}
...
DRM_API DRM_BOOL DRM_CALL WmdrmNetUnpackOctets(
    __inout                           WMDRMNET_MESSAGE_DESCRIPTOR * const MessageDescriptor,
    __deref_out_bcount_opt(ValueSize) DRM_BYTE                          **Value,
    __in const                        DRM_DWORD                           ValueSize
)
{
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetUnpackRegistrationRequestMessage(
    ...
    __deref_out_bcount_opt(*f_pcbDeviceCertificate) DRM_BYTE        **f_ppbDeviceCertificate,
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetPackRegistrationResponseMessage(
    ...
    __in const                     DRM_RSA_PUBLIC_KEY_2048BIT * const f_pDevicePublicKey,
...
)
{
    ...
    DRM_BYTE  rgbEncryptedSeed[__CB_DECL( DRM_RSA_CB_MODULUS( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) )] = {0};
    ...
    */
    ChkArg( f_cbPubKeyModulus == DRM_RSA_CB_MODULUS(eDRM_RSA_KEY_LENGTH_1024_2048BIT) );
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetUnpackRegistrationResponseMessage(
    __in                            const DRM_RSA_PRIVATE_KEY_2048BIT * const f_pPrivateKey,
    __in_bcount(f_cbMessage)              DRM_BYTE                    * const f_pbMessage,
    __in                            const DRM_DWORD                           f_cbMessage,
    __out                                 DRM_ID                      * const f_pidSeed,
    __out                                 DRM_ID                      * const f_pidSerialNumber,
    __out                                 DRM_ID                      * const f_pidSession,
    __deref_out_bcount_opt(*f_pcbAddress) DRM_BYTE                          **f_ppbAddress,
    ...
    __in                                  DRM_CRYPTO_CONTEXT                 *f_pCryptoCtx )
{
    ...
    DRM_BYTE     rgbPlainSeed[__CB_DECL( DRM_RSA_CB_MAX_PLAINTEXT( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) )] = {0};
    ...
    ChkArg( pbEncryptedSeed != NULL );
    dr = OEM_RSA_OaepDecrypt_2048BIT( f_pPrivateKey,
                                      wEncryptedSeedSize,
                                      pbEncryptedSeed,
                                      &cbPlainSeed,
                                      rgbPlainSeed,
...
ErrorExit:
    ...
    SAFE_SECURE_OEM_FREE( pAesKey, SIZEOF( DRM_AES_KEY ) );
...
)
{
     ...
     */
    DRMSIZEASSERT( SIZEOF(DRM_ID), DRM_AES_BLOCKLEN );      /* encrypted challenge is DRM_ID size */
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetUnpackLicenseRequestMessage(
    ...
    __deref_out_bcount_opt(*f_pcbAction) DRM_BYTE        **f_ppbAction,
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetUnpackLicenseResponseMessage(
    __in_bcount(MessageSize)           DRM_BYTE  * const Message,
    __in                         const DRM_DWORD         MessageSize,
    __deref_out_bcount_opt(*cbLicense) DRM_BYTE        **ppbLicense,
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetUnpackRevocationListRequestMessage(
    ...
    __deref_out_ecount_opt(*f_pcguidsRevocationLists) DRM_GUID        **f_ppguidsRevocationLists )
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_BYTE cEntries = 0;
...
DRM_API DRM_RESULT DRM_CALL WmdrmNetPackRevocationListResponseMessage(
    ...
    __in_ecount_opt(f_cRevocationLists) const DRM_RVK_LIST * const f_pRevocationLists,
...
)
{
    ...
    for(i = 0; i < f_cRevocationLists; i++)
    {
        _PackVal128( &MessageDescriptor, (const DRM_ID *)&f_pRevocationLists[i].guidRevocationList );
...
)
{
    ...
    {
        _PackVal128( &MessageDescriptor, (const DRM_ID *)&f_pguidsRevocationLists[i] );
    }
