                          ...
                          DRM_DWORD f_cch )
{
    ...
    for( ich = 1; ich <= f_cch; ich++ )
    {
        f_pPackedString[f_cch - ich] = (DRM_CHAR)GET_CHAR( f_pPackedString, f_cch - ich );
    }
}
    ...
    __in       DRM_DWORD  f_dwAttributes )
{
    ...
    {
#if DRM_SUPPORT_LOCKING
        pfile = (FILE *) _fsopen (pszFName, pszMode, (int)fShare);
#else
    ...
    __out_opt DRM_DWORD *f_pdwNewFilePointer)
{
    ...
    if (fseek((FILE *)f_hFile,f_lDistanceToMove,(DRM_LONG)f_dwMoveMethod)==0)
    {
        ...
        {
            *f_pdwNewFilePointer = (DRM_DWORD)ftell((FILE *)f_hFile);
        }
    }
    else
    {
    ...
    __in DRM_BOOL f_fWait)
{
    ...
    if (fseek((FILE *)f_hFile, (long)f_dwFileOffset, SEEK_SET) == 0 )
    {
    ...
    __out DRM_DWORD *f_pcbFile)
{
    ...
    oFileCurr = (DRM_DWORD)ftell (pfile);  /* remember the current location within the file */

    if (fseek (pfile, 0, SEEK_END) == 0)  /* go to the end of the file */
    {
        *f_pcbFile = (DRM_DWORD)ftell (pfile);
        if (fseek (pfile, (long)oFileCurr, SEEK_SET) != 0)  /* reset back to the original location */
        {
            ChkDR( DRM_E_FILE_SEEK_ERROR );
        }
    }
