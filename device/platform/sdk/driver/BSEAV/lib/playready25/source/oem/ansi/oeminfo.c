...
DRM_EXPORT_VAR DRM_DWORD   g_dwPRNDSessionTimeout = DRM_PRND_REGISTRATION_VALIDITY;
DRM_EXPORT_VAR DRM_DWORD   g_dwPRNDCRLExpiration  = MAX_REVOCATION_EXPIRE_TIME;
    ...
    __in DRM_DWORD f_cbPrivKey )
{
    ...
    if ( !Oem_File_SetFilePointer( fp,
                                   (DRM_LONG)( f_fGCprivkey? 0 : SIZEOF( PRIVKEY ) ),
                                   OEM_FILE_BEGIN,
                                   NULL) )
    {
    ...
    __inout DRM_DWORD *f_pcbValue )
{
    ...
    /*
    ** ignore comparing b64 encoded sessionID, which is appended to the key
    */
    if( MEMCMP( f_szKey, Oem_Config_SETimeout_Key, Oem_Config_SETimeout_Key_LEN ) == 0 )
    {
        *( DRM_DWORD* ) f_pbValue  = g_dwPRNDSessionTimeout;
        dr = DRM_SUCCESS;
    }

    if( MEMCMP( f_szKey, Oem_Config_RLTimeout_Key, Oem_Config_RLTimeout_Key_LEN ) == 0 )
    {
        *( DRM_DWORD* ) f_pbValue  = g_dwPRNDCRLExpiration;
        dr = DRM_SUCCESS;
    }
