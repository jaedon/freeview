    ...
    if (OK) {
        ...
        else
        {
            ...
            if (dtemps == NULL) {
                ...
                OEM_SECURE_ZERO_MEMORY(dtemps,( ndigtemp)*SIZEOF( digit_t ));
...
*/
        OEM_SECURE_DIGITTCPY( p0location(1),P0,2*E->fdesc->elng );
        if (w == 2) {
            OEM_SECURE_DIGITTCPY( Presult,P0,2*E->fdesc->elng );
            ...
            if (w == 3) {
                OEM_SECURE_DIGITTCPY( Presult,p0location(3),2*E->fdesc->elng);
                                                           // P0 and 3*P0 saved
            } else {
...
// combining the inversions
    ...
    delay = TRUE;
    while (OK && delay) {
        const DRM_BOOL expon_partial_odd = ( 0 != IS_ODD(expon_partial) );
        const digit_t expon_partial3 = (const digit_t)( expon_partial - doublings3pwr );
                                         /* May be negative */
        const DRM_BOOL expon_partial3_odd = ( 0 != IS_ODD(expon_partial3) );
        ...
        if (expon_partial == 0) {
            ...
            OEM_SECURE_DIGITTCPY( Presult,p0location(expon_partial),2*E->fdesc->elng);
