...
#define ECPROJ5_TEMPS_COUNT 10
       // _ecproj5_add_ecaffine needs 10
...
typedef struct {
           ...
           DRM_DWORD            aib_recursion_level;  // Nesting level in
                                            // _affine_insert_bucket
           digit_t             *aib_temps[2];   // Two arrays, each length 2*elng
    } ecexpon_data_t;

static DRM_VOID DRM_CALL _affine_insert_bucket
     (const digit_t *, const DRM_LONG, ecexpon_data_t*, const DRM_LONG, struct bigctx_t *f_pBigCtx);

static DRM_VOID DRM_CALL _ec_concurrent_operations(ecexpon_data_t*, struct bigctx_t *f_pBigCtx);

static DRM_DWORD DRM_CALL _get_ec_concurrent_slot(ecexpon_data_t*, struct bigctx_t *f_pBigCtx);

static DRM_VOID DRM_CALL _proj5_insert_bucket
        (const digit_t*, const DRM_LONG, ecexpon_data_t*, const DRM_LONG, struct bigctx_t *f_pBigCtx);

static DRM_VOID DRM_CALL _schedule_affine_add_and_sub
        (const digit_t *, const digit_t *, const DRM_LONG, const DRM_LONG,
            ecexpon_data_t*, struct bigctx_t *f_pBigCtx);

static DRM_VOID DRM_CALL _schedule_affine_add_or_sub
          (const digit_t *, const digit_t *,
           const DRM_LONG, ecexpon_data_t*, const DRM_LONG, struct bigctx_t *f_pBigCtx);


static DRM_BOOL DRM_CALL _ecproj5_add_ecaffine
            (const digit_t *, const digit_t *,
            digit_t *, const struct ecurve_t*, digit_t *, struct bigctx_t *f_pBigCtx);

static DRM_BOOL DRM_CALL _ecproj5_doubling(const digit_t *, digit_t *,
                                    const struct ecurve_t*, digit_t *, struct bigctx_t *f_pBigCtx);

static DRM_BOOL DRM_CALL _ecproj5_set_infinite
...
DRM_API DRM_NO_INLINE DRM_BOOL DRM_CALL ecaffine_exponentiation_tabular
                 ...
                 struct bigctx_t *f_pBigCtx)
{
    ...
    */
    ...
    if (!OK) {
        ...
        dtemps = digit_allocate(num_dtemps, f_pBigCtx);
...
*/
            if (ibit >= lg2_latest_power_known + table_spacing) {
                ...
                if (ibucket != BUCKET_ZERO) {
                    ...
                    _proj5_insert_bucket(pecxdat->latest_power_known,
            ...
            while (     ibit >= lg2_latest_power_known + table_spacing
                ...
                carry_bits /= 2;
                pecxdat->OK = pecxdat->OK
                        && _ecproj5_doubling(pecxdat->latest_power_known,
                                        pecxdat->latest_power_known,
                                        ecurve, p5temps, f_pBigCtx);
 ...
 */
        ...
        while (pecxdat->OK && progress) {
            ...
            for (iaddsub = 0; iaddsub != naddsub; iaddsub++) {
                ...
                if (   pecxdat->bucket[in1].anticipating == 0
                    ...
                    _schedule_affine_add_and_sub(p1, p2, out1,
            ...
            if (!progress && pecxdat->slots_in_use > 0) {
                progress = TRUE;
                _ec_concurrent_operations(pecxdat, f_pBigCtx);
            }
        } // while

        for (ibucket = 1; ibucket <= bucket_max; ibucket++) {
            const DRM_LONG antic = pecxdat->bucket[ibucket].anticipating;
            if (antic != 0 && pecxdat->OK) {
                ...
                TRACE( ( "ecaffine_exponentiation_tabular -- bucket %ld has anticipating = %d\n",
                       ( DRM_LONG )ibucket, ( DRM_LONG )antic ) );
            }
        } // for ibucket
        ...
        pecxdat->OK = pecxdat->OK
            && _ecproj5_set_infinite(pecxdat->output_point, ecurve,
                                    p5temps);
        for (ibatch = 0;
             ibatch <= table_spacing - 3 && pecxdat->OK;
             ibatch++) {
            ...
            pecxdat->OK = pecxdat->OK
                && _ecproj5_doubling(pecxdat->output_point,
                                    pecxdat->output_point,
                                    ecurve, p5temps, f_pBigCtx)
                && _ecproj5_add_ecaffine(pecxdat->output_point, p1,
                                        pecxdat->output_point,
                                        ecurve, p5temps, f_pBigCtx)
                && ecaffine_set_infinite(p1, ecurve, f_pBigCtx);
        } // for ibatch
...
//      as 2*(output_point + latest_power_known) + (output_point + Q1).
        ...
        pecxdat->OK = pecxdat->OK
            ...
            && _ecproj5_add_ecaffine(pecxdat->latest_power_known,
            ...
            && ecproj5_add_ecproj5(pecxdat->latest_power_known,
                                   ...
                                   ecurve, p5temps, f_pBigCtx)
                                                             // 2*Q4 + Q2 + Q3
            && _ecproj5_add_ecaffine(pecxdat->output_point,
...

{
    ...
    {
        dtemps = digit_allocate(numdtemps, f_pBigCtx);
    }
    if (dtemps == NULL) {
        OK = FALSE;
    ...
    for (i = 1; OK && i <= table_last; i++) {
        ...
        if (Kiszero(z, fdesc, f_pBigCtx)) {// Doubling gave point at infinity
            ...
            num_finite_points++;
            OEM_SECURE_DIGITTCPY( tablei,ecpoint,2*fdesc->elng);        // x and y
            OEM_SECURE_DIGITTCPY( denoms + elng*(i-1),z,fdesc->elng); // z (nonzero)
        }
    } // for i
    ...
    if (dtemps != NULL) {Free_Temporaries(dtemps, f_pBigCtx);}
    return OK;
} // end ecaffine_table_construction
/* --------------------------------------------------------  */
static DRM_NO_INLINE DRM_VOID DRM_CALL _affine_insert_bucket
         ...
         struct bigctx_t *f_pBigCtx)
{
    ...
    if (OK) {
        ...
        if (recursion_level != 0 && recursion_level != 1) {
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INTERNAL_ERROR,
                            "_affine_insert_bucket recursion");
            TRACE( ( "_affine_insert_bucket, recursion_level = %ld\n",
            ...
            OEM_SECURE_DIGITTCPY( p1,bucket_point,( 2*  ecurve->fdesc->elng));
            OK = OK && ecaffine_set_infinite(bucket_point,
                                             ecurve, f_pBigCtx);
            _schedule_affine_add_or_sub(p1, EC_point,
                                   jbucket, pecxdat, negate_flag, f_pBigCtx);
        }
        pecxdat->OK = pecxdat->OK && OK;
        pecxdat->aib_recursion_level--;
    } // if OK
} // end _affine_insert_bucket
// ---------------------------------------------------------------------
static DRM_NO_INLINE DRM_VOID DRM_CALL _ec_concurrent_operations(
    ecexpon_data_t  *pecxdat,
    struct bigctx_t *f_pBigCtx )
{
    const ecurve_t     *ecurve  = pecxdat->ecurve;
    ...
    for (jslot = 0; jslot != num_slot_old; jslot++) {
        ...
        case ECOP_ADD:
            ...
            if (Kiszero(den, fdesc, f_pBigCtx)) {
                if (Kequal(y1, y2, fdesc, f_pBigCtx)) {
                    ...
                    if (char2) {
                        OEM_SECURE_DIGITTCPY( den,x1, fdesc->elng);
                    } else {
        ...
        case ECOP_AFFINIZATION:
            OEM_SECURE_DIGITTCPY( den,z1, fdesc->elng);
            break;
        ...
        default:
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                            "_ec_concurrent_operations");
            TRACE( ( "_ec_concurrent_operations -- bad operation = %ld\n",
                   ( DRM_LONG )operation ) );
            break;
        }
    ...
    for (jslot = 0;
        ...
        DRM_LONG negate_flag = pecxdat->pending_op[jslot].negate_flag;
...
//        const digit_t *z1 = x2;       // Affinization input
        ...
        DRM_BOOL OK = pecxdat->OK;
           ...
           // The calls to _affine_insert_bucket below may
        ...
        if (OK && jslot < pecxdat->slots_in_use) {
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                            "_ec_concurrent_operations slot");
            TRACE( ( "_ec_concurrent_operations -- slots re-allocated too fast.\n" ) );
        }
        switch(operation) {
        case ECOP_DO_NOTHING:
            break;
        ...
        default:
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                            "_ec_concurrent_operations 2nd loop");
            TRACE( ( "_ec_concurrent_operations -- bad operation = %ld in second loop\n",
                   ( DRM_LONG )operation ) );
            break;
        ...
        if (!OK) {
            ...
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                            "_ec_concurrent_operations -- ibucket");
            TRACE( ( "_ec_concurrent_operations -- bad ibucket = %ld\n",
                   ( DRM_LONG )ibucket ) );
        } else if (pecxdat->bucket[ibucket].anticipating <= 0) {
            OK = FALSE;
            SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                            "_ec_concurrent_operations, anticipating <= 0");
            TRACE( ( "_ec_concurrent_operations -- anticipating = %ld not positive\n",
...
#endif
            _affine_insert_bucket(new_point, ibucket,
                                 pecxdat, negate_flag, f_pBigCtx);
        }
        pecxdat->OK = pecxdat->OK && OK;
    } // for jslot
} // end _ec_concurrent_operations
/* --------------------------------------------------------- */
static DRM_NO_INLINE DRM_DWORD DRM_CALL _get_ec_concurrent_slot
       (ecexpon_data_t *pecxdat,
       struct bigctx_t *f_pBigCtx)
{
...
} // end _get_ec_concurrent_slot
// --------------------------------------------------------------------
static DRM_NO_INLINE DRM_VOID DRM_CALL _proj5_insert_bucket
        (const digit_t      *EC_point,
         const DRM_LONG    ibucket,
         ecexpon_data_t  *pecxdat,
         const DRM_LONG       negate_flag,
...
*/
{
    ...
    if (!pecxdat->OK) {
        ...
        _affine_insert_bucket(EC_point, ibucket, pecxdat,
                             negate_flag, f_pBigCtx);
                          // N.B. -- x and y occur first in EC_point,
                          // so we can treat it as affine if z == 1.
    } else {
        const DRM_DWORD islot = _get_ec_concurrent_slot(pecxdat, f_pBigCtx);
        ...
        pecxdat->bucket[ibucket].anticipating++;
        OEM_SECURE_DIGITTCPY( pecxdat->pending_op[islot].inputs,EC_point,3*fdesc->elng);
                              // Copy x, y, z
    }
} // end _proj5_insert_bucket
// ----------------------------------------------------------------------
DRM_NO_INLINE DRM_BOOL DRM_CALL ecaffine_to_ecproj5
        (const digit_t      *p1,
         digit_t        *p2,
         const ecurve_t        *ecurve,
         digit_t *   p5temps,
...
// Convert p1 from affine to projective form, result in p2.
{
    ...
    if (ecaffine_is_infinite(p1, ecurve, f_pBigCtx)) {
        OK = OK && _ecproj5_set_infinite(p2, ecurve, p5temps);
    } else {
        OEM_SECURE_DIGITTCPY( p2,p1,2*fdesc->elng);
        OEM_SECURE_DIGITTCPY( p2 + 2*elng,fdesc->one, fdesc->elng);
...
#error "_ecproj5_add_ecaffine -- Temps array too short"
#endif
    ...
    OK = OK && ecaffine_to_ecproj5(p2, temp_point, ecurve,
            ...
            && ecproj5_add_ecproj5(p1, temp_point, psum, ecurve,
                                   p5temps, f_pBigCtx);
    return OK;
} // end _ecproj5_add_ecaffine
//-----------------------------------------------------------------------

DRM_NO_INLINE DRM_BOOL DRM_CALL ecproj5_add_ecproj5
        (const digit_t      *p1,
         const digit_t      *p2,
         digit_t       *psum,
         const ecurve_t       *ecurve,
...
//  psum may overlap p1 or p2
{
    ...
    if (Kiszero(p1z, fdesc, f_pBigCtx)) {
        OEM_SECURE_DIGITTCPY( psum,p2,5* ecurve->fdesc->elng);
    } else if (Kiszero(p2z, fdesc, f_pBigCtx)) {
        OEM_SECURE_DIGITTCPY( psum,p1,5* ecurve->fdesc->elng);
    } else {
    ...
    // CAUTION -- _ecproj5_add_ecaffine assumes nothing
    // beyond here is modified until x, y inputs have been read.
        digit_t *W  = p5temps + 5*elng;
        digit_t *R  = p5temps + 6*elng;

        if (p1affine) {
            OEM_SECURE_DIGITTCPY( U2,p2x, fdesc->elng);
            OEM_SECURE_DIGITTCPY( S2,p2y, fdesc->elng);
        } else {
        ...
        if (p2affine) {
            OEM_SECURE_DIGITTCPY( U1,p1x, fdesc->elng);
            OEM_SECURE_DIGITTCPY( S1,p1y, fdesc->elng);
        } else {
        ...
        if (Kequal(U1, U2, fdesc, f_pBigCtx)) {
            if (Kequal(S1, S2, fdesc, f_pBigCtx)) {
                OK = OK && _ecproj5_doubling(p2, psum, ecurve,
                                            p5temps, f_pBigCtx);
            } else {
                OK = OK && _ecproj5_set_infinite(psum, ecurve,
                                                p5temps);
            }
        } else {
            ...
            if (p1affine) {
                OEM_SECURE_DIGITTCPY( T0,W, fdesc->elng);
            } else {
                OK = OK && Kmul(W, p1z, T0, fdesc, ftemps, f_pBigCtx);
            }
            if (p2affine) {
                OEM_SECURE_DIGITTCPY( p3z,T0, fdesc->elng);
            } else {
...
// ----------------------------------------------------------------------
static DRM_NO_INLINE DRM_BOOL DRM_CALL _ecproj5_doubling
        (const digit_t       *p1,
         digit_t        *p2,
         const ecurve_t        *ecurve,
         digit_t *   p5temps,
...
// Return p2 = 2*p1.  p1 and p2 may overlap
{
    ...
    if (!OK) {
        ...
        OK = OK && _ecproj5_set_infinite(p2, ecurve,
                                        p5temps);
    ...
    return OK;
} // end _ecproj5_doubling
// --------------------------------------------------------------------
static DRM_NO_INLINE DRM_BOOL DRM_CALL _ecproj5_set_infinite
        (digit_t       *p1,
         const ecurve_t       *ecurve,
         digit_t *  p5temps)
{
    ...
    OEM_SECURE_DIGITTCPY( p1,fdesc->one, fdesc->elng);
    OEM_SECURE_DIGITTCPY( p1 + elng,fdesc->one, fdesc->elng);
    Kclear_many(p1 + 2*elng, 3, fdesc, NULL);
    return OK;
} // end _ecproj5_set_infinite
// ---------------------------------------------------------------------
DRM_NO_INLINE DRM_BOOL DRM_CALL ecproj5_to_ecaffine
        (const digit_t       *p1,
         digit_t       *p2,
         const ecurve_t        *ecurve,
         digit_t *   p5temps,
...
/* ------------------------------------------------------------ */
static DRM_NO_INLINE DRM_VOID DRM_CALL _schedule_affine_add_and_sub(
...
*/
{
    ...
    if (!pecxdat->OK) {
        ...
        pecxdat->OK = FALSE;
        SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                        "_schedule_affine_add_and_sub 1");
        TRACE( ( "_schedule_affine_add_and_sub -- Bad output buckets = %ld %ld\n",
        ...
        _schedule_affine_add_or_sub(point1, point2, out1,
                                   pecxdat, +1, f_pBigCtx);
        _schedule_affine_add_or_sub(point1, point2, out2,
                                   pecxdat, -1, f_pBigCtx);
    }
} // end _schedule_affine_add_and_sub
/* ------------------------------------------------------------ */
static DRM_NO_INLINE DRM_VOID DRM_CALL _schedule_affine_add_or_sub(
    const digit_t    *point1,
    const digit_t    *point2,
    const DRM_LONG    jbucket,
    ecexpon_data_t   *pecxdat,
    const DRM_LONG    negate_second,
    struct bigctx_t  *f_pBigCtx)
//
...

{
    ...
    if (!OK) {
        ...
        OK = FALSE;
        SetMpErrno_clue(MP_ERRNO_INVALID_DATA,
                        "_schedule_affine_add_or_sub");
        TRACE( ( "_schedule_affine_add_or_sub, jbucket = %ld\n",
               ( DRM_LONG )jbucket ) );
    }

    if (!OK) {
    }else if (ecaffine_is_infinite(point1, ecurve, f_pBigCtx)) {
        _affine_insert_bucket(point2, jbucket, pecxdat,
                             negate_second, f_pBigCtx);
    } else if (ecaffine_is_infinite(point2, ecurve, f_pBigCtx)) {
        _affine_insert_bucket(point1, jbucket, pecxdat,
                             +1, f_pBigCtx);
    } else {
        DRM_DWORD islot = _get_ec_concurrent_slot(pecxdat, f_pBigCtx);
        digit_t *inputs = pecxdat->pending_op[islot].inputs;
        OEM_SECURE_DIGITTCPY( inputs,point1, 2* ecurve->fdesc->elng);
    ...
    pecxdat->OK = pecxdat->OK && OK;
} // end _schedule_affine_add_or_sub
