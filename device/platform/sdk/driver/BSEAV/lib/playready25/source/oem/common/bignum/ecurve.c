...
#error -- Need more EC temporaries
    ...
    if (ecaffine_is_infinite(p2, E, f_pBigCtx)) {
        OEM_SECURE_DIGITTCPY( p3,p1,2* E->fdesc->elng);
        simple_case = TRUE;
    } else if (ecaffine_is_infinite(p1, E, f_pBigCtx)) {
        if (addsub == +1) {
            OEM_SECURE_DIGITTCPY( p3,p2, 2* E->fdesc->elng);
        } else {
        ...
        if (addsub == -1) {
            OEM_SECURE_DIGITTCPY( t2,y2, fdesc->elng );  // t2 = y2
        } else if (char2) {
    ...
    if (simple_case || !OK) {
        ...
        OK = OK && Ksub(x1, t2, t3, fdesc,
                        f_pBigCtx);     // t3 = x1 - x3
        OEM_SECURE_DIGITTCPY( x3,t2, fdesc->elng);
...
// This also requires that ecaffine_addition use only three field temps.
        ...
        if (   ecaffine_is_infinite(p1, E, f_pBigCtx)
            ...
            OK = OK && ecaffine_addition(p1, p2, pdif, -1, E,
                                         special_case_temps, f_pBigCtx);
            OEM_SECURE_DIGITTCPY( psum,ps, 2*  E->fdesc->elng);
        }
...

{
    DRM_BOOL OK = TRUE;
    if (negate_flag == +1) {
        OEM_SECURE_DIGITTCPY( p2,p1,2*  E->fdesc->elng);
...
// Compute p2 = -p1 with elliptic curve arithmetic.
{
    ...
    if (ecaffine_is_infinite(p1, E, f_pBigCtx)) {
        OEM_SECURE_DIGITTCPY( p2 + elng,p1 + elng, fdesc->elng);    // y2 = y1
    ...
    OEM_SECURE_DIGITTCPY( p2,p1, fdesc->elng);                // x2 = x1
    return OK;
} // end ecaffine_negate
...
// Initialize struct for elliptic curve with parameters a and b
{
    ...
    if (OK) {
        E->fdesc = fdesc;
        OEM_SECURE_DIGITTCPY( E->a,a, fdesc->elng);
        OEM_SECURE_DIGITTCPY( E->b,b, fdesc->elng);
