...
//   For internal use only.
    ...
    if (OK) {
        OEM_SECURE_ZERO_MEMORY(fdesc, SIZEOF(*fdesc));
    ...
    if (OK && nalloc != 0) {
        ...
        if (fdesc->deallocate == NULL) {
            ...
            OEM_SECURE_ZERO_MEMORY(fdesc->deallocate,( nalloc)*SIZEOF( digit_t ));
        }
    }
    return OK;
} // Kfdesc_initialize
...
*/
{
    DRM_BOOL OK = TRUE;
    (DRM_VOID)f_pBigCtx;
    OEM_SECURE_ZERO_MEMORY(f3,( nelmt*fdesc->elng)*SIZEOF( digit_t ));
    return OK;
