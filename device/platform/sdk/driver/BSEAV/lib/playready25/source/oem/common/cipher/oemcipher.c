...
#define DRM_BUILDING_OEMCIPHER_C
#include <oemcipher.h>
#include <oemaes.h>
#include <byteorder.h>
#include <oem.h>
    ...
    */
    if( f_eCipherType == eDRM_RC4_CIPHER )
    {
        ...
        */
        ...
        DRM_CBC64InvKey( &f_pContext->cipher.cipherRC4.mackey, &f_pContext->cipher.cipherRC4.invmackey );
...
/*********************************************************************
**
**  Function:  DRM_CPHR_InitWithKeyHandle
**
**  Synopsis:  Initializes a content cipher context.  This must be done prior to using
**             DRM_CPHR_Encrypt or DRM_CHPR_Decrypt.
**
...
**     f_pContext     :  The cipher context to be initialized
**     f_eCipherType  :  The type of cipher to be used
**     f_hContKey     :  The content key handle
**
**  Returns:
**      DRM_SUCCESS
**          success
    ...
    __in                OEM_CRYPTO_HANDLE      f_hContKey )
{
    ...
    ChkArg( f_pContext    != NULL );
    ChkArg( f_pOpaqueBBX  != NULL );
    ChkArg( f_hContKey    != OEM_CRYPTO_HANDLE_INVALID );
    ChkArg( f_eCipherType == eDRM_AES_COUNTER_CIPHER );

    ZEROMEM( f_pContext, SIZEOF( DRM_CIPHER_CONTEXT ) );

    f_pContext->cipher.cipherAES.hAesKey = f_hContKey;

    f_pContext->eCipherType = f_eCipherType;
    f_pContext->pOpaqueBbx  = f_pOpaqueBBX;
    f_pContext->fInited     = TRUE;
    ...
    __in                                                               DRM_DWORD           f_cbData )
{
    ...
    ChkBOOL( f_pContext->fInited, DRM_E_CIPHER_NOT_INITIALIZED );
    ...
    __inout_bcount( f_cbData )   DRM_BYTE                     *f_pbData )
{
    ...
    */
    if ( f_pContext->eCipherType == eDRM_RC4_CIPHER )
    {
        ChkBOOL( f_pContext->cipher.cipherRC4.fDecryptInited, DRM_E_DECRYPT_NOT_INITIALIZED );
        ...
        */
        if ( f_pContext->cipher.cipherRC4.cbPacket < 16 )
        {
            ...
            for ( iData = 0; iData < f_cbData; iData++ )
            {
                ...
                PUT_BYTE( f_pbData, iData, (DRM_BYTE)( bData ^ bSHA ) );
            }
