...
static DRMFORCEINLINE DRM_VOID _endian_reverse_dwords( const DRM_DWORD f_rgdwIn[], DRM_DWORD f_rgdwOut[], DRM_DWORD cdw )
{
...
static DRM_NO_INLINE DRM_BOOL _dw_to_modular(
...
static DRM_NO_INLINE DRM_BOOL words_to_ecaffine(
                       const DRM_BYTE           *in,
                       const DRM_DWORD           cP,
    __out_ecount( cP )       digit_t            *P,
                       const ecurve_t           *E,
                             struct bigctx_t    *f_pBigCtx )
    ...
    case FIELD_2_NORMAL:
        for (tries = 0;
            ...
            tries++) {
                ...
                tmp[LNGQDW-1] = tries;
                if( !_dw_to_modular( tmp, LNGQDW, P, E->fdesc->modulo, f_pBigCtx) )
                {
                    found = FALSE;
                    break;
                }
...
static DRM_BOOL _ecaffine_to_dwords( const digit_t *P, DRM_DWORD *out, const ecurve_t  *E)
{
         ...
         tmp != NULL )
    {
        ...
        _endian_reverse_dwords( (const DRM_DWORD *)tmp, out, LNGQDW-1 );
...
static DRM_NO_INLINE DRM_RESULT _ec_init(
    PK_DATA         *pEc,
    const DRM_DWORD *q,
    ...
    struct bigctx_t *f_pBigCtx)
{
    ...
    ChkBOOL( _dw_to_modular( a, LNGQDW, pEc->a, &pEc->qmodulus, f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE);
    ChkBOOL( _dw_to_modular( b, LNGQDW, pEc->b, &pEc->qmodulus, f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE);
    ...
    ChkBOOL( _dw_to_modular( ecdw_genx, LNGQDW, pEc->g, &pEc->qmodulus, f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE);
    ChkBOOL( _dw_to_modular( ecdw_geny, LNGQDW, pEc->g + LNGQ, &pEc->qmodulus, f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE);
    ...
    __in_opt DRM_VOID* f_pOEMContext )
{
    ...
    ChkDR( OEM_ECC_InitializeBignumStackImpl( pContext ) ); /* we have to initialize the bignum context since _ec_init needs it */
    ...
    dr = _ec_init(&pContext->pkData, ecdw_q, ecdw_order, ecdw_a, ecdw_b, ecdw_genx, ecdw_geny, (struct bigctx_t*)&pContext->oHeap);
    if( DRM_SUCCEEDED( dr ) )
    {
    ...
    __out PRIVKEY  *f_pprivkey )
{
    ...
    if( ecaffine_exponentiation_tabular( pContext->pkData.TABLE, TABLE_SPACING, TABLE_LAST, priv, LNGQ, pub, &(pContext->pkData.ecurve), pBigCtx ) )
    {
        ...
        _endian_reverse_dwords( (DRM_DWORD*)f_pprivkey->x, (DRM_DWORD*)f_pprivkey->x, DRM_ECC160_PRIVATE_KEY_LEN / SIZEOF( DRM_DWORD ) );
        ...
        _endian_reverse_dwords( (DRM_DWORD*)f_ppubkey->y, (DRM_DWORD *)f_ppubkey->y, DRM_ECC160_PUBLIC_KEY_LEN / SIZEOF( DRM_DWORD ) );
    ...
    __out_bcount( DRM_ECC160_CIPHERTEXT_LEN )       DRM_BYTE  f_rgbOut[__CB_DECL(DRM_ECC160_CIPHERTEXT_LEN)] )
{
    ...
    _endian_reverse_dwords( (const DRM_DWORD*)f_ppubkey->y, rgdwPubKey, DRM_ECC160_PUBLIC_KEY_LEN / SIZEOF( DRM_DWORD ) );

    ChkArg( pContext->pkData.qmodulus.length == LNGQ );

    _dw_to_modular( rgdwPubKey,          LNGQDW, pk,      &(pContext->pkData.qmodulus), pBigCtx );
    _dw_to_modular( rgdwPubKey + LNGQDW, LNGQDW, pk+LNGQ, &(pContext->pkData.qmodulus), pBigCtx );
    ...
    */
    ...
    _endian_reverse_dwords( (const DRM_DWORD*)f_rgbOut, (DRM_DWORD*)f_rgbOut, DRM_ECC160_PLAINTEXT_LEN / SIZEOF(DRM_DWORD) );
    if( !words_to_ecaffine( f_rgbOut, 2 * LNGQ, ecM, &(pContext->pkData.ecurve), pBigCtx ) )
    {
        ChkDR( DRM_E_PKCRYPTO_FAILURE );
    }
    ...
    _endian_reverse_dwords( (const DRM_DWORD *)f_rgbOut, (DRM_DWORD*)f_rgbOut, 4*LNGQDW );
    ...
    __out_bcount( DRM_ECC160_PLAINTEXT_LEN )       DRM_BYTE  f_rgbOut[__CB_DECL(DRM_ECC160_PLAINTEXT_LEN) ] )
{
    ...
    _endian_reverse_dwords( (const DRM_DWORD *)f_pprivkey->x, rgdwPrivKey, DRM_ECC160_PRIVATE_KEY_LEN / SIZEOF( DRM_DWORD ) );
    ...
    */
    ...
    _endian_reverse_dwords( (const DRM_DWORD *)rgdwIn, rgdwIn, DRM_ECC160_CIPHERTEXT_LEN / SIZEOF(DRM_DWORD) );

    _dw_to_modular( rgdwIn,          LNGQDW, a,      &pContext->pkData.qmodulus, pBigCtx );
    _dw_to_modular( rgdwIn + LNGQDW, LNGQDW, a+LNGQ, &pContext->pkData.qmodulus, pBigCtx );
    _dw_to_modular( rgdwIn+2*LNGQDW, LNGQDW, b,      &pContext->pkData.qmodulus, pBigCtx );
    _dw_to_modular( rgdwIn+3*LNGQDW, LNGQDW, b+LNGQ, &pContext->pkData.qmodulus, pBigCtx );
    ...
    if( !_ecaffine_to_dwords( ecM, rgdwOut, &(pContext->pkData.ecurve) ) )
    {
        ChkDR( DRM_E_PKCRYPTO_FAILURE );
    }
...
static void DRMFORCEINLINE _byte_array_to_digits(
                        const  DRM_BYTE     buf[__CB_DECL(SIGN_BUF_LEN)],
    __in_ecount(LNGQDW)        digit_t      dividend[LNGQDW] )
{
    DRM_DWORD ib;
...
static DRM_NO_INLINE DRM_BOOL DRM_CALL byte_array_mod_bignum(
    const  DRM_BYTE     buf[__CB_DECL(SIGN_BUF_LEN)],
           digit_t      remainder[],
    const  PK_DATA     *pPKData )               /* Given a message to be signed, in byte form reduce it modulo pkData.r[].  */
{
    ...
    _byte_array_to_digits( buf, dividend );
...
*/
static DRM_NO_INLINE DRM_BOOL _FE2IPmod(
    const digit_t     *fdata,
    ...
    __out_bcount( DRM_ECC160_SIGNATURE_LEN )       DRM_BYTE   f_rgbSignature[ __CB_DECL(DRM_ECC160_SIGNATURE_LEN) ] )
{
    ...
    _endian_reverse_dwords( (const DRM_DWORD *)f_privkey->x, rgdwPrivKey, DRM_ECC160_PRIVATE_KEY_LEN / SIZEOF( DRM_DWORD ) );
    ...
    for( ntry = 0; ntry < 1000; ntry++ )
    {
        ...
        ChkBOOL( _FE2IPmod(Gpriv2, pContext->pkData.ecurve.fdesc, pContext->pkData.r, pContext->pkData.lngr, &pContext->pkData.rrecip, sigc, pBigCtx ), DRM_E_PKCRYPTO_FAILURE);
        ...
        _byte_array_to_digits(buffer, gcd);
        ...
        if( significant_digit_count(sigd,  pContext->pkData.lngr) != 0 )
        {
            ...
            _endian_reverse_dwords( (const DRM_DWORD *)f_rgbSignature, (DRM_DWORD*) f_rgbSignature, 2*LNGQDW );
    ...
    __in_bcount( DRM_ECC160_SIGNATURE_LEN ) const DRM_BYTE   f_rgbSignature[__CB_DECL(DRM_ECC160_SIGNATURE_LEN)] )
{
    ...
    _endian_reverse_dwords( (const DRM_DWORD *)f_rgbSignature, rgdwSignature, 2*LNGQDW );
    ...
    ChkBOOL( _dw_to_modular( rgdwPubKey,        LNGQDW, pubkey,      &(pContext->pkData.qmodulus), pBigCtx ), DRM_E_PKCRYPTO_FAILURE );
    ChkBOOL( _dw_to_modular( rgdwPubKey+LNGQDW, LNGQDW, pubkey+LNGQ, &(pContext->pkData.qmodulus), pBigCtx ), DRM_E_PKCRYPTO_FAILURE );
    ...
    ChkBOOL( _FE2IPmod(h1G, ecurve->fdesc, pContext->pkData.r, pContext->pkData.lngr, &(pContext->pkData.rrecip), temp, pBigCtx ), DRM_E_PKCRYPTO_FAILURE );
...
DRM_API DRM_NO_INLINE DRM_VOID* DRM_CALL bignum_alloc(__in const DRM_DWORD cblen, __in struct bigctx_t *f_pBigCtx )
{
    DRM_VOID *pbRet = NULL;
...
**
*********************************************************************/
DRM_API DRM_NO_INLINE DRM_VOID* DRM_CALL bignum_alloc_align(__in const DRM_DWORD cblen,
