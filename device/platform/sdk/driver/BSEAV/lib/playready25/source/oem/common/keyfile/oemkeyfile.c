...
#define DRM_BUILDING_OEMKEYFILE_C
...
DRM_EXPORT_VAR DRM_DWORD g_dwPlatformID = DRM_BCERT_SECURITY_VERSION_PLATFORM_WM_7;
#elif DRM_BUILD_PROFILE == DRM_BUILD_PROFILE_IOS
/*
** Robustness version and platform ID for DRM_BUILD_PROFILE_IOS
*/
DRM_EXPORT_VAR DRM_DWORD g_dwWMRobustnessVersion = 0x1000003;
DRM_EXPORT_VAR DRM_DWORD g_dwPRRobustnessVersion = 0x1000003;

DRM_EXPORT_VAR DRM_DWORD g_dwPlatformID = DRM_BCERT_SECURITY_VERSION_PLATFORM_IOS_ARM;
...
DRM_EXPORT_VAR DRM_DWORD g_dwPlatformID = DRM_BCERT_SECURITY_VERSION_PLATFORM_XBOX_PPC;
...
DRM_EXPORT_VAR DRM_DWORD g_dwPlatformID = DRM_BCERT_SECURITY_VERSION_PLATFORM_WINDOWS;
#endif
...
{
    WCHAR_CAST('k'), WCHAR_CAST('e'), WCHAR_CAST('y'), WCHAR_CAST('f'),
    WCHAR_CAST('i'), WCHAR_CAST('l'), WCHAR_CAST('e'), WCHAR_CAST('.'),
    ...
    __in     DRM_DWORD   f_dwAttributes )
{
    ...
    */
    if( g_fUseDefaultKeyFile )
    {
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen( g_rgwchKeyFileName ), &dstrKeyFileName.cchString ) );
...
DRM_API DRM_RESULT Oem_Keyfile_GetHash( __out_bcount( SHA256_DIGEST_SIZE_IN_BYTES ) DRM_BYTE *f_pbFileHash )
{
    ...
    */
    ...
    if ( !Oem_File_GetHash(pwszPath, f_pbFileHash) )
    {
...
/*****************************************************************************************************************
** Function:    Oem_Keyfile_Initialize
**
** Synopsis:    Initializes the memory for the Keyfile context.
**
** Arguments:   [f_poAppContextInternal] : Pointer to DRM app context.
*****************************************************************************************************************/
    ...
    __inout DRM_APP_CONTEXT_INTERNAL* f_poAppContextInternal )
{
    ...
    f_poAppContextInternal->pKeyFileContext = &f_poAppContextInternal->oKeyFileContext;
...
/*****************************************************************************************************************
** Function:    Oem_Keyfile_Uninitialize
**
** Synopsis:    Uninitializes the memory for the Keyfile context.
**
** Arguments:   [f_poAppContextInternal] : Pointer to DRM app context.
*****************************************************************************************************************/
    ...
    __inout DRM_APP_CONTEXT_INTERNAL* f_poAppContextInternal )
{
    ...
    ZEROMEM( &f_poAppContextInternal->oKeyFileContext, SIZEOF( f_poAppContextInternal->oKeyFileContext ) );

    f_poAppContextInternal->pKeyFileContext = NULL;
     ...
     __inout DRM_KEYFILE_CONTEXT *f_pKeyFile )
{
    ...
    ChkBOOL( Oem_File_SetFilePointer( f_pKeyFile->hKeyfile, 0, OEM_FILE_BEGIN, NULL ), DRM_E_FILE_SEEK_ERROR );
    ChkBOOL( Oem_File_Read( f_pKeyFile->hKeyfile, f_pKeyFile->pbKeyfileBuffer, cbKeyfileLen, &cbRead ), DRM_E_FILE_READ_ERROR );
    ChkBOOL( cbRead == cbKeyfileLen, DRM_E_FILE_READ_ERROR );
     ...
     __inout DRM_KEYFILE_CONTEXT *f_pKeyFile )
{
    ...
    ChkBOOL( Oem_File_SetFilePointer( f_pKeyFile->hKeyfile, 0, OEM_FILE_BEGIN, NULL ), DRM_E_FILE_SEEK_ERROR );
    ChkBOOL( Oem_File_Write( f_pKeyFile->hKeyfile, f_pKeyFile->pbKeyfileBuffer, f_pKeyFile->cbKeyfileBuffer, &cbWritten ), DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == f_pKeyFile->cbKeyfileBuffer, DRM_E_FILE_WRITE_ERROR );
    ...
    ChkBOOL( Oem_File_FlushBuffers( f_pKeyFile->hKeyfile ),  DRM_E_FILE_WRITE_ERROR );
