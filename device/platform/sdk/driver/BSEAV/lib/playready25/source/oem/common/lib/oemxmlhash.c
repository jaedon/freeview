...
#define DRM_BUILDING_OEMXMLHASH_C
#include <oemcommon.h>
    ...
    __out DRM_XML_HASH_SIG *f_poSig )
{
    ...
    ** It also assumes that we won't have multiple strings with different
    ** high-dword values on a 64-bit system (hence downcast to DRM_DWORD).
    */
    f_poSig->m_dwSourceHash1 = ( DRM_DWORD )( DRM_DWORD_PTR )f_pdstrXML->pwszString;
