...
static DRM_NO_INLINE DRM_VOID _sha1(
    __inout_ecount(5)       DRM_DWORD ABCDE[5],
    __in              const DRM_BYTE  bInput[__CB_DECL(DRM_SHA1_BLOCK_SIZE)] )
{
    ...
    {
        TEMP = S(ABCDE[A],5);
        TEMP += _ft( (DRM_LONG)ABCDE[B], (DRM_LONG)ABCDE[C], (DRM_LONG)ABCDE[D], cCount );
        TEMP += ABCDE[E];
        TEMP += W[cCount];
        TEMP += _K(cCount);
        ABCDE[E] = ABCDE[D];
    ...
    __out_bcount( DRM_SHA1_DIGEST_LEN ) DRM_BYTE             rgbDigest[__CB_DECL(DRM_SHA1_DIGEST_LEN)] )
{
    ...
    DRM_BYTE   bPaddingBuffer[__CB_DECL(DRM_SHA1_BLOCK_SIZE+12)];    /* Maximum block size we may need. */

    ChkArg( pShaContext != NULL );

    OEM_SECURE_ZERO_MEMORY( bPaddingBuffer, SIZEOF(bPaddingBuffer) ); 
