                     ...
                     DRM_DWORD   cb )
{
        ...
        && cb == SIZEOF( DRM_GUID ) )
    {
        ...
        ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrKey, (DRM_GUID*)pb ) );
    }
    else if( pwszKey[0] == '$' )
    {
        cch--;
...
/***********************************************************************/
static DRM_VOID _PrintCharString(
    __in                const DRM_CHAR  *pszText1,
    __in                const DRM_CHAR  *pszText2,
    __in_ecount(cbBuff)       DRM_BYTE  *pbBuff,
    __in                      DRM_DWORD  cbBuff )
{
    ...
    {
        const DRM_WCHAR *pwch = (const DRM_WCHAR*)&pbBuff[nIndex];
...
static DRM_RESULT _OpenStore(
    const DRM_CONST_STRING *pPathname,
    DRM_DST          *pDst )
{
    DRM_RESULT    dr                               = DRM_SUCCESS;
    ...
    dr = DRM_DST_OpenStore( NULL,
                            ...
                            0,
                            pDst );
    if ( DRM_SUCCEEDED(dr) || dr==DRM_E_HASH_MISMATCH )
    {
        /* get file size */
...
**
*/
static DRM_VOID _PrintSlots(
    __in                   const _HdsContext *pHDS,
    __in                         DRM_DWORD    dwBlocknum,
    __in_ecount(cbPayload)       DRM_BYTE    *pbPayload,
    __in                         DRM_DWORD    cbPayload,
    __in                         DRM_DWORD    dwFreeMem,
    __in                         EDumpMode    eMode )
{
    DRM_DWORD dwSearch=0;
    DRM_DWORD dwBytesToSkip=0;
    const _SlotHeader *pSlotHdr=NULL;
    ...
    for (dwSearch=0; dwSearch<(cbPayload-dwFreeMem); dwSearch+=dwBytesToSkip)
    {
        ...
        pSlotHdr = (const _SlotHeader*)&pbPayload[dwSearch];
        ...
        {
            const DRM_DWORD *pdwDatablocknum = (const DRM_DWORD*)(&pbPayload[dwSearch] + SIZEOF(_SlotHeader));
            printf("\t[Datablock#]\t%d\n", *pdwDatablocknum);
...
**
*/
static DRM_BOOL _ParseChildBlock(
    const _HdsContext *pHDS,
    const DRM_BYTE *pBlockBuff,
    ...
    DRM_DWORD  cbCommonBlockHdr)
{
    ...
    const _ChildBlockFileHDR *pChildBlockHeader=(const _ChildBlockFileHDR*)(pBlockBuff + cbCommonBlockHdr);
...
**
*/
PREFAST_PUSH_DISABLE_EXPLAINED(__WARNING_COUNT_REQUIRED_FOR_WRITABLE_BUFFER_25057, "pbChildAllocTable can't be const annotated with a size" )
static DRM_DWORD _GetChildBlockNum(
    const _HdsContext *pHDS,
...
_HdsTraverseBlocksInPreOrder(
    __in      _NsContext *pCFG,
    __in_opt  _CommBlockHDR *pCurrBlock,
    __out     _CommBlockHDR *pNextBlock,
    __out     DRM_LONG *pdwLevelsVisited,
...
***********************************************************************
*/
static DRM_RESULT DoDumpChildblock(
    __in                     _HdsContext   *pHDS,
    __in                     DRM_DWORD      dwBlocknum,
    __in_ecount(cbBlockBuff) DRM_BYTE      *pBlockBuff,
    __in                     DRM_DWORD      cbBlockBuff,
    __in                     _CommBlockHDR *pCcommonBlockHdr,
    __in                     DRM_DWORD      cbCommonBlockHdr,
    __in                     EDumpMode      eMode)
{
    DRM_RESULT dr=DRM_SUCCESS;
...
***********************************************************************
*/
static DRM_RESULT DoDumpDatablock(
    __in                     _HdsContext    *pHDS,
    __in                     DRM_DWORD       dwBlocknum,
    __in_ecount(cbBlockBuff) DRM_BYTE       *pBlockBuff,
    __in                     DRM_DWORD       cbBlockBuff,
    ...
    __in                     DRM_DWORD       cbCommonBlockHdr,
    __in                     EDumpMode       eMode,
    __out_opt                DRM_DWORD      *pdwCascadingBlock)
{
    DRM_RESULT dr=DRM_SUCCESS;
    _CommBlockHDR *pDataBlock=NULL;
    DRM_DWORD dwPayloadBeg=0;
    DRM_DWORD dwPayloadSize=0;
    const _DataBlockFileHDR *pDataBlockHeader=(const _DataBlockFileHDR*)(pBlockBuff + cbCommonBlockHdr);
    ...
    EDumpMode eMode)
{
    ...
    if (!Oem_File_SetFilePointer(pHDS->fp, (DRM_LONG)dwFilePos, OEM_FILE_BEGIN, NULL))
    {
    ...
    switch(GET_BYTE( &commonBlockHdr.File._image.bBlockType,0))
    {
        case eCHILDBLOCK:
            printf("CHILD\n");
            DoDumpChildblock(pHDS, dwBlocknum, pBlockBuff, pHDS->oSRN.dwBlockSize, &commonBlockHdr, cbCommonBlockHdr, eMode);
            break;
        case eDATABLOCK:
            printf("DATA\n");
            DoDumpDatablock(pHDS, dwBlocknum, pBlockBuff, pHDS->oSRN.dwBlockSize, &commonBlockHdr, cbCommonBlockHdr, eMode, NULL);
            break;
        case eFREEBLOCK:
            printf("FREE\n");
            DoDumpDatablock(pHDS, dwBlocknum, pBlockBuff, pHDS->oSRN.dwBlockSize, &commonBlockHdr, cbCommonBlockHdr, eMode, NULL);
...
DoSlotTrace(
    DRM_DST          *pDatastore,
    const DRM_CONST_STRING *pwszParam,
    EDumpMode eMode)
{
    DRM_RESULT dr=DRM_SUCCESS;
    _HdsContext *pHDS=(_HdsContext*)(pDatastore->poContextDST);
    ...
    pSlotCtx = (const _SlotContext *)&SlotContext;
    printf("\n\t[Slot Info]\n");
    printf("\t[Size]\t\t\t%d\n", cbData);
    ...
    else
    {
        ...
        while (dwDatablock)
        {
            ...
            {
                case eDATABLOCK:
                    DoDumpDatablock(pHDS, dwDatablock, pBlockBuff, pHDS->oSRN.dwBlockSize, &commonBlockHdr, cbCommonBlockHdr, eMode, &dwDatablock);
                    break;
                case eCHILDBLOCK:
...
static DRM_RESULT
DoDumpSRN(
    const DRM_CONST_STRING *pStoreName,
    DRM_BOOL fFreeBlockDetail)
{
    ...
    if ( DRM_FAILED(dr2) && dr2!=DRM_E_HASH_MISMATCH )
    {
        ChkDR(dr2);
    }
    ...
    else
    {
        ...
        while (dwFreeBlocknum)
        {
            ...
            {
                case eFREEBLOCK:
                    DoDumpDatablock(pHDS, dwFreeBlocknum, pBlockBuff, pHDS->oSRN.dwBlockSize, &commonBlockHdr, cbCommonBlockHdr, eDumpRaw, &dwFreeBlocknum);
                    break;
                case eCHILDBLOCK:
                    printf("\n** Incorrect block type: CHILD **\n** Aborting **\n");
                    goto ErrorExit;
