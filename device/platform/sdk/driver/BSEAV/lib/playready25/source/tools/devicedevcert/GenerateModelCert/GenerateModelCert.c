...
typedef struct DRM_WMDRMND_DATA
{
    DRM_RSA_PRIVATE_KEY_2048BIT           oCompanyRSAPrivKey;
    ...
    DRM_RSA_SUPPORTED_KEY_LENGTHS_2048BIT eKeyLength ;                  /* Key length of the certificate */
...
typedef struct DRMSTATE
{
    ...
    */
    ...
    DRM_WMDRMND_DATA     *pWMDRMNDData; /* data structure for RSA-based certificate */
...
[-c:WMDRMNDCompanyCert]       - Optional parameter. Path to the input file with WMDRMND company certificate\n\
[-m:WMDRMNDModelCert]         - Optional parameter. Path to the ouput file that receives WMDRMND model certificate\n\
[-n:WMDRMNDModelKey]          - Optional parameter. Path to the ouput file that receives WMDRMND model private key\n\
[-v:WMDRMNDUnsignedTemplate]  - Optional parameter. Path to the input file with parameters for the new certificate, should be XML file saved as ASCII\n",
            pwszAppName );
}
...
DRM_RESULT DRMInit( void )
{
    ...
    ChkMem ( g_oState.pWMDRMNDData = (DRM_WMDRMND_DATA *) Oem_MemAlloc(SIZEOF(DRM_WMDRMND_DATA) ) );
    ZEROMEM( g_oState.pWMDRMNDData, SIZEOF(DRM_WMDRMND_DATA) );
    g_oState.pWMDRMNDData->hModelCert = OEM_INVALID_HANDLE_VALUE;
    g_oState.pWMDRMNDData->hModelCertKey = OEM_INVALID_HANDLE_VALUE;
...
static void _DRMClose( void )
{
    ...
    if ( g_oState.pWMDRMNDData != NULL )
    {
        if (g_oState.pWMDRMNDData->hModelCertKey != OEM_INVALID_HANDLE_VALUE)
        {
            Oem_File_Close(g_oState.pWMDRMNDData->hModelCertKey);
        }
        if (g_oState.pWMDRMNDData->hModelCert != OEM_INVALID_HANDLE_VALUE)
        {
            Oem_File_Close(g_oState.pWMDRMNDData->hModelCert);
        }
        g_oState.pWMDRMNDData->dastrCompanyPubKeyModulusB64.pszString = NULL;
        SAFE_OEM_FREE( g_oState.pWMDRMNDData->dastrCompanyCert.pszString );
        SAFE_OEM_FREE( g_oState.pWMDRMNDData->dastrUnsignedTemplate.pszString );
        SAFE_OEM_FREE( g_oState.pWMDRMNDData );
    }
   ...
   __in DRM_BOOL f_PlayReadyCertRequired)
{
    ...
    */
    ...
    if ( f_NDCertRequired )
    {
        ...
        const DRM_CONST_STRING *pdstrKeys = (const DRM_CONST_STRING *)&g_oState.dstrOEMPrivKeys;

        ChkDR( _GetRootKeys( pdstrKeys, &g_dstrWMDRMNDPRIVKEYPrime0,       &dstrPrivKeyPrime0 ) );
        ChkDR( _GetRootKeys( pdstrKeys, &g_dstrWMDRMNDPRIVKEYPrime1,       &dstrPrivKeyPrime1 ) );
        ChkDR( _GetRootKeys( pdstrKeys, &g_dstrWMDRMNDPRIVKEYCRTExponent0, &dstrPrivKeyCRTExponent0 ) );
        ChkDR( _GetRootKeys( pdstrKeys, &g_dstrWMDRMNDPRIVKEYCRTExponent1, &dstrPrivKeyCRTExponent1 ) );
        ChkDR( _GetRootKeys( pdstrKeys, &g_dstrWMDRMNDPRIVKEYIQMP,         &dstrPrivKeyIQMP ) );
        ...
        */
        ChkDR( DRM_XML_RSA_ParseBase64PrivateKey( &dstrPrivKeyPrime0,
                                                  ...
                                                  &dstrPrivKeyIQMP,
                                                  &(g_oState.pWMDRMNDData->oCompanyRSAPrivKey) ) );
    }
...
DRM_RESULT VerifyDevcert(
    __in DRM_STRING *f_pdstrDevcertTemplate )
{
    DRM_RESULT  dr = DRM_SUCCESS;
    ...
    )
{
    ...
    */
    for ( dwCount = 0; dwCount < 2; dwCount++ )
    {
        dr = DRM_XML_GetSubNodeByPath(
                                (const DRM_CONST_STRING *)&(g_oState.dstrUnsignedTemplate),
    ...
    */
    ...
    if ( DRM_SUCCEEDED( dr ) )
    {
        ...
        dr = DRM_XML_GetSubNodeByPath(
                 ( const DRM_CONST_STRING * ) &dstrData,
...
static DRM_BOOL _UnsignedTempleteIsMissingIssuerLink(
    __in const DRM_CONST_STRING *f_pdstrKeyUsagesXML )
{
    if ( DRM_FAILED ( DRM_XML_GetSubNodeByPath( 
                                     f_pdstrKeyUsagesXML,  
                                     &g_dstrKeyUsageIssuerLink,               /* "IssuerLink" */
                                     NULL,
                                     NULL,
                                     NULL,
                                     NULL,
...
static DRM_BOOL _UnsignedTempleteHasKeyUsageNodeButNoIssuerLink()
{
    DRM_CONST_STRING dstrKeyUsages = EMPTY_DRM_STRING;
    if ( DRM_FAILED ( DRM_XML_GetSubNodeByPath( 
                                     ( const DRM_CONST_STRING * )&( g_oState.dstrUnsignedTemplate ),  
                                     &g_dstrCertKeyUsages,               /* "keyusages" */
                                     NULL,
                                     NULL,
                                     (DRM_CONST_STRING *)&dstrKeyUsages, /* <keyusages>...</keyusages> */
                                     NULL,
                                     g_wchForwardSlash ) ) )
    {
        return FALSE;
    ...
    {
        return _UnsignedTempleteIsMissingIssuerLink( ( DRM_CONST_STRING* )&dstrKeyUsages ) ;
    }
}
...
/******************************************************************************
**
** Function :   GenerateBinaryModelCertificate
**
...
**
******************************************************************************/
static DRM_RESULT GenerateBinaryModelCertificate()
{
    DRM_RESULT             dr                   = DRM_SUCCESS;
    ...
    DRM_DWORD              rgdwKeyUsageSet[ 3 ] = { DRM_BCERT_KEYUSAGE_SIGN, DRM_BCERT_KEYUSAGE_ISSUER_DEVICE, 0 };
    DRM_DWORD              dwNumKeyUsages       = 2;
    ...
    */
    if( oCertificate.KeyInfo.rgoKeys[0].dwUsageSet & BCERT_KEYUSAGE_BIT(DRM_BCERT_KEYUSAGE_ISSUER_LINK ) )
    {
        dwNumKeyUsages = 3;
        rgdwKeyUsageSet[2] = DRM_BCERT_KEYUSAGE_ISSUER_LINK;
    } 
    if( _UnsignedTempleteHasKeyUsageNodeButNoIssuerLink() )
    {
        dwNumKeyUsages = 2;
        rgdwKeyUsageSet[2] = 0;
    }
    oCertKey.dwNumKeyUsages = dwNumKeyUsages;
    ...
    */
    ...
    oVerificationContext.rgdwRequiredKeyUsages[2] = DRM_BCERT_KEYUSAGE_ISSUER_LINK;
    oVerificationContext.cRequiredKeyUsages = dwNumKeyUsages;
...
static DRM_RESULT _checkRequiredUnsignedTemplateXMLTags(
    __in const DRM_STRING    *f_pdstrData
    )
{
    DRM_RESULT dr             = DRM_SUCCESS;
    DRM_STRING dstrData       = EMPTY_DRM_STRING;
    ...
    __in       OEM_FILEHDL                  f_fpPrivKey )
{
    ...
    */
    ...
    ChkBOOL( Oem_File_Write( f_fpPrivKey, (DRM_BYTE*)g_rgbFFFE, cbWrite, &cbWritten )==TRUE, DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILE_WRITE_ERROR );

    cbWrite = CB_DSTR( &dstrClose );
    ChkBOOL( Oem_File_Write( f_fpPrivKey, dstrClose.pwszString, cbWrite, &cbWritten )==TRUE, DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == cbWrite, DRM_E_FILE_WRITE_ERROR );
...
/******************************************************************************
**
** Function :   ParseWMDRMNDCompanyCert
**
** Synopsis :   Parses WMDRMND company certificate and gets  company base64 encoded
...
**
******************************************************************************/
static DRM_RESULT ParseWMDRMNDCompanyCert( __in DRM_WMDRMND_DATA *f_pData )
{
    DRM_RESULT            dr = DRM_SUCCESS;
    ...
    __analysis_assume( f_pData != NULL );
    ...
    f_pData->eKeyLength = eDRM_RSA_KEY_LENGTH_1024_2048BIT;
...
**
** Synopsis :   Updates WMDRMND unsigned template with default data,
...
static DRM_RESULT _UpdateNDTemplate (
    __inout    DRM_WMDRMND_DATA   *f_pData )
{
    DRM_RESULT             dr = DRM_SUCCESS;
    ...
    {
        rgpdastrValues[0] = &g_dastrDefaultSecurityLevel;
...
**
******************************************************************************/
static DRM_RESULT ProcessUnsignedNDTemplate( __inout DRM_WMDRMND_DATA *f_pData )
{
    DRM_RESULT    dr                  = DRM_SUCCESS;
    DRM_SUBSTRING dasstrData          = EMPTY_DRM_SUBSTRING;
    DRM_SUBSTRING dasstrTemplate      = EMPTY_DRM_SUBSTRING;
    ...
    if ( DRM_FAILED( dr ) )
    {
        printf( "Invalid WMDRMND unsigned template format, exitting...\n" );
        goto ErrorExit;
    }
    ...
    __out       DRM_STRING          *f_pdstrFileData )
{
    ...
    */
    dr = DRM_XML_GetNode( (const DRM_CONST_STRING *)f_pdstrFileData,
                          &g_dstrUnsignedtemplate,
                          NULL,
                          NULL,
                          0,
...
/******************************************************************************
**
** Function :   GenerateWMDRMNDModelCert
**
** Synopsis :   Generates WMDRMND model certificate, verifies correctness
**              and saves it to file, file handler: [f_pState->pWMDRMNDData->fpModelCert]
**
** Arguments :  [f_pState] - pointer to tool's state structure and data
**
...
**
******************************************************************************/
static DRM_RESULT GenerateWMDRMNDModelCert( __in DRMSTATE *f_pState )
{
    DRM_RESULT                    dr                 = DRM_SUCCESS;
    _XMBContextA                 *pXMLCtxA           = NULL;
    DRM_RSA_PRIVATE_KEY_2048BIT  *poRSAModelPrivKey  = NULL;
    DRM_RSA_PUBLIC_KEY_2048BIT   *poRSAModelPubKey   = NULL;
    DRM_BOOL                      fResult            = FALSE;
    DRM_DWORD                     cbWritten          = 0;
    DRM_CRYPTO_CONTEXT           *pCryptoCtx         = NULL;
    DRM_WMDRMND_DATA             *pWMDRMNDData       = NULL;
    ...
    ChkArg( f_pState->pWMDRMNDData != NULL );

    pWMDRMNDData = f_pState->pWMDRMNDData;
    ...
    ChkMem( poRSAModelPrivKey = (DRM_RSA_PRIVATE_KEY_2048BIT*) Oem_MemAlloc( SIZEOF( DRM_RSA_PRIVATE_KEY_2048BIT ) ) );
    ZEROMEM( poRSAModelPrivKey, SIZEOF( poRSAModelPrivKey ) );
    ChkMem( poRSAModelPubKey = (DRM_RSA_PUBLIC_KEY_2048BIT*) Oem_MemAlloc( SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT ) ) );
    ZEROMEM( poRSAModelPubKey, SIZEOF( poRSAModelPubKey ) );

    /* Validate source certificate */
    ChkDR( ParseWMDRMNDCompanyCert( pWMDRMNDData ) );

    /* Process unsigned template, update missing data if necessary */
    ChkDR( ProcessUnsignedNDTemplate( pWMDRMNDData ) );

    ChkDR( DRMTOOLS_CreateKeyPair( pCryptoCtx,
                                   eDRM_RSA,
                                   pWMDRMNDData->eKeyLength,
    ...
    {
        ChkDR( OEM_RSA_ParsePublicKey_2048BIT( poRSAModelPubKey,
                                               NULL,
                                               NULL,
                                               NULL,
                                               &cbModulus,
    ...
    */
    dasstrCompanyCertCollection.m_cch = pWMDRMNDData->dastrCompanyCert.cchString;

    ChkDR( DRM_XML_GetNodeA( pWMDRMNDData->dastrCompanyCert.pszString,
                            &dasstrCompanyCertCollection,
                            &g_dastrTagCertificateCollection,
                             NULL,
                             NULL,
    ...
    dastrCompanyCerts.pszString = pWMDRMNDData->dastrCompanyCert.pszString
    ...
    ChkDR( DRM_ND_CB_CreateCertificateFile( pXMLCtxA,
                                            certtypeRoot,
                                            poRSAModelPubKey,
                                            (const DRM_ANSI_CONST_STRING*) &pWMDRMNDData->dastrCompanyPubKeyModulusB64,
                                            &g_dastrWMDRMCertExponent,
                                            &pWMDRMNDData->oCompanyRSAPrivKey,
                                            (DRM_ANSI_CONST_STRING*)&pWMDRMNDData->dastrUnsignedTemplate,
                                            &dastrCompanyCerts,
                                            pCryptoCtx ) );
    {
    ...
    fResult = Oem_File_Write( f_pState->pWMDRMNDData->hModelCert,
                              pXMLCtxA->rgchXML,
                              pXMLCtxA->cchBuffer,
                             &cbWritten);
    ChkBOOL( fResult && (cbWritten == pXMLCtxA->cchBuffer ), DRM_E_FILE_WRITE_ERROR );


    /* Save model private key */
    ChkDR( GenerateRSAPrivKeyFile( poRSAModelPrivKey, pWMDRMNDData->hModelCertKey ) );

ErrorExit:
    SAFE_OEM_FREE( pXMLCtxA );
    if( poRSAModelPubKey != NULL )
    {
        OEM_RSA_ZeroPublicKey_2048BIT( poRSAModelPubKey );
        SAFE_OEM_FREE( poRSAModelPubKey );
    }
    if( poRSAModelPrivKey != NULL )
    {
        OEM_RSA_ZeroPrivateKey_2048BIT( poRSAModelPrivKey );
    ...
    DRM_WCHAR** argv )
{
    ...
    DRM_CONST_STRING dstrWMDRMNDCompanyCert          = EMPTY_DRM_STRING;
    DRM_CONST_STRING dstrWMDRMNDModelCert            = EMPTY_DRM_STRING;
    DRM_CONST_STRING dstrWMDRMNDModelKeyFile         = EMPTY_DRM_STRING;
    DRM_CONST_STRING dstrWMDRMNDUnsignedTemplate     = EMPTY_DRM_STRING;
    ...
    */
    ...
    for( iParam = 1; iParam < argc; iParam++ )
    {
        ...
        case ND_SETBITS | (DRM_DWORD)(WCHAR_CAST( 'b' )):
            ASSIGN_DRM_STRING( dstrWMDRMNDCompanyCert, dstrParam );
            break;

        case              (DRM_DWORD)(WCHAR_CAST( 'm' )):
        case ND_SETBITS | (DRM_DWORD)(WCHAR_CAST( 'g' )):
            ASSIGN_DRM_STRING( dstrWMDRMNDModelCert, dstrParam );
            break;

        case              (DRM_DWORD)(WCHAR_CAST( 'n' )):
        case ND_SETBITS | (DRM_DWORD)(WCHAR_CAST( 'h' )):
            ASSIGN_DRM_STRING( dstrWMDRMNDModelKeyFile, dstrParam );
            break;

        case              (DRM_DWORD)(WCHAR_CAST( 'v' )):
        case ND_SETBITS | (DRM_DWORD)(WCHAR_CAST( 'u' )):
            ASSIGN_DRM_STRING( dstrWMDRMNDUnsignedTemplate, dstrParam );
            break;
    ...
    if ( dstrDACResponseFile.cchString             > MAX_FILENAME
      ...
      || dstrWMDRMNDCompanyCert.cchString          > MAX_FILENAME
      || dstrWMDRMNDModelCert.cchString            > MAX_FILENAME
      || dstrWMDRMNDModelKeyFile.cchString         > MAX_FILENAME
      || dstrWMDRMNDUnsignedTemplate.cchString     > MAX_FILENAME )
    {
    ...
    if( dstrOEMPrivateKeyFile.cchString    != 0 ) /* all cert types share OEM privkey file data */
    {
        ...
        /*
        ** Check if we can build WMDRMND certificate that uses different unsigned template file.
        */
        if ( dstrWMDRMNDCompanyCert.cchString != 0
          || dstrWMDRMNDModelCert.cchString != 0
          || dstrWMDRMNDModelKeyFile.cchString != 0
          || dstrWMDRMNDUnsignedTemplate.cchString != 0 )
        {
            if ( dstrWMDRMNDCompanyCert.cchString == 0
              || dstrWMDRMNDModelCert.cchString == 0
              || dstrWMDRMNDModelKeyFile.cchString == 0
              || dstrWMDRMNDUnsignedTemplate.cchString == 0 )
            {
                if ( dwMappingFlags == 0 )
                {
                    printf( "Warning: to build WMDRMND certificate these parameters are required: -f, -v, -c, -m, -n\n ");
                }
    ...
    */
    ...
    {
        /*
        ** Read WMDRMND unsigned template
        */
        printf( "Reading WMDRMND unsigned template...\n" );
        ChkDR( DRMTOOLS_ReadBinaryFileData( &dstrWMDRMNDUnsignedTemplate,
                                            (DRM_BYTE**)&g_oState.pWMDRMNDData->dastrUnsignedTemplate.pszString,
                                            &g_oState.pWMDRMNDData->dastrUnsignedTemplate.cchString) );

        if ( g_oState.pWMDRMNDData->dastrUnsignedTemplate.cchString == 0 )
        {
            printf( "WMDRMND unsigned template cannot be empty\n" );
            ChkDR( DRM_E_FILE_READ_ERROR );
        }

        /*
        ** Read WMDRMND Company cert
        */
        printf( "Reading PlayReady-PD company certificate...\n" );
        ChkDR( DRMTOOLS_ReadBinaryFileData( &dstrWMDRMNDCompanyCert,
                                            (DRM_BYTE**)&g_oState.pWMDRMNDData->dastrCompanyCert.pszString,
                                            &g_oState.pWMDRMNDData->dastrCompanyCert.cchString) );

        if ( g_oState.pWMDRMNDData->dastrCompanyCert.cchString == 0 )
        {
            printf( "WMDRMND company certificate cannot be empty\n" );
            ChkDR( DRM_E_FILE_READ_ERROR );
        }

        /*
        ** Open Model cert and Model key files for WMDRMND
        */
        ChkDR( DRMTOOLS_OpenOutputFile( &dstrWMDRMNDModelCert,
                                        &g_oState.pWMDRMNDData->hModelCert ) );
        ChkDR( DRMTOOLS_OpenOutputFile( &dstrWMDRMNDModelKeyFile,
                                        &g_oState.pWMDRMNDData->hModelCertKey ) );
    }
    /*
    ** Parse OEM keys
    */
    ...
    if( fPlayReadyCertRequired )
    {
        ...
        */
        ...
        ChkDR( GenerateBinaryModelCertificate() );
    ...
    if ( fNDCertRequired )
    {
        printf( "Generating WMDRMND certificate...\n" );
        ChkDR( GenerateWMDRMNDModelCert( &g_oState ) );
    }
