...
static DRM_RESULT DRM_CALL _PrintELS(
    __in_ecount(f_cbEmbeddedObject) DRM_BYTE   *f_pbEmbeddedObject,
    __in                            DRM_DWORD   f_cbEmbeddedObject )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
**
**********************************************************************/
static DRM_RESULT DRM_CALL _PrintEnvelopeFileInfo( __in DRM_ENVELOPED_FILE_CONTEXT *f_pEnvFile )
{
    DRM_RESULT          dr            = DRM_SUCCESS;
...
**
**********************************************************************/
static DRM_RESULT DRM_CALL _PrintAsfFileInfo( __in DRM_TOOLS_ASF_CONTEXT *f_poAsfFile )
{
    DRM_RESULT dr      = DRM_SUCCESS;
    DRM_DWORD  iStream = 0;
...
static DRM_RESULT _PrintB64StringAsGuidString( __in const DRM_CONST_STRING *f_pdstrB64 )
{
    DRM_RESULT dr       = DRM_SUCCESS;
...
static DRM_RESULT _PrintServiceId( __in const DRM_CONST_STRING *f_pdstrHeader )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT _PrintKID( __in const DRM_CONST_STRING *f_pdstrHeader )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
**
**********************************************************************/
static DRM_RESULT _PrintFileInfo( __in DRM_MEDIA_FILE_CONTEXT *f_poMediaFile )
{
    DRM_RESULT dr            = DRM_SUCCESS;
    ...
    if (    ( DRM_PRO_IsValidObject( f_poMediaFile->rgbDRMHeaderData, f_poMediaFile->cbDRMHeaderData, &dwRecordTypes ) == DRM_SUCCESS )
         && ( dwRecordTypes != 0 ) )
    {
    ...
    else
    {
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen( dstrDRMHeader.pwszString ), &dstrDRMHeader.cchString ) );
    }
