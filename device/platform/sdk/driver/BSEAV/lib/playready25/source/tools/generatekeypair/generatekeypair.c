...
static DRM_RESULT _OutputArrayToText(
    ...
    __in                                      const DRM_DWORD  f_dwElemCount,
    __in                                      const DRM_DWORD  f_dwElemSize,
    __in                                      const DRM_DWORD  f_dwElemPerLine,
    __in_z __format_string                    const DRM_CHAR  *f_pchNumberFormat )
{
    ...
    DRMASSERT( f_fp != NULL );
    __analysis_assume( f_fp != NULL );
    DRMASSERT( f_pArray != NULL );
    __analysis_assume( f_pArray != NULL );
    ...
    for ( i = 0; i < f_dwElemCount; i++ )
    {
        ...
        {
            dwValue = *(const DRM_BYTE*)((DRM_BYTE*)f_pArray + i*f_dwElemSize);
        }
        else if ( SIZEOF( DRM_WORD ) == f_dwElemSize )
        {
            dwValue = *(const DRM_WORD*)((DRM_BYTE*)f_pArray + i*f_dwElemSize);
        }
        else if ( SIZEOF( DRM_DWORD ) == f_dwElemSize )
        {
            dwValue = *(const DRM_DWORD*)((DRM_BYTE*)f_pArray + i*f_dwElemSize);
        }
...
static DRM_RESULT _OutputECCKeysToText(
    ...
    __in                       const DRM_DWORD   f_cbPubKey,
    __in_bcount( f_cbPrivKey ) const DRM_BYTE   *f_pPrivKey,
    __in                       const DRM_DWORD   f_cbPrivKey )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
static DRM_RESULT _OutputRSAKeysToText(
    __in       FILE                        *f_fp,
    __in const DRM_RSA_PUBLIC_KEY_2048BIT  *f_pPubKey,
    __in const DRM_RSA_PRIVATE_KEY_2048BIT *f_pPrivKey )
{
    DRM_RESULT dr = DRM_SUCCESS;
    const INTERNAL_DRM_RSA_PRIVATE_KEY_2048BIT *pPvtKeyInternal = ( const INTERNAL_DRM_RSA_PRIVATE_KEY_2048BIT * )f_pPrivKey;
    const INTERNAL_DRM_RSA_PUBLIC_KEY_2048BIT  *pPubKeyInternal = ( const INTERNAL_DRM_RSA_PUBLIC_KEY_2048BIT  * )f_pPubKey;

    DRMASSERT( f_fp != NULL );
    __analysis_assume( f_fp != NULL );
    DRMASSERT( f_pPubKey != NULL );
    DRMASSERT( f_pPrivKey != NULL );
    __analysis_assume( pPubKeyInternal != NULL );
    ...
    __inout DRM_CRYPTO_CONTEXT                        *f_pCryptoCtx )
{
    ...
    */
    ...
    case eDRM_ECC_P256:
        ...
        break;
    case eDRM_RSA:
        cbPrivKey = SIZEOF( DRM_RSA_PRIVATE_KEY_2048BIT );
        cbPubKey  = SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT );
        break;
    ...
    */
    if ( NULL != f_pState->Outfp )
    {
        ...
        {
            ChkDR( _OutputRSAKeysToText( f_pState->Outfp, (DRM_RSA_PUBLIC_KEY_2048BIT*) pPubKey, (DRM_RSA_PRIVATE_KEY_2048BIT*) pPrivKey ) ) ;
        }
    ...
    */
    if ( NULL != f_pState->OutfpXml )
    {
        ...
        {
            ChkDR( _OutputRSAKeysToXML( f_pState->OutfpXml, (DRM_RSA_PUBLIC_KEY_2048BIT*) pPubKey, (DRM_RSA_PRIVATE_KEY_2048BIT*) pPrivKey ) ) ;
        }
    } /* end if */
