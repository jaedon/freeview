...
static DRM_RESULT DRM_CALL HTMLEncode(
    __in_ecount(f_cchIn)   const DRM_CHAR  *f_rgchIn,
    __in                         DRM_DWORD  f_cchIn,
    __out_ecount_opt(*f_pcchOut) DRM_CHAR  *f_rgchOut,
    __inout                      DRM_DWORD *f_pcchOut )
{
    DRM_RESULT       dr         = DRM_SUCCESS;
...
**
**********************************************************************/
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_FUNCTION_NEEDS_REVIEW, "vprintf usage reviewed." )
DRM_VOID DRM_CALL LogMessage(
    __in_z __format_string const DRM_CHAR *f_pszMessage, ... )
{
    va_list va;
...
DRM_RESULT DRM_CALL NetClientSendRequest(
    ...
    __inout                     HINTERNET *f_phConnect,
    __inout                     HINTERNET *f_phHttpFile,
    __in_ecount(f_cbChallenge)  DRM_BYTE  *f_pbChallenge,
    __in                        DRM_DWORD  f_cbChallenge,
    __in_opt              const DRM_CHAR  *f_pszHeader,
    __in                        DRM_DWORD  f_dwLevel,
    __in                        DRM_BOOL   f_fUseSOAP,
    __in_z_opt            const DRM_CHAR  *f_pszURL )
{
    ...
    else
    {
        ...
        else
        {
            if ( f_fUseSOAP )
            {
                ...
                ChkDR( DRM_DWordToWord( g_oServer.m_dwHttpPort, &oUrlComponents.nPort ) );
            ...
            else
            {
                ...
                ChkDR( DRM_DWordToWord( g_oFallback.m_dwHttpPort, &oUrlComponents.nPort ) );
    ...
    *f_phHttpFile = HttpOpenRequestA( *f_phConnect,
                            ...
                            f_cbChallenge ) )
    {
        ...
        */
        DRM_RESULT drNetwork = HRESULT_FROM_WIN32( GetLastError() );
...
static DRM_RESULT DRM_CALL NetClient(
    __in                         OTADLA_NET_COMMAND_TYPE   f_Type,
    __in_ecount(f_cbChallenge)   DRM_BYTE                 *f_pbChallenge,
    __in                         DRM_DWORD                 f_cbChallenge,
    __in                         DRM_BOOL                  f_fUseSOAP,
    __in_z_opt             const DRM_CHAR                 *f_pszURL,
    __out_ecount(*f_pcbResponse) DRM_BYTE                **f_ppbResponse,
    __out                        DRM_DWORD                *f_pcbResponse )
{
    ...
    *f_pcbResponse = (DRM_DWORD)atol( ( DRM_CHAR * )rgbHeaderBuffer );
...
DRM_RESULT DRM_CALL SetContentHeaderUsingRMHeader(
    __in       DRM_APP_CONTEXT        *f_poAppContext,
    __in const DRM_CONST_STRING       *f_pdstrHeaderFile )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    ChkBOOL( Oem_File_SetFilePointer( Infp,
                                      0,
                                      OEM_FILE_END,
                                      &cbHeaderData ), DRM_E_FILE_SEEK_ERROR );
    ...
    ChkBOOL( cbRead == cbHeaderData, DRM_E_FILE_READ_ERROR );
...
DRM_RESULT DRM_CALL ProcessAdditionalInformation(
    ...
    __out_ecount(*f_pcchData)       DRM_CHAR        *f_pchData,
    __inout                         DRM_DWORD       *f_pcchData )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL GenerateDomainJoinChallenge(
    ...
    __in                          DRM_BOOL         f_fOldRevision,
    __in                          DRM_BOOL         f_fEchoDomainID,
    __out_ecount(*f_pcbChallenge) DRM_BYTE       **f_ppbChallenge,
...
DRM_RESULT DRM_CALL GenerateLicenseChallenge(
    __in                                    DRM_APP_CONTEXT           *f_poAppContext,
    __in                                    DRM_BOOL                   f_fDomain,
    __in                                    DRM_BOOL                   f_fDomainExcluded,
    __in                                    DRM_BOOL                   f_fAESPackage,
    __in                                    DRM_BOOL                   f_fProtocolFallback,
    __in                                    DRM_LICENSE_PROTOCOL_TYPE  f_ProtocolVersion,
    __in                              const DRM_DOMAIN_ID             *f_poDomainID,
    __out_ecount_opt(*f_pcbChallenge)       DRM_BYTE                 **f_ppbChallenge,
    __inout                                 DRM_DWORD                 *f_pcbChallenge,
    __out_ecount(*f_pcchURL)                DRM_CHAR                  *f_pchURL,
    __inout                                 DRM_DWORD                 *f_pcchURL )
{
    DRM_RESULT dr = DRM_SUCCESS;
    const DRM_CONST_STRING *rgpdstrRights[ 1 ]= { &g_dstrWMDRM_RIGHT_PLAYBACK };
    ...
    */
    ...
    if ( dr == DRM_E_BUFFERTOOSMALL )
    {
        if ( f_ProtocolVersion == eV2Protocol )
        {
            ...
            ** prefix.
            */
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( V2_LICENSE_CHALLENGE_PREFIX ), &cbOffset ) );
...
DRM_RESULT DRM_CALL ProcessLicenseChallengeResponse(
    ...
    __in_ecount(f_cbResponse) DRM_BYTE             *f_pbResponse,
    __in                      DRM_DWORD             f_cbResponse,
    __out                     DRM_LICENSE_RESPONSE *f_poLicResponse )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL ProcessLicenseAckResponse(
    IN const DRM_APP_CONTEXT *f_poAppContext,
...
**
**********************************************************************/
static DRM_RESULT DRM_CALL GenerateMeterCertChallenge(
    __in                                               DRM_APP_CONTEXT *f_poAppContext,
    ...
    IN OUT   DRM_DWORD       *f_pcbMeterDataChallenge )
{
    ...
    if ( dr == DRM_E_BUFFERTOOSMALL )
    {
        ...
        if ( f_fIsV2 )
        {
            ...
            dr = DRM_STR_UTF16toUTF8( ( const DRM_WCHAR * )pbTemp1, cbTemp1, NULL, 0, &cbTemp2 );
            ...
            */
            dr = HTMLEncode( ( const DRM_CHAR * )pbTemp2, cbTemp2, NULL, &cbTemp3 );

            if ( DRM_FAILED( dr ) )
            {
                if ( dr == DRM_E_BUFFERTOOSMALL )
                {
                    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( V2_METERDATA_CHALLENGE_PREFIX ), &cbOffset ) );
...
static DRM_RESULT DRM_CALL HandleLicenseError(
    __in                      const DRM_APP_CONTEXT      *f_poAppContext,
    __in                            OTADLA_STATE         *f_poState,
    __in                      const DRM_LICENSE_RESPONSE *f_poLicResponse,
    __in_ecount(f_cbResponse) const DRM_BYTE             *f_pbResponse,
    __in                            DRM_DWORD             f_cbResponse )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    IN OTADLA_STATE *f_poState )
{
    ...
    DRM_BOOL            fEnvReadInitialized = FALSE;
    ...
    */
    ...
    if(  f_poState->m_fExecute )
    {
        ...
        rgpdstrRights[ 0 ] = &g_dstrExecuteRight;
    }
    else
    {
...
ErrorExit:
    ...
    if( fEnvReadInitialized )
    {
        Drm_Envelope_Close( &f_poState->m_poMediaFile->FormatContext.oEnvFile );
    }
...
DRM_RESULT DRM_CALL ParseHttpParams(
    ...
    __inout       OTADLA_HttpParams *f_poHttpParams )
{
    DRM_RESULT      dr             = DRM_SUCCESS;
    URL_COMPONENTSA oUrlComponents = { 0 };
...
ErrorExit:
    ...
    if( DRM_FAILED( dr ) && f_pdstrURL != NULL )
    {
    ...
    IN DRM_BOOL                f_fExecuteRight )
{
    ...
    */
    if( !f_fJoinDomainOnly && !f_fLeaveDomainOnly && !f_fRevocationOnly && !f_fHeaderOnly )
    {
        ...
        */
        ChkBOOL( f_poMediaFile != NULL && f_poMediaFile->fOpened, DRM_E_FILE_READ_ERROR );
