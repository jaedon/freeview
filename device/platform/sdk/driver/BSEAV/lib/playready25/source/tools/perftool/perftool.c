...
static void PackedCharsToNative( __inout_ecount(cch) DRM_CHAR *f_pPackedString, DRM_DWORD cch );
...
DRM_RESULT ProcessConfigFile(DRM_CONST_STRING dstrConfigFile, DRM_CONST_STRING dstrTestCaseID)
{
    ...
    ChkDR( StringCchPrintfA(chOutput,MAX_LINE_SIZE,"Test Case: %s \n", chTestCaseID) );
    ...
    while(!(feof(fpConfigFile)) && fgets(chLine,MAX_LINE_SIZE,fpConfigFile))
    {
        ...
        pFunctionName++;
        ChkDR( StringCchLengthA(pFunctionName, MAX_LINE_SIZE, (size_t *)&cchFunctionNameLen) );
        if(pFunctionName[cchFunctionNameLen - 1] == '\n')
        {
            pFunctionName[cchFunctionNameLen - 1] = '\0';
            cchFunctionNameLen--;
        }
        ChkDR( StringCchLengthA(chLine,MAX_LINE_SIZE, (size_t*)&cchLineLen) );
        ChkDR( StringCchCopyNA(pModuleName,MAX_LINE_SIZE,chLine, cchLineLen - cchFunctionNameLen - 1) );
        ...
        ChkDR( StringCchPrintfA(chOutput,MAX_LINE_SIZE,"%s %lu:%lu %lu:%lu\r\n",
...
DRM_RESULT GetIndex(const DRM_CHAR *pStr, DRM_DWORD *pdwRetVal)
{
    ...
    ChkDR( StringCchLengthA(pStr,MAX_LINE_SIZE, (size_t *)&cchStr) );
    ChkArg(cchStr > 0);
...
Returns: DRM_RESULT indicating a pass or a fail
****************************************************************/
DRM_RESULT PrintNode(const PERF_SCOPE_NODE *pNode, DRM_LONG n_indent)
{
    ...
    */
    StringCchPrintfA(cWriteBuffer,MAX_LINE_SIZE,"%s %s - Duration: %.3g ms - Level: %lu \r\n",cSpaceHolder, pPerfCounter->m_pszScopeName, (float)pNode->m_dwDuration / 1000.0, pNode->m_dwScopeLevel);

    ChkDR( StringCchLengthA(cWriteBuffer,MAX_LINE_SIZE, (size_t *)&cchLen) );
    ChkDR( DRM_SizeTToDWord( cchLen, &dwcchLen ) );
    ChkDR(Oem_File_Write(fp_output, cWriteBuffer, dwcchLen, &dwWritten));
 ...
 DRM_RESULT ParseStackLog()
{
    ...
    do
    {
        ...
        while(i < (DRM_LONG)dwRead)
        {
            ...
            ChkDR(StringCchPrintfA(chOutput,MAX_LINE_SIZE,"%s %lu \r\n", pPerfCounter->m_pszScopeName, dwStackUsage));
            ChkDR( StringCchLengthA(chOutput,MAX_LINE_SIZE, (size_t *)&cchLen));
            ChkDR( DRM_SizeTToDWord( cchLen, &dwcchLen ) );
            ChkBOOL(Oem_File_Write(fp_output, chOutput, dwcchLen, &dwWritten),DRM_E_FILE_WRITE_ERROR);
        }
    }while(dwTotalRead < dwFileSize);
...
static void PackedCharsToNative( __inout_ecount(cch) DRM_CHAR *f_pPackedString, DRM_DWORD cch )
{
    ...
    if( f_pPackedString != NULL && cch != 0 )
    {
        ...
        {
            f_pPackedString[ich] = (DRM_CHAR)GET_CHAR( f_pPackedString, ich );
        }
        f_pPackedString[0]   = (DRM_CHAR)GET_CHAR( f_pPackedString, 0 ) ;
