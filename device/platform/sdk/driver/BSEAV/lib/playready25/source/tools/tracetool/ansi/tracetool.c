...
DRM_RESULT DRM_CALL IsBlankLine(
    __in_z const DRM_CHAR *f_pszData )
{
    DRM_BOOL        fResult = TRUE;
    const DRM_CHAR *ptr     = f_pszData;
...
**                          DRM_E_FILE_WRITE_ERROR if file write error occurs.
**
**********************************************************************/
    ...
    __in_z DRM_CHAR *f_pszData )
{
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(f_pszData), &cch ) );
    ChkBOOL( Oem_File_Write( f_Outfp,
                             f_pszData,
                             cch,
                             &cbWritten ), DRM_E_FILE_WRITE_ERROR );

    ChkBOOL( cbWritten == DRMCRT_strlen( f_pszData ), DRM_E_FILE_WRITE_ERROR );
...
DRM_RESULT DRM_CALL PrintMessage(
    __in const OEM_FILEHDL    f_Outfp,
    __in const DRMSYSTEMTIME *f_ptmTraceTime,
...
DRM_RESULT DRM_CALL DumpStringPacket(
    ...
    __in       OEM_FILEHDL   f_Outfp,
    __in const TRACE_HEADER *f_poHeader )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL DumpTraceLog(
    __in const DRM_CONST_STRING *f_pdstrTraceLogFile,
