...
#define DRM_BUILDING_DRMCERTPARSER_C
...
**
******************************************************************************/
static DRM_NO_INLINE DRM_RESULT DRM_CALL  _ExtractValueFromAttribute(
    __in  const DRM_CHAR              *f_pszBase,
    __in  const DRM_SUBSTRING         *f_pdasstrAttribValue,
...
static DRM_NO_INLINE DRM_RESULT _VerifyCertificateSignature(
    ...
    __out                                        DRM_SUBSTRING       *f_pdasstrPubkeySigner,
...
static DRM_NO_INLINE DRM_RESULT _GetPublicKey(
    __in const DRM_CHAR       *f_pszBase,
    __in const DRM_SUBSTRING  *f_pdasstrCert,
    __in       DRM_SUBSTRING  *f_pdasstrPubkey )
{
    DRM_RESULT    dr = DRM_SUCCESS;
    DRM_SUBSTRING dasstrData = EMPTY_DRM_SUBSTRING;
    DRM_SUBSTRING dasstr0    = EMPTY_DRM_SUBSTRING;
...
static DRM_NO_INLINE DRM_RESULT _ExtractPublicKey(
    __in const DRM_CHAR                   *f_pszBase,
    __in const DRM_SUBSTRING              *f_pdasstrKeyInfo,
    __in       DRM_RSA_PUBLIC_KEY_2048BIT *f_pPubkeyRSA )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    ChkDR( OEM_RSA_GetKeyLengthFromModulusLength_2048BIT( cbModulus, &eKeyLength ) );

    ChkDR( OEM_RSA_SetPublicKey_2048BIT(
        eKeyLength,
    ...
    __in                                        DRM_CRYPTO_CONTEXT   *f_pCryptoCtx )
{
    ...
    DRM_BYTE                    rgbSignature     [__CB_DECL(DRM_RSA_CB_MODULUS_MAX_2048BIT)];
    DRM_DWORD                   cbSHAEncoded         = SIZEOF(rgbSHAEncoded);
    DRM_DWORD                   cbSignature          = SIZEOF(rgbSignature);
    DRM_RSA_PUBLIC_KEY_2048BIT  pubkeyRSA            = { 0 };

    DRM_PROFILING_ENTER_SCOPE( PERF_MOD_NDT, PERF_FUNC_DRM_WCP_VerifyDigitalSignature );

    ChkArg( f_pszBase          != NULL
        &&  f_pdasstrDataInclusive      != NULL
        &&  f_pdasstrSignatureInclusive != NULL
        &&  f_pCryptoCtx       != NULL );

    ChkDR( DRM_XML_GetNodeA( f_pszBase,
                             f_pdasstrSignatureInclusive,
                            &g_dastrTagSignature,
                             NULL,
                             NULL,
                             0,
    ...
    if (f_fWMDRMNET == DRM_DSIG_TYPE_WMDRMNET)
    {
        ChkDR( OEM_RSA_PssVerify_2048BIT(
                                  &pubkeyRSA,
                                   f_pdasstrDataInclusive->m_cch,
                                  ( const DRM_BYTE * )f_pszBase,
                                   f_pdasstrDataInclusive->m_ich,
                                   cbSignature,
                                   rgbSignature,
                                   f_pCryptoCtx ) );
    }
    ...
    {
        ChkDR( OEM_RSA_PssVerify_2048BIT( &pubkeyRSA,
...
ErrorExit:
    OEM_RSA_ZeroPublicKey_2048BIT( &pubkeyRSA );
    DRM_PROFILING_LEAVE_SCOPE;
    return dr;
}
