    ...
    *f_pdwSize = (DRM_DWORD) strtoul( f_szSize , &pchEnd, 0);
    ...
    if( *f_pdwSize == 0 && f_szSize == pchEnd )
    {
        ...
        else if( DRMCRT_strncmp( f_szSize, "RSA", 3 ) == 0 )
        {
            ...
            {
                *f_pdwSize = DRM_RSA_CB_MAX_PLAINTEXT( eDRM_RSA_KEY_LENGTH_1024_2048BIT );
                pchEnd = f_szSize + 22;
            }
            else if( DRMCRT_strncmp( f_szSize, "RSA_2048_MAX_PLAINTEXT", 22 ) == 0 )
            {
                *f_pdwSize = DRM_RSA_CB_MAX_PLAINTEXT( eDRM_RSA_KEY_LENGTH_2048_2048BIT );
                pchEnd = f_szSize + 22;
            }
            else if( DRMCRT_strncmp( f_szSize, "RSA_PUBKEY", 10 ) == 0 )
            {
                *f_pdwSize = DRM_RSA_CB_PUBKEY_2048BIT;
                pchEnd = f_szSize + 10;
            }
            else if( DRMCRT_strncmp( f_szSize, "RSA_PRIVKEY", 11 ) == 0 )
            {
                *f_pdwSize = DRM_RSA_CB_PRIVKEY_2048BIT;
                pchEnd = f_szSize + 11;
            }
            else if( DRMCRT_strncmp( f_szSize, "RSA_1024_MODULUS", 16 ) == 0 )
            {
                *f_pdwSize = DRM_RSA_CB_MODULUS( eDRM_RSA_KEY_LENGTH_1024_2048BIT );
                pchEnd = f_szSize + 16;
            }
            else if( DRMCRT_strncmp( f_szSize, "RSA_2048_MODULUS", 16 ) == 0 )
            {
                *f_pdwSize = DRM_RSA_CB_MODULUS( eDRM_RSA_KEY_LENGTH_2048_2048BIT );
                pchEnd = f_szSize + 16;
            }
        }
...
static DRM_RESULT DRM_CALL _ParsePKAlgorithmType(
    __in_z      const char                              *f_szAlgorithm,
...
DRM_RESULT DRM_CALL Test_PK_GenKeyPair_Encrypt_Decrypt( long argc, __in_ecount(argc) char **argv )
{
    ...
    PUBKEY          *tmpPubKey   = g_pPubKey;
    const PRIVKEY   *tmpPrivKey  = g_pPrivKey;
    ...
    DRM_BYTE        *tmpCryCtx   = g_pbCryptoContext;
    const DRM_BYTE  *tmpRandBuf  = g_pbRandBufIn;
    DRM_BYTE        *tmpCipher   = g_pbCipher;
    DRM_BYTE        *tmpBufOut   = g_pbDecryptBufOut;
    ...
    ChkDR( DRM_PK_GenKeyPair( tmpCryCtx,
                              eDRM_ECC_P160,
    ...
    ChkDR( DRM_PK_Decrypt( tmpCryCtx,
                           eDRM_ECC_P160,
    ...
    ChkBOOL( MEMCMP( tmpRandBuf, tmpBufOut, DRM_ECC160_PLAINTEXT_LEN ) == 0,
             DRM_E_TEST_PKCRYPTO_FAILURE );

    MEMCPY( tmp, tmpBufOut, DRM_ECC160_PLAINTEXT_LEN );
...
DRM_RESULT DRM_CALL Test_PK_Sign_Verify( long argc, __in_ecount(argc) char **argv )
{
    ...
    PUBKEY           *pubKey2     = NULL;
    PRIVKEY          *privKey2    = NULL;
    const PUBKEY     *tmpPubKey   = NULL;
    const PRIVKEY    *tmpPrivKey  = NULL;
    DRM_DWORD         cbPubKey    = DRM_ECC160_PUBLIC_KEY_LEN;
    DRM_DWORD         cbPrivKey   = DRM_ECC160_PRIVATE_KEY_LEN;
...
DRM_RESULT DRM_CALL Test_DRM_CPHR_Init( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    cbContentKey = (DRM_DWORD)atoi( argv[0] );
...
DRM_RESULT DRM_CALL Test_DRM_CPHR_InitDecrypt( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argv[1] != NULL )
    {
        ...
        {
            ibLast15 = (DRM_DWORD)atoi( argv[1] );
        }
...
DRM_RESULT DRM_CALL Test_DRM_SHA256_Update( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    ChkDR( DRM_SHA256_Update( pSha256Context,
                              pBuffer,
                              (DRM_DWORD)atol( argv[ 2 ] ) ) ); /* len of buffer */
...
DRM_RESULT DRM_CALL Test_DRM_SHA256_UpdateOffset( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    ChkDR( DRM_SHA256_UpdateOffset( pSha256Context,
                                    ...
                                    (DRM_DWORD)atol( argv[3] ) ) ); /* len of buffer */
...
DRM_RESULT DRM_CALL Test_DRM_HMAC_Init( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argc == 5  && argv[4] != NULL )
    {
        ChkBOOL( LoadTestFile( "files", argv[4], &pbTestKey, &cbDatalength), DRM_E_FILE_READ_ERROR );
    }
    else if ( argv[1] != NULL )
    {
...
DRM_RESULT DRM_CALL Test_DRM_HMAC_Update( long argc, __in_ecount(argc) char **argv )
{
    ...
    {
        ChkBOOL( LoadTestFile( "files", argv[3], &pbData, &cbDatalength), DRM_E_FILE_READ_ERROR );
    }
    else if ( argv[1] != NULL )
    {
        GET_SIZE( 1, cbDatalength );
    ...
    */
    ...
    /* DRM_HMAC_Update should have failed if pHmacContext is NULL */
    DRMASSERT( pHmacContext != NULL );
    __analysis_assume( pHmacContext != NULL );
...
DRM_RESULT DRM_CALL Test_DRM_HMAC_Finalize( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argc == 4 && argv[3] != NULL )
    {
        ChkBOOL( LoadTestFile( "files", argv[3], &pbKnownHash, &cbDatalength), DRM_E_FILE_READ_ERROR );
    }
...
DRM_RESULT DRM_CALL Test_RC4_Cipher( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argv[1] != NULL )
    {
        cbKeyLen = (DRM_DWORD)atoi( argv[1] );
    ...
    if ( argv[2] != NULL )
    {
        cbDataSize = (DRM_DWORD)atoi( argv[2] );
...
DRM_RESULT DRM_CALL Test_DES_Cipher( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argv[2] != NULL )
    {
        cbInDataSize = (DRM_DWORD)atoi( argv[2] );
    ...
    if ( argv[3] != NULL )
    {
        cbOutDataSize = (DRM_DWORD)atoi( argv[3] );
...
DRM_RESULT DRM_CALL LoadSymmetricTestData( long argc, __inout_ecount( argc ) char **argv )
{
    ...
    if( strncmp( argv[0], "RC4", 3 ) != 0 )
    {
        ...
        ChkArg( argv[4] != NULL );
        eCipher = eDRM_AES_COUNTER_CIPHER;
        ChkBOOL( LoadTestFile( "files", argv[4], &pbDataID, &cbDataID), DRM_E_FILE_READ_ERROR );
        NETWORKBYTES_TO_QWORD(g_qwSymmetricDataID, pbDataID, 0);
    }

    ChkBOOL( LoadTestFile( "files", argv[1], &pbKey, &cbKeySize ), DRM_E_FILE_READ_ERROR );
    ChkBOOL( LoadTestFile( "files", argv[2], &pbClearData, &cbClearData ), DRM_E_FILE_READ_ERROR );
    ChkBOOL( LoadTestFile( "files", argv[3], &pbCipherData, &cbCipherData ), DRM_E_FILE_READ_ERROR );
...
DRM_RESULT DRM_CALL Test_DRM_AES_Cipher( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    if ( fRandomTest )
    {
        ...
        {
            cbDataLen = (DRM_DWORD)strtoul( argv[2], &pszTemp, 0 );
        }
        else
        {
    ...
    */
    ...
    else if ( eMode == eDRM_AES_CTR_MODE )
    {
        ...
        ctrContext.qwBlockOffset = qwZero;
        ctrContext.bByteOffset   = 0x00;
        DRM_PERF_LogMicrosecondCount( eDRM_PERF_LogTag_Enter, eDRM_PERF_LogFunction_Oem_Aes_CtrProcessData );
        dr = Oem_Aes_CtrProcessData( &g_aesKey, pbData, cbDataLen, &ctrContext );
        DRM_PERF_LogMicrosecondCount( eDRM_PERF_LogTag_Exit, eDRM_PERF_LogFunction_Oem_Aes_CtrProcessData );
        ChkDR( dr );
    }
    else /* if ( eMode == eDRM_AES_CBC_MODE ) */
    {
    ...
    /*
    ** Special test for AES_CTR_MODE:
    ** Makes sure that one call equals many on a buffer. 
    */
    if ( eMode == eDRM_AES_CTR_MODE )
    {
        DRM_DWORD i = 0; 
        ctrContext.qwInitializationVector = qwDataId;
        ctrContext.qwBlockOffset = qwZero;
        ...
        {
            ChkDR( Oem_Aes_CtrProcessData( &g_aesKey, pbData+i, 1, &ctrContext ) );
        }        
...
DRM_RESULT DRM_CALL Test_DRM_OMAC( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    ...
    {
        rgbSignTag[0] = (DRM_BYTE)~rgbSignTag[0];
    }
    ...
    )
{
    ...
    DRM_RSA_SUPPORTED_KEY_LENGTHS_4096BIT eReturnedKeyLength                    = eDRM_RSA_KEY_LENGTH_UNKNOWN_4096BIT;
    DRM_BYTE    rgbReturnedModulus[__CB_DECL( DRM_RSA_CB_MODULUS_MAX_4096BIT )] = { 0 };
    ...
    eKeyLength = ( DRM_RSA_SUPPORTED_KEY_LENGTHS_4096BIT )atol( argv[ 0 ] );
    ...
    switch ( eKeyLength )
    {
    case eDRM_RSA_KEY_LENGTH_1024_4096BIT:
    ...
    case eDRM_RSA_KEY_LENGTH_2048_4096BIT:
    ...
    case eDRM_RSA_KEY_LENGTH_4096_4096BIT:
        pbModulus     = g_rgbRSAModulus4096;
        pbPrivExp     = g_rgbRSAPrivExp4096;
        pbPrimes[ 0 ] = g_rgbRSAPrimes4096[ 0 ];
        pbPrimes[ 1 ] = g_rgbRSAPrimes4096[ 1 ];
        break;
    ...
    ChkDR( OEM_RSA_SetPublicKey_4096BIT(  eKeyLength,
                                          SIZEOF( g_rgbRSAPubExp ),
                                          g_rgbRSAPubExp,
                                          cbModulus,
                                          pbModulus,
                                         &g_RsaPubKey ) );
    ChkDR( OEM_RSA_ParsePublicKey_4096BIT( &g_RsaPubKey,
                                           &eReturnedKeyLength,
                                           &cbReturnedPubExp,
                                            rgbReturnedPubExp,
                                           &cbReturnedModulus,
    ...
    ChkDR( OEM_RSA_SetPrivateKey_4096BIT(  eKeyLength,
                                           cbPrivExp,
                                           pbPrivExp,
                                           cbPrime,
                                           pbPrimes[ 0 ],
    ...
    )
{
    ...
    ChkDR( OEM_RSA_ZeroPublicKey_4096BIT( &g_RsaPubKey ) );

    pbKey = ( const DRM_BYTE* )&g_RsaPubKey;
    ...
    ChkDR( OEM_RSA_ZeroPrivateKey_4096BIT( &g_RsaPrivKey ) );

    pbKey = ( const DRM_BYTE* )&g_RsaPrivKey;
    ...
    )
{
    ...
    case eDRM_RSA_KEY_LENGTH_4096_4096BIT:
        pbRSAKnownPlaintext[ 0 ] = g_rgbRSAKnownPlaintext4096[ 0 ];
        pbRSAKnownPlaintext[ 1 ] = g_rgbRSAKnownPlaintext4096[ 1 ];
        pbRSAKnownCiphertext[ 0 ] = g_rgbRSAKnownCiphertext4096[ 0 ];
        pbRSAKnownCiphertext[ 1 ] = g_rgbRSAKnownCiphertext4096[ 1 ];
        break;
        ...
        */
        ChkDR( Oem_Random_GetBytes(  NULL,
                                    &( g_rgbRSAPlaintext[__CB_DECL( 4 )] ),
                                     (DRM_DWORD)( DRM_RSA_CB_MODULUS( eKeyLength ) - 4 ) ) );

        ChkDR( OEM_RSA_Encrypt_4096BIT( &g_RsaPubKey,
    ...
    )
{
    ...
    DRM_RSA_SUPPORTED_KEY_LENGTHS_4096BIT eKeyLength                     = eDRM_RSA_KEY_LENGTH_UNKNOWN_4096BIT;
    DRM_BYTE rgbPlaintext[ __CB_DECL( DRM_RSA_CB_MODULUS_MAX_4096BIT ) ] = { 0 };
    ...
    ChkDR( OEM_RSA_Decrypt_4096BIT( &g_RsaPrivKey,
                                     DRM_RSA_CB_MODULUS( eKeyLength ),
                                     g_rgbRSACiphertext,
                                     rgbPlaintext,
                                    &g_Crypto ) );
    ...
    )
{
    ...
    eKeyLength = ( DRM_RSA_SUPPORTED_KEY_LENGTHS_4096BIT )atol( argv[ 0 ] );
    cbPlaintext = (DRM_DWORD)atol( argv[ 1 ] );
    ...
    */
    ...
    ChkDR( OEM_RSA_OaepEncrypt_4096BIT(  NULL,
                                        &g_RsaPubKey,
                                         cbPlaintext,
                                         g_rgbRSAPlaintext,
                                        &g_cbRsaCiphertext,
    ...
    )
{
    ...
    DRM_BYTE          rgbPlaintext[DRM_RSA_CB_MAX_PLAINTEXT_MAX_4096BIT] = { 0 };
    ...
    ChkArg( cbCiphertext <= DRM_RSA_CB_MODULUS_MAX_4096BIT );

    /*
    ** Decrypt the plaintext and compare buffers
    */
    cbPlaintext2 = SIZEOF( rgbPlaintext );
    ChkDR( OEM_RSA_OaepDecrypt_4096BIT( &g_RsaPrivKey,
                                         cbCiphertext,
                                         pbCiphertext,
                                        &cbPlaintext2,
                                         rgbPlaintext,
    ...
    )
{
    ...
    */
    ...
    ChkDR( OEM_RSA_PssSign_4096BIT( &g_RsaPrivKey,
                                     cbPlaintext,
                                     g_rgbRSAPlaintext,
                                     0,
                                    &g_cbRsaCiphertext,
    ...
    )
{
    ...
    */
    ...
    {
        g_rgbRSACiphertext[cbPlaintext - 1] = (DRM_BYTE)~g_rgbRSACiphertext[cbPlaintext - 1];
    }
    /*
    ** Verify the signature
    */
    ChkDR( OEM_RSA_PssVerify_4096BIT( &g_RsaPubKey,
                                       cbPlaintext,
                                       g_rgbRSAPlaintext,
                                       0,
                                       g_cbRsaCiphertext,
...
DRM_RESULT DRM_CALL Test_DRM_ECDSA_Sign_P256( long argc, __in_ecount(argc) char **argv )
{
    ...
    */
    cbMessage = (DRM_DWORD)atol( argv[ 1 ] );
