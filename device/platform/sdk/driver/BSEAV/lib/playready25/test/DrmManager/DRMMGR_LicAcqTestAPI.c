...
DRM_RESULT DRM_CALL _LicenseProcessResponseCallback(
    __in const DRM_VOID                        *pvData,
    __in       DRM_STORE_LICENSE_CALLBACK_ENUM  eReason,
    __in       DRM_VOID                        *pvContext )
{
    DRM_RESULT                                dr              = DRM_SUCCESS;
    const DRM_INCLUSION_LIST_CALLBACK_STRUCT *pInclStruct     = (const DRM_INCLUSION_LIST_CALLBACK_STRUCT*)pvData;
    DRM_DWORD                                 i               = 0;
    DRM_DWORD                                 j               = 0;
...
**
*******************************************************************************/
static DRM_RESULT DRM_CALL _ParseAdditionalDataType( __in const DRM_CHAR *f_pszArg, __out_ecount(1) DRM_DWORD *f_pdwType )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL TestManagerGenerateChallenge( long argc, __in_ecount( argc ) char **argv )
{
    ...
    {
        DRM_DWORD cch = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[13]), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[13], cch, 10, &dwOption ) );
    }
    ...
    */
    if( argv[1] != NULL )
    {
        ...
        ChkMem( pszSilentUrl = (DRM_CHAR*)Oem_MemAlloc( cch + 1 ) );
        ZEROMEM( pszSilentUrl, cch + 1 );
        MEMCPY( pszSilentUrl, argv[1], cch );
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( pszSilentUrl ), &cchSilentUrl ) );
    }
    ...
    */
    if( argv[2] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 10, &dwTempSilentURLSize ) );
    ...
    */
    if( argv[3] != NULL )
    {
        ...
        ChkMem( pszNonSilentUrl = (DRM_CHAR*)Oem_MemAlloc( cch + 1 ) );
        ZEROMEM( pszNonSilentUrl, cch + 1 );
        MEMCPY( pszNonSilentUrl, argv[3], cch );
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( pszNonSilentUrl ), &cchNonSilentUrl ) );
    }
    ...
    */
    if( argv[4] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[4], cch, 10, &dwTempNonSilentURLSize ) );
    ...
    */
    if( argv[5] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[5] ), &cchChallenge ) );
    ...
    */
    if( argv[6] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[6], cch, 10, &dwTempChlgSize ) );
    ...
    */
    ...
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[15] ), &dastrCustomData.cchString ) );
    ...
    */
    ...
    if( drInternal == DRM_E_BUFFERTOOSMALL )
    {
        ...
        */
        DRM_DWORD cch = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[12]), &cch ) );
        ChkArg( argv[12] != NULL );
        ChkDR( DRMCRT_AtoDWORD( argv[12], cch, 10, &dwMisAlignment ) );
    ...
    */
    ...
    if( argv[8] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[8], cch, 10, &dwTempSilentURLSize ) );
    ...
    */
    ...
    {
        DRM_DWORD cch = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[10]), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[10], cch, 10, &dwTempNonSilentURLSize ) );
    ...
    */
    ...
    if(argc >= 17 && argv[16] != NULL)
    {
        ...
        {
            DRM_DWORD cch = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[16]), &cch ) );
            ChkDR( DRMCRT_AtoDWORD( argv[16], cch, 10, &dwMaxChallengeSize ) );
...
DRM_RESULT DRM_CALL VerifySymmetricOptimizations( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if( DRM_SYMOPT_IsSymOptSupported() )
    {
        ...
        */
        ...
        else if ( dr == DRM_E_LICENSE_NOT_FOUND )
        {
...
#undef __cbMaxStack
        ...
        else
        {
            ...
            dstrLicense.pwszString = ( const DRM_WCHAR * )pbLicense;
    ...
    */
    if( iOption & 32 )
    {
        ...
        if( argv[1] )
        {
            DRM_DWORD cch = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
            ChkMem( pbResponse=(DRM_BYTE*)Oem_MemAlloc( cch + 1) );
            ZEROMEM( pbResponse, cch + 1 );
            MEMCPY( pbResponse, argv[1], cch + 1 );
            *pcbResponse = cch;
        }
        else
        {
...
ErrorExit:
    ...
    if( iOption & 128 && poLicenseResponse != NULL )
    {
        SAFE_OEM_FREE( poLicenseResponse->m_pAcks );
    }
    return dr;
}
        ...
        if ( fMatchKID )
        {
            ...
            */
            ...
            else
            {
                ...
                ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrLID, ( DRM_GUID * )poLid ) );
            }
