...
DRM_RESULT DRM_CALL TestManagerInitResponse( long argc, __in_ecount( argc ) char **argv )
{
    ...
    DRM_BYTE  *pbSlkFree        = NULL;
    DRM_BYTE  *pbSlkDataFree    = NULL;
        ...
        *pfUseSlk  = fUseSlk;
        ...
        if( *pfSymmOpt )
        {
            ...
            if( fNewSlk )
            {
                ...
                ChkMem( pbSlkFree = (DRM_BYTE*)Oem_MemAlloc( cbSlk ) );
                ZEROMEM( pbSlkFree, cbSlk );

                ChkMem( pbSlkDataFree = (DRM_BYTE*)Oem_MemAlloc( cbSlkData ) );
                ZEROMEM( pbSlkDataFree, cbSlkData );

                ChkDR( Oem_Random_GetBytes( NULL, pbSlkKey, cbSlkKey ) );
                MEMCPY( pbSlkFree, pbSlkKey, cbSlkKey );
                ...
                ChkDR( DRM_B64_EncodeA( (DRM_BYTE*)pbSlkDataFree, cbSlkData, pbSlkDataEnc, &cchSlkDataEnc, 0 ) );
    ...
    else
    {
        ...
        *pfUseSlk  = fUseSlk;
        ...
        if( *pfSymmOpt )
        {
            ...
            cbSlkData  = ( SLK_DATA_SIZE_IN_CHARS + 1 ) * SIZEOF(DRM_CHAR);
            ...
            if( fNewSlk )
            {
                ...
                */
                ...
                MEMCPY( &machineKey,
                        &(poAppContext->oBlackBoxContext.cachedBCertValues.DeviceKeyEncrypt.PublicKeyContext.PublicKey),
                        SIZEOF( PUBKEY_P256 ) );
...
ErrorExit:
    SAFE_OEM_FREE( pbSlkFree );
    SAFE_OEM_FREE( pbSlkDataFree );
    return dr;
}
...
DRM_RESULT DRM_CALL TestManagerSetExtensiblePolicy( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    ChkDR( DRM_DWordToWord( dwTemp, &poXPolData->wObjectType ) );
    ...
    */
    ...
    ChkDR( DRM_DWordToWord( dwTemp, &poXPolData->wParent ) );

    /*
    ** Set xmr flags
    */
    ChkDR( DRMCRT_AtoDWORD( argv[4], (DRM_DWORD)strlen( argv[4] ), 10, &dwTemp ) );
    ChkDR( DRM_DWordToWord( dwTemp, &poXPolData->wFlags ) );
      ...
      && !fRight )
    {
        ...
        ChkDR( DRM_DWordToByte( dwTemp, &poXPolData->fSecureStateType ) );
        ...
        case XMR_EXTENSIBLE_RESTRICTON_STATE_BYTEARRAY:
            ...
            ChkDR( DRM_DWordToWord( dwTemp, &poXPolData->cStateDateCountorBinary ) );
            ChkMem( poXPolData->pStateDateCountorBinary = (DRM_VOID*)Oem_MemAlloc( poXPolData->cStateDateCountorBinary * SIZEOF( DRM_BYTE ) ) );
            ...
            LogBlob( "TestManagerSetExtensiblePolicy", "Secure Bytes:", (const DRM_BYTE*)poXPolData->pStateDateCountorBinary, poXPolData->cStateDateCountorBinary );
            break;
        default:
            break;
        }
...
DRM_RESULT DRM_CALL TestManagerAddLicenseToResponse( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    if ( argv[3] != NULL )
    {
        ...
        {
            char pSystemTime[20];
            const char *pszXMLEnding = "Z#]]></ACTION></ONSTORE>";
            long      lOffset       = 0;
    ...
    {
        dwNumSlk = (DRM_DWORD)atoi( argv[10] );
    }
    ...
    if( ( pfUseSlk != NULL) && ( *pfUseSlk ) )
    {
        ...
        ChkDR( TestLicGenMakeLicense( (DRM_DWORD)atoi( argv[2] ),
    ...
    LogBlob( "TestManagerAddLicenseToResponse", "XML License:", (const DRM_BYTE*)(pwszLicense.pwszString), pwszLicense.cchString * SIZEOF(DRM_WCHAR) );
...
ErrorExit:
    TestLicGenShutDown();
    FREEDRMSTRING( dstrSlkId );
    FREEDRMSTRING( dstrSlkData );
    return dr;
}
...
DRM_RESULT TstGetClientECCPubKey( __in DRM_DWORD f_cbPubKey, __out_bcount(f_cbPubKey) DRM_BYTE *f_pbPubKey )
{
    ...
    memcpy( f_pbPubKey, &pAppContext->oBlackBoxContext.cachedBCertValues.DeviceKeyEncrypt.PublicKeyContext.PublicKey, f_cbPubKey );
                                        ...
                                        __out LICENSERIGHTS  **f_ppLicenseRights )
{
    ...
    {
        RightsPool.pwszString = ( const DRM_WCHAR* )pbPoolStart;
...
#define DRM_TEST_MAX_LICENSES_PER_RESPONSE 20
    ...
    dwLicenses = (DRM_DWORD)atol( argv[0] );
    ChkArg( dwLicenses > 0 );
    ...
    for( iLicense = 0; iLicense < dwLicenses; iLicense++ )
    {
        if( f_fUsePool )
        {
            ...
            */
            ...
            {
                DRM_CHAR *pch = NULL;
                ChkMem( pch = (DRM_CHAR*)Oem_MemAlloc( pParams->dstrKIDUsed.cchString + 1 ) );
                ZEROMEM( pch, SIZEOF(DRM_CHAR) * ( pParams->dstrKIDUsed.cchString + 1 ) );
                DRM_UTL_DemoteUNICODEtoASCII( pParams->dstrKIDUsed.pwszString, pch, pParams->dstrKIDUsed.cchString );
                Log( "Trace", "\t KID         %d = %s", ( dwKIDListIndex + 1 ), pch );
                SAFE_OEM_FREE( pch );
            }
...
DRM_RESULT DRM_CALL GenerateRandomHeader( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    dwHeaderVersion = (DRM_DWORD)atoi( argv[5] );
    ...
    */
    ...
    ChkBOOL( Oem_File_Write( fp, pbHeaderXML, cbHeaderXML, &cbWritten ), DRM_E_FILE_WRITE_ERROR );
    ChkBOOL( cbWritten == cbHeaderXML, DRM_E_FILE_WRITE_ERROR );
