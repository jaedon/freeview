...
DRM_RESULT DRM_CALL TestManagerPrepareOPLCallback( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if( argc > 7 && argv[7] != NULL)
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[7], cch, 10, pdwExtRestrictMod ) );
    ...
    {
        DRM_DWORD cch = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[8]), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[8], cch, 10, pdwSourceID ) );
    }
    else
    {
        *pdwSourceID = 0;
    ...
    if( f_dwCallbackType == DRM_PLAY_OPL_CALLBACK )
    {
        ...
        */
        if( f_pOutputLevels->vopi.cEntries >=1 )
        {
            ...
            DRM_UTL_StringToLittleEndianGuid( &wszGUID, (DRM_GUID*)&localGuid1 );
            FREEDRMSTRING( wszGUID );

            ChkDR( MakeDRMString( &wszGUID, g_guidExtension3 ) );
            DRM_UTL_StringToLittleEndianGuid( &wszGUID, (DRM_GUID*)&localGuid3 );
            FREEDRMSTRING( wszGUID );
...
DRM_RESULT DRM_CALL TestManagerBind( long argc, __in_ecount( argc ) char **argv )
{
    ...
    if ( iOption&1 )
    {
        ...
        */
        if ( argc > 1 && argv[1] != NULL )
        {
            ...
            MEMCPY( pLocalManagerContext->rgbOpaqueBuffer, argv[1], min( cch + 1, DRM_APP_CONTEXT_BUFFER_SIZE ) );
        }
    }
    else
    {
    ...
    */
    if ( iOption&2 )
    {
        ...
        */
        if ( argc > 2 && argv[2] != NULL )
        {
            ...
            MEMCPY( pLocalDecryptContext->rgbBuffer, argv[2], min( cch + 1, SIZEOF( DRM_CIPHER_CONTEXT ) ) );
        }
    }
    else
    {
    ...
    /*
    ** Make sure any existing decrypt context is freed
    */
    Drm_Reader_Close( pLocalDecryptContext );
...
PREFAST_POP
    ...
    */
    ...
    {
        TST_UTL_mbstowcs( pwszVerifyLID, argv[5], (DRM_DWORD)strlen( argv[5] ) + 1 );
        wszVerifyLID.pwszString = pwszVerifyLID;
        wszVerifyLID.cchString = ( DRM_DWORD )strlen( argv[5] );
        ChkDR( DRM_UTL_StringToLittleEndianGuid( &wszVerifyLID, ( DRM_GUID * )oVerifyLID.rgb ) );
    ...
    if (  iOption & 8 )
    {
        ...
        */
        ...
        /*
        ** As the context has been saved to a variable, clear the local context so that it doesn't get
        ** unintentionally freed
        */
        OEM_SECURE_ZERO_MEMORY( pLocalDecryptContext, SIZEOF( DRM_DECRYPT_CONTEXT ) );
    }
...
DRM_RESULT DRM_CALL TestManagerDecrypt( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    {
        // Drm_Reader_Decrypt is not supported by the obfuscated crypto library that activation requires
        goto ErrorExit;
    }
    ...
    if ( argv[4] != NULL )
    {
        ...
        */
        if ( argc >= 6 && argv[5] != NULL )
        {
            ...
            ChkDR( TST_UTL_AtoUI64( argv[5], cch, &pSharedAESCntxt->qwInitializationVector ) );
        }
        else
        {
        ...
        if ( argc >= 7 && argv[6] != NULL )
        {
            ...
            ChkDR( TST_UTL_AtoUI64( argv[6], cch, &pSharedAESCntxt->qwBlockOffset ) );
        }
        else
        {
...
/*****************************************************************************
** Function:    TestManagerClose
**
** Synopsis:
**
** Arguments:   argv[0] : indicates the decrypt context to close. Closes the 
**                        default if argv[0] is NULL or omitted
**
** Returns:     DRM_SUCCESS                  - on success
**
** Note:        
**
******************************************************************************/
DRM_RESULT DRM_CALL TestManagerClose( long argc, __in_ecount( argc ) char **argv )
{
    DRM_RESULT           dr                     = DRM_SUCCESS;
    DRM_DECRYPT_CONTEXT *pLocalDecryptContext   = NULL;
    ...
    if( argc == 0 
     || ( argc == 1 && DRMCRT_strlen( argv[0] ) == 0 ) )
    {
        MAP_TO_GLOBAL_TEST_VAR( pLocalDecryptContext, DECRYPT_CONTEXT_VAR );
    }
    else
    {
        ChkArg( argc == 1 );

        MAP_TO_GLOBAL_TEST_VAR( pLocalDecryptContext, argv[0] );
    }
...
**   argv[2] -- Optional; when present, indicates to use AES crypto
**
** Returns:     DRM_SUCCESS                  - on success
**
** Note:
...
DRM_RESULT DRM_CALL TestManagerCheckDecryptContext( long argc, __in_ecount( argc ) char **argv )
{
    ...
    DRM_AES_COUNTER_MODE_CONTEXT *pSharedAESCntxtSave                            = NULL;                             /* Holds the shared AES context */
    DRM_BOOL                      fAESCrypto                                     = FALSE;                            /* Whether or not to use AES crypto*/
    ...
    ** a failure code.
    */
    ChkBOOL( pcontextDCRY->eCipherType != eDRM_CIPHER_INVALID, DRM_E_CIPHER_NOT_INITIALIZED );
    ...
    */
    ChkDR( tDRM_KG_GenerateContentKey2( &dstrKeySeed, &dstrKID, &dstrContentKey, fAESCrypto ) );
    ChkDR( tDRM_B64_Decode( &dstrContentKey, &cbExpectedContentKey, &pbExpectedContentKey ) );  /* allocates memory */
    ...
    */
    ...
    /*
    ** Save an untainted copy of the shared AES context for the decrypt function
    */
    ChkMem( pSharedAESCntxtSave = (DRM_AES_COUNTER_MODE_CONTEXT*)Oem_MemAlloc( SIZEOF( DRM_AES_COUNTER_MODE_CONTEXT ) ) );
    MEMCPY( pSharedAESCntxtSave, pSharedAESCntxt, SIZEOF( DRM_AES_COUNTER_MODE_CONTEXT ) );
    ...
    ** Decrypt.
    */
    ChkDR( DRM_BBX_CPHR_Decrypt( pcontextDCRY, pSharedAESCntxtSave, SIZEOF(rgbCipherText), rgbCipherText ) );
...
ErrorExit:
    ...
    */
    ...
    SAFE_OEM_FREE( pSharedAESCntxtSave );
