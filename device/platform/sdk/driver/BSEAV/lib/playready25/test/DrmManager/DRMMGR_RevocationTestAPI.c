...
DRM_RESULT DRM_CALL TestManagerSetRevocationInfo(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    AssertChkArg( pRevocationInfo != NULL );
    if ( argv[0] )
    {
...
DRM_RESULT DRM_CALL TestManagerSetRevocation(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[0] ), &dstrGUID.cchString ) );
    dstrGUID.pwszString = (const DRM_WCHAR *) Oem_MemAlloc( SIZEOF(DRM_WCHAR) * ( dstrGUID.cchString + 1 ) );
    ...
    for( i = 2; i + 1 < argc; i = i + 2 )
    {
        ...
        */
        if( argv[i] )
        {
            ...
            ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGUID, &pTemp->guidRevocationList ) );
        }
...
ErrorExit:
    ...
    if( pRevLists != NULL )
    {
        ...
        for( i = 2; i + 1 < argc; i = i + 2 )
        {
            SAFE_OEM_FREE( pTemp->pbRevocationList );
            pTemp++;
        }
    }
...
DRM_RESULT DRM_CALL TestManagerGetLicenseRevInfoVersion(long argc, __in_ecount(argc) char **argv)
{
    ...
    dwRevInfoVer = (DRM_DWORD)atoi( argv[0] );
...
**            DRM_E_FILE_READ_ERROR if reading the file failed
**            errors returned by DRM_UTL_StringToLittleEndianGuid
...
)
{
    ...
    */
    ...
    ChkDR ( DRM_UTL_StringToLittleEndianGuid ( &dstrGUID , &oGUID ) );
    ...
    */
    ChkBOOL( Oem_File_GetSize( FileHdl , &dwCRLData ), DRM_E_FILE_READ_ERROR );
    ...
    ** Read the data
    */
    ChkBOOL( Oem_File_Read( FileHdl , pData , dwCRLData , &dwCRLData ), DRM_E_FILE_READ_ERROR );
    /*
    ** Set the CRL data.
    */
    if ( dwCRLData > 0 )
