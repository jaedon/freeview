...
static DRM_RESULT DupDRMString(
          DRM_CONST_STRING* pTarget,
    const DRM_CONST_STRING* pSource )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL TestLoadWideCharHeader(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    if( argv[0] != NULL )
    {
        if (*argv[0] != '\0' )
        {
            ChkBOOL( LoadTestFile( NULL, argv[0], (DRM_BYTE **) &(g_strContentHeader.pwszString), &(g_strContentHeader.cchString) ), DRM_E_FILE_READ_ERROR );
...
DRM_RESULT DRM_CALL Test_HB_CreateHeader(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    if ( ( argc >= 2 ) && ( argv[1] != NULL ) )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[1], cch, 10, &g_cbHBContext ) );
    ...
    */
    if ( ( argc >= 3 ) && ( argv[2] != NULL ) )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 10, &dwBufferSize ) );
...
DRM_RESULT DRM_CALL Test_HB_ReallocHeader(long argc, __in_ecount(argc) char **argv)
{
    ...
    const _HBContext        *pbNewHB          = NULL;
    const _HBContext        *pOldHB           = NULL;
    ...
    */
    if ( argv[0] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[0], cch, 10, &dwNewSize ) );
    ...
    */
    ...
    pbNewHB = (const _HBContext *)pbNewHBContext;
    pOldHB = (const _HBContext *)g_pbHBContext;
...
DRM_RESULT DRM_CALL Test_HB_GetContextSize(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argv[0] != NULL )
    {
        DRM_DWORD cch = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[0] ), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[0], cch, 10, &dwExpectedSize ) );
    }
...
DRM_RESULT DRM_CALL TestPlayReadyObjectParser(long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkBOOL( LoadTestFile( NULL, argv[0], &pbPlayReadyObject, &cbPlayReadyObject ), DRM_E_FILE_READ_ERROR );
    ...
    ChkBOOL( SaveToFileW(dstrOutFilename.pwszString, (DRM_BYTE*)pwszV4XMLHeader, cchV4XMLHeader*SIZEOF(DRM_WCHAR) ), DRM_E_FILE_WRITE_ERROR );
ErrorExit:
...
{   DRM_RESULT          dr                  = DRM_SUCCESS;
    ...
    */
    if ( argv[2] != NULL )
    {
        cbPlayReadyHeader = (DRM_DWORD)atoi( argv[2] );
    }
    else
    {   /*
   ...
   */
   dr = DRM_PRO_GetRecord( pbPlayReadyHeader, cbPlayReadyHeader, PLAYREADY_WRMHEADER, (DRM_BYTE **)&pwszV4Header, &dwV4HeaderSize );
...
DRM_RESULT DRM_CALL Test_DRM_PRO_IsValidObject(long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkDR( DRM_PRO_IsValidObject( pbPlayReadyObject, cbPlayReadyObject, &dwRecordTypes ) );
    ...
    __in_ecount( argc ) char **argv )
{
    ...
    TST_UTL_mbstowcs( ( DRM_WCHAR * )pbFileDataV2_2, ( const DRM_CHAR * )pbFileDataV2_1, cbFileDataV2 );
    ...
    TST_UTL_mbstowcs( ( DRM_WCHAR * )pbFileDataV4_2, ( const DRM_CHAR * )pbFileDataV4_1, cbFileDataV4 );
    ...
    __in_ecount( argc ) char **argv )
{
    ...
    const DRM_BYTE     *pbV4ExpectedHeader  = NULL;
