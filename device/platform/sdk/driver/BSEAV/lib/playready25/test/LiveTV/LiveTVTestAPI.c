...
DRM_RESULT DRM_CALL TestVerifyStoreSlotCount( __in DRM_LONG argc, __in_ecount( argc ) DRM_CHAR** argv )
{
...

    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[1], cch, 10, &dwExpected ) );
    }
...
DRM_RESULT DRM_CALL TestVerifySecureStoreEntry( __in DRM_LONG argc, __in_ecount( argc ) DRM_CHAR** argv )
{
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[0] ), &dsstrTemp.m_cch ) );
    dsstrTemp.m_ich = 0;
...
DRM_RESULT DRM_CALL TestAddLicenseToPRO( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    else
    {
        ...
        ChkDR( DRM_STR_UTF8toDSTR( ( DRM_CHAR* )( argv[0] ), cch, &dstrKID ) );
    }

    /*
    ** LID
    */
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[1] ), &dstrLID.cchString ) );
    TST_UTL_mbstowcs( rgwcLID, argv[1], dstrLID.cchString + 1 );
...
*************************************************************************************************/
static DRM_RESULT _TstCreateWRMHeaderFromPoolHelper( __in     const DRM_CHAR               *f_pszHeaderPoolName,
...
DRM_RESULT DRM_CALL TestCreateWRMHeaderFromPool( long argc, __in_ecount( argc ) char **argv )
{
    ...
    */
    ...
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[3] ), &pParams->dstrKID.cchString ) );
        TST_UTL_mbstowcs( rgwcKID, argv[3], pParams->dstrKID.cchString + 1 );
        pParams->dstrKID.pwszString = rgwcKID;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 4 && argv[4] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[4] ), &pParams->dstrContentKey.cchString ) );
        TST_UTL_mbstowcs( rgwcContentKey, argv[4], pParams->dstrContentKey.cchString + 1 );
        pParams->dstrContentKey.pwszString = rgwcContentKey;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 5 && argv[5] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[5] ), &pParams->dstrCipherType.cchString ) );
        TST_UTL_mbstowcs( rgwcCipherType, argv[5], pParams->dstrCipherType.cchString + 1 );
        pParams->dstrCipherType.pwszString = rgwcCipherType;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 6 && argv[6] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[6] ), &pParams->dstrPlayReadySilentURL.cchString ) );
        TST_UTL_mbstowcs( rgwcLAURL, argv[6], pParams->dstrPlayReadySilentURL.cchString + 1 );
        pParams->dstrPlayReadySilentURL.pwszString = rgwcLAURL;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 7 && argv[7] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[7] ), &pParams->dstrPlayReadyNonSilentURL.cchString ) );
        TST_UTL_mbstowcs( rgwcLUIURL, argv[7], pParams->dstrPlayReadyNonSilentURL.cchString + 1 );
        pParams->dstrPlayReadyNonSilentURL.pwszString = rgwcLUIURL;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 8 && argv[8] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[8] ), &pParams->dstrServiceID.cchString ) );
        TST_UTL_mbstowcs( rgwcDS_ID, argv[8], pParams->dstrServiceID.cchString + 1 );
        pParams->dstrServiceID.pwszString = rgwcDS_ID;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 9 && argv[9] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[9] ), &pParams->dstrCustomAttriburesXML.cchString ) );
        TST_UTL_mbstowcs( rgwcCustom, argv[9], pParams->dstrCustomAttriburesXML.cchString + 1 );
        pParams->dstrCustomAttriburesXML.pwszString = rgwcCustom;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 10 && argv[10] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[10] ), &pParams->dstrDecryptorSetup.cchString ) );
        TST_UTL_mbstowcs( rgwcDecryptor, argv[10], pParams->dstrDecryptorSetup.cchString + 1 );
        pParams->dstrDecryptorSetup.pwszString = rgwcDecryptor;
        pParams->bOverrideSet = TRUE;
    }
    if ( argc > 11 && argv[11] != NULL )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[11] ), &pParams->dstrWRMHeaderVersion.cchString ) );
        TST_UTL_mbstowcs( rgwcVersion, argv[11], pParams->dstrWRMHeaderVersion.cchString + 1 );
...
{   DRM_RESULT            dr                  = DRM_SUCCESS;
    ...
    */
    if ( argv[1] != NULL )
    {
        cbPlayReadyHeader = (DRM_DWORD)atoi( argv[1] );
    }
    else
    {
        cbPlayReadyHeader = 0;
    ...
    */
    ...
    if ( argc == 4 && argv[3] != NULL )
    {
        ...
        */
        ...
        ChkBOOL( SaveToFileW( dstrOutFilename.pwszString, pbPlayReadyHeader, cbPlayReadyHeader ), DRM_E_FILE_WRITE_ERROR );
    }
...
IMPLEMENT_DEFAULT_PRETESTCASE_FOR_MODULE(LiveTV);
