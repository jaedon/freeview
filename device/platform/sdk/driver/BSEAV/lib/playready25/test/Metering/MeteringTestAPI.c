...
DRM_RESULT _VerifyMeterChallenge(
    __in_ecount(f_cExpectedData) const DRM_CONST_STRING *f_pdstrExpectedData,
    ...
    __in                               DRM_CONST_STRING  f_dstrRecords )
{
    ...
    {
        fFound = FALSE;
        for( dwSubindex = 0; !fFound; dwSubindex++ )
        {
            ChkDR( DRM_XML_GetNode( &f_dstrRecords, &g_dstrTagKID, NULL, NULL, dwSubindex, &dstrKidData, &dstrKID ) );
...
static DRM_RESULT _GetMeterDataFromFile( const DRM_WCHAR *pwchInFile, DRM_WCHAR  **ppwchMeterData )
{
    DRM_RESULT   dr         = DRM_SUCCESS;
    ...
    *ppwchMeterData = NULL;
    ...
    ChkBOOL( Oem_File_Read( hfile, pwchFile, cbFile, &cbRead ), DRM_E_FILE_READ_ERROR );

    *ppwchMeterData = pwchFile;
    pwchFile        = NULL;
    LogBlob( "", "Reading Meter Cert: ", (const DRM_BYTE *) *ppwchMeterData, cbFile );
          ...
          PRIVKEY          *pSignKey )
{
    ...
    ChkBOOL( hFileOut != OEM_INVALID_HANDLE_VALUE, DRM_E_FILE_WRITE_ERROR );
    ...
    */
    ...
    LogBlob( "", "Writing Meter Response: ", ( const DRM_BYTE* ) pwszOut, CB_DSTR( &strXML ) );

    ChkBOOL( Oem_File_Write(hFileOut, (DRM_VOID *) pwszOut, CB_DSTR( &strXML ), &cbWritten ), DRM_E_FILE_WRITE_ERROR );
...
DRM_RESULT _GetMeteringDataAsXML(
          _XMBContext      *pbData,
          DRM_DWORD         cbData,
...
DRM_RESULT DRM_CALL TestDRM_VerifyTID( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkDR(_GetMeterDataFromFile(dstrMeterResp1.pwszString, &pwchMeterResponse));
    ChkArg(pwchMeterResponse);

    FREEDRMSTRING( dstrMeterResp1 );

    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(pwchMeterResponse), &dstrMeterResp1.cchString ) );
    ...
    ChkDR(_GetMeterDataFromFile(dstrMeterResp2.pwszString, &pwchMeterResponse));
    ChkArg(pwchMeterResponse);

    FREEDRMSTRING( dstrMeterResp2 );

    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(pwchMeterResponse), &dstrMeterResp2.cchString ) );
...
DRM_RESULT DRM_CALL TestDRM_Manager_AddManyMeteringData( long argc, __in_ecount(argc) char **argv )
{
    ...
    for ( index = 0; index < kidCount; index++ )
    {
        ...
        DRM_B64_EncodeW( (const DRM_BYTE*)kidTempKID.rgb, SIZEOF (DRM_KID), kidbuf, &cch, 0 );
...
DRM_RESULT DRM_CALL TestDRM_Manager_GeneratePartialMeterChallenge( long argc, __in_ecount(argc) char **argv )
{
    ...
    else if ( dstrRecords.cchString != 0 )
    {
        ...
        LogBlob( "", "Current Records: ", (const DRM_BYTE *)dstrRecords.pwszString, dstrRecords.cchString );
        ChkDR( DRM_E_FAIL );
    }
...
DRM_RESULT DRM_CALL TestDRM_Manager_GenerateMeterChallenge( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argc > 4 )
    {
        ...
        dwExpectedDataSize = (DRM_DWORD)(argc - 4);
...
DRM_RESULT DRM_CALL TestDRM_Manager_ProcessMeterResponse( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkDR( _SignResponse( &dstrMeterResp, &dstrMeterCert, &dstrSignedReponse, (PRIVKEY*)dstrPrivKey.pwszString, &pctxToFree ) );
...
DRM_RESULT DRM_CALL TestDRM_CompareMeterCert( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkBOOL( LoadTestFile( "files", argv[ 1 ], &pbMeterCertInFile, &cbMeterCertInFile ), DRM_E_FILE_READ_ERROR );
...
DRM_RESULT DRM_CALL TestDRM_Manager_GenerateMeterChallengeV3( long argc, __in_ecount(argc) char **argv )
{
    ...
    if( argv[ 1 ] != NULL )
    {
        ChkBOOL( LoadTestFile( NULL, argv[ 1 ], &pbMeterCert, &cbMeterCert ), DRM_E_FILE_READ_ERROR );
    }
    ...
    {
        cchCustomData = (DRM_DWORD)atoi( argv[3] );
    }
    ...
    */
    ...
    {
        cbChallenge = (DRM_DWORD)atol( argv[ 5 ] );
    }
    ...
    if ( argc == 7 )
    {
        ...
        ChkBOOL( LoadTestFile( "files", METERRESPONSEV3TPLT, (DRM_BYTE **)&pbBuffer, &cbBuffer ), DRM_E_FILE_READ_ERROR );
...
DRM_RESULT DRM_CALL DRM_API TestDRM_Manager_GeneratePartialMeterChallengeV3( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkBOOL( LoadTestFile( "files", argv[ 0 ], &pbMeterCert, &cbMeterCert ), DRM_E_FILE_READ_ERROR );
    ...
    ** data of all KIDs.
    */
    cbActualChallenge = (DRM_DWORD)atol( argv[ 2 ] );
...
DRM_RESULT DRM_CALL DRM_API TestDRM_Manager_ProcessMeterResponseV3( long argc, __in_ecount(argc) char **argv )
{
    ...
    if( argv[1] != NULL )
    {
        if ( DRMCRT_strncmp( argv[1], "http://", 7 ) == 0 )
        {
            ...
            ChkBOOL( LoadTestFileW( ( DRM_WCHAR * )L"",
                                    ...
                                    &pbChallenge,
                                    &cbChallenge ), DRM_E_FILE_READ_ERROR );
        ...
        {
            ChkBOOL( LoadTestFile( "files", argv[ 1 ], &pbResponse, &cbResponse ), DRM_E_FILE_READ_ERROR );
        }
    }

    if( argv[2] != NULL && (0 != strcmp( argv[2], "NORMAL")) )
    {
        cbResponse = (DRM_DWORD)atol( argv[2] );
    }
    ...
    ChkDR( _VerifyMeterState( &oMID, (DRM_DWORD)atoi(argv[3]) ) );
...
DRM_RESULT DRM_CALL TestDRM_MTR_ProcessMeterCertResponse( long argc, __in_ecount(argc) char **argv )
{
    ...
    ChkBOOL( LoadTestFile( "files", argv[ 0 ], &pbResponse, &cbResponse ), DRM_E_FILE_READ_ERROR );
