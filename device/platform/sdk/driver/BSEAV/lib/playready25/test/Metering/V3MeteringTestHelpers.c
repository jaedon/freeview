...
**
**********************************************************************/
static DRM_RESULT DRM_CALL _DO_NOT_USE_ExtractEncryptedData(
    __in const PRIVKEY_P256   *f_pPrivKey,
    __in const DRM_CHAR *f_pszBase,
    __in const DRM_SUBSTRING *f_pdasstrXML,
    __out_ecount(*f_pcbData) DRM_BYTE *f_pbData,
    __inout DRM_DWORD *f_pcbData,
    __out_ecount_opt(DRM_AES_KEYSIZE_128) DRM_BYTE     *f_pbKey )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    while( DRM_UI64Les( cbExamined, cbHeaderData ) && !fPROFound )
    {
        ...
        */
        ...
        if( MEMCMP( ( DRM_BYTE * ) ( pbHeaderData + DRM_UI64Low32( cbExamined ) ), ( DRM_BYTE * ) &g_guidPlayreadyHeaderObject, SIZEOF( DRM_GUID ) ) == 0 )
        {
            ...
            */
            *f_pcbPlayreadySize = *( const DRM_DWORD * )( pbHeaderData
            ...
            */
            ...
            ChkBOOL( Oem_File_SetFilePointer( f_pFileHandle, (DRM_LONG)DRM_UI64Low32( cbOffset ), OEM_FILE_BEGIN, NULL ), DRM_E_FILE_SEEK_ERROR );
        ...
        else if( MEMCMP( ( DRM_BYTE * ) ( pbHeaderData + DRM_UI64Low32( cbExamined ) ), ( DRM_BYTE * ) &g_guidASFExtendedHeaderObject, SIZEOF( DRM_GUID ) ) == 0 )
        {
            ...
            */
            ...
            ChkBOOL( Oem_File_SetFilePointer( f_pFileHandle, (DRM_LONG)DRM_UI64Low32( qwOffsetInExtendedHeader ), OEM_FILE_BEGIN, NULL ), DRM_E_FILE_SEEK_ERROR );
            ...
            while( DRM_UI64Les( cbExaminedInExtendedHeader, qwExtendedHeaderDataSize ) && !fPROFound )
            {
                ...
                */
                ...
                if( MEMCMP( ( DRM_BYTE * ) ( pbExtendedHeaderData + DRM_UI64Low32( cbExaminedInExtendedHeader ) ), ( DRM_BYTE * ) &g_guidPlayreadyHeaderObject, SIZEOF( DRM_GUID ) ) == 0 )
                {
                    ...
                    */
                    ChkBOOL( DRM_UI64High32( cbExaminedInExtendedHeader ) == 0, DRM_E_ARITHMETIC_OVERFLOW );
                    *f_pcbPlayreadySize = *( const DRM_DWORD * )( pbExtendedHeaderData
                                    ...
                                    __inout       DRM_ANSI_STRING *f_pdastrKid )
{
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(f_pszFileName), &dsstrFilename.m_cch ) );
    ...
    TST_UTL_FixPathSeparatorW( dstrFilename.pwszString );
    ...
    dstrHeaderXML.pwszString = ( const DRM_WCHAR * ) pbXML;
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( f_pdastrKid->pszString ), &f_pdastrKid->cchString ) );
    ...
    __in_ecount(argc)                char     **argv )
{
    ...
    for ( iArg = 0; iArg + 3 <= argc; iArg += 3)
    {
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[iArg] ), &dastrKIDToFind.cchString ) );
        ...
        */
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[iArg + 1] ), &dastrActionToFind.cchString ) );

        dasstrExpectedActionValue.m_ich = 0;
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[iArg + 2] ), &dasstrExpectedActionValue.m_cch ) );
...
**
**  Synopsis:    Decrypts the Metering Challenge using the supplied Private Key and
**               creates a response by filling in a template.
**
**  Notes:       We cannot use an unmodified template file as the response.
**               A template contains a metering cert, which may be different from the one that was
**               used to create the metering challenge. If the certs do not match then the response
**               will be rejected. So, generate a new cert (and signature) in the template.
**
**  Arguments:
**
