/**@@@+++@@@@******************************************************************
**
** Microsoft (r) PlayReady (r)
** Copyright (c) Microsoft Corporation. All rights reserved.
**
***@@@---@@@@******************************************************************
*/

#include "stdafx.h"
#include "drmbase64.h"
#include <OEMTestAPI.h>
#include <drmhdsblockheadercache.h>

IMPLEMENT_DEFAULT_WRAPTEST

BEGIN_TEST_SCRIPT_LIST
    TEST_SCRIPT_ENTRY("OEMTest")
    TEST_SCRIPT_ENTRY("OEMHalTest")
    TEST_SCRIPT_ENTRY_CONDITIONAL("OEMBlockHeaderCacheTest", DRM_HBHC_IsHdsBlockHeaderCacheSupported)
END_TEST_SCRIPT_LIST

BEGIN_TEST_MAIN(OEM)
    BEGIN_TEST_MODULE_LIST_NO_DEFAULTS
        PKTEST_MODULE_ENTRY(OEM)
    END_TEST_MODULE_LIST
END_TEST_MAIN

