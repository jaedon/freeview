...
static DRM_RESULT iGetSecStorePassword(DRM_BYTE *rgbPassword)
{
    ...
    ChkDR( DRM_BBX_SetupSecureStoreKey( pBlackboxContext, NULL ) );
    ...
    ChkDR( DRM_BBX_TestDeviceKeyPair( pBlackboxContext ) );

    ChkDR(DRM_SST_CreateGlobalStorePassword(NULL, rgbPassword, (DRM_BYTE*)pBlackboxContext));
ErrorExit:
    if (pBlackboxContext != NULL)
    {
        if( pBlackboxContext->pKeyFileContext != NULL )
        {
...
static DRM_RESULT _SetSecureClockData(DRM_SECSTORE_CLOCK_DATA *pClock, DRM_DST *pDST)
{
    ...
    if( DRM_CLK_IsSecureClockSupported() )
    {
        ...
        _PrepareSecureClockDataForWriting( pClock );
        dr = DRM_SST_SetData( pSecStoreContext,
                              &g_idSData,
                              &g_idSDataPrev,
                              rgbSecStoreInPswd,
                              SECURE_STORE_GLOBAL_DATA,
                              pDST,
                              (const DRM_BYTE*)pClock,
                              sizeof(DRM_SECSTORE_CLOCK_DATA) );
        _PrepareSecureClockDataForReading( pClock );
        ChkDR( dr );
    }
    else
    {
...
static DRM_RESULT _VerifyTIDInChallenge(const DRM_ANSI_STRING *pdastrChallenge)
{
    DRM_RESULT dr;
    DRM_SECSTORE_CLOCK_DATA oClock;
    ...
    const DRM_WCHAR *pStart = NULL;
    const DRM_WCHAR *pStop  = NULL;
    DRM_DWORD        cch1   = 0;
    DRM_DWORD        cch2   = 0;
    ...
    dstr.pwszString = (const DRM_WCHAR *) (pdastrChallenge->pszString + dasstr.m_ich);
    ((DRM_WCHAR *)(dstr.pwszString))[cb/SIZEOF(DRM_WCHAR)] = 0;

    /* Get the TID within <TID></TID> */

    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(dstr.pwszString), &cch1 ) );
    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(g_rgwchTIDOpen), &cch2 ) );
    ChkArg( ( pStart = (DRM_WCHAR*)DRMCRT_wcsnstr(dstr.pwszString, cch1, g_rgwchTIDOpen, cch2 ) ) != NULL );
    pStart += 5;
    dstr.pwszString = pStart;
    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(dstr.pwszString), &cch1 ) );
    ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(g_rgwchTIDOpen), &cch2 ) );
    ChkArg( ( pStop = (DRM_WCHAR*)DRMCRT_wcsnstr(dstr.pwszString, cch1, g_rgwchTIDClose, cch2 ) ) != NULL );
...
DRM_RESULT DRM_CALL Test_API_MGR_ClkGenerateChallenge(long argc, __in_ecount(argc) char **argv)
{
    ...
    const DRM_CONST_STRING *pwszURLToVerify = NULL;
    ...
    if( DRM_CLK_IsSecureClockSupported() )
    {
        ...
        if (!strcmp(argv[1], "TRUE"))
        {
            ...
            {
                pwszURLToVerify = &g_dstrDefaultPlayReadySecureClockServerUrl;
            }
...
DRM_RESULT DRM_CALL Test_SetChallengeState(long argc, __in_ecount(argc) char **argv)
{
    ...
    FIX_ENDIAN_QWORD( oClock.ChallengeTime );
