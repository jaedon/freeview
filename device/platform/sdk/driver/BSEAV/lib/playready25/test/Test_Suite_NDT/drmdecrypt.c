...
static DRM_BOOL _WmdrmNetUnpackUI64(
    __in  WMDRMNET_MESSAGE_DESCRIPTOR *MessageDescriptor,
    __out DRM_UINT64                  *Value
    )
{
    int i;
...
static DRM_RESULT _UnpackDataSegmentDescriptor(
    __in      DRM_BYTEBLOB                    *pBlob,
    __out     WMDRMNET_CRYPTO_DATA_LINK_DESC  *pDescriptor
    )
{
    ...
    while(cExtensions > 0)
    {
        ...
        */
        ...
        case WMDRMNET_LINK_EXTENSION_AES_IV_TYPE:
            ...
            */
            if( !_WmdrmNetUnpackUI64( &MessageDescriptor, &pDescriptor->qwSegmentID) )
            {
            ...
            if( cbExtension == 2*SIZEOF(DRM_UINT64) )
            {
                ...
                */
                if( !_WmdrmNetUnpackUI64( &MessageDescriptor, &pDescriptor->qwBlockID) )
                {
                    TRACE(( "Data Segment Descriptor message too short (AES IV Extension Length)\n" ));
                    ChkDR( DRM_E_ND_INVALID_MESSAGE );
                }
...
DRM_RESULT DRM_API WmdrmNetLinkDecrypt(
    __in                            const DRM_AES_KEY              *f_pContentEncryptionKey,
    __in                                  WMDRMNET_ENCRYPTION_MODE  f_EncryptionMode,
    __in                                  DRM_VOID                 *f_pvCryptoDataDescriptor,
    __inout_ecount_opt(*f_pcbCryptoData)  DRM_BYTE                 *f_pbCryptoData,
    __inout                               DRM_DWORD                *f_pcbCryptoData
    )
{
    DRM_RESULT    dr = DRM_SUCCESS;
...
//         && f_pbCryptoData != NULL );
    ...
    case WMDRMNET_ENCRYPTION_MODE_ASF:
        ...
        ChkDR( DRM_DWordToByte( DRM_UI64Low32( DRM_UI64Mod( ((WMDRMNET_CRYPTO_DATA_ASF*)f_pvCryptoDataDescriptor )->qwOffset, DRM_UI64( DRM_AES_BLOCKLEN ) ) ), &aesCtrContext.bByteOffset ) );
