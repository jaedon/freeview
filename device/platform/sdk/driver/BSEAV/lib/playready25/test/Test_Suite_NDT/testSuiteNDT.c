...
const DRM_BYTE g_rgbRSAPrivExp[__CB_DECL( DRM_RSA_CB_PRIVEXP( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) )] =
{
    0x0E, 0xA9, 0x00, 0xCE,
    0x2C, 0xE4, 0x39, 0x07,
    0xE3, 0x87, 0xED, 0x98,
...
DRM_DWORD g_cbSampleDataOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);
...
DRM_DWORD g_cbHeaderOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);

DRM_BOOL  g_fpbLeafLicenseResponseNULL = FALSE;
DRM_BOOL  g_fpcbLeafLicenseResponseNULL = FALSE;
DRM_DWORD g_cbLeafLicenseResponseOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);
...
static DRM_RESULT _FindSession(
    ...
    __in        NDTMGR_SESSION_TYPE                  f_SessionType,
...
)
{
    ...
    {
        if ( 0 == MEMCMP(&pSession->idSession, f_pidSession, DRM_ID_SIZE) )
        {
            break;
        }
    }
...
DRM_RESULT ParseDRMUINT64( DRM_UINT64 *pqwTo, const char *pFrom )
{
    ...
    *pqwTo = DRM_UI64( 0 );
    ...
    for(; *p; p++)
    {
        ...
        {
            wDigit = (DRM_DWORD)( 10 + NATIVE_WCHAR( DRMCRT_towlower( WCHAR_CAST ( *p ) ) ) - 'a' );
        }
...
DRM_RESULT DRM_CALL CloseClearSampleFile()
{
    ...
    for (i = 1; i <= MAX_STREAMS_IN_ASF; i++)
    {
        while ( g_aryPtrListClearFileSamples[i].dwCurrentSize > 0 )
        {
            ...
            __analysis_assume( pTmpAsfSample != NULL );
...
static DRM_RESULT OpenClearSampleFile(__in const char *szFileName)
{
    DRM_RESULT           dr                 = DRM_SUCCESS;
    ...
    /* Open the file */
    g_hClearFile = Oem_File_Open(NULL, dstr.pwszString, OEM_GENERIC_READ, OEM_FILE_SHARE_READ, OEM_OPEN_EXISTING, OEM_ATTRIBUTE_NORMAL);
...
static DRM_RESULT GetClearSampleFromFile(__in DRM_BOOL fResetFileOffset, __in DRM_DWORD dwStreamID, __in DRM_DWORD dwPresentationTime, __out DRM_DWORD* pcbSampleData, __deref_out_ecount(*pcbSampleData) DRM_BYTE** ppbSampleData)
{
    DRM_RESULT       dr                = DRM_SUCCESS;
    DRM_DWORD        cbActual          = 0;
    ...
    while (fFoundSample == FALSE && g_ClearFileState.dwPacketsProcessed < g_ClearFileState.dwTotalPackets)
    {
               ...
               g_ClearFileState.pPacketInfo->fParityPacket ) )
        {
            while ( g_ClearFileState.dwPayload < g_ClearFileState.pPacketInfo->dwPayloads )
            {
                ...
                else
                {
                    ...
                    g_ClearFileState.wPayStart = (DRM_WORD)( g_ClearFileState.pPayloadInfo->wPacketOffset +
        ...
        fResult = Oem_File_SetFilePointer(g_hClearFile, (DRM_LONG)DRM_UI64Low32(DRM_I2UI64(iNextPacketOffset)), OEM_FILE_BEGIN, NULL);
        DRMASSERT( fResult );
...
static DRM_RESULT MapProximityResultToDR(WMDRMND_RESULT wrResult)
{
    ...
    case WMDRMND_E_INVALIDCERTIFICATE:
        dr = DRM_E_INVALID_DEVICE_CERTIFICATE;
        break;
...
static DRM_RESULT TestCreateAppContext(
    __in  const DRM_CONST_STRING *f_HdsPath,
    ...
    )
{
    ...
    if ( ( !f_fUsingSharedAppContext ) && ( f_pAppContext != NULL ) )
    {
        ...
        dr = Drm_GetOpaqueBuffer( f_pAppContext, &pbOpaqueBuffer, &cbOpaqueBuffer );
...
**              [f_pidSession]    - Supplies the ID of the session to delete.
**
** Returns:     DRM_RESULT.
**
**********************************************************************/
static DRM_RESULT _TestCloseSession(
    ...
    )
{
    ...
    ChkArg( f_pidSession != NULL );
    ChkDR( Drm_Ndt_Session_Close( f_pNdContext->pNdtMgrContext,
                                 *f_pidSession,
                                 &pAppContext ) );
    ...
    )
{
         ...
         pNdcObject = pNdcNext)
    {
        ...
        */
        ...
        case NdcNdtSession:
            if( pNdcObject->pArg != NULL )
            {
                _TestCloseSession( f_pNdContext, (const DRM_ID *)pNdcObject->pArg );
                SAFE_OEM_FREE( pNdcObject->pArg );
            }
            break;
...
DRM_RESULT DRM_CALL Test_SetPrivateKey(long argc, __in_ecount(argc) char *argv[])
{
    ...
    OEM_RSA_ZeroPrivateKey_2048BIT(&g_PrivateKey);
...
static void _FreeLicenses(void)
{
    ...
    if ( NULL != g_RootXmrLicenseInfo.pAesContentKey )
    {
        ...
        SAFE_OEM_FREE( g_RootXmrLicenseInfo.pAesContentKey );
    }
...
DRM_RESULT DRM_CALL Test_SetMediaSessionID(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkArg(argc == 1 && argv[0]);
    ChkDR( ParseDRMID(&g_MediaSessionID, argv[0]) );
...
DRM_RESULT DRM_CALL Test_DRM_NDT_Uninitialize(long argc, __in_ecount(argc) char *argv[])
{
    ...
    if ( g_pCurNdtContext )
    {
        ...
        SAFE_OEM_FREE( g_pCurNdtContext->pNdtMgrContext );
    }
...
DRM_RESULT DRM_CALL Test_SwitchNdtContexts(long argc, __in_ecount(argc) char *argv[])
{
    ...
    nContext = (int)strtoul( argv[0], NULL, 0 );
...
DRM_RESULT DRM_CALL Test_SwitchMediaSession(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR( DRM_DWordToWord( strtoul( argv[0], NULL, 0 ), &nSession ) );
...
DRM_RESULT DRM_CALL Test_NDTMGR_ProcessLicenseChallenge(long argc, __in_ecount(argc) char *argv[])
{
    ...
    {
        g_cbAction = (DRM_WORD) DRMCRT_strlen(argv[1]);
    }
    ...
    if ( NULL != g_pRootContentEncryptionKey )
    {
        ...
        SAFE_OEM_FREE( g_pRootContentEncryptionKey );
    }
...
DRM_RESULT DRM_CALL Test_NDTMGR_ASF_Init(long argc, __in_ecount(argc) char *argv[])
{
    ...
    if ( DRMCRT_strlen( argv[0] ) > 0 )
    {
        /* Drm_Ndt_Asf_Init takes DRM_STRING instead of DRM_CONST_STRING, so we cast here. */
        ChkDR( TST_UTL_MakePathString( NULL, argv[0], (DRM_CONST_STRING*)&dstrInputFileName ) );
    }
    ChkDR( TST_UTL_MakePathString( NULL, argv[2], &dstrOutputFileName ) );
    ...
    g_hOutputFile = Oem_File_Open(NULL, dstrOutputFileName.pwszString, OEM_GENERIC_WRITE|OEM_GENERIC_READ, OEM_FILE_SHARE_NONE, OEM_CREATE_ALWAYS, OEM_ATTRIBUTE_NORMAL );
    if(g_hOutputFile == OEM_INVALID_HANDLE_VALUE)
    {
...
ErrorExit:
    if( DRM_FAILED(dr) )
    {
        if ( ( fAsfContextInitialized ) && ( pMediaSessionID ) )
        {
...
DRM_RESULT DRM_CALL Test_NDTMGR_ASF_InitTranscryption(long argc, __in_ecount(argc) char *argv[])
{
    ...
    if(argc > 2 && argv[2])
    {
        ...
        ChkDR( TST_UTL_MakePathString( NULL, argv[2], &dstr ) );

        hOutFile = Oem_File_Open(NULL, dstr.pwszString, OEM_GENERIC_WRITE, OEM_FILE_SHARE_NONE, OEM_CREATE_ALWAYS, OEM_ATTRIBUTE_NORMAL );
        FREEDRMSTRING( dstr );
...
DRM_RESULT DRM_CALL Test_WmdrmNetUnpackRegistrationResponseMessage(long argc, __in_ecount(argc) char *argv[])
{
  ...
  ChkBOOL(LoadTestFile(NULL, argv[0], &pbData, &pcbData), DRM_E_TEST_FILE_LOAD_ERROR);
...
DRM_RESULT DRM_CALL Test_NDTMGR_ASF_GetHeader(long argc, __in_ecount(argc) char *argv[])
{
    ...
    {
        /* Open the file */
        ChkBOOL(LoadTestFile(NULL, argv[0], &pFileHeader, &cbFileHeader), DRM_E_TEST_FILE_LOAD_ERROR);
        ChkMem(pFileHeader);
...
DRM_RESULT DRM_CALL Test_GetHeaders(long argc, __in_ecount(argc) char *argv[])
{
    ...
    FIX_ENDIAN_QWORD(cbDataObjectOffset);
    Oem_File_Write( g_hOutputFile, pbNextDataBlock, cbNextDataBlock, &cbWritten );
    SAFE_OEM_FREE( pbNextDataBlock );
...
DRM_RESULT DRM_CALL Test_LinkEncryption(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkDR( TST_UTL_MakePathString( NULL, argv[1], &dstrOutputFilename ) );
    ...
    hFile = Oem_File_Open(NULL, dstrInputFilename.pwszString, OEM_GENERIC_READ,  OEM_FILE_SHARE_NONE, OEM_OPEN_EXISTING, OEM_ATTRIBUTE_NORMAL);
    ...
    hOutFile = Oem_File_Open(NULL, dstrOutputFilename.pwszString, OEM_GENERIC_WRITE, OEM_FILE_SHARE_NONE, OEM_CREATE_ALWAYS, OEM_ATTRIBUTE_NORMAL );
    if(hOutFile == OEM_INVALID_HANDLE_VALUE)
    {
        TRACE(("Unable to open file %s.\n", argv[1]));
        ChkDR( DRM_E_FAIL );
    ...
    if(argc > 3)
    {
        ndtCryptoDataLink.fNoBlockID = 0 != strtoul(argv[3], NULL, 0);
    }
    if(argc > 4)
    {
        ndtCryptoDataLink.fNoEncrypt = 0 != strtoul(argv[4], NULL, 0);
    }
    ...
    hOutFile = Oem_File_Open(NULL, dstrOutputFilename.pwszString, OEM_GENERIC_READ,  OEM_FILE_SHARE_NONE, OEM_OPEN_EXISTING, OEM_ATTRIBUTE_NORMAL);
    if(hOutFile == OEM_INVALID_HANDLE_VALUE)
    {
        TRACE(("Unable to open file %s.\n", argv[1]));
        ChkDR( DRM_E_FAIL );
...
DRM_RESULT DRM_CALL Test_SetProximityDetectionAddress(long argc, __in_ecount(argc) char *argv[])
{
    ...
    g_cbProximityDetectionAddress = ( DRM_DWORD ) DRMCRT_strlen(argv[0]) + 1; /* Don't forget the NUL terminator */
...
static DRM_RESULT _DrmRevocationTypeEnumFromGUID(
    const DRM_GUID                 *pGuid,
          DRM_REVOCATION_TYPE_ENUM *peRevType )
{
    DRM_RESULT dr = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL Test_AddRevocationListToResponse(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkArg( g_pRevocationLists != NULL );
    ChkArg( argc >= 1 && argc <= 2 );
    ChkArg( g_dwRevocationList < g_cRevocationLists );
...
DRM_RESULT DRM_CALL Test_NDT_RetrieveLicenseDataV2(long argc, __in_ecount(argc) char *argv[])
{
    ...
    ChkBOOL( LoadTestFile( NULL, argv[0], &pbContentEncryptionObject, &cbContentEncryptionObject ), DRM_E_TEST_FILE_LOAD_ERROR );
    dsubstr.m_ich = 0;
    if( argc < 2 )
    {
        ChkDR( DRM_E_INVALIDARG);
    ...
    dsubstr.m_cch = ( DRM_DWORD ) DRMCRT_strlen(argv[1]);
...
DRM_RESULT DRM_CALL Test_SetFailAllocation(long argc, __in_ecount(argc) char *argv[])
{
    ...
    g_fFailAllocation = 0 != strtoul(argv[0], NULL, 0);
...
DRM_RESULT DRM_CALL MODULE_PRETESTCASE(NDTTestSuite)(long lTCID, const char *strTCName)
{
    if( DRM_DVS_IsDeviceStoreSupported() )
    {
        ...
        */
        ...
        g_cbHeaderOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);
        g_fpbLeafLicenseResponseNULL = FALSE;
        g_fpcbLeafLicenseResponseNULL = FALSE;
        g_cbLeafLicenseResponseOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);
        ...
        g_cbSampleDataOverride = MAX_UNSIGNED_TYPE(DRM_DWORD);
        ...
        ChkDR( _CopyFileToDRMFolder( dstrSourceHDS.pwszString, dstrDestHDS.pwszString ) );
        ChkDR( _CopyFileToDRMFolder( dstrSourceCocktailHDS.pwszString, dstrDestCocktailHDS.pwszString ) );
        ChkDR( PreTestCase_OEM_DeviceStore( lTCID, strTCName ) );

        ChkDR( OEM_RSA_SetPrivateKey_2048BIT( eDRM_RSA_KEY_LENGTH_1024_2048BIT,
...
ErrorExit:
        FREEDRMSTRING( dstrSourceHDS );
        FREEDRMSTRING( dstrDestHDS );
        FREEDRMSTRING( dstrSourceCocktailHDS );
        FREEDRMSTRING( dstrDestCocktailHDS );
    ...
    if( DRM_DVS_IsDeviceStoreSupported() )
    {
        ...
        for ( i = 0; i < MAXMEDIASESSIONS; i++ )
        {
            ...
            if ( NULL != g_RootXmrLicenseInfos[i].pAesContentKey )
            {
                ...
                SAFE_OEM_FREE( g_RootXmrLicenseInfos[i].pAesContentKey );
            }
        }
