...
static DRM_BYTE *GetBuf( __in_z_opt const char *szIndex )
{
    if (szIndex) {
        int i = atoi(szIndex);
        if (i >= 0 && i < MAX_BUFF)
...
{
    DRM_RESULT      dr  = DRM_SUCCESS;
    const DRM_BYTE *pb1 = NULL;
    const DRM_BYTE *pb2 = NULL;
    DRM_DWORD       len = 0;
...
DRM_RESULT DRM_CALL TestSetBuffer(long argc, __in_ecount(argc) char **argv)
{
    ...
    if (argv[1]) {
        ...
        dwOffset = (DRM_DWORD)atol(argv[2]);
...
static DRM_BOOL _memcmpwm(const DRM_BYTE *pw, const DRM_BYTE *pm, DRM_DWORD cch, DRM_BOOL fpwIsWide)
{
    DRM_BOOL ret;
    if (fpwIsWide) {
        DRM_WCHAR *wsz = NULL;
...
DRM_RESULT DRM_CALL TestVerifyBuffer(long argc, __in_ecount(argc) char **argv)
{
    ...
    if (pBuf && argv[2] && _memcmpwm(pBuf, (const DRM_BYTE *)argv[2], (DRM_DWORD)cbBuf, fWide))
    {
        dr = DRM_S_FALSE;
        goto ErrorExit;
    }
...
DRM_RESULT DRM_CALL Test_API_DRMCRT_strncmp(long argc, __in_ecount(argc) char **argv)
{
    ...
    sz1 = (const DRM_CHAR*)GetBuf(argv[0]);
    sz2 = (const DRM_CHAR*)GetBuf(argv[1]);
    count = (DRM_DWORD)atol(argv[2]);
...
{
    DRM_RESULT       dr = DRM_SUCCESS;
    const DRM_WCHAR *wsz1;
    const DRM_WCHAR *wsz2;
    DRM_DWORD        count;
    DRM_LONG         lRet;

    ChkArg(argc == 4 && argv[2] && argv[3]);
    wsz1 = (const DRM_WCHAR*)GetBuf(argv[0]);
    wsz2 = (const DRM_WCHAR*)GetBuf(argv[1]);
    count = (DRM_DWORD)atol(argv[2]);
...
DRM_RESULT DRM_CALL Test_API_DRMCRT_wcslen(long argc, __in_ecount(argc) char **argv)
{
    ...
    wsz = (const DRM_WCHAR*)GetBuf(argv[0]);
    cchExpected = (DRM_DWORD)atol(argv[1]);
...
DRM_RESULT DRM_CALL Test_API_DRMCRT_isxxx(long argc, __in_ecount(argc) char **argv)
{
    ...
    for( lNow = lFrom; DRM_SUCCESS == dr && lNow < lTo; lNow++) 
    {
        DRM_WCHAR wchNow = (DRM_WCHAR)WCHAR_CAST(lNow);     /* Convert from integer to little-endian (DRM) wchar */
        DRM_WCHAR chNow  = (DRM_WCHAR)NATIVE_WCHAR(wchNow); /* Convert from little-endian (DRM) wchar to NATIVE (little or big endian) wchar */

        dr = DRM_E_TEST_UNEXPECTED_OUTPUT;

        if (!DRMCRT_iswspace(wchNow) != !iswspace(chNow)) 
        {
            Log("DRMCRT_iswspace", "DRMCRT_iswspace and iswspace returns different values for %d", chNow);
        }
        else if (!DRMCRT_iswxdigit(wchNow) != !iswxdigit(chNow)) 
        {
            Log("DRMCRT_iswxdigit", "DRMCRT_iswxdigit and iswxdigit returns different values for %d", chNow);
        }
        else if (!DRMCRT_iswdigit(wchNow) != !iswdigit(chNow)) 
        {
            Log("DRMCRT_iswdigit", "DRMCRT_iswdigit and iswdigit returns different values for %d", chNow);
        }
        else if (!DRMCRT_iswalpha(wchNow) != !iswalpha(chNow)) 
        {
            Log("DRMCRT_iswalpha", "DRMCRT_iswalpha and iswalpha returns different values for %d", chNow);
        }
        else if (NATIVE_WCHAR(DRMCRT_towlower(wchNow)) != (DRM_WCHAR)towlower(chNow)) 
        {
...
DRM_RESULT DRM_CALL Test_API_DRM_Base64Encode(long argc, __in_ecount(argc) char **argv)
{
    ...
    if (argv[4])
    {
        cchEncoded = cchMaxEncoded = (DRM_DWORD)atol(argv[4]);
    }
...
DRM_RESULT DRM_CALL Test_API_DRM_Base64Decode(long argc, __in_ecount(argc) char **argv)
{
    ...
    if (!strcmp(argv[0], "ANSI")) {
        if ( ( dastrSource.pszString = (const DRM_CHAR*)GetBuf(argv[1]) ) != NULL )
        {
       ...
       dasstrSource.m_cch = dastrSource.cchString;
        ...
        if ( ( wszSource.pwszString = (const DRM_WCHAR*)GetBuf(argv[1]) ) != NULL )
...
DRM_RESULT DRM_CALL TestUtilAreGUIDEqual(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGuid1, &dguid1 ) );
    ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGuid2, &dguid2 ) );
...
** the older dev API DRM_UTL_StringToLittleEndianGUID to convert an input
...
DRM_RESULT DRM_CALL TestUtilGUIDToString(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGuid, &dguid ) );
    ...
    /* Put the GUID in NATIVE endianness */
    FIX_ENDIAN_DWORD( dguid.Data1 );
    FIX_ENDIAN_WORD( dguid.Data2 );
    FIX_ENDIAN_WORD( dguid.Data3 );
...
** the nature of DRM_UTL_ReadLittleEndianBytesToNativeGUID, this MUST be tested on both
...
DRM_RESULT DRM_CALL TestUtilReadGUID(long argc, __in_ecount(argc) char **argv)
{
    ...
    /* Convert input guid */
    ChkDR( MakeDRMString( &dstrGuid, argv[0] ) );
    ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGuid, &dguidIn ) );
    ...
    ChkDR( DRM_UTL_ReadLittleEndianBytesToNativeGUID( fUseNullArray? NULL : pbData,
                                                      SIZEOF(DRM_GUID) + dwGuidOffset + iArraySizeChange,
                                                      dwGuidOffset,
                                                      fUseNullGuid? NULL : &dguidOut ) );

    /* Put the GUID in NATIVE endianness */
    FIX_ENDIAN_DWORD( dguidIn.Data1 );
    FIX_ENDIAN_WORD( dguidIn.Data2 );
    FIX_ENDIAN_WORD( dguidIn.Data3 );
...
DRM_RESULT DRM_CALL TestCRTwcstr(long argc, __in_ecount(argc) char **argv)
{
    ...
    const DRM_WCHAR *pwszExpectedOutput  = NULL;
    const DRM_WCHAR *pwszOutput          = NULL;
    DRM_BOOL         fUseNullString      = FALSE;
    ...
    {
        pwszExpectedOutput = dstrString.pwszString;
...
DRM_RESULT DRM_CALL TestECCExpectedErrorCode(long argc, __in_ecount(argc) char **argv)
{
    ...
    if( DRM_CONTRACT_IsContractSupported() )
    {
        ...
        drTestResult = (DRM_RESULT)strtoul( argv[1], (char **)NULL, 0 );
        ChkDR( StringToBool( argv[2], &fExpectedResult ) );
