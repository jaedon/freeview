...
static DRM_RESULT DRM_CALL _LogARGs(
    ...
    __in long                  f_argcMax,
    __in const char           *f_pszFunctionName )
{
    ...
    if(iOption&1){
        ...
        if(argv[1]!=NULL){
            ...
            if(iOption&2){
                ...
                ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cbXmlContext ) );
                pbXmlContext=(_XMBContext*)Oem_MemAlloc(cbXmlContext*sizeof(DRM_BYTE)+1);
                if(!pbXmlContext)
                {
                    ChkDR(DRM_E_FAIL);
    ...
    if(iOption&4){
        if(argv[2]!=NULL){
            if(strcmp(argv[2],"nullstring")){
                ...
                TST_UTL_mbstowcs(pwszLocalString, argv[2], cch+1);
                pwszRootNodeName.pwszString=pwszLocalString;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(pwszLocalString), &pwszRootNodeName.cchString ) );
            }
            else{
...
DRM_RESULT DRM_CALL TestXMLBReallocDocument(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(iOption&1)
    {
        if(argv[0]!=NULL)
        {
            ...
            pbLocalXmlContext=(_XMBContext*)Oem_MemAlloc(cch*sizeof(DRM_BYTE)+1);
            if(!pbLocalXmlContext)
            {
                ChkDR(DRM_E_FAIL);
            }
            memcpy(pbLocalXmlContext, argv[0], cch+1);
        }
        j=1;
    }
...
DRM_RESULT DRM_CALL TestXMLBCloseDocument(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        if(strcmp(argv[1], "nullstring")){
            ...
            TST_UTL_mbstowcs(szArrayXMLString, argv[1], cch+1);
            pszXMLString.pwszString=szArrayXMLString;
            ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLString), &pszXMLString.cchString ) );
        }
    }
    else{
        iStringNullFlag=1;
    ...
    if(argv[3]!=NULL){
        ...
        TST_UTL_mbstowcs(szVerifyArrayXMLString, argv[3], cch+1);
    ...
    if(iOption&4){
        if(argv[1]!=NULL){
            if(strcmp(argv[1],"nullstring")){
                DRM_DWORD cch = 0;
                ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
                TST_UTL_mbstowcs(szArrayNodeName, argv[1], cch+1);
                pszNodeName.pwszString=szArrayNodeName;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayNodeName), &pszNodeName.cchString ) );
            }
            else{
...
DRM_RESULT DRM_CALL TestXMLBCloseCurrNode(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        if(strcmp(argv[1], "nullstring")){
            DRM_DWORD cch = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
            TST_UTL_mbstowcs(szArrayXMLFragment, argv[1], cch+1);
            pszXMLFragment.pwszString=szArrayXMLFragment;
            ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLFragment), &pszXMLFragment.cchString ) );
        }
    }
    else{
        iFragmentNullFlag=1;
...
DRM_RESULT DRM_CALL TestXMLBGetContextSize(long argc, __in_ecount(argc) char **argv)
{
    ...
    cbLocalXmlContext=(DRM_DWORD)atol(argv[1]);;
...
DRM_RESULT DRM_CALL TestXMLBAddAttribute(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        if(strcmp(argv[1], "nullstring"))
        {
            ...
            DRM_DWORD  cchAttr = ( DRM_DWORD ) min( cb, NO_OF( szArrayAttrName ) - 2 );
...
#pragma prefast(pop)
                ...
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayAttr), &pszAttrName.cchString ) );
            }
            else{
                pszAttrName.pwszString=szArrayAttrName;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayAttrName), &pszAttrName.cchString ) );
            }
        }
    }
    else{
    ...
    if(argv[3]!=NULL){
        if(strcmp(argv[3], "nullstring"))
        {
            ...
            DRM_DWORD cchAttr = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[3]), &cch ) );
            cchAttr = min( cch, NO_OF( szArrayAttrName ) - 2 );
            TST_UTL_mbstowcs( szArrayAttrValue, argv[3], cch + 1 );
            if((iOption&64)||(iOption&128)){
                ...
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayAttr), &pszAttrValue.cchString ) );
            }
            else{
                pszAttrValue.pwszString=szArrayAttrValue;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayAttrValue), &pszAttrValue.cchString ) );
            }
        }
    }
    else{
...
DRM_RESULT DRM_CALL TestXMLBAddData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        ...
        {
            DRM_SIZE_T cb         = DRMCRT_strlen( argv[1] );
            DRM_DWORD  cchXMLData = ( DRM_DWORD ) min( cb, NO_OF( szArrayXMLData ) - 2 );
...
#pragma prefast(pop)
                ...
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLDataSpace), &pszXMLData.cchString ) );
            }
            else{
                pszXMLData.pwszString=szArrayXMLData;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLData), &pszXMLData.cchString ) );
            }
        }
    }
    else{
...
DRM_RESULT DRM_CALL TestXMLBAddCData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if(argv[1]!=NULL){
        ...
        {
            DRM_DWORD cch = 0;
            DRM_DWORD cchXMLCData = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
            ChkDR( DRM_SizeTToDWord( ( DRM_DWORD ) min( cch, NO_OF( szArrayXMLCData ) - 2 ), &cchXMLCData ) );

            TST_UTL_mbstowcs(szArrayXMLCData, argv[1], cch+1);
            if((iOption&16)||(iOption&32)){
                if(iOption&16){
...
#pragma prefast(pop)
                ...
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLCDataSpace), &pszXMLCData.cchString ) );
            }
            else{
                pszXMLCData.pwszString=szArrayXMLCData;
                ChkDR( DRM_SizeTToDWord( DRMCRT_wcslen(szArrayXMLCData), &pszXMLCData.cchString ) );
            }
        }
    }
    else{
