...
static DRM_RESULT ArgToBindingInfo(
__in_z_opt const char              *arg,
                 DRM_BINDING_INFO **ppB )
{
    int i;
    if (arg) {
        i = atoi(arg);
...
static void SetCheckSum(
    __in_ecount( cbData )          const DRM_BYTE  *pData,
    __in                                 DRM_DWORD  cbData,
    __out_ecount( CHECKSUM_LENGTH )      DRM_BYTE   pbCheckSum[CHECKSUM_LENGTH] )
{
...
DRM_RESULT DRM_CALL Test_CreateBindingInfo(long argc, __in_ecount(argc) char **argv)
{
    ...
    /*
    ** Generate the random content key.
    */
    ...
    /*
    ** The first byte is the key length. The actual key starts from the second byte.
    */
    ClearContentKey[0] = keyLength;

    /*
    ** Now that we have set the key length in the content key, with a value possibly from a
    ** parameter, ensure that the value of keyLength is <= max for subsequent usages.
    */
    if ( keyLength > ( DRM_ECC160_PLAINTEXT_LEN - 1 ) )
    {
        keyLength = ( DRM_ECC160_PLAINTEXT_LEN - 1 );
    }
