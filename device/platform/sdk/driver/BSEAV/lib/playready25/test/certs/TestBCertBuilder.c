...
**              argv[7]   : Whether to submit the model private key as a key handle or a buffer
**                          0        -> handle
**                          1        -> buffer
**              argv[8]   : Optional, Security Version DWORD
**              argv[9]   : Optional, Platform Identifier. Should be given if argv[7] is given
**
** Returns:     DRM_SUCCESS                                 - on success
**              DRM_E_INVALIDARG                            - if any parameter is invalid
...
DRM_RESULT DRM_CALL Test_SetDefaultBuilderData(long argc, __in_ecount(argc) char **argv)
{
    ...
    DRM_DWORD               fUseModelKeyBuffer = 0;
    DRM_DWORD               dwCurKeyUsage = 0;
    DRM_DWORD               dwCurKeyUseType = 1;
    DRM_DWORD               dwCurKeyUseFlag = 1; /* should equal 2^(dwCurKeyUseType-1) */
    OEM_CRYPTO_HANDLE       hPrivateKey     = OEM_CRYPTO_HANDLE_INVALID;

    ChkArg( argc == 8 || argc >= 10 ); /* argv[9] is required if argv[8] is present */
    ...
    ChkDR( DRMCRT_AtoDWORD( argv[7], (DRM_DWORD)strlen( argv[7] ), 10, &fUseModelKeyBuffer ) );

    if( fUseModelKeyBuffer == 0 )
    {
        /*
        ** To use a key handle, we need to inject a specific model key into the HAL.
        ...
        */
        ...
        ChkDR( _Hal_AllocateKeyHandle( &g_BbxCtx, OEM_HAL_KEY_TYPE_ECC_P256_MODEL_SIGN, pbTestPrivKey, SIZEOF( PRIVKEY_P256 ), &hPrivateKey, &pKeyEntry, &fCachedHandle ) );
        ChkPtr( pKeyEntry );
        if( !fCachedHandle )
        {
            OEM_HAL_UNWRAP_CONTEXT unwrapContext;

            ChkDR( OEM_HAL_TEST_InitTestKeysUnwrap( OEM_HAL_KEY_TYPE_ECC_P256_MODEL_SIGN,
                                                    pKeyEntry->indexKeyRegister,
                                                    OEM_HAL_KEY_TYPE_INVALID,
                                                    OEM_HAL_KEY_REGISTER_INDEX_INVALID,
                                                    SIZEOF( PRIVKEY_P256 ),
                                                    &unwrapContext ) );
            ChkDR( OEM_HAL_TEST_InjectCleartextKey( &unwrapContext, pbTestPrivKey, SIZEOF( PRIVKEY_P256 ) ) );

            DRM_BYT_CopyBytes( pKeyEntry->pbCacheKey, 0, pbTestPrivKey, 0, SIZEOF( PRIVKEY_P256 ) );
            pKeyEntry->cbCacheKey = SIZEOF( PRIVKEY_P256 );
        }

        Log("Trace", "Call BCert_SetDefaultBuilderData passing in key handle");
    ...
    {
        Log("Trace", "Call BCert_SetDefaultBuilderData pasing in key buffer");
        BCert_SetDefaultBuilderData( pCertificateID,
                                     dwType,
                                     ...
                                     (PUBKEY_P256*)&keyIssuerKey,
                                     &Data );
        pbTestPrivKey = NULL; /* ownership transferred to "Data" */
    }

    /*
    ** The crypto handle has been assigned to Data.hPrivateKey, and will be freed
    ** during, or at the end of, the test
    */
    hPrivateKey = OEM_CRYPTO_HANDLE_INVALID;
    ...
    if ( argc == 10 )
    {
        ...
        */
        if( argv[8] != NULL )
        {
            ChkDR( DRMCRT_AtoDWORD( argv[8], (DRM_DWORD)strlen( argv[8] ), 10, &Data.SecurityVersion.dwSecurityVersion ) );
        }
        /*
        ** set the PlatForm ID
        */
        if( argv[9] != NULL )
        {
            ChkDR( DRMCRT_AtoDWORD( argv[9], (DRM_DWORD)strlen( argv[9] ), 10, &Data.SecurityVersion.dwPlatformIdentifier ) );
        }
...
DRM_RESULT DRM_CALL Test_ClearBuilderData(long argc, __in_ecount(argc) char **argv)
{
    ...
    DRM_BBX_SafeReleaseKey( &g_BbxCtx, &Data.hPrivateKey );
    ...
    if( Data.ExtendedData.pExtDataRecords != NULL )
    {
        SAFE_OEM_FREE( Data.ExtendedData.pExtDataRecords->pbData );
    }
    ...
    if ( argc >= 4 ) /* how many times to add a key */
    {
       ...
       if ( DRM_FAILED( DRMCRT_AtoDWORD( argv[3], (DRM_DWORD)strlen( argv[3] ), 10, &dwNumberOfKeysToAdd ) )
            || dwNumberOfKeysToAdd == 0 )
        {
            ...
            {
                const DRM_CHAR  *pchStr = argv[3] + 4;
...
DRM_RESULT DRM_CALL Test_SetPCBuilderData(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRM_LongToDWord( atoi(argv[4]), &poExtData->dwDataLength ) );

    /* Read type of ext data */
    ChkDR( DRM_LongToWord( atoi(argv[5]), &poExtData->Header.wType ) );
...
DRM_RESULT DRM_CALL Test_SetSilverLightBuilderData(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argv[1] != NULL )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[1], cch, 16, &dwSecurityVersion ) );
    }

    /* Get platform identifier from arguments */
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[2]), &cch ) );
    ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 16, &dwPlatformId ) );
...
DRM_RESULT DRM_CALL Test_AddCert(long argc, __in_ecount(argc) char **argv)
{
    ...
    /* This test codepath signs certificates with the blackbox which are not supported in production. */
    ChkDR( OEM_HAL_TEST_EnableSkipLeafCertificateValidityCheckDuringSignOperation() );
    ...
    dr = BCert_AddCert( pbParentChain, pbNewChain, &cbNewChain, &Data, &g_BbxCtx, NULL, NULL );
    if( dr == DRM_E_BUFFERTOOSMALL )
    {
        ChkMem( pbNewChain = (DRM_BYTE*)Oem_MemAlloc( cbNewChain * SIZEOF(DRM_BYTE) ) );
        ZEROMEM( pbNewChain, cbNewChain * SIZEOF(DRM_BYTE) );
        ChkDR( BCert_AddCert( pbParentChain, pbNewChain, &cbNewChain, &Data, &g_BbxCtx, NULL, NULL ) );
    }
    else
    {
        ChkDR( dr );
...
DRM_RESULT DRM_CALL Test_AddExtendedDataToCert(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( BCert_AddExtendedDataToCert( pbChain, cbChain, fNullExtData ? NULL : &(Data.ExtendedData), &g_BbxCtx.CryptoContext ) );
    ...
    __in               const  DRM_DWORD                f_cbSize )
{
    ...
    */
    ChkBOOL( f_pData->hPrivateKey       != OEM_CRYPTO_HANDLE_INVALID, DRM_E_BCERT_PRIVATE_KEY_NOT_SPECIFIED );
    ...
    */
    ...
    if ( f_pData->wSignatureType == DRM_BCERT_SIGNATURE_TYPE_P256 )
    {
        ...
        */
        ...
        dwSignatureLength = SIZEOF( SIGNATURE_P256 );

        /*
        ** Sign the data in this certificate
        */
        ChkDR( DRM_BBX_SignWithCertKeys( &g_BbxCtx,
        ...
        ChkBOOL( dwSignatureLength == SIZEOF( SIGNATURE_P256 ), DRM_E_LOGICERR );
...
DRM_RESULT DRM_CALL Test_SignAndAddCert(long argc, __in_ecount(argc) char **argv)
{
    ...
    */
    ...
    {
        ChkDR( _addCertChainHeader( pbNewChain, cbNewChain, 0 ) );
    }
    else
    {
