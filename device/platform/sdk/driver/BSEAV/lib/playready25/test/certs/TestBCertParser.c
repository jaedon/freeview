...
DRM_RESULT DRM_CALL Test_FindObjectInCertByType(long argc, __in_ecount(argc) char **argv)
{
    ...
    ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 10, &cbBytesToSkip ) );
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[3]), &cch ) );
    ChkDR( DRMCRT_AtoDWORD( argv[3], cch, 10, &dwExpectedObjType ) );

    if ( ( argc > 4 ) && ( argv[4]!= NULL ) )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[4]), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[4], cch, 10, &cbTestBinCertChain ) );
    }
    ...
    */
    ...
    if ( ( argc > 5 ) && ( argv[5] != NULL ) )
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[5], cch, 10, &dwExpectedOffset ) );
...
ErrorExit:
    ...
    SAFE_OEM_FREE( prgdwKeyUsageSet );
    return dr;
}
...
DRM_RESULT DRM_CALL Test_VerifySecurityVersionForCert( long argc, __in_ecount( argc ) char **argv )
{
    ...
    ChkDR( DRMCRT_AtoDWORD( argv[0], cch, 0, &dwCertIndex ) );
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
    ChkDR( DRMCRT_AtoDWORD( argv[1], cch, 0, &dwSecVer ) );
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[2]), &cch ) );
    ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 0, &dwPlatID ) );
