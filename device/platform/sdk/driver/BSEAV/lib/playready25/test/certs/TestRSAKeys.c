...
static DRM_RESULT TestConversionRSAKeyToXML(
    __in const DRM_RSA_PUBLIC_KEY_2048BIT  *poRSAPubKey,
    __in const DRM_RSA_PRIVATE_KEY_2048BIT *poRSAPrivKey )
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_RSA_PUBLIC_KEY_2048BIT  oRSAPubKeyTest = { 0 };
    DRM_RSA_PRIVATE_KEY_2048BIT oRSAPrivKeyTest = { 0 } ;
    ...
    ZEROMEM( &oRSAPubKeyTest,  SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT ) );
    ZEROMEM( &oRSAPrivKeyTest, SIZEOF( DRM_RSA_PRIVATE_KEY_2048BIT ) );
    ...
    ChkBOOL( MEMCMP( poRSAPubKey , &oRSAPubKeyTest,  SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT  ) ) == 0, DRM_E_KEY_MISMATCH );
    ChkBOOL( MEMCMP( poRSAPrivKey, &oRSAPrivKeyTest, SIZEOF( DRM_RSA_PRIVATE_KEY_2048BIT ) ) == 0, DRM_E_KEY_MISMATCH );
    ...
    __in const DRM_RSA_PUBLIC_KEY_2048BIT *poRSAPubKey )
{
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(rgchPubKeyModulus), &cch ) );
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(rgchPubKeyExponent), &cch ) );
    ChkDR( _GetSubNodeByPathA( pbXML->rgchXML,
                               &dasstrClose,
                               rgchPubKeyExponent,
                               cch,
    ...
    ChkBOOL( MEMCMP( poRSAPubKey , &oRSAPubKeyTest,  SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT  ) ) == 0,
             DRM_E_KEY_MISMATCH );
...
DRM_RESULT DRM_CALL Test_RSAToXML( long argc, __in_ecount(argc) char **argv )
{
    ...
    DRM_RSA_PRIVATE_KEY_2048BIT oRSAPrivKey = { 0 };

    ZEROMEM( &g_CryptoCtx, DRM_PKCRYPTO_CONTEXT_BUFFER_SIZE );
    ZEROMEM( &oRSAPubKey,  SIZEOF( DRM_RSA_PUBLIC_KEY_2048BIT ) );
    ZEROMEM( &oRSAPrivKey, SIZEOF( DRM_RSA_PRIVATE_KEY_2048BIT ) );
...
/* RSA key length of 2048 is supported only if WMDRMNET is supported */
    if( DRM_WMDRMNET_IsWmdrmnetSupported() )
    {
        ...
        ChkDR( DRMTOOLS_CreateKeyPair( &g_CryptoCtx,
                                       eDRM_RSA,
                                       eDRM_RSA_KEY_LENGTH_2048_2048BIT,
                                       (DRM_BYTE*) &oRSAPubKey,
                                       DRM_RSA_CB_PUBKEY_2048BIT,
                                       (DRM_BYTE*) &oRSAPrivKey,
                                       DRM_RSA_CB_PRIVKEY_2048BIT,
