...
DRM_RESULT DRM_CALL Test_SetRobustnessVersion( long argc, __in_ecount( argc ) char **argv )
{
    ...
    ChkDR( DRMCRT_AtoDWORD( argv[1], cch, 10, &g_dwPlatformID ) );

    if ( argc == 3 )
    {
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( argv[2] ), &cch ) );
        ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 10, &dwDRMFlavor ) );
    }
...
DRM_RESULT DRM_CALL Test_API_GetDeviceCertificate(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argv[0] != NULL )
    {
        ...
        ChkArg( DRM_SUCCESS == DRMCRT_AtoDWORD( argv[3], cch, 10, &dwIndex ) );
        ChkArg( dwIndex < GDATA_COUNT );

        FREEDRMSTRING( g_dstrData_DDC[dwIndex] );

        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[0]), &cch ) );
        ChkArg( DRM_SUCCESS == DRMCRT_AtoDWORD( argv[0], cch, 10, &cbDataStr ) );
    ...
    ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[1]), &cch ) );
    ChkArg( DRM_SUCCESS == DRMCRT_AtoDWORD( argv[1], cch, 10, &dwFlags ) );
    ...
    s_tempBlackboxContext.pKeyFileContext = poKeyfileContext;
    ...
    if ( DRM_FAILED(dr) )
    {
        if ( ( dwFlags & DRM_DCP_CREATE_DEVCERT_IF_NOT_EXISTING ) == DRM_DCP_CREATE_DEVCERT_IF_NOT_EXISTING )
        {
            ...
            ChkDR( DRM_DDC_SetXMLDeviceCertificateAndKey( &s_tempBlackboxContext,
                                                          poKeyfileContext,
                                                          &g_dstrData_DDC[dwIndex] ) );
...
DRM_RESULT DRM_CALL Test_API_GetBinaryDeviceCertificate(long argc, __in_ecount(argc) char **argv)
{
    ...
    if ( argv[0] != NULL )
    {
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(argv[0]), &cch ) );
        ChkArg( DRM_SUCCESS == DRMCRT_AtoDWORD( argv[0], cch, 10, &g_cbData_DDC[dwIndex] ) );
    ...
    if ( argv[2] != NULL )
    {
        ChkMem( pBbxContext = ( DRM_BB_CONTEXT* )Oem_MemAlloc( SIZEOF( DRM_BB_CONTEXT ) ) );
        ZEROMEM( pBbxContext, SIZEOF( DRM_BB_CONTEXT ) );
        ChkDR( DRM_BBX_Initialize( NULL, pBbxContext ) );
        pBbxContext->pKeyFileContext = poKeyfileContext;
    }
...
ErrorExit:
    ...
    if( pBbxContext != NULL )
    {
        DRM_BBX_Shutdown( pBbxContext );
        SAFE_OEM_FREE( pBbxContext );
    }
...
static DRM_STRING* _GetXMLDataBuffer(const char *szArg)
{
    ...
    if ( szArg != NULL )
    {
        ...
        {
            DRM_DWORD cch = 0;
            ChkDR( DRM_SizeTToDWord( DRMCRT_strlen(szArg), &cch ) );
            ChkArg( DRM_SUCCESS == DRMCRT_AtoDWORD( szArg, cch, 10, &dwIndex ) );
            ChkArg( dwIndex < GDATA_COUNT );
...
static DRM_RESULT _GetBinaryDataBuffer(const char *szArg, __deref_out_ecount(*cbOut) DRM_BYTE **pbOut, __out DRM_DWORD *cbOut)
{
    DRM_RESULT  dr      = DRM_E_TEST_INCOMPLETE;
    DRM_DWORD   dwIndex = 0;
    DRM_KEYFILE_CONTEXT *poKeyfileContext = NULL;
...
{
    DRM_RESULT        dr      = DRM_E_TEST_INCOMPLETE;
    const DRM_STRING *pdstr1  = NULL;
    const DRM_STRING *pdstr2  = NULL;
    DRM_LONG          i       = 0;
...
DRM_RESULT DRM_CALL Test_VerifyXMLDevcert(long argc, __in_ecount(argc) char **argv)
{
    ...
    const DRM_STRING         *pdstrDevcert  = NULL;
    DRM_CRYPTO_CONTEXT       *pCrypto       = NULL;
    DRM_LONG                  i             = 0;
...
DRM_RESULT DRM_CALL Test_VerifyBinaryDevcert(long argc, __in_ecount(argc) char **argv)
{
    ...
    DRM_BOOL                fPrndEncryptFlag    = FALSE;
    DRM_BOOL                fRevocationFlag     = FALSE;
    DRM_BOOL                fTransmitterFlag    = FALSE;
    DRM_BOOL                fReceiverFlag       = FALSE;
    ...
    */
    ...
    if( argc >= 3 && argv[1] != NULL && argv[2] != NULL)
    {
        ...
        ChkDR( DRMCRT_AtoDWORD( argv[2], cch, 0, &dwExpectedPlatformID ) );
        ChkDR( DRM_UTL_GetVersionFromStringAsDWORD( dstrSecurityVersion.pwszString,
                                                    dstrSecurityVersion.cchString,
    ...
    */
    for ( dw = 0; dw < oDeviceCert.FeatureInfo.dwNumFeatureEntries; dw++ )
    {
        ...
        case DRM_BCERT_FEATURE_TRANSMITTER:
            ChkBOOL( DRM_PRND_IsPRNDTransmitterSupported(), DRM_E_TEST_UNEXPECTED_OUTPUT );
            fTransmitterFlag = TRUE;
            break;
        case DRM_BCERT_FEATURE_RECEIVER:
            ChkBOOL( DRM_PRND_IsPRNDReceiverSupported(), DRM_E_TEST_UNEXPECTED_OUTPUT );
            fReceiverFlag = TRUE;
            break;
        default:
            /*
    ...
    */
    ...
    ChkBOOL( !DRM_PRND_IsPRNDTransmitterSupported()  || fTransmitterFlag, DRM_E_TEST_UNEXPECTED_OUTPUT );
    ChkBOOL( !DRM_PRND_IsPRNDReceiverSupported()     || fReceiverFlag,    DRM_E_TEST_UNEXPECTED_OUTPUT );
            ...
            if ( ( ( oDeviceCert.KeyInfo.rgoKeys[dw].dwUsageSet ) & BCERT_KEYUSAGE_BIT( iUsage ) ) > 0 )
            {
                switch ( iUsage )
                {
                    ...
                    case DRM_BCERT_KEYUSAGE_PRND_ENCRYPT_KEY:
                        /*
                        ** Check that the prnd encrypt usage flag has not already
                        ** been found
                        */
                        ChkBOOL( !fPrndEncryptFlag, DRM_E_TEST_UNEXPECTED_OUTPUT );
                        fPrndEncryptFlag     = TRUE;
                        break;
                    default:
                        /*
...
DRM_RESULT DRM_CALL Test_ReplaceKeyFile( long argc, __in_ecount( argc ) char **argv )
{
    DRM_RESULT       dr          = DRM_SUCCESS;
...
DRM_RESULT DRM_CALL Test_ReplaceHDSFile( long argc, __in_ecount( argc ) char **argv )
{
    DRM_RESULT       dr          = DRM_SUCCESS;
    DRM_CONST_STRING dstrHDSFile = EMPTY_DRM_STRING;

    ChkArg( HASARG( 0 ) );

    ChkDR( MakeDRMString( &dstrHDSFile, argv[0] ) );

    ChkDR( _CopyFileToDRMFolder( dstrHDSFile.pwszString , DEVICE_STORE_FILE_NAME ) );

ErrorExit:
    FREEDRMSTRING( dstrHDSFile );
    return dr;
}
...
DRM_RESULT DRM_CALL MODULE_PRETESTCASE(DeviceDevCert)(long lTCID, const char *strTCName)
{
    ...
    ChkDR( Test_RemoveDeviceData( 0, NULL ) );

    /*
    ** There are some ddc test area executables that link to this lib and CANNOT call 
    ** the DRMManager's PRETESTCASE function before their test case(s) start.
    ** The following lines take care of necessary DrmManager functionality dependencies
    ** for these ddc test executables.
    */
    
    INIT_SHARED_APP_CONTEXT_NO_DRMINIT( poAppContext, pbUnused, pwszDeviceStoreName );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( pdwNumRights, NUMBER_OF_RIGHTS_VAR, SIZEOF( DRM_DWORD ) );
    *pdwNumRights = 0;
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( ppRightsArray, RIGHTS_ARRAY_VAR, MAX_RIGHTS * SIZEOF( DRM_CONST_STRING* ) );

    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( pfNeedsCallBack, NEEDS_CALLBACK_VAR, SIZEOF( DRM_BOOL ) );
    *pfNeedsCallBack = FALSE;
   
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( poDecryptContext, DECRYPT_CONTEXT_VAR, SIZEOF( DRM_DECRYPT_CONTEXT ) );
    ZEROMEM( poDecryptContext, SIZEOF( DRM_CIPHER_CONTEXT ) );
 
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( pAesCtrContext, AES_COUNTER_MODE_CONTEXT_VAR, SIZEOF( DRM_AES_COUNTER_MODE_CONTEXT ) );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( prgbAESKey, AES_KEY_VAR, DRM_AES_KEYSIZE_128 * SIZEOF( DRM_BYTE ) );
    MEMCPY( prgbAESKey, rgbDefaultAESKey, DRM_AES_KEYSIZE_128 * SIZEOF( DRM_BYTE ) );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( prgbRC4Key, RC4_KEY_VAR, DRMCIPHERKEYLEN_RC4 * SIZEOF( DRM_BYTE ) );
    MEMCPY( prgbRC4Key, rgbDefaultRC4Key, DRMCIPHERKEYLEN_RC4 * SIZEOF( DRM_BYTE ) );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( poXRightData, RESPONSE_XPOL_RIGHT_VAR, SIZEOF( DRM_TEST_XPOL_DATA ) );
    ZEROMEM( poXRightData, SIZEOF( DRM_TEST_XPOL_DATA ) );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( poXRestrictionData, RESPONSE_XPOL_RESTRICTION_VAR, SIZEOF( DRM_TEST_XPOL_DATA ) );
    ZEROMEM( poXRestrictionData, SIZEOF( DRM_TEST_XPOL_DATA ) );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( pdrmidLID, LICENSE_ID_VAR, SIZEOF( DRM_GUID ) + 1 );
    ZEROMEM( pdrmidLID, SIZEOF( DRM_GUID ) + 1 );
    
    MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( poLicenseResponse, LICENSE_RESPONSE_VAR, SIZEOF( DRM_LICENSE_RESPONSE ) );
    MEMCPY( poLicenseResponse, &oDefaultLicResp, SIZEOF( DRM_LICENSE_RESPONSE ) );

    if( DRM_REVOCATION_IsRevocationSupported() )
    {
        MAP_TO_GLOBAL_TEST_VAR_AllocIfNotExists( pbRevocationBuffer, REVOCATION_BUFFER_VAR, REVOCATION_BUFFER_SIZE );
...
DRM_RESULT DRM_CALL MODULE_POSTTESTCASE(DeviceDevCert)(long lTCID, const char *strTCName)
{
    ...
    POSTTESTCASE_FUNCTION_CALL( DRM_TST_GetTestVar( NUMBER_OF_RIGHTS_VAR, ( DRM_VOID ** ) &pdwNumRights, NULL ) );
    POSTTESTCASE_FUNCTION_CALL( DRM_TST_GetTestVar( RIGHTS_ARRAY_VAR, ( DRM_VOID ** ) &ppRightsArray, NULL ) );
...
BEGIN_TEST_API_LIST( DeviceDevCert )
    ...
    API_ENTRY(Test_ReplaceKeyFile)
    API_ENTRY(Test_ReplaceHDSFile)
