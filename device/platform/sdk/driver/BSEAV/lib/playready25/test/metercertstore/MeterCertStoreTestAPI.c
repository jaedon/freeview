...
DRM_RESULT DRM_CALL TestMeterCertUpdate( long argc, __in_ecount(argc) char **argv )
{
    ...
    if ( argv[2] != NULL )
    {
        ...
        pdcstrLAINFO = ( const DRM_CONST_STRING * ) &dstrLAINFO;
    }
    if ( argv[1] != NULL )
    {
...
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_INVALID_PARAM_VALUE_1, "Test code is potentially passing invalid parameters intentionally." );
    ChkDR( DRM_MCS_OpenStore( fValidDST ? &( ( DRM_APP_CONTEXT_INTERNAL * ) poManagerContext )->oDatastoreHDS : NULL,
                              ...
                              (DRM_DWORD)(fValidpbBuffer ? METERCERT_BUFFER_SIZE : 0),
                              pMeterCertContext ) );
PREFAST_POP
