    ...
    __in_ecount( argc ) char **argv )
{
    ...
    {
        dwPriority = (DRM_DWORD)atoi( argv[ 5 ] );
    }

    ChkDR( DRM_NST_AddLicense( poNST,
                               cbTemp,      /* size WITHOUT priority */
                               pbLicense,   /* buffer WITH priority */
                               &oKID,
                               &oLID,
                               dwPriority ) );
    ...
    __in_ecount( argc ) char **argv )
{
    ...
    AssertChkArg( poNSTInternal != NULL );
    ...
    {
        ChkBOOL( cbLicense1 == cbLicense2, DRM_E_TEST_UNEXPECTED_OUTPUT );
