...
DRM_RESULT DRM_CALL tGetBBContext(DRM_BB_CONTEXT **ppBB)
{
    ...
    OEM_SECURE_ZERO_MEMORY( *ppBB,
                            SIZEOF( DRM_BB_CONTEXT ) );

    OEM_SECURE_ZERO_MEMORY( *ppBB, SIZEOF(DRM_BB_CONTEXT) );
    ...
    ChkDR( DRM_BBX_SetupSecureStoreKey( *ppBB, NULL ) );
    ...
    ChkDR( DRM_BBX_TestDeviceKeyPair( *ppBB ) );
    ...
    )
{
    ...
    *f_pfUseSoap = TRUE; /* defaults to TRUE */
    ...
    case eDRM_TEST_NET_LICGET:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_LICGET ), &f_pdastrHeader->cchString ) );
    ...
    case eDRM_TEST_NET_LICGET_LEGACY:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_LEGACY ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( LEGACY_CHALLENGE_FORMAT ), &f_pdastrFormat->cchString ) );
    ...
    case eDRM_TEST_NET_METERDATA_LEGACY:
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( LEGACY_METERCHALLENGE_FORMAT ), &f_pdastrFormat->cchString ) );
        ChkMem( f_pdastrFormat->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrFormat->cchString + 1 ) );
    ...
    case eDRM_TEST_NET_LICACK:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_LICACK ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
    ...
    case eDRM_TEST_NET_DOMLEAVE:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_LEAVE ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
    ...
    case eDRM_TEST_NET_DOMJOIN:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_JOIN ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
    ...
    case eDRM_TEST_NET_METERCERT:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_METERCERT ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
    ...
    case eDRM_TEST_NET_METERDATA:
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( HTTP_HEADER_METERDATA ), &f_pdastrHeader->cchString ) );
        ChkMem( f_pdastrHeader->pszString = ( DRM_CHAR *) Oem_MemAlloc( f_pdastrHeader->cchString + 1 ) );
...
static DRM_RESULT DRM_CALL _tOEM_SendRequestRedirectable(
    ...
    __in URL_COMPONENTSA *f_pUrlComponents,
    __in DRM_BYTE *f_pbChallenge,
    __in DRM_DWORD f_cbChallenge,
    __in DRM_CHAR *f_pszHeader,
 ...
 )
{
    ...
    if ( f_dwLevel > 0 )
    {
        ...
        if ( atoi( rgbQuery ) == HTTP_STATUS_TEMPORARY_REDIRECT )
        {
            ...
            ZEROMEM( f_pUrlComponents, SIZEOF( URL_COMPONENTSA ) );

            f_pUrlComponents->dwStructSize = SIZEOF( URL_COMPONENTSA );
            f_pUrlComponents->lpszHostName = rgbHostName;
            f_pUrlComponents->dwHostNameLength = SIZEOF( rgbHostName );
            f_pUrlComponents->lpszUrlPath = rgbUrlPath;
            f_pUrlComponents->dwUrlPathLength = SIZEOF( rgbUrlPath );
    ...
    {
        Log( "", "\n\nSending HTTP request to:\n\tHost:%s\n\tPort:%d\n\t Url:%s\n\n", f_pUrlComponents->lpszHostName, f_pUrlComponents->nPort, f_pUrlComponents->lpszUrlPath );
    }
    ...
    for ( retries = 0; retries < MAX_HTTP_RETRIES; ++retries )
    {
        ...
        *f_phConnect = InternetConnectA( f_hSession,
                                         f_pUrlComponents->lpszHostName,
                                         f_pUrlComponents->nPort,
                                         NULL,
                                         NULL,
    ...
    {
        DRM_RESULT drInner = DRM_RESULT_FROM_WIN32( GetLastError() );
