...
/**********************************************************************
** Function: ReadFileToTestVariable
**
** Synopsis: Reads the data in a file into a Test Variable.
**
** Arguments:   argc     -- Count of arguments
**              argv[0]  -- Name of the variable
**              argv[1]  -- file to open
**
** Returns:     DRM_E_TEST_FILE_LOAD_ERROR - If opening the file failed.
**
***********************************************************************/
DRM_RESULT DRM_CALL ReadFileToTestVariable( long argc, __in_ecount( argc) char **argv )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    ChkBOOL( LoadTestFile( NULL, argv[1], &pbFile, &cbFile ), DRM_E_TEST_FILE_LOAD_ERROR );
    ChkDR( DRM_TST_SetTestVar( argv[0], (DRM_VOID **) pbFile, cbFile, NULL ) );
...
/**********************************************************************
** Function: WriteTestVariableToFile
**
** Synopsis: Writes the data in a test variable to a file.
**
** Arguments:   argc     -- Count of arguments
**              argv[0]  -- Name of the variable
**              argv[1]  -- file to write
**
** Returns:     DRM_E_TEST_INCOMPLETE - If writing to the file failed.
**
***********************************************************************/
DRM_RESULT DRM_CALL WriteTestVariableToFile( long argc, __in_ecount( argc) char **argv )
{
    DRM_RESULT       dr       = DRM_SUCCESS;
    DRM_BYTE        *pbFile   = NULL;
    DRM_DWORD       *pcbFile  = NULL;
    ...
    ChkDR( DRM_TST_GetTestVar( argv[0], (DRM_VOID**) &pbFile, &pcbFile ) );
    ChkDR( TST_UTL_MakePathString( NULL, argv[1], &dstrPath ) );
    ChkBOOL( SaveToFileW( dstrPath.pwszString, pbFile, *pcbFile ), DRM_E_TEST_INCOMPLETE );

ErrorExit:
    SAFE_OEM_FREE(dstrPath.pwszString);
    return dr;
}
