...
typedef DRM_RESULT ( DRM_CALL *signXmrLicenseFuncFP )(
...
static const DRM_BYTE g_rgbRSAModulus1024[__CB_DECL( DRM_RSA_CB_MODULUS( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) )] =
{
    0x97, 0x91, 0x29, 0x85,
    0xDD, 0x1F, 0x9E, 0x45,
    0xE3, 0x7B, 0x19, 0x31,
...
static DRM_RESULT DRM_CALL _NoSignXmrLicense(
    __in                                const DRM_VOID  *f_pvSignatureContext,
    __in_bcount( f_cbLicense )          const DRM_BYTE  *f_pbLicense,
    __in                                const DRM_DWORD  f_cbLicense,
    __out_bcount( f_cbSignatureBuffer )       DRM_BYTE  *f_pbSignatureBuffer,
    ...
    __in                                const DRM_DWORD  f_ibSignatureBuffer )
{
    ...
    UNREFERENCED_PARAMETER(f_pbLicense);
    UNREFERENCED_PARAMETER(f_cbLicense);
    UNREFERENCED_PARAMETER(f_pbSignatureBuffer);
    UNREFERENCED_PARAMETER(f_cbSignatureBuffer);
    UNREFERENCED_PARAMETER(f_ibSignatureBuffer);
    return DRM_SUCCESS;
}
PREFAST_POP;
...
static DRM_DWORD _CalcV3LicenseResponseCharCount(
    __in       DRM_ANSI_CONST_STRING  f_rgdastrXMRLic[],
    __in       DRM_DWORD              f_cXMRLic,
    __in const DRM_ANSI_CONST_STRING *f_pdastrRevInfo,
    __in const DRM_ANSI_CONST_STRING *f_pdastrCustomData,
    __in const DRM_ANSI_CONST_STRING *f_pdastrTransactionID,
    __in       DRM_BOOL               f_fUseSlk )
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_DWORD i = 0;
...
DRM_RESULT DRM_CALL Test_BuildV3LicenseResponse(
    ...
    __in                         const DRM_ANSI_CONST_STRING *f_pdastrCustomData,
    __in                         const DRM_ANSI_CONST_STRING *f_pdastrTransactionID,
    __out_ecount_opt(*f_pcbXMRLicResp) DRM_BYTE              *f_pbXMRLicResp,
    __inout                            DRM_DWORD             *f_pcbXMRLicResp )
{
    DRM_RESULT dr = DRM_SUCCESS;
    ...
    */
    ...
    if ( *pfUseSlk )
    {
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( dastrSlkID.pszString ), &dastrSlkID.cchString ) );
        ...
        ChkDR( DRM_SizeTToDWord( DRMCRT_strlen( dastrSlkData.pszString ), &dastrSlkData.cchString ) );
...
**              argv[39]: optional. Boolean indicating do not sign the license.
**
** Returns:     DRM_SUCCESS                  - on success
**
** Note:
...
DRM_RESULT DRM_CALL Test_CreateXMRLicense( __deref_out_ecount( *f_pcbLicense ) DRM_BYTE **f_ppbLicense, __out DRM_DWORD *f_pcbLicense, __in DRM_BOOL f_fLog, long argc, __in_ecount( argc ) char **argv )
{
    ...
    DRM_XMR_OPTIMIZED_CONTENT_KEY               optContentKey                                   = {0};
    DRM_XMR_CONTENT_KEY                         contentKey                                      = {0};
    DRM_XMR_ECC_DEVICE_KEY                      deviceKey                                       = {0};
    DRM_XMR_DEVICE_KEY                          rsaDeviceKey                                    = {0};
    DRM_XMR_GENERATION_NUMBER                   xmrGenerationNumber                             = {0};
    DRM_XMR_BUILDER_CONTEXT                     xmrBuilder                                      = {0};
    DRM_XMR_MINIMUM_ENVIRONMENT                 xmrMinEnv                                       = {0};
    DRM_XMR_METERING                            xmrMID                                          = {0};
    ...
    DRM_XMR_EXPIRATION_AFTER_FIRSTSTORE         xmrExpirationOnStore                            = {0};
    ...
    DRM_BYTE                                   *pbCryptoBuffer                                  = NULL;
    DRM_BYTE                                    rgbEncKey[max( ECC_P256_CIPHERTEXT_SIZE_IN_BYTES + DRM_AES_KEYSIZE_128, DRM_AES_KEYSIZE_128*2 )] = {0};
    ...
    enum XMR_ASYMMETRIC_ENCRYPTION_TYPE         eKeyEncryptionCipherType                        = XMR_ASYMMETRIC_ENCRYPTION_TYPE_INVALID;
    ...
    DRMFILETIME                                 ft                                              = {0};
    DRM_LONG                                    iArg                                            = 0;
    DRM_RSA_PUBLIC_KEY_2048BIT                  oRsaPubKey                                      = {0};
    ...
    signXmrLicenseFuncFP                        fpSignXmrLicenseFunc                            = _SignXmrLicense;
    ...
    ChkArg( argv[1] != NULL );
    ChkArg( argv[2] != NULL );
    ChkMem( pbCryptoBuffer = (DRM_BYTE*)Oem_MemAlloc( max( SIZEOF( DRM_CRYPTO_CONTEXT ), SIZEOF( DRM_SHA_CONTEXT ) ) ) );
    ...
    */
    ...
    else
    {
        ...
        {
            ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrLID, (DRM_GUID*)pLID ) );
        }
        else
        {
            ChkDR( DRM_UTL_DecodeKID( &dstrLID, (DRM_KID*)pLID ) );
...
#pragma prefast(pop)
    ...
    */
    ...
    ChkDR( OEM_ECC_GenerateHMACKey_P256( (PLAINTEXT_P256 *)rgbKey, (struct bigctx_t *)pbCryptoBuffer ) );
     ...
     && strcmp( argv[4], "ROOT" ) != 0 )
    {
        ...
        */
        ChkDR( DRM_DWordToWord( ( ( dwVersion < XMR_VERSION_3 ) ? XMR_OBJECT_TYPE_UPLINK_KID_OBJECT : XMR_OBJECT_TYPE_UPLINK_KID_2_OBJECT ), &wObjectType ) );
    ...
    else if ( argc > 17 && argv[17] != NULL ) /* This is Domain Bound */
    {
        ...
        */
        ...
        {
            DRM_DWORD cbPublicKey = SIZEOF( oDomainPubKey );
            ...
            */
            {
                ...
                ChkDR( _Hal_AllocateKeyHandle( &pAppContext->oBlackBoxContext, OEM_HAL_KEY_TYPE_ECC_P256_DOMAIN_ENCRYPT, NULL, 0, &hPrivKey, &pKeyEntry, &fCachedHandle ) );

                ChkDR( OEM_HAL_TEST_GenerateEcc256KeyPair( &pAppContext->oBlackBoxContext.CryptoContext,
                                                           OEM_HAL_KEY_TYPE_ECC_P256_DOMAIN_ENCRYPT,
                                                           pKeyEntry->indexKeyRegister ) );
            }

            /*
            ** Export the public key component from the register
            */
            ChkDR( _Hal_WrapKey( &pAppContext->oBlackBoxContext,
                                 hPrivKey,
                                 OEM_CRYPTO_HANDLE_INVALID,
                                 ( DRM_BYTE* )&oDomainPubKey,
                                 &cbPublicKey ) );

            DRMASSERT( cbPublicKey == SIZEOF( oDomainPubKey ) );
        ...
        */
        if( hPrivKey == OEM_CRYPTO_HANDLE_INVALID )
        {
            ...
            if( DRM_SUCCEEDED( dr ) )
            {
                ...
                */
                ChkDR( _SymmetricUnwrapKey( &pAppContext->oBlackBoxContext,
                                            (DRM_BYTE*)oDomainKeyRecord.oDomainPrivKeyEncrypted.m_rgbPrivkey,
                                            SIZEOF( PRIVKEY_P256 ),
                                            &hPrivKey ) );
            }
        }
        ...
        ChkDR( OEM_ECC_Encrypt_P256( ( PUBKEY_P256 * )&oDomainPubKey,
                                     ...
                                     (struct bigctx_t *)pbCryptoBuffer ) );
        ...
        if( f_fLog )
        {
            ...
            ChkMem( pch = (DRM_CHAR*)Oem_MemAlloc( dstrAccountID.cchString + 1 ) );
            ZEROMEM( pch, SIZEOF(DRM_CHAR) * ( dstrAccountID.cchString + 1 ) );
            DRM_UTL_DemoteUNICODEtoASCII( dstrAccountID.pwszString, pch, dstrAccountID.cchString );
            Log( "", "\t_CreateXMRLicense: Using domain account %s\n", pch );
            SAFE_OEM_FREE( pch );

            ChkMem( pch = (DRM_CHAR*)Oem_MemAlloc( dstrDomainKey.cchString + 1 ) );
            ZEROMEM( pch, SIZEOF(DRM_CHAR) * ( dstrDomainKey.cchString + 1 ) );
            DRM_UTL_DemoteUNICODEtoASCII( dstrDomainKey.pwszString, pch, dstrDomainKey.cchString );
    ...
    else if( dwVersion == XMR_VERSION_1 )
    {
        ...
        */
        ChkDR( OEM_RSA_SetPublicKey_2048BIT(  eDRM_RSA_KEY_LENGTH_1024_2048BIT,
        ...
        ChkDR( OEM_RSA_OaepEncrypt_2048BIT( NULL,
                                           &oRsaPubKey,
                                            DRM_AES_KEYSIZE_128,
                                            prgbAesSharedKey,
                                            &cbEncKey,
                                            rgbEncKey,
                                            (DRM_CRYPTO_CONTEXT *)pbCryptoBuffer ) );
    ...
    {
        ChkDR( OEM_ECC_Encrypt_P256( &pAppContext->oBlackBoxContext.cachedBCertValues.DeviceKeyEncrypt.PublicKeyContext.PublicKey,
                                        (PLAINTEXT_P256 *)rgbKey,
                                        (CIPHERTEXT_P256 *)rgbEncKey,
                                        (struct bigctx_t *)pbCryptoBuffer ) );
        ...
        deviceKey.fValid          = TRUE;
        deviceKey.wEccCurveType   = XMR_ECC_CURVE_TYPE_P256;
        deviceKey.pbKeyData       = (DRM_BYTE*)&pAppContext->oBlackBoxContext.cachedBCertValues.DeviceKeyEncrypt.PublicKeyContext.PublicKey;
        deviceKey.cbKeyData       = SIZEOF( PUBKEY_P256 );
        deviceKey.iKeyData        = 0;
    ...
    */
    ...
    ChkDR( DRM_DWordToWord( eSymmetricEncryptionType, &contentKey.wSymmetricCipherType ) );
    ChkDR( DRM_DWordToWord( eKeyEncryptionCipherType, &contentKey.wKeyEncryptionCipherType ) );
    contentKey.pbEncryptedKeyBuffer     = rgbEncKey;
    ChkDR( DRM_DWordToWord( cbEncKey, &contentKey.cbEncryptedKey ) );
    ...
    */
    if ( *pfSymmOpt && eKeyEncryptionCipherType != XMR_ASYMMETRIC_ENCRYPTION_TYPE_CHAINED_LICENSE )
    {
        ...
        MEMCPY( rgbOptEncKey, rgbKey, cbOptEncKey );

        if( hPrivKey != OEM_CRYPTO_HANDLE_INVALID )
        {
            /*
            ** Directly inject the content key into the register.
            ** We'll use functions only available in test code to accomplish this.
            ** This test codepath here mimics the behavior of _Hal_UnwrapKey, but uses
            ** OEM_HAL_TEST_InitTestKeysUnwrap instead of static _InitializeUnwrapContext,
            ** uses OEM_HAL_TEST_InjectCleartextKey instead of one of the legitimate
            ** *_Unwrap* function codepaths, and has reduced reliability and robustness checks.
            */
            DRM_BB_KEY_REGISTER_ENTRY   *pKeyEntry      = NULL;
            DRM_BOOL                     fCachedHandle  = FALSE;

            ChkDR( _Hal_AllocateKeyHandle( &pAppContext->oBlackBoxContext, OEM_HAL_KEY_TYPE_AES_CTR_CONTENT, rgbOptEncKey, cbOptEncKey, &hContentKey, &pKeyEntry, &fCachedHandle ) );
            ChkPtr( pKeyEntry );
            if( !fCachedHandle )
            {
                OEM_HAL_UNWRAP_CONTEXT unwrapContext;

                ChkDR( OEM_HAL_TEST_InitTestKeysUnwrap( OEM_HAL_KEY_TYPE_AES_CTR_CONTENT,
                                                        pKeyEntry->indexKeyRegister,
                                                        OEM_HAL_KEY_TYPE_INVALID,
                                                        OEM_HAL_KEY_REGISTER_INDEX_INVALID,
                                                        cbOptEncKey,
                                                        &unwrapContext ) );
                ChkDR( OEM_HAL_TEST_InjectCleartextKey( &unwrapContext, rgbOptEncKey, cbOptEncKey ) );

                DRM_BYT_CopyBytes( pKeyEntry->pbCacheKey, 0, rgbOptEncKey, 0, cbOptEncKey );
                pKeyEntry->cbCacheKey = cbOptEncKey;
            }

            /*
            ** Wrap with the private key, which will symmetrically encrypt it
            */
            ChkDR( _Hal_WrapKey( &pAppContext->oBlackBoxContext,
                                 hContentKey,
                                 hPrivKey,
        ...
        ChkDR( DRM_DWordToWord( cbOptEncKey, &optContentKey.cbEncryptedKey ) );
    ...
    */
    ChkDR( DRM_DWordToWord( ( ( dwVersion < XMR_VERSION_3 ) ? XMR_OBJECT_TYPE_MINIMUM_ENVIRONMENT_OBJECT : XMR_OBJECT_TYPE_SECURITY_LEVEL ), &wObjectType ) );
    ChkDR( DRM_XMR_InitializeObject( wObjectType, &xmrMinEnv ) );
    xmrMinEnv.fValid                = TRUE;
    if ( argc > 12
      && argv[12] != NULL )
    {
        ChkDR( DRM_DWordToWord( strtoul( argv[12], NULL, 10 ), &xmrMinEnv.wMinimumSecurityLevel ) );
    }
    else
    {
        ChkDR( DRM_DWordToWord( 150, &xmrMinEnv.wMinimumSecurityLevel ) );
    }
    ...
    */
      ...
      || strcmp( argv[13], "OMIT" ) != 0 )
    {
        ...
        ChkDR( DRM_DWordToWord( ( ( dwVersion < XMR_VERSION_3 ) ? XMR_OBJECT_TYPE_COPY_POLICY_CONTAINER : XMR_OBJECT_TYPE_COPY_POLICY_2_CONTAINER ), &wObjectType ) );
    ...
    */
      ...
      && argv[7] != NULL )
    {
        ...
        ChkDR( DRM_DWordToWord( ( ( dwVersion < XMR_VERSION_3 ) ? XMR_OBJECT_TYPE_EXPIRATION_AFTER_FIRSTUSE_OBJECT : XMR_OBJECT_TYPE_EXPIRATION_AFTER_FIRSTPLAY_OBJECT ), &wObjectType ) );
    ...
    */
    ...
    if ( argc > 18 )
    {
        ...
        if( argv[18] != NULL )
        {
            ChkDR( DRM_DWordToWord( strtoul( argv[18], NULL, 10 ), &xmrOPLs.wAnalogVideo ) );
            xmrOPLs.fValid            = TRUE;
        }

        if ( argc > 19 && argv[19] != NULL )
        {
            ChkDR( DRM_DWordToWord( strtoul( argv[19], NULL, 10 ), &xmrOPLs.wCompressedDigitalAudio ) );
            xmrOPLs.fValid            = TRUE;
        }

        if ( argc > 20 && argv[20] != NULL )
        {
            ChkDR( DRM_DWordToWord( strtoul( argv[20], NULL, 10 ), &xmrOPLs.wUncompressedDigitalAudio ) );
            xmrOPLs.fValid            = TRUE;
        }

        if ( argc > 21 && argv[21] != NULL )
        {
            ChkDR( DRM_DWordToWord( strtoul( argv[21], NULL, 10 ), &xmrOPLs.wCompressedDigitalVideo ) );
            xmrOPLs.fValid            = TRUE;
        }

        if ( argc > 22 && argv[22] != NULL )
        {
            ChkDR( DRM_DWordToWord( strtoul( argv[22], NULL, 10 ), &xmrOPLs.wUncompressedDigitalVideo ) );
            xmrOPLs.fValid            = TRUE;
        }
    ...
    */
    ...
    if ( argc > 35 && argv[35] != NULL )
    {
        ...
        if ( fUseXpol )
        {
            ...
            ChkDR( DRM_DWordToWord( poXRestrictionData->cbXPolData, &wcbData ) );
    ...
    */
    ...
    if ( HASARG( 39 ) )
    {
        fpSignXmrLicenseFunc = _NoSignXmrLicense;
    }

    *f_pcbLicense = 0;
    dr = DRM_XMR_FinishLicense( &xmrBuilder,
                                rgbKey,
                                fpSignXmrLicenseFunc,
                                NULL,
                                f_pcbLicense );
...
ErrorExit:
    ...
    SAFE_OEM_FREE( pbCryptoBuffer );
    FREEDRMSTRING( dstrUplinkKID );
    SAFE_OEM_FREE( xmrSerialNum.pbDataBuffer );

    if( pAppContext != NULL )
    {
        DRM_BBX_SafeReleaseKey( &pAppContext->oBlackBoxContext, &hPrivKey );
        DRM_BBX_SafeReleaseKey( &pAppContext->oBlackBoxContext, &hContentKey );
    }
