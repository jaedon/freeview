...
*****************************************************************************/
DRM_RESULT DRM_CALL _tRotateContentKey( __in            const DRM_BYTE   *f_pbBaseContentKey,
                                        __in                  DRM_DWORD   f_cbBaseContentKey,
                                        __in                  DRM_DWORD   f_dwCounter,
                                        __out_bcount(f_cbKey) DRM_BYTE   *f_pbKey,
...
DRM_RESULT _MakeRandomKID(DRM_CONST_STRING *pKID)
{
    ...
    DRM_RESULT dr        = DRM_SUCCESS;
    DRM_DWORD  cbKID     = LEGACY_KID_B64ENC_LEN * SIZEOF(DRM_CHAR);
    DRM_GUID   keyGuid   = {0};
    ...
    ChkMem( pKID->pwszString = ( DRM_WCHAR * )Oem_MemAlloc( LEGACY_KID_B64ENC_LEN * SIZEOF( DRM_WCHAR ) ) );
...
*/
static DRM_RESULT _GetNonRandomBytes(__in_ecount( cbLen ) DRM_BYTE *pBuffer, DRM_DWORD cbLen)
{
    DRM_RESULT dr = DRM_SUCCESS;
    DRM_BYTE val = 0;
    DRM_DWORD index = 0;
...
DRM_RESULT tSetECCPubKey(const DRM_BYTE *pPubKey, DRM_DWORD cbPubKey)
{
    DRM_RESULT dr = DRM_SUCCESS;
...
**          DRM_SUCCESS or an error code
*****************************************************************************/
static DRM_RESULT _GetAuxKeyEntries( __in                       const DRM_CONST_STRING       *f_pdstrParent,
                                     __in                             DRM_DWORD               f_cbBaseKey,
                                     __in_bcount( f_cbBaseKey ) const DRM_BYTE               *f_pbBaseKey,
 ...
 )
{
    ...
    DRM_BYTE abEncryptedKeyBlob[DRM_RSA_CB_CIPHERTEXT( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) + DRM_AES_KEYSIZE_128] = {0};
    ...
    DRM_BYTE  rgbLGPubKeyModulus [__CB_DECL(DRM_RSA_CB_MODULUS( eDRM_RSA_KEY_LENGTH_1024_2048BIT ) * 2 + 8)] = { 0 };
    ...
    const DRM_BYTE                     *pbKeyToRotate           = 0; /* do not free this */
    ...
    // xbox tests run out of stack space, so move this to heap.
    DRM_BYTE                           *pbBaseLicStack          = NULL;
    DRM_CRYPTO_CONTEXT                 *pctx                    = NULL;

    ChkArg(pRights);
    ChkArg(ppList);

    ChkMem( pbBaseLicStack = (DRM_BYTE*)Oem_MemAlloc( STACK_SIZE ));
    ZEROMEM( pbBaseLicStack, STACK_SIZE );
    ChkMem( pctx = (DRM_CRYPTO_CONTEXT*)Oem_MemAlloc( SIZEOF( DRM_CRYPTO_CONTEXT ) ) );
    ZEROMEM( pctx, SIZEOF( DRM_CRYPTO_CONTEXT ) );
    ...
    if (pOptionalParams)
    {
        FREEDRMSTRING( pOptionalParams->dstrKIDUsed );
    ...
    if (pdstrUplinkKID)
    {
        if (pOptionalParams)
        {
            FREEDRMSTRING( pOptionalParams->dstrRootKIDUsed );
    ...
    if (pOptionalParams && pOptionalParams->pdstrRIDOverride)
    {
        ...
        {   /* GUID text string including '{' and '}' */
            ...
            ChkDR(DRM_UTL_StringToLittleEndianGuid( &dstrLID, (DRM_GUID *)&drmLID));
            fUseRandomLID = FALSE;
        }
    }
    ...
    ChkDR(DRM_XMR_StartLicense(pbBaseLicStack, STACK_SIZE, &drmLID, dwXmrVersion, &xmrBuilder));
    ...
    if ( pOptionalParams )
    {
        FREEDRMSTRING( pOptionalParams->dstrLIDUsed );
    ...
    {
        cbCheckSum = (IsRightTrue(pRights->Use16ByteCheckSum) ? (DRM_DWORD)16 : (DRM_DWORD)DRM_AES_CK_CHECKSUM_LENGTH);
    ...
    if( IsRightSet( pRights->UplinkX ) )
    {
        ...
        {
            bCheckSumUplinkX[0] = (DRM_BYTE)~bCheckSumUplinkX[0];
        }
        ...
        ChkDR( DRM_DWordToWord( cLocations, &xmrUplinkX.cEntries ) );
    ...
    else /* Simple or Root */
    {
        ...
        memcpy(rgbIntegrityKey, rgbKeyMaterial, SIZEOF(rgbKeyMaterial));
        ChkDR( OEM_ECC_GenerateHMACKey_P256( (PLAINTEXT_P256 *)rgbIntegrityKey, (struct bigctx_t *)pctx ) );

        cbEncryptedKeyBlob = ECC_P256_CIPHERTEXT_SIZE_IN_BYTES;

        ChkDR( DRM_PK_Encrypt( NULL, pctx, eDRM_ECC_P256,
    ...
    if( IsRightSet( pRights->AuxKey ) )
    {
        ...
        memcpy(rgbIntegrityKey, rgbKeyMaterial, SIZEOF(rgbKeyMaterial));
        ChkDR( OEM_ECC_GenerateHMACKey_P256( (PLAINTEXT_P256 *)rgbKeyMaterial, (struct bigctx_t *)pctx ) );
        ...
        ChkDR( DRM_DWordToWord( cLocations, &xmrAuxKey.cEntries ) );
    ...
    if ( IsRightSet( pRights->DomainID ) || ( pOptionalParams && pOptionalParams->pguidDomainAccountID != NULL ) )
    {
        ...
        {
            ChkDR( DRM_UTL_StringToLittleEndianGuid( &pRights->DomainAccountID, &guidAccountID ) );
    ...
    if (IsRightSet(pRights->PlayEnablerGUIDList))
    {
                                                 ...
                                                 0) ) )  /* is the first layer */
        {
            ...
            ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrGUID,
        ...
        || IsRightSet(pRights->CompressedDigitalAudio))
    {
        ...
        {
            ChkDR( DRMCRT_WtoDWORD( pRights->AnalogVideo.pwszString, pRights->AnalogVideo.cchString, 10, &dwTemp2, NULL ) );
            opl.wAnalogVideo = (DRM_WORD)dwTemp2;
        }
        if (IsRightSet(pRights->UnCompressedDigitalVideo))
        {
            ChkDR( DRMCRT_WtoDWORD( pRights->UnCompressedDigitalVideo.pwszString, pRights->UnCompressedDigitalVideo.cchString, 10, &dwTemp2, NULL ) );
            opl.wUncompressedDigitalVideo = (DRM_WORD)dwTemp2;
        }
        if (IsRightSet(pRights->UnCompressedDigitalAudio))
        {
            ChkDR( DRMCRT_WtoDWORD( pRights->UnCompressedDigitalAudio.pwszString, pRights->UnCompressedDigitalAudio.cchString, 10, &dwTemp2, NULL ) );
            opl.wUncompressedDigitalAudio = (DRM_WORD)dwTemp2;
        }
        if (IsRightSet(pRights->CompressedDigitalVideo))
        {
            ChkDR( DRMCRT_WtoDWORD( pRights->CompressedDigitalVideo.pwszString, pRights->CompressedDigitalVideo.cchString, 10, &dwTemp2, NULL ) );
            opl.wCompressedDigitalVideo = (DRM_WORD)dwTemp2;
        }
        if (IsRightSet(pRights->CompressedDigitalAudio))
        {
            ChkDR( DRMCRT_WtoDWORD( pRights->CompressedDigitalAudio.pwszString, pRights->CompressedDigitalAudio.cchString, 10, &dwTemp2, NULL ) );
            opl.wCompressedDigitalAudio = (DRM_WORD)dwTemp2;
        }
        opl.fValid = TRUE;
...
/************************************E X P O L I C Y ***********************************************/
    ...
    for( idx = 0; idx < g_cdwExtensiblePolicyMapMax; idx++ )
    {
        if( IsRightTrue( pRights->AllowExtensibleRights[idx] ) )
        {
            ...
            ChkDR( DRM_DWordToWord( g_XMRLicGenExtensiblePolicyMap[idx].dwXMRObjectTypeId, &wObjectType ) );
            ChkDR( DRM_XMR_AddUnknownObject(
                &xmrBuilder,
                wObjectType,
                TRUE,
    ...
    if( pdstrUplinkKID && !IsRightSet( pRights->UplinkX ) )
    {
        ...
        {
            bCheckSum[0] = (DRM_BYTE)~bCheckSum[0];    /* should be enough to invalidate the checksum. */
        }
    ...
    if (IsRightSet(pRights->InclusionList))
    {
        ...
        for( i=0; i<pRights->InclusionList.cchString && (xmrInclusionList.cGUIDs)<dwGuids; ++i)
        {
            if ( *(pRights->InclusionList.pwszString+i) == L'{' )
            {
                ...
                ChkDR( DRM_UTL_StringToLittleEndianGuid( &wszGUIDString, (DRM_GUID *)(xmrInclusionList.pguidBuffer + xmrInclusionList.cGUIDs*SIZEOF(DRM_GUID))));
    ...
    else
    {
        ...
        if (IsRightSet(pRights->RevInfoVersion))
        {
            if (pRights->RevInfoVersion.pwszString[0] == L'*')
            {
                ...
                /* set RIV to the current RIV2 value */
    ...
    if ( dwTestOptions & OP_LICGEN_TAMPERED_SIGNATURE )
    {
        if (cbLicense > 0)
        {
           pbLicense[cbLicense-1] = (DRM_BYTE)~pbLicense[cbLicense-1]; /* temp the last bit */
        }
    }
 ...
 )
{
    ...
    ELicenseType                     eLicenseType                = MAKE_SIMPLE_LICENSE;
    OptionalTestMakeXMRLicenseParams oOptionalParams             = {0};
    DRM_DWORD                        cbContentKey                = 0;

    if( pOptionalParams != NULL )
    {
        cbContentKey = pOptionalParams->cbContentKeyUsed;
    }
    ...
    while( pNextRight != NULL )
    {
        ...
        while( pLeaf != NULL )
        {
            if( pOptionalParams != NULL )
            {
                oOptionalParams.pbContentKeyUsed = pOptionalParams->pbContentKeyUsed;
                oOptionalParams.cbContentKeyUsed = cbContentKey;
            }
            ChkDR( _MakeXMRLicense( dwOptions
                ...
                , &pTempNode
                , NULL
                , ( fPassOptionalParamsToLeaf ? pOptionalParams : &oOptionalParams ) ) );
...
ErrorExit:
    FreeOptionalTestMakeXMRLicenseParams( &oOptionalParams );
    FREEDRMSTRING(dstrKID);
    return dr;
};
