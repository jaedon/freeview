...
const _TEST_XMR_TYPES g_rgXMRObjectTypes[] =
{
    ...
    { XMR_OBJECT_TYPE_ND_TX_AUTH_CONTAINER,      "XMR_OBJECT_TYPE_ND_TX_AUTH_CONTAINER",      SIZEOF( "XMR_OBJECT_TYPE_ND_TX_AUTH_CONTAINER"     )  },
};
                        ...
                        DRM_DWORD                        iArgContainer )
{
    ...
    ChkDR( DRM_UTL_StringToLittleEndianGuid(&dstrEnabler, &drmidEnabler) );
...
DRM_RESULT DRM_CALL  TestStartLicense(long argc, __in_ecount( argc )char **argv )
{
    ...
    else
    {
        ...
        ChkDR( DRM_UTL_StringToLittleEndianGuid( &dstrLID, (DRM_GUID*)&drmidLID ) );
    }
...
static DRM_RESULT DRM_CALL  _FinishLicense( const DRM_BYTE* pbKey )
{
    DRM_RESULT                   dr                                 = DRM_SUCCESS;
    DRM_XMR_BUILDER_CONTEXT      *pXmrBuilder                        = NULL;
...
DRM_RESULT DRM_CALL  TestVerifyUnpackedLicense(long argc, __in_ecount( argc ) char **argv)
{
    ...
    const DRM_XMR_UNKNOWN_CONTAINER *pUnkContainer = NULL;
        ...
        switch(wObjectType)
        {
            ...
            case XMR_OBJECT_TYPE_COPY_ENABLER_CONTAINER:
                ...
                while( pUnkContainer != NULL && pUnkContainer->fValid && !fFound )
                {
                    ...
                    dr = _TestVerifyEnablerGuid( argc, argv, pUnkContainer, (DRM_DWORD)iArg );
                    if( dr == DRM_SUCCESS )
                    {
                        fFound = TRUE;
                    }
            ...
            {
                DRM_DWORD iAuthObject = 0;
                DRM_DWORD cAuthObject = (DRM_DWORD)(atol(argv[++iArg]));
                const DRM_XMR_ND_TX_AUTH_OBJECT *pAuthObject = NULL;

                ChkBOOL( pXmrLicense->containerOuter.containerNDTxAuth.fValid, DRM_E_TEST_UNEXPECTED_OUTPUT );

                for( pAuthObject = pXmrLicense->containerOuter.containerNDTxAuth.pAuthObject; 
                    (iAuthObject < cAuthObject) && pAuthObject != NULL; 
                    iAuthObject++, pAuthObject = pAuthObject->pNext )
                {
                    DRM_DWORD cbData = 0;

                    ChkBOOL( pAuthObject != NULL, DRM_E_TEST_UNEXPECTED_OUTPUT );
                    ChkBOOL( pAuthObject->fValid, DRM_E_TEST_UNEXPECTED_OUTPUT );
                    ChkBOOL( pAuthObject->wVersion == (DRM_WORD)(atol(argv[++iArg])), DRM_E_TEST_UNEXPECTED_OUTPUT);

                    cbData = (DRM_DWORD)strlen(argv[++iArg]);
                    ChkBOOL( pAuthObject->cbDataClient == cbData, DRM_E_TEST_UNEXPECTED_OUTPUT);
                    ChkBOOL( memcmp( pAuthObject->pbBufferClient
                                   + pAuthObject->ibDataClient, argv[iArg], cbData ) == 0, DRM_E_TEST_UNEXPECTED_OUTPUT );

                    cbData = (DRM_DWORD)strlen(argv[++iArg]);
                    ChkBOOL( pAuthObject->cbDataService == cbData, DRM_E_TEST_UNEXPECTED_OUTPUT);
                    ChkBOOL( memcmp( pAuthObject->pbBufferService
                                   + pAuthObject->ibDataService, argv[iArg], cbData ) == 0, DRM_E_TEST_UNEXPECTED_OUTPUT );
                }

                ChkBOOL( iAuthObject == cAuthObject, DRM_E_TEST_UNEXPECTED_OUTPUT );
            }
            break;
            default:
                ChkDR( DRM_E_NOTIMPL );
...
**
*** Arguments:  In this order.  If an argument is indented it means that it only exists if the preceeding argument is Non-NULL
**              security level
**              Playback container
**                  Playcount
**              Copy container
...
**              KID
**              play enablers count
**                  play enablers (1 to N)
**              copy enablers count
**                  copy enablers (1 to N)
**              ND Auth object count
**                  version
**                  client data
**                  service data
**
...
DRM_RESULT DRM_CALL TestAddXMRObjectsAndFinishLicense( long argc, __in_ecount( argc ) char **argv )
{
    ...
    DRM_XMR_ND_TX_AUTH_CONTAINER                authContainer = {0};
     ...
     && argv[++iArg] != NULL )
    {
        ...
        xmrPriority.dwValue = strtoul( argv[iArg], NULL, 0 );
    ...
    */
    ...
    if( argc          > (iArg + 1)
     && argv[++iArg] != NULL )
    {
        xmrMinEnv.wMinimumSecurityLevel = (DRM_WORD)strtoul( argv[iArg], NULL, 0 );
    }
    else
    {
    ...
    */
     ...
     && argv[++iArg] != NULL )
    {
         ...
         && argv[++iArg] != NULL )
        {
            ...
            xmrPlay.PlayCount.dwValue = strtoul( argv[iArg], NULL, 0 );
        }
    ...
    */
     ...
     && argv[++iArg] != NULL )
    {
         ...
         && argv[++iArg] != NULL )
        {
            ...
            xmrCopy.CopyCount.dwValue = strtoul( argv[iArg], NULL, 0 );
        }
        ChkDR( DRM_XMR_AddObject( pXmrBuilder,
    ...
    */
    if( argc          > (iArg + 1)
     && argv[++iArg] != NULL )
    {
        ChkDR( MakeDRMString( &dstrMID, argv[iArg] ) );
...
#pragma prefast(pop)
    ...
    */
    if( argc > (++iArg) )
    {
        if( DRMCRT_strncmp( argv[iArg], "PREGENERATEDRANDOM", 18 ) == 0 && pstrRandomKeyID != NULL)
        {
            ChkDR( MakeDRMString( &dstrKID, pstrRandomKeyID ) );
        }
        else if( DRMCRT_strncmp( argv[iArg], "RANDOM", 6 ) == 0 )
        {
    ...
    */
    ...
    if( argc          > (iArg + 1)
     && argv[++iArg] != NULL )
    {
        DRM_DWORD cEnablers = (DRM_DWORD)(atol(argv[iArg]));

        ChkArg(
            ( cEnablers > 0 )
         && ( cEnablers <= DRM_MAX_ENABLERS )
         && ( (DRM_DWORD)argc > ( cEnablers + iArg ) ) );

        for( iEnabler = 0; iEnabler < cEnablers; iEnabler++ )
        {
            ChkArg( argv[++iArg] != NULL );

            ChkDR( MakeDRMString( &rgdstrDTCPGuid[iEnabler], argv[iArg] ) );
            ChkDR( DRM_UTL_StringToLittleEndianGuid(&rgdstrDTCPGuid[iEnabler], &drmidDTCP[iEnabler]) );
     ...
     && argv[++iArg] != NULL )
    {
        ...
        for( iEnabler = 0; iEnabler < cEnablers; iEnabler++ )
        {
            ...
            ChkDR( MakeDRMString( &rgdstrCSSGuid[iEnabler], argv[iArg] ) );
            ChkDR( DRM_UTL_StringToLittleEndianGuid(&rgdstrCSSGuid[iEnabler], &drmidCSS[iEnabler]) );
            ChkDR( DRM_XMR_AddUnknownObject(
    ...
    if( argc          > (iArg + 1)
     && argv[++iArg] != NULL )
    {
        DRM_DWORD cNDAuthObjects = (DRM_DWORD)(atol(argv[iArg]));

        ChkArg( ( cNDAuthObjects <= DRM_MAX_ENABLERS )
             && ( (DRM_DWORD)argc >= ( (cNDAuthObjects*3) + iArg ) ) );

        authContainer.fValid = TRUE;

        for( iEnabler = 0; iEnabler < cNDAuthObjects; iEnabler++ )
        {
            DRM_XMR_ND_TX_AUTH_OBJECT *pObject = NULL;

            ChkMem( pObject = (DRM_XMR_ND_TX_AUTH_OBJECT *)Oem_MemAlloc( SIZEOF(*pObject) ) );

            iArg++;
            pObject->fValid = TRUE;
            pObject->wVersion = (DRM_WORD)(atol(argv[iArg]));

            iArg++;
            pObject->ibDataClient = 0;
            pObject->cbDataClient = (DRM_DWORD)strlen(argv[iArg]);
            pObject->pbBufferClient = (DRM_BYTE*)argv[iArg];

            iArg++;
            pObject->ibDataService = 0;
            pObject->cbDataService = (DRM_DWORD)strlen(argv[iArg]);
            pObject->pbBufferService = (DRM_BYTE*)argv[iArg];

            pObject->pNext = authContainer.pAuthObject;
            authContainer.pAuthObject = pObject;
        }

        ChkDR( DRM_XMR_AddObject(
            pXmrBuilder, XMR_OBJECT_TYPE_ND_TX_AUTH_CONTAINER, &authContainer ) );
    }
...
ErrorExit:
    ...
    while( authContainer.pAuthObject != NULL )
    {
        DRM_XMR_ND_TX_AUTH_OBJECT *pObject = authContainer.pAuthObject;
        authContainer.pAuthObject = authContainer.pAuthObject->pNext;
        SAFE_OEM_FREE( pObject );
    }
