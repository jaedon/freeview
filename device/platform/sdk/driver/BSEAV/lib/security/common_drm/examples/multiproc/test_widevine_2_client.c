 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: test_widevine_2_client.c $
 * $brcm_Revision: 1 $
...
#if 0 /* defined but not used ... for now?! */
...
BDBG_MODULE(test_widevine_2_client);
...
int main(int argc, char* argv[])
{
	...
	bool test_flashed_bin_file = false;
	int got_char = 0;
	DRM_Widevine_Init_t inputWvInitStruct;
 
	if (argc < 2)
	{
		printf("\n\n\tMAIN - You did not specify a DRM bin file in the command line argument.\n");
		printf("\tMAIN - Continue and test app will assume a flashed DRM bin file exists? (y/n)\n\t>>");
		got_char = getchar();
		if(got_char != 'y')
		{
			printf("\nPlease specify the DRM bin file and/or CNN file to verify\n");
			printf("EXAMPLE: \n");
			printf("./nexus test_widevine_2 ABC_DRM_00000000.bin \n");
			exit(0);
		}
		else{
			test_flashed_bin_file = true;
		}
	}
	...
	if(test_flashed_bin_file == true)
	{
        inputWvInitStruct.key_file = NULL;
        printf("\n\tMAIN - NOT Calling to DRM_Init_Module....\n");
