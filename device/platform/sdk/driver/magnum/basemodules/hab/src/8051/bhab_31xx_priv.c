 ...
 *
 * $brcm_Workfile: bhab_31xx_priv.c $
 * $brcm_Revision: Hydra_Software_Devel/52 $
 * $brcm_Date: 2/29/12 4:58p $
 *
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/hab/3117/bhab_31xx_priv.c $
 * 
 * Hydra_Software_Devel/52   2/29/12 4:58p vishk
 * SW7231-691: Fixing the reset value for SH_IND_OSC_CTRL1 register.
 * 
 * Hydra_Software_Devel/50   1/4/12 11:48a vishk
 * SW7425-1860: Coverity Defect ID:35742 DEADCODE bhab_312x_priv.c
 * Product=97425
 * 
 * Hydra_Software_Devel/49   11/10/11 5:03p mward
 * SW7125-1148:  In BHAB_31xx_P_DecodeInterrupt(), check for null handler
 * functions for the receiver conditions which may not be initialized yet
 * in the (not loading AP code) scenario.
 * 
 * Hydra_Software_Devel/48   11/3/11 3:59p vishk
 * SW3461-80: Means of returning BCM31xx chip revsion and firmware
 * revision via NEXUS i/f
 * 
 * Hydra_Software_Devel/47   10/18/11 3:20p atanugul
 * SW3461-67: change write_len, read_len and command_len in
 * BHAB_SendHabCommand() from type uint8_t to uint16_t
 * 
 * Hydra_Software_Devel/46   8/5/11 5:49p mward
 * SW7125-1057:  Eliminate dead code.  Disable host interrupt in
 * BHAB_31xx_InitAp() whether or not loading AP code.
 * 
 * Hydra_Software_Devel/45   3/10/11 5:18p mward
 * SW7125-845:  Reconcile "deep power down" with BBS power-down script.
 * If not loading AP, skip operations which can cause poor performance
 * after power down without AP load.
 * 
 * Hydra_Software_Devel/44   3/3/11 6:43p mward
 * SW3128-2: Add Support for BCM3128.  Change n argument of _ReadMemory
 * and _WriteMemory to uint32_t.
 * 
 * Hydra_Software_Devel/43   2/1/11 5:35p VISHK
 ...
 * 
 * Hydra_Software_Devel/42   12/16/10 1:41p mward
 * SW7125-737:  Complete power down whether or not AP was loaded.
 * 
 * Hydra_Software_Devel/41   12/16/10 1:38p mward
 * SW7125-737:  BHAB_31xx_Close() free allocated resources in case of I2C
 * error.
 * 
 * Hydra_Software_Devel/SW7125-630/2   11/23/10 1:59p mward
 * SW7125-737:  BHAB_31xx_Close() free allocated resources in case of I2C
 * error.
 * 
 * Hydra_Software_Devel/40   12/15/10 4:39p mward
 * SW7125-737:  Reset AP and clear interrupts when NULL image indicates
 * not to load it.
 * 
 * Hydra_Software_Devel/39   12/8/10 2:55p mward
 * SW7125-737:  Annotate SH_IND register accesses in BHAB_31xx_Close() and
 * BHAB_31xx_P_ResetAp().
 * 
 * Hydra_Software_Devel/38   12/2/10 8:38p VISHK
 * SW7405-5037: To make the i2c address programmable for the slave
 * device(LNA) of 31xx frontend chip.
 * 
 * Hydra_Software_Devel/37   12/2/10 8:29p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/36   11/2/10 6:01p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/35   10/28/10 4:25p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/34   10/22/10 3:55p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/33   10/22/10 3:02p VISHK
 * SW7125-630: Fixing deadlock with CTRL-C in kernel mode
 * 
 * Hydra_Software_Devel/32   10/21/10 7:24p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/31   10/21/10 6:42p VISHK
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/31   10/21/10 6:30p vishk
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/31   10/21/10 6:29p vishk
 * SW7420-1020: Request for non-blocking call support in Nexus frontend
 * 
 * Hydra_Software_Devel/30   8/31/10 3:36p mward
 * SW7125-35:  When closing, don't reset AP if it wasn't loaded by the
 * host at start time.
 * 
 * Hydra_Software_Devel/29   8/26/10 1:47p mward
 * SW7125-597:  To avoid a resource leak, BHAB_31xx_Open() should return
 * the allocated BHAB_Handle with the error code, for clean-up in
 * BHAB_31xx_Close().  BHAB_31xx_Close() should check resources to clean
 * up partial failures.
 * 
 * Hydra_Software_Devel/28   7/2/10 4:22p vishk
 ...
 * to acquire
 * 
 * Hydra_Software_Devel/27   7/2/10 3:13p mward
 * SW7420-839: In function BHAB_31xx_SetNmiConfig() array subscript was
 * above array bounds.
 * 
 * Hydra_Software_Devel/26   6/22/10 6:01p vishk
 * SW7420-839: Add support to program AGC gain to 31xx TNR PI
 * 
 * Hydra_Software_Devel/25   6/21/10 11:04a mward
 * SW7125-35:  Add an option to init AP without re-loading AP image.
 * Increase buf[] in BHAB_31xx_SetNmiConfig.  Free the pImpl when closing
 * to avoid leak.
 * 
 * Hydra_Software_Devel/24   3/4/10 3:24p VISHK
 * SW7405-3978: bcm3117: BHAB_31xx_InitAp() has retry logic error
 * 
 * Hydra_Software_Devel/23   1/21/10 12:31p VISHK
 * SW7405-3820: BCM3109 reports poor BER at a specific range of signal
 * power
 * 
 * Hydra_Software_Devel/22   11/18/09 11:50a VISHK
 * SW7405-3356: 3117 tuner wil get HAB timeout on Motorola DCX700 by plug
 * and unplug RF feed back and force, it's been test against
 * REFSW_MOT_31XX_20091105 label
 * 
 * Hydra_Software_Devel/21   11/5/09 2:00p VISHK
 * SW7405-3210: BADS_31xx_GetStatus not returning values for
 * correctedCount, uncorrectedCount and berRawCount
 * 
 * Hydra_Software_Devel/20   10/14/09 1:48p mward
 * SW7405-2851: Compiler warning:  Added "const" to BHAB_NmiSettings and
 * BHAB_WatchDogTimerSettings.
 * 
 * Hydra_Software_Devel/19   10/5/09 10:46a vishk
 * SW7405-2851: 31xx: Add support to configure the Watchdog feature of
 * 31xx.
 * 
 * Hydra_Software_Devel/18   8/27/09 12:45p vishk
 * SW7405-2939: i2c communication issue (bhab error in
 * BHAB_31xx_P_ResetAp)
 * 
 * Hydra_Software_Devel/17   8/26/09 10:52a vishk
 * SW7405-2700: 3117 does not report lock status to the app.
 * 
 * Hydra_Software_Devel/16   7/13/09 2:26p vishk
 * PR 56618: 3117 does not report lock status to the app.
 * 
 * Hydra_Software_Devel/15   7/8/09 10:55a vishk
 * PR 56618: 3117 does not report lock status to the app.
 * 
 * Hydra_Software_Devel/14   7/7/09 1:44a vishk
 * PR 56618: 3117 does not report lock status to the app.
 * 
 * Hydra_Software_Devel/13   6/2/09 1:40p vishk
 * PR 53208: 3117 AP does not initialize on firs attemp consistently
 * 
 * Hydra_Software_Devel/12   6/1/09 3:30p vishk
 * PR 53208: 3117 AP does not initialize on firs attemp consistently
 * 
 * Hydra_Software_Devel/11   5/12/09 10:53a vishk
 * PR 46431: Add support for BCM3117, BCM3114, and BCM3112
 * 
 * Hydra_Software_Devel/10   4/16/09 11:28a vishk
 * PR 46431: Add support for BCM3117, BCM3114, and BCM3112
 * 
 * Hydra_Software_Devel/9   3/13/09 11:18a vishk
 * PR 46431: Add support for BCM3117, BCM3114, and BCM3112
 * 
 * Hydra_Software_Devel/8   3/12/09 4:36p vishk
 * PR 46431: Add support for BCM3117, BCM3114, and BCM3112
 * 
 * Hydra_Software_Devel/7   3/10/09 1:51p vishk
 * PR 46431: Add support for BCM3117, BCM3114, and BCM3112
 ***************************************************************************/
...
)
{
    ...
    if (pHexImage)
    {
        ...
        BHAB_CHK_RETCODE(BHAB_31xx_P_ResetAp(handle));
...
/******************************************************************************
 BHAB_31xx_P_ResetAp()
******************************************************************************/
BERR_Code BHAB_31xx_P_ResetAp(BHAB_Handle handle)
{   
...
{  
    ...
    {   
        ...
        if (callback && callback->func && (h_fstat[2] & DEVICE(STAT3_GET_GAIN)))
        {
        ...
        if (callback && callback->func && (h_fstat[2] & DEVICE(STAT3_QAM_STATUS_READY)))
        {
        ...
        if (callback && callback->func && (h_fstat[2] & DEVICE(STAT3_NO_SIGNAL)))
        {
        ...
        if(callback && callback->func && (h_fstat[1] & DEVICE(STAT2_LOCK_MASK))) {
            new_stat2 |= DEVICE(STAT2_LOCK_MASK);
            callback->parm2 = (int) BHAB_Interrupt_eLockChange;
            h_ie[1] &= ~DEVICE(STAT2_LOCK_MASK);
            BDBG_MSG(("Lock Change"));
            BKNI_EnterCriticalSection();
            callback->func(callback->pParm1, callback->parm2);
            BKNI_LeaveCriticalSection();
        }
        ...
        else if (h_fstat[1] & DEVICE(STAT2_OUT_OF_LOCK))
        {
