 ...
 *
 * $brcm_Workfile: bhab_4517_priv.c $
 * $brcm_Revision: Hydra_Software_Devel/11 $
 * $brcm_Date: 10/23/12 10:47a $
 *
 * [File Description:]
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/hab/src/leapsatfe/4517/bhab_4517_priv.c $
 * 
 * Hydra_Software_Devel/11   10/23/12 10:47a enavarro
 * SWSATFE-220: version info struct changed
 * 
 * Hydra_Software_Devel/10   10/19/12 10:19a ronchan
 * SW3461-274: changes to bfec structure
 * 
 * Hydra_Software_Devel/9   10/16/12 9:18a enavarro
 * SWSATFE-220: add BHAB_GetVersionInfo()
 * 
 * Hydra_Software_Devel/8   7/17/12 4:28p enavarro
 * SWSATFE-140: fixed chip version in BHAB_4517_P_GetApVersion()
 * 
 * Hydra_Software_Devel/7   3/6/12 3:35p enavarro
 * SWSATFE-140: fixed bug in BHAB_4517_P_UninstallInterruptCallback()
 * 
 * Hydra_Software_Devel/6   2/9/12 11:23a enavarro
 * SWSATFE-140: changed BHAB_4517_P_WriteRbus()
 * 
 * Hydra_Software_Devel/5   12/14/11 3:42p enavarro
 * SWSATFE-140: fixed compiler warnings
 * 
 * Hydra_Software_Devel/4   11/3/11 9:48a enavarro
 * SWSATFE-140: implemented VERSION command
 * 
 * Hydra_Software_Devel/3   10/25/11 11:16a enavarro
 * SWSATFE-140: fixed compiler errors
 * 
 * Hydra_Software_Devel/2   10/24/11 11:37a enavarro
 * SWSATFE-140: ongoing development
 * 
...
BERR_Code BHAB_4517_P_Close(BHAB_Handle handle)
{
   ...
   /* reset the LEAP */
   retCode = BHAB_4517_P_ResetAp(handle);
...
)
{
   ...
   {      
      ...
      BHAB_CHK_RETCODE(BHAB_4517_P_ResetAp(h));
      ...
      if (BHAB_4517_P_WaitForEvent(h, pImpl->hInitDoneEvent, 1000) != BERR_SUCCESS)
      {
         BDBG_ERR(("AP initialization timeout\n")); 
         BERR_TRACE(retCode = BHAB_ERR_AP_NOT_INIT);  
         goto done;
...
/******************************************************************************
 BHAB_4517_P_ResetAp()
******************************************************************************/
BERR_Code BHAB_4517_P_ResetAp(BHAB_Handle h)
{
   BERR_Code retCode;
   uint32_t ctrl, val32;
   uint8_t status, buf;
