 ...
 *
 * $brcm_Workfile: bhab_4528_priv.c $
 * $brcm_Revision: Hydra_Software_Devel/12 $
 * $brcm_Date: 12/14/11 3:18p $
 *
 * [File Description:]
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/hab/4528/bhab_4528_priv.c $
 * 
 * Hydra_Software_Devel/12   12/14/11 3:18p enavarro
 * SWSATFE-86: removed unused variable
 * 
 * Hydra_Software_Devel/11   12/1/11 4:13p mphillip
 * SW7425-1304: Change 4528 to HAB
 * 
 * Hydra_Software_Devel/10   11/3/11 10:28a enavarro
 * SWSATFE-86: changed GET_VERSION HAB command
 * 
 * Hydra_Software_Devel/9   10/18/11 4:04p enavarro
 * SWSATFE-86: BHAB_SendHabCommand() input parameter data type changed
 * 
 * Hydra_Software_Devel/8   9/21/11 5:10p enavarro
 * SWSATFE-86: fixed bug with variable length HAB commands
 * 
 * Hydra_Software_Devel/7   9/20/11 4:47p enavarro
 * SWSATFE-86: fixed bug in BHAB_4528_P_InstallInterruptCallback()
 * 
 * Hydra_Software_Devel/6   9/14/11 4:23p enavarro
 * SWSATFE-86: add support for flash memory read
 * 
 * Hydra_Software_Devel/5   9/13/11 2:49p enavarro
 * SWSATFE-86: added boot mode 1 support
 * 
 * Hydra_Software_Devel/4   9/13/11 11:19a enavarro
 * SW7425-883: added ability to access IRAM/DRAM memory when LEAP is
 * running
 * 
 * Hydra_Software_Devel/3   9/12/11 3:58p enavarro
 * SW7425-883: calculate checksum in HAB
 * 
 * Hydra_Software_Devel/2   9/7/11 3:18p enavarro
 * SW7425-883: added BHAB_4528_P_DisableFrontendInterrupts()
 * 
 * Hydra_Software_Devel/1   8/11/11 4:04p enavarro
...
BERR_Code BHAB_4528_P_Close(BHAB_Handle handle)
{
   ...
   if (retCode == BERR_SUCCESS)
   {
      ...
      if ((retCode == BERR_SUCCESS) && (strap == 0))
         retCode = BHAB_4528_P_ResetAp(handle);
   }
...
)
{
   ...
   {      
      ...
      BHAB_CHK_RETCODE(BHAB_4528_P_ResetAp(h));
...
/******************************************************************************
 BHAB_4528_P_ResetAp()
******************************************************************************/
BERR_Code BHAB_4528_P_ResetAp(BHAB_Handle h)
{
   BERR_Code retCode;
   uint32_t ctrl, val32;
   uint8_t status, buf;
