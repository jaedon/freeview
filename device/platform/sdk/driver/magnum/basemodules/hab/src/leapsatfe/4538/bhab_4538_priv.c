 ...
 *
 * $brcm_Workfile: bhab_4538_priv.c $
 * $brcm_Revision: Hydra_Software_Devel/9 $
 * $brcm_Date: 10/23/12 11:34a $
 *
 * [File Description:]
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/hab/src/leapsatfe/4538/bhab_4538_priv.c $
 * 
 * Hydra_Software_Devel/9   10/23/12 11:34a enavarro
 * SWSATFE-174: removed GET_VERSION HAB command
 * 
 * Hydra_Software_Devel/8   10/19/12 10:15a ronchan
 * SW3461-274: changes to bfec structure
 * 
 * Hydra_Software_Devel/7   10/15/12 2:11p enavarro
 * SWSATFE-220: implemented BHAB_GetVersionInfo()
 * 
 * Hydra_Software_Devel/6   10/5/12 10:25a enavarro
 * SWSATFE-174: removed the hab write verify code
 * 
 * Hydra_Software_Devel/5   9/25/12 5:15p enavarro
 * SWSATFE-174: add retries in case of i2c errors
 * 
 * Hydra_Software_Devel/4   9/25/12 2:24p enavarro
 * SWSATFE-174: extend AP initialization timeout for boot mode 1
 * 
 * Hydra_Software_Devel/3   7/23/12 1:52p enavarro
 * SWSATFE-174: ongoing development
 * 
 * Hydra_Software_Devel/2   7/13/12 1:58p enavarro
 * SWSATFE-174: added boot mode1 support; updated parameter checking in
 * some of the functions; update hab error handling
 * 
 * Hydra_Software_Devel/1   7/11/12 4:03p enavarro
...
BERR_Code BHAB_4538_P_DecodeHabError(BHAB_Handle h);
...
)
{
   ...
   h4538Dev->callbackInfo.func = NULL;
   h4538Dev->callbackInfo.callbackParams.pParm1 = NULL;
   h4538Dev->callbackInfo.callbackParams.parm2 = (int)NULL;
...
BERR_Code BHAB_4538_P_Close(BHAB_Handle handle)
{
   ...
   /* reset the LEAP */
   retCode = BHAB_4538_P_ResetAp(handle);
...
)
{
   ...
   BHAB_CHK_RETCODE(BHAB_4538_P_ResetAp(h));
   ...
   {      
      ...
      if (BHAB_4538_P_WaitForEvent(h, pImpl->hInitDoneEvent, 1000) != BERR_SUCCESS)
      {
         BDBG_ERR(("AP initialization timeout\n")); 
         BERR_TRACE(retCode = BHAB_ERR_AP_NOT_INIT);  
         goto done;
...
)
{
   ...
   p4538->callbackInfo.func = fCallBack;
   p4538->callbackInfo.callbackParams.pParm1 = pParm1;
   p4538->callbackInfo.callbackParams.parm2 = parm2;
   return BERR_SUCCESS;
}
...
)
{
   ...
   p4538->callbackInfo.func = NULL;
   p4538->callbackInfo.callbackParams.pParm1 = NULL;
   p4538->callbackInfo.callbackParams.parm2 = 0;
   return BERR_SUCCESS;
}   
...
/******************************************************************************
 BHAB_4538_P_ResetAp()
******************************************************************************/
BERR_Code BHAB_4538_P_ResetAp(BHAB_Handle h)
{
   BERR_Code retCode;
...
{
   return BREG_I2C_Write(((BHAB_4538_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
}
...
{
   return BREG_I2C_Read(((BHAB_4538_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
}
...
)
{
   ...
   if (ack_byte != read_buf[0])
   {
      retCode = BHAB_4538_P_DecodeHabError(h);
      if (retCode == BERR_SUCCESS)
      {
         BDBG_ERR(("HAB command not serviced, ack_byte_expected=0x%02X, read=0x%02X\n", ack_byte, read_buf[0]));
...
BERR_Code BHAB_4538_P_DecodeInterrupt(BHAB_Handle h)
{
   ...
   if ((fstatus1 != 0) || (fstatus0 & ~(BHAB_4538_HIRQ0_INIT_DONE | BHAB_4538_HIRQ0_HAB_DONE | BHAB_4538_HIRQ0_HAB_ERROR | BHAB_4538_HIRQ0_FW_ERROR)))
   {
...
/******************************************************************************
 BHAB_4538_P_DecodeHabError() - called when the command was not ack'd
******************************************************************************/
BERR_Code BHAB_4538_P_DecodeHabError(BHAB_Handle h)
{
   BERR_Code retCode = BERR_SUCCESS;
