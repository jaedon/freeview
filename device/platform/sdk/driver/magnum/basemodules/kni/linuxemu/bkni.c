 ...
 * $brcm_Date: 8/27/12 3:57p $
 *
 * Module Description:
 *
 * Implementatation of the Magnum KNI for IKOS-based Linux applications.
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/kni/linuxemu/bkni.c $ * 
 * 
 * Hydra_Software_Devel/6   8/27/12 3:57p katrep
 * SW7445-8:add more tagged routines
 * 
 * Hydra_Software_Devel/5   10/26/10 1:00p hongtaoz
 * SW7425-10: fixed infinite wat for event;
 * 
 * Hydra_Software_Devel/4   10/12/10 9:47a hongtaoz
 * SW7425-10: increased time scale for the timeout event;
 * 
 * Hydra_Software_Devel/3   9/28/10 3:25p hongtaoz
 * SW7425-10: reduce the timer scale for linuxemu;
 * 
 * Hydra_Software_Devel/2   3/10/10 2:58p mward
 * SW7400-2712:  Add annotation to indicate to Coverity that BKNI_Fail()
 * is a "kill path".
 * 
 * Hydra_Software_Devel/4   4/28/09 3:04p jessem
 * PR 54380: Changed all IKOS references to EMU instead.
 * 
 * Hydra_Software_Devel/3   4/20/09 1:21p jessem
 * PR 54380: Updated BKNI functions to add tagged versions and malloc
 * tracking features.
 * 
 * Hydra_Software_Devel/2   4/9/08 11:06a mward
 * PR41378: Use tv_nsec >= 1000000000, NPTL pthread_cond_timedwait()
 * rejects = 1000000000.
 * 
 * Hydra_Software_Devel/1   10/16/07 9:41a jessem
 * PR 36129: Added IKOS ticktime  factor to functions that have have time
 * elements.
 * 
 ***************************************************************************/
