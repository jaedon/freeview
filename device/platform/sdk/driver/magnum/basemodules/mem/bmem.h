 ...
 *
 * $brcm_Workfile: bmem.h $
 * $brcm_Revision: Hydra_Software_Devel/31 $
 * $brcm_Date: 7/24/12 6:26p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/basemodules/mem/bmem.h $
 * 
 * Hydra_Software_Devel/31   7/24/12 6:26p jtna
 * SW7425-3537: add #define BMEM_FreeCached BMEM_Heap_FreeCached
 * 
 * Hydra_Software_Devel/30   8/24/11 5:35p albertl
 * SW7405-5349:  Added line parameter to memory monitor interface.
 * 
 * Hydra_Software_Devel/28   8/23/11 3:33p albertl
 * SW7405-5349: Added line parameter to memory monitor interface.
 * 
 * Hydra_Software_Devel/27   1/10/11 3:31p albertl
 * SW7408-193: BMEM_Heapinfo now includes high watermark.  Added
 * BMEM_Heap_ResetHighWatermark().
 * 
 * Hydra_Software_Devel/26   4/8/10 1:34p erickson
 * SW7550-370: specify units for all alignment params
 *
 * Hydra_Software_Devel/25   2/25/10 5:01p albertl
 * SW7325-655: Added BMEM_Heap_FreeCached.
 *
 * Hydra_Software_Devel/24   12/3/08 5:17p albertl
 * PR48035: Added ulTotalFree and ulTotalAllocated to BMEM_HeapInfo.
 *
 * Hydra_Software_Devel/23   10/24/08 5:43p albertl
 * PR48199, PR48046: Removed BMEM_SetCache entirely as it is obsolete.
 *
 * Hydra_Software_Devel/22   6/16/06 3:22p albertl
 ...
 *
 * Hydra_Software_Devel/21   2/8/06 7:29p hongtaoz
 * PR19082: BMEM_Alloc returns NULL pointer if size is zero;
 *
 * Hydra_Software_Devel/20   10/7/05 3:53p jasonh
 * PR 17374: Allowed GetHeapInfo to return original creation parameters.
 *
 * Hydra_Software_Devel/19   3/10/05 5:05p albertl
 ...
 *
 * Hydra_Software_Devel/18   11/2/04 5:27p pntruong
 * PR13076: Added cached memory support for rul.
 *
 * Hydra_Software_Devel/17   11/2/04 10:32a hongtaoz
 * PR13076: added cached memory support;
 *
 * Hydra_Software_Devel/16   10/8/04 3:27p yuxiaz
 * PR9076: Change isr functions to _isr.
 *
 * Hydra_Software_Devel/15   4/9/04 12:48p vsilyaev
 * PR 10201: Changed description for the memory monitor interface
 *
 * Hydra_Software_Devel/14   4/1/04 12:49p vsilyaev
 * PR 10201: Added memory monitor
 *
 * Hydra_Software_Devel/13   2/25/04 2:52p hongtaoz
 ...
 *
 * Hydra_Software_Devel/12   12/31/03 1:02p jasonh
 * PR 8796: Added documentation to clarify cached/uncached memory.
 *
 * Hydra_Software_Devel/11   12/31/03 12:02p jasonh
 * PR 8940: Changed return type of destroyheap to void. Removed use of
 * DEBUG macro and printf.
 *
 * Hydra_Software_Devel/10   10/6/03 11:39a jasonh
 * Documentation update to clarify creation arguments.
 *
 * Hydra_Software_Devel/9   9/15/03 5:19p jasonh
 * Fixed offset argument. Changed report functions to return void.
 *
 * Hydra_Software_Devel/8   9/5/03 2:48p jasonh
 * Added documentation for function arguments.
 *
 * Hydra_Software_Devel/7   9/5/03 2:01p jasonh
 * Added documentation. Added GetHeapInfo and GetLargestAvailableBlockSize
 * prototypes.
 *
 * Hydra_Software_Devel/6   9/2/03 1:52p vadim
 * Conforms to magnum.
 *
 * Hydra_Software_Devel/3   3/21/03 6:04p erickson
 * edited comments
 *
...
typedef struct BMEM_HeapInfo
{
	...
	size_t        ulSmallestFree;       /* Size of the smallest free block
	                                       in bytes */
	size_t        ulTotalFree;          /* Total free memory in heap.  This
										   is the combined size of all free
										   blocks.  Just because a certain
										   amount of memory is free does not
										   mean that amount can be allocated.
										   Allocated memory must still fit
										   within a block, but this provides
										   information on the overall usage of
										   memory in a system. */
	...
	size_t        ulSmallestAllocated;  /* Size of the smallest allocated
	                                       block in bytes */
	size_t        ulTotalAllocated;     /* Total allocated memory in heap */
	size_t        ulHighWatermark;      /* Most memory used at one time */
	unsigned int  ulNumAllocated;       /* Number of allocated blocks */
...
{
    unsigned int        uiAlignment;    /* enforced byte alignment on allocated memory (default 0) (power of 2 units) */
...
Input:
	Heap - Heap to allocate from.
	Size - The number of bytes to allocate.
	Alignment - Required alignment of the returned memory (power of 2).
...
/***************************************************************************
Summary:
	Free cached memory allocated from heap.

Description:
	This function flushes allocated cached memory then frees it.

	If the supplied address is NULL, it will return an error code.
...
Returns:
	BERR_SUCCESS - Memory was returned to the heap.
	BERR_INVALID_PARAMETER - Heap address or cached address was invalid.
...
See Also:
	...
	BMEM_Heap_AllocAligned
****************************************************************************/
BERR_Code BMEM_Heap_FreeCached
(
	BMEM_Heap_Handle  Heap,           /* Heap from which the block was allocated. */
...
Description:
	The heap tracks the high memory watermark (the most amount of memory
	used at one time) when BMEM_Heap_GetInfo() is called.  This function
	resets that value back to zero so it can track a new maximum.
...
typedef struct BMEM_MonitorInterface
{
	...
	void (*alloc)(void *cnxt, uint32_t addr, size_t size, const char *fname, int line); /* callback function called when new block was allocated */
...
#define BMEM_FreeCached BMEM_Heap_FreeCached
