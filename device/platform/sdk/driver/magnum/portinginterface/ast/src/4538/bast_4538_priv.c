 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/ast/src/4538/bast_4538_priv.c $
 * 
 * Hydra_Software_Devel/6   10/15/12 2:13p enavarro
 * SWSATFE-220: implemented BAST_GetVersionInfo()
 * 
 * Hydra_Software_Devel/5   9/25/12 3:28p enavarro
 * SWSATFE-174: updated BAST_4538_P_GetAdcSelect() and
 * BAST_4538_P_SetAdcSelect()
 * 
 * Hydra_Software_Devel/4   9/25/12 2:25p enavarro
 * SWSATFE-174: fixed bug in BAST_4538_P_GetAdcSelect()
 * 
...
)
{
   ...
   BDBG_ASSERT(chnNo < BAST_4538_MAX_CHANNELS);
...
)
{
   ...
   BAST_4538_CHK_RETCODE(BHAB_GetApVersion(pImpl->hHab, &familyId, &chipId, &chipVer, &majVer, &minVer));
...
BERR_Code BAST_4538_P_InterruptCallback(void *pParm1, int parm2)
{
   ...
   if (fstatus1)
   {
      for (channel = 0; channel < BAST_4538_MAX_CHANNELS; channel++)
      {
...
)
{
   ...
   BAST_4538_CHK_RETCODE(BHAB_SendHabCommand(pDevImpl->hHab, buf, 8, buf, 8, true, true, 8));
...
******************************************************************************/
BERR_Code BAST_4538_P_WriteAgc(
   BAST_ChannelHandle h,  /* [in] BAST handle */
   BAST_Agc agcSelect,    /* [in] AGC select */
...
******************************************************************************/
BERR_Code BAST_4538_P_FreezeAgc(
   BAST_ChannelHandle h,  /* [in] BAST handle */
   BAST_Agc agcSelect,    /* [in] AGC select */
   bool bFreeze           /* [in] true = freeze, false = unfreeze */
)
...
/******************************************************************************
 BAST_4538_P_SetAdcSelect()
******************************************************************************/
BERR_Code BAST_4538_P_SetAdcSelect(BAST_ChannelHandle h, BAST_AdcSelect s)
{
   BAST_4538_P_Handle *pDevImpl = (BAST_4538_P_Handle *)(h->pDevice->pImpl);
   BERR_Code retCode;
   uint8_t buf[8];

   if (s > BAST_AdcSelect_e3)
      return (BERR_TRACE(BERR_INVALID_PARAMETER));
...
/******************************************************************************
 BAST_4538_P_GetAdcSelect()
******************************************************************************/
BERR_Code BAST_4538_P_GetAdcSelect(BAST_ChannelHandle h, BAST_AdcSelect *p)
{
   BAST_4538_P_Handle *pDevImpl = (BAST_4538_P_Handle *)(h->pDevice->pImpl);
   BERR_Code retCode;
   uint8_t buf[8];
