...
{                                        /*   all but 7401A0*/
    BCMD_XptSecKeySlot_eType0,      /*  10 entries per slot*/
    BCMD_XptSecKeySlot_eType1,     /* 12 entries per slot*/
    BCMD_XptSecKeySlot_eType2,     /*  14 entries per slot */
    BCMD_XptSecKeySlot_eType3,  	/*  16 entries per slot*/
    BCMD_XptSecKeySlot_eType4,  /*  16 entries  per slot*/
    BCMD_XptSecKeySlot_eType5,  /*  20 entries  per slot*/
    BCMD_XptSecKeySlot_eType6,   /*  26 entries  per slot*/
...
/* key ladder key source/destination enum used for key ladder. non-root key generation from key3 - key5, 

Refer to <<Access Control Table & Key Management>> for permitted key source/destination selection. 
which only permits a subset of the entire definitions.   A brief of ACL Tables:
 1) root key generation (Key3):  			swizzle types, swizzle key sources, algorithms,  key destinations
 2) regular key generation (key4, Key5):  	key data(i.e. procin, seed), key sources/destinations, algorithms
 3) challenge and response:		swizzle inpuit data, swizzle key sources, algorithm
 4) CA :						key sources, operation algorithms
 5) M2M :					key sources, operation algorithms
 6) HDMI/HDCP key source: 		key sources
 7) Initial Vector (IV):		 	key sources	
 8) REMUX: 		 			key source, operation algorithms
 9) Proc-out:		 			key sources
 10) user commands:  	many be ACL controlled for RSA, DSA, DH, DES_AES, SHA1HMAC
 11) user commands: 	may be ACL controlled for OTP_Read (no Prog), MSP_Read/Prog, DataSection_Read/Prog
 
 If users/applications are using HSM for any of the above features, please be aware that ACL control may be exising.
 */
...
/* #if (BCHP_CHIP==7440) &&  (BCHP_VER >= BCHP_VER_B1)   it should be added, yet for build convenience, comment out  */
  BCMD_KeyRamBuf_eSecretKey_Km_1   = 0x8f, 
  BCMD_KeyRamBuf_eSecretKey_Kvu_1  = 0x90,  
/* #endif  */

  BCMD_KeyRamBuf_eSecretKey_Kpriv  = 0x91, 

  BCMD_KeyRamBuf_eSecretKey_End, 			/* NOTE : End ID should NOT be used (Modular code :) */
  BCMD_KeyRamBuf_eSecretKey_Invalid  = BCMD_KeyRamBuf_eSecretKey_End,  /* Second 1024 bit user ram*/
  BCMD_KeyRamBuf_eInvalid_DVD, 

  /********************  END of DVD Enums (Secret Keys) *************************/
   
  /* Used to route keys without copying key to some KeySrc */
  BCMD_KeyRamBuf_eTempKeySrc      = 0xa0,  /* Internal Use Only */

  BCMD_KeyRamBuf_eLastIfNeeded    = 0xa1   /* LAST*/
...
typedef enum BCMD_KeyDestBlockType_e
{
    ...
    BCMD_KeyDestBlockType_eInvalid
...
/* #if defined (MS_DRM)    put this back when makefile is ready. 3/23/07*/
...
/*#endif  */	
    ...
    BCMD_M2MSecCryptoAlg_eMaxNumber   /* never assign eMax with hardcoding */
