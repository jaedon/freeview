 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/7425/b0/common/bhsm.c $
 * 
 ...
 * 
 * Hydra_Software_Devel/5   9/15/11 4:59p atruong
 * SW7346-381: Coverity Defect 32704 fix: Possible NULL pointer
 * dereferencing
 * 
 * Hydra_Software_Devel/4   6/29/11 8:23p atruong
...
)
{
	...
	*outp_handle = NULL;
	...
	if ( moduleHandle->currentSettings.hHeap  != NULL )
	{
		moduleHandle->hHeap = moduleHandle->currentSettings.hHeap;		
		moduleHandle->pContiguousMem= BMEM_AllocAligned( moduleHandle->currentSettings.hHeap, BHSM_CONTIGUOUS_MEMORY_SIZE, 6,0 ); /* 64 bytes allignement*/
		...
		moduleHandle->shaContext.bufferSizeMax = BHSM_SHA_BUFFER_SIZE;
		moduleHandle->shaContext.pBuffer = BMEM_AllocAligned( moduleHandle->currentSettings.hHeap, BHSM_SHA_BUFFER_SIZE, 6,0 );
		BDBG_ASSERT( moduleHandle->shaContext.pBuffer != NULL );
		BHSM_P_CHECK_ERR_CODE_CONDITION(errCode, BHSM_STATUS_MEMORY_PHYCOVERTING_ERR,
				BMEM_ConvertAddressToOffset( moduleHandle->hHeap, 
							moduleHandle->shaContext.pBuffer, 
							&moduleHandle->shaContext.shaBufferOffset));
		moduleHandle->shaContext.bufferSize = 0;
		moduleHandle->shaContext.bMultiple = false;
	}
...
)
{
	...
	if (inout_handle->shaContext.pBuffer)
	{
		BMEM_Free (inout_handle->hHeap, (void*)inout_handle->shaContext.pBuffer);
	}
	if (inout_handle->pContiguousMem)
	{
		BMEM_Free (inout_handle->hHeap, (void*)inout_handle->pContiguousMem);
	}
