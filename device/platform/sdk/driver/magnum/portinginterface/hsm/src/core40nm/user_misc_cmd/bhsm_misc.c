 ...
 * $brcm_Date: 12/4/12 8:12p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/src/core40nm/user_misc_cmd/bhsm_misc.c $
 * 
 * Hydra_Software_Devel/5   12/4/12 8:12p atruong
 * SW7584-13: HSM PI support for 7584A0 - restore conditional Zeus code
 * 
 * Hydra_Software_Devel/4   11/19/12 11:11a atruong
 * SW7584-46: HSM PI/Nexus Security support for 7584A0 - Also SW7435-232
 * 
 * Hydra_Software_Devel/3   10/31/12 5:53p yili
 * SW7425-4156:Bug fix
 * 
 * Hydra_Software_Devel/2   2/15/12 1:03p atruong
 * SWSECURITY-109: HMAC-SHA command support for 40-nm platforms
 * 
 * Hydra_Software_Devel/1   10/12/11 8:35p atruong
 * SW7425-1251: HSM PI Support For BCM7425B0
 * 
 * Hydra_Software_Devel/2   7/6/11 9:03p yili
 * SW7231-272:Bug fix
 * 
...
)
{
	...
	commandData.cmdId = BCMD_cmdType_eOFFLINE_SetMiscBits_COMMAND;
#else
...
#if HSM_IS_ASKM_40NM_ZEUS_2_0
	commandData.unInputParamsBuf[unParamLen/4 ] = 
			(inoutp_startAVDIO->virtualKeyLadderID & 0xFF) | 
			((inoutp_startAVDIO->keyLayer & 0xFF) << 8      );
	unParamLen += 4;
#endif
