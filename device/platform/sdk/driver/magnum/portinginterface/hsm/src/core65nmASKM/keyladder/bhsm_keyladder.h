 ...
 * $brcm_Date: 12/9/09 4:31p $
 *
 * Module Description: 
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/7420/b0/keyladder/bhsm_keyladder.h $
 * 
 ...
 * 
 * Hydra_Software_Devel/15   4/23/08 11:12a qxie
 * PR41995: ICAM:add support to 7405
 * 
 * Hydra_Software_Devel/14   4/22/08 6:37p qxie
 * PR41995: ICAM:add support to 7405
 * 
 * Hydra_Software_Devel/13   2/26/08 11:00a yili
 * PR39632:Add M2M timestamp handling to HSM
 * 
 * Hydra_Software_Devel/13   2/15/08 6:12p yili
 * PR39632:Add M2M timestamp handling to HSM
 * 
 * Hydra_Software_Devel/12   10/25/07 12:33p lyang
 * PR36043: Modify makefile and split the code to allow HSM to build
 * without source covered by Security SLA
 * 
 * Hydra_Software_Devel/11   9/26/07 2:53p lyang
 * PR 34706: add support for 7400 D0 to portinginterface/hsm
 * 
 * Hydra_Software_Devel/10   8/31/07 5:36p lyang
 * PR 34552: HSM: add the support M2M external key mode, external IV, MSC
 * length selection
 * 
 * Hydra_Software_Devel/9   8/13/07 5:47p lyang
 * PR 33325: add 7118B0 support and link
 * 
 * Hydra_Software_Devel/8   12/1/06 10:40p lyang
 * PR 25188: HSM, add function header, data struct header description for
 * those public APIs
 * 
 * Hydra_Software_Devel/7   10/27/06 11:36a lyang
 ...
 * 
 * Hydra_Software_Devel/5   10/16/06 5:31p lyang
 * PR 24938: HSM: Added enum for AESCounterKeyMode ( moved from
 * bsp_s_keycommon.h to this *.h, avoiding too many check-out/-in
 * 
 * Hydra_Software_Devel/4   10/13/06 5:25p lyang
 ...
 * 
 * Hydra_Software_Devel/3   9/26/06 3:10p btan
 ...
 * 
 * Hydra_Software_Devel/1   1/20/06 5:37p btan
 * PR 19140: Do not encrypt generic mode files.
 * 
 * Hydra_Software_Devel/4   1/17/06 10:29p btan
 * PR 19140: Do not encrypt generic mode files.
 * 
 *
 ***************************************************************************/
...
typedef struct BHSM_InitKeySlotIO {
	...
	uint32_t			unKeySlotType5Num; /* In: how many key slots are required for type5   */
	uint32_t			unKeySlotType6Num; /* In: how many key slots are required for type6   */
	uint32_t			unKeySlotType7Num; /* In: how many key slots are required for type7   */


	
	uint32_t			unStatus;  			/* Out: 0 for success, otherwise failed */		
...
of different number of entries as follow:
	...
	Key slot type 7 always has 26 entries.
...
typedef struct BHSM_ConfigPidKeyPointerTableIO {
	/* In:  which PID channcel is to be associated with an allocated key slot, 0~127   */	
	unsigned int					unPidChannel;

	/* In:   slot type of the allocated key slot , 0~7 at max depending on keyslot  initializaiton*/
...
typedef struct BHSM_LoadRouteUserKeyIO {
	/* In:  load into which BSP KeyRAM buffer, a subset of BCMD_KeyRamBuf_e can be used
	           BCMD_KeyRamBuf_eFirstRam~BCMD_KeyRamBuf_eThirdRam for a key up to 192 bits
	           BCMD_KeyRamBuf_eFirstRam2048~BCMD_KeyRamBuf_eThirdRam2048 for a key up t0 2048bits
	*/
	BCMD_KeyRamBuf_e			keySource;
	
	/* In: the key size, either 64/128/192 bits as in BCMD_KeySize_e or 192~2048bits  */
	BCMD_KeySize_u				keySize;

	/* In: the actual key to be loaded, big endian order, starting from the 1st byte, padding at bottom */
	unsigned char 				aucKeyData[BCMD_USERKEY_KEYRAM2048_SIZE]; 
...
typedef enum BHSM_CaModeShift {
	...
	/* Module Key Mode Shift */

#if ((BCHP_CHIP==7420) && (BCHP_VER >= BCHP_VER_B0)) || (BCHP_CHIP == 7340) || \
		(BCHP_CHIP == 7342) || (BCHP_CHIP == 7125) || (BCHP_CHIP == 7468)
	BHSM_CaModeShift_eKeyOffset                = 0,
	BHSM_CaModeShift_eCryptoAlgorithmShift     = 5,
	BHSM_CaModeShift_eCipherModeShift          = 10,  /* 3 bit, refer to BCMD_CipherModeSelect_e */
	BHSM_CaModeShift_eDVBScrambleLevelShift    = 10,  /* 3 bit, refer to BCMD_CipherModeSelect_e */		
	BHSM_CaModeShift_eTerminationModeShift     = 13,  /* 2 bits, refer to  BHSM_ResidueMode_e */
	BHSM_CaModeShift_eIVModeShift              = 16, 
	BHSM_CaModeShift_eSolitaryTermShift        = 18,
	BHSM_CaModeShift_eMscSelShift              = 21, 
	BHSM_CaModeShift_eRestrEnShift             = 22,
	BHSM_CaModeShift_eGlobalEnShift            = 23,
	BHSM_CaModeShift_eRestrScModShift          = 24,
	BHSM_CaModeShift_eGlobalScModShift         = 25,	
	BHSM_CaModeShift_eScValueShift             = 26,  /* 2 bits for this field*/
#else
	BHSM_CaModeShift_eCryptoAlgorithmShift     = 0,
	BHSM_CaModeShift_eResidueModeShift         = 13,  /* 2 bits, refer to  BHSM_ResidueMode_e */
	BHSM_CaModeShift_eDVBScrambleLevelShift    = 13,  /* 1 bit, refer to BHSM_DVBScrambleLevel_e */		
	/* BHSM_CaModeShift_eScModShift = 15, */
	BHSM_CaModeShift_eRestrEnShift             = 16,
	BHSM_CaModeShift_eGlobalEnShift            = 17,
	BHSM_CaModeShift_eRestrScModShift          = 18,
	BHSM_CaModeShift_eGlobalScModShift         = 19,	
	BHSM_CaModeShift_eMscSelShift              = 20, 
	BHSM_CaModeShift_eScValueShift             = 21,	/* 2 bit for this field*/
	BHSM_CaModeShift_eSynEnableShift           = 23,

#endif
	BHSM_CaModeShift_eCAMask                   = 0xFFFFFFF,	 /* in total there are 28 bits defined now */

	/* Global Key Mode Shift */
	BHSM_CaModeShift_eEncScPolarityShift       = 0,    /* Rev A0, A1 */
	BHSM_CaModeShift_eMulti2KeySelectShift     = 1,
	BHSM_CaModeShift_eAtscScrambleCtrlShift    = 3,
	BHSM_CaModeShift_eAtscScModShift           = 4,	
	BHSM_CaModeShift_eGlobalDropPktCtrlShift   = 5,
	BHSM_CaModeShift_eRestrictDropPktCtrlShift = 6,	
	BHSM_CaModeShift_eGlobalRegOverwriteShift  = 7,
	BHSM_CaModeShift_eEncryptBeforeRaveShift   = 8,
	BHSM_CaModeShift_eCPDDisableShift          = 8,	   /* Rev A0, A1 chips */
	BHSM_CaModeShift_eCPSDisableShift          = 9     /* Rev A0, A1 chips */
...
typedef enum BHSM_M2mModeShift {
	...
	BHSM_M2mModeShift_eUseExtKey              = 0,
	BHSM_M2mModeShift_eUseExtIV               = 1,
	...
	BHSM_M2mModeShift_eKeyOffset              = 0,
	BHSM_M2mModeShift_eCryptoAlgorithmShift   = 5,
	BHSM_M2mModeShift_eCipherModeSel          = 10,  /* 3 bit field */
	BHSM_M2mModeShift_eAESCounterKeyModeShift = 13,
	BHSM_M2mModeShift_eIVModeSel              = 16,
	BHSM_M2mModeShift_eSolitaryTermSel        = 18,  
	BHSM_M2mModeShift_eMscCtrlSel             = 21,
	BHSM_M2mModeShift_eAuthCtrlShift          = 22,  /* Encryption Mode */
	...
	BHSM_M2mModeShift_eAESCounterKeyModeShift = 13,  
	BHSM_M2mModeShift_eAuthCtrlShift = 16,
	BHSM_M2mModeShift_eMscCtrlSel = 18,
	BHSM_M2mModeShift_eUseExtKey = 19,
	BHSM_M2mModeShift_eUseExtIV = 20,
	BHSM_M2mModeShift_eEnableTimeStamp = 25,
...
typedef struct BHSM_CACryptoAlg {
	...
	BCMD_CipherModeSelect_e			cipherDVBMode; 
...
#else
	/* In: select the algorith among BCMD_XptSecCryptoAlg_e */
	BCMD_XptSecCryptoAlg_e			caSecAlg;	

	/* In:	set termination mode for non-DVB algorithms or scrambling level for DVB, see BHSM_ResidueMode_u */
	BHSM_ResidueMode_u				residueMode; 
...
#if (BCHP_CHIP == 7420)  && (BCHP_VER < BCHP_VER_B0)
	bool							bEncScPolarity;  /* In: future feature enhancements */
	bool							bCPDDisable;	/* In: future feature enhancements */
	bool							bCPSDisable;	/* In: future feature enhancements */
	bool							bSynEnable;    /* enable Synamedia AES */
...
#if (BCHP_CHIP == 7420)  && (BCHP_VER < BCHP_VER_B0)
	BCMD_M2MSecCryptoAlg_e			m2mSecAlg; 
	/* In: used in two ways. 
...
typedef struct BHSM_ConfigAlgorithmIO {
	...
	/* In: the key source selection among a subset of BCMD_KeyRamBuf_e valued between 0~11, i.e.
	    BCMD_KeyRamBuf_eFirstRam ~ BCMD_KeyRamBuf_eKey5KeyLadder3 */
	BCMD_KeyRamBuf_e			keySource;  			
...
typedef struct BHSM_ConfigMulti2IO {
	...
	/* In: which system key to configure, HW specific, so far only 0~3 is legal */
...
#if   BHSM_SECURE_PI_SUPPORT     /*  to hide certain PIs that need special PGP keys */
