 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/3548/b3/common/bhsm_misc.c $
 * 
 * Hydra_Software_Devel/3   11/6/09 5:40p atruong
 * SW3548-2595: HSM PI Support for BCM93548-B4
 * 
 * Hydra_Software_Devel/2   11/5/09 5:38p atruong
...
)
{
	...
	commandData.cmdId      = 0x19;         /* BCMD_cmdType_eOFFLINE_OTP_READ */
	commandData.unContMode = 0;
	
	/* only fill the command specific input parameters */
	commandData.unInputParamsBuf[0] = 0x01;   /* subCommand ID 0x01 - OTP_KEY_ID_READ */
	...
	/* DWORD is 32-bit  OTP Key ID */
	/* output buffer is    [byte3][byte2][byte1][byte0] */

	for (i = 0, j = 1; i < BHSM_OTP_ID_LEN; j--, i += 4) {
...
BERR_Code	BHSM_EnableSDRAMEncryption(
		BHSM_Handle		in_handle
)
{
	...
	BDBG_MSG(("Inside BHSM_EnableSDRAMEncryption\n"));
	BDBG_ENTER(BHSM_EnableSDRAMEncryption);
	BDBG_ASSERT( in_handle );
	BHSM_P_CHECK_ERR_CODE_CONDITION( errCode, BHSM_STATUS_FAILED, 
		(in_handle->ulMagicNumber != BHSM_P_HANDLE_MAGIC_NUMBER ) );	
	...
	commandData.cmdId      = BCMD_cmdType_eUSER_ENABLE_SDRAM_ENCRYPTION; 
	commandData.unContMode = 0;
...
BHSM_P_DONE_LABEL:
	...
	BDBG_LEAVE(BHSM_EnableSDRAMEncryption);
