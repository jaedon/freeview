 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/7440/b0/dvd_cmds/bhsm_dvd_cmds.c $
 * 
...
)
{
	...
	commandData.unInputParamsBuf[(BCMD_UserLoadKeyAfterOp_CmdInputField_eReserved13/4 - BCMD_HEADER_BYTE_SIZE/4)] = 
			inoutp_loadKeyAfterOpIO->unReserved13;		/* 13 */
...
)
{
	...
	commandData.unInputParamsBuf[(BCMD_UserLoadSecretKey_CmdInputField_eSlotNum/4 - BCMD_HEADER_BYTE_SIZE/4)] = 
			inoutp_loadSecretKeyIO->unKeySlot;   /* 7 */
	...
	/* returned fields are ended now, Check output paramter length */
	BHSM_P_CHECK_ERR_CODE_CONDITION( errCode, BHSM_STATUS_PARM_LEN_ERR, 
		(unParamLen != commandData.unOutputParamLen) );	
