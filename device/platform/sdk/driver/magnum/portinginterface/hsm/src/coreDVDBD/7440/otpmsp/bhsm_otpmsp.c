 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/7440/c0/otpmsp/bhsm_otpmsp.c $
 * 
...
#if (BCHP_CHIP==7400)||(BCHP_CHIP==7401) ||(BCHP_CHIP==7118) ||(BCHP_CHIP==7403) ||(BCHP_CHIP==7405) ||(BCHP_CHIP==7325) ||(BCHP_CHIP==7335)
	BHSM_ChannelHandle 	channelHandle = in_handle->channelHandles[BHSM_HwModule_eCmdInterface2];
...
)
{
...
#if (BCHP_CHIP==7400)||(BCHP_CHIP==7401) ||(BCHP_CHIP==7118) ||(BCHP_CHIP==7403) ||(BCHP_CHIP==7405) ||(BCHP_CHIP==7325) ||(BCHP_CHIP==7335)||(BCHP_CHIP==7440) 
	BHSM_ChannelHandle 	channelHandle = in_handle->channelHandles[BHSM_HwModule_eCmdInterface2];
	uint32_t 				unParamLen = 0, i,j;
	BHSM_P_CommandData_t commandData;	
#endif
	...
	/*  need this urgently on 12/11/06, for STB only */
#if (BCHP_CHIP==7400)||(BCHP_CHIP==7401) ||(BCHP_CHIP==7118) ||(BCHP_CHIP==7403) ||(BCHP_CHIP==7405) ||(BCHP_CHIP==7325) ||(BCHP_CHIP==7335)||(BCHP_CHIP==7440) 
	...
	/* Check output paramter length, status code */
	BHSM_P_CHECK_ERR_CODE_CONDITION( errCode, BHSM_STATUS_PARM_LEN_ERR, 
		(unParamLen != commandData.unOutputParamLen) );	
...
#else	
	/* BSTD_UNUSED(inoutp_readDataSectIO   );   Removed this when we start using this variable*/
#endif
...
#else	
	/* BSTD_UNUSED(BHSM_ProgramDataSect   );   Removed this when we start using this variable*/
#endif	
