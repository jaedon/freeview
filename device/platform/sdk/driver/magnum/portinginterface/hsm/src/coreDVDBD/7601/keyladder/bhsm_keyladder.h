 ...
 * $brcm_Date: 1/13/09 12:45p $
 *
 * Module Description: 
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/hsm/7601/a0/keyladder/bhsm_keyladder.h $
 * 
 * Hydra_Software_Devel/2   1/13/09 12:45p atruong
 * PR49290: HSM PI Support for 7601B0
 * 
 * Hydra_Software_Devel/1   8/12/08 6:02p atruong
 * PR45631: HSM PI support for BCM7601 platform
 * 
 * Hydra_Software_Devel/2   8/13/07 5:52p lyang
...
typedef struct BHSM_ConfigPidKeyPointerTableIO {
	...
	/* In:   slot type of the allocated key slot , 0~6 at max depending on keyslot  initializaiton*/
...
typedef struct BHSM_GenerateRouteKeyIO {
	...
	/* In: for key4 & key5 generation; no meaning for key 3 generation, only to select from a subset of
	    BCMD_KeyRamBuf_e, the value is among (3,4, 6,7,9,10)
	     i.e. 	BCMD_KeyRamBuf_eKey3KeyLadder1,BCMD_KeyRamBuf_eKey4KeyLadder1 
	     		BCMD_KeyRamBuf_eKey3KeyLadder1,BCMD_KeyRamBuf_eKey4KeyLadder2 
	     		BCMD_KeyRamBuf_eKey3KeyLadder1,BCMD_KeyRamBuf_eKey4KeyLadder3 
	*/
	BCMD_KeyRamBuf_e			keySrc;  		
	
	/* In: for key 3, key4, key5 generation of Keyladder 1, 2 3, only to select from a subset of
	    BCMD_KeyRamBuf_e, the value is 3 through 11
	     i.e. BCMD_KeyRamBuf_eKey3KeyLadder1 ~BCMD_KeyRamBuf_eKey5KeyLadder3
	*/
...
typedef struct BHSM_ProcOutCmdIO {
	...
	/* In: which key is used to decrypt the input data or key, select from only a subset of 
	BCMD_KeyRamBuf_e, i.e. those key5 of keyladder1/2/3, the value is 5 or 8 or 11
		BCMD_KeyRamBuf_eKey5KeyLadder1,
		BCMD_KeyRamBuf_eKey5KeyLadder2,
		BCMD_KeyRamBuf_eKey5KeyLadder3
	*/
	BCMD_KeyRamBuf_e		keyId;
...
typedef struct BHSM_InvalidateKeyIO {
	...
	/* In: valid only if to invalidate KeyRAM or Both, select from a subset of BCMD_KeyRamBuf_e, 0 to 11, 
		 i.e. BCMD_KeyRamBuf_eFirstRam ~ BCMD_KeyRamBuf_eThirdRam
	  		BCMD_KeyRamBuf_eKey3KeyLadder1 ~ BCMD_KeyRamBuf_eKey5KeyLadder3
	*/
	BCMD_KeyRamBuf_e			keySrc;		
...
typedef enum BHSM_CaModeShift {
	...
	BHSM_CaModeShift_eCAMask = 0xFFFFF,

	
	BHSM_CaModeShift_eEncScPolarityShift = 0,
	...
	BHSM_CaModeShift_eCPDDisableShift = 8,	
	BHSM_CaModeShift_eCPSDisableShift = 9
...
typedef struct BHSM_CACryptoAlg {
	...
	bool							bRestrictEnable;  /* In: future feature enhancements */
	bool							bGlobalEnable;    /* In: future feature enhancements */  
	bool							bRestrictScMod;  /* In: future feature enhancements */
	bool							bGlobalScMod;    /* In: used to clear scrambling control bits */		
	
	bool							bEncScPolarity;  /* In: future feature enhancements */
	unsigned char					ucMulti2KeySelect;/* In: Multi2 system key destination */
	bool							bAtscScrambleCtrl;/* In: future feature enhancements */
	bool							bAtscMod;	/* In: future feature enhancements */
	bool							bGlobalDropPktCtrl;/* In: future feature enhancements */
	bool							bRestrictDropPktCtrl;	/* In: future feature enhancements */
	bool							bGlobalRegOverwrite; /* In: future feature enhancements */
