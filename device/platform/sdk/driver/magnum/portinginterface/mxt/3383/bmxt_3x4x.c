...
#define HUMAX_BMXT_TSMF_P_REG_OFFSET (BCHP_DEMOD_XPT_FE_TSMF1_CTRL - BCHP_DEMOD_XPT_FE_TSMF0_CTRL)
#endif
...
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_DEMOD_TSMF)
/*
        pConfig->ParserTSMF_SEL = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_MINI_PID_PARSER0_CTRL1, PARSER_TSMF_SEL);
        pConfig->ParserTSMF_EN = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_MINI_PID_PARSER0_CTRL1, PARSER_TSMF_EN) ? true : false;
*/
		if (handle->settings.chip!=BMXT_Chip_e3383) {
			...
			pConfig->ParserTSMF_SEL = ((Reg & TSMF_SEL_MASK) >> TSMF_SEL_SHIFT);
			pConfig->ParserTSMF_EN = ((Reg & TSMF_EN_MASK) >> TSMF_EN_SHIFT) ? true : false;
		}

	tsmfNum = pConfig->ParserTSMF_SEL;
	RegOffset = HUMAX_BMXT_TSMF_P_REG_OFFSET * tsmfNum;

	Reg = BMXT_RegRead32(handle, BCHP_DEMOD_XPT_FE_TSMF0_CTRL + RegOffset);
	pConfig->TSMF_InputSel = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_TSMF0_CTRL, TSMF_INPUT_SEL);
        pConfig->TSMF_AutoEN = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_TSMF0_CTRL, TSMF_AUTO_EN) ? true : false;
        pConfig->TSMF_EN = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_TSMF0_CTRL, TSMF_ENABLE) ? true : false;
	pConfig->TSMF_RelativeTsNum = BCHP_GET_FIELD_DATA(Reg, DEMOD_XPT_FE_TSMF0_CTRL, TSMF_REL_TS_NO);
#endif
    }
