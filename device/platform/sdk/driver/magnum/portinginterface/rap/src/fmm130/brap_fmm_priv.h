...
* Revision History:
*
* $brcm_Log: /magnum/portinginterface/rap/base/brap_fmm_priv.h $
* 
...
#define	BRAP_FMM_P_MAX_EXT_MCLK		1
#else
...
#if 0 /* now RBUF memory is allocated on RBUF open. therefore dont need to store
         start addresses here */
    void *                 pRBufStartAddr[BRAP_RM_P_MAX_RBUFS]; /* Start pointer
                                            for internally allocated Rbufs */
#endif
...
#else
    BRAP_MIXER_P_Handle    hMixer[BRAP_RM_P_MAX_MIXERS];  /* Mixer handles */
#endif

#ifndef BCHP_7411_VER
#if ( BRAP_DVD_FAMILY == 1 ) 
    BRAP_ExtMClkSettings sExtMClkSettings[BRAP_FMM_P_MAX_EXT_MCLK];
#else
