...
BERR_Code BSAT_7366_P_Open(BSAT_Handle *h, BCHP_Handle hChip, void *pReg, BINT_Handle hInterrupt, const BSAT_Settings *pSettings)
{
   ...
   BDBG_ASSERT(hDev);
   hImplDev = (BSAT_7366_P_Handle *)BKNI_Malloc(sizeof(BSAT_7366_P_Handle));
   BDBG_ASSERT(hImplDev);
   hDev->pImpl = (void*)hImplDev;
   hImplDev->hHab = (BHAB_Handle)pReg;
   hImplDev->hChip = (BCHP_Handle)hChip;

   retCode = BSAT_7366_P_GetTotalChannels(hDev, &numChannels);
   ...
   BHAB_InstallInterruptCallback(hHab, BHAB_DevId_eSAT, BSAT_7366_P_InterruptCallback, (void*)hDev, 0);
...
{
   BSAT_7366_P_Handle *pDevImpl = (BSAT_7366_P_Handle *)(h->pImpl);
   uint32_t mask;
   BERR_Code retCode;

   BDBG_ASSERT(h);

   mask = BHAB_7366_HIRQ0_SAT_MASK;
...
{
   BSAT_7366_P_Handle *pImpl = (BSAT_7366_P_Handle *)(h->pImpl);
   BERR_Code retCode;
   uint32_t product_id, strap0;

#if (BHAB_CHIP==4548)
   BSAT_7366_CHK_RETCODE(BHAB_ReadRegister(pImpl->hHab, BCHP_SUN_TOP_CTRL_PRODUCT_ID, &product_id));
...
#endif
   BSAT_7366_CHK_RETCODE(BHAB_ReadRegister(pImpl->hHab, BCHP_SUN_TOP_CTRL_STRAP_VALUE_0, &strap0));
   ...
   if ((product_id == 0x7399) || ((product_id == 0) && ((strap0 & 0x1) == 0)))   /* TBD product ID workaround for bringup*/
      ...
      *totalChannels = BSAT_7366_MAX_CHANNELS;
   else
      *totalChannels = BSAT_7366_MAX_CHANNELS;
...
BERR_Code BSAT_7366_P_OpenChannel(BSAT_Handle h, BSAT_ChannelHandle *pChannelHandle, uint32_t chan, const BSAT_ChannelSettings *pSettings)
{
   ...
   BSAT_7366_P_ChannelHandle *chImpl;

   BDBG_ENTER(BSAT_7366_P_OpenChannel);
   BDBG_ASSERT(h);
   BDBG_ASSERT(h->totalChannels <= BSAT_7366_MAX_CHANNELS);
   BDBG_ASSERT(chan < h->totalChannels);

   if (pSettings == NULL)
      BSAT_7366_GetChannelDefaultSettings(h, chan, &cs);
   else
      cs = *pSettings;
   ...
   chImpl = (BSAT_7366_P_ChannelHandle *)BKNI_Malloc(sizeof(BSAT_7366_P_ChannelHandle));
   BDBG_ASSERT(chImpl);
   ...
   BDBG_LEAVE(BSAT_7366_P_OpenChannel);
   return retCode;
}
...
BERR_Code BSAT_7366_P_CloseChannel(BSAT_ChannelHandle h)
{
   ...
   BDBG_ENTER(BSAT_7366_P_CloseChannel);

   chImpl = (BSAT_7366_P_ChannelHandle *)(h->pImpl);
   BKNI_DestroyEvent(chImpl->hLockChangeEvent);
   BKNI_DestroyEvent(chImpl->hAcqDoneEvent);
   BKNI_DestroyEvent(chImpl->hSignalNotificationEvent);
   ...
   BDBG_LEAVE(BSAT_7366_P_CloseChannel);
   return BERR_SUCCESS;
}
...
******************************************************************************/
BERR_Code BSAT_7366_P_GetVersionInfo(BSAT_Handle h, BFEC_VersionInfo *pVersionInfo)
{
   BERR_Code retCode;
   uint32_t hab[6];
   BSAT_7366_P_Handle *pImpl = (BSAT_7366_P_Handle *)(h->pImpl);

   BDBG_ENTER(BSAT_7366_P_GetVersionInfo);

   hab[0] = BHAB_7366_InitHeader(0x2B, 0, 0, BHAB_7366_MODULE_SAT);
   hab[1] = hab[2] = hab[3] = hab[4] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 6));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetVersionInfo);
   return retCode;
}
...
{
   #define BSAT_RESET_TIMEOUT 1000
   BERR_Code retCode;
   uint32_t hab[2], irq_mask, i, n;
   BSAT_7366_P_Handle *pImpl = (BSAT_7366_P_Handle *)(h->pImpl);

   BDBG_ENTER(BSAT_7366_P_Reset);

   BSAT_7366_CHK_RETCODE(BSAT_7366_P_GetTotalChannels(h, &n));
   ...
   for (i = 0; i < n; i++)
   {
      if (h->pChannels[i])
      {
         BSAT_7366_CHK_RETCODE(BSAT_7366_P_EnableChannelInterrupt(h->pChannels[i], false, BHAB_7366_CHAN_IRQ_ALL));
      }
   }

   /* enable SAT reset done interrupt */
   irq_mask = BHAB_7366_HIRQ0_SAT_INIT_DONE;
   BSAT_7366_CHK_RETCODE(BHAB_WriteRegister(pImpl->hHab, BCHP_LEAP_HOST_L2_CLEAR0, &irq_mask));
   BSAT_7366_CHK_RETCODE(BHAB_WriteRegister(pImpl->hHab, BCHP_LEAP_HOST_L2_MASK_CLEAR0, &irq_mask));

   hab[0] = BHAB_7366_InitHeader(0x09, 0, 0, BHAB_7366_MODULE_SAT);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 2));

   done:
   BDBG_LEAVE(BSAT_7366_P_Reset);
   return retCode;
}
...
BERR_Code BSAT_7366_P_PowerDownChannel(BSAT_ChannelHandle h)
{
   ...
   BDBG_ENTER(BSAT_7366_P_PowerDownChannel);

   BSAT_7366_CHK_RETCODE(BSAT_7366_P_EnableChannelInterrupt(h, true, BHAB_7366_CHAN_IRQ_READY));

   hab[0] = BHAB_7366_InitHeader(0x25, h->channel, 0, BHAB_7366_MODULE_SAT);
   hab[1] = 1; /* power down */
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_PowerDownChannel);
   return retCode;
}
...
BERR_Code BSAT_7366_P_PowerUpChannel(BSAT_ChannelHandle h)
{
   ...
   BDBG_ENTER(BSAT_7366_P_PowerUpChannel);
   ...
   BDBG_LEAVE(BSAT_7366_P_PowerUpChannel);
   return retCode;
}
...
******************************************************************************/
BERR_Code BSAT_7366_P_IsChannelOn(BSAT_ChannelHandle h, bool *pbOn)
{
   BERR_Code retCode;
   uint32_t hab[3];
   BSAT_7366_P_Handle *pImpl = (BSAT_7366_P_Handle *)(h->pDevice->pImpl);

   BDBG_ENTER(BSAT_7366_P_IsChannelOn);

   hab[0] = BHAB_7366_InitHeader(0x32, h->channel, 0, BHAB_7366_MODULE_SAT);
   hab[1] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 3));
   ...
   *pbOn = (hab[1] == 1) ? true : false;
   ...
   BDBG_LEAVE(BSAT_7366_P_IsChannelOn);
   return retCode;
}
...
BERR_Code BSAT_7366_P_Acquire(BSAT_ChannelHandle h, BSAT_AcqSettings *pParams)
{
   ...
   uint32_t hab[7];
   BSAT_7366_P_Handle *pImpl = (BSAT_7366_P_Handle *)(h->pDevice->pImpl);
   BSAT_7366_P_ChannelHandle *hChn = (BSAT_7366_P_ChannelHandle *)(h->pImpl);

   BDBG_ENTER(BSAT_7366_P_Acquire);

   if (pParams->mode > BSAT_Mode_eBlindScan)
   {
      BDBG_WRN(("BSAT_7366_P_Acquire(): invalid mode (0x%X)", pParams->mode));
   ...
   {
      BDBG_WRN(("BSAT_7366_P_Acquire(): unsupported mode (0x%X)", pParams->mode));
   ...
   {
      BDBG_WRN(("BSAT_7366_P_Acquire(): invalid symbol rate (%d)", pParams->symbolRate));
      goto invalid_parameter;
   }

   if ((pParams->freq < 200000000UL) || (pParams->freq > 2451000000UL))
   {
      BDBG_WRN(("BSAT_7366_P_Acquire(): invalid frequency (%d)", pParams->freq));
      goto invalid_parameter;
   }

   if (pParams->adcSelect > 3)
   {
      BDBG_WRN(("BSAT_7366_P_Acquire(): invalid adcSelect (%d)", pParams->adcSelect));
      invalid_parameter:
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }

   BSAT_7366_CHK_RETCODE(BSAT_7366_P_EnableChannelInterrupt(h, true, BHAB_7366_CHAN_IRQ_ACQ));

   hab[0] = BHAB_7366_InitHeader(0x0F, h->channel, 0, 0);
   hab[1] = pParams->mode;
   ...
   BDBG_LEAVE(BSAT_7366_P_Acquire);
   return retCode;
}
...
{
   BSAT_7366_P_Handle *pDevImpl = (BSAT_7366_P_Handle *)(h->pDevice->pImpl);
   BERR_Code retCode;
   uint32_t spare0, mask;

   BDBG_ENTER(BSAT_7366_P_GetLockStatus);

   mask = 1 << h->channel;
   BSAT_7366_CHK_RETCODE(BHAB_ReadRegister(pDevImpl->hHab,  BCHP_LEAP_CTRL_SW_SPARE0, &spare0));
   mask = 1 << h->channel;
   if (spare0 & mask)
      *pbLocked = true;
   else
   ...
   BDBG_LEAVE(BSAT_7366_P_GetLockStatus);
   return retCode;
}
...
{
   BSAT_7366_P_Handle *pDevImpl = (BSAT_7366_P_Handle *)(h->pDevice->pImpl);
   BSAT_7366_P_ChannelHandle *pImpl = (BSAT_7366_P_ChannelHandle *)(h->pImpl);
   BERR_Code retCode;
   BSAT_Mode mode = pImpl->lastMode;
   bool bRetry = false;
   uint32_t hab[29];

   BDBG_ENTER(BSAT_7366_P_GetChannelStatus);
   if (BSAT_MODE_IS_DVBS2(mode))
   {
      /* get dvb-s2 status */
      get_dvbs2_status:
      hab[0] = BHAB_7366_InitHeader(0x11, h->channel, 0, 0);
      BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 29));
   ...
   else if (BSAT_MODE_IS_TURBO(mode))
   {
      /* get turbo status */
      get_turbo_status:
      hab[0] = BHAB_7366_InitHeader(0x12, h->channel, 0, 0);
      BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 29));
      mode = (BSAT_Mode)hab[7];
      if (!bRetry)
      {
   ...
   else
   {
      /* get qpsk status */
      get_qpsk_status:
      hab[0] = BHAB_7366_InitHeader(0x10, h->channel, 0, 0);
      BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 27));
      mode = (BSAT_Mode)hab[7];
      if (!bRetry)
      {
         bRetry = true;
   ...
   BDBG_LEAVE(BSAT_7366_P_GetChannelStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_ResetChannelStatus(BSAT_ChannelHandle h)
{
   ...
   BDBG_ENTER(BSAT_7366_P_ResetChannelStatus);

   hab[0] = BHAB_7366_InitHeader(0x15, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 2));

   done:
   BDBG_LEAVE(BSAT_7366_P_ResetChannelStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetSoftDecisions(BSAT_ChannelHandle h, uint32_t n, int16_t *pI, int16_t *pQ)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetSoftDecisions);

   if ((n < 1) || (n > 125))
   {
      BDBG_WRN(("BSAT_7366_P_GetSoftDecisions(): invalid number of I/Q pairs specified (%d)", n));
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }

   hab[0] = BHAB_7366_InitHeader(0x18, h->channel, 0, 0);
   hab[1] = n;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 3+n));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetSoftDecisions);
   return retCode;
}
...
BERR_Code BSAT_7366_P_ResetChannel(BSAT_ChannelHandle h, bool bDisableDemod)
{
   ...
   BDBG_ENTER(BSAT_7366_P_ResetChannel);

   BSAT_7366_CHK_RETCODE(BSAT_7366_P_EnableChannelInterrupt(h, false, BHAB_7366_CHAN_IRQ_ALL));

   hab[0] = BHAB_7366_InitHeader(0x14, h->channel, 0, 0);
   hab[1] = bDisableDemod ? 1 : 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_ResetChannel);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetBertSettings(BSAT_ChannelHandle h, BSAT_BertSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetBertSettings);

   hab[0] = BHAB_7366_InitHeader(0x1B, h->channel, BHAB_7366_WRITE, 0);
   hab[1] = pSettings->bInvert ? 1 : 0;
   if (pSettings->mode == BSAT_BertMode_ePRBS15)
      hab[1] |= 0x2;
   else if (pSettings->mode == BSAT_BertMode_ePRBS31)
      hab[1] |= 0x4;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetBertSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetBertSettings(BSAT_ChannelHandle h, BSAT_BertSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetBertSettings);

   hab[0] = BHAB_7366_InitHeader(0x1B, h->channel, BHAB_7366_READ, 0);
   hab[1] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetBertSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetBertStatus(BSAT_ChannelHandle h, BSAT_BertStatus *pStatus)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetBertStatus);

   hab[0] = BHAB_7366_InitHeader(0x1A, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 7));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetBertStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetSearchRange(BSAT_ChannelHandle h, uint32_t searchRange)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetSearchRange);

   if ((searchRange < 1000000) || (searchRange > 15000000))
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x1E, h->channel, BHAB_7366_WRITE, 0);
   hab[1] = searchRange;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetSearchRange);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetSearchRange(BSAT_ChannelHandle h, uint32_t *pSearchRange)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetSearchRange);

   hab[0] = BHAB_7366_InitHeader(0x1E, h->channel, BHAB_7366_READ, 0);
   hab[1] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));
   ...
   *pSearchRange = hab[1];
   ...
   BDBG_LEAVE(BSAT_7366_P_GetSearchRange);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetAmcScramblingSeq(BSAT_ChannelHandle h, BSAT_AmcScramblingSeq *pSeq)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetAmcScramblingSeq);

   hab[0] = BHAB_7366_InitHeader(0x13, h->channel, 0, 0);
   hab[1] = pSeq->xseed;
   hab[2] = pSeq->plhdrscr1;
   hab[3] = pSeq->plhdrscr2;
   hab[4] = pSeq->plhdrscr3;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 6));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetAmcScramblingSeq);
   return retCode;
}
#endif
...
BERR_Code BSAT_7366_P_SetNetworkSpec(BSAT_Handle h, BSAT_NetworkSpec networkSpec)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetNetworkSpec);

   if (networkSpec > BSAT_NetworkSpec_eEuro)
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x1D, 0, BHAB_7366_WRITE, 0);
   hab[1] = (uint32_t)networkSpec;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetNetworkSpec);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetNetworkSpec(BSAT_Handle h, BSAT_NetworkSpec *pNetworkSpec)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetNetworkSpec);

   hab[0] = BHAB_7366_InitHeader(0x1D, 0, BHAB_7366_READ, 0);
   hab[1] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));
   ...
   *pNetworkSpec = (BSAT_NetworkSpec)hab[1];
   ...
   BDBG_LEAVE(BSAT_7366_P_GetNetworkSpec);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetOutputTransportSettings(BSAT_ChannelHandle h, BSAT_OutputTransportSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetOutputTransportSettings);

   hab[0] = BHAB_7366_InitHeader(0x1C, h->channel, BHAB_7366_WRITE, 0);
   ...
   BDBG_LEAVE(BSAT_7366_P_SetOutputTransportSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetOutputTransportSettings(BSAT_ChannelHandle h, BSAT_OutputTransportSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetOutputTransportSettings);

   hab[0] = BHAB_7366_InitHeader(0x1C, h->channel, BHAB_7366_READ, 0);
   hab[1] = 0;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetOutputTransportSettings);
   return retCode;
}
...
{
   BDBG_ENTER(BSAT_7366_P_GetInitDoneEventHandle);

   *hEvent = ((BSAT_7366_P_Handle *)(h->pImpl))->hInitDoneEvent;

   BDBG_LEAVE(BSAT_7366_P_GetInitDoneEventHandle);
   return BERR_SUCCESS;
}
...
{
   BDBG_ENTER(BSAT_7366_P_GetLockStateChangeEventHandle);

   *hEvent = ((BSAT_7366_P_ChannelHandle *)(h->pImpl))->hLockChangeEvent;

   BDBG_LEAVE(BSAT_7366_P_GetLockStateChangeEventHandle);
   return BERR_SUCCESS;
}
...
{
   BDBG_ENTER(BSAT_7366_P_GetAcqDoneEventHandle);

   *hEvent = ((BSAT_7366_P_ChannelHandle *)(h->pImpl))->hAcqDoneEvent;

   BDBG_LEAVE(BSAT_7366_P_GetAcqDoneEventHandle);
   return BERR_SUCCESS;
}
...
{
   BDBG_ENTER(BSAT_7366_P_GetSignalNotificationEventHandle);

   *hEvent = ((BSAT_7366_P_ChannelHandle *)(h->pImpl))->hSignalNotificationEvent;

   BDBG_LEAVE(BSAT_7366_P_GetSignalNotificationEventHandle);
   return BERR_SUCCESS;
}
...
{
   BDBG_ENTER(BSAT_7366_P_GetReadyEventHandle);

   *hEvent = ((BSAT_7366_P_ChannelHandle *)(h->pImpl))->hReadyEvent;

   BDBG_LEAVE(BSAT_7366_P_GetReadyEventHandle);
   return BERR_SUCCESS;
}
...
BERR_Code BSAT_7366_P_StartToneDetect(BSAT_ChannelHandle h, uint32_t freq, uint32_t adcSelect)
{
   ...
   BDBG_ENTER(BSAT_7366_P_StartToneDetect);

   if ((freq < 250000000UL) || (freq > 2300000000UL))
   {
      BDBG_WRN(("BSAT_7366_P_StartToneDetect(): invalid frequency (%d)", freq));
      goto invalid_parameter;
   }

   if (adcSelect > 3)
   {
      BDBG_WRN(("BSAT_7366_P_StartToneDetect(): invalid adcSelect (%d)", adcSelect));
      invalid_parameter:
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }

   BSAT_7366_CHK_RETCODE(BSAT_7366_P_EnableChannelInterrupt(h, true, BHAB_7366_CHAN_IRQ_ACQ_DONE));

   hab[0] = BHAB_7366_InitHeader(0x29, h->channel, 0, 0);
   hab[1] = freq;
   hab[2] = adcSelect;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 4));

   hChn->lastMode = BSAT_Mode_eUnknown;

   done:
   BDBG_LEAVE(BSAT_7366_P_StartToneDetect);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetToneDetectStatus(BSAT_ChannelHandle h, BSAT_ToneDetectStatus *pStatus)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetToneDetectStatus);

   hab[0] = BHAB_7366_InitHeader(0x2A, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 7));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetToneDetectStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_StartSymbolRateScan(BSAT_ChannelHandle h, uint32_t freq, uint32_t minSymbolRate, uint32_t maxSymbolRate, uint32_t adcSelect)
{
   ...
   BDBG_ENTER(BSAT_7366_P_StartSymbolRateScan);

   if ((minSymbolRate < 1000000) || (minSymbolRate > 45000000))
   {
      BDBG_WRN(("BSAT_7366_P_StartSymbolRateScan(): invalid symbol rate (%d)", minSymbolRate));
      goto invalid_parameter;
   }

   if ((maxSymbolRate < 1000000) || (maxSymbolRate > 45000000))
   {
      BDBG_WRN(("BSAT_7366_P_StartSymbolRateScan(): invalid symbol rate (%d)", maxSymbolRate));
      goto invalid_parameter;
   }

   if (maxSymbolRate <= minSymbolRate)
   {
      BDBG_WRN(("BSAT_7366_P_StartSymbolRateScan(): maxSymbolRate should not be <= minSymbolRate"));
      goto invalid_parameter;
   }

   if ((freq < 250000000UL) || (freq > 2300000000UL))
   {
      BDBG_WRN(("BSAT_7366_P_StartSymbolRateScan(): invalid frequency (%d)", freq));
      goto invalid_parameter;
   }

   if (adcSelect > 3)
   {
      BDBG_WRN(("BSAT_7366_P_StartSymbolRateScan(): invalid adcSelect (%d)", adcSelect));
      invalid_parameter:
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }
   ...
   hab[0] = BHAB_7366_InitHeader(0x2C, h->channel, 0, 0);
   hab[1] = freq;
   hab[2] = minSymbolRate;
   hab[3] = maxSymbolRate;
   hab[4] = adcSelect;
   ...
   BDBG_LEAVE(BSAT_7366_P_StartSymbolRateScan);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetSymbolRateScanStatus(BSAT_ChannelHandle h, BSAT_SymbolRateScanStatus *pStatus)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetSymbolRateScanStatus);

   hab[0] = BHAB_7366_InitHeader(0x2D, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 9));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetSymbolRateScanStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetConfig(BSAT_Handle h, uint32_t addr, uint32_t val)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetConfig);

   if (addr >= BSAT_7366_CONFIG_MAX)
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x05, 0, BHAB_7366_WRITE, BHAB_7366_MODULE_LEAP);
   hab[1] = addr;
   hab[2] = val;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetConfig);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetConfig(BSAT_Handle h, uint32_t addr, uint32_t *pVal)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetConfig);

   if (addr >= BSAT_7366_CONFIG_MAX)
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x05, 0, BHAB_7366_READ, BHAB_7366_MODULE_LEAP);
   hab[1] = addr;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));
   ...
   *pVal = hab[2];
   ...
   BDBG_LEAVE(BSAT_7366_P_GetConfig);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetChannelConfig(BSAT_ChannelHandle h, uint32_t addr, uint32_t val)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetChannelConfig);

   if (addr >= BSAT_7366_CHAN_CONFIG_MAX)
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x05, h->channel, BHAB_7366_WRITE, BHAB_7366_MODULE_SAT);
   hab[1] = addr;
   hab[2] = val;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetChannelConfig);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetChannelConfig(BSAT_ChannelHandle h, uint32_t addr, uint32_t *pVal)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetChannelConfig);

   if (addr >= BSAT_7366_CHAN_CONFIG_MAX)
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x05, h->channel, BHAB_7366_READ, BHAB_7366_MODULE_SAT);
   hab[1] = addr;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));
   ...
   *pVal = hab[2];
   ...
   BDBG_LEAVE(BSAT_7366_P_GetChannelConfig);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetAcqDoneEventSettings(BSAT_Handle h, uint8_t reacqs)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetAcqDoneEventSettings);



   hab[0] = BHAB_7366_InitHeader(0x31, 0, 0, 0);
   hab[1] = (uint32_t)reacqs;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetAcqDoneEventSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetSignalNotificationSettings(BSAT_ChannelHandle h, BSAT_SignalNotificationSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetSignalNotificationSettings);

   hab[0] = BHAB_7366_InitHeader(0x22, h->channel, BHAB_7366_WRITE, 0);
   hab[1] = pSettings->rainFadeThreshold;
   hab[2] = pSettings->rainFadeWindow;
   hab[3] = pSettings->freqDriftThreshold;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 5));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetSignalNotificationSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetSignalNotificationSettings(BSAT_ChannelHandle h, BSAT_SignalNotificationSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetSignalNotificationSettings);

   hab[0] = BHAB_7366_InitHeader(0x22, h->channel, BHAB_7366_READ, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 5));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetSignalNotificationSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetACIBandwidth(BSAT_ChannelHandle h, uint32_t bw)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetACIBandwidth);

   if ((bw != 0) && ((bw < 1000000) || (bw > 45000000)))
      return BERR_INVALID_PARAMETER;

   hab[0] = BHAB_7366_InitHeader(0x19, h->channel, 0, 0);
   hab[1] = bw;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetACIBandwidth);
   return retCode;
}
...
BERR_Code BSAT_7366_P_StartSignalDetect(BSAT_ChannelHandle h, uint32_t symbolRate, uint32_t freq, uint32_t adcSelect)
{
   ...
   BDBG_ENTER(BSAT_7366_P_StartSignalDetect);

   if ((symbolRate < 1000000) || (symbolRate > 45000000))
   {
      BDBG_WRN(("BSAT_7366_P_StartSignalDetect(): invalid symbol rate (%d)", symbolRate));
      goto invalid_parameter;
   }

   if ((freq < 250000000UL) || (freq > 2300000000UL))
   {
      BDBG_WRN(("BSAT_7366_P_StartSignalDetect(): invalid frequency (%d)", freq));
      goto invalid_parameter;
   }

   if (adcSelect > 3)
   {
      BDBG_WRN(("BSAT_7366_P_StartSignalDetect(): invalid adcSelect (%d)", adcSelect));
      invalid_parameter:
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }
   ...
   hab[0] = BHAB_7366_InitHeader(0x16, h->channel, 0, 0);
   hab[1] = symbolRate;
   hab[2] = freq;
   hab[3] = adcSelect;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 5));

   hChn->lastMode = BSAT_Mode_eUnknown;

   done:
   BDBG_LEAVE(BSAT_7366_P_StartSignalDetect);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetSignalDetectStatus(BSAT_ChannelHandle h, BSAT_SignalDetectStatus *pStatus)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetSignalDetectStatus);

   hab[0] = BHAB_7366_InitHeader(0x17, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 3));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetSignalDetectStatus);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetLegacyQpskAcqSettings(BSAT_ChannelHandle h, BSAT_LegacyQpskAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetLegacyQpskAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x1F, h->channel, BHAB_7366_READ, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetLegacyQpskAcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetLegacyQpskAcqSettings(BSAT_ChannelHandle h, BSAT_LegacyQpskAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetLegacyQpskAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x1F, h->channel, BHAB_7366_WRITE, 0);
   ...
   BDBG_LEAVE(BSAT_7366_P_SetLegacyQpskAcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetExtAcqSettings(BSAT_ChannelHandle h, BSAT_ExtAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetExtAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x24, h->channel, BHAB_7366_READ, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 6));
...
BERR_Code BSAT_7366_P_SetExtAcqSettings(BSAT_ChannelHandle h, BSAT_ExtAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetExtAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x24, h->channel, BHAB_7366_WRITE, 0);
   ...
   BDBG_LEAVE(BSAT_7366_P_SetExtAcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetDvbs2AcqSettings(BSAT_ChannelHandle h, BSAT_Dvbs2AcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetDvbs2AcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x20, h->channel, BHAB_7366_READ, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 5));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetDvbs2AcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetDvbs2AcqSettings(BSAT_ChannelHandle h, BSAT_Dvbs2AcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetDvbs2AcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x20, h->channel, BHAB_7366_WRITE, 0);
   ...
   BDBG_LEAVE(BSAT_7366_P_SetDvbs2AcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetTurboAcqSettings(BSAT_ChannelHandle h, BSAT_TurboAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetTurboAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x21, h->channel, BHAB_7366_READ, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetTurboAcqSettings);
   return retCode;
}
...
BERR_Code BSAT_7366_P_SetTurboAcqSettings(BSAT_ChannelHandle h, BSAT_TurboAcqSettings *pSettings)
{
   ...
   BDBG_ENTER(BSAT_7366_P_SetTurboAcqSettings);

   hab[0] = BHAB_7366_InitHeader(0x21, h->channel, BHAB_7366_WRITE, 0);
   hab[1] = pSettings->scanModes;
   hab[2] = pSettings->ctl;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 4));

   done:
   BDBG_LEAVE(BSAT_7366_P_SetTurboAcqSettings);
   return retCode;
}
...
******************************************************************************/
BERR_Code BSAT_7366_P_SetNotchSettings(BSAT_Handle h, uint32_t n, BSAT_NotchSettings *pSettings)
{
   BDBG_ENTER(BSAT_7366_P_SetNotchSettings);

   BSTD_UNUSED(h);
   BSTD_UNUSED(n);
   BSTD_UNUSED(pSettings);

   BDBG_LEAVE(BSAT_7366_P_SetNotchSettings);
...
******************************************************************************/
BERR_Code BSAT_7366_P_GetNotchSettings(BSAT_Handle h, uint32_t *pNum, BSAT_NotchSettings *pSettings)
{
   BDBG_ENTER(BSAT_7366_P_GetNotchSettings);

   BSTD_UNUSED(h);
   BSTD_UNUSED(pNum);
   BSTD_UNUSED(pSettings);

   BDBG_LEAVE(BSAT_7366_P_GetNotchSettings);
   return BSAT_ERR_NOT_IMPLEMENTED;
}
...
BERR_Code BSAT_7366_P_InterruptCallback(void *pParm1, int parm2)
{
   ...
   BSAT_7366_P_ChannelHandle *pChn;

   BSTD_UNUSED(parm2);

   BSAT_DEBUG(BDBG_MSG(("BSAT_7366_P_InterruptCallback(): fstatus0=0x%X, fstatus1=0x%X", fstatus0, fstatus1)));
...
)
{
   ...
   shift = h->channel * 4;
   irq_mask = (mask & 0x0F) << shift;

   BSAT_7366_CHK_RETCODE(BHAB_WriteRegister(pImpl->hHab, BCHP_LEAP_HOST_L2_CLEAR1, &irq_mask));

   if (bEnable)
   {
      BSAT_7366_CHK_RETCODE(BHAB_WriteRegister(pImpl->hHab, BCHP_LEAP_HOST_L2_MASK_CLEAR1, &irq_mask));
   }
   else
   {
      BSAT_7366_CHK_RETCODE(BHAB_WriteRegister(pImpl->hHab, BCHP_LEAP_HOST_L2_MASK_SET1, &irq_mask));
   }
...
******************************************************************************/
BERR_Code BSAT_7366_P_GetTraceInfo(BSAT_ChannelHandle h, BSAT_TraceInfo *pInfo)
{
   BSAT_7366_P_Handle *pDevImpl = (BSAT_7366_P_Handle *)(h->pDevice->pImpl);
   BERR_Code retCode;
   uint32_t hab[11], i;

   BDBG_ENTER(BSAT_7366_P_GetTraceInfo);

   hab[0] = BHAB_7366_InitHeader(0x33, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 11));

   for (i = 0; i < 9; i++)
      pInfo->eventTime[i] = hab[i+1];

   done:
   BDBG_LEAVE(BSAT_7366_P_GetTraceInfo);
   return retCode;
}
...
/******************************************************************************
 BSAT_7366_P_SetCwc()
******************************************************************************/
BERR_Code BSAT_7366_P_SetCwc(BSAT_Handle h, uint32_t n, uint32_t *pFreq)
{
   BSAT_7366_P_Handle *pDevImpl = (BSAT_7366_P_Handle *)(h->pImpl);
   BERR_Code retCode;
   uint32_t i, hab[8];

   BDBG_ENTER(BSAT_7366_P_SetCwc);

   hab[0] = BHAB_7366_InitHeader(0x37, 0, 0, 0);
   for (i = 0; i < 6; i++)
   {
      if (i < n)
         hab[i+1] = pFreq[i];
   ...
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 8));
