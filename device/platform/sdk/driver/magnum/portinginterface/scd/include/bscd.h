 ...
 *
 * $brcm_Workfile: bscd.h $
 * $brcm_Revision: Hydra_Software_Devel/1 $
 * $brcm_Date: 9/7/12 10:57a $
 *
 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/scd/include/bscd.h $
 * 
 * Hydra_Software_Devel/1   9/7/12 10:57a qxie
 * SW7429-283:Smartcard: remove symlink in PI code
 * 
 * Hydra_Software_Devel/1   10/18/10 10:09a qxie
 * SW7422-78: initial version
 * 
 * Hydra_Software_Devel/18   7/13/09 3:20p qxie
 * PR56770: Smartcard: move BSCD_Channel_P_DumpRegisters to public PI,
 * changed name to BSCD_Channel_DumpRegisters
 * 
 * Hydra_Software_Devel/17   3/22/06 10:13a qcheng
 * PR19076: Support BCM7400.
 * 
 * Hydra_Software_Devel/16   11/19/04 6:23p btan
 * PR 13298: Refer to the functions BSCD_CLI_T0Test( ) or BSCD_CLI_T1Test(
 * ) in bscd_cli_infra.c for sample codes.
 * 
 * Hydra_Software_Devel/15   9/9/04 12:40p btan
 ...
 * bytes are received.
 * 
 * Hydra_Software_Devel/14   7/20/04 2:11p btan
 * PR 10898: Pass EMV 2000 tests with either event interrupt or WWT to
 * detect CWT +4
 * 
 * Hydra_Software_Devel/13   12/31/03 11:05a btan
 * PR 8080: Fixed auto_clk set before sc_clk_en
 * 
 * Hydra_Software_Devel/12   12/1/03 5:55p btan
 * Added BSCD_Channel_ResetBlockWaitTimer.
 * 
 * Hydra_Software_Devel/11   11/26/03 4:59p btan
 * Fixes for EMV T=0
 * 
 * Hydra_Software_Devel/9   11/18/03 5:04p btan
 * Fixed a typo.
 * 
 * Hydra_Software_Devel/8   10/13/03 12:05p btan
 * updated to latest version.
 * 
 * Hydra_Software_Devel/7   9/17/03 4:49p lseverin
 * fixed a few documentation warnings
 * 
 * Hydra_Software_Devel/6   7/23/03 5:40p btan
 * Added some functions headers.
 * 
 * Hydra_Software_Devel/5   7/21/03 6:28p btan
 * Updated after 1st review.
 * 
 * Hydra_Software_Devel/3   6/10/03 5:56p btan
 * Fixed some comments.
 * 
 * Hydra_Software_Devel/2   6/5/03 6:48p btan
...
****************************************************************************/
typedef struct BSCD_P_Handle            *BSCD_Handle;
...
Input:
in_regHandle  - BREG_Handle, use to access smart card register.
in_chipHandle - BCHP_Handle,  Chip handle.
in_interruptHandle - BINT_Handle, Interrupt handle to handle smart card 
						interrupt.          
...
/*****************************************************************************
Summary:
This function returns the number of smart card interfaces 
...
The caller shall then call BSCD_Channel_Open or BSCD_GetChannelDefaultSettings
with in_channelNo that smaller than outp_totalChannels.
  
Calling Context:
The function shall be called from application level (for example in 
VxWorks or no-os) or from driver level (for example in Linux,
recommended ioctl: BSCD_IOCTL_GET_TOTAL_CHANNELS or device open )
...
Output:
outp_totalChannels - unsigned char, a ref/pointer to the maximum number of 
						supported smart card channel handles. 
...
See Also:
BSCD_GetChannelDefaultSettings
BSCD_Channel_Open
...
Input/output:
inout_channelHandle  - BSCD_ChannelHandle,  smart card channel handle.
...
If this function returns successfully, outp_handle shall not be NULL.

Calling Context:
The function shall be called from application level (for example in 
VxWorks or no-os) or from driver level (for example in Linux,
recommended ioctl: BSCD_IOCTL_GET_DEVICE)
...
/*****************************************************************************
Summary:
This function will dump the smartcard control register values.
