...
#include "bchp_bspi.h"
	...
	(BCHP_CHIP==7346) || (BCHP_CHIP==7231) || (BCHP_CHIP==7358) || (BCHP_CHIP==7552) || (BCHP_CHIP==7435) || \
	(BCHP_CHIP==7360) || (BCHP_CHIP==7584) || (BCHP_CHIP==7563) || (BCHP_CHIP==7445)
...
#define SPI_POLLING_INTERVAL        50      /* in usecs */
#define SPI_MINIMUM_TIMEOUT			1000	/* ms */
#else
    ...
    )
{
    ...
    *pSpi = hDev;
    ...
    /* new bit added in 3548/3556 to enable the pin */
#if BSPI_USE_MAST_N_BOOT || HAS_HIF_MSPI
    lval = BREG_Read32(hDev->hRegister, BCHP_BSPI_MAST_N_BOOT_CTRL);
    lval |= BCHP_BSPI_MAST_N_BOOT_CTRL_mast_n_boot_MASK;
    ...
    )
{
	 ...
	 */
	if ((pChnDefSettings->spiCore == BSPI_SpiCore_Hif) && (channelNo < MAX_SPI_CHANNELS))
	{
...
#if defined(HUMAX_PLATFORM_BASE)
	(*pSpiReg)->BREG_SPI_WritebyMode_Func 	= BSPI_P_Write_By_Mode;
	(*pSpiReg)->BREG_SPI_ReadbyMode_Func	= BSPI_P_Read_By_Mode;
#endif
...
)
{
...
#if (BCHP_CHIP==7584)
    if (((mode == 8) && (length > MAX_SPI_XFER)) ||
        ((mode == 16) && (length > (MAX_SPI_XFER * 2))) || 
        ((mode == 32) && (length > (MAX_SPI_XFER * 4))) || 
        ((mode == 64) && (length > (MAX_SPI_XFER * 8))))
#else
    if (((mode == 8) && (length > MAX_SPI_XFER)) ||
        ((mode == 16) && (length > (MAX_SPI_XFER * 2))))
#endif
    ...
    if ((mode == 16) && (length & 0x01))       /* if the length is odd, round it up */
        length++;
#if (BCHP_CHIP==7584)
    if ((mode == 32) && (length & 0x03))       /* round up length */
        length = (length & ~0x03) + 4;
    if ((mode == 64) && (length & 0x07))       /* round up length */
        length = (length & ~0x07) + 8;
#endif
...
)
{
    ...
    if ((mode == 16) && (length & 0x01))       /* if the length is odd, reject it */
    {
        retCode = BERR_INVALID_PARAMETER;
        goto done;
    }
#if (BCHP_CHIP==7584)
    if (((mode == 32) && (length & 0x03)) || 
        ((mode == 64) && (length & 0x07)))
    {
        retCode = BERR_INVALID_PARAMETER;
        goto done;
...
#if defined(HUMAX_PLATFORM_BASE)
    timeoutMs = ((timeoutMs * 110) / 100) + SPI_MINIMUM_TIMEOUT;  /* add 10% fudge factor and round up */
#else
