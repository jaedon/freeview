...
#define INVERSE_FRAMERATE_2000_IN_SECONDS_MANTISSA  0x3333
#define INVERSE_FRAMERATE_1500_IN_SECONDS_MANTISSA  0x4444
#define INVERSE_FRAMERATE_1498_IN_SECONDS_MANTISSA  0x445C
...
#define INVERSE_FRAMERATE_2000_IN_SECONDS_EXPONENT  18
...
/* define max buffer size according to profile and level for VC1 */
#define VC1_MAX_VBV_SP_LEVEL_LOW                327680 
#define VC1_MAX_VBV_SP_LEVEL_MEDIUM             1261568 

#define VC1_MAX_VBV_MAIN_LEVEL_LOW              5013504              
#define VC1_MAX_VBV_MAIN_LEVEL_MEDIUM           10010624   
#define VC1_MAX_VBV_MAIN_LEVEL_HIGH             40009728 
...
#define MAX_DEE_IN_MILLISECONDS                 750
...
static void NumToFP(FPNumber_t *outputFPNumber, uint32_t inputNumber, uint8_t numBits);
static uint32_t FPToNum(FPNumber_t localFPNumber);
static uint32_t u32ScaleFP(FPNumber_t FP_operand, uint32_t scaleFactor, uint8_t bypassNormFlag);
...
extern void_t NumToFP(FPNumber_t *outputFPNumber, u32_t inputNumber, u8_t numBits);
extern u32_t FPToNum(FPNumber_t localFPNumber);
extern u32_t u32ScaleFP(FPNumber_t FP_operand, u32_t scaleFactor, uint8_t bypassNormFlag);
...
uint32_t BVCE_FW_P_CalcHRDbufferSize(uint32_t  Protocol, EncoderProfile_e  Profile, EncoderLevel_e  Level);
uint32_t BVCE_FW_P_Compute_DeeIn27MhzTicks(uint32_t averageBitsPerPic, uint32_t hrdBufferSize, uint32_t pictureIntervalIn27MhzTicks, FPNumber_t FP_ticksPerBit, uint32_t maxPictureIntervalIn27MhzTicks, uint32_t *pMaxAllowedBitsPerPicture);
uint32_t BVCE_FW_P_Compute_MaxDeeIn27MhzTicks(uint32_t MaxSupportedEndToEndDelayIn27MHz, uint32_t hrdBufferSize, FPNumber_t FP_ticksPerBit);
...
)
{
    ...
    uint32_t   minPictureIntervalIn27MhzTicks, minPtsDtsDelayOffset = 0;
    ...
    averageBitsPerPic = FPToNum(FP_inverse);
    ...
    if(RateBufferDelay == 0)
    {
        ...
        else
        {
            ...
            delayEndToEndIn27MhzTicks = BVCE_FW_P_Compute_DeeIn27MhzTicks(averageBitsPerPic, hrdBufferSize, minPictureIntervalIn27MhzTicks,
                                                                          FP_ticksPerBit, MAX_PIC_INTERVAL_IN_27MHZ_TICKS, &maxAllowedBitsPerPicture);
        }
    }
    else
    {
        if(RateBufferDelay > MAX_DEE_IN_MILLISECONDS)
        {
            RateBufferDelay = MAX_DEE_IN_MILLISECONDS;
        }
        /* convert milliseconds to 27 MHz ticks */
        FP_inverse.mantissa = RateBufferDelay;
        FP_inverse.exponent = 0;
        delayEndToEndIn27MhzTicks = u32ScaleFP(FP_inverse,  TWENTY_SEVEN_KHZ, 0);
    }

    /* Can not have delay in encoder more than what was set aside for EBM delay storage of coded picture info (no overflow expected) */
    /* This delay is too tight for some potential sequences (reduce by 5 pictures) */
    MaxSupportedEndToEndDelayIn27MHz = U32xU16MULTIPLY(minPictureIntervalIn27MhzTicks, MAX_NUM_EBM_PIC_DATA - 1 - 4);

    /* Compute the true max end to end delay supported based on HRD size and bit rate */
    MaxSupportedEndToEndDelayIn27MHz = BVCE_FW_P_Compute_MaxDeeIn27MhzTicks(MaxSupportedEndToEndDelayIn27MHz, hrdBufferSize, FP_ticksPerBit);
        ...
        (MaxAllowedGopStruct == ENCODING_GOP_STRUCT_TRACK_INPUT))
    {
        ...
        minPtsDtsDelayOffset = minPictureIntervalIn27MhzTicks;
    }
    else
    {
        minPtsDtsDelayOffset = 0;
    }

    MaxAllowedA2Pdelay[0] = MaxSupportedEndToEndDelayIn27MHz + encodingDelays + minPtsDtsDelayOffset;
    ...
    A2PdelayIn27MHzTicks  = delayEndToEndIn27MhzTicks + encodingDelays + minPtsDtsDelayOffset + DTS_OFFSET_IN_27MHZ_TICKS;
    ...
    )
{
    ...
    else
    {
        ...
        mbpCmeDelay = u32ScaleFP(FP_temp, pixelsPerFrame, 1);
...
)
{
    ...
    tempDelay = u32ScaleFP(FP_ticksPerBit, hrdBufferSize, 1);

    if(tempDelay > MaxSupportedEndToEndDelayIn27MHz)
    {
        MaxSupportedEndToEndDelayIn27MHz = tempDelay;
    }
...
BVCE_FW_P_Compute_DeeIn27MhzTicks(
    uint32_t        averageBitsPerPic,
    uint32_t        hrdBufferSize,
    uint32_t        minPictureIntervalIn27MhzTicks,
    FPNumber_t      FP_ticksPerBit,
...
)
{
    ...
    else
    {
        ...
        delayEndToEndIn27MhzTicks = u32ScaleFP(FP_ticksPerBit, delayEndToEndTimesMaxRate, 1);
    ...
    /* Can not have delay in encoder more than what was set aside for EBM delay storage of coded picture info (no overflow expected) */
    /* This delay is too tight for some potential sequences (reduce by 3 pictures) */
    MaxSupportedEndToEndDelayIn27MHz = U32xU16MULTIPLY(minPictureIntervalIn27MhzTicks, MAX_NUM_EBM_PIC_DATA - 1 - 4);
...
)
{
    ...
    NumToFP(&FP_inverseRate, bitrateBPS, 15);
...
)
{
    ...
    case ENCODING_STD_VC1:
        switch(Profile)
        {
        case ENCODING_VC1_PROFILE_MAIN:  /*Main profile*/
            switch(Level)
            {
            case ENCODING_VC1_LEVEL_LOW:    hrdBufferSize = VC1_MAX_VBV_MAIN_LEVEL_LOW; break;
            case ENCODING_VC1_LEVEL_MEDIUM: hrdBufferSize = VC1_MAX_VBV_MAIN_LEVEL_MEDIUM; break;
            default: hrdBufferSize = 0; /*error*/
            }
            break;
        default:
...
/****************************************************************
*  FPNumber_t NumToFP(uint32_t Number, uint8_t Bits)
...
/****************************************************************
*  FPToNum(FPNumber_t localFPNumber)
...
static uint32_t FPToNum(FPNumber_t localFPNumber)
{
    ...
    {
        Number = (localFPNumber.mantissa + (0x1<<(localFPNumber.exponent - 1))) >>  localFPNumber.exponent;
    }
    else
    {
...
/**********************************************************************************
*  u32ScaleFP(FPNumber_t FP_operand, uint32_t scaleFactor, uint8_t bypassNormFlag)
**********************************************************************************/
static uint32_t u32ScaleFP(FPNumber_t FP_operand, uint32_t scaleFactor, uint8_t bypassNormFlag)
{
    ...
    {
        NumToFP(&FP_short, FP_operand.mantissa, 16);
            FP_short.exponent += FP_operand.exponent;
    }
...
static void NormalizeFP(FPNumber_t *pOutputFPNumber, FPNumber_t pInputFPNumber, uint8_t numBits)
{
    ...
    NumToFP(pOutputFPNumber, pInputFPNumber.mantissa, numBits);
...
if (InputType==ENCODER_INPUT_TYPE_PROGRESSIVE)
    {
        ...
        NmbyChroma=bvceEpmCalcNMBY(max_vertical_size_in_mbs>>1);
