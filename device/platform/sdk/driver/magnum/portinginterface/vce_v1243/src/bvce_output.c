 ...
 * 
 * Hydra_Software_Devel/30   4/18/13 10:54p nilesh
 * SW7425-4731: Prevent assert in ConsumeBufferDescriptors when releasing
 * dummy metadata/EOS
 * 
 * Hydra_Software_Devel/29   4/18/13 10:09p nilesh
 * SW7425-4731: If no frames are encoded, on stop, GetBufferDescriptors
 * will generate a dummy metadata descriptor followed by an EOS
 * descriptor.  In NRT mode, the EOS descriptor will also have a valid
 * ESCR=0.
 * 
 * Hydra_Software_Devel/28   4/18/13 3:42p nilesh
 * SW7425-4614: Fixed issue with invalid ITB/CDB depth after a cold boot
 * 
 * Hydra_Software_Devel/27   4/18/13 3:30p nilesh
 * SW7425-4614: Fixed calculation of ITB/CDB depth when read==valid
 * 
 * Hydra_Software_Devel/26   4/18/13 3:28p nilesh
 * SW7425-4614: Fixed deadlock when CABAC is not ready and the CDB/ITB is
 * full from previous unclean stop
 * 
 * Hydra_Software_Devel/25   4/11/13 3:33p nilesh
 * FWVICE2-457: Reset previousESCR on chunk boundaries
 * 
 * Hydra_Software_Devel/24   4/9/13 4:31p nilesh
 * FWVICE2-457: Added out of order ESCR debug warning
 * 
 * Hydra_Software_Devel/23   4/3/13 2:39p nilesh
 * SW7425-4614: Added support for flushing when ITB/CDB accessed directly
 * 
 * Hydra_Software_Devel/22   4/1/13 4:31p nilesh
 * SW7425-4614: Fixed issue if channel is closed before output is closed
 * 
 * Hydra_Software_Devel/21   3/21/13 12:41p nilesh
 ...
 * prematurely.
 * 
 * Hydra_Software_Devel/20   3/19/13 4:19p nilesh
 * SW7425-4614: Fixed stop/close/open/start sequence when muxer thread
 * dies prematurely
 * 
 * Hydra_Software_Devel/19   3/7/13 3:04p nilesh
 * SW7425-4612: Fixed bug in calculation of number of frames in ITB when
 * ITB is empty
 * 
 * Hydra_Software_Devel/18   3/1/13 10:46a nilesh
 * SW7425-2568: Fixed typo in buffer log creation
 * 
 * Hydra_Software_Devel/17   2/28/13 10:47a nilesh
 * SW7425-4355: Populate uiEtsDtsOffset in status and video metadata
 * descriptor
 * 
 * Hydra_Software_Devel/16   2/27/13 4:59p nilesh
 * SW7425-4589: Still send EOS descriptor when immediate stop is used
 * 
 * Hydra_Software_Devel/15   2/27/13 3:46p nilesh
 * SW7435-667: Assert if EOC is set on the first picture of a chunk
 * 
 * Hydra_Software_Devel/14   2/27/13 2:54p nilesh
 * SW7425-2568: Buffer descriptor dump now includes metadata entries.
 * Added EOC column
 * 
 * Hydra_Software_Devel/13   2/26/13 11:11a nilesh
 * SW7435-590: Enforce platform specific ITB size alignment
 * 
 * Hydra_Software_Devel/12   2/26/13 9:36a nilesh
 * SW7445-163: No longer flush CDB cache if NAL Data Unit deteciton is
 * disabled
 * 
 * Hydra_Software_Devel/11   1/16/13 10:58a nilesh
 * SW7425-4421: Fixed memset of uncached memory
 * 
 * Hydra_Software_Devel/10   1/15/13 12:02p nilesh
 * SW7425-4410: Added reset of ITB index to prevent stale data across a
 * stop/start boundary
 * 
 * Hydra_Software_Devel/9   1/14/13 10:58a nilesh
 * SW7425-4410: Fixed BVCE_DUMP_OUTPUT_ITB=y behavior
 * 
 * Hydra_Software_Devel/8   12/18/12 3:58p nilesh
 * SW7435-562: Added BVCE_Output_OpenSettings.hDescriptorMem
 * 
 * Hydra_Software_Devel/7   12/3/12 5:16p nilesh
 * SW7445-39: No longer call BVCE_Channel_GetStatus when cabac is not
 * ready
 * 
 * Hydra_Software_Devel/SW7445-39/1   11/20/12 2:31p nilesh
 ...
 * 
 * Hydra_Software_Devel/6   11/6/12 4:08p nilesh
   ...
   )
{
   ...
   {
      hVceOutput->hDescriptorMem = hVceOutput->hVce->channel_memory.hMem[BVCE_P_HeapId_eSystem];
   }
   else
   {
      hVceOutput->hDescriptorMem = hVceOutput->stOpenSettings.hDescriptorMem;
   }

   /* Allocate video buffer descriptors */
   hVceOutput->astDescriptorsUnCached = (BAVC_VideoBufferDescriptor *) BMEM_Alloc(
      hVceOutput->hDescriptorMem,
...
{
   BDBG_ENTER( BVCE_Output_P_DetachFromChannel );
   ...
   BDBG_LEAVE( BVCE_Output_P_DetachFromChannel );
}
   ...
   while ( hVceOutput->state.stBufferCache.uiITBCacheValidOffset != uiITBValidOffset )
   {
      ...
      BDBG_MODULE_MSG(BVCE_OUTPUT_CACHE, ( "ITB Flushed: %p %d bytes",
                  ...
                  if ( NULL == hVceOutput->hITBDumpFile )
                  {
                     ...
                     char fname[256];
                     sprintf(fname, "BVCE_OUTPUT_%02d_%02d_%03d.itb", hVceOutput->state.hVceCh->hVce->stOpenSettings.uiInstance, hVceOutput->state.hVceCh->stOpenSettings.uiInstance, uiInstance);
                     uiInstance++;
...
{
   BDBG_ENTER( BVCE_Output_P_Reset );
   ...
   BDBG_LEAVE( BVCE_Output_P_Reset );
   ...
   )
{
   ...
   BDBG_ENTER( BVCE_Output_P_FlushDirect );
   ...
   BDBG_LEAVE( BVCE_Output_P_FlushDirect );
}
   ...
   )
{
   ...
   if ( iDiff < 0 )
   {
      BDBG_MSG(("[%d][%d] Detected ESCR < minESCR (%08x < %08x) (DTS(90Khz): %09llx. EtsDtsDelta: %08x)",
   ...
   )
{
   ...
   /* Scan ITB for entries */
   {
      ...
      if ( false == hVceOutput->state.bChannelStatusValid )
      {
         ...
         rc = BERR_TRACE( BVCE_Channel_GetStatus(
               hVceOutput->state.hVceCh,
      ...
      while ( uiReadOffset != hVceOutput->state.stITBBuffer.uiValidOffset ) /* Until end of ITB is reached */
      {
         ...
         switch ( eIndexState )
         {
            ...
            case BVCE_OUTPUT_P_ITB_IndexingState_eESCREntry:
               ...
               {
                  BDBG_MSG(("[%d][%d] EOC ITB Seen", hVceOutput->hVce->stOpenSettings.uiInstance, hVceOutput->state.hVceCh->stOpenSettings.uiInstance));
                  pVideoDescriptor->stCommon.uiFlags |= BAVC_COMPRESSEDBUFFERDESCRIPTOR_FLAGS_EOC;
   ...
   )
{
   if ( NULL == hVceOutput->hDescriptorLog )
   {
      ...
      {
         BVCE_Debug_P_WriteLog_isr( hVceOutput->hDescriptorLog, "count,flags,metadata,extended,frame start,eoc,eos,empty frame,offset,length,opts valid,opts,pts valid,pts,escr valid,escr,tpb valid,tpb, shr valid, shr, stc valid, stc" );
   ...
   {
      /* Common */
      BVCE_Debug_P_WriteLog_isr( hVceOutput->hDescriptorLog, "%u,0x%08x,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%llu,%u,%u,%u,%u,%u,%d,%u,%llu",
   ...
   )
{
   ...
   *puiNumDescriptors1 = 0;
...

      {
         ...
         if ( true == hVceOutput->state.bCabacInitialized )
         {
            ...
            if ( 0 != ( ( hVceOutput->state.stITBBuffer.uiValidOffset - uiITBReadOffset ) % 16 ) )
            {
                ...
                */
               hVceOutput->state.bCabacInitialized = false;
        ...
        &&  ( BVCE_P_Status_eStopping == BVCE_Channel_P_GetState( hVceOutput->state.hVceCh ) ) )
   {
      ...
      pMetadataDescriptor = &hVceOutput->astMetadataDescriptors[hVceOutput->state.uiMetadataDescriptorWriteOffset];
      ...
      while ( 1 )
      {
         ...
         if ( true == hVceOutput->state.bFrameStart )
         {
               ...
               )
            {
               ...
               if ( pMetadataDescriptor->stTiming.uiChunkId != uiChunkId )
               {
   ...
   BDBG_LEAVE( BVCE_Output_P_GetBufferDescriptors );
   return BERR_TRACE( BERR_SUCCESS );
}
   ...
   )
{
   ...
   BDBG_ENTER( BVCE_Output_P_ConsumeBufferDescriptors );
   ...
   BDBG_LEAVE( BVCE_Output_P_ConsumeBufferDescriptors );
   return BERR_TRACE( BERR_SUCCESS );
}
...
{
   BDBG_ENTER( BVCE_Output_P_Flush );
   ...
   BDBG_LEAVE( BVCE_Output_P_Flush );
}
