 ...
 * Revision History:
 *
 * $brcm_Log: /magnum/portinginterface/xpt/src/core40nm/bxpt_mesg.c $
 * 
 * Hydra_Software_Devel/18   8/21/12 10:26a gmullen
 * SW7425-3785: Don't automatically enable PID channels during mesg
 * handling
 * 
 * Hydra_Software_Devel/17   8/2/12 4:31p gmullen
 ...
 * 
 * Hydra_Software_Devel/16   12/20/11 4:27p gmullen
 ...
 * 
 * Hydra_Software_Devel/15   12/15/11 2:32p gmullen
 * SW7425-1952: Merged changes to mainline
 * 
 * Hydra_Software_Devel/SW7425-1952/1   12/14/11 4:16p gmullen
 * SW7425-1952: Do not change SPECIAL_TYPE field. BXPT_SAM_* uses it
 * 
 * Hydra_Software_Devel/14   11/15/11 2:54p gmullen
 * SW7346-562: Fixed addresses calculation bug
 * 
 * Hydra_Software_Devel/13   10/28/11 4:15p jtna
 * SW7425-1486: remove BXPT_GetMesgBufferErrorStatus() in favor of
 * BXPT_Mesg_GetErrorStatus()
 * 
 * Hydra_Software_Devel/12   10/28/11 4:03p jtna
 * SW7425-1486: added BXPT_Mesg_GetErrorStatus()
 * 
 * Hydra_Software_Devel/11   10/26/11 11:41a gmullen
 ...
 * subsequent mappings
 * 
 * Hydra_Software_Devel/10   8/4/11 11:04a gmullen
 * SW7346-398: Fixed coverity warning
 * 
 * Hydra_Software_Devel/9   7/15/11 9:05a gmullen
 * SW7346-308: Merged fix to Hydra
 * 
 * Hydra_Software_Devel/SW7346-308/1   7/14/11 4:59p gmullen
 * SW7346-308: Added BXPT_RemoveFilterFromGroupAndBuffer() and
 * BXPT_Mesg_GetPidChannelFromBufferNum(), editted comments in bxpt.h
 * 
 * Hydra_Software_Devel/8   7/13/11 1:37p gmullen
 ...
 * 
 * Hydra_Software_Devel/7   12/17/10 4:08p gmullen
 * SW7425-60: Set PID2BUF mapping when starting message capture and PID
 * channel recording
 * 
 * Hydra_Software_Devel/6   12/9/10 11:28a gmullen
 * SW7422-20: Fixed incorrect calc of PID2BUF register address
 * 
 * Hydra_Software_Devel/5   12/3/10 5:22p gmullen
 * SW7422-87: Added per-PID channel R-pipe support for mesg filters
 * 
 * Hydra_Software_Devel/4   11/15/10 5:17p gmullen
 * SW7422-20: Removed restriction that a PID channel must be allocated
 * before a message filter can be attached to it's buffer
 * 
	...
	)
{
	...
	for (i=0; i<(BXPT_NUM_MESG_BUFFERS * 4); i++) 
    ...
    )
{
    ...
    *PidChannelNum = BXPT_NUM_PID_CHANNELS;
	if ( MesgBufferNum >= BXPT_NUM_MESG_BUFFERS ) 
    {
        /* Bad Message Buffer number. Complain. */
        BDBG_ERR(( "Message Buffer Number %lu is out of range!", ( unsigned long ) MesgBufferNum ));
    ...
    else
    {
        ...
        {
            uint32_t Reg;
            RegAddr = BCHP_XPT_MSG_PID2BUF_MAP_i_ARRAY_BASE + (Index * 16) + (RegOffset * 4);
...
unsigned GetPidChannelByFilter( 
    BXPT_Handle hXpt, 
    unsigned int Bank, 
    unsigned int FilterNum 
    )
