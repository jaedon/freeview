 ...
 *
 * [Revision History:]
 * $brcm_Log: /magnum/syslib/muxlib/7425/stream/ts/bmuxlib_ts_userdata.c $
 * 
...
BERR_Code BMUXlib_TS_P_Userdata_ProcessInputs(BMUXlib_TS_Handle hMuxTS)
{
   ...
   for (uiUserdataIndex = 0; (uiUserdataIndex < pStatus->uiNumInputs) && (BERR_SUCCESS == rc); uiUserdataIndex++)
   {
             ...
             && (false == BMUXlib_List_IsFull(hMuxTS->hUserdataPendingList[uiUserdataIndex])))
      {
         ...
         /* FIXME: if a packet did not meet selection timing last iteration, could we make this skip the
            parsing and timing adjustments, and use saved values from last time? */
                   ...
                   && (pUserdata->uiNumSegments <= uiNumAvailableDescriptors))
               {
                  ...
                  if (bInsertPacket)
                  {
                     ...
                     for (uiSegment = 0; uiSegment < uiNumSegments; uiSegment++)
                     {
                        ...
                        rc = BERR_TRACE( BMUXlib_List_Add(hMuxTS->hTransportDescriptorMetaDataPendingList[uiTransportChannelIndex], pMetaDesc));
                        /* this should be successful, since we check for available space up-front */
                        BDBG_ASSERT(BERR_SUCCESS == rc);
                        ...
                        rc = BERR_TRACE(BMUXlib_TS_P_AddTransportDescriptor(hMuxTS, uiTransportChannelIndex, pDesc));
                        BDBG_ASSERT(BERR_SUCCESS == rc);
...
#ifdef BMUXLIB_TS_P_TEST_MODE
   pPacketInfo->uiPID = ((uint16_t)(pTSPacket[1] & 0x1F) << 8) | pTSPacket[2];
#endif
   ...
   if (pPacketInfo->bPayloadPresent)
   {
      ...
      if (pPacketInfo->bPayloadStart)
      {
         ...
         if (BMUXLIB_PES_HAS_OPTIONAL_FIELDS(uiStreamID))
         {
            ...
            if (!pPacketInfo->bPTSPresent && pPacketInfo->bDTSPresent)
            {
               ...
               BDBG_WRN(("[%d]: PES has invalid PTS/DTS flags: 01 (using DTS as PTS)", pUserdataInfo->uiIndex));
      ...
      else
      {
         ...
         if (!pUserdataInfo->bFirstPESSeen)
         {
            ...
            BDBG_WRN(("[%d]: Skipping TS Packet ... no PES found", pUserdataInfo->uiIndex));
#endif
