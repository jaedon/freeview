 ...
 * $brcm_Date: 5/24/13 9:19a $
 *
 * [File Description:]
 *
 * Revision History:
 *
 * $brcm_Log: /magnum/syslib/muxlib/src/common/bmuxlib_input.c $
 * 
 * Hydra_Software_Devel/39   5/24/13 9:19a nilesh
 * SW7425-4610: Fixed typo
 * 
 * Hydra_Software_Devel/38   5/23/13 5:11p nilesh
 * SW7425-4610: Fixed with return code from
 * BMUXlib_Input_ConsumeDescriptors()
 * 
 * Hydra_Software_Devel/37   3/29/13 12:02p delkert
 * SW7425-4681: Replace the safety check in the loop for finding current
 * DTS
 *
 * Hydra_Software_Devel/36   3/29/13 10:46a delkert
 * SW7425-4681: Fix ConsumeDescriptors to set InitialDTS and CurrentDTS
 * correctly.
 *
 * Hydra_Software_Devel/35   3/21/13 2:29p nilesh
 * SW7425-4614: Fixed duration reporting when filter until metadata seen
 * is enabled
 *
 * Hydra_Software_Devel/34   3/20/13 3:01p nilesh
 * SW7425-4614: Added code to filter descriptors until the first metadata
 * packet is seen
 *
   ...
   if ( BERR_SUCCESS == rc )
   {
      ...
      if ( BERR_SUCCESS == rc )
      {
              ...
              && ( false == hInput->stats.bMetadataSeen ) )
         {
            ...
            if ( 0 != uiFilterCount )
            {
               BDBG_WRN(("Filtered %d stale descriptors", uiFilterCount));
           ...
           || ( false == hInput->stCreateSettings.bFilterUntilMetadataSeen ) )
      {
         if ( uiCount > 0 )
         {
            ...
            if ( false == hInput->stats.stStatus.bInitialDTSValid )
            {
               ...
               for (i = hInput->uiReadOffset; i < hInput->uiReadOffset + uiCount; i++)
               {
                  BMUXlib_Input_P_PeekAtDescriptor( hInput, i, &stDescriptor );
                  /* the following should not happen  */
                  BDBG_ASSERT(stDescriptor.descriptor.pstCommon != NULL);
