...
# Revision History:
#
# $brcm_Log: /nexus/build/os/bare/module_rules.pl $
# 
...
print OUTFILE "\t\$(Q_)cd \$(NEXUS_TOP)/build/tools/kernelproxy && \$(PERL) class_list.pl \$(CLASS_LIST) \$(CLASS_VERIFICATION) \$(PUBLIC_INCLUDES)\n";
print OUTFILE "\n";
...
foreach $moduleUpper (@ARGV) {
    ...
    if ($proxy == 1) {
        ...
        print OUTFILE "\n";
        print OUTFILE "\$(NEXUS_SYNCTHUNK_DIR)/nexus_$moduleLower\_driver.c: \$(NEXUS_SYNCTHUNK_DIR)/nexus_$moduleLower\_ioctl.h \$(CLASS_VERIFICATION)\n";
        print OUTFILE "\n";
        ...
        print OUTFILE "\t\$(Q_)cd \$(NEXUS_TOP)/build/tools/kernelproxy && \$(PERL) bapi_build.pl $moduleUpper \$(NEXUS_SYNCTHUNK_DIR) \$(CLASS_LIST) \$(addsuffix /*.h,\$(NEXUS_$moduleUpper\_PUBLIC_INCLUDES))\n";
        print OUTFILE "\n";
    }
    else {
    ...
    print OUTFILE "\t\@echo \"[Compile... \$(notdir \$<) ($moduleLower)]\"\n";
    print OUTFILE "\t\$(Q_)\$(CC) \$(if \$(filter \$<, \$(NEXUS_$moduleUpper\_SYNCTHUNKS) \$(NEXUS_SYNCTHUNK_DIR)/nexus_$moduleLower\_proxy.c),, -include \${NEXUS_OBJ_DIR}/$moduleUpper/nexus_$moduleLower\_precompiled.h) \$(CDEP_FLAG) \$(NEXUS_CFLAGS) \$(NEXUS_CFLAGS_BPROFILE) \$(NEXUS_$moduleUpper\_CFLAGS) -c \$< -o \$@\n";
    print OUTFILE "\n";
    print OUTFILE "else\n";
