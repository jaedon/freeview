 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: timeshift_chunk.c $
 * $brcm_Revision: 4 $
 * $brcm_Date: 2/15/12 10:55a $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /nexus/examples/dvr/timeshift_chunk.c $
 * 
 * 4   2/15/12 10:55a erickson
 * SWDEPRECATED-3955: switch to live decode when timeshifted playback hits
 *  end of stream
 * 
 * 3   1/26/12 3:01p rjlewis
 ...
 * 
 * SW7405-3587/2   4/22/10 12:01p mphillip
 * SW7405-4251: Update chunked PVR after API review, prepare for merge to
 *  main.
 * 
 * SW7405-3587/1   2/4/10 10:14a mphillip
 * SW7405-3587: chunked fifo example (forked from timeshift.c)
 * 
...
#define DATA_FILE_NAME "videos/chunked_stream.fifompg"
#define INDEX_FILE_NAME "videos/chunked_stream.fifonav"
...
int main(void)
{
...
#endif
    NEXUS_ChunkedFifoRecordHandle fifofile;
    ...
    **/
    ...
    fifofile = NEXUS_ChunkedFifoRecord_Create(datafilename, indexfilename);

    NEXUS_ChunkedFifoRecord_GetSettings(fifofile, &app->fifoRecordSettings);
    app->fifoRecordSettings.interval = 60; /* 1 minute buffer */
    app->fifoRecordSettings.data.chunkSize = 32*4096*4096;
    rc = NEXUS_ChunkedFifoRecord_SetSettings(fifofile, &app->fifoRecordSettings);
    BDBG_ASSERT(!rc);

    recordfile = NEXUS_ChunkedFifoRecord_GetFile(fifofile);
    ...
    while (1) {
        ...
        NEXUS_Record_GetStatus(app->record, &status);
        if (status.picturesIndexed) {
            BDBG_WRN(("%u pictures indexed", status.picturesIndexed));
            break;
        }
        BKNI_Sleep(10);
    }
    ...
    **/
    ...
    app->playbackfile = NEXUS_ChunkedFifoPlay_Open(datafilename, indexfilename, fifofile);
    if (!app->playbackfile) {
        ...
        for(i=0;cmd[i]!='\0';i++) {
            ...
            case ' ':
                ...
                if (app->decode_state == decode_state_playback) {
                    rc = NEXUS_Playback_GetStatus(app->playback, &playbackStatus);
                    BDBG_ASSERT(!rc);
                    NEXUS_ChunkedFifoRecord_GetPosition(fifofile, &first, &last);
