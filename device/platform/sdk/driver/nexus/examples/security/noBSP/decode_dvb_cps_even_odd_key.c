 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: decode_dvb_cps_even_odd_key.c $
 * $brcm_Revision: 4 $
 ...
 * Revision History:
 * 
 * $brcm_Log: /nexus/examples/security/noBSP/decode_dvb_cps_even_odd_key.c $
 * 
 * 4   1/27/12 11:46a rjlewis
											 ...
											 unsigned char * pKeyOdd)
{
	...
	for (i=0; i<NEXUS_SecurityPacketType_eMax;i++) {
#if 1
		AlgConfig.modifyScValue[i] = false; /* Since the SC bits were modified by CA No need to modify it by CPS */
#else
		AlgConfig.modifyScValue[i] = true; 
