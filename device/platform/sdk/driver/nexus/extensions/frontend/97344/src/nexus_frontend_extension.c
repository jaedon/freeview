...
* Revision History:
*
* $brcm_Log: /nexus/extensions/frontend/97344/src/nexus_frontend_extension.c $
* 
* 1   12/7/10 11:34a jrubio
* SW7344-9: initial version of extension
* 
\ 
* PR50837/1   1/29/09 10:38a jgarrett
* PR 50837: Adding 97344 frontend extension
*
***************************************************************************/
...
BDBG_OBJECT_ID(NEXUS_97344QpskExtension);

typedef struct NEXUS_97344QpskExtension
{
    BDBG_OBJECT(NEXUS_97344QpskExtension)
    NEXUS_FrontendHandle frontend;
    NEXUS_TunerHandle tuner;
} NEXUS_97344QpskExtension;

static void NEXUS_Frontend_P_CloseSds97344(NEXUS_FrontendHandle handle);
static NEXUS_Error NEXUS_Frontend_P_TuneSds97344(void *handle, const NEXUS_FrontendSatelliteSettings *pSettings);

static void NEXUS_Frontend_P_CloseQpsk97344(NEXUS_FrontendHandle handle);
static NEXUS_Error NEXUS_Frontend_P_TuneQpsk97344(void *handle, const NEXUS_FrontendSatelliteSettings *pSettings);
...
NEXUS_Error NEXUS_Frontend_Create97344Extension(
    ...
    NEXUS_97344FrontendExtensionHandles *pHandles
    )
{
    ...
    if ( pSettings->primary )
    {
        ...
        extensionSettings.tuneSatellite = NEXUS_Frontend_P_TuneSds97344;
        extensionSettings.parent = pSettings->primary;
        extensionSettings.close = NEXUS_Frontend_P_CloseSds97344;
    ...
    {
        NEXUS_97344QpskExtension *pQpskExtension;

        pQpskExtension = BKNI_Malloc(sizeof(NEXUS_97344QpskExtension));
        ...
        BDBG_OBJECT_SET(pQpskExtension, NEXUS_97344QpskExtension);
        pQpskExtension->frontend = pSettings->qpsk;
        pQpskExtension->tuner = pSettings->qpskTuner;

        NEXUS_Frontend_P_GetDefaultExtensionSettings(&extensionSettings);
        extensionSettings.tuneSatellite = NEXUS_Frontend_P_TuneQpsk97344;
        extensionSettings.parent = pSettings->qpsk;
        extensionSettings.close = NEXUS_Frontend_P_CloseQpsk97344;
...
static void NEXUS_Frontend_P_CloseQpsk97344(NEXUS_FrontendHandle handle)
{
    NEXUS_97344QpskExtension *pExtension;
    pExtension = handle->pDeviceHandle;
    BDBG_OBJECT_ASSERT(pExtension, NEXUS_97344QpskExtension);
    BDBG_OBJECT_DESTROY(pExtension, NEXUS_97344QpskExtension);
