 ...
 * $brcm_Revision: 4 $
 * $brcm_Date: 7/1/11 1:13p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /nexus/extensions/transport/security/7231/src/nexus_transport_security.c $
 * 
 * 4   7/1/11 1:13p yili
 * SW7231-272:Clean up
 * 
 * 3   6/29/11 5:27p yili
 * SW7422-404:Clean up
 * 
 * 2   4/20/11 1:37p yili
...
#define RAVE_REGION_ID 0x08


#define RAVE_FW_SIZE (2*8192)
...
static void Dump_RaveFW (void)
{
	...
	unsigned char dIn [4];
	int i;
	//int total = 130*1024*64;
	int total = 188*1024*256;

#if defined (_WIN32_WCE)
	pOut = fopen ("\\release\\rave.bin", "wb");
#else
...
static BERR_Code rave_verification(void)
{
...
#ifdef NEXUS_SECURITY_RAVE_BAD_SIGNATGURE_TEST
	pAddr[0] = ~pAddr[0]; /* Tamper the first byte */
#endif

    NEXUS_FlushCache((const void*)pAddr, 256);

    BDBG_MSG(("Enable rave FW checking"));
	...
	RegionSetting.cpuType = NEXUS_SecurityRegverCPUType_eRave;
