...
* Revision History:
*
* $brcm_Log: /nexus/modules/audio/src/ape_mips/nexus_audio_capture.c $
* 
* 5   8/2/12 8:10p bandrews
* SW7420-2085: fix location of NEXUS_OBJECT_INIT, since it memsets
* 
* 4   7/27/12 4:38p jgarrett
* SW7420-2085: Converting from BDBG_OBJECT to NEXUS_OBJECT
* 
* 3   5/9/12 4:43p jgarrett
* SW7435-174: Adding NEXUS_AudioCapture_GetStatus
* 
* 2   3/17/10 4:32p jgarrett
* SW7408-50: Adding PCM capture support
* 
* 1   12/9/09 1:15p jgarrett
* SW7408-17: Adding capture
* 
***************************************************************************/
    ...
    )
{
    ...
    BMEM_Heap_ConvertAddressToCached(g_pCoreHandles->heap[0], handle->pMemory, &handle->pCachedMemory);
...
static void NEXUS_AudioCapture_P_ConvertStereo24(NEXUS_AudioCaptureHandle handle)
{
    ...
    for ( ;; )
    {
        ...
        BMEM_Heap_ConvertAddressToCached(g_pCoreHandles->heap[0], pBuffer, &pCachedBuffer);
        ...
        BMEM_Heap_FlushCache(g_pCoreHandles->heap[0], pCachedBuffer, copied);
...
static void NEXUS_AudioCapture_P_SampleRate_isr(void *pParam, int param, unsigned sampleRate)
{
    NEXUS_AudioCaptureHandle handle;
