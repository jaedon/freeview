 ...
 * $brcm_Revision: 11 $
 * $brcm_Date: 6/20/12 2:30p $
 * 
 * Module Description:
 * 
 * Revision History:
 * 
 * $brcm_Log: /nexus/modules/video_decoder/7400/secure_ext/src/nexus_video_decoder_security.c $
 * 
 * 11   6/20/12 2:30p erickson
 * SW7435-164: remove internal xvd #include
 * 
 * 10   7/14/11 7:37p mphillip
...
void NEXUS_VideoDecoder_P_EnableFwVerification (
...
void NEXUS_VideoDecoder_P_DisableFwVerification (unsigned int index)
{
    ...
    if (gBootContext[index].hFWMem)
        BMEM_Heap_Destroy(gBootContext[index].hFWMem);
    if (gBootContext[index].pVirtualAddr)
