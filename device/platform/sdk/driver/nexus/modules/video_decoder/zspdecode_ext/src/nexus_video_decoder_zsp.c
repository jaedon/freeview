 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: nexus_video_decoder_zsp.c $
 * $brcm_Revision: 5 $
 ...
 * Revision History:
 *
 * $brcm_Log: /nexus/modules/video_decoder/zspdecode_ext/src/nexus_video_decoder_zsp.c $
 * 
 * 5   9/7/12 11:56a erickson
 * SW7435-349: add standard header
 * 
 * 4   3/17/11 12:06p vsilyaev
 * SW7422-216: Added video coded
 * 
 * 3   10/11/10 12:58p vsilyaev
 * SW7422-65, SW7468-295: Added ZSP video flush
 * 
 ...
 * 
 * SW7468-129/10   8/30/10 6:24p vsilyaev
 * SW7468-129: Added link with the pcrlibrary
 * 
 * SW7468-129/9   6/8/10 2:12p vsilyaev
 * SW7468-129: Use RAP PI to obtain rave configuration
 * 
 * SW7468-129/8   3/18/10 12:14p vsilyaev
 * SW7468-129: Pass rave context to the ZSP video decode
 * 
 * SW7468-129/7   3/15/10 2:39p vsilyaev
 * SW7468-129: Added code to use dummy decoder instead of ZSP decode
 * 
 ...
 * 
 * SW7468-129/4   3/8/10 6:32p vsilyaev
 * SW7468-129: Added basic implementation for video audio decoder
 * 
 * SW7468-129/3   3/5/10 7:31p vsilyaev
...
BDBG_MODULE(nexus_video_decoder_decoder_zsp);

BDBG_OBJECT_ID(NEXUS_VideoDecoder_Zsp);

#define BDBG_MSG_TRACE(x)  BDBG_MSG(x)

#define NEXUS_VIDEO_DECODER_P_ZSP_DUMMY_DECODER 0

#if NEXUS_VIDEO_DECODER_P_ZSP_DUMMY_DECODER
...
static void NEXUS_VideoDecoder_P_Close_Zsp( NEXUS_VideoDecoderHandle decoder)
{
    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_P_Close_Zsp: %#x", (unsigned)decoder));
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    NEXUS_VideoDecoder_P_Xdm_Shutdown(decoder);
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_Close_priv(decoder->decoder.zsp.decoder);
    UNLOCK_AUDIO();
...
static NEXUS_Error NEXUS_VideoDecoder_P_SetSettings_Zsp( NEXUS_VideoDecoderHandle decoder, const NEXUS_VideoDecoderSettings *pSettings)
{
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
...
static NEXUS_Error NEXUS_VideoDecoder_P_Start_Zsp( NEXUS_VideoDecoderHandle decoder, const NEXUS_VideoDecoderStartSettings *pSettings)
{
    NEXUS_Error  rc;
    ...
    NEXUS_ZspVideoDecoderStartSettings zspSstartSettings;

    BDBG_MSG(("NEXUS_VideoDecoder_P_Start_Zsp:%#x", (unsigned)decoder));
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
...
#if !NEXUS_VIDEO_DECODER_P_ZSP_DUMMY_DECODER
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_GetDefaultStartSettings_priv(&zspSstartSettings);
    zspSstartSettings.raveContext = decoder->rave;
    zspSstartSettings.codec = pSettings->codec;
    rc = NEXUS_ZspVideoDecoder_Start_priv(decoder->decoder.zsp.decoder, &zspSstartSettings);
    UNLOCK_AUDIO();
    if(rc!=NEXUS_SUCCESS) {return BERR_TRACE(rc);}
#endif
...
static void NEXUS_VideoDecoder_P_Stop_Zsp( NEXUS_VideoDecoderHandle decoder)
{
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_P_Stop_Zsp: %#x", (unsigned)decoder));
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_Stop_priv(decoder->decoder.zsp.decoder);
    UNLOCK_AUDIO();
    NEXUS_VideoDecoder_P_Xdm_Stop(decoder);
    return;
}

static void NEXUS_VideoDecoder_P_Flush_Zsp( NEXUS_VideoDecoderHandle decoder)
{
    NEXUS_Error rc;
    ...
    NEXUS_VideoDecoder_P_Xdm_Stop(decoder);
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_Flush_priv(decoder->decoder.zsp.decoder);
    UNLOCK_AUDIO();
    rc = NEXUS_VideoDecoder_P_Xdm_Start(decoder);
...
static NEXUS_Error NEXUS_VideoDecoder_P_GetStatus_Zsp( NEXUS_VideoDecoderHandle decoder, NEXUS_VideoDecoderStatus *pStatus)
{
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
...
NEXUS_VideoDecoder_P_Open_Zsp( unsigned index, const NEXUS_VideoDecoderOpenSettings *pOpenSettings)
{
    NEXUS_VideoDecoderHandle decoder;
    NEXUS_VideoDecoderOpenMosaicSettings mosaicSettings;
    NEXUS_RaveOpenSettings raveSettings;
    NEXUS_ZspVideoDecoderOpenSettings  zspOpenSettings;
    const BXDM_Decoder_Interface *decoderInterface;
    void *decoderContext;
    NEXUS_Error rc;

    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_P_Open_Zsp: %u", index));
    if(index>=NEXUS_NUM_ZSP_VIDEO_DECODERS) { rc=BERR_TRACE(NEXUS_INVALID_PARAMETER); goto err_args;}
    ...
    BDBG_OBJECT_SET(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    decoder->interface = &NEXUS_VideoDecoder_P_Interface_Zsp;
    ...
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_GetRaveSettings_priv(&raveSettings);
    UNLOCK_AUDIO();
    ...
    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_GetDefaultOpenSettings_priv(&zspOpenSettings);
    decoder->decoder.zsp.decoder = NEXUS_ZspVideoDecoder_Open_priv(index, &zspOpenSettings);
    UNLOCK_AUDIO();
    if(!decoder->decoder.zsp.decoder) {rc=BERR_TRACE(NEXUS_NOT_SUPPORTED);goto error;} 

    LOCK_AUDIO();
    NEXUS_ZspVideoDecoder_GetDecoderInterface_priv(decoder->decoder.zsp.decoder, &decoderInterface, &decoderContext);
    UNLOCK_AUDIO();
#if NEXUS_VIDEO_DECODER_P_ZSP_DUMMY_DECODER
...
static void NEXUS_VideoDecoder_GetDisplayConnection_priv_Zsp( NEXUS_VideoDecoderHandle decoder, NEXUS_VideoDecoderDisplayConnection *pConnection)
{
    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_GetDisplayConnection_priv_Zsp: %#x", (unsigned)decoder));
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    NEXUS_ASSERT_MODULE();
...
static void NEXUS_VideoDecoder_GetSourceId_priv_Zsp( NEXUS_VideoDecoderHandle decoder, BAVC_SourceId *pSource)
{
    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_GetSourceId_priv_Zsp: %#x", (unsigned)decoder));
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    NEXUS_ASSERT_MODULE();
...
static NEXUS_Error NEXUS_VideoDecoder_P_SetTrickState_Zsp( NEXUS_VideoDecoderHandle decoder, const NEXUS_VideoDecoderTrickState *pState)
{
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
...
void NEXUS_VideoDecoder_P_IsCodecSupported_Zsp( NEXUS_VideoDecoderHandle decoder, NEXUS_VideoCodec codec, bool *pSupported )
{
    BDBG_OBJECT_ASSERT(decoder, NEXUS_VideoDecoder);
    BDBG_OBJECT_ASSERT(&decoder->decoder.zsp, NEXUS_VideoDecoder_Zsp);
    ...
    *pSupported = (codec == NEXUS_VideoCodec_eRv40);
    return;
}
...
const NEXUS_VideoDecoder_P_Interface NEXUS_VideoDecoder_P_Interface_Zsp = {
    NEXUS_VideoDecoder_P_Close_Zsp,
    NEXUS_VideoDecoder_P_GetOpenSettings_Common,
    NEXUS_VideoDecoder_P_GetSettings_Common,
    NEXUS_VideoDecoder_P_SetSettings_Zsp,
    NEXUS_VideoDecoder_P_Start_Zsp,
    NEXUS_VideoDecoder_P_Stop_Zsp,
    NEXUS_VideoDecoder_P_Flush_Zsp,
    NEXUS_VideoDecoder_P_GetStatus_Zsp,
    NEXUS_VideoDecoder_P_GetConnector_Common,
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_GetStreamInformation_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_SetStartPts_Zsp),
    NEXUS_VideoDecoder_P_IsCodecSupported_Zsp,
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_SetPowerState_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_Reset_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_GetExtendedStatus_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_GetExtendedSettings_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_SetExtendedSettings_Zsp),
    ...
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_StartPrimer_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_StopPrimer_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_StartDecodeWithPrimer_Zsp),
    NEXUS_VideoDecoder_P_GetTrickState_Common,
    NEXUS_VideoDecoder_P_SetTrickState_Zsp,
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_FrameAdvance_Zsp),
    NOT_SUPPORTED(NEXUS_VideoDecoder_P_GetNextPts_Zsp),
    NEXUS_VideoDecoder_P_GetPlaybackSettings_Common,
    NEXUS_VideoDecoder_P_SetPlaybackSettings_Common,
    NOT_SUPPORTED(NEXUS_StillDecoder_P_Open_Zsp),

#if NEXUS_HAS_ASTM
    NEXUS_VideoDecoder_GetAstmSettings_priv_Common,
    NOT_SUPPORTED(NEXUS_VideoDecoder_SetAstmSettings_priv_Zsp),
    NEXUS_VideoDecoder_GetAstmStatus_isr_Common,
#endif

    NEXUS_VideoDecoder_GetDisplayConnection_priv_Zsp,
    NEXUS_VideoDecoder_SetDisplayConnection_priv_Xdm,
    NEXUS_VideoDecoder_GetSourceId_priv_Zsp,
    NEXUS_VideoDecoder_GetHeap_priv_Common,
    NEXUS_VideoDecoder_GetSyncSettings_priv_Common,
    NOT_SUPPORTED(NEXUS_VideoDecoder_SetSyncSettings_priv_Zsp),
...
{
    dummyState *decoder = context;
    NEXUS_VideoDecoder_P_FrameElement_Dummy *frame;
    ...
    *pUnifiedPicture = &frame->picture;
    ...
    BDBG_MSG_TRACE(("NEXUS_VideoDecoder_P_Dummy_GetNextPicture_isr<:%#x %#x %#x[%u]", (unsigned)decoder, (unsigned)frame, (unsigned)frame->picture.stBufferInfo.pLumaBufferVirtualAddress, frame - decoder->frames));
    return BERR_SUCCESS;
}
...
static void NEXUS_VideoDecoder_P_Dummy_Init(void)
{
    ...
    BKNI_Memset(decoder->frameMemory, 0x80, bufferInfo.lumaSize);
    BKNI_Memset((uint8_t *)decoder->frameMemory + bufferInfo.lumaSize, 0x80, bufferInfo.chromaSize);
    ...
    for(i=0;i<NEXUS_VIDEO_DECODER_P_DUMMYDECODE_N_FRAMES;i++) {
        NEXUS_VideoDecoder_P_FrameInit_Dummy(&decoder->frames[i], decoder->frameMemory);
...
#endif /* NEXUS_VIDEO_DECODER_P_ZSP_DUMMY_DECODER */
