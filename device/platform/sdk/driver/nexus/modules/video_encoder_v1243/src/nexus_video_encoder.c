 ...
 * 
 * 67   1/31/13 2:02p vsilyaev
 * SW7425-4492: Added API to control max number of channels per device
 * 
 * 66   1/31/13 2:02p vsilyaev
 * SW7425-4489: Added target bitrate
 * 
 * 65   1/24/13 3:18p vsilyaev
 * SW7425-4356: Added numParallelEncodes field
 * 
 * 64   12/20/12 1:20p vsilyaev
 * SW7425-4230: Added firware version into the encoder status
 * 
 * 63   12/20/12 1:13p vsilyaev
 * SW7435-562: Added option to set heaps for payload and descriptors
 * 
 * 62   11/20/12 3:48p vsilyaev
 * SW7425-2697: Added handling for BVCE_Debug environment variable
 * 
 * 61   11/9/12 4:53p mphillip
 * SW7425-4156: Update VICE security support
 * 
 * 60   11/8/12 4:55p vsilyaev
 * SW7425-4186: Actually propagate events to application
 * 
 * 59   11/8/12 3:20p vsilyaev
 * SW7425-4186: Added Encoder EOS event
 * 
...
#if (defined(NEXUS_SECURITY_VICE_VERIFICATION) || defined (NEXUS_ENABLE_VICH))
...
NEXUS_VideoEncoderModule_Init(const NEXUS_VideoEncoderModuleSettings *pSettings)
{
    ...
    for(i=0;i<NEXUS_NUM_VCE_DEVICES;i++ ) {
...
#if (defined(NEXUS_SECURITY_VICE_VERIFICATION) || defined (NEXUS_ENABLE_VICH))
        {
            NEXUS_VideoEncoderSecureConfig config;

            config.hReg = g_pCoreHandles->reg;
            config.index = i;
            NEXUS_VideoEncoder_P_EnableSecurity (&vceSettings, &config);
        }
#endif
        ...
        if(rc!=BERR_SUCCESS) {rc=BERR_TRACE(rc);goto error;}
        {
            ...
            if(bvce_debug) {
                char arcInstance = bvce_debug[0];
                char bvce_debug_line[64];
                if(arcInstance>='0' && arcInstance<='9') {
                    unsigned i;
                    bvce_debug++;
                    /* convert from "ItbStcOn_DbgXXX" to "ItbStcOn\nDbgXXX\n\n" */
                    for(i=0;i<sizeof(bvce_debug_line)/sizeof(*bvce_debug_line)-3;i++) {
                    ...
                    rc = BVCE_Debug_SendCommand(device->vce, (BVCE_ArcInstance)arcInstance, bvce_debug_line);
...
#ifdef NEXUS_SECURITY_VICE_VERIFICATION
...
NEXUS_VideoEncoder_Open(unsigned index, const NEXUS_VideoEncoderOpenSettings *pSettings)
{
    ...
    if (heap) {
		outputSettings.hDescriptorMem = NEXUS_Heap_GetMemHandle(heap);
    }
    ...
    heap = NEXUS_P_DefaultHeap(pSettings->index.heap, NEXUS_DefaultHeapType_eFull);
    if (heap) {
		outputBuffers.hITBMem = NEXUS_Heap_GetMemHandle(heap);
    }
    heap = NEXUS_P_DefaultHeap(pSettings->data.heap, NEXUS_DefaultHeapType_eFull);
    if (heap) {
		outputBuffers.hCDBMem = NEXUS_Heap_GetMemHandle(heap);
    }
    ...
    encoder->settings.bitrateTarget = encodeSettings.stBitRate.uiTarget;
