...
* Revision History:
 *
 * $brcm_Log: /nexus/platforms/97125/src/nexus_platform_pinmux.c $
 * 
 * 26   2/23/11 2:27p mward
 * SW7125-817:  Preserve expected old XVD UART functionality.  Route
 *  BNM_SPI2 to the pins only if BNM_SPI2 is defined in run-time
 *  environment.
 * 
 * 25   2/2/11 9:03a mward
 * SW7125-817:  Change pinmux to allow for use of the SPI2 on the BNM as
 *  well as the PCM.
 * 
 * 24   1/20/11 10:33a agin
 * SWNOOS-429:  Support UARTC for no-os.
 * 
 * 23   11/12/10 9:10a mward
 * SW7125-729: Fix inverted logic in
 *  NEXUS_Platform_P_IsHostFrontendPinmux().
 * 
 * 22   11/10/10 5:02p mward
 * SW7125-729: Add NEXUS_Platform_P_IsHostFrontendPinmux() to check the
 *  state of I2C control.
 * 
 * 21   6/21/10 10:27a mward
 * SW7125-35:  Switch between IRQ0/1 to host and BNM.  Don't pull down the
 *  IRQ0/1 pins.
 * 
 * 20   6/11/10 3:40p mward
 ...
 *  and BNM.
 * 
 * 19   6/9/10 1:18p mward
 * SW7125-470:  Fixed misplaced line.
 * 
 * 18   6/8/10 11:27a mward
 * SW7125-470:  Default RAP audio decoder UART to UART2.
 * 
 * 17   5/13/10 1:57p mward
 * SW7125-176: Pinmux change for UART1 in C0.  Fix AVD OL UART output on
 *  C0.
 * 
 * 16   5/6/10 2:35p mward
 * SW7125-389: Leave gpio_26, gpio_27 alone to be set by CFE for BNM.
 * 
 * 15   3/2/10 10:33a mward
 * SW7125-176: BCHP_VER_C0, not BCHP_REV_C0.
 * 
 * 13   3/1/10 11:16a mward
 * SW7125-176: Set gpio_32 to high to activate Bluetooth module.
 * 
 * 12   1/29/10 5:39p mward
 * SW7125-176: 7125C0 has no gpio_32_pad_ctrl.
 * 
 * 11   12/22/09 5:43p mward
 * SW7125-150: Set gpio_32 to PULL_UP to activate Bluetooth module.
 * 
 * 10   11/30/09 6:42p mward
 * SW7125-131: Select pkt1 pins, instead of GPIOs.
 * 
 * 9   10/29/09 7:01p mward
 * SW7125-39: Eliminate additional pinmux settings which will be set by
 *  the MPOD driver.
 * 
 * 8   10/27/09 5:00p mward
 * SW7125-39: Correct pinmux for MPOD.
 * 
 * 7   10/14/09 2:20p mward
 * SW7125-35:  Support control of BCM3112's by host.
 * 
 * 6   9/18/09 10:36a mward
...
NEXUS_Error NEXUS_Platform_P_InitPinmux(void)
{
	...
	reg = BREG_Read32(hReg,BCHP_SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_3);
	reg &= ~(
	...
	;
	BREG_Write32(hReg,BCHP_SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_3,reg);
	#else
	reg = BREG_Read32(hReg,BCHP_SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_4);
	reg &= ~(
	...
	;
	BREG_Write32(hReg,BCHP_SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_4,reg);
	#endif
	...
	#if ((BCHP_CHIP==7125) && (BCHP_VER<BCHP_VER_C0))
	{
		...
		reg &= ~(BCHP_MASK(SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_7 , gpio_32_pad_ctrl ));
		reg |= BCHP_FIELD_DATA(SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_7 , gpio_32_pad_ctrl, 2 );   /* PULL_UP (BT enable) */
		BREG_Write32(hReg,BCHP_SUN_TOP_CTRL_PIN_MUX_PAD_CTRL_7,reg);
	}
	#elif ((BCHP_CHIP==7125) && (BCHP_VER>=BCHP_VER_C0))
	{
		reg = BREG_Read32(hReg,BCHP_GIO_IODIR_HI);
		reg &= ~(1);	/* Set bit 0 = 0, gpio_32 = output */
		BREG_Write32(hReg,BCHP_GIO_IODIR_HI,reg);
		reg = BREG_Read32(hReg,BCHP_GIO_DATA_HI);
		reg |= 1;	/* Set bit 0 = 1, gpio_32 = high (BT enable) */
		BREG_Write32(hReg,BCHP_GIO_DATA_HI,reg);
	}
	#endif
	...
	reg &= ~(BCHP_MASK(SUN_TOP_CTRL_UART_ROUTER_SEL, port_2_cpu_sel));
	reg |= BCHP_FIELD_ENUM(SUN_TOP_CTRL_UART_ROUTER_SEL, port_2_cpu_sel, AUDIO_ZSP);
	#endif
