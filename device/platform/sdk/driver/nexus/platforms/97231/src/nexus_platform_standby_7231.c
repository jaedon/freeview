 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: nexus_platform_standby_7231.c $
 * $brcm_Revision: 5 $
 * $brcm_Date: 1/30/12 3:47p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /nexus/platforms/97231/src/nexus_platform_standby_7231.c $
 * 
 * 5   1/30/12 3:47p gmohile
 * SW7231-630 : Add nmi wakeup
 * 
 * 4   10/18/11 3:21p gmohile
 * SW7425-1183 : Add xpt packet wakeup support
 * 
 * 3   8/23/11 3:06p gmohile
 * SW7231-345 : Fix wake up devices
 * 
 * 2   5/16/11 3:53p gmohile
 * SW7231-128 : Add wake up devce support
 * 
 *
 *****************************************************************************/
...
BDBG_MODULE(nexus_platform_standby_7420);
...
{     
    ...
    /* Set nmi for wakeup */
    if(pSettings->wakeupSettings.nmi) {
	val |= BCHP_AON_PM_L2_CPU_MASK_CLEAR_NMI_B_INTR_MASK;
    }
...
void NEXUS_Platform_P_GetStandbyStatus(NEXUS_PlatformStandbyStatus *pStatus)
{
    ...
    if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_STATUS_NMI_B_INTR_MASK) {
	pStatus->wakeupStatus.nmi = true;
    }
