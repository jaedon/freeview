 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: nexus_platform_standby_7346.c $
 * $brcm_Revision: 5 $
 * $brcm_Date: 3/15/12 4:03p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /nexus/platforms/97346/src/nexus_platform_standby_7346.c $
 * 
 * 5   3/15/12 4:03p randyjew
 * SW7418-73: Workaround, disable KBD3 Seijin device.
 * 
 * 4   10/10/11 9:50a gmohile
 * SW7425-1183 : Add support for transport packet wakeup
 * 
 * 3   10/4/11 11:33a gmohile
 * SW7344-104 : Fix wakeup devices
 * 
 * 2   6/22/11 10:54a randyjew
 * SW7344-104:Update for PM2.0
 * 
 * 1   12/8/10 10:39a jrubio
 * SW7344-9: add initial file
 * 
 * 1   12/2/10 6:07p jrubio
 * SW7346-9: add initial standby file
 * 
 *
 *****************************************************************************/
...
BDBG_MODULE(nexus_platform_standby_7346);
...
{     
	...
	val |= BCHP_AON_PM_L2_CPU_MASK_CLEAR_UHFR_MASK;
    }	
	...
	/* Temporary WorkAround for random Seijin IR wakeups. Turn off Seijin device_en */
	val = BREG_Read32(g_pCoreHandles->reg, BCHP_KBD3_CMD);
    val &= ~(0x2); /*Seijin - device_en[1]*/
	BREG_Write32(g_pCoreHandles->reg, BCHP_KBD3_CMD, val);
...
void NEXUS_Platform_P_GetStandbyStatus(NEXUS_PlatformStandbyStatus *pStatus)
{
    ...
    if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_MASK_CLEAR_UHFR_MASK) {
