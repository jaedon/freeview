 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: nexus_platform_standby_7429.c $
 * $brcm_Revision: 1 $
 * $brcm_Date: 8/9/11 8:01p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /nexus/platforms/97429/src/nexus_platform_standby_7429.c $
 * 
...
void NEXUS_Platform_P_GetStandbyStatus(NEXUS_PlatformStandbyStatus *pStatus)
{
    ...
    else if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_MASK_CLEAR_KPD_INTR_MASK) {
	pStatus->wakeupStatus.keypad = true;
    }
    else if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_MASK_CLEAR_GPIO_MASK) {
	pStatus->wakeupStatus.gpio = true;
    }
    else if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_MASK_CLEAR_CEC_INTR_MASK) {
	pStatus->wakeupStatus.cec = true;
    }
    else if (l2CpuIntStatus & BCHP_AON_PM_L2_CPU_STATUS_TIMER_INTR_MASK) {
