 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/appframework/src/linuxclient/client.c $
 * 
...
/* globals */
static BEMU_Client_MutexHandle g_hRegMutex;
static BEMU_Client_MutexHandle g_hMemMutex;
	...
	( const char *hostname )
{
	...
	dest_addr.sin_port = htons(EMU_PORT);	 /* short, network byte order */
...
int BEMU_Client_Open
	( void )
{
	if (BEMU_Client_CreateMutex(&g_hRegMutex) != 0)
		return 1;
	
	if (BEMU_Client_CreateMutex(&g_hMemMutex) != 0)
	{
		BEMU_Client_DestroyMutex(g_hRegMutex);
		return 1;
	}
	...
	( void )
{
	...
	BEMU_Client_DestroyMutex(g_hRegMutex);
	BEMU_Client_DestroyMutex(g_hMemMutex);
	  ...
	  uint32_t *pData )
{
	...
	if (0 != BEMU_Client_AcquireMutex(g_hRegMutex))		
	{
	...
	*pData =
	...
	BEMU_Client_ReleaseMutex(g_hRegMutex);
	  ...
	  size_t size )
{
	...
	if (0 != BEMU_Client_AcquireMutex(g_hMemMutex)) 	
	{
	...
	BEMU_Client_ReleaseMutex(g_hMemMutex);
	return iReturn;
}
