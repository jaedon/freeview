...
* Revision History:
*
* $brcm_Log: /rockford/appframework/src/linuxuser/intmgr.c $
* 
* Hydra_Software_Devel/17   9/14/12 10:18a pblanco
   ...
   while(1)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            {
               /* done */
               BDBG_MSG(("stop thread: bottom"));
               break;
            }

            if( gInterruptControlStruct.IntrW0Status || 
                gInterruptControlStruct.IntrW1Status || 
                gInterruptControlStruct.IntrW2Status )
            {
               /* use critical section to protect isr */
...
static int intmgr_Start(void)
{
   ...
   if (gTaskId)
   {
      BDBG_MSG(("Interrupt manager already running"));
      return -1;
   }
...
int intmgr_DebugLevelSet(int level)
{
   ...
   if ( gInitialized )
   {
      ...
      if ( ret != 0)
      {
         printf("intmgr_DebugLevelSet() ioctl  BRCM_IOCTL_DEBUG_LEVEL failed %d", ret);
      }
   }
   return ret;
#endif    
