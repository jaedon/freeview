 ...
 * $brcm_Date: 10/31/12 6:43p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /rockford/applications/nocs3/api/csd/7231/csd.c $
 * 
 * Hydra_Software_Devel/4   10/31/12 6:43p zqian
 * SW7346-555: fix csdGetNuid bug
 * 
 * Hydra_Software_Devel/2   7/5/12 6:51p yili
 * SW7346-555:Nocs3 integration
 * 
 * Hydra_Software_Devel/1   3/31/12 5:18p yili
 * SW7346-555:NOC3 certification
 * 
 * Hydra_Software_Devel/9   2/10/12 3:27p yili
 * SW7346-555:Code cleanup
 * 
 * Hydra_Software_Devel/8   2/7/12 5:35p yili
 * SW7346-555:Code clean up
 * 
 * Hydra_Software_Devel/7   1/27/12 2:00p yili
 * SW7346-555:Feature enhancement.
 * 
 * Hydra_Software_Devel/6   1/12/12 4:03p yili
 * SW7346-555:Feature Enhancement
 * 
 * Hydra_Software_Devel/5   12/7/11 3:17p yili
 * SW7346-555:Feature enhancement
 * 
 * Hydra_Software_Devel/4   12/6/11 5:59p yili
 * SW7346-555:Feature Enhancement
 * 
 * Hydra_Software_Devel/3   12/2/11 3:29p yili
 * SW7346-555:Feature Enhancement
 * 
 * Hydra_Software_Devel/2   11/22/11 4:50p yili
 * SW7346-555:NOCS3 certification
 * 
 * Hydra_Software_Devel/1   11/14/11 4:48p yili
 * SW7346-555:NOCS3 implementation
 * 
...
#else
	sprintf(xSoftwareVersion, "v%01x.%01x.%01x", CSDAPI_VERSION_MAJOR, CSDAPI_VERSION_MEDIUM, CSDAPI_VERSION_MINOR);
#endif
...
#define  CSD_DSC_CONFIG_ALGORITHM(xClearTextDscOddHostKeySize,xClearTextDscEvenHostKeySize)\
	keyHandle = NEXUS_Security_LocateCaKeySlotAssigned(pxDscKeyPathHandle->pidchannel);\
	if (!keyHandle)\
	{	\
...
)
{
	...
	/* Config Algorithm */
	CSD_DSC_CONFIG_ALGORITHM(xClearTextDscOddHostKeySize,xClearTextDscEvenHostKeySize);
...
)
{
	...
	/* Config Algorithm */
	CSD_DSC_CONFIG_ALGORITHM(xCipheredDscOddContentKeySize,xCipheredDscEvenContentKeySize);
...
#define CSD_SCR_VALIDATE_PARAMS \
	if((xEmi != CSD_Algorithm_AES_128_CBC_IV_0_SCTE52) &&\
		...
		(xEmi != CSD_Algorithm_RAW_AES_128_COUNTER_CLEAR) &&\
	...
	pxScrKeyPathHandle->bCounterMode = false;
...
#define CSD_SCR_PARSE_EMI_ALGORITHMS \
	...
	{\
		case CSD_Algorithm_AES_128_CBC_IV_0_SCTE52:\
			...
			algorithm.solitarySelect = NEXUS_SecuritySolitarySelect_eCbcXorIV;\
		...
		case CSD_Algorithm_RAW_0x4024:\
			...
			pxScrKeyPathHandle->ivSize = xInitializationVectorSize;\
			pxScrKeyPathHandle->bCounterMode = true;\
			pxScrKeyPathHandle->dataFormat = NEXUS_DmaDataFormat_eBlock;\
			break;\
...
{
	unsigned int status_reg, bond_option;
...
)
{
	...
	encryptedSessionkey.operationKey2 	= NEXUS_SecurityOperation_eEncrypt;
	encryptedSessionkey.bASKMMode 	= true;
#if 0 /* HUMAX_PLATFORM_BASE redundant with hdcp key used by VKL1*/
			...
			{\
				...
				{\
					...
					case CSD_R2R_CRYPTO_OPERATION_MODE_CBC:\
						xEmi = CSD_Algorithm_RAW_AES_128_CBC_CLEAR;\
						break;\
					case CSD_R2R_CRYPTO_OPERATION_MODE_CTR:\
						xEmi = CSD_Algorithm_RAW_AES_128_COUNTER_CLEAR;\
						break;\
					default:\
