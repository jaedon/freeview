 ...
 * Revision History:
 * 
 * $brcm_Log: /rockford/applications/nocs3/harness_test/cert/nocs_test.c $
 * 
...
#include "bcm7358_Test_Vectors_1_x.h"
#include "nexus_dma.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
	...
	{	
		...
		switch(select_test)
		{
			...
			case 2:
				...
				break;
			case 3:
				printf("DVB CSAv3 not supported \n");
				break;
			case 4:
...
{    
	...
	g_numchld = 0;
	opterr = 0; /* let us handle bad parameters */
	/* The ':' after the option specifies it requires an input.  For
		this test, the -i, -o, -i, -b require input arguments. */
	...
	{
		fprintf(stderr, "The current versio fo the CERT IP is: %x\n", CERT_BRIDGE_VERSION);
	}
	else if(strcmp(argv[1], "-bridge") == 0x0)
	{
		...
		if(filemode) 
		{
			...
			{
				g_infd = STDIN_FILENO;
				syslog(LOG_DEBUG, "infile set to stdin");			
			}
			
			if(outfile != NULL) 
			{
				g_outfd = open(outfile, O_CREAT|O_WRONLY);
				if( g_outfd < 0) 
				{
					perror(outfile);
					return -2;
				}
				syslog(LOG_DEBUG, "outfile set to: %s", outfile);			
			} 
			else 
			{
				g_outfd = STDOUT_FILENO;
				syslog(LOG_DEBUG, "outfile set to stdout");			
			}
