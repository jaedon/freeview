 ...
 * $brcm_Date: 9/23/08 6:29p $
 *
 * Module Description: Clipping private functions.
 *
 * Revision History:
 *
 * $brcm_Log: /rockford/middleware/opengles/gl_clip.c $
 * 
	...
	BP3D_Vertex *pV0 )
{
	...
	BP3D_DrawPoint( pDisplay->hP3d, &v );
}
	...
	if( pContext->ulVertexOutCode )
	{
		for( ii = 0; ii < iVertexCount; ++ii )
		{
			...
			if( GL_P_GET_OUTCODE(pV0) == 0 )
				GL_P_RenderPoint( pDisplay, pContext, pV0 );
		}
	}
	else
	{
		for( ii = 0; ii < iVertexCount; ++ii )
			BP3D_DrawPoint( pDisplay->hP3d, pContext->pVertices + ii );
	}
}
	...
	BP3D_Vertex *pV1 )
{
	...
	BP3D_DrawLine( pDisplay->hP3d, v, v + 1 );
}
	...
	BP3D_Vertex *pV1 )
{
	...
	for( ii = 0; ii < iPingCount - 1; ++ii )
		BP3D_DrawLine( pDisplay->hP3d, &aVerticesOut[0], &aVerticesOut[ii + 1] );
}
	...
	GLsizei iVertexCount )
{
		...
		for( ii = 0; ii < iVertexCount - 1; ++ii )
		{
			...
			if( (ulOutCode0 & ulOutCode1) == 0 )
			{
				if( ulOutCode0 | ulOutCode1 )
					GL_P_ClipAndRenderLine( pDisplay, pContext, pV0, pV1 );
				else
					GL_P_RenderLine( pDisplay, pContext, pV0, pV1 );
			}
		}
	}
	else
	{
		for( ii = 0; ii < iVertexCount - 1; ++ii )
			BP3D_DrawLine( pDisplay->hP3d, pContext->pVertices + ii, pContext->pVertices + ii + 1 );
	}
}
	...
	GLsizei iVertexCount )
{
	...
	{
		for( ii = 0; ii < iVertexCount; ++ii )
			BP3D_DrawLine( pDisplay->hP3d, pContext->pVertices + ii, 
	...
	GLsizei iVertexCount )
{
	...
	{
		for( ii = 0; ii < iVertexCount / 2; ++ii )
			BP3D_DrawLine( pDisplay->hP3d, pContext->pVertices + ii * 2, pContext->pVertices + ii * 2 + 1 );
	}
}
	...
	BP3D_Vertex *pV2 )
{
	...
	BP3D_DrawTriangle( pDisplay->hP3d, v, v + 1, v + 2 );
}
	...
	BP3D_Vertex *pV2 )
{
	...
	BP3D_DrawTriangleFan( pDisplay->hP3d, aVerticesOut, iPingCount - 2 );
}
		...
		for( ii = 0; ii < iVertexCount - 2; ++ii )
		{
			...
			if( (ulOutCode0 & ulOutCode1 & ulOutCode2) == 0 )
			{
				if( ulOutCode0 | ulOutCode1 | ulOutCode2 )
					GL_P_ClipAndRenderTriangle( pDisplay, pContext, pV0, pV1, pV2 );
				else
					GL_P_RenderTriangle( pDisplay, pContext, pV0, pV1, pV2 );
			}
		}
	}
	else
	{
		BP3D_DrawTriangleStrip( pDisplay->hP3d, pContext->pVertices, iVertexCount - 2 );
	}
}
	...
	GLsizei iVertexCount )
{
	...
	{
		BP3D_DrawTriangleFan( pDisplay->hP3d, pContext->pVertices, iVertexCount - 2 );
	}
}
	...
	GLsizei iVertexCount )
{
	...
	{
		BP3D_DrawTriangleList( pDisplay->hP3d, pContext->pVertices, iVertexCount / 3 );
	}
}
	...
	BP3D_Vertex *pV1 )
{
	...
	GL_P_TransformVertex( pContext, v + 0, pV0 );
	GL_P_TransformVertex( pContext, v + 1, pV1 );
	BP3D_DrawRectangle( pDisplay->hP3d, v, v + 1 );
}
	...
	BP3D_Vertex *pV1 )
{
	...
	BP3D_DrawRectangle( pDisplay->hP3d, &v0, &v1 );
}
	...
	GLsizei iVertexCount )
{
	...
	if( pContext->bRectangleBlit2D )
	{
		for( ii = 0; ii < iVertexCount / 2; ++ii )
		{
			...
			GL_P_BlitRectangle( pDisplay, pContext, pV0, pV1, ii );
		}
	...
	if( pContext->ulVertexOutCode )
	{
		for( ii = 0; ii < iVertexCount / 2; ++ii )
		{
			...
			{
				if( ulOutCode0 | ulOutCode1 )
					GL_P_ClipAndRenderRectangle( pDisplay, pContext, pV0, pV1 );
				else
					GL_P_RenderRectangle( pDisplay, pContext, pV0, pV1 );
			}
		}
	}
	else
	{
		for( ii = 0; ii < iVertexCount / 2; ++ii )
		{
			BP3D_Vertex *pV0 = pContext->pVertices + ii * 2;
			BP3D_Vertex *pV1 = pContext->pVertices + ii * 2 + 1;
			BP3D_DrawRectangle( pDisplay->hP3d, pV0, pV1 );
		}
	}
}
