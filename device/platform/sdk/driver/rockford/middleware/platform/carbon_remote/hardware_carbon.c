...
static bool DriverAcquire(void *context, const BEGL_HWAcquireSettings *settings)
{
   ...
   if (data->cm == NULL)
   {
      ...
      data->cm = carbon_v3d_open(settings->mainISR, carbon_readmem, carbon_writemem);
#else
      data->cm = carbon_v3d_open(settings->mainISR, NULL, NULL);
#endif
...
/* Release 3D hardware*/
static bool DriverRelease(void *context)
{
   return true;
}
