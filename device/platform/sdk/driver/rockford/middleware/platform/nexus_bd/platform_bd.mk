...
CFLAGS += \
	...
	-I$(NEXUS_TOP)/../ndvd/stage/usr/local/include/settop \
	-I$(NEXUS_TOP)/../ndvd/stage/usr/local/include/common \
	-I$(NEXUS_TOP)/../ndvd/stage/usr/local/include/base \
	-I$(NEXUS_TOP)/../ndvd/stage/usr/local/include \
	-I$(NEXUS_TOP)/../rockford/middleware/v3d_b0/interface/khronos/include/EGL \
...
# Null display
SOURCES += ../nexus/null_display_nexus.c
...
SOURCES += ../nexus/exclusive_display_nexus.c

endif # NULL_DISPLAY

ifeq ($(BUILD_DYNAMIC),1)
all: $(LIBDIR)/libbdpl.so
else
all: $(LIBDIR)/libbdpl.a
endif
...
$(LIBDIR)/libbdpl.so: $(PRE_BUILD_RULES) V3DDriver $(OBJS)
	$(Q)echo Linking ... libbdpl.so
	$(Q)mkdir -p $(LIBDIR)
	$(Q)$(CC) $(LDFLAGS) -shared -o $(LIBDIR)/libbdpl.so $(OBJS)
	$(Q)cp $(LIBDIR)/libbdpl.so $(NEXUS_BIN_DIR)

$(LIBDIR)/libbdpl.a: V3DDriver $(PRE_BUILD_RULES) $(OBJS)
	$(Q)echo Archiving ... libbdpl.a
	$(Q)mkdir -p $(LIBDIR)
	$(Q)ar -rcs $(LIBDIR)/libbdpl.a $(OBJS)

# clean out the dross
clean:
	$(Q)rm -f $(LIBDIR)/libbdpl.so *~ $(OBJS)
	$(Q)rm -f $(LIBDIR)/libbdpl.a
	$(Q)rm -f $(OBJDIR)/*.d
