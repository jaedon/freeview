...
uint64_t khrn_worker_exit_pos;
VCOS_MUTEX_T worker_protector;
...
#ifdef __mips__
/* always align to 4 bytes anyway */
static uint8_t msgs[MSGS_SIZE];
#else
...
bool khrn_worker_init(void)
{
   ...
   khrn_worker_exit_pos = 0;
   vcos_mutex_create(&worker_protector, "worker_protector");
...
void khrn_worker_term(void)
{
   ...
   vcos_event_delete(&event);
#endif
   vcos_mutex_delete(&worker_protector);
