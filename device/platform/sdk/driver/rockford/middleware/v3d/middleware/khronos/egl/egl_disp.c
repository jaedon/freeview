...
#define DISPS_N_MAX 32
/* currently limited to 32 because we map the disps to bits in a uint32_t in a
 * couple of places. could be changed pretty easily if needed... */
...
static void finish(DISP_T *disp)
{
   ...
   while (disp->in_use.finish && res == true) {
      /* Timeout after 2s */
      res = khrn_sync_master_wait_timeout(2000);
   }
   khrn_barrier();
}
...
void egl_disp_free(EGL_DISP_HANDLE_T disp_handle)
{
   ...
   if (disp->in_use.last_win != EGL_PLATFORM_WIN_NONE) {
      ...
      BEGL_WindowState *state = NULL;
      for (i = 0; i != SLOTS_N; ++i) {
         if (disp->in_use.slots[i].win == disp->in_use.last_win) {
            state = disp->in_use.slots[i].image.window_state;
            vcos_assert(state->window == (BEGL_WindowHandle)disp->in_use.last_win);
            break;
         }
      }
      vcos_assert(state != NULL);
      egl_server_platform_display_nothing_sync(disp->in_use.last_win, state);
   }
