...
void BEGL_RegisterDriverInterfaces(BEGL_DriverInterfaces *driverInterfaces)
{
   bool bad = false;
   bool term = false;
   ...
   else
   {
      ...
      {
         /* Must have these */
         if (driverInterfaces->displayInterface->BufferCreate == NULL ||
...
BEGL_DriverInterfaces * BEGL_GetDriverInterfaces(void)
{
   return s_driverInterfaces;
}

extern void vsync_callback(uint32_t);
extern BEGL_BufferHandle BEGLint_PixmapCreateCompatiblePixmap(BEGL_PixmapInfo *pixmapInfo);
extern void BEGLint_BufferGetRequirements(BEGL_PixmapInfo *bufferRequirements, BEGL_BufferSettings * bufferConstrainedRequirements);

void BEGL_GetDefaultDriverInterfaces(BEGL_DriverInterfaces *driverInterfaces)
{
   memset(driverInterfaces, 0, sizeof(BEGL_DriverInterfaces));

   driverInterfaces->displayCallbacks.BufferOnDisplay = vsync_callback;
   driverInterfaces->displayCallbacks.PixmapCreateCompatiblePixmap = BEGLint_PixmapCreateCompatiblePixmap;
   driverInterfaces->displayCallbacks.BufferGetRequirements = BEGLint_BufferGetRequirements;
}
...
void egl_server_surface_term(void *v, uint32_t size)
{
   ...
   for (i = 0; i < EGL_MAX_BUFFERS; i++)
   {
      ...
      {
         KHRN_IMAGE_T *image = mem_lock(surface->mh_color[i]);
         if (image->window_state && image->window_state->window == (BEGL_WindowHandle)surface->win)
         {
   ...
   if (windowState != NULL)
   {
      ...
      if (driverInterfaces != NULL && driverInterfaces->displayInterface->WindowPlatformStateDestroy != NULL)
      {
         driverInterfaces->displayInterface->WindowPlatformStateDestroy(windowState->platformState);
      }
   ...
   uint32_t type)
{
   ...
   if (hpixmapimage != MEM_INVALID_HANDLE) {
      ...
      if (type == WINDOW)
         platformState = egl_server_platform_create_window_state(win);
         ...
         if (i == 0 && vgimagehandle != MEM_INVALID_HANDLE) {
            ...
            else
            {
               ...
               if (platform_supported_format(final_format))
               {
                  ...
                  {
#ifdef ANDROID
                     LOGD("%s: swapchain_count = %d, type = %d\n", __FUNCTION__, swapchainc, type);
#endif
                                 ...
                                 uint32_t config_stencil_bits)
{
   if (handle != MEM_INVALID_HANDLE) {
      ...
      case EGL_SERVER_GL20:
      {
         ...
         glxx_server_state_set_buffers(state, draw, swapchainc, read, depth, multi);
...
void eglIntCheckCurrent_impl(uint32_t pid_0, uint32_t pid_1)
{
   ...
   if(state->vgcontext!=MEM_INVALID_HANDLE) {
      VG_SERVER_STATE_T *vgstate = mem_lock(state->vgcontext);
...
void eglIntSwapBuffers_impl(EGL_SURFACE_ID_T s, uint32_t width, uint32_t height, uint32_t swapchainc, uint32_t handle, uint32_t preserve, uint32_t position)
{
   ...
   /* We do not allow anything other than swapInterval==1 for external swapChains */
   if (swapchainc > 0)
   {
      KHRN_IMAGE_T *currentImage;
      swapIntervalAllowed = false;

      currentImage = (KHRN_IMAGE_T *)mem_lock(surface->mh_color[surface->back_buffer_index]);
   ...
   egl_disp_next(surface->disp_handle, surface->mh_color[surface->back_buffer_index],
      surface->win, swapIntervalAllowed ? surface->swap_interval : 0);
   ...
   */
   ...
   /* on an abstract platform, the buffers might be provided from the underlying windowing system.
      Don't provide any resize in this case, as its not appropriate.  It's up to the window manager to do it */

   /* It's also down to the underlying system to preserve */
   if (swapchainc == 0)
   {
      ...
      /* resize the depth and multisample seperatly, as the color buffer could have dynamically changed without resize being called */
      /* In the case of the buffers being owned by the platform (ANDROID / DFB) do this in install framebuffer where the size is known */
      if (surface->mh_depth) {
         int scale = surface->mh_multi ? 2 : 1;
         KHRN_IMAGE_T *depth = (KHRN_IMAGE_T *)mem_lock(surface->mh_depth);
         if (depth->width != (width*scale) || depth->height != (height*scale))
         {
      ...
      if (surface->mh_multi) {
         ...
         if (multi->width != (width*2) || multi->height != (height*2))
         {
...
EGL_SYNC_ID_T eglIntCreateSync_impl(uint32_t type, uint32_t condition, int32_t threshold, uint32_t sem)
{
...
static void sync_update(int32_t type)
{
   EGL_SERVER_STATE_T *state = EGL_GET_SERVER_STATE();

   if (state->syncs.entries) {
      khrn_map_iterate(&state->syncs, egl_khr_sync_callback, &type);
   }
}
...
void egl_khr_fence_update()
{
   sync_update(EGL_SYNC_FENCE_KHR);
}
   ...
   uint32_t buffer_stride)
{
   ...
   whandle = mem_wrap((void *)buffer, buffer_height * buffer_stride, 1, 0, "EGL_IMAGE_WRAP_BRCM");
