...
void eglFillGlobalImageBRCM_impl(EGLint id_0, EGLint id_1, EGLint y, EGLint height, const void *data, EGLint data_stride, EGLint data_pixel_format)
{
   ...
   */
   ...
   khrn_image_lock_interlock_wrap(image, &dst, NULL);
   /* its type is marked as 0, which means no conversion.  The caller can control the orientation of the image */
   khrn_image_interlock_wrap(&src, get_fill_format(data_pixel_format), image->width, height, data_stride, 0, (void *)data, NULL); /* casting away const, but not going to modify */
