...
static GLXX_FRAGMENT_CARD_T *fshader(GL11_CACHE_KEY_T *v)
{
   ...
   card->root = glxx_backend(v->common.blend, color, discard, v->common.stencil_config, v->common.use_depth, v->common.render_alpha, v->common.rgb565, false);
...
bool gl11_hw_emit_shaders(GL11_CACHE_KEY_T *v, GLXX_LINK_RESULT_DATA_T *data, uint32_t *color_varyings)
{
   ...
   *color_varyings = 0;
   ...
   result &= glxx_schedule(last_vpm_write, GLSL_BACKEND_TYPE_COORD, &data->mh_ccode, &data->mh_cuniform_map, NULL, NULL, NULL, v->common.fb_rb_swap);
   ...
   glxx_iodep(last_vpm_write, vcard->force_vpm_read);
   result &= glxx_schedule(last_vpm_write, GLSL_BACKEND_TYPE_VERTEX, &data->mh_vcode, &data->mh_vuniform_map, NULL, NULL, NULL, v->common.fb_rb_swap);
