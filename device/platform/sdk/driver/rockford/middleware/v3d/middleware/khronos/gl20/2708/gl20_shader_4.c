...
static bool gl20_hw_emit_shaders(GL20_LINK_RESULT_T *link_result, GLXX_LINK_RESULT_KEY_T *key, GLXX_LINK_RESULT_DATA_T *data, void *base)
{
   ...
   root = glxx_backend(key->blend, glxx_vec4(frag_color[0], frag_color[1], frag_color[2], frag_color[3]), frag_color[4], key->stencil_config, key->use_depth, key->render_alpha, key->rgb565, boilerplate);
   glxx_iodep(root, dep);
   data->threaded = true;
...
#endif
   {
      ...
      glxx_iodep(last_vpm_write, last_vpm_read);
      result &= glxx_schedule(last_vpm_write, GLSL_BACKEND_TYPE_COORD, &data->mh_ccode, &data->mh_cuniform_map, NULL, NULL, NULL, key->fb_rb_swap);
      ...
      glxx_iodep(last_vpm_write, last_vpm_read);
      result &= glxx_schedule(last_vpm_write, GLSL_BACKEND_TYPE_VERTEX, &data->mh_vcode, &data->mh_vuniform_map, NULL, NULL, NULL, key->fb_rb_swap);
