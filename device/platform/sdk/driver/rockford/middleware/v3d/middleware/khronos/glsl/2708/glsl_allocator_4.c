...
void (*line_capture_fragment)(char * str) = NULL;
void (*line_capture_vertex)(char * str) = NULL;

KHAPI void glsl_set_line_capture_fragment(void(*p)(char *))
{
   line_capture_fragment = p;
}

KHAPI void glsl_set_line_capture_vertex(void(*p)(char *))
{
   line_capture_vertex = p;
}
...
static uint32_t instr_add_sig(INSTR_SIG_T sig, OP_SRC_T *deps, uint32_t dep_count)
{
   ...
   if (khrn_workarounds.SBWAIT)
   {
      ...
      if (sig == SIG_SB_WAIT && first_timestamp == 0) 
         first_timestamp = 1;
   }
   else
   {
      /* in the case of >= B2 rev, the sbwait is implicit in the loadc, tlbc.... destination, it therefor
         wont be in the first instruction (as this is a nop in the very short shader case) */
   }
...
//#define KHRN_FORCE_SINGLE_TEXTURE_UNIT /* for performance simulation only */
static uint32_t generate(uint32_t *output)
{
   uint32_t i;
   ...
   *(output++) = 0x009e7000; *(output++) = 0x100009e7; /* nop */
   ...
   if (addThrendSBDone)
   {
      ...
      bool implied_scoreboard = false;
      INSTR_STRUCT_T *instr;
      uint32_t i, j;

      /* assess suitability */
      i = ((int)last_used_timestamp - 3) > 0 ? (last_used_timestamp - 3) : 0;
      while (1)
      {
         bool bad = false;
         i++;
      ...
      if (!khrn_workarounds.SBWAIT)
      {
         /* if the slot is unused, move the sbdone to the
            previous slot */
         INSTR_STRUCT_T *instr1;
         OP_STRUCT_T *op0, *op1;
         instr = get_instr(i + 2);
         instr1 = get_instr(i + 1);
         op0 = get_op_struct(op_from_timestamp(i + 2, 0));
         op1 = get_op_struct(op_from_timestamp(i + 2, 1));
         /* only pull the sbdone back if the last instruction is empty
            and the destination signal slot can be replaced (SIG_UNUSED) */
         if (op0->flavour == FLAVOUR_UNUSED &&
             op1->flavour == FLAVOUR_UNUSED &&
             instr->sig == SIG_UNUSED &&
             instr1->sig == SIG_UNUSED)
            sbdone_timestamp = i + 1;
         else
      ...
      {
         /* strip SBWAIT */
         for (i = 0; i <= (last_used_timestamp - 3); i++)
         {
            OP_STRUCT_T *op0, *op1;
            OP_DEST_T dest0, dest1;
            INSTR_STRUCT_T *instr;
            ...
            if (instr->sig == SIG_SB_WAIT)
               instr->sig = SIG_UNUSED;

            if (instr->sig == SIG_GET_COL || instr->sig == SIG_GET_COL_THREND)
               implied_scoreboard = true;

            /* check for implicit SB access in this block (disallowed in the last 3 instructions) */
            dest0 = (op0->flavour != FLAVOUR_UNUSED) ? get_dest_without_pack(op0->dest) : (OP_FLAVOUR_T)~0;
            dest1 = (op1->flavour != FLAVOUR_UNUSED) ? get_dest_without_pack(op1->dest) : (OP_FLAVOUR_T)~0;
         ...
         /* if no implied scoreboard exists, work backwards and drop it in at the eariest conveniance */
         if (!implied_scoreboard)
         {
            for (i = (last_used_timestamp - 3); i > 0; i--)
            {
               INSTR_STRUCT_T *instr;
               instr = get_instr(i);
   ...
   if (khrn_workarounds.HW2924)
   {
      ...
      {
         khrn_memcpy(output, simple_hw2924, sizeof(simple_hw2924));
         output += sizeof(simple_hw2924) / sizeof(uint32_t);
      }
      if (shader_type & GLSL_BACKEND_TYPE_MULTISAMPLE_HW2924)
      {
         khrn_memcpy(output, multisample_hw2924, sizeof(multisample_hw2924));
...
void glsl_allocator_add_node(Dataflow *dataflow)
{
   ...
   switch (flavour)
   {
      ...
      case DATAFLOW_CONDITIONAL:
      {
         ...
         OP_STRUCT_T *result_struct;

         result = op_join_output(src_detail, background, dest_pack(PACK_NONE), deps, dep_count, cond, flags_src, true);

         result_struct = get_op_struct(result);

         if (dest_is_a(result_struct->dest)) {
            a_scheduler_node[get_dest_index(result_struct->dest)] = dataflow;
         }
         else if (dest_is_b(result_struct->dest)) {
      ...
      case DATAFLOW_INTRINSIC_MINABS:
         binop = true; op = FLAVOUR_FMINABS; break;
      case DATAFLOW_INTRINSIC_MAXABS:
         binop = true; op = FLAVOUR_FMAXABS; break;
...
bool glsl_allocator_finish()
{
   ...
   if ((shader_type & GLSL_BACKEND_TYPE_VERTEX) || (shader_type & GLSL_BACKEND_TYPE_COORD))
   {
      last_used_timestamp++;
...
void glsl_allocator_dump(void)
{
   ...
   /*DEBUG*/
   {
      ...
      for (i = 0; i < generated_instruction_count; i++)
      {
         ...
         sprintf(buffer2, "[%2d,%2d] %s", op_struct[i][0].schedule_order, op_struct[i][1].schedule_order, buffer);
         if  ((line_capture_fragment) && (shader_type & GLSL_BACKEND_TYPE_FRAGMENT))
            line_capture_fragment(buffer2);
         else if ((line_capture_vertex) && (shader_type & GLSL_BACKEND_TYPE_VERTEX))
            line_capture_vertex(buffer2);
         debug_print(buffer2);
         debug_print("\n");
      }
      debug_print("\n");
