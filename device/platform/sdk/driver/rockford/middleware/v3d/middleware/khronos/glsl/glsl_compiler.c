...
bool glsl_compile_and_link(slang_program* program)
{
   ...
   */
   ...
   // the mov node tells us that a uniform used to initialise a varying is live
   {
      ...
      // For every uniform variable in the fragment shader...
      for (node = f_dataflow_sources->uniforms->head; node; node = node->next)
      {
         DataflowSource* f_uniform = (DataflowSource *)node->v;
         DataflowSource* v_uniform = NULL;

         // Find this variable in the vertex shader.
         v_uniform = (DataflowSource *)glsl_map_get(v_dataflow_sources->uniforms, f_uniform->source->name, true);
         if (v_uniform)
         {
            // Check that the types match.
            if (!glsl_deep_match_nonfunction_types(f_uniform->source->type, v_uniform->source->type))
            {
               // Global variables must have the same type.
               glsl_compile_error(ERROR_LINKER, 1, f_uniform->initial_scalar_values[0]->line_num, NULL);
               return false;
            }
         }

         // Assert that the fragment uniform has not been handled before,
         // and if the uniform also exists in the vertex shader, it has not been handled before.
         vcos_assert(!f_uniform->handled && (!v_uniform || !v_uniform->handled));

         // Check it will fit.
         if (context.row + f_uniform->source->type->scalar_count > SLANG_UNIFORMS_LIMIT_ROW)
         {
            // Too many uniforms.
            glsl_compile_error(ERROR_LINKER, 5, f_uniform->initial_scalar_values[0]->line_num, NULL);
            return false;
         }
         ...
         len = strlen(f_uniform->source->name);
         glsl_sb_append(&context.sb, len, "%s", f_uniform->source->name);
         context.vert_scalar_values = v_uniform ? v_uniform->initial_scalar_values : NULL;
         context.frag_scalar_values = f_uniform->initial_scalar_values;

         // Pack.
         pack_uniform(&context, f_uniform->source->type);

         // Set handled flag(s).
         f_uniform->handled = true;
         if (v_uniform) v_uniform->handled = true;
      }

      // For every uniform variable in the vertex shader...
      for (node = v_dataflow_sources->uniforms->head; node; node = node->next)
      {
         DataflowSource* v_uniform = (DataflowSource *)node->v;

         // If we handled this uniform above (because it's also in the fragment shader), skip it.
         if (v_uniform->handled) continue;

         // Check it will fit.
         if (context.row + v_uniform->source->type->scalar_count > SLANG_UNIFORMS_LIMIT_ROW)
         {
            // Too many uniforms.
            glsl_compile_error(ERROR_LINKER, 5, v_uniform->initial_scalar_values[0]->line_num, NULL);
            return false;
         }
         ...
         // Update context.
         glsl_sb_reset(&context.sb);
         len = strlen(v_uniform->source->name);
         glsl_sb_append(&context.sb, len, "%s", v_uniform->source->name);
         context.vert_scalar_values = v_uniform->initial_scalar_values;
         context.frag_scalar_values = NULL;

         // Pack.
         pack_uniform(&context, v_uniform->source->type);

         // Set handled flag.
         v_uniform->handled = true;
      }
