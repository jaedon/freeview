...
static int num_samplers[SHADER_FLAVOUR_COUNT];
...
Dataflow* glsl_dataflow_construct_pack_col(int line_num, Dataflow* red, Dataflow *green, Dataflow *blue, Dataflow *alpha)
{
   ...
   {
      dataflow = glsl_dataflow_construct_unary_op(line_num, DATAFLOW_MOV, red->u.unary_op.operand);
      dataflow->type_index = PRIM_INT;
      unpack_operand = red->u.unary_op.operand;
   }
   else if (green->flavour == DATAFLOW_UNPACK_COL_G)
   {
      dataflow = glsl_dataflow_construct_unary_op(line_num, DATAFLOW_MOV, green->u.unary_op.operand);
      dataflow->type_index = PRIM_INT;
      unpack_operand = green->u.unary_op.operand;
   }
   else if (blue->flavour == DATAFLOW_UNPACK_COL_B)
   {
      dataflow = glsl_dataflow_construct_unary_op(line_num, DATAFLOW_MOV, blue->u.unary_op.operand);
      dataflow->type_index = PRIM_INT;
      unpack_operand = blue->u.unary_op.operand;
   }
   else if (alpha->flavour == DATAFLOW_UNPACK_COL_A)
   {
      dataflow = glsl_dataflow_construct_unary_op(line_num, DATAFLOW_MOV, alpha->u.unary_op.operand);
      dataflow->type_index = PRIM_INT;
      unpack_operand = alpha->u.unary_op.operand;
   }
   ...
   switch (statement->flavour)
   {
	   ...
	   case STATEMENT_ITERATOR_FOR:
      {
         ...
         for (i = 0; i < statement->u.iterator_for.iterations; i++) {
            glsl_statement_accept_prefix(statement->u.iterator_for.block, &f, sprev_calculate_dataflow, NULL);
...
static INLINE void expr_calculate_dataflow_subscript(Dataflow** scalar_values, Expr* expr)
{
   ...
   else
   {
      ...
      //2708B0
      {
         ...
         subscript_scalar_value = glsl_dataflow_construct_binary_op(expr->line_num, DATAFLOW_INTRINSIC_MIN, subscript_scalar_value, glsl_dataflow_construct_const_int(expr->line_num, size - expr->type->scalar_count));
...
static INLINE void expr_calculate_dataflow_binary_op_divide(Dataflow** scalar_values, Expr* expr)
{
   ...
   // The following control flow is based on that in ast.c and should be kept in sync with it.
   {
	      ...
	      && (left_flags & PRIM_FLOAT_TYPE) && (left_flags & (PRIM_VECTOR_TYPE | PRIM_MATRIX_TYPE)))
      {
         // Component-wise on same scalar type, different scalar counts.
         int i;
         Dataflow *rcp = glsl_dataflow_construct_unary_op(expr->line_num,
                                           DATAFLOW_INTRINSIC_RCP,
                                           right_scalar_values[0]);
