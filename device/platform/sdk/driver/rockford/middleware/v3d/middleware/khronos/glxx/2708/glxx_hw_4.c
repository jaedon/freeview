   ...
   if (win != NULL)
   {
      ...
      if (state->swapchainc > 0)
      {
         ...
         if (!color->alreadyLocked)
         {
            ...
            bool locked = platform_lock(color->opaque_buffer_handle, color->window_state);
            vcos_assert(locked);
            ...
            */
            if (locked)
            {
               ...
               color->mh_storage = mem_wrap((void *)bufferSettings.cachedAddr, bufferSettings.pitchBytes * bufferSettings.height,
               ...
               if (state->mh_depth) {
                  ...
                  KHRN_IMAGE_T *depth = (KHRN_IMAGE_T *)mem_lock(state->mh_depth);
               ...
               if (multisample) {
                  KHRN_IMAGE_T *multi = (KHRN_IMAGE_T *)mem_lock(state->mh_multi);
                  if (multi->width != (bufferSettings.width*2) || multi->height != (bufferSettings.height*2))
                  {
                     khrn_interlock_write_immediate(&multi->interlock);
                     khrn_image_resize(multi, bufferSettings.width*2, bufferSettings.height*2);
                  }
                  mem_unlock(state->mh_multi);
               }
#if 0
               if (khrn_image_is_tformat(color->format) || khrn_image_is_lineartile(color->format))
               {
                  void * p = khrn_hw_addr(mem_lock(color->mh_storage));
                  LOGD("Lock complete : glxx_install_framebuffer() %p %p, %d x %d, %d, %d, %d =====> %p %p %d x %d, %d\n",
                     color->mh_storage, p, color->width, color->height, color->stride, color->format, color->flags,
                     color->opaque_buffer_handle, khrn_hw_addr(bufferSettings.cachedAddr), bufferSettings.width, bufferSettings.height,
                     bufferSettings.format);
                  mem_unlock(color->mh_storage);
               }
#endif
            }
         }
         ...
         else
         {
            if (khrn_image_is_tformat(color->format) || khrn_image_is_lineartile(color->format))
            {
               platform_get_buffersettings(color->opaque_buffer_handle, &bufferSettings);
               void * p = khrn_hw_addr(mem_lock(color->mh_storage));
               LOGD("DUP CALL TO glxx_install_framebuffer() %p %p, %d x %d, %d, %d =====> %p %p %d x %d\n",
                  color->mh_storage, p, color->width, color->height, color->stride, color->format,
                  color->opaque_buffer_handle, khrn_hw_addr(bufferSettings.cachedAddr), bufferSettings.width, bufferSettings.height);
               mem_unlock(color->mh_storage);
            }
         }
...
static void calculate_and_hide(GLXX_SERVER_STATE_T *state, GLXX_HW_FRAMEBUFFER_T *fb, GLXX_ATTRIB_T *attrib)
{
   float *modelview;
   int i;
   bool stencil_magicvals;
   unsigned int stencil_front, stencil_back;
   bool fwd_rev_match;
   ...
   /* bit 0 = use_stencil
      bit 1 = masks are one of the magic values, for both FWD/REV
      bit 2 = FWD/REV match */
   stencil_magicvals = false;
   stencil_front = state->stencil_mask.front & 0xFF;
   stencil_back = state->stencil_mask.back & 0xFF;
   if ((stencil_front == 0xFF || stencil_front == 0xF || stencil_front == 0x3 || stencil_front == 0x1) &&
       (stencil_back ==  0xFF || stencil_back ==  0xF || stencil_back ==  0x3 || stencil_back ==  0x1))
       stencil_magicvals = true;
   /* structure compare */
   ...
   fwd_rev_match = (((stencil_front != stencil_back) && !stencil_magicvals) || ((stencil_front == stencil_back) && stencil_magicvals)) &&
   ...
   unsigned int fb_height)
{
   ...
   case BACKEND_UNIFORM:
   {
      ...
      else// gl 1.1 and 2.0 specials
      {
         ...
         case BACKEND_UNIFORM_STENCIL_FRONTBACK:
            {
               unsigned int mask = state->stencil_mask.front & 0xff;
               switch(mask)
               {
               case 0xff:
                  mask = 3;
               ...
               *ptr =
                  ...
                  mask << 28 |
                  ((u == BACKEND_UNIFORM_STENCIL_FRONT) ? 1 : 3) << 30;
            }
            break;
         case BACKEND_UNIFORM_STENCIL_BACK:
            {
               unsigned int mask = state->stencil_mask.back & 0xff;
               switch(mask)
               {
               case 0xff:
                  mask = 3;
...
static bool use_dummy(uint32_t *location, uint32_t u0)
{
   ...
   if (dummy_texture_handle == MEM_HANDLE_INVALID)
   {
      ...
      dummy_texture_data = mem_lock(dummy_texture_handle);
...
static bool glxx_install_tex_param(GLXX_SERVER_STATE_T *state, uint32_t *location, uint32_t u0, uint32_t u1)
{
   ...
   *location = 0;
...
#ifdef ANDROID
   /* abstract_buffer is set if the texture is provided as an eglImage */
   if (texture->abstract_buffer)
   {
      /* the eglimage bound platform data */
      PSHARED_DATA pSharedData;
      android_native_buffer_t *android_buffer = (android_native_buffer_t *)texture->abstract_buffer;

      pSharedData = (uint32_t)khrn_hw_unaddr(((struct private_handle_t *)android_buffer->handle)->sharedDataPhyAddr);
      /* only drop via here if the nexus(RSO) surface is valid, in lots of cases it wont be */
      if (pSharedData->nxSurfacePhysicalAddress && pSharedData->dirty)
      {
         MEM_HANDLE_T handle;
         unsigned int stride;
         KHRN_IMAGE_T *dst;
         EGL_IMAGE_T *eglImage = (EGL_IMAGE_T*)mem_lock(texture->external_image);

         dst = (KHRN_IMAGE_T *)mem_lock(eglImage->mh_image);

         /* wrap the src image */
         /* LOGD("glxx_install_tex_param() : pSharedData (%d %d), RSO (%d, %d)\n",
            dst->width, dst->height,
            pSharedData->width, pSharedData->height); */
         /* TODO: gralloc assumes stride = width * bpp, but this is not the case.  Just use it for now (bpp is somehow bytes per pixel!) */
         stride = pSharedData->width * pSharedData->bpp;
         handle = mem_wrap(khrn_hw_unaddr((void *)pSharedData->nxSurfacePhysicalAddress), pSharedData->height * stride, 1, 0, "EGL_NATIVE_BUFFER_ANDROID");

         eglImage->mh_shadow_image = khrn_image_create_from_storage(khrn_image_to_rso_format(dst->format),
            pSharedData->width, pSharedData->height, stride,
            MEM_INVALID_HANDLE, handle, 0, IMAGE_CREATE_FLAG_DISPLAY);
         mem_unlock(eglImage->mh_image);

         mem_release(handle);

         if (eglImage->mh_shadow_image != MEM_INVALID_HANDLE)
         {
            KHRN_IMAGE_T *tfImage = (KHRN_IMAGE_T*)mem_lock(eglImage->mh_shadow_image);
            tfImage->flags |= IMAGE_FLAG_BOUND_EGLIMAGE;
            mem_unlock(eglImage->mh_shadow_image);
         }

         khrn_rso_to_tf_convert(eglImage);

         /* this will run the termination on the mh_shadow_image, but it shouldnt matter at this point */
         MEM_ASSIGN(eglImage->mh_shadow_image, MEM_INVALID_HANDLE);
   ...
   {
      EGL_IMAGE_T *eglImage = (EGL_IMAGE_T*)mem_lock(texture->external_image);
      if (eglImage->convertedFrame != state->frame_number && eglImage->mh_shadow_image != MEM_INVALID_HANDLE)
      {
         if (khrn_rso_to_tf_convert(eglImage))
   ...
   switch (u0)
   {
      case BACKEND_UNIFORM_TEX_PARAM0:
      {
         ...
         if (texture->binding_type != TEXTURE_STATE_COMPLETE_UNBOUND || (!is_cube && texture->framebuffer_sharing))
         {
            ...
            image = mem_lock(texture->mh_mipmaps[buf_index][0]);
