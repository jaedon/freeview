...
{
   return glsl_dataflow_construct_binary_op(0, DATAFLOW_INTEGER_ADD, a, b);
}
...
Dataflow *glxx_backend(GLXX_HW_BLEND_T blend, GLXX_VEC4_T *color, Dataflow *discard, unsigned int stencil_config, bool use_depth, bool render_alpha, bool rgb565, bool boilerplate)
{
   bool use_stencil = (bool)(stencil_config & 0x1);
   bool use_magic = (bool)(stencil_config & 0x2);
   bool use_fwdrev = (bool)(stencil_config & 0x4);
   bool hw2924flag;
   Dataflow *sbwait;
   ...
   if (use_stencil)
   {
      /* Mask must come last as the other things overwrite it. */
      if (use_fwdrev)
         stencil = glsl_dataflow_construct_fragment_submit(-1, DATAFLOW_FRAG_SUBMIT_STENCIL, glxx_ustandard_int(BACKEND_UNIFORM_STENCIL_FRONTBACK), stencil, NULL);
      else
      {
   ...
   if (khrn_workarounds.HW2924 && boilerplate)
   {
      data_z = sbwait;
   }
   else if (use_depth || use_stencil || (discard && !(discard->flavour == DATAFLOW_CONST_BOOL && discard->u.const_bool.value == CONST_BOOL_FALSE)))
   {
   ...
   else if (!glxx_hw_blend_enabled(blend) && is_logic_nop(blend.logic_op) && blend.color_mask == 0xffffffff
      ...
      && !blend.sample_coverage)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            packed = glsl_dataflow_construct_binary_op(-1, DATAFLOW_BITWISE_OR, packed, glxx_cint((int)0xFF000000));
#endif
         }
      }
      /* Write opaque color value */
      result = glsl_dataflow_construct_fragment_submit(-1,
         (khrn_workarounds.HW2924 && boilerplate) ? DATAFLOW_FRAG_SUBMIT_R0 : DATAFLOW_FRAG_SUBMIT_ALL,
         packed, data_z, NULL);
   }
   else
   {
      ...
      for (i = 0; i < samples; i++)
      {
         ...
         if (khrn_workarounds.HW2924)
         {
            ...
            target = boilerplate ? boilerplate_targets[i] : (blend.ms ? DATAFLOW_FRAG_SUBMIT_MS : DATAFLOW_FRAG_SUBMIT_ALL);
         }
         else
...
bool glxx_schedule(Dataflow *root, uint32_t type, MEM_HANDLE_T *mh_code, MEM_HANDLE_T *mh_uniform_map, bool *threaded, uint32_t *vary_map, uint32_t *vary_count, bool fb_rb_swap)
{
   ...
   khrn_memcpy(mem_lock(hcode), glsl_allocator_get_shader_pointer(), code_size);
   mem_unlock(hcode);
   mem_release(hcode);

   khrn_memcpy(mem_lock(huniform_map), glsl_allocator_get_unif_pointer(), uniform_map_size);
