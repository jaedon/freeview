...
static MEM_HANDLE_T create_texture(GLenum target)
{
   ...
   glxx_texture_init(mem_lock(ret), 0, target);
   mem_unlock(ret);
   return ret;
}
...
void glCopyTexImage2D_impl (GLenum target, GLint level, GLenum internalformat, GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
   ...
   if (level < 0 || level > LOG2_MAX_TEXTURE_SIZE)
      ...
      glxx_server_state_set_error(state, GL_INVALID_VALUE);
   else if (khrn_image_is_yuv422(internalformat) && ((width & 1) != 0 || (x & 1) != 0))
...
int glGetBufferParameteriv_impl (GLenum target, GLenum pname, GLint *params)
{
   ...
   if (buffer) {
      ...
      case GL_BUFFER_SIZE:
         ...
         {
            /* if the buffer is > 0 bytes then its had 32bytes added for the workaround.  Dont report these additional bytes back to GL */
            params[0] -= (params[0]) ? 32 : 0;
         }
         result = 1;
         break;
...
void glReadPixels_impl (GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLint alignment, void *pixels)
{
   ...
   else if ((format == GL_RGBA || format == GL_RGB) && type == GL_UNSIGNED_BYTE)
   {
      ...
      if (width > 0 && height > 0)
      {
         ...
         khrn_image_interlock_wrap(&dst_wrap, (format == GL_RGBA) ? RGBA_8888_RSO : RGB_888_RSO, width, height, k, 0, pixels, NULL);
#else
         /* ABGR_8888_RSO means 0xAABBGGRR as a variable, and RRGGBBAA in memory when written as a word (on little-endian machines) */
         khrn_image_interlock_wrap(&dst_wrap, (format == GL_RGBA) ? ABGR_8888_RSO : BGR_888_RSO, width, height, k, 0, pixels, NULL);
#endif
...
GLboolean glTexImage2D_impl (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, GLint alignment, const GLvoid *pixels)
{
   ...
   else if (khrn_image_is_yuv422(format) && ((width & 1) != 0))
      error = GL_INVALID_OPERATION;
...
void glTexSubImage2D_impl (GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, GLint alignment, const GLvoid *pixels)
{
   ...
   if (texture) {
      if (!glxx_is_texture_target(state, target))
         ...
         glxx_server_state_set_error(state, GL_INVALID_OPERATION);
      else if (khrn_image_is_yuv422(format) && ((width & 1) != 0 || (xoffset & 1) != 0))
         glxx_server_state_set_error(state, GL_INVALID_OPERATION);
      else if (width > 0 && height > 0)
      {
         //TODO: check formats are compatible?
...
GLboolean glintCacheGrow_impl()
{
   ...
   if (khrn_workarounds.GFXH776)
   {
      ...
      /* the size in the client is determined via a pow2() calculation, so overallocating here
      doesn't just cause the cache to become physically bigger */
      if (size)
         size = ((size - 32) * 2) + 32;
      else
         size = 64 + 32;
...
void glintGetCoreRevision_impl(GLsizei bufSize, char *revisionStr)
{
   ...
   if (bufSize >= 4)
   {
      vcos_assert(numSlices <= 9);
      vcos_assert(tmusPerSlice <= 2);

      if (ident0 == 0 && ident1 == 0 && ident2 == 0)
      {
         /* We couldn't read the registers for some reason */
         revisionStr[0] = '?';
         revisionStr[1] = '?';
         revisionStr[2] = '?';
      }
      ...
      {
         revisionStr[0] = '4';
         revisionStr[1] = '0' + numSlices;
         revisionStr[2] = '0' + 5 * (tmusPerSlice - 1);
      }
   ...
   if (target == GL_RENDERBUFFER) {
      if (state->mh_bound_renderbuffer != MEM_INVALID_HANDLE) {
         ...
         case GL_RENDERBUFFER_INTERNAL_FORMAT:
            if (!renderbuffer || renderbuffer->type == RB_COLOR_T)
            {
               if (!storage)
                  params[0] = 0;
            ...
            else
            {
               switch (renderbuffer->type)
               {
               case RB_DEPTH16_T :params[0] = GL_DEPTH_COMPONENT16; break;
               case RB_DEPTH24_T : params[0] = GL_DEPTH_COMPONENT24_OES; break;
               case RB_STENCIL_T : params[0] = GL_STENCIL_INDEX8; break;
               case RB_DEPTH24_STENCIL8_T : params[0] = GL_DEPTH24_STENCIL8_OES; break;
               }
            }
            result = 1;
            break;
            ...
            if (level != 0)
               ...
               if (!glxx_is_texture_target(state, textarget))
                  ...
                  if (handle == MEM_INVALID_HANDLE)
                     ...
                     if ((((GLXX_TEXTURE_T *)mem_lock(handle))->target == GL_TEXTURE_CUBE_MAP) != (textarget != GL_TEXTURE_2D))
                        glxx_server_state_set_error(state, GL_INVALID_OPERATION);
                     else {
