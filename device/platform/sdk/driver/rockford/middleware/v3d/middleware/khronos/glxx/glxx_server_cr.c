...
static int get_texture_binding(GLXX_SERVER_STATE_T *state, GLenum binding)
{
    ...
    ret = ((GLXX_TEXTURE_T *) mem_lock(handle))->name;
    mem_unlock(handle);
    return ret;
}
...
int glxx_get_integer_internal(GLXX_SERVER_STATE_T *state, GLenum pname, int *params)
{
   ...
   case GL_DEPTH_BITS:
      // We might physically have a depth buffer, but we could have given back a config without one
      if (state->mh_depth == MEM_INVALID_HANDLE || state->config_depth_bits == 0)
         params[0] = 0;
      else {
   ...
   {
      // We might physically have a stencil buffer, but we could have given back a config without one
...
void glActiveTexture_impl (GLenum texture)
{
   ...
   if (texture >= GL_TEXTURE0 && (
         ...
         (IS_GL_20(state) && texture < GL_TEXTURE0 + GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS)
      ))
...
void glBindTexture_impl (GLenum target, GLuint texture)
{
   ...
   if (!texture) {
      ...
      GLXX_SHARED_T *shared = mem_lock(state->mh_shared);
