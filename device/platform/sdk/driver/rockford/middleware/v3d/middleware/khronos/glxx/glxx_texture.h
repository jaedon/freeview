...
#define TEXTURE_BUFFER_EXTERNAL        1
#define TEXTURE_BUFFER_POSITIVE_X      2
#define TEXTURE_BUFFER_NEGATIVE_X      3
#define TEXTURE_BUFFER_POSITIVE_Y      4
#define TEXTURE_BUFFER_NEGATIVE_Y      5
#define TEXTURE_BUFFER_POSITIVE_Z      6
#define TEXTURE_BUFFER_NEGATIVE_Z      7
#define TEXTURE_BUFFER_COUNT           8
...
#define TEXTURE_STATE_EGLIMAGE_TARGET 5
...
- it is either a 2D texture, an external texture, or a cube map texture
- it has some information concerning how the texture is rendered (mag, min,
  wrap.s, wrap.t)
- it has 1 or 6 arrays of mipmaps. Each mipmap is a (possibly zero-size) 2D
  array of pixels, and has a width, height and format.
- in GLES1.1 it has a flag indicating whether mipmaps should be generated
  automatically

Textures may be used for rendering iff they are "complete", i.e. all the base
level mipmaps are nonzero and the same size (for cube maps), all mipmap
images are the same format and each is half the dimensions of the previous
(rounded down until you get to 1). If the minification filter is GL_LINEAR or
GL_NEAREST, only the base level images are required.

Additionally, texture images can be shared with other API objects. Individual
mipmaps may be used as the color buffer of a framebuffer object, or they may be
used as EGLImage sources. Texture data may also come from an EGL pbuffer (via a
call to eglBindTexImage) or an EGLImage (making the texture an EGLImage
target). Once bound, it's not really possible to distinguish between EGLImage
sources and targets - they're all considered to be EGLImage "siblings".
...
Target data is supplied by EGLImage with a call such as
glEGLImageTargetTexture2DOES_impl_11

Our internal representation is a little more complicated than this, because we
require a fast, memory-efficient path as well as supporting all of the obscure
corner cases. So texture data can be stored in two ways:
...
(3) the image is in the blob but is shared with something (or might be).
    blob_valid is set and mh_mipmaps is valid. In this case, mh_mipmaps[ ][
    ].mh_storage == mh_blob, i.e. the image is pointing directly into our blob.

The fast path is that everything is either (0) or (1).

When a blob is required (i.e. for (1) or (3)), binding_type can be anything
other than TEXTURE_BLOB_NULL and the width, height, format, offset and
blob_mip_count parameters are valid, and describe the structure of the blob.

If a base level image is specified and the binding_type is TEXTURE_BLOB_NULL, a
new blob will be created. This will include space for mipmaps iff the current
min filter suggests they are needed (and this will be recorded in
blob_mip_count).

When a texture is to be used, it must be checked for completeness and "blobbed"
first. Bound textures (in the TEXTURE_STATE_BOUND_TEXIMAGE or
TEXTURE_STATE_BOUND_EGLIMAGE state, and created via a different mechanism) are
...
typedef struct {
   int32_t name;
   /* One of: GL_TEXTURE_2D, GL_TEXTURE_CUBE_MAP, GL_TEXTURE_EXTERNAL_OES */
   GLenum target;
   ...
   uint32_t offset;
   uint32_t blob_valid[TEXTURE_BUFFER_COUNT];   /* Bitmask. (blob_valid[buf] & 1<<lev) != 0   indicates image in buffer buf at level lev is present in the blob */
   ...
   /*
      ...
      TEXTURE_BLOB_NULL
      Blob is empty, though some mipmaps may be individually specified. New
      textures start off in this state. width, height, format, etc. are all
      invalid. blob_valid should all be zero.
      ...
      Bound using eglBindTexImage. All of our mh_mipmaps have the
      IMAGE_FLAG_BOUND_TEXIMAGE flag and are attached to the same pbuffer. When
      a new mipmap is attached to this texture, all old mipmaps get set to
      invalid. When eglReleaseTexImage is called, the IMAGE_FLAG_BOUND_TEXIMAGE
      flag disappears from the images. We detect this and set all our mipmaps
      to invalid.

      TEXTURE_STATE_BOUND_EGLIMAGE
      Bound using glEGLImageTargetTexture2DOES (EGLImage target) or
      eglCreateImageKHR (EGLImage source). All of our mipmaps have the
      IMAGE_FLAG_BOUND_EGLIMAGE flag. When a new mipmap is attached to this
      texture, we take a copy of all attached images. (TODO: it says this
      explicitly for EGLImage targets, but what about sources?)
   */
