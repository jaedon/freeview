...
static bool append_multiply(uint32_t **p, bool *need_pmc, VG_SHADER_FD_KEY_T key, WHERE_T where, bool pre)
{
   ...
   *need_pmc = true;
   if (where == WHERE_TU) { APPEND(*p, %[ nop                   ; nop                 ; ldtmu0 %]); TMU_LOAD; }
   ...
   */
   ...
   if (key.draw_type != VG_SHADER_FD_DRAW_TYPE_IMAGE) {
      ...
      case VG_SHADER_FD_PAINT_TYPE_PATTERN:
      {
         ...
         if (key.paint_pattern_child) { APPEND(p,                                %[ mov  t0b, 0         ; nop                %]); TMU_WRITE(1);}
         if (key.paint_pattern_tile_fill || key.paint_pattern_child) { APPEND(p, %[ mov  t0r, r3        ; nop                %]); TMU_WRITE(1);}
      ...
      */
      ...
      if (pre) {
         ...
         case WHERE_TU: { APPEND(p,      %[ nop                  ; nop                      ; ldtmu0
                                            ...
                                            mov.ifz  r1, f(0.0)  ; fmul  r2, r4.8cc, unif            %]); TMU_LOAD; break; }
         case WHERE_RA0:
      ...
      */
      ...
      if (pre) {
         ...
         if (stencil) { 
            ...
            APPEND(p,                                  %[ mov  r0, r4                  ; fmul  t0s, r1, r4                   %]);
            TMU_WRITE_S(1);
         }
         ...
         switch (where) {
         case WHERE_TU: {
            APPEND(p,                                  %[ mov  r0, f(0.0)              ; nop                        ; ldtmu0
                                                          mov  t0t, f(0.0)             ; mov  ra0, r4                        %]); 
            TMU_LOAD; TMU_WRITE(1); break; }
         case WHERE_RA0:
         case WHERE_RA0_OLD: { 
            early_image_load = !(TMU_AVAILABLE > 0);
            if (stencil && early_image_load) {
               APPEND(p,                               %[ mov  r0, f(0.0)              ; nop                                 %]);
               APPEND_I(p, where == WHERE_TU,          %[ fadd  r1, %0.8ac, r0         ; nop                        ; ldtmu0 %| ra0 %, r4 %]);
               TMU_LOAD;
            }
            APPEND(p,                                  %[ mov  t0t, f(0.0)             ; mov  r0, f(0.0)                     %]);
            TMU_WRITE(1);
            break; }
         default: UNREACHABLE(); }
         if (stencil) { 
            ...
            APPEND_I(p, where == WHERE_TU,             %[ fadd  t0s, %0.8ac, r0        ; nop                                 %| ra0 %, r4 %]);
            TMU_WRITE_S(1);
         }
         ...
         if (pre || (key.paint_type == VG_SHADER_FD_PAINT_TYPE_SOLID)) {
            if (key.paint_type == VG_SHADER_FD_PAINT_TYPE_SOLID) {
               ...
               case WHERE_RA0: {                             xor(p - 2, %[ nop ; mov  ra32, 0 %], %[ nop ; mov  ra0, 0 %]); /* change mul write from ra0 --> r0 */
                  ...
                  TMU_LOAD;
                  break; }
               case WHERE_RA0_OLD: { APPEND(p,               %[ mov  r0, ra0          ; mov ra1, ra0            ; ldtmu0 %]); TMU_LOAD; break; }
               default: UNREACHABLE(); } }
