...
static IO_CHUNK_T *alloc_io_chunk(void)
{
   ...
   while (next_io_chunk(io_chunk_alloc) == io_chunk_free) {
      khrn_sync_master_wait_timeout(-1); /* wait for one to be freed */
   }
   khrn_barrier();
   ...
   const float *clip)
{
   ...
   io_chunk->i.t = t;
   khrn_memcpy(io_chunk->i.user_to_surface, user_to_surface, sizeof(io_chunk->i.user_to_surface));
   io_chunk->i.user_to_surface[2] += BIAS;
   io_chunk->i.user_to_surface[5] += BIAS;
   khrn_memcpy(io_chunk->i.clip, clip, sizeof(io_chunk->i.clip));
   ...
   const float *clip_inner, const float *clip_outer)
{
   ...
   io_chunk->i.user_to_surface[2] += BIAS;
   io_chunk->i.user_to_surface[5] += BIAS;
   khrn_memcpy(io_chunk->i.clip, clip_inner, sizeof(io_chunk->i.clip));

   io_chunk2 = alloc_io_chunk();
   io_chunk2->stroke_i2.line_width = line_width;
   khrn_memcpy(io_chunk2->stroke_i2.clip_outer, clip_outer, sizeof(io_chunk2->stroke_i2.clip_outer));
...
static bool do_fill(const IO_CHUNK_T *io_chunk, REP_T *r)
{
   ...
   chunk_a = (const uint8_t *)io_chunk->i.chunks_a;
   chunk_a_end = chunk_a + io_chunk->i.chunks_size;
   chunk_b = (const uint8_t *)io_chunk->i.chunks_b;
...
static void vg_tess_llat_callback(void)
{
   ...
   while (more_msgs(vg_tess_msg.submit, vg_tess_msg.post)) {
      ...
      case MSG_TYPE_FILL_STROKE:
      {
         ...
         msg->io_chunk->i.chunks_a = mem_lock(msg->chunks_a);
         msg->io_chunk->i.chunks_b = mem_maybe_lock(msg->chunks_b);
   ...
   while (vg_tess_msg.done != vg_tess_msg.submit) {
      ...
      case MSG_TYPE_WAIT:
      {
         ...
         if (msg->tess->u.in_use.nmem_entered) {
            wait_exit_nmem_pos = msg->tess->u.in_use.nmem_pos;
         }
