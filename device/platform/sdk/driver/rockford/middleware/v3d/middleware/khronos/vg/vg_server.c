   ...
   VGPath src_vg_handle)
{
   ...
   */
   ...
   khrn_memcpy((uint8_t *)mem_lock(dst->segments) + (mem_get_size(dst->segments) - segments_count), mem_lock(src->segments), segments_count);
   ...
   VGuint coords_size, const void *coords)
{
   ...
   khrn_memcpy((uint8_t *)mem_lock(path->segments) + (mem_get_size(path->segments) - segments_count), segments, segments_count);
   ...
   VGint width, VGint height)
{
   ...
   if (!khrn_image_wrap_copy_region_tlb(&dst_wrap, child_x + dst_x, child_y + dst_y, width, height, &src_wrap, src_x, 0, IMAGE_CONV_VG)) {
   ...
   VGint width, VGint height)
{
   ...
   if (!khrn_image_wrap_copy_region_tlb(&dst_wrap, dst_x, 0, width, height, &src_wrap, child_x + src_x, child_y + src_y, IMAGE_CONV_VG)) {
      khrn_interlock_write_immediate(&tmp_interlock);
   ...
   VGint width, VGint height)
{
   ...
   if (state->scissoring) {
      ...
      if(!khrn_image_wrap_copy_region_tlb(&dst_wrap, dst_x, dst_y, width, height, &src_wrap, src_x, 0, IMAGE_CONV_VG)) {
         khrn_interlock_write_immediate(&dst->interlock);
         khrn_interlock_read_immediate(&tmp_interlock);
   ...
   VGint width, VGint height)
{
   ...
   if (!khrn_image_wrap_copy_region_tlb(&dst_wrap, dst_x, 0, width, height, &src_wrap, src_x, src_y, IMAGE_CONV_VG)) {
