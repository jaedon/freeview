...
#define MAGIC 0xB00B1E55
   ...
   #define RCM_INLINE __inline
      ...
      #define RCM_INLINE __attribute__((always_inline)) __attribute__((no_instrument_function))
...
typedef struct {
   ...
   void * ptr;          /* actual allocation (virtual address) */
...
/* Wrap an existing block of memory (that was allocated from a different heap) in a MEM_HANDLE_T */
extern MEM_HANDLE_T mem_wrap_ex(void *p, void *physAddr, uint32_t size, uint32_t align, MEM_FLAG_T flags, const char *desc);
...
static RCM_INLINE void * mem_lock(MEM_HANDLE_T handle)
{
   MEM_HEADER_T *h = (MEM_HEADER_T *)handle;
...
static RCM_INLINE void mem_unlock(MEM_HANDLE_T handle)
{
   MEM_HEADER_T *h = (MEM_HEADER_T *)handle;
...
extern uintptr_t mem_min_cached_addr(void);

extern uintptr_t mem_max_cached_addr(void);
