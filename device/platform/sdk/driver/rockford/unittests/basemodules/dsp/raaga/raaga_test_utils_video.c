 ...
 *
 * $brcm_Workfile: raaga_test_utils_video.c $
 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/basemodules/dsp/raaga/raaga_test_utils_video.c $
 * 
...
b_time_get(struct timeval *time)
{
	...
	if (rc!=0) {
		BDBG_ERR(("gettimeofday returned %d, ignored", rc));
	}
	return;
}
	...
	while(1)
			{
                ...
                for(i=0; i<8; i+=4)
                {
                    BDBG_ERR(("ITB 0x%08x%08x%08x%08x",mydata[i+0], mydata[i+1], mydata[i+2], mydata[i+3]));
                }
#endif
                
				regVal = BREG_Read32( hReg, 0x746030);
				BDBG_ERR(("\t*********** STC = 0x%08x **************",regVal));
