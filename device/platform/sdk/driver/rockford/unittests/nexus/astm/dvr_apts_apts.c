 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/astm/dvr_apts_apts.c $
 * 
...
int main(void)
{
	...
	printf("Generating audio pts errors (1)\n");
	for (i = 0; i < 50; i++)
	{
		BKNI_EnterCriticalSection();
	...
	printf("Generating audio pts errors (2)\n");
	for (i = 0; i < 50; i++)
	{
		BKNI_EnterCriticalSection();
		gTestData.audio[0].settings.tsmFail_isr(gTestData.audio[0].settings.callbackContext, 0);
