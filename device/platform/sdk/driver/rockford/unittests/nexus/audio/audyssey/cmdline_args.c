 ...
 * $brcm_Date: 7/8/10 12:38p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/audio/audyssey/cmdline_args.c $
 * 
 * Hydra_Software_Devel/3   7/8/10 12:38p nvu
...
int cmdline_parse(int argc, const char *argv[], struct util_opts_t *opts)
{
    ...
    for (i=1;i<argc;i++) {
        ...
        else if (!strcmp(argv[i], "-audio_eq")) {
            opts->audio_eq=true;
        }
        ...
        if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
            ...
            while (j<MAX_RECORD_PIDS) {
                if ((cur=strchr(prev,','))) {
                    ...
                    opts->pids[j] = strtoul(pid, NULL, 0);
                    if (opts->pids[j]) { j++; }
                    prev = cur+1;
                }
                else {
                    ...
                    opts->pids[j] = strtoul(pid, NULL, 0);
                    if (opts->pids[j]) { j++; }
                    if (j<MAX_RECORD_PIDS) {
                        BDBG_ASSERT(opts->pids[j]==0);
                    }
                    break;
                }
                pid[0]='\0';
            ...
            i++;
#if 1 /* debug */
            for (cur=0; (unsigned)cur<j; cur++) {
                printf("[%d] %#x\n", (unsigned)cur, opts->pids[(unsigned)cur]);
            }
#endif
        }
        else if (!strcmp(argv[i], "-video_type") && i+1<argc) {
