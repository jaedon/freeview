 ...
 *  ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: dolby_digitalplus_multistream_audiomixing.c $
 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/dolby/dolby_digitalplus_multistream_audiomixing.c $
 * 
...
/*---------------------------------------------------------------Command line arguments for DDP-----------------------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------Parsing of the config file and initialization of the config parameters-----------------------------------------------------------------------*/
...
int dolby_digital_plus_cmdline_parse(int argc, const char *argv[], struct dolby_digital_plus_command_args *dolby)
{
    int i;
    unsigned int value;
...
/*--------------------------------------------------------------------Initialization----------------------------------------------------------------------------------------------------------------*/
	if (dolby_digital_plus_cmdline_parse(argc, argv, &dolby_args)) {
		 return 0;
	 }
...
/*-------------------------------------------------------------Adding audio source to the corresponding output ports--------------------------------------------------------------------------------*/
    ...
    if(dolby_args.MixingOutput==1) {
                        ...
                        if(dolby_args.ddc_enabled==false) {
                            ...
                            NEXUS_AudioOutput_AddInput(NEXUS_SpdifOutput_GetConnector(platformConfig.outputs.spdif[0]),
													   NEXUS_AudioDecoder_GetConnector(mainDecoder, NEXUS_AudioDecoderConnectorType_eCompressed));
							NEXUS_AudioOutput_AddInput(NEXUS_AudioDac_GetConnector(platformConfig.outputs.audioDacs[0]),
												   NEXUS_AudioDecoder_GetConnector(mainDecoder, NEXUS_AudioDecoderConnectorType_eStereo));
							NEXUS_AudioOutput_AddInput(NEXUS_AudioCapture_GetConnector(capture),
														NEXUS_AudioDecoder_GetConnector(mainDecoder, NEXUS_AudioDecoderConnectorType_eCompressed));
...
/*-----------------------------------------------------------Handling the run time change into the config parameters-----------------------------------------------------------------------------------------------------------------*/
	for(exit=false;!exit;) {
        char buffer[256];
        char *buf;
		...
		do {
            if (!strcmp(buf, "?")) {
		       ...
		       printf("\n ./\\nexus dolby_digitalplus_multistream_audiomixing -config configfilename -single/\\-associated/\\-mixing -vidpid videopid_value -audpid1 mainaudiopid_value -audpid2 associatedaudiopid_value \n");
               printf("\n\n To change the value of any config parameters, type the command as 'CONFIG PARAMETER NAME= VALUE' \n Type 'quit' to terminate the application. \n\n\n");
...
/*--------------------------------------------------------------Stopping,closing,removing and uninitializing the used resources---------------------------------------------------------------------*/
