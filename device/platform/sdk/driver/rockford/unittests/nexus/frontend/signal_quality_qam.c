 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: signal_quality_qam.c $
 * $brcm_Revision: Hydra_Software_Devel/1 $
 * $brcm_Date: 7/11/12 6:43p $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/frontend/signal_quality_qam.c $
 * 
 * Hydra_Software_Devel/1   7/11/12 6:43p mphillip
 * SW7425-2888: Add QAM signal quality unittest
 * 
            ...
            if (frontend) {
                ...
                if (capabilities.qam) {
                    ...
                    for (j=0; j < num_entries; j++) {
                        ...
                        BKNI_Memcpy(&qamSettings, &entries[j].fe_settings.qam, sizeof(qamSettings));
                        ...
                        rc = NEXUS_Frontend_GetQamStatus(frontend, &status);
                        BDBG_ASSERT(!rc);
                        printf("fec: %d, receiver: %d\n", status.fecLock, status.receiverLock);
                        ...
                        if (comment_has_token(entries[j].trailing_comment,"no_signal")) {
                            ...
                            rc = NEXUS_Frontend_GetQamStatus(frontend, &status);
                            BDBG_ASSERT(!rc);
                            if (status.fecLock && status.receiverLock) {
                                rc = NEXUS_Frontend_GetQamStatus(frontend, &status);
                                BDBG_ASSERT(!rc);
