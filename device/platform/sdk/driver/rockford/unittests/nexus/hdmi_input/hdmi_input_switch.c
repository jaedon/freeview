...
*  ANY LIMITED REMEDY.
*
* $brcm_Workfile: hdmi_input_switch.c $
...
* Revision History:
*
* $brcm_Log: /rockford/unittests/nexus/hdmi_input/hdmi_input_switch.c $
* 
...
int main(void)
{
    ...
    hdmiInput[0] = NEXUS_HdmiInput_Open(0, &hdmiInputSettings);
    BDBG_ASSERT(hdmiInput[0]);
    hdmiInput[1] = NEXUS_HdmiInput_Open(1, &hdmiInputSettings);
    BDBG_ASSERT(hdmiInput[1]);
    ...
    while (loops--) {
        unsigned input = loops%2;
        printf("watching HDMI %d\n", input);

        rc = NEXUS_VideoWindow_AddInput(window, NEXUS_HdmiInput_GetVideoConnector(hdmiInput[input]));
        BDBG_ASSERT(!rc);

        NEXUS_AudioDecoder_GetDefaultStartSettings(&audioProgram);
        audioProgram.input = NEXUS_HdmiInput_GetAudioConnector(hdmiInput[0]);
        ...
        printf("remove input from window\n");
        rc = NEXUS_VideoWindow_RemoveInput(window, NEXUS_HdmiInput_GetVideoConnector(hdmiInput[input]));
        BDBG_ASSERT(!rc);

        BKNI_Sleep(2000);

        printf("re-add input to window\n");
        rc = NEXUS_VideoWindow_AddInput(window, NEXUS_HdmiInput_GetVideoConnector(hdmiInput[input]));
        BDBG_ASSERT(!rc);
