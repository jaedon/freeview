 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: ir_input_quiet.c $
 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/io/ir_input_quiet.c $
 * 
...
#define SAFE_SETTINGS 1

#if SAFE_SETTINGS
/* mirror internal code, but expect no Open or SetSettings failures */
#define INTERRUPT_DEVICES 5
#define NUM_CHANNELS 3
bool in_use[NUM_CHANNELS][INTERRUPT_DEVICES];
unsigned total = 0;
#else
...
again:
    ...
    case 0:
    case NEXUS_IrInputMode_eCustom:
    case NEXUS_IrInputMode_eSonySejin:
    case NEXUS_IrInputMode_eCirRC6Mode0:
        goto again;
    case NEXUS_IrInputMode_eTwirpKbd:
        interruptDevice = 0; break;
    case NEXUS_IrInputMode_eSejin38KhzKbd:
    case NEXUS_IrInputMode_eSejin56KhzKbd:
        interruptDevice = 1; break;
    case NEXUS_IrInputMode_eRemoteA:
        interruptDevice = 2; break;
    case NEXUS_IrInputMode_eRemoteB:
        interruptDevice = 3; break;
    default:
        interruptDevice = 4; break;
    }
    BDBG_MSG(("in_use[%d][%d] = %d (total %d)", pSettings->channel_number, interruptDevice, in_use[pSettings->channel_number][interruptDevice], total));
    if (in_use[pSettings->channel_number][interruptDevice])
        goto again;
    in_use[pSettings->channel_number][interruptDevice] = true;
    total++;
#endif
}

#if SAFE_SETTINGS
static void release(const NEXUS_IrInputSettings *pSettings)
{
    unsigned interruptDevice;
    switch (pSettings->mode) {
    case NEXUS_IrInputMode_eTwirpKbd:
    ...
    BDBG_ASSERT(in_use[pSettings->channel_number][interruptDevice]);
    in_use[pSettings->channel_number][interruptDevice] = false;
    --total;
}
#endif
