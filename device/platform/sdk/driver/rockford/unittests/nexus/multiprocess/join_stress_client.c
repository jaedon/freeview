 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: join_stress_client.c $
 * $brcm_Revision: Hydra_Software_Devel/3 $
 * $brcm_Date: 9/5/12 9:10a $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/multiprocess/join_stress_client.c $
 * 
 * Hydra_Software_Devel/3   9/5/12 9:10a erickson
 * SW7420-1671: don't assert on null handle
 * 
 * Hydra_Software_Devel/2   8/16/12 4:36p erickson
...
BDBG_MODULE(join_stress_client);
...
#define NUM_HANDLES 10
    NEXUS_Graphics2DHandle gfx[NUM_HANDLES];
    ...
    /* must use authenticated clients so we can count joins on the server */
    ...
    if (rc) {
        fprintf(stderr, "NEXUS_Platform_Join failed: %d\n", rc);
        return rc;
    }

    BDBG_WRN(("client joined"));
    ...
    for (i=0;i<loops;i++) {
        ...
        case 1:
            if (!gfx[index]) {
                gfx[index] = NEXUS_Graphics2D_Open(0, NULL);
				/* may be NULL if client is terminated */
            }
            break;
        case 2:
            if (gfx[index]) {
                NEXUS_Graphics2D_Close(gfx[index]);
                gfx[index] = NULL;
            }
            break;
        }
