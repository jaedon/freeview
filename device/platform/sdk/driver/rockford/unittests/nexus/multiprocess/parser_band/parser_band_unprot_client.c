 ...
 * ANY LIMITED REMEDY.
 *
 * $brcm_Workfile: parser_band_unprot_client.c $
 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/multiprocess/parser_band/parser_band_unprot_client.c $
 * 
 * Hydra_Software_Devel/4   11/4/11 10:25p bandrews
 * SW7231-446: update unit tests
 * 
 * Hydra_Software_Devel/3   10/25/11 7:28p bandrews
...
BDBG_MODULE(parser_band_unprot_client);
...
int main(int argc, char **argv)  {
    ...
    printf("Starting parser band unprotected client\n");
    ...
    printf("Client using parser band 0 again...");
    NEXUS_ParserBand_GetSettings(NEXUS_ParserBand_e0, &settings);
    settings.allPass = true;
    rc = NEXUS_ParserBand_SetSettings(NEXUS_ParserBand_e0, &settings);
    BDBG_ASSERT(!rc);
    ...
    printf("Client opening parser band 0..."); /* should fail */
    BDBG_ASSERT(NEXUS_ParserBand_Open(0) == NEXUS_ParserBand_eInvalid);
    printf("PASS\n");
    ...
    BDBG_ASSERT(band != NEXUS_ParserBand_eInvalid);
    printf("PASS\n");

    printf("Client opening parser band 1..."); /* should fail */
    BDBG_ASSERT(NEXUS_ParserBand_Open(1) == NEXUS_ParserBand_eInvalid);
    printf("PASS\n");
    ...
    printf("Client using parser band 1...");
    NEXUS_ParserBand_GetSettings(NEXUS_ParserBand_e1, &settings);
    settings.allPass = true;
    rc = NEXUS_ParserBand_SetSettings(NEXUS_ParserBand_e1, &settings);
    BDBG_ASSERT(!rc);
    printf("PASS\n");

    printf("Client using parser band 1 again...");
    NEXUS_ParserBand_GetSettings(NEXUS_ParserBand_e1, &settings);
    settings.allPass = true;
    rc = NEXUS_ParserBand_SetSettings(NEXUS_ParserBand_e1, &settings);
    BDBG_ASSERT(!rc);
