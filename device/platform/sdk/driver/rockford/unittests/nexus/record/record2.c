 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/record/record2.c $
 * 
 * Hydra_Software_Devel/3   5/27/11 11:56a erickson
 * SW7405-691: update tests
 * 
 * Hydra_Software_Devel/2   7/29/10 12:09p calvinho
...
IpsOpenSettings ipsOpenSettings;
    ...
    for (i=0;i<200;i++) {
        ...
        recpumpOpenSettings.data.bufferSize = 192512*4 + rand()%1000000;
        recpumpOpenSettings.data.dataReadyThreshold = 192512 + rand()%(recpumpOpenSettings.data.bufferSize/2-192512);
        recpumpOpenSettings.index.bufferSize = 512 + rand()%1000;
        recpumpOpenSettings.index.dataReadyThreshold = rand()%(recpumpOpenSettings.index.bufferSize/2);
        recpump = NEXUS_Recpump_Open(rand()%NEXUS_NUM_RECPUMPS, &recpumpOpenSettings);
        BDBG_ASSERT(recpump);
