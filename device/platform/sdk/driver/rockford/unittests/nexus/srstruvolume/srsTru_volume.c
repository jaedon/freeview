 ...
 * Revision History:
 *
 * $brcm_Log: /rockford/unittests/nexus/srstruvolume/srsTru_volume.c $
 * 
 * Hydra_Software_Devel/3   9/6/12 6:02p erickson
...
/*---------------------------------------------------------------------SRS Tru Volume command line arguments------------------------------------------------------------------------------------------*/
struct tru_volume_command_args {
	 int loopback;
     const char *config_file;
     NEXUS_TruVolumeSettings pSettings;
...
static void change_config(char *input,struct tru_volume_command_args *tru)
{
    char *value;
    value = strchr(input, '=');
	  *value = 0;
	  ...
	  if(strstr(input,"SRSTVOL_ENABLED"))
		  tru->pSettings.enabled= atoi(value);	      
      else if(strstr(input,"BLOCKSIZE"))
		  tru->pSettings.blockSize = atoi(value);
	   else if(strstr(input,"NORMAL_GAIN_ENABLED"))
		   tru->pSettings.enableNormalGain= atoi(value);
		  ...
		  tru->pSettings.mode = atoi(value);	      
     else if(strstr(input,"BYPASSGAIN"))
		 tru->pSettings.bypassGain= atoi(value);
      else if(strstr(input,"REFERENCELEVEL"))
		  tru->pSettings.referenceLevel = atoi(value);
     else if(strstr(input,"SPEAKER_RESOLUTION"))
		   tru->pSettings.speakerResolution = atoi(value);	  
       else if(strstr(input,"MAXGAIN"))
		   tru->pSettings.maxGain= atoi(value);	      
	   else if(strstr(input,"NOISE_MANAGER_ENABLED"))
		   tru->pSettings.enableNoiseManager= atoi(value);	      
       else if(strstr(input,"NOISE_MANAGER_THRESHOLD"))
		  tru->pSettings.noiseManagerThreshold= atoi(value);	      
      else if(strstr(input,"NORMALIZER_ENABLED"))
		  tru->pSettings.enableNormalizer= atoi(value);	  
	  else if(strstr(input,"CALIBRATE"))
		  tru->pSettings.calibrate= atoi(value);	
...
/*------------------------------------------------------------Initialize the user configuration parameters through the config file----------------------------------------------------------------- */
 void parse_config_file(struct tru_volume_command_args *tru)
{
	...
	while(fgets( line, 400, f )){
	  ...
	  *value = 0;
	  ...
	  if(!strcmp(line,"SRSTVOL_ENABLED"))
		  tru->pSettings.enabled= atoi(value);
     else if(!strcmp(line,"BLOCKSIZE"))
		  tru->pSettings.blockSize = atoi(value);
     else if(!strcmp(line,"NORMAL_GAIN_ENABLED"))
			   tru->pSettings.enableNormalGain= atoi(value);
		 ...
		 tru->pSettings.bypassGain= atoi(value);
     else if(!strcmp(line,"REFERENCELEVEL"))
		  tru->pSettings.referenceLevel = atoi(value);
     else if(!strcmp(line,"SPEAKER_RESOLUTION"))
		   tru->pSettings.speakerResolution = atoi(value);	  
	   else if(!strcmp(line,"MAXGAIN"))
		   tru->pSettings.maxGain= atoi(value);	      
	   else if(!strcmp(line,"NOISE_MANAGER_ENABLED"))
		   tru->pSettings.enableNoiseManager= atoi(value);	      
       else if(!strcmp(line,"NOISE_MANAGER_THRESHOLD"))
		  tru->pSettings.noiseManagerThreshold= atoi(value);	      
       else if(!strcmp(line,"NORMALIZER_ENABLED"))
		  tru->pSettings.enableNormalizer= atoi(value);	  
	  else if(!strcmp(line,"CALIBRATE"))
		  tru->pSettings.calibrate= atoi(value);	
...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/*----------------------------------------------------------- Printing the current set values for the SRS Tru volume user config parameters --------------------------------------------------------- */
 void print_settings(NEXUS_TruVolumeSettings pSettings)
{
	printf("\n\n---------------- The current values of the configuration parameters are as follows: ----------------- \n\n");
	printf ("\n\n");

	
    printf("SRSTVOL_ENABLED=%d\n",pSettings.enabled);
    printf("BLOCKSIZE=%d\n",pSettings.blockSize);
    printf("NORMAL_GAIN_ENABLED=%d\n",pSettings.enableNormalGain);
	printf("INPUTGAIN=%d\n",pSettings.inputGain);
	printf("OUTPUTGAIN=%d\n",pSettings.outputGain);

	printf("MODE=%d\n",pSettings.mode);
	printf("BYPASSGAIN=%d\n",pSettings.bypassGain);
	printf("REFERENCELEVEL=%d\n", pSettings.referenceLevel );
	printf("SPEAKER_RESOLUTION=%d\n", pSettings.speakerResolution );

    printf("MAXGAIN=%d\n", pSettings.maxGain );
	printf("NOISE_MANAGER_ENABLED=%d\n", pSettings.enableNoiseManager );

    printf("NOISE_MANAGER_THRESHOLD=%d\n",pSettings.noiseManagerThreshold);
    printf("NORMALIZER_ENABLED=%d\n", pSettings.enableNormalizer);
...
int tru_volume_cmdline_parse(int argc, const char *argv[], struct util_opts_t *opts, struct tru_volume_command_args *tru)
{
    int i;
    ...
    for (i=1;i<argc;i++) {
        if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
		   ...
		   printf("\n ./\ nexus srsTru_volume -probe streamname -config configfilename -loopback \n");
...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
 ...
 void start_audio(const struct util_opts_t *opts, NEXUS_AudioDecoderHandle audioDecoder, NEXUS_AudioDecoderHandle compressedDecoder,  NEXUS_AudioDecoderStartSettings *audioProgram,NEXUS_AudioCaptureHandle capture, FILE *pFile)
{
    NEXUS_Error rc;
...
void stop_audio(const struct util_opts_t *opts, NEXUS_AudioDecoderHandle audioDecoder, NEXUS_AudioDecoderHandle compressedDecoder, NEXUS_AudioCaptureHandle capture)
{
...
int main(int argc, const char *argv[]) {
    ...
    NEXUS_TruVolumeHandle srsTruvol;
	...
	struct tru_volume_command_args tru_args;
    bool exit1;
    


/*-------------------------------------------------------------Initialization---------------------------------------------------------------------------------------------------------------------- */   
    if (tru_volume_cmdline_parse(argc, argv, &opts, &tru_args)) {
        return 0;
    }
   ...
   /* Bring up all modules for a platform in a default configuraiton for this platform */
	...
	srsTruvol = NEXUS_TruVolume_Open(&tru_args.pSettings);

    NEXUS_TruVolume_GetSettings(srsTruvol,&tru_args.pSettings);
    ...
    NEXUS_TruVolume_SetSettings(srsTruvol,&tru_args.pSettings);
	...
	print_settings(tru_args.pSettings); 
	printf("\n\n To change the value of any config parameters, type the command as 'CONFIG PARAMETER NAME= VALUE' \n Type 'quit' to terminate the application. \n\n\n");
...
/*-----------------------------------------------------------Adding inputs to the corresponding output ports--------------------------------------------------------------------------------------- */
	...
	NEXUS_TruVolume_AddInput(srsTruvol, NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioDecoderConnectorType_eMultichannel));

    rc = NEXUS_AudioOutput_AddInput(
         NEXUS_AudioDac_GetConnector(platformConfig.outputs.audioDacs[0]),NEXUS_TruVolume_GetConnector(srsTruvol));

	BDBG_ASSERT(!rc);

	rc = NEXUS_AudioOutput_AddInput(
         NEXUS_SpdifOutput_GetConnector(platformConfig.outputs.spdif[0]),NEXUS_TruVolume_GetConnector(srsTruvol));
   
    BDBG_ASSERT(!rc);

 	rc =NEXUS_AudioOutput_AddInput(NEXUS_AudioCapture_GetConnector(capture),
							   NEXUS_TruVolume_GetConnector(srsTruvol));
			...
			char buffer[256];
				char *buf;
					...
					do {
							...
							printf("SRS_truvolume>"); fflush(stdout);
						...
						if (!strcmp(buf, "?")) {
						 ...
						 else if ((strstr(buf, "SRSTVOL_ENABLED")!=NULL)||(strstr(buf, "BLOCKSIZE")!=NULL)||(strstr(buf, "NORMAL_GAIN_ENABLED")!=NULL)||(strstr(buf, "INPUTGAIN")!=NULL)||(strstr(buf, "OUTPUTGAIN")!=NULL)||(strstr(buf, "MODE")!=NULL)||(strstr(buf, "BYPASSGAIN")!=NULL)||(strstr(buf, "REFERENCELEVEL")!=NULL)||(strstr(buf, "SPEAKER_RESOLUTION")!=NULL)||(strstr(buf, "MAXGAIN")!=NULL)||(strstr(buf, "NOISE_MANAGER_ENABLED")!=NULL)||(strstr(buf, "NOISE_MANAGER_THRESHOLD")!=NULL)||(strstr(buf, "NORMALIZER_ENABLED")!=NULL)||(strstr(buf, "CALIBRATE")!=NULL) ) {
							...
							rc = NEXUS_AudioOutput_AddInput(
								 NEXUS_AudioCapture_GetConnector(capture),NEXUS_TruVolume_GetConnector(srsTruvol));
