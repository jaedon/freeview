 ...
 *
 * $Id: main.c,v 1.29 2010-05-19 15:26:04 franckf Exp $
 */
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
...
#else
	_vsnprintf(logstr + 6, sizeof(logstr), fmt, argp);
#endif
	va_end(argp);
...
void *DHCP_init(unsigned long subnet, unsigned char starting_ip, unsigned char ending_ip, 
                unsigned long gateway, unsigned long dns, char *interface, DHCP_REQUEST_CALLBACK callback,
                void *pParam)
{
	int rVal;
	gDHCPStatus = 0;
	...
	InitDHCPGlobalConfig(subnet, starting_ip, ending_ip, gateway, dns, callback, pParam);
	...
	while (1) {
		if (DDOK == socketGet(&pPkt)) {
			...
			if (PKT_DISCOVER == pp->packetType) {
...
#if 0 /* SJL -- on hold */
				else if (pp->optServerID == NULL) // a request to verify or extend an existing lease
				{
					// if the DHCPREQUEST message contains a 'Server Identifier' option, the message
					// is in response to a DHCPOFFER message. Otherwise, the message is a request
					// to verify or extend an existing lease.
					DHCPLOG(("DHCP_main: verify the RequestedIP since DHCPREQUEST contains a Server Identifier"));
					// verify if the RequestedIP has been used by another OR
					// if lease has expired
					if (IsIPStillValid(pp) != DDOK)
					{
						DHCPLOG(("DHCP_main: requestedIP is used by another or lease expired, ignored.\n"));
						DHCPLOG(("DHCP_main: tx DHCPNAK(in response to DHCPREQUEST verify) to client\n"));
						SendNAKPkt(pp);
						goto next;
					}
				}
				...
				if (gConfig.callback)
				{
				    DHCPLOG(("DHCP_main: Calling callback now param=%d!\n", gConfig.pParam));
				    (gConfig.callback)(htonl(pIP->IPaddress), gConfig.pParam);
				}
...
#if 0 /* SJL -- on hold */
			} else if (PKT_INFORM == pp->packetType) {
				//
				DHCPLOG(("DHCP_main: rx DHCPINFORM clientIP=0x%08x",
						htonl(*(pp->ClientIP)) ));
				// if the clientIP matches to what we have offered/assigned
				if (DDOK == IPLookup(ntohl(*pp->ClientIP), &pIP)) {
					DHCPLOG(("DHCP_main: tx DHCPACK (in response to DHCPINFORM) to client, IP=0x%08x\n",
							htonl(*pp->ClientIP)));
					SendACKInformPkt(pp, pIP);
				}
#endif
			}
			else
