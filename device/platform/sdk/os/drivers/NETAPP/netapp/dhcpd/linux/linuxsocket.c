 ...
 *
 * $Id: linuxsocket.c,v 1.9 2010-04-01 17:36:33 sjlin Exp $
 */
...
int socketBind(char *interface) {
    ...
    // Bind to a particular interface or to none if interface is not set
    if(setsockopt(hSocket, SOL_SOCKET, SO_BINDTODEVICE, interface, strlen(interface)) < 0){
      perror("setsockopt");
    }
    return DDOK;
