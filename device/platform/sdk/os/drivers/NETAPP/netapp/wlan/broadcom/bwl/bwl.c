 ...
 *
 * $Id: bwl.c,v 1.18 2010/08/13 23:21:01 Exp $
 */
...
//  { BWL_E_ACTION_FRAME,           WLC_E_ACTION_FRAME,             "WLC_E_ACTION_FRAME"},
    ...
    { BWL_E_ESCAN_RESULT,           WLC_E_ESCAN_RESULT,             "BWL_E_ESCAN_RESULT"},
...
static int BWL_ParseBssInfo(wl_bss_info_t *bi, ScanInfo_t *pScanInfo)
{
    ...
    else
    {
        ...
        {
            uint r = dtoh32(bi->rateset.rates[j]) & 0x7f;
            if (r == 0)
                break;
...
)
{
    ...
    err = BWL_GetWSecKey( hBwl, pCred->acKey, sizeof(pCred->acKey) );
    BWL_CHECK_ERR( err  );
    PRINTF(( "acKey = %s\n", pCred->acKey ));
...
#endif
    chanSpec = htod32(chanSpec);
    err = wlu_iovar_set( wl, "chanspec", &chanSpec, sizeof( chanSpec ) );
    BWL_CHECK_ERR( err );
...
*
*   Purpose:
*       Set the data rate (used for transmission test).
*
*   Returns:
*       BWL_ERR_xxx
*
...
(
    BWL_Handle  hBwl,  /* [in] BWL Handle */
    uint32_t      ulVal  /* [in] Data rate */
)
{
    int32_t     err = 0;
    void        *wl = hBwl->wl;
    uint32_t    nrate = 0;
#if defined(NRATE_RATE_MASK) && defined(NRATE_STF_SISO) && defined(NRATE_STF_SHIFT) && defined(NRATE_STF_MASK)
    nrate |= (ulVal * 2) & NRATE_RATE_MASK;
    nrate |= (NRATE_STF_SISO << NRATE_STF_SHIFT) & NRATE_STF_MASK;

    nrate = htod32(nrate);
    err = wlu_iovar_set( wl, "nrate", &nrate, sizeof( nrate ) );
#else
    err = -1;
#endif
    BWL_CHECK_ERR( err );
...
*
*   Purpose:
*       Set the MCS index (used for transmission test).
*
*   Returns:
*       BWL_ERR_xxx
*
...
#if defined(NRATE_RATE_MASK) && defined(NRATE_MCS_INUSE) && defined(HIGHEST_SINGLE_STREAM_MCS) && defined(NRATE_STF_SDM) && defined(NRATE_STF_SISO) && defined(NRATE_STF_SHIFT) && defined(NRATE_STF_MASK)
    nrate |= ulVal & NRATE_RATE_MASK;
    nrate |= NRATE_MCS_INUSE;

    if ((nrate & NRATE_RATE_MASK) <= HIGHEST_SINGLE_STREAM_MCS || (nrate & NRATE_RATE_MASK) == 32)
    {
        nrate |= (NRATE_STF_SISO << NRATE_STF_SHIFT) & NRATE_STF_MASK;
    }
    else
    {
        nrate |= (NRATE_STF_SDM << NRATE_STF_SHIFT) & NRATE_STF_MASK;
    }
...
(
    ...
    uint32          *pVal    /* [out] the counter value */
)
{
    int32_t   err = 0;
    void     *wl = hBwl->wl;
...
int32_t BWL_WoWLPatternAdd(BWL_Handle hBwl, uint32_t ulOffset, char *pMask, char *pValue)
{
    ...
    memset(s_bufdata, 0, sizeof(s_bufdata));
...
#define WLC_BSS_RSSI_ON_CHANNEL 0x0002 /* Copied from wlc.h. Is there a better way to do this? */
                    ...
                    if (!memcmp(bi->SSID, bss->SSID, ETHER_ADDR_LEN) &&
    ...
    if ( (count > 0) && (status != WLC_E_STATUS_ABORT) )
    {
        ...
        if (pScanData != NULL)
        {
            ...
            for (result = escan_bss_head; result; result = result->next, i++) {
                BWL_ParseBssInfo(result->bss, &pScanData[i]);
            #ifdef BWL_DEBUG
                    dump_bss_info(result->bss);
            #endif
            }
...
int32_t BWL_SetAntenna(BWL_Handle hBwl, AntMode_t eAntMode, uint32_t ulAntenna)
{
    ...
    BWL_CHECK_ERR((eAntMode < eAntModeRx) || (eAntMode > eAntModeTx));
