 ...
 *
 * $Id: bwltest.c,v 1.12 2010/08/13 23:21:01 Exp $
 */
...
BWL_Test(int argc, char* argv[])
{
    ...
    else if( !strcmp( argv[1], "connect_no_wep" ) )
    {
        ...
        if( argc == 4 )
        {
            ...
            lErr = BWL_ConnectNoWep( hBwl, eNetOpModeInfra, argv[2] , &peBSSID);
            BWL_CHECK_ERR( lErr );
        } else if ( argc == 3 ) {
            lErr = BWL_ConnectNoWep( hBwl, eNetOpModeInfra, argv[2] , NULL);
            BWL_CHECK_ERR( lErr );
        }
        else
        {
    ...
    else if( !strcmp( argv[1], "get_pmk" ) )
    {
        if( argc > 1 )
        {
            lErr = BWL_GetWSecKey( hBwl, pmk, WSEC_MAX_PSK_LEN );
            BWL_CHECK_ERR( lErr );
        }
        else
        {
