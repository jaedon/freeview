	...
	switch (mlme.im_op) {
		case IEEE80211_MLME_ASSOC: {
			...
			if (mlme.im_ssid_len != 0) {
				...
				join_params.ssid.SSID_len = mlme.im_ssid_len;
...
dhd_net80211_scanresults_get(dhd_pub_t *dhdp, struct ifnet *ifp, struct ieee80211req *ireq)
{
	...
	DHD_TRACE(("%s(): wl_sr->version = %d, wl_sr->count = %d \n",
		__FUNCTION__, wl_sr->version, wl_sr->count));
	...
	for (index = 0; index < wl_sr->count; index++) {
	...
	struct ieee80211req_chaninfo *chans, int max_channels)
{
	...
	for (index = 0; index < list->count; index++) {
		...
		channel = list->element[index];

		/* Get per_chan_info */
		error = _net80211_doiovar(dhdp, "per_chan_info", WLC_GET_VAR, (char *)&channel,
			sizeof(channel), (char *)chan_buf, sizeof(chan_buf), IOV_GET);
		if (error != BCME_OK) {
			break;
		}
...
dhd_net80211_ioctl_get(dhd_pub_t *dhdp, struct ifnet *ifp, u_long cmd, caddr_t data)
{
	...
	switch (ireq->i_type) {
		...
		case IEEE80211_IOC_CURCHAN: {
			...
			channel = chan_info.hw_channel;
			error = _net80211_doiovar(dhdp, "per_chan_info", WLC_GET_VAR,
				(char *)&channel, sizeof(channel),
				(char *)iov_buf, sizeof(iov_buf), IOV_GET);
			if (error != BCME_OK) {
				break;
			}

			chan_bitmap = *((uint *)iov_buf);
