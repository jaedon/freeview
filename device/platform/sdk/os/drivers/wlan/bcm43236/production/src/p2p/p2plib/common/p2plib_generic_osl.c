...
/* Adding API to set an IP Address without calling ifconfig.
...
#define P2P_OSL_RT_METRIC 1
#define P2P_OSL_RT_WINDOW 0
    ...
    int                 tGateway)
{
	...
	rtent.rt_metric         = P2P_OSL_RT_METRIC;
	rtent.rt_window         = P2P_OSL_RT_WINDOW;
...
bool p2papi_osl_set_ap_ipaddr_linux(char *pIFaceName, uint32 ip, uint32 netmask)
{
	...
	if (ip)
	{
		...
		{
			BCMP2PLOG((BCMP2P_LOG_MED, TRUE, "%s(): Could not set netmask",
				__FUNCTION__));
		}
...
#if defined(STBLINUX)
	return (p2papi_osl_set_ap_ipaddr_linux(p2papi_osl_get_ap_mode_ifname(hdl),
	ip, netmask) >= 0);
#else
