...
/* Adding IOCTL API to bring up/down the interface
...
static int p2posl_set_iface(const char* pIFaceName, bool bUp)
{
	...
	{
		p2posl_sleep_ms(P2POSL_IFACE_WAIT);
	}
	...
	ifr.ifr_flags = (bUp) ? (ifr.ifr_flags | IFF_UP | IFF_RUNNING) :
