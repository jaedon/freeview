...
/* Macros */
#define MUTEX_LOCK_INIT(sc)       mtx_init(&(sc)->sc_mtx, device_get_nameunit(self), NULL, MTX_DEF)
#define MUTEX_LOCK_DESTROY(sc)    mtx_destroy(&(sc)->sc_mtx)
#define MUTEX_LOCK(sc)            mtx_lock(&(sc)->sc_mtx)
#define MUTEX_UNLOCK(sc)          mtx_unlock(&(sc)->sc_mtx)
#define MUTEX_LOCK_ASSERT(sc, t)  mtx_assert(&(sc)->sc_mtx, t)
...
#define BCMWL_HW_IN_PENDING_FRAMES  3
#define BCMWL_HW_OUT_PENDING_FRAMES 10
...
static const struct usb_config bwl_config[BWL_N_TRANSFER] =
{
	...
	[BWL_BULK_WR] = {
		...
		.bufsize   = 1700,         /* BCM_RPC_TP_HOST_AGG_MAX_BYTE, */
		.flags     = {
	...
	[BWL_CTRL_RD] = {
		...
		.bufsize   = WLC_IOCTL_MAXLEN + sizeof(cdc_ioctl_t),
		.flags     = {
	...
	[BWL_CTRL_WR] = {
		...
		.endpoint  = 0x00,        /* Control pipe */
		.direction = UE_DIR_OUT,
		.bufsize   = CDC_MAX_MSG_SIZE + sizeof(struct usb_device_request),
		.flags     = {
...
dbus_usbos_intf_send_irb(void *bus, dbus_irb_tx_t *txirb)
{
	...
	if (!txirb->buf || !txirb->len) {
		DBUSERR(("%s(): txirb was expected with valid buf and len\n", __FUNCTION__));
		ASSERT(0);
...
{
	struct usb_xfer *xfer = usbinfo->sc_xfer[BWL_BULK_DL];

	DBUSTRACE(("%s(): Enter \n", __FUNCTION__));

	if ((usbinfo == NULL) || (xfer == NULL) ||(buf == NULL)) {
...
SET_UP_XFER:
			...
			STAILQ_FOREACH(data, &usbos_info->tx_q, next) {
				...
				++nr_frames;
				if (nr_frames >= BCMWL_HW_OUT_PENDING_FRAMES) {
	...
	disconnect_cb_t discb, void *prarg, dbus_intf_t **intf, void *param1, void *param2)
{
	...
	mtx_init(&mod_lock, "mod_lock", NULL, MTX_DEF);
