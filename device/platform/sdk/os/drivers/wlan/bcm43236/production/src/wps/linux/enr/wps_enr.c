...
config_init()
{
...
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_SUPPORT_WPS)
	strcpy(info.deviceName, "Humax Client");
	info.primDeviceCategory = 1;
	info.primDeviceOui = 0x0050F204;
	info.primDeviceSubCategory = 1;
	strcpy(info.manufacturer, "Humax");
	strcpy(info.modelName, "Humax WPS Wireless Client");
	strcpy(info.modelNumber, "0000");
	strcpy(info.serialNumber, "0000");
#else
	 ...
	 */
	if (b_wps_version2) {
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_SUPPORT_WPS) && defined(CONFIG_SUPPORT_PBC_PHYSICAL)
		info.configMethods = (WPS_CONFMET_LABEL | WPS_CONFMET_PHY_PBC | WPS_CONFMET_VIRT_DISPLAY);
#else
		info.configMethods = (/* WPS_CONFMET_LABEL |*/ WPS_CONFMET_VIRT_PBC | WPS_CONFMET_VIRT_DISPLAY);
#endif
	} else {
	 ...
	 */
#if defined(HUMAX_PLATFORM_BASE) && !defined(CONFIG_SUPPORT_ENTERPRISE)
	if (b_wps_version2) {
		info.authTypeFlags = (uint16)(WPS_AUTHTYPE_OPEN | WPS_AUTHTYPE_WPAPSK | WPS_AUTHTYPE_WPA2PSK);
	} else {
		info.authTypeFlags = (uint16)(WPS_AUTHTYPE_OPEN | WPS_AUTHTYPE_WPAPSK |	WPS_AUTHTYPE_SHARED | WPS_AUTHTYPE_WPA2PSK);
	}
#else	 
	if (b_wps_version2) {
