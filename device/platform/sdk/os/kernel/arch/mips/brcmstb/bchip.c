...
#define USB_SOFT_RESET 		BCHP_USB_CTRL_SETUP_soft_reset_MASK
...
static void bchip_usb_init_one(int id, uintptr_t base)
{
	...
	/* soft reset */
	BDEV_SET_RB(USB_REG(base, SETUP), USB_SOFT_RESET);
#endif
	/* endianness setup */
...
{
/*
 * For the 7425B0 with marginal output timing, set the Output Delay
 * when in High Speed mode. See SWLINUX-3023.
 */
#if defined(CONFIG_BCM7425B0)
#define SDIO_CFG_REG_FROM_HOST(x, y) (x + \
	(BCHP_SDIO_0_CFG_##y -	BCHP_SDIO_0_CFG_REG_START) + \
	(BCHP_SDIO_0_CFG_REG_START - BCHP_SDIO_0_HOST_REG_START))


#define BCM7425B0_ODLY 0x80010008 /* Enable=1, Resolution=1, Delay=8 */
	if (reg == SDHCI_HOST_CONTROL) {
		u32 old;
		old = __raw_readl(SDIO_CFG_REG_FROM_HOST(host->ioaddr, OP_DLY));
		if ((old == BCM7425B0_ODLY) !=
			((val & SDHCI_CTRL_HISPD) != 0)) {
			u32 odly;
			...
			__raw_writel(odly,
				SDIO_CFG_REG_FROM_HOST(host->ioaddr, OP_DLY));
		}
	}
#endif
...
static struct sdhci_ops sdhci_ops = {
	.write_b		= sdhci_brcm_writeb,
...
#define SDIO_CFG_FIELD(base, reg, field, val) do {			\
	...
	sdhci_brcm_pdata.ops = &sdhci_ops;
