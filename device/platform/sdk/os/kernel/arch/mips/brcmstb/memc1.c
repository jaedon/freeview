...
static inline void brcm_pm_memc1_sspd_control(int enable)
{
	if (enable) {
...
static void brcm_pm_memc1_suspend_s3(void)
{
	...
	/*
	 * turn off arbiter refresh client
	 * Need to do an uncached read afterwards to be sure
	 * the arbiter wasn't in the middle of a refresh cycle.
	 */
	val = BDEV_RD(BCHP_MEMC_ARB_1_CLIENT_INFO_127);
	val |= 0xffff << 12;
	BDEV_WR(BCHP_MEMC_ARB_1_CLIENT_INFO_127, val);

	/* force memory read */
	_dma_cache_wback_inv(KSEG0ADDR(0x1000), 64);
	val = *(int *)KSEG0ADDR(0x1000);

	/* clear standby field */
	val = BDEV_RD(BCHP_DDR40_PHY_CONTROL_REGS_1_STANDBY_CONTROL);
	val &= ~0x0f;
	BDEV_WR(BCHP_DDR40_PHY_CONTROL_REGS_1_STANDBY_CONTROL, val);
...
#if 0	/* DEBUG Only, will reset DDR when system goes to S3 */
	/*
	 * Go into standby.
	 * No write operations to register between arm and fire.
	 */
	val = BDEV_RD(BCHP_DDR40_PHY_CONTROL_REGS_1_STANDBY_CONTROL);
	val &= ~0x5;
	val |= 0xa;
	...
	/* get status */
	val = BDEV_RD(BCHP_DDR40_PHY_CONTROL_REGS_1_STANDBY_CONTROL);
	printk(KERN_WARNING "%s: memc1 phy control reg 0x%08x\n",
		__func__, val);
#endif
...
int brcm_pm_memc1_suspend(int mode)
{
	CHECK_MEMC1_INIT();
...
int brcm_pm_memc1_resume(int mode)
{
	CHECK_MEMC1_INIT();
