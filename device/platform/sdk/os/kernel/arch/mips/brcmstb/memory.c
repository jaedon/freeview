...
#if defined(HUMAX_PLATFORM_BASE) && !defined(CONFIG_BRCM_FORCED_BMEM_SIZE)
#else
static unsigned int bmem_get_from_kernelconfig = 1;
 ...
 ***********************************************************************/
/* parsing the BMEM information from kernel .config, not detecting the BMEM information from bootloader */	
#if defined(HUMAX_PLATFORM_BASE) && !defined(CONFIG_BRCM_FORCED_BMEM_SIZE)
/*
...
#else /* THE ELSE OF HUMAX_PLATFORM_BASE */
...
static int __init bmem_setup(char *str)
{
	...
	/* Even though the boot evn has many bmem region information, this function is going to execute once */
	if( bmem_get_from_kernelconfig ) {
		bmem_get_from_kernelconfig = 0;
	}else {
		return 0;
	}
	...
	strlcpy(str, CONFIG_BRCM_FORCED_BMEM_SIZE_ADDRESS , MAX_INPUT);
#endif

	/* getting bmem regions information from the kernel config */
	pch = strchr(str, '@');
	while (pch != NULL) {

		/* skip any space if it has more bmem region information */
		if (str[0] == ' ') {
			str = str + 1;
			continue;
		}
		...
		/* checking bmem resion to parse bmem regions */
		pch = strchr(str, '@');
	}
