...
#ifdef CONFIG_BRCM_DDR_RST_CONTROL
/*
 * Set which aon gpio bit is used
 */
#define AON_GPIO_CLAMP BIT(CONFIG_BRCM_DDR_RST_GPIO_PIN)

void brcm_ddr_clamp(int on)
{
	int val;
	int mask, pin;
	/*
	 * setup pin mux controls
	 * pin number is contrained by kconfig from 0 to 17
	 * Clear field to set pinmux to AON_GPIO_pin#
	 */
	pin = CONFIG_BRCM_DDR_RST_GPIO_PIN;

	mask = 0xf << (pin & 0x7);	/* modulo 8 4-bit fields */

	if (pin < 8) {
		val = BDEV_RD(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_0);
		val &= ~mask;
		BDEV_WR(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_0, val);

	} else if (pin < 16) {
		val = BDEV_RD(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_1);
		val &= ~mask;
		BDEV_WR(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_1, val);
	} else {
		val = BDEV_RD(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_2);
		val &= ~mask;
		BDEV_WR(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_2, val);
	}

	val = BDEV_RD(BCHP_GIO_AON_DATA_LO);

#ifdef CONFIG_BRCM_DDR_RST_GPIO_PIN_INVERT
	/* Invert the polarity of the clamp signal */
	on = on ? 0 : 1;
#endif
	if (on)
		val &= ~AON_GPIO_CLAMP;
		...
		val |= AON_GPIO_CLAMP;
	BDEV_WR(BCHP_GIO_AON_DATA_LO, val);
}

void brcm_ddr_clamp_init(void)
{
	int val;

	/* Initial state is 'clamp off' */
	brcm_ddr_clamp(0);

	val = BDEV_RD(BCHP_GIO_AON_DATA_LO);
	val &= ~AON_GPIO_CLAMP;
	BDEV_WR(BCHP_GIO_AON_IODIR_LO, val);

	brcm_ddr_clamp(0);
}
#else
#define  brcm_ddr_clamp(x) do {; }  while (0)
#define brcm_ddr_clamp_init() do {; } while (0)
#endif
...
int __weak brcm_pm_memc1_suspend(int mode) { return 0; }
int __weak brcm_pm_memc1_resume(int mode) { return 0; }
...
struct brcm_chip_pm_block_ops {
	void (*enable)(u32 flags);
	void (*disable)(u32 flags);
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_BCM7429)
/* This temporary code will be substitued with Broadcom official patch */
...
struct brcm_chip_pm_ops {
	 ...
	 */
	void (*late_suspend)(int mode);
	void (*early_resume)(int mode);
...
static void bcm7425_pm_late_suspend(int mode)
{
	/*
...
static void bcm7425_pm_early_resume(int mode)
{
	if (brcm_pm_memc1_power == BRCM_PM_MEMC1_SSPD) {
		brcm_pm_memc1_resume(mode);
		brcm_pm_memc1_power = BRCM_PM_MEMC1_ON;
...
static void brcm_system_late_standby(int mode)
{
	if (chip_pm_ops.late_suspend)
		chip_pm_ops.late_suspend(mode);
}

static void brcm_system_early_resume(int mode)
{
	if (chip_pm_ops.early_resume)
		chip_pm_ops.early_resume(mode);
}
