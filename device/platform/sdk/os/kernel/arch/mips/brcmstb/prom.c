...
#include "linux/flash_map.h"
#endif /* THE END OF HUMAX_PLATFORM_BASE */
 ...
 * Main entry point
 ***********************************************************************/
#if defined(CONFIG_BMIPS3300) && defined(CONFIG_CPU_CLOCK_600_FORCED)
static void set_clocks_for_profile(void)
{
	unsigned long reg;

#define CPU_MDIV		5
#define AVD_MDIV		10
#define AVD_CPU_MDIV	8
#define DSP_MDIV		8
#define SCB_MDIV		11

	/* Setting CPU clock */

	reg = BDEV_RD(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0);
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0_MDIV_CH0_MASK;
	reg |= (CPU_MDIV << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0_MDIV_CH0_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0, reg);

	/* Generate the pulse by writing 1 followed by 0 to POST_DIVIDER_LOAD_EN_CH0 bit*/
	reg |= (1 << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0_POST_DIVIDER_LOAD_EN_CH0_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0, reg);
	/* clear the POST_DIVIDER_LOAD_EN_CH0 bit */
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0_POST_DIVIDER_LOAD_EN_CH0_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_0, reg);

	/* Setting AVD core clock */
	reg = BDEV_RD(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1);
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1_MDIV_CH1_MASK;
	reg |= (AVD_MDIV << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1_MDIV_CH1_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1, reg);

	/* Generate the pulse by writing 1 followed by 0 to POST_DIVIDER_LOAD_EN_CH1 bit*/
	reg |= (1 << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1_POST_DIVIDER_LOAD_EN_CH1_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1, reg);
	/* clear the POST_DIVIDER_LOAD_EN_CH1 bit */
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1_POST_DIVIDER_LOAD_EN_CH1_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_1, reg);

	/* Setting AVD CPU clock */
	reg = BDEV_RD(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2);
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2_MDIV_CH2_MASK;
	reg |= (AVD_CPU_MDIV << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2_MDIV_CH2_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2, reg);

	/* Generate the pulse by writing 1 followed by 0 to POST_DIVIDER_LOAD_EN_CH2 bit*/
	reg |= (1 << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2_POST_DIVIDER_LOAD_EN_CH2_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2, reg);
	/* clear the POST_DIVIDER_LOAD_EN_CH2 bit */
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2_POST_DIVIDER_LOAD_EN_CH2_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_2, reg);

	/* Setting DSP clock */
	reg = BDEV_RD(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3);
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3_MDIV_CH3_MASK;
	reg |= (DSP_MDIV << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3_MDIV_CH3_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3, reg);

	/* Generate the pulse by writing 1 followed by 0 to POST_DIVIDER_LOAD_EN_CH3 bit*/
	reg |= (1 << BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3_POST_DIVIDER_LOAD_EN_CH3_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3, reg);
	/* clear the POST_DIVIDER_LOAD_EN_CH3 bit */
	reg &= ~BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3_POST_DIVIDER_LOAD_EN_CH3_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_AVD_MIPS_PLL_CHANNEL_CTRL_CH_3, reg);

	/* Setting SCB clock */
	reg = BDEV_RD(BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3);
	reg &= ~BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3_MDIV_CH3_MASK;
	reg |= (SCB_MDIV << BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3_MDIV_CH3_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3, reg);

	/* Generate the pulse by writing 1 followed by 0 to POST_DIVIDER_LOAD_EN_CH3 bit*/
	reg |= (1 << BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3_POST_DIVIDER_LOAD_EN_CH3_SHIFT);
	BDEV_WR(BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3, reg);
	/* clear the POST_DIVIDER_LOAD_EN_CH3 bit */
	reg &= ~BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3_POST_DIVIDER_LOAD_EN_CH3_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_3, reg);
}
#endif
...
void __init prom_init(void)
{
	...
	if (!strstr(arcs_cmdline, "console"))
	{
		...
		sprintf(buf, " console=ttyS%d,115200", CONFIG_BRCM_CONSOLE_DEVICE);
		strcat(arcs_cmdline, buf);
	}
#endif
	...
	/* provide "mtd2ubi" alias to finish the dependence with the bootloader parameter */
#if defined(HUMAX_PLATFORM_BASE)
#if defined(CONFIG_MTD_BRCMNAND)
	if (strstr(arcs_cmdline, "mtd2ubi")) {
		char buf[64];
		sprintf(buf, " ubi.mtd=%d root=%s ro", N_HMX_MTD2UBIBLOCK_NODE, N_HMX_MTD2UBIROOTFS_NODE );
		strcat(arcs_cmdline, buf);
	}
#endif	

	ptr = strstr(arcs_cmdline, "macaddr=");
	if (ptr) {
		if (parse_eth0_hwaddr( ptr + sizeof("macaddr=") - 1, brcm_primary_macaddr) != 0) {
			/* default setting again because of invalid mac address */
			brcm_primary_macaddr[0] = 0x00; brcm_primary_macaddr[1] = 0x00; brcm_primary_macaddr[2] = 0xde;
			brcm_primary_macaddr[3] = 0xad; brcm_primary_macaddr[4] = 0xbe; brcm_primary_macaddr[5] = 0xef;
		}
	}
#endif /* THE END OF HUMAX_PLATFORM_BASE */

#if defined(CONFIG_BRCM_FORCED_BMEM_SIZE)
	ptr = strstr(CONFIG_BRCM_FORCED_BMEM_SIZE_ADDRESS, "memc1=");
#else
