...
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_BRCMNAND_CHECK_ERASEDPAGE_BITFLIP)
#include <linux/bch.h>
#endif
...
static struct brcmstb_nand_exception brcmstb_exceptions_list[] = {
	...
	{"Spansion S34ML04G2",
		{0x01, 0xDC, 0x90, 0x95, 0x56, 0x00, 0x00},
		5, 0x200, 2048, 0x20000, 64},
	{"JSC JS27HU4G08",
		{0xAD, 0xDC, 0x90, 0x95, 0x56, 0x00, 0x00},
		5, 0x200, 2048, 0x20000, 64},
#endif
	{NULL,}
};
...
#if defined(HUMAX_PLATFORM_BASE)&& defined(CONFIG_BRCMNAND_CHECK_ERASEDPAGE_BITFLIP)
struct brcmstb_nand_bch_control {
	struct bch_control *pstBCH;
	unsigned int *punBCH_Errloc;
	unsigned char *pucErasedPageECC;
	int nEccCalculateSize;
};

static struct brcmstb_nand_bch_control bch_ctrl;
static void brcmstb_nand_swap32(unsigned char *pucData);
static int brcmstb_nand_calculate(u8 *data, int nSectorSize, u8 *ecc_code);
static int brcmstb_nand_correct(u8 *data, u8 *calc_ecc);
static int brcmstb_nand_check_ErasedPageBitflip(struct mtd_info *mtd, struct nand_chip *chip, u64 err_addr, u8 *data, u8 *oob);
#endif
	...
	u32 *buf, u8 *oob)
{
	...
	if (err_addr != 0) {
		...
		if (buf && oob)
		{
			if(brcmstb_nand_check_ErasedPageBitflip(mtd, chip, err_addr, (u8*)buf, oob) == 0)
			{
				dev_warn(&host->pdev->dev, "Ignore UECC error(0x%llx)\n",(unsigned long long)err_addr);
				memset(buf, 0xFF, (FC_BYTES*trans));
				mtd->ecc_stats.corrected += CORR_ERROR_COUNT;
			}
			else
			{
				dev_warn(&host->pdev->dev, "It's real UECC(0x%llx)\n",(unsigned long long)err_addr);
...
static int brcmstb_nand_calculate(u8 *data, int nSectorSize, u8 *ecc_code)
{
	unsigned char *pucDataForCalculate;
	int nBICount;
	int nIdx;
	int ret=0;
	...
	if((!data)||(!ecc_code))
	{
		printk(KERN_ERR "%s: Invalid argument, data(%p), ecc_code(%p)\n", __func__, data, ecc_code);
		ret = -EINVAL;
		return ret;
	}

	pucDataForCalculate = kmalloc(bch_ctrl.nEccCalculateSize, GFP_KERNEL);
	if(!pucDataForCalculate)
	{
		printk(KERN_ERR "%s: can't allocate memory\n", __func__);
		ret = -ENOMEM;
		return ret;
	...
	nBICount = bch_ctrl.nEccCalculateSize - nSectorSize - bch_ctrl.pstBCH->ecc_bytes;

	printk(KERN_DEBUG "nSectorSize = %d\n", nSectorSize);
	printk(KERN_DEBUG "nBIcount = %d\n", nBICount);

	memset(pucDataForCalculate, 0xFF, bch_ctrl.nEccCalculateSize);
	memset(ecc_code, 0xFF, bch_ctrl.pstBCH->ecc_bytes);


	/**************************************************************************************************/
 	/*     | ECC Data (bch_ctrl.pstBCH->ecc_bytes) | Data(nSectorSize) | BI(total_spare - ECC data)|  */
	/*                                                                                                */
 	/* ex) For, MLC(Micron MT29F32G08CBACA)                                                           */
 	/* ex) | ECC Data (42 Bytes) | Data(1024 Bytes) | BI(12 Bytes)|                                   */
	/**************************************************************************************************/

	/* Data swap */
	for(nIdx=0; nIdx<nSectorSize; nIdx+=4)
	{
		brcmstb_nand_swap32(&data[nIdx]);
	}

	/* Data relocation */
	for(nIdx=0; nIdx<nSectorSize; nIdx++)
	{
		pucDataForCalculate[nIdx+bch_ctrl.pstBCH->ecc_bytes] = data[nIdx];
	}

	/* oob relocation(ECC Data) */
	for(nIdx=0; nIdx<bch_ctrl.pstBCH->ecc_bytes; nIdx++)
	{
		pucDataForCalculate[nIdx] = data[nSectorSize + nBICount + nIdx];
	}

	/* oob relocation(BI) */
	for(nIdx=0; nIdx<nBICount; nIdx++)
	{
		pucDataForCalculate[nIdx+bch_ctrl.pstBCH->ecc_bytes+nSectorSize] = data[nSectorSize + nIdx];
	}

	encode_bch(bch_ctrl.pstBCH, pucDataForCalculate, bch_ctrl.nEccCalculateSize, ecc_code);

	kfree(pucDataForCalculate);

	for(nIdx=0; nIdx < bch_ctrl.pstBCH->ecc_bytes; nIdx++)
	{
		printk(KERN_DEBUG "[%d] 0x%x\n", nIdx, ecc_code[nIdx]);
	}
...
static int brcmstb_nand_correct(u8 *data, u8 *calc_ecc)
{
	...
	if(!calc_ecc)
	{
		printk(KERN_ERR "%s: Invalid argument, calc_ecc(%p)\n", __func__, calc_ecc);
		ret = -EINVAL;
		return ret;
		
	}

	nBitflipCount = decode_bch(bch_ctrl.pstBCH, NULL, bch_ctrl.nEccCalculateSize, bch_ctrl.pucErasedPageECC, calc_ecc, NULL, bch_ctrl.punBCH_Errloc);
	if(nBitflipCount > 0)
	{
		ret=0;

		printk(KERN_ERR "%s : Correctable Bitflip Count(%d)\n", __func__, nBitflipCount);
	
		for(nIdx=0; nIdx<nBitflipCount; nIdx++)
		{
			if(bch_ctrl.punBCH_Errloc[nIdx] < (1024+54)*8)
			{
				/* There are bitfilps in data area */
				nBuffloc = bch_ctrl.punBCH_Errloc[nIdx]>>3;
				printk(KERN_DEBUG "Bitflip loc(%d)\n", nBuffloc);
				
			}
			else
			{
				printk(KERN_DEBUG "Bitflip in ECC loc\n");
			}
			
			printk(KERN_DEBUG "Correctable Bitflip (%u)\n", bch_ctrl.punBCH_Errloc[nIdx]);
		}	

	}
	else if(nBitflipCount < 0)
	{
		ret = -1;
...
{
	struct brcmstb_nand_host *host = chip->priv;
	unsigned char *pucDataForCheck;
	unsigned char *pucCalcEcc;
	int nCheckBCH=0;
	int nSectorSize;
	int nOOBSizePerSector;
	int nErrIdx;
	int nIdx;
	int ret;
	...
	if((!data)||(!oob))
	{
		printk(KERN_ERR "%s: Invalid argument, data(%p), oob(%p)\n", __func__, data, oob);
		return -EINVAL;
	}

	nSectorSize = host->hwcfg.sector_size_1k == 1 ? 1024 : 512;
	nOOBSizePerSector = host->hwcfg.spare_area_size << host->hwcfg.sector_size_1k;
	
	printk(KERN_DEBUG "nSectorSize(%d), nOOBSizePerSector(%d)\n", nSectorSize, nOOBSizePerSector);

	nErrIdx = nSectorSize == 1024 ? ((err_addr>>10)&0x3) : ((err_addr>>9)&0x3);
	printk(KERN_DEBUG "Err_addr(0x%llx), nErrIdx(%d)\n", err_addr, nErrIdx);

	/* Check Whether OOB Data are 0xFF or not*/
	for(nIdx=0; nIdx<nOOBSizePerSector; nIdx++)
	{
		if(oob[(nErrIdx*nOOBSizePerSector)+nIdx] != 0xFF)
		{
			nCheckBCH=1;
			break;
		}
	...
	if(!nCheckBCH)
	{
		printk(KERN_ERR "There are bit flip in erased page.(by OOB).\n");
		return 0;
	}
	else
	{
		pucDataForCheck  = kmalloc(bch_ctrl.nEccCalculateSize, GFP_KERNEL);
		if(!pucDataForCheck)
		{
			printk(KERN_ERR "%s: can't allocate memory\n", __func__);
			return -ENOMEM;
		...
		pucCalcEcc = kmalloc(bch_ctrl.pstBCH->ecc_bytes, GFP_KERNEL);
		if(!pucCalcEcc)
		{
			printk(KERN_ERR "%s: can't allocate memory\n", __func__);
			kfree(pucDataForCheck);
			return -ENOMEM;
		}
		...
		for(nIdx=0; nIdx<nSectorSize; nIdx++)
		{
			pucDataForCheck[nIdx] = data[(nErrIdx*nSectorSize)+nIdx];
		}	

		/* Uncorrectable OOB */
		for(nIdx=0; nIdx<nOOBSizePerSector; nIdx++)
		{
			pucDataForCheck[nSectorSize + nIdx] = oob[(nErrIdx*nOOBSizePerSector)+nIdx];
		}	
	
		ret = brcmstb_nand_calculate(pucDataForCheck, nSectorSize, pucCalcEcc);
		if(ret<0)
		{
			printk(KERN_ERR "%s: Error in brcmstb_nand_calculate\n", __func__);
			kfree(pucDataForCheck);
			kfree(pucCalcEcc);
			return -1;
		}

		ret = brcmstb_nand_correct(NULL, pucCalcEcc);
		if(ret<0)
		{
			printk(KERN_DEBUG "%s: Uncorrectable Error in brcmstb_nand_correct\n", __func__);
		}
		else
		{
			printk(KERN_ERR "There are bit flip in erased page.(by BCH).\n");
		}
...
static int __devinit brcmstb_nand_probe(struct platform_device *pdev)
{
...
#if defined(HUMAX_PLATFORM_BASE)&& defined(CONFIG_BRCMNAND_CHECK_ERASEDPAGE_BITFLIP)
	struct brcmstb_nand_cfg stCfg;
	int nEccCorrectCab;
	int nFieldOrder;
	int nIdx;
	unsigned char *pucErasedPage;
#endif
...
#if defined(HUMAX_PLATFORM_BASE)&& defined(CONFIG_BRCMNAND_CHECK_ERASEDPAGE_BITFLIP)
	brcmstb_nand_get_cfg(host, &stCfg);
	printk(KERN_DEBUG "stCfg.sector_size_1k = %d\n", stCfg.sector_size_1k);
	printk(KERN_DEBUG "stCfg.ecc_level = %d\n", stCfg.ecc_level);
	printk(KERN_DEBUG "stCfg.spare_area_size = %d\n", stCfg.spare_area_size);

	bch_ctrl.nEccCalculateSize = stCfg.sector_size_1k == 1 ? (1024+(stCfg.spare_area_size<<1)) : (512+stCfg.spare_area_size);
	
	nEccCorrectCab = stCfg.ecc_level << stCfg.sector_size_1k;
	nFieldOrder = fls(1+8*bch_ctrl.nEccCalculateSize);

	printk(KERN_DEBUG "nEccCalculateSize = %d\n", bch_ctrl.nEccCalculateSize);
	printk(KERN_DEBUG "nEccCorrectCab = %d\n", nEccCorrectCab);
	printk(KERN_DEBUG "nFieldOrder = %d\n", nFieldOrder);

	bch_ctrl.pstBCH = init_bch(nFieldOrder, nEccCorrectCab, CONFIG_BRCMNAND_BCH_POLYNOMIAL);
	if(!bch_ctrl.pstBCH)
	{
		ret = -ENXIO;
		goto out;
	...
	printk(KERN_DEBUG "BCH ECC bytes = %d\n", bch_ctrl.pstBCH->ecc_bytes);	
	
	bch_ctrl.pucErasedPageECC = kmalloc(bch_ctrl.pstBCH->ecc_bytes, GFP_KERNEL);
	if(!bch_ctrl.pucErasedPageECC)
	{
		dev_err(&pdev->dev, "can't allocate memory\n");
		ret = -ENOMEM;
		goto out;
	...
	bch_ctrl.punBCH_Errloc = kmalloc(nEccCorrectCab*sizeof(bch_ctrl.punBCH_Errloc), GFP_KERNEL);
	if(!bch_ctrl.punBCH_Errloc)
	{
		dev_err(&pdev->dev, "can't allocate memory\n");
		ret = -ENOMEM;
		goto out;
	...
	pucErasedPage = kmalloc(bch_ctrl.nEccCalculateSize, GFP_KERNEL);
	if(!pucErasedPage)
	{
		dev_err(&pdev->dev, "can't allocate memory\n");
		ret = -ENOMEM;
	...
	memset(pucErasedPage, 0xFF, bch_ctrl.nEccCalculateSize);
	memset(bch_ctrl.pucErasedPageECC, 0xFF, bch_ctrl.pstBCH->ecc_bytes);

	encode_bch(bch_ctrl.pstBCH, pucErasedPage, bch_ctrl.nEccCalculateSize, bch_ctrl.pucErasedPageECC);
	kfree(pucErasedPage);

	for(nIdx=0; nIdx < bch_ctrl.pstBCH->ecc_bytes; nIdx++)
	{
		printk(KERN_DEBUG "First[%d] 0x%x\n", nIdx, bch_ctrl.pucErasedPageECC[nIdx]);
	}
#endif
