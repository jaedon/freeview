...
static void *pDummy_buf;
static loff_t Dummy_addr;
#endif
...
#if defined(HUMAX_PLATFORM_BASE)
	/*Apply UBI official patch: Commit_ID: 276832d878d8a892ac7b40fd0ee07fe757e080c7*/
	/*
...
/* DUMMY READ */
	...
	if(!pDummy_buf)
	{
		printk(KERN_ERR "pDummy_buf is null (0x%x)\n", (unsigned int)pDummy_buf);
		pDummy_buf = kzalloc(ubi->peb_size, GFP_NOFS);
		if (!pDummy_buf)
			return -ENOMEM;

		printk(KERN_ERR "Alloc pDummy_buf : size(%d)\n", ubi->peb_size);
	}
	
	Dummy_addr = (loff_t)pnum * ubi->peb_size;
//	printk(KERN_ERR "pDummy_buf is NOT null (0x%x) and addr(0x%x)\n", pDummy_buf, Dummy_addr);
#endif
...
#if defined(HUMAX_PLATFORM_BASE)
	/*Apply UBI official patch: Commit_ID: a87f29cbbcbd5bd1e4990367cd18967e9bbeacff*/
		...
		if (read != len && retries++ < UBI_IO_RETRIES) {
			...
			printk(KERN_ERR "DUMMY READ size(%d) addr(0x%x)\n", ubi->peb_size, (unsigned int)Dummy_addr);
			ubi->mtd->read(ubi->mtd, Dummy_addr, ubi->peb_size, &read, pDummy_buf);
#else
