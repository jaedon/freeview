...
#if defined(CONFIG_GIGA_ETH_EXT_PHY)
#if defined(CONFIG_RTL8211E)
static void mii_set_mdc_clock(struct BcmEnet_devctrl *pDevCtrl);
static void mii_disable_clk125(struct BcmEnet_devctrl *pDevCtrl);
static void mii_enable_ssc(struct BcmEnet_devctrl *pDevCtrl);
#endif
#endif
#endif
...
#define CLOCK_DIVIDER_RESETXORVAL	0x00000160
#define CLOCK_DIVIDER_RESETANDVAL	0xFFFFFC0F

/* 
For Realtek RTL8211E ext phy.
Defaults clock devider is 0x02 so, MDC is generated as 17.99Mhz. 
RTL8211E need to set this 0x16 to generate 2.347MHz 
 */	
static void mii_set_mdc_clock(struct BcmEnet_devctrl *pDevCtrl)
{
	volatile unsigned long clock_dev = 0;

	clock_dev = pDevCtrl->umac->mdio_cfg;
	clock_dev = ((clock_dev & CLOCK_DIVIDER_RESETANDVAL)|CLOCK_DIVIDER_RESETXORVAL);
	pDevCtrl->umac->mdio_cfg = clock_dev;
	printk(KERN_ERR "mii_set_mdc_clock 0x%lx \n", pDevCtrl->umac->mdio_cfg);

}

/* HMS-1000S EMI test failed : request from HW team. */
static void mii_disable_clk125(struct BcmEnet_devctrl *pDevCtrl)
{
	unsigned short data = 0;
	
	/* Disable CLK125 */
	data = pDevCtrl->mii.mdio_read(pDevCtrl->dev, pDevCtrl->phyAddr, 0x10);
	data = (data | 0x10);
	pDevCtrl->mii.mdio_write(pDevCtrl->dev, pDevCtrl->phyAddr, 0x10, data);
	pDevCtrl->mii.mdio_read(pDevCtrl->dev, pDevCtrl->phyAddr, 0x10);
}
...
static void mii_enable_ssc(struct BcmEnet_devctrl *pDevCtrl)
{
	...
	/* Enable RXC Spread Spectrum Clock (SSC) */
	...
	/* Extension page 160 */
	data = 0x00A0;
	pDevCtrl->mii.mdio_write(pDevCtrl->dev, pDevCtrl->phyAddr, 0x1E, data);

	/* Enable RXC SSC : bit[2]=0 */
	data = pDevCtrl->mii.mdio_read(pDevCtrl->dev, pDevCtrl->phyAddr, 0x1A);
	data = (data & 0xFFFB);	
	pDevCtrl->mii.mdio_write(pDevCtrl->dev, pDevCtrl->phyAddr, 0x1A, data);
...
static int bcmgenet_open(struct net_device *dev)
{
	...
	{
		mii_set_mdc_clock(pDevCtrl);
		mii_disable_clk125(pDevCtrl);
		mii_enable_ssc(pDevCtrl);
	}
#endif 
#endif
#endif
