	...
	do {
		...
		do {
			...
			the_result = scsi_execute_req(sdkp->device, cmd,
				...
				printk("[Trying again Spin up drives [%d] [%s] [%s] the_result = %d\n", __LINE__, __FUNCTION__, __FILE__, the_result);
				try_again++;
				msleep(300);
			} while ( try_again > 5 );
#endif
...
#if defined(HUMAX_PLATFORM_BASE)
	if(index<24 && index>=0) {
		const char *template_name = sdkp->device->host->hostt->name;
		if(template_name && strstr(template_name, "usb")) {
			atomic_add((1<<index),&conn_status);
			flag_wakeup = 1;
...
#if defined(HUMAX_PLATFORM_BASE)
	if(sdkp->disk->disk_name) {
		const char *template_name = sdkp->device->host->hostt->name;
		int i;
		
		if(template_name && strstr(template_name, "usb")) {		
			i = sdkp->disk->disk_name[2]-'a';
			atomic_sub((1<<i),&conn_status);
			flag_wakeup = 1;
			wake_up_interruptible(&sd_conn_wait);		
		}
	}
