		...
		if(current_option==0)
		{
			if(argv[i][0]=='-')
			{
				...
				switch(argv[i][1])
				{
					...
					case 'i':	//	id
					case 'p':	//	passwd
					case 'c':	//	default folder
					case 's':	//	CAS security
						if (len > 2)
						{
							assign_option(argv[i][1], argv[i] + 2, copts, len - 2);
							current_option = 0;
						} else
	...
	int len;
{
	...
	{
		case	'i' :
			copts->acceptUserId = malloc(sizeof(char*) * strlen(arg) + 1);
			strcpy(copts->acceptUserId, arg);
			//	Set data to low case
			if (copts->acceptUserId)
			{
				//	Set data to low case
				int		i;
				for (i = 0; i < strlen(copts->acceptUserId); i++)
				{
					if ((copts->acceptUserId[i] >= 0x41) && (copts->acceptUserId[i] <= 0x5A))
						copts->acceptUserId[i] += 0x20;
				}
			}
		...
		case	'p' :
			copts->password = malloc(sizeof(char*) * strlen(arg) + 1);
			strcpy(copts->password, arg);
		...
		case 's': // cas security 
			if (arg && strcmp(arg, "ir") == 0)
			{
				copts->cas_type	= CAS_IRDETO;
			}
			break;
	}
#else
