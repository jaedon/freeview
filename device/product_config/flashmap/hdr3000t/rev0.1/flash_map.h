...
#define N_TOTAL_SIZE						0x10000000
...
#define FLASH_DUMY_BYTE_SIZE				0x4
#else
#define FLASH_DUMY_BYTE_SIZE				0x0
#endif

#define DUMY_BYTE_SIZE		0x0



#define FSBL_SIZE			(128*KILOBYTES)
#define SSBL_SIZE			(128*KILOBYTES)
#define LOADER_SIZE					(1*MEGABYTES)

/* NAND Map */
#define FSBL_LOGICAL_OFFSET			(0x0)
#define SSBL_LOGICAL_OFFSET			(FSBL_LOGICAL_OFFSET+FSBL_SIZE)
#define LOADER_LOGICAL_OFFSET		(SSBL_LOGICAL_OFFSET+SSBL_SIZE)
#define LDR_ACTUAL_LDR_SIZE		(LOADER_LOGICAL_OFFSET+LOADER_SIZE)

/* NOR Map */
#define BOOT_ENTRY_BASE				(0x9FC00000)
#define FSBL_OFFSET					(BOOT_ENTRY_BASE)
#define SSBL_OFFSET					(FSBL_OFFSET+FSBL_SIZE)
#define LOADER_OFFSET				(SSBL_OFFSET+SSBL_SIZE)
...
#define FLASH_UBI_AREA_ROOTFS_SIZE			(64*MEGABYTES+60*KILOBYTES)		/* Root FS(64MB+60kB) */
#define FLASH_UBI_AREA_KERNEL_SIZE			(4*MEGABYTES-4*KILOBYTES)		/* Kernel(4MB-4kB) */
#define FLASH_UBI_AREA_RAWFS_SIZE			(68*MEGABYTES+180*KILOBYTES)	/* RAW FS(68MB+180kB) */
#define FLASH_UBI_AREA_NVRAM_SIZE			(1984*KILOBYTES)				/* NVRAM(1984kB) */
#define FLASH_UBI_AREA_RESERVED_SIZE		(12*MEGABYTES+852*KILOBYTES)	/* Reserved(12MB+852kB) */
...
#define FLASH_UBIFS_AREA_SIZE				(100*MEGABYTES)				/* DB backup(16MB) + Reserved(1MB) + DB(16MB) + Reserved(1MB) + DB user(58MB)+ Reserved(8MB) */
#define UBIFS_PART1_SIZE					(16*MEGABYTES) 				/* db_backup area */
#define UBIFS_PART2_SIZE					(16*MEGABYTES) 				/* db area */
#define UBIFS_PART3_SIZE					(58*MEGABYTES)				/* db user area*/

#define NUM_UBIFS_PART						(3)							/* total number of ubifs partition*/
...
#define N_HMX_MTD2UBIDBUSER_NODE			"/dev/mtdblock8"			/* ubi volume */
