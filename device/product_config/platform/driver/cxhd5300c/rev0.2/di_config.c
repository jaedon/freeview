...
const GPIO_ConfigInfo_t GPIO_ConfigInfoTable[] =
{
	...
	{GPIO_ID_HW_MUTE,	NULL,	NEXUS_GpioType_eAonStandard,	6,	GPIO_MODE_WRITE,	GPIO_INT_MODE_DISABLED,		GPIO_HI},

    	{GPIO_ID_HW_REV0,	NULL,	NEXUS_GpioType_eAonStandard,	12,	GPIO_MODE_READ,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
    	{GPIO_ID_HW_REV1,	NULL,	NEXUS_GpioType_eAonStandard,	13,	GPIO_MODE_READ,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
    	{GPIO_ID_HW_REV2,	NULL,	NEXUS_GpioType_eAonStandard,	14,	GPIO_MODE_READ,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},

#if defined(CONFIG_FRONTKEY_GPIO)
	/* front gpio key */
	{GPIO_ID_KEY_STBY,	NULL,	NEXUS_GpioType_eAonStandard,	3,	GPIO_MODE_READ,		GPIO_INT_MODE_FALLING_EDGE,		GPIO_INVAILD},
#endif
...
HUINT32 DRV_CFG_PlatformInit(void)
{
	...
	/*
		...
		nexus example 에서 위와 같이 명시하고 잇지만. DI init 구조상 frontend init 이 display 모듈보다 상위에 잇으므로
		그냥 platform init 시에 처리하도록 하자.
	*/
...
#endif
	platformSettings.openFpga = false; /* 7584 의 경우 기본적으로 NEXUS_FPGA_SUPPORT 을 지원하지 않으므로 그냥 냅둬도 상관은 없음. */
