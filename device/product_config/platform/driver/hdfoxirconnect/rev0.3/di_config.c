...
const GPIO_ConfigInfo_t GPIO_ConfigInfoTable[] =
{
	...
	{GPIO_ID_LOOPTHROUGH,       NULL,  	NEXUS_GpioType_eStandard, 		64,    	GPIO_MODE_WRITE,    	GPIO_INT_MODE_DISABLED,    	GPIO_HI},
	{GPIO_ID_LED_STBY,			NULL,	NEXUS_GpioType_eStandard,		62,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_LOW},
	{GPIO_ID_IRQ_FRONT,			NULL,	NEXUS_GpioType_eStandard,		61,		GPIO_MODE_INTERRUPT,	GPIO_INT_MODE_FALLING_EDGE,	GPIO_INVAILD},
	{GPIO_ID_IRQ_FRONT2,		NULL,	NEXUS_GpioType_eStandard,		63,		GPIO_MODE_INTERRUPT,	GPIO_INT_MODE_FALLING_EDGE,	GPIO_INVAILD},
	{GPIO_ID_LNBP_SW_EN,    	NULL,   NEXUS_GpioType_eAonStandard, 	10,     GPIO_MODE_WRITE,      	GPIO_INT_MODE_DISABLED,     GPIO_HI},
	{GPIO_ID_SC0_PWR_SEL,  		NULL,   NEXUS_GpioType_eAonStandard, 	8,   	GPIO_MODE_WRITE,    	GPIO_INT_MODE_DISABLED,   	GPIO_LOW},
	{GPIO_ID_SC1_PWR_SEL,    	NULL,   NEXUS_GpioType_eStandard, 		22,		GPIO_MODE_WRITE,    	GPIO_INT_MODE_DISABLED,   	GPIO_LOW}
};
...
#define BSPCFG_P_HEAP_XVD_GENERAL_SIZE			25*1024*1024 /* 25M */	
#define BSPCFG_P_HEAP_XVD_PICTURE_SIZE			65*1024*1024 /* 65M */
...
HUINT32 DRV_CFG_PlatformInit(void)
{
	...
	{
		uint8_t i;
		platformSettings.heap[NEXUS_MEMC0_PICTURE_BUFFER_HEAP].size = (129*1024*1024);
		for(i = 0; i < NEXUS_MAX_HEAPS; i++)
		{			
			PrintDebug("heap(%d) memcIndex(%d), subIndex(%d), size(%d), memoryType(%d)\n", i, \
			platformSettings.heap[i].memcIndex, platformSettings.heap[i].subIndex, platformSettings.heap[i].size, platformSettings.heap[i].memoryType);
		}
	}
#endif
	...
	{
		PrintDebug("numDecodes (%d), numStillDecodes (%d), numMosaicDecodes (%d)\n", 
		platformSettings.videoDecoderModuleSettings.numDecodes, platformSettings.videoDecoderModuleSettings.numStillDecodes, platformSettings.videoDecoderModuleSettings.numMosaicDecodes);
		
		PrintDebug("general (%d), secure (%d), picture (%d)\n", 
		platformSettings.videoDecoderModuleSettings.heapSize[0].general, platformSettings.videoDecoderModuleSettings.heapSize[0].secure, platformSettings.videoDecoderModuleSettings.heapSize[0].picture);
		
		platformSettings.videoDecoderModuleSettings.heapSize[0].general = BSPCFG_P_HEAP_XVD_GENERAL_SIZE;
		platformSettings.videoDecoderModuleSettings.heapSize[0].secure = BSPCFG_P_HEAP_XVD_SECURE_SIZE;
		platformSettings.videoDecoderModuleSettings.heapSize[0].picture = BSPCFG_P_HEAP_XVD_PICTURE_SIZE;
		platformSettings.videoDecoderModuleSettings.numDecodes = BSPCFG_P_VIDEO_DECODE_NUMBER;
		platformSettings.videoDecoderModuleSettings.numStillDecodes = BSPCFG_P_VDIDE_STILL_NUMBER_PER_DECODER;
		platformSettings.videoDecoderModuleSettings.numMosaicDecodes = BSPCFG_P_VIDEO_MOSAIC_DECODE_NUMBER;
	}
#endif
	...
	/* Set the Transport configuration */
	{
		...
		{
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].rave = false;
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].message = false;
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].remux[0] = false;
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].remux[1] = false;
		}
		for(uParserBandLoop = NEXUS_ParserBand_e0; uParserBandLoop < CONFIG_NUM_PARSER_BANDS; uParserBandLoop++)
		{
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].rave = true;
			platformSettings.transportModuleSettings.clientEnabled.parserBand[uParserBandLoop].message = true;
		}
		
		for(uParserBandLoop = 0; uParserBandLoop < NEXUS_NUM_PLAYPUMPS; uParserBandLoop++)
		{			
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].rave = false;
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].message = false;
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].remux[0] = false;
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].remux[1] = false;
		}
		for(uParserBandLoop = 0; uParserBandLoop < CONFIG_NUM_PLAYPUMPS; uParserBandLoop++)
		{
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].rave = true;
			platformSettings.transportModuleSettings.clientEnabled.playback[uParserBandLoop].message = true;
		}
	}
#endif
...
HUINT32 LED_CAPABILITY = (DI_LED_NUM8); /* STANDBY_LED */
#endif
