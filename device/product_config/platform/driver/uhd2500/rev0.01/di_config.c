...
static void NEXUS_Platform_P_GetDefaultSettings_by_Custom(NEXUS_PlatformSettings *pSettings)
{
    ...
    pSettings->heap[NEXUS_MEMC0_MAIN_HEAP].size 		= 308*1024*1024; /*decoder FW+general,xpt playback,audio other general purpose */
    ...
    pSettings->heap[NEXUS_MEMC1_PICTURE_BUFFER_HEAP].size 		= 300*1024*1024; /* calculated */
    pSettings->heap[NEXUS_MEMC1_PICTURE_BUFFER_HEAP].memoryType = NEXUS_MEMORY_TYPE_MANAGED | NEXUS_MEMORY_TYPE_NOT_MAPPED;

#if (NEXUS_VERSION >= 1403) || ((NEXUS_VERSION == 1402) && (NEXUS_VERSION_MINOR >= 2))
