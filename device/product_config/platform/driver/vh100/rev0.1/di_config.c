...
#if defined(CONFIG_STLED) && defined(CONFIG_LED_REFACTORING)
#include "drv_stled.h"
#endif
...
{
	{GPIO_ID_HW_MUTE,   NULL,   NEXUS_GpioType_eAonStandard,    13,     GPIO_MODE_WRITE,    GPIO_INT_MODE_DISABLED,     GPIO_HI},
	{GPIO_ID_SPI_WP,	NULL,	NEXUS_GpioType_eAonStandard,	18,		GPIO_MODE_WRITE,	GPIO_INT_MODE_DISABLED,		GPIO_HI},
   ...
   /* STLED - MC2302 */
   {GPIO_ID_SPI_M_SS0,         NULL,   NEXUS_GpioType_eAonStandard,    15, GPIO_MODE_WRITE,    GPIO_INT_MODE_DISABLED,     GPIO_HI},   // STB
   {GPIO_ID_SPI_M_CLK,         NULL,   NEXUS_GpioType_eAonStandard,    16, GPIO_MODE_WRITE,    GPIO_INT_MODE_DISABLED,     GPIO_HI},   // CLK
   {GPIO_ID_SPI_M_MOSI,        NULL,   NEXUS_GpioType_eAonStandard,    17, GPIO_MODE_WRITE,    GPIO_INT_MODE_DISABLED,     GPIO_HI},   // DIN
   {GPIO_ID_SPI_M_MISO,        NULL,   NEXUS_GpioType_eAonStandard,    19, GPIO_MODE_READ,     GPIO_INT_MODE_DISABLED,     GPIO_INVAILD}        // DOUT
#elif defined(CONFIG_STLED_MC2302_NEXUS_SPI) && defined(CONFIG_STLED_MC2302_NEXUS_SPI_USING_GPIO_FOR_STROBE)
	{GPIO_ID_SPI_M_SS0,		NULL,	NEXUS_GpioType_eAonStandard,	15,	GPIO_MODE_WRITE,	GPIO_INT_MODE_DISABLED,	GPIO_HI},	// STB
#endif
#endif
};
...
{
	{GPIO_ID_KEY_STBY,		"STANDBY",		0,	GPIO_INVAILD,	DI_KEY_RELEASED}
};
...
{
		DI_KEY_FRONT_PWR,				/* Micom KeyID 0 */
		DI_KEY_FRONT_RESERVED_01,		/* Micom KeyID 1 */
		DI_KEY_FRONT_VOL_DOWN,			/* Micom KeyID 2 */
		DI_KEY_FRONT_RESERVED_01,		/* Micom KeyID 3 */
		DI_KEY_FRONT_CH_DOWN,			/* Micom KeyID 4 */
		DI_KEY_FRONT_MENU,				/* Micom KeyID 5 */
		DI_KEY_FRONT_RESERVED_01,		/* Micom KeyID 6 */
		DI_KEY_FRONT_CH_UP,				/* Micom KeyID 7 */
		DI_KEY_FRONT_RESERVED_01,		/* Micom KeyID 8 */
		DI_KEY_FRONT_RESERVED_01,		/* Micom KeyID 9 */
		DI_KEY_FRONT_OK,				/* Micom KeyID 10 */
		DI_KEY_FRONT_VOL_UP,			/* Micom KeyID 11 */
...
/* Image_Usage_t */				Image_Usage_RootfsInfo,
								{
/* ulStoredAddress */				N_IMAGE_ROOTFS_OFFSET,
/* ulLoadingAddress */				0,
/* ulSignatureCheckAddress */		0,
/* ulRamdiskRootfsLoadingAddress */	0,
/* ulEntryAddress */				0,
...
/* Image_Usage_t */				Image_Usage_CrytKernelInfo,
								{
/* ulStoredAddress */				(N_IMAGE_KERNEL_OFFSET | RAM_CACHED_BASE),
...
/* Image_Usage_t */				Image_Usage_CrytRootfsInfo,
								{
/* ulStoredAddress */				(N_IMAGE_ROOTFS_OFFSET | RAM_CACHED_BASE),
...
HUINT32 LED_CAPABILITY = (LED_ID_POWER); /* POWER */
#endif
...
static int set_led(struct led_setting_t *pSetting)
{
	...
	switch(pSetting->func)
	{
		case LED_FUNC_NORMAL:
			if ( pSetting->on == TRUE )	// turn on all LEDs
			{
				ucData[0] = 0x07;
				eDrvErr = DRV_STLED_DisplayLED(ucData);
				if ( eDrvErr != DRV_OK )
				{
					PrintError("%s->DRV_STLED_DisplayLED(): Error! Line(%d), %s\n", __FUNCTION__, __LINE__, __FILE__);
					rc = -1;
				}
			}
		...
		case LED_FUNC_WATCHING:	// M1 (Power Green)
			ucData[0] = 0x01;
			eDrvErr = DRV_STLED_DisplayLED(ucData);
			if ( eDrvErr != DRV_OK )
			{
		...
		case LED_FUNC_STANDBY:		// R1 (Power Red)
			ucData[0] = 0x20;
			eDrvErr = DRV_STLED_DisplayLED(ucData);
			if ( eDrvErr != DRV_OK )
			{
...
DRV_Error DRV_CFG_LED_PlatformDrvInit(struct led_platform_driver_t *tLedDrv)
{
	...
	tLedDrv->led_cap = (LED_ID_POWER);	/* POWER */
