...
/*	{GPIO_ID_HW_MUTE,		NULL,   NEXUS_GpioType_eStandard,		33,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},	*/
	...
	{GPIO_ID_MICOM_ISP,		NULL,	NEXUS_GpioType_eStandard,		40,	GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
	{GPIO_ID_MICOM_RESET,	NULL,	NEXUS_GpioType_eAonStandard,	3,	GPIO_MODE_INTERRUPT,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
#endif
	{GPIO_ID_HW_MUTE,	    NULL,	NEXUS_GpioType_eStandard,		59,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD}, /*AUD_MUTE*/
	{GPIO_ID_SC0_PWR_SEL,	NULL,	NEXUS_GpioType_eStandard,		56,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD}, /*C-CAS*/
	{GPIO_ID_SC1_PWR_SEL,	NULL,	NEXUS_GpioType_eStandard,		93,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD}, /*B-CAS*/
/*	{GPIO_ID_SC0_DIV,		NULL,	NEXUS_GpioType_eStandard,		57,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},*/ /*C-CAS*/
/*	{GPIO_ID_SC1_DIV,		NULL,	NEXUS_GpioType_eStandard,		94,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},*/ /*B-CAS*/
	{GPIO_ID_ALIVE,			NULL,	NEXUS_GpioType_eStandard,		42,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_LOW},
	{GPIO_ID_FAN_SPEED, 	NULL,	NEXUS_GpioType_eStandard,		84, 	GPIO_MODE_INTERRUPT,	GPIO_INT_MODE_RISING_EDGE,	GPIO_INVAILD},
	{GPIO_ID_FAN_CONTROL,	NULL,	NEXUS_GpioType_eStandard,		85,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_HI},
	{GPIO_ID_nCH0_RST,		NULL,	NEXUS_GpioType_eStandard,		37,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
	{GPIO_ID_nCH1_RST,		NULL,	NEXUS_GpioType_eStandard,		38,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD},
	{GPIO_ID_nCH2_RST,		NULL,	NEXUS_GpioType_eStandard,		39,		GPIO_MODE_WRITE,		GPIO_INT_MODE_DISABLED,		GPIO_INVAILD}
};
...
const DRV_TS_PATH_ID s_TsIds[] =
{
	...
	{NEXUS_InputBand_e1, 5},
	{NEXUS_InputBand_e2, 6},
};
...
/*		Platform: 97439 B1 */
/*		Boxmode: 2, Display:HD/SD,Video:HD Main/PIP */
...
static void mem_usage( NEXUS_PlatformSettings *pPlatformSettings, NEXUS_MemoryConfigurationSettings *pMemConfigSettings ) {
	...
	pMemConfigSettings->videoDecoder[0].mosaic.maxNumber = 9;
	pMemConfigSettings->videoDecoder[0].mosaic.maxWidth = 512;
	...
	pMemConfigSettings->videoDecoder[1].maxFormat = 30; /* 1080p30 */
	...
	pMemConfigSettings->videoDecoder[1].supportedCodecs[13] = 1; /* vp6 */
	pMemConfigSettings->videoDecoder[1].supportedCodecs[14] = 1; /* vp7 */
	pMemConfigSettings->videoDecoder[1].supportedCodecs[15] = 1; /* vp8 */
	pMemConfigSettings->videoDecoder[1].supportedCodecs[16] = 1; /* vp9 */
	...
	pMemConfigSettings->videoDecoder[2].maxFormat = 30; /* 1080p30 */
	...
	pMemConfigSettings->videoDecoder[2].supportedCodecs[13] = 1; /* vp6 */
	pMemConfigSettings->videoDecoder[2].supportedCodecs[14] = 1; /* vp7 */
	pMemConfigSettings->videoDecoder[2].supportedCodecs[15] = 1; /* vp8 */
	pMemConfigSettings->videoDecoder[2].supportedCodecs[16] = 1; /* vp9 */
	pMemConfigSettings->videoDecoder[2].supportedCodecs[17] = 1; /* spark */
	pMemConfigSettings->videoDecoder[2].supportedCodecs[19] = 1; /* hevc */
	pMemConfigSettings->videoDecoder[2].colorDepth = 8;
	...
	pMemConfigSettings->videoDecoder[3].used      = 0;
	...
	pMemConfigSettings->display[0].window[0].userCaptureBufferCount = 1;
	pMemConfigSettings->display[0].window[1].used = 0;
	pMemConfigSettings->display[1].maxFormat = 1; /* 480i (NTSC) */
	...
	pMemConfigSettings->display[2].window[0].precisionLipSync    = 1;
	...
	pMemConfigSettings->display[3].window[0].used = 0;
	pMemConfigSettings->display[3].window[0].convertAnyFrameRate = 0;
	pMemConfigSettings->display[3].window[0].precisionLipSync    = 0;
	pMemConfigSettings->display[3].window[0].capture             = 0;
	pMemConfigSettings->display[3].window[0].deinterlacer        = 0;
	...
	pPlatformSettings->heap[0].size = 220*1024*1024; /* Main */

	pPlatformSettings->heap[3].size = 140*1024*1024; /* Secure */

	pPlatformSettings->heap[4].size = 160*1024*1024; /* Picture */

	pPlatformSettings->heap[5].size = 210*1024*1024; /* Picture */

	pPlatformSettings->heap[6].size = 280*1024*1024; /* Graphics */

	pPlatformSettings->heap[7].size = 4*1024*1024; /* Driver */

	pPlatformSettings->heap[9].size = 0*1024*1024; /* MANAGED HIGH_MEMORY ONDEMAND_MAPPED */
...
HUINT32 DRV_CFG_PlatformInit(void)
{
	...
	NEXUS_SetEnv("B_REFSW_BOXMODE", "5");
	...
	/* Image Decryption ?? 제??????:
	 : Encrypted Image??Nexus Heap ???? alloc?? ?彭？?? decryption ?????瞞???.
	   (alloc ???? ??은 ?彭？?? decryption ?????? BMRC_MONITOR???? 'violation access' ???? error ?澁?.)
	 =>alloc???? ?歌? ?????歐? 위?漫???'custom_arc'?? enable ??킴. */
	NEXUS_SetEnv("custom_arc","y");

	platformSettings.heap[NEXUS_MEMC0_MAIN_HEAP].size = 424*1024*1024; /* MEMC0_MAIN_HEAP */
	platformSettings.heap[NEXUS_MEMC0_MAIN_HEAP].memcIndex = 0;
	platformSettings.heap[NEXUS_MEMC0_MAIN_HEAP].subIndex = 0;
	platformSettings.heap[NEXUS_VIDEO_SECURE_HEAP].size = 112*1024*1024; /* VIDEO_SECURE_HEAP */
	platformSettings.heap[NEXUS_VIDEO_SECURE_HEAP].memcIndex = 0;
	platformSettings.heap[NEXUS_VIDEO_SECURE_HEAP].subIndex = 1;
	platformSettings.heap[NEXUS_MEMC0_PICTURE_BUFFER_HEAP].size = 28*1024*1024; /* MEMC0_PICTURE_BUFFER_HEAP */
	...
	platformSettings.heap[NEXUS_MEMC1_PICTURE_BUFFER_HEAP].size = 120*1024*1024; /* MEMC1_PICTURE_BUFFER_HEAP */
	...
	platformSettings.heap[NEXUS_MEMC1_GRAPHICS_HEAP].size = 256*1024*1024; /* MEMC1_GRAPHICS_HEAP */
	platformSettings.heap[NEXUS_MEMC1_GRAPHICS_HEAP].memcIndex = 1;
	platformSettings.heap[NEXUS_MEMC1_GRAPHICS_HEAP].subIndex = 0;
	platformSettings.heap[NEXUS_MEMC1_DRIVER_HEAP].size = 4*1024*1024; /* MEMC1_DRIVER_HEAP */
	platformSettings.heap[NEXUS_MEMC1_DRIVER_HEAP].memcIndex = 1;
	platformSettings.heap[NEXUS_MEMC1_DRIVER_HEAP].subIndex = 0;
	platformSettings.heap[NEXUS_MEMC1_HIGH_MEMORY_HEAP].size = 64*1024*1024; /* MEMC1_HIGH_MEMORY_HEAP */
	platformSettings.heap[NEXUS_MEMC1_HIGH_MEMORY_HEAP].memcIndex = 1;
	platformSettings.heap[NEXUS_MEMC1_HIGH_MEMORY_HEAP].subIndex = 0;
	...
	/*platformSettings.heap[NEXUS_VIDEO_SECURE_HEAP].size = 80*1024*1024; 112M * 0.7 */
#endif
