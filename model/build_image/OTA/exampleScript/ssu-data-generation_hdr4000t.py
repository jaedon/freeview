...
unt = update_notification_section(
	...
	compatibility_descriptor_len = len(comp_desc1.pack()),
	compatibility_descriptor_loop = [
		unt_compatibility_descriptor_loop_item (
			...
			operational_descriptor_loop = [
				...
				update_descriptor(
					update_flag = 0x43, # combination of update_flag(2bit), update_method(4bit) and update_priority(2bit)
					private_data_bytes = "",
				),				
			],
		),
...
g1.set(
    ...
    modules = [
         Module(
    	    INPUT="ocdir1/hdr2000t_upgrade_app_1.00.07.hdf",
    	    moduleId = 0x0001,
    	    moduleVersion = 0x00,
    	    descriptors = [name_descriptor(name="hdr1000t_sw1")],
	),
    ],
    )
...
out = open("./datacarousel/unt.sec", "wb")
out.write(unt.pack())
out.close
...
# import pdb
# import dvbobjects.DVB.DataCarousel
# pdb.run(dsi.generate("datacarousel"))
