...
/* mux constants */
#define INVALID_SAMPLE_TIME                  DRM_UI64HL( MAX_UNSIGNED_TYPE( DRM_DWORD ), MAX_UNSIGNED_TYPE( DRM_DWORD ) )
...
{                                                                                    \
    DRM_WORD __temp = word;                                                          \
    FIX_ENDIAN_WORD(__temp);                                                         \
    DRM_BYT_CopyBytes((byteptr),(i),(DRM_BYTE*)&(__temp),0,SIZEOF(DRM_WORD));        \
    (i) += SIZEOF( DRM_WORD );                                                       \
}

#define SaveDWORDOffset( byteptr, dword, i )                                         \
{                                                                                    \
    DRM_DWORD __temp = dword;                                                        \
    FIX_ENDIAN_DWORD(__temp);                                                        \
    DRM_BYT_CopyBytes((byteptr),(i),(DRM_BYTE*)&(__temp),0,SIZEOF(DRM_DWORD));       \
    (i) += SIZEOF( DRM_DWORD );                                                      \
}

#define SaveQWORDOffset( byteptr, qword, i )                                         \
{                                                                                    \
    DRM_UINT64 __temp = qword;                                                       \
    FIX_ENDIAN_QWORD((__temp));                                                      \
    DRM_BYT_CopyBytes((byteptr),(i),(DRM_BYTE*)&(__temp),0,SIZEOF(DRM_UINT64));      \
    (i) += SIZEOF( DRM_UINT64 );                                                     \
}
