...
#define DIRTY_TYPE_ID  (-2) // use to indicate (for now) only a session recently loaded from disc
...
typedef struct {
    ...
    long update_time; // last playback-in-progress time - updated multiple times
...
typedef struct {
    ...
    SECURE_STOP ss;
    int first_update;
    long persist_time; // last secure stop persistence time
    int decrypt_count;
    DRM_UINT64 last_iv;
    DRM_DWORD ldl_seconds;
    DRM_DWORD ldl_expire_time;
} TEE_SESSION;

typedef struct {
    DRM_BYTE session_id[TEE_SESSION_ID_LEN];
    DRM_BOOL initialized;
    DRM_BOOL isLdl;
} TEE_SESSION_ID;

// The number of decrypt calls with new IV (translating into the new frames) before checking the DRM time to see if secure stop must be persisted
#define NUM_DECRYPTS_BEFORE_TIME_CHECK 120
...
int _tee_update(const DRM_BYTE * const inbuf, DRM_DWORD  const inlen);
int    _tee_destroy();

/*
 * session lifecycle
 */
int _tee_add_session_id(const DRM_BYTE * const session_id, const DRM_BOOL isLdl);
int _tee_cancel_session_id(const DRM_BYTE * const session_id);
int _tee_bind_license(const DRM_BYTE * const session_id, const DRM_BYTE * const kid, const DRM_DWORD inLDLSeconds);
int _tee_has_session_by_kid(const DRM_BYTE * const inKID);
int _tee_bind_session(const DRM_BYTE * const session_id, const OEM_HAL_KEY_REGISTER_INDEX secKey);
int _tee_remove_session(const OEM_HAL_KEY_REGISTER_INDEX key_id, const DRM_BOOL ss_enabled);
int _tee_get_ldl_sessions_limit(DRM_DWORD * const ldl_sessions_limit);
int _tee_flush_ldl_challenges();
int _tee_decrypt_update(const int key_id,
                        const DRM_UINT64 inInitializationVector,
...
int _tee_update_secure_stops(const DRM_BYTE * const inbuf, DRM_DWORD const inlen);
int _tee_clear_secure_stops();
